-- Object: PROCEDURE dbo.MimansaV2_COMBINED_CONTRACTNOTE_DETAILWithSplit
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

CREATE PROCEDURE [dbo].[MimansaV2_COMBINED_CONTRACTNOTE_DETAILWithSplit]                       
...
--MimansaV2_COMBINED_CONTRACTNOTE_DETAILWithSplit    'broker','broker','NOV 14 2019','NOV 14 2019','a','ZZZZZZZZ','','zzzzzzzzzzz','','zzzzzzzzzzz','','99999999','CONTRACT','ALL',0,'','Angel Broking Pvt Ltd.(Erstwhile Angel Broking Ltd.)','ONLYEQ'                  
...
--SELECT @PRINTF = 'ALL'                                                
Declare @Whichprocess Varchar(20)          
Create Table  #EXCHANGELIST  
(EXCHANGE Varchar(30),
 SHAREDB Varchar(30),
 SHARESERVER Varchar(30))

Create Table #Partylist (EParty_Code Varchar(20), ESauda_Date Varchar(11))
             
If @CONTRACTGROUP = 'ONLYEQ'
Begin
     Select @Whichprocess = 'ONLYEQ'       
	 Select @ContractGroup = 'CASHFNO'
	 truncate table #Partylist
	 Insert into #Partylist Select * from Mimansapartylist  --mimansa.Crm.Dbo.OnlyEquityClientsTradedtoday
	 
	 /* 
	 Delete Mimansapartylist where sauda_date >= @Fromdate and Sauda_Date <=@Todate 
	 Insert into Mimansapartylist Select * from mimansa.Crm.Dbo.OnlyEquityClientsTradedtoday

	 --Select * from MimansapartylistNov192019  --mimansa.Crm.Dbo.OnlyEquityClientsTradedtoday
	 Select * into Mimansapartylist from mimansa.Crm.Dbo.OnlyEquityClientsTradedtoday
	 Select * into MimansaOpenPos from mimansa.clientservice.dbo.foopenpos
	 drop table MimansaOpenPosNov192019
	 drop table MimansapartylistNov192019
	 */

	 --Select * into MimansapartylistNov152019 from mimansa.Crm.Dbo.OnlyEquityClientsTradedtoday
End

If @CONTRACTGROUP = 'EQANDFO'
Begin
     Print 'Reached here'
     Select @Whichprocess = 'EQANDFO'       
	 Select @ContractGroup = 'CASHFNO'
	 
	 truncate table #Partylist
	 Insert into #Partylist select distinct party_code,Left(Convert(Varchar,Sauda_Date,109),11) 
	 from Msajag.dbo.COMMON_CONTRACT_DATA where sauda_Date >= @FromDate  and sauda_Date <= @Fromdate + ' 23:59'
	 and Exchange = 'NSE' and Segment = 'FUTURES'

	 /* Delete #Partylist from MimansapartylistNov192019 where 
	 #PartyList.EParty_code = MimansapartylistNov192019.Party_code */
...
End		
 ...
 BEGIN                      
  ...
  FROM Msajag.dbo.V2_CONTRACTPRINT_SETTING_COMBINED                      
...
If @WhichProcess = 'ONLYEQ'
Begin
Insert into #EXCHANGELIST 
SELECT DISTINCT   
...
End
  ...
  /* 
SELECT * INTO #COMMON_DATA FROM Msajag.dbo.COMMON_CONTRACT_DATA C (NOLOCK),  #Partylist P
WHERE C.SAUDA_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59:59'   
...
AND 
EXISTS (SELECT EXCHANGE FROM #EXCHANGELIST E WHERE E.EXCHANGE = LTRIM(RTRIM(C.EXCHANGE))+LTRIM(RTRIM(C.SEGMENT)) )  
And P.Eparty_code = C.Party_code
*/

SELECT * INTO #COMMON_DATA From 
( Select SRNO,CONTRACTNO,CONTRACTNO_NEW,SAUDA_DATE,SETT_NO,SETT_TYPE,SETT_DATE,
C.EXCHANGE,C.SEGMENT,ORDER_NO,ORDER_TIME,TRADE_NO,TRADE_TIME,SCRIPNAME,QTY,TMARK,SELL_BUY,MARKETRATE,MARKETAMT,BROKERAGE,
SERVICE_TAX,INS_CHRG,NETAMOUNT,SEBI_TAX,TURN_TAX,BROKER_CHRG,OTHER_CHRG,NETAMOUNTALL,BROK,NETRATE,CL_RATE,BROKERSEBIREGNO,
MEMBERCODE,CINNO,SETTTYPE_DESC,BFCF_FLAG,CONTRACT_HEADER_DET,REMARK,COMPANYNAME,[USER_ID],REMARK_ID,REMARK_DESC,
NETOBLIGATION,BRANCH_CD,SUB_BROKER,TRADER,AREA,REGION,FAMILY,PARTY_CODE,PARTYNAME,
L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_STATE,L_CITY,L_ZIP,OFF_PHONE1,OFF_PHONE2,PAN_GIR_NO,MAPIDID,UCC_CODE,SEBI_NO,
PARTICIPANT_CODE,CL_TYPE,SERVICE_CHRG,PRINTF,PRICEUNIT,QTY_UNIT,ISIN
FROM Msajag.dbo.COMMON_CONTRACT_DATA C (NOLOCK),  #Partylist P, #EXCHANGELIST E
WHERE C.SAUDA_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59:59'   
AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY   
AND SUB_BROKER BETWEEN @FROMSUB_BROKER AND @TOSUB_BROKER  
AND CONTRACTNO BETWEEN @FROMCONTRACT AND @TOCONTRACT  
AND P.Eparty_code = C.Party_code
And E.EXCHANGE=LTRIM(RTRIM(C.EXCHANGE))+LTRIM(RTRIM(C.SEGMENT)) 
)A
 ...
 BEGIN                      
 INSERT INTO #PARTY                      
 SELECT DISTINCT PARTY_CODE FROM #COMMON_DATA C WITH (NOLOCK)   , #Partylist P                   
 WHERE C.SAUDA_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59:59' AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY                      
 ...
 BEGIN                      
  ...
  SELECT DISTINCT PARTY_CODE FROM Msajag.dbo.TBL_ECNBOUNCED WITH (NOLOCK) WHERE SDate BETWEEN @FROMDATE AND @TODATE + ' 23:59:59'                 
 ...
 END   
   ...
   print @PRINTFLAG       
                      select * from MSAJAG.DBO.FUN_PRINTF(@PRINTFLAG)*/
 ...
 SELECT                      
  ...
  NETRATE=(CASE WHEN LEFT(SCRIPNAME,3)='OPT' AND EXCHANGE='NSE' THEN (CASE WHEN SELL_BUY =1 THEN NETRATE+BROK ELSE NETRATE-BROK END)    
            ELSE NETRATE END),                      
...
TGST_NO = L.GST_NO  
INTO #GSTDATA  
FROM msajag.dbo.TBL_CLIENT_GST_DATA G, msajag.dbo.TBL_GST_LOCATION L, #CONTRACT P  
...
DEAL_CLIENT_ADD = L.ADDRESS1 +' '+ L.ADDRESS2,-- +' '+ L.CITY +' '+ L.ZIP,  
TGST_NO = L.GST_NO  
FROM msajag.dbo.CLIENT_DETAILS G, msajag.dbo.TBL_GST_LOCATION L, #CONTRACT P  
WHERE GST_LOCATION = LOC_CODE  
---AND CONVERT(DATETIME,SAUDA_DATE,103) BETWEEN G.EFF_FROM_DATE AND G.EFF_TO_DATE   
AND G.PARTY_CODE = P.PARTY_CODE     
...
DEAL_CLIENT_ADD = '',  
P.TGST_NO  
FROM #CONTRACT P, Msajag.dbo.OWNER   
...
WHERE TARGETSTATE NOT IN (SELECT STATE FROM msajag.dbo.STATE_MASTER)  
...
FROM msajag.dbo.TBL_STATE_GST_DATA D  
...
FROM msajag.dbo.TBL_CLIENT_GST_DATA G (NOLOCK)  
...
SET  TAXABLE_VALUE = BROKERAGE_TAXEX + (  
  ...
  CASE  
   ...
   ELSE 0  
  END)  
FROM msajag.dbo.GLOBALS G (NOLOCK)  
...
Select count(*) from #Partylist
Select count(*) from #PArty
Select count(*) from #Common_data
Select count(*) from #Contract 
Select top 1 * from #COMMON_DATA
...
If @WhichProcess = 'ONLYEQ'
Begin
Delete Contractsplit where sauda_date = CONVERT(VARCHAR,CONVERT(DATETIME,@Fromdate,109),103) and exchange in('BSE','NSE') and Segment = 'Capital'
Insert into Contractsplit Select * from #Contract 
End

If @WhichProcess <> 'ONLYEQ'
Begin
Delete Contractsplit where sauda_date = CONVERT(VARCHAR,CONVERT(DATETIME,@Fromdate,109),103) and exchange in('BSE','NSE') and Segment = 'FUTURES'
Insert into Contractsplit Select * from #Contract 
End
