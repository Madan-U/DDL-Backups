-- Object: PROCEDURE dbo.NRMS_LiveLedger
...
BEGIN            
 ...
 Truncate table NRMS_NSECM_LEDGER            
         
 insert into NRMS_NSECM_LEDGER(ProcessDateTime,Cltcode,VBal,UCV,UnRecoCr,OtherDr)            
 ...
 update NRMS_NSECM_LEDGER set UNRecoCr=-b.UnRecoCr from             
 (select cltcode,SUM(Cramt) as UnRecoCr from BO_client_deposit_recno with (nolock) group by cltcode) b            
 where NRMS_NSECM_LEDGER.cltcode=b.cltcode            
         
 update NRMS_NSECM_LEDGER set OTherDr=b.Vbal from            
 (select substring(Cltcode,3,10) as Cltcode,VBAl from NRMS_NSECM_LEDGER  with (nolock) where cltcode like '98%' and VBAL < 0) b            
 where NRMS_NSECM_LEDGER.cltcode=b.cltcode            
 ...
 update NRMS_NSECM_LEDGER set vbal=b2bamt,UCV=0,UnRecoCr=0,OtherDr=0 from #b2B b where NRMS_NSECM_LEDGER.cltcode=b.cltcode      
 /* B2B End */      
        
 --truncate table NRMS_NSECM_LEDGER_NSEIPO          
 --insert into NRMS_NSECM_LEDGER_NSEIPO (ProcessDateTime,Cltcode,VBal,UCV,UnRecoCr,OtherDr)          
 --select ProcessDateTime,substring(Cltcode,2,10) as cltcode,VBal,UCV,UnRecoCr,OtherDr           
 --from NRMS_NSECM_LEDGER  with (nolock)           
...
BEGIN            
 ...
 delete from NRMS_NSECM_LEDGER where Cltcode=@pcode            
 ...
 update NRMS_NSECM_LEDGER set UNRecoCr=-b.UnRecoCr from             
 (select cltcode,SUM(Cramt) as UnRecoCr from BO_client_deposit_recno with (nolock) group by cltcode) b            
 where NRMS_NSECM_LEDGER.cltcode=b.cltcode and NRMS_NSECM_LEDGER.Cltcode=@pcode            
 ...
 BEGIN    
  update NRMS_NSECM_LEDGER set vbal=b2bamt,UCV=0,UnRecoCr=0,OtherDr=0 from     
  (    
   select     
   CLTCODE, B2Bamt=sum(case when drcr='D' then -VAMT else VAMT end)    
   ...
   group by cltcode    
  ) b where NRMS_NSECM_LEDGER.cltcode=b.cltcode    
 END     
 /* B2B End */    
    
        
-- delete from NRMS_NSECM_LEDGER_NSEIPO where Cltcode=@pcode            
-- insert into NRMS_NSECM_LEDGER_NSEIPO (ProcessDateTime,Cltcode,VBal,UCV,UnRecoCr,OtherDr)          
-- select ProcessDateTime,substring(Cltcode,2,10) as cltcode,VBal,UCV,UnRecoCr,OtherDr           
-- from NRMS_NSECM_LEDGER   with (nolock)         
