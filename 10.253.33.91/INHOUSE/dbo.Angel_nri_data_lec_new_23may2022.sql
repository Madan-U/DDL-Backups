-- Object: PROCEDURE dbo.Angel_nri_data_lec_new_23may2022
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

--use 196-- inhouse                                
--exec Angel_nri_data_lec_new '10/05/2022',2,0,'NSE'                                  
create PROC Angel_nri_data_lec_new_23may2022(@SDATE  AS VARCHAR(11),            
                                   @TYPE   AS INT,            
                                   @BANKID AS INT,            
                                   @SEG    AS VARCHAR(5))            
AS            
  BEGIN            
    ...
    --  DECLARE @SDATE AS VARCHAR(11),@TYPE AS VARCHAR(2),@BANKID AS INT,@SEG AS VARCHAR(5)                                                    
     --SET @SDATE ='04/05/2022' SET @TYPE ='1' SET @BANKID =0 SET @SEG='NSE'                                                    
	   ...
	   End		
	        ...
	        BEGIN
			 ...
			 from Intranet.risk.dbo.tbl_NRIClientMaster with (nolock)  where fld_clientcode = @PARTYCODE
			  ...
			  left(c.Scripname,75)+','+CASE WHEN @TYPE = 1 THEN 'Buy'  WHEN @TYPE = 2 THEN 'Sell'  END +','+Ltrim(Rtrim(CONVERT(VARCHAR,c.QTY))) +','+CASE WHEN @TYPE = 1 THEN Convert(Varchar,c.RATE)  WHEN @TYPE = 2 THEN Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(15,2), c.TRATE))))  END +','+cast(Ltrim(Rtrim(CONVERT(DEC(10,2),c.TRANACTIONAMT))) as varchar)+','+
			  CASE WHEN @TYPE = 1 THEN Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(15,2), c.TRATE))))  WHEN @TYPE = 2 THEN  Convert(Varchar,c.RATE) END +','+Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(10,2), c.BROK_PERSCRIP))))+','+
			  CASE WHEN @DIFFAMT BETWEEN -0.03 AND 0.03       
			   THEN CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),( Sum(CONVERT(DECIMAL(14,2),Isnull(MINBROKERAGE,0)) + CONVERT(DECIMAL(14,2),Isnull(OTHER_CHARGES,0))) + @DIFFAMT )))        
			   ELSE CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),( Sum(Isnull(MINBROKERAGE,0) + Isnull(OTHER_CHARGES,0)) )))        
		     END +','+  --Other Charges end+','+
			  ...
			  c.QTY, Convert(Varchar,c.RATE),c.TRATE,c.RATE
