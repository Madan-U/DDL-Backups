-- Object: PROCEDURE dbo.mimansa_usp_ucc_nse_cm_Deactive_client_data_bkp_16Dec2020
...
CREATE proc
	...
	[dbo].[mimansa_usp_ucc_nse_cm_Deactive_client_data_bkp_16Dec2020]
	(
...
begin
	print 'DATARANGE'
		...
		select distinct cl_code = ltrim(rtrim(isnull(c5.cl_code, '')))
		from anand1.msajag.dbo.client5 c5 with(nolock)join anand1.msajag.dbo.client_brok_details c with(nolock)
...
begin
	print 'SELECTED'
		insert into #TempClient
		select distinct cl_code = ltrim(rtrim(isnull(c5.cl_code, '')))
		from anand1.msajag.dbo.client5 c5 with(nolock),tblUCCClientCode A with(nolock), anand1.msajag.dbo.client_brok_details c with(nolock)		
	...
	from 
		...
		[172.31.16.94].DMAT.citrus_usr.VW_EXP_NEWCLIENT_POA a1 
...
from
	...
	[196.1.115.132].mimansa.dbo.UCC_AllSebiBannedEntities bp with (nolock)
where
...
select distinct Segment='C', Client_Code, Inactive='I', ClosureStatus='Account Closure'
from #mimansa_angel_ucc_nse_cm_client_data_new_changes with(nolock)
where len(ltrim(rtrim(isnull(pan, '')))) > 0 and isnull(pan, '' ) <> 'NA'
...
/*Insert Log*/
if exists(select * from [196.1.115.200].INHOUSE.dbo.mimansa_tbl_ucc_nse_Deactive_file_count with(nolock) where time_stamp>=cast(left(getdate(),11) as datetime))
begin
	update [196.1.115.200].INHOUSE.dbo.mimansa_tbl_ucc_nse_Deactive_file_count 
	set file_number=file_number+1
	where time_stamp>=cast(left(getdate(),11) as datetime)
end
else
begin
	insert into [196.1.115.200].INHOUSE.dbo.mimansa_tbl_ucc_nse_Deactive_file_count(file_number,time_stamp)
	select 1,getdate()
end
...
select FileName='CLT_STATUS_'+replace(convert(varchar, getdate(), 102), '.', '')+(case when isnull(@processid,'')<>'' then '_selected_clients' else '' end)
				+'.T'+(case when file_number<10 then  '0'+cast(file_number  as varchar) else cast(file_number  as varchar) end)
				,					
	   FirstRow=''
	   --'10|M|12798|'+replace(convert(varchar, getdate(), 103), '/', '')+'|'
...
from [196.1.115.200].INHOUSE.dbo.mimansa_tbl_ucc_nse_Deactive_file_count with(nolock) where time_stamp>=cast(left(getdate(),11) as datetime)
