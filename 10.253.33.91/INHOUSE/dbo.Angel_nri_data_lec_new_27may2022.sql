-- Object: PROCEDURE dbo.Angel_nri_data_lec_new_27may2022
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

--use 196-- inhouse                                
--exec Angel_nri_data_lec_new '10/05/2022',2,0,'NSE'                                  
create PROC Angel_nri_data_lec_new_27may2022(@SDATE  AS VARCHAR(11),            
                                   @TYPE   AS INT,            
                                   @BANKID AS INT,            
                                   @SEG    AS VARCHAR(5))            
AS            
	   ...
	   End		
	        ...
	        BEGIN
			 ...
			 from Intranet.risk.dbo.tbl_NRIClientMaster with (nolock)  where fld_clientcode = @PARTYCODE
			  ...
			  left(c.Scripname,75)+','+CASE WHEN @TYPE = 1 THEN 'Buy'  WHEN @TYPE = 2 THEN 'Sell'  END +','+Ltrim(Rtrim(CONVERT(VARCHAR,c.QTY))) +','+CASE WHEN @TYPE = 2 THEN Convert(Varchar,c.RATE)+','+cast((CONVERT(DEC(15,2),c.TRATE) *c.QTY) as varchar)  WHEN @TYPE = 1 THEN Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(15,2), c.TRATE)))) +','+cast((CONVERT(DEC(15,2),c.RATE) *c.QTY) as varchar) END +','+
			  --CASE WHEN @TYPE = 2 THEN Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(15,2), c.TRATE))))  WHEN @TYPE = 1 THEN  Convert(Varchar,c.RATE) END +','+Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(10,2), c.BROK_PERSCRIP))))+','+
			  --CASE WHEN @DIFFAMT BETWEEN -0.03 AND 0.03       
			  -- THEN CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),( Sum(CONVERT(DECIMAL(14,2),Isnull(MINBROKERAGE,0)) + CONVERT(DECIMAL(14,2),Isnull(OTHER_CHARGES,0))) + @DIFFAMT )))        
			  -- ELSE CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),( Sum(Isnull(MINBROKERAGE,0) + Isnull(OTHER_CHARGES,0)) )))        
		   --  END +','+  --Other Charges end+','+
			  ...
			  Convert(Varchar,CONVERT(DECIMAL(14,2),a.TOTALAMT))+','+'@@@[RBI Approval Number]'+','+@BseCliPISNo+','+@BseCliBnkAcc+','+@BsecliBankBranch+','+'Angel One Ltd'+','+CASE WHEN @TYPE = 1 THEN  Convert(varchar,(a.sauda_date +1),103)  WHEN @TYPE = 2 THEN Convert(varchar,(a.sauda_date+2),103)  END +','+
	  ...
	  Begin
       ...
       End 	   
		   ...
		   --  END +','+  --Other Charges end+','+
			  ...
			  CONVERT(VARCHAR,CONVERT(DECIMAL(14,2), a.TOTALAMT))+','+'@@@[RBI Approval Number]'+','+@CliPISNo+','+@CliBnkAcc+','+@cliBankBranch+','+'Angel One Ltd'+','+ CASE WHEN @TYPE = 1 THEN  Convert(varchar,(a.sauda_date +1),103)  WHEN @TYPE = 2 THEN Convert(varchar,(a.sauda_date+2),103)  END+','+
