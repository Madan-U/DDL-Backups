-- Object: PROCEDURE dbo.mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_Test09April2015
...
CREATE proc
	...
	[dbo].[mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_Test09April2015]
	(
...
as
--exec [dbo].[mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_Test09April2015] 'Aug 03 2015','Aug 04 2015','',''
	set nocount on
...
CREATE TABLE #mimansa_angel_ucc_nse_cm_client_data_new_changes(
	...
	where
		...
		select party_code,
		lenght=len(isnull(rtrim(ltrim(c1.AADHAR_UID)),'')),
		UIDNUMBER=replace(isnull(rtrim(ltrim(c1.AADHAR_UID)),''),' ','')
	from
		anand1.msajag.dbo.CLIENT_MASTER_UCC_DATA c1 with(nolock),#mimansa_angel_ucc_nse_cm_client_data_new_changes
	where
		client_code = party_code
		and len(isnull(client_code, '')) > 0
		and len(isnull(party_code, '')) > 0	
		and len(replace(isnull(rtrim(ltrim(c1.AADHAR_UID)),''),' ',''))<>12
