-- Object: PROCEDURE dbo.mimansa_usp_ucc_nse_cm_Deactive_client_data
...
CREATE proc  
 ...
 [dbo].[mimansa_usp_ucc_nse_cm_Deactive_client_data]  
 (  
...
begin  
 print 'DATARANGE'  
  ...
  from AngelNseCM.msajag.dbo.client5 c5 with(nolock)join AngelNseCM.msajag.dbo.client_brok_details c with(nolock)  
...
begin  
 print 'SELECTED'  
  insert into #TempClient  
  select distinct cl_code = ltrim(rtrim(isnull(c5.cl_code, '')))  
  from AngelNseCM.msajag.dbo.client5 c5 with(nolock),tblUCCClientCode A with(nolock), AngelNseCM.msajag.dbo.client_brok_details c with(nolock)    
