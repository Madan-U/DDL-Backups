-- Object: PROCEDURE dbo.PL_NT
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------


CREATE PROC [dbo].[PL_NT]                                                                    
(                                                                      
 @STATUSID VARCHAR(25),                                                                      
...
--WHERE C1.CL_CODE=C2.CL_CODE                              -- Commented  By Nitesh                
...
BEGIN                              
 ...
 SCRIP_CD, T.SERIES, T.BSECODE, ISIN,                                                                     
 ...
 OPPDATE,                                                                    
 FLAG=RECTYPE,                                                     
 SCRIP_NAME = LEFT(V.Scrip_NAME, 50)   ,                              
 BRANCH_CD = '',                                                                   
...
REGION  = ''       
 ...
 FROM  MSAJAG.dbo.TBL_PNL_DATA T WITH (NOLOCK) Left Outer join Vw_ScripnameBse V on( T.BSECODE = V.BSECODE )                                                                     
 AND SCRIP_NAME = ''                                                           
 ...
 AND BILLFLAG < 4  and Exchange = 'BSE'                                 
 GROUP BY SAUDA_DATE, SETT_NO, SETT_TYPE, T.PARTY_CODE,EXCHANGE,                                                                     
 SCRIP_CD, T.SERIES, T.BSECODE, ISIN, PNLRATE, SELL_BUY, OPPDATE,                                     
 RECTYPE  ,V.Scrip_NAME                                                                  
...
BEGIN                              
 INSERT INTO #TBL_PNL_DATA                                                               
 SELECT SAUDA_DATE, SETT_NO, SETT_TYPE, T.PARTY_CODE,                                                                     
 T.SCRIP_CD, T.SERIES, T.BSECODE, ISIN,                                                                     
 SELLQTY = SUM(CASE WHEN SELL_BUY = 2 THEN TRADEQTY ELSE 0 END),                                                                     
 SELLRATE = (CASE WHEN SELL_BUY = 2 THEN PNLRATE ELSE 0 END),                                                                    
 SELLVAL = SUM(CASE WHEN SELL_BUY = 2 THEN PNLRATE*TRADEQTY ELSE 0 END),                                                                    
 BUYQTY = SUM(CASE WHEN SELL_BUY = 1 THEN TRADEQTY ELSE 0 END),                       
...
AREA  = '',                                    
REGION  = ''                                    
 FROM MSAJAG.dbo.TBL_PNL_DATA T WITH (NOLOCK) Left Outer join Vw_ScripnameNse V on(T.Scrip_cd = V.Scrip_cd and T.Series = V.Series)                                                             
 AND SCRIP_NAME = ''                                                           
 WHERE T.PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY                                                                      
 AND (SAUDA_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59'                                                        
   OR OPPDATE BETWEEN @FROMDATE AND @TODATE + ' 23:59')                                                          
 AND SAUDA_DATE <= @TODATE + ' 23:59'                                                               
 AND OPPDATE <= @TODATE + ' 23:59'                                                        
 AND BILLFLAG < 4  and Exchange = 'NSE'                                 
 GROUP BY SAUDA_DATE, SETT_NO, SETT_TYPE, T.PARTY_CODE,EXCHANGE,                                                                     
 T.SCRIP_CD, T.SERIES, T.BSECODE, ISIN, PNLRATE, SELL_BUY, OPPDATE,                                     
 RECTYPE    ,V.Scrip_NAME                                                                
...
UPDATE #TBL_PNL_DATA SET SCRIP_NAME = LEFT(S1.LONG_NAME, 50)                                                                      
 ...
 --#CLIENT_DETAILS C                                    
 --WHERE                                    
 --#TBL_PNL_DATA.PARTY_CODE = C.CL_CODE                   -- Commented By Nitesh                  
...
ISIN,                                                              
 ...
 #TBL_PNL_DATA T                                    
 WHERE                                    
 PARTY_CODE IN (SELECT DISTINCT PARTY_CODE FROM #CLIENT_DETAILS )           and     isnull(SCRIP_NAME,'')<>''                                         
    /*                                    
