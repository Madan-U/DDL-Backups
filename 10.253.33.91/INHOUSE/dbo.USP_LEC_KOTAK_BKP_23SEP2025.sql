-- Object: PROCEDURE dbo.USP_LEC_KOTAK_BKP_23SEP2025
...
CREATE PROC [dbo].[USP_LEC_KOTAK_BKP_23SEP2025] ( @SDATE AS VARCHAR(11) )          
AS          
BEGIN        
    
---- EXEC USP_LEC_KOTAK 'JUN 10 2025'    
--DECLARE @SDATE AS VARCHAR(11)='JUL 30 2025'          
...
TRUNCATE TABLE INHOUSE.DBO.TBL_NRILECKOTAK_TEMP    
    
INSERT INTO INHOUSE.DBO.TBL_NRILECKOTAK_TEMP        
...
IF ((SELECT COUNT(1) FROM INHOUSE.DBO.TBL_NRILECKOTAK_TEMP)>0)                        
	BEGIN        
		DECLARE @FILENAME1 VARCHAR(100) = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLients\KOTAK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.CSV'                     
		...
		SET @ALL1 = @ALL1+ ' UNION ALL SELECT * FROM INHOUSE.DBO.TBL_NRILECKOTAK_TEMP'                            
		--PRINT @ALL1                              
		SET @ALL1=@ALL1+' " QUERYOUT ' +@FILENAME1+ ' -c -t"," -c -t"," -r"\n" -T'', NO_OUTPUT'                              
		PRINT @ALL1                              
		EXEC(@ALL1)        
		SELECT '<a href='+replace(@FILENAME1,'\\INHOUSELIVEAPP1-FS.angelone.in','')+'>Right click and save -- KOTAK Bank</a><br>' 
	END
