-- Object: PROCEDURE dbo.BBGSettBrokDisplayVer1_ADTest
...
as
-- Exec BBGSettBrokDisplayVer1 '1','D','A031','532296','','1',4,1070.40,4,'JAN  1 2000','broker',''
...
Broker_chrg =(Case when #ViewSettlement.status = 'N' then 0 else ((CT.broker_note * #ViewSettlement.marketrate * #ViewSettlement.Tradeqty)/100) end)
      ...
      And Broktable.Line_no = (Select min(Broktable.line_no) from broktable with(nolock), Msajag.Dbo.ClientBrok_Scheme S with(nolock) where
		        --       S.Table_No = Broktable.Table_no
			       --And 
				   Trd_Del = 'D' And #ViewSettlement.Party_Code =  Client2.Party_code
			       And #ViewSettlement.marketrate <= Broktable.upper_lim 
				   and S.party_code = #ViewSettlement.Party_Code
				   )
...
Select  Broktable.Table_no   FROM Msajag.dbo.BrokTable BrokTable,Msajag.Dbo.Client2,globals, #ViewSettlement,Msajag.Dbo.Client1, Msajag.Dbo.ClientBrok_Scheme S, Msajag.Dbo.ClientTaxes_New CT, Msajag.Dbo.Owner
      WHERE #ViewSettlement.Party_Code = Client2.Party_code
      And Client2.Cl_code = Client1.Cl_code
      And Client2.Party_Code = CT.Party_Code
      And CT.trans_Cat = 'DEL'
...
Select NBrokApp from #ViewSettlement
...
Declare @ClientState varchar(50)
Declare @StateStampDuty decimal(4,4)

Select @ClientState = isnull(ltrim(rtrim([state])),'') from Msajag.dbo.Client_Details CD with(nolock), Msajag.dbo.STATE_MASTER SM with(nolock)
where CD.Party_code = @Party_Code
and CD.l_State=SM.[State]
...
BEGIN
	...
	Levies_TurnoverTax=turnover_tax, Levies_SebiTax=sebiturn_tax, Levies_StampDuty=broker_note, Levies_Brokerage=@Levies_Brokerage, 
...
ELSE
BEGIN
	select @StateStampDuty = (case when @Trans_Cat = 'TRD' then cast(isnull(TRDSTAMPDUTY,0) as decimal(4,4)) else  cast (isnull(DELSTAMPDUTY,0) as decimal(4,4)) end) 
	from Msajag.dbo.STATE_MASTER with(nolock) where ltrim(rtrim([state])) = @ClientState

	--Select @StateStampDuty

	Select *,LeviesSTT=@STT_From_Global,LeviesServ_Tax=@Service_tax_From_Global,
	Levies_TurnoverTax=turnover_tax, Levies_SebiTax=sebiturn_tax, Levies_StampDuty= @StateStampDuty, Levies_Brokerage=@Levies_Brokerage, 
	Brokerage_Type=isnull(@Brokerage_Type,'P') 
	from #ViewSettlement , Msajag.dbo.ClientTaxes_New CT with(nolock) 
	where #ViewSettlement.Party_Code= CT.Party_Code
	and CT.Todate>getdate()
