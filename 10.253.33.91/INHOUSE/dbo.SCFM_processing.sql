-- Object: PROCEDURE dbo.SCFM_processing
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------


--exec SCFM_processing 'mar 31 2015 23:59:59:000','apr  1 2015 23:59:59'      
CREATE procedure [dbo].[SCFM_processing]      
(      
@startdate datetime,      
@enddate datetime      
)      
 ...
 */      
 ...
 select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)      
 into #bsecm      
 from AngelBSECM.account_ab.dbo.ledger where vdt >@startdate      
 and edt <=@enddate      
 --and cltcode between 'a' and 'zzzz9999'      
 group by cltcode      
 select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)      
...
WHERE  CLTCODE LIKE '98%' AND CLTCODE=CL_cODE 
 ...
 delete from SCFM_FY1314 where effectivedate=@enddate      
       
 insert into SCFM_FY1314      
