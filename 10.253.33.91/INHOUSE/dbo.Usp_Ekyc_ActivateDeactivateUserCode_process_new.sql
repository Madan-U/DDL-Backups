-- Object: PROCEDURE dbo.Usp_Ekyc_ActivateDeactivateUserCode_process_new
...
-- =============================================
CREATE PROCEDURE [dbo].[Usp_Ekyc_ActivateDeactivateUserCode_process_new]
...
select top 1 @ProcessID=ProcessID ,@Empid=Emp_code from mimansa.CRM.dbo.Ekyc_ActivateDeactivateUserCode_process with(nolock) where
ProcessingStatus='P' order by  RequestReceivedOn desc --ProcessID desc

Print @ProcessID

if exists (select * from mimansa.CRM.dbo.tbl_EKyc_BulkDeactivateProcess_Clients with(nolock) where ProcessID=@ProcessID)
Begin

	Select distinct party_Code,Exchange,Segment,Remark as 'NewRemark', Inactive_Date,Deactive_Value,entryBy as 'updatedby'
	Into #tempDeactiveInfo
	from mimansa.CRM.dbo.tbl_EKyc_BulkDeactivateProcess_Clients with(nolock)
	...
	Declare @Deactive_Remarks varchar(500),@InActive_Date varchar(500),@Exchange  varchar(50),@InactiveDate datetime,
	@Deactive_Value varchar(50)



	DECLARE cur_Deactive CURSOR  FOR           
	select party_Code,Exchange,segment,NewRemark,Inactive_Date,Deactive_Value,updatedby
	from #tempDeactiveInfo 

	OPEN cur_Deactive
		FETCH NEXT FROM cur_Deactive INTO @party_Code,@Exchange,@segment,@NewRemark,@InactiveDate,@Deactive_Value,@updatedby
	      
		 WHILE @@FETCH_STATUS = 0          
		 BEGIN 


		 if(@segment in('EQUITY','CAPITAL','FUTURES')  and @Exchange in('BSE','NSE','NSX'))
		 Begin
			...
			Begin
				...
				BEGIN					
					...
					Set @InActive_Date= (Select top 1 InActive_From from msajag.dbo.client_brok_details where cl_code=@party_Code and Exchange in('BSE','NSE') order by InActive_From desc )
	...
	InActive_From,Print_Options,No_Of_Copies,Participant_Code,Custodian_Code,Inst_Contract,Round_Style,STP_Provider,STP_Rp_Style,Market_Type,Multiplier,Charged,Maintenance,Reqd_By_Exch,Reqd_By_Broker,Client_Rating,Debit_Balance,Inter_Sett, TRD_STT,Trd_Tran_Chrgs,Trd_Sebi_Fees,Trd_Stamp_Duty,Trd_Other_Chrgs,Trd_Eff_Dt,Del_Stt,               Del_Tran_Chrgs,Del_SEBI_Fees,Del_Stamp_Duty,Del_Other_Chrgs,Del_Eff_Dt,              Rounding_Method,Round_To_Digit,Round_To_Paise,Fut_Brok,Fut_Opt_Brok,Fut_Fut_Fin_Brok, Fut_Opt_Exc,Fut_Brok_Applicable,Fut_Stt,Fut_Tran_Chrgs,Fut_Sebi_Fees,Fut_Stamp_Duty, Fut_Other_Chrgs,Status,Modifiedon,Modifiedby,Imp_Status,Pay_B3B_Payment,Pay_Bank_name,                                      Pay_Branch_name,Pay_AC_No,Pay_payment_Mode,Brok_Eff_Date,Inst_Trd_Brok,Inst_Del_Brok,'E36024',GETDATE(),SYSTEMDATE,Active_Date,CheckActiveClient,Deactive_Remarks,Deactive_value
					...
					Deactive_value=@Deactive_Value,Imp_Status=0,Modifiedon=getdate(),Modifiedby=@updatedby where 
		...
		if(@segment in('COMMODITY','FUTURES')  and @Exchange in('MCX','NCX'))
		 Begin
...
From msajag.dbo.client_brok_details with(nolock)
where cl_code=@party_Code and Exchange in('MCX','NCX')   -- ,'MCD'
					and Exchange=@Exchange


					Update msajag.dbo.client_brok_details set Deactive_Remarks=@NewRemark, InActive_From=GETDATE(),
					Deactive_value=@Deactive_Value,Imp_Status=0,Modifiedon=getdate(),Modifiedby=@updatedby where 
					cl_code=@party_Code and Exchange in('MCX','NCX') and Exchange=@Exchange
					...
					BEGIN
					Update msajag.dbo.client_brok_details set InActive_From=GETDATE()
					where cl_code=@party_Code and Exchange in('MCX','NCX') and Exchange=@Exchange
					end 
...
'
   ...
   @recipients = 'arjun.singh@angelbroking.com;dinesh.jadhav@angelbroking.com;gupta.ajaykumar@angelbroking.com;',
...
BEGIN CATCH
   ...
   update mimansa.CRM.dbo.Ekyc_ActivateDeactivateUserCode_process set ProcessingStatus='F',mailType='Fail',isMailSend='Y',EmailSentOn=getdate() where  ProcessID=@ProcessID
	...
	Print @ErrorFlag
	  ...
	  exec msdb.dbo.sp_send_dbmail   
     @recipients = 'arjun.singh@angelbroking.com;dinesh.jadhav@angelbroking.com;',
      @body_format='HTML',                            
      @subject = @Subject,                            
      @body = @tableHTML,                           
      @profile_name ='MimansaTeam'
