-- Object: PROCEDURE dbo.Angel_nri_data_lec_new_25mar2022
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------


--use 196-- inhouse                                
--exec Angel_nri_data_lec_new '23/03/2022',1,0,'NSE'                                  
create PROC Angel_nri_data_lec_new_25mar2022(@SDATE  AS VARCHAR(11),            
                                   @TYPE   AS INT,            
                                   @BANKID AS INT,            
                                   @SEG    AS VARCHAR(5))            
AS            
  BEGIN            
    ...
    --  SET @SDATE ='22/02/2022' SET @TYPE ='1' SET @BANKID =0 SET @SEG='NSE'                                                    
     ...
     Exec master.dbo.xp_cmdshell 'NET USE \\196.1.115.147\IPC$ /USER:Administrator Winw0rld@1604'  
              ...
              BEGIN            
                 ...
                 WHERE  PARTY_CODE = @PARTYCODE      
				  ...
				  declare @setdate as varchar(10)				 
				   declare @bseBank as varchar(50),@BsecliBankBranch as varchar(100),@BseCliBnkAcc as varchar(50),@BseangelAcc as varchar(50)
	  ...
	  CASE   WHEN @TYPE = 1 THEN 'B'  WHEN @TYPE = 2 THEN 'S'  END + '|A|'+'T2'+'|'+@SETT_NO+'|'+@setdate ,@bseBank            
	   ...
	   End		
	        ...
	        BEGIN
			  ...
			  select @bseBank+','+Convert(varchar,a.SAUDA_DATE,103)+','+@PARTYCODE+','+b.DPid1 +','+cast(right(b.cltdpID1,8) as varchar) +','+b.Short_Name +','+@SEG +','+isnull(@SETT_NO,'') +','+
			  CONVERT(VARCHAR,a.CONTRACTNO)+','+ltrim(rtrim(c.scripcode))+','+Ltrim(Rtrim(Isnull(c.ISIN,'')))+','+
			  left(c.Scripname,75)+','+CASE WHEN @TYPE = 1 THEN 'B'  WHEN @TYPE = 2 THEN 'S'  END +','+Ltrim(Rtrim(CONVERT(VARCHAR,c.QTY))) +','+'@@@[Weighted Average Market Rate]' +','+cast(Ltrim(Rtrim(CONVERT(DEC(10,2),c.TRANACTIONAMT))) as varchar)+','+
		     ...
		     END +','+  --Other Charges end+','+
			  cast((CONVERT(DEC(15,2),c.RATE) *c.QTY) as varchar)+','+ CONVERT(VARCHAR,CONVERT(DECIMAL(14, 2),Sum(Isnull(STT, 0))))+','+
			  '@@@[Exact Amount Paid By/To The Customer (For Purchase/Sell)]'+','+'@@@[RBI Approval Number]'+','+@BseCliBnkAcc+','+'@@@[Saving Account Number]'+','+@BsecliBankBranch+','+'Angel One Ltd'+','+'@@@[Date of Receipt/Payment]'+','+
			  '@@@[Demat Debit/Credit Date]'+','+@BseangelAcc,@BseBank
			  from #BSE_HEADER a			  
			  left outer join [196.1.115.132].Risk.dbo.client_Details b with (nolock) on a.PARTY_CODE=b.party_code	
			  left outer join #BSE_DETAILS c on a.party_code=c.party_code
			  where a.party_code= @PARTYCODE
			  group by
     ...
     SELECT @CALCULATION = CONVERT(DECIMAL(14, 2), ( @TRANS_AMOUNT ) - ( SERVICE_TAX + EDUCESS + EXCHAGE_LEVIES + STT + STAMPDUTY + MINBROKERAGE + OTHER_CHARGES) )      
               ...
               WHERE  PARTY_CODE = @PARTYCODE            
				  ...
				  declare @nseBank as varchar(50),@cliBankBranch as varchar(100),@CliBnkAcc as varchar(50),@angelAcc as varchar(50)
				...
				--set  @nsesetdate =(select top 1 Sett_Date from Msajag.dbo.common_CONTRACT_DATA with(nolock) where sett_no=@nseSETT_NO)
				 ...
				 select @nseBank=fld_bankName,@cliBankBranch=Fld_branchlocation,@CliBnkAcc=fld_ClientPISac,@angelAcc=Fld_AngelAcc from Intranet.risk.dbo.V_NriBank_Details with (nolock) where fld_clientcode = @PARTYCODE
	  ...
	  CASE   WHEN @TYPE = 1 THEN 'B'  WHEN @TYPE = 2 THEN 'S'  END + '|A|'+'T2'+'|'+isnull(@nseSETT_NO,'')+'|'+isnull(@nsesetdate,'') ,@nseBank  
      ...
      + '|' + Ltrim(Rtrim(CONVERT(VARCHAR, CONVERT(DEC(10, 2), BROK_PERSCRIP)))) + '|' + Ltrim(Rtrim(CONVERT(DEC(10, 2), TRANACTIONAMT )))+'|'+left(Scripname,75),@nseBank          
       ...
       End     
	        ...
	        BEGIN
			  ...
			  select @nseBank+','+Convert(varchar,a.SAUDA_DATE,103)+','+@PARTYCODE+','+b.DPid1 +','+cast(right(b.cltdpID1,8) as varchar) +','+b.Short_Name +','+@SEG +','+isnull(@nseSETT_NO,'') +','+
			  CONVERT(VARCHAR,a.CONTRACTNO)+','+ltrim(rtrim(c.scripcode))+','+Ltrim(Rtrim(Isnull(c.ISIN,'')))+','+
			  left(c.Scripname,75)+','+CASE WHEN @TYPE = 1 THEN 'B'  WHEN @TYPE = 2 THEN 'S'  END +','+Ltrim(Rtrim(CONVERT(VARCHAR,c.QTY))) +','+'@@@[Weighted Average Market Rate]' +','+cast(Ltrim(Rtrim(CONVERT(DEC(10,2),c.TRANACTIONAMT))) as varchar)+','+
			  Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(15,2), c.RATE))))+','+Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(10,2), c.BROK_PERSCRIP))))+','+
			  CASE WHEN @diffamt BETWEEN -0.03 AND 0.03       
		     ...
		     END +','+  --Other Charges end+','+
			  cast((CONVERT(DEC(15,2),c.RATE) *c.QTY) as varchar)+','+ CONVERT(VARCHAR,CONVERT(DECIMAL(14, 2),Sum(Isnull(STT, 0))))+','+
			  '@@@[Exact Amount Paid By/To The Customer (For Purchase/Sell)]'+','+'@@@[RBI Approval Number]'+','+@CliBnkAcc+','+'@@@[Saving Account Number]'+','+@cliBankBranch+','+'Angel One Ltd'+','+'@@@[Date of Receipt/Payment]'+','+
			  '@@@[Demat Debit/Credit Date]'+','+@angelAcc,@nseBank
			  from #NSE_HEADER a			  
			  left outer join [196.1.115.132].Risk.dbo.client_Details b with (nolock) on a.PARTY_CODE=b.party_code	
	 ...
	 if ((select count(1) from tbl_NRILec_temp) >0)
	   Begin
		   ...
		   DECLARE @Axisfilename2 as varchar(200)
		   DECLARE @indusfilename2 as varchar(200)
		 ...
		 --set @Ftype=  'BUY_NSE'    
		  ...
		  if ((select count(1) from tbl_NRILec_temp where bankname like '%AXIS%')>0)
		   Begin
			  ...
			  SET @BCPCOMMAND1 = @BCPCOMMAND1 + @Axisfilename2 + '" -c -t, -S196.1.115.196 -Uinhouse -Pinh6014'                                  
		   ...
		   Begin
			...
			-- print(@BCPCOMMAND1)                      
			  SET @BCPCOMMAND1 = @BCPCOMMAND1 + @indusfilename2 + '" -c -t, -S196.1.115.196 -Uinhouse -Pinh6014'                                  
			  EXEC MASTER..XP_CMDSHELL @BCPCOMMAND1 
		   ...
		   Begin
			...
			-- print(@BCPCOMMAND1)                      
			  SET @BCPCOMMAND1 = @BCPCOMMAND1 + @Idfcfilename2 + '" -c -t, -S196.1.115.196 -Uinhouse -Pinh6014'                                  
			  EXEC MASTER..XP_CMDSHELL @BCPCOMMAND1 
		   ...
		   Begin
			...
			-- print(@BCPCOMMAND1)                      
			  SET @BCPCOMMAND1 = @BCPCOMMAND1 + @hdfcfilename2 + '" -c -t, -S196.1.115.196 -Uinhouse -Pinh6014'                                  
			  EXEC MASTER..XP_CMDSHELL @BCPCOMMAND1 
		   ...
		   Begin
			...
			-- print(@BCPCOMMAND1)                      
			  SET @BCPCOMMAND1 = @BCPCOMMAND1 + @IDBIfilename2 + '" -c -t, -S196.1.115.196 -Uinhouse -Pinh6014'                                  
			  EXEC MASTER..XP_CMDSHELL @BCPCOMMAND1
		   ...
		   Begin
			...
			-- print(@BCPCOMMAND1)                      
			  SET @BCPCOMMAND1 = @BCPCOMMAND1 + @YESfilename2 + '" -c -t, -S196.1.115.196 -Uinhouse -Pinh6014'                                  
			  EXEC MASTER..XP_CMDSHELL @BCPCOMMAND1 
		  ...
		  SET @BCPCOMMAND1 = 'BCP "select FLDCOL from Inhouse.dbo.tbl_NRILec_temp" QUERYOUT "'                            
			-- print(@BCPCOMMAND1)                      
		  SET @BCPCOMMAND1 = @BCPCOMMAND1 + @Allfilename2 + '" -c -t, -S196.1.115.196 -Uinhouse -Pinh6014'                                  
		  EXEC MASTER..XP_CMDSHELL @BCPCOMMAND1             
