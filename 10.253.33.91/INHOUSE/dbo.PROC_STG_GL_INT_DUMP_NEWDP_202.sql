-- Object: PROCEDURE dbo.PROC_STG_GL_INT_DUMP_NEWDP_202
...
CREATE PROCEDURE [dbo].[PROC_STG_GL_INT_DUMP_NEWDP_202] (@SEGMENT VARCHAR(10))        
AS            
...
BEGIN                
 ...
 SELECT L.LDG_ACCOUNT_ID, null, SUM(CASE                 
    ...
    WHEN LDG_AMOUNT >= '0'              
     THEN LDG_AMOUNT * - 1                
    ELSE 0                
    END) AS CRAMT, 0 AS CLBALANCE, NULL, NULL, NULL,   'DP202','145'      
                      
   FROM angeldp202.DMAT.citrus_usr.LEDGER10 L WITH(NOLOCK)  ------------3 for April---              
...
when FINA_ACC_CODE like '%UGST' then 'UGST'  
      ...
      FROM angeldp202.DMAT.CITRUS_USR.FIN_ACCOUNT_MSTR       WITH(NOLOCK)      
      WHERE CLTCODE  =   FINA_ACC_ID               
                 
  UPDATE #TMP_TB_PUSH                
     SET GRPCATEGORY = FINGM_GROUP_NAME                 
     FROM angeldp202.DMAT.CITRUS_USR.FIN_ACCOUNT_MSTR  WITH(NOLOCK)      ,angeldp202.DMAT.CITRUS_USR.FIN_GROUP_MSTR    WITH(NOLOCK)                
     WHERE FINA_GROUP_ID =FINGM_GROUP_CODE AND CLTCODE =FINA_ACC_CODE       
	 
	 SELECT DPAM_ID,DPAM_SBA_NO INTO #DP  FROM angeldp202.DMAT.CITRUS_USR.DP_ACCT_MSTR  WITH(NOLOCK)     --  WHERE  EXISTS (SELECT  CLTCODE FROM  #TMP_TB_PUSH WHERE CONVERT(VARCHAR(8), DPAM_ID)=CLTCODE)
 ...
 DELETE FROM STG_GL_INT_DUMP WHERE SEGMENT1 ='145'               
