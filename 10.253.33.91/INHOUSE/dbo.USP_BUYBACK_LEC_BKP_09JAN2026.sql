-- Object: PROCEDURE dbo.USP_BUYBACK_LEC_BKP_09JAN2026
...
CREATED BY :- HRISHI ON 28-10-2025
DESCREPTION:- BUYBACK LEC FOR ALL BANKS
*/

CREATE PROC [dbo].[USP_BUYBACK_LEC_BKP_09JAN2026] ( @SDATE AS VARCHAR(11) )
AS
BEGIN

SET @SDATE = CONVERT(VARCHAR(11), CONVERT(DATETIME, @SDATE, 103)) ----// 28/10/2025
--DECLARE @SDATE AS VARCHAR(11)='DEC  2 2025'   --, @CODE VARCHAR (50)='ZR0050096R'
...
( SUM(PAMTTRD + PAMTDEL) / CASE WHEN SUM(PQTYTRD + PQTYDEL) > 0 THEN SUM(PQTYTRD + PQTYDEL) ELSE 1 END) ,                          
SNETRATE = ( SUM((CASE WHEN SQTYTRD =0 AND SAMTTRD<>0 THEN -SAMTTRD ELSE SAMTTRD END) +                          
(CASE WHEN SQTYDEL=0 AND SAMTDEL<>0 THEN -SAMTDEL ELSE SAMTDEL END)) / (CASE WHEN SUM(SQTYTRD + SQTYDEL) > 0 THEN SUM(SQTYTRD + SQTYDEL) ELSE 1 END ) ), TRDAMT= SUM(TRDAMT) ,DELAMT=SUM(DELAMT), SERINEX=SUM(SERINEX),SERVICE_TAX= SUM(SERVICE_TAX) ,                          
EXSERVICE_TAX= SUM(EXSERVICE_TAX),TURN_TAX=SUM(TURN_TAX),SEBI_TAX=SUM(SEBI_TAX),INS_CHRG=SUM(INS_CHRG),BROKER_CHRG=SUM(BROKER_CHRG),                          
OTHER_CHRG=SUM(OTHER_CHRG), MEMBERTYPE,COMPANYNAME,PNL = SUM(SAMTTRD-PAMTTRD),SERIES          
...
UPDATE A SET A.CONTRACTNO_NEW=B.CONTRACTNO_NEW, A.IPFT=B.IPFT_CHRG, A.ISIN=B.ISIN      
--SELECT A.PARTY_CODE,A.CONTRACTNO_NEW,B.CONTRACTNO_NEW, A.IPFT,B.IPFT_CHRG,B.ISIN,A.ISIN,A.SCRIP_NAME,B.SCRIPNAME  
FROM #MAIN_DATA A, #CONTRACT B       
WHERE A.PARTY_CODE=B.PARTY_CODE AND A.SAUDA_DATE=B.SAUDA_DATE AND A.SCRIP_NAME=B.SCRIPNAME       
...
[INVESTOR NAME]=B.LONG_NAME, C.EXCHANGE, [SETTLEMENT NO]=C.SETT_NO, [CONTRACT NO]=C.CONTRACTNO_NEW, [SCRIP CODE]=C.SCRIP_CD, C.ISIN, C.SCRIP_NAME, [BUY/SELL],   
...
[NET RATE]=ROUND (C.PMARKETRATE+C.SMARKETRATE ,2), BROKEARGE=ROUND(C.BUYDELIVERYCHRG+C.SELLDELIVERYCHRG, 2),   
[OTHER CHARGES]=ROUND(C.SERVICE_TAX+C.TURN_TAX+C.SEBI_TAX+C.BROKER_CHRG+ISNULL(C.IPFT,0), 2)+ROUND(T.TDS,2),  
[NET PURCHASE/SALE VALUE]=ROUND((C.SMARKETRATE*STRADEDQTY-C.BUYDELIVERYCHRG-C.SELLDELIVERYCHRG-C.SERVICE_TAX-C.TURN_TAX-C.SEBI_TAX-C.BROKER_CHRG-ISNULL(C.IPFT,0)),2)-ROUND(T.TDS,2), 
[STT ON BUY & SALE]=C.INS_CHRG,
[EXACT AMOUNT PAID BY / TO  THE CUSTOMER (FOR PURCHASE/SALE)]= ROUND((C.SMARKETRATE*STRADEDQTY-C.BUYDELIVERYCHRG-C.SELLDELIVERYCHRG-C.SERVICE_TAX-C.TURN_TAX-C.SEBI_TAX-C.BROKER_CHRG-ISNULL(C.IPFT,0))-C.INS_CHRG,2)-ROUND(T.TDS,2),
...
INTO #FINAL_HDFC_YES
FROM #MAIN_DATA C INNER JOIN TBL_TDS_UPLOAD_BUYBACK_LEC T ON C.PARTY_CODE= T.CLIENT_CODE  
LEFT JOIN #TBL_NRICLIENTMASTER N ON N.FLD_CLIENTCODE=C.PARTY_CODE LEFT JOIN #V_NRIBANK_DETAILS V ON V.FLD_CLIENTCODE=N.FLD_CLIENTCODE  
LEFT OUTER JOIN #CLIENT_DETAILS B WITH (NOLOCK) ON C.PARTY_CODE=B.PARTY_CODE  
--WHERE C.PARTY_CODE IN ('ZR1577R','ZR0050096R')  

TRUNCATE TABLE INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_HDFC_YES

INSERT INTO INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_HDFC_YES
...
SELECT N.CLIPISNO AS BANK_ACC_NO, SCHEME='OFFLINE', M.PARTY_CODE AS CLIENT_CODE, CLIENT_NAME =B.LONG_NAME, M.SAUDA_DATE AS TRADE_DATE,VALAN=M.SETT_TYPE+'/'+M.SETT_NO,          
...
CASE WHEN M.BILLPAMT<>'0.00' AND M.BILLSAMT='0.00' THEN M.TRDAMT+BUYDELIVERYCHRG+(M.SERVICE_TAX+M.TURN_TAX+M.SEBI_TAX+M.BROKER_CHRG+ISNULL(M.IPFT,0))+INS_CHRG+ROUND(T.TDS,2)      
ELSE M.TRDAMT-SELLDELIVERYCHRG-(M.SERVICE_TAX+M.TURN_TAX+M.SEBI_TAX+M.BROKER_CHRG+ISNULL(M.IPFT,0))-INS_CHRG-ROUND(T.TDS,2) END AS AMOUNT,  
...
(M.TURN_TAX+M.SEBI_TAX+ISNULL(M.IPFT,0)+ROUND(T.TDS,2)) AS OTHER_CHARGES, -- ADDED ON 15 JUL 2025       
...
INTO #FINAL_KOTAK          
FROM #MAIN_DATA M INNER JOIN TBL_TDS_UPLOAD_BUYBACK_LEC T ON M.PARTY_CODE= T.CLIENT_CODE  
LEFT JOIN #TBL_NRICLIENTMASTER N ON M.PARTY_CODE = N.FLD_CLIENTCODE
LEFT OUTER JOIN #CLIENT_DETAILS B WITH (NOLOCK) ON M.PARTY_CODE=B.PARTY_CODE
...
TRUNCATE TABLE INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_KOTAK

INSERT INTO INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_KOTAK
...
ROUND(SERVICE_TAX,2) AS SERVICE_TAX, '0' AS 'HARD_CODE1',ROUND(TURN_TAX,2) AS TURN_TAX, ROUND(INS_CHRG,2) AS 'STT', OTHER_CHARGES=ROUND(BROKER_CHRG,2)+ROUND(T.TDS,2) ,
ROUND(SEBI_TAX,2) AS SEBI_TAX, '1' AS 'NO_OF_TRADE', 
NET_AMOUNT= CASE WHEN [BUY/SELL]='S' THEN ROUND(M.BILLSAMT,2)-ROUND(T.TDS,2) WHEN [BUY/SELL]='B' THEN M.BILLPAMT+T.TDS END,DP_ACCOUNT=SUBSTRING(N.FLD_DEMATACNO,1,8), 
...
TRUNCATE TABLE INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS

INSERT INTO INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS
...
----SELECT * FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS ORDER BY PARTY_CODE

DECLARE @AXISFILENAME AS VARCHAR(200)
DECLARE @BCPCOMMAND1 AS VARCHAR(1000)
IF ((SELECT COUNT(1) FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%AXIS%')>0)                          
     BEGIN                          
      SET @AXISFILENAME = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\AXIS_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.TXT'                           
      SET @BCPCOMMAND1 = 'BCP "SELECT DETAILS FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%AXIS%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
	 ...
	 END
SELECT '<a href='+replace(@AXISFILENAME,'\\INHOUSELIVEAPP1-FS.angelone.in','')+'>Right click and save -- AXISBUYBACK</a><br>'

DECLARE @INDUSINDFILENAME AS VARCHAR(200)
DECLARE @BCPCOMMAND2 AS VARCHAR(1000)
IF ((SELECT COUNT(1) FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%INDUSIND%')>0)                          
     BEGIN                          
      SET @INDUSINDFILENAME = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\INDUSIND_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.TXT'                           
      SET @BCPCOMMAND2 = 'BCP "SELECT DETAILS FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%INDUSIND%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
	 ...
	 END
SELECT '<a href='+replace(@INDUSINDFILENAME,'\\INHOUSELIVEAPP1-FS.angelone.in','')+'>Right click and save -- INDUSINDBUYBACK</a><br>'

DECLARE @IDFCFILENAME AS VARCHAR(200)
DECLARE @BCPCOMMAND3 AS VARCHAR(1000)
IF ((SELECT COUNT(1) FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%IDFC%')>0)                          
     BEGIN                          
      SET @IDFCFILENAME = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\IDFC_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.TXT'                           
      SET @BCPCOMMAND3 = 'BCP "SELECT DETAILS FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%IDFC%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
	 ...
	 END
SELECT '<a href='+replace(@IDFCFILENAME,'\\INHOUSELIVEAPP1-FS.angelone.in','')+'>Right click and save -- IDFCBUYBACK</a><br>'

------- ALL BANK END

IF ((SELECT COUNT(1) FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_HDFC_YES where [BANK NAME] like '%hdfc%')>0)                          
 BEGIN          
  ...
  SET @ALL1 = 'EXEC MASTER.DBO.XP_CMDSHELL ''BCP "SELECT ''''BANK NAME'''',''''TRADE DATE'''',''''TRADING ID'''',''''DP ID'''',''''DEMAT A/C NO'''',''''INVESTOR NAME'''',''''EXCHANGE'''',''''SETTLEMENT NO'''',''''CONTRACT NO'''',''''SCRIP CODE'''',''''ISIN NUMBER'''',''''SCRIPT NAME'''',''''BUY/SELL'''',''''QUANTITY'''',''''WEIGHTED AVERAGE MARKET RATE'''',''''TOTAL MARKET VALUE'''',''''NET RATE'''',''''BROKERAGE VALUE'''',''''OTHER CHARGES'''',''''NET PURCHASE/SALE VALUE'''',''''STT ON BUY & SALE'''',''''EXACT AMOUNT PAID BY / TO  THE CUSTOMER (FOR PURCHASE/SALE)'''',''''RBI APPROVAL NO'''',''''PIS ACCOUNT NO'''',''''SAVING BANK ACCOUNT NO'''',''''BANK BRANCH NAME'''',''''BROKER NAME'''',''''DATE OF RECEIPT /PAYMENT'''',''''DEMAT DEBIT /CREDIT DATE'''',''''BROKER A/C NO'''''
  SET @ALL1 = @ALL1+ ' UNION ALL SELECT * FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_HDFC_YES WHERE [BANK NAME] like ''''%hdfc%'''''                              
  --PRINT @ALL1                                
 ...
 IF ((SELECT COUNT(1) FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_HDFC_YES where [BANK NAME] like '%YES%')>0)                          
 BEGIN          
  ...
  SET @ALL2 = @ALL2+ ' UNION ALL SELECT * FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_HDFC_YES WHERE [BANK NAME] like ''''%YES%'''''                              
  --PRINT @ALL2
 ...
 END  
  ...
  IF ((SELECT COUNT(1) FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_KOTAK)>0)                          
 BEGIN          
  ...
  SET @ALL3 = 'EXEC MASTER.DBO.XP_CMDSHELL ''BCP "SELECT ''''BANK_ACC_NO'''',''''SCHEME'''',''''CLIENT_CODE'''',''''CLIENT_NAME'''',''''TRADE_DATE'''',''''VALAN'''',''''DEAL_TYPE'''',''''ISIN NUMBER'''',''''SCRIPT NAME'''',''''QUANTITY'''',''''GROSS_RATE'''',''''BROKERAGE VALUE'''',''''NET_RATE'''',''''AMOUNT'''',''''STT'''',''''TXN_TYPE'''',''''DP_ID'''',''''SHORT_GAIN'''',''''SHORT_LOSS'''',''''LONG_GAIN'''',''''LONG_LOSS'''',''''TDS'''',''''PMS_PRODUCT'''',''''CONTRACT_NUMBER'''',''''SERVICE_TAX'''',''''EDUCATION_CESS'''',''''EXCHANGE_LEVY'''',''''STAMPDUTY'''',''''OTHER_CHARGES'''',''''DEPOSITORY_ID'''',''''SETTLEMENT_TYPE'''',''''SETTLEMENT_NUMBER'''',''''SETTLEMENT_DATE'''',''''SEBI_REG_NO1'''',''''SEBI_SUB_CATEGORY'''',''''INSTRUMENT_SUB_TYPE'''',''''PANNO'''',''''NON_PINS_ACCOUNT'''''
  SET @ALL3 = @ALL3+ ' UNION ALL SELECT * FROM INHOUSE.DBO.TBL_NRI_BUYBACK_LEC_KOTAK'                              
  --PRINT @ALL3                                
