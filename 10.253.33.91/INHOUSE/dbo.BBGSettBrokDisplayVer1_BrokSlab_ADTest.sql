-- Object: PROCEDURE dbo.BBGSettBrokDisplayVer1_BrokSlab_ADTest
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

CREATE  procedure [dbo].[BBGSettBrokDisplayVer1_BrokSlab_ADTest]
(
	@Sett_no Varchar(7),
	@Sett_type Varchar(2),
	...
	@MarginSlab varchar(100),
	@emp_no varchar(20)
)

as
-- Exec [BBGSettBrokDisplayVer1_BrokSlab_ADTest] '1','D','JOD16610','532296','','1',4,1070.40,4,'JAN  1 2000','broker','','MAMT001','E60022'
-- Exec [BBGSettBrokDisplayVer1_BrokSlab] '1','D','M98194','532296','','1',4,1070.40,4,'JAN  1 2000','broker','','E60022','E60022'

	SET @Sauda_date = convert(varchar(11),getdate(),109) -- Added by Anand (Approved by BG sir)

	Declare @@Party_code As Varchar(12),
	...
	Begin
		...
		Begin
			...
			update #ViewSettlement set Tmark = Tmark,
			...
			FROM Msajag.dbo.BrokTable BrokTable,#ViewSettlement ,Msajag.dbo.globals globals, 
		   Msajag.dbo.ClientTaxes_New CT, Msajag.dbo.Owner,
		   [MIMANSA].CRM.[dbo].[tbl_BrokCalculator_MarginSlab] S
		   WHERE #ViewSettlement.Sauda_Date Between FromDate And ToDate
	...
	Begin
			Update #ViewSettlement set Tmark = Tmark , table_no = broktable.table_no, line_no = broktable.line_no,val_perc = broktable.val_perc,
				Normal = Broktable.Normal, day_puc= Broktable.Day_puc,day_sales = Broktable.day_sales,
				sett_purch =   Broktable.Sett_purch,sett_sales = broktable.Sett_sales,
				NBrokApp = case when #ViewSettlement.SELL_BUY = 1 then Broktable.Day_puc
								else Broktable.Day_Sales end
									 ,
				   N_NetRate = (  case
								  when (  broktable.val_perc ='V' AND #ViewSettlement.SELL_BUY = 1)
										 Then #ViewSettlement.marketrate + Broktable.Day_puc
			...
			FROM Msajag.dbo.BrokTable BrokTable,globals, #ViewSettlement, Msajag.Dbo.Owner,
			[MIMANSA].CRM.[dbo].[tbl_BrokCalculator_MarginSlab] S
			WHERE S.MarginSlab = ''
			and S.Trading_Delivery = Broktable.table_No
