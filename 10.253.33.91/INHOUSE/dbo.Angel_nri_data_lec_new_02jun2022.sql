-- Object: PROCEDURE dbo.Angel_nri_data_lec_new_02jun2022
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

--use 196-- inhouse                                    
--exec Angel_nri_data_lec_new '10/05/2022',2,0,'NSE'                                      
CREATE PROC Angel_nri_data_lec_new_02jun2022(@SDATE  AS VARCHAR(11),                
                                   @TYPE   AS INT,                
                                   @BANKID AS INT,                
                                   @SEG    AS VARCHAR(5))                
AS                
  ...
  select @nsemax=max(Srno) from #NSEsettno    
   ...
   from Intranet.risk.dbo.tbl_NRIClientMaster with (nolock)  where fld_clientcode = @PARTYCODE    
     ...
     CONVERT(VARCHAR,CONVERT(DECIMAL(14,2), a.TOTALAMT))+','+'@@@[RBI Approval Number]'+','+@CliPISNo+','+@CliBnkAcc+','+@cliBankBranch+','+'Angel One Ltd'+','+ CASE WHEN @TYPE = 1 THEN  Convert(varchar,(a.sauda_date +1),103)  WHEN @TYPE = 2 THEN Convert(
  
varchar,(a.sauda_date+2),103)  END+','+    
