-- Object: PROCEDURE dbo.Angel_nri_data_lec_new_13apr2022
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------


--use 196-- inhouse                                
--exec Angel_nri_data_lec_new '29/03/2022',1,0,'NSE'                                  
create PROC Angel_nri_data_lec_new_13apr2022(@SDATE  AS VARCHAR(11),            
                                   @TYPE   AS INT,            
                                   @BANKID AS INT,            
                                   @SEG    AS VARCHAR(5))            
AS            
  BEGIN            
    ...
    --  DECLARE @SDATE AS VARCHAR(11),@TYPE AS VARCHAR(2),@BANKID AS INT,@SEG AS VARCHAR(5)                                                    
     -- SET @SDATE ='29/03/2022' SET @TYPE ='1' SET @BANKID =0 SET @SEG='NSE'                                                    
	 ...
	 while (@rw<=@max)
	  ...
	  Begin
			  ...
			  CASE   WHEN @TYPE = 1 THEN 'B'  WHEN @TYPE = 2 THEN 'S'  END + '|A|'+CASE WHEN (@TYPE = 2 and right(@bseSETT_NO,3)<500) THEN 'T2' else 'T1'end+'|'+@bseSETT_NO+'|'+@setdate ,@bseBank            
	   ...
	   End		
	        ...
	        BEGIN
			 ...
			 select @BseTotalAmt=(sum(TRANACTIONAMT *-1)) from #BSE_DETAILS where party_code =@PARTYCODE
			 ...
			 from Intranet.risk.dbo.tbl_NRIClientMaster with (nolock)  where fld_clientcode = @PARTYCODE
			  ...
			  left(c.Scripname,75)+','+CASE WHEN @TYPE = 1 THEN 'Buy'  WHEN @TYPE = 2 THEN 'Sell'  END +','+Ltrim(Rtrim(CONVERT(VARCHAR,c.QTY))) +','+Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(15,2), c.RATE)))) +','+cast(Ltrim(Rtrim(CONVERT(DEC(10,2),c.TRANACTIONAMT *-1))) as varchar)+','+
			  Convert(Varchar,c.PRate)+','+Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(10,2), c.BROK_PERSCRIP))))+','+
			  CASE WHEN @DIFFAMT BETWEEN -0.03 AND 0.03       
			   THEN CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),( Sum(CONVERT(DECIMAL(14,2),Isnull(MINBROKERAGE,0)) + CONVERT(DECIMAL(14,2),Isnull(OTHER_CHARGES,0))) + @DIFFAMT )))        
			   ELSE CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),( Sum(Isnull(MINBROKERAGE,0) + Isnull(OTHER_CHARGES,0)) )))        
		     END +','+  --Other Charges end+','+
			  cast((CONVERT(DEC(15,2),c.RATE) *c.QTY) as varchar)+','+ CONVERT(VARCHAR,CONVERT(DECIMAL(14, 2),Sum(Isnull(STT, 0))))+','+
			  Convert(Varchar,CONVERT(DECIMAL(14,2),@BseTotalAmt))+','+'@@@[RBI Approval Number]'+','+@BseCliPISNo+','+@BseCliBnkAcc+','+@BsecliBankBranch+','+'Angel One Ltd'+','+CASE WHEN @TYPE = 1 THEN  Convert(varchar,(a.sauda_date +1),103)  WHEN @TYPE = 2 THEN Convert(varchar,(a.sauda_date+2),103)  END +','+
			  ...
			  left(c.Scripname,75) ,Ltrim(Rtrim(CONVERT(VARCHAR, c.QTY))) ,Ltrim(Rtrim(CONVERT(DEC(10, 2), c.TRANACTIONAMT*-1 ))),c.scripcode,
			  Ltrim(Rtrim(CONVERT(VARCHAR, CONVERT(DEC(15, 2), c.RATE)))),Ltrim(Rtrim(CONVERT(VARCHAR, CONVERT(DEC(10, 2), c.BROK_PERSCRIP)))),
			  c.QTY,CONVERT(DEC(15, 2), c.Rate),c.PRate
     ...
     SELECT @CALCULATION = CONVERT(DECIMAL(14, 2), ( @TRANS_AMOUNT ) - ( SERVICE_TAX + EDUCESS + EXCHAGE_LEVIES + STT + STAMPDUTY + MINBROKERAGE + OTHER_CHARGES) )      
	  ...
	  Begin
			  ...
			  CASE   WHEN @TYPE = 1 THEN 'B'  WHEN @TYPE = 2 THEN 'S'  END + '|A|'+CASE WHEN (@TYPE = 2 and right(@nseSett_no2,3)<500 )THEN 'T2' else 'T1'end+'|'+isnull(@nseSett_no2,'')+'|'+isnull(@nsesetdate,'') ,@nseBank  
       ...
       End 	   
			...
			select @TotalAmt=(sum(TRANACTIONAMT *-1)) from #NSE_DETAILS where party_code =@PARTYCODE
			...
			from Intranet.risk.dbo.tbl_NRIClientMaster with (nolock)  where fld_clientcode = @PARTYCODE
		     ...
		     END +','+  --Other Charges end+','+
			  cast((CONVERT(DEC(15,2),c.RATE) *c.QTY) as varchar)+','+ CONVERT(VARCHAR,CONVERT(DECIMAL(14, 2),Sum(Isnull(STT, 0))))+','+
			  CONVERT(VARCHAR,CONVERT(DECIMAL(14,2), @TotalAmt))+','+'@@@[RBI Approval Number]'+','+@CliPISNo+','+@CliBnkAcc+','+@cliBankBranch+','+'Angel One Ltd'+','+ CASE WHEN @TYPE = 1 THEN  Convert(varchar,(a.sauda_date +1),103)  WHEN @TYPE = 2 THEN Convert(varchar,(a.sauda_date+2),103)  END+','+
