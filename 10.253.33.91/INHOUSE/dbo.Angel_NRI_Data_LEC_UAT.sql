-- Object: PROCEDURE dbo.Angel_NRI_Data_LEC_UAT
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

--use inhouse                            
--exec Angel_nri_data_lec_uat '03/07/2013',2,0,'BSE'                              
CREATE PROC Angel_nri_data_lec_uat(@SDATE  AS VARCHAR(11),        
                                   @TYPE   AS INT,        
                                   @BANKID AS INT,        
                                   @SEG    AS VARCHAR(5))        
AS        
  BEGIN        
      ...
      WHILE @CNT > 0        
            ...
            IF @SEG = 'BSE'        
              BEGIN        
                 ...
                 WHERE  PARTY_CODE = @PARTYCODE  
                  ...
                  --select * from #TEMP1                            
                  INSERT INTO #TEMP1        
                  SELECT 'H' + '|' + Ltrim(Rtrim(PARTY_CODE)) + '|' + Replace(CONVERT(VARCHAR(11), SAUDA_DATE, 102), '.', '') + '|' + CONVERT(VARCHAR, CONTRACTNO) + '|' + CONVERT(VARCHAR, CONVERT(DECIMAL(14, 2), Sum(Isnull(SERVICE_TAX, 0)))) + '|' + '0.00' + '|' + CONVERT(VARCHAR, CONVERT(DECIMAL(14, 2), Sum(Isnull(EXCHAGE_LEVIES, 0)))) + '|' + CONVERT(VARCHAR, CONVERT(DECIMAL(14, 2), Sum(Isnull(STT, 0)))) + '|' + CONVERT(VARCHAR, CONVERT(DECIMAL(14, 2), Sum(Isnull(STAMPDUTY, 0)))) + '|' +   
        ...
        CONVERT(VARCHAR, CONVERT(DECIMAL( 14, 2), Sum(TOTALAMT) ))        
                  FROM   #BSE_HEADER WITH (NOLOCK)        
