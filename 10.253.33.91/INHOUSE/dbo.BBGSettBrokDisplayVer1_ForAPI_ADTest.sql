-- Object: PROCEDURE dbo.BBGSettBrokDisplayVer1_ForAPI_ADTest
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------


---- Exec BBGSettBrokDisplayVer1_ForAPI 'M114540','','1',500,88.15,2,'E60022'
---- Exec BBGSettBrokDisplayVer1_ForAPI_ADTest 'ALWR1937','','1',100,1000,4,'ALWR1937'
-- http://172.17.41.190/BrokCalculatorAPI/Service1.svc/GetBrokCalEQ/AEMobile/8D139516-D89B-4371-BB47-D1D4D456F34A/M114540/''/1/500/88.15/2/E60022/172.17.41.190/NSE
-- http://172.17.41.190/BrokCalculatorAPI/Service1.svc/GetBrokCalEQ/AEMobile/8D139516-D89B-4371-BB47-D1D4D456F34A/ALWR1937/''/1/100/1000/1/ALWR1937/172.29.24.132/NSE
...
Begin
   ...
   --and BrokTable.Lower_lim>=@TradeQty
	...
	Select  @MyBroker_chrg = Broker_Note from [MIMANSA].[ClientService].dbo.NseClientTaxes_New where party_code = @Party_code
	...
	From [MIMANSA].[ClientService].dbo.NSeSTATE_MASTER_DATA Where [State] = @Mystate
	...
	Begin
		...
		Set @BROKER_NOTE = @BROKER_CHRG
		Set @TOBECHRG_STAMPDUTY = @BROKER_CHRG
		Set @TRD_TURNOVER = CASE WHEN @SettFlag < 4 THEN @TRADEQTY*@MARKETRATE ELSE 0 END                    
		Set @DEL_TURNOVER = CASE WHEN @SettFLAG > 3 THEN @TRADEQTY*@MARKETRATE ELSE 0 END                     
		Set @ROUNDEDTURNOVER = @TRADEQTY*@MARKETRATE                   
		Set @TRD_ROUNDEDTURNOVER = CASE WHEN @Settflag < 4 THEN @TRADEQTY*@MARKETRATE ELSE 0 END                    
		Set @DEL_ROUNDEDTURNOVER = CASE WHEN @SettFlag > 3 THEN @TRADEQTY*@MARKETRATE ELSE 0 END           

   

		Select @ROUNDEDTURNOVER = dbo.ROUNDEDTURNOVER(@TRADEQTY*@MARKETRATE, @For_TurnOver)         
		Select @TRD_ROUNDEDTURNOVER = dbo.ROUNDEDTURNOVER(@TRD_TURNOVER, @For_TurnOver)             
		Select  @DEL_ROUNDEDTURNOVER = dbo.ROUNDEDTURNOVER(@DEL_TURNOVER, @Del_For_TurnOver)                       
 

			Select @TOBECHRG_STAMPDUTY =  (CASE WHEN @FOR_TURNOVER = @DEL_FOR_TURNOVER           
												THEN (CASE WHEN @FOR_TURNOVER > 0 and @ROUNDEDTURNOVER/@FOR_TURNOVER * @MIN_MULTIPLIER > @MAXIMUM_LIMIT AND @MAXIMUM_LIMIT > 0                         
															THEN @MAXIMUM_LIMIT                        
		...
		Select @StampExists = @TOBECHRG_STAMPDUTY
	...
	Select #ViewSettlement.*, LeviesSTT = @STT_From_Global, LeviesServ_Tax = @Service_tax_From_Global,
	Levies_TurnoverTax = turnover_tax, Levies_SebiTax = sebiturn_tax, Levies_StampDuty = @broker_note, 
	Levies_Brokerage = @Levies_Brokerage, Brokerage_Type = isnull(@Brokerage_Type,'P') 
	into #BrokCal
	from #ViewSettlement, Msajag.dbo.ClientTaxes_New CT with(nolock) 
	where #ViewSettlement.Party_Code = CT.Party_Code
	and CT.Todate > getdate()
	...
	Select RecordDate = Convert(varchar,getdate()), Exchange = 'NSE', Party_code,
