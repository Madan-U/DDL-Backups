-- Object: PROCEDURE dbo.Angel_NRI_Data_LEC
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

--use inhouse                              
--exec Angel_nri_data_lec '03/07/2013',2,0,'BSE'                                
CREATE PROC [dbo].[Angel_NRI_Data_LEC](@SDATE  AS VARCHAR(11),          
                                   @TYPE   AS INT,          
                                   @BANKID AS INT,          
                                   @SEG    AS VARCHAR(5))          
AS          
  BEGIN          
      ...
      ELSE          
        BEGIN          
            ...
            EXEC AngelBSECM.BSEDB_AB.DBO.Bsenriheader          
                      @SDATE,          
                      @TYPE          
            ...
            EXEC AngelBSECM.BSEDB_AB.DBO.Bsenridetail          
              @SDATE,          
              @TYPE          
            ...
            IF @SEG = 'BSE'          
              BEGIN          
                 ...
                 WHERE  PARTY_CODE = @PARTYCODE    
                  ...
                  --select * from #TEMP1                              
                  INSERT INTO #TEMP1          
                  SELECT 'H' + '|' + Ltrim(Rtrim(PARTY_CODE)) + '|' 
                  + Replace(CONVERT(VARCHAR(11), SAUDA_DATE, 102), '.', '') + '|' 
