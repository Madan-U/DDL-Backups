-- Object: PROCEDURE dbo.mimansa_usp_ucc_nse_cm_Inactive_clients_data_rit_Vt
...
CREATE proc  
 ...
 [dbo].[mimansa_usp_ucc_nse_cm_Inactive_clients_data_rit_Vt]  
 (  
...
as  
--EXEC mimansa_usp_ucc_nse_cm_Inactive_clients_data_rit  @from_date='',@to_date='',@ClientCode='',@processid ='sp51yhetdv01sfbucqrwkcvk31072014_140923'  
...
begin  
 print 'DATARANGE'  
  ...
  select distinct Client_Code = ltrim(rtrim(isnull(cl_code, '')))  
...
begin  
 ...
 select  
  ...
  Client_Code=ltrim(rtrim(isnull(c1.Cl_Code,''))),  
 ...
 select   
  ...
  --c5.*  
  Client_Code=ltrim(rtrim(isnull(c5.cl_code,'')))  
...
File NAme Syntax: uci_<YYYYMMDD>.t<LastFileNumber+1> CLT_STATUS_YYYYMMDD.Tnn  
    Ex- uci_20130307.t04  
...
/*DISPLAY INVALID PAN*/  
select ltrim(rtrim(Client_Code)),Client_Name,PAN   
from #invalidpan
