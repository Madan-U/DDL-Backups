...
select * into #t from  account.dbo.ledger  where  cltcode='400002' and left(convert(varchar,vdt,121),11)>=@fvdt and left(convert(varchar,vdt,121),11)<=@tvdt                                
...
select left(vdt,11) as [Date],vno,cltcode,acname, segment='ACDL',vamt,space(10) as [Expense Code],space(100) as [Expense Code Name],space(20) as [Expense Amount], space(20) as [400002],space(20) as [410002],space(20) as [420002],space(20) as [Total],space
...
(select cltcode,vno,vamt,vtyp from  account.dbo.ledger  where cltcode like '5%' and vno in ( select vno from #t))b, #file1 a          
 where a.vno=b.vno  and a.vtyp=b.vtyp             
          
update #file1 set [Expense Code Name]=b.acname from                                
(select cltcode,vno,vamt,acname from  account.dbo.ledger  where cltcode like '5%' and vno in ( select vno from #t))b, #file1 a          
 where a.vno=b.vno             
                        
----------------    cltcode like '5%' and                            
update #file1 set [Service Tax No.]=v.st_no, [PAN No.]= v.pan_no, [Address]=v.add1+' '+v.add2+' '+v.add3+' '+v.city+' '+v.pin+' '+v.state  from                                
(select cltcode,vno from  account.dbo.ledger  where  vno in ( select vno from #t))b, #file1 a, [172.19.2.98].account123.dbo.vendormaster v          
 where a.vno=b.vno and v.segment= 'NSECM' and convert(varchar,v.cltcode)=b.cltcode          
-----------------          
                                
update #file1 set [400002]=b.vamt from                   
(select cltcode,vno,vamt from  account.dbo.ledger  where cltcode ='400002' and vno in ( select vno from #t))b,#file1 a   where a.vno=b.vno                                  
                                
                                
update #file1 set [410002]=b.vamt from                             
(select cltcode,vno,vamt from  account.dbo.ledger  where cltcode ='410002' and vno in ( select vno from #t))b,#file1 a                          
 where a.vno=b.vno                                  
                                
                                
update #file1 set [420002]=b.vamt from                                
(select cltcode,vno,vamt from  account.dbo.ledger  where cltcode ='420002' and vno in ( select vno from #t))b,#file1 a        where a.vno=b.vno                  
