...
/*added for unsetteledDrbills*/
    select cltcode,isnull(sum(VAMT),0) as UnsettledDrBill into #drbills from Account.dbo.ledger WITH(NOLOCK)  
  where cast(EDT as date)>cast(getdate() as date) and VTYP in ('79','15') and DRCR='D' group by cltcode

  update  a set a.UnsettledDrBill=isnull(b.UnsettledDrBill,0) from SEBI_PO a, (select cltcode,UnsettledDrBill from #drbills) b where a.Cltcode=b.CLTCODE
