-- Object: PROCEDURE dbo.mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_yptest
...
CREATE proc  
 ...
 [dbo].[mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_yptest]  
 (  
...
create table #TempClient (party_code varchar(20))  
 ...
 print 'SELECTED'  
  ...
  where c5.cl_code=@ClientCode
   
  
  /*Delete selected clients*/   
 
 select * from #TempClient--for test
 ...
 where   
  ...
  c2.party_code = t.party_code   
   ...
   select '#temp_Client1'
   select * from #temp_Client1 --for testing
 ...
 where   
  ...
  select '#temp_Client2'
  select * from #temp_Client2 --fro testing
 ...
 where   
  ...
  select '#temp_Client5'
  select * from #temp_Client5 --for testing
 ...
 from  
  ...
  msajag.dbo.pobank b with(nolock)  
 where  
  ...
  select '#pobank'
  select * from #pobank --for testing
 ...
 select '#temp_client_bank'
 select * from #temp_client_bank--for testing
...
CREATE TABLE #mimansa_angel_ucc_nse_cm_client_data_new_changes(  
 ...
 where  
  ...
  select '#mimansa_angel_ucc_nse_cm_client_data_new_changes'
  select * from #mimansa_angel_ucc_nse_cm_client_data_new_changes -- for testing
 --update  
 -- mimansa_angel_ucc_nse_cm_client_data  
 --set  
 -- client_name = ltrim(rtrim(isnull(addemailid, '')))  
 --from  
 -- msajag.dbo.client_details  
 --where  
 ...
 from  
  msajag.dbo.CLIENT_MASTER_UCC_DATA c1 with(nolock)  
 where  
 ...
 from  
  msajag.dbo.CLIENT_MASTER_NOMINEE_DATA c1 with(nolock)  
 where  
  client_code = party_code  
  and len(isnull(client_code, '')) > 0  
  and len(isnull(party_code, '')) > 0   
 ...
 from  
  msajag.dbo.client_contact_details c1 with(nolock)  
 where  
  Client_Code = cl_code  
 ...
 where  
  ...
  Update   
    ...
    from  
     msajag.dbo.Multicltid m with(nolock)  
       where  
...
where  
 ...
 and len(bp.panno)=10  
  ...
  select 'deleted UCC_AllSebiBannedEntities '+cast(@@rowcount as varchar(10)) 
/*FINAL RESULT*/  
...
and D.Deactive_value='p'  
   ...
   select 'deleted INTEROP ACCOUNT UCC NOT DONE '+cast(@@rowcount as varchar(10)) 
/* End  */  
