-- Object: PROCEDURE dbo.DPC_TB_NB_Opt
...
vamt money            
 ...
 from account.dbo.LEDGER with (nolock) where vdt < @aCdate and edt >=@aCdate and drcr='D'      
 group by  cltcode      
 END      
 ELSE      
 BEGIN      
 insert into #TB          
 select cltcode,'D',sum(case when drcr='D' then vamt else -vamt end) from account.dbo.LEDGER with (nolock)             
