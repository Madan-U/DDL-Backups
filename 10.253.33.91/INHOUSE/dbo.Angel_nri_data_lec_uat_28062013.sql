-- Object: PROCEDURE dbo.Angel_nri_data_lec_uat_28062013
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

--use inhouse                      
--exec Angel_nri_data_lec_uat '27/06/2013',1,0,'BSE'                        
CREATE PROC Angel_nri_data_lec_uat_28062013(@SDATE  AS VARCHAR(11),  
                                   @TYPE   AS INT,  
                                   @BANKID AS INT,  
                                   @SEG    AS VARCHAR(5))  
AS  
  BEGIN  
      ...
      --SET @SDATE ='27/06/2013' SET @TYPE ='1' SET @BANKID =0 SET @SEG='BSE'                                          
           ...
           FROM   #TT  
            ...
            IF @SEG = 'BSE'  
              BEGIN  
                  ...
                  IF @TYPE = 1  
                    BEGIN  
                        SELECT @CALCULATION = CONVERT(DECIMAL(14, 2), @TRANS_AMOUNT) + CONVERT(DECIMAL(14, 2), SERVICE_TAX) + CONVERT(DECIMAL(14, 2), EDUCESS) + CONVERT(DECIMAL(14, 2), EXCHAGE_LEVIES) + CONVERT(DECIMAL(14, 2), STT) + CONVERT(DECIMAL(14, 2
 ), STAMPDUTY) + CONVERT(DECIMAL(14, 2), MINBROKERAGE) + CONVERT(DECIMAL(14, 2), OTHER_CHARGES)  
                        FROM   #BSE_HEADER WITH (NOLOCK)  
                        WHERE  PARTY_CODE = @PARTYCODE  
                    END  
                  ELSE  
                    BEGIN  
                        SELECT @CALCULATION = CONVERT(DECIMAL(14, 2), @TRANS_AMOUNT) - ( CONVERT(DECIMAL(14, 2), SERVICE_TAX) + CONVERT(DECIMAL(14, 2), EDUCESS) + CONVERT(DECIMAL(14, 2), EXCHAGE_LEVIES) + CONVERT(DECIMAL(14, 2), STT) + CONVERT(DECIMAL(14,
 2), STAMPDUTY) + CONVERT(DECIMAL(14, 2), MINBROKERAGE) + CONVERT(DECIMAL(14, 2), OTHER_CHARGES) )  
                        FROM   #BSE_HEADER WITH (NOLOCK)  
                        WHERE  PARTY_CODE = @PARTYCODE  
                    END  
