-- Object: PROCEDURE dbo.mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_Test04March2015
...
CREATE proc
	...
	[dbo].[mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_Test04March2015]
	(
...
as
--exec [mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_Test04March2015] 'Mar 03 2015','Mar 03 2015','',''
	set nocount on
...
CREATE TABLE #mimansa_angel_ucc_nse_cm_client_data_new_changes(
	...
	where
		...
		select tt.* from 
		(select party_code,AADHAR_UID ,max(len(AADHAR_UID)) 'AADHAR_UID_Lenght' from	anand1.msajag.dbo.CLIENT_MASTER_UCC_DATA a with(nolock)
group by party_code,AADHAR_UID
having max(len(AADHAR_UID))>12 
--order by AADHAR_UID_Lenght desc
) as tt ,#mimansa_angel_ucc_nse_cm_client_data_new_changes b
where
		client_code = party_code
		and len(isnull(client_code, '')) > 0
		and len(isnull(party_code, '')) > 0	

	--select max(len(AADHAR_UID)) as 'length',* from	anand1.msajag.dbo.CLIENT_MASTER_UCC_DATA a with(nolock),#mimansa_angel_ucc_nse_cm_client_data_new_changes b
	--where
	--	b.client_code = a.party_code
	--	and len(isnull(b.client_code, '')) > 0
	--	and len(isnull(a.party_code, '')) > 0	

	--	order by max(len(AADHAR_UID)) desc

	--	select UIDNUMBER from #mimansa_angel_ucc_nse_cm_client_data_new_changes
		--return
