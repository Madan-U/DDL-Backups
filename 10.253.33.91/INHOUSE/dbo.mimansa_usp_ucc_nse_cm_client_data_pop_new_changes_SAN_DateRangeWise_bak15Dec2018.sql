-- Object: PROCEDURE dbo.mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_DateRangeWise_bak15Dec2018
...
create proc
	...
	[dbo].[mimansa_usp_ucc_nse_cm_client_data_pop_new_changes_SAN_DateRangeWise_bak15Dec2018]
	(
...
/*Initialized Parameters*/
	...
	create table #TempTable1
       ...
       set @@ToDate1=convert(datetime,@@ToDate)
		...
		from anand1.msajag.dbo.CLIENT_M_CI c1 with(nolock), anand1.msajag.dbo.client5 c5 with(nolock)
		where	c1.cl_code>=@@FromClientCode
				and c1.cl_code<=@@ToClientCode
				and UPDTIME >= @@FromDate
				and UPDTIME <= @@ToDate
				and c5.inactivefrom >@@getdate
				union
		select distinct cl_code = ltrim(rtrim(isnull(c1.client_code, '')))
		from #TempTable1 c1  with(nolock),anand1.msajag.dbo.client5 c5 with(nolock)
	...
	where 
        ...
        update #temp_Client1 
			set 
				Email='notprovided@notprovided.com'
				where Email=''
