-- Object: PROCEDURE dbo.USP_BUYBACK_LEC_BKP_04DEC2025
...
CREATE PROC [dbo].[USP_BUYBACK_LEC_BKP_04DEC2025] ( @SDATE AS VARCHAR(11) )
AS
BEGIN

SET @SDATE = CONVERT(VARCHAR(11), CONVERT(DATETIME, @SDATE, 103)) ----// 28/10/2025
--DECLARE @SDATE AS VARCHAR(11)='DEC  1 2025'   --, @CODE VARCHAR (50)='ZR0050096R'
...
[NET PURCHASE/SALE VALUE]=ROUND((C.PMARKETRATE*PTRADEDQTY+C.BUYDELIVERYCHRG+C.SELLDELIVERYCHRG+C.SERVICE_TAX+C.TURN_TAX+C.SEBI_TAX+C.BROKER_CHRG+ISNULL(C.IPFT,0)),2), 
[STT ON BUY & SALE]=C.INS_CHRG,
[EXACT AMOUNT PAID BY / TO  THE CUSTOMER (FOR PURCHASE/SALE)]= ROUND((C.PMARKETRATE*PTRADEDQTY+C.BUYDELIVERYCHRG+C.SELLDELIVERYCHRG+C.SERVICE_TAX+C.TURN_TAX+C.SEBI_TAX+C.BROKER_CHRG+ISNULL(C.IPFT,0)+C.INS_CHRG),2)-ROUND(T.TDS,2),
...
TRUNCATE TABLE DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS

INSERT INTO DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS
...
----SELECT * FROM DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS ORDER BY PARTY_CODE

DECLARE @AXISFILENAME AS VARCHAR(200)
DECLARE @BCPCOMMAND1 AS VARCHAR(1000)
IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%AXIS%')>0)                          
     BEGIN                          
      ...
      SET @BCPCOMMAND1 = 'BCP "SELECT DETAILS FROM DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%AXIS%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
...
IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%INDUSIND%')>0)                          
     BEGIN                          
      ...
      SET @BCPCOMMAND2 = 'BCP "SELECT DETAILS FROM DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%INDUSIND%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
...
IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%IDFC%')>0)                          
     BEGIN                          
      ...
      SET @BCPCOMMAND3 = 'BCP "SELECT DETAILS FROM DUSTBIN.DBO.TBL_NRI_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%IDFC%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
