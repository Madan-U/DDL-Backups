-- Object: PROCEDURE dbo.Angel_nri_data_lec_new_18may2022
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

--use 196-- inhouse                                  
--exec Angel_nri_data_lec_new '04/05/2022',1,0,'NSE'                                    
CREATE PROC Angel_nri_data_lec_new_18may2022(@SDATE  AS VARCHAR(11),              
                                   @TYPE   AS INT,              
                                   @BANKID AS INT,              
                                   @SEG    AS VARCHAR(5))              
AS              
  BEGIN              
    ...
    -- SET @SDATE ='04/05/2022' SET @TYPE ='2' SET @BANKID =0 SET @SEG='NSE'                                                      
  ...
  select @bseSETT_NO=Sett_no from #settno where Srno=@rw  
   ...
   Begin  
    ...
    from Intranet.risk.dbo.tbl_NRIClientMaster with (nolock)  where fld_clientcode = @PARTYCODE  
     ...
     left(c.Scripname,75)+','+CASE WHEN @TYPE = 1 THEN 'Buy'  WHEN @TYPE = 2 THEN 'Sell'  END +','+Ltrim(Rtrim(CONVERT(VARCHAR,c.QTY))) +','+Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(15,2), c.RATE)))) +','+cast(Ltrim(Rtrim(CONVERT(DEC(10,2),c.TRANACTIONAMT)
)) as varchar)+','+  
     Convert(Varchar,c.TRATE)+','+Ltrim(Rtrim(CONVERT(VARCHAR,CONVERT(DEC(10,2), c.BROK_PERSCRIP))))+','+  
     CASE WHEN @DIFFAMT BETWEEN -0.03 AND 0.03         
      THEN CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),( Sum(CONVERT(DECIMAL(14,2),Isnull(MINBROKERAGE,0)) + CONVERT(DECIMAL(14,2),Isnull(OTHER_CHARGES,0))) + @DIFFAMT )))          
      ELSE CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),( Sum(Isnull(MINBROKERAGE,0) + Isnull(OTHER_CHARGES,0)) )))          
       END +','+  --Other Charges end+','+  
     cast((CONVERT(DEC(15,2),c.RATE) *c.QTY) as varchar)+','+ CONVERT(VARCHAR,CONVERT(DECIMAL(14, 2),Sum(Isnull(STT, 0))))+','+  
     Convert(Varchar,CONVERT(DECIMAL(14,2),@BseTotalAmt))+','+'@@@[RBI Approval Number]'+','+@BseCliPISNo+','+@BseCliBnkAcc+','+@BsecliBankBranch+','+'Angel One Ltd'+','+CASE WHEN @TYPE = 1 THEN  Convert(varchar,(a.sauda_date +1),103)  WHEN @TYPE = 2 THEN
 Convert(varchar,(a.sauda_date+2),103)  END +','+  
     ...
     c.QTY,CONVERT(DEC(15, 2), c.Rate),c.TRATE  
