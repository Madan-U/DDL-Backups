-- Object: PROCEDURE dbo.PLBBG
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

CREATE PROC [dbo].[PLBBG]                                                                    
(                                                                      
 @STATUSID VARCHAR(25),                                                                      
...
AREA  = '',                                    
REGION  = ''                                    
 FROM TBL_PNL_DATA T WITH (NOLOCK) Left Outer join Vw_ScripnameBse V on(#TBL_PNL_DATA.BSECODE = V.BSECODE )                                                                     
 AND SCRIP_NAME = ''                                                           
...
AREA  = '',                                    
REGION  = ''                                    
 FROM TBL_PNL_DATA T WITH (NOLOCK) Left Outer join Vw_ScripnameNse V on(#TBL_PNL_DATA.Scrip_cd = V.Scrip_cd and #TBL_PNL_DATA.Series = V.Series)                                                             
 AND SCRIP_NAME = ''                                                           
 WHERE T.PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY                                                                      
 AND (SAUDA_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59'                                                        
   OR OPPDATE BETWEEN @FROMDATE AND @TODATE + ' 23:59')                                                          
