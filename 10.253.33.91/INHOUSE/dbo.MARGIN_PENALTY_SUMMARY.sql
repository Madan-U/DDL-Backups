-- Object: PROCEDURE dbo.MARGIN_PENALTY_SUMMARY
...
CREATE PROC [dbo].[MARGIN_PENALTY_SUMMARY] ( @FROMDATE VARCHAR(11) ,@TODATE VARCHAR(11) ,@CLT VARCHAR(1) = 'D')                          
AS                          
...
CASE WHEN EXCHANGE = 'NSE' AND SEGMENT = 'CAPITAL' THEN   
  ...
  CASE  WHEN (EOD_SHRT)+(MTM_SHRT) <=  (PEAK_SHRT) THEN (EOD_SHRT)+(MTM_SHRT) ELSE (PEAK_SHRT) END  
    WHEN EXCHANGE = 'NCE' AND SEGMENT = 'FUTURES' THEN   
  CASE  WHEN (EOD_SHRT)+(MTM_SHRT) <=  (PEAK_SHRT) THEN (EOD_SHRT)+(MTM_SHRT) ELSE (PEAK_SHRT) END  
...
WHERE MARGIN_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59'AND PARTY_CODE = CLTCODE                          
UNION ALL      
SELECT MARGIN_DATE,PARTY_CODE,SHORTAGE,PENALTY_PERC,PENALTY_AMT,DAYS_3_COUNT,MONTH_5_COUNT,LEDGER_POST_FLAG,SERVICE_TAX,EXCHANGE = 'NCE',SEGMENT = 'FUTURES'                          
FROM ANGELCOMMODITY.NCE.DBO.FOMARGIN_PENALTY WITH (NOLOCK)  , #CLIENTDATA                          
WHERE MARGIN_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59'AND PARTY_CODE = CLTCODE                          
UNION ALL      
...
UNION ALL                          
SELECT MARGIN_DATE,PARTY_CODE,SHORTAGE,PENALTY_PERC,PENALTY_AMT,DAYS_3_COUNT,MONTH_5_COUNT,LEDGER_POST_FLAG,SERVICE_TAX,EXCHANGE = 'NCE',SEGMENT = 'FUTURES'                          
FROM ANGELCOMMODITY.NCE.DBO.FOMARGIN_PENALTY WITH (NOLOCK)                           
WHERE MARGIN_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59'             
UNION ALL
SELECT MARGIN_DATE,PARTY_CODE,SHORTAGE,PENALTY_PERC,PENALTY_AMT,DAYS_3_COUNT,MONTH_5_COUNT,LEDGER_POST_FLAG,SERVICE_TAX,EXCHANGE = 'NSX',SEGMENT = 'FUTURES'                          
