-- Object: PROCEDURE dbo.usp_Angel_nri_Data_lec_allfiles_18may2023
-- Server: 10.253.33.91 | DB: INHOUSE
--------------------------------------------------

--use 196-- inhouse                                        
--exec usp_Angel_nri_Data_lec_allfiles '27/01/2023',0    
  
CREATE proc usp_Angel_nri_Data_lec_allfiles_18may2023  
(  
@SDATE  AS VARCHAR(11),                                    
@BANKID AS INT                    
)  
...
exec Angel_nri_data_lec_new_15may2023 @SDATE,2,@BANKID,'BSE'  
  ...
  SET @SDATE = CONVERT(VARCHAR(11), CONVERT(DATETIME, @SDATE, 103))     
   ...
   select a.nseBank+','+Convert(varchar,a.SAUDA_DATE,103)+','+A.PARTY_CODE+','+b.DPid1 +','+cast(right(b.cltdpID1,8) as varchar) +','+b.Short_Name +','+left(C.Exchange,3) +','+isnull(A.nseSett_no2,'') +','+        
     ...
     left(c.Scripname,75)+','+CASE WHEN A.Sell_Buy = 1 THEN 'Buy'  WHEN A.Sell_Buy = 2 THEN 'Sell'  END +','+Ltrim(Rtrim(CONVERT(VARCHAR,c.QTY))) +','+ Convert(Varchar,c.RATE)+','+cast((CONVERT(DEC(15,2),c.TRATE) *c.QTY) as varchar) +','+        
     ...
     CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),(c.TRANACTIONAMT+c.NseStt+c.OtherCharges)))+','+isnull(A.rbi_refno,'')+','+isnull(A.CliPISNo,'')+','+isnull(A.CliBnkAcc,'')+','+A.cliBankBranch+','+'Angel One Ltd'+','+   
  CASE WHEN A.Sell_Buy = 1 THEN  Convert(varchar,(a.sauda_date +1),103)  WHEN A.Sell_Buy = 2 THEN Convert(varchar,(a.sauda_date+2),103)  END+','+        
     ...
     c.QTY, Convert(Varchar,c.RATE),c.TRATE,c.RATE, Isnull(c.OTHERCHARGES,0) ,a.nseBank,A.rbi_refno,A.Sell_Buy,A.CliPISNo,  
  A.CliBnkAcc,A.cliBankBranch,a.angelAcc,left(C.Exchange,3),A.PARTY_CODE,a.nseSett_no2,c.NseStt, c.TRANACTIONAMT,c.OtherCharges  
