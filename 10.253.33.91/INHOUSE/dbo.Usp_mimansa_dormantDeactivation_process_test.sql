-- Object: PROCEDURE dbo.Usp_mimansa_dormantDeactivation_process_test
...
-- =============================================
CREATE PROCEDURE [dbo].[Usp_mimansa_dormantDeactivation_process_test]
...
ProcessingStatus='P'  and processid='vbmrcn4u5rnfgxrxkqa2w1dy29052024_173310' order by  RequestReceivedOn desc --ProcessID desc
	...
	OPEN cur_Deactive
		FETCH NEXT FROM cur_Deactive INTO @party_Code,@segment,@NewRemark,@updatedby
		 ...
		 BEGIN 
		   ...
		   if exists(select 1  from msajag.dbo.client_brok_details with(nolock) where cl_code=@party_Code and Deactive_value='C' )
		   begin
		 ...
		 Begin
			...
			Begin
				...
				BEGIN					
					...
					Set @InActive_Date= (Select top 1 InActive_From from msajag.dbo.client_brok_details where cl_code=@party_Code and Exchange in('MCX','NCX') and Segment='FUTURES')
					Print  'InActive_From: ' +@InActive_Date
