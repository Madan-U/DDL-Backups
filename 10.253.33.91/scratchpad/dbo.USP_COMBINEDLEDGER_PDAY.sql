-- Object: PROCEDURE dbo.USP_COMBINEDLEDGER_PDAY
...
SELECT 
    ...
    PARTY_CODE,
    MTFAMOUNT = SUM(MTF_CASHLEDGER) - SUM(BALANCE)
INTO #MTFAMOUNT
FROM 
(
    SELECT 
        PARTY_CODE,
        SUM(NETAMT) + MTFLEDBAL AS MTF_CASHLEDGER,
        0 AS BALANCE 
    FROM MTFTRADE.DBO.TBL_MTF_DATA
    WHERE SAUDA_DATE = (
            SELECT MAX(SAUDA_DATE) 
    ...
    SELECT 
        PARTY_CODE,
        0 AS MTF_CASHLEDGER,
        SUM(M_FRESHBUY) AS BALANCE
    FROM MTFTRADE.DBO.TBL_MTR_REPORTING
    WHERE SAUDA_DATE = (
            SELECT MAX(SAUDA_DATE) 
            FROM MTFTRADE.DBO.TBL_MTF_DATA WITH (NOLOCK) 
            WHERE SAUDA_DATE <= @SAUDA_DATE
