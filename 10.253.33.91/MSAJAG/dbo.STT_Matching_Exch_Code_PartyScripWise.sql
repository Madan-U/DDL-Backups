-- Object: PROCEDURE dbo.STT_Matching_Exch_Code_PartyScripWise
...
CREATE PROC
	STT_Matching_Exch_Code_PartyScripWise
	(
		@FromDate VarChar(11),
		@ToDate VarChar(11),
		@CliType VarChar(2),
...
If len(ltrim(rtrim(@FromCode))) = 0 select @FromCode = min(branch_id) from stt_clientdetail
If len(ltrim(rtrim(@ToCode))) = 0 select @ToCode = max(branch_id) from stt_clientdetail
...
select
	isnull(c.branch_id, 'zzzzzzzzzz') as party_code,
	c.scrip_cd,
	...
	c.rectype
from
	stt_clientdetail c, stt_exchg_view_party_scrip e
where
	c.branch_id = e.party_code and	/*for exch party code*/
	c.scrip_cd = e.scrip_cd and
	c.sauda_date = e.stt_date and
	c.sett_type = e.sett_type and
	c.rectype = e.rectype and
	c.sauda_date >= @FromDate + ' 00:00:00' and
	c.sauda_date <= @ToDate + ' 23:59:59' and
	c.branch_id >= @FromCode and
	c.branch_id <= @ToCode

group by
	c.branch_id, c.scrip_cd, e.tpsttdel, e.tssttdel, e.tsstttrd, e.totalstt, c.rectype
order by
	1, c.rectype desc
