-- Object: PROCEDURE dbo.aws_contract_note
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


--EXEC aws_contract_note '2025-08-08','A' --,'2020-04-15'
--ALTERED this SP under SRE-39282

CREATE  proc [dbo].[aws_contract_note] (@dt datetime,@cnflag VARCHAR(2))
as
...
truncate table nse_bulk_process
 ...
 SELECT DISTINCT PARTY_CODE, SUB_BROKER,TGST_NO,SOURCESTATE,DEAL_CLIENT_ADD INTO #DEALING FROM Contractnote_Daily

 CREATE INDEX IDX_DEL ON #DEALING(SUB_BROKER)
...
Create index #s on #TEMP12(ORDER_NO,EXCHANGE,SEGMENT,PARTY_CODE)
Create index #O  on #ORDER_WISE(CD_Order_No,EXCHANGE,SEGMENT,cd_PARTY_CODE)
...
AND T.PARTY_CODE =O.CD_Party_Code
 ...
 SELECT PARTY_CODE,TRD_DT = CONVERT(VARCHAR(11),SAUDA_DATE,103),TRADETYPE,INST_TYPE,
  ...
  SELECT * into #BSECASH FROM ANGELBSECM.BSEDB_AB.DBO.CMBILLVALAN S WITH (NOLOCK) WHERE SAUDA_DATE >=@DT  AND  SAUDA_DATE < = @DT  +' 23:59:59'
AND (PQTYTRD+SQTYTRD+PQTYDEL+SQTYDEL )<>0

SELECT * into #NSECASH  FROM CMBILLVALAN S(NOLOCK)  WHERE SAUDA_DATE >=@DT  AND SAUDA_DATE <= @DT  +' 23:59:59' ---AND PARTY_CODE IN (SELECT * FROM #CLIENT)
AND (PQTYTRD+SQTYTRD+PQTYDEL+SQTYDEL) <>0
...
SELECT * FROM #NSECASH S(NOLOCK)  WHERE SAUDA_DATE >=@DT  AND SAUDA_DATE <= @DT  +' 23:59:59' ---AND PARTY_CODE IN (SELECT * FROM #CLIENT)
AND (PQTYTRD+SQTYTRD+PQTYDEL+SQTYDEL) <>0
UNION ALL
SELECT * FROM #BSECASH S WITH (NOLOCK) WHERE SAUDA_DATE >=@DT  AND  SAUDA_DATE < = @DT  +' 23:59:59'
...
SELECT DISTINCT PARTY_CODE,CONTRACTNO_NEW,SAUDA_DATE,CLGSTNO INTO #SPARTY FROM CONTRACTNOTE_DAILY
CREATE INDEX #S ON #SPARTY(PARTY_CODE)
...
FROM #Sparty (Nolock) D,CLIENT_DETAILS (Nolock) C
...
DELETE N FROM   NSE_BULK_PROCESS N  WHERE     NOT EXISTS(
SELECT DISTINCT PARTY_CODE FROM #P P  WHERE P.PARTY_CODE=N.PARTY_CODE)
...
CREATE INDEX #B ON #BTWOB (B2C_SB)

SELECT CL_CODE,SUB_BROKER INTO #CLIENT_DETAILS
FROM CLIENT_DETAILS WHERE EXISTS (SELECT * FROM #PARTY P WHERE CL_CODE= P.PARTY_CODE)
CREATE INDEX #CL ON #CLIENT_DETAILS(CL_CODE)
