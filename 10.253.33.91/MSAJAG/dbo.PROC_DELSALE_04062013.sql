-- Object: PROCEDURE dbo.PROC_DELSALE_04062013
...
--EXEC PROC_DELSALE 'may 15 2013', 7, 500, 0, 0, '', 'gfs274', 'gfs274'                                     
                                               
CREATE PROC [DBO].[PROC_DELSALE_04062013]                                                                                           
(                                                                                  
...
INSERT INTO #LEDBAL                                                                         
EXEC ANAND.ACCOUNT_AB.DBO.PROC_DELSALE_ACC @START_DATE, @PROCESS_DATE, @FROMPARTY, @TOPARTY                                                                                                 
...
INSERT INTO #TBL_RMS_SALE_LED_DET                                                  
SELECT CLTCODE, AMT=0,HOLD_AMT=0, @NOOFDAYS, EXCESS_COLL=SUM(AMOUNT),@PROCESS_DATE,GETDATE(),0                                   
FROM #LEDBAL                                  
GROUP BY CLTCODE     
...
DELETE FROM #DEL WHERE PARTY_CODE IN (SELECT PARTY_CODE FROM #TBL_RMS_SALE_LED_DET        
            GROUP BY PARTY_CODE HAVING SUM(HOLD_AMT) = 0 AND SUM(EXCESS_COLL) = 0 )        
