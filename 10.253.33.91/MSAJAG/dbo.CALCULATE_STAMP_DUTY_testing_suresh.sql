-- Object: PROCEDURE dbo.CALCULATE_STAMP_DUTY_testing_suresh
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

create  PROC [dbo].[CALCULATE_STAMP_DUTY_testing_suresh]                    
(                          
 @SETT_TYPE VARCHAR(2),                    
...
SELECT @CLEARINGCODE = ISNULL(CLEARINGCODE,''), @BROK_SQ_OFF = ISNULL(BROK_SQ_OFF,0), @STAMP_SQ_OFF = ISNULL(STAMP_SQ_OFF,0)   
...
--BEGIN TRAN  
 ...
 BEGIN      
  ...
  SELECT * INTO #CLIENT1 FROM CLIENT1 WHERE CL_CODE IN (SELECT PARTY_CODE FROM #SETT)    
    ...
    CREATE CLUSTERED INDEX CLCOD1E ON #CLIENT1 
	(
		CL_CODE
	) 
	
    CREATE CLUSTERED INDEX CLCOD2E ON #CLIENT2 
	(
		PARTY_CODE
	) 
    --------ADDED FOR WDM GUJRAT  
  
 SELECT DISTINCT PARTY_CODE INTO #WDM FROM  COMBINE_SETTLEMENT S,CLIENT1 C  
...
AND L_STATE ='GUJARAT' AND S.PARTY_CODE =CL_CODE   
  ...
  WHERE COMBINE_SETTLEMENT.PARTY_CODE = #CLTAX.PARTY_CODE)    
      
  UPDATE COMBINE_SETTLEMENT SET BROKER_CHRG =         
  ...
  AND C2.PARTY_CODE = COMBINE_SETTLEMENT.PARTY_CODE                     
  AND L_STATE=STATE          
  AND TRADE_NO NOT LIKE '%C%'                  
  AND AUCTIONPART NOT IN ('AP', 'AR', 'FP', 'FL', 'FC', 'FP', 'FA', 'FS')                  
...
--STAMP DUTY FOR BOND-      
  ...
  UPDATE COMBINE_SETTLEMENT SET Broker_Chrg = TRADEQTY*MARKETRATE*WDMSTAMPDUTY /100                 
  FROM #STATE_MASTER, #CLIENT1 C1, #CLIENT2 C2                
  WHERE C1.CL_CODE = C2.PARTY_CODE                
  AND C2.PARTY_CODE = COMBINE_SETTLEMENT.PARTY_CODE                   
  and l_state=state        
  AND SETT_NO = @SETT_NO                 
  AND SETT_TYPE = @SETT_TYPE                
...
'YA','YB','YC','YD','YG'  
  ...
  AND NOT EXISTS (SELECT PARTY_CODE FROM #CLTAX                    
  WHERE COMBINE_SETTLEMENT.PARTY_CODE = #CLTAX.PARTY_CODE)      
  AND COMBINE_SETTLEMENT.PARTY_CODE NOT IN (SELECT * FROM #WDM)    
  AND WDMSTAMPDUTY > -1    
...
'YA','YB','YC','YD','YG'  
  ...
  AND AUCTIONPART NOT IN ('AP', 'AR', 'FP', 'FL', 'FC', 'FP', 'FA', 'FS')    
           THEN TRADEQTY*MARKETRATE ELSE 0 END)      
  INTO #STAMP                    
        FROM COMBINE_SETTLEMENT S, #CLIENT1 C1, #CLIENT2 C2                          
  WHERE SETT_NO = @SETT_NO                    
  AND SETT_TYPE = @SETT_TYPE                    
  AND C1.CL_CODE = C2.PARTY_CODE                    
  ...
  CREATE CLUSTERED INDEX STDUTY ON #STAMP 
  (	
	SETT_NO,
	SETT_TYPE,
	PARTY_CODE
...
UPDATE #STAMP SET TOBECHRG_STAMPDUTY = TOBECHRG_STAMPDUTY + TOBECHRG_STAMPDUTY_WDM    
  ...
  UPDATE COMBINE_SETTLEMENT SET BROKER_CHRG = 0                     
  WHERE  COMBINE_SETTLEMENT.SETT_NO = @SETT_NO     
  AND COMBINE_SETTLEMENT.SETT_TYPE = @SETT_TYPE              
  AND EXISTS (SELECT PARTY_CODE FROM #STAMP                    
  WHERE #STAMP.SETT_NO = COMBINE_SETTLEMENT.SETT_NO                    
  AND #STAMP.SETT_TYPE = COMBINE_SETTLEMENT.SETT_TYPE                    
  AND #STAMP.PARTY_CODE = COMBINE_SETTLEMENT.PARTY_CODE)   
                      
      
  SELECT SETT_NO, SETT_TYPE, PARTY_CODE,                     
  --TRADESCRIPSELL = MIN(RTRIM(TRADE_NO)+RTRIM(SCRIP_CD)+CONVERT(VARCHAR,SELL_BUY))                    
  SRNO = MIN(SRNO)      
  INTO #SETTSTAMP FROM COMBINE_SETTLEMENT                    
  WHERE COMBINE_SETTLEMENT.SETT_NO = @SETT_NO                    
  AND COMBINE_SETTLEMENT.SETT_TYPE = @SETT_TYPE                    
  AND EXISTS (SELECT PARTY_CODE FROM #STAMP                    
  WHERE #STAMP.SETT_NO = COMBINE_SETTLEMENT.SETT_NO                
  ...
  CREATE CLUSTERED INDEX STDUTY ON #SETTSTAMP 
  (	
	SETT_NO,
	SETT_TYPE,
	PARTY_CODE
  ...
  UPDATE COMBINE_SETTLEMENT SET BROKER_CHRG = TOBECHRG_STAMPDUTY                    
  FROM #STAMP, #SETTSTAMP                    
  WHERE COMBINE_SETTLEMENT.SETT_NO = @SETT_NO                    
  AND COMBINE_SETTLEMENT.SETT_TYPE = @SETT_TYPE                    
  AND #STAMP.SETT_NO = COMBINE_SETTLEMENT.SETT_NO                    
  AND #STAMP.SETT_TYPE = COMBINE_SETTLEMENT.SETT_TYPE                    
  AND #STAMP.PARTY_CODE = COMBINE_SETTLEMENT.PARTY_CODE                    
  AND #SETTSTAMP.SETT_NO = COMBINE_SETTLEMENT.SETT_NO                    
  AND #SETTSTAMP.SETT_TYPE = COMBINE_SETTLEMENT.SETT_TYPE                    
  AND #SETTSTAMP.PARTY_CODE = COMBINE_SETTLEMENT.PARTY_CODE              
  --AND #SETTSTAMP.TRADESCRIPSELL = RTRIM(TRADE_NO)+RTRIM(SCRIP_CD)+CONVERT(VARCHAR,SELL_BUY)      
  AND #SETTSTAMP.SRNO = COMBINE_SETTLEMENT.SRNO      
 END     
   
 EXEC PROC_SETTFLAG_END @SETT_NO, @SETT_TYPE, @SAUDA_DATE, @FROMPARTY, @TOPARTY 
 
 EXEC ANAND.BSEDB_AB.DBO.PROC_SETTFLAG_END_BSE @SETT_NO, @SETT_TYPE, @SAUDA_DATE, @FROMPARTY, @TOPARTY 
