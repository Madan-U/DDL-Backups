-- Object: PROCEDURE dbo.singlecode_OFFLINE_CLIENTMASTER
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE PROC singlecode_OFFLINE_CLIENTMASTER(                          
          @ModifyType    VARCHAR(1),                          
--          @FromPartyCode VARCHAR(10),                          
--          @ToPartyCode   VARCHAR(10),                          
          @BranchCd      VARCHAR(10),                          
          @SubBroker     VARCHAR(10),          
          @modifycode varchar(20))                           
AS                                        
/*                                        
...
SELECT   DISTINCT CD.CL_CODE,                        
      ...
      --   WHERE LEFT(SYSTEMDATE,11) = @ModifyDate                      
         WHERE           
--LEFT(active_date,11) = @ModifyDate                 
cbd.cl_code=@modifycode          
...
--      WHERE    LEFT(CBD.SYSTEMDATE,11) = @Modifydate                        
         WHERE           
--LEFT(CBD.active_date,11) = @ModifyDate                 
cbd.cl_code=@modifycode          
--and cbd.cl_code not in (select cl_code from nsecourier.dbo.hist_temp_offlinemaster)          
...
and cbd.CL_CODE not in (select cl_code from intranet.nsecourier.dbo.hist_temp_offlinemaster)            
      ORDER BY 2                        
END
