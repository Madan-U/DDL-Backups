...
---EXEC Proc_Service_Tax_Update 'N','AUG  3 2020','M72352','M72352'
...
AND TURN_TAX > 0      
 ...
 ----SELECT 1,SUM(turn_tax) FROM SETTLEMENT WHERE Party_Code='M72352'AND Sauda_date>='2020-08-03' AND Scrip_Cd='ADANIGAS'
EXEC PROC_TURNTAX_EXCEPTION @SETT_TYPE, @SAUDA_DATE  
            
           ------SELECT 2,SUM(turn_tax) FROM SETTLEMENT WHERE Party_Code='M72352'AND Sauda_date>='2020-08-03' AND Scrip_Cd='ADANIGAS'
...
------ KFC CHANGES
-- Optimize Start--
Select Distinct Party_code   into #party from Settlement (Nolock)
where  Sauda_Date Like @Sauda_Date + '%' and Sett_type=@SETT_TYPE

Select T.* Into #TBL_CLIENT_GST_DATA from TBL_CLIENT_GST_DATA T (NolOCK)
inner Join 
#party P On T.party_Code =P.party_code 
where @Sauda_Date Between EFF_From_date and eff_to_date

Create Clustered index #s on #TBL_CLIENT_GST_DATA(Party_Code,EFF_From_date,eff_to_date)
...
-- Optimize End --
	...
	FROM Globals, #TBL_CLIENT_GST_DATA C (NOLOCK), TBL_STATE_GST_DATA G (NOLOCK), TBL_GST_LOCATION L (NOLOCK)
	...
	Else 0  
	End)) * (Globals.Service_Tax+CESSTAX)/100    
	From Globals, #TBL_CLIENT_GST_DATA C (NOLOCK), TBL_STATE_GST_DATA G (NOLOCK), TBL_GST_LOCATION L (NOLOCK), CLIENT2 C2 (NOLOCK)
...
DROP TABLE #PARTY
DROP TABLE #TBL_CLIENT_GST_DATA
