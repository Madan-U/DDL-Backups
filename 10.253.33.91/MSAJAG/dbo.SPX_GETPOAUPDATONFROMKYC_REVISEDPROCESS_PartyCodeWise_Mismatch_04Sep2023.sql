-- Object: PROCEDURE dbo.SPX_GETPOAUPDATONFROMKYC_REVISEDPROCESS_PartyCodeWise_Mismatch_04Sep2023
...
Create PROC [dbo].[SPX_GETPOAUPDATONFROMKYC_REVISEDPROCESS_PartyCodeWise_Mismatch_04Sep2023]                      
@UserId varchar(15)             
AS                      
        
SELECT DISTINCT * INTO #SYNERGY_DATA_FINAL FROM [196.1.115.167].KYC.DBO.TBL_DP_MISMATCH With(Nolock)          
...
[172.31.16.108].Dmat.citrus_usr.Tbl_Client_Master a WITH(NOLOCK)
--[172.31.16.94].Dmat.citrus_usr.Tbl_Client_Master a WITH(NOLOCK)
...
--**********************************CODE START FOR CLIENT4 PROCESS **********************------------------------                    
 ...
 from client4 a where nise_party_code=Party_code        
 and DefDp='1' and Depository='CDSL' and isnull(nsdlflag,'')<>'y' and BankID NOT IN ('12033200','12033201')        
   --AND ISNULL(NSE,'') NOT IN ('NO','N')       
 ...
 and BankID in ( '12033200', '12033201')        
  --AND ISNULL(NSE,'') NOT IN ('NO','N')       
      
        
 update #SYNERGY_DATA_FINAL set CDSlStatusFlag='y'        
 from client4 a where nise_party_code=Party_code        
 and DefDp='1' and Depository='CDSL' and isnull(nsdlflag,'')<>'y' AND ISNULL(NonAngelCDSLFlag,'')<>'y'  and BankID in ('12033200', '12033201')      
...
DELETE FROM client4 WHERE depositOry='CDSL' AND BankID in ('12033200', '12033201') AND Party_code IN      
 (      
...
) AND DEPOSITORY='CDSL' AND BankID in ('12033200', '12033201') AND DefDp='0'      
...
--AND ISNULL(NSE,'N')='Y'       
AND DPID in ('12033200', '12033201') AND A.DpType='CDSL'          
...
AND #SYNERGY_DATA_FINAL.CLIENT_CODE=LTRIM(RTRIM(CLTDPNO))                    
--AND ISNULL(NSE,'N')='Y'       
AND DPID in ('12033200', '12033201') AND STATUS='Active' AND A.DpType='CDSL'                 
...
--AND ISNULL(NSE,'N')='Y'       
AND DPID in ('12033200', '12033201') AND DEF='0' AND ISNULL(MultiFlag,'')='Y' AND A.DpType='CDSL'       
