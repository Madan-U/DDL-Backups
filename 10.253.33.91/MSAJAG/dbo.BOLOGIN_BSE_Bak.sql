-- Object: PROCEDURE dbo.BOLOGIN_BSE_Bak
...
CREATE PROC [dbo].[BOLOGIN_BSE_Bak]               
...
------ LAST LOGIN DETAILS                 
 ...
 BEGIN                 
  IF @EXCH ='NSEFO'                
    ...
    SELECT UPPER(FLDUSERNAME) AS LOGIN_ID,UPPER(FLDFIRSTNAME) AS USER_NAME,                
     MAX(ADDDT) AS LAST_LOGIN                
     FROM   [196.1.115.200].NSEFO.dbo.V2_REPORT_ACCESS_LOG U, [196.1.115.200].NSEFO.dbo.TBLPRADNYAUSERS P WHERE                 
      ADDDT  >=@FROMDATE AND ADDDT <=@TODATE + ' 23:59'                
   AND  USERNAME=FLDUSERNAME                   
   GROUP BY FLDUSERNAME,FLDFIRSTNAME                
 END                   
...
IF @RPT ='U'                
 BEGIN                 
  ...
  SELECT A.FLDUSERNAME,B.FLDCATEGORYNAME,A.ACTIONDATE from    
  [196.1.115.200].NSECURFO.DBO.tblpradnyausers_Jrnl A WITH (NOLOCK)  LEFT OUTER JOIN [196.1.115.200].NSECURFO.DBO.tblcategory  B WITH (NOLOCK)    
  ON A.fldcategory = B.FLDCATEGORYCODE    
  WHERE A.ACTIONFLAG='I' AND  A.ActionDate >=@FROMDATE AND A.ActionDate <=@TODATE + ' 23:59' AND A.fldcategory NOT IN ('22','272')    
  ORDER BY A.fldusername              
 END                   
...
IF @RPT ='D'                
 BEGIN                 
  IF @EXCH ='NSECD'                
   ...
   [196.1.115.200].NSECURFO.DBO.TBLUSERCONTROLMASTER U, [196.1.115.200].NSECURFO.DBO.TBLPRADNYAUSERS P,      
   [196.1.115.132].risk.dbo.emp_info G       
   WHERE               
   G.SEPARATIONDATE  >=@FROMDATE AND G.SEPARATIONDATE <=@TODATE + ' 23:59' and      
   D.login_inactive_date  >=@FROMDATE AND D.login_inactive_date <=@TODATE + ' 23:59' and      
  ...
  IF @EXCH ='NSE'                
    ...
    SELECT UPPER(FLDUSERNAME) AS LOGIN_ID,UPPER(FLDFIRSTNAME) AS USER_NAME,                
     ...
     FROM   V2_REPORT_ACCESS_LOG U, TBLPRADNYAUSERS P WHERE                 
      ADDDT  >=@FROMDATE AND ADDDT <=@TODATE + ' 23:59'                
   AND  USERNAME=FLDUSERNAME                   
   GROUP BY FLDUSERNAME,FLDFIRSTNAME               END                   
 END                
...
--------- CATEGORY CHANGE                
 ...
 BEGIN                 
  IF @EXCH ='NSECD'                
    ...
    SELECT UPPER(FLDUSERNAME) AS LOGIN_ID,UPPER(FLDFIRSTNAME) AS USER_NAME,                
     MAX(ADDDT) AS LAST_LOGIN                
     FROM   [196.1.115.200].NSECURFO.dbo.V2_REPORT_ACCESS_LOG U, [196.1.115.200].NSECURFO.dbo.TBLPRADNYAUSERS P WHERE                 
      ADDDT  >=@FROMDATE AND ADDDT <=@TODATE + ' 23:59'                
   AND  USERNAME=FLDUSERNAME                   
   GROUP BY FLDUSERNAME,FLDFIRSTNAME                
 END                   
