-- Object: PROCEDURE dbo.angel_shift_clientDPDemat
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE procedure [dbo].[angel_shift_clientDPDemat]                  
as                                      
...
-----------------------------Anand1 server------------            
Select * into #MultiCltIdOld_NK from MSAJAG.dbo.MultiCltId where Party_Code in (Select OLD_KYC_CODE from #Shift)                        
        
Insert into MSAJAG.dbo.Client4                     
Select S.NEW_KYC_CODE,S.NEW_KYC_CODE,Instru,BankId,CltDpId,Depository,DefDp from #Shift S ,MSAJAG.dbo.Client4 C4 where S.OLD_KYC_CODE = C4.Party_Code  
...
Insert into MSAJAG.dbo.MultiCltId                    
...
Update MSAJAG.dbo.MultiCltId Set Def = 0 where Party_Code in (Select Party_Code from #MultiCltIdOld_NK)                    
        
--------------------------Delete-----------        
delete from MSAJAG.dbo.Client4 where cl_code in        
(select old_Kyc_Code from #Shift) and depository in ('CDSL','NSDL')        
        
delete from MSAJAG.dbo.MultiCltId where party_code in        
