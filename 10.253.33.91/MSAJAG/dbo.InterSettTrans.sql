-- Object: PROCEDURE dbo.InterSettTrans
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE Proc InterSettTrans ( @Sett_no Varchar(7),@Sett_Type Varchar(2),@RefNo Int) aS
Declare 
 @@Scrip_Cd Varchar(12),
 @@series Varchar(3),
...
begin 
 ...
 select @@CQty = 0 
 Set @@Cfwd = Cursor for 
 select IsNull(Sum(Qty),0) from DelTrans where delivered = '0' And SETT_NO = @Sett_No
 ...
 begin
	Set @@Cfwd = Cursor for 	
	Select Qty,FromNo,ToNo,CertNo,FolioNo,Sno,TrType,TCode from DelTrans where delivered = '0' 
