-- Object: PROCEDURE dbo.BBGC_CollateralCalculationSp
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



CREATE PROCEDURE BBGC_CollateralCalculationSp(@Exchange Varchar(3), @Segment Varchar(20),@FromParty Varchar(10), @ToParty Varchar(10),@EffDate Varchar(11),@Status int OUTPUT)
As
Declare @@Cur  cursor,
@@Get Cursor,
@@GetFdBg Cursor,
...
Set @@Cur = Cursor For	
    Select distinct F.party_code, Cl_type = Cl_type  from Fixeddepositmst F, ClientMaster C   where F.Party_code >= @FromParty 
    and F.Party_Code <= @ToParty and Exchange = @Exchange and Segment =@Segment and F.Party_code = C.Party_code
Union
    Select distinct F.party_code,Cl_type = Cl_type  from BankGuaranteemst F , ClientMaster C  
...
Union
    Select distinct F.party_code ,Cl_type = CL_Type  from C_SecuritiesMst F, ClientMaster C  where F.party_code <> 'Broker' and filler11 = 'Party' and F.Party_code >= @FromParty and F.Party_Code <= @ToParty and Exchange = @Exchange and Segment =@Segment and F.Party_code = C.Party_code
Union 
    Select distinct  F.Party_code ,Cl_type = Cl_type  from C_MarginLedger F, ClientMaster C 
...
Begin
	...
	/* set @@Get = Cursor for
		Select isnull(Cl_type,'CLI') Cl_type from msajag.dbo.clientmaster where party_code =  @@Party_Code
	Open @@Get
...
Select @@Party_code,@@Cl_Type				
	...
	Delete from Msajag.Dbo.CollateralDetails Where Exchange = @Exchange and Segment = @Segment and Party_code = @@Party_Code and EffDate like @EffDate + '%' and Coll_Type = 'MARGIN'

	/*Get The Margin amount as a Cash*/		
	Set @@Get = Cursor for
		Select Bal = Isnull((Sum(Damt) - Sum(Camt)),0) From Msajag.Dbo.C_MarginLedger Where Party_code = @@Party_code
		Group By Party_code
	Open @@Get
	...
	Begin		
		Set @Status = 1
		Delete From Msajag.dbo.Collateral Where Party_code = @@Party_code and Exchange = @Exchange and Segment = @Segment and Trans_Date Like @EffDate + '%'
