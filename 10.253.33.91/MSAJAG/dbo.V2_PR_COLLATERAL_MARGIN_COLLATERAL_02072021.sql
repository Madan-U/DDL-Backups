-- Object: PROCEDURE dbo.V2_PR_COLLATERAL_MARGIN_COLLATERAL_02072021
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

--BEGIN TRAN    
--   EXEC V2_PR_COLLATERAL_MARGIN_COLLATERAL 'SEP 29 2020'       
--   ROLLBACK           

CREATE PROCEDURE [dbo].[V2_PR_COLLATERAL_MARGIN_COLLATERAL_02072021]
(@EFFDATE VARCHAR(11), 
...
=====================================================================================================================*/
     ...
     DECLARE @MRGCUR CURSOR, @STRSQL VARCHAR(2000), @ACCOUNTSRVR VARCHAR(100), @ACCOUNTDB VARCHAR(100), @EXCHANGE VARCHAR(3), @SEGMENT VARCHAR(7), @ILOOP INT;
     ...
     INSERT INTO #TBL_COLLATERAL_MARGIN_ALL
            SELECT EXCHANGE, 
                   ...
                   MARGIN_CALL = 0, 
                   LEDGER_AMT = 0
            FROM COLLATERALDETAILS A(NOLOCK)
            WHERE EFFDATE BETWEEN @EFFDATE AND @EFFDATE + ' 23:59:59'
                  AND EXISTS
            (
                SELECT EXCHANGE
                FROM V2_TBL_EXCHANGE_MARGIN_REPORT B
     ...
     IF LEFT(GETDATE(), 11) = @EFFDATE
         BEGIN
             ...
             SELECT DISTINCT 
                    C.*
             INTO #CLIENT_NEWPOA
             FROM ANGELFO.NSEFO.DBO.CLIENT_NEWPOA C;
             DELETE FROM #CLIENT_NEWPOA
             WHERE CLIENT_CODE IN
             (
                 SELECT PARTY_CODE
                 FROM [172.31.16.57].NBFC.DBO.TBLCLIENTMARGIN
...
/*  
 IF (@EFFDATE <='Aug  1 2020')  
  ...
  begin         
   ...
   FROM [172.31.16.94].DMAT.CITRUS_USR.HOLDINGALLFORVIEW, [172.31.16.94].DMAT.CITRUS_USR.DP_ACCT_MSTR, #CLIENT_NEWPOA C        
   ...
   AND FLOOR(DPHMCD_FREE_QTY) <> 0        
             ...
             (
                 ...
                 BEGIN
                     ...
                     SELECT DISTINCT TOP 7 TRDATE
                     INTO #TDATE
                     FROM DELCDSLBALANCE_7DAYS
                     ORDER BY 1 DESC;
                     SELECT @TDATE = MIN(TRDATE)
                     FROM #TDATE;
                     DELETE DELCDSLBALANCE_7DAYS
                     WHERE TRDATE < @TDATE;
             END;
                 ELSE
                 BEGIN
                     DELETE FROM DELCDSLBALANCE_7DAYS
                     WHERE TRDATE = @EFFDATE;
             END;
             IF
             (
                 ...
                 BEGIN
                     SELECT *
                     INTO #VW_MULTIISIN_MF
                     FROM VW_MULTIISIN_MF;
                     CREATE INDEX #ISIN ON #VW_MULTIISIN_MF(ISIN, VALID);
                     UPDATE DELCDSLBALANCE
                       SET 
                           SCRIP_CD = LEFT(M.SCRIP_CD, 12), 
                           SERIES = M.SERIES
                     FROM #VW_MULTIISIN_MF M
                     WHERE M.ISIN = DELCDSLBALANCE.ISIN
                           AND VALID = 1;
...
*/
     ...
     UPDATE DELCDSLBALANCE_7DAYS
       SET 
           PARTY_CODE = M.PARTY_CODE
     FROM MULTICLTID M
     WHERE TRDATE = @EFFDATE
           AND M.DPID = DELCDSLBALANCE_7DAYS.DPID
           AND M.CltDpNo = CLTDPID
           AND DEF = 1
           AND DELCDSLBALANCE_7DAYS.PARTY_CODE = '';
...
*/
     ...
     IF CONVERT(DATETIME, @EFFDATE) >= CONVERT(DATETIME, 'SEP 29 2020')
         BEGIN
             EXEC PR_GET_CASH_HOLDING 
                  @EFFDATE;
             INSERT INTO #TBL_COLLATERAL_MARGIN_ALL
                    SELECT EXCHANGE = (CASE
                                           WHEN EXCHANGE = 'NNP'
                                                OR EXCHANGE = 'BNP'
                                           THEN 'NSE'
                                           ELSE EXCHANGE
     ...
     WHERE M.ISIN = #TBL_COLLATERAL_MARGIN_ALL.ISIN
           AND VALID = 1;
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           SCRIP_CD = LEFT(M.SCRIP_CD, 12), 
     ...
     SELECT @PRV_DATE = MAX(Start_date)
     FROM SETT_MST
     WHERE Start_date < @EFFDATE
           AND SETT_TYPE = 'N';
     ...
     WHERE LEFT(CLOSING.SYSDATE, 11) = @PRV_DATE
     ...
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           #TBL_COLLATERAL_MARGIN_ALL.MRG_CL_RATE = C.CL_RATE
     FROM
     (
         SELECT CL_RATE, 
                M.ISIN, 
                C.SCRIP_CD, 
                C.SERIES
         FROM CLOSING C(NOLOCK), 
              MULTIISIN M(NOLOCK)
         WHERE SYSDATE = @PRV_DATE
               AND M.VALID = 1
               AND C.SCRIP_CD = M.SCRIP_CD
               AND C.SERIES IN('BE', 'EQ', 'BZ')
     ...
     (
         SELECT CL_RATE, 
                ...
                C.SCRIP_CD, 
                C.SERIES
         FROM ANAND.BSEDB_AB.DBO.CLOSING C(NOLOCK), 
              ANAND.BSEDB_AB.DBO.MULTIISIN M(NOLOCK)
         WHERE SYSDATE = @PRV_DATE
               AND M.VALID = 1
               AND C.SCRIP_CD = M.SCRIP_CD
               AND C.SERIES = M.SERIES
     ...
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           MRG_CL_RATE = CLOSING.CL_RATE
     FROM CLOSING CLOSING(NOLOCK)
     WHERE LEFT(CLOSING.SYSDATE, 11) =
     (
         SELECT LEFT(MAX(SYSDATE), 11)
         FROM CLOSING C(NOLOCK)
     ...
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           #TBL_COLLATERAL_MARGIN_ALL.MRG_CL_RATE = C.MRG_CL_RATE
     FROM #CLDATA C
     WHERE C.ISIN = #TBL_COLLATERAL_MARGIN_ALL.ISIN
           AND #TBL_COLLATERAL_MARGIN_ALL.MRG_CL_RATE = 0;               
     --AND #TBL_COLLATERAL_MARGIN_ALL.EXCHANGE <> 'POA'               
...
*/
     ...
     WHERE EXCHANGE = 'POA' --AND SEGMENT ='POA'              
           AND MRG_CL_RATE = 0;
     CREATE CLUSTERED INDEX IDXISIN ON #ISIN(ISIN);
     DELETE DELCDSLBALANCE_7DAYS
     WHERE TRDATE = @EFFDATE
           AND EXISTS
     (
     ...
     DELETE #TBL_COLLATERAL_MARGIN_ALL
     WHERE EXCHANGE = 'POA' --AND SEGMENT ='POA'              
           AND MRG_CL_RATE = 0;
...
*/
     ...
     WHERE C.SCRIP_CD = #TBL_COLLATERAL_MARGIN_ALL.SCRIP_CD
           AND C.SERIES = #TBL_COLLATERAL_MARGIN_ALL.SERIES
           AND C.STATUS = '3';
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
...
*/
     ...
     CREATE CLUSTERED INDEX IDXSERIES ON #VAR
     (SERIES
     );
     CREATE NONCLUSTERED INDEX IDXISIN ON #VAR(ISIN);
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           MRG_VAR_MARGIN = VARMARGINRATE
     FROM
     (
         SELECT SCRIP_CD, 
     ...
     WHERE #TBL_COLLATERAL_MARGIN_ALL.ISIN = A.ISIN
           AND (MRG_VAR_MARGIN = 0
                OR MRG_VAR_MARGIN = 100)
           AND #TBL_COLLATERAL_MARGIN_ALL.COLL_TYPE = 'SEC';
...
*/
     ...
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           MRG_VAR_MARGIN = SECURITYHAIRCUT.HAIRCUT
     ...
     (
         ...
         WHERE EFFDATE <= @PRV_DATE + ' 23:59'
           ...
           AND (MRG_VAR_MARGIN = 0
                OR MRG_VAR_MARGIN = 100)
           AND #TBL_COLLATERAL_MARGIN_ALL.SERIES = 'MF'
           AND (#TBL_COLLATERAL_MARGIN_ALL.MRG_VAR_MARGIN = 0
                OR #TBL_COLLATERAL_MARGIN_ALL.MRG_VAR_MARGIN = 100);  --- CHANGED 17 JUL 2014     
     ...
     (
         SELECT COUNT(1)
         FROM MSAJAG.DBO.SCRIP_UNAPPROVED WITH(NOLOCK)
     ) > 0
         BEGIN
             UPDATE #TBL_COLLATERAL_MARGIN_ALL
               SET 
             ...
             (
                 SELECT ISIN
                 FROM MSAJAG.DBO.SCRIP_UNAPPROVED S(NOLOCK)
     ...
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           ...
           MRG_VAR_MARGIN = 20
     WHERE EXCHANGE = 'CUS'
           AND MRG_VAR_MARGIN <= 20; ---ADDED ON SEP 29 2020  

     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           RMS_CL_RATE = MRG_CL_RATE, 
           RMS_HAIRCUT = MRG_VAR_MARGIN;
     DECLARE @HAIRCUTDATE DATETIME;
     SELECT @HAIRCUTDATE = MAX(TRADE_DATE)
     FROM TBL_SCRIP_HAIRCUT A(NOLOCK)
     ...
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           RMS_HAIRCUT = TBL_SCRIP_HAIRCUT.HAIRCUT
     ...
     (
         SELECT COUNT(1)
         FROM SCRIP_UNAPPROVED WITH(NOLOCK)
     ) > 0
         BEGIN
             UPDATE #TBL_COLLATERAL_MARGIN_ALL
               SET 
             ...
             (
                 SELECT ISIN
                 FROM SCRIP_UNAPPROVED S WITH(NOLOCK)
                 WHERE S.ISIN = #TBL_COLLATERAL_MARGIN_ALL.ISIN
             );
     END;
     ...
     IF @EFFDATE >= '2019-07-31'
         BEGIN
             ...
             (
                 SELECT ISIN
                 FROM ANGELFO.NSEFO.DBO.SCRIP_TYPE S WITH(NOLOCK)
                 WHERE S.ISIN = #TBL_COLLATERAL_MARGIN_ALL.ISIN
                       AND STATUS = 'POOR'
             )
     ...
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           #TBL_COLLATERAL_MARGIN_ALL.QTY = #TBL_COLLATERAL_MARGIN_ALL.QTY - S.QTY
     FROM #MTFMARKING S
     ...
     UPDATE #TBL_COLLATERAL_MARGIN_ALL
       SET 
           FINALAMOUNT = CASE
                             WHEN COLL_TYPE = 'SEC'
                             THEN((QTY) * MRG_CL_RATE) - ((QTY) * MRG_CL_RATE * MRG_VAR_MARGIN / 100)
                             ELSE(AMOUNT)
                         END, 
           RMS_FINALAMOUNT = CASE
                                 WHEN COLL_TYPE = 'SEC'
                                 THEN((QTY) * RMS_CL_RATE) - ((QTY) * RMS_CL_RATE * RMS_HAIRCUT / 100)
                                 ELSE(AMOUNT)
                             END, 
           ...
           AMount = CASE
                        WHEN COLL_TYPE = 'SEC'
                        THEN((QTY) * MRG_CL_RATE)
                        ELSE(AMOUNT)
                    END, 
           RMS_AMOUNT = CASE
                            WHEN COLL_TYPE = 'SEC'
                            THEN((QTY) * RMS_CL_RATE)
                            ELSE(AMOUNT)
                        END;
     ...
     INSERT INTO V2_TBL_COLLATERAL_MARGIN
            SELECT *, 
                   ...
                   SEGMENT_ORG = SEGMENT
            FROM #TBL_COLLATERAL_MARGIN_ALL;
     UPDATE V2_TBL_COLLATERAL_MARGIN
       SET 
           EXCHANGE = CASE
                          WHEN EXCHANGE = 'CUS'
                          THEN 'NSE'
                      ...
                      END, 
           BANK_CODE = 'CUSA'
     WHERE EXCHANGE = 'CUS';   --ADDED ON SEP 29 2020                 
     DELETE FROM V2_TBL_COLLATERAL_MARGIN_COMBINE
