-- Object: PROCEDURE dbo.SSRS_CLIENT_MCX
...
--------------------------------------------------
 ...
 CREATE PROCEDURE [dbo].[SSRS_CLIENT_MCX]  
 AS   
 BEGIN  
   
 EXEC ABVSINSURANCEBO.msdb.DBO.SP_START_JOB 'KYC_DUMP_DATA_MCX'     
 WAITFOR DELAY '00:0:45';    
  
TRUNCATE TABLE SSRS_CLIENT_DUMP_DATA  
SELECT DISTINCT CL_cODE INTO #MCX FROM CLIENT_BROK_DETAILS WHERE EXCHANGE ='MCX'  
  
  
SELECT * INTO #CLI FROM #MCX WHERE CL_cODE NOT IN (SELECT * FROM SSIS_CLIENT_DUMP)  
...
SELECT CL_cODE,EXCHANGE,SEGMENT,Active_Date,InActive_From,Deactive_Remarks,Deactive_value   
 FROM CLIENT_BROK_DETAILS WHERE CL_CODE IN (SELECT * FROM #CLI)AND EXCHANGE='MCX'  
   
 ----FILE GENERATION  
  
DECLARE @FILE VARCHAR(MAX),@PATH VARCHAR(MAX) = 'J:\BackOffice\test\AUTOMATION\MCDX_DATA\'                           
SET @FILE = @PATH + 'MCDX_DATA_UCC' +'_'+ CONVERT(VARCHAR(11),GETDATE() , 112) + '.csv' --Folder Name         
