-- Object: PROCEDURE dbo.POP_COMMON_CONTRACTDATA_BKP_08102024_SRE_30366
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE PROCEDURE [dbo].[POP_COMMON_CONTRACTDATA_BKP_08102024_SRE_30366]   
(  
@FROMDATE VARCHAR(11),  
@USERNAME VARCHAR(20) = '',  
...
*/ 
/*
****IPFT changes doen under SRE 30366****
*/
...
IF (CONVERT(DATETIME,@FROMDATE,109) > 'JUN  6 2023')
	BEGIN 
	...
	SELECT	PARTY_CODE,SETT_NO,SETT_TYPE,CONTRACTNO,SELL_BUY,IPFT_CHRG=SUM(IPFT_CHRG)
	INTO	#CASH_IPFT FROM TBL_ADDITIONAL_TAX_DATA  (NOLOCK) 
	WHERE	SAUDA_DATE BETWEEN @FROMDATE AND @FROMDATE + ' 23:59:59' AND TRDTYPE='N'
	GROUP BY
			PARTY_CODE,SETT_NO,SETT_TYPE,CONTRACTNO,SELL_BUY
	...
	UPDATE	#CONTRACT_CASH SET IPFT_CHRG = ISNULL(S.IPFT_CHRG,0) 
	FROM	#CASH_IPFT S (NOLOCK)
	WHERE	#CONTRACT_CASH.PARTY_CODE = S.PARTY_CODE
			AND #CONTRACT_CASH.SETT_NO = S.SETT_NO
			AND #CONTRACT_CASH.SETT_TYPE = S.SETT_TYPE
			AND #CONTRACT_CASH.CONTRACTNO = S.CONTRACTNO
			AND #CONTRACT_CASH.SELL_BUY = S.SELL_BUY
	...
	SELECT	PARTY_CODE,CONTRACTNO,SELL_BUY,IPFT_CHRG=SUM(IPFT_CHRG) 
	INTO	#FNO_IPFT 
	...
	UPDATE	#CONTRACT_1 SET IPFT_CHRG = ISNULL(S.IPFT_CHRG,0)
	FROM	#FNO_IPFT S (NOLOCK)
	WHERE	#CONTRACT_1.PARTY_CODE = S.PARTY_CODE
			AND #CONTRACT_1.CONTRACTNO = S.CONTRACTNO
			AND #CONTRACT_1.SELL_BUY = S.SELL_BUY
	...
	UPDATE	#CONTRACT_CURFO SET IPFT_CHRG = ISNULL(S.IPFT_CHRG,0)
