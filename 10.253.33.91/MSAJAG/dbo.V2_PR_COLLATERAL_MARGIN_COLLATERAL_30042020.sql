-- Object: PROCEDURE dbo.V2_PR_COLLATERAL_MARGIN_COLLATERAL_30042020
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

  --BEGIN TRAN
  --   EXEC V2_PR_COLLATERAL_MARGIN_COLLATERAL 'APR 23 2020'   
     
     
  --   ROLLBACK       
          
 CREATE PROCEDURE [dbo].[V2_PR_COLLATERAL_MARGIN_COLLATERAL]          
(                
  @EFFDATE VARCHAR(11),                
...
/*-------------------------------------------------- INCLUDING POA BALANCE --STARTS HERE ------------------------------------------------*/          
 ...
 WHERE CLIENT_CODE IN (SELECT PARTY_CODE FROM [172.31.16.57].NBFC.DBO.TBLCLIENTMARGIN   
...
---- SELECT * FROM #VAR WHERE ISIN='INF209K01BR9'
...
CREATE NONCLUSTERED INDEX IDXISIN ON #VAR (ISIN)                
 ...
 AND #TBL_COLLATERAL_MARGIN_ALL.COLL_TYPE ='SEC'  
  ...
  --RETURN            
  SELECT * FROM #TBL_COLLATERAL_MARGIN_ALL WHERE PARTY_CODE='N61543'AND SERIES='MF'
      
  
 UPDATE #TBL_COLLATERAL_MARGIN_ALL                
 SET     MRG_VAR_MARGIN = 15          
 WHERE  SERIES = 'MF'    AND  MRG_VAR_MARGIN <>100
 ...
 FROM                
  ...
  SELECT ISIN,                
  VARMARGINRATE=MAX(AppVar) -- MAX(VARMARGINRATE)          
  FROM #VAR    WHERE SERIES='MF' and  (AppVar)>15          
  GROUP BY ISIN              
 ) A                
 WHERE #TBL_COLLATERAL_MARGIN_ALL.ISIN = A.ISIN                
 AND (#TBL_COLLATERAL_MARGIN_ALL.MRG_VAR_MARGIN <>100)          
 AND #TBL_COLLATERAL_MARGIN_ALL.COLL_TYPE ='SEC' 
 AND #TBL_COLLATERAL_MARGIN_ALL.SERIES ='MF'
-------     
