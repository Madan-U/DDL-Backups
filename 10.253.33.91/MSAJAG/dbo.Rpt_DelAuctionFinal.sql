-- Object: PROCEDURE dbo.Rpt_DelAuctionFinal
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE Proc Rpt_DelAuctionFinal(      
...
Penalty = Round(Sum(Case When AuctionPart Like 'F%' Then A.Other_Chrg Else 0 End),2),AuctionPart,PenaltyPer=AuctionChrg,C1.Branch_Cd,C1.sub_broker      
...
Group by S.Sett_No, S.Sett_Type,Sec_Payin,Sec_Payout,C1.Branch_Cd,C1.sub_broker,A.Party_Code,C1.Long_Name,C1.L_Address1,C1.L_Address2,      
C1.L_Address3,L_City,L_State,L_Zip,BillNo,A.Scrip_CD,Sell_buy ,AuctionPart,AuctionChrg,Convert(Varchar,Sauda_Date,103)      
UNION ALL
Select S.Sett_No, S.Sett_Type,PayIn=Convert(Varchar,Sec_Payin,103),      
PayOut=Convert(Varchar,Sec_Payout,103),Sauda_Date=Convert(Varchar,Sauda_Date,103),      
A.Party_Code,C1.Long_Name,C1.L_Address1,C1.L_Address2,C1.L_Address3,L_City,      
...
Sabi_Tax = Sum(Case When Sebi_Turn_tax = 1 Then sebi_tax Else 0 End),      
Broker_Chrg = Sum(Case When BrokerNote = 1 Then Broker_chrg Else 0 End),Remark = @Remark,   
Penalty = Round(Sum(Case When AuctionPart Like 'F%' Then A.Other_Chrg Else 0 End),2),
AuctionPart,PenaltyPer=AuctionChrg,C1.Branch_Cd,C1.sub_broker      
