-- Object: PROCEDURE dbo.rpt_branchDelGive
...
/****** Object:  Stored Procedure dbo.rpt_branchDelGive    Script Date: 05/08/2002 12:35:08 PM ******/
...
CREATE PROCEDURE rpt_branchDelGive
...
begin
	 ...
	 GiveNse = (case when inout = 'O' then Sum(d.Qty) else 0 end ),
	 GivenNse =/* (case when inout='O' then Sum(d.Qty)else 0 end),*/
	 isnull(( select Sum(qty) from DelTrans where sett_no = d.sett_no and sett_type = d.sett_type and 
		...
		and DrCr = 'D' And Filler2 = 1 ),0), inout,
