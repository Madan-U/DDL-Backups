-- Object: PROCEDURE dbo.PR_GET_CASH_HOLDING_BKUP_11APR2023
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



CREATE PROC [dbo].[PR_GET_CASH_HOLDING_BKUP_11APR2023]      
(    
@EFFDATE Varchar(11),    
@INTMODE INT = 0      
) AS      
...
SELECT       
 ...
 ISIN = CERTNO,      
 QTY = SUM(QTY)      
FROM MSAJAG.DBO.DELTRANS D , --MSAJAG.DBO.DELIVERYDP DP  (NOLOCK),     
(SELECT SETT_NO, SETT_TYPE, SEC_PAYIN FROM MSAJAG.DBO.SETT_MST      
...
SELECT       
 ...
 QTY = SUM(QTY)      
FROM       
BSEDB.DBO.DELTRANS D , --MSAJAG.DBO.DELIVERYDP DP  (NOLOCK),     
(SELECT SETT_NO, SETT_TYPE, SEC_PAYIN FROM BSEDB.DBO.SETT_MST  
WHERE LEFT(GETDATE(),11) BETWEEN START_DATE AND SEC_PAYIN) S    
WHERE       
D.SETT_NO = S.SETT_NO     
AND D.SETT_TYPE = S.SETT_TYPE      
