-- Object: PROCEDURE dbo.aws_contract_note_Bkp_04062020
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

--aws_contract_note '2020-04-16' --,'2020-04-15'


Create  proc [dbo].[aws_contract_note_Bkp_04062020] (@dt datetime,@cnflag VARCHAR(2))
as
...
IF (@CNFLAG='F')
  BEGIN 
			...
			where  C.PARTY_CODE = F.Party_code
			AND CONVERT(VARCHAR(11),SAUDA_DATE ,120)=CONVERT(VARCHAR(11),SDATE ,120)
  END
...
INTO #ORDER_WISE
FROM Charges_Detail   WHERE CD_SAUDA_DATE =@dt AND CD_Scrip_Cd <>'BROKERAGE'  AND CD_PARTY_CODE >='A000' AND CD_PARTY_CODE <='ZZZZZ'
UNION ALL 
SELECT CD_PARTY_CODE, EXCHANGE='BSE' ,SEGMENT ='CAPITAL',CONVERT(VARCHAR(200),CD_Scrip_Cd) SCRIP_CD ,SELL_FLAG=CONVERT(VARCHAR(10),''), CD_Order_No ,
 CD_TrdBuy_Qty+	CD_TrdSell_Qty+	CD_DelBuy_Qty+CD_DelSell_Qty AS TRD_QTY,CD_BUYRATE+CD_SELLRATE AS PRICE,
(CD_TrdBuy_TurnOver+	CD_TrdSell_TurnOver+	CD_DelBuy_TurnOver	+CD_DelSell_TurnOver) TURNOVER ,CONVERT(VARCHAR,CD_TotalBrokerage) AS BROKERAGE
FROM ANAND.BSEDB_AB.DBO.Charges_Detail   WHERE CD_SAUDA_DATE =@dt AND CD_Scrip_Cd <>'BRKSCR' AND CD_PARTY_CODE >='A000' AND CD_PARTY_CODE <='ZZZZZ'
UNION ALL 
...
FROM ANGELFO.NSEFO.DBO.Charges_Detail   WHERE CD_SAUDA_DATE =@dt  AND CD_ComputationLevel ='O'  AND CD_PARTY_CODE >='A000' AND CD_PARTY_CODE <='ZZZZZ'
...
FROM Tbl_CashTradeSummaryforCN 
...
FROM tbl_DailyTradesumary_NSEFO 
...
FROM Contractnote_Daily D,CLIENT_DETAILS C
...
update n set Data_text=data_text+'|'+convert(varchar(30),balance)   from nse_bulk_process n,#final f where sno=1 and cltcode=party_code 
