-- Object: PROCEDURE dbo.PROC_MARING_REPORT_NEW_BKP_27DEC2024_SRE_32587
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


--exec PROC_MARING_REPORT_NEW 'Aug  9 2022' ,'E61465'
------ PROC_MARING_REPORT_NEW_BKP_20220810 
CREATE PROC [dbo].[PROC_MARING_REPORT_NEW_BKP_27122024_SRE_32587]                                                
(                                                          
...
--SET @SAUDA_DATE ='Aug  9 2022'
--SET @USERNAME ='E61465'

DECLARE @ONDATE DATE
SET @ONDATE = @SAUDA_DATE
...
WHERE EFFDATE <=@ONDATE --<= @SAUDA_DATE + ' 23:59'                                                          
...
FROM                                                
 ...
 SELECT                                                 
  PARTY_CODE                                                
 FROM                                                
  CMBILLVALAN WITH(NOLOCK)                                                
 WHERE                                                 
  SAUDA_DATE  =@MARGINDATE --BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59'                                                
 ...
 SELECT                                                 
    PARTY_CODE                                                
 FROM                                                
  AngelBSECM.BSEDB_AB.DBO.CMBILLVALAN WITH (NOLOCK)                                                
 WHERE                                                 
  SAUDA_DATE =@MARGINDATE --BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59'                                                
  AND TRADETYPE NOT IN ( 'SCF','ICF','IR' )    
  group by PARTY_CODE
 ...
 SELECT  PARTY_CODE     
 FROM TBL_CMMARGIN 
 where MARGINDATE =@MARGINDATE--BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59'     
 group by PARTY_CODE
...
-- ------------------- Below Statement Added By Anatha 20220627  -- FOR NSX MTM NOT COMING  
--INSERT INTO #TPARTY     
--SELECT DISTINCT PARTY_CODE FROM         
--(SELECT DISTINCT PARTY_CODE FROM     
--TBL_COMBINE_REPORTING_PEAK_DETAIL T (NOLOCK)     
--  WHERE MARGINDATE BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59' AND EXCHANGE IN('NSE','NSX')   AND TDAY_MTM <> 0     
--UNION ALL    
--SELECT DISTINCT PARTY_CODE  FROM     
--TBL_COMBINE_REPORTING_DETAIL  T (NOLOCK)     
--  WHERE MARGINDATE BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59' AND EXCHANGE IN('NSE', 'NSX')   AND TDAY_MTM <> 0 )A    
--WHERE [PARTY_CODE] NOT IN (SELECT PARTY_CODE FROM #TPARTY  ) AND PARTY_CODE <> '12798'    
-------------------------- END   -- FOR NSX MTM NOT COMING  
  
  
  
------------------- Below Statement Added By Anatha 20220720  -- FOR NSX MTM NOT COMING   
  

INSERT INTO #TPARTY   
  SELECT 
  PARTY_CODE FROM      
  TBL_COMBINE_REPORTING_PEAK_DETAIL T WITH(NOLOCK)      
  WHERE MARGINDATE BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59'  AND TDAY_MARGIN <> 0     AND EXCHANGE IN('NSE','NSX') 
  GROUP BY PARTY_CODE
 INSERT INTO #TPARTY     
  SELECT PARTY_CODE  
  FROM      
  TBL_COMBINE_REPORTING_DETAIL  T WITH(NOLOCK)      
  WHERE MARGINDATE BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59'   AND  (TDAY_MARGIN <> 0 OR TDAY_MTM <> 0 ) AND EXCHANGE IN('NSE', 'NSX')  
  GROUP BY PARTY_CODE 
--)A     
--WHERE [PARTY_CODE] NOT IN (SELECT PARTY_CODE FROM #TPARTY  ) AND PARTY_CODE <> '12798'     
--GROUP BY PARTY_CODE
...
(                          
 ...
 AS
 (
 Select *, ROw_Number() OVER(Partition By PARTY_CODE ORDER BY PARTY_CODE ) RNK from #TPARTY A 
 ) DELETE from A WHERE RNK >1



SELECT * INTO #TBL_CLIENTMARGIN_NSEFO
FROM ANGELFO.NSEFO.DBO.TBL_CLIENTMARGIN D WITH (NOLOCK)    
WHERE MARGINDATE >=  @COLLDATE AND MARGINDATE <= @COLLDATE  +' 23:59' 
...
--BEGIN                    
 ...
 IF OBJECT_ID('tempdb.dbo.#Temp_2', 'U') IS NOT NULL
  DROP TABLE #Temp_2; 
 ...
 IF OBJECT_ID('tempdb.dbo.#MAR_REPORT', 'U') IS NOT NULL
  DROP TABLE #MAR_REPORT; 
 SELECT 
EXCHANGE = 'NSE', SEGMENT = 'CAPITAL', A.PARTY_CODE, CL_TYPE= CONVERT(VARCHAR(10),''),                                                          
CASH = SUM(CASH),                       
--CASH =  sum (case when vtyp=18 then balamt*-1 else CASE WHEN DRCR = 'C' THEN VAMT ELSE -VAMT END end ),                          
NONCASH = CONVERT(NUMERIC(18,4),0), FD_BG = CONVERT(NUMERIC(18,4),0),                                                          
INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                                                          
...
--------------------------- Second Step               
 ...
 IF OBJECT_ID('tempdb.dbo.#Temp_1', 'U') IS NOT NULL
  DROP TABLE #Temp_1; 
  SELECT 
  EXCHANGE = 'BSE', SEGMENT = 'CAPITAL', PARTY_CODE, CL_TYPE= CONVERT(VARCHAR(10),''),                                                          
  CASH = SUM(CASE WHEN DRCR = 'C' THEN VAMT ELSE -VAMT END),                       
  --CASH =  sum (case when vtyp=18 then balamt*-1 else CASE WHEN DRCR = 'C' THEN VAMT ELSE -VAMT END end ),                          
  NONCASH = CONVERT(NUMERIC(18,4),0), FD_BG = CONVERT(NUMERIC(18,4),0),                                                          
  INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                                                          
  ADD_MARGIN = CONVERT(NUMERIC(18,4),0)       
  INTO #Temp_1
  FROM AngelBSECM.ACCOUNT_AB.DBO.LEDGER L WITH (NOLOCK), #TPARTY C, AngelBSECM.ACCOUNT_AB.DBO.PARAMETER P                                                          
  WHERE L.CLTCODE = C.PARTY_CODE AND L.EDT BETWEEN P.SDTCUR AND P.LDTCUR                                                           
  AND @SAUDA_DATE BETWEEN P.SDTCUR AND P.LDTCUR                                                           
  AND EDT <= @SAUDA_DATE + ' 23:59'                                                   
  --AND EDT <= (CASE WHEN DRCR = 'D' THEN @SAUDA_DATE + ' 23:59' ELSE EDT END)                                                  
...
FROM AngelBSECM.ACCOUNT_AB.DBO.LEDGER L WITH (NOLOCK), 
     #TPARTY C, 
     AngelBSECM.ACCOUNT_AB.DBO.PARAMETER P                                                           
WHERE L.CLTCODE = C.PARTY_CODE AND L.EDT BETWEEN P.SDTCUR AND P.LDTCUR                                                         
AND @SAUDA_DATE BETWEEN P.SDTCUR AND P.LDTCUR                                                           
AND EDT <= @SAUDA_DATE + ' 23:59'                  
and VDT <  P.SDTCUR                                                    
...
SELECT 
EXCHANGE = 'BSE', SEGMENT = 'CAPITAL', A.PARTY_CODE, CL_TYPE= CONVERT(VARCHAR(10),''),                                                          
CASH = SUM(CASH),                       
--CASH =  sum (case when vtyp=18 then balamt*-1 else CASE WHEN DRCR = 'C' THEN VAMT ELSE -VAMT END end ),                          
NONCASH = CONVERT(NUMERIC(18,4),0), FD_BG = CONVERT(NUMERIC(18,4),0),                                                          
INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                                                          
...
--BEGIN                                                
 ...
 AND @SAUDA_DATE BETWEEN P.SDTCUR AND P.LDTCUR                                                           
 AND EDT <= @Ondate --@SAUDA_DATE + ' 23:59'                                                  
 --AND EDT <= (CASE WHEN DRCR = 'D' THEN @SAUDA_DATE + ' 23:59' ELSE EDT END)                                                          
 AND VTYP <> '15'                                                
 GROUP BY PARTY_CODE                                                      
...
CREATE INDEX #PARTY ON #TBL_CLIENTMARGIN_NSEFO (PARTY_CODE)                                
...
FROM #TBL_CLIENTMARGIN_NSEFO D WITH (NOLOCK)    
...
--  MARGIN = SUM(VARAMT + ELM) - (CASE WHEN SUM(MTOM) > 0 THEN 0 ELSE SUM(MTOM) END)                                                
 ...
 FROM AngelBSECM.BSEDB_AB.DBO.TBL_MG02 M, #TPARTY C                                                          
 WHERE M.MARGIN_DATE BETWEEN  @MARGINDATE AND @MARGINDATE  +' 23:59'                                                          
 AND M.PARTY_CODE = C.PARTY_CODE                 
 GROUP BY                                                
  M.PARTY_CODE,                                                
...
SELECT                                                
 ...
 PARTY_CODE,                                                 
 CL_TYPE='',           
 CASH = SUM(CASH), NONCASH = SUM(NONCASH), FD_BG = SUM(FD_BG),                                                           
 INIT_MARGIN = SUM(INIT_MARGIN),                                                
 EXP_MARGIN = SUM(EXP_MARGIN),                                                          
 ADD_MARGIN = SUM(ADD_MARGIN) INTO #MAR_REPORT1  FROM #MAR_REPORT    
 GROUP BY  EXCHANGE  , SEGMENT ,                                                 
 PARTY_CODE    
    
 TRUNCATE TABLE #MAR_REPORT    
 INSERT INTO #MAR_REPORT    
 SELECT * FROM #MAR_REPORT1    
                                      
                                   
                                                
UPDATE #MAR_REPORT SET INIT_MARGIN =PREV_VAR + MAR_ADJUST                                      
,NONCASH=PREV_SEC+CURR_SEC                                      
FROM AngelBSECM.BSEDB_AB.DBO.MARGIN_REPORTING T                                      
...
BEGIN    
 ...
 FROM  AngelBSECM.BSEDB_AB.DBO.TBL_CMMARGIN M WITH(NOLOCK)     
                        WHERE M.MARGINDATE BETWEEN @MARGINDATE AND @MARGINDATE  +' 23:59'                        
                        GROUP BY    
                              M.PARTY_CODE    
...
BEGIN                                           
 ...
 INIT_MARGIN = 0,                                                          
 ADD_MARGIN = 0,    
 MAR_PLEDGE_AMT = SUM(TDAY_NONCASH),    
 DEL_MARGIN = 0                                                             
 FROM TBL_COMBINE_REPORTING_DETAIL D WITH (NOLOCK) , #TPARTY C              
 WHERE MARGINDATE BETWEEN  @COLLDATE AND @COLLDATE  +' 23:59'                                                              
 AND D.PARTY_CODE = C.PARTY_CODE                                                          
 AND EXCHANGE = 'NSE'                                           
 AND SEGMENT = 'FUTURES'              
 AND TDAY_MARGIN+TDAY_MTM =0    and TDAY_NONCASH <>0                                                    
 GROUP BY EXCHANGE, SEGMENT, D.PARTY_CODE   */    
 ...
 DEL_MARGIN = 0    
 FROM #TBL_CLIENTMARGIN_NSEFO D WITH (NOLOCK)    
 LEFT OUTER JOIN #FOFDBG1 F ON (F.PARTY_CODE = D.PARTY_CODE)   
inner join  #TPARTY C   ON D.PARTY_CODE = C.PARTY_CODE                                                        
 WHERE MARGINDATE >=CONVERT(DATE,@COLLDATE) --  @COLLDATE AND MARGINDATE <=  @COLLDATE  +' 23:59'                                                          
 --AND D.PARTY_CODE = C.PARTY_CODE                                                          
 GROUP BY D.PARTY_CODE,ISNULL(FD_BG,0)         
     
 --ADDED FOR NOT COMING OF PLED VALUE N DMS    
...
CREATE INDEX IX_#MArClient On #MArClient(PARTY_CODE)	   
	
  
 IF OBJECT_ID('tempdb.dbo.#TempTBL_COMBINE_REPORTING_DETAIL', 'U') IS NOT NULL
  DROP TABLE #TempTBL_COMBINE_REPORTING_DETAIL; 
 Select * INTO #TempTBL_COMBINE_REPORTING_DETAIL FROM TBL_COMBINE_REPORTING_DETAIL  A WITH (NOLOCK)                                
 WHERE MARGINDATE =CONVERT(DATE,@COLLDATE)    
 AND EXCHANGE = 'NSE' AND SEGMENT = 'FUTURES'  
   
 CREATE INDEX IX_#TempTBL_COMBINE_REPORTING_DETAIL ON #TempTBL_COMBINE_REPORTING_DETAIL(PARTY_CODE) INCLUDE (TDAY_NONCASH)
  
  INSERT INTO #MAR_REPORT         
  SELECT EXCHANGE='NSE', SEGMENT='FUTURES', A.PARTY_CODE, CL_TYPE='',                                                              
  CASH = 0,                                                            
  NONCASH = 0,                                   
  FD_BG = 0,                                                            
  INIT_MARGIN = CONVERT(NUMERIC(18,4),0),                                             
  ...
  MAR_PLEDGE_AMT = SUM(TDAY_NONCASH),      
  DEL_MARGIN = 0      
 FROM #TempTBL_COMBINE_REPORTING_DETAIL  A WITH (NOLOCK)                                
      INNER JOIN  #TPARTY C ON  A.PARTY_CODE = C.PARTY_CODE
 WHERE       
 NOT EXISTS (Select PARTY_CODE from #MArClient  X WHERE X.PARTY_CODE =A.PARTY_CODE   )      
 AND EXISTS (SELECT PARTY_CODE FROM #MAR_REPORT Y WITH (NOLOCK) WHERE  C.PARTY_CODE =Y.PARTY_CODE AND MAR_PLEDGE_AMT <>0 )    
 GROUP BY A.PARTY_CODE    
 HAVING SUM(TDAY_NONCASH) <> 0    
   
 -- INSERT INTO #MAR_REPORT         
 -- SELECT EXCHANGE='NSE', SEGMENT='FUTURES', A.PARTY_CODE, CL_TYPE='',                                                               CASH = 0,                                                            
 -- NONCASH = 0,                                   
 -- FD_BG = 0,                                                            
 -- INIT_MARGIN = CONVERT(NUMERIC(18,4),0),                                             
 -- EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                                                      
 ...
 -- MAR_PLEDGE_AMT = SUM(TDAY_NONCASH),      
 -- DEL_MARGIN = 0      
 --FROM TBL_COMBINE_REPORTING_DETAIL  A WITH (NOLOCK)                                
 --     INNER JOIN  #TPARTY C ON  A.PARTY_CODE = C.PARTY_CODE
 --WHERE MARGINDATE BETWEEN  @COLLDATE AND  @COLLDATE + ' 23:59:59' --AND PARTY_CODE BETWEEN 'A1001024' AND 'A1001024'      
 --AND EXCHANGE = 'NSE' AND SEGMENT = 'FUTURES'      
 --AND  NOT EXISTS (Select PARTY_CODE from #MArClient  X (Nolock) WHERE X.PARTY_CODE =A.PARTY_CODE   )      
 --AND A.PARTY_CODE = C.PARTY_CODE     
 --AND EXISTS (SELECT PARTY_CODE FROM #MAR_REPORT Y WITH (NOLOCK) WHERE  C.PARTY_CODE =Y.PARTY_CODE AND EXCHANGE = 'NSE' AND SEGMENT = 'FUTURES' AND MAR_PLEDGE_AMT <>0)    
 --GROUP BY A.PARTY_CODE    
 --HAVING SUM(TDAY_NONCASH) <> 0    
 ...
 MAR_PLEDGE_AMT = SUM(MRG_REP_COLL_NCASH),    
 DEL_MARGIN = 0                                                            
 FROM ANGELFO.NSECURFO.DBO.TBL_CLIENTMARGIN D WITH (NOLOCK) 
 INNER JOIN #TPARTY C    On D.PARTY_CODE = C.PARTY_CODE                                                        
 WHERE MARGINDATE =Convert(Date,@COLLDATE)--BETWEEN  @COLLDATE AND @COLLDATE  +' 23:59'                                                                                                         
 GROUP BY D.PARTY_CODE                                      
 ...
 MAR_PLEDGE_AMT = SUM(MRG_REP_COLL_NCASH),    
 DEL_MARGIN = 0                                                            
 FROM ANGELCOMMODITY.MCDXCDS.DBO.TBL_CLIENTMARGIN D WITH (NOLOCK) 
 INNER JOIN #TPARTY C ON D.PARTY_CODE = C.PARTY_CODE                                                        
 WHERE MARGINDATE = CONVERT(DATE,@COLLDATE)--BETWEEN  @COLLDATE AND @COLLDATE  +' 23:59'                                                                          
 GROUP BY D.PARTY_CODE    
...
SELECT PARTY_CODE,SUM(TDAY_MARGIN) AS TOTAL_MARGIN,0 AS PEAK_MARGIN ,    
MARGINDATE    
...
FROM AngelNSECM.MSAJAG.DBO.TBL_COMBINE_REPORTING_PEAK WITH(NOLOCK) WHERE MARGINDATE =@SAUDA_DATE AND (TDAY_MARGIN >0 OR TDAY_MTM >0)  --AND PARTY_CODE ='HEMC004'    
GROUP BY PARTY_CODE,MARGINDATE    
)A    
  GROUP BY PARTY_CODE,MARGINDATE    
...
BEGIN    
   ...
   SELECT EXCHANGE = 'NSE', SEGMENT = 'FUTURES',    
     ...
     NONCASH = 0,    
     FD_BG = 0,    
     INIT_MARGIN = SUM(SPREADMARGIN+MTOM+NONSPREADMARGIN),    
