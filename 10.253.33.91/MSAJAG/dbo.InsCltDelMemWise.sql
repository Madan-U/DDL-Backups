-- Object: PROCEDURE dbo.InsCltDelMemWise
...
/****** Object:  Stored Procedure dbo.InsCltDelMemWise    Script Date: 05/08/2002 12:35:03 PM ******/




/****** Object:  Stored Procedure dbo.InsCltDelMemWise    Script Date: 4/12/01 1:05:15 PM ******/
CREATE Proc InsCltDelMemWise ( @Sett_no Varchar(7),@Sett_Type Varchar(2),@RefNo int) as
...
Set @@DelClt = CurSor for
 select scrip_cd,series,Party_code,Qty,PartiPantCode from DeliveryClt where inout = 'O'
 and sett_no = @Sett_No and Sett_Type = @Sett_Type
 order by scrip_cd,series,qty,party_code desc
Open @@DelClt
fetch next from @@DelClt into @@Scrip_Cd,@@Series,@@Party_code,@@DelQty,@@PartiPantCode
...
begin
       ...
       Begin  
		...
		where sett_no = @sett_no 
         		...
         		and scrip_cd  = @@scrip_cd 
       	 	and series = @@series and TrType <> 1000
	 	and DrCr = 'D' and PartiPantCode = @@PartiPantCode and Filler2 = 1 
...
and Sett_no = @Sett_No and Sett_Type = @Sett_Type And DefDp = 1 and DpId = '' and DrCr = 'D' and Filler2 = 1
