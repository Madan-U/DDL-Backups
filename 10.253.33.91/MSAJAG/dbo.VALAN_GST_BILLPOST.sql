-- Object: PROCEDURE dbo.VALAN_GST_BILLPOST
...
DECLARE @START_DATE DATETIME  --KFC CHANGE

DECLARE @NARRATION VARCHAR(100)

SELECT @NARRATION = ISNULL(MAX(NARRATION),'BILL POSTED') FROM ACCBILL WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE

SELECT @START_DATE = START_DATE FROM SETT_MST WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE --KFC CHANGE
...
SGST_CESS_PER = CONVERT(NUMERIC(18,4),0),--KFC CHANGE
Start_Date
INTO #GSTDATA
...
WHERE TARGETSTATE NOT IN (select distinct state from tbl_statecity_master with (NOLOCK) where State <> '')
...
/* KFC CHANGE */
UPDATE #GSTDATA SET SGST_PER = G.SGST_PER + CESSTAX , GST_PER = G.GST_PER + CESSTAX,
SGST_CESS_PER = CESSTAX
FROM TBL_CLIENT_GST_DATA C, TBL_STATE_GST_DATA G, TBL_GST_LOCATION L
WHERE #GSTDATA.Party_Code = C.PARTY_CODE
AND @START_DATE BETWEEN C.EFF_FROM_DATE AND C.EFF_TO_DATE 
AND @START_DATE BETWEEN G.EFF_FROM_DATE AND G.EFF_TO_DATE
...
SGST_CESS_PER = SERVICE_TAX * SGST_CESS_PER / GST_PER --KFC CHANGE

UPDATE #GSTDATA SET SGST_PER = SERVICE_TAX - (CGST_PER + IGST_PER + UGST_PER + AGST_PER + SGST_CESS_PER) --KFC CHANGE
WHERE SGST_PER > 0 
UPDATE #GSTDATA SET UGST_PER = SERVICE_TAX - (CGST_PER + IGST_PER + SGST_PER + AGST_PER + SGST_CESS_PER) --KFC CHANGE
WHERE UGST_PER > 0 
...
INSERT INTO ACCBILL   --KFC CHANGE
SELECT PARTY_CODE=ACCODE,BILL_NO=0,SELL_BUY=2,SETT_NO,SETT_TYPE,S.START_DATE,END_DATE,PAYIN_DATE=FUNDS_PAYIN,
PAYOUT_DATE=FUNDS_PAYOUT,AMOUNT=ROUND(SUM(SGST_CESS_PER),2),BRANCH_CD,NARRATION=@NARRATION
FROM #GSTDATA G, SETT_MST S, VALANACCOUNT 
WHERE S.SETT_NO = @SETT_NO
AND S.SETT_TYPE = @SETT_TYPE
AND ACNAME = STATE_CODE + '_CESS'
AND Trdtype = 'N'
GROUP BY ACCODE, SETT_NO,SETT_TYPE,S.START_DATE,END_DATE,FUNDS_PAYIN,FUNDS_PAYOUT,BRANCH_CD 
HAVING ROUND(SUM(SGST_CESS_PER),2) > 0
...
INSERT INTO ACCBILL  --KFC CHANGE
SELECT PARTY_CODE=ACCODE,BILL_NO=0,SELL_BUY=2,SETT_NO,SETT_TYPE,S.START_DATE,END_DATE,PAYIN_DATE=FUNDS_PAYIN,
PAYOUT_DATE=FUNDS_PAYOUT,AMOUNT=ROUND(SUM(SGST_CESS_PER),2),BRANCH_CD='ZZZ',NARRATION=@NARRATION
FROM #GSTDATA G, SETT_MST S, VALANACCOUNT 
WHERE S.SETT_NO = @SETT_NO
AND S.SETT_TYPE = @SETT_TYPE
AND ACNAME = STATE_CODE + '_CESS'
...
SELECT @NARRATION = ISNULL(MAX(NARRATION),'BILL POSTED') FROM IACCBILL WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE
...
INSERT INTO IACCBILL  --KFC CHANGE
SELECT PARTY_CODE=ACCODE,BILL_NO=0,SELL_BUY=2,SETT_NO,SETT_TYPE,S.START_DATE,END_DATE,PAYIN_DATE=FUNDS_PAYIN,
PAYOUT_DATE=FUNDS_PAYOUT,AMOUNT=ROUND(SUM(SGST_CESS_PER),2),BRANCH_CD,NARRATION=@NARRATION
FROM #GSTDATA G, SETT_MST S, VALANACCOUNT 
WHERE S.SETT_NO = @SETT_NO
