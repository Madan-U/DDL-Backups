-- Object: PROCEDURE dbo.STT_Charges_Final_BKP_14AUG2025_SRE35483
...
CREATE  PROC [dbo].[STT_Charges_Final_BKP_14AUG2025_SRE35483] (                        
 @Sett_Type VARCHAR(2)                        
...
DECLARE @Party_Code VARCHAR(10)                        
 ...
 ,@DelSellTrans NUMERIC(18, 4),                        
  ...
  @WRSELLTRANS  NUMERIC(18,4),      
  @RTSELLTRANS  NUMERIC(18,4)                         
 ,@SETTFLAG INT                        
 ,@ISETTFLAG INT                        
 ,@STTCur CURSOR                        

 DECLARE @SETT_NO VARCHAR(7)                                
 SELECT @SETT_NO= SETT_NO FROM SETT_MST WHERE  START_DATE LIKE @SAUDA_DATE + '%' AND SETT_TYPE = @SETT_TYPE                                      


EXEC PROC_SETTFLAG_START @SETT_NO, @SETT_TYPE, @SAUDA_DATE, @FROMPARTY, @TOPARTY 

---select  @SETT_NO, @SETT_TYPE, @SAUDA_DATE, @FROMPARTY, @TOPARTY 
...
BEGIN                        
 ...
 SELECT @RTSELLTRANS = 0  
 SELECT @WRSELLTRANS = 0
...
/* end */                      
 ...
 SELECT @TrdBuyTrans = TrdBuyTrans                        
  ...
  ,  @RTSELLTRANS = RTSELLTRANS ,
  @WRSELLTRANS = WRSELLTRANS
 ...
 IF (                      
   ...
   OR @TrdSellTrans > 0                        
   OR @DelSellTrans > 0                        
   OR @DelBuyTrans > 0 OR @RTSELLTRANS > 0            
   )                        
 BEGIN                        
...
STT_SETTLEMENT 
  (orgSrNo,Contractno,Billno,Trade_No,Party_Code,Scrip_Cd,User_Id,Tradeqty,Auctionpart,Markettype,Series,Order_No,Marketrate,Sauda_Date,
  Table_No,Line_No,Val_Perc,Normal,Day_Puc,Day_Sales,Sett_Purch,Sett_Sales,Sell_Buy,Settflag,Brokapplied,Netrate,Amount,Ins_Chrg,Turn_Tax,
  Other_Chrg,Sebi_Tax,Broker_Chrg,Service_Tax,Trade_Amount,Billflag,Sett_No,Nbrokapp,Nsertax,N_Netrate,Sett_Type,Partipantcode,Status,
  Pro_Cli,Cpid,Instrument,Booktype,Branch_Id,Tmark,Scheme,Dummy1,Dummy2,exchg,org_contractno,org_sett_no,org_sett_type,org_scrip_cd,org_series,ISIN
)
  ...
  SELECT SrNo                        
   ...
   ,Booktype                        
   ,Branch_Id                        
   ,TMARK,SCHEME  ,S.DUMMY1,S.DUMMY2,exchg,org_contractno,org_sett_no,org_sett_type,org_scrip_cd,org_series,ISIN
  FROM COMBINE_SETTLEMENT S                        
   ,CLIENT2 C2                        
  WHERE SAUDA_DATE LIKE @SAUDA_DATE + '%'                        
  ...
  OR Upper(LTrim(RTrim(@Sett_Type))) = 'M'                       
   ...
   WHERE STT_SETT_TABLE.Sett_No = A.Sett_No                        
    ...
    SET                             
     INS_CHRG = (CASE WHEN SELL_BUY = 1                                
      THEN 0                                
      ELSE ROUND(TRADEQTY * TRDVAL * @WRSELLTRANS / 100,4)                                
     END)                    
    FROM                             
     STT_ANISETT_TEMP A                               
    WHERE                             
	 ...
	 AND STT_SETT_TABLE.SERIES LIKE 'W%'
	  AND @WRSELLTRANS > 0         
 ...
 UPDATE STT_SETT_TABLE                              
    SET                               
     INS_CHRG = (CASE WHEN SELL_BUY = 1                                  
      THEN 0                                  
      ELSE ROUND(TRADEQTY * TRDVAL * @RTSELLTRANS / 100,4)                                  
     END)                      
    FROM                               
     STT_ANISETT_TEMP A                                 
    WHERE                               
  ...
  AND STT_SETT_TABLE.SETTFLAG IN (1,4,5)           
  AND EXISTS (SELECT SCRIP_CD FROM TBL_STT_SCRIP WHERE @SAUDA_DATE BETWEEN DATEFROM AND DATETO           
          AND STT_SETT_TABLE.SCRIP_CD = TBL_STT_SCRIP.SCRIP_CD AND STT_SETT_TABLE.SERIES = TBL_STT_SCRIP.SERIES AND CHARGE_FLAG = 2)          
  AND @RTSELLTRANS > 0  
                        
   -- INS COMBINE_SETTLEMENT                                                   
   SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED                        
   ...
   WHERE STT_ISETT_TABLE.Sett_No = A.Sett_No                        
    ...
    WHERE                             
     ...
     -- AND STT_ISETT_TABLE.TRADE_NO NOT LIKE '%C%'                                
     AND STT_ISETT_TABLE.CONTRACTNO = A.CONTRACTNO 
	 AND STT_ISETT_TABLE.SERIES LIKE 'W%'   
  AND @WRSELLTRANS > 0                       
  ...
  AND @ETFSELLTRANS > 0                          
    ...
    WHERE                               
     ...
     AND STT_ISETT_TABLE.CONTRACTNO = A.CONTRACTNO           
  AND EXISTS (SELECT SCRIP_CD FROM TBL_STT_SCRIP WHERE @SAUDA_DATE BETWEEN DATEFROM AND DATETO           
          AND STT_ISETT_TABLE.SCRIP_CD = TBL_STT_SCRIP.SCRIP_CD AND STT_ISETT_TABLE.SERIES = TBL_STT_SCRIP.SERIES AND CHARGE_FLAG = 2)          
  AND @RTSELLTRANS > 0 
...
*/    
   ...
   SET PSTTDEL = Round(PQtyDel * TrdVal * @DelBuyTrans / 100, 4)                        
	...
	Round(SQtyDel * TrdVal * @DelSellTrans / 100, 4) + Round(SQtyTrd * TrdVal * @TrdSellTrans / 100, 4)                        

	UPDATE STT_ANISETTORG_TEMP                             
	SET PSTTDEL = 0, SSTTDEL  = ROUND(SQTYDEL*TRDVAL*@WRSELLTRANS/100,4),         
	TOTALTAX =  ROUND(SQTYDEL*TRDVAL*@WRSELLTRANS/100,4) + SSTTTRD        
	WHERE SERIES LIKE 'W%'    
	AND @WRSELLTRANS > 0
...
/*         
 UPDATE STT_ANISETTORG_TEMP                               
    SET PSTTDEL = 0, SSTTDEL  = ROUND(SQTYDEL*TRDVAL*@RTSELLTRANS/100,4),           
 TOTALTAX =  ROUND(SQTYDEL*TRDVAL*@RTSELLTRANS/100,4) + SSTTTRD          
 WHERE EXISTS (SELECT SCRIP_CD FROM TBL_STT_SCRIP WHERE @SAUDA_DATE BETWEEN DATEFROM AND DATETO           
          AND STT_ANISETTORG_TEMP.SCRIP_CD = TBL_STT_SCRIP.SCRIP_CD AND STT_ANISETTORG_TEMP.SERIES = TBL_STT_SCRIP.SERIES AND CHARGE_FLAG = 2)          
 AND @RTSELLTRANS > 0  
...
/*                        
 ...
 AND STT_SETT_TABLE.Sett_No = A.Sett_No                        
  ...
  IF Upper(LTrim(RTrim(@Sett_Type))) = 'A'                        
   ...
   OR Upper(LTrim(RTrim(@Sett_Type))) = 'Z'  --Changes for t1                     
  ...
  BEGIN                        
   ...
   UPDATE STT_SETT_TABLE                             
    SET                             
     INS_CHRG =  (CASE WHEN SELL_BUY = 1            
        THEN 0                            
        ELSE ROUND(TRADEQTY * MARKETRATE * @WRSELLTRANS / 100,4)                                
       END)                                
    WHERE                             
     TRADEQTY > 0                                
 AND SERIES LIKE 'W%'                         
 ...
 UPDATE STT_SETT_TABLE                               
    SET                               
     INS_CHRG =  (CASE WHEN SELL_BUY = 1                                   
        THEN 0                              
    ELSE ROUND(TRADEQTY * MARKETRATE * @RTSELLTRANS / 100,4)                                  
       END)                                  
    WHERE                               
     TRADEQTY > 0                                  
 AND EXISTS (SELECT SCRIP_CD FROM TBL_STT_SCRIP WHERE @SAUDA_DATE BETWEEN DATEFROM AND DATETO           
   ...
   WHERE TradeQty > 0        
    ...
    WHERE                             
     TRADEQTY > 0                                
 AND ( SERIES LIKE 'W%'     OR SERIES LIKE 'Z%')   
    AND @WRSELLTRANS > 0                                                    
...
*/                  
 ...
 WHERE STT_ISETT_TABLE.Sauda_Date BETWEEN FromDate AND ToDate                       
  ...
  /* NOW FINAL UPDATE IN COMBINE_SETTLEMENT TABLE*/                        
  ...
  WHERE COMBINE_SETTLEMENT.SRNO = A.ORGSRNO                        
...
/* end */                    
 ...
 EXEC BBGSETTBROKRECAL_ONLINE @Sett_Type  --- ADDED ON MAR 27 2015            
  ...
  ,@ToParty      
				 ...
				 EXEC CLS_ADDITIONAL_TAX_CAL @SETT_TYPE, @SAUDA_DATE, @FROMPARTY, @TOPARTY 
