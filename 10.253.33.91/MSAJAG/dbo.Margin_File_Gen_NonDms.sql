-- Object: PROCEDURE dbo.Margin_File_Gen_NonDms
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE PROC [dbo].[Margin_File_Gen_NonDms]        
AS        
...
--EXEC  [Margin_File_Gen_NonDms] 'APR 28 2022','KHAO1017'
...
SELECT * INTO #TDAY_PEAK1 FROM TBL_COMBINE_REPORTING_PEAK_DETAIL_HIST T WITH(NOLOCK),#TEMP S        
...
Insert into #tday_peak1        
Select * from TBL_COMBINE_REPORTING_PEAK_DETAIL  t,#temp s        
where t.PARTY_CODE =s.cl_code and t.MARGINDATE =s.Mdate    
   
 Select party_code,MARGINDATE,Max(TDAY_MARGIN) as TDAY_Margin into #peak_high from #tday_peak1  
 Group by party_code,MARGINDATE  
  
 Select t.* into #peak_high_Val from #tday_peak1 t,#peak_high p  
 where t.PARTY_CODE =p.PARTY_CODE and t.MARGINDATE =p.MARGINDATE  
 and t.TDAY_MARGIN =p.TDAY_Margin  
   
  
 Update #tday_peak1 Set TDAY_MARGIN=0  
  
  Update t Set TDAY_MARGIN =p.TDAY_MARGIN  
 From #peak_high_Val p ,#tday_peak1 t  
 Where P.PARTY_CODE =t.PARTY_CODE and p.EXCHANGE =t.EXCHANGE and p.SEGMENT =t.SEGMENT  
 and p.MARGINDATE =t.MARGINDATE  
  
 Update t Set Tday_MTM =p.Tday_MTM  
 From #tday p ,#tday_peak1 t  
 Where P.PARTY_CODE =t.PARTY_CODE and p.EXCHANGE =t.EXCHANGE and p.SEGMENT =t.SEGMENT  
 and p.MARGINDATE =t.MARGINDATE  
  
 Select * into #tday_peak  from #tday_peak1   
...
from         
(        
Select Party_code,Sum(Tday_margin) as Margin  from #tday Group by Party_code)A        
Full outer Join        
(        
Select Party_code,Sum(Tday_margin ) as PMargin from #tday_peak Group by Party_code)B        
...
FROM  AngelNSECM.msajag.dbo.CLIENT_DETAILS C (NOLOCK),#temp d        
...
--set @NSEQUERY = @NSEQUERY + ' from AngelNSECM.msajag.dbo.Margin_process order by party_code ,sno,SRNO " queryout J:\Comb_Margin\ContractNote_Margin_'+@Cl_code+'_'+replace(convert(varchar,@mdate,103),'/','')+'.txt -c -t"," -Sangelcommodity -Uclassmkt -Pdd$$gnfDTVs244648ysjgZAcc'  
