-- Object: PROCEDURE dbo.rpt_datesettwisebrkalbm
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



/****** Object:  Stored Procedure dbo.rpt_datesettwisebrkalbm    Script Date: 04/27/2001 4:32:38 PM ******/
...
begin
	if @flag = 1 
	begin
		select sett_type,sett_no,party_code, short_name, trader, DeliveryCharge , Brok  , left(convert(varchar,sauda_date,109),11) as sauda_date, series,scrip_cd 
	...
	union all
		...
		and trader like  ltrim(@trader)+'%' 
		and scrip_cd like   ltrim(@scrip)+'%'
		order by party_code,left(convert(varchar,sauda_date,109),11) ,short_name,sett_no,sett_type,series,scrip_cd 
	end 
	else if @flag = 2  
	begin
		select sett_type,sett_no,party_code, short_name, trader, DeliveryCharge , Brok  ,left(convert(varchar,sauda_date,109),11) as sauda_date, series,scrip_cd 
