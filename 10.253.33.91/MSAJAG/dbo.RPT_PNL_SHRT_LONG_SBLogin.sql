-- Object: PROCEDURE dbo.RPT_PNL_SHRT_LONG_SBLogin
...
CREATE PROC [dbo].[RPT_PNL_SHRT_LONG_SBLogin]                                                                        
(                                                                          
 @STATUSID VARCHAR(25),                                                                          
...
DECLARE @FINYEARFROM DATETIME, @FINYEARTO DATETIME 

Declare @Column VARCHAR(10)

IF EXISTS(Select * from INTRANET.Mimansa.dbo.B2B_Bulk_DealerMapping WHERE Dealer1 = @STATUSNAME)
BEGIN
	SET @Column = 'Dealer'
END
ELSE
...
SELECT 
	SAUDA_DATE, SETT_NO, SETT_TYPE, T.PARTY_CODE,                                                                         
...
BEGIN
	...
	FROM                                              
		CLIENT_DETAILS C (NOLOCK) 
		INNER JOIN  INTRANET.Mimansa.Dbo.tbl_Emp_Master em With (NOLOCK) ON C.Sub_Broker = em.sb_Tag
		LEFT JOIN   INTRANET.Mimansa.Dbo.B2B_Bulk_DealerMapping dm With (NOLOCK) ON em.EMP_No = dm.Dealer1 AND C.Party_Code = dm.Party_Code                                                        
		/*INNER JOIN TBL_PNL_DATA T (NOLOCK)                                         
		ON (C.PARTY_CODE = T.PARTY_CODE)*/                                        
	WHERE                                              
	CL_CODE BETWEEN @FROMPARTY AND @TOPARTY                                         
	...
	AND @STATUSNAME = (CASE                                                                                       
	   WHEN @STATUSID = 'BRANCH' THEN C.BRANCH_CD                                 
	   WHEN @STATUSID = 'SUBBROKER' THEN dm.Dealer1                                                                                      
	   WHEN @STATUSID = 'TRADER' THEN C.TRADER                                              
	   WHEN @STATUSID = 'FAMILY' THEN C.FAMILY                                                                                      
	   WHEN @STATUSID = 'AREA' THEN C.AREA                                                   
	   WHEN @STATUSID = 'REGION' THEN C.REGION                                                                                      
...
DROP TABLE  #TBL_PNL_DATA ,#CLIENT_DETAILS        
