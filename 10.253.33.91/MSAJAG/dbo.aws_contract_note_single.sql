-- Object: PROCEDURE dbo.aws_contract_note_single
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE  proc [dbo].[aws_contract_note_single] (@dt datetime,@cnflag VARCHAR(2),@CLTCODE VARCHAR(10))  
as  
...
AND A.SETT_TYPE =B.SETT_TYPE AND A.SETT_TYPE IN ('N','W','D','C','M','Z','RC','RD')   
...
*/
--added as per new format---Chnage under SRE-37277
 ...
 SELECT PARTY_CODE,TRD_DT = CONVERT(VARCHAR(11),SAUDA_DATE,103),TRADETYPE,INST_TYPE,
  ...
  WHERE SAUDA_DATE >=@DT AND SAUDA_DATE <= @DT +' 23:59:59' AND STATUSNAME <>'BRK' AND PARTY_CODE >= @CLTCODE AND PARTY_CODE <= @CLTCODE
...
AND (PQTYTRD+SQTYTRD+PQTYDEL+SQTYDEL) <>0 AND PARTY_CODE >= @CLTCODE AND PARTY_CODE <= @CLTCODE
UNION ALL
SELECT * FROM ANGELBSECM.BSEDB_AB.DBO.CMBILLVALAN S WITH (NOLOCK) WHERE SAUDA_DATE >=@DT  AND  SAUDA_DATE < = @DT  +' 23:59:59'
AND (PQTYTRD+SQTYTRD+PQTYDEL+SQTYDEL )<>0  AND PARTY_CODE >= @CLTCODE AND PARTY_CODE <= @CLTCODE )A
