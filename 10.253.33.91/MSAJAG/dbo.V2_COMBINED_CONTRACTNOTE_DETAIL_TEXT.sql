-- Object: PROCEDURE dbo.V2_COMBINED_CONTRACTNOTE_DETAIL_TEXT
...
-- @PRINTF         VARCHAR(6) = 'ALL'                  
 ...
 AS       
 ---EXEC V2_COMBINED_CONTRACTNOTE_DETAIL_TEXT 'broker','broker','Oct 28 2021','Oct 28 2021','A000000','ZZZZZZZ','ZPUR','ZPUR','BRANCHGROUP','','zzzzzzzzzzz','','99999999','CONTRACT','ALL',0,'NONECN','Angel Broking Limited (Formerly known as Angel Broking Private Limited.)','CASHFNO'      
...
WHERE 
 ...
 CReate index #P on #PARTY(PARTYCODE)
            
 IF @BOUNCEDFLAG = 0            
 BEGIN      
 
  SELECT * INTO #BRANCH FROM BRANCHGROUP WHERE BRANCH_GROUP  BETWEEN  @FROMBRANCH  AND  @TOBRANCH 

  CREATE INDEX #B ON #BRANCH(BRANCH_CODE)


  INSERT INTO #PARTY            
  SELECT DISTINCT C.PARTY_CODE FROM COMMON_CONTRACT_DATA C(NOLOCK) ,#BRANCH B            
  WHERE SAUDA_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59:59' AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY   AND B.BRANCH_CODE=BRANCH_CD         
...
AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY  AND PARTY_CODE IN (SELECT * FROM #PARTY)
