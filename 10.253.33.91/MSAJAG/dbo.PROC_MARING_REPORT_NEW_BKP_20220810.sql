-- Object: PROCEDURE dbo.PROC_MARING_REPORT_NEW_BKP_20220810
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

    
                                   
CREATE PROC [dbo].[PROC_MARING_REPORT_NEW_BKP_20220810]                                                
(                                                          
...
SELECT DISTINCT PARTY_CODE  FROM      
TBL_COMBINE_REPORTING_DETAIL  T (NOLOCK)      
WHERE MARGINDATE BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59'   AND  (TDAY_MARGIN <> 0 OR TDAY_MTM <> 0 ) AND EXCHANGE IN('NSE', 'NSX')  )A     
WHERE [PARTY_CODE] NOT IN (SELECT PARTY_CODE FROM #TPARTY  ) AND PARTY_CODE <> '12798'     
------------------------ END   -- FOR NSX MTM NOT COMING   
...
CASH = SUM(CASE WHEN DRCR = 'C' THEN -VAMT ELSE VAMT END),                    
 ...
 INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                                                          
 ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                                 
  FROM ANAND.ACCOUNT_AB.DBO.LEDGER L WITH (NOLOCK), #TPARTY C, ANAND.ACCOUNT_AB.DBO.PARAMETER P                                                           WHERE L.CLTCODE = C.PARTY_CODE AND L.EDT BETWEEN P.SDTCUR AND P.LDTCUR                              
...
BEGIN                                           
 ...
 --ADDED FOR NOT COMING OF PLED VALUE N DMS    
    ...
    SELECT DISTINCT D.PARTY_CODE   Into #MArClient  
    FROM ANGELFO.NSEFO.DBO.TBL_CLIENTMARGIN D WITH (NOLOCK)      
    LEFT OUTER JOIN #FOFDBG1 F      
    ON (F.PARTY_CODE = D.PARTY_CODE),      
    #TPARTY C                                                            
  ...
  INSERT INTO #MAR_REPORT         
  SELECT EXCHANGE='NSE', SEGMENT='FUTURES', TBL_COMBINE_REPORTING_DETAIL.PARTY_CODE, CL_TYPE='',                                                               CASH = 0,                                                            
  NONCASH = 0,                                   
  FD_BG = 0,                                                            
  INIT_MARGIN = CONVERT(NUMERIC(18,4),0),                                             
  EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                                                            
  ...
  MAR_PLEDGE_AMT = SUM(TDAY_NONCASH),      
  DEL_MARGIN = 0      
 FROM TBL_COMBINE_REPORTING_DETAIL  WITH (NOLOCK)  ,      
    #TPARTY C                           
 WHERE MARGINDATE BETWEEN  @COLLDATE AND  @COLLDATE + ' 23:59:59' --AND PARTY_CODE BETWEEN 'A1001024' AND 'A1001024'      
 AND EXCHANGE = 'NSE' AND SEGMENT = 'FUTURES' AND TBL_COMBINE_REPORTING_DETAIL.PARTY_CODE     
   NOT IN (    Select PARTY_CODE from #MArClient
   )      
 AND TBL_COMBINE_REPORTING_DETAIL.PARTY_CODE = C.PARTY_CODE     
 AND C.PARTY_CODE NOT IN (SELECT PARTY_CODE FROM #MAR_REPORT WITH (NOLOCK) WHERE EXCHANGE = 'NSE' AND SEGMENT = 'FUTURES' AND MAR_PLEDGE_AMT <>0)    
 GROUP BY TBL_COMBINE_REPORTING_DETAIL.PARTY_CODE    
