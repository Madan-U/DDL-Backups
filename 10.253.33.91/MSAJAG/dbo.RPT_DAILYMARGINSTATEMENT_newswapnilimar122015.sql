-- Object: PROCEDURE dbo.RPT_DAILYMARGINSTATEMENT_newswapnilimar122015
...
CREATE PROC [dbo].[RPT_DAILYMARGINSTATEMENT]                                        
 ...
 EXEC RPT_DAILYMARGINSTATEMENT 'APR 13 2010', '0A141','0A141','','ZZZZZZZZZ','','ZZZZZZZZZ','BROKER','BROKER',1,0,'C',1                                        
 */                           
...
FROM #COLL_DETAIL T, ANGELFO.NSECURFO.DBO.TBL_COLLATERAL_MARGIN T1 WITH (NOLOCK)                                        
WHERE                                         
T.PARTY_CODE=T1.PARTY_CODE                                        
...
UPDATE #FINAL_REPORT
SET NONCASH_AMT = ISNULL(S.AMT,0)
FROM (
		SELECT 
			AMT = SUM(SEC_FAMOUNT),
			PARTY_CODE
...
WHERE					
	S.PARTY_CODE = #FINAL_REPORT.PARTY_CODE
	AND #FINAL_REPORT.EXCHANGE='NSX' AND #FINAL_REPORT.SEGMENT='FUTURES'
	AND RPT_ORD = '1DET'
