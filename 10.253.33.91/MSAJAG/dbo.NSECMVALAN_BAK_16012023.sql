-- Object: PROCEDURE dbo.NSECMVALAN_BAK_16012023
...
Create PROC [dbo].[NSECMVALAN_BAK_16012023] (                                
@SETT_NO	VARCHAR(7),                                           
@SETT_TYPE	VARCHAR(2),                                
@SDATE		VARCHAR(11),                                
...
SERVICE_CHRG,BRANCH_CD,CL_TYPE, DUMMY8, DUMMY9, DUMMY10, REGION, AREA OPTION(QUERYTRACEON 8649)                          
    
                                
INSERT INTO PROCESS_MONITOR 
VALUES ('VALAN GENERATION',@SETT_NO_L + @SETT_TYPE_L,'','NSECMVALAN','DONE-INSERT NORMAL TRADE FROM SETTLEMENT_VALAN W/O ND SCRIP. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'INSERT INSTITUTIONAL TRADE FROM ISETTLEMENT_VALAN',GETDATE(),GETDATE())
...
ELSE (PTRDRATE) * @TRDBROKER_CHRG / 100              
 ...
 ----jul 19 2020 changes 
  ...
  if convert(varchar(11),@SDATE) >= Convert(varchar(11),'Jul  1 2020')
	--print 'punit'
	--print @SDATE
	begin
	--print 'vin'
	----select 11, * from   #CMBILLVALAN    where  party_code='DIYD36438'
	----select *
 ----from #CMBILLVALAN where sauda_Date >='2020-10-16'  and party_code='DIYD36438'
 ----select  * from TBL_STAMP_DATA where sauda_Date >='2020-10-16'  and party_code='DIYD36438' 


		UPDATE #CMBILLVALAN SET BROKER_CHRG = BUYTOTALSTAMP + SELLTOTALSTAMP,
		PBROKTRD = 0, PBROKDEL =  PBROKTRD + PBROKDEL - BUYTOTALSTAMP,
		SBROKTRD = 0, SBROKDEL = SBROKTRD + SBROKDEL - SELLTOTALSTAMP
		FROM (SELECT CONTRACTNO, PARTY_CODE, SCRIP_CD, SERIES, TRADE_TYPE,
			  BUYTOTALSTAMP = CONVERT(NUMERIC(18,0),SUM(BUYSQSTAMP+BUYDELSTAMP)),		  
			  SELLTOTALSTAMP = CONVERT(NUMERIC(18,0),SUM(SELLSQSTAMP+SELLDELSTAMP)) 
			  FROM MSAJAG.DBO.TBL_STAMP_DATA 
			  WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE 
			  GROUP BY CONTRACTNO, PARTY_CODE, SCRIP_CD, SERIES, TRADE_TYPE) A
		WHERE A.PARTY_CODE = #CMBILLVALAN.PARTY_CODE
		AND A.SCRIP_CD = #CMBILLVALAN.SCRIP_CD 
		AND A.SERIES = #CMBILLVALAN.SERIES  
		AND A.CONTRACTNO = #CMBILLVALAN.CONTRACTNO  
		AND #CMBILLVALAN.TRADETYPE = TRADE_TYPE 
		AND #CMBILLVALAN.BROKER_CHRG = 0 	  
...
INSERT INTO PROCESS_MONITOR 
VALUES ('VALAN GENERATION',@SETT_NO_L + @SETT_TYPE_L,'','NSECMVALAN','DONE-INSERT INSTITUTIONAL TRADE FROM ISETTLEMENT_VALAN. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'UPDATE SCRIPNAME/SECTOR/TRACK FROM SCRIP1 & 2',GETDATE(),GETDATE())   
       
 ----NEWLY ADDED 01012021
  UPDATE CMBILLVALAN
  SET  CMBILLVALAN.TURN_TAX = TBL_TOT_DETAIL.TURN_TAX 
  FROM CMBILLVALAN, TBL_TOT_DETAIL 
  WHERE  CMBILLVALAN.SETT_NO = @SETT_NO      
	AND CMBILLVALAN.SETT_NO = TBL_TOT_DETAIL.SETT_NO 
        AND CMBILLVALAN.SETT_TYPE = @SETT_TYPE      
	AND CMBILLVALAN.SETT_TYPE = TBL_TOT_DETAIL.SETT_TYPE
        AND TRADETYPE = 'I'
	AND CMBILLVALAN.CONTRACTNO = TBL_TOT_DETAIL.CONTRACTNO 
	AND CMBILLVALAN.SCRIP_CD = TBL_TOT_DETAIL.SCRIP_CD 
	AND CMBILLVALAN.PARTY_CODE = TBL_TOT_DETAIL.PARTY_CODE   
...
UPDATE #CMBILLVALAN SET SCRIP_NAME = S1.SHORT_NAME/*, SECTOR = S2.SECTOR, TRACK = S2.TRACK*/ FROM SCRIP1 S1, SCRIP2 S2                                 
...
FROM #CMBILLVALAN OPTION(MAXDOP 8)                             
