-- Object: PROCEDURE dbo.Proc_NewBasicReport_Parent_client_test
...
CREATE  Procedure [dbo].[Proc_NewBasicReport_Parent_client_test] (   
...
Begin  
          ...
          If @consol = "sub_broker"  
             Set  @@myorder = " Order  By c.Sub_broker,C.party_code, S.scrip_cd, S.series Option (fast 1 )"  
End  
...
Begin  
 ...
 Set @@fromtable = " From Cmbillvalan S , #PARTYVIEW C (NOLOCK) "  
End  
Else  
Begin  
...
Begin     If @consol = "region"  
     Begin  
          Set  @@mygroup = " Group By Region,S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,C.Branch_CD,C.SUB_BROKER,C.party_code,PARTY_NAME,membertype, Companyname,scrip_cd,series,scrip_name "  
          Set  @@selectflex =  " Select Region,S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BranchName=C.Branch_CD,SubBrokerName =C.SUB_BROKER,C.party_code,PARTY_NAME,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) "   
     End  
     If @consol = "area"  
     Begin  
          Set  @@mygroup = " Group By Area,S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,C.Branch_CD,C.SUB_BROKER,C.party_code,PARTY_NAME,membertype, Companyname,scrip_cd,series,scrip_name "  
          Set  @@selectflex =  " Select Area,S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BranchName=C.Branch_CD,SubBrokerName = C.SUB_BROKER,C.party_code,PARTY_NAME,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) "   
     End  
  
     If @consol = "party_code"  
     Begin   
          Set  @@mygroup = " Group By S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,C.Branch_CD,C.SUB_BROKER,C.Party_code,PARTY_NAME,membertype, Companyname,scrip_cd,series,scrip_name "  
          Set  @@selectflex =  " Select S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BranchName=C.Branch_CD,SubBrokerName = C.SUB_BROKER,C.Party_code, PARTY_NAME,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd) ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),    Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt= Sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) "   
     End   
     If @consol = "branch_cd"  
     Begin  
          Set  @@mygroup = " Group By S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,C.Branch_CD,C.SUB_BROKER,C.party_code,PARTY_NAME,membertype, Companyname,scrip_cd,series,scrip_name "  
          Set  @@selectflex =  " Select S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BranchName=BR.Branch,SubBrokerName = C.SUB_BROKER,C.party_code,PARTY_NAME,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) "   
     End  
     If @consol = "family"  
     Begin  
          Set  @@mygroup = " Group By S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BR.Branch,C.SUB_BROKER,C.party_code,PARTY_NAME,membertype, Companyname,scrip_cd,series,scrip_name "  
          Set  @@selectflex =  " Select S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BranchName=BR.Branch,SubBrokerName = C.SUB_BROKER,C.party_code,PARTY_NAME,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) "   
     End  
     If @consol = "trader"  
     Begin  
     ...
     If @consol = "sub_broker"  
     Begin  
          Set  @@mygroup = " Group By S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,C.party_code,c.BRANCH_CD,C.SUB_BROKER,PARTY_NAME,membertype, Companyname,scrip_cd,series,scrip_name "  
          Set  @@selectflex =  " Select S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BranchName=c.BRANCH_CD,SubBrokerName = C.Sub_broker,C.party_code,PARTY_NAME,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) "   
     End  
End  
...
Set @@wheretext =  @@wheretext + " AND C.BRANCH_CD = S.BRANCH_CD " 
Set @@wheretext =  @@wheretext + " AND c.SUB_BROKER = '"+ @statusname +"' "
...
Begin  
 Set @@wheretext =  @@wheretext + " And C.Sub_broker Between '" + @statusname  + "' And '" + @statusname   +"'  "   
End  
