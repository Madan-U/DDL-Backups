-- Object: PROCEDURE dbo.RPT_PNL_SHRT_LONG_DEALER
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



--dbo.RPT_PNL_SHRT_LONG_DEALER @STATUSID='DEALER',@STATUSNAME='SUB001',@FROMDATE='APR  1 2015',@TODATE='MAR 31 2018',@FROMPARTY='A100035',@TOPARTY='A100035',@BRANCH_CD='',@CHARGES='Y',@RPTOPT='S',@RPTOPENSELL='W',@SINGLE='N'

                              
---EXEC RPT_PNL_SHRT_LONG 'BROKER','BROKER','APR  1 2015','MAR 31 2016','P72652','P72652','','Y', 'S','W'--944                                        
                                          
CREATE PROC [dbo].[RPT_PNL_SHRT_LONG_DEALER]                                                                        
(                                                                          
 @STATUSID VARCHAR(25),                                                                          
...
REGION  = CONVERT(VARCHAR(20),'')                                        
 ...
 Alter Table #TBL_PNL_DATA ADD  DEALER VARCHAR(20)               
 ...
 CREATE CLUSTERED INDEX  [INX_PNL_DATA]                                               
  ...
  SELECT A.*,B.EMP_NO INTO #TBL_PNL_DATA_N             
  FROM TBL_PNL_DATA A WITH(NOLOCK),INTRANET.MIMANSA.[dbo].[Vw_B2B_CurrentClientDealerMapping] B  WITH(NOLOCK)           
  WHERE A.PARTY_CODE =B.PARTY_CODE
  AND A.PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY                      
...
INTO #CLIENT_DETAILS                                        
FROM                                              
 CLIENT_DETAILS C (NOLOCK) ,INTRANET.MIMANSA.[dbo].[Vw_B2B_CurrentClientDealerMapping]  D WITH (NOLOCK)                                         
                        
 /*INNER JOIN TBL_PNL_DATA T (NOLOCK)                                         
 ON (C.PARTY_CODE = T.PARTY_CODE)*/                                        
WHERE    C.PARTY_CODE = D.PARTY_CODE                                           
...
AND @STATUSNAME = (CASE                                                                                       
   ...
   WHEN @STATUSID = 'DEALER' THEN D.EMP_NO                                                                                      
 ...
 */                                        
 RECTYPE       
  ...
  --  SELECT * FROM #TBL_PNL_DATA WHERE Isin='INE062A01020'                                                                     
...
END                                          
 ...
 UPDATE #TBL_PNL_DATA SET CL_RATE = C.CL_RATE FROM ANAND.BSEDB_AB.DBO.CLOSING C WITH (NOLOCK)                                                                      
 WHERE SYSDATE BETWEEN @CL_DATE AND @CL_DATE + ' 23:59'                                        
 ...
 UPDATE #TBL_PNL_DATA SET CL_RATE = C.CL_RATE FROM ANAND.BSEDB_AB.DBO.CLOSING C WITH (NOLOCK)                                                                      
 WHERE SYSDATE = (SELECT MAX(SYSDATE) FROM ANAND.BSEDB_AB.DBO.CLOSING C1 WITH (NOLOCK)                                               
      WHERE C1.SCRIP_CD = C.SCRIP_CD                                                                                                   
      AND SYSDATE <= @TODATE + ' 23:59' )                                                               
 AND C.SCRIP_CD = #TBL_PNL_DATA.BSECODE                                                                                                      
 AND #TBL_PNL_DATA.CL_RATE = 0                                                                      
...
ISIN,                                                                  
 ...
 GROUP BY T.PARTY_CODE,/*SCRIP_CD/*,SERIES*/,BSECODE,*/ISIN,FLAG,SCRIP_NAME,BRANCH_CD,CL_RATE,DEALER ,                                                     
 LONG_NAME,                      
 L_ADDRESS1,                                                    
 L_ADDRESS2,                                                    
 L_ADDRESS3,                                                    
