-- Object: PROCEDURE dbo.CONTRACT_CONTROL_SHEET_18082015_bak
...
CREATE PROC [DBO].[CONTRACT_CONTROL_SHEET_18082015_bak]                      
(                        
  @FROMDATE VARCHAR(11),                        
  @TODATE VARCHAR(11),                        
...
BEGIN                                  
 SET @TODATE = @FROMDATE                                  
END                       
--EXEC CONTRACT_CONTROL_SHEET 'aug 18 2015', 'aug 18 2015', '00', 'ZZZZ', 'ALL', 'ALL', 'BROKER', 'BROKER', 'ALL'                       
...
FROM                      
 ...
 [196.1.115.201].BSEDB_AB.DBO.BRANCHGROUP C (NOLOCK)            
 WHERE                      
...
SELECT DISTINCT CL_CODE into #CL          
 FROM CLIENT_BROK_DETAILS (NOLOCK) WHERE PRINT_OPTIONS = 0 AND EXCHANGE NOT IN ('MCX','NCX') AND CL_CODE IN          
...
SELECT DISTINCT PARTY_CODE INTO #RPT          
 ...
 WHERE SAUDA_DATE LIKE @FROMDATE +'%'          
  ...
  ---          
  INSERT INTO [#CON]          
  SELECT CL_CODE,long_name as PARTYNAME,BRANCH_CD,SUB_BROKER,BRANCH_GROUP  FROM CLIENT_DETAILS AS A,[196.1.115.201].BSEDB_AB.DBO.BRANCHGROUP C (NOLOCK)  WHERE CL_CODE IN          
