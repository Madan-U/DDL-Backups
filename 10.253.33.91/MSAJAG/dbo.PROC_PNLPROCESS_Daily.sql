-- Object: PROCEDURE dbo.PROC_PNLPROCESS_Daily
...
CREATE PROC [dbo].[PROC_PNLPROCESS_Daily]                                          
(                                                                      
...
SELECT DISTINCT PARTY_CODE INTO #PARTY FROM  SETTLEMENT  WITH(NOLOCK)  WHERE    SAUDA_DATE LIKE @SAUDA_DATE + '%'                                                   
AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY      AND SELL_BUY = 2                              

INSERT INTO  #PARTY
SELECT DISTINCT PARTY_CODE FROM AngelBSECM.BSEDB_AB.DBO.SETTLEMENT  WITH(NOLOCK)   WHERE    SAUDA_DATE LIKE @SAUDA_DATE + '%'                                                   
AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY       AND SELL_BUY = 2     
...
FROM TBL_PNL_DATA T    
WHERE SETT_TYPE IN ('A','X','AC','AD') AND RECTYPE = 'OPEN'
...
INSERT INTO TBL_PNL_DATA_TMP                                                                
  ...
  ORGDATE=LEFT(SAUDA_DATE,11),                                                                  
   0, 1, RECTYPE = 'OPEN'                                                           
 FROM  AngelBSECM.BSEDB_AB.DBO.SETTLEMENT  WITH(NOLOCK)                                
   -- FROM DUSTBIN.DBO.SETTLEMENT_BSE_18102022 (Nolock)                                
   /*                       
...
INTO #CHARGES_DETAIL FROM AngelBSECM.BSEDB_AB.DBO.CHARGES_DETAIL  WHERE CD_SAUDA_DATE >= @SAUDA_DATE  AND CD_SAUDA_DATE <= @SAUDA_DATE +' 23:59:59'
-- AND CD_PARTY_CODE >= @FROMPARTY AND CD_PARTY_CODE<= @TOPARTY                                              
...
WHERE CD_SAUDA_DATE >= @SAUDA_DATE  AND CD_SAUDA_DATE <= @SAUDA_DATE +' 23:59:59'      --    AND CD_PARTY_CODE >= @FROMPARTY AND CD_PARTY_CODE<= @TOPARTY                                        
GROUP BY CD_SETT_NO, CD_SETT_TYPE, CD_SAUDA_DATE, CD_PARTY_CODE, CD_SCRIP_CD, CD_SERIES                                                
...
SELECT   SNO, PARTY_CODE, ISIN, TRADEQTY, ORGDATE1=LEFT(ORGDATE,11), ORGDATE, EXCHANGE  INTO #INTRADAY                                                    
							...
							AND SAUDA_DATE >= @SAUDA_DATE AND SAUDA_DATE <= @SAUDA_DATE +' 23:59:59'
