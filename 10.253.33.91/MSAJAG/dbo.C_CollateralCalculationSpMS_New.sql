...
SET @APPRSEC = 1                                                                        
 ...
 AND TRTYPE in (904, 905, 909, 1002) /*Change to remove invoke Holding 17072023 */
 --AND TRTYPE in ( 905, 909, 1002)  
 AND DRCR = 'D'                                                                                 
 AND DELIVERED = '0'                                       
 AND FILLER2 = 1                                                                                
 AND SHARETYPE = 'DEMAT'                                 
 AND EXCHANGE = @EXCHANGE                                                                                
 AND SEGMENT = @SEGMENT        AND BranchCd <> 'INVOKE'                                                                              
 AND ACCOUNTTYPE = 'MAR'                  AND dpcltno <> '1203320051669051'                                                    
 and Party_Code Between @FromParty And @ToParty                                                                                 
 ...
 AND EXCHANGE = @EXCHANGE                                                                                
 AND SEGMENT = @SEGMENT                                   
 AND ACCOUNTTYPE = 'MAR'     AND BranchCd <> 'INVOKE'    AND dpcltno <> '1203320051669051'                                                                                
 and Party_Code Between @FromParty And @ToParty                                                                                 
 GROUP BY PARTY_CODE, SCRIP_CD, SERIES, CERTNO                                                               
 ...
 AND SEGMENT = @SEGMENT                                  
 AND ACCOUNTTYPE = 'MAR'                                                                            
 AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY       AND dpcltno <> '1203320051669051'                                                                          
 GROUP BY PARTY_CODE, SCRIP_CD, SERIES, CERTNO                                             
...
FROM    INSTRUTYPEMST                                                 
    ...
    (SELECT MAX(SYSDATE)                                                                        
      ...
      WHERE   EXCHANGE    = @EXCHANGE                                                                        
                ...
                AND SYSDATE    >= CONVERT(DATETIME,@EFFDATE) - 7 
...
AND #C_CALCULATESECVIEW.PARTY_CODE = ''                             
    ...
    END                                                
        ...
        UPDATE  #C_CALCULATESECVIEW SET SECHAIRCUT=10.00
   WHERE  #C_CALCULATESECVIEW.SERIES='MF'                                                                
   AND (#C_CALCULATESECVIEW.SECHAIRCUT =0 OR #C_CALCULATESECVIEW.SECHAIRCUT=100)
 ...
 AND V.SERIES NOT  IN ('EQ', 'BE')                                                                        
  ...
  SELECT   RECTYPE=1,SCRIP_CD,SERIES='BSE',ISIN,SECVAR=MARGIN,INDEXVAR=MARGIN,APPVAR=MARGIN,                                                        
   ...
   FROM    AngelBSECM.bsedb_ab.dbo.VARDETAIL V  (nolock)                                                                          
   WHERE   V.FDATE   =                                                                            
     (SELECT MAX(FDATE)                                                        
     FROM     AngelBSECM.bsedb_ab.dbo.VARDETAIL V1 (nolock)                                                                         
 ...
 UPDATE #C_CALCULATESECVIEW                                                                        
  ...
  FROM    #VAR                                                                        
    ...
    WHERE   #C_CALCULATESECVIEW.SCRIP_CD = A.SCRIP_CD                                                                        
		...
		/* MF VALue */
   UPDATE #C_CALCULATESECVIEW                                                            
    SET     SECHAIRCUT = (CASE WHEN VARMARGINRATE <10 THEN SECHAIRCUT ELSE    VARMARGINRATE END   )                                                                  
    FROM                                                                        
            (SELECT SCRIP_CD,                                                                        
                    SERIES ,                                                                        
                    VARMARGINRATE = VARMARGINRATE                                
...
FDAMOUNT MONEY          ,                                                                        
 ...
 CASH_COMPO                                                                        
   ...
   THEN                                                                        
    ...
    DELETE                                                                        
    FROM    COLLATERAL                       
    WHERE   TRANS_DATE BETWEEN @EFFDATE AND @EFFDATE  +' 23:59'                                                                        
			AND EXCHANGE = @EXCHANGE                                    
			AND SEGMENT  = @SEGMENT                                                                        
			AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY                                                
    ...
    INSERT                                                                        
    INTO    COLLATERAL SELECT EXCHANGE = @EXCHANGE   ,                    
            SEGMENT                    = @SEGMENT    ,                                                   
