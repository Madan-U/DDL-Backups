-- Object: PROCEDURE dbo.PROC_MARING_REPORT_NEW_02082014
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

        
CREATE PROC [dbo].[PROC_MARING_REPORT_NEW_02082014]                    
(                              
...
--BEGIN                    
 ...
 INTO #MAR_REPORT                              
 FROM ACCOUNT.DBO.LEDGER L, CLIENT_DETAILS C, ACCOUNT.DBO.PARAMETER P                              
 WHERE L.CLTCODE = C.CL_CODE AND L.EDT BETWEEN P.SDTCUR AND P.LDTCUR                               
...
--BEGIN                    
 ...
 INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                              
 ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                               
 FROM ANAND.ACCOUNT_AB.DBO.LEDGER L, CLIENT_DETAILS C, ANAND.ACCOUNT_AB.DBO.PARAMETER P                              
...
--BEGIN                    
 ...
 INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                              
 ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                               
 FROM ANGELCOMMODITY.ACCOUNTBFO.DBO.LEDGER L, CLIENT_DETAILS C, ANGELCOMMODITY.ACCOUNTBFO.DBO.PARAMETER P                              
 WHERE L.CLTCODE = C.CL_CODE AND L.VDT BETWEEN P.SDTCUR AND P.LDTCUR                               
 AND @SAUDA_DATE BETWEEN P.SDTCUR AND P.LDTCUR                               
 AND EDT <= @SAUDA_DATE + ' 23:59'                      
 --AND EDT <= (CASE WHEN DRCR = 'D' THEN @SAUDA_DATE + ' 23:59' ELSE EDT END)                              
...
--BEGIN                    
 ...
 INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                              
 ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                               
 FROM ANGELFO.ACCOUNTFO.DBO.LEDGER L, CLIENT_DETAILS C, ANGELFO.ACCOUNTFO.DBO.PARAMETER P                              
 WHERE L.CLTCODE = C.CL_CODE AND L.VDT BETWEEN P.SDTCUR AND P.LDTCUR                               
 AND @SAUDA_DATE BETWEEN P.SDTCUR AND P.LDTCUR                               
 AND VDT <= @SAUDA_DATE + ' 23:59'                      
 AND EDT <= (CASE WHEN DRCR = 'D' THEN @SAUDA_DATE + ' 23:59' ELSE EDT END)                              
...
--BEGIN                    
 ...
 INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                              
 ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                               
 FROM ANGELFO.ACCOUNTCURFO.DBO.LEDGER L, CLIENT_DETAILS C, ANGELFO.ACCOUNTCURFO.DBO.PARAMETER P                              
 WHERE L.CLTCODE = C.CL_CODE AND L.VDT BETWEEN P.SDTCUR AND P.LDTCUR                               
 AND @SAUDA_DATE BETWEEN P.SDTCUR AND P.LDTCUR                               
 AND VDT <= @SAUDA_DATE + ' 23:59'                      
 AND EDT <= (CASE WHEN DRCR = 'D' THEN @SAUDA_DATE + ' 23:59' ELSE EDT END)                              
...
--BEGIN                    
 ...
 INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                              
 ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                               
 FROM ANGELCOMMODITY.ACCOUNTMCDXCDS.DBO.LEDGER L, CLIENT_DETAILS C, ANGELCOMMODITY.ACCOUNTMCDXCDS.DBO.PARAMETER P                              
 WHERE L.CLTCODE = C.CL_CODE AND L.VDT BETWEEN P.SDTCUR AND P.LDTCUR                               
...
INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                              
ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                                
FROM COLLATERALDETAILS D, CLIENT_DETAILS C                              
...
ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                                
FROM ANGELFO.NSEFO.DBO.TBL_CLIENTMARGIN D, CLIENT_DETAILS C                              
...
EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                              
ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                                
FROM ANGELCOMMODITY.MCDXCDS.DBO.TBL_CLIENTMARGIN D, CLIENT_DETAILS C                              
WHERE MARGINDATE BETWEEN  @COLLDATE AND @COLLDATE  +' 23:59'             
AND D.PARTY_CODE = C.CL_CODE                              
GROUP BY D.PARTY_CODE, CL_TYPE              
...
--BEGIN                    
 INSERT INTO #MAR_REPORT                              
 SELECT EXCHANGE = 'BSE', SEGMENT = 'FUTURES', M.PARTY_CODE, C.CL_TYPE,                              
 CASH = 0, NONCASH = 0, FD_BG = 0,                               
 INIT_MARGIN = SUM(SPAN_MARGIN), EXP_MARGIN = SUM(EXTREME_LOSS_MARGIN),                              
 ADD_MARGIN = 0                              
 FROM ANGELCOMMODITY.BSEFO.DBO.BFOMARGIN M, CLIENT_DETAILS C                              
...
--BEGIN                    
 INSERT INTO #MAR_REPORT                              
 SELECT EXCHANGE = 'NSE', SEGMENT = 'FUTURES', M.PARTY_CODE, C.CL_TYPE,                              
 CASH = 0, NONCASH = 0, FD_BG = 0,                               
 INIT_MARGIN = SUM(PSPANMARGIN)+SUM(NONSPREADMARGIN), EXP_MARGIN = SUM(MTOM),                              
 ADD_MARGIN = 0                     
 FROM ANGELFO.NSEFO.DBO.FOMARGINNEW M, CLIENT_DETAILS C                              
...
--BEGIN                    
 INSERT INTO #MAR_REPORT                    
 SELECT EXCHANGE = 'NSX', SEGMENT = 'FUTURES', M.PARTY_CODE, C.CL_TYPE,                              
 CASH = 0, NONCASH = 0, FD_BG = 0,                               
 INIT_MARGIN = SUM(PSPANMARGIN)+SUM(NONSPREADMARGIN), EXP_MARGIN = SUM(MTOM),                              
 ADD_MARGIN = 0                              
 FROM ANGELFO.NSECURFO.DBO.FOMARGINNEW M, CLIENT_DETAILS C                              
 WHERE M.MDATE BETWEEN  @MARGINDATE AND @MARGINDATE  +' 23:59'                              
 AND M.PARTY_CODE = C.PARTY_CODE                              
 GROUP BY M.PARTY_CODE, C.CL_TYPE                      
--END                     
...
--BEGIN                    
 INSERT INTO #MAR_REPORT                              
 SELECT EXCHANGE = 'MCD', SEGMENT = 'FUTURES', M.PARTY_CODE, C.CL_TYPE,                              
 CASH = 0, NONCASH = 0, FD_BG = 0,                               
-- INIT_MARGIN = SUM(TOTALMARGIN)+SUM(NONSPREADMARGIN), EXP_MARGIN = SUM(MTOM),                              
 INIT_MARGIN = SUM(TOTALMARGIN), EXP_MARGIN = SUM(MTOM),                              
 ADD_MARGIN = 0                              
 FROM ANGELCOMMODITY.MCDXCDS.DBO.FOMARGINNEW M, CLIENT_DETAILS C                              
 WHERE M.MDATE BETWEEN  @MARGINDATE AND @MARGINDATE  +' 23:59'                              
 AND M.PARTY_CODE = C.PARTY_CODE                              
 GROUP BY M.PARTY_CODE, C.CL_TYPE                    
--END                     
...
FROM                    
 ...
 FROM TBL_MG02 M, CLIENT_DETAILS C                              
 WHERE M.MARGIN_DATE BETWEEN  @MARGINDATE AND @MARGINDATE  +' 23:59'                              
 AND M.PARTY_CODE = C.PARTY_CODE                              
 GROUP BY                    
  M.PARTY_CODE,                    
...
--  MARGIN = SUM(VARAMT + ELM) - (CASE WHEN SUM(MTOM) > 0 THEN 0 ELSE SUM(MTOM) END)                    
 ...
 FROM ANAND.BSEDB_AB.DBO.TBL_MG02 M, CLIENT_DETAILS C                              
 WHERE M.MARGIN_DATE BETWEEN  @MARGINDATE AND @MARGINDATE  +' 23:59'                               
 AND M.PARTY_CODE = C.PARTY_CODE                              
 GROUP BY                    
  M.PARTY_CODE,                    
...
SELECT PARTY_CODE,CL_TYPE,TRADE_DAY=@SAUDA_DATE,EXCHANGE,SEGMENT,                              
CASH=SUM(CASH),                              
NONCASH=SUM(NONCASH),                              
FD_BG=SUM(FD_BG),                              
ANY_OTHER_COLL=0,                              
