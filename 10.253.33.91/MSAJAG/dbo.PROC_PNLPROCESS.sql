-- Object: PROCEDURE dbo.PROC_PNLPROCESS
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

                  
                  
CREATE PROC [dbo].[PROC_PNLPROCESS]              
(                                          
...
INSERT INTO TBL_PNL_DATA_TMP                                          
  ...
  FROM   BSEDB_AB_HISTORY010411TO180711.dbo.NSEHISTORY_010711_To_310811 WITH(NOLOCK)    
  ---FROM   BSEDB_AB_HISTORY010411TO180711.dbo.NSEHISTORY WITH(NOLOCK)     
    
   /* select * into BSEDB_AB_HISTORY010411TO180711.dbo.NSEHISTORY_010711_To_310811    
  From  MSAJAG.DBO.HISTORY   
  where Sauda_date between '2011-07-01 00:00:00.000' and '2011-08-31 23:59:59.000'  
  Create clustered index IX_CLParty_code on BSEDB_AB_HISTORY010411TO180711.dbo.NSEHISTORY_010711_To_310811   
...
INSERT INTO TBL_PNL_DATA_TMP                                    
  ...
  ORGDATE=LEFT(SAUDA_DATE,11),                                      
   0, 1, RECTYPE = (CASE WHEN SETTFLAG IN (2,3) THEN 'SPEC' ELSE 'OPEN' END)                                
  FROM   BSEDB_AB_HISTORY010411TO180711.dbo.HISTORY_010711_To_310811 WITH(NOLOCK)      
    
  /* select * into BSEDB_AB_HISTORY010411TO180711.dbo.HISTORY_010711_To_310811   
  From  [AngelBSECM].BSEDB_AB.DBO.HISTORY   
  where Sauda_date between '2011-07-01 00:00:00.000' and '2011-08-31 23:59:59.000'  
  Create clustered index IX_Sauda_date on BSEDB_AB_HISTORY010411TO180711.dbo.HISTORY_010711_To_310811   
  (Party_Code, Sauda_date)  
  */  
...
INTO #CHARGES_DETAIL FROM AngelBSECM.BSEDB_AB.DBO.CHARGES_DETAIL  WHERE CD_SAUDA_DATE LIKE @SAUDA_DATE + '%'                    
...
RECTYPE = (CASE WHEN ABS(DATEDIFF(D,OPPDATE,SAUDA_DATE)) > 365 AND OPPDATE <> 'DEC 31 2049'       
				THEN 'LONG TERM'       
				WHEN ABS(DATEDIFF(D,OPPDATE,SAUDA_DATE)) <= 365 AND ABS(DATEDIFF(D,OPPDATE,SAUDA_DATE)) <> 0
				THEN 'SHORT TERM'       
				WHEN ABS(DATEDIFF(D,OPPDATE,SAUDA_DATE)) = 0    
				THEN 'SPECULATION'      
				ELSE 'OPEN'      
			END)                         
