...
Select SNo,D.Party_Code,Scrip_CD,D.Series,Qty,TCode,TrDate=Left(Convert(Varchar,TrDate,109),11) From DematTrans D
...
Begin	
	...
	Begin
		/* Taking Party's Recevied Position i.e. How much Qty he had Given against Position*/
...
/*	
		Set @@SettDelTrans = Cursor For
		Select IsNull(Sum(Qty),0) From DelTrans Where ISett_No = @@Sett_No
		And ISett_Type = @@Sett_Type And Party_Code = @@Party_Code And RefNo = @RefNo
		And Scrip_Cd = @@Scrip_Cd And Series = @@Series And DrCr = 'D' And Filler2 = 1 
		Open @@SettDelTrans
		Fetch Next From @@SettDelTrans into @@SettDelTransQty
		Close @@SettDelTrans
		DeAllocate @@SettDelTrans
