-- Object: PROCEDURE dbo.SPX_GETPOAUPDATONFROMKYC_REVISEDPROCESS_PartyCodeWise_OldDP
...
CREATE PROC [dbo].[SPX_GETPOAUPDATONFROMKYC_REVISEDPROCESS_PartyCodeWise_OldDP]                  
@UserId varchar(15)          
AS                  
                  
      
SELECT DISTINCT * INTO #SYNERGY_DATA_FINAL FROM [196.1.115.167].KYC.DBO.TBL_OLDDP_POAPROCESS       
WHERE ISNULL(Process_Flag,'N')='P' AND Process_By=@UserId        
and nise_party_code in (select Party_code from [196.1.115.167].KYC.DBO.TEMP_POADATA_OLDDP      
...
--WHERE ISNULL(NSESEG,'') NOT IN ('NO','N')       
AND ISNULL(b.NISE_PARTY_CODE,'')<>''      
...
--**********************************CODE START FOR CLIENT4 PROCESS **********************------------------------                    
 ...
 and DefDp='1' and Depository='nsdl' --AND ISNULL(NSESEG,'') NOT IN ('NO','N')       
 ...
 from client4 a where nise_party_code=Party_code        
 and DefDp='1' and Depository='CDSL' and isnull(nsdlflag,'')<>'y' AND ISNULL(NonAngelCDSLFlag,'')<>'y'  and BankID='12033200'        
  --AND ISNULL(NSESEG,'') NOT IN ('NO','N')       
 ...
 from client4 a where nise_party_code=Party_code        
 and DefDp='1' and Depository='CDSL' and isnull(nsdlflag,'')<>'y' AND ISNULL(NonAngelCDSLFlag,'')<>'y'  and BankID='12033200'      
  AND cdslflag='y' AND status='Active' --AND ISNULL(NSESEG,'') NOT IN ('NO','N')       
        
 -- SELECT * FROM #SYNERGY_DATA_FINAL WHERE CDSlStatusFlag='y'      
         
   -- need to delete other than active      
         
 SELECT NISE_PARTY_CODE,CLIENT_CODE,1 defdp,'CDSL' depository,NSESEG INTO #tempdeleteinsertfinal FROM #SYNERGY_DATA_FINAL,       
 (      
...
DELETE FROM client4 WHERE depositOry='CDSL' AND BankID='12033200' AND Party_code IN      
 (      
SELECT nise_parTY_CODE FROM  #tempdeleteinsertfinal --WHERE ISNULL(NSESEG,'') NOT IN ('NO','N')       
)      
...
FROM #tempdeleteinsertfinal --WHERE ISNULL(NSESEG,'') NOT IN ('NO','N')        
...
--AND ISNULL(NSESEG,'N') NOT in ('NO','N')       
 ...
 SELECT NISE_PARTY_CODE,CLIENT_CODE,1 defdp,'CDSL' depository,NSESEG INTO #tempinsertfinal_New  FROM #SYNERGY_DATA_FINAL, (      
...
--AND ISNULL(NSESEG,'N')='Y'     
AND DPID='12033200' AND DEF='0' AND ISNULL(MultiFlag,'')='Y' AND DpType='CDSL'     
...
FROM  #SYNERGY_DATA_FINAL A inner join [196.1.115.167].KYC.DBO.TBL_OLDDP_POAPROCESS N with(nolock)    
...
--DELETE FROM CLIENT4 WHERE DEPOSITORY = 'CDSL'                    
--AND LTRIM(RTRIM(PARTY_CODE))  IN                   
--(                  
--  SELECT LTRIM(RTRIM(NISE_PARTY_CODE)) FROM  #SYNERGY_DATA_FINAL       
--  WHERE ISNULL(NSESEG,'N')='Y'                   
--)  AND DEFDP=0        
...
--(SELECT COUNT(*) FROM CLIENT4 B1 WHERE B1.CL_CODE=NISE_PARTY_CODE AND B1.PARTY_CODE=NISE_PARTY_CODE AND           
--B1.DEFDP='1') AS COUNT1          
--FROM #SYNERGY_DATA_FINAL A LEFT OUTER JOIN CLIENT4 B WITH(NOLOCK)                
--ON A.NISE_PARTY_CODE=B.PARTY_CODE AND ISNULL(NSESEG,'N')='Y'            
----WHERE A.BSEDB_CLIENT4_197='N' AND A.NSESEG='Y' AND SNO='1' AND BOEXIST='Y'        
...
--(SELECT DISTINCT NISE_PARTY_CODE,CLIENT_CODE,'12033200' AS DP,'ANGEL BROKING LTD' AS INTRODUCER,'CDSL' AS DEPOSIT,'1' AS DEF,PARTY_CODE,DPTYPE ,            
--(SELECT COUNT(*) FROM MULTICLTID B1 WHERE ltrim(rtrim(B1.PARTY_CODE))=ltrim(rtrim(NISE_PARTY_CODE)) AND           
--'12033200'=ltrim(rtrim(B1.DpId)) AND ltrim(rtrim(CLIENT_CODE))=ltrim(rtrim(B1.CltDpNo)) AND 'CDSL'=ltrim(rtrim(B1.DPTYPE)) AND B1.DEF='1' ) AS COUNT1          
--FROM #SYNERGY_DATA_FINAL A LEFT OUTER JOIN  MULTICLTID B WITH(NOLOCK)                
--ON A.NISE_PARTY_CODE=B.PARTY_CODE AND ISNULL(NSESEG,'N')='Y'             
