-- Object: PROCEDURE dbo.NSESauda_Combined_Trade_TEST
...
-- Description:	
-- =============================================
CREATE Proc [dbo].[NSESauda_Combined_Trade_TEST]
(@fromdate datetime,
...
AS
 ...
 SELECT CL_CODE ,PARENTCODE  INTO #CLIENT2 FROM CLIENT2 WHERE PARENTCODE > =@from_party_code AND PARENTCODE < =@to_party_code
 ...
 CREATE INDEX #CL ON  #CLIENT2 (CL_CODE)
    ...
    (
        SELECT EXCHANGE = 'NSECM', 
               ...
               order_no
        FROM MSAJAG.DBO.SETTLEMENT S WITH(NOLOCK),MSAJAG.DBO.SETT_MST M,#CLIENT2 C
     ...
     --               END,  
		...
		UNION ALL
			   ...
			   other_chrg=SUM(other_chrg),
               order_no
        FROM MSAJAG.DBO.HISTORY S WITH(NOLOCK),MSAJAG.DBO.SETT_MST M,#CLIENT2 C
        WHERE AuctionPart NOT IN('ar', 'ap','fa')
		AND START_DATE >=@FROMDATE AND Start_date <=@TODATE +' 23:59'
		AND S.SETT_NO=M.SETT_NO AND S.SETT_TYPE =M.SETT_TYPE
		AND S.PARTY_CODE = C.Cl_Code
   ...
   --                 END,   
        ...
        SELECT EXCHANGE = 'NSECM', 
			   ...
			   other_chrg=0,
               CD_order_no
        FROM MSAJAG.DBO.CHARGES_DETAIL WITH(NOLOCK),#CLIENT2 C
		WHERE CD_Sauda_Date >=@fromdate AND CD_Sauda_Date <=@todate + ' 23:59'
		AND CD_Party_Code  =CL_CODE 
		GROUP BY PARENTCODE,
