-- Object: PROCEDURE dbo.SpSHAllBillSecAngel
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE  Proc SpSHAllBillSecAngel (@Sett_No varchar(10),@Sett_Type Varchar(2), @Party_code varchar(10)) as
...
NewfIXMARGIN = ( case when (client2.albmdelchrg = 0 or albmdelchrg = 2 ) then (Case when TMARK <> '$' then ISNULL(( SELECT fixmargin FROM MARGIN1 WHERE SETT_NO= s.SETT_NO AND SETT_TYPE =  s.SETT_TYPE ),0)  *SUM(TRADEQTY*MARKETRATE)/100 else 0 end ) else 0
 end) , 
...
NewFixmargin=0,
NewAddMar = 0,
OldfIXMARGIN = ( case when (client2.albmdelchrg = 0 or albmdelchrg = 2 ) then (Case when TMARK <> '$' then ISNULL(( SELECT fixmargin FROM MARGIN1 WHERE SETT_NO= s.SETT_NO AND SETT_TYPE =  s.SETT_TYPE ),0)  *SUM(TRADEQTY*MARKETRATE)/100 else 0 end ) else 0
 end) , 
...
NewfIXMARGIN = ( case when (client2.albmdelchrg = 0 or albmdelchrg = 2 ) then (Case when TMARK <> '$' then ISNULL(( SELECT fixmargin FROM MARGIN1 WHERE SETT_NO=  s.SETT_NO AND SETT_TYPE =  s.SETT_TYPE ),0)  *SUM(TRADEQTY*MARKETRATE)/100 else 0 end ) else 
0 end) , 
NewADDMAR =  ( case when (client2.albmdelchrg = 0 or albmdelchrg = 2 ) then ISNULL((Case when TMARK <> '$' then ISNULL(( SELECT AddMargin FROM MARGIN2 WHERE SETT_TYPE =  s.SETT_TYPE AND SCRIP_CD =  s.SCRIP_CD  and sett_no =  s.sett_no ),0)*SUM(TRADEQTY*MARKETRATE)/100 else 0 end ),0) else 0 end ),
OldFixmargin=0,
OldAddMar = 0,settlementflag='*'
...
GROUP BY   s.party_code,  s.billno,  s.scrip_cd,  s.series, scrip1.short_name,
substring(convert(VARchar,sauda_date,109),1,11),  s.sell_buy,  s.markettype,
n_netrate,client2.SERVICE_CHRG,SETT_NO,SETT_TYPE,TMARK,client2.albmdelchrg
...
NewFixmargin=0,
NewAddMar = 0,
 OldfIXMARGIN = ( case when (client2.albmdelchrg = 0 or albmdelchrg = 2 ) then (Case when TMARK <> '$' then ISNULL(( SELECT fixmargin FROM MARGIN1 WHERE SETT_NO=  s.SETT_NO AND SETT_TYPE =  s.SETT_TYPE ),0)  *SUM(TRADEQTY*MARKETRATE)/100 else 0 end ) else
 0 end) , 
 OldADDMAR =  ( case when (client2.albmdelchrg = 0 or albmdelchrg = 2 ) then ISNULL((Case when TMARK <> '$' then ISNULL(( SELECT AddMargin FROM MARGIN2 WHERE SETT_TYPE =  s.SETT_TYPE AND SCRIP_CD =  s.SCRIP_CD  and sett_no =  s.sett_no ),0)*SUM(TRADEQTY*MARKETRATE)/100 else 0 end ),0) else 0 end )
,settlementflag= '~'
...
GROUP BY   s.party_code,  s.billno,  s.scrip_cd,  s.series, scrip1.short_name,
substring(convert(VARchar,sauda_date,109),1,11),  s.sell_buy,  s.markettype,
n_netrate,client2.SERVICE_CHRG,  s.SETT_NO,  s.SETT_TYPE,TMARK,client2.albmdelchrg,Rate
/*ORDER BY scrip1.short_name,SELL_BUY*/
ORDER BY  s.scrip_cd,SELL_BUY
