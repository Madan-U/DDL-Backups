-- Object: PROCEDURE dbo.POP_COMMON_CONTRACTDATA_Ang
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

  
CREATE PROCEDURE [dbo].[POP_COMMON_CONTRACTDATA_Ang] (  
 @FROMDATE VARCHAR(11),  
 @USERNAME VARCHAR(20) = ''  
 )  
AS  
...
/*  
EXEC POP_COMMON_CONTRACTDATA 'JUL 10 2012','ABC'  
...
BEGIN  
 ...
 BEGIN  
  SET @STRSQL = 'IF (SELECT COUNT(1) FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.TABLES WHERE NAME=''CONTRACT_DATA_ANG'') > 0 '  
  ...
  SET @STRSQL = @STRSQL + ' ' + @SHARESERVER + '.' + @SHAREDB + '.DBO.CONTRACT_MASTER_ang WITH (NOLOCK) WHERE CONVERT(DATETIME,START_DATE,103) BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'' AND PARTY_CODE BETWEEN ''' + @FROMPARTY + ''' AND ''' + @TOPARTY + ''''  
  ...
  SET @STRSQL = @STRSQL + ' ' + @SHARESERVER + '.' + @SHAREDB + '.DBO.CONTRACT_DATA_ANG D WITH (NOLOCK),'  
  SET @STRSQL = @STRSQL + ' ' + @SHARESERVER + '.' + @SHAREDB + '.DBO.CONTRACT_MASTER_ang M WITH (NOLOCK),'  
  ...
  SET @STRSQL = @STRSQL + 'BEGIN '  
  SET @STRSQL = @STRSQL + 'IF (SELECT COUNT(1) FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.COLUMNS WHERE [NAME] = ''USER_ID'' AND [OBJECT_ID] IN (SELECT DISTINCT OBJECT_ID FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.TABLES WHERE NAME = ''CONTRACT_DATA_ANG'')) > 0 '  
  SET @STRSQL = @STRSQL + 'BEGIN '  
