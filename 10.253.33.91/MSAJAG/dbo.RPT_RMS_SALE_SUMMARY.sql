-- Object: PROCEDURE dbo.RPT_RMS_SALE_SUMMARY
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE PROC RPT_RMS_SALE_SUMMARY            
(                      
 @STATUSID VARCHAR(20),                    
...
AND (HOLD_AMT<>0 OR EXCESS_COLL <> 0)
...
NOOFDAYS,EXCESS_COLL=SUM(ABS(EXCESS_COLL)),                      
VDTBAL=SUM(VDTBAL),
SQ_OFF_BAL = CONVERT(NUMERIC(18,2),0),                    
...
UPDATE #TBL_RMS_SALE_LED_DET SET SQ_OFF_BAL = AMT                     
FROM (                    
  SELECT PARTY_CODE, AMT = SUM(SELLAMT)                    
  FROM TBL_RMS_SALE                    
