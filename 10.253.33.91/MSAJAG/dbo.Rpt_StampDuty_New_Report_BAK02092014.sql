-- Object: PROCEDURE dbo.Rpt_StampDuty_New_Report_BAK02092014
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE  Proc Rpt_StampDuty_New_Report_BAK02092014 (@Start_Date Varchar(11), @End_Date Varchar(11), @State Varchar(50))    
As    
...
Into #StampDuty From settlement(NOLOCK), Client2(NOLOCK), Client1(NOLOCK), sett_mst s2      (NOLOCK)     
...
from isettlement (NOLOCK) ,Client2(NOLOCK), Client1(NOLOCK), sett_mst s2(NOLOCK)           
...
Sdamt = sum(Case when sell_buy = 2 And BillFlag in (1,4,5) Then tradeqty * Marketrate Else 0 End),              
Totamt = Sum(tradeqty * marketrate), Cl_Type, L_State           
From History(NOLOCK), Client2(NOLOCK), Client1(NOLOCK), sett_mst s2 (NOLOCK)          
...
from IHistory(NOLOCK) ,Client2(NOLOCK), Client1(NOLOCK), sett_mst s2(NOLOCK)           
