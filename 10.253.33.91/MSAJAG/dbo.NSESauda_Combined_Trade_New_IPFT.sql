-- Object: PROCEDURE dbo.NSESauda_Combined_Trade_New_IPFT
...
-- Description:	
-- =============================================
CREATE Proc [dbo].[NSESauda_Combined_Trade_New_IPFT]
(@fromdate datetime,
 ...
 CREATE INDEX #CL ON  #CLIENT2 (CL_CODE)
    ...
    (
        SELECT EXCHANGE = 'NSECM', 
               ...
               SCRIP_CD = LTRIM(RTRIM(S.SCRIP_CD)), 
               S.Series, 
               SCRIPName = '', 
               BILLFLAG, 
               Trade_Type = CASE
			   ...
			   S.sett_type,
		      IPFT_CHRG = SUM(IPFT_CHRG)
        FROM MSAJAG.DBO.SETTLEMENT S WITH(NOLOCK)
		Left outer join  
		(
		Select Sauda_date as SDATE,Party_code as cl_code,Order_no as ordno ,Trade_no as trdno,IPFT_CHRG  ,
		SCRIP_CD
		from MSAJAG.DBO.TBL_ADDITIONAL_TAX_DATA  WITH(NOLOCK) 
		where Party_code>= @from_party_code and  Party_code<= @to_party_code and  SAUDA_DATE >= @FromDate AND SAUDA_DATE <= @ToDate 
		)R  
        ON S.Party_code=R.cl_code and S.order_no=R.ordno and S.trade_no=R.trdno  
		and s.SCRIP_CD=R.SCRIP_CD
		,MSAJAG.DBO.SETT_MST M 
		...
		UNION ALL
			   ...
			   S.sett_type,
		       IPFT_CHRG = SUM(IPFT_CHRG)
        FROM MSAJAG.DBO.HISTORY S WITH(NOLOCK)
		Left outer join  
		(
		Select Sauda_date as SDATE,Party_code as cl_code,Order_no as ordno ,Trade_no as trdno,IPFT_CHRG  ,
		SCRIP_CD
