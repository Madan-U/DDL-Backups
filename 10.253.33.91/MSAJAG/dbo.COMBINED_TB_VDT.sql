-- Object: PROCEDURE dbo.COMBINED_TB_VDT
...
--------------------------------------------------
 ...
 CREATE PROCEDURE COMBINED_TB_VDT      
 (      
 @FROMDATE VARCHAR(20)      
 )      
 AS       
...
UPDATE #FINAL SET SLBS =VAMT FROM #MTF A WHERE A.CLTCODE =#FINAL.CLTCODE       
 ...
 into #TEMP1      
   ...
   WHERE       
    ...
    ---AND CLTCODE IN ('ZZZU1002','ZZZW1012','ZZZW1044','ZZZW1042')      
     ...
     SELECT * FROM #TEMP1  WHERE   TOTAL<>0 and cltcode between 's0'and 'zzz999'      
      SELECT * FROM #TEMP1  WHERE   TOTAL<>0 and cltcode between 'n0'and 'zzz999'      
