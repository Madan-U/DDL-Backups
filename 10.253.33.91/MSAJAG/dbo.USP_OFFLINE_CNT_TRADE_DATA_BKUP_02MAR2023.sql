-- Object: PROCEDURE dbo.USP_OFFLINE_CNT_TRADE_DATA_BKUP_02MAR2023
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE PROCEDURE [dbo].[USP_OFFLINE_CNT_TRADE_DATA_BKUP_02MAR2023]  
(  
 @OnDate DATE   
)  
AS  
...
select * INTO #tbl_CNT_Omnesys_Final from tbl_CNT_Omnesys_Final WHERE FILE_DATE =@OnDate  
...
--------------------- PURGING DATA  
  IF OBJECT_ID('tempdb..#OFFLIne') IS NOT NULL  
    DROP TABLE #OFFLIne  
  Select * INTO #OFFLIne from #tbl_CNT_Omnesys_Final  
  WHERE   OrderSource ='TWS' AND (REMARKS ='' OR REMARKS IS NULL)   
...
------------------ Remove duplicate entries   
 ...
 -- LEFT OUTER JOIN dustbin.dbo.[tbl_CTCL_Terminal Data_2_Mar_22_temp]  A  WITH (NOLOCK) ON  A.OdinId =B.UserId  
  ...
  [Type] ,sub_broker  
   ...
   SELECT  * FROM tbl_CTCL_Terminal_OFFLINE_CNT_TRADE  where CTCLID <>ACC_ID
