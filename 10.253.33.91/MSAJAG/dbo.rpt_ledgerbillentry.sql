...
--sp_helptext rpt_ledgerbillentry '15','200604030003','19','C','A1485'
 ...
 /****** Object:  Stored Procedure dbo.rpt_ledgerbillentry    Script Date: 01/19/2002 12:15:15 ******/  
  
/****** Object:  Stored Procedure dbo.rpt_ledgerbillentry    Script Date: 01/04/1980 5:06:27 AM ******/  
...
SELECT BNKNAME, L.VTYP,L.VNO,L.LNO,L.DRCR, L.CLTCODE ,  
 narr=isnull((case when (select top 1 l3.narr from account.dbo.ledger3 l3  where L.VTYP = L3.VTYP AND L.VNO = L3.VNO and l.booktype = l3.booktype AND l.lno = l3.naratno) is  not null  
                 then (select top 1 l3.narr from account.dbo.ledger3 l3  where L.VTYP = L3.VTYP AND L.VNO = L3.VNO and l.booktype = l3.booktype  AND l.lno = l3.naratno)   
                 else (select top 1  l3.narr from account.dbo.ledger3 l3  where L.VTYP = L3.VTYP AND L.VNO = L3.VNO and l.booktype = l3.booktype and  l3.naratno = 0 )   
                 end),'')  
