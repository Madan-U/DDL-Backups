...
From DelTrans where BCltDpId like @BCltDpID And BDpId Like @BDpID
...
Union All
select Sett_No, Sett_type, TCode, Party_Code, scrip_cd , series, certno, Qty, Transno = FolioNo, DrCr, TransDate,Reason=(Case When TrType = 909 Then 'Pledge' Else 'Holding' End),CltDpId,DpID, @BDpId, @BcltDpID 
From DelTrans d where BCltDpId Not like @BCltDpID And BDpId Not Like @BDpID
And DrCr = 'D' And Filler2 = 0 and Delivered in ('G','D')
And TransDate < @HoldDate And TrType <> 906
And ( TCode in ( select TCode From DelTrans d1 where 
...
if @statusid = 'branch'
begin
select scrip_cd, series,CertNo ,Sett_No, Sett_type,QTy=Sum(Qty) from DelHold D,Client1 C1,Client2 C2, branches br  
where BDpId = @BDpId and BCltDpId = @BCltDpId 
...
begin
select scrip_cd, series,CertNo ,Sett_No, Sett_type,QTy=Sum(Qty) from DelHold D,Client1 C1,Client2 C2, subbrokers sb 
where BDpId = @BDpId and BCltDpId = @BCltDpId 
And C1.Cl_Code = C2.Cl_Code And C2.Party_Code = D.Party_Code
...
if @statusid = 'trader'
begin
select scrip_cd, series,CertNo ,Sett_No, Sett_type,QTy=Sum(Qty) from DelHold D,Client1 C1,Client2 C2
where BDpId = @BDpId and BCltDpId = @BCltDpId 
And C1.Cl_Code = C2.Cl_Code And C2.Party_Code = D.Party_Code
