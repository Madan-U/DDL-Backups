-- Object: PROCEDURE dbo.BBG1C_CollateralCalculationSp
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



CREATE PROCEDURE BBG1C_CollateralCalculationSp(@Exchange Varchar(3), @Segment Varchar(20),@FromParty Varchar(10), @ToParty Varchar(10),@EffDate Varchar(11),@Status int OUTPUT)
As
Declare @@Cur  cursor,
@@Get Cursor,
@@GetFdBg Cursor,
...
Union
    Select distinct F.party_code , isnull(Cl_type,'CLI')  from C_SecuritiesMst F, ClientMaster C  where F.party_code <> 'Broker' and filler11 = 'Party' and F.Party_code >= @FromParty and F.Party_Code <= @ToParty and Exchange = @Exchange and Segment =@Segment and F.Party_code = C.Party_code
Union 
...
Print 'Party IS  ' + @@Party_code + ' Cl Type is   ' +@@Cl_type  
	...
	Delete from Msajag.Dbo.CollateralDetails Where Exchange = @Exchange and Segment = @Segment and 
        Party_code = @@Party_Code and Left(Convert(Varchar,EffDate,109),11) /* like */ =  @EffDate  /* + '%' */ and Coll_Type = 'MARGIN'
	...
	Delete from Msajag.Dbo.CollateralDetails Where Exchange = @Exchange and Segment = @Segment and 
        Party_code = @@Party_Code and Left(Convert(Varchar,EffDate,109),11) = /* Like */ @EffDate /* + '%' */  and Coll_Type = 'FD'
	...
	Delete from Msajag.Dbo.CollateralDetails Where Exchange = @Exchange and Segment = @Segment and Party_code = @@Party_Code
        and Left(Convert(Varchar,EffDate,109),11) /* like */ =  @EffDate /* + '%' */ and Coll_Type = 'BG'
	...
	Delete from Msajag.Dbo.CollateralDetails Where Exchange = @Exchange and Segment = @Segment and 
        Party_code = @@Party_Code and Left(Convert(Varchar,EffDate,109),11) =  /* like  */ @EffDate /* + '%' */ and Coll_Type = 'SEC'
	...
	Begin		
		Set @Status = 1
		Delete From Msajag.dbo.Collateral Where Party_code = @@Party_code and Exchange = @Exchange and 
                Segment = @Segment and Left(Convert(Varchar,Trans_Date,109),11) = /* Like */ @EffDate  /* + '%' */
