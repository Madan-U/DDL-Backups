...
@EarlyOrAuto Varchar(1), @GrossOrNet Varchar(1), @BDPTYPE VARCHAR(4) = 'NSDL')
AS
...
And S2.Series = S1.Series And TrType = 906 And Filler2 = 1   
And ShareType = 'DEMAT' And DrCr = 'D'  
Group By Sett_no,Sett_Type,Short_Name,D.Series,CertNo   

if @GrossOrNet = 'G' 
Begin
	SELECT SETT_NO,SETT_TYPE,SCRIP_CD,SERIES,PARTY_CODE,QTY,INOUT 
	INTO #DELIVERYCLT FROM DELIVERYCLT
	WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE
	AND INOUT = 'I'  
	
	INSERT INTO #DELIVERYCLT 
	SELECT SETT_NO,SETT_TYPE,SCRIP_CD,SERIES,PARTY_CODE,QTY=SUM(QTY),INOUT='I'
	FROM DELTRANS 
	WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE
	AND FILLER2 = 1 AND SHARETYPE = 'DEMAT' AND TRTYPE <> 906 
	AND DRCR = 'C' AND NOT EXISTS (SELECT SCRIP_CD FROM DELIVERYCLT
	WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE)
...
Begin	
	...
	INSERT INTO #DELIVERYCLT 
	SELECT SETT_NO,SETT_TYPE,SCRIP_CD,SERIES,QTY=SUM(QTY),INOUT='I'
	FROM DELTRANS 
	WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE
	AND FILLER2 = 1 AND SHARETYPE = 'DEMAT' AND TRTYPE <> 906 
	AND DRCR = 'C' AND NOT EXISTS (SELECT SCRIP_CD FROM DELIVERYCLT
	...
	And S1.Co_Code = S2.Co_Code  And S2.SCRIP_CD = D.Scrip_Cd   
	AND D.SERIES = S2.SERIES And S2.Series = S1.Series 
	Group By Sett_no,Sett_Type,Short_Name,D.Series,M.IsIn   
...
Begin
	...
	AND D.SERIES = S2.SERIES And S2.Series = S1.Series and BDpType = @BDPTYPE
