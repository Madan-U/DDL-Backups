-- Object: PROCEDURE dbo.usp_nxt_getOrderBasedBrokerageReport_test_28122023
...
--EXEC usp_nxt_getOrderBasedBrokerageReport 'client','CBHO','S194675'  
--usp_nxt_getOrderBasedBrokerageReport_test_28122023  'client','CBHO','M123175'  
exec usp_nxt_getOrderBasedBrokerageReport_Opti04Dec2023 @ReportLevel = 'SB', @access_code = 'ET', @party_code = null  
exec usp_nxt_getOrderBasedBrokerageReport @ReportLevel = 'SB', @access_code = 'ET', @party_code = null  
...
CREATE PROCEDURE usp_nxt_getOrderBasedBrokerageReport_test_28122023
 --declare 
...
select distinct cl_code,long_name,max(a.SP_CreatedOn) SP_CreatedOn,max(a.SP_CreatedOn) mailsentDate
,sub_broker,
...
Print 'AAA'
SELECT cl_code,sub_broker,long_name INTO #clientDetails FROM client_details WITH(NOLOCK) 
WHERE sub_broker= @access_code 


select SP_CreatedOn,cl_code,SCHEME_NAME into #Vw_VBB_SCHEME_NAME_SB from Vw_VBB_SCHEME_NAME VSN WITH(NOLOCK)  
 where sub_broker=@access_code  
...
case when SCHEME_NAME like '%prime%' then 'iTrade Prime Plan' else 'iTrade Plan' end end ) as SCHEME_NAME,sn.sub_broker   
...
select cl_code,status,long_name,SP_CreatedOn,mailsentDate,SCHEME_NAME,sub_broker 
