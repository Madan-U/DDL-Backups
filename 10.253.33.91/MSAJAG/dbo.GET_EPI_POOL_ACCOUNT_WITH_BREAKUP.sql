-- Object: PROCEDURE dbo.GET_EPI_POOL_ACCOUNT_WITH_BREAKUP
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE PROCEDURE [dbo].[GET_EPI_POOL_ACCOUNT_WITH_BREAKUP]
...
SELECT @EPNDATE = ISNULL(MAX(Margin_Date),'APR  1 2020') FROM TBL_MG02
...
SELECT @Start_date = ISNULL(MAX(Start_date),'APR  1 2020') FROM Sett_Mst
...
SELECT EXCHANGE, SEGMENT, PARTY_CODE, POOLACCOUNT = SUM(POOLACCOUNT), EPIACCOUNT_100_PER = SUM(EPIACCOUNT_100_PER), EPIACCOUNT_20_REV = SUM(EPIACCOUNT_20_REV), LEDACCOUNT = 0--SUM(LEDACCOUNT)
FROM
(
SELECT 
EXCHANGE = 'NSE', SEGMENT = 'CAPITAL', PARTY_CODE, 
POOLACCOUNT = SUM(MARGIN_BENEFIT+MTOMLOSS+ADNL_MARGIN), EPIACCOUNT_100_PER = CONVERT(NUMERIC(18,4),0), EPIACCOUNT_20_REV = CONVERT(NUMERIC(18,4),0), LEDACCOUNT = CONVERT(NUMERIC(18,4),0) 
FROM #PAYIN 
GROUP BY PARTY_CODE 
UNION ALL
SELECT 
EXCHANGE = 'NSE', SEGMENT = 'CAPITAL', PARTY_CODE,  
POOLACCOUNT = CONVERT(NUMERIC(18,4),0), EPIACCOUNT_100_PER = SUM(CASHBENEFIT),EPIACCOUNT_20_REV = CONVERT(NUMERIC(18,4),0), LEDACCOUNT = SUM(CASHBENEFIT)
FROM TBL_EPN_BENEFIT T
WHERE TRANSDATE = @EPNDATE
AND CASHBENEFIT > 0 
AND SETT_NO NOT IN (SELECT SETT_NO FROM SETT_MST S WHERE T.SETT_NO = S.SETT_NO AND T.SETT_TYPE = S.SETT_TYPE
AND Sec_Payin < @MARGINDATE + ' 23:59')
GROUP BY TRANSDATE, PARTY_CODE
UNION ALL
SELECT EXCHANGE = 'NSE', SEGMENT = 'CAPITAL', PARTY_CODE, POOLACCOUNT = CONVERT(NUMERIC(18,4),0),EPIACCOUNT_100_PER = CONVERT(NUMERIC(18,4),0), EPIACCOUNT_20_REV=-SUM(CASHBENEFIT)*20/100, LEDACCOUNT = CONVERT(NUMERIC(18,4),0) 
FROM (
