-- Object: PROCEDURE dbo.Usp_PoolFile_Backoffice
...
CREATE Proc [dbo].[Usp_PoolFile_Backoffice]        
As        
BEGIN      
...
and Sec_Payin >convert(varchar(11),getdate(),120) and Sett_Type in ('N','W')   And Sec_Payin <> convert(varchar(11),getdate(),120)    
and   Start_date <>  convert(varchar(11),getdate(),120)       
        
--Exchange Var details         
 
Declare @Vardate datetime
Select @Vardate= max(Start_date)  from #sett
...
where Exists  (select Sett_no from #sett s where del.sett_no=s.Sett_No and del.sett_type =S.Sett_Type ) and inout ='o'        
...
select SCRIP_CD,SERIES,ISIN,CL_RATE into #cls_Rate from AngelNSECM.msajag.dbo.closing_mtm             
...
into #ClsRate_Varmgr            
from #cls_Rate a inner join AngelNSECM.msajag.dbo.VARDETAIL b             
on a.ISIN=b.IsIN            
and a.SCRIP_CD=b.SCRIP_CD            
where b.DetailKey=replace(convert(varchar,@Vardate, 103),'/','')      
...
from #tbl_Final_Pool_Data_New a inner join [csokyc-6].general.dbo.tbl_ISIN_100per_Haricut b      
on a.I_ISIN=b.ISIN      
      
      
update #tbl_Final_Pool_Data_New      
set AppVar =100      
where I_ISIN in  (select [ISIN No] from  [csokyc-6].general.dbo.TBL_NRMS_RESTRICTED_SCRIPS where [Angel scrip category]='Poor')      
      
insert into Tbl_PoolFile        
select Party_code,I_ISIN,Sum (Toatl_Qty) as Toatl_Qty ,'1' as Format_1,'0' as Format_2,sum (limit_Qty) as limit_Qty,((ceiling (AppVar))/100) as FinalVarMargin,CL_RATE,'CNC' as Product ,GETDATE() from        
#tbl_Final_Pool_Data_New group by Party_code,I_ISIN,AppVar,CL_RATE    
...
----------------///**********B2C GM7 serevr Pool File Download on 02 July 2022 *******************////////////////--------------------------      
   ...
   select Ltrim (rtrim(Party_code))+'|'+I_Isin+'|'+convert (varchar (20),Total_Qty)+'|'+Format_1+'|'+Format_2+'|'+limit_Qty+'|'+Haircut+'|'+convert(varchar,convert(decimal(18,2), CL_Rate))+'|'+Product     
   from  Tbl_PoolFile      
          
      
           
   DECLARE @filename_GM7 as varchar(100)                                                               
   select @filename_GM7='\\INHOUSELIVEAPP1-FS.angelone.in\Upload1\OmnesysFileFormat\GM7_Pool_File\BO_OmnPoolFile_GM7'+replace (convert (varchar(12),GETDATE(),103),'/','')+'.txt'            
   ...
   SET @FILENAME_GM7 = '\\INHOUSELIVEAPP1-FS.angelone.in\Upload1\OmnesysFileFormat\GM7_Pool_File\BO_OmnPoolFile_GM7'+replace (convert (varchar(12),GETDATE(),103),'/','')+'.txt'                                                                                  
   SET @BCPCOMMAND_GM7 = 'BCP "SELECT   FileFormat FROM MSAJAG.dbo.Tbl_Pool_File_BO with (nolock)" QUERYOUT "'                          
   SET @BCPCOMMAND_GM7 = @BCPCOMMAND_GM7 + @filename_GM7 + '" -c -t, -SABVSNSECM.angelone.in -Uaolinhouse -Pe$$gnfDTVs2455GZAcc'                          
