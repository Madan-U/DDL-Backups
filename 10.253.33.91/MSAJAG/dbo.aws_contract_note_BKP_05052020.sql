-- Object: PROCEDURE dbo.aws_contract_note_BKP_05052020
...
CREATE  proc [dbo].[aws_contract_note_BKP_05052020] (@dt datetime)
as
...
delete from nse_bulk_process where sno=1
 ...
 INSERT INTO nse_bulk_process
SELECT DISTINCT C.PARTY_CODE,'1' AS SNO,C.PARTY_CODE+'|'+'H'+'|'+C.PARTY_CODE+'('+c.branch_cd+'/'+c.sub_broker+')'+'|'+PARTYNAME+'|'+C.L_ADDRESS1+'|'+C.L_ADDRESS2+'|'+C.L_ADDRESS3+'|'+C.L_CITY+'|'+C.L_STATE+'-'+C.L_ZIP+'|'+CONTRACTNO_NEW+'|'+C.PAN_GIR_NO+'|'+
...
SELECT DISTINCT PARTY_CODE FROM #P )
 ...
 from account.dbo.ledger_all where vdt >='2020-04-01' and vdt <=@dt +' 23:59' and cltcode in (select * from #party)
 group by cltcode
  
  insert into #vdt
   select cltcode,sum(case when drcr ='d' then vamt else -vamt end) as balance  
 from mtftrade.dbo.ledger where vdt >='2020-04-01' and vdt <=@dt +' 23:59' and cltcode in (select * from #party)
 group by cltcode
