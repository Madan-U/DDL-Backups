...
Set @@Cur = Cursor For	
	Select Party_Code From C_PartySelectView where Party_code >= @FromParty and Party_Code <= @ToParty and Exchange = @Exchange and Segment =@Segment
	union 
	select distinct Party_code from C_MarginLedger
	...
	Set @@GetFdBg = Cursor for
		...
		where F.Trans_Date = (select max(f1.Trans_Date) from FixedDepositTrans F1
 					where f.party_code = f1.party_code and f.bank_code = f1.bank_code and f.fdr_no = f1.fdr_no
		       			and f1.Trans_Date <=  @EffDate + ' 23:59' and f1.branch_code = f1.branch_code AND LEFT(CONVERT(VARCHAR,FM.rECEIVE_DATE,109),11) = LEFT(CONVERT(VARCHAR,F1.TRANS_DATE,109),11))
		...
		and @EffDate + ' 23:59:59' >= fm.receive_date and @EffDate +' 00:00:00'< fm.maturity_date  
