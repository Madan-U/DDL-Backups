-- Object: PROCEDURE dbo.CLASS_AUTO_PROCESS_SETUP_bkp_07FEB2023
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE PROC [dbo].[CLASS_AUTO_PROCESS_SETUP_bkp_07FEB2023]            
(            
...
BEGIN            
 ...
 BEGIN            
  ...
  FROM TBL_AUTO_PROCESS_MASTER T, SETT_MST M             
   ...
   --AND M.SETT_TYPE NOT IN ('O', 'Z', 'L' ,'S', 'P', 'B', 'A', 'AD', 'X', 'AC', 'H', 'T', 'OS', 'TS', 'G', 'Q', 'I')            
   AND M.SETT_TYPE IN ('N', 'W','M','Z')
  ...
  FROM TBL_AUTO_PROCESS_MASTER T, SETT_MST M             
   ...
   AND T.SETT_TYPE <> ''            
   AND PROCESS_FREQ = 'ONCE'            
   AND M.SETT_TYPE IN ('N', 'W', 'C', 'D','M','Z')            
  ORDER BY 1            
  ...
  SELECT PROCESS_ID, M.SETT_NO, M.SETT_TYPE,             
   ...
   PROCESS_END_DATE=CONVERT(DATETIME,LEFT(GETDATE(),11) + ' ' + PROCESS_END_TIME)            
  FROM TBL_AUTO_PROCESS_MASTER T, SETT_MST M  
WHERE SEC_PAYIN = (SELECT MIN(SEC_PAYIN) FROM SETT_MST  
WHERE SEC_PAYIN > @BUSINESS_DATE AND SETT_TYPE IN  ('N', 'W','M','Z'))  
AND M.SETT_TYPE IN ('N', 'D', 'W', 'C','M','Z')  
AND START_DATE < @BUSINESS_DATE  
AND @BUSINESS_DATE IN (SELECT SEC_PAYIN FROM SETT_MST WHERE SEC_PAYIN = @BUSINESS_DATE)             
   AND PROCESS_TYPE = 'SETTLEMENT'            
   AND PROCESS_FOR = 'FINOBL'            
   AND T.SETT_TYPE <> ''            
   AND PROCESS_FREQ = 'ONCE'            
...
DELETE  FROM  TBL_AUTO_PROCESS_DETAIL            
 WHERE PROCESS_DATE = LEFT(GETDATE(),11)
 AND (PROCESS_FOR LIKE '%COMMON CONT%Z%'
 OR PROCESS_FOR LIKE '%COMBINE MARGIN%Z%'
  OR PROCESS_FOR='COMMON CONT HALT'
   OR PROCESS_FOR ='COMMON CONT HALT' 
 OR PROCESS_FOR ='COMMON CONT STATUS'
 OR  PROCESS_FOR ='COMBINE MARGIN FILE GEN HALT') 
 
 DELETE  FROM  TBL_AUTO_PROCESS_DETAIL            
 WHERE PROCESS_DATE = LEFT(GETDATE(),11)
 AND   PROCESS_FOR in
('COMMON CONT CURR HALT',
'COMMON CONT CURRENCY',
'COMMON CONT NON CURRENCY')            
