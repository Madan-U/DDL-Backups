-- Object: PROCEDURE dbo.CLASS_AUTO_PROCESS_SETTLEMENT_BAK
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE   PROCEDURE [dbo].[CLASS_AUTO_PROCESS_SETTLEMENT_BAK]            
(             
 @PROCESS_DATE VARCHAR(11)            
)            
AS            
            
-- EXEC CLASS_AUTO_PROCESS_SETTLEMENT 'AUG 17 2015'            
...
SELECT @PREV_DATE = LEFT(ISNULL(MAX(SYSDATE),@BUSINESS_DATE),11) FROM CLOSING      
...
PROCESS_CHECK_QUERY = REPLACE(PRADNYA.DBO.fn_CLASS_Auto_Process_Replace(DBO.FN_DATA_DECRYPT('MKTTECH.IN',PROCESS_CHECK_QUERY),BUSINESS_DATE,MEMBERCODE,D.SETT_NO,LTRIM(RTRIM(D.SETT_TYPE))),'<SNO>',D.SNO),            
IS_FILE_BASED,            
...
FROM TBL_AUTO_PROCESS_DETAIL_BAK D (NOLOCK),             
     TBL_AUTO_PROCESS_MASTER_BAK M (NOLOCK),             
  OWNER O            
...
AND D.IS_DEPEND_ON = ''            
AND IS_FILE_BASED <> ''   
--AND D.PROCESS_FOR LIKE '%COMBINE MARGIN NON 0-FZ%'  
--AND D.SETT_TYPE = 'W'     
ORDER BY D.SNO            
 ...
 */
 ...
 SELECT @CHKFLAG = ISNULL(COUNT(1),0) FROM TBL_AUTO_PROCESS_DETAIL_BAK (NOLOCK)            
 ...
 IF @CHKFLAG = 0             
 BEGIN            
  IF (SELECT ISNULL(COUNT(1),0) FROM TBL_AUTO_PROCESS_DETAIL_BAK (NOLOCK)            
   WHERE SNO = @SNO            
 ...
 BEGIN            
  ...
  EXEC (@CMD)            
   ...
   BEGIN        
    IF @PROCESS_FOR = 'EXCHANGE APP VAR' AND @PREV_DATE = LEFT(@FLD_FILEDATE,11)       
    BEGIN        
     SET @CMD = ''            
    END      
    ELSE       
 ...
 END     
   ...
   IF @ISEXISTS = 0             
   BEGIN            
    UPDATE TBL_AUTO_PROCESS_DETAIL_BAK SET PROCESS_HALTED_REASON = 'FILE NOT AVAILABLE IN THE SPECIFIED LOCATION.',            
   ...
   BEGIN               
    SET @ERRFLAG = 0            
    UPDATE TBL_AUTO_PROCESS_DETAIL_BAK SET PROCESS_STATUS = 99, PROCESS_STARTED_DATE = GETDATE(), PROCESS_FILE_NAME = DBO.FN_DATA_ENCRYPT('MKTTECH.IN',ISNULL(@PROCESS_FILE_NAME_NEW,''))            
    WHERE SNO = @SNO     
  ...
  BEGIN       
   ROLLBACK TRAN            
  END    
       UPDATE TBL_AUTO_PROCESS_DETAIL_BAK SET PROCESS_HALTED_REASON = ERROR_MESSAGE(),            
       PROCESS_STATUS = 0            
       WHERE SNO = @SNO            
       SET @ERRFLAG = 1            
      END CATCH  
  ...
  INSERT INTO TBL_AUTO_PROCESS_FILE VALUES (@FLD_FILENAME, @FLD_FILEDATE, GETDATE())            
   ...
   END    
      ...
      BEGIN            
       ...
       SET @CMD = @CMD + ' BEGIN '            
       SET @CMD = @CMD + '  UPDATE TBL_AUTO_PROCESS_DETAIL_BAK SET PROCESS_HALTED_REASON = @MSG, PROCESS_STATUS_ALERT_INTERVAL = ''00:00:00'','            
       ...
       SET @CMD = @CMD + '  UPDATE TBL_AUTO_PROCESS_DETAIL_BAK SET IS_DEPEND_ON = REPLACE(IS_DEPEND_ON,''#'+CONVERT(VARCHAR,@PROCESS_ID)+','',''''), '            
    ...
    SET @CMD = @CMD + '  WHERE PROCESS_DATE = ''' + @PROCESS_DATE + ''' AND IS_DEPEND_ON LIKE ' + '''%#'+CONVERT(VARCHAR,@PROCESS_ID)+',%'''            
       ...
       IF @OTHER_DBDETAILS <> ''            
       BEGIN            
        SET @CMD = @CMD + '  UPDATE ' + @OTHER_DBDETAILS + '.TBL_AUTO_PROCESS_DETAIL_BAK SET IS_DEPEND_ON = REPLACE(IS_DEPEND_ON,''#0,'','''') , '            
        SET @CMD = @CMD + '  PROCESS_START_DATE = GETDATE() + '' 00:01:00'', '            
        SET @CMD = @CMD + '  PROCESS_END_DATE = (CASE WHEN PROCESS_END_DATE > (GETDATE() + '' 00:01:00'' + APPROX_END_TIME) THEN PROCESS_END_DATE '             
        SET @CMD = @CMD + '  ELSE GETDATE() + '' 00:01:00'' + APPROX_END_TIME END)'            
       ...
       SET @CMD = @CMD + ' ELSE '             
       SET @CMD = @CMD + ' BEGIN '             
       SET @CMD = @CMD + '  UPDATE TBL_AUTO_PROCESS_DETAIL_BAK SET '            
      ...
      ELSE            
      BEGIN            
       SET @CMD = ' IF (SELECT ISNULL(COUNT(1),0) FROM TBL_AUTO_PROCESS_DETAIL_BAK '            
...
--print ERROR_MESSAGE()            
 ...
 BEGIN            
  IF @CHKFLAG = 1             
  BEGIN            
   UPDATE TBL_AUTO_PROCESS_DETAIL_BAK SET PROCESS_HALTED_REASON = @PROCESS_HALTED_REASON,            
   PROCESS_STARTED_DATE = GETDATE(), PROCESS_ENDED_DATE = GETDATE()            
