-- Object: PROCEDURE dbo.AGEING_SSIS_RPT
...
CREATE procedure  [dbo].[AGEING_SSIS_RPT]    
(@DATE DATETIME)    
AS     
BEGIN     
    
  TRUNCATE TABLE AGEING_REPORT_DATA  
    
   TRUNCATE TABLE TBL_AGEING_SSRS_DATA  
     
    EXEC ABVSINSURANCEBO.msdb.DBO.SP_START_JOB 'AGEING_REPORT_INSERT'     
...
( CLTCODE VARCHAR(25),    
  ...
  ----SELECT   * INTO TBL_AGEING_SSRS_DATA FROM AGMUBODPL3.DMAT.CITRUS_USR.TBL_AGEING_SSRS_DATA  
  ----SELECT TOP 2 * INTO AGEING_REPORT_DATA FROM AGMUBODPL3.DMAT.CITRUS_USR.AGEING_REPORT_DATA  
...
select LD_CLIENTCD,BALAMT=SUM(LD_AMOUNT*-1),    
  ...
  FROM (SELECT LD_CLIENTCD,LD_AMOUNT,LD_DT   FROM AGMUBODPL3.DMAT.CITRUS_USR.SYNERGY_LEDGER WITH(NOLOCK) WHERE LD_DT <=@DATE    
...
,SUM(CR1081Total)CR1081Total,SUM(DR1081Total) DR1081Total    
 ...
 SELECT * INTO #HOLD1 FROM AGMUBODPL3.DMAT.DBO.SYNERGY_HOLDING WITH(NOLOCK) WHERE HLD_HOLD_DATE =@DATE    
 CREATE NONCLUSTERED INDEX #D ON #HOLD1    
(HLD_AC_CODE ASC ,HLD_ISIN_CODE ASC )   
    
    
    
  SELECT V.* INTO #CLOSIN1 FROM  AGMUBODPL3.DMAT.CITRUS_USR.VW_ISIN_RATE_MASTER V WITH(NOLOCK),    
  (SELECT ISIN,MAX(RATE_DATE) AS RD FROM  AGMUBODPL3.DMAT.CITRUS_USR.VW_ISIN_RATE_MASTER WITH(NOLOCK)  WHERE  RATE_DATE <=@DATE GROUP BY ISIN ) B    
...
INSERT INTO AGEING_REPORT_DATA  
   
SELECT *,DEF=(CASE WHEN ISNULL(VALUATION,0)>NET_LD_DOCUMENTTYPE THEN 'Secured' else 'RISK' END),'DEBTORS'as TYPE   FROM #DEBTORS    
UNION ALL    
SELECT *,DEF=(CASE WHEN ISNULL(VALUATION,0)>NET_LD_DOCUMENTTYPE THEN 'Secured' else 'RISK' END),'CREDITORS'AS TYPE FROM #CREDITORS    
  
DECLARE @FILE1 VARCHAR(MAX),@PATH1 VARCHAR(MAX) = 'J:\Backoffice\Automation\ACCOUNTS\AGEING_DATA\'                     
SET @FILE1 = @PATH1 + 'AGEING_FILE' +'_'+ CONVERT(VARCHAR(11),@DATE , 112) + '.csv' --Folder Name   
  
DECLARE @S1 VARCHAR(MAX)                            
SET @S1 = 'EXEC MASTER.DBO.XP_CMDSHELL ''BCP "SELECT ''''CLTCODE'''',''''NET_LD_DOCUMENTTYPE'''',''''DAY7'''',''''DAY15'''',''''DAY30'''',''''DAY60'''',''''DAY90'''',''''DAY180'''',''''DAY360'''',''''DAY720'''',''''DAY1080'''',''''DAY1081'''',''''VALUATIO
N'''',''''DEF'''',''''TYPE'''''    --Column Name  
SET @S1 = @S1 + ' UNION ALL SELECT  cast([CLTCODE] as varchar), cast([NET_LD_DOCUMENTTYPE] as varchar), cast([DAY7] as varchar), cast([DAY15] as varchar), cast([DAY30] as varchar), cast([DAY60] as varchar), cast([DAY90] as varchar) ,cast([DAY180] as varch
ar),cast([DAY360] as varchar),cast([DAY720] as varchar),cast([DAY1080] as varchar),cast([DAY1081] as varchar),cast([VALUATION] as varchar),cast([DEF] as varchar),cast([TYPE] as varchar)  FROM [MSAJAG].[dbo].[AGEING_REPORT_DATA]    " QUERYOUT ' --Convert data type if required  
