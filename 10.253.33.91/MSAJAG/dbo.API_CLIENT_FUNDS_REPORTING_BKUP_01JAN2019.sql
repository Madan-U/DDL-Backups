-- Object: PROCEDURE dbo.API_CLIENT_FUNDS_REPORTING_BKUP_01JAN2019
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



CREATE PROCEDURE [dbo].[API_CLIENT_FUNDS_REPORTING_BKUP_01JAN2019]
	@FR_DATE VARCHAR(11),
	@TO_DATE VARCHAR(11),
...
BEGIN
	...
	SET @@SQL = "INSERT INTO #LEDGER_PL_DETAILS EXEC " + @@ACCOUNTSVR1 + "." + @@ACCOUNTDB1 + ".DBO.CLIENT_FUNDS_REPORTING '" + @FR_DATE + "','" + @TO_DATE + "','" + @FR_CODE + "','" + @TO_CODE + "'"
	PRINT @@SQL
	EXEC (@@SQL)
...
UPDATE L SET RUNNING_BALANCE = OPBAL + (SELECT SUM(CRAMOUNT - DRAMOUNT) FROM #LEDGER_PL_DETAILS_FINAL L1 WHERE L.CLTCODE = L1.CLTCODE AND L1.SNO <= L.SNO) FROM #LEDGER_PL_DETAILS_FINAL L
...
SELECT FILENAME = @FILENAME, DATA = @MEMBERCODE +','+ REPLACE(CONVERT(VARCHAR, CONVERT(DATETIME, @FR_DATE , 109), 103), '/', '') + ' to ' + REPLACE(CONVERT(VARCHAR, CONVERT(DATETIME, @TO_DATE , 109), 103), '/', '')
UNION ALL
