-- Object: PROCEDURE dbo.rpt_accpandlthirdlevel1
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


/****** Object:  Stored Procedure dbo.rpt_accpandlthirdlevel1    Script Date: 01/19/2002 12:15:12 ******/

/****** Object:  Stored Procedure dbo.rpt_accpandlthirdlevel1    Script Date: 01/04/1980 5:06:25 AM ******/

CREATE proc rpt_accpandlthirdlevel1
...
/*only change is made in nextlevel query if you compare rpt_accpandlthirdlevel1 and rpt_accpandlthirdlevel query 
 since if there is no data for l0000000000 level and there is data for l0100000000 then with previous query l0000000000 level never came and therefore
we were not been able to find   l0100000000 level also
*/


select distinct gr.grpcode,gr.grpname, flag='nextlevel', dispflag=gr.dispdetail,amount=isnull((select sum(amt) from rpt_accbalsheetgrpvdtdrcr g
			where vdt > =@fromdt and vdt <= @todt + ' 23:59:59'
			and gr.grpcode=g.grpcode and drcr='d'),0) -
			isnull((select sum(amt) from rpt_accbalsheetgrpvdtdrcr g
			where vdt > =@fromdt and vdt <= @todt + ' 23:59:59'
			and gr.grpcode=g.grpcode and drcr='c') ,0) 	
from rpt_accbalsheetgrpvdtdrcr gr
where gr.grpcode like @nextgrpcode
and gr.grpcode not like @grpcode
group by  gr.grpcode,gr.grpname,gr.dispdetail,drcr
...
select gr.grpcode, gr.grpname,flag='samelevel', dispflag=gr.dispdetail,
amount=isnull((select isnull(sum(amt),0) from rpt_accbalsheetgrpvdtdrcr  g  
				 where  vdt>=@fromdt and vdt <= @todt + ' 23:59:59' and drcr='d' 
				and  g.grpcode=gr.grpcode 
				group by g.grpcode),0) -  
				isnull((select isnull(sum(amt),0) from rpt_accbalsheetgrpvdtdrcr  g  
				 where  vdt>=@fromdt  and vdt<= @todt + ' 23:59:59'   and drcr='c' 
				and  g.grpcode=gr.grpcode 				
				group by g.grpcode),0)
from rpt_accbalsheetgrpvdtdrcr  gr 
where gr.grpcode like ltrim(@samelevel)
and gr.grpcode not like ltrim(@grpcode)
group by gr.grpcode, gr.grpname,gr.dispdetail
...
amount=isnull((select isnull(sum(amt),0) from rpt_accbalsheetgrpvdtdrcr  g  
				...
				group by g.grpcode),0)
from rpt_accbalsheetgrpvdtdrcr  gr 
where gr.grpcode like  @abovelevel  
and gr.grpcode not like @grpcode
group by gr.grpcode, gr.grpname,gr.dispdetail
order by gr.grpcode
end
...
*/
	...
	amount=isnull((select isnull(sum(amt),0) from rpt_accbalsheetgrpvdtdrcr  g  
				...
				group by g.grpcode),0)
	from rpt_accbalsheetgrpvdtdrcr  gr 
	where gr.grpcode like @samelevel
	and gr.grpcode not like @grpcode
	group by gr.grpcode, gr.grpname,gr.dispdetail
	...
	amount=isnull((select isnull(sum(amt),0) from rpt_accbalsheetgrpvdtdrcr  g  
				...
				group by g.grpcode),0)
	from rpt_accbalsheetgrpvdtdrcr  gr 
	where gr.grpcode in (@firstlevelcode,@secondlevelcode,@toplevelcode,@abovelevel)
	and gr.grpcode not like @grpcode
	group by gr.grpcode, gr.grpname,gr.dispdetail
