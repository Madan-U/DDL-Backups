-- Object: PROCEDURE dbo.CLS_PARTY_ACCOUNT
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE PROCEDURE [dbo].[CLS_PARTY_ACCOUNT]  
AS  
  
SET NOCOUNT ON  
 update client1 set short_name='(c) '+substring(ltrim(rtrim(replace(short_name,'(c)',''))),1,17),  
 long_name='(c) '+substring(ltrim(rtrim(replace(long_name,'(c)',''))),1,96)   
...
(SELECT segment,remark,client_code,reason from ABVSKYCMIS.KYC.DBO.closing_master where INACT_DATE <= GETDATE() AND  
Segment IN ('ACDLCM','ALL')) b   
