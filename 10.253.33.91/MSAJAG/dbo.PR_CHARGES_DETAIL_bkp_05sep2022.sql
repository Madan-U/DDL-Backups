-- Object: PROCEDURE dbo.PR_CHARGES_DETAIL_bkp_05sep2022
...
CREATE  PROC [dbo].[PR_CHARGES_DETAIL_bkp_05sep2022]                       
(                    
 @F_SETT_NO VARCHAR(7),                      
...
FROM CLIENT2   (Nolock)              
WHERE EXISTS (SELECT PARTY_CODE FROM #SETT (Nolock)  WHERE #SETT.PARTY_CODE = CLIENT2.PARTY_CODE)          
ORDER BY CL_CODE            
...
FROM CLIENT1 (Nolock)            
WHERE EXISTS (SELECT PARTY_CODE FROM #SETT (Nolock)  WHERE #SETT.PARTY_CODE = CLIENT1.CL_CODE)         
ORDER BY CL_CODE            
...
/* POPULATE TRD TABLES FOR BROKERAGE MAPPING AND ROUNDING */                    
  ...
  SELECT *                    
  FROM   SCHEME_MAPPING M    With(NOLOCK)                
  WHERE  SP_DATE_TO > @SAUDA_DATE_FROM  
		 and SP_Date_From <=   @SAUDA_DATE_FROM                 
         AND EXISTS (SELECT DISTINCT PARTY_CODE                    
                     FROM   VBB_SETTLEMENT S With(NOLOCK)                      
  ...
  WHERE  CR_DATE_TO > @SAUDA_DATE_FROM  
		 and CR_Date_From <=   @SAUDA_DATE_FROM                   
         AND EXISTS (SELECT DISTINCT PARTY_CODE                    
                     FROM   VBB_SETTLEMENT S     (Nolock)                
...
SELECT DISTINCT PARTY_CODE = SP_PARTY_CODE  INTO #PARTYVBB                    
            FROM   TRD_SCHEME_MAPPING (NOLOCK)                                       

			INSERT INTO #PARTYVBB                     
            SELECT PARTY_CODE = CR_PARTY_CODE                    
            FROM   TRD_CONTRACT_ROUNDING (NOLOCK)           

CREATE CLUSTERED INDEX IDXPARTY ON #PARTYVBB
(
	PARTY_CODE
)
  DELETE FROM VBB_SETTLEMENT                    
  WHERE       NOT EXISTS ( SELECT PARTY_CODE FROM #PARTYVBB A
...
THEN TRADEQTY*MARKETRATE                       
	 ...
	 into #SETT_MAPPING                      
   FROM VBB_SETTLEMENT S (Nolock) , TRD_SCHEME_MAPPING M(Nolock) , TRD_SETT_MST SM (Nolock) ,
...
THEN TRADEQTY*MARKETRATE                       
   ...
   AND SCRIP_CD LIKE (CASE WHEN SP_SCRIP = 'ALL' THEN '%' ELSE SP_SCRIP END)                   
   AND SCRIP_CD NOT IN                            
            (SELECT SP_SCRIP FROM TRD_SCHEME_MAPPING M     With(NOLOCK)                    
         WHERE SAUDA_DATE BETWEEN SP_DATE_FROM AND SP_DATE_TO                            
                       AND S.PARTY_CODE = SP_PARTY_CODE)        
            AND SP_TRD_TYPE = (CASE WHEN BILLFLAG < 4                       
               THEN 'TRD'                       
...
FROM (                     
 SELECT                         
  ...
  SP_MIN_BROKAMT,SP_MAX_BROKAMT,SP_MIN_SCRIPAMT,SP_MAX_SCRIPAMT,SP_TRD_TYPE                      
 FROM                     
  VBB_SETTLEMENT S(Nolock) , TRD_SCHEME_MAPPING M(Nolock) , TRD_SETT_MST SM(Nolock) ,
   TRD_CLIENT1 C1(Nolock) , TRD_CLIENT2 C2(Nolock)                       
 WHERE C1.CL_CODE = C2.CL_CODE                      
 AND C2.PARTY_CODE = S.PARTY_CODE                      
 AND C1.CL_TYPE <> 'INS'                      
...
CD_COMPUTATION_LEVEL,CD_MIN_BROKAMT,CD_MAX_BROKAMT,CD_MIN_SCRIPAMT,CD_MAX_SCRIPAMT                      
FROM VBB_CHARGES_DETAIL_TEMP_2(Nolock) WHERE CD_COMPUTATION_LEVEL NOT IN ('C', 'S')                       
...
SUM(CD_TOTALBROKERAGE) <> (CASE WHEN CD_COMPUTATION_LEVEL IN ('C', 'S')                       
	...
	/* 
INSERT INTO VBB_CHARGES_DETAIL_TEMP_1                      
SELECT DISTINCT C2.* FROM VBB_CHARGES_DETAIL_TEMP_2 C2  (Nolock)                    
WHERE NOT EXISTS (SELECT CD_PARTY_CODE FROM VBB_CHARGES_DETAIL_TEMP_1 C1 (Nolock)                   
...
SELECT DISTINCT C2.* INTO #VBB_CHARGES_DETAIL_TEMP_2 FROM VBB_CHARGES_DETAIL_TEMP_2 C2  (Nolock)                    
WHERE NOT EXISTS (SELECT CD_PARTY_CODE FROM VBB_CHARGES_DETAIL_TEMP_1 C1 (Nolock)                   
WHERE C1.CD_PARTY_CODE = C2.CD_PARTY_CODE                      
AND C1.CD_SETT_NO = C2.CD_SETT_NO                      
AND C1.CD_SETT_TYPE = C2.CD_SETT_TYPE                      
...
select S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE,SAUDA_DATE=LEFT(START_DATE,11),                       
  ...
  DELBUY_BROKERAGE = SUM(CASE WHEN SELL_BUY = 1 AND BILLFLAG = 4 THEN TRADEQTY*NBROKAPP ELSE 0 END),                            
  DELSELL_BROKERAGE = SUM(CASE WHEN SELL_BUY = 2 AND BILLFLAG = 5 THEN TRADEQTY*NBROKAPP ELSE 0 END),                      
  settbrok=SUM(TRADEQTY*NBROKAPP)  ,                      
  TRDBUY_TURNOVER = SUM(CASE WHEN SELL_BUY = 1 AND BILLFLAG = 2                       
     THEN TRADEQTY*MARKETRATE                       
     ELSE 0                      
                     END),                            
  ...
  DELSELL_TURNOVER = SUM(CASE WHEN SELL_BUY = 2 AND BILLFLAG = 5                       
     ...
     ELSE 0                      
                     END) 
into #vbb_1 FROM VBB_SETTLEMENT S WITH (NOLOCK), TRD_SETT_MST SM WITH (NOLOCK),
TRD_CLIENT1 C1 WITH (NOLOCK), TRD_CLIENT2 C2 WITH (NOLOCK) 
WHERE s.Sett_No = @F_SETT_NO and s.Sett_Type =@F_SETT_TYPE 
and S.SETT_NO = SM.SETT_NO                       
AND S.SETT_TYPE = SM.SETT_TYPE                      
AND C1.CL_CODE = C2.PARTY_CODE                      
AND C1.CL_CODE = S.PARTY_CODE                      
AND C1.CL_TYPE <> 'INS'                      
AND C1.CL_TYPE <> 'NRI'                    
AND AUCTIONPART NOT LIKE 'A%' AND AUCTIONPART NOT LIKE 'F%'                      
AND TRADE_NO NOT LIKE '%C%'                      
GROUP BY S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE, CONTRACTNO, LEFT(START_DATE,11) 

insert into VBB_CHARGES_DETAIL_TEMP_2
SELECT S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE,SAUDA_DATE,
  CONTRACTNO, TRADE_NO='',                       
  ORDER_NO='', SCRIP_CD = 'BROKERAGE',                       
  SERIES,                       
  BUYRATE,                      
  ...
  DELBUY_BROKERAGE,
  DELSELL_BROKERAGE,
  TOTAL_BROKERAGE = (CASE WHEN settbrok > 0                       
                   THEN (CASE WHEN CR_MAX_CONTRACTAMT = - 1                       
                         THEN (CASE WHEN CR_MIN_CONTRACTAMT <> 0                      
                                  THEN PRADNYA.DBO.FNMAX(CR_MIN_CONTRACTAMT,settbrok)                      
                           ELSE settbrok                         
                             END)                         
                              ELSE PRADNYA.DBO.FNMIN(CR_MAX_CONTRACTAMT,                      
              (CASE WHEN CR_MIN_CONTRACTAMT <> 0                      
  ...
  DELSELL_TURNOVER,                      
CD_COMPUTATION_LEVEL='',CR_MIN_CONTRACTAMT,CR_MAX_CONTRACTAMT,CD_MIN_SCRIPAMT=0,CD_MAX_SCRIPAMT=-1
from #vbb_1 s, TRD_CONTRACT_ROUNDING C WITH (NOLOCK)
where  C.CR_PARTY_CODE = S.PARTY_CODE                      
--AND AUCTIONPART NOT LIKE 'A%' AND AUCTIONPART NOT LIKE 'F%'                      
--AND TRADE_NO NOT LIKE '%C%'                      
AND SAUDA_DATE BETWEEN CR_DATE_FROM AND CR_DATE_TO  + ' 23:59'
AND NOT EXISTS (SELECT CD_PARTY_CODE FROM VBB_CHARGES_DETAIL_TEMP_1 C1                      
...
select S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE,SAUDA_DATE=LEFT(START_DATE,11),                       
  ...
  DELSELL_TURNOVER = SUM(CASE WHEN SELL_BUY = 2 AND BILLFLAG = 5                       
     ...
     ELSE 0                      
                     END) 
into #vbb_2 FROM VBB_SETTLEMENT S WITH (NOLOCK), TRD_SETT_MST SM WITH (NOLOCK),
TRD_CLIENT1 C1 WITH (NOLOCK), TRD_CLIENT2 C2 WITH (NOLOCK) 
WHERE s.Sett_No = @F_SETT_NO and s.Sett_Type =@F_SETT_TYPE 
and S.SETT_NO = SM.SETT_NO                       
AND S.SETT_TYPE = SM.SETT_TYPE                      
...
SELECT S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE,SAUDA_DATE,
  ...
  DELSELL_TURNOVER,                      
CD_COMPUTATION_LEVEL='',CR_MIN_CONTRACTAMT,CR_MAX_CONTRACTAMT,CD_MIN_SCRIPAMT=0,CD_MAX_SCRIPAMT=-1
from #vbb_2 s, TRD_CONTRACT_ROUNDING C WITH (NOLOCK)
where  C.CR_PARTY_CODE = S.PARTY_CODE                      
--AND AUCTIONPART NOT LIKE 'A%' AND AUCTIONPART NOT LIKE 'F%'                      
--AND TRADE_NO NOT LIKE '%C%'                      
AND SAUDA_DATE BETWEEN CR_DATE_FROM AND CR_DATE_TO  + ' 23:59'
...
NBROKAPP = 0, N_NETRATE = MARKETRATE,                       
SERVICE_TAX = 0, NSERTAX = 0                       
FROM SETTLEMENT S (nolock)
, VBB_CHARGES_DETAIL_TEMP_2 T (nolock)                      
WHERE s.SETT_NO = @F_SETT_NO AND                 
s.SETT_TYPE = @F_SETT_TYPE AND                    
 s.PARTY_CODE BETWEEN  @FROM_PARTY AND @TO_PARTY AND                    
 s.TRADEQTY > 0                
 and T.CD_SETT_NO = S.SETT_NO                      
AND T.CD_SETT_TYPE = S.SETT_TYPE                      
AND T.CD_PARTY_CODE = S.PARTY_CODE                       
AND T.CD_CONTRACTNO = S.CONTRACTNO                    
...
SERVICE_TAX = 0, NSERTAX = 0                       
FROM SETTLEMENT S (nolock),
VBB_CHARGES_DETAIL_TEMP_1  (Nolock)                      
WHERE S.SETT_NO = @F_SETT_NO AND                 
S.SETT_TYPE = @F_SETT_TYPE AND                    
 S.PARTY_CODE BETWEEN  @FROM_PARTY AND @TO_PARTY AND                    
 S.TRADEQTY > 0                
 and VBB_CHARGES_DETAIL_TEMP_1.CD_SETT_NO = S.SETT_NO                      
AND VBB_CHARGES_DETAIL_TEMP_1.CD_SETT_TYPE = S.SETT_TYPE                      
AND VBB_CHARGES_DETAIL_TEMP_1.CD_PARTY_CODE = S.PARTY_CODE                       
AND VBB_CHARGES_DETAIL_TEMP_1.CD_SCRIP_CD = S.SCRIP_CD                      
AND VBB_CHARGES_DETAIL_TEMP_1.CD_SERIES = S.SERIES                      
...
select S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE,SAUDA_DATE=LEFT(START_DATE,11),                       
 ...
 TRDBUY_TURNOVER = 0,                            
  ...
  DELBUY_TURNOVER = 0,                 
  DELSELL_TURNOVER = 0,
  tot_turnover=sum(tradeqty*marketrate)   
into #vbb_3 FROM VBB_SETTLEMENT S WITH (NOLOCK), TRD_SETT_MST SM WITH (NOLOCK),
TRD_CLIENT1 C1 WITH (NOLOCK), TRD_CLIENT2 C2 WITH (NOLOCK) 
WHERE s.Sett_No = @F_SETT_NO and s.Sett_Type =@F_SETT_TYPE 
and S.SETT_NO = SM.SETT_NO                       
AND S.SETT_TYPE = SM.SETT_TYPE                      
...
SELECT S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE,SAUDA_DATE,
  ...
  TOTAL_BROKERAGE = (CASE WHEN (settbrok) > 0                       
                   THEN (CASE WHEN CR_MAX_CONTRACTAMT = - 1                       
                         THEN (CASE WHEN CR_MIN_CONTRACTAMT <> 0                      
                              THEN PRADNYA.DBO.FNMAX(PRADNYA.DBO.FNMIN(tot_turnover*@SEBI_LIMIT/100,CR_MIN_CONTRACTAMT),settbrok)                      
                           ELSE settbrok                         
                             END)                         
                              ELSE PRADNYA.DBO.FNMIN(CR_MAX_CONTRACTAMT,                      
              (CASE WHEN CR_MIN_CONTRACTAMT <> 0                      
  ...
  DELSELL_TURNOVER,                      
CD_COMPUTATION_LEVEL='',CR_MIN_CONTRACTAMT,CR_MAX_CONTRACTAMT,CD_MIN_SCRIPAMT=0,CD_MAX_SCRIPAMT=-1
from #vbb_3 s, TRD_CONTRACT_ROUNDING C WITH (NOLOCK)
where  C.CR_PARTY_CODE = S.PARTY_CODE                      
--AND AUCTIONPART NOT LIKE 'A%' AND AUCTIONPART NOT LIKE 'F%'                      
--AND TRADE_NO NOT LIKE '%C%'                      
AND SAUDA_DATE BETWEEN CR_DATE_FROM AND CR_DATE_TO  + ' 23:59'
...
select S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE,SAUDA_DATE=LEFT(START_DATE,11),                       
 ...
 TRDBUY_TURNOVER = 0,                            
  ...
  DELSELL_TURNOVER = 0,
  tot_turnover=sum(tradeqty*marketrate)   
into #vbb_4 FROM VBB_SETTLEMENT S WITH (NOLOCK), TRD_SETT_MST SM WITH (NOLOCK),
TRD_CLIENT1 C1 WITH (NOLOCK), TRD_CLIENT2 C2 WITH (NOLOCK) 
WHERE s.Sett_No = @F_SETT_NO and s.Sett_Type =@F_SETT_TYPE 
and S.SETT_NO = SM.SETT_NO                       
AND S.SETT_TYPE = SM.SETT_TYPE                      
...
SELECT S.PARTY_CODE,S.SETT_NO,S.SETT_TYPE,SAUDA_DATE,
  ...
  DELSELL_TURNOVER,                      
CD_COMPUTATION_LEVEL='',CR_MIN_CONTRACTAMT,CR_MAX_CONTRACTAMT,CD_MIN_SCRIPAMT=0,CD_MAX_SCRIPAMT=-1
from #vbb_4 s, TRD_CONTRACT_ROUNDING C WITH (NOLOCK)
where  C.CR_PARTY_CODE = S.PARTY_CODE                      
--AND AUCTIONPART NOT LIKE 'A%' AND AUCTIONPART NOT LIKE 'F%'                      
--AND TRADE_NO NOT LIKE '%C%'                      
AND SAUDA_DATE BETWEEN CR_DATE_FROM AND CR_DATE_TO  + ' 23:59'
...
FROM VBB_CHARGES_DETAIL_TEMP_2(Nolock) , GLOBALS   (Nolock)                    
...
AND CD_PARTY_CODE BETWEEN @FROM_PARTY AND @TO_PARTY                         
 ...
 --FOR LIQUIDBEES by suresh
 
 UPDATE CHARGES_DETAIL            
SET             
CD_TRDBUYBROKERAGE=BROKERAGE,            
CD_TRDSELLBROKERAGE=BROKERAGE,            
CD_DELBUYBROKERAGE=BROKERAGE,            
CD_DELSELLBROKERAGE=BROKERAGE,            
CD_TOTALBROKERAGE=BROKERAGE            
FROM                       
 TBL_EXCEPTION_SCRIP T  (NOLOCK)                      
WHERE                  
  CD_SETT_NO = @F_SETT_NO                    
AND CD_SETT_TYPE = @F_SETT_TYPE                    
AND CD_PARTY_CODE BETWEEN @FROM_PARTY AND @TO_PARTY                         
 AND CD_SCRIP_CD = T.SCRIP_CD 
 AND CD_SERIES= T.SERIES
AND CD_SAUDA_DATE BETWEEN T.FROMDATE AND T.TODATE
