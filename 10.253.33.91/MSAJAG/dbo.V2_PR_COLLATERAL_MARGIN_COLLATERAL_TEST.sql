-- Object: PROCEDURE dbo.V2_PR_COLLATERAL_MARGIN_COLLATERAL_TEST
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

  
-- EXEC V2_PR_COLLATERAL_MARGIN_COLLATERAL 'JUL  2 2018'        
        
CREATE PROCEDURE [dbo].[V2_PR_COLLATERAL_MARGIN_COLLATERAL_TEST]        
(              
  @EFFDATE VARCHAR(11),              
...
/*-------------------------------------------------- INCLUDING POA BALANCE --STARTS HERE ------------------------------------------------*/        
 ...
 INSERT INTO #TBL_COLLATERAL_MARGIN_ALL              
  SELECT          
    ...
    LEDGER_AMT = 0          
  FROM         
  DELHOLD_COLL WHERE TRANSDATE = @EFFDATE    AND ISIN NOT LIKE 'INF%'   
 ...
 CREATE INDEX IDXPARTY ON #TBL_COLLATERAL_MARGIN_ALL  (PARTY_CODE, COLL_TYPE)               
...
WHERE MFSS_NAV01.NAV_DATE BETWEEN @PRV_DATE AND  @PRV_DATE + ' 23:59:59'        
 ...
 /*  Changes for poor scrip valualtion */ 
 
-- UPDATE #TBL_COLLATERAL_MARGIN_ALL              
-- SET   MRG_HAIRCUT = 100, RMS_HAIRCUT = 100   
-- FROM ANGELFO.NSEFO.DBO.SCRIP_TYPE S(nolock)        
-- --WHERE MFSS_NAV01.NAV_DATE BETWEEN @EFFDATE AND  @EFFDATE + ' 23:59'        
--WHERE S.ISIN = #TBL_COLLATERAL_MARGIN_ALL.ISIN        
    
	/*  Changes for poor scrip valualtion end */ 
...
SELECT DISTINCT ISIN,  MRG_CL_RATE, SCRIP_CD = CONVERT(VARCHAR(6),'') INTO #CLDATA         
...
(        
 ...
 DELETE DELCDSLBALANCE_7DAYS_NEW WHERE TRDATE = @EFFDATE AND EXISTS (SELECT ISIN FROM #ISIN         
           WHERE #ISIN.ISIN = DELCDSLBALANCE_7DAYS_NEW.ISIN)        
...
/*-----------------------------------------------  GETIING T-1 DAY'S VAR MARGIN  -----------------------------------------------*/               
 ...
 SELECT             
  ...
  SECSPECVAR=MARGIN,VARMARGINRATE=MARGIN,DETAILKEY=1      INTO  #BSEVAR                       
...
UPDATE #TBL_COLLATERAL_MARGIN_ALL SET MRG_VAR_MARGIN = VARMARGINRATE
FROM #BSEVAR WHERE #TBL_COLLATERAL_MARGIN_ALL.ISIN = #BSEVAR.ISIN   
 AND #TBL_COLLATERAL_MARGIN_ALL.ISIN NOT IN (SELECT ISIN FROM #VAR)    
...
END             
 ...
 TRUNCATE TABLE V2_TBL_COLLATERAL_MARGIN_TEST          
           
 INSERT INTO V2_TBL_COLLATERAL_MARGIN_TEST          
 SELECT *,RELEASE_QTY=0,RELEASE_AMT=0,RECORD_TYPE= 1,EXCHANGE_ORG = EXCHANGE,SEGMENT_ORG=SEGMENT          
 FROM #TBL_COLLATERAL_MARGIN_ALL          
             
  DELETE FROM V2_TBL_COLLATERAL_MARGIN_COMBINE_TEST WHERE EFFDATE = @EFFDATE        
          
  INSERT INTO V2_TBL_COLLATERAL_MARGIN_COMBINE_TEST         
  ...
  @EFFDATE,GETDATE(),EXCHANGE_ORG,SEGMENT_ORG FROM V2_TBL_COLLATERAL_MARGIN_TEST      
