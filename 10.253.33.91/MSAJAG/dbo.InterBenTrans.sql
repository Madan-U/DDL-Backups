-- Object: PROCEDURE dbo.InterBenTrans
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE Proc InterBenTrans ( @Sett_no Varchar(7),@Sett_Type Varchar(2),@RefNo Int) As
Declare 
 @@Scrip_Cd Varchar(12),
 @@series Varchar(3),
...
Insert Into DelInterBen
Select DT.Sett_No,DT.Sett_Type,DT.PARTY_CODE,DT.SCRIP_CD,DT.Series,DT.BSECODE,DT.Psettno,DT.Psetttype,DT.Exchg,BCQty=0,CQty=IsNull(Sum(Qty),0),0,0,0,0,IsIn
...
Insert Into DelInterBen
Select DT.Sett_No,DT.Sett_Type,DT.PARTY_CODE,DT.SCRIP_CD,DT.Series,DT.BSECODE,DT.Psettno,DT.Psetttype,DT.Exchg,BCQty=0,0,XQty=IsNull(Sum(Qty),0),0,0,0,IsIn
...
Insert Into DelInterBen
Select DT.Sett_No,DT.Sett_Type,DT.PARTY_CODE,DT.SCRIP_CD,DT.Series,DT.BSECODE,DT.Psettno,DT.Psetttype,DT.Exchg,BCQty=0,0,XQty=0,IQty=IsNull(Sum(Qty),0),0,0,IsIn
...
Insert Into DelInterBen
Select DT.Sett_No,DT.Sett_Type,DT.PARTY_CODE,DT.SCRIP_CD,DT.Series,DT.BSECODE,DT.Psettno,DT.Psetttype,DT.Exchg,BCQty=0,0,XQty=0,0,BDQty=IsNull(Sum(Qty),0),0,IsIn
...
Insert Into DelInterBen
Select DT.Sett_No,DT.Sett_Type,DT.PARTY_CODE,DT.SCRIP_CD,DT.Series,DT.BSECODE,DT.Psettno,DT.Psetttype,DT.Exchg,BCQty=0,0,XQty=0,0,BDQty=0,DQty=IsNull(Sum(Qty),0),IsIn
...
ORDER BY PARTY_CODE,SCRIP_CD,Series,Psettno,psetttype
open @@inter
...
Begin
 ...
 Begin
 Set @@Cfwd = Cursor for 
 select IsNull(Sum(Qty),0) from MSAJAG.DBO.DelTrans where delivered = '0' And SETT_NO = @@PSett_No
 ...
 Begin
 Set @@Cfwd = Cursor for 
 select IsNull(Sum(Qty),0) from BSEDB.DBO.DelTrans where delivered = '0' And SETT_NO = @@PSett_No
 And sett_type = @@PSett_Type and Party_Code = @@Party_Code
 and CertNo = @@IsIn and DrCr = 'D' 
 and Filler2 = 1 and TrType = 904 and sharetype <> 'auction'
 End
 ...
 begin
	...
	Begin
	Set @@Cfwd = Cursor for 	
	Select Qty,FromNo,ToNo,CertNo,FolioNo,Sno,TrType,TCode from MSAJAG.DBO.DelTrans where delivered = '0' 
	...
	Begin
	Set @@Cfwd = Cursor for 	
	Select Qty,FromNo,ToNo,CertNo,FolioNo,Sno,TrType,TCode from BSEDB.DBO.DelTrans where delivered = '0' 
	And SETT_NO = @@PSett_No And sett_type = @@PSett_Type and Party_Code = @@Party_Code
 	and CertNo = @@IsIn and DrCr = 'D' and Filler2 = 1 and TrType = 904  and sharetype <> 'auction'
	order by Qty asc
	End
