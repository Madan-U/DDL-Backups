...
/* Drop Proc InsDelInterSettUpdateInEx */
CREATE Proc InsDelInterSettUpdateInEx As
Declare 
@@SNo int,
@@DelSNo int,
...
Select SNo,IsIn,Qty,TCode,TrDate=Left(Convert(Varchar,TrDate,109),11),D.Sett_No,D.sett_Type,D.CltAccNo
From DematTrans D, BSEDB.DBO.Sett_Mst S Where D.CltAccNo = S.Sett_No And D.Sett_Type = S.Sett_Type And Party_Code = 'Party' 
Order By IsIn,Qty Desc,D.Sett_No,D.sett_Type,D.CltAccNo
Open @@DematCur 
Fetch Next From @@DematCur Into @@SNo,@@IsIn,@@Qty,@@TCode,@@TrDate,@@Sett_No,@@Sett_Type,@@ISett_No 
While @@Fetch_Status = 0
Begin	
	/* Taking Party's Sell Position for That Scrip from DeliveryClt Table. */
	Set @@SettDel = Cursor For
	Select SNO,Qty,Party_Code From BSEDB.DBO.DelTrans Where Sett_No = @@ISett_No
	And CertNo = @@IsIn
	And ISett_No = @@Sett_No and Filler2 = 1 And DrCr = 'D' and Delivered <> 'D'  And TrType <> 908 
		...
		Begin
			...
			Begin
				...
				Begin
					...
					From DelTrans Where SNo = @@DelSNo And Sett_No = @@ISett_No And CertNo = @@IsIn 
					And Party_Code = @@Party_Code And ISett_No = @@Sett_No and Filler2 = 1 And DrCr = 'D' 
	
					Update BSEDB.DBO.DelTrans Set Qty=@@SettDelQty-@@Qty Where SNo = @@DelSNo And Sett_No = @@ISett_No
				...
				Else	
				Begin	
					Update BSEDB.DBO.DelTrans Set Delivered = 'D' Where SNo = @@DelSNo And Sett_No = @@ISett_No
