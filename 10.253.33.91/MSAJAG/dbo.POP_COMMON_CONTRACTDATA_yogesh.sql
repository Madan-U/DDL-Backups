-- Object: PROCEDURE dbo.POP_COMMON_CONTRACTDATA_yogesh
...
CREATE PROCEDURE [dbo].[POP_COMMON_CONTRACTDATA_yogesh] (                                    
 @FROMDATE VARCHAR(11),                                    
 @USERNAME VARCHAR(20) = ''                                    
 )                                    
AS                                    
...
BEGIN                                    
 ...
 PRINT 'D1'                                                                                                           
  ...
  SET @STRSQL = @STRSQL + ' ' + @SHARESERVER + '.' + @SHAREDB + '.DBO.CONTRACT_MASTER_COMMON WITH (NOLOCK) WHERE CONVERT(DATETIME,START_DATE,103) BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'' AND PARTY_CODE BETWEEN ''' + @FROMPARTY + ''' AND ''' + @TOPARTY + ''''                                    
...
/* ADDED BY VINAY ON OCT 27 2014 FOR ND TRADE */     
--SET @STRSQL = @STRSQL + ' AND CONVERT(DATETIME,D.SAUDA_DATE,109) BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'''                                    
  SET @STRSQL = @STRSQL + ' AND CONVERT(VARCHAR(11),CONVERT(DATETIME,M.START_DATE,103),109) BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'''                                    
...
/* ADDED BY VINAY ON OCT 27 2014 FOR ND TRADE */     
  ...
  SET @STRSQL = @STRSQL + 'BEGIN '                                    
  SET @STRSQL = @STRSQL + 'IF (SELECT COUNT(1) FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.COLUMNS WHERE [NAME] = ''USER_ID'' AND [OBJECT_ID] IN (SELECT DISTINCT OBJECT_ID FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.TABLES WHERE NAME = ''CONTRACT_DATA_COMMON'')) > 0 '                                    
  SET @STRSQL = @STRSQL + 'BEGIN '                                    
...
END                                    
   ...
   SET @STRSQL = 'IF (SELECT COUNT(1) FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.TABLES WHERE NAME=''TBL_CONTRACT_TERMINAL_MAPPING'') > 0 '                                    
   SET @STRSQL = @STRSQL + 'BEGIN '                                    
   SET @STRSQL = @STRSQL + 'IF (SELECT COUNT(1) FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.COLUMNS WHERE [NAME] = ''USER_ID'' AND [OBJECT_ID] IN (SELECT DISTINCT OBJECT_ID FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.TABLES WHERE NAME = ''CONTRACT_DATA'')) > 0 '                                    
   SET @STRSQL = @STRSQL + 'BEGIN '                                    
   SET @STRSQL = @STRSQL + ' UPDATE #CONTRACT SET REMARK_ID = ISNULL(T.REMARK_ID,''''), REMARK_DESC = ISNULL(T.REMARK_DESC,'''') '                                    
   SET @STRSQL = @STRSQL + ' FROM ' + @SHARESERVER + '.' + @SHAREDB + '.DBO.TBL_CONTRACT_TERMINAL_MAPPING T WITH (NOLOCK) '                                    
   SET @STRSQL = @STRSQL + ' WHERE #CONTRACT.USER_ID = (CASE WHEN ISNULL(T.TERMINAL_ID,'''') <> '''' THEN ISNULL(T.TERMINAL_ID,'''') ELSE #CONTRACT.USER_ID END) '                                    
...
DECLARE @CONTGEN_COMMON_YOGESH INT                                    
                                    
IF (                                    
  SELECT COUNT(1)                                    
  FROM CONTGEN_COMMON_YOGESH                                    
...
SELECT @CONTGEN_COMMON_YOGESH = ISNULL(CONVERT(INT, CONTRACTNO), 0)                                    
FROM CONTGEN_COMMON_YOGESH                                    
WHERE @FROMDATE BETWEEN START_DATE AND END_DATE                                    
...
FROM COMMON_CONTRACT_DATA_ISIN C(NOLOCK)                                    
...
UPDATE #CONTRACT                                    
SET CONTRACTNO_NEW = STUFF('0000000', 8 - LEN(@CONTGEN_COMMON_YOGESH + SNO), LEN(@CONTGEN_COMMON_YOGESH + SNO), @CONTGEN_COMMON_YOGESH + SNO)                                    
FROM #CONTRACT B,                                    
 #CONTRACTNO T                                    
WHERE B.PARTY_CODE = T.PARTY_CODE                                    
                                    
SELECT @CONTGEN_COMMON_YOGESH = @CONTGEN_COMMON_YOGESH + ISNULL(MAX(SNO), 0)                                    
FROM #CONTRACTNO                                    
                                    
UPDATE CONTGEN_COMMON_YOGESH                               
SET CONTRACTNO = @CONTGEN_COMMON_YOGESH                                    
WHERE @FROMDATE BETWEEN START_DATE AND END_DATE                                    
...
DELETE                                    
FROM COMMON_CONTRACT_DATA_ISIN                                    
WHERE SAUDA_DATE BETWEEN @FROMDATE AND @FROMDATE + ' 23:59:59'                                    
 AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY                                    
                                    
INSERT INTO COMMON_CONTRACT_DATA_ISIN                                    
SELECT *                                    
FROM #CONTRACT                                    
ORDER BY PARTY_CODE,                                    
 EXCHANGE,                                    
