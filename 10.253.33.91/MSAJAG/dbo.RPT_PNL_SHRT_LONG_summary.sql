-- Object: PROCEDURE dbo.RPT_PNL_SHRT_LONG_summary
...
CREATE PROC [dbo].[RPT_PNL_SHRT_LONG_summary]                                                                    
(                                                                      
 @STATUSID VARCHAR(25),                                                                      
...
IF @RPTOPT = 'S'                                                               
BEGIN      
SELECT PARTY_CODE,FLAG,
--SUM(BUYQTY) BUYQTY,SUM(BUYVAL*-1) AS BUYVAL,SUM(SELLQTY) AS SELLQTY ,SUM(SELLVAL) AS SELLVAL,
SUM(PNL) AS PNL --, SUM(CLVAL) AS CL_VAL
 INTO #SUMMARY  FROM (                                                      
...
ISIN,                                                              
 ...
 GROUP BY PARTY_CODE,FLAG
 ORDER BY PARTY_CODE,FLAG DESC                                                              
END                                                              
ELSE                                                              
BEGIN                                                               
...
SELECT PARTY_CODE,ISNULL(SPECULATION,0)SPECULATION,ISNULL([SHORT TERM],0)[SHORT TERM],ISNULL([LONG TERM],0)[LONG TERM],
ISNULL([FREE],0) AS NOTIOANL,ISNULL([1OPEN],0) AS [OPEN/SELL] FROM #SUMMARY  
PIVOT (SUM(PNL) FOR FLAG IN ([SPECULATION],[SHORT TERM],[LONG TERM],[FREE],[1OPEN]))AS PVT
