...
/*
Modified by VNS on 06-Aug-2001
To remove rounding from all options as this was generating difference in Trial balance 
*/
/*
Modified by Neelambari on  14 june  2001
added a flag to this procedure which decides if the query should be ordered by cltcode ot by partyname
*/

/*
Modified by Neelambari on 1 mar 2001
removed hardcoding for share database and added hardcoding for newaccount database 
*/
...
/* 		 select l.cltcode , clcode=isnull((select cl_code from client2 c2 
  		...
  		and vdt >= @sdt and vdt <= @vdt + ' 23:59:59') - 
 		 (select isnull(sum(round(vamt,2)),0) from account.dbo.ledger where drcr = 'C' and cltcode = l.cltcode and  vdt >=@sdt and  vdt <= @vdt + ' 23:59:59') 
  		From account.dbo.Ledger l where  vdt >= @sdt and vdt <= @vdt + ' 23:59:59'  
...
*/
  		...
  		and vdt >= @sdt and vdt <= @vdt + ' 23:59:59') - 
 		 (select isnull(sum(vamt),0)  from account.dbo.ledger where drcr = 'C' and cltcode = l.cltcode and  vdt >=@sdt and  vdt <= @vdt + ' 23:59:59') 
  		From account.dbo.Ledger l where  vdt >= @sdt and vdt <= @vdt + ' 23:59:59'  
  		group by l.Cltcode , l.acname 
		order by   l.Cltcode, l.acname 
	 ...
	 begin
		  ...
		  where  vdt >= @sdt and vdt <= @vdt + ' 23:59:59' 
...
/*		  select l.cltcode , clcode=isnull((select cl_code from client2 c2 
		  ...
		  and  vdt >= @sdt and  vdt <= @vdt + ' 23:59:59')- 
		  (select isnull(round(sum(round(vamt,2)),2),0) from account.dbo.ledger where drcr = 'C' and cltcode = l.cltcode and vdt >= @sdt and  vdt <= @vdt + ' 23:59:59') 
...
/*
		  ...
		  (select isnull(round(sum(round(vamt,2)),2),0) from account.dbo.ledger where drcr = 'C' and cltcode = l.cltcode and  vdt >= @sdt and vdt <= @vdt + ' 23:59:59') 
		  From account.dbo.Ledger l , client1 C1, client2 C2
		  where  vdt >= @sdt and  vdt <= @vdt + ' 23:59:59' and
