-- Object: PROCEDURE dbo.NewUpdBillTaxReCalIns
...
Update ISettlement set Tmark = Tmark , table_no = broktable.table_no, line_no = broktable.line_no,val_perc = broktable.val_perc,
    ...
    when (  broktable.val_perc ='V' )
		...
		power(10,CT.Round_To))
                        ...
                        when (  broktable.val_perc ='P' )
                             Then      ((floor ( (((broktable.Normal /100 ) * ISettlement.marketrate)  * power(10,CT.Round_To) + CT.RoFig + CT.ErrNum ) /  (CT.RoFig + CT.NoZero )) * (CT.RoFig +CT.NoZero ) )  / power(10,CT.Round_To))
   Else
          BrokApplied
                        End
                         ),
       N_NetRate = (  case
                      when (  broktable.val_perc ='V' AND ISettlement.SELL_BUY = 1)
                             Then
                      ...
                      when (  broktable.val_perc ='P' AND ISettlement.SELL_BUY = 1 )
                             Then ISettlement.marketrate + ((floor(( (((broktable.Normal /100 )* ISettlement.marketrate)) * power(10,CT.Round_To)+CT.RoFig + CT.ErrNum ) /  (CT.RoFig + CT.NoZero )) * (CT.RoFig +CT.NoZero ) )  / power(10,CT.Round_To))

                      when (broktable.val_perc ='V' AND ISettlement.SELL_BUY =2 )
                             Then ISettlement.marketrate - ((floor(( (( broktable.Normal )) * power(10,CT.Round_To)+CT.RoFig + CT.ErrNum ) /  (CT.RoFig + CT.NoZero )) * (CT.RoFig +CT.NoZero ) )  / 	power(10,CT.Round_To))
                      when ( broktable.val_perc ='P' AND ISettlement.SELL_BUY = 2 )
                             Then
        ...
        NSertax = (Case When Client2.Service_Chrg = 1 And Client2.SerTaxMethod = 1
			...
			Then ( ( ((floor(( broktable.Normal * power(10,CT.Round_To)+CT.RoFig + CT.ErrNum ) /
				...
				power(10,CT.Round_To)) ) * ( ISettlement.Tradeqty * Globals.service_tax) )  / ( CASE WHEN CLIENT2.Service_chrg <>  3 THEN 100 ELSE (100 + Globals.service_tax)  END )
                        when (broktable.val_perc ='P' )
                        Then ((((floor ( (((broktable.Normal /100 ) * ISettlement.marketrate)  * power(10,CT.Round_To) + CT.RoFig + CT.ErrNum ) /  (CT.RoFig + CT.NoZero )) * (CT.RoFig +CT.NoZero ) )  / power(10,CT.Round_To)) ) * ( ISettlement.Tradeqty * Globals.service_tax) )  / ( CASE WHEN CLIENT2.Service_chrg <>  3 THEN 100 ELSE (100 + Globals.service_tax)  END )
   			Else ISettlement.Service_tax
                        End ) End
                         ) /*  /  ( CASE WHEN CLIENT2.Service_chrg =  0 THEN 100 ELSE (100 + ISettlement.service_tax)  END )*/,
Ins_chrg  =(Case when ISettlement.status = 'N' then 0 else ((CT.insurance_chrg * ISettlement.marketrate * ISettlement.Tradeqty)/100) End),
turn_tax  = (Case when ISettlement.status = 'N' then 0 else ((CT.turnover_tax * ISettlement.marketrate * ISettlement.Tradeqty)/100 ) end),
other_chrg =(Case when ISettlement.status = 'N' then 0 else ((CT.other_chrg * ISettlement.marketrate * ISettlement.Tradeqty)/100 ) end),
sebi_tax = (Case when ISettlement.status = 'N' then 0 else ((CT.sebiturn_tax * ISettlement.marketrate * ISettlement.Tradeqty)/100) end),
Broker_chrg =(Case when ISettlement.status = 'N' then 0 else ((CT.broker_note * ISettlement.marketrate * ISettlement.Tradeqty)/100) end)

      FROM BrokTable BrokTable,Client2,globals, ISettlement,Client1, ClientBrok_Scheme S, ClientTaxes_New CT, Owner
      ...
      And Broktable.Line_no = (Select min(Broktable.line_no) from broktable where
			       ...
			       And Trd_Del = 'D' And ISettlement.Party_Code =  Client2.Party_code
			       And ISettlement.marketrate <= Broktable.upper_lim )
      And (Globals.exchange = 'NSE')

      AND ISettlement.SETT_NO = @SETTNO
      AND ISettlement.SETT_TYPE = @SETTYPE
      and ISettlement.billflag in(4,5)
      and ISettlement.status <> 'E'
      and ISettlement.party_code = @party_code
      and ISettlement.scrip_cd = @Scrip_cd
      and ISettlement.Series Like @Series
      And ISettlement.Sauda_Date like @Sauda_Date + '%'
...
begin
	...
	(
		...
		'NewUpdBillTaxReCalIns',	/*called_from*/
		getdate(),	/*timestamp*/
