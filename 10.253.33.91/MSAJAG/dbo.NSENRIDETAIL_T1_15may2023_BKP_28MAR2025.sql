-- Object: PROCEDURE dbo.NSENRIDETAIL_T1_15may2023_BKP_28MAR2025
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE PROC  [dbo].[NSENRIDETAIL_T1_15may2023_BKP_28MAR2025] (@SDATE AS VARCHAR(11),@SELL_BUY AS VARCHAR(1))                                    
AS                 
...
SELECT @@SETT_NO1 = SETT_NO FROM SETT_MST WHERE START_DATE LIKE @SDATE+'%'  AND SETT_TYPE ='M'          
              
select * into #party from (SELECT cl_code FROM Client1 with (nolock) WHERE Cl_type ='NRI') a              
select a.* into #contractdata from CONTRACT_DATA a with (nolock) inner join #party b on a.party_code=b.cl_code where SETT_NO in(@@SETT_NO,@@SETT_NO1) and SELL_BUY =@SELL_BUY                
...
AS TRANACTIONAMT ,cd.SCRIPNAME,cd.SCRIP_CD ,SETT_NO, (Cd.PRATE+cd.SRATE)TRATE  ,Service_Tax OtherCharges,INS_CHRG STT ,(Cd.broker_chrg+Cd.Turn_Tax+Cd.sebi_tax) OtherTax                        
