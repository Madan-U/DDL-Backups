-- Object: PROCEDURE dbo.PROC_MARING_REPORT_NEW_JAYNAT
...
CREATE PROC [dbo].[PROC_MARING_REPORT_NEW_JAYNAT]                                            
(                                                      
...
FROM                                            
 ...
 WHERE                             
  ...
  DISTINCT PARTY_CODE                                            
 FROM                                            
  ANGELCOMMODITY.BSECURFO.DBO.FOBILLVALAN WITH (NOLOCK)                                            
 WHERE                             
  SAUDA_DATE BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59'                                            
  --AND TRADETYPE = 'BT'                                              
 ) A                                            
...
/*--- BSX FUTURES ---*/   
 INSERT INTO #MAR_REPORT                                                      
 SELECT EXCHANGE = 'BSX', SEGMENT = 'FUTURES', PARTY_CODE, CL_TYPE='',                                                      
 CASH = SUM(CASE WHEN DRCR = 'C' THEN VAMT ELSE -VAMT END),                                
 NONCASH = CONVERT(NUMERIC(18,4),0), FD_BG = CONVERT(NUMERIC(18,4),0),                                                      
 INIT_MARGIN = CONVERT(NUMERIC(18,4),0), EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                                                      
 ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                                                       
 FROM ANGELCOMMODITY.ACCOUNTCURBFO.DBO.LEDGER L WITH (NOLOCK) , #TPARTY C, ANGELCOMMODITY.ACCOUNTCURBFO.DBO.PARAMETER P                                                      
 WHERE L.CLTCODE = C.PARTY_CODE AND L.VDT BETWEEN P.SDTCUR AND P.LDTCUR                                                       
 AND GETDATE() BETWEEN P.SDTCUR AND P.LDTCUR                                                       
 AND VDT <= @SAUDA_DATE + ' 23:59'                                              
 AND EDT <= (CASE WHEN DRCR = 'D' THEN @SAUDA_DATE + ' 23:59' ELSE EDT END)                                                      
...
INSERT INTO #MAR_REPORT                                    
SELECT EXCHANGE='BSX', SEGMENT='FUTURES', D.PARTY_CODE, CL_TYPE='',                                                       
CASH = SUM(CASH_COLL),                                                      
NONCASH = SUM(MRG_REP_COLL_NCASH),                                                       
FD_BG = 0,                                                      
INIT_MARGIN = CONVERT(NUMERIC(18,4),0),                                       
EXP_MARGIN = CONVERT(NUMERIC(18,4),0),                                                      
ADD_MARGIN = CONVERT(NUMERIC(18,4),0)                                                        
FROM ANGELCOMMODITY.BSECURFO.DBO.TBL_CLIENTMARGIN D WITH (NOLOCK) , #TPARTY C                                                      
WHERE MARGINDATE BETWEEN  @COLLDATE AND @COLLDATE  +' 23:59'           
AND D.PARTY_CODE = C.PARTY_CODE                                                      
GROUP BY D.PARTY_CODE                                   
...
SELECT DISTINCT PARTY_CODE INTO #FO FROM ANGELFO.NSEFO.DBO.TBL_CLIENTMARGIN WITH (NOLOCK)                      
 WHERE MARGINDATE BETWEEN @SAUDA_DATE AND @SAUDA_DATE  + ' 23:59' AND INITIALMARGIN+PREMIUM_MARGIN <> 0                      
 SELECT DISTINCT PARTY_CODE INTO #NSX FROM ANGELFO.NSECURFO.DBO.TBL_CLIENTMARGIN WITH (NOLOCK)                       
WHERE MARGINDATE BETWEEN @SAUDA_DATE AND @SAUDA_DATE  + ' 23:59'  AND  INITIALMARGIN+PREMIUM_MARGIN <> 0                      
SELECT DISTINCT PARTY_CODE INTO #MCD FROM ANGELCOMMODITY.MCDXCDS.DBO.TBL_CLIENTMARGIN WITH (NOLOCK)                       
WHERE MARGINDATE BETWEEN @SAUDA_DATE AND @SAUDA_DATE  + ' 23:59'  AND INITIALMARGIN+PREMIUM_MARGIN <> 0 
SELECT DISTINCT PARTY_CODE INTO #BSX FROM ANGELCOMMODITY.BSECURFO.DBO.TBL_CLIENTMARGIN WITH (NOLOCK)                       
WHERE MARGINDATE BETWEEN @SAUDA_DATE AND @SAUDA_DATE  + ' 23:59'  AND INITIALMARGIN+PREMIUM_MARGIN <> 0                     
