-- Object: PROCEDURE dbo.SPX_GETPOAUPDATONFROMKYC_REVISEDPROCESS_PartyCodeWise_Mismatch_bk_25Feb2022
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

--[SPX_GETPOAUPDATONFROMKYC_REVISEDPROCESS_PartyCodeWise_Mismatch] 'E53975'      
      
Create PROC [dbo].[SPX_GETPOAUPDATONFROMKYC_REVISEDPROCESS_PartyCodeWise_Mismatch_bk_25Feb2022]                      
@UserId varchar(15)             
AS                      
...
anand1.msajag.dbo.client_details b with(nolock)        
...
--**********************************CODE START FOR CLIENT4 PROCESS **********************------------------------                    
 ...
 SELECT NISE_PARTY_CODE,CLIENT_CODE,1 defdp,'CDSL' depository,NSE INTO #tempdeleteinsertfinal FROM #SYNERGY_DATA_FINAL,       
 (      
...
--AND ISNULL(NSE,'N') NOT in ('NO','N')       
 ...
 SELECT NISE_PARTY_CODE,CLIENT_CODE,1 defdp,'CDSL' depository,NSE INTO #tempinsertfinal_New  FROM #SYNERGY_DATA_FINAL, (      
...
--AND ISNULL(NSE,'N')='Y'       
AND DPID='12033200' AND A.DpType='CDSL'          
...
AND #SYNERGY_DATA_FINAL.CLIENT_CODE=LTRIM(RTRIM(CLTDPNO))                    
--AND ISNULL(NSE,'N')='Y'       
AND DPID='12033200' AND STATUS='Active' AND A.DpType='CDSL'                 
...
--AND ISNULL(NSE,'N')='Y'       
AND DPID='12033200' AND DEF='0' AND ISNULL(MultiFlag,'')='Y' AND A.DpType='CDSL'       
