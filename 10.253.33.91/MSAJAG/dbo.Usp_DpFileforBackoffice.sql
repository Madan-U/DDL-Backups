-- Object: PROCEDURE dbo.Usp_DpFileforBackoffice
...
CREATE Proc [dbo].[Usp_DpFileforBackoffice]          
...
---settlement Cal           
--select * into  #sett from [AngelDemat].msajag.dbo.sett_mst where  Sec_Payin  between '2022-05-31 00:00:00.000' and '2022-06-01 23:59:59.000' and Sett_Type in ('N','W')--,'F')          
...
Select * into  #sett from [AngelDemat].msajag.dbo.sett_mst S where Start_date <=convert(varchar(11),getdate(),120)          
and Sec_Payin >=convert(varchar(11),getdate(),120) and Sett_Type in ('N','W','M','Z')          
          
          
          
---Combine New DP and Old Dp from Backoffice          
select tradingid,hld_isin_code,netqty,PLEDGE_QTY,Rate,FREE_QTY,FREEZE_QTY          
into #Dp_FreeHolding_Data          
from AngelDP4.dmat.citrus_usr.holding     
    
select a.*,b.cl_rate into #Dp_FreeHolding_New  from #Dp_FreeHolding_Data a left join #tbl_closing_mtm b    
on a.hld_isin_code=b.ISIN    
  
  
select tradingid,hld_isin_code,sum (netqty) as netqty,Rate,sum(FREE_QTY) as FREE_QTY,cl_rate    
into #Dp_FreeHolding  
from #Dp_FreeHolding_New    
group by tradingid,hld_isin_code,Rate,cl_rate  
      
          
-----CUsa Mtf and Margin pledge details Table           
select * into  #Cusa_MTF_MP from [AngelDemat].msajag.dbo.deltrans where DrCr='D' and Delivered='0' and Filler2='1'           
and Party_Code not in ('exe','broker','NSE')           
Union all          
select * from [AngelDemat].BSEDB.dbo.deltrans where DrCr='D' and Delivered='0' and Filler2='1'           
...
select * into #deliveryclt  from [AngelDemat].msajag.dbo.deliveryclt del          
...
--select * from [AngelDemat].msajag.dbo.deliveryclt del          
...
select a.*,b.Party_code as deli_Party_code,b.Qty as deli_Qty,b.I_ISIN as  deli_ISIN           
into #tbl_Final_Data_Right         
from #FreeDp_CMM a Right join #deliveryclt b           
on a.tradingid=b.Party_code          
and a.hld_isin_code=b.I_ISIN   
where a.tradingid is NULL     
         
           
select a.*,b.AppVar into #TBl_DP_Final_Data from #tbl_Final_Data a left join #VARDETAIL b           
on a.hld_isin_code=b.IsIN   
  
select a.*,b.AppVar into #TBl_DP_Final_Data_Var_Right from #tbl_Final_Data_Right a left join #VARDETAIL b           
on a.deli_ISIN=b.IsIN   
  
select a.deli_Party_code,a.deli_Qty,a.deli_ISIN,a.AppVar,b.CL_RATE into #Tbl_Final_Cl_rate from #TBl_DP_Final_Data_Var_Right a left join  #tbl_closing_mtm b   
on a.deli_ISIN=b.ISIN

update a      
set a.AppVar =b.VARMARGIN      
from #TBl_DP_Final_Data a inner join [csokyc-6].general.dbo.tbl_ISIN_100per_Haricut b      
on a.hld_isin_code=b.ISIN      
      
      
update #TBl_DP_Final_Data      
set AppVar =100      
where hld_isin_code in  (select [ISIN No] from  [csokyc-6].general.dbo.TBL_NRMS_RESTRICTED_SCRIPS where [Angel scrip category]='Poor') 

update a      
set a.AppVar =b.VARMARGIN      
from #TBl_DP_Final_Data_Var_Right a inner join [csokyc-6].general.dbo.tbl_ISIN_100per_Haricut b      
on a.deli_ISIN=b.ISIN      
      
      
update #TBl_DP_Final_Data_Var_Right      
set AppVar =100      
where deli_ISIN in  (select [ISIN No] from  [csokyc-6].general.dbo.TBL_NRMS_RESTRICTED_SCRIPS where [Angel scrip category]='Poor')          
          
          
insert into tbl_Final_Dp_File                
select  tradingid,hld_isin_code,(FREE_QTY+isnull (Qty,0)+isnull (deli_Qty,0))as Total_Qty,          
(ceiling (AppVar))/100 as AppVar,(isnull (Qty,0)+isnull (deli_Qty,0)) as Limit_Qty, (ISNULL(convert (decimal(18,2), Rate),0)),  --(case when Rate IS null then 0 else Rate end) as Rate ,
GETDATE() from #TBl_DP_Final_Data  order by hld_isin_code desc  
  
  
  
insert into tbl_Final_Dp_File   
select deli_Party_code,deli_ISIN,deli_Qty as Total_Qty,               
(ceiling (AppVar))/100 as AppVar, deli_Qty as Limit_Qty,(ISNULL(convert (decimal(18,2), Rate),0)),--(case when Rate IS null then 0 else Rate end) as Rate ,
GETDATE() from #TBl_DP_Final_Data_Var_Right  order by hld_isin_code desc 
...
select @filename_GM7='\\INHOUSELIVEAPP1-FS.angelone.in\Upload1\OmnesysFileFormat\GM7_DP_File\BO_GM7_Dp_FileFormat'+replace (convert (varchar(12),GETDATE(),103),'/','')+'.txt'          
...
SET @filename_GM7 = '\\INHOUSELIVEAPP1-FS.angelone.in\Upload1\OmnesysFileFormat\GM7_DP_File\BO_GM7_Dp_FileFormat'+replace (convert (varchar(12),GETDATE(),103),'/','')+'.txt'                                                                                
...
--select * from [csokyc-6].general.dbo. tbl_Omne_NewPoolBgFile where PARTY_CODE in ('A102597') order by ISIN desc
