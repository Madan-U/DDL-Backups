...
END
  ...
  IF @PROCESSFLAG <> 'IMPORT TRADE'  
    BEGIN  
      IF (SELECT COUNT(1)  
		  ...
		  UNION SELECT TOP 1 PARTY_CODE  
                  FROM   [AngelBSECM].bsedb_ab.DBO.ISETTLEMENT  
                  WHERE  SETT_TYPE = @OTHER_SETT_TYPE  
  ...
  IF @PROCESSFLAG = 'BILLING'  
	...
	--	EXEC PROC_SETTFLAG_START @SETT_NO, @SETT_TYPE, @PROCESSDATE, '0', 'ZZZZZZZZZZ'
		...
		EXEC [AngelBSECM].bsedb_ab.DBO.BSEBILLALLTRADE  
			 @OTHER_SETT_NO ,  
			 @OTHER_SETT_TYPE 
		
		EXEC [AngelBSECM].bsedb_ab.DBO.NODELCARRYFORWARD  
         @OTHER_SETT_NO ,  
			 @OTHER_SETT_TYPE  
	  END
  ...
  IF @PROCESSFLAG = 'VBB'  
    BEGIN  
	  ...
	  IF @OTHER_SETT_TYPE <> ''
	  BEGIN
		EXEC [AngelBSECM].bsedb_ab.DBO.PR_CHARGES_DETAIL  
			 @OTHER_SETT_NO ,  
			 @OTHER_SETT_TYPE,  
			 '0','ZZZZZZZZZZ'   
	  END
  ...
  IF @PROCESSFLAG = 'STT'  
    BEGIN  
		 ...
		 print @OTHER_SETT_TYPE
              ...
              EXEC [AngelBSECM].bsedb_ab.DBO.STT_CHARGES_FINAL  
                      @OTHER_SETT_TYPE ,  
                      @PROCESSDATE,  
                      '0','ZZZZZZZZZZ'   
...
print 'Suresh1'
  ...
  IF @PROCESSFLAG = 'VALAN'  
    BEGIN  
	  ...
	  IF @OTHER_SETT_TYPE <> ''
	  BEGIN
		EXEC [AngelBSECM].bsedb_ab.DBO.BSEVALANGENERATE  
			 @OTHER_SETT_NO ,  
			 @OTHER_SETT_TYPE,  
			 '0'  
  ...
  IF @PROCESSFLAG = 'CONTRACTPOP'  
    BEGIN  
	  IF @OTHER_SETT_TYPE <> ''
	  BEGIN	
		EXEC [AngelBSECM].bsedb_ab.DBO.POP_CONTRACTDATA  
			 @OTHER_SETT_NO ,  
			 @OTHER_SETT_TYPE,  
			 @PROCESSDATE ,  
			 '0' ,  
  ...
  IF @PROCESSFLAG = 'IMPORT TRADE'  
	...
	BEGIN	
		UPDATE [AngelBSECM].bsedb_ab.DBO.V2_BUSINESS_PROCESS  
		SET    IMPORT_TRADE = IMPORT_TRADE + 1,
		BILLING = 0,  
		VBB = 0,  
		STT = 0,  
