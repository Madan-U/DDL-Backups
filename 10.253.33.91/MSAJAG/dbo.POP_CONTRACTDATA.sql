...
AS                                        
 ...
 SERVICE_TAX,                                        
  ...
  SELECT CONTRACTNO,                                        
         BILLNO,                                        
         TRADE_NO = CONVERT(VARCHAR(20),'00000000000'),                                        
         PARTY_CODE,                                        
         SCRIP_CD,                                        
         TRADEQTY,                                        
         SERIES,                                        
...
AND S.SETT_NO = M.SETT_NO           ---ADD BY SURESH                            
  ...
  WHERE  M.SCRIP_CD = CONTRACT_DATA_TMP.SCRIP_CD                                        
         ...
         --                END)   --- CHANGED ON 05-AUG-2022
	  AND M.SERIES = (CASE WHEN CONTRACT_DATA_TMP.SERIES IN ('IL','BT','BL','Q1') THEN 'EQ' ELSE CONTRACT_DATA_TMP.SERIES END)                                     
         AND VALID = 1        
		 
/* TO ADD ISIN CHANGES DONE ON 15072022 */
		 SELECT * INTO #SCRIP_MAPP FROM TBL_EXCHG_SCRIP_MAP WHERE @SAUDA_DATE BETWEEN FROM_DATE AND TO_DATE
		 CREATE INDEX #SCRIP ON #SCRIP_MAPP (NSE_SCRIP_CD,	NSE_SERIES)
 
		 UPDATE CONTRACT_DATA_TMP                                        
		  SET    ISIN = M.ISIN                                        
		  FROM   #SCRIP_MAPP M                                        
		  WHERE  M.NSE_SCRIP_CD = CONTRACT_DATA_TMP.SCRIP_CD                                        
				 AND M.NSE_SERIES = (CASE                                         
								   WHEN CONTRACT_DATA_TMP.SERIES = 'BL' THEN 'EQ'                                        
								   WHEN CONTRACT_DATA_TMP.SERIES = 'IL' THEN 'EQ'                                        
								   ELSE CONTRACT_DATA_TMP.SERIES                                        
								 END)   
			  AND CONTRACT_DATA_TMP.ISIN=''						
/* TO ADD ISIN CHANGES DONE ON 15072022 */
