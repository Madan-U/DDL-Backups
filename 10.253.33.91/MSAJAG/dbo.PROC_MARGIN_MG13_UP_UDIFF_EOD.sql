-- Object: PROCEDURE dbo.PROC_MARGIN_MG13_UP_UDIFF_EOD
...
IF RIGHT(REPLACE(@FILENAME,'|',''),10) = 'P_0000.CSV' 
	BEGIN
			...
			SELECT	MARGINDATE,
					PARTY_CODE,
					VARMARGIN = SUM(ISNULL(ValAtRskMrgn,0) + ISNULL(LossMrgn,0)),
					EXTREME_LOSS_MARGIN = SUM(ISNULL(MinMrgn,0)),
					ADDITIONAL_MARGIN = SUM(ISNULL(AddtlMrgn,0)),
					MTOM_LOSS = (CASE WHEN SUM(ISNULL(MrkToMktNetd,0)) >0 THEN ABS(SUM(ISNULL(MrkToMktNetd,0))) ELSE 0 END),
					TOTAL_MARGIN = SUM(ISNULL(MinMrgn,0)) + SUM(ISNULL(AddtlMrgn,0)) + (CASE WHEN SUM(ISNULL(MrkToMktNetd,0)) >0 THEN ABS(SUM(ISNULL(MrkToMktNetd,0))) ELSE 0 END),
					CL_FLAG = 'C', 
					UPLOAD_ON = GETDATE(),
					UPLOAD_BY = 'SYSTEM',
					TOTAL_MARGIN_VAR =  SUM(ISNULL(TtlMrgnAsPerActlParams,0)),
					PEAKMARGIN = SUM(ISNULL(AggtPeakLbltyRptd,0))
			FROM	TBL_CMMARGINNEW_UDIFF_EOD
			WHERE	MARGINDATE >= @TRADEDATE AND MARGINDATE <= @TRADEDATE + ' 23:59'
			AND LVL = 'P'
			AND ABS(ISNULL(ValAtRskMrgn,0) + ISNULL(LossMrgn,0)) + ABS(ISNULL(MinMrgn,0)) + ABS(ISNULL(AddtlMrgn,0)) 
