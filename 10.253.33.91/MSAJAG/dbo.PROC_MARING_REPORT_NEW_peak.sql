-- Object: PROCEDURE dbo.PROC_MARING_REPORT_NEW_peak
...
Create PROC [dbo].[PROC_MARING_REPORT_NEW_peak]                                            
(                                                      
...
FROM                                            
 ...
 FROM TBL_MG02 where MARGIN_DATE BETWEEN @MARGINDATE AND @MARGINDATE + ' 23:59:59'    
...
BEGIN
            ...
            FROM        (
                        ...
                        FROM  ANAND.BSEDB_AB.DBO.TBL_CMMARGIN M (NOLOCK)
                        WHERE M.MARGINDATE BETWEEN @MARGINDATE AND @MARGINDATE  +' 23:59'                    
                        GROUP BY
                              M.PARTY_CODE
                        ) A
...
BEGIN                                       
	...
	BEGIN
		...
		SELECT PARTY_CODE,TDAY_MARGIN,TDAY_MTM,
		SUM(TDAY_MARGIN+TDAY_MTM) AS TOTAL_MARGIN,0 AS PEAK_MARGIN ,
		MARGINDATE
		FROM TBL_COMBINE_REPORTING WHERE MARGINDATE =@SAUDA_DATE   ---AND PARTY_CODE ='HEMC004'
		GROUP BY PARTY_CODE,TDAY_MARGIN,TDAY_MTM,MARGINDATE
		UNION ALL
		SELECT PARTY_CODE,0,0,SUM(0+0) AS TOTAL_MARGIN,TDAY_MARGIN AS PEAK_MARGIN ,MARGINDATE
		FROM  TBL_COMBINE_REPORTING_PEAK WHERE MARGINDATE =@SAUDA_DATE 
		--AND PARTY_CODE ='HEMC004'
		GROUP BY PARTY_CODE,TDAY_MARGIN,MARGINDATE
		)A
		  GROUP BY PARTY_CODE,MARGINDATE
		
		Select Party_code,FLAG=(Case when TOTAL_MARGIN > PEAK_MARGIN Then 'M' ELSE 'P' END) 
		into #final from #FIN 
	...
	--	FROM	TBL_EPN_BENEFIT T, CLIENT_DETAILS C (NOLOCK)  
		FROM    TBL_COMBINE_REPORTING_DETAIL T(NOLOCK),#final F (NOLOCK)  , CLIENT_DETAILS C (NOLOCK)
	...
	INSERT INTO #MAR_REPORT  
		SELECT	EXCHANGE='EPN', SEGMENT='EPN',
				...
				DEL_MARGIN = 0                  
	--	FROM	TBL_EPN_BENEFIT T, CLIENT_DETAILS C (NOLOCK)  
		FROM    TBL_COMBINE_REPORTING_PEAK_DETAIL T(NOLOCK),#final F (NOLOCK)  , CLIENT_DETAILS C (NOLOCK)
		WHERE	MARGINDATE BETWEEN @SAUDA_DATE AND @SAUDA_DATE + ' 23:59:59'
				AND T.PARTY_CODE = F.PARTY_CODE
				AND T.PARTY_CODE = C.PARTY_CODE
				AND  T.EXCHANGE='EPN'
