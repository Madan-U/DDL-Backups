-- Object: PROCEDURE dbo.RPT_CUSTOM_EDT_LEDGER
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE PROCEDURE [dbo].[RPT_CUSTOM_EDT_LEDGER] (@TODATE VARCHAR(11))

AS --- EXEC RPT_CUSTOM_EDT_LEDGER 'OCT 07 2022'
...
WHERE EDT LIKE @FROMDATE + '%' AND VTYP = 18 AND CLTCODE IN (SELECT PARTY_CODE FROM IMPORT_PARTY_CODE)
GROUP BY CLTCODE 
UNION ALL
...
WHERE B.VDT < @FROMDATE AND EDT >= @FROMDATE AND CLTCODE IN (SELECT PARTY_CODE FROM IMPORT_PARTY_CODE)
GROUP BY CLTCODE
UNION ALL
...
WHERE L.EDT >= @FROMDATE + ' 00:00:00' 	AND L.EDT <= @TODATE + ' 23:59:59' AND VTYP <> 18  AND CLTCODE IN (SELECT PARTY_CODE FROM IMPORT_PARTY_CODE)
GROUP BY CLTCODE
...
AND CONVERT(DATETIME,CONVERT(VARCHAR(11),EDT))>=@FROMDATE  
UNION ALL
SELECT B.CLTCODE,DRAMOUNT=(-B.DRAMOUNT),B.CRAMOUNT FROM ANGELFO.BBO_FA.DBO.ACC_TBL1 A (NOLOCK), ANGELFO.BBO_FA.DBO.ACC_TBL3 B (NOLOCK)
WHERE A.SNO = B.MASTERSNO
AND CONVERT(DATETIME,CONVERT(VARCHAR(11),EDT)) >= @FROMDATE + ' 00:00:00' 
AND CONVERT(DATETIME,CONVERT(VARCHAR(11),EDT)) <= @TODATE + ' 23:59:59' AND A.VTYPE <> 18   ) A 
WHERE CLTCODE IN (SELECT PARTY_CODE FROM IMPORT_PARTY_CODE) GROUP BY A.CLTCODE
...
SELECT EDT_REPORT_DATE=CONVERT(VARCHAR(11),@TODATE,105),CLTCODE,[NSECM]=ISNULL([NSECM],0),[BSECM]=ISNULL([BSECM],0),
...
+ISNULL([SLBS],0)+ISNULL([NSECD],0)+ISNULL([BSECD],0)+ISNULL([MCXCD],0)+ISNULL([MCDX],0)+ISNULL([NCDX],0) INTO #RPT_EDT_LEDGER
FROM ( SELECT CLTCODE,LEDBAL,EXCHANGE FROM #EDTLED WHERE CLTCODE IN (SELECT PARTY_CODE FROM MSAJAG.DBO.Client_Details WITH (NOLOCK))) AS SOURCETABLE
PIVOT
(SUM(LEDBAL) FOR EXCHANGE IN ([NSECM],[BSECM],[NSEFO],[BSEFO],[BSEMF],[MTF],[SLBS],[NSECD],[BSECD],[MCXCD],[MCDX],[NCDX])) AS PIVOTTABLE
ORDER BY PIVOTTABLE.CLTCODE

SELECT * FROM #RPT_EDT_LEDGER ORDER BY CLTCODE
