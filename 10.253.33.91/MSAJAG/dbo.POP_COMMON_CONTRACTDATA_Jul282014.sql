-- Object: PROCEDURE dbo.POP_COMMON_CONTRACTDATA_Jul282014
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

create PROCEDURE [dbo].[POP_COMMON_CONTRACTDATA_Jul282014] (  
 @FROMDATE VARCHAR(11),  
 @USERNAME VARCHAR(20) = ''  
 )  
AS  
/*  
EXEC POP_COMMON_CONTRACTDATA 'JUL 24 2012','ABC'  
...
BEGIN  
 ...
 BEGIN  
  ...
  SET @STRSQL = @STRSQL + ' ' + @SHARESERVER + '.' + @SHAREDB + '.DBO.CONTRACT_MASTER WITH (NOLOCK) WHERE CONVERT(DATETIME,START_DATE,103) BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'' AND PARTY_CODE BETWEEN ''' + @FROMPARTY + ''' AND ''' + @TOPARTY + ''''  
  ...
  SET @STRSQL = @STRSQL + ' AND LEFT(S.SAUDA_DATE,11) = LEFT(#CONTRACT.SAUDA_DATE,11) '  
 ...
 ELSE  
  ...
  BEGIN  
   ...
   BEGIN  
    ...
    SET @STRSQL = @STRSQL + ' ELSE RTRIM(INST_TYPE) + '' '' + RTRIM(SYMBOL) + '' '' + LEFT(EXPIRYDATE,11) + '' ''   
         + RTRIM(CAST(CAST(STRIKE_PRICE AS NUMERIC(18,0)) AS VARCHAR)) + '' '' + RTRIM(OPTION_TYPE) '  
    SET @STRSQL = @STRSQL + ' END),'  
   END  
   ...
   ELSE  
    ...
    BEGIN  
     ...
     SET @STRSQL = @STRSQL + ' WHEN INST_TYPE LIKE ''FUT%'' THEN RTRIM(INST_TYPE) + '' '' + RTRIM(SYMBOL) + '' '' + LEFT(EXPIRYDATE,11) '  
     SET @STRSQL = @STRSQL + ' ELSE RTRIM(INST_TYPE) + '' '' + RTRIM(SYMBOL) + '' '' + LEFT(EXPIRYDATE,11) + '' ''   
           + RTRIM(CAST(CAST(STRIKE_PRICE AS NUMERIC(18,2)) AS VARCHAR)) + '' '' + RTRIM(AUCTIONPART) '  
     SET @STRSQL = @STRSQL + ' END),'  
    END  
    ELSE  
    BEGIN  
...
UPDATE #CONTRACT  
SET CONTRACT_HEADER_DET = EXCHANGE + ' - ' + SEGMENT + ' - ' + (CASE WHEN SEGMENT = 'CAPITAL' THEN SETTTYPE_DESC + ' (' + LTRIM(RTRIM(SETT_NO)) + ' - ' + LTRIM(RTRIM(SETT_TYPE)) + ' - ' + LTRIM(RTRIM(SETT_DATE)) + ' - ' + LTRIM(RTRIM(CONTRACTNO)) + ')' ELSE LTRIM(RTRIM(CONTRACTNO)) END)  
