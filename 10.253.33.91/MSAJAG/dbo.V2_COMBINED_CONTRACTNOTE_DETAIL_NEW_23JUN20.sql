-- Object: PROCEDURE dbo.V2_COMBINED_CONTRACTNOTE_DETAIL_NEW_23JUN20
...
CREATE PROCEDURE [dbo].[V2_COMBINED_CONTRACTNOTE_DETAIL_NEW_23JUN20]                       
...
--SELECT @PRINTF = 'ALL'                                                
 ...
 SELECT @INTEROP = CLEARINGCODE FROM TBL_INTEROP_SETTING
...
UPDATE	#CONTRACT SET INTEROPCODE = @INTEROP
WHERE	#CONTRACT.SEGMENT = 'CAPITAL'


UPDATE	#CONTRACT
SET		SETT_NO = ISNULL(C.SETT_NO,''),
		SETT_DATE = ISNULL(C.SETT_DATE,''),
		SETT_TYPE = (
		CASE 
			WHEN #CONTRACT.SETT_TYPE = 'D' AND INTEROPCODE = 'NSCCL' THEN 'N'
			WHEN #CONTRACT.SETT_TYPE = 'C' AND INTEROPCODE = 'NSCCL' THEN 'W'
			WHEN #CONTRACT.SETT_TYPE = 'AD' AND INTEROPCODE = 'NSCCL' THEN 'A'
			WHEN #CONTRACT.SETT_TYPE = 'AC' AND INTEROPCODE = 'NSCCL' THEN 'X'
			WHEN #CONTRACT.SETT_TYPE = 'N' AND INTEROPCODE = 'ICCL' THEN 'D'
			WHEN #CONTRACT.SETT_TYPE = 'W' AND INTEROPCODE = 'ICCL' THEN 'C'
			WHEN #CONTRACT.SETT_TYPE = 'A' AND INTEROPCODE = 'ICCL' THEN 'AD'
			WHEN #CONTRACT.SETT_TYPE = 'X' AND INTEROPCODE = 'ICCL' THEN 'AC'
			ELSE #CONTRACT.SETT_TYPE
		END),
		SETTTYPE_DESC = (
		CASE
...
SET		SETT_NO = ISNULL(SM.SETT_NO,''),
		...
		CASE
			...
			ELSE #CONTRACT.SETTTYPE_DESC
		END)
FROM	ANAND.BSEDB_AB.DBO.SETT_MST SM WITH(NOLOCK)
WHERE	SM.START_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59:59'
		AND LEFT(CONVERT(DATETIME,#CONTRACT.SAUDA_DATE,103),11) = LEFT(SM.START_DATE,11)
...
/*
UPDATE	#CONTRACT
SET		SETT_NO = BSEDB_AB.DBO.CONVERTSETTNO(#CONTRACT.SETT_NO,#CONTRACT.SAUDA_DATE,1)
