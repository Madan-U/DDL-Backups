-- Object: PROCEDURE dbo.RPT_PNL_SHRT_LONG_DATATEST
...
CREATE PROC [DBO].[RPT_PNL_SHRT_LONG_DATATEST]                                                                          
...
REGION  = CONVERT(VARCHAR(20),'')                                          
 ...
 --WHERE 1 = 2                               
 WHERE T.PARTY_CODE IN (SELECT CLTCODE FROM OLDNEW)   
...
CL_CODE  IN (SELECT CLTCODE FROM OLDNEW)                                         
...
AND CL_CODE  IN (SELECT CLTCODE FROM OLDNEW)                                        
  
                          
  
DELETE C1 FROM #CLIENT_DETAILS C1, #T C2 WITH (NOLOCK)                          
  
WHERE C1.CL_CODE=C2.CL_CODE  AND   cl_code IN (SELECT cltcode FROM OLDNEW)                           
