-- Object: PROCEDURE dbo.RPT_PNL_SHRT_LONG_ANGBBG
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

                            
---EXEC RPT_PNL_SHRT_LONGBBG 'BROKER','BROKER','APR  1 2011','MAR 31 2012','A0001','AZ999','','Y', 'S','W'--944                                      
                                        
CREATE PROC [dbo].[RPT_PNL_SHRT_LONG_ANGBBG]                                                                      
(                                                                        
 @STATUSID VARCHAR(25),                                                                        
...
REGION  = ''                                      
 FROM TBL_PNL_DATA T WITH (NOLOCK) --, #CLIENT_DETAILS C WITH (NOLOCK)                                                                        
 WHERE  SAUDA_DATE <= @TODATE + ' 23:59'                                                  
 AND OPPDATE > @TODATE                                                              
 AND BILLFLAG <> 0 --(CASE WHEN @RPTOPENSELL = 'W' THEN 0 ELSE 5 END)                                            
 ...
 SCRIP_CD, SERIES, BSECODE, ISIN, PNLRATE, SELL_BUY, OPPDATE,                                
 BILLFLAG                                                                  
 --select distinct billflag from TBL_PNL_DATA where sauda_date >='JUL 11 2024' and sauda_date <='JUL 11 2024 23:59'
