...
/*getting Client List*/                  
   ...
   C1.Long_Name,            
      ...
      LEFT OUTER JOIN             
            nsefo.dbo.Client4 C4 WITH(NoLock)             
            ON             
            (            
...
/*getting Lddger Entries*/                  
      ...
      SELECT             
            L.Booktype,             
 Voudt = CONVERT(VARCHAR(11),L.Vdt,120),             
            Effdt = L.Edt,             
            L.Shortdesc,             
            Dramt = (            
            CASE             
...
/*updating Bank Name*/                                 
      ...
      FROM nsefo.dbo.Pobank B WITH(NoLock)             
      WHERE Ltrim(Rtrim(Cast(B.Bankid AS Char))) = Ltrim(Rtrim(#tmpledger.Bank_name))             


/* extra patch added to update the billposting time */
UPDATE T SET PDT = ISNULL(ENDTIME, T.PDT) FROM #tmpledger T, BILLPOSTED P, (select SETT_NO, SETT_TYPE, ENDTIME = MAX(ENDTIME) from BILLLOG GROUP BY SETT_NO, SETT_TYPE) B
WHERE T.VNO = P.VNO AND T.VTYP = 15 AND P.SETT_NO = B.SETT_NO AND P.SETT_TYPE = B.SETT_TYPE
/* extra patch added to update the billposting time */
