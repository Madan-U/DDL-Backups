-- Object: PROCEDURE dbo.NSECMVALAN_bakmar12016
...
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','START','DELETE FROM #CMBILLVALAN',GETDATE(),GETDATE())                                
                                
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','DONE-DELETE FROM #CMBILLVALAN. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'INSERT NORMAL TRADE FROM SETTLEMENT W/O ND SCRIP',GETDATE(),GETDATE())                    
...
OTHER_CHRG = SUM(CASE WHEN DISPCHARGE = 1                                 
     ...
     ELSE 0                                 
       END),                                  
REGION=ISNULL(REGION,''), START_DATE='', END_DATE='', UPDATE_DATE=LEFT(CONVERT(VARCHAR,GETDATE(),109),11), STATUS_NAME=@STATUSNAME,                                
EXCHANGE='NSE',SEGMENT='CAPITAL',MEMBERTYPE=MEMBERCODE,COMPANYNAME='',DUMMY1='',DUMMY2='',DUMMY3='',DUMMY4=0,DUMMY5=0,  AREA = ISNULL(AREA,''),              
PTRDRATE = SUM(CASE               
              WHEN SELL_BUY = 1              
                   AND BILLFLAG = 2              
...
WHERE S.SETT_NO = @SETT_NO AND S.SETT_TYPE = @SETT_TYPE AND TRADEQTY > 0                
...
C1.FAMILY,TRADER,SUB_BROKER,PARTIPANTCODE,MEMBERCODE,                                
SERVICE_CHRG,BRANCH_CD,CL_TYPE, DUMMY8, DUMMY9, DUMMY10, REGION, AREA                              
    
                                
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','DONE-INSERT NORMAL TRADE FROM SETTLEMENT W/O ND SCRIP. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'INSERT INSTITUTIONAL TRADE FROM ISETTlement',GETDATE(),GETDATE())
...
*/        
  ...
  */        
  ...
  WHERE  #CMBILLVALAN.SETT_NO = @SETT_NO              
  ...
  END)              
      ...
      (CASE WHEN PBROKTRD - (CASE               
       ...
       ELSE 0 END))) >= 0              
         ...
         AND NOT EXISTS (SELECT C.PARTY_CODE FROM #CMBILLVALAN C where C.SETT_NO = @SETT_NO              
...
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','DONE-INSERT INSTITUTIONAL TRADE FROM ISETTlement. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'UPDATE SCRIPNAME/SECTOR/TRACK FROM SCRIP1 & 2',GETDATE(),GETDATE())   
...
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','DONE-UPDATE SCRIPNAME/SECTOR/TRACK FROM SCRIP1 & 2. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'UPDATE ISIN FROM MULTIISIN',GETDATE(),GETDATE())                     
...
AND #CMBILLVALAN.SETT_NO = @SETT_NO AND #CMBILLVALAN.SETT_TYPE = @SETT_TYPE                             
                             
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','DONE-UPDATE ISIN FROM MULTIISIN. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'UPDATE FAMILY_NAME FROM CLIENT1',GETDATE(),GETDATE())                                
                                
UPDATE #CMBILLVALAN SET FAMILY_NAME = C1.Short_Name FROM CLIENT1 C1                                
WHERE C1.CL_CODE = #CMBILLVALAN.FAMILY                                 
AND #CMBILLVALAN.SETT_NO = @SETT_NO AND #CMBILLVALAN.SETT_TYPE = @SETT_TYPE                                
AND #CMBILLVALAN.SAUDA_DATE LIKE @SDATE + '%'                                
AND #CMBILLVALAN.SCRIP_CD LIKE @SCRIP_CD AND #CMBILLVALAN.SERIES LIKE @SERIES                                
AND #CMBILLVALAN.PARTY_CODE LIKE @PARTY_CODE                                
                                
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','DONE-UPDATE FAMILY_NAME FROM CLIENT1. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'UPDATE START/ENDDATE FROM SETT_MST',GETDATE(),GETDATE())                           
...
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','DONE-UPDATE START/ENDDATE FROM SETT_MST. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'UPDATE REGION FROM CLIENTSETTINGS',GETDATE(),GETDATE())                         
...
INSERT INTO PROCESS_MONITOR VALUES ('VALAN GENERATION',@SETT_NO + @SETT_TYPE,'','NSECMVALAN','DONE-UPDATE REGION FROM CLIENTSETTINGS. RECCOUNT: ' + CONVERT(CHAR,@@ROWCOUNT),'RETURN TO NSEVALANGENERATE',GETDATE(),GETDATE())
