-- Object: PROCEDURE dbo.Bbgdirectgencontract_BAK_06NOV2019
...
CREATE PROCEDURE [dbo].[Bbgdirectgencontract_BAK_06NOV2019]        
...
IF EXISTS(SELECT * FROM sys.indexes WHERE object_id = object_id('dbo.BBGSettlement') AND NAME ='IDXPARTY')
BEGIN
DROP INDEX [IDXPARTY] ON [dbo].[BBGSettlement]
END
...
BEGIN
CREATE CLUSTERED INDEX [Partysett] ON [dbo].[BBGSettlement](sett_type ASC, PARENTCODE ASC) 
WITH (PAD_INDEX = OFF, ALLOW_PAGE_LOCKS = ON, ALLOW_ROW_LOCKS = ON, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, SORT_IN_TEMPDB = OFF) ON [THIRD];
END

IF NOT EXISTS(SELECT * FROM sys.indexes WHERE object_id = object_id('dbo.BBGSettlement') AND NAME ='IDXPARTY')
BEGIN
CREATE NONCLUSTERED INDEX [IDXPARTY] ON [dbo].[BBGSettlement](Party_Code ASC) 
