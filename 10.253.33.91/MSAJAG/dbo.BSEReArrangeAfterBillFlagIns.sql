-- Object: PROCEDURE dbo.BSEReArrangeAfterBillFlagIns
...
CREATE procedure
	[dbo].[BSEReArrangeAfterBillFlagIns]
	(
		@Sett_No varchar(7),
		@Sett_Type varchar(2),
...
Insert into errorlog Values (Getdate(), 'In BseRearrangeAfterbillflag  ', + @Party_code + '  ' +@Scrip_cd +'      ' + @Sett_type +'   '+@Sett_no ,' ','  ')
 ...
 Set @@Flag = Cursor for
 Select Isnull(Max(Convert(Int,Billno)),0) from iSettlement where sett_no = @Sett_no and sett_type = @Sett_type  and party_code = @Party_code
 Open @@Flag
 Fetch Next from @@Flag into @@Billno

 Select @@Tran_cat = Tran_cat from Client2 where Party_code = Rtrim(Ltrim(@Party_code))

 If (  ( @@Fetch_status <> 0 )  Or ( @@Billno = 0 ))
 Begin
           Print 'in Bill no = 0'
           Set @@Flag = Cursor for
           Select Isnull(Max(Convert(Int,Billno)),0)  from iSettlement where  Sett_no = @Sett_no and Sett_type = @Sett_type
           Open @@Flag
           Fetch Next from @@Flag into @@BillNo
...
End
 ...
 Begin
      ...
      Update isettlement Set Billflag = SettFlag ,Billno = @@Billno
 ...
 Begin
      If @@Tran_cat = 'TRD'
      Begin
      Update isettlement Set Billflag = (Case When Sell_Buy = 1 then 2 else 3 end ) ,Billno = @@Billno
      Where Party_code = @Party_code
      and Scrip_cd = @Scrip_Cd
 and Series = @Series
 and Sett_no = @Sett_no
 ...
 If  @@Sqty = 0
 Begin
      Update isettlement Set BillFlag = 4 ,Tmark = 'D'
      Where Party_code = @Party_code
      and Scrip_cd = @Scrip_cd
      and Series = @Series
      and Sell_buy = 1
  ...
  If  @@Pqty = 0
  Begin
       Update isettlement Set BillFlag = 5 ,Tmark = 'D'
       Where party_code = @Party_code
       and Scrip_cd = @Scrip_cd
       and Series = @Series
       and Sell_buy = 2
...
/*
  If @@Pqty = @@Sqty
  Begin
       Update isettlement Set Billflag = (Case When Sell_Buy = 1 then 2 else 3 end )
       Where Party_code = @Party_code
       and Scrip_cd = @Scrip_Cd
       and Series = @Series
       and Sett_no = @Sett_no
 ...
 */
  ...
  Begin
       ...
       Begin
            If @@Pdiff >= @@Lqty
            Begin
                 Update isettlement Set Billflag = 4
                 Where Party_code = @Party_code
                 and Scrip_cd = @Scrip_cd
                 and Series = @Series
                 and Sell_buy = 1
            ...
            Begin
                 ...
                 Update isettlement Set Billflag = 2,Tradeqty = @@Lqty - @@Pdiff,Trade_Amount = (@@Lqty - @@pdiff)*MarketRate
                 Where Party_code = @Party_code
                 and Scrip_cd = @Scrip_cd
                 and Series = @Series
                 and Sell_Buy = 1
 ...
 Begin
      ...
      Begin
           ...
           Begin
                ...
                and Sett_Type = @Sett_type
                and Partipantcode = @Participantcode
                Update isettlement Set Billflag = 3,Tradeqty = @@Lqty - @@Pdiff,Trade_Amount = (@@Lqty - @@pdiff)*MarketRate
                Where Party_code = @Party_code
                and Scrip_cd = @Scrip_cd
                and Series = @Series
                and Sell_Buy = 2
...
End
/*
update isettlement set tmark = 'D' where party_code = @Party and scrip_cd = @scrip and series = @series
and Sett_no = @Sett_No and Sett_Type = @Sett_type
and tmark <> 'D'  and billflag > 3  */

/* EXEC NEWUPDBILLTAXafterbillins  @Sett_No,@Sett_Type,@Party_code,@Scrip_cd */
...
begin
	...
	(
		...
		'BSEReArrangeAfterBillFlagIns',	/*called_from*/
		getdate(),	/*timestamp*/
