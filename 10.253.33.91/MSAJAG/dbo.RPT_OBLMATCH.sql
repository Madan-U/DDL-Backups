-- Object: PROCEDURE dbo.RPT_OBLMATCH
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE  PROC [dbo].[RPT_OBLMATCH]       
(@SETT_NO VARCHAR(7), @SETT_TYPE VARCHAR(2), @TDATE VARCHAR(11), @OBLFLAG VARCHAR(1))      
AS      


EXEC RPT_OBLMATCH_NEW @SETT_NO, @SETT_TYPE, @TDATE, @OBLFLAG, 1


/*
-- EXEC RPT_OBLMATCH '2019502','N','JUN 22 2019','Y'
...
BEGIN
      ...
      BEGIN 
            ...
            SELECT @OTHER_SETT_NO = SETT_NO FROM bsedb.DBO.Sett_Mst 
            Where Start_date >= @Start_Date and Start_date <= @Start_Date + ' 23:59' 
            And Sett_Type = @OTHER_SETT_TYPE  
       
            INSERT INTO #BSE_OBL
            EXEC BSEDB..RPT_OBLMATCH @OTHER_SETT_NO, @OTHER_SETT_TYPE
...
SELECT       
PARTICIPANTCODE=(CASE WHEN PARTICIPANTCODE = '' OR PARTICIPANTCODE = '0' 
                                THEN MEMBERCODE 
                              ELSE PARTICIPANTCODE 
                         END), 
SCRIP_CD, SERIES,       
...
--WHERE SCRIP_CD = 'ICICIBANK'

--SELECT * FROM #SETT_FIN
--WHERE SCRIP_CD = 'ICICIBANK'
