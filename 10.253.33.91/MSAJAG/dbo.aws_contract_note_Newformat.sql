-- Object: PROCEDURE dbo.aws_contract_note_Newformat
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


--aws_contract_note '2020-05-21','A' --,'2020-04-15'
--CREATE THE SP UNDER SRE-35529

CREATE  proc [dbo].[aws_contract_note_Newformat] (@dt datetime,@cnflag VARCHAR(2))
as
...
AND T.PARTY_CODE =O.CD_Party_Code
 ...
 /* New Format **/
  ...
  CMclosing =(Case when OPTION_TYPE ='' Then CL_RATE Else '' END) ,
 ...
 Select PARTY_CODE,'3',PARTY_CODE+'|S|CAPITAL|'+ISIN+'|'+SCRIP_NAME+'|'+CONVERT(VARCHAR,PQTY)+'|'+CONVERT(VARCHAR,P_Rate_before_Brok)+'|'+CONVERT(VARCHAR,P_Brok_unit)+'|'+CONVERT(VARCHAR,P_Rate_After_Brok)+'|'+CONVERT(VARCHAR,P_VALUE)+'|'+CONVERT(VARCHAR,SQTY)
+'|'+CONVERT(VARCHAR,S_Rate_before_Brok)+'|'+CONVERT(VARCHAR,S_Brok_unit)+'|'+CONVERT(VARCHAR,S_Rate_After_Brok)+'|'+CONVERT(VARCHAR,S_VALUE) +'|'+CONVERT(VARCHAR, PQTY-SQTY)+'|'+ CONVERT(VARCHAR,S_VALUE-P_VALUE),'' A,''B,''C,''D ,EXCHANGE='CAPITAL' 
from (
...
P_Rate_After_Brok =(Case when PQTY <>0 Then P_VALUE/PQTY  Else 0  End),P_VALUE ,SQTY,
...
S_Rate_After_Brok =(Case when SQTY <>0 Then S_VALUE/SQTY  Else 0  End),S_VALUE
from (
SELECT PARTY_CODE,ISIN,SCRIP_NAME,PQTY=SUM(PQTYTRD+PQTYDEL),P_VALUE=SUM(PAMTTRD+PAMTDEL),P_BROK_Total=SUM(PBROKTRD+PBROKDEL),
SQTY=SUM(SQTYTRD+SQTYDEL),S_VALUE=SUM(SAMTTRD+SAMTDEL),S_BROK_Total=SUM(SBROKTRD+SBROKDEL)FROM (
...
UNION ALL
Select PARTY_CODE,'4',PARTY_CODE+'|S|FUTURES|'+Sec_Name+'|'+ TRADETYPE+'|'+Convert(varchar,BUYQTY)+'|'+Convert(varchar,Foreign_price)+'|'+Convert(varchar,BUYRATE)+'|'+Convert(varchar,Brok_per_unit)+'|'+	Convert(varchar,price_after_brok)+'|'+Convert(varchar,CMclosing)+'|'+Convert(varchar,Net_Amount)+'| ',SNO A,''B,''C,''D ,EXCHANGE='FUTURES'   from (
...
TRADETYPE='BUY' ,BUYQTY,'' As Foreign_price,BUYRATE ,BROKERAGE/BUYQTY as Brok_per_unit ,
BUYRATE+(BROKERAGE/BUYQTY) as price_after_brok,(Case when left(INST_TYPE,3)='OPT' Then PBIllamt else   BUYQTY*(CLOSING_RATE-(BUYRATE+(BROKERAGE/BUYQTY))) end) as Net_Amount  ,CMclosing,SNO
...
SELLRATE,BROKERAGE/SELLQTY as SBrok_per_unit ,SELLRATE-(BROKERAGE/SELLQTY) ,
(Case when  left(INST_TYPE,3)='OPT' Then SBIllamt else   SELLQTY*((SELLRATE-(BROKERAGE/SELLQTY)) -CLOSING_RATE) end),CMclosing ,SNO
...
SELECT 'CONTRACT NOTE PROCESS IN REVISED FORMAT IS COMPLETED SUCESSFULLY...!!!'  AS [STATUS]
