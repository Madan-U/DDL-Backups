-- Object: PROCEDURE dbo.RPT_DAILY_MG13
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE PROCEDURE [dbo].[RPT_DAILY_MG13] (@RUNDATE VARCHAR(11),@PARTY VARCHAR(11),@RPT VARCHAR(11))  
...
----- ADDED CONDITION ON DT 28TH JUN 2024 BY VAIBHAV K / SRE-26921 -----------------------------
IF @RPT = 'NCE'  

BEGIN  
SELECT [Trade Date]=convert(varchar(11),mdate,105),[Client Code]=party_code,[Span Margin]=spreadmargin,  
Filler=NonSpreadMargin,[Extreme Loss Margin]=mtom,[Delivery margin]=DEL_MARGIN,[Cystallized Obligation]=mtomloss,  
[Total Margin to be Collected]=TotalMargin,Peakmargin,[Client Flag]=Cl_Type,Exchange='NCE'  
from ANGELCOMMODITY.NCE.DBO.FOMARGINNEW with (nolock)  
WHERE MDATE BETWEEN @RUNDATE AND @RUNDATE + ' 23:59'   
AND PARTY_CODE = @PARTY  
ORDER BY PARTY_CODE  
END  
