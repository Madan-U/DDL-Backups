-- Object: PROCEDURE dbo.V2_CONTCUMBILL_SECTION_HISTORY_text
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE PROC [DBO].[V2_CONTCUMBILL_SECTION_HISTORY_text]      
(      
 @STATUSID VARCHAR(15),      
...
IF (SELECT ISNULL(COUNT(1),0) FROM (SELECT TOP 1 PARTY_CODE FROM HISTORY WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE) A) > 0       
BEGIN      
...
IF @BRANCHFLAG='BRANCH'    
  ...
  WHERE T.BRANCH_CD >= @FROMBRANCH                
              AND T.BRANCH_CD <= @TOBRANCH     
 ELSE    
  SELECT * FROM CONTRACTTEXT T LEFT OUTER JOIN BRANCHGROUP BG       
      ON T.BRANCH_CD=BG.BRANCH_CODE                  
  WHERE  BG.BRANCH_GROUP >= @FROMBRANCH      
  AND BG.BRANCH_GROUP<= @TOBRANCH                
