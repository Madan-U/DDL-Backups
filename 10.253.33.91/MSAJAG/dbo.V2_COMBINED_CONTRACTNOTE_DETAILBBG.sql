-- Object: PROCEDURE dbo.V2_COMBINED_CONTRACTNOTE_DETAILBBG
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE PROCEDURE [dbo].[V2_COMBINED_CONTRACTNOTE_DETAILBBG]                       
...
Select * into #COMMON_DATA from  (Select top 1 * from COMMON_CONTRACT_DATA )C
  
Insert INTO #COMMON_DATA Select * FROM COMMON_CONTRACT_DATA C (NOLOCK)   
...
And Sauda_Date >='JUL  1 2020'

Insert INTO #COMMON_DATA Select * FROM COMMON_CONTRACT_DATA_Arhive C (NOLOCK)   
WHERE SAUDA_DATE >= @FROMDATE AND SAUDA_DATE<= @TODATE + ' 23:59:59'   
AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY   
--AND BRANCH_CD BETWEEN @FROMBRANCH AND @TOBRANCH  
AND SUB_BROKER BETWEEN @FROMSUB_BROKER AND @TOSUB_BROKER  
