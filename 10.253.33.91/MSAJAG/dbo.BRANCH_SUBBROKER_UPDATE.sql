...
delete from BRANCH_SUBBROKER where BRANCH_CODE_NEW is null or 	SUB_BROKER_NEW	is null or TRADER_NEW is null or	AREA_NEW is null or	REGION_NEW is null 
...
SELECT @@ERROR_COUNT=COUNT(1)                 
FROM BRANCH_SUBBROKER T WHERE BRANCH_CODE_NEW  NOT IN (SELECT DISTINCT  R.BRANCH_CODE FROM MSAJAG..REGION R, MSAJAG..BRANCH  B
 where R.branch_code=B.BRANCH_CODE and T.REGION_NEW =R.Regioncode and T.BRANCH_CODE_NEW=B.BRANCH_CODE )                 
                
IF @@ERROR_COUNT > 0    
 BEGIN                
 SELECT  'BRANCH CODE & Region Code Not Matched'                
 UNION ALL                
 SELECT 'Region_Code'              
 UNION ALL            
 SELECT '------------------'                
 UNION ALL                
 SELECT BRANCH_CODE_NEW +'-'+REGION_NEW                
 FROM BRANCH_SUBBROKER  T WHERE BRANCH_CODE_NEW  NOT IN (SELECT DISTINCT  B.BRANCH_CODE FROM MSAJAG..REGION R, MSAJAG..BRANCH  B
 where R.branch_code=B.BRANCH_CODE and T.REGION_NEW =R.Regioncode and T.BRANCH_CODE_NEW=B.BRANCH_CODE )                
 ...
 SELECT @@ERROR_COUNT=COUNT(1) FROM BRANCH_SUBBROKER T WHERE SUB_BROKER_NEW NOT IN (SELECT DISTINCT  SUB_BROKER FROM MSAJAG..SUBBROKERS S, MSAJAG..BRANCH  B
 where Ltrim(rtrim(S.branch_code))=Ltrim(rtrim(B.BRANCH_CODE)) and T.SUB_BROKER_NEW=s.Sub_Broker and Ltrim(rtrim(T.BRANCH_CODE_NEW))=Ltrim(rtrim(B.BRANCH_CODE)) )                
...
IF @@ERROR_COUNT > 0                                                                          
 ...
 FROM BRANCH_SUBBROKER T WHERE SUB_BROKER_NEW NOT IN (SELECT DISTINCT  SUB_BROKER FROM MSAJAG..SUBBROKERS S, MSAJAG..BRANCH  B
 where Ltrim(rtrim(S.branch_code))=Ltrim(rtrim(B.BRANCH_CODE)) and T.SUB_BROKER_NEW=s.Sub_Broker and Ltrim(rtrim(T.BRANCH_CODE_NEW))=Ltrim(rtrim(B.BRANCH_CODE)) )                
...
SET @@ERROR_COUNT = 0           
 ...
 SELECT @@ERROR_COUNT=COUNT(1) FROM BRANCH_SUBBROKER T WHERE TRADER_NEW NOT IN (SELECT DISTINCT SHORT_NAME FROM MSAJAG..BRANCHES S, MSAJAG..BRANCH  B
 where Ltrim(rtrim(S.branch_cd))=Ltrim(rtrim(B.BRANCH_CODE)) and T.TRADER_NEW=s.SHORT_NAME and Ltrim(rtrim(T.BRANCH_CODE_NEW))=Ltrim(rtrim(B.BRANCH_CODE)) )              
                
IF @@ERROR_COUNT > 0                                                                          
    BEGIN                                                                             
    SELECT 'TRADER CODE & Branch Code Not Matching'                
    UNION ALL                
 SELECT 'BRANCH_CD'                
 UNION ALL                
 SELECT '------------------'                
 UNION ALL                
 SELECT TRADER_NEW                 
 FROM BRANCH_SUBBROKER T WHERE TRADER_NEW NOT IN (SELECT DISTINCT SHORT_NAME FROM MSAJAG..BRANCHES S, MSAJAG..BRANCH  B
 where Ltrim(rtrim(S.branch_cd))=Ltrim(rtrim(B.BRANCH_CODE)) and T.TRADER_NEW=s.SHORT_NAME and Ltrim(rtrim(T.BRANCH_CODE_NEW))=Ltrim(rtrim(B.BRANCH_CODE)) )                
...
SET @@ERROR_COUNT = 0                
 ...
 SELECT @@ERROR_COUNT=COUNT(1) FROM BRANCH_SUBBROKER T WHERE AREA_NEW NOT IN (SELECT DISTINCT  AREACODE FROM MSAJAG..AREA S, MSAJAG..BRANCH  B
 where Ltrim(rtrim(S.Branch_Code))=Ltrim(rtrim(B.BRANCH_CODE)) and T.AREA_NEW=s.AREACODE and Ltrim(rtrim(T.BRANCH_CODE_NEW))=Ltrim(rtrim(B.BRANCH_CODE)) )  
                
IF @@ERROR_COUNT > 0                                                                          
    BEGIN                                                                             
    SELECT 'AREA CODE DOSE NOT ESIXT IN MASTER Or Branch Code and Area is Mismatching'                
    UNION ALL                
 SELECT 'AREACODE'                
 UNION ALL                
 SELECT '------------------'    
 UNION ALL        
 SELECT AREA_NEW                 
 FROM BRANCH_SUBBROKER T WHERE AREA_NEW NOT IN (SELECT DISTINCT  AREACODE FROM MSAJAG..AREA S, MSAJAG..BRANCH  B
 where Ltrim(rtrim(S.Branch_Code))=Ltrim(rtrim(B.BRANCH_CODE)) and T.AREA_NEW=s.AREACODE and Ltrim(rtrim(T.BRANCH_CODE_NEW))=Ltrim(rtrim(B.BRANCH_CODE)) )             
