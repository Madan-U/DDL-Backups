-- Object: PROCEDURE dbo.aws_Margin_Statement_Holiday
...
CREATE   proc [dbo].[aws_Margin_Statement_Holiday] (@dt datetime)
as
...
EXEC RPT_DAILYMARGINSTATEMENT_Aws_Holiday @sdt,'a001','zzzzz','','zzzzzzzzzzz','','zzzzzzzzzzz','broker','broker',1,0,'C','ALL'
...
FROM aws_margin_details D,anand1.msajag.dbo.CLIENT_DETAILS C
...
set @NSEQUERY = @NSEQUERY + ' from anand1.msajag.dbo.Margin_process  order by party_code ,sno,SRNO " queryout J:\Comb_Margin\ContractNote_Margin_'+replace(convert(varchar,@dt,103),'/','')+'.txt -c -t"," -Sangelcommodity -Uaolinhouse -Pe$$gnfDTVs2455GZAcc'
