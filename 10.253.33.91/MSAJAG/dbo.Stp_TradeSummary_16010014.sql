-- Object: PROCEDURE dbo.Stp_TradeSummary_16010014
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

      
CREATE  Proc [dbo].[Stp_TradeSummary_16010014]        
@SaudaDate Varchar(8),        
...
@Trade_No = Right('0000000000000000' +Replace(Replace(Replace(Ltrim(Rtrim(Min( convert(int ,Trade_no)))),'R',''),'C',''),'A',''),16),        
@TradeTime = Right(Replace(Replace(Replace(Convert(Varchar,Min(Sauda_date),120),'-',''),':',''),' ',''),6)    
--@TradeTime=min(replace(Right(CpId,8),':','') )       
