-- Object: PROCEDURE dbo.AngelResetNSEBrok1_Bkup1Feb2017
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



CREATE Procedure [dbo].[AngelResetNSEBrok1_Bkup1Feb2017] (@sauda_date Varchar(11))
As
...
If @Sauda_date = ''
   Return
Select Top 1 @@Sett_no =  Sett_no from settlement where    
...
Select Party_code,ContractNo = '',Sauda_date = Cast(Left(Convert(varchar,sauda_date,109),11) As SmallDatetime) , 
Turnover = Sum(Trade_amount),NSEBrokerage = Sum(NBrokApp * Tradeqty) ,CD_Tot_Brok = 0, 
Sett_no,Sett_type = 'N',FLAG1 = 'Y',SettCount = Count(Distinct Sett_type),MaxTurnover = 0
from settlement 
...
Delete AngelNSEResetBrok where settcount = 1 and sauda_date like @Sauda_date+ '%'

Delete AngelNSEResetBrok  where party_code in(select cr_party_code from anand1.msajag.dbo.contract_rounding 
...
and sauda_date like @Sauda_date+ '%'
	...
	into #ContractRounding from  anand1.msajag.dbo.contract_rounding with (nolock) group by CR_Party_code 
...
Delete  charges_detail where cd_party_code in (Select Party_code from AngelNSEResetBrok where Nsebrokerage >= 30 and sauda_date like @Sauda_date+ '%')
And cd_sauda_date like @Sauda_date+ '%' and cd_sett_type in('N','W')
...
and
Settlement.Sauda_date like @Sauda_date +'%'
and AngelNSEResetBrok.sauda_date like @Sauda_date+ '%'


Select * into #Temp1 from (Select party_code,sett_type,Amount = Sum(trade_amount) from settlement where party_code in (select party_code from AngelNSEResetBrok where sauda_date like @Sauda_date + '%')
and sauda_date like @Sauda_date + '%'
...
( Select Party_code,Amount = Max(Amount) from #Temp1 
  ...
  and
  AngelNSEResetBrok.Sauda_date like @Sauda_date +'%'
...
Delete charges_detail where cd_party_code in(Select party_code from AngelNSEResetBrok Where Flag1 = 'Y')
and cd_Sauda_date like @Sauda_date + '%'
...
CD_DelBuySerTax = CD_TOT_BROK * (15.0000 /100), -- Service Tax Rate changed by Rajesh on Jun 01 2016, as per requirement from Vijay Mali
...
CD_TotalSerTax = CD_TOT_BROK * (15.0000 /100), -- Service Tax Rate changed by Rajesh on Jun 01 2016, as per requirement from Vijay Mali
...
Select Party_code,ContractNo,Sauda_date = Cast(Left(Convert(varchar,sauda_date,109),11) As SmallDatetime) , Turnover = Sum(Trade_amount),NSEBrokerage = Sum(NBrokApp * Tradeqty) ,CD_Tot_Brok = 0, 
Sett_no,Sett_type,FLAG1 = 'Y',SettCount = Count(Distinct Sett_type),MaxTurnover = Sum(Trade_amount),MaxTurnbrok = Cast(0 As Money)
from settlement 
where sauda_date like @Sauda_date+ '%'
and Sett_type in('N','W')
and [status] <> 'N'
...
and
Flag1 = 'N')
Group by Party_code,Cast(Left(Convert(varchar,sauda_date,109),11) As SmallDatetime),sett_no,Sett_type,Contractno
)A
...
Select * Into #Temp2 From ( select party_code,contractno,Sauda_date,Turnover,NSEBrokerage,CD_Tot_Brok = (Case When MaxTurnbrok = Cast((MaxTurnover * (2.5000/100)) As Money) Then MaxTurnbrok - NseBrokerage Else 30 - Maxturnbrok - NseBrokerage End),
Sett_no,Sett_type,Flag1 = 'Y',SettCount = 1 ,MaxTurnover
From 
#Temp)A
...
where CD_Sauda_date like @Sauda_date + '%' and CD_Trade_no <> '' and CD_Party_code in
...
Begin
     ...
     Where
          ...
          A.Todate = @@MaxDate
          and
          C.Cd_sauda_date like @Sauda_date +'%'
          and
          C.Cd_party_code >= ''
          and
          ...
          and
          Sauda_date = Cast(@sauda_date As Smalldatetime)
          and
...
(Select CONTRACTNO ='0',PARTY_CODE = CD_PARTY_CODE,ORDER_NO ='',ORDER_TIME ='',TM ='',TRADE_NO = CD_TRADE_NO ,SAUDA_DATE = CD_SAUDA_DATE,SCRIP_CD = CD_SCRIP_CD,SERIES = CD_SERIES,SCRIPNAME='BROKERAGE',
...
BROKERAGE = CD_TotalBrokerage,SETT_NO = CD_SETT_NO,SETT_TYPE = Cd_SETT_TYPE,TRADETYPE ='',TMARK ='',PRINTF = 2,ORDERFLAG = 4,SCRIPNAMEForOrderBy ='ZZZZZBROKERAGE',
