...
CREATE procedure
[dbo].[AfterContCursorIns_With_ContNo]
(
	@orderno varchar(16),
	@tradeno varchar (20),
	@partycd VARCHAR(10),
...
select distinct @net_rate = netrate, @orig_mkt_rate = marketrate from isettlement
where
convert(bigint, contractno) = convert(bigint, @ContractNo)
group by netrate, marketrate
...
Begin
        ...
        Begin
         Select   @@contractno = convert(bigint, contractno) from isettlement
...
--   /*END - ADDED BY SHYAM*/
     ...
     if (  (@@ContractNo Is Null ) And (@@Contstyle = 'O'))
         Begin
                          If @@style = 0
                          Begin
     Select @@ContractNo = Isnull(max(convert(bigint, contractno))+1,'0000001') from isettlement
...
--           /*END - ADDED BY SHYAM*/
                        ...
                        Select @@TempContractNo = Isnull(max(convert(bigint, contractno))+1,'0000001') from Settlement
                                   Where sauda_date Like  @TDate +'%'
                               and sett_type like @sett_type
            and sett_no = @@Sett_no
            /*ADDED BY SHYAM*/
            ...
            /*END - ADDED BY SHYAM*/
                                   ...
                                   If @@TempContractno > @@Contractno                                  Begin
                          ...
                          If @@Style = 1
                          Begin
                                  Select @@ContractNo = convert(bigint, contractno) + 1  From Contgen
...
If @@Error < > 0
Insert into errorlog Values (Getdate(), 'In AfterContcursorins   Error Occured in section Institutional Trade Transfer  ', @OrderNo + '     ' +@Tradeno + '   '+ @Partycd + '      ' +@Scripcd +'   ' +@Tdate + '   ' +Convert(Varchar,@Mkrate) + '    ' +
@Sellbuy  +  '   ' + @Sett_type +'   '+ @@Sett_no ,' ' + Convert(Varchar,@Flag) + '     ' + @Memcode + '   ' +@ToParty  + '    ' +  Convert(Varchar,@Tqty),'  ')
...
-- Select @@Sett_no,@Partycd,@Scripcd,@Tdate,@Sellbuy,@sett_type,@tradeno,@orderno,@tmark,@memcode,@terminalid
  ...
  and convert(bigint, contractno) = convert(bigint, @ContractNo)
  /*END - ADDED BY SHYAM*/
       order by Order_no,tradeqty desc END
Else If @Flag = 3
BEGIN
/* Print 'Flag = 3 '
...
Select @@trade_no ,@@tradeqty,@@marketrate,@@netrate, 'ANI'
...
--print '1-@@contractno:' + convert(varchar, @@contractno)
	...
	update Isettlement set Party_code = @ToParty ,Trade_no = @@TTradeNo,
--	Contractno = @@contractno
	Contractno = STUFF('0000000', 8 - Len(@@Contractno), Len(@@Contractno), @@Contractno)
 ...
 Begin
	...
	--@@contractno,
	Contractno = STUFF('0000000', 8 - Len(@@Contractno), Len(@@Contractno), @@Contractno),
	0, @@ttradeno,@toparty,Scrip_Cd,user_id,@tqty, AUCTIONPART, MarketType,
	...
	where sett_type = @sett_type and trade_no = @@trade_no and scrip_cd = @scripcd     and sauda_date Like @TDate + '%'  and sell_buy =@sellbuy  and party_code = @partycd
...
--print '7-@@contractno:' + convert(varchar, @@contractno)
	...
	update isettlement set tradeqty= 0,Normal = 0,Day_puc = 0,day_sales = 0,Sett_purch = 0,Sett_sales  = 0,                 marketrate = 0,Brokapplied = 0,NetRate = 0,Amount = 0,Ins_chrg = 0,turn_tax = 0,other_chrg = 0,
	sebi_tax = 0,Broker_chrg = 0,Service_tax = 0,Trade_amount = 0,NBrokApp = 0,NSerTax = 0,N_NetRate = 0
	where sett_type = @sett_type and trade_no = @@trade_no and scrip_cd = @scripcd
	and sauda_date Like @TDate + '%'  and sell_buy =@sellbuy  and party_code = @partycd
	and netrate= @@NetRate and  partipantcode = @Memcode And Sett_no = @@Sett_no
...
--print '9-@@contractno:' + convert(varchar, @@contractno)
 ...
 Begin
                ...
                Insert into isettlement select
  @@contractno,0,@@ttradeno,@toparty,Scrip_Cd,user_id,@@TradeQty, AUCTIONPART, MarketType,
    series,order_no,MarketRate, Sauda_date,Table_no, Line_No ,Val_perc, Normal,Day_puc,day_sales,     Sett_purch,Sett_sales,Sell_buy,settflag ,Brokapplied, NetRate,amount =  (@@Tradeqty *@@NetRate) ,
    Ins_chrg,turn_tax,other_chrg, sebi_tax,Broker_chrg,Service_tax,Trade_amount= (@tqty * @@MarketRate) ,
     Billflag,sett_no,nbrokapp ,nsertax ,n_netrate ,sett_type,partipantcode,status,pro_cli,cpid,instrument,
    bookType,branch_id, tmark ,scheme,dummy1,dummy2 from isettlement
  where sett_type = @sett_type and trade_no = @@trade_no and scrip_cd = @scripcd
...
If @@Error < > 0
Insert into errorlog Values (Getdate(), '  In AfterContcursorins   Error Occured in section Actual Transfer  ', @OrderNo + '     ' +@Tradeno + '   '+ @Partycd + '      ' +@Scripcd +'   ' +@Tdate + '   ' +Convert(Varchar,@Mkrate) + '    ' +  @Sellbuy  +   '   ' + @Sett_type +'   '+ @@Sett_no ,' ' + Convert(Varchar,@Flag) + '     ' + @Memcode + '   ' +@ToParty  + '    ' +  Convert(Varchar,@Tqty),'  ')

select @orig_qty = isnull(sum(tradeqty), 0) from isettlement where convert(bigint, contractno) = convert(bigint, @contractno)
