...
/****** Object:  Stored Procedure dbo.Stp_ContractDetailsISO515    Script Date: 02/09/2008 5:51:17 PM ******/          
          
/****** Object:  Stored Procedure dbo.Stp_ContractDetailsISO515    Script Date: 02/08/2008 3:17:14 PM ******/          
...
CREATE  PROCEDURE [dbo].[Stp_ContractDetailsISO515]          
(          
 @FromDate Varchar(11),            
 @ToDate Varchar(11),            
...
BEGIN          
 ...
 DECLARE @T_ContractDetails TABLE (          
  ...
  ClientFmCode VARCHAR(15) DEFAULT(''),          
  ClientMapinOrUCC VARCHAR(25) DEFAULT(''),          
  ...
  TradeType VARCHAR(10) DEFAULT(''), --(CH/DVP)          
  TradeQty INT DEFAULT(0),          
  Custodiancode VARCHAR(50) DEFAULT(''),          
  CustSebiNo VARCHAR(25) DEFAULT(''),          
  ScripISIN VARCHAR(25) DEFAULT(''),          
  ...
  SettlementType VARCHAR(10) DEFAULT(''),          
  Series VARCHAR(5) DEFAULT(''),          
  PartySebiRegNo VARCHAR(25) DEFAULT(''),-- (MAPIN No.)          
  Remark   VARCHAR(135) DEFAULT(''),---(Free Entry 35Chrs)          
...
-- --     SEBI No. for (For Hand Delivery Trades)          
 ...
 If Ltrim(Rtrim(@@FromContract)) = ''       
 Begin            
  Select @@FromContract = '0000000'      
 End            
      
 If Ltrim(Rtrim(@@ToContract)) = ''            
 Begin            
  Select @@ToContract = '9999999'            
 End           
...
-- --   OrderTime          
 ...
 SELECT          
  ...
  ClientMapinOrUCC = '',          
  Exchange = CASE @ExchangeCode WHEN 'BSE' THEN '01' WHEN 'NSE' THEN '23' END,    --- NSE=>01; BSE=>02          
  ...
  BrokerageAmount = CONVERT(VARCHAR,CONVERT(NUMERIC(18,4),round((Case When Service_chrg = 1             
                       Then Round(Sum(I.Brokapplied* I.Tradeqty)+Sum(I.Service_tax),Convert(integer,T.brokerage))            
          Else Round(Sum(I.Brokapplied* I.Tradeqty),Convert(integer,T.brokerage))            
                      End),4))),             
           ...
           Else (Sum(Round(I.MarketRate,Convert(integer,T.marketrate)) * I.Tradeqty)/(Case When Sum(I.Tradeqty)>0 Then Sum(I.Tradeqty) Else 1 End)) * Sum(I.Tradeqty)             
                ...
                - (Case When Service_chrg = 0             
                            ...
                            Else 0            
                                 End)             
          End ),4)))            
  --TradeRefNo = Trade_No,         --OrderNo = Order_No,          
...
-- --   REPLACE(CONVERT(CHAR, CONVERT(DATETIME, CPID, 103), 108), ':', ''),          
 ...
 UPDATE @T_ContractDetails SET           
   ClientMapinOrUCC = CASE WHEN ISNULL(UCC_Code, '') <> '' THEN ISNULL(UCC_Code, '') ELSE ISNULL(MAPIN_ID, '') END,          
   PartySebiRegNo = REPLACE(sebi_regn_no,'/', ''),          
   ClientFmCode = CASE WHEN ISNULL(FmCode, '') <> '' THEN FmCode ELSE ucc_code END          
...
SettlementDate =RTRIM(CONVERT(CHAR, B.Sec_Payout, 112))                
 ...
 UPDATE @T_ContractDetails SET           
  CustodianCode = B.Custodian_Code,           
  CustSebiNo = C.SebiRegNo          
  FROM @T_ContractDetails A, msajag.dbo.Client_Brok_Details B, Custodian C          
   WHERE A.PartyCode = B.Cl_Code AND B.Exchange = @ExchangeCode AND B.Segment = 'CAPITAL'          
  AND B.Custodian_Code = C.CustodianCode          
 ...
 IF @ExchangeCode = 'NSE'   --- 4 Decimals           
  SELECT           
   ContractNo,    
   Message1 =           
   '{IFN515}' + '{' + RTRim(@BrokerSebiRegNo) + '}{' + CASE WHEN @FileType = 'C' THEN RTRim(CustSebiNo) ELSE RTRim(ClientFmCode) END  + '}' + '[~]' + --{INB011247633}{INCUS003}    PartySebiRegNo          
   '{' + '[~]' +          
   ':16R:' + 'GENL' + '[~]' + -- GENL           
   ...
   ScripCode + '[~]' +         --- R-Money  (ScripCompanyName)          
...
-- --    ':95Q:' + ':EXCH//ORDER DETAILS' + '[~]',  --:EXCH//ORDER DETAILS          
 ...
 -- --   ':20C:' + ':PROC//' + Left(Replicate('0',15)+LTrim(RTrim(OrderNo)), 16) + '[~]' + --:PROC//0000000018864003          
   ...
   Message3 = '-}',          
   Message4 = CASE WHEN @FileType = 'C' THEN RTRim(CustSebiNo) ELSE RTRim(ClientFmCode) END           
  FROM  @T_ContractDetails           
 ELSE     --- BSE   2 DECIMALS            
  SELECT           
   ContractNo,          
...
-- --    ':95Q:' + ':EXCH//ORDER DETAILS' + '[~]',  --:EXCH//ORDER DETAILS          
 ...
 -- --   RTRim(OrderDate) + ' ' + RTrim(OrderTime) + '[~]' + --20070627 122855          
 -- --   ':20C:' + ':PROC//' + Left(Replicate('0',15)+LTrim(RTrim(OrderNo)), 16) + '[~]' + --:PROC//0000000018864003          
   Message2 =  ':16R:' + 'OTHRPRTY' + '[~]' + --OTHRPRTY          
      ':95Q:' + ':EXCH//ORDER DETAILS' + '[~]',  --:EXCH//ORDER DETAILS          
      ':16S:OTHRPRTY' + '[~]', --          
   Message3 = '-}',          
   Message4 = CASE WHEN @FileType = 'C' THEN RTRim(CustSebiNo) ELSE RTRim(ClientFmCode) END            
