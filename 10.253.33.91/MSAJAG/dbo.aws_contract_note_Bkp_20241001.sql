-- Object: PROCEDURE dbo.aws_contract_note_Bkp_20241001
...
Create  proc [dbo].[aws_contract_note_Bkp_20241001] (@dt datetime,@cnflag VARCHAR(2))
as
...
+'|'+CONVERT(VARCHAR(20),CAST(SUM(CGST) AS DECIMAL(18,2)))+'|'+CONVERT(VARCHAR(20),CAST(SUM(SGST) AS DECIMAL(18,2)))+'|'+CONVERT(VARCHAR,SUM(TURN_TAX))
+'|'+CONVERT(VARCHAR,SUM(SEBI_TAX))+'|'+CONVERT(VARCHAR,SUM(BROKER_CHRG))++'|'+CONVERT(VARCHAR,SUM(OTHER_CHRG))
+'|'+CONVERT(VARCHAR(20),CAST(SUM(NETAMOUNT-(INS_CHRG+TURN_TAX+SEBI_TAX+CGST+SGST+BROKER_CHRG+IGST+UGST+OTHER_CHRG)) AS DECIMAL(18,2)))+'|'+CONVERT(VARCHAR,SUM(BROKERAGE)) 
...
set @NSEQUERY = @NSEQUERY + ' from angelnsecm.msajag.dbo.nse_bulk_process  order by party_code ,sno,SRNO,EXCHANGE,SEGMENT,SCRIP_CD,OD_TIME " queryout j:\Contract_Note\ContractNote_'+replace(convert(varchar,@dt,103),'/','')+'.txt -c -t"," -Sanand1 -Uclassmkt -Pdd$$gnfDTVs244648ysjgZAcc'
