-- Object: PROCEDURE dbo.RPT_RMS_SALE_SUMMARY_HISTORY
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE PROC RPT_RMS_SALE_SUMMARY_HISTORY    
(              
 @STATUSID VARCHAR(20),            
...
FROM TBL_RMS_SALE_LED_DET_HIS T,              
     #CLIENT C              
...
INTO #TBL_RMS_SALE_LED_DET_HIS               
FROM TBL_RMS_SALE_LED_DET_HIS T, #CLIENT C            
WHERE PROCESSDATE = @PROCESS_DATE            
...
INSERT INTO #TBL_RMS_SALE_LED_DET_HIS               
SELECT C.PARTY_CODE,CASH_SEG_BAL=SUM(CASH_SEG_BAL),              
HOLD_AMT=SUM(HOLD_AMT),NOOFDAYS,EXCESS_COLL=SUM(EXCESS_COLL),              
VDTBAL=SUM(VDTBAL),             
ACT_SQ_OFF_BAL = CONVERT(NUMERIC(18,2),0),            
...
AND C.PARTY_CODE = T.PARTY_CODE            
AND NOOFDAYS = 6               
AND NOT EXISTS (SELECT PARTY_CODE FROM #TBL_RMS_SALE_LED_DET_HIS              
...
UPDATE #TBL_RMS_SALE_LED_DET_HIS SET ACT_SQ_OFF_BAL = AMT             
FROM (            
...
WHERE A.PARTY_CODE = #TBL_RMS_SALE_LED_DET_HIS.PARTY_CODE            
            
UPDATE #TBL_RMS_SALE_LED_DET_HIS SET ACT_SQ_OFF_BAL = ACT_SQ_OFF_BAL + AMT             
FROM (            
  SELECT PARTY_CODE, AMT = SUM(TRADEQTY*MARKETRATE)            
...
UPDATE #TBL_RMS_SALE_LED_DET_HIS SET CATEGORY = CLIENT_CATEGORY,            
CL_TYPE = CLIENT_TYPE, SB_CATEGORY = C.SB_CATEGORY            
FROM MKT_CLIENTDETAILS C            
WHERE #TBL_RMS_SALE_LED_DET_HIS.PARTY_CODE = C.PARTY_CODE            
