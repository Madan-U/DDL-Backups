-- Object: PROCEDURE dbo.InterBenTrans_MTF
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE PROCEDURE [dbo].[InterBenTrans_MTF] ( @Sett_no Varchar(7),@Sett_Type Varchar(2),@RefNo Int, @DPTYPE VARCHAR(4) = 'NSDL' ) As
Declare   
 @@Scrip_Cd Varchar(12),  
 @@series Varchar(3),  
...
FROM InsInterBenTrans_MTF where sett_no = @Sett_no  
and sett_type = @sett_type And Party_Code Not in ( Select Party_code From DelPartyFlag Where InterFlag = 1 And Party_code = InsInterBenTrans_MTF.Party_Code)   
...
AND CLTDPID IN (SELECT CLTDPID_MTF FROM #MTFACCOUNT)      
...
AND BCLTDPID IN (SELECT CLTDPID_MTF FROM #MTFACCOUNT)  
Group By DT.Sett_No,DT.Sett_Type,DT.PARTY_CODE,DT.SCRIP_CD,DT.Series,DT.BSECODE,DT.Psettno,DT.Psetttype,DT.Exchg,IsIn  
...
From BSEDB.DBO.deliveryclt_GROSS_FLAG D, DelInterBen DT where D.SETT_NO = DT.Sett_no  
And D.sett_type = DT.Sett_Type and D.party_code = DT.Party_Code  
and D.scrip_cd = DT.BseCode and InOut = 'I' AND FLAG = 'M'  
...
From MSAJAG.DBO.deliveryclt_GROSS_FLAG D, DelInterBen DT where D.SETT_NO = DT.Sett_no  
And D.sett_type = DT.Sett_Type and D.party_code = DT.Party_Code  
and D.scrip_cd = DT.Scrip_Cd And D.Series = DT.Series and InOut = 'I'  AND FLAG = 'M'
and BCQty = 0 And CQty = 0 And XQty = 0 And IQty = 0 And BDQty = 0 And DQty = 0   
Group By DT.Sett_No,DT.Sett_Type,DT.PARTY_CODE,DT.SCRIP_CD,DT.Series,DT.BSECODE,DT.Psettno,DT.Psetttype,DT.Exchg,IsIn  
...
Begin  
 ...
 And Dp.DpId = D.BDpId And Dp.DpCltNo = D.BCltDpId And Dp.Description Not Like '%POOL%' 
 And Description not Like '%PLEDGE%' AND ACCOUNTTYPE <> 'MAR'  
 AND SEGMENT = 'MTF' --Excluding MTF
 End  
 Else  
 Begin  
 Set @@Cfwd = Cursor for   
...
AND D.DPTYPE = @DPTYPE AND DESCRIPTION LIKE '%POOL%' AND DELIVERED = '0'    
AND ISETT_NO=@SETT_NO AND ISETT_TYPE=@SETT_TYPE AND FILLER2 = 1 AND REASON LIKE 'INTER BEN%'    
AND CLTDPID NOT IN (SELECT DPCLTNO FROM DELIVERYDP WHERE DESCRIPTION LIKE '%POOL%')
