...
begin
	 ...
	 begin
		  ...
		  Amount = ( select isnull(round(sum(round(vamt,2)),2),0) from account.dbo.ledger where drcr = 'D' 
				and cltcode = l.cltcode  and  vdt <= @vdt + ' 23:59:59')- 
	 ...
	 begin
		  ...
		  and   vdt <= @vdt + ' 23:59:59')- 
		  (select isnull(round(sum(round(vamt,2)),2),0) from account.dbo.ledger where drcr = 'C' and cltcode = l.cltcode and  vdt <= @vdt + ' 23:59:59') 
...
begin
 	...
 	begin
  		...
  		Amount = ( select isnull(sum(round(vamt,2)),0) from account.dbo.ledger where drcr = 'D' and cltcode = l.cltcode 
  		and vdt <= @vdt + ' 23:59:59') - 
 		 (select isnull(sum(round(vamt,2)),0) from account.dbo.ledger where drcr = 'C' and cltcode = l.cltcode and  vdt <= @vdt + ' 23:59:59') 
  		From account.dbo.Ledger l where vdt <= @vdt + ' 23:59:59'  
  		group by l.Cltcode , l.acname 
	 ...
	 begin
		  ...
		  Amount = ( select isnull(sum(vamt),0) from account.dbo.ledger where drcr = 'D' and cltcode = l.cltcode 
		  and  vdt <= @vdt + ' 23:59:59') - 
		  (select isnull(sum(vamt),0) from account.dbo.ledger where drcr = 'C' and cltcode = l.cltcode and vdt <= @vdt + ' 23:59:59') 
		  From account.dbo.Ledger l, branches b,client1 c1, client2 c2
		  where  vdt <= @vdt + ' 23:59:59' 
		  and b.branch_cd=@statusname
		  and b.short_name=c1.trader
