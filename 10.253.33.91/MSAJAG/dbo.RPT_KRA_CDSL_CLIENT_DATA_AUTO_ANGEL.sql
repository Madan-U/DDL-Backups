-- Object: PROCEDURE dbo.RPT_KRA_CDSL_CLIENT_DATA_AUTO_ANGEL
...
CREATE PROC [dbo].[RPT_KRA_CDSL_CLIENT_DATA_AUTO_ANGEL] (
	@OPT_TYPE VARCHAR(2),
	@CL_TYPE VARCHAR(1)
	)
AS

---EXEC RPT_KRA_CDSL_CLIENT_DATA_AUTO_ANGEL 'A','N'
---EXEC RPT_KRA_CDSL_CLIENT_DATA_AUTO 'JAN  1 2000','DEC 31 2015','','','','','zzzzzzzzzzzz','broker','broker','', 'SWAPNIL','O','0',''
...
SELECT	@FROMDATE	= LEFT(GETDATE(),11) ,  @TODATE	= LEFT(GETDATE(),11)
--SELECT	@FROMDATE	= 'APR 16 2015',  @TODATE	= 'APR 17 2015'
SET		@FROMPARTY	= ''
SET		@TOPARTY	= 'zzzzzzzzzzzz'
SET		@FROMBRANCH = ''
SET		@TOBRANCH	= 'zzzzzzzzzzzz'
SET		@STATUSID	= 'BROKER'
...
BEGIN
	...
	SELECT PARTY_CODE = (
			CASE
				WHEN ISNULL(CHARINDEX(',', 1), 0) > 0
					THEN LEFT(FILE_DATA, ISNULL(CHARINDEX(',', FILE_DATA, 1), 0) - 1)
				ELSE FILE_DATA
				END
			)
	INTO #SEL_PARTY
...
BEGIN
	...
	BEGIN
		...
		CREATE TABLE #TCLIENT (PARTY_CODE VARCHAR(15))

		INSERT INTO #TCLIENT
		SELECT DISTINCT PARTY_CODE
		FROM CMBILLVALAN C(NOLOCK)
		...
		SELECT DISTINCT PARTY_CODE
		FROM BSEDB_AB.DBO.CMBILLVALAN C(NOLOCK)
		WHERE SAUDA_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59:59'
			AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY
			AND BRANCH_CD BETWEEN @FROMBRANCH AND @TOBRANCH
		...
		SELECT DISTINCT PARTY_CODE
		FROM NSEFO.DBO.FOBILLVALAN C(NOLOCK)
		WHERE SAUDA_DATE BETWEEN @FROMDATE AND @TODATE + ' 23:59:59'
			AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY
...
SELECT COMPANY_CODE = (
	...
	APP_TYPE = (
		CASE
			WHEN @CL_TYPE = 'N'
				THEN ISNULL(CU.CATEGORY, '')
			ELSE CASE
					WHEN ISNULL(C.CL_STATUS, 'N') = 'IND'
						THEN 'I'
					ELSE 'N'
					END
			END
	...
	APP_EXMT_CAT = (
		CASE
			WHEN ISNULL(CU.PAN_EXEMPT, 'N') = 'Y'
				THEN ISNULL(CU.PAN_EXEMPT_CATEGORY, '')
			ELSE ''
			END
		),
	...
	APP_IPV_DATE = (
		CASE
			WHEN CU.CATEGORY = 'I'
				THEN ISNULL(CONVERT(VARCHAR(11), IN_PERSON_DATE, 103), '')
			ELSE ''
			END
		),
	/* APP_GEN   = (
  ...
  END), */
	...
	APP_DOB_INCORP = (
		CASE
			WHEN CU.CATEGORY = 'I'
				THEN CONVERT(VARCHAR(11), DOB, 103)
...
/*	APP_COMMENCE_DT = (
		...
		*/
 APP_COMMENCE_DT = (CASE WHEN C.CL_STATUS <>'HUF' AND CONVERT(VARCHAR(11),CU.DATEOFCOMMENCMENT,103)<>'' THEN   (                           
  CASE                            
  WHEN CU.CATEGORY = 'I' THEN ''                           
  ELSE ISNULL(CONVERT(VARCHAR(11),CU.DATEOFCOMMENCMENT,103),'')                           
  END) ELSE CONVERT(VARCHAR(11),DOB,103)  END ) ,     
  ...
  END), */
	...
	APP_OTH_NATIONALITY = (
		CASE
			WHEN CU.CATEGORY = 'I'
				THEN ISNULL(CU.NATIONALITY_OTHER, '')
			ELSE ''
			END
		),
	APP_COMP_STATUS = (
	...
	APP_COR_ADD_PROOF = (
		CASE
			WHEN @CL_TYPE = 'N'
				THEN ISNULL(CU.PROOF_OF_ADDRESS, '')
			ELSE '32'
			END
		),
  ...
  END),   */
	...
	APP_OTH_OCC = (
		CASE
			WHEN CU.CATEGORY = 'I'
				THEN ISNULL(CU.OCCUPATION_OTHER, '')
			ELSE ''
			END
		),
...
INTO #KRADATA
FROM CLIENT_DETAILS C(NOLOCK)
LEFT OUTER JOIN CLIENTSTATUS CS(NOLOCK) ON (C.CL_STATUS = CS.CL_STATUS),
	CLIENT_MASTER_UCC_DATA CU(NOLOCK),
	(
		SELECT DISTINCT F.PARTY_CODE,
			SYSTEMDATE
		FROM #CLIENT C,
			#FINAL_CLIENT F
...
WHERE C.PARTY_CODE = CB.PARTY_CODE
...
UPDATE #KRADATA
SET APP_COR_CTRY = ISNULL(CODE, '')
FROM CLIENT_STATIC_CODES
WHERE APP_COR_CTRY = DESCRIPTION
	AND KRA_TYPE = 'CDSL'
	AND CATEGORY = 'COUNTRY'

UPDATE #KRADATA
SET APP_PER_CTRY = ISNULL(CODE, '')
FROM CLIENT_STATIC_CODES
WHERE APP_PER_CTRY = DESCRIPTION
	AND KRA_TYPE = 'CDSL'
	AND CATEGORY = 'COUNTRY'

UPDATE #KRADATA
SET APP_COR_STATE = ISNULL(CODE, '')
FROM CLIENT_STATIC_CODES
WHERE ( CASE WHEN APP_COR_STATE ='TamilNadu' THEN 'Tamil Nadu'
			 WHEN APP_COR_STATE ='Uttaranchal' THEN 'Uttarakhand'
			 WHEN APP_COR_STATE ='Pondicherry' THEN 'Puducherry'
			 WHEN APP_COR_STATE ='Jammu & Kashmir' THEN 'Jammu and Kashmir'  
			 ELSE APP_COR_STATE END) = DESCRIPTION
	AND KRA_TYPE = 'CDSL'
	AND CATEGORY = 'STATE'

UPDATE #KRADATA
SET APP_PER_STATE = ISNULL(CODE, '')
FROM CLIENT_STATIC_CODES
WHERE ( CASE WHEN APP_PER_STATE ='TamilNadu' THEN 'Tamil Nadu'
			 WHEN APP_PER_STATE ='Uttaranchal' THEN 'Uttarakhand'
			 WHEN APP_PER_STATE ='Pondicherry' THEN 'Puducherry'
			 WHEN APP_PER_STATE ='Jammu & Kashmir' THEN 'Jammu and Kashmir'  
			 ELSE APP_PER_STATE END) = DESCRIPTION
	AND KRA_TYPE = 'CDSL'
	AND CATEGORY = 'STATE'
...
SET APP_RES_STATUS = (
		CASE
			WHEN APP_RES_STATUS = '01'
				THEN 'R'
			WHEN APP_RES_STATUS = '02'
				THEN 'N'
			WHEN APP_RES_STATUS = '03'
				THEN 'P'
			ELSE APP_RES_STATUS
			END
		)
...
SELECT COMPANY_CODE = '',
	...
	APP_ADDLDATA_DNLDDT = ''
/* --- RESPONSE FILE ADDITIONAL TAG --- */
FROM CLIENT_CONTACT_DETAILS C(NOLOCK),
	#KRADATA K(NOLOCK)
WHERE C.CL_CODE = K.APP_INTERNAL_REF
	AND APP_TYPE = 'N'
...
CREATE	TABLE #KRADATA_TMP (
		HEADER			NVARCHAR(MAX),
		KYCDATA			NVARCHAR(MAX),	
		ADDITIONALDATA	NVARCHAR(MAX),
		APP_FILE_FLAG	VARCHAR(10),
		APP_INTERNAL_REF VARCHAR(20)
		)

IF @CL_TYPE = 'N'
BEGIN
	INSERT INTO #KRADATA_TMP
	SELECT 
		--'<ROOT>',
		HEADER = '<HEADER><COMPANY_CODE>' + COMPANY_CODE + '</COMPANY_CODE><BATCH_DATE>' + BATCH_DATE + '</BATCH_DATE></HEADER>',
		KYCDATA= '<KYCDATA>' +
		...
		*/
		'</ADDITIONALDATA>',
		APP_FILE_FLAG,
		APP_INTERNAL_REF = ISNULL(APP_INTERNAL_REF,'')
	FROM #KRADATA
	ORDER BY APP_FILE_FLAG
END
ELSE
BEGIN
	INSERT INTO #KRADATA_TMP
...
@FILE_UPLOAD_MAP_DRIVE	VARCHAR(100),
@FILE_UPLOAD_LOCATION	VARCHAR(200),
@FILE_UPLOAD_USERID	VARCHAR(100),
@FILE_UPLOAD_PWD	VARCHAR(100),
@FILE_UPLOAD_NAME	VARCHAR(200)
...
SELECT	@ROWCHECK = COUNT(1) FROM #KRADATA_TMP
SELECT	@CL_COUNT = COUNT(1) FROM #KRADATA_TMP WHERE APP_FILE_FLAG = '1'

IF (@ROWCHECK > 0)
BEGIN
	SET @FILEPATH = 'C:\KRA\'
	SELECT @FILENAME = CONVERT(VARCHAR,GETDATE(),112) + '01_' + CONVERT(VARCHAR,@CL_COUNT) + '.xml'
	SET @FILE_UPLOAD_MAP_DRIVE = 'A:'
	SET @FILE_UPLOAD_LOCATION = '\\196.1.115.133\KRATeam'
	SET @FILE_UPLOAD_USERID = 'KRA Team'
	SET @FILE_UPLOAD_PWD = 'angel#456'

	/*SET @FILE_UPLOAD_LOCATION = '\\172.29.30.110\D$\KRA'
	SET @FILE_UPLOAD_USERID = 'Angelcso\sivakumar.kopparapu'
	SET @FILE_UPLOAD_PWD = 'pass@123'*/

	SET @FILE_UPLOAD_NAME = @FILENAME

	TRUNCATE TABLE TBL_KRA_DATA_FILE		

	INSERT INTO TBL_KRA_DATA_FILE VALUES ( '<ROOT>','',0 )
	INSERT INTO TBL_KRA_DATA_FILE
	SELECT TOP 1 HEADER,APP_INTERNAL_REF,1 FROM #KRADATA_TMP

	INSERT INTO TBL_KRA_DATA_FILE
	SELECT KYCDATA,APP_INTERNAL_REF,2 FROM #KRADATA_TMP
	WHERE APP_FILE_FLAG = '1'

	INSERT INTO TBL_KRA_DATA_FILE
	SELECT ADDITIONALDATA,APP_INTERNAL_REF,3 FROM #KRADATA_TMP
	WHERE APP_FILE_FLAG = '2'

	DECLARE @@TOT_KRARECORD	INT
	DECLARE @@TOT_ADDRECORD INT

	SELECT @@TOT_KRARECORD = COUNT(1) FROM #KRADATA_TMP WHERE APP_FILE_FLAG = '1'
	SELECT @@TOT_ADDRECORD = COUNT(1) FROM #KRADATA_TMP WHERE APP_FILE_FLAG = '2'

	INSERT INTO TBL_KRA_DATA_FILE VALUES ( '<FOOTER>','zzzzzzzzzzzz',4 )
	INSERT INTO TBL_KRA_DATA_FILE VALUES ( '<NO_OF_KYC_RECORDS>'+ CONVERT(VARCHAR,@@TOT_KRARECORD) +'</NO_OF_KYC_RECORDS><NO_OF_ADDLDATA_RECORDS>'+ CONVERT(VARCHAR,@@TOT_ADDRECORD) +'</NO_OF_ADDLDATA_RECORDS>','zzzzzzzzzzzz',4 )
	INSERT INTO TBL_KRA_DATA_FILE VALUES ( '</FOOTER>','zzzzzzzzzzzz',4 )
	INSERT INTO TBL_KRA_DATA_FILE VALUES ( '</ROOT>','zzzzzzzzzzzz',5 )

	--SELECT * FROM TBL_KRA_DATA_FILE ORDER BY PARTY_CODE, ORD_FLAG

	DROP TABLE #KRADATA_TMP

	SET @SQLSTR = 'SELECT KRA_TEXT FROM ['+ @@SERVERNAME +'].MSAJAG.DBO.TBL_KRA_DATA_FILE ORDER BY PARTY_CODE, ORD_FLAG '
	SET @SQLCMD = 'bcp "' + @SQLSTR + '" queryout ' + @FILEPATH + @FILENAME + ' -T -S -c -t'
	EXEC XP_CMDSHELL @SQLCMD
	
	SET @CMD = 'EXEC XP_CMDSHELL ''net use ' + @FILE_UPLOAD_MAP_DRIVE + ' ' + @FILE_UPLOAD_LOCATION + ' /USER:' + @FILE_UPLOAD_USERID + ' ' + @FILE_UPLOAD_PWD + ' /persistent:Y'' ' --- , NO_OUTPUT'
	EXEC (@CMD)

	SET @CMD = 'EXEC XP_CMDSHELL ''COPY ' + @FILEPATH + @FILENAME + ' ' + @FILE_UPLOAD_MAP_DRIVE + '\' + @FILE_UPLOAD_NAME + ''' ' -- , NO_OUTPUT'
	EXEC (@CMD)

	SET @CMD = 'EXEC XP_CMDSHELL ''net use ' + @FILE_UPLOAD_MAP_DRIVE + ' /delete'', NO_OUTPUT'
	EXEC (@CMD)
