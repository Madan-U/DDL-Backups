-- Object: PROCEDURE dbo.rpt_FTpartydetails_original
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE Proc rpt_FTpartydetails     
...
,cl2.Std_rate,cl2.ALBMDelchrg,cl2.ALBMDelivery,cl2.AlbmCF_tableno,cl2.MF_tableno,cl2.SB_tableno,     
...
Into #ClientDetails    
from client2 cl2 Left Outer Join UCC_Client UC On (cl2.Party_Code = UC.Party_Code), client1 cl1 Left Outer Join Client5 C5 On ( C5.Cl_Code = cl1.Cl_Code ),
...
IF (SELECT COUNT(1) FROM MSAJAG.DBO.SYSOBJECTS WHERE NAME = 'CLIENT_BROK_DETAILS') > 0
BEGIN
	UPDATE 	
		#ClientDetails SET
		PAYMENT_MODE = PAYCODENAME
	FROM	
		CLIENT_BROK_DETAILS C, PAYMENTMODE P
	WHERE
...
IF (SELECT COUNT(1) FROM MSAJAG.DBO.SYSOBJECTS WHERE NAME = 'CLIENT_DETAILS') > 0
BEGIN
	UPDATE 	
		#ClientDetails SET
