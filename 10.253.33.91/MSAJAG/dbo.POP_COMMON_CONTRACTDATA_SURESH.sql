-- Object: PROCEDURE dbo.POP_COMMON_CONTRACTDATA_SURESH
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

           
 --ROLLBACK          
 --- POP_COMMON_CONTRACTDATA_SURESH 'AUG 27 2014'                              
CREATE  PROCEDURE [dbo].[POP_COMMON_CONTRACTDATA_SURESH] (                                
 @FROMDATE VARCHAR(11),                                
 @USERNAME VARCHAR(20) = ''                                
 )                                
AS                                
...
/*                                
EXEC POP_COMMON_CONTRACTDATA 'JUL 31 2014','SURESH'                                
...
ATA_COMMON'')) > 0 '                                
 ...
 ELSE                                
  ...
  BEGIN               
   ...
   SET @STRSQL = @STRSQL + ' SEGMENT  = ''' + @SEGMENT_NEW + ''','                                
   SET @STRSQL = @STRSQL + ' ORDER_NO = '''','                                
   SET @STRSQL = @STRSQL + ' ORDER_TIME = '''','                         SET @STRSQL = @STRSQL + ' TRADE_NO = '''','                                
...
WHERE F.PARTY_CODE = #CONTRACT.PARTY_CODE                                
 ...
 --select * into contract_suresh from #CONTRACT          
 --return    
...
FROM COMMON_CONTRACT_DATA_SURESH C(NOLOCK)                                
...
DELETE                                
FROM COMMON_CONTRACT_DATA_SURESH                                
WHERE SAUDA_DATE BETWEEN @FROMDATE AND @FROMDATE + ' 23:59:59'                                
 AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY                                
                                
INSERT INTO COMMON_CONTRACT_DATA_SURESH                                
SELECT *                                
FROM #CONTRACT                                
ORDER BY PARTY_CODE,                                
 EXCHANGE,                                
