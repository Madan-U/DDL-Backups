-- Object: PROCEDURE dbo.POP_COMMON_CONTRACTDATA_BAK260615
...
CREATE PROCEDURE [dbo].[POP_COMMON_CONTRACTDATA_BAK260615] (                                      
 @FROMDATE VARCHAR(11),                                      
 @USERNAME VARCHAR(20) = ''                                      
 )                                      
AS                                      
...
END                                      
   ...
   SET @STRSQL = 'IF (SELECT COUNT(1) FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.TABLES WHERE NAME=''TBL_CONTRACT_TERMINAL_MAPPING'') > 0 '                                      
   SET @STRSQL = @STRSQL + 'BEGIN '                                      
   SET @STRSQL = @STRSQL + 'IF (SELECT COUNT(1) FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.COLUMNS WHERE [NAME] = ''USER_ID'' AND [OBJECT_ID] IN (SELECT DISTINCT OBJECT_ID FROM ' + @SHARESERVER + '.' + @SHAREDB + '.SYS.TABLES WHERE NAME = ''CONTRACT_
DATA'')) > 0 '                                      
   SET @STRSQL = @STRSQL + 'BEGIN '                                      
   SET @STRSQL = @STRSQL + ' UPDATE #CONTRACT SET REMARK_ID = ISNULL(T.REMARK_ID,''''), REMARK_DESC = ISNULL(T.REMARK_DESC,'''') '                   
   SET @STRSQL = @STRSQL + ' FROM ' + @SHARESERVER + '.' + @SHAREDB + '.DBO.TBL_CONTRACT_TERMINAL_MAPPING T WITH (NOLOCK) '                                      
