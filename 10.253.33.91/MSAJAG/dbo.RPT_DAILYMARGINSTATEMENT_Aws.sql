-- Object: PROCEDURE dbo.RPT_DAILYMARGINSTATEMENT_Aws
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


      
--EXEC RPT_DAILYMARGINSTATEMENT_Aws 'OCT 17 2024', 'A00927','A00927','','ZZZZZZZZZ',  '','ZZZZZZZZZ','BROKER','BROKER',1,0,'C','ECN'          
        
CREATE PROC [dbo].[RPT_DAILYMARGINSTATEMENT_Aws]                                                  
...
------CHANGE START BECAUSE OF SLOWNESS SRE-30773

CREATE TABLE [dbo].[#Tbl_Mar_Report](
	[PARTY_CODE] [varchar](10) NULL,
	[CL_TYPE] [varchar](3) NOT NULL,
	[TRADE_DAY] [datetime] NULL,
	[EXCHANGE] [varchar](3) NOT NULL,
	[SEGMENT] [varchar](7) NOT NULL,
	[CASH] [money] NULL,
	[NONCASH] [numeric](38, 4) NULL,
	[FD_BG] [numeric](38, 4) NULL,
	[ANY_OTHER_COLL] [numeric](18, 4) NULL,
	[TOTALCOLL] [numeric](38, 4) NULL,
	[INIT_MARGIN] [numeric](38, 4) NULL,
	[EXP_MARGIN] [numeric](38, 4) NULL,
	[TOTALMARGIN] [numeric](38, 4) NULL,
	[EXG_MAR_SHORT] [numeric](38, 4) NULL,
	[ADD_MARGIN] [numeric](38, 4) NULL,
	[MARGIN_STATUS] [numeric](38, 4) NULL,
	[MAR_PLEDGE_AMT] [numeric](18, 6) NULL,
	[DELIVERY_MARGIN] [numeric](18, 6) NULL
	)

IF @TOPARTY='zzzzzzzzz' 
BEGIN

insert into #Tbl_Mar_Report
select * FROM  ANGELCOMMODITY.MCDX.DBO.Tbl_Mar_Report WITH (NOLOCK)      
 --Select * into #Tbl_Mar_Report   FROM  ANGELCOMMODITY.MCDX.DBO.Tbl_Mar_Report WITH (NOLOCK)                                                 
...
CREATE INDEX #C on #Tbl_Mar_Report (TRADE_DAY,PARTY_CODE)
...
FROM  #Tbl_Mar_Report WITH (NOLOCK)                                                 
...
WHERE                                                  
 ...
 /*Below Code Commented ORE-769 on 2022 June 6*/        
 --AND CL_TYPE <>'NRI'             
 /*End By Ahsan Farooqui */        
...
CREATE NONCLUSTERED INDEX [MARGIN_DETAIL_temp]
ON [dbo].[#MARGIN_DETAIL] ([EXCHANGE],[SEGMENT],[PARTY_CODE])
...
WHERE                                                  
 ...
 /* ADDED FOR addition ledger details  Start*/        
 ...
 BGFD_AMT  = SUM(0),                                                  
 OTHER_MARGIN = SUM(0),                                                  
 TOTAL_MARGIN_AVL= SUM(0),               
 INITIALMARGIN = ISNULL(SUM(0),0),                                                  
 EXPOSURE_MARGIN = ISNULL(SUM(0),0),                                                  
 TOTAL_MARGIN = ISNULL(SUM(0),0),                                      
 EXCESS_SHORTFALL= ISNULL(SUM(0),0),                                         
 ADD_MARGIN  = 0,                                       
 MARGIN_STATUS = SUM(0),        
 MAR_PLEDGE_AMT = 0,        
 DELIVERY_MARGIN = 0          
...
WHERE                                                  
 MARGINDATE BETWEEN @MDATE AND @MDATE + ' 23:59:59'                                                  
 AND T.PARTY_CODE = C.PARTY_CODE   AND NOT ExISTS (        
 SELECT PARTY_CODE FROM   #MARGIN_DETAIL   M        
 WHERE M.PARTY_CODE=T.PARTY_CODE and M.EXCHANGE=T.ExCHANGE and M.SEGMENT=T.SEGMENT)                                       
 GROUP BY                                               
 T.PARTY_CODE,                                                  
 LEFT(MARGINDATE,11),                                                  
 EXCHANGE,                                                  
...
UPDATE  M        
SET LED_MARGIN_AMT= TDAY_LEDGER        
FROM TBL_COMBINE_REPORTING_PEAK_DETAIL T ,#MARGIN_DETAIL M        
WHERE T.PARTY_CODE =M.PARTY_CODE        
AND T.EXCHANGE =M.EXCHANGE AND T.SEGMENT =M.SEGMENT        
AND T.SEGMENT  IN ('CAPITAL','FUTURES')        
...
--ADDED FOR CASH HOLIDAY DATE        
 /*
UPDATE  M        
SET INITIALMARGIN= TDAY_MARGIN ,EXPOSURE_MARGIN =  TDAY_MTM,TOTAL_MARGIN= TDAY_MARGIN+TDAY_MTM        
FROM TBL_COMBINE_REPORTING_DETAIL T ,#MARGIN_DETAIL M        
WHERE T.PARTY_CODE =M.PARTY_CODE        
AND T.EXCHANGE =M.EXCHANGE AND T.SEGMENT =M.SEGMENT        
AND T.SEGMENT  IN ('CAPITAL','FUTURES')        
...
UPDATE #MARGIN_DETAIL SET NONCASH_AMT =0 Where Exchange Not in ('MTF','EPN')        
UPDATE #MARGIN_DETAIL SET MAR_PLEDGE_AMT =0 Where Exchange Not in ('MTF','EPN') 
...
Select Party_code into #ppeak from TBL_COMBINE_REPORTING_PEAK        
...
select party_code,TDAY_MARGIN_SHORT,TDAY_MTM_SHORT,sum(TDAY_MARGIN) as total_margin,0 as peak_margin ,MARGINDATE        
from msajag.dbo.TBL_COMBINE_REPORTING where MARGINDATE =@MDATE    ---and party_code ='HEMC004'        
...
from msajag.dbo.TBL_COMBINE_REPORTING_peak where MARGINDATE =@MDATE  --and party_code ='HEMC004'        
group by party_code,TDAY_MARGIN_SHORT,TDAY_MTM_SHORT,MARGINDATE        
)A        
...
SELECT T.PARTY_CODE,TDAY_MARGIN=MAX(TDAY_MARGIN)  INTO #PEAK_HIGH FROM TBL_COMBINE_REPORTING_PEAK_DETAIL T,#FINAL F        
WHERE  T.MARGINDATE=@MDATE         
AND F.PARTY_CODE=T.PARTY_CODE        
 GROUP BY t.PARTY_CODE        
        
Select T.* iNto #peak from TBL_COMBINE_REPORTING_PEAK_DETAIL T(NOlock),#final F(NOlock) ,#peak_high P(NOlock)        
where  T.MARGINDATE=@MDATE  And T.PARTY_CODE=P.PARTY_CODE and P.TDAY_MARGIN=T.TDAY_MARGIN        
and F.PARTY_CODE=T.PARTY_CODE        
        
        
         
UPDATE #FINAL_REPORT SET INITIALMARGIN=0,TOTAL_MARGIN=0,--DELIVERY_MARGIN=0,        
EXCESS_SHORTFALL=0 --, Margin_Status=TOTAL_MARGIN_AVL        
FROM #final F        
WHERE         
...
UPDATE #FINAL_REPORT SET INITIALMARGIN=T.TDAY_MARGIN,TOTAL_MARGIN=T.TDAY_MARGIN+EXPOSURE_MARGIN,ADD_MARGIN=0,--DELIVERY_MARGIN=0,        
EXCESS_SHORTFALL=T_MARGINAVL --,MARGIN_STATUS=T_MARGINAVL        
FROM #peak T        
...
UPDATE #FINAL_REPORT SET  Margin_Status=TOTAL_MARGIN_AVL-INITIALMARGIN-ADD_MARGIN-EXPOSURE_MARGIN-DELIVERY_MARGIN,        
EXCESS_SHORTFALL=TOTAL_MARGIN_AVL-INITIALMARGIN-ADD_MARGIN-EXPOSURE_MARGIN-DELIVERY_MARGIN       
FROM #final F        
WHERE         
 #FINAL_REPORT.PARTY_CODE=F.PARTY_CODE        
And #FINAL_REPORT.RPT_ORD='1DET'        
...
UPDATE #FINAL_REPORT SET  TOTAL_MARGIN =EXPOSURE_MARGIN       
WHERE         
TOTAL_MARGIN =0 AND EXPOSURE_MARGIN <>0       
AND #FINAL_REPORT.RPT_ORD='1DET'        
