...
begin
   ...
   begin
      select @@selectqury = "select l.cltcode , acname=isnull(l.acname,''), branchcode, Amount = sum(case when drcr = 'd' then vamt else -vamt end)"
...
begin
   ...
   begin
      if @curryrstdate = @stdate 
         begin
            select @@selectqury = "select l.cltcode , acname=isnull(l.acname,''), branchcode, drtot = sum(case when drcr = 'd' then vamt else 0 end), crtot = sum(case when drcr = 'c' then vamt else 0 end),opbal = 0 "
            select @@fromtables = " from account.dbo.ledger l left outer join account.dbo.acmast a on l.cltcode=  a.cltcode " 
            select @@wherepart  = @@wherepart1 + @@addwhere
         end
      else
         begin
           select @@selectqury = "select l.cltcode , acname=isnull(l.acname,''), branchcode, drtot = sum(case when vtyp <> 18 and vdt >= '" + @stdate + "' then ( case when drcr = 'd' then vamt else 0 end) else 0 end), crtot = sum(case when vtyp <> 18 and
...
vdt >= '" + @stdate + "' then ( case when drcr = 'c' then vamt else 0 end) else 0 end), opbal = sum(case when vdt < '" + @stdate + "' then ( case when drcr = 'd' then vamt else -vamt end) else 0 end) "
   ...
   begin
      select @@selectqury = "select l.cltcode , acname=isnull(l.acname,''), branchcode, drtot = sum(case when vtyp <> 18 and vdt >= '" + @stdate + "' then ( case when drcr = 'd' then vamt else 0 end) else 0 end), crtot = sum(case when vtyp <> 18 and vdt >
...
begin
   if @openentryflag = 0   /* Opening entry ( vtyp = 18 ) not found in Ledger */
   begin
      select @@selectqury = " select l2.cltcode , acname=isnull(l.acname,''), branchcode, Amount = sum(case when ucase(l2.drcr) = 'D' then camt else -camt end)"
   ...
   begin
      select @@selectqury = " select l.cltcode , acname=isnull(l.acname,''), branchcode, Amount = sum(case when l2.drcr = 'd' then camt else -camt end)"
      select @@fromtables = " from account.dbo.ledger2 l2, account.dbo.ledger l left outer join account.dbo.acmast a on l.cltcode=  a.cltcode  " 
      select @@wherepart  = @@wherepart2 + @@addwhere
   end
   else if @openentryflag = 2  /* Opening entry ( vtyp = 18 ) found in Ledger for earlier year */
...
begin
   ...
   begin
      if @curryrstdate = @stdate 
         begin
            select @@selectqury = "select l.cltcode , acname=isnull(l.acname,''), branchcode, drtot = sum(case when l2.drcr = 'd' then camt else 0 end), crtot = sum(case when l2.drcr = 'c' then camt else 0 end),opbal = 0 "
            select @@fromtables = " from account.dbo.ledger2 l2, account.dbo.ledger l left outer join account.dbo.acmast a on l.cltcode=  a.cltcode  " 
            select @@wherepart  = @@wherepart1 + @@addwhere
         end
      else
         begin
           select @@selectqury = "select l.cltcode , acname=isnull(l.acname,''), branchcode, drtot = sum(case when l.vtyp <> 18 and l.vdt >= '" + @stdate + "' then ( case when l2.drcr = 'd' then camt else 0 end) else 0 end), crtot = sum(case when l.vtyp <
...
> 18 and l.vdt >= '" + @stdate + "' then ( case when l2.drcr = 'c' then camt else 0 end) else 0 end), opbal = sum(case when l.vdt < '" + @stdate + "' then ( case when l2.drcr = 'd' then camt else -camt end) else 0 end) "
   ...
   begin
      select @@selectqury = "select l.cltcode , acname=isnull(l.acname,''), branchcode, drtot = sum(case when l.vtyp <> 18 and l.vdt >= '" + @stdate + "' then ( case when l2.drcr = 'd' then camt else 0 end) else 0 end), crtot = sum(case when l.vtyp <> 18 
