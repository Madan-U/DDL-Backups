...
DBCC DBREINDEX(VBB_CHARGES_DETAIL_TEMP_1)
...
INNER JOIN VBB_CHARGES_DETAIL_TEMP_1  B WITH(Nolock)  
ON     B.CD_SETT_NO = S.SETT_NO                      
   AND B.CD_SETT_TYPE = S.SETT_TYPE                      
   AND B.CD_PARTY_CODE = S.PARTY_CODE                       
   AND B.CD_SCRIP_CD = S.SCRIP_CD                      
   AND B.CD_SERIES = S.SERIES   
WHERE 
      S.SETT_NO = @F_SETT_NO AND                 
      S.SETT_TYPE = @F_SETT_TYPE AND                    
      S.PARTY_CODE BETWEEN  @FROM_PARTY AND @TO_PARTY AND                    
      S.TRADEQTY > 0     
