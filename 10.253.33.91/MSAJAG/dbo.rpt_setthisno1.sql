-- Object: PROCEDURE dbo.rpt_setthisno1
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



/****** Object:  Stored Procedure dbo.rpt_setthisno1    Script Date: 04/27/2001 4:32:49 PM ******/
...
*/
CREATE PROCEDURE rpt_setthisno1
@statusid varchar(15),
...
if @statusid = 'broker'
begin
select distinct s.sett_no, end_date from settlement s, sett_mst st
where s.sett_type like ltrim(@settype)+'%'
and s.sett_type=st.sett_type and s.sett_no=st.sett_no
union
select distinct s.sett_no, end_date from history s, sett_mst st
where s.sett_type like ltrim(@settype)+'%'
and s.sett_type=st.sett_type and s.sett_no=st.sett_no
order by end_date 
end
...
begin
select distinct h.sett_no, end_date from settlement h, client1 c1, client2 c2, subbrokers sb, sett_mst st
...
union
select distinct h.sett_no , end_date from history h, client1 c1, client2 c2, subbrokers sb, sett_mst st
where h.party_code=c2.party_code and c2.cl_code=c1.cl_code and c1.sub_broker=sb.sub_broker and 
sb.sub_broker = @statusname
and h.sett_type like ltrim(@settype)+'%'
and h.sett_type=st.sett_type and h.sett_no=st.sett_no
...
and h.sett_type=st.sett_type and h.sett_no=st.sett_no
union
select distinct h.sett_no , end_date from history h,  client1 c1, client2 c2, branches b, sett_mst st
where b.short_name = c1.trader and h.party_code=c2.party_code and c1.cl_code = c2.cl_code and
b.branch_cd = @statusname
and h.sett_type like ltrim(@settype)+'%'
and h.sett_type=st.sett_type and h.sett_no=st.sett_no
...
if @statusid = 'client'
begin
select distinct h.sett_no , end_date from settlement h, client1 c1, client2 c2, sett_mst st
where h.party_code=c2.party_code and c2.cl_code=c1.cl_code and 
h.party_code = @statusname
and h.sett_type like ltrim(@settype)+'%'
and h.sett_type=st.sett_type and h.sett_no=st.sett_no
union
select distinct h.sett_no , end_date from history h, client1 c1, client2 c2, sett_mst st
