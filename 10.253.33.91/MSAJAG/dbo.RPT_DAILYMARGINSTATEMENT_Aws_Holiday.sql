-- Object: PROCEDURE dbo.RPT_DAILYMARGINSTATEMENT_Aws_Holiday
...
CREATE PROC [dbo].[RPT_DAILYMARGINSTATEMENT_Aws_Holiday]                                          
(                                          
@MDATE VARCHAR(11),                                          
...
SELECT                                          
 ...
 MARGIN_STATUS = SUM(TDAY_NONCASH),
 MAR_PLEDGE_AMT = 0,
 DELIVERY_MARGIN = 0     Into #Missing_Codes                                    

FROM                                          

 TBL_COMBINE_REPORTING_DETAIL T (NOLOCK) ,#CLIENTMASTER C
WHERE                                          

 T.MARGINDATE BETWEEN @MDATE AND @MDATE + ' 23:59:59'  
 and t.PARTY_CODE=C.PARTY_CODE  
ANd Not exists ( Select Party_Code from #MARGIN_DETAIL  M
where T.PARTY_CODE=m.PARTY_CODE and t.EXCHANGE =m.EXCHANGE  and T.SEGMENT =M.SEGMENT )                                        
...
SET LED_MARGIN_AMT= TDAY_CASHCOLL,TOTAL_MARGIN_AVL =TDAY_CASHCOLL,EXCESS_SHORTFALL=TDAY_CASHCOLL
...
UPDATE #MARGIN_DETAIL SET NONCASH_AMT =0 Where  Exchange <>'MTF'
UPDATE #MARGIN_DETAIL SET MAR_PLEDGE_AMT =0 Where  Exchange <>'MTF'
...
Select * Into #TEOD from TBL_COMBINE_REPORTING_DETAIL T 
where   T.MARGINDATE =@MDATE

Select * Into #TPeak from TBL_COMBINE_REPORTING_PEAK_DETAIL T 
where   T.MARGINDATE =@MDATE
AND PARTY_CODE Not in (SELECT PARTY_CODE From #TEOD)

UPdate F SET LED_MARGIN_AMT =TDAY_LEDGER ,EXPOSURE_MARGIN=TDAY_MTM, TOTAL_MARGIN_AVL=TDAY_LEDGER	+ TDAY_FDBG	+TDAY_NONCASH from #TEOD T,#FINAL_REPORT F
where T.PARTY_CODE=F.PARTY_CODE and T.MARGINDATE =@MDATE AND T.EXCHANGE=F.EXCHANGE and T.SEGMENT =F.SEGMENT

UPdate F SET LED_MARGIN_AMT =TDAY_LEDGER  ,TOTAL_MARGIN_AVL=TDAY_LEDGER	+ TDAY_FDBG	+TDAY_NONCASH from #TPeak T,#FINAL_REPORT F
where T.PARTY_CODE=F.PARTY_CODE and T.MARGINDATE =@MDATE AND T.EXCHANGE=F.EXCHANGE and T.SEGMENT =F.SEGMENT
...
from  TBL_COMBINE_REPORTING where MARGINDATE =@MDATE    ---and party_code ='HEMC004'
...
from  TBL_COMBINE_REPORTING_peak where MARGINDATE =@MDATE  --and party_code ='HEMC004'
group by party_code,TDAY_MARGIN_SHORT,TDAY_MTM_SHORT,MARGINDATE
)A
...
Select T.* iNto #peak from TBL_COMBINE_REPORTING_PEAK_DETAIL T,#final F
where  T.MARGINDATE=@MDATE 
and F.PARTY_CODE=T.PARTY_CODE

Select party_Code ,Max(TDAY_MARGIN) As INITIALMARGIN INto #PM from #peak M
Group By party_Code


Select * Into #FINAL_P from #peak P Where Exists (
Select party_Code , INITIALMARGIN from #PM M
where P.PARTY_CODE=M.PARTY_CODE  and TDAY_MARGIN=INITIALMARGIN
) 

 
UPDATE #FINAL_REPORT SET  LED_MARGIN_AMT =TDAY_LEDGER  ,TOTAL_MARGIN_AVL=TDAY_LEDGER	+ TDAY_FDBG	+TDAY_NONCASH
FROM #peak T
WHERE 
 #FINAL_REPORT.PARTY_CODE=T.PARTY_CODE
...
---select *   from #FINAL_REPORT where   party_code ='A166727'
 
UPDATE #FINAL_REPORT SET  Margin_Status=TOTAL_MARGIN_AVL-INITIALMARGIN-ADD_MARGIN-EXPOSURE_MARGIN,
EXCESS_SHORTFALL=TOTAL_MARGIN_AVL-INITIALMARGIN-ADD_MARGIN-EXPOSURE_MARGIN
FROM #final F
WHERE 
 #FINAL_REPORT.PARTY_CODE=F.PARTY_CODE
And #FINAL_REPORT.RPT_ORD='1DET'
