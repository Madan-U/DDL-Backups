-- Object: PROCEDURE dbo.POP_COMMON_CONTRACTDATA_AUCTION
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------



CREATE PROCEDURE [dbo].[POP_COMMON_CONTRACTDATA_AUCTION]   
(  
@FROMDATE VARCHAR(11),  
@USERNAME VARCHAR(20) = '',  
...
---EXEC POP_COMMON_CONTRACTDATA_BKP_NEW_03SEP2024 'OCT  4 2024','E73799','CURR'
...
SELECT DISTINCT PARTY_CODE AS CL_CODE INTO #AUCPARTY
FROM SETTLEMENT S WHERE SETT_TYPE IN ('A','X') AND SAUDA_DATE >=@SDDATE AND SAUDA_DATE <=@SDDATE +' 23:59:59'

CREATE INDEX #C ON #AUCPARTY(PARTY_CODE)
 ...
 --PRINT 'D1'                                                                                                             
  ...
  SET @STRSQL = @STRSQL + ' ' + @SHARESERVER + @SHAREDB + '.DBO.CONTRACT_MASTER WITH (NOLOCK) WHERE CONVERT(DATETIME,START_DATE,103)  AND EXISTS (SELECT PARTY_CODE FROM #AUCPARTY from CL_CODE=PARTY_CODE) BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'' 
  ...
  SET @STRSQL = @STRSQL + ' AND M.PARTY_CODE = D.PARTY_CODE  AND EXISTS (SELECT PARTY_CODE FROM #AUCPARTY from CL_CODE=PARTY_CODE) '                                      
  ...
  SET @STRSQL = @STRSQL + ' AND S.SETT_TYPE = #CONTRACT.SETT_TYPE  AND EXISTS (SELECT PARTY_CODE FROM #AUCPARTY from CL_CODE=PARTY_CODE) '                 
...
END  
 ...
 ELSE   
  ...
  --PRINT 'ABC2'                                                                          
   ...
   BEGIN                                      
    SET @STRSQL = @STRSQL + ' SELECT DISTINCT PARTY_CODE FROM '                                      
    SET @STRSQL = @STRSQL + ' ' + @SHARESERVER + @SHAREDB + '.DBO.BFOBILLVALAN WITH (NOLOCK) WHERE  EXISTS (SELECT PARTY_CODE FROM #AUCPARTY from CL_CODE=PARTY_CODE)  AND PARTY_CODE BETWEEN  ''' + @FROMPARTY + ''' AND ''' + @TOPARTY + ''' '                                      
    SET @STRSQL = @STRSQL + ' AND SAUDA_DATE BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'''                                      
   END                          
   ELSE                                      
   BEGIN                                      
    SET @STRSQL = @STRSQL + ' SELECT DISTINCT PARTY_CODE FROM '                                      
    SET @STRSQL = @STRSQL + ' ' + @SHARESERVER + @SHAREDB + '.DBO.FOBILLVALAN WITH (NOLOCK) WHERE EXISTS (SELECT PARTY_CODE FROM #AUCPARTY from CL_CODE=PARTY_CODE)  AND PARTY_CODE BETWEEN  ''' + @FROMPARTY + ''' AND ''' + @TOPARTY + ''' '                                      
    SET @STRSQL = @STRSQL + ' AND SAUDA_DATE BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'''                                      
   END                                      
...
SET @STRSQL = @STRSQL + ' AND M.CL_CODE = C2.PARTY_CODE AND EXISTS (SELECT PARTY_CODE FROM #AUCPARTY from CL_CODE=PARTY_CODE) '    
...
--  SET @STRSQL = @STRSQL + ' AND CONVERT(DATETIME,SAUDA_DATE,103) BETWEEN CONVERT(DATETIME,CONVERT(VARCHAR,FROM_DATE,103),103) AND END_DATE '    
   ...
   */                                      
   ...
   SET @STRSQL = @STRSQL + ' WHERE    EXISTS (SELECT PARTY_CODE FROM #AUCPARTY from CL_CODE=PARTY_CODE) AND SAUDA_DATE BETWEEN ''' + @FROMDATE + ''' AND ''' + @FROMDATE + ' 23:59:59'''                                      
...
CREATE CLUSTERED INDEX IDXPARTY ON #OLDCONTRACTNO (PARTY_CODE)                                      
 ...
 WHERE SAUDA_DATE BETWEEN @FROMDATE AND @FROMDATE + ' 23:59:59'  AND EXISTS (SELECT CLTCODE FROM #AUCPARTY WHERE PARTY_CODE =CL_CODE)
...
IF (CONVERT(DATETIME,@FROMDATE,109) > 'OCT  1 2024')
	...
	WHERE	SAUDA_DATE BETWEEN @FROMDATE AND @FROMDATE + ' 23:59:59' AND TRDTYPE='N' AND IPFT_CHRG <>0
	GROUP BY
	...
	WHERE	SAUDA_DATE BETWEEN @FROMDATE AND @FROMDATE + ' 23:59:59' AND TRDTYPE='N' AND IPFT_CHRG <>0
	 and EXISTS (SELECT CLTCODE FROM #AUCPARTY WHERE PARTY_CODE =CL_CODE)
	GROUP BY
...
WHERE SAUDA_DATE BETWEEN @FROMDATE AND @FROMDATE + ' 23:59:59'                                      
AND PARTY_CODE BETWEEN @FROMPARTY AND @TOPARTY         AND    EXISTS (SELECT CLTCODE FROM #AUCPARTY WHERE PARTY_CODE =CL_CODE)                              
