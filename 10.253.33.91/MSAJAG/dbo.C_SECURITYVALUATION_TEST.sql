-- Object: PROCEDURE dbo.C_SECURITYVALUATION_TEST
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

-- EXEC C_SECURITYVALUATION_TEST 'NSE', 'FUTURES', 'NSEFO', 'AUG  6 2013'   
CREATE PROC [dbo].[C_SECURITYVALUATION_TEST](    
          @EXCHANGE VARCHAR(3),    
          @SEGMENT  VARCHAR(7),    
          @SHAREDB  VARCHAR(20),    
          @VALDATE  VARCHAR(11))    
...
=============================================================================*/    
  ...
  into #delttr  
  from ANGELDEMAT.BSEDB.DBO.bak_deltrans_inst D WITH(NOLOCK),  
 ...
 insert   into #delttr  
  select party_code, scrip_Cd, series, trtype, drcr, delivered, filler2, sharetype,   
  bdptype, bdpid, bcltdpid, certno, qty = sum(Qty)  
  from ANGELDEMAT.msajag.DBO.bak_deltrans_inst D WITH(NOLOCK),  
 ...
 insert   into #delttr  
  ...
  FROM   #SECMST    
  WHERE  FLAG = 1    
  and Scrip_Cd not in (Select scrip_cd from C_VALUATION 
						where C_VALUATION.Scrip_Cd = #SECMST.Scrip_Cd 
						and C_VALUATION.Series = #SECMST.Series 
						and C_VALUATION.Exchange = @EXCHANGE
						and C_Valuation.Segment = @SEGMENT ) 
