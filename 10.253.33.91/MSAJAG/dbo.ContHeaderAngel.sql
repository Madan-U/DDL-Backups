-- Object: PROCEDURE dbo.ContHeaderAngel
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

CREATE proc ContHeaderAngel       
(      
@Sdate varchar(12),       
@Sett_Type varchar(2),       
@ContNo varchar(7),       
...
select distinct settlement.party_code,       
Partyname = client1.Long_name,      
settlement.billno,       
settlement.contractno,       
sett_mst.sett_no,       
sett_mst.sett_type,           
...
client1.l_city,       
client1.l_zip,       
convert(varchar,settlement.sauda_date,103) as sauda_date ,      
client1.email,       
client1.pan_gir_no,          
client1.Sub_Broker,       
client1.branch_cd          
into #sett from settlement With(Index(Party),NoLock), client1 (nolock), client2 (nolock), sett_mst (nolock)  
...
--and settlement.contractno = Replicate('0',7-len(@CONTNO)) + @CONTNO        
...
Begin    
 ...
 client1.Sub_Broker,       
 client1.branch_cd          
 from settlement With(Index(Party),NoLock), client1 (nolock), client2 (nolock), sett_mst (nolock)        
 where client1.cl_code = client2.cl_code           
 --And client2.Party_code = @Party_code          
 And Settlement.Party_code = @Party_code          
 and settlement.party_code = client2.party_code          
