-- Object: PROCEDURE dbo.V2_COMBINED_CONTRACTNOTE_DETAIL_INTEROP
...
CREATE PROCEDURE [dbo].[V2_COMBINED_CONTRACTNOTE_DETAIL_INTEROP]                       
...
--'Angel Broking Pvt Ltd.(Erstwhile Angel Broking Ltd.)','ALL'                 
 ...
 EXEC [V2_COMBINED_CONTRACTNOTE_DETAIL_TEST] 'BROKER','BROKER','JUL 10 2012','JUL 10 2012','','ZZZZZZ','','ZZZZZZZZ','','ZZZZZZ','','99999999','','',0,''                      
...
SELECT * 
INTO #COMMON_DATA 
FROM vw_common_contract_Data C (NOLOCK)   
...
AND EXISTS (SELECT EXCHANGE FROM #EXCHANGELIST E WHERE E.EXCHANGE = LTRIM(RTRIM(C.EXCHANGE))+LTRIM(RTRIM(C.SEGMENT)) )  
 ...
 BEGIN                      
 INSERT INTO #PARTY                      
 SELECT DISTINCT PARTY_CODE FROM #COMMON_DATA WITH (NOLOCK)                       
 ...
 SELECT                      
  ...
  TRADE_TIME,                      
-- SCRIPNAME = CASE WHEN SEGMENT ='CAPITAL' THEN REPLACE(SCRIPNAME,'BROKERAGE','DELIVERY HANDLNG CHRG') ELSE SCRIPNAME END + '-' + EXCHANGE,
  SCRIPNAME = SCRIPNAME + (CASE WHEN @INTEROP <> '' THEN '-' + EXCHANGE ELSE '' END),        
  QTY,                      
 TMARK = (CASE WHEN @INTEROP <> '' THEN '' ELSE CASE WHEN SEGMENT ='FUTURES' THEN '' ELSE TMARK END END),  
  ...
  NETRATE=(CASE WHEN LEFT(SCRIPNAME,3)='OPT' AND EXCHANGE='NSE' AND SEGMENT='FUTURES' THEN (CASE WHEN SELL_BUY =1 THEN NETRATE+BROK ELSE NETRATE-BROK END)    
            ELSE NETRATE END),                      
...
WHERE	T.SETT_TYPE = #CONTRACT.SETT_TYPE 
		AND #CONTRACT.SETTTYPE_DESC = ''
...
SET		CONTRACTNO = ISNULL(C.CONTRACTNO,''),
		...
		SETT_TYPE = (
		CASE
			WHEN C.SETT_TYPE = (CASE WHEN @INTEROP = 'NSCCL' THEN 'D' ELSE 'N' END) THEN (CASE WHEN @INTEROP = 'NSCCL' THEN 'N' ELSE 'D' END)
			WHEN C.SETT_TYPE = (CASE WHEN @INTEROP = 'NSCCL' THEN 'C' ELSE 'W' END) THEN (CASE WHEN @INTEROP = 'NSCCL' THEN 'W' ELSE 'C' END)
			ELSE C.SETT_TYPE
		END)/*,
		EXCHANGE = @INTEROP*/
FROM	#COMMON_DATA C
WHERE	LEFT(CONVERT(DATETIME,#CONTRACT.SAUDA_DATE,103),11) = LEFT(C.SAUDA_DATE,11)
		...
		AND #CONTRACT.EXCHANGE = (CASE WHEN @INTEROP = 'NSCCL' THEN 'BSE' ELSE 'NSE' END)
		AND #CONTRACT.SEGMENT = 'CAPITAL'
		AND C.EXCHANGE = (CASE WHEN @INTEROP = 'NSCCL' THEN 'NSE' ELSE 'BSE' END)
		AND C.SEGMENT = 'CAPITAL'
		AND C.SETT_TYPE IN ('N','W','D','C')
		AND #CONTRACT.SETT_TYPE IN ('N','W','D','C')
		AND @INTEROP <> ''
...
WHERE	LEFT(CONVERT(DATETIME,#CONTRACT.SAUDA_DATE,103),11) = LEFT(C.SAUDA_DATE,11)
		...
		AND #CONTRACT.SEGMENT = C.SEGMENT
		AND C.EXCHANGE = (CASE WHEN @INTEROP = 'NSCCL' THEN 'NSX' ELSE C.EXCHANGE END)
		AND C.SEGMENT = 'FUTURES'
...
SET		CONTRACT_HEADER_DET = (
		CASE
			WHEN @INTEROP = 'NSCCL' THEN  + 'Clearing Corporation Cash Market (BSE & NSE)::  NCL (NSE Clearing Ltd.)'
			WHEN @INTEROP = 'ICCL' THEN  + 'Clearing Corporation Cash Market (BSE & NSE)::  ICCL (Indian Clearing Corporation Ltd.)'
			ELSE ''
		END) + (
		CASE 
		...
		END)
WHERE	SETT_TYPE IN ('N','W','D','C','') AND SEGMENT = 'CAPITAL'
		AND @INTEROP <> ''

UPDATE	#CONTRACT
--SET		CONTRACT_HEADER_DET = @INTEROP + ' - ' + SEGMENT
SET		CONTRACT_HEADER_DET = (
		CASE 
			WHEN @INTEROP = 'NSCCL' THEN 'Clearing Coporation for Equity Derivative :: NCL - (NSE Clearing Ltd.)'
			WHEN @INTEROP = 'ICCL' THEN 'Clearing Coporation for Equity Derivative :: ICCL (Indian Clearing Corporation Ltd.)'
...
SET		CONTRACT_HEADER_DET = (
		CASE
			WHEN @INTEROP = 'NSCCL' THEN 'Clearing Coporation for Currency Derivatives Market :: NCL - (NSE Clearing Ltd.)'
			WHEN @INTEROP = 'ICCL' THEN 'Clearing Coporation for Currency Derivatives Market :: ICCL (Indian Clearing Corporation Ltd.)'
			ELSE CONTRACT_HEADER_DET
		END)			
...
UPDATE	#CONTRACT SET EXCHANGE = (CASE WHEN @INTEROP = 'NSCCL' THEN 'NCL' ELSE @INTEROP END) WHERE SEGMENT = 'CAPITAL'
AND @INTEROP <> ''

--UPDATE	#CONTRACT SET EXCHANGE = (CASE WHEN @INTEROP = 'NSCCL' THEN 'NCL' ELSE @INTEROP END) WHERE SEGMENT = 'FUTURES'
...
WHERE	T.BSE_SETT_TYPE = #CONTRACT.SETT_TYPE
		...
		AND #CONTRACT.SETT_TYPE IN ('M','Z','RD','RC')
...
UPDATE	#CONTRACT SET SETTTYPE_DESC = LTRIM(RTRIM(SETTTYPE_DESC)) + ' ' + (
		CASE
...
SET  TAXABLE_VALUE = BROKERAGE_TAXEX + (  
  ...
  END) + (  
  CASE  
   WHEN ISNULL(G.IPFTCHRG,0) <> 0 THEN IPFT_CHRG  
   ELSE 0  
  END)+ (  
  CASE  
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SET @STRSQL = @STRSQL + '		AND REPLACE(REPLACE(REPLACE(#CONTRACT.SCRIPNAME,''(BF)'',''''),''(BT)'',''''),''-NSX'','''') = (CASE '
				SET @STRSQL = @STRSQL + '			WHEN LEFT(T.INST_TYPE,3) = ''FUT'' THEN RTRIM(T.INST_TYPE) + '' '' + RTRIM(T.SYMBOL) + '' '' + UPPER(CONVERT(VARCHAR(11),T.EXPIRY_DATE,109)) '
				SET @STRSQL = @STRSQL + '			ELSE RTRIM(T.INST_TYPE) + '' '' + RTRIM(T.SYMBOL) + '' '' + UPPER(CONVERT(VARCHAR(11),T.EXPIRY_DATE,109)) '
				SET @STRSQL = @STRSQL + '			END) '				
				SET @STRSQL = @STRSQL + '		AND #CONTRACT.EXCHANGE_OLD IN (''NSX'') '
				SET @STRSQL = @STRSQL + '		AND #CONTRACT.EXCHANGE_OLD+#CONTRACT.SEGMENT_OLD = ''' + @EXCHANGE_NEW +''' '
				SET @STRSQL = @STRSQL + ' END '
				PRINT @STRSQL
				EXEC (@STRSQL)
			END
			...
			BEGIN
				...
				SET @STRSQL = @STRSQL + 'WHERE	LEFT(T.TRADE_DATE,11) = LEFT(CONVERT(DATETIME,#CONTRACT.SAUDA_DATE,103),11) '
				SET @STRSQL = @STRSQL + '		AND REPLACE(REPLACE(REPLACE(#CONTRACT.SCRIPNAME,''(BF)'',''''),''(BT)'',''''),''-BSX'','''') = (CASE '
				SET @STRSQL = @STRSQL + '			WHEN LEFT(T.INST_TYPE,3) = ''FUT'' THEN RTRIM(T.INST_TYPE) + '' '' + RTRIM(T.SYMBOL) + '' '' + UPPER(CONVERT(VARCHAR(11),T.EXPIRYDATE,109)) '
				SET @STRSQL = @STRSQL + '			ELSE RTRIM(T.INST_TYPE) + '' '' + RTRIM(T.SYMBOL) + '' '' + UPPER(CONVERT(VARCHAR(11),T.EXPIRYDATE,109)) + '' '' + CONVERT(VARCHAR,T.STRIKE_PRICE) + '' '' + RTRIM(T.OPTION_TYPE) '
				SET @STRSQL = @STRSQL + '			END) '				
...
--### END CROSS CURRENCY CHANGES ###---
 ...
 SET  SCRIPNAME_NEW = 'BROKERAGE' ,SCRIPNAME='BROKERAGE',ORDER_TIME='',TRADE_TIME='',SELL_BUY=''  WHERE  SCRIPNAME LIKE '%BROKERAGE%'       
...
--CREATE INDEX Idx_Contract  ON #CONTRACT   
--(BRANCH_CD, PARTY_CODE, EXCHANGE,SEGMENT 
-- )  
 ...
 SELECT * FROM #CONTRACT -- WHERE QTY > 0  
 --where CONTRACTNO_NEW <> ''                      
