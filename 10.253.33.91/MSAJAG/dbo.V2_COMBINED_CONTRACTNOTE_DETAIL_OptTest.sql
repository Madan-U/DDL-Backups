-- Object: PROCEDURE dbo.V2_COMBINED_CONTRACTNOTE_DETAIL_OptTest
...
CREATE PROCEDURE [dbo].[V2_COMBINED_CONTRACTNOTE_DETAIL_OptTest]                       
...
--SELECT @PRINTF = 'ALL'                                                
      ...
      IF OBJECT_ID('dbo.Contractnote_Daily', 'U') IS NOT NULL 
      drop table Contractnote_Daily 
...
WHERE   
 ...
 CREATE INDEX #EXCHANGE ON #EXCHANGELIST (EXCHANGE) 
...
AND EXISTS (SELECT EXCHANGE FROM #EXCHANGELIST E WHERE E.EXCHANGE = LTRIM(RTRIM(C.EXCHANGE))+LTRIM(RTRIM(C.SEGMENT)) )  
 ...
 Create index #PARTY1 on #PARTY ( PARTYCODE)
 ...
 */                      
 ...
 ORDER BY PARTY_CODE              
  ...
  Create index  #ix_sett on #SETTLEMENT_DET  (party_code,sauda_date)                    
 ...
 ORDER BY PARTY_CODE                      
       ...
       Create index  #ix_party on #CONTRACTNO_DET  (party_code,sauda_date)                         
 ...
 ORDER BY PARTY_CODE                      
          ...
          Create index  #ix_party on #BROKERSEBIREGNO_DET  (party_code,sauda_date)                               
 ...
 ORDER BY PARTY_CODE                      
               ...
               Create index  #ix_party on #MEMBERCODE_DET  (party_code,sauda_date)                            
 ...
 ORDER BY PARTY_CODE                  
          ...
          Create index  #ix_party on #CIN_DET  (party_code,sauda_date)                               
 ...
 ORDER BY PARTY_CODE                      
                ...
                Create index  #ix_party on #REMARK_DET  (party_code,sauda_date)                                  
 ...
 (                      
  ...
  SELECT DISTINCT PARTY_CODE,SAUDA_DATE FROM Contractnote_Daily                      
 ...
 ON (A1.SAUDA_DATE = F.SAUDA_DATE AND A1.PARTY_CODE = F.PARTY_CODE)                      
           ...
           create INDEX #IX_PARTY ON    #CONTSETT_DET (PARTY_CODE,SAUDA_DATE)
 ...
 WHERE Contractnote_Daily.PARTY_CODE = C.PARTY_CODE                      
          
 AND  Contractnote_Daily.SAUDA_DATE = C.SAUDA_DATE                      
 ...
 GROUP BY                       
   ...
   SEGMENT                      
        ...
        CREATE INDEX #XC_CON ON    #CONTRACT1             (PARTY_CODE,SAUDA_DATE,EXCHANGE,SEGMENT)    
 ...
 FROM Contractnote_Daily C(NOLOCK),     
...
ALTER TABLE Contractnote_Daily  
...
TGST_NO = L.GST_NO  
INTO #GSTDATA  
FROM TBL_CLIENT_GST_DATA G, TBL_GST_LOCATION L, Contractnote_Daily P  
...
DEAL_CLIENT_ADD = L.ADDRESS1 +' '+ L.ADDRESS2,-- +' '+ L.CITY +' '+ L.ZIP,  
TGST_NO = L.GST_NO  
FROM CLIENT_DETAILS G, TBL_GST_LOCATION L, Contractnote_Daily P  
WHERE GST_LOCATION = LOC_CODE  
---AND CONVERT(DATETIME,SAUDA_DATE,103) BETWEEN G.EFF_FROM_DATE AND G.EFF_TO_DATE   
AND G.PARTY_CODE = P.PARTY_CODE     
...
DEAL_CLIENT_ADD = '',  
P.TGST_NO  
FROM Contractnote_Daily P, OWNER   
WHERE NOT EXISTS (SELECT PARTY_CODE FROM #GSTDATA G  
WHERE P.PARTY_CODE = G.PARTY_CODE  
AND P.SAUDA_DATE = G.SAUDA_DATE)  
  
    CREATE INDEX #IX_STATE ON #GSTDATA (TARGETSTATE,SOURCESTATE)
    CREATE INDEX #IX_PARTY  ON #GSTDATA (PARTY_CODE,SAUDA_DATE)
...
UPDATE Contractnote_Daily SET   
 ...
 TGST_NO = G.TGST_NO  
FROM #GSTDATA G  
WHERE Contractnote_Daily.PARTY_CODE = G.PARTY_CODE  
 AND Contractnote_Daily.SAUDA_DATE = G.SAUDA_DATE  
 AND G.GST_PER > 0   
...
WHERE G.PARTY_CODE = Contractnote_Daily.PARTY_CODE  
  AND CONVERT(DATETIME,SAUDA_DATE,103) BETWEEN G.EFF_FROM_DATE AND G.EFF_TO_DATE   
  
  
UPDATE Contractnote_Daily SET SGST_TAXEX = SERVICE_TAXEX - CGST_TAXEX - IGST_TAXEX - UGST_TAXEX,  
SGST = SERVICE_TAX - CGST - IGST - UGST  
WHERE  SGST_TAXEX > 0   
  
UPDATE Contractnote_Daily SET UGST_TAXEX = SERVICE_TAXEX - CGST_TAXEX - IGST_TAXEX - SGST_TAXEX,  
...
WHERE C.PARTY_CODE = Contractnote_Daily.PARTY_CODE  
  AND C.SAUDA_DATE = Contractnote_Daily.SAUDA_DATE  
  AND C.EXCHANGE = Contractnote_Daily.EXCHANGE  
  AND C.SEGMENT = Contractnote_Daily.SEGMENT  
  AND C.CONTRACTNO_NEW = Contractnote_Daily.CONTRACTNO_NEW   
...
SET  TAXABLE_VALUE = BROKERAGE_TAXEX + (  
  ...
  END) + (  
  CASE  
   WHEN ISNULL(G.IPFTCHRG,0) <> 0 THEN IPFT_CHRGEX  
   ELSE 0  
  END)+ (  
  CASE  
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SET @STRSQL = @STRSQL + 'UPDATE	Contractnote_Daily '
				SET @STRSQL = @STRSQL + 'SET	FOREIGN_PRICE = (CASE WHEN LEFT(BFCF_FLAG, 2) = ''BT'' THEN Contractnote_Daily.MARKETRATE/ISNULL(T.RBI_REFERENCE_RATE,1) ELSE ISNULL(T.CROSS_CURRENCY_PRICE,0) END) '
				SET @STRSQL = @STRSQL + 'FROM	' + @SHARESERVER + @SHAREDB + '.DBO.TBL_RBI_RATE T '
				SET @STRSQL = @STRSQL + 'WHERE	LEFT(T.TRADE_DATE,11) = LEFT(CONVERT(DATETIME,Contractnote_Daily.SAUDA_DATE,103),11) '
				SET @STRSQL = @STRSQL + '		AND REPLACE(REPLACE(Contractnote_Daily.SCRIPNAME,''(BF)'',''''),''(BT)'','''') = (CASE '
				...
				SET @STRSQL = @STRSQL + '		AND Contractnote_Daily.EXCHANGE IN (''NSX'') '
				SET @STRSQL = @STRSQL + '		AND Contractnote_Daily.EXCHANGE+Contractnote_Daily.SEGMENT = ''' + @EXCHANGE_NEW +''' '
				SET @STRSQL = @STRSQL + ' END '
				PRINT @STRSQL
				EXEC (@STRSQL)
			END
			...
			BEGIN
				...
				SET @STRSQL = @STRSQL + '			END) '				
				SET @STRSQL = @STRSQL + '		AND Contractnote_Daily.EXCHANGE IN (''BSX'') '
				SET @STRSQL = @STRSQL + '		AND Contractnote_Daily.EXCHANGE+Contractnote_Daily.SEGMENT = ''' + @EXCHANGE_NEW +''' '
				PRINT @STRSQL
				EXEC (@STRSQL)
			END
...
--CREATE INDEX Idx_Contract  ON Contractnote_Daily   
...
-- )  
 ...
 SELECT * FROM Contractnote_Daily -- WHERE QTY > 0  
