-- Object: PROCEDURE dbo.aws_contract_Cur_note
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------

--aws_contract_cur_note '2020-06-04' --,'2020-04-15'


CREATE  proc [dbo].[aws_contract_Cur_note] (@dt datetime)
as
...
+'|'+CONVERT(VARCHAR(20),CAST(SUM(CGST) AS DECIMAL(18,2)))+'|'+CONVERT(VARCHAR(20),CAST(SUM(SGST) AS DECIMAL(18,2)))+'|'+CONVERT(VARCHAR,SUM(TURN_TAX))
+'|'+CONVERT(VARCHAR,SUM(SEBI_TAX))+'|'+CONVERT(VARCHAR,SUM(BROKER_CHRG))++'|'+CONVERT(VARCHAR,SUM(OTHER_CHRG))
+'|'+CONVERT(VARCHAR(20),CAST(SUM(NETAMOUNT-(INS_CHRG+TURN_TAX+SEBI_TAX+CGST+SGST+BROKER_CHRG+IGST+UGST+IPFT_CHRG)) AS DECIMAL(18,2)))+'|'+CONVERT(VARCHAR,SUM(BROKERAGE)) 
...
from cur_bulk_process n,#final f where sno=1 and cltcode=party_code 
...
set @NSEQUERY = @NSEQUERY + ' from AngelNseCM.msajag.dbo.cur_bulk_process  order by party_code ,sno,SRNO,EXCHANGE,SEGMENT,SCRIP_CD,OD_TIME " queryout j:\Contract_Note\ContractNote_Curr_'+replace(convert(varchar,@dt,103),'/','')+'.txt -c -t"," -SANAND1 -Uaolinhouse -Pe$$gnfDTVs2455GZAcc'
