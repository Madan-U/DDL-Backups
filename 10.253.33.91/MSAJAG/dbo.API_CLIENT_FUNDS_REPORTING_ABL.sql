-- Object: PROCEDURE dbo.API_CLIENT_FUNDS_REPORTING_ABL
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


CREATE PROCEDURE [dbo].[API_CLIENT_FUNDS_REPORTING_ABL]
	@FR_DATE VARCHAR(11),
	@TO_DATE VARCHAR(11),
...
/*
EXEC MSAJAG.DBO.API_CLIENT_FUNDS_REPORTING_ABL 'APR  1 2021','AUG 31 2021','A48002','A48002'
*/
...
SELECT DATA = @MEMBERNAME+','+ REPLACE(CONVERT(VARCHAR, CONVERT(DATETIME, @FR_DATE , 109), 103), '/', '') + ' to ' + REPLACE(CONVERT(VARCHAR, CONVERT(DATETIME, @TO_DATE , 109), 103), '/', '')
UNION ALL
SELECT 'Party Code,PARTY NAME,PAN,Voucher Date,Effective Date,Exchange,Segment,Settlement No.,Cheque/DD No.,Vtype,Particulars,Voucher Number,Debit,Credit,Balance'
UNION ALL
SELECT DATA = UCC_CODE + ',' + LONG_NAME + ',' + CLIENT_PAN + ',' + CONVERT(VARCHAR(11), VDT, 105) + ',' + CONVERT(VARCHAR(11), EDT, 105) + ',' + EXCHANGE + ',' + SEGMENT + ',' + ISNULL(SETT_NO, '') + ',' + ISNULL(CHQ_NO, '') + ',' + VTYPE + ',' + NARRATION + ',' + VNO + ',' + CONVERT(VARCHAR, DRAMOUNT) + ',' + CONVERT(VARCHAR, CRAMOUNT) + ',' + CONVERT(VARCHAR, RUNNING_BALANCE) FROM #LEDGER_PL_DETAILS_FINAL
