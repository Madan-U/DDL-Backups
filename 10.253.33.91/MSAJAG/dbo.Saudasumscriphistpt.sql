-- Object: PROCEDURE dbo.Saudasumscriphistpt
-- Server: 10.253.33.91 | DB: MSAJAG
--------------------------------------------------


/****** Object:  Stored Procedure dbo.Saudasumscriphistpt    Script Date: 3/17/01 9:56:05 PM ******/

/****** Object:  Stored Procedure dbo.Saudasumscriphistpt    Script Date: 3/21/01 12:50:25 PM ******/

/****** Object:  Stored Procedure dbo.Saudasumscriphistpt    Script Date: 20-Mar-01 11:39:04 PM ******/

/* This sp is used in the saudasumopenctlprj
    created by amt on16/02/2001 
   table used history*/

CREATE PROCEDURE Saudasumscriphistpt
@partycode varchar(10),
@fromdate varchar(20),
@todate  varchar(20),
@fromscrip varchar(30),
@toscrip  varchar(30),
@setttype varchar(2)
 AS
...
from history h where h.PARTY_CODE=@partycode and h.tradeqty > 0 and
 h. Sauda_date  >=@fromdate and h. Sauda_date <=@todate  and h.scrip_cd >=@fromscrip  and h.scrip_cd <=@toscrip and h.sett_type like @setttype 
...
Pbrk=round((case when s.sell_buy = 1 then sum(Brokapplied*tradeqty) else 0 end ),2),
 ...
 from history s where s.PARTY_CODE=@partycode and s.tradeqty > 0 and s.scrip_cd >=@fromscrip  and s.scrip_cd <=@toscrip and
