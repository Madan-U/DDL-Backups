-- Object: PROCEDURE dbo.USP_NKAB_BUYBACK_LEC
...
CREATED BY :- HRISHI ON 06-02-2026
DESCREPTION:- NKAB CLIENTS BUYBACK LEC FOR ALL BANKS
*/

CREATE PROC [dbo].[USP_NKAB_BUYBACK_LEC] ( @SDATE AS VARCHAR(11) )
AS
BEGIN
...
WHERE SAUDA_DATE=@SDATE AND SUB_BROKER IN ('BHKWT','BH','NKAB') AND SETT_TYPE='BX'
...
WHERE SAUDA_DATE=@SDATE AND SUB_BROKER IN ('BHKWT','BH','NKAB') AND SETT_TYPE='OB'
--WHERE SAUDA_DATE=@SDATE AND PARTY_CODE IN ( @CODE )  
GROUP BY S.PARTY_CODE, PARTY_NAME,CLIENTTYPE ,MEMBERTYPE,COMPANYNAME,SCRIP_CD,SAUDA_DATE,SCRIP_NAME,ISIN,CONTRACTNO,SETT_NO,SETT_TYPE,EXCHANGE,SEGMENT,SERIES   
...
TRUNCATE TABLE DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_HDFC_YES

INSERT INTO DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_HDFC_YES
...
TRUNCATE TABLE DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_KOTAK

INSERT INTO DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_KOTAK
...
SELECT V.Fld_BankName AS 'BANKNAME', 'T' AS 'HEAD', LTRIM(RTRIM(PARTY_CODE)) AS PARTY_CODE, CONTRACTNO, [BUY/SELL], M.ISIN, M.STRADEDQTY, SMARKETRATE, SELLDELIVERYCHRG, STRADEDAMT, SCRIP_NAME
INTO #TAIL_DATA
FROM #MAIN_DATA M LEFT JOIN #V_NRIBANK_DETAILS V ON M.PARTY_CODE=V.Fld_ClientCode
--WHERE PARTY_CODE IN ('ZR1577R','ZR1297R','ZR844R')

TRUNCATE TABLE DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS

INSERT INTO DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS
...
----SELECT * FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS ORDER BY PARTY_CODE

DECLARE @AXISFILENAME AS VARCHAR(200)
DECLARE @BCPCOMMAND1 AS VARCHAR(1000)
IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%AXIS%')>0)                          
     BEGIN                          
      SET @AXISFILENAME = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\AXIS_NKAB_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.TXT'                           
      SET @BCPCOMMAND1 = 'BCP "SELECT DETAILS FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%AXIS%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
...
IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%INDUSIND%')>0)                          
     BEGIN                          
      SET @INDUSINDFILENAME = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\INDUSIND_NKAB_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.TXT'                           
      SET @BCPCOMMAND2 = 'BCP "SELECT DETAILS FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%INDUSIND%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
...
IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE '%IDFC%')>0)                          
     BEGIN                          
      SET @IDFCFILENAME = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\IDFC_NKAB_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.TXT'                           
      SET @BCPCOMMAND3 = 'BCP "SELECT DETAILS FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_ALLBANKS WHERE BANKNAME LIKE ''%IDFC%'' ORDER BY PARTY_CODE,HEAD" QUERYOUT "'                          
...
IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_HDFC_YES where [BANK NAME] like '%hdfc%')>0)                          
 BEGIN          
  DECLARE @FILENAME1 VARCHAR(100) = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\HDFC_NKAB_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.CSV'                       
  ...
  SET @ALL1 = @ALL1+ ' UNION ALL SELECT * FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_HDFC_YES WHERE [BANK NAME] like ''''%hdfc%'''''                              
  --PRINT @ALL1                                
 ...
 IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_HDFC_YES where [BANK NAME] like '%YES%')>0)                          
 BEGIN          
  DECLARE @FILENAME2 VARCHAR(100) = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\YES_NKAB_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.CSV'                       
  DECLARE @ALL2 VARCHAR(MAX)                                
                                
  SET @ALL2 = 'EXEC MASTER.DBO.XP_CMDSHELL ''BCP "SELECT ''''[BANK NAME]'''',''''[TRADE DATE]'''',''''[TRADING ID]'''',''''[DP ID]'''',''''[DEMAT A/C NO]'''',''''[INVESTOR NAME]'''',''''[EXCHANGE]'''',''''[SETTLEMENT NO]'''',''''[CONTRACT NO]'''',''''[SCRIP CODE]'''',''''[ISIN]'''',''''[SCRIP_NAME]'''',''''[BUY/SELL]'''',''''[QUANTITY]'''',''''[WEIGHTED AVERAGE MARKET RATE]'''',''''[TOTAL MARKET VALUE]'''',''''[NET RATE]'''',''''[BROKEARGE]'''',''''[OTHER CHARGES]'''',''''[NET PURCHASE/SALE VALUE]'''',''''[STT ON BUY & SALE]'''',''''[EXACT AMOUNT PAID BY / TO  THE CUSTOMER (FOR PURCHASE/SALE)]'''',''''[RBI APPROVAL NO]'''',''''[PIS ACCOUNT NO]'''',''''[SAVING BANK ACCOUNT NO]'''',''''[BANK BRANCH NAME]'''',''''[BROKER NAME]'''',''''[DATE OF RECEIPT /PAYMENT]'''',''''[DEMAT DEBIT /CREDIT DATE]'''',''''[BROKER A/C NO]'''''
  SET @ALL2 = @ALL2+ ' UNION ALL SELECT * FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_HDFC_YES WHERE [BANK NAME] like ''''%YES%'''''                              
  --PRINT @ALL2
...
IF ((SELECT COUNT(1) FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_KOTAK)>0)                          
 BEGIN          
  DECLARE @FILENAME3 VARCHAR(100) = '\\INHOUSELIVEAPP1-FS.ANGELONE.IN\UPLOAD1\NRI_CLIENTS\KOTAK_NKAB_BUYBACK_'+REPLACE (CONVERT (VARCHAR(12),GETDATE(),103),'/','')+'.CSV'                       
  ...
  SET @ALL3 = @ALL3+ ' UNION ALL SELECT * FROM DUSTBIN.DBO.TBL_NRI_NKAB_BUYBACK_LEC_KOTAK'                              
  --PRINT @ALL3                                
