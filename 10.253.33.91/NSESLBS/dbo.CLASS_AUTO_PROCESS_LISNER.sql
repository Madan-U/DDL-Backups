...
DECLARE 
  ...
  @FILE_DWLOAD_LOCATION VARCHAR(500),          
...
BEGIN
	...
	BEGIN
		...
		IF @PROCESS_SET = 'PMSTRADE'
		BEGIN 
			IF (SELECT ISNULL(COUNT(1),0) FROM TBL_AUTO_PROCESS_DETAIL WHERE PROCESS_DATE = LEFT(GETDATE(),11)
				AND PROCESS_FOR = 'PMS TRADE' AND (IS_DEPEND_ON = '#-1,' OR PROCESS_STATUS = 100)) > 0
			BEGIN
		...
		BEGIN
			...
			AND PROCESS_SET = @PROCESS_SET AND L.PROCESS_FOR <> 'PMS TRADE'

			UPDATE TBL_AUTO_PROCESS_DETAIL SET IS_DEPEND_ON = '',PROCESS_STATUS = 0, PROCESS_HALTED_REASON = '',
			SUB_PROCESS_ID = 0
			FROM TBL_AUTO_PROCESS_LISNER L
			WHERE PROCESS_DATE = LEFT(GETDATE(),11) AND (IS_DEPEND_ON LIKE '%#-1,%' OR PROCESS_STATUS = 100)
			AND L.PROCESS_FOR = TBL_AUTO_PROCESS_DETAIL.PROCESS_FOR
