...
CREATE Proc DelProbableShortage ( @OptionFlag  int = 0) As                       
DeClare                       
@@Sett_No Varchar(7),                      
@@Sett_Type Varchar(2),                      
...
And */Sett_Type Not Like 'A%'  
 ...
 Into #NSE_DelPayInMatch From #NSESett_Mst N, MultiIsIn M,DeliveryClt D Left Outer Join DelTrans c          
...
Insert into #NSE_DelPayInMatch  
 ...
 select D.Sett_no,D.Sett_Type,D.Party_Code,CertNo=IsIn,DelQty=D.Qty,RecQty=Sum(IsNull(C.Qty,0)),    
 ISettQty = 0,IBenQty=0          
 Into #BSE_DelPayInMatch From #BSESett_Mst N, BSEDB.DBO.MultiIsIn M, BSEDB.DBO.DeliveryClt D Left Outer Join BSEDB.DBO.DelTrans c          
 On ( D.Sett_no = C.Sett_No And D.Sett_Type = C.Sett_Type          
 And D.Scrip_cd = C.Scrip_cd And D.Series = C.Series           
 And D.Party_Code = C.Party_Code And DrCr = 'C'           
 And Filler2 = 1 And ShareType <> 'AUCTION' )          
