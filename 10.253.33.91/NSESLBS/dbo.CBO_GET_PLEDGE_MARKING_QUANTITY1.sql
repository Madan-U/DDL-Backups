-- Object: PROCEDURE dbo.CBO_GET_PLEDGE_MARKING_QUANTITY1
...
create  PROCEDURE [dbo].[CBO_GET_PLEDGE_MARKING_QUANTITY1]
	@FLAG   VARCHAR(1),
        @DPID VARCHAR(10),
        @CLTDPID VARCHAR(10),
	...
	IF @STATUSID <> 'BROKER'
        ...
        ELSE IF @FLAG = 'I'
			BEGIN
				...
				SET @SQL="select Isnull(Sum(Qty),0)AS Quantity from bsedb.dbo.DelTrans D With(Index(DelHold), NoLock) Where Party_Code ='"+@PARTYCODE+"' And Scrip_Cd = '"+@SCRIPCODE+"' And Series = '"+@PARTYCODE+"' And Delivered = '0' And  DrCr = 'D' And Filler2 = '1' And BDpId = '"+@DPID+"' And BCltDpId = '"+@CLTDPID+"'" 
	                        EXEC(@SQL)
			END
