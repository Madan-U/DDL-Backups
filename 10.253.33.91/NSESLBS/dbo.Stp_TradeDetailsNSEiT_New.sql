-- Object: PROCEDURE dbo.Stp_TradeDetailsNSEiT_New
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------


CREATE      Proc Stp_TradeDetailsNSEiT_New
...
End
	...
	Sauda_Date, TradeQty, MarketRate= Dummy1, Party_Code, Sett_No, Sett_Type, Scrip_Cd
	...
	and Sett_Type = (Case When (@Sett_Type= 'DR' OR @SETT_TYPE = 'DI') and @ExchangeCode='23'  Then 'N' 
		                               When @Sett_Type= 'TT' and @ExchangeCode='23'  Then 'W' 
	                                               When (@Sett_Type= 'DR' OR @SETT_TYPE = 'DI') and @ExchangeCode='1'  Then 'D'
		                               When @Sett_Type= 'TT' and @ExchangeCode='1'  Then 'C'
	                                               When @Sett_Type= 'AD' and @ExchangeCode='23'  Then 'A' 
	                                               When @Sett_Type= 'AD' and @ExchangeCode='1'  Then 'AD' Else '' End)
	...
	Select Header = @BatchNo, '13', '*****', 
	ContractNo =  (Case When @ExchangeCode='23' 
			    Then 'A23'+ Ltrim(Rtrim(I.ContractNo)) 
			    When @ExchangeCode='1' 
			    Then 'A01'+ Ltrim(Rtrim(I.ContractNo)) 
			    Else '' 
		       End) ,
       	order_no = lTrim(rTrim(Order_no)) ,trade_no = Replace(Replace(Replace(lTrim(rTrim(Trade_no)),'R',''),'C',''),'A',''),
	TradeTime = Left(Replace(Replace(Replace(Convert(Varchar,Sauda_date,120),'-',''),':',''),' ',''),8) + ' ' + right(Replace(Replace(Replace(Convert(Varchar,Sauda_date,120),'-',''),':',''),' ',''),6),
	TradeQty = TradeQty,  MarketRate=CONVERT(VARCHAR,Convert(Numeric(10,4),I.MarketRate)), I.Party_code
	...
	and Sett_No = @Sett_No  and Sett_Type = (Case
		 ...
		 When @Sett_Type= 'TT' and @ExchangeCode='1'  Then 'C'
	         When @Sett_Type= 'AD' and @ExchangeCode='23'  Then 'A' 
	         When @Sett_Type= 'AD' and @ExchangeCode='1'  Then 'AD' Else '' End)
	And C2.Party_code = I.Party_Code And C2.Dummy6 = 'NSEIT' and C2.Dummy7 = '0'
	And Scrip_Cd = @ScripCd 
