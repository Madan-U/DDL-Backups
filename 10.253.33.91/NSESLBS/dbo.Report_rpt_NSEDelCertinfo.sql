-- Object: PROCEDURE dbo.Report_rpt_NSEDelCertinfo
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

CREATE Proc Report_rpt_NSEDelCertinfo  
@targetid varchar(2),  
@settno varchar(7),  
@settype varchar(3),  
@scripcd varchar(20),  
...
begin  
/* select C.Sett_no,C.Sett_Type,C.Party_Code,C.Scrip_cd,C.Series,TargetParty=D.Party_Code,D.Qty,C.TransDate,C.CertNo,C.HolderName,C.FolioNo, D.TrType, D.Reason, D.CltDpId,D.DpId from Deltrans_Report D,Deltrans_Report C where C.sett_no = @SettNo and C.sett
...
begin  
/*  
select C.Sett_no,C.Sett_Type,C.Party_Code,C.Scrip_cd,C.Series,TargetParty=D.Party_Code,d.Qty,D.TransDate,D.CertNo,D.HolderName,D.FolioNo, D.TrType, D.Reason, D.CltDpId,D.DpId  from Deltrans_Report D,Deltrans_Report C where C.sett_no = @SettNo and C.sett_t
...
from Deltrans_Report (nolock) where sett_no = @SettNo and sett_type = @Settype  
