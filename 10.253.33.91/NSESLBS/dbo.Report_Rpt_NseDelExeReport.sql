-- Object: PROCEDURE dbo.Report_Rpt_NseDelExeReport
...
CREATE Proc Report_Rpt_NseDelExeReport 
(@TDate Varchar(11),
 @BDpId Varchar(8),
 @BCltDpId Varchar(16),
 @FParty Varchar(10),
...
From Deltrans_Report D, DeliveryDp DP, Client1_Report C1, Client2_Report C2 Where DrCr = 'D' And Delivered = 'D'
...
Union All
Select Sett_no,Sett_Type,D.Party_Code,C1.Short_Name,Scrip_CD,Series,Qty=Sum(Qty),CertNo,FromNo,D.DpType,D.DpId,D.CltDpId,ISett_No,ISett_Type,Remark='Pay-Out' 
From Deltrans_Report D, DeliveryDp DP, Client1_Report C1, Client2_Report C2 Where DrCr = 'D' And Delivered = 'G'
And D.BDpId = Dp.DpId And BCltDpId = Dp.DpCltNo And TrType In ( 904 , 905 )
...
And D.Party_Code >= @FParty And D.Party_Code <= @TParty
And Scrip_Cd >= @FScrip And Scrip_Cd <= @TScrip
And D.SNo in ( Select Sno From Deltrans_ReportTemp Where BDpId = @BDpId And BCltDpId = @BCltDpId and TransDate Like @TDate + '%') 
Group By Sett_no,Sett_Type,D.Party_Code,C1.Short_Name,Scrip_CD,Series,CertNo,FromNo,D.DpType,D.DpId,D.CltDpId,ISett_No,ISett_Type
Union All
...
From Deltrans_Report D, DeliveryDp DP, Client1_Report C1, Client2_Report C2  Where DrCr = 'D' And Delivered in ('G','D')
...
From Deltrans_Report D, DeliveryDp DP, DeliveryDp Dp1, Client1_Report C1, Client2_Report C2  Where DrCr = 'D' And Delivered = 'G'
And D.BDpId = Dp.DpId And BCltDpId = Dp.DpCltNo And TrType In ( 904 , 905 )
...
And D.Party_Code >= @FParty And D.Party_Code <= @TParty
And Scrip_Cd >= @FScrip And Scrip_Cd <= @TScrip
And TCode in (Select TCode From Deltrans_Report Where DrCr = 'D' 
...
And D.Party_Code >= @FParty And D.Party_Code <= @TParty
And Scrip_Cd >= @FScrip And Scrip_Cd <= @TScrip
And D.Sno in (Select Sno From Deltrans_ReportTemp Where BDpId = Dp1.DpId And BCltDpId = Dp1.DpCltNo And Party_Code = D.Party_Code 
...
And D.Party_Code >= @FParty And D.Party_Code <= @TParty
And Scrip_Cd >= @FScrip And Scrip_Cd <= @TScrip
And D.Sno In ( Select SNo From Deltrans_ReportTemp Where DP.DpId = @BDpId And DP.DpCltNo = @BCltDpId and TransDate Like @TDate + '%')
Group By Sett_no,Sett_Type,D.Party_Code,C1.Short_Name,Scrip_CD,Series,CertNo,FromNo,D.DpType,D.DpId,D.CltDpId,ISett_No,ISett_Type
Union All
