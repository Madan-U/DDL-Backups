-- Object: PROCEDURE dbo.REPORT_rpt_shortagepayout1
...
CREATE procedure REPORT_rpt_shortagepayout1    
...
/*ToGive=(Case When Inout = 'O' Then D.Qty Else -D.Qty End )+Sum(Case When DrCr = 'C' Then isnull(De.qty,0)Else 0 End),    
Given = Sum(Case When DrCr = 'D' Then isnull(De.qty,0)Else 0 End) */    
ToGive=(Case When Inout = 'O' Then D.Qty Else -D.Qty End )+(Case When D.Sett_Type <> 'W' Then     
Sum(Case When DrCr = 'C' Then isnull(De.qty,0)Else 0 End) Else 0 End),    
Given = Sum(Case When DrCr = 'D' Then isnull(De.qty,0)Else 0 End)    
 ...
 and d.sett_no >= @Settno And D.Sett_No <= @toSettno and d.sett_type = @settype    
 group by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series,d.qty,Inout    
having (Case When Inout = 'O' Then D.Qty Else -D.Qty End )+(Case When D.Sett_Type <> 'W' Then     
Sum(Case When DrCr = 'C' Then isnull(De.qty,0)Else 0 End) Else 0 End)    
- Sum(Case When DrCr = 'D' Then isnull(De.qty,0)Else 0 End) > 0     
Union     
select d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.Scrip_cd,d.Series,    
ToGive=(Case When D.Sett_Type <> 'W' Then Sum(Case When DrCr = 'C' Then isnull(D.qty,0)Else 0 End) Else 0 End),    
...
and Filler2 = 1 And D.Party_Code not in ( Select Party_Code From DeliveryClt_Report Where     
sett_no = d.sett_no and sett_type = d.sett_type and scrip_cd = d.scrip_cd     
and series = d.series and party_code = d.party_code ) And ShareType <> (Case When D.Sett_Type Not Like 'A%' Then 'AUCTION' Else '' End)    
group by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
having (Case When D.Sett_Type <> 'W' Then Sum(Case When DrCr = 'C' Then isnull(D.qty,0)Else 0 End) Else 0 End)    
...
ToGive=(Case When D.Sett_Type <> 'W' Then Sum(Case When DrCr = 'C' Then isnull(D.qty,0)Else 0 End) Else 0 End),    
Given = Sum(Case When DrCr = 'D' Then isnull(D.qty,0)Else 0 End)    
from Client2_Report C2,Client1_Report C1, Branches br,Deltrans_Report d    
where D.Party_Code = C2.Party_Code and C1.Cl_Code = C2.Cl_Code    
and d.sett_no >= @settno and d.sett_no <= @tosettno and d.sett_type like @settype     
and Filler2 = 1 And D.Party_Code not in ( Select Party_Code From DeliveryClt_Report Where     
sett_no = d.sett_no and sett_type = d.sett_type and scrip_cd = d.scrip_cd     
and series = d.series and party_code = d.party_code and inout = 'I')    
and br.short_name = c1.trader and br.branch_cd = @statusname And ShareType <> (Case When D.Sett_Type Not Like 'A%' Then 'AUCTION' Else '' End)    
group by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
having (Case When D.Sett_Type <> 'W' Then Sum(Case When DrCr = 'C' Then isnull(D.qty,0)Else 0 End) Else 0 End)    
- Sum(Case When DrCr = 'D' Then isnull(D.qty,0)Else 0 End) > 0     
order by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
...
ToGive=(Case When D.Sett_Type <> 'W' Then Sum(Case When DrCr = 'C' Then isnull(D.qty,0)Else 0 End) Else 0 End),    
Given = Sum(Case When DrCr = 'D' Then isnull(D.qty,0)Else 0 End)    
from Client2_Report C2,Client1_Report C1, subbrokers sb,Deltrans_Report d    
where D.Party_Code = C2.Party_Code and C1.Cl_Code = C2.Cl_Code    
and d.sett_no >= @settno and d.sett_no <= @tosettno and d.sett_type like @settype     
and Filler2 = 1 And D.Party_Code not in ( Select Party_Code From DeliveryClt_Report Where     
sett_no = d.sett_no and sett_type = d.sett_type and scrip_cd = d.scrip_cd     
and series = d.series and party_code = d.party_code and inout = 'I')    
and sb.sub_broker = c1.sub_broker and sb.sub_broker = @statusname And ShareType <> (Case When D.Sett_Type Not Like 'A%' Then 'AUCTION' Else '' End)    
group by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
having (Case When D.Sett_Type <> 'W' Then Sum(Case When DrCr = 'C' Then isnull(D.qty,0)Else 0 End) Else 0 End)    
- Sum(Case When DrCr = 'D' Then isnull(D.qty,0)Else 0 End) > 0     
order by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
...
sett_no = d.sett_no and sett_type = d.sett_type and scrip_cd = d.scrip_cd     
and series = d.series and party_code = d.party_code and inout = 'I')    
and c1.trader = @statusname And ShareType <> (Case When D.Sett_Type Not Like 'A%' Then 'AUCTION' Else '' End)    
group by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
...
sett_no = d.sett_no and sett_type = d.sett_type and scrip_cd = d.scrip_cd     
and series = d.series and party_code = d.party_code and inout = 'I')    
and c2.party_code = @statusname And ShareType <> (Case When D.Sett_Type Not Like 'A%' Then 'AUCTION' Else '' End)    
group by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
having (Case When D.Sett_Type <> 'C' Then Sum(Case When DrCr = 'C' Then isnull(D.qty,0)Else 0 End) Else 0 End)    
- Sum(Case When DrCr = 'D' Then isnull(D.qty,0)Else 0 End) > 0     
order by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
...
sett_no = d.sett_no and sett_type = d.sett_type and scrip_cd = d.scrip_cd     
and series = d.series and party_code = d.party_code and inout = 'I')    
and c1.family = @statusname And ShareType <> (Case When D.Sett_Type Not Like 'A%' Then 'AUCTION' Else '' End)    
group by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
having (Case When D.Sett_Type <> 'W' Then Sum(Case When DrCr = 'C' Then isnull(D.qty,0)Else 0 End) Else 0 End)    
- Sum(Case When DrCr = 'D' Then isnull(D.qty,0)Else 0 End) > 0     
order by d.sett_no,d.sett_type,d.Party_Code,c1.Short_Name,d.scrip_cd,d.series    
