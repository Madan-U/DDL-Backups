-- Object: PROCEDURE dbo.Stp_ContractDetailsNSEiT_Group_INCR
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

--Exec Stp_ContractDetailsNSEiT_Group_INCR 'Mar 22 2004','Mar 22 2004', '0017852','0017852', 'UT056', 'UT056', '','0000005', '10229','IN231111131', 'NSE','','Brokerage inclusive of STT'  
  
CREATE            Proc Stp_ContractDetailsNSEiT_Group_INCR  
@FromDate Varchar(11),  
@ToDate Varchar(11),  
...
Sett_Type = (Case  
          When  I.Sett_Type in( 'N', 'D') Then 'DR'   
...
TradeAmount=CONVERT(VARCHAR,CONVERT(NUMERIC(18,4),round((Sum(Round(I.MarketRate,Convert(integer,T.marketrate)) * I.Tradeqty)/(Case When Sum(I.Tradeqty)>0 Then Sum(I.Tradeqty) Else 1 End))*Sum(I.Tradeqty),4))),   
Brokerage =CONVERT(VARCHAR,CONVERT(NUMERIC(18,4),round((Case When Service_chrg = 1   
                      Then Round(Sum(I.Brokapplied* I.Tradeqty)+Sum(I.Service_tax),Convert(integer,T.brokerage))  
         Else Round(Sum(I.Brokapplied* I.Tradeqty),Convert(integer,T.brokerage))  
                     End),4))),   
ServiceTax = CONVERT(VARCHAR,CONVERT(NUMERIC(18,4),round((Case When Service_chrg = 0   
...
Ins_Chrg = CONVERT(VARCHAR,CONVERT(NUMERIC(18,0),round((Case When Insurance_chrg = 1   
                   Then Round(Sum(I.Ins_Chrg),0)  
                    Else 0  
                   End),4))),   
SettAmount = CONVERT(VARCHAR,CONVERT(NUMERIC(18,4),round((Case When Sell_Buy = 1   
...
pan_gir_no, C.DpId,U.mapidid, U.Ucc_code,CounterParty, Insurance_Chrg  
