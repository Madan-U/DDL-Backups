-- Object: PROCEDURE dbo.InterBenTrans_Pld
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------



CREATE  Proc InterBenTrans_Pld ( @Sett_no Varchar(7),@Sett_Type Varchar(2),@RefNo Int, @SEGMENT VARCHAR(7) = '%') As
Declare 
 @@Scrip_Cd Varchar(12),
 @@series Varchar(3),
...
Begin
 ...
 begin
	...
	And SETT_NO = @@PSett_No And sett_type = @@PSett_Type and Party_Code = @@Party_Code
 	and CertNo = @@IsIn and DrCr = 'D' and Filler2 = 1 and TrType = 904  and sharetype <> 'auction'
             And Dp.DpId = D.BDpId And Dp.DpCltNo = D.BCltDpId And Dp.Description Not Like '%POOL%' and Description not Like '%PLEDGE%' AND ACCOUNTTYPE = 'MAR' AND SEGMENT LIKE @SEGMENT
	order by Qty asc
	End
	Else
	Begin
 ...
 Begin
	...
	And Description Not Like '%POOL%' and Description not Like '%PLEDGE%' AND ACCOUNTTYPE = 'MAR'  AND SEGMENT LIKE @SEGMENT
	Else
	update BSEDB.DBO.DelTrans set ISett_no=@Sett_No,ISett_Type=@Sett_Type,TrType=1000,Reason = 'Inter Exc Ben Transfer' 
