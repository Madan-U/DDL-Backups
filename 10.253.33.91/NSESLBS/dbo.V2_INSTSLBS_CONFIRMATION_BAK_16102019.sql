-- Object: PROCEDURE dbo.V2_INSTSLBS_CONFIRMATION_BAK_16102019
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

--EXEC V2_INSTSLBS_CONFIRMATION 'broker','broker','Jun 13 2012','2012112','L','Z10412','Z10412','','zzzzzzzzzzzz','','zzzzzzzzzzzz','','','CONTRACT' 

--EXEC V2_INSTSLBS_CONFIRMATION 'broker','broker','May 25 2007','2007097','N','0','ZZZ','','zzzzzzzzzz','','zzzzzzzzzz','0','99999999999','CONTRACT' 
...
IF @CONTFLAG = 'CONTRACT' 
	...
	AND SERIES = 'EQ'   
		...
		PSCHEME =(
		CASE 
			WHEN S.SETT_TYPE <> 'P' AND SELL_BUY = 1
			THEN Sum(SCHEME*TradeQty)/Sum(TradeQty)
			ELSE 0
			END),
		SSCHEME =(
		CASE 
			WHEN S.SETT_TYPE <> 'L' AND SELL_BUY = 2
			THEN Sum(SCHEME*TradeQty)/Sum(TradeQty)
			ELSE 0
			END),
		SCHEME =(
		CASE 
			...
			WHEN S.SETT_TYPE = 'P' AND SELL_BUY = 1 
			THEN 0 
			ELSE Sum(SCHEME*TradeQty)/Sum(TradeQty) 
			END),
		...
		PSCHEMENETRATE =(   
				...
				WHEN S.SETT_TYPE <> 'P' AND SELL_BUY = 1
				THEN S.SCHEME
				ELSE 0 END) + S.MARKETRATE)*TRADEQTY) / SUM(TRADEQTY)
                        ELSE 0 END ),		
		SSCHEMENETRATE =(   
                CASE   
                        WHEN SELL_BUY = 2   
				...
				ELSE 0 END)+ S.MARKETRATE)*TRADEQTY) / SUM(TRADEQTY)
                        ELSE 0 END ),
		SCHEMENETRATE = SUM(((CASE
					WHEN S.SETT_TYPE = 'L' AND SELL_BUY = 2 
					THEN 0 
					WHEN S.SETT_TYPE = 'P' AND SELL_BUY = 1 
					THEN 0
					ELSE S.SCHEME END) + S.MARKETRATE)* TRADEQTY) / SUM(TRADEQTY),
                SRate = (    
                CASE     
                        WHEN Sell_Buy = 2     
                        THEN Sum(MarketRate*TradeQty)/Sum(TradeQty)     
             ...
             WHEN Sell_Buy = 2     
                ...
                CASE     
                        ...
                        THEN Sum(NserTax)     
                        ELSE 0 END),     
                PTOTCHARGE = SUM(CASE WHEN SELL_BUY = 1 
				THEN (TRADEQTY*NBROKAPP) + (NSERTAX+TURN_TAX+SEBI_TAX+INS_CHRG+BROKER_CHRG+S.OTHER_CHRG)
				ELSE 0 END),
                STOTCHARGE = SUM(CASE WHEN SELL_BUY = 2 
				THEN (TRADEQTY*NBROKAPP) + (NSERTAX+TURN_TAX+SEBI_TAX+INS_CHRG+BROKER_CHRG+S.OTHER_CHRG)
				ELSE 0 END),
				POTHER_CHRG=SUM(                  
					CASE                  
				...
				ReturnSessionNo = CONVERT(VARCHAR(30),'') 
	...
	FROM #CLOSING         
	WHERE #INSTCONTCUMBILL.SCRIP_CD = #CLOSING.SCRIP_CD           


	UPDATE #INSTCONTCUMBILL 
	SET ReturnSessionNo = R.SETT_NO+ ' ' + REPLACE(CONVERT(VARCHAR,CONVERT(DATETIME,EXPIRYDATE),106),' ','-'), SCRIPNAME = RTRIM(LTRIM(SCRIPNAME)) + '-' + REPLACE(CONVERT(VARCHAR,CONVERT(DATETIME,EXPIRYDATE),106),' ','')
	FROM (        
	...
	AND S.SETT_TYPE = 'P' AND RIGHT(SETT_NO,3) >= '500'     
	) R        
	WHERE #INSTCONTCUMBILL.SERIES = R.SERIES        
