-- Object: PROCEDURE dbo.ADD_MULTISERVER_SUBBROKER
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

CREATE PROCEDURE ADD_MULTISERVER_SUBBROKER
(
...
DECLARE
		...
		@SQL Varchar(2000),
		@SB_COUNT INT
CREATE TABLE #SUBBROKEREXIST (SUBBROKER VARCHAR(10))
...
While @@Fetch_Status = 0       
	BEGIN
		SET @SQL = "INSERT INTO #SUBBROKEREXIST (SUBBROKER) SELECT SUBBROKER FROM " + @share_server + "." + @share_db + ".DBO.SubBrokers WHERE SUBBROKER = '" + @subbroker + "'"
		--PRINT @SQL
	    EXEC(@SQL)
