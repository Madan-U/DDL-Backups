-- Object: PROCEDURE dbo.Rpt_StampDuty_New_123
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

CREATE Proc Rpt_StampDuty_New_123 (@Start_Date Varchar(11), @End_Date Varchar(11))  
As  
Select S2.Sett_Type,   
...
Into #san From settlement, Client2, Client1, sett_mst s2   
where settlement.sett_no = s2.sett_no And settlement.sett_type = s2.sett_type  
and s2.end_date >= @Start_Date and s2.end_date <= @End_Date + ' 23:59'  
...
Cl_Type, L_State  
Into #san_report1  
From #san Where L_State = 'MAHARASHTRA'  
Group by Cl_Type, L_State  
  
Insert Into #san_report1  
Select     
...
From #san, Owner Where Not Exists (Select State From State_Master Where State = L_State )  
...
From #san Where Exists (Select State_Master.State From State_Master, Owner Where State_Master.State = L_State And State_Master.State <> Owner.State)  
Group by Cl_Type, L_State  
