-- Object: PROCEDURE dbo.Pr_Mig_DMS_Client_Details
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------


CREATE  PROCEDURE Pr_Mig_DMS_Client_Details
(              
...
BEGIN              
 ...
 SET @strPayBank = 'HDFC'    
	...
	---- Select the data from DMCClientDetails table into #CLIENT_DETAILS table 
  INSERT INTO #CLIENT_DETAILS              
	...
	SELECT
		...
		RECORD_FOUND = 'N', 
		VALID_RECORD = 'N',
		AC_NUM = Left(AccountNo,20),             
	   AC_TYPE = 'S',              
	   ADDEMAILID = Left(ClientName,230),              
	   APPROVER = '',              
	   AREA = Dbo.fnDmsGetAreaForGivenParty(CL.ClientCode),              
	   BANK_ID = Dbo.fnGetPoBankIdForGivenBankDetails(Left(BankName, 50), Left(BanksBranch, 50)),		
	   BANK_NAME = Left(BankName, 50),              
	   BRANCH_CD = Dbo.fnDmsGetBranchForGivenParty(CL.ClientCode),              
	   BRANCH_NAME = Left(BanksBranch, 50),             
	   C_GROUP = '',              
	   CHK_ANNUAL_REPORT = 0,              
	   CHK_BANK_CERTIFICATE =0 ,              
	   CHK_CORP_DTLS_RECD =0,              
	   CHK_CORPORATE_DEED =0,              
	   CHK_KYC_FORM = 0,              
	   CHK_NETWORTH_CERT =0 ,              
	   CL_CODE = CL.ClientCode,              
	   CL_STATUS = CL.ClientTypeCode, --dbo.fnDmsGetStatusForGivenParty(CL.ClientCode),              
	   CL_TYPE = 'CLI',--dbo.fnDmsGetTypeForGivenParty(CL.ClientCode),    -- As received mail on 08-JAN-2008                        
	   CLIENT_AGREEMENT_ON = RegistrationDate,              
	   CLTDPID1 = DP.ClientID,              
	   CLTDPID2= '',              
	   CLTDPID3 = '',              
	   DEALING_WITH_OTHER_TM = '',              
	   DEPOSITORY1 = CASE LEN(RTrim(DP.ClientID)) WHEN 16 THEN 'CDSL' ELSE 'NSDL' END ,              
	   DEPOSITORY2 = '',              
	   DEPOSITORY3 = '',              
	   DIRECTOR_NAME = '',              
	   DOB = CASE WHEN ISNULL(DateOfBirth, '') <> '' THEN CONVERT(CHAR(11), ISNULL(DateOfBirth, ''), 109) ELSE RegistrationDate END ,              
	   DPID1 = DP.DPID,              
	   DPID2= '',              
	   DPID3= '',              
	   EMAIL = Left(MailId,50),              
	   FAMILY = CASE WHEN FamilyGroup <> '' THEN Left(FamilyGroup, 10) ELSE cl.ClientCode END,              
	   FAX = Left(FaxNo1,15),              
	   FMCODE = '',              
	   IMP_STATUS = 0,              
	   INTERACTMODE = 0,              
	   INTRODUCER = Left(IntroducedBy,30),              
	   INTRODUCER_ID = '',              
	   INTRODUCER_RELATION = '',              
	   IT_RETURN_FILED_ON = 'DEC 31 1949',              
	   IT_RETURN_YR = '',              
-- -- 	   L_ADDRESS1 = Left((RTrim(AddressLine1) + RTrim(AddressLine2)), 39) +',' ,  			--Left(AddressLine1, 40)              
-- -- 	   L_ADDRESS2 = SubString((RTrim(AddressLine1) + RTrim(AddressLine2)), 40,39) + ',',   --Left(AddressLine2, 40)              
-- -- 	   L_ADDRESS3 = SubString((RTrim(AddressLine1) + RTrim(AddressLine2)), 80,40),              
	   L_ADDRESS1 = Left((RTrim(AddressLine1) + RTrim(AddressLine2)), 40) ,  			--Left(AddressLine1, 40)              
	   L_ADDRESS2 = SubString((RTrim(AddressLine1) + RTrim(AddressLine2)), 41,40) ,   --Left(AddressLine2, 40)              
	   L_ADDRESS3 = SubString((RTrim(AddressLine1) + RTrim(AddressLine2)), 81,40),              
	   L_CITY = Left(City,40),              
	   L_NATION = Left(Country,15),              
	   L_STATE = Left(State,50),              
	   L_ZIP = Left(ZipCode,10),              
	   LICENCE_EXPIRES_ON = 'JAN  1 1900',              
	   LICENCE_ISSUED_AT = Left(DrivingPlaceOfIssue, 30),             
	   LICENCE_ISSUED_ON = DrivingDateOfIssue,             
	   LICENCE_NO = Left(DrivingLicenseNo,30),              
	   LONG_NAME = Left(ClientName,100),              
	   MAPIN_ID = Left(MAPINCHAR,12),              
	   MICR_NO = '',              
	   MOBILE_PAGER = '',              
	   MODIFIDEDBY = 'DMS Migration',              
	   MODIFIDEDON = GetDate(),              
	   OFF_PHONE1 = '',              
	   OFF_PHONE2 = '',              
	   OTHER_AC_NO = '',              
	   ...
	   P_STATE = '',              
	   P_ZIP = '',              
	   PAN_GIR_NO = Left(PAN,50),              
	   PARTY_CODE = CL.ClientCode,              
	   PASSPORT_EXPIRES_ON = 'JAN  1 1900',              
	   PASSPORT_ISSUED_AT = Left(PassportPlaceOfIssue,30),              
	   PASSPORT_ISSUED_ON = PassportDateOfIssue,              
	   PASSPORT_NO = Left(PassportNumber, 30),              
	   PAYLOCATION = '', --BanksBranch, --dbo.fnGetLocCodeForGivenLocDetails(BanksBranch,BankName),              
	   POA1 = '',              
	   POA2 = '',              
	   POA3 = '',              
	   RAT_CARD_ISSUED_AT = Left(RationPlaceOfIssue, 30),             
	   RAT_CARD_ISSUED_ON = RationDateOfIssue,              
	   RAT_CARD_NO = Left(RationCardNo,30),              
	   REGION = dbo.fnDmsGetRegionForGivenParty(CL.ClientCode),              
	   REGR_AT = Left(PlaceOfRegistration,30),              
	   REGR_AUTHORITY =Left(RegisteringAuthority, 50),              
	   REGR_NO = Left(RegistrationNo,50),              
	   REGR_ON = DateOfRegistration,              
	   REL_MGR = dbo.fnDmsGetSbuForGivenParty(CL.ClientCode),              
	   REPATRIAT_BANK = 0,              
	   REPATRIAT_BANK_AC_NO = '',              
	   RES_PHONE1 = '',              
	   RES_PHONE2 = '',              
	   SBU = '',--dbo.fnDmsGetSbuForGivenParty(CL.ClientCode),              
	   SEBI_REGN_NO = Left(SEBIRegnNo,25),              
	   SETT_MODE = '',              
	   SEX = '',              
	   SHORT_NAME =Left(ClientName,21),              
	   STATUS = 'U',              
	   SUB_BROKER = dbo.fnDmsGetSubBrokerForGivenParty(CL.ClientCode),               
	   TRADER = dbo.fnDmsGetTraderForGivenParty(CL.ClientCode),                             
	   UCC_CODE = Left(UCCCode,12),              
	   VOTERSID_ISSUED_AT = Left(VoterPlaceOfIssue,30),              
	   VOTERSID_ISSUED_ON = VoterDateOfIssue,              
	   VOTERSID_NO = Left(VoterIdNo, 30),             
	   WARD_NO  = ''      
	FROM DMSClientDetails CL LEFT OUTER JOIN DMSClientDPDetails DP
	ON CL.ClientCode = DP.ClientCode
...
--   CL_CODE = B.CL_CODE,               
   ...
   L_STATE = B.L_STATE,               
   L_ZIP = B.L_ZIP,               
   LICENCE_EXPIRES_ON = 'DEC 31 2049', --B.LICENCE_EXPIRES_ON,               
...
--   PARTY_CODE = B.PARTY_CODE,               
   PASSPORT_EXPIRES_ON = 'DEC 31 2049', --B.PASSPORT_EXPIRES_ON,               
...
--- Insert Client Brokerage Details
EXEC Pr_Mig_DMS_Client_Brok_Details        
	  @pa_CanAddData ,        
	  @pa_CanModifyData ,        
	  @pa_CanDeleteData ,        
	  @pa_err OUTPUT 
...
--- Insert Client Brokerage Details
EXEC Pr_Mig_DMS_Client_Scheme_Mappings        
	  @pa_err OUTPUT 
...
--- Update the Imported Flag for imporated records
UPDATE DMSClientDetails SET Imported = 'Y' 
 FROM DMSClientDetails AS A, #CLIENT_DETAILS AS B               
   WHERE B.VALID_RECORD = 'Y' AND A.ClientCode = B.CL_CODE 
...
EXEC Pr_Mig_DMS_Client_Details @CanAddData, @CanModifyData, @CanDeleteData, @pa_err OUTPUT              
PRINT @pa_err              
              
select bank_id, FMCODE, * from CLIENT_DETAILS

DELETE FROM CLIENT_DETAILS WHERE ModifidedBy = 'DMS Migration'
