...
--EXEC PROC_CALCULATE_CHARGES @SETT_NO, @SETT_TYPE, '0', 'ZZZZ'     
EXEC PROC_SERVICE_TAX_UPDATE @SETT_TYPE, @START_DATE, '0', 'ZZZZ'     
                   
UPDATE SETTLEMENT SET SERVICE_TAX = 0, NSERTAX = 0              
...
AND SERVICE_CHRG = 0              
AND SETTLEMENT.SAUDA_DATE >= 'APR  6 2007'              
              
UPDATE ISETTLEMENT SET SERVICE_TAX = 0, NSERTAX = 0              
FROM CLIENT1 C1, CLIENT2 C2              
WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE              
AND C1.CL_CODE = C2.CL_CODE              
...
AND L_STATE =  'JAMMU AND KASHMIR'              
AND SERVICE_CHRG = 0              
AND ISETTLEMENT.SAUDA_DATE >= 'APR  6 2007'              
...
ORDER_NO,          
MARKETRATE,          
SAUDA_DATE,TABLE_NO,LINE_NO,VAL_PERC,NORMAL,DAY_PUC,DAY_SALES,SETT_PURCH,SETT_SALES,          
SELL_BUY,SETTFLAG,BROKAPPLIED,          
NETRATE=(CASE WHEN SETT_TYPE = 'L' AND SELL_BUY = 1           
       THEN SCHEME + NETRATE           
       ELSE NETRATE           
     END),        
AMOUNT,INS_CHRG,TURN_TAX,OTHER_CHRG,SEBI_TAX,BROKER_CHRG,          
SERVICE_TAX,TRADE_AMOUNT,BILLFLAG,SETT_NO,NBROKAPP,NSERTAX,          
N_NETRATE=(CASE WHEN SETT_TYPE = 'L' AND SELL_BUY = 1           
       THEN SCHEME + N_NETRATE           
       ELSE N_NETRATE           
     END),          
SETT_TYPE,PARTIPANTCODE,STATUS,      
PRO_CLI,CPID,INSTRUMENT,BOOKTYPE,BRANCH_ID,TMARK,SCHEME,          
DUMMY1=(CASE WHEN SETT_TYPE = 'L' AND SELL_BUY = 1           
    THEN SCHEME + DUMMY1           
    ELSE DUMMY1           
  END),          
DUMMY2              
...
AND TRDTYPE = 'N'                    
GROUP BY SETT_NO,SETT_TYPE,BRANCH_CD,PARTY_CODE,SELL_BUY                    
...
BEGIN                    
 ...
 IF @ACNAME = 'BROKERAGE REALISED'                  SELECT @BRKRELPARTY = @ACCODE                    
...
INSERT INTO ACCBILL                      
SELECT PARTY_CODE=@CLRHSGPARTY,'0',                    
SELL_BUY = (CASE WHEN SUM(PRATE) >= SUM(SRATE)                     
     THEN 2                    
     ELSE 1                    
   END),                    
SETT_NO,SETT_TYPE,@START_DATE,@END_DATE,@SEC_PAYIN,@SEC_PAYOUT,                    
AMOUNT = ROUND(ABS(SUM(PRATE) - SUM(SRATE)),2),                    
BRANCH_CD,'BILL POSTED'                    
FROM NSEBILLVALAN WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE                    
AND TRDTYPE = 'N'                    
GROUP BY SETT_NO,SETT_TYPE,BRANCH_CD,SELL_BUY                    
HAVING ABS(SUM(PAMT) - SUM(SAMT)) > 0          
...
/*   SELECT @TBRKREVERSED =  @TBRKREVERSED + ABS(@TBROKERAGE)*/ /* COMMENTERD BY ANIMESH FOR -VE BROKERAGE */                    
 ...
 INSERT INTO ACCBILL VALUES ( @EDUCESSTAXPARTY,0,'2',@SETT_NO,@SETT_TYPE,@START_DATE,@END_DATE,@SEC_PAYIN,@SEC_PAYOUT,                    
         ROUND(@TEDUCESS_TAX,2),@DUMMYBRANCH_CD,'BILL POSTED')                    
 ...
 INSERT INTO ACCBILL VALUES ( @EDUCESSTAXPAYPARTY,0,'1',@SETT_NO,@SETT_TYPE,@START_DATE,@END_DATE,@SEC_PAYIN,@SEC_PAYOUT,                    
         ROUND(@TEXEDUCESS_TAX,2),@DUMMYBRANCH_CD,'BILL POSTED')                    
...
INSERT INTO ACCBILL VALUES ( @EDUCESSTAXPARTY,0,'2',@SETT_NO,@SETT_TYPE,@START_DATE,@END_DATE,@SEC_PAYIN,@SEC_PAYOUT,                    
       ROUND(@NETEDUCESS_TAX,2),'ZZZ','BILL POSTED')                    
...
INSERT INTO ACCBILL VALUES ( @EDUCESSTAXPAYPARTY,0,'1',@SETT_NO,@SETT_TYPE,@START_DATE,@END_DATE,@SEC_PAYIN,@SEC_PAYOUT,                    
       ROUND(@NETEXEDUCESS_TAX,2),'ZZZ','BILL POSTED')                    
...
SELECT AMOUNT = ISNULL(SUM(AMOUNT),0),SELL_BUY FROM ACCBILL A,ACCOUNTSLBS.DBO.ACMAST M                     
WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE                     
...
/*   SELECT @TBRKREVERSED =  @TBRKREVERSED + ABS(@TBROKERAGE)*/ /* COMMENTERD BY ANIMESH FOR -VE BROKER*/                    
 ...
 IF ROUND(@TEDUCESS_TAX,2) <> 0                  
 INSERT INTO IACCBILL VALUES ( @EDUCESSTAXPARTY,0,'2',@SETT_NO,@SETT_TYPE,@START_DATE,@END_DATE,@SEC_PAYIN,@SEC_PAYOUT,                    
         ROUND(@TEDUCESS_TAX,2),@DUMMYBRANCH_CD,'BILL POSTED')                    
 ...
 IF ROUND(@TEXEDUCESS_TAX,2) <> 0                  
 INSERT INTO IACCBILL VALUES ( @EDUCESSTAXPAYPARTY,0,'1',@SETT_NO,@SETT_TYPE,@START_DATE,@END_DATE,@SEC_PAYIN,@SEC_PAYOUT,                    
         ROUND(@TEXEDUCESS_TAX,2),@DUMMYBRANCH_CD,'BILL POSTED')                    
...
SELECT AMOUNT = ISNULL(SUM(AMOUNT),0),SELL_BUY FROM IACCBILL A,ACCOUNTSLBS.DBO.ACMAST M                     
WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE                     
...
EXEC NSESLBS_MARGIN_VALAN @START_DATE

Exec VALAN_GST_BILLPOST @SETT_NO,@SETT_TYPE
