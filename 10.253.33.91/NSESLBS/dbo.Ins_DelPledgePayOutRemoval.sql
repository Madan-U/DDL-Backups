-- Object: PROCEDURE dbo.Ins_DelPledgePayOutRemoval
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

CREATE PROC Ins_DelPledgePayOutRemoval As  
Declare   
@Sett_No Varchar(7),  
@Sett_Type Varchar(2),  
...
Select Sett_No,Sett_Type,Party_Code,Scrip_Cd,Series,IsIn,BDpId,BCltDpId,PledgeQty From DelPledgePayOut  
Where PledgeQty > 0   
Order By Sett_No,Sett_Type,Party_Code  
Open @PldCur   
Fetch Next From @PldCur Into @Sett_No,@Sett_Type,@Party_Code,@Scrip_Cd,@Series,@IsIn,@BDpId,@BCltDpId,@PldQty  
While @@FETCH_STATUS = 0   
Begin  
 Set @DelPldCur = Cursor For  
 Select Sno, Qty From DelTrans  
 Where Sett_No = @Sett_No And Sett_Type = @Sett_Type  
 And Party_Code = @Party_Code And Scrip_Cd = @Scrip_Cd  
 ...
 Begin  
  ...
  Begin  
   ...
   Update DelTrans Set TrType = 904, Reason = 'DEMAT', Qty = @PldQty  
   Where Sno = @Sno  
   Set @PldQty = 0  
  End  
  Fetch Next From @DelPldCur Into @Sno, @HoldQty      
...
TRUNCATE TABLE DelPledgePayOut
