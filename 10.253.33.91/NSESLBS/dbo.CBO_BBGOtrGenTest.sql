-- Object: PROCEDURE dbo.CBO_BBGOtrGenTest
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

-- sp_helptext CBO_BBGOtrGen
--exec CBO_BBGOtrGenTest'16/02/2006'
CREATE Procedure CBO_BBGOtrGenTest      
...
Begin      
  ...
  Set @Sauda_date = Convert(CHAR(11), Convert(DateTime, @Sauda_date, 103), 109)  
End      
...
--	 Set @Sauda_date = Convert(CHAR(11), Convert(DateTime, @Sauda_date, 103), 109)      
     ...
     FROM     
       ...
       And F.CP_Code = T.CP_Code And sauda_date like  + @Sauda_date + '%'    
     GROUP BY     
       F.Rec_Type,     
       F.Sell_BuyFlag,     
       F.Sett_Type,     
     ...
     WHERE sauda_date LIKE + @Sauda_date + '%' AND ToExchange.cp_code = 'INST'     
       AND     
       S.Rec_type = 10     
