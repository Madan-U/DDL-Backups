-- Object: PROCEDURE dbo.BBGInsSettBrokReCal
...
SELECT * INTO #Client2 FROM Client2 WHERE PARTY_CODE = @PARTY_CODE
SELECT C1.* INTO #client1 FROM Client1 C1, #client2 C2 
WHERE C1.CL_CODE = C2.CL_CODE AND C2.PARTY_CODE = @PARTY_CODE
SELECT * INTO #ClientBrok_Scheme FROM ClientBrok_Scheme WHERE PARTY_CODE = @PARTY_CODE
AND @SAUDA_DATE BETWEEN FROM_DATE AND TO_DATE
SELECT * INTO #ClientTaxes_New FROM ClientTaxes_New WHERE PARTY_CODE = @PARTY_CODE
...
Select distinct Party_code,Scrip_cd,Series,PartipantCode From Isettlement
Where
Sett_no = @sett_no
And
Sett_type = @Sett_type
...
*/
 ...
 when ( isettlement.SettFlag = 5  and broktable.val_perc ='P' )
   ...
   Else  0
                     ...
                     END) End    )/*  /  ( CASE WHEN CLIENT2.Service_chrg =  0 THEN 100 ELSE (100 + Globals.service_tax)  END )*/,
...
(  case
  ...
  And Isettlement.PartipantCode = @@PartipantCode
...
begin
	...
	(
		...
		'BBGInsSettBrokReCal',	/*called_from*/
		getdate(),	/*timestamp*/
