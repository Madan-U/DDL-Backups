-- Object: PROCEDURE dbo.FASTTRADE_DepositDetails_FO
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

Create Procedure FASTTRADE_DepositDetails_FO (@TradeDate Varchar(11)) AS
...
If @DataBaseName <> ''
Begin
	Select @@opendate = Max(vdt) From AccountFo.dbo.Ledger (nolock) Where Vdt < @TradeDate
	if @@opendate <> ''
		Begin
...
If @DataBaseName <> ''
Begin
	Select @@opendate = Max(vdt) From AccountNCDX.dbo.Ledger (nolock) Where Vdt < @TradeDate
	if @@opendate <> ''
		Begin
			Insert Into #LedgerBalance
			Select
...
If @DataBaseName <> ''
Begin
	Select @@opendate = Max(vdt) From AccountMCDX.dbo.Ledger (nolock) Where Vdt < @TradeDate
	if @@opendate <> ''
		Begin
			Insert Into #LedgerBalance
			Select
