-- Object: PROCEDURE dbo.ContCumBill_Section_NSECM_OLD
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

CREATE Proc ContCumBill_Section_NSECM_OLD
...
Into #Settlement From Settlement with(index(stt_index))              
...
select Contractno,Billno,Trade_No,Party_Code,Scrip_Cd,User_Id,Tradeqty,Auctionpart,s.Markettype,S.Series,Order_No,Marketrate,Sauda_Date,Table_No,Line_No,Val_Perc,Normal,Day_Puc,Day_Sales,Sett_Purch,Sett_Sales,Sell_Buy,Settflag,Brokapplied,Netrate,Amount,Ins_Chrg,Turn_Tax,Other_Chrg,Sebi_Tax,Broker_Chrg,Service_Tax,Trade_Amount,Billflag,S.Sett_No,Nbrokapp,Nsertax,N_Netrate,S.Sett_Type,Partipantcode,Status,Pro_Cli,Cpid,Instrument,Booktype,Branch_Id,Tmark,Scheme,Dummy1,Dummy2 
from Settlement S with(index(stt_index))    , Sett_Mst M          
Where S.Sett_No = M.Sett_No And S.Sett_Type = M.Sett_Type              
...
/*To display the header part*/      Partyname = c1.Long_name,      
...
billflag      
Into #ContSett      
From #settlement S left outer join  ucc_client uc on s.party_code = uc.party_code,      
...
l_city,l_zip,service_chrg,Branch_cd,sub_broker,pan_gir_no,Off_Phone1,Off_Phone2,Printf,mapidid, OrderFlag,ScripName1 , billflag      
...
order by Branch_cd,Sub_Broker,Party_code,Partyname,ScripName1,OrderFlag,Sett_No, Sett_Type,  Order_No, Trade_No
