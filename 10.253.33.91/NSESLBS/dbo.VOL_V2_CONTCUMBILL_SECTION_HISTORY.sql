-- Object: PROCEDURE dbo.VOL_V2_CONTCUMBILL_SECTION_HISTORY
...
CREATE    PROC VOL_V2_CONTCUMBILL_SECTION_HISTORY
(
	@STATUSID VARCHAR(15),
...
UPDATE   
 #CONTSETT  
SET  
 Brokerage = Case When Sell_Buy =1 then CD_TrdBuyBrokerage+CD_DelBuyBrokerage Else CD_TrdSellBrokerage+CD_DelSellBrokerage End  
      + (Case When Service_Chrg = 1 Then   
   Case When Sell_Buy =1 then CD_TrdBuySerTax+CD_DelBuySerTax Else CD_TrdSellSerTax+CD_DelSellSerTax end  
        Else 0 End),  
 ...
 NSerTax = (Case When Service_Chrg = 0 Then   
   Case When Sell_Buy =1 then CD_TrdBuySerTax+CD_DelBuySerTax Else CD_TrdSellSerTax+CD_DelSellSerTax end  
   Else 0 End),  
        pAmt=(CASE WHEN sell_buy = 1 THEN pAmt+ CD_TrdBuyBrokerage+CD_DelBuyBrokerage   
  + (Case When Service_Chrg = 1 Then isnull((CD_TrdBuySerTax+CD_DelBuySerTax),0) Else 0 End)  
              ELSE 0 END),   
        SAmt=(CASE WHEN sell_buy = 2 THEN SAmt - CD_TrdSellBrokerage-CD_DelSellBrokerage   
  - (Case When Service_Chrg = 1 Then isnull((CD_TrdSellSerTax+CD_DelSellSerTax),0) Else 0 End)  
              ELSE 0 END)     
FROM  
 CHARGES_DETAIL  
WHERE  
...
INSERT         INTO    #CONTSETT   
 SELECT  CONTRACTNO='0',   
                ...
                CASE   
                        ...
                        THEN 'BF-'   
                        ELSE '' END ) + ISNULL(S2.SHORT_NAME,'CONT BROK') + '   ' ,   
                SDT = CONVERT(VARCHAR,@SDT,103),   
                SELL_BUY=1,   
                BROKER_CHRG =0,   
                TURN_TAX =0,   
  ...
  UCC_CODE,  
                ORDERFLAG  = 0,   
  SCRIPNAMEForOrderBy=ISNULL(S2.SHORT_NAME,'CONT BROK'),
                SCRIPNAME1 = Convert(Varchar,(Case When Cl_Type = 'NRI'   
       Then LTRIM(RTRIM(ISNULL(S2.SHORT_NAME,'CONT BROK'))) + '_' + LTRIM(RTRIM(1))   
              Else ''   
         End), 52),         
  	SEBI_NO,  
  	Participant_Code,  
