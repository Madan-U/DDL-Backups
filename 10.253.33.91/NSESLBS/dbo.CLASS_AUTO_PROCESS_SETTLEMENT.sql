-- Object: PROCEDURE dbo.CLASS_AUTO_PROCESS_SETTLEMENT
 ...
 BEGIN                  
  ...
  EXEC (@CMD)                  
    ...
    BEGIN    
	 ...
	 SET @FILE_DWLOAD_PWD = REPLACE(@FILE_DWLOAD_PWD,'&','^&')
	 ...
	 IF @FILE_DWLOAD_NAME LIKE '%*%'
	 BEGIN
		SET @FILE_DWLOAD_NAME_TEMP = '14665_SOTD_M_P_FNL_18012016.GZ' + '_1'
		...
		SELECT @FILE_DWLOAD_NAME = LTRIM(RTRIM(REVERSE(DBO.PIECE(REVERSE(FILENAMELIST),'#',1)))) FROM #FILELIST
		WHERE FILENAMELIST LIKE '%' + REPLACE(@FILE_DWLOAD_NAME,'*',LEFT(REPLACE(CONVERT(VARCHAR,CONVERT(DATETIME,@BUSINESS_DATE),103),'/',''),4)) + '%'
		
		SELECT @FILE_DWLOAD_NAME=LEFT(@FILE_DWLOAD_NAME,LEN(@FILE_DWLOAD_NAME)-1)

		SET @PROCESS_SP_NAME = REPLACE(@PROCESS_SP_NAME,@PROCESS_FILE_NAME,REPLACE(@FILE_DWLOAD_NAME,'.GZ',''))
		SET @PROCESS_FILE_NAME = REPLACE(@FILE_DWLOAD_NAME,'.GZ','')
	 END
