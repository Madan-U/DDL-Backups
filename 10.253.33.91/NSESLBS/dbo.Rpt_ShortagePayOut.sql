-- Object: PROCEDURE dbo.Rpt_ShortagePayOut
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------

CREATE Proc Rpt_ShortagePayOut (@StatusId Varchar(15), @StatusName Varchar(25),@FromSett_no Varchar(7),@ToSett_no Varchar(7), @sett_Type Varchar(2), @Flag Int) As         
...
BuyShortage = /*(Case When BuyQty - Sum(Case When DrCr = 'D' Then isnull(De.qty,0) Else 0 End) > 0         
 ...
 + (Case When (SellQty - Sum(Case When DrCr = 'C' Then isnull(De.qty,0) Else 0 End)) < 0 Then         
  abs(SellQty - Sum(Case When DrCr = 'C' Then isnull(De.qty,0) Else 0 End)) Else 0 End),*/  
(Case When Sum(Case When DrCr = 'C' Then isnull(De.qty,0) Else 0 End) > SellQty Then
