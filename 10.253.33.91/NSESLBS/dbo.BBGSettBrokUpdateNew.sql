-- Object: PROCEDURE dbo.BBGSettBrokUpdateNew
...
where Sett_type = @Sett_type and Start_Date >= @Tdate 
and End_date <= @Tdate + ' 23:59:59'
...
Where Cl_Code = @tparty

Select * Into #Client2 From Client2
Where Cl_Code = @tparty

Select * Into #ClientTaxes_New From ClientTaxes_New
Where Party_Code = @tparty And @Tdate Between FromDate and todate

Select * Into #ClientBrok_Scheme From ClientBrok_Scheme
Where Party_Code = @tparty
And Scheme_Type = 'TRD'
And @Tdate Between From_Date and to_date

Select * Into #Broktable From BrokTable
...
(  case
   ...
   FROM #BrokTable BrokTable, #Client2 Client2,Settlement ,globals, #client1 Client1, #ClientBrok_Scheme S, #ClientTaxes_New CT, Owner
   ...
   And Broktable.Line_no = ( case when S.BrokScheme = 2 then
							    (Select min(Broktable.line_no) from #BrokTable broktable where
							    ...
							    and Settlement.Party_Code =  Client2.Party_code
					       		   ...
					       		   (Select min(Broktable.line_no) from #BrokTable broktable where
							    	   ...
							    	   (Select min(Broktable.line_no) from #BrokTable broktable
  ...
  and RTrim(Settlement.party_code ) = @tparty
...
Begin
	...
	begin
		...
		(
			...
			'BBGSettBrokUpdateNew',	/*called_from*/
			getdate(),	/*timestamp*/
