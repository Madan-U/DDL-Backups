...
MarketRate=Round((Sum(TradeQty*N_NetRate)+Sum(Case When Service_Chrg = 1 Then NSerTAx Else 0 End))/Sum(TradeQty),4),          
...
Broker_Chrg = Sum(Case When BrokerNote = 1 Then Broker_chrg Else 0 End),Remark = @Remark /*Sum(Case When BrokerNote = 1 Then Broker_chrg Else 0 End)*/,          
Penalty = Sum(Case When AuctionPart Like 'F%' Then A.Other_Chrg Else 0 End),AuctionPart,PenaltyPer=AuctionChrg
