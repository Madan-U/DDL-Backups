...
CREATE Proc InsNseDelAllocate (@Sett_No Varchar(7),@Sett_Type Varchar(2),@RefNo int,@ScripCd Varchar(12),@Series Varchar(3),@EntityCode Varchar(10),@DelQty int,@CQty int,@TrType int,@DelFlag Varchar(3)) As    
Declare @@Qty int,    
 @@CertNo Varchar(15),    
 @@FromNo Varchar(15),    
 @@FolioNo Varchar(15),    
...
Begin    
 Set @@DelMov = Cursor For     
  ...
  and DrCr = 'D' and Filler2 = 1 And DelTrans.BDpType Like (Case When @DelFlag = 'PHY' Then '%'     
         When AllocationType = 0 Then '%' Else 'CDSL' End)       
And TCode not in ( Select TCode From DelTrans    
  where sett_no = @sett_no    
         and sett_type = @sett_type     
  and DelTrans.RefNo = @RefNo    
  and TrType = 906    
  ...
  order by DelTrans.BDpType Asc,TransDate , foliono , certno, qty desc     
End    
Else    
Begin    
 Set @@DelMov = Cursor For     
...
End    
 ...
 select @@Qty,@@CertNo,@@FromNo,@@FolioNo,@@Date,@@OrgQty,@@SNo,@@TCode,@DelQty    
 if @@Fetch_Status = 0     
 Begin    
  Select @@PCount = 0      
