-- Object: PROCEDURE dbo.Proc_newbasicreport_1
-- Server: 10.253.33.91 | DB: NSESLBS
--------------------------------------------------


--EXEC Proc_Newbasicreport_1 '', '', '%', 'Jan 21 2006 ', 'Jan 21 2008 ', '', '', '', '', 'product', 'S', '0', '1', '1', '', '', 'broker', 'broker'
...
CREATE   Procedure [dbo].[Proc_newbasicreport_1] ( 
...
@PRODUCTCODE VARCHAR(10),        ---16 USED FOR PRODUCT CODE
@statusid Varchar(15),
...
@@fromproduct Varchar(15),
@@toproduct Varchar(15),
@@fromparent Varchar(15),
@@toparent Varchar(15),
...
If (@consol = "product")  And  ((@from <> "") And (@to <> "" ) ) 
Begin
     Select @@toparty_code =  Max(party_code) ,@@fromparty_code = Min(party_code) From Details 
     Select @@fromproduct = @from 
     Select @@toproduct = @to 	
End
Else If (@consol = "product")  And  ((@from = "") And (@to = "" ) ) 
Begin
     Select @@toparty_code =  Max(party_code) ,@@fromparty_code = Min(party_code) From Details 
     Select @@fromproduct = Min(ProductCode), @@toproduct=max(ProductCode) From Cmbillvalan
End

If (@consol = "parent")  And  ((@from <> "") And (@to <> "" ) ) 
Begin
     Select @@toparty_code =  Max(party_code) ,@@fromparty_code = Min(party_code) From Details 
     Select @@fromparent = @from 
     Select @@toparent = @to 	
End
Else If (@consol = "parent")  And  ((@from = "") And (@to = "" ) ) 
Begin
     Select @@toparty_code =  Max(party_code) ,@@fromparty_code = Min(party_code) From Details 
     Select @@fromparent = Min(ParentCode), @@toparent = Max(ParentCode) From Cmbillvalan
End
...
Begin
	  ...
	  If @consol = "area"
          ...
          If @consol = "product"
             Set  @@myorder = " Order  By S.ProductCode Option (fast 1 ) "
          If @consol = "parent"
             Set  @@myorder = " Order  By S.ParentCode Option (fast 1 ) "
End
...
Begin
          ...
          If @consol = "product"
             Set  @@myorder = " Order By S.scrip_cd, S.series, S.ProductCode Option (fast 1 ) "
          If @consol = "parent"
             Set  @@myorder = " Order By S.scrip_cd, S.series, S.ParentCode Option (fast 1 ) "
End
...
Begin
          ...
          If @consol = "product"
             Set  @@myorder = " Order  By S.ProductCode, S.sett_no,s.sett_type , S.scrip_cd, S.series Option (fast 1 ) "
          If @consol = "parent"
             Set  @@myorder = " Order  By S.ParentCode, S.sett_no,s.sett_type , S.scrip_cd, S.series Option (fast 1 ) "
End
...
Begin
          ...
          If @consol = "product"
             Set  @@myorder = " Order  By S.ProductCode, S.party_code Option (fast 1 ) "
          If @consol = "parent"
             Set  @@myorder = " Order  By S.ParentCode, S.party_code Option (fast 1 ) "
End
...
Begin
          ...
          If @consol = "product"
             Set  @@myorder = " Order  By S.ProductCode, S.sauda_date ,s.sett_no,s.sett_type , S.scrip_cd, S.series Option (fast 1 ) "
          If @consol = "parent"
             Set  @@myorder = " Order  By S.ParentCode, S.sauda_date ,s.sett_no,s.sett_type , S.scrip_cd, S.series Option (fast 1 ) "
End
...
Begin
          ...
          If @consol = "product"
             Set  @@myorder = " Order  By S.ProductCode, S.party_code, S.scrip_cd, S.series Option (fast 1 ) "
          If @consol = "parent"
             Set  @@myorder = " Order  By S.ParentCode, S.party_code, S.scrip_cd, S.series Option (fast 1 ) "
End
...
Begin
          ...
          If @consol = "product"
             Set  @@myorder = " Order  By S.ProductCode, S.sett_no,s.sett_type , S.scrip_cd,s.series ,s.sauda_date Option (fast 1 ) "
          If @consol = "parent"
             Set  @@myorder = " Order  By S.ParentCode, S.sett_no,s.sett_type , S.scrip_cd,s.series ,s.sauda_date Option (fast 1 ) "
End
...
Begin
     ...
     If @consol = "branch_cd"
     Begin 
          Set  @@mygroup = " Group By Branch_cd, Branch, Clienttype, Membertype, Companyname"
     ...
     If @consol = "sub_broker"
     Begin 
          Set  @@mygroup = " Group By S.Sub_broker, Name, membertype ,clienttype, Companyname "                    
     ...
     If @consol = "product"
     Begin 
          Set @@mygroup = " Group By ProductCode, Membertype, Companyname"
          Set @@selectflex = " Select ProductCode, Long_name=ProductCode, ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel), Buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd) , Buydeliverychrg = Sum(pbrokdel), Selldeliverychrg = Sum(sbrokdel), Clienttype = '', Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt= Sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
     If @consol = "parent"
     Begin 
          Set  @@mygroup = " Group By S.ParentCode, Name, membertype ,clienttype, Companyname "                    
          Set @@selectflex =  " Select S.ParentCode, long_name=Name, ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd) ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) , Clienttype, Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Trdamt= Sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
End
...
Begin
     ...
     If @consol = "product"
     Begin 
          Set  @@mygroup = " Group By Scrip_cd, Series, Scrip_name, ProductCode ,membertype ,clienttype, Companyname "                    
          Set @@selectflex =  " Select Scrip_cd, Series, Scrip_name, ProductCode,long_name=ProductCode,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd) ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) , Clienttype, Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Trdamt= Sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
     If @consol = "parent"
     Begin 
          Set  @@mygroup = " Group By Scrip_cd, Series, Scrip_name, ParentCode ,membertype ,clienttype, Companyname "                    
          Set @@selectflex =  " Select Scrip_cd, Series, Scrip_name, ParentCode,long_name=ParentCode,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd) ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) , Clienttype, Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Trdamt= Sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
End
...
Begin
     ...
     If @consol = "product"
     Begin
          Set  @@mygroup = " Group By Sett_no, Sett_type,ProductCode,party_code,party_name,membertype, Companyname,scrip_cd,series,scrip_name "
          Set  @@selectflex =  " Select Sett_no, Sett_type,ProductCode,party_code,party_name,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
     If @consol = "parent"
     Begin
          Set  @@mygroup = " Group By Sett_no, Sett_type,ParentCode,party_code,party_name,membertype, Companyname,scrip_cd,series,scrip_name "
          Set  @@selectflex =  " Select Sett_no, Sett_type,ParentCode,party_code,party_name,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
End
...
Begin
     ...
     If @consol = "product"
     Begin
          Set  @@mygroup = " Group By ProductCode,party_code,party_name,clienttype,membertype, Companyname  "
          Set  @@selectflex =  " Select ProductCode,party_code,party_name,clienttype,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
     If @consol = "parent"
     Begin
          Set  @@mygroup = " Group By ParentCode,party_code,party_name,clienttype,membertype, Companyname  "
          Set  @@selectflex =  " Select ParentCode,party_code,party_name,clienttype,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
End
...
Begin
     ...
     If @consol = "product"
     Begin
          Set  @@mygroup = " Group By Sett_no, Sett_type,sauda_date,left(convert(varchar,sauda_date,109),11),sauda_date,left(convert(varchar,sauda_date,109),11),ProductCode,party_code,party_name,membertype, Companyname,scrip_cd,series,scrip_name "
          Set  @@selectflex =  " Select Sett_no, Sett_type,trade_date = S.sauda_date,sauda_date = Left(convert(varchar,sauda_date,109),11),ProductCode,party_code,party_name,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
     If @consol = "parent"
     Begin
          Set  @@mygroup = " Group By Sett_no, Sett_type,sauda_date,left(convert(varchar,sauda_date,109),11),sauda_date,left(convert(varchar,sauda_date,109),11),ParentCode,party_code,party_name,membertype, Companyname,scrip_cd,series,scrip_name "
          Set  @@selectflex =  " Select Sett_no, Sett_type,trade_date = S.sauda_date,sauda_date = Left(convert(varchar,sauda_date,109),11),ParentCode,party_code,party_name,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
End
...
Begin     If @consol = "region"
     ...
     If @consol = "product"
     Begin
          Set  @@mygroup = " Group By S.ProductCode,S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BR.Branch,SB.Name,party_code,party_name,membertype, Companyname,scrip_cd,series,scrip_name "
          Set  @@selectflex =  " Select S.ProductCode,S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BranchName=BR.Branch,SubBrokerName = SB.Name,party_code,party_name,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
     If @consol = "parent"
     Begin
          Set  @@mygroup = " Group By S.ParentCode,S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BR.Branch,SB.Name,party_code,party_name,membertype, Companyname,scrip_cd,series,scrip_name "
          Set  @@selectflex =  " Select S.ParentCode,S.Branch_Cd,S.Sub_Broker,S.Trader,S.Family,BranchName=BR.Branch,SubBrokerName = SB.Name,party_code,party_name,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd)  ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) ,  Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ), Trdamt=sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname,pnl = Sum(samttrd-pamttrd) " 
     End
End
...
Begin
     ...
     Begin 
          Set  @@mygroup = " Group By Sett_no, Sett_type,sauda_date,left(convert(varchar,sauda_date,109),11),sub_broker,clienttype,membertype, Companyname,scrip_cd,series,scrip_name "
          Set  @@selectflex =  "select Sett_no, Sett_type,trade_date = S.sauda_date,sauda_date = Left(convert(varchar,sauda_date,109),11),ProductCode,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd) ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) , Clienttype, Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Trdamt= Sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname ,pnl = Sum(samttrd-pamttrd)" 
     End  
     If @consol = "parent"
     Begin 
          Set  @@mygroup = " Group By Sett_no, Sett_type,sauda_date,left(convert(varchar,sauda_date,109),11),sub_broker,clienttype,membertype, Companyname,scrip_cd,series,scrip_name "
          Set  @@selectflex =  "select Sett_no, Sett_type,trade_date = S.sauda_date,sauda_date = Left(convert(varchar,sauda_date,109),11),ParentCode,scrip_cd,series,scrip_name,ptradedqty = Sum(pqtytrd + Pqtydel) ,ptradedamt = Sum(pamttrd + Pamtdel) ,stradedqty = Sum(sqtytrd + Sqtydel), Stradedamt = Sum(samttrd + Samtdel),buybrokerage = Sum(pbroktrd) , Selbrokerage= Sum(sbroktrd) ,buydeliverychrg = Sum(pbrokdel) ,selldeliverychrg = Sum(sbrokdel) , Clienttype, Billpamt = Sum(pamt) , Billsamt = Sum(samt) , Pmarketrate = ( Sum(prate) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Smarketrate = ( Sum(srate) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),   Pnetrate = ( Sum(pamttrd + Pamtdel) / Case When Sum(pqtytrd + Pqtydel) > 0 Then Sum(pqtytrd + Pqtydel) Else 1 End) , Snetrate = ( Sum(samttrd+samtdel) / (case When Sum(sqtytrd + Sqtydel) > 0 Then Sum(sqtytrd + Sqtydel) Else 1 End ) ),  Trdamt= Sum(trdamt) ,delamt=sum(delamt), Serinex=sum(serinex),service_tax= Sum(service_tax) ,exservice_tax= Sum(exservice_tax),turn_tax=sum(turn_tax),sebi_tax=sum(sebi_tax),ins_chrg=sum(ins_chrg),broker_chrg=sum(broker_chrg),other_chrg=sum(other_chrg), Membertype, Companyname ,pnl = Sum(samttrd-pamttrd)" 
     End  
End
...
If @groupf = "3.3"
Begin
	Set @@wheretext =  " Where /*S.Branch_CD = BR.Branch_Code And S.sub_broker= SB.Sub_Broker And*/ S.sauda_date Between '" + @sauda_date + " 00:00:00' And '" + @todate + " 23:59:00'"
End
ELse
Begin
	Set @@wheretext =  " Where S.sauda_date Between '" + @sauda_date + " 00:00:00' And '" + @todate + " 23:59:00'"
End
Set @@wheretext =  @@wheretext + "  And S.scrip_cd Between '" + @fromscrip + "' And '"+  @toscrip +"' /*and S.sett_no Between '" + @sett_no + "' And '" + @tosett_no +"' */ "  

If (Upper(@use1) <> 'ALL' OR Upper(@use1) <> '') 
Begin
   If Len(@use1) > 0 
...
--Set @@wheretext =  @@wheretext + " And PRODUCTCODE = '"+ ISNULL(@PRODUCTCODE,PRODUCTCODE) +"' "
Set @@wheretext =  @@wheretext + " And PRODUCTCODE Like '"+ @PRODUCTCODE +"%' "
...
If @consol = "product" 
Set @@wheretext =  @@wheretext + " And  ProductCode Between '" + @@fromproduct + "' And '" + @@toproduct +"' " 

If @consol = "parent" 
Set @@wheretext =  @@wheretext + " And  ParentCode Between '" + @@fromparent  + "' And '" + @@toparent  +"' " 
...
If @statusid = "product" 
Begin
	Set @@wheretext =  @@wheretext +   "  And ProductCode Between '" + @statusname  + "' And '" + @statusname   +"'  " 
End

If @statusid = "parent" 
Begin
	Set @@wheretext =  @@wheretext +   "  And ParentCode Between '" + @statusname  + "' And '" + @statusname   +"'  " 
End
