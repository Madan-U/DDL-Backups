-- Object: PROCEDURE dbo.PROC_POPULATE_TBL_MTF_DATA_PRE_BKUP_15DEC2025_SRE-42649
-- Server: 10.253.33.91 | DB: MTFTRADE
--------------------------------------------------

CREATE PROC [dbo].[PROC_POPULATE_TBL_MTF_DATA_PRE_BKUP_15DEC2025_SRE-42649] (@SAUDA_DATE DATETIME)
AS
    ...
    -- Extract and aggregate MTF positions
		...
		--Select top 1 * FROM AngelBSECM.BSEDB_AB.DBO.VARDETAIL
		--		Select top 1 * FROM MSAJAG.DBO.VARDETAIL
        ...
        SET MTOM_LOSS = (
            CASE
                WHEN NETAMT > TDAY_CLOSING_PRICE * QTY
                THEN NETAMT - (TDAY_CLOSING_PRICE * QTY)
                ELSE 0
            END
        )
        WHERE SAUDA_DATE = @SAUDA_DATE;
