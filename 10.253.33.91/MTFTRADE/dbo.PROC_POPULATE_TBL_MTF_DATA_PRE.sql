-- Object: PROCEDURE dbo.PROC_POPULATE_TBL_MTF_DATA_PRE
-- Server: 10.253.33.91 | DB: MTFTRADE
--------------------------------------------------

CREATE PROC [dbo].[PROC_POPULATE_TBL_MTF_DATA_PRE] (@SAUDA_DATE DATETIME)
AS
    ...
    -- Extract and aggregate MTF positions
        ...
        -- Load MTF_ANGEL_Varmargin data for haircut comparison
        SELECT ISIN, VARMARGIN AS HAIRCUT_OVERRIDE
        INTO #B_MTF_VARMARGIN
        FROM AngelNSECM.mtftrade.dbo.MTF_ANGEL_Varmargin
        WHERE @SAUDA_DATE BETWEEN from_date AND end_date
            AND varmargin > 0
            AND ISIN IN (SELECT DISTINCT ISIN FROM TBL_MTF_DATA_PRE WHERE SAUDA_DATE = @SAUDA_DATE);

        CREATE CLUSTERED INDEX #MISIN ON #B_MTF_VARMARGIN (ISIN);
        ...
        -- Apply MAX(calculated_haircut, MTF_ANGEL_Varmargin) for stocks with override
        UPDATE TBL_MTF_DATA_PRE
        SET HAIRCUT = M.HAIRCUT_OVERRIDE
        FROM #B_MTF_VARMARGIN M
        WHERE M.ISIN = TBL_MTF_DATA_PRE.ISIN
            AND TBL_MTF_DATA_PRE.SAUDA_DATE = @SAUDA_DATE
            AND M.HAIRCUT_OVERRIDE > TBL_MTF_DATA_PRE.HAIRCUT;

        -- Clean up temp tables
        DROP TABLE #B_NSE_VAR;
        DROP TABLE #B_BSE_VAR;
        DROP TABLE #B_MTF_VARMARGIN;
        ...
        -- Query NSE Commodity ledger for current settlement period
        INSERT INTO #B_LEDBAL
        SELECT L.CLTCODE,
               VDTLEDBAL = SUM(CASE
                                   WHEN DRCR = 'C' THEN VAMT
        ...
        -- Query NSE Commodity ledger for future-dated entries
        INSERT INTO #B_LEDBAL
        SELECT L.CLTCODE,
               VDTLEDBAL = SUM(CASE
                                   WHEN DRCR = 'D' THEN VAMT
