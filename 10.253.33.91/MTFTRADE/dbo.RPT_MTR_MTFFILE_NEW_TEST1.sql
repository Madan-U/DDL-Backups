-- Object: PROCEDURE dbo.RPT_MTR_MTFFILE_NEW_TEST1
...
-------------MODIFIED THIS SP UNDER SRE-40935---(08 OCT 2025)------------------------


CREATE PROC [dbo].[RPT_MTR_MTFFILE_NEW_TEST1]      
(      
...
M_BODQTY,
M_BODVALUE,
M_FRESHBQTY =MTR_CLI_QTY,
M_FRESHBUY = (CASE WHEN NETQTY = MTR_CLI_QTY THEN (BODVALUE + FRESHBUY - FRESHSELL) ELSE M_FRESHBUY END),
M_FRESHSQTY,
M_FRESHSELL,
M_NETQTY,
M_NETAMOUNT,
...
END
	...
	--UPDATE	#NSERPT SET #NSERPT.VARMARGIN = F.VARMARGIN
	--From MTF_ANGEL_Varmargin F
	--WHERE @SAUDA_DATE BETWEEN F.From_date AND F.end_date
	--AND #NSERPT.ISIN = F.ISIN 
	--AND #NSERPT.VARMARGIN < F.VARMARGIN
	...
	/** --- NEW CHANGES DONE ON 09/02/2026 FOR EXPOSURE QTY --- **/
	...
	SELECT	PARTY_CODE,
			...
			SISIN,
			sEXCHANGE,
			NEXPOSUREQTY = SUM(NEXPOSUREQTY)
	INTO	#MQTY
	FROM	TBL_MTR_MAINTMARGINQTY M (NOLOCK)
	WHERE	SAUDA_DATE = @SAUDA_DATE
			AND M.PARTY_CODE = (CASE WHEN @PARTY_CODE = '' THEN M.PARTY_CODE ELSE @PARTY_CODE END) 
	GROUP BY
			PARTY_CODE,
			SCRIP_CD,
			SERIES,
	...
	CREATE INDEX #MMQ ON #MQTY
	( PARTY_CODE,SCRIP_CD,SISIN,SEXCHANGE )


	UPDATE	#NSERPT 
	SET		EXPOSURE_QTY = CEILING(M.NEXPOSUREQTY)
	FROM	#MQTY M (NOLOCK)
	WHERE	M.PARTY_CODE = #NSERPT.PARTY_CODE
			AND M.SCRIP_CD = #NSERPT.SCRIP_CD
			AND M.SISIN = #NSERPT.ISIN
			AND M.sEXCHANGE = #NSERPT.EXCHANGE
	...
	SET		--CORP_QTY = ISNULL(P.QTY,0),
			...
			FRESHSQTY = (CASE WHEN FRESHSQTY-(#NSERPT.BODQTY) > 0 THEN FRESHSQTY-(#NSERPT.BODQTY) ELSE ABS(FRESHSQTY-(#NSERPT.BODQTY)) END), --- CHANGES dONE ON 02/12/2025 FOR CORP A. IN CASE oF SELL --
	...
	UPDATE	#NSERPT
	SET		NETQTY = BODQTY+FRESHBQTY-FRESHSQTY+CORP_QTY,      
...
SELECT 'MR', @BATCHNO, @SAUDA_DATE, GETDATE()    
...
--CONVERT(VARCHAR,CONVERT(NUMERIC(18,0),ROUND(PRADNYA.DBO.FNMIN(SUM(NETQTY-M_FRESHBQTY+CORP_QTY),SUM(CASE WHEN M_FRESHBQTY > 0 THEN (EXPOSURE_QTY) ELSE 0 END)),0))) 
CONVERT(VARCHAR,CONVERT(NUMERIC(36,0),ROUND(PRADNYA.DBO.FNMIN(SUM(NETQTY-M_FRESHBQTY),(SUM(CASE WHEN M_FRESHBQTY > 0 THEN (EXPOSURE_QTY) ELSE 0 END))),0)))
+ ',' + 'N,'+ RIGHT(EXCHANGE,3)
FROM #NSERPT    
...
SELECT	PARTY_CODE, 
		CASHCOLL_AMT = CONVERT(NUMERIC(18,4),0),
		CASHPAYIN_AMT = SUM(CASE WHEN NETQTY = M_FRESHBQTY THEN (BODVALUE + FRESHBUY - FRESHSELL) ELSE M_FRESHBUY END ),
		COLL_CASH_EQV_AMT = CONVERT(NUMERIC(18,4),0),
		COLL_CASH_NONEQV_AMT = CONVERT(NUMERIC(18,4),0)
...
SET		CASHCOLL_AMT = CASE WHEN CASHCOLL_AMT<0 THEN  CASHCOLL_AMT + ISNULL(ABS(#INT_LEDBAL.INTEREST_AMT),0) ELSE CASHCOLL_AMT END
FROM	#INT_LEDBAL
WHERE	#CASHCOLLBAL_NEW.PARTY_CODE = #INT_LEDBAL.PARTY_CODE

UPDATE #CASHCOLLBAL_NEW SET CASHCOLL_AMT = CASHPAYIN_AMT WHERE CASHCOLL_AMT < CASHPAYIN_AMT
*/
