-- Object: PROCEDURE dbo.VIRTUAL_MIS_LEDGER_CONVERTER_30042016
...
*/   
CREATE PROCEDURE [dbo].[VIRTUAL_MIS_LEDGER_CONVERTER_30042016] @FILENAME VARCHAR(1000),   
                                                     @UNAME    VARCHAR(50)   
AS   
...
--PRINT @@SQL 
 ...
 INSERT INTO #VIRTUAL_MIS   
    ...
    WHERE  B.CLTCODE = #VIRTUAL_MIS.CLTCODE   
           AND substring(B.ACCNO, patindex('%[^0]%',B.ACCNO),20)    = #VIRTUAL_MIS.ACCOUNTNO   
   ...
   INSERT INTO #VIRTUAL_MIS1   
    SELECT VOUCHERTYPE=CONVERT(VARCHAR(2), '2'),   
      ...
      VDATE,   
           ...
           BRANCHCODE,   
           REF_NO=CONVERT(VARCHAR(15), REF_NO),   
           CHEQUEMODE='C',   
...
@DATASERVER + CHAR(34) + "'"                                   
    ...
    WHERE   
      ...
      .DBO.PIECE(TRANSACTION_DETAILS1, '|', 1) NOT IN (SELECT   
      TXN_REF_NO   
                                                                            FROM   
