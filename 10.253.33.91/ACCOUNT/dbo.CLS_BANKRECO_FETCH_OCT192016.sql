-- Object: PROCEDURE dbo.CLS_BANKRECO_FETCH_OCT192016
...
--SET @AMOUNT = (CASE WHEN @AMOUNT = 0 THEN 999999999 ELSE @AMOUNT END)
	IF @FLAG ='' OR @FLAG ='L'  /****BOTH BANK STATEMENT AND BANK BOOK AS WELL AS FOR LEDGER ENTRY****/
	BEGIN
	
		SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED 
			SELECT /*TOP 15*/ 
				ROW_NUMBER() OVER (ORDER BY REFERENCENO,VALUEDATE,DESCRIPTION,AMOUNT,DRCR,CROSSREFERENCENO,BR_SNO,BOOKDATE) AS "RNO",	
				REFERENCENO
				...
				,MATCH_FLAG =''
				,COLOR='' FROM BANKRECO WITH (NOLOCK) 
			WHERE 
				--MICRNO = '123456' AND 
				--DUMMY1 = '0' and	
				 CLTCODE = @BANKCODE 	
	...
	BEGIN
		 SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
            SELECT
				ROW_NUMBER() OVER (ORDER BY REFERENCENO,VALUEDATE,DESCRIPTION,AMOUNT,B.DRCR,CROSSREFERENCENO,BR_SNO) AS "RNO",
				REFERENCENO,
				VALUEDATE = CONVERT(VARCHAR(11),
				CONVERT(DATETIME,VALUEDATE),103),
            ...
            FROM
				BANKRECO B WITH (NOLOCK) 
            WHERE 
				CROSSREFERENCENO = (CASE WHEN @REFNO = '' THEN CROSSREFERENCENO ELSE @REFNO END)
				--AND DUMMY1 = '0' 
				...
				AND AMOUNT =  CASE WHEN @AMOUNT = 0 THEN AMOUNT ELSE @AMOUNT END 
				AND Valuedate  BETWEEN CONVERT(VARCHAR(11),CONVERT(DATETIME,@DATE_FROM,103),109) AND CONVERT(VARCHAR(11),CONVERT(DATETIME,@DATE_TO,103),109)
