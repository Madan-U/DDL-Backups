-- Object: PROCEDURE dbo.SCFM_PRATHAM_CR_NBFC
...
--SCFM_PRATHAM_CR_NBFC '2021-01-29','CREDITORS'                            
                              
                              
                        
 CREATE  proc [dbo].[SCFM_PRATHAM_CR_NBFC]                     
  (                                  
...
[NETAMT]  ASC
 ...
 select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)                                        
 into #NBFC                       
 from ABCSONBFC.accountNBFC.dbo.ledger where Vdt >=@FROMDATE                                    
 and edt <=@todate +' 23:59'                  
 and cltcode between 'a' and 'zzzz9999'                               
 group by cltcode    
   CREATE NONCLUSTERED INDEX [IX_CLTCODE] ON [dbo].[#NBFC]
...
UNION ALL                                
SELECT CLTCODE,SUM(CASE WHEN L.DRCR ='C' THEN VAMT*-1 ELSE VAMT END) AS BALANCE ,'NBFC' AS EXCHANGE FROM ABCSONBFC.ACCOUNTNBFC.DBO.LEDGER L WITH (NOLOCK) ,                              
ABCSONBFC.ACCOUNTNBFC.DBO.LEDGER1 L1 WITH (NOLOCK)                              
WHERE vdt >=@RECSDATE AND vdt <=@todate +' 23:59'   AND L.VTYP=L1.VTYP AND L.VTYP IN (2,3) AND L.VNO=L1.VNO AND L.BOOKTYPE =L1.BOOKTYPE                               
AND ENTEREDBY <>'B_TPR' AND ( RELDT ='' OR RELDT >=@RECODATE) GROUP BY CLTCODE                               
 ) A                              
...
select  'CR'AS DATA ,*,convert(varchar,(sum(balance)/10000000))  AS Balance_new   from #CC where balance >0 and cltcode IN (SELECT * FROM OLDNEW) group by    cltcode,long_name,PAN_GIR_NO ,balance                
---union ALL                   
