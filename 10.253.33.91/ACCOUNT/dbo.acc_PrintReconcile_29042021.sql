-- Object: PROCEDURE dbo.acc_PrintReconcile_29042021
...
FROM     
 ...
 (SELECT VNO, VTYP, BOOKTYPE FROM LEDGER WITH(NOLOCK) WHERE CLTCODE = @code AND VDT <= @tdate ) L2    
WHERE     
 L.VTYP = L2.VTYP AND L.VNO= L2.VNO AND L.BOOKTYPE = L2.BOOKTYPE                     
 AND L.VTYP = L1.VTYP AND L.BOOKTYPE = L1.BOOKTYPE AND L.VNO = L1.VNO AND L.LNO = L1.LNO        
 AND CLTCODE <> @code    
 AND NOT EXISTS    
  (SELECT L3.DDNO FROM LEDGER1 L3 WITH(NOLOCK), (SELECT VNO, VTYP, BOOKTYPE FROM LEDGER WITH(NOLOCK) WHERE CLTCODE = @code AND VDT <= @tdate AND VTYP = 16) L4    
...
select distinct vtyp, booktype, vno, lno into #tblbankreco_TMP from ledger WITH(NOLOCK) where cltcode = @code           
