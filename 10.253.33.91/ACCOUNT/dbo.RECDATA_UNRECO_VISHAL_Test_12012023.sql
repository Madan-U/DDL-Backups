-- Object: PROCEDURE dbo.RECDATA_UNRECO_VISHAL_Test_12012023
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

      
--EXEC RECDATA_UNRECO_VISHAL_Test_03012021 'JUL 01 2020', 'SEP 30 2020'        
        
        
        
                        
CREATE PROC [dbo].[RECDATA_UNRECO_VISHAL_Test_12012023] (        
  @FROMDATE VARCHAR(11),         
  @TODATE   VARCHAR(11)        
...
,'.', ''),        
    ...
    VDT1= Convert(Varchar(11),vdt,120) INTO #FINAL        
      FROM   #temp AS A   (nolock)      
             LEFT OUTER JOIN msajag.dbo.client_details AS B  (nolock)       
  ...
  update f set ac_num = CltAccNo from #FINAL f (nolock), [AngelBSECM].account_ab.dbo.Recon_cms_data r  With(Nolock)      
  where f.vno = r.vno and f.vtyp = r.vtyp and f.booktype = r.booktype and exchange = 'bse' and ISNULL (CltAccNo, '') <> ''        
        
update f set ac_num = CltAccNo from #FINAL f (nolock), [AngelFO].accountcurfo.dbo.Recon_cms_data r  With(Nolock)      
  where f.vno = r.vno and f.vtyp = r.vtyp and f.booktype = r.booktype and exchange = 'nsecurfo' and ISNULL (CltAccNo, '') <> ''        
        
  update f set ac_num = CltAccNo from #FINAL f (nolock), [AngelFO].accountfo.dbo.Recon_cms_data r  With(Nolock)      
  where f.vno = r.vno and f.vtyp = r.vtyp and f.booktype = r.booktype and exchange = 'nsefo' and ISNULL (CltAccNo, '') <> ''        
        
  update f set ac_num = CltAccNo from #FINAL f (nolock), Recon_cms_data r  With(Nolock)      
  where f.vno = r.vno and f.vtyp = r.vtyp and f.booktype = r.booktype and exchange = 'nse' and ISNULL (CltAccNo, '') <> ''        
        
          
  update f set ac_num = CltAccNo from #FINAL f (NOlock), [AngelCommodity].accountmcdx.dbo.Recon_cms_data r  With(Nolock)      
 ...
 select distinct a.Party_code,a.processdatetime,a.segapprovedamt  as amount,b.Req_PayBankId,a.Req_RefNo into #ab        
 from #aa a with(nolock)        
  left join #ncms b With(Nolock) on a.party_code=b.party_code and        
 a.req_refno=b.req_refno and a.requestid=b.porequestid        
  ...
  insert into #tmp1        
  select Client_Code,Acc_No,Amount,'2' as vtyp ,REFERENCE_NO,Convert(varchar(11),POST_DATE,120)       
  from  [AngelBSECM].APIDetails.dbo.API_PAYMENT_DETAILS where post_date between @FROMDATE and @TODATE + ' 23:50:50'        
        
 Update #FINAL set   Ac_NUM=isnull(AccNO,'')      
 From #tmp1 b where #FINAL.CLTCODE=b.CltCode and #FINAL.VAMT=round(b.amt,2) and #FINAL.vTYP=b.vtyp and        
 (convert(varchar(11),entry_date,120)= GeneratedDate or VDT1 = GeneratedDate ) AND #FINAL.vTYP=3      
...
Create index #b on #final         
(Vno,vtyp,booktype)       
        
Create index #s on #tmp1(Cltcode,vtyp)        
      
      
Select Cltcode into #tbank from (      
Select Cltcode from Acmast where accat =2      
union all      
Select Cltcode from anand.account_ab.dbo.Acmast where accat =2      
union all      
Select Cltcode from angelfo.accountfo.dbo.Acmast where accat =2      
union all      
Select Cltcode from angelfo.accountcurfo.dbo.Acmast where accat =2      
union all      
Select Cltcode from angelcommodity.accountmcdx.dbo.Acmast where accat =2      
union all      
Select Cltcode from angelcommodity.accountncdx.dbo.Acmast where accat =2 )A      
      
        
  select  *, Row_Number() over(order by Cltcode) Sno into #Bank  from (        
  Select distinct exchange,Cltcode from #FINAL With(Nolock) where cltcode in (select * from #tbank) ) A      
...
BEGIN        
  ...
  Select Row_Number() over(order by a.vdt1,a.vno) Sno, A.EXCHANGE,a.cltcode Bank_Code, replace(a.vdt,'/','-') as VDT,replace(a.RELDT,'/','-') as Value_date,        
  ...
  --where a.Cltcode  in ('02020' )a ,        
  where a.Cltcode  =@Bankcode and EXCHANGE=@exchange)a ,        
  ( SELECT a.*        
  FROM  #FINAL a With(Nolock)      
  --where a.Cltcode not in ('02020'))b        
...
Select @fdate =sdtcur from parameter where @FROMDATE between sdtcur and ldtcur        
 ...
 IF @fdate=@FROMDATE      
  ...
  insert into tmp_bank_book        
select 0,l.EXCHANGE,l.CLTCODE,'','','','','','','','Opening Balance','','','',        
...
sum(Case when Drcr='D' Then Vamt Else Vamt *-1 end)  Else 0 End) as Receipt,''         
 from ledger_all L With(Nolock)      
 inner join (Select distinct cltcode from  #bank With(Nolock) )B on L.CLTCODE=b.CLTCODE   where vdt =@fdate  and VTYP =18      
 Group by l.Exchange,l.Cltcode        
...
sum(Case when Drcr='D' Then Vamt Else Vamt *-1 end)  Else 0 End)*-1 as Payment,        
(Case When sum(Case when Drcr='D' Then Vamt Else Vamt *-1 end) >0 Then         
sum(Case when Drcr='D' Then Vamt Else Vamt *-1 end)  Else 0 End) as Receipt,''     from ledger_all L With(Nolock)       
 inner join (Select distinct cltcode from  #bank With(Nolock) )B  on L.CLTCODE=b.CLTCODE       
 where vdt>=@fdate and vdt< @FROMDATE        
 Group by l.Exchange,l.Cltcode        
  END      
...
,Convert(Varchar(50),cast(SUM (Payment-receipt) OVER (ORDER BY SNO)as decimal(18,2)))      
--, Convert(Varchar(50),cast( (Payment-receipt) + COALESCE(        
--(        
--  SELECT SUM(Payment-receipt)        
--    FROM dbo.tmp_bank_book AS i  With(Nolock)      
--    WHERE i.Sno < o.SNO and Bankcode=@bank_code and exchange=@fexchange), 0        
--) as decimal(18,2)))        
FROM dbo.tmp_bank_book  AS o With(Nolock) where  Bankcode=@bank_code and exchange=@fexchange        
ORDER BY SNO;       
...
set @NSEQUERY = @NSEQUERY + ' from anand1.Account.dbo.Bfile  order by Sno " queryout J:\Backoffice\BANKBOOK1_JULY\BankBook_'+convert(varchar,@bank_code+'-'+@fexchange)+'.txt -c -t"," -Sanand1 -Uclassmkt -Pdd$$gnfDTVs244648ysjgZAcc'        
