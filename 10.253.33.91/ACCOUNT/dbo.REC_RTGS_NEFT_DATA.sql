-- Object: PROCEDURE dbo.REC_RTGS_NEFT_DATA
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


 --EXEC RECDATA '13/12/2017','13/12/2017'  
                  
CREATE PROC [dbo].[REC_RTGS_NEFT_DATA] (  
  @FROMDATE VARCHAR(11),   
  @TODATE   VARCHAR(11)  
  ...
  BEGIN   
      ...
      [Entry_DAte]=cdt,  
              ...
              WHERE  L.vno = L1.vno   
                     ...
                     AND L.drcr = 'D'   
                     AND L.vtyp = 2   
                     AND (L.NARRATION LIKE '%RTGS%'
					 OR L.NARRATION LIKE '%Fund Payout Rejected%')
                     AND L.vdt >= @FROMDATE   
    ...
    WHERE   T1.SNO =T4.MASTERSNO  AND L.VTYPE=T1.VTYPE AND   (L.NARRATION LIKE '%RTGS%'  
	OR L.NARRATION LIKE '%Fund Payout Rejected%' )
	and  L.VDT >= @fromdate AND L.VDT < =@todate + ' 23:59'      
