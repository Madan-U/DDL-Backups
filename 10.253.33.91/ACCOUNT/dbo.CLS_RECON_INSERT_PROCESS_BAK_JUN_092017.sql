-- Object: PROCEDURE dbo.CLS_RECON_INSERT_PROCESS_BAK_JUN_092017
...
CREATE  PROCEDURE [dbo].[CLS_RECON_INSERT_PROCESS_BAK_JUN_092017] (@UPLOADID   NVARCHAR(50)
...
WHERE SUBSTRING(L.DDNO, PATINDEX('%[^0]%',L.DDNO), 10) = SUBSTRING(N.REFERENCENO, PATINDEX('%[^0]%',N.REFERENCENO), 10)
...
, #LEDGER_TO_MAP_NON L (NOLOCK)
WHERE SUBSTRING(L.DDNO, PATINDEX('%[^0]%',L.DDNO), 10) = SUBSTRING(N.REFERENCENO, PATINDEX('%[^0]%',N.REFERENCENO), 10)
--LEFT(L.DDNO, 15) = LEFT(N.REFERENCENO, 15)
AND L.DDNO<>''
AND L.RELAMT = CONVERT(MONEY, N.AMT)
AND N.DRCR = L.DRCR
AND CONVERT (DATETIME,CONVERT (VARCHAR(11) ,L.VDT,109))<=N.VALUEDATE
