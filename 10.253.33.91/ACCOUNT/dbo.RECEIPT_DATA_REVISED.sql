-- Object: PROCEDURE dbo.RECEIPT_DATA_REVISED
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

--RECEIPT_DATA_REVISED_nikhil '01/09/2015','10/09/2015'                           
CREATE PROC [dbo].[RECEIPT_DATA_REVISED] (@FROMDATE VARCHAR(11), 
                                                 @TODATE   VARCHAR(11)) 
AS 
    IF Len(@FROMDATE) = 10 
  ...
  BEGIN 
      ...
      SELECT [CLTCODE]=Replace(Ltrim(Rtrim(A.cltcode)), ' ', ''), 
             ...
             [NARRATION]=replace(replace(REPLACE(Replace(LEFT(Replace(Ltrim(Rtrim(A.narration)), '"', '' ),100), ',',''),'|', ''),'{','('),'}',')'),
