-- Object: PROCEDURE dbo.COMPUTE_DPC_CHARGES__15042013
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

CREATE PROCEDURE [dbo].[COMPUTE_DPC_CHARGES__15042013]                    
(                    
...
ALTER TABLE [dbo].[#DPC_CLIENTS] ADD  CONSTRAINT [PK_dpc_client] PRIMARY KEY CLUSTERED     
(    
 [PARTY_CODE] ASC    
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON)                 
    
INSERT INTO #DPC_CLIENTS    
EXEC CHK_GET_DPC_CLIENTS @FROM_PARTY, @TO_PARTY, @FROM_DATE                  
...
CL_TYPE,              
  0                    
 FROM                    
  #DEBIT_DEPC_BILLS L, #DPC_CLIENTS C                   
 WHERE                    
  L.CLTCODE = PARTY_CODE                    
...
/* SET @@SQL = " INSERT INTO #DPC_CHARGES(DPC_DATE, CLTCODE, LONG_NAME, DPC_BALANCE, LEDGER_BALANCE, CASH_COLLATREAL, NONCASH_COLLATREAL, MARGIN_REQ, BRANCH_CD, SUB_BROKER, TRADER, REGION, AREA, CL_TYPE) "                    
 ...
 SELECT                    
  ...
  CL_TYPE                    
 FROM                    
  MSAJAG.DBO.COLLATERAL CO, #DPC_CLIENTS C                   
 WHERE                     
...
FROM                    
 ...
 FROM                    
  #DPC_CHARGES                     
 GROUP BY DPC_DATE, CLTCODE, LONG_NAME, BRANCH_CD, SUB_BROKER, TRADER, REGION, AREA, CL_TYPE                     ) C /*LEFT OUTER JOIN (SELECT CLTCODE, VDT, BILL_AMOUNT = SUM(BILL_AMOUNT) FROM #DEBIT_DEPC_BILLS GROUP BY CLTCODE, VDT) D                 ON                      CONVERT(DATETIME, CONVERT(VARCHAR(11), DPC_DATE, 109)) = CONVERT(DATETIME, CONVERT(VARCHAR(11), VDT, 109))                    
  AND D.CLTCODE = C.CLTCODE              */      
