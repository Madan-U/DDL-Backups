-- Object: PROCEDURE dbo.inserttoledger2_backup
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

/****** Object:  Stored Procedure dbo.inserttoledger2    Script Date: 05/07/2003 5:59:58 PM ******/  
...
CREATE proc inserttoledger2_backup   
...
begin  
    ...
    and party_code in ( select distinct party_code from templedger2 where rtrim(sessionid) = rtrim(@sessionid) and vno = @vno and costflag = 'C' )  
...
/* print "costbreakup= " + str(@@costbreakup,2)  */  
   ...
   if @@costbreakup > 0 and (@@vtype = '3' or @@vtype = '4')   
      begin  
        ...
        delete from templedger2     where rtrim(sessionid) = rtrim(@sessionid) and category = 'BRANCH' and upper(drcr) = 'C'  
        and vno = @vno and costflag = 'C'  
...
if @@Onlyall = 0   
 ...
 select @@brnachcnt = (select count(branch) from templedger2   
