-- Object: PROCEDURE dbo.CLS_RECON_NONCMS_HDFC_PROCESS_NewHDFc_05Mar2020
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


create PROCEDURE [dbo].[CLS_RECON_NONCMS_HDFC_PROCESS_NewHDFc_05Mar2020]
...
END
--select @VALUEDATE
IF CHARINDEX('.DAT',UPPER(@UPLOADFILENAME))>1
BEGIN
	...
	, BANKACCOUNT	, BANKCODE	, DESCRIPTION	, LEDGERBALANCE	, DRCR
	, AMT	, VALUEDATE	, REFERENCENO	, CLTACCOUNT	, CustomerReference
	, TransaCtionDetail	, VNO	, VTYP	, BOOKTYPE	, MATCHEDFLAG	, UPLOADSTATUS
	, UPLOADEDDATE	, MODIFIEDDATE	, UPLOADBY)
		SELECT
			UPLOADID
			,BANKACCOUNT, BANKCODE,DESCRIPTION,LEDGERBALANCE
			,CASE WHEN CREDITAMT <> '' THEN 'C'	WHEN DEBITAMT <> '' THEN 'D' ELSE ' '	END AS DRCR
			,CASE
...
BEGIN
	CREATE TABLE #TEMP (PROGID VARCHAR(50),UPLOADBY VARCHAR(50),FILE_DATA VARCHAR(4000))
