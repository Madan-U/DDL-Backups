-- Object: PROCEDURE dbo.CLS_GET_LEDGER2_FORMULA_BALANCES
...
SELECT CLTCODE
	...
	,MARGIN_BAL = 0
FROM (
	SELECT CLTCODE = L2.CLTCODE
		,VDTBAL_OPBAL = CASE 
			WHEN (VDT >= @START_DATE
				AND VDT < @FR_DATE) OR VTYPE = 18
				THEN SUM(CASE L2.DRCR
							WHEN 'D'
								THEN CAMT
							ELSE - CAMT
							END)
		...
		,EDTBAL_DATES_CREDIT = CASE 
			WHEN EDT >= @FR_DATE 
				AND EDT <= @EFFDATE + ' 23:59' AND VTYPE <> 18
				THEN SUM(CASE L2.DRCR
							WHEN 'C'
								THEN CAMT
							ELSE 0
	...
	WHERE A.CLTCODE = L2.CLTCODE --AND SESSION_ID = @SESSIONID
		 AND L.VNO = L2.VNO AND L.VTYP = L2.VTYPE AND L.BOOKTYPE = L2.BOOKTYPE AND L.LNO = L2.LNO AND L.DRCR = L2.DRCR
		AND (
			VDT >= @START_DATE
			OR EDT >= @START_DATE
			)
