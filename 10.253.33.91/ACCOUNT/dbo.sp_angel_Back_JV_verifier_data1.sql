-- Object: PROCEDURE dbo.sp_angel_Back_JV_verifier_data1
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

CREATE procedure sp_angel_Back_JV_verifier_data1        
as        
...
--set @getdate = convert(varchar(11),getdate()-1)+' 23:59:59'                              
set @getdate = (select convert(varchar(11),ledger_updated_date + 1)+' 06:00:00' from MIs.Jvfile.dbo.legder_date)  
--print @getdate                              
...
and cltcode in (select Party_Code from mis.jvfile.dbo.back_jvrequest where JVdate = @gdate and BatchNo = 0 and ValidJV = 'N')        
group by cltcode                                   
...
-- and cltcode in (select Party_Code from mis.jvfile.dbo.jvrequest where JVdate = @gdate and BatchNo = 0 and ValidJV = 'N')        
-- group by cltcode                    
--         
...
truncate table Back_Angel_JV1        
        
insert into Back_Angel_JV1        
select a.*,b.ACDL_Balanceb from  #nse_led_a a left outer join #nse_led_b b on a.cltcode=b.cltcode --where a.acdl_balancea <> 0        
        
--delete from Back_Angel_JV1 where acdl_balancea+acdl_balanceb = 0        
