-- Object: PROCEDURE dbo.acc_reports_pratham
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------



CREATE PROCEDURE [dbo].acc_reports_pratham                          
...
IF @FCODE = " "                          
BEGIN                          
   SELECT @FCODE = '5100000018'   
                              
END  

IF @TCODE = " "                          
BEGIN                          
   SELECT @TCODE = '5100000018'   
...
print 'vin222'                     
        ...
        SELECT @@FROMTABLES = " FROM LEDGER L  (NOLOCK) ,VMAST V (NOLOCK) , LEDGER2 L2  (NOLOCK) LEFT OUTER JOIN ACMAST A  (NOLOCK) ON L2.CLTCODE = A.CLTCODE, COSTMAST C (NOLOCK), (SELECT BRANCH_CODE FROM MSAJAG.DBO.REGION WHERE UPPER(REGIONCODE) = '" + @STATUSNAME + "') R "                          
