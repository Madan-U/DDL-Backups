-- Object: PROCEDURE dbo.VIRTUAL_MIS_LEDGER_CONVERTER_SURESH
...
*/   
CREATE PROCEDURE [dbo].[VIRTUAL_MIS_LEDGER_CONVERTER_suresh] @FILENAME VARCHAR(1000),   
                                                     @UNAME    VARCHAR(50)   
AS   
    ...
    WHERE   
      ...
      NOT EXISTS(SELECT TXN_REF_NO   
				...
				-- .DBO.PIECE(TRANSACTION_DETAILS1, '|' , 1) = TXN_REF_NO)
				 ...
				 (.DBO.PIECE(TRANSACTION_DETAILS1, '|' , 1) = TXN_REF_NO or .DBO.PIECE(TRANSACTION_DETAILS1, '/', 3) = TXN_REF_NO or .DBO.PIECE(TRANSACTION_DETAILS1, '|', 3) = TXN_REF_NO ))   
