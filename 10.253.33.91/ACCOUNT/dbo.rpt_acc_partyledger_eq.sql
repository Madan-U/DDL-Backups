-- Object: PROCEDURE dbo.rpt_acc_partyledger_eq
...
CREATE PROCEDURE [dbo].[rpt_acc_partyledger_eq]                
(                  
...
IF @@REPTYPE = 'P'          
BEGIN          
 EXEC RPT_ACC_PARTYLEDGER_PARENT_EQ @FDATE, @TDATE, @FCODE, @TCODE, @@STRORDER, @SELECTBY, @STATUSID, @STATUSNAME, @STRBRANCH          
 RETURN          
END          
    ...
    AND (CRAMT <> 0 OR DRAMT <> 0 OR OPBAL <> 0) and  L.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)                 
            ...
            BEGIN                        
                  ...
                  FROM #TMPLEDGER L WITH(NOLOCK),                   
                        ACMAST A WITH(NOLOCK)                   
                  WHERE L.CLTCODE = A.CLTCODE                   
                        AND ACCAT IN ('4', '104') and  L.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)                  
                  ORDER BY L.CLTCODE,                  
                        EFFDT, PDT                
            END                        
      END                        
