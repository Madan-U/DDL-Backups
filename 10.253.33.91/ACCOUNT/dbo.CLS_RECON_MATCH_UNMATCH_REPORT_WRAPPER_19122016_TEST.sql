-- Object: PROCEDURE dbo.CLS_RECON_MATCH_UNMATCH_REPORT_WRAPPER_19122016_TEST
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------



CREATE PROCEDURE [DBO].[CLS_RECON_MATCH_UNMATCH_REPORT_WRAPPER_19122016_TEST]
(
...
CREATE TABLE #TEMP_REPORT_WRAPPER_Final(ROWID INT NOT NULL IDENTITY (1, 1)
, FILEUPLOADID NVARCHAR(50)
, UPLOADDATE DATETIME
, EDATE DATETIME
, VALUEDATE DATETIME
...
WHILE (SELECT	COUNT(*)	FROM @TEMP_SEGMENT) > 0 
	BEGIN
		...
		PRINT @@SQL
			...
			INSERT INTO #TEMP_REPORT_WRAPPER_Final
			SELECT FILEUPLOADID,
			UPLOADDATE,
			EDATE,
			VALUEDATE,
			...
			FILEUPLOAD_SDATE,
			FILEUPLOAD_EDATE,
			USERNAME FROM #TEMP_REPORT_WRAPPER

			DELETE FROM #LEDGER_TO_MAP
			DELETE FROM #TEMP_REPORT_WRAPPER
	...
	,MIN(CONVERT(NVARCHAR, FILEUPLOAD_SDATE, 120)) FILEUPLOAD_SDATE
	,MAX(CONVERT(NVARCHAR, FILEUPLOAD_EDATE, 120)) FILEUPLOAD_EDATE
	FROM #TEMP_REPORT_WRAPPER_Final
	...
	SELECT	* FROM #TEMP_REPORT_WRAPPER_Final RL
	WHERE (RL.ARRANGEMENTTYPE = @FILETYPE OR @FILETYPE='')
	AND (RL.BANK = @BANKNAME OR @BANKNAME='')
	AND (RL.SEGMENT = @SEGMENT OR @SEGMENT='')
