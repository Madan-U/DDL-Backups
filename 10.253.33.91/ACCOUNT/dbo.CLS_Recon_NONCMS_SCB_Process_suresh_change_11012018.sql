-- Object: PROCEDURE dbo.CLS_Recon_NONCMS_SCB_Process_suresh_change_11012018
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------



 create PROCEDURE [dbo].[CLS_Recon_NONCMS_SCB_Process_suresh_change_11012018] (@UPLOADID nvarchar(50), @UPLOAD_FLAG CHAR(1),@BANKCODE NVARCHAR(50) ) 
AS
BEGIN
...
INSERT INTO #NONCMS_DATA
	SELECT
		...
		,CASE WHEN LEFT(.dbo.piece(FILE_DATA, ',', 30),5) <>'IMPS/' THEN .dbo.piece(FILE_DATA, ',', 30)
			  ELSE
		 CASE WHEN LEFT(.dbo.piece(FILE_DATA, ',', 38),5)='IMPS/' THEN .DBO.PIECE(.dbo.piece(FILE_DATA, ',', 38), '/', 3)
			  ...
			  ELSE .DBO.PIECE(.dbo.piece(FILE_DATA, ',', 38), '|', 1)END END AS 'REFERENCENO'
		,'' AS CLTACCOUNT
