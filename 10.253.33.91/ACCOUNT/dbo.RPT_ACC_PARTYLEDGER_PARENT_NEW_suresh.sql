-- Object: PROCEDURE dbo.RPT_ACC_PARTYLEDGER_PARENT_NEW_suresh
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

--  EXEC RPT_ACC_PARTYLEDGER_PARENT 'Oct  1 2010','Oct  4 2010','98RP61','98RP61','ACCODE','vdt','broker','broker',''              
      
--RPT_ACC_PARTYLEDGER_PARENT_NEW_suresh 'May  1 2013','May 11 2013','S117389','S117389','ACCODE','vdt','client','S117389',''      
CREATE PROCEDURE [dbo].[RPT_ACC_PARTYLEDGER_PARENT_NEW_suresh]              
(              
...
SELECT * FROM MSAJAG.DBO.GETCLIENTS(@STATUSID, @STATUSNAME, @FCODE, @TCODE, @STRBRANCH)                
...
/*   AND PARENTCODE > = @FCODE              
 ...
 select * into #LEDGERCLIENTS1 from msajag..client2       
 where PARENTCODE=@FCODE      
       
/*      
  SELECT * FROM LEDGER L1 WITH(NOLOCK) , #LEDGERCLIENTS1 c2              
...
*/         
  ...
  FROM              
   ...
   WHERE VDT > = @FDATE + ' 00:00:00' AND VDT < = @TDATE +' 23:59:59'              
   AND EXISTS(              
    SELECT PARTY_CODE, PARENTCODE FROM #LEDGERCLIENTS1 C2              
    WHERE              
...
BEGIN              
 ...
 BEGIN              
  --PRINT 'T1'              
  SELECT              
  L.BOOKTYPE,VOUDT,EFFDT,SHORTDESC,DRAMT,CRAMT,VNO,DDNO,NARRATION,CLTCODE=L.PARENTCODE,VTYP,          
VDT,EDT,L.ACNAME,OPBAL,L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_CITY,L_ZIP,RES_PHONE1,BRANCH_CD,          
CROSAC = L.CLTCODE,EDIFF,FAMILY,SUB_BROKER,TRADER,CL_TYPE, BANK_NAME,CLTDPID,LNO,PDT              
  FROM #TMPLEDGER L WITH(NOLOCK) WHERE (CRAMT <> 0 OR DRAMT <> 0 OR OPBAL <> 0)              
  ORDER BY L.CLTCODE,              
  VOUDT, PDT              
 END              
 ELSE              
...
VDT,EDT,L.ACNAME,OPBAL,L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_CITY,L_ZIP,RES_PHONE1,BRANCH_CD,          
CROSAC = L.CLTCODE,EDIFF,FAMILY,SUB_BROKER,TRADER,CL_TYPE,BANK_NAME,CLTDPID,LNO,PDT              
  FROM #TMPLEDGER L WITH(NOLOCK) WHERE               
  (CRAMT <> 0 OR DRAMT <> 0 OR OPBAL <> 0)              
  ORDER BY L.ACNAME,              
  EFFDT, PDT              
 END              
END              
