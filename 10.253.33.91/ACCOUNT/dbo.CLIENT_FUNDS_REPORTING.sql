...
DECLARE @FROMDATE DATETIME =Convert(datetime ,@FR_DATE)
DECLARE @TODATE DATETIME =Convert(datetime ,@TO_DATE)
...
CREATE INDEX IDX_LEDGER_PL ON #LEDGER_PL (VNO, VTYP ,BOOKTYPE );  
...
FROM  
 #CLIENT_DETAILS C,   
 (SELECT CLTCODE, VAMT, DRCR, NARRATION, VDT, EDT, PDT, VTYP, VNO, BOOKTYPE 
 FROM LEDGER WITH (NOLOCK) WHERE VDT BETWEEN @FROMDATE AND @TO_DATE + ' 23:59' AND CLTCODE BETWEEN @FR_CODE AND @TO_CODE  AND VTYP <> 18 ) L,   
 VMAST_DESC V  
WHERE  
 C.CL_CODE = CLTCODE  
 AND L.VTYP = V.VTYPE  
...
UPDATE L SET L.CHQ_NO = L1.DDNO FROM #LEDGER_PL L, LEDGER1 L1 WITH (NOLOCK)  
...
(  
 ...
 FROM LEDGER L WITH (NOLOCK), (SELECT DISTINCT CL_CODE FROM #CLIENT_DETAILS) L1, PARAMETER P  
 WHERE L.CLTCODE = L1.CL_CODE AND L.VDT >= SDTCUR AND VDT < @FROMDATE AND @FROMDATE BETWEEN SDTCUR AND LDTCUR AND VTYP <> 18  
 GROUP BY L.CLTCODE  
 UNION ALL  
   
 SELECT CLTCODE = L.CLTCODE, AMOUNT = SUM(CASE WHEN DRCR = 'C' THEN VAMT ELSE -VAMT END)  
 FROM LEDGER L WITH (NOLOCK), (SELECT DISTINCT CL_CODE FROM #CLIENT_DETAILS) L1, PARAMETER P  
 WHERE L.CLTCODE = L1.CL_CODE AND @FROMDATE BETWEEN SDTCUR AND LDTCUR  
