-- Object: PROCEDURE dbo.Usp_LimitGenerater_File_New_28062022
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

            
--exec Usp_LimitGenerater_File_New           
CREATE Proc [dbo].[Usp_LimitGenerater_File_New_28062022]            
As            
BEGIN            
...
----------------///**********B2C GM7 serevr Limit File Download on 23 June 2022 *******************////////////////--------------------------  
   ...
   select CLTCODE+'||||'+convert(varchar,convert(decimal(18,2), Amt))+'|9999999999999|||9999999999999||||||||INTRADAY||9999999999999|||||||0|ANGEL||||||||||||||||||||||||||' from  #wsxc1  
      
     
       
   DECLARE @filename_GM7 as varchar(100)                                                           
   select @filename_GM7='\\196.1.115.147\Upload1\OmnesysFileFormat\Limit_File_GM7\BO_OmnLimitFile_GM7'+replace (convert (varchar(12),GETDATE(),103),'/','')+'.txt'        
   ...
   SET @FILENAME_GM7 = '\\196.1.115.147\Upload1\OmnesysFileFormat\Limit_File_GM7\BO_OmnLimitFile_GM7'+replace (convert (varchar(12),GETDATE(),103),'/','')+'.txt'                                                                              
   SET @BCPCOMMAND_GM7 = 'BCP "SELECT   FileFormat FROM Account.dbo.tbl_OmnLimitFile_GM7 with (nolock)" QUERYOUT "'                      
   SET @BCPCOMMAND_GM7 = @BCPCOMMAND_GM7 + @filename_GM7 + '" -c -t, -SANAND1 -Uclassmkt -Pdd$$gnfDTVs244648ysjgZAcc'                      
