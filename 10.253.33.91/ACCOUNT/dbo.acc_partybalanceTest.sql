...
Exec acc_partybalanceTest 'Jun 16 2003', 'vdt','Partywise','Normal','Apr 1 2003', 'Apr 1 2003', 1,'Apr 1 2003', 'BROKER', '%', 'edt', 'br0023', 'br0023' 
...
begin
   ...
   begin
      ...
      select @@fromtables = @@wherepart + " and m.party_code = l.cltcode ),0) " + "from ledger l left outer join acmast a on l.cltcode=  a.cltcode , msajag.dbo.client1 c1  ,msajag.dbo.client2 c2  " 
   ...
   begin
      if @sortbydate = 'vdt'
         begin
            ...
            select @@fromtables = @@wherepart2 + " and m.party_code = l.cltcode ),0) " +" from ledger l left outer join acmast a on l.cltcode=  a.cltcode , msajag.dbo.client1 c1  ,msajag.dbo.client2 c2  " 
	   ...
	   select @@fromtables = " from ledger l left outer join acmast a on l.cltcode=  a.cltcode , msajag.dbo.client1 c1  ,msajag.dbo.client2 c2 "
           select @@wherepart  = @@wherepart2 + @@addwhere +   @@addwhere1
         end
   end
   ...
   begin
      if @sortbydate = 'vdt'
         begin
            select @@selectqury = "select l.cltcode , acname=isnull(l.acname,''), branchcode, Amount = sum(case when upper(drcr) = 'D' then vamt else -vamt end), unrealamt = sum(case when edt > getdate() then ( case when upper(drcr) = 'D' then vamt else -vamt end)else 0 end), margin = isnull(( select sum(case when upper(m.drcr) = 'D' then amount else -amount end) from marginledger m "
            select @@fromtables = @@wherepart3 + " and m.party_code = l.cltcode ),0) " +" from ledger l left outer join acmast a on l.cltcode=  a.cltcode , msajag.dbo.client1 c1  ,msajag.dbo.client2 c2  " 
	   ...
	   select @@fromtables = @@mwherepart2 + " and m.party_code = l.cltcode ),0) " + "from ledger l left outer join acmast a on l.cltcode=  a.cltcode , msajag.dbo.client1 c1  ,msajag.dbo.client2 c2  " 
      	   select @@wherepart  = @@wherepart2 + @@addwhere + @@addwhere1
