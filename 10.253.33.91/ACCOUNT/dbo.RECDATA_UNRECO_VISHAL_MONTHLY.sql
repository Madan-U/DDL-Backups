-- Object: PROCEDURE dbo.RECDATA_UNRECO_VISHAL_MONTHLY
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

      
      
--EXEC RECDATA_UNRECO_VISHAL_MONTHLY 'Nov 20 2021', 'Nov 20 2021'      
        
        
        
                        
CREATE PROC [dbo].[RECDATA_UNRECO_VISHAL_MONTHLY] (        
  @FROMDATE VARCHAR(11),         
  @TODATE   VARCHAR(11)        
...
T1.POSTED_ON AS CDT,                                                                                    
    [BOOKTYPE]='01',                                                                          
    [EXCHANGE]='MFSS'     ,      ENTEREDBY =POSTED_BY                                                                            
    FROM ANGELFO.BBO_FA.DBO.MFSS_LEDGER_BSE L (NOLOCK) , ANGELFO.BBO_FA.DBO.ACC_TBL4  T4 WITH(NOLOCK)  ,ANGELFO.BBO_FA.DBO.ACC_TBL1   T1   WITH(NOLOCK), [196.1.115.253].MutualFund.dbo.vw_SIPPayinTransdetails N1 with (Nolock)                               
...
set @NSEQUERY = @NSEQUERY + ' from anand1.Account.dbo.Bfile  order by Sno " queryout j:\MONTHLY_BANKBOOK\BankBook_'+convert(varchar,@bank_code+'-'+@fexchange)+'.txt -c -t"," -Sanand1 -Usa -Psuropt09'        
