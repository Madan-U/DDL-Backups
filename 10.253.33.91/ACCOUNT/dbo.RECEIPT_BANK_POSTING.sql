-- Object: PROCEDURE dbo.RECEIPT_BANK_POSTING
...
DECLARE
       @INACT_ACOUNT TINYINT
 
SELECT @INACT_ACOUNT = COUNT(1) FROM #RECPAY_TABLE WHERE UPDFLAG = 'N'
 
 
--CEHCK WITH CLIENT_BROK_DETAILS
IF @INACT_ACOUNT >= 1
BEGIN
  UPDATE
   #RECPAY_TABLE
  SET
   ...
   MICRNO = ISNULL(B.MICRNO, 0)
  FROM
   MSAJAG.DBO.CLIENT_BROK_DETAILS A with(NOLOCK), ACMAST B, MSAJAG.DBO.CLIENT_DETAILS C with(NOLOCK)
  WHERE
   A.CL_CODE = CLIENT_CODE
...
(
       ...
       WHERE  
			  L.CLTCODE =RP.CLIENT_CODE
			  AND L.VDT >=CONVERT(VARCHAR(11),GETDATE()-30,120)
			  AND L.VTYP ='2' 
			  AND L.VTYP =L1.vtyp 
