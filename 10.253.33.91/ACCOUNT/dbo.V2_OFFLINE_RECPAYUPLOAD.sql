-- Object: PROCEDURE dbo.V2_OFFLINE_RECPAYUPLOAD
...
BEGIN DISTRIBUTED TRANSACTION                                                              
  ...
  CREATE NONCLUSTERED INDEX [RECPAY_FLDAUTO] ON #RECPAY_TABLE
(
	[FLD_AUTO] ASC
)                              
...
BEGIN 
  ...
  WHERE  VOUCHERTYPE = 2                                                                
   ...
   AND  ADDBY <>'TPR'
   AND NOT EXISTS(SELECT FLD_AUTO FROM OFFLINE_DATA_LOG O WHERE  FLDAUTO = FLD_AUTO AND ROWSTATE IN(9, 1))
   END 
   ELSE
    BEGIN                                               
  INSERT INTO #RECPAY_TABLE                                                                
 ...
 INSERT INTO OFFLINE_DATA_LOG (FLD_AUTO, ROWSTATE)
 SELECT FLD_AUTO, 9
 FROM #RECPAY_TABLE
                                                
  ---  EXEC ANGELBSECM.ACCOUNT_AB.Dbo.V2_REC_FLAG_UPDATE 9                                       
                                                   
  --UPDATE AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES-- WITH (ROWLOCK)                                                               
  --SET    ROWSTATE = 9                                                              
  --FROM   #RECPAY_TABLE                                                              
...
-- APPROVEDBY = 'SYSTEM',                                                            
 LEDGERVNO = VNO                                                              
FROM   #RECPAY_TABLE   R,OFFLINE_DATA_LOG O                                           
WHERE  O.FLD_AUTO = R.FLD_AUTO
...
--WHERE  FLDAUTO = #RECPAY_TABLE.FLD_AUTO    
 ...
 EXEC ANGELBSECM.ACCOUNT_AB.Dbo.V2_REC_FLAG_UPDATE 1                                                              
