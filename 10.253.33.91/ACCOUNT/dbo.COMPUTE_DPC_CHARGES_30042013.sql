-- Object: PROCEDURE dbo.COMPUTE_DPC_CHARGES_30042013
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

CREATE PROCEDURE [dbo].[COMPUTE_DPC_CHARGES_30042013]    
(                            
...
CREATE TABLE [dbo].[#DPC_CLIENTS_CHARGES](  
 ...
 CONSTRAINT [PK1_clientidx] PRIMARY KEY CLUSTERED   
(  
 [PARTY_CODE] ASC  
...
INSERT INTO #DPC_CLIENTS_CHARGES  
SELECT * FROM DPC_CLIENTS_CHARGES  
...
SELECT                             
  ...
  0                            
 FROM                            
  #DEBIT_DEPC_BILLS L, DPC_CLIENTS_CHARGES C                            
 WHERE                            
  L.CLTCODE = PARTY_CODE                            
...
/* SET @@SQL = " INSERT INTO #DPC_CHARGES(DPC_DATE, CLTCODE, LONG_NAME, DPC_BALANCE, LEDGER_BALANCE, CASH_COLLATREAL, NONCASH_COLLATREAL, MARGIN_REQ, BRANCH_CD, SUB_BROKER, TRADER, REGION, AREA, CL_TYPE) "                            
 ...
 SELECT                            
  ...
  CL_TYPE                            
 FROM                            
  MSAJAG.DBO.COLLATERAL CO, #DPC_CLIENTS_CHARGES C                         
 WHERE                             
  CO.PARTY_CODE = C.PARTY_CODE                            
  AND TRANS_DATE >= @FROM_DATE AND TRANS_DATE <= @TO_DATE                            
  AND CO.EXCHANGE = @EXCHANGE                            
  AND CO.SEGMENT = @SEGMENT                            
...
INSERT INTO DEBIT_DEPC_BILLS(CLTCODE, BILL_AMOUNT, T_4_DATE)                      
SELECT DISTINCT CLTCODE, 0, DPC_DATE                      
FROM #ALL_DATES A WHERE NOT EXISTS (SELECT VDT FROM DEBIT_DEPC_BILLS TEMP WHERE A.CLTCODE = TEMP.CLTCODE AND A.DPC_DATE = TEMP.T_4_DATE)                      
...
FROM               
 ...
 ) C LEFT OUTER JOIN (SELECT CLTCODE, VDT, BILL_AMOUNT = SUM(BILL_AMOUNT) FROM #DEBIT_DEPC_BILLS GROUP BY CLTCODE, VDT) D                 ON                            
