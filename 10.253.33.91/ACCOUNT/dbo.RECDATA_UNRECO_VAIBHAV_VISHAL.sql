-- Object: PROCEDURE dbo.RECDATA_UNRECO_VAIBHAV_VISHAL
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

      
      
--EXEC RECDATA_UNRECO_VAIBHAV_VISHAL 'MAY  1 2020', 'MAY 31 2020'      
        
        
        
                        
CREATE PROC [dbo].[RECDATA_UNRECO_VAIBHAV_VISHAL] (        
  @FROMDATE VARCHAR(11),         
  @TODATE   VARCHAR(11)        
...
BEGIN        
  ...
  SELECT a.*        
  FROM  #FINAL a         
  --where a.Cltcode  in ('02093' )a ,        
  where a.Cltcode  =@Bankcode and EXCHANGE=@exchange)a ,        
  ( SELECT a.*        
  FROM  #FINAL a       
  --where a.Cltcode not in ('02093'))b        
...
set @NSEQUERY = @NSEQUERY + ' from anand1.Account.dbo.Bfile  order by Sno " queryout j:\MONTHLY_BANKBOOK\BankBook_'+convert(varchar,@bank_code+'-'+@fexchange)+'.txt -c -t"," -Sanand1 -Uaolinhouse -Pe$$gnfDTVs2455GZAcc'        
