-- Object: PROCEDURE dbo.USP_VIRTUAL_MIS_LEDGER_REPORT_bkp_11012018
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

CREATE PROC [dbo].[USP_VIRTUAL_MIS_LEDGER_REPORT_bkp_11012018]        
(        
...
END            
 ...
 SELECT A.*, (CASE WHEN VAMT <> 0 THEN 'Y' ELSE 'N' END) AS LEDGERPOST FROM       
(SELECT        
...
VIRTUAL_MIS_LEDGER_REPORT V ,MSAJAG..CLIENT_DETAILS C1      
 ...
 AND V.PARTY_CODE=C1.CL_CODE      
  ...
  AND .DBO.PIECE(REPLACE(NARRATION,'VIRTUAL ',''), '|', 1) =TXN_REF_NO          
