-- Object: PROCEDURE dbo.V2_OFFLINE_JVDRCRUPLOAD_bkp08Oct2017
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


CREATE PROC [dbo].[V2_OFFLINE_JVDRCRUPLOAD_bkp08Oct2017](            
...
IF @@ACCOUNTSVR <> @@SERVERNAME            
 BEGIN             
SET @@SQLPARA = @@SQLPARA + "  FROM   " + @@ACCOUNTSVR + ".ANAND.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES O  "            
...
ELSE            
 BEGIN            
SET @@SQLPARA = @@SQLPARA + " FROM   ANAND.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES O "            
...
IF @@ACCOUNTSVR <> @@SERVERNAME            
 BEGIN            
 SET @@SQLPARA = "   UPDATE " + @@ACCOUNTSVR + ".ANAND.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES "            
 END            
ELSE            
 BEGIN            
 SET @@SQLPARA = "   UPDATE ANAND.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES  "            
END             
...
UPDATE #RECPAY_TABLE SET LNO = LNO + (SELECT SUM(SRNO) FROM #RECPAY_TABLE L1 WHERE #RECPAY_TABLE.VNO = L1.VNO AND #RECPAY_TABLE.SERIAL_NO <= L1.SERIAL_NO)    
