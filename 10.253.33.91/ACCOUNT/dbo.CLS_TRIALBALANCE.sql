-- Object: PROCEDURE dbo.CLS_TRIALBALANCE
...
BEGIN
	...
	BEGIN
		...
		SET @DISPLAY_AMOUNT = '-ISNULL(AMOUNT, 0)'
	...
	BEGIN
		...
		SET @DISPLAY_AMOUNT = '-ISNULL(OPENING_AMOUNT, 0), ISNULL(PERIOD_CREDIT, 0), ISNULL(PERIOD_DEBIT, 0), -ISNULL(AMOUNT, 0)'
...
BEGIN
	...
	EXEC CLS_GET_LEDGER2_FORMULA_BALANCES @STATUSID, @STATUSNAME, @FR_DATE, @VDT, 0, @AMOUNT_PARAMETER, @EXCHANGE, @SEGMENT , 'M.CLTCODE', ''
...
SET @@SQL = @@SQL + " SELECT TB.CLTCODE, ACNAME, BRANCHCODE,	'" + @EXCHANGE + "','" +  @SEGMENT + "', " + @DISPLAY_AMOUNT + ", ACCAT, A.GRPCODE, GRPNAME='test' "
SET @@SQL = @@SQL + "FROM "
--SET @@SQL = @@SQL + "	#TB TB , ACMAST A /*, GRPMAST G */"
SET @@SQL = @@SQL + "	#TB TB INNER JOIN  ACMAST A on TB.CLTCODE = A.CLTCODE /*, GRPMAST G */"
SET @@SQL = @@SQL + " LEFT OUTER JOIN GRPMAST G ON  A.GRPCODE = G.GRPCODE "
...
--SET @@SQL = @@SQL + "UNION "
--SET @@SQL = @@SQL + " SELECT CLTCODE = 'ZZZZ', ACNAME = 'TOTAL', BRANCHCODE = '', '" + @EXCHANGE + "','" +  @SEGMENT + "', " + @NET_AMOUNT_PARAMETER + ", ACCAT = 0, GRPCODE = '', GRPNAME = '' "
--SET @@SQL = @@SQL + "FROM "
--SET @@SQL = @@SQL + "	#TB TB "
