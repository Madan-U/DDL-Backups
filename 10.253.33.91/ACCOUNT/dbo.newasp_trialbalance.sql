...
SELECT * INTO #LEDGER FROM LEDGER WHERE edt >= @curryrstdate and VDT <= @vdt + ' 23:59:59'
...
begin
   ...
   begin
      ...
      select @@fromtables = " from #LEDGER l INNER join acmast a on l.cltcode=  a.cltcode " 
   ...
   begin
      if @sortbydate = 'vdt'
         ...
         begin
            ...
            select @@selectqury = @@selectqury + " from #LEDGER l INNER join acmast a on l.cltcode=  a.cltcode " 
...
begin
   ...
   begin
      ...
      select @@fromtables = " from #LEDGER l, ledger2 l2 INNER join acmast a on l2.cltcode=  a.cltcode  " 
      select @@wherepart  = @@wherepart1 + @@addwhere
   end
   else if @openentryflag = 1  /* Opening entry ( vtyp = 18 ) found in Ledger for selected year */
   begin
...
l.vtyp <> 18 and l.vdt >= '" + @stdate + "' then ( case when upper(l2.drcr) = 'C' then camt else 0 end) else 0 end), opbal = sum(case when l.vdt < '" + @stdate + "' then ( case when upper(l2.drcr) = 'D' then camt else -camt end) else 0 end) "
           select @@fromtables = " from ledger2 l2, #LEDGER l INNER join acmast a on l.cltcode=  a.cltcode  " 
           select @@wherepart  = @@wherepart1 + @@addwhere
         end
   end
   else if @openentryflag = 1  /* Opening entry ( vtyp = 18 ) found in Ledger for selected year */
