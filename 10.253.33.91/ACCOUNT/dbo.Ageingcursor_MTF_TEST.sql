...
SELECT @@OPENBALDTMIN =  LEFT(CONVERT(VARCHAR,ISNULL(MIN(VDT),0),109),11) FROM VW_LEDGER_TEMP_NEW_TEST L
...
BEGIN
	/* --------------     GETTING  CLOSING BAL     ---------*/
	SELECT @@OPENBALDT=LEFT(CONVERT(VARCHAR,ISNULL(MAX(VDT),0),109),11) FROM VW_LEDGER_TEMP_NEW_TEST
	WHERE VTYP = '18' AND VDT < = @OPENDATE
	...
	BEGIN
		...
		FROM VW_LEDGER_TEMP_NEW_TEST L /*WITH (INDEX(LEDIND_TEMP))*/ 
	...
	BEGIN
		...
		CLOSBAL = SUM( CASE WHEN DRCR = 'D' THEN VAMT ELSE -VAMT END)
		FROM VW_LEDGER_TEMP_NEW_TEST_TEST /*WITH (INDEX(LEDIND_TEMP))*/ WHERE VDT >= @@OPENBALDT + ' 00:00:00' AND VDT <= @TODATE + ' 23:59:59'
...
/*		INSERT   INTO #TEMPAGEING1
	...
	BEGIN
		...
		SELECT L.CLTCODE,
		CLOSBAL = SUM( CASE WHEN DRCR = 'D' THEN VAMT ELSE -VAMT END)
		FROM VW_LEDGER_TEMP_NEW_TEST L /*WITH (INDEX(LEDIND_TEMP))*/ WHERE VDT >= @@OPENBALDT + ' 00:00:00' AND VDT <= @TODATE + ' 23:59:59'
	...
	BEGIN
		...
		BEGIN
			...
			FROM VW_LEDGER_TEMP_NEW_TEST L , #CLOSEBAL A  WITH(INDEX(INDXTMP))
    ...
    BEGIN
		...
		BEGIN
			...
			FROM VW_LEDGER_TEMP_NEW_TEST L , #CLOSEBAL A  WHERE A.CLTCODE = FAMILY
...
BEGIN
	/* --------------     GETTING  CLOSING BAL     ---------*/
	SELECT @@OPENBALDT=LEFT(CONVERT(VARCHAR,ISNULL(MAX(EDT),0),109),11) FROM VW_LEDGER_TEMP_NEW_TEST
	...
	BEGIN
		...
		SELECT L.CLTCODE,
		CLOSBAL = SUM( CASE WHEN DRCR = 'D' THEN VAMT ELSE -VAMT END)
		FROM VW_LEDGER_TEMP_NEW_TEST L /*WITH(INDEX(LED2IND_EDT_TEMP))*/ WHERE EDT >= @@OPENBALDT + ' 00:00:00' AND EDT <= @TODATE + ' 23:59:59'
		AND L.CLTCODE >= @FROMPARTY AND L.CLTCODE <= @TOPARTY AND BRANCH_CD LIKE @@BRANCHCODE+'%'
		AND @TODATE BETWEEN FROM_DATE AND TO_DATE
		GROUP BY L.CLTCODE
		HAVING ABS(SUM( CASE WHEN DRCR = 'D' THEN VAMT ELSE -VAMT END)) > @AMOUTGTTHAN
	...
	BEGIN
		...
		SELECT FAMILY AS CLTCODE,
		CLOSBAL = SUM( CASE WHEN DRCR = 'D' THEN VAMT ELSE -VAMT END)
		FROM VW_LEDGER_TEMP_NEW_TEST L /*WITH(INDEX(LED2IND_EDT_TEMP))*/
    ...
    BEGIN
		...
		BEGIN
			INSERT INTO #LEDGERTRAN
			SELECT CLTCODE=FAMILY, VTYP, VNO, EDT, VAMT, CLOSBAL, 0
			FROM VW_LEDGER_TEMP_NEW_TEST L , #CLOSEBAL A  WITH(INDEX(INDXTMP)) WHERE  A.CLTCODE = FAMILY
			AND CLOSBAL < 0 AND DRCR = ( CASE WHEN CLOSBAL >= 0 THEN 'D' ELSE 'C' END )
			AND EDT <= @TODATE  + ' 23:59:59'
			--    AND EDT > DATEADD(MONTH,-12,@@OPENBALDT)
			AND FAMILY >= @FROMPARTY AND FAMILY <= @TOPARTY
...
BEGIN
	PRINT 'ALL'
	insert INTO [TBL_MTF_SHORTAGE_SQOFF_DATA]
	...
	FROM #AGEINGLIST A1 WITH(INDEX(INDXTMP5)) ,   ACMAST A2 ,MSAJAG.DBO.CLIENT1 C1
	WHERE
