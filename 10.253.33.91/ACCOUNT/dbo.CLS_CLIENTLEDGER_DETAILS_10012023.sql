-- Object: PROCEDURE dbo.CLS_CLIENTLEDGER_DETAILS_10012023
...
CREATE  PROCEDURE [dbo].[CLS_CLIENTLEDGER_DETAILS_10012023]        
                @FDATE      VARCHAR(11),                 
                @TDATE      VARCHAR(11),             
                @FCODE      VARCHAR(10),        
                @TCODE      VARCHAR(10),        
...
---select * from #TMPLEDGERNEW where EXCHANGE = 'NSX' and segment = 'FUTURES'
...
SNO INT IDENTITY(1, 1),
 ...
 FROM   #TMPLEDGERNEW
 where NARRATION <> 'NET BALANCE' AND NARRATION <> 'TOTAL BALANCE'
 ...
 UPDATE #PARTYLEDGERCSV SET ENTITY = '' WHERE PERTIULARS IN('TOTAL BALANCE', 'NET BALANCE')
          ...
          BEGIN      
            ...
            IF @ORDERFLG = 'N'        
              BEGIN 
				...
				INSERT INTO #LEDGER_RUNNING_DATA    
			    SELECT    BOOKTYPE,VOUDT,EFFDT,VDT = CASE WHEN SHORTDESC = 'NTBAL' OR SHORTDESC = 'TBAL' THEN '' ELSE CONVERT(VARCHAR(11),VDT,103) END,
				EDT = CASE WHEN SHORTDESC = 'NTBAL' OR SHORTDESC = 'TBAL' THEN '' ELSE CONVERT(VARCHAR(11),EDT,103) END,
				SHORTDESC = CASE WHEN SHORTDESC = 'NTBAL' OR SHORTDESC = 'TBAL'  THEN '' ELSE SHORTDESC END,VNO,CROSAC,DDNO  = CASE WHEN DDNO = '0' THEN NULL ELSE DDNO END,
				NARRATION,DRAMT,CRAMT,CLTCODE,VTYP,ACNAME,OPBAL,L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_CITY,L_ZIP,RES_PHONE1,BRANCH_CD,EDIFF=ISNULL(EDIFF,0),FAMILY,SUB_BROKER,TRADER,CL_TYPE,
				BANK_NAME,CLTDPID,LNO,PDT,EMAIL,MOBILE_PAGER,PAN,EXCHANGE = t.EXCHANGE,SEGMENT = t.SEGMENT,ENTITY,ORDERSEG,ACTIVE_SEG,PARTYROWCOUNT,TOTALPAGES , 
				CLTCODE_TODISPLAY =  CASE WHEN SHORTDESC = 'NTBAL' OR SHORTDESC = 'TBAL' THEN '' ELSE CLTCODE END, 
				CHECKSTATUS = CASE WHEN SHORTDESC = 'NTBAL' OR SHORTDESC = 'TBAL' THEN '' ELSE (CASE WHEN EFFDT > @TDATE THEN 'U' ELSE '' END) END, 
				COLORFLAG = CASE WHEN EFFDT > CONVERT(VARCHAR(11),GETDATE(),109)+' 23:59' THEN '0' ELSE '1' END, ENTITY_DESC = ISNULL(C.SEGMENT_DESCRIPTION,'CHARGES ENTRIES'), 
				REPORT_DATE = CONVERT(VARCHAR(10), GETDATE(), 103), RUNNING_BAL = 0        
                FROM     #TMPLEDGERNEW T LEFT OUTER JOIN CLS_COMPANY_MASTER_SETTING C
				ON    T.EXCHANGE = C.EXCHANGE AND T.SEGMENT = C.SEGMENT	         
                ORDER BY cltcode,        
                         ORDERSEG,        
