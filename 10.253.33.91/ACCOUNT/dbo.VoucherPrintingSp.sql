...
/****** Object:  Stored Procedure dbo.VoucherPrintingSp    Script Date: 02/18/2003 10:37:43 AM ******/
...
/* Conditions of VNO only */
 		select   CltCode = ISNULL((CASE WHEN L.VTYP = 19 OR L.VTYP = 20  OR L.VTYP = 24 
  			...
  			acname= ISNULL((CASE WHEN L.VTYP = 19 OR L.VTYP = 20 OR L.VTYP = 24 
     					             THEN (SELECT ACNAME FROM MARGINLEDGER M , ACMAST A WHERE VTYP = L.VTYP AND m.VNO = L.VNO AND LNO = L.LNO AND M.BOOKTYPE = L.BOOKTYPE AND PARTY_CODE = L.CLTCODE) 
     					              ELSE L.acname END ),0),  
	 	 	...
	 	 	narr = isnull(l.narration ,'') ,/*isnull((select narr from ledger3 l3 where l3.vtyp = l.vtyp and l3.vno = l.vno and l3.naratno = l.lno and l3.booktype = l.booktype),'') ,*/
  			cnar = isnull(l.narration ,'') ,/*isnull((select narr from ledger3 l3 where l3.vtyp = l.vtyp and l3.vno = l.vno and l3.naratno = l.lno and l3.booktype = l.booktype),'') ,*/
			cltcode2 = l.cltcode, acname2 = l.acname,branchcode
  		from ledger l ,LEDGER1 L1,acmast ac
  		...
  		and l.vno >= @VnoFrom  and l.vno <= @VnoTo and l.cltcode=ac.cltcode
      ...
      begin
		...
		cltcode2 = l.cltcode, acname2 = l.acname , branchcode
	 	...
	 	and l1.vtyp = l.vtyp and l1.vno = l.vno and l1.booktype = l.booktype 
  		and vdt <=@EndDate + ' 23:59:59' and l.cltcode=ac.cltcode
