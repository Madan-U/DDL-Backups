-- Object: PROCEDURE dbo.ninserttoledger2
...
select @@vtype = (select distinct vtype from t10 
                  where rtrim(sessionid) = rtrim(@sessionid) and vno = @vno)
   
select @@costbreakup = (select count(*) from t10 
        where rtrim(sessionid) = rtrim(@sessionid) and category = 'BRANCH' 
        and vno = @vno and costflag = 'C')
/*
if @@vtype = 8 and @@costbreakup > 0 
begin
    delete from t10 where rtrim(sessionid) = rtrim(@sessionid) and upper(rtrim(branch)) = 'ALL'
    and vno = @vno and costflag = 'A' 
    and party_code in ( select distinct party_code from t10 where costflag = 'C' )
    ...
    from t10 left outer join branchaccounts b on upper(b.MainControlAc) = upper(party_code) and upper(b.branchname) = upper(branch)
    where sessionid = rtrim(@sessionid) and vno = @vno
    and party_code in ( select distinct MainControlAc from branchaccounts )

    update t40 set branch = b.branchname from branchaccounts b 
    where upper(b.BrControlAc) = upper(party_code) 

    update t40 set costcode = b.costcode from costmast b 
    where rtrim(upper(costname)) = rtrim(upper(branch)) 

    insert into t10 
    select category, branch, paidamt, vtype, vno, lno, drcr, costcode,
    booktype, sessionid, BrControlAc, costflag, rowid from t40
...
from t10 where category = 'BRANCH' and rtrim(sessionid) = rtrim(@sessionid) and vno =@vno and costflag = 'A'
open @@rcursor
...
begin
   ...
   select @@party2 = (select distinct party_code from t10 
      ...
      begin
        ...
        insert into t10
         select category, branch, paidamt, vtype, vno, 1, ( case when drcr = 'D' then 'C' else 'D' end), costcode,
             booktype, sessionid, @@party2, costflag, 1 from t10
             where rtrim(sessionid) = rtrim(@sessionid) and category = 'BRANCH' 
             and vno = @vno
      end
   ...
   begin
     if @statusid = 'BRANCH' 
        ...
        begin
         insert into tl2
         select vtype, vno, lno, drcr, paidamt, c.costcode, BookType,upper(party_code)
         from t10 t, Branchaccounts b, costmast c
   ...
   if @@OneBranch = 0
      ...
      if @@MultiBranch = 0
         Begin
            ...
            from t10 t, costmast c, branchaccounts b
            where rtrim(sessionid) = rtrim(@sessionid) and rtrim(branch) = rtrim(costname)
         ...
         if @@OneBranchAll = 0 
            begin
               set @@rcursor = cursor for
               select branch from t10 where category = 'BRANCH' and rtrim(sessionid) = rtrim(@sessionid)
