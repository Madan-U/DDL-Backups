-- Object: PROCEDURE dbo.CLS_CLIENTLEDGER_DETAILS_bkp_11042022
...
SET @@SQL = @@SQL +" MOBILE_PAGER, "        
 ...
 SET @@SQL = @@SQL +" EXEC "+@@ACCOUNTSVR+"."+@@ACCOUNTDB+".dbo.CLS_CLIENTLEDGER_SEG_DETAILS "        
 ELSE        
 SET @@SQL = @@SQL +" EXEC "+@@ACCOUNTDB+".dbo.CLS_CLIENTLEDGER_SEG_DETAILS "        
 SET @@SQL = @@SQL +"'" +@FDATE +"', "        
 SET @@SQL = @@SQL +"'" +@TDATE +"', "        
 SET @@SQL = @@SQL +"'" +@FCODE +"', "        
 SET @@SQL = @@SQL +"'" +@TCODE +"', "        
...
IF @@SERVERNAME <> @@ACCOUNTSVR        
SET @@SQL = @@SQL + "    EXEC "+ @@ACCOUNTSVR +"."+@@ACCOUNTDB+".dbo.CLS_RPT_ACC_MARGINLEDGER "        
ELSE        
SET @@SQL = @@SQL + "    EXEC "+@@ACCOUNTDB+".dbo.CLS_RPT_ACC_MARGINLEDGER "        
SET @@SQL = @@SQL +"'" + @FDATE +"', "        
SET @@SQL = @@SQL +"'" + @TDATE +"', "        
SET @@SQL = @@SQL +"'" + @FCODE +"', "        
SET @@SQL = @@SQL +"'" + @TCODE +"', "        
...
BEGIN
	...
	SET @@SQL = @@SQL +" PAN) "    
	IF @@SERVERNAME <> @@ACCOUNTSVR        
	SET @@SQL = @@SQL +" EXEC "+".ACCOUNT.dbo.CLS_RPT_ACC_CHARGELEDGER_WPARENT "        
	ELSE        
	SET @@SQL = @@SQL +" EXEC ACCOUNT.dbo.CLS_RPT_ACC_CHARGELEDGER_WPARENT "        
	SET @@SQL = @@SQL +"'" +@FDATE +"', "        
	SET @@SQL = @@SQL +"'" +@TDATE +"', "        
...
--			[RES_PHONE1]= B.MOBILE_PAGER
	from #TMPLEDGERNEW A
			inner join msajag.dbo.CLIENT_DETAILS B
...
--		FROM #TMPLEDGERNEW T WHERE NARRATION = 'OPENING BALANCE' AND NOT EXISTS(SELECT CLTCODE FROM  #FINOPBAL O WHERE T.CLTCODE = O.CLTCODE AND T.EXCHANGE = O.EXCHANGE AND T.SEGMENT = O.SEGMENT)
      END        
      UPDATE #TMPLEDGERNEW SET ACTIVE_SEG = EXCHLIST FROM MSAJAG.dbo.CLS_GET_CLIENT_FAM_EXCH_LIST C WHERE #TMPLEDGERNEW.CLTCODE = CL_CODE
