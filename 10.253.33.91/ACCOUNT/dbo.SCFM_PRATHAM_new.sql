-- Object: PROCEDURE dbo.SCFM_PRATHAM_new
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

     
    
 ---SCFM_PRATHAM_new 'Mar 30 2021'        
        
 CREATE proc [dbo].[SCFM_PRATHAM_new]  (            
...
WHERE VDT >=@RECSDATE AND VDT <=@todate +' 23:59' AND EDT <=@todate +' 23:59' AND L.VTYP =2  AND L.VTYP=L1.VTYP AND L.VNO=L1.VNO AND L.BOOKTYPE =L1.BOOKTYPE         
...
WHERE VDT >=@RECSDATE AND VDT <=@todate +' 23:59' AND EDT <=@todate +' 23:59' AND L.VTYP =2  AND L.VTYP=L1.VTYP 
AND L.VNO=L1.VNO AND L.BOOKTYPE =L1.BOOKTYPE    AND ENTEREDBY Not in ('B_TPR' ,'TPR')         
AND ( RELDT ='' OR RELDT >=@RECODATE)         
GROUP BY CLTCODE     
 ) A        
...
SELECT * FROM #TDAY_UNRECO        
		...
		----select * from #ab where cltcode ='A137777'
...
WHERE  CLTCODE LIKE '98%' AND CLTCODE=CL_cODE        
 ...
 where cltcode between 'a' and 'zzzz9999' and a.CLTCODE=m.cl_code group by cltcode            
          ...
          insert into temp_bal_new 
		  select * ,@todate as request_date,1 from #cc
