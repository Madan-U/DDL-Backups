-- Object: PROCEDURE dbo.RPT_ACC_PARTYLEDGER_PARENT_eq
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


CREATE PROCEDURE [dbo].[RPT_ACC_PARTYLEDGER_PARENT_eq]                                          
(                                            
...
BEGIN                                               
      ...
      BEGIN                                                  
         ...
         AND PARENTCODE > = @FCODE                
                        ...
                        AND VTYP = 18  and  B.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)                                           
                  GROUP BY PARENTCODE                                             
            END                                                  
            ELSE                                                  
            BEGIN                                                  
      ...
      AND PARENTCODE > = @FCODE                
                        ...
                        AND VDT < @FDATE and  B.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)                                               
               GROUP BY PARENTCODE                                             
            END                                          
  END                                                   
      ELSE                                                  
     ...
     FROM                                             
        ...
        WHERE                 
         ...
         AND PARENTCODE < = @TCODE                                               
                                    AND B.VDT < @@OPENDATE                                            
                                    AND EDT >= @@OPENDATE  and  B.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)                                              
                         GROUP BY PARENTCODE                                             
                        )                                             
                        T                                             
                  GROUP BY CLTCODE                                             
   ...
   FROM LEDGER L WITH(NOLOCK),                
      ...
      AND L.CLTCODE = C2.PARTY_CODE  and  L.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)              
      END                                                   
      ELSE                                                  
      BEGIN                                                  
            INSERT                               
...
BEGIN        
 /*GETTING OPENING BALANCE*/                                                  
      ...
      AND PARENTCODE > = @FCODE                
                        ...
                        AND B.VDT > = @@OPENDATE + ' 00:00:00'                                             
                        AND VDT < @FDATE              
      AND VTYP <> 8 and  B.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)                                        
               GROUP BY PARENTCODE                                             
            END                                          
  END                                                   
      ELSE                                                  
   ...
   CASE                                             
        ...
        AND PARENTCODE < = @TCODE                                              
                                AND EDT > = @@OPENDATE + ' 00:00:00'                                             
                                AND EDT < @FDATE                                             
                                AND VTYP NOT IN (8, 18)  and  B.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)        
      GROUP BY PARENTCODE                                             
                              UNION ALL                                             
         SELECT                                             
                                    CLTCODE = PARENTCODE,                                             
...
/*GETTING CLIENT LIST*/                             
 INSERT INTO #LEDGERCLIENTS                                          
   ...
   AND C2.PARENTCODE BETWEEN @FCODE AND @TCODE  and  C2.PARTY_CODE IN (SELECT CLTCODE FROM OLDNEWEQ)               
...
SUB_BROKER,TRADER,CL_TYPE,BANK_NAME,CLTDPID,LNO,PDT                
                  ...
                  WHERE L.CLTCODE = A.CLTCODE                                             
                        AND ACCAT IN ('4', '104')                                             
      AND (CRAMT <> 0 OR DRAMT <> 0 OR OPBAL <> 0) and  L.CLTCODE IN (SELECT CLTCODE FROM OLDNEWEQ)                                  
                  ORDER BY L.CLTCODE,                                          
                        VOUDT, PDT                                          
            END                                                  
            ELSE                                                  
