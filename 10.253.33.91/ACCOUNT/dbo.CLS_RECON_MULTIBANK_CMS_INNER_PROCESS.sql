-- Object: PROCEDURE dbo.CLS_RECON_MULTIBANK_CMS_INNER_PROCESS
...
SELECT
	@@CROSS_NO = ISNULL(MAX(CONVERT(BIGINT, CROSSREFERENCENO)), 0)--ISNULL(MAX(CROSSREFERENCENO), 0)
FROM BANKRECO(NOLOCK)



				DELETE FROM T 
				FROM TEMP_CMS_DATA T (NOLOCK) ,RECON_CMS_DATA R (NOLOCK)
				...
				IF EXISTS(SELECT * FROM  TEMP_CMS_DATA (NOLOCK) WHERE UPLOADID=@UPLOADID)			
...
CREATE CLUSTERED INDEX IDXR_LEDGER_TO_MAP_DUPLICATE ON #LEDGER_TO_MAP_DUPLICATE
(   
...
INSERT INTO #LEDGER_TO_MAP
	...
	---------add 91 days logic
			...
			AND L.LNO = L1.LNO
			AND L.BOOKTYPE = L1.BOOKTYPE
			AND	VDT>=DATEADD(DAY,-121,GETDATE())
			INNER JOIN (SELECT
			VNO, VTYP, BOOKTYPE
			FROM LEDGER (NOLOCK)
			WHERE CLTCODE = @BANKCODE AND	VDT>=DATEADD(DAY,-121,GETDATE())
			AND VTYP IN (2, 3, 17) 
			) L11
			ON L11.VNO = L1.VNO
			AND L11.VTYP = L1.VTYP
...
, CLTCODE, CLTACCNO INTO #CMS_DATA_DUPLICATE FROM TEMP_CMS_DATA (NOLOCK)
...
SELECT 	@TOTALFILERECORD = COUNT(*) FROM RECON_CMS_DATA (NOLOCK) WHERE UPLOADID = @UPLOADID
	...
	IF EXISTS(SELECT * FROM RECON_BANK_FILEUPLOAD_LOGS(NOLOCK) WHERE UPLOADID=@UPLOADID)
		...
		/*
			...
			*/
					...
					,L.UPLOADFILENAME=R.UPLOADFILENAME,L.UPLOADBY=R.UPLOADBY,L.VALUEDATE=@VALUEDATE,FILEUPLOAD_EDATE=GETDATE()
					FROM RECON_BANK_FILEUPLOAD_LOGS L (NOLOCK),RECON_BANK_FILEUPLOAD_LOGS_VW R (NOLOCK)
					WHERE L.UPLOADID=R.UPLOADID
					AND L.UPLOADID = @UPLOADID
	...
	ELSE
		BEGIN
			...
			WHERE UPLOADID=@UPLOADID
					...
					SET L.UPLOADSTATUS = 'SUCCESS', L.SEGMENT=@SEGMENT
					,L.UPLOADFILENAME=R.UPLOADFILENAME,L.UPLOADBY=R.UPLOADBY,L.VALUEDATE=@VALUEDATE,FILEUPLOAD_EDATE=GETDATE()
					FROM RECON_BANK_FILEUPLOAD_LOGS L (NOLOCK),RECON_BANK_FILEUPLOAD_LOGS_VW R
					WHERE L.UPLOADID=R.UPLOADID
					AND L.UPLOADID = @UPLOADID
...
BEGIN
IF EXISTS(SELECT * FROM RECON_BANK_FILEUPLOAD_LOGS(NOLOCK) WHERE UPLOADID=@UPLOADID)
			DELETE D FROM RECON_BANK_FILEUPLOAD_LOGS D(NOLOCK) WHERE UPLOADID = @UPLOADID

END

DELETE D FROM TEMP_CMS_DATA D (NOLOCK) WHERE UPLOADID=@UPLOADID
