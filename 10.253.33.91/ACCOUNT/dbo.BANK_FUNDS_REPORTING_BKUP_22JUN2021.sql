-- Object: PROCEDURE dbo.BANK_FUNDS_REPORTING_BKUP_22JUN2021
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


--EXEC BANK_FUNDS_REPORTING 'APR  1 2018', 'NOV 27 2019'
CREATE PROCEDURE [dbo].[BANK_FUNDS_REPORTING_BKUP_22JUN2021]
	@FR_DATE VARCHAR(11),
	@TO_DATE VARCHAR(11),
	@EXCHANGE VARCHAR(10),
	@SEGMENT VARCHAR(10),
...
FROM MSAJAG..VW_CLIENT_DETAILS_REPORTING 
...
FROM
	#CLIENT_DETAILS C, (SELECT CLTCODE, VAMT, DRCR, NARRATION, VDT, EDT, PDT, VTYP, VNO, BOOKTYPE FROM LEDGER WHERE VDT BETWEEN @FR_DATE AND @TO_DATE + ' 23:59' AND VTYP <> 18 ) L, VMAST V, 
	(SELECT CLTCODE = A.CLTCODE, VNO, VTYP, L2.BOOKTYPE FROM LEDGER L2, ACMAST A WHERE VDT BETWEEN @FR_DATE AND @TO_DATE + ' 23:59'  AND VTYP <> 18 AND L2.CLTCODE = A.CLTCODE AND ACCAT = 2 AND A.CLTCODE = CASE WHEN @BANK_CODE <> '' THEN @BANK_CODE ELSE A.CLTCODE END) LL
WHERE
	C.CL_CODE = L.CLTCODE
	AND L.VTYP = V.VTYPE
...
(
	...
	FROM LEDGER L, PARAMETER P, ACMAST A
	WHERE L.CLTCODE = A.CLTCODE AND ACCAT = 2 AND L.VDT >= SDTCUR AND VDT < @FR_DATE AND @FR_DATE BETWEEN SDTCUR AND LDTCUR AND VTYP <> 18
	GROUP BY L.CLTCODE
	UNION ALL
	SELECT CLTCODE = L.CLTCODE, AMOUNT = SUM(CASE WHEN DRCR = 'C' THEN VAMT ELSE -VAMT END)
	FROM LEDGER L, PARAMETER P, ACMAST A
	WHERE L.CLTCODE = A.CLTCODE AND ACCAT = 2 AND @FR_DATE BETWEEN SDTCUR AND LDTCUR
