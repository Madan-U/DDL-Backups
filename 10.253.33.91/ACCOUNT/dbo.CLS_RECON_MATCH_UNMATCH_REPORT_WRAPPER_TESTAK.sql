-- Object: PROCEDURE dbo.CLS_RECON_MATCH_UNMATCH_REPORT_WRAPPER_TESTAK
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


CREATE  PROCEDURE [dbo].[CLS_RECON_MATCH_UNMATCH_REPORT_WRAPPER_TESTAK]
(
...
EXEC [CLS_RECON_MATCH_UNMATCH_REPORT_TESTAK]	@FROM_DATE
										,@TO_DATE
										,@BANKCODE
										,@MATCHEDFLAG
										,@FILETYPE
...
WHILE (SELECT	COUNT(*)	FROM @TEMP_SEGMENT) > 0 
	...
	IF ( (@@ACCOUNTSERVER +( CASE WHEN REPLACE(@@SERVICENAME,'MSSQLSERVER', '') ='' THEN '' ELSE '\'+@@SERVICENAME  END ))<> @@SERVERNAME  )
		...
		+ @@ACCOUNTSERVER + "." + @@ACCOUNTDB+ ".DBO.CLS_RECON_MATCH_UNMATCH_REPORT_TESTAK '"+ @FROM_DATE + "','" + @TO_DATE + "','" + @BANKCODE+ "','" 
	...
	ELSE
		...
		SET @@SQL = ''
		SET @@SQL ="INSERT INTO #TEMP_REPORT_WRAPPER EXEC " 
		+ @@ACCOUNTDB+ ".DBO.CLS_RECON_MATCH_UNMATCH_REPORT_TESTAK '"+ @FROM_DATE + "','" + @TO_DATE + "','" + @BANKCODE+ "','" 
		+ @MATCHEDFLAG + "','" + @FILETYPE + "','" + @BANKNAME + "','" +@@SEGMENT + "','" + @PARAM_VAL1 + "','" +  @BULKDELETE + "','" +  @LOGINNAME + "'"				
		END		
	...
	END
		...
		SELECT
		FILEUPLOADID AS UPLOADID
		,CASE WHEN FILEFORMATE=1 THEN '' ELSE MAX(T.SEGMENT) END SEGMENT
		,BANK AS 'BANKNAME'
		,MAX(T.BANKACCOUNT)BANKACCOUNT
		,ARRANGEMENTTYPE AS FILETYPE
		,CONVERT(VARCHAR(10), MAX(VALUEDATE), 105) VALUEDATE --R.VALUEDATE
		,UPLOADFILENAME
		,UPLOADSTATUS
		,MIN(CONVERT(NVARCHAR, FILEUPLOAD_SDATE, 120)) FILEUPLOAD_SDATE
		,MAX(CONVERT(NVARCHAR, FILEUPLOAD_EDATE, 120)) FILEUPLOAD_EDATE
		FROM #TEMP_REPORT_WRAPPER T, RECON_BANK_MASTER_VW BM
		WHERE T.BANK=BM.BANKNAME
		AND T.SEGMENT=BM.SEGMENT
		AND T.ARRANGEMENTTYPE=BM.FILETYPE
		AND (T.ARRANGEMENTTYPE = @FILETYPE OR @FILETYPE='') 
		AND (BANK = @BANKNAME OR @BANKNAME='') 
		AND (T.SEGMENT = @SEGMENT OR @SEGMENT='')
		GROUP BY	FILEUPLOADID
		,BANK
		,ARRANGEMENTTYPE
		,UPLOADFILENAME
