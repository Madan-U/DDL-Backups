-- Object: PROCEDURE dbo.V2_OFFLINE_RECPAYUPLOAD_BULK_BAK_09012024
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


CREATE PROC [dbo].[V2_OFFLINE_RECPAYUPLOAD_BULK_BAK_09012024]                      
 @FNAME VARCHAR(100),                      
 @UNAME VARCHAR(25),                      
 @USERCAT SMALLINT,                  
 @EXCHANGE VARCHAR(3),                    
...
--CREATE NONCLUSTERED INDEX INX_RECPAY_TABLE ON #RECPAY_TABLE FROM(CLIENT_CODE,VNO);    

CREATE NONCLUSTERED INDEX [RECPAY_TABLE_TMP] ON [dbo].[#RECPAY_TABLE_TMP]  (SERIAL_NO,CLIENT_CODE);
 ...
 (                          
  ...
  FROM              
   ...
   LEDGER1 L1 WITH (NOLOCK),LEDGER L              
    WITH(NOLOCK)              
  WHERE              
   L.VDT>= CONVERT(VARCHAR(11),GETDATE()-45,120)
     AND  L.CLTCODE=RP.CLIENT_CODE  
   AND L.VNO=L1.VNO
   AND L.VTYP=L1.VTYP
   AND L.BOOKTYPE=L1.BOOKTYPE   AND L1.DDNO = RP.REF_NO  
    AND L1.VTYP IN (2,3)
  ...
  WHERE      
    L1.vdate>= CONVERT(VARCHAR(11),GETDATE()-45,120)   
