-- Object: PROCEDURE dbo.acc_reports_NEW
...
CREATE procedure acc_reports_NEW
...
begin
   ...
   if rtrim(@branch) = '' or rtrim(@branch) = '%' 
      begin
         ...
         select @@fromtables = " From (select distinct vtyp, vno, booktype from ledger where cltcode = '" + @fcode + "' AND " + @@datefilter1 + " and vtyp <> 18 ) t, ledger l "
         select @@wherepart = "where l.cltcode <> '" + @fcode + "' and l.vtyp = t.vtyp and l.vno = t.vno and l.booktype = t.booktype "
         select @@groupby = " "
      end
   else
      begin
	 ...
	 select @@fromtables = " From (select distinct vtyp, vno, booktype from ledger where cltcode = '" + @fcode + "' AND " + @@datefilter1 + " and vtyp <> 18 ) t, ledger2 l2, costmast c, ledger l "
         select @@wherepart = "where l.cltcode <> '" + @fcode + "' and l.vtyp = t.vtyp and l.vno = t.vno and l.booktype = t.booktype and l2.vtype = l.vtyp and l2.vno = l.vno and l2.lno = l.lno and l2.booktype = l.booktype and costname = rtrim('" + @branch + "') and l2.costcode = c.costcode and l2.cltcode =l.cltcode "
         select @@groupby = " " 
...
begin
   ...
   if rtrim(@branch) = '' or rtrim(@branch) = '%' 
      ...
      begin
/*         select @@selectqury = "SELECT l.booktype, Vdt = convert(varchar,l.vdt,103), effdt = l.edt, Edt = convert(varchar,l.edt,103), l2.cltcode, acname, L.VNO, l.vtyp, voudt=l.vdt, ddno= isnull((select max(ddno) from ledger1 l1 where l1.vno=l.vno and l1.vty
...
and rtrim(costname) = rtrim('" + @branch + "') and l2.costcode = c.costcode and l2.cltcode =l.cltcode " 
         ...
         select @@fromtables = " From (select distinct vtype, vno, booktype from ledger2 where cltcode = '" + @fcode + "'  AND " + @@datefilter1 + " and vtyp <> 18 ) t, ledger2 l2, costmast c, ledger l " 
/*         select @@wherepart = @@datefilter + " and l.vtyp <> 18 and l.cltcode <> '" + @fcode + "' and l.vtyp = t.vtyp and l.vno = t.vno and l.booktype = t.booktype and l2.vtype = l.vtyp and l2.vno = l.vno and l2.lno = l.lno and l2.booktype = l.booktype 
and rtrim(costname) = rtrim('" + @branch + "') and l2.costcode = c.costcode and l2.cltcode not in ( select brcontrolac from branchaccounts ) " */
         select @@wherepart = " where l2.cltcode <> '" + @fcode + "' and l.vtyp = t.vtype and l.vno = t.vno and l.booktype = t.booktype and l2.vtype = l.vtyp and l2.vno = l.vno and l2.lno = l.lno and l2.booktype = l.booktype and rtrim(costname) = rtrim('" + @branch + "') and l2.costcode = c.costcode " 
         select @@groupby = " " 
      end
end
...
begin
 ...
 if left(@@sortby,6) <> ' order' 
   ...
   begin
      if rtrim(@branch) = '' or rtrim(@branch) = '%' 
         begin
            select @@selectqury = "select l.booktype, voudt=l.vdt, effdt=l.edt, isnull(shortdesc,'') shortdesc, dramt=(case when upper(l.drcr) = 'D' then vamt else 0 end),  cramt=(case when upper(l.drcr) = 'C' then vamt else 0 end), l.vno,ddno= isnull((select max(ddno) from ledger1 l1 where l1.vno=l.vno and l1.vtyp=l.vtyp and l1.booktype=l.booktype and l1.lno = l.lno),''), Narration = replace(replace(l.Narration,'''',''),'""','') , l.cltcode, a.longname, vtyp, l.booktype, convert(varchar,l.vdt,103) vdt, convert(varchar,l.edt,103) edt, l.Acname, ediff=datediff(d,l.edt,getdate()) "
