-- Object: PROCEDURE dbo.GET_LEDGER_AUDIT_DATA_BAK_30012023
...
CREATE PROCEDURE [dbo].[GET_LEDGER_AUDIT_DATA_BAK_30012023]      
 @EFFDATE VARCHAR(11),    
...
FROM       
 ...
 (SELECT  CLTCODE,VDT,VTYP,EDT,DRCR,VAMT  FROM LEDGER (NOLOCK) WHERE VDT <= @EFFDATE + ' 23:59' AND
 --(VDT >= @START_DATE OR 
 EDT >= @START_DATE
  --AND CDT <= DATEADD(MI, -15, GETDATE())
 --)      
 --AND  --AND CDT <= DATEADD(MI, -15, GETDATE())
 )    L 
 GROUP BY CLTCODE /*    
