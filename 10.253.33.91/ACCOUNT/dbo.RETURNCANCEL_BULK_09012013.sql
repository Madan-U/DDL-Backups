-- Object: PROCEDURE dbo.RETURNCANCEL_BULK_09012013
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

create PROC [dbo].[RETURNCANCEL_BULK_09012013]                            
 @VTYPE SMALLINT,              
 @FNAME VARCHAR(100),                          
 @UNAME VARCHAR(25),                          
 @USERCAT SMALLINT                          
...
INSERT INTO #BANK      
SELECT DISTINCT LOC_DESCRIPTION FROM #RECPAY_TABLE_TMP      
...
FROM                          
 ...
 (                          
  SELECT                                       
   CLTCODE = L.CLTCODE, BANK_CODE = L11.CLTCODE, DDNO, RELAMT, VNO = MAX(L.VNO), VTYP = MAX(L.VTYP), BOOKTYPE = MAX(L.BOOKTYPE)                                      
  FROM                                       
   LEDGER L,                                       
   LEDGER1 L1,      
   (SELECT VNO, VTYP, BOOKTYPE, CLTCODE FROM LEDGER L12 WHERE VTYP = 2 AND LNO = 1 AND EXISTS(SELECT CLTCODE FROM #BANK B WHERE L12.CLTCODE = B.CLTCODE)) L11      
  WHERE                                       
   L.VNO = L1.VNO AND L.VTYP = L1.VTYP AND L.BOOKTYPE = L1.BOOKTYPE AND L.LNO = L1.LNO                                      
   AND L.VNO = L11.VNO AND L.VTYP = L11.VTYP AND L.BOOKTYPE = L11.BOOKTYPE       
   AND L1.CLEAR_MODE <> 'R' AND L.VTYP = 2      
  GROUP BY L.CLTCODE, L11.CLTCODE, DDNO, RELAMT                                      
  HAVING COUNT(1) = 1                                      
 ) R1                                      
 ON CLTCODE = ENRICH_VALUE  AND INST_NMBR = DDNO AND CONVERT(MONEY, INSTRUMENT_AMNT) = RELAMT AND BANK_CODE = LOC_DESCRIPTION                          
 ...
 IF @@ERROR_COUNT > 1                                              
  ...
  END  */     
   ...
   /*--------------------------VALIDATION WITH USERRIGHTS TABLE ------------------------*/            
    ...
    SELECT                          
     @@ERROR_COUNT = ISNULL(COUNT(INST_DATE), 0)                          
    FROM                          
     #RECPAY_TABLE                          
    WHERE                          
     INST_DATE < @@BACKDATE            
...
BEGIN                           
 ...
 UNION ALL                          
 SELECT ENRICH_VALUE + ' , ' +  INST_NMBR + ' , ' + CONVERT(VARCHAR, INSTRUMENT_AMNT) + ' , ' + LOC_DESCRIPTION AS RESULT                          
 FROM             
  #RECPAY_TABLE                          
 WHERE                           
  VNO IS NULL                          
