-- Object: PROCEDURE dbo.pms_cldetails_sta1
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

CREATE procedure pms_cldetails_sta1 (@pmscode as varchar(10),@pmsname as varchar(50), @fdate as datetime, @tdate as datetime, @afdate as datetime, @loca as varchar(25))
as
...
--(select clientid=upper(a.party_code),befffrom=a.fromdate,location=c.tradername from bsedb_ab.dbo.angeltermbrok a, 
--intranet.pms_st.dbo.clientmast cm, bsedb_ab.dbo.client1 b, (select * from intranet.risk.dbo.branch where tradername=@loca)c where cm.clientid =a.party_Code 
--and a.party_code=b.cl_code and b.branch_cd=c.sbtag and todate >= @tdate ) b
...
(
select party_code,eff_from,not_pl=sum(case when netqty > 0 then open_position-(netqty*cls) else 0 end),
...
cls=isnull(cls,case when sum(pqtydel-sqtydel) <> 0 then sum(pamtdel-samtdel)/sum(pqtydel-sqtydel) else 0 end)
from 
(select a.*, b.cls from anand1.msajag.dbo.cmbillvalan a left outer join 
(select x.* from intranet.risk.dbo.mdcumm x, (select scrip, mfdate=max(mfdate) 
from intranet.risk.dbo.mdcumm where mfdate <= @tdate+' 23:59:59' group by scrip ) y 
where x.scrip=y.scrip and y.mfdate=x.mfdate
) b on b.scrip=a.scrip_cd ) CM, 
(select * from intranet.PMS_ST.DBO.CLIENTMAST WHERE PMSCODE=@pmscode and eff_to >= @tdate and location LIKE @loca) CL
--(select clientid=upper(a.party_code),eff_from=a.fromdate,location=c.tradername from bsedb_ab.dbo.angeltermbrok a, 
--intranet.pms_st.dbo.clientmast cm, bsedb_ab.dbo.client1 b, (select * from intranet.risk.dbo.branch where tradername=@loca)c where cm.clientid =a.party_Code 
--and a.party_code=b.cl_code and b.branch_cd=c.sbtag and todate >= @tdate ) CL
where cl.clientid=cm.party_Code and sauda_date >= cl.eff_from+' 00:00:00.00' and sauda_date <= @tdate+' 23:59:59.000'
group by party_code, cl.eff_from,scrip_cd,cls
) tvalan
/*
, 
(
