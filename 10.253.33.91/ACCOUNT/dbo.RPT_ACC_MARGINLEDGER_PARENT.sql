-- Object: PROCEDURE dbo.RPT_ACC_MARGINLEDGER_PARENT
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

--EXEC RPT_ACC_MARGINLEDGER_PARENT 'APR  1 2008', 'JAN  5 2009', '0A141', '0A143', 'ACCODE', 'VDT', 'BROKER', 'BROKER', ''  
CREATE PROCEDURE [dbo].[RPT_ACC_MARGINLEDGER_PARENT]     
...
BEGIN                                    
 ...
 BEGIN                                    
  ...
  FROM MARGINLEDGER M, MSAJAG.DBO.CLIENT2 C2    
...
BEGIN                              
 ...
 AND M.LNO = L.LNO LEFT OUTER JOIN ACMAST A ON M.PARTY_CODE = A.CLTCODE , VMAST V, MSAJAG.DBO.CLIENT2 C2                             
  WHERE     
 L.VDT >= @FDATE + ' 00:00:00' AND L.VDT <= @TDATE + ' 23:59:59' AND M.PARTY_CODE = C2.PARTY_CODE    
 AND PARENTCODE >= @FCODE AND PARENTCODE <= @TCODE AND M.VTYP <> 18 AND ACCAT = '4' AND M.VTYP = V.VTYPE                               
END                              
ELSE                              
BEGIN                              
  INSERT INTO #MARGINLEDGERDATA                              
...
SELECT     
 C2.PARTY_CODE,BANK_NAME =ISNULL(C4.BANKID,''),                                    
 L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_CITY,L_ZIP,RES_PHONE1,C1.BRANCH_CD,                            
 FAMILY,C1.SUB_BROKER,TRADER,CL_TYPE,CLTDPID =ISNULL(CLTDPID,''), PARENTCODE    
FROM     
 MSAJAG.DBO.CLIENT1 C1 ,MSAJAG.DBO.CLIENT2 C2 LEFT OUTER JOIN MSAJAG.DBO.CLIENT4  C4 ON                                    
...
IF @STRORDER = 'ACCODE'                                    
 BEGIN                                    
  IF @SELECTBY = 'VDT'                                    
   BEGIN                                     
    SELECT L.BOOKTYPE,VOUDT,EFFDT,SHORTDESC,DRAMT,CRAMT,VNO,DDNO,NARRATION,CLTCODE=L.PARENTCODE,VTYP,VDT,EDT,L.ACNAME,OPBAL,L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_CITY,L_ZIP,RES_PHONE1,BRANCH_CD,CROSAC,EDIFF,FAMILY,SUB_BROKER,TRADER,CL_TYPE,BANK_NAME,CLTDPID,LNO,PDT 
	FROM #TMPMLEDGER L,ACMAST A  WHERE  L.CLTCODE = A.CLTCODE AND ACCAT IN ('4','104') 
	AND (CRAMT <> 0 OR DRAMT <> 0 OR OPBAL <> 0) ORDER BY L.CLTCODE,VOUDT                                    
   END                                    
  ELSE                                    
   BEGIN                                    
    SELECT L.BOOKTYPE,VOUDT,EFFDT,SHORTDESC,DRAMT,CRAMT,VNO,DDNO,NARRATION,CLTCODE=L.PARENTCODE,VTYP,VDT,EDT,L.ACNAME,OPBAL,L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_CITY,L_ZIP,RES_PHONE1,BRANCH_CD,CROSAC,EDIFF,FAMILY,SUB_BROKER,TRADER,CL_TYPE,BANK_NAME,CLTDPID,LNO,PDT 
	FROM #TMPMLEDGER L,ACMAST A   WHERE  L.CLTCODE = A.CLTCODE AND ACCAT IN ('4','104') 
	AND (CRAMT <> 0 OR DRAMT <> 0 OR OPBAL <> 0)ORDER BY L.CLTCODE,EFFDT                                    
   END                                    
 END               
ELSE                                    
  BEGIN                                    
   IF @SELECTBY = 'VDT'                                    
    BEGIN                                     
     SELECT L.BOOKTYPE,VOUDT,EFFDT,SHORTDESC,DRAMT,CRAMT,VNO,DDNO,NARRATION,CLTCODE=L.PARENTCODE,VTYP,VDT,EDT,L.ACNAME,OPBAL,L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_CITY,L_ZIP,RES_PHONE1,BRANCH_CD,CROSAC,EDIFF,FAMILY,SUB_BROKER,TRADER,CL_TYPE,BANK_NAME,CLTDPID,LNO,PDT 
	 FROM #TMPMLEDGER L,ACMAST A   WHERE L.CLTCODE = A.CLTCODE AND ACCAT IN ('4','104') 
	 AND (CRAMT <> 0 OR DRAMT <> 0 OR OPBAL <> 0) ORDER BY L.ACNAME,VOUDT                                    
    END                                    
   ELSE                                    
    BEGIN                                    
     SELECT L.BOOKTYPE,VOUDT,EFFDT,SHORTDESC,DRAMT,CRAMT,VNO,DDNO,NARRATION,CLTCODE=L.PARENTCODE,VTYP,VDT,EDT,L.ACNAME,OPBAL,L_ADDRESS1,L_ADDRESS2,L_ADDRESS3,L_CITY,L_ZIP,RES_PHONE1,BRANCH_CD,CROSAC,EDIFF,FAMILY,SUB_BROKER,TRADER,CL_TYPE,BANK_NAME,CLTDPID,LNO,PDT 
	 FROM #TMPMLEDGER L,ACMAST A  WHERE L.CLTCODE = A.CLTCODE AND ACCAT IN ('4','104') 
	 AND (CRAMT <> 0 OR DRAMT <> 0 OR OPBAL <> 0) ORDER BY L.ACNAME,EFFDT                                    
    END                                    
  END                                    
