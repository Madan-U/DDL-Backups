-- Object: PROCEDURE dbo.VIRTUAL_MIS_LEDGER_CONVERTER_org_09112017
...
EXEC VIRTUAL_MIS_LEDGER_CONVERTER 'D:\BACKOFFICE\12022016.12.022016.05.20pm.CSV', 'SURESH'                                            
...
*/   
create PROCEDURE [dbo].[VIRTUAL_MIS_LEDGER_CONVERTER_org_09112017] @FILENAME VARCHAR(1000),   
                                                     @UNAME    VARCHAR(50)   
AS   
...
--PRINT @@SQL 
 ...
 INSERT INTO #VIRTUAL_MIS   
    SELECT CONVERT(datetime, BUSINESS_DATE),   
         ...
         --  .DBO.PIECE(TRANSACTION_DETAILS1, '|', 1),   
		 case when left(TRANSACTION_DETAILS1,4)='imps' then .DBO.piece(TRANSACTION_DETAILS1, '/', 3) else .DBO.PIECE(TRANSACTION_DETAILS1, '|', 1)end  ,                                  
           'HO',   
           'ALL',   
           'C',   
    ...
    WHERE   
      ...
      NOT EXISTS(SELECT TXN_REF_NO   
                 FROM   VIRTUAL_MIS_LEDGER_REPORT   
                 WHERE  .DBO.PIECE(TRANSACTION_DETAILS1, '|' , 1)     
                        =   
                        TXN_REF_NO)   
