-- Object: PROCEDURE dbo.V2_OFFLINE_JVDRCR_UPLOAD_APR092015
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


CREATE PROC [dbo].[V2_OFFLINE_JVDRCR_UPLOAD_APR092015]        
...
/* '--------------------------VALIDATION FOR SINGLE VOUCHER TYPE BASED ON DRCR ------------------------*/                                                
 ...
 SELECT @@ERROR_COUNT = F1 FROM #FEXIST                                       IF @@ERROR_COUNT = 0                                                
  BEGIN                                                
 ...
 --AND NOT EXISTS (SELECT PARTY_CODE, INACTIVEFROM FROM BSEDB_AB.DBO.CLIENT5 C5 WITH(NOLOCK), BSEDB_AB.DBO.CLIENT2 C2 WITH(NOLOCK)                                  
            
--WHERE LTRIM(RTRIM(C2.CL_CODE)) = C5.CL_CODE AND DATEADD(DAY,@@ACTIVEDAYS,INACTIVEFROM) <= GETDATE() AND A.CLTCODE = PARTY_CODE)                                                
             
--   AND NOT EXISTS (SELECT DATEADD(DAY,@@ACTIVEDAYS,INACTIVEFROM) , PARTY_CODE, INACTIVEFROM FROM BSEDB_AB.DBO.CLIENT5 C5 WITH(NOLOCK), BSEDB_AB.DBO.CLIENT2 C2 WITH(NOLOCK)                                   
--WHERE LTRIM(RTRIM(C2.CL_CODE)) = C5.CL_CODE AND CASE WHEN INACTIVEFROM LIKE 'DEC 31 2049%' THEN INACTIVEFROM ELSE DATEADD(DAY,@@ACTIVEDAYS,INACTIVEFROM) END <= GETDATE())                                  
  ...
  UPDFLAG = 'Y',                                                
     ...
     COSTCODE = (SELECT TOP 1 COSTCODE FROM COSTMAST WHERE COSTNAME = 'HO')                                                
        ...
        AND A.ACCAT IN ('3','4','104','18')                                     
        AND A.BRANCHCODE = 'ALL'                                         
  --AND NOT EXISTS (SELECT PARTY_CODE, INACTIVEFROM FROM BSEDB_AB.DBO.CLIENT5 C5 WITH(NOLOCK), BSEDB_AB.DBO.CLIENT2 C2 WITH(NOLOCK) WHERE LTRIM(RTRIM(C2.CL_CODE)) = C5.CL_CODE AND DATEADD(DAY,@@ACTIVEDAYS,INACTIVEFROM) <= GETDATE() AND A.CLTCODE = PARTY_CODE)                                                
 -- AND NOT EXISTS (SELECT DATEADD(DAY,@@ACTIVEDAYS,INACTIVEFROM) , PARTY_CODE, INACTIVEFROM FROM BSEDB_AB.DBO.CLIENT5 C5 WITH(NOLOCK), BSEDB_AB.DBO.CLIENT2 C2 WITH(NOLOCK)                                   
--WHERE LTRIM(RTRIM(C2.CL_CODE)) = C5.CL_CODE AND CASE WHEN INACTIVEFROM LIKE 'DEC 31 2049%' THEN INACTIVEFROM ELSE DATEADD(DAY,@@ACTIVEDAYS,INACTIVEFROM) END <= GETDATE())                                  
...
/* '--------------------------FINAL VALIDATION BASED ON UPDFLAG ------------------------*/                                                
 ...
 SELECT                                                 
  ...
  CASE WHEN DRCR = 'C' THEN AMOUNT ELSE 0 END, CASE WHEN DRCR = 'D' THEN AMOUNT ELSE 0 END, NARRATION,BRANCHCODE,                                                 
