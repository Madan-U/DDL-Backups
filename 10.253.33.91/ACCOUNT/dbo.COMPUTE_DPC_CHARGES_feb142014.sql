-- Object: PROCEDURE dbo.COMPUTE_DPC_CHARGES_feb142014
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

create PROCEDURE [dbo].[COMPUTE_DPC_CHARGES_feb142014]          
(          
...
SELECT @FROM_DATE = DATEADD(D, -5, @FROM_DATE)          
...
(          
 SELECT          
  ...
  CL_TYPE          
 FROM          
  LEDGER L (NOLOCK), DPC_CLIENTS_CHARGES C (NOLOCK)          
 WHERE          
 ...
 SELECT          
  ...
  CL_TYPE          
 FROM          
  LEDGER L(NOLOCK), DPC_CLIENTS_CHARGES C(NOLOCK), MSAJAG.DBO.GET_SETT_CALENDER(@@OPEN_DATE, @TO_DATE, 4)          
 WHERE          
  L.CLTCODE = PARTY_CODE          
  AND CONVERT(VARCHAR(11), VDT, 109) = CONVERT(VARCHAR(11), START_DATE, 109)          
  AND END_DATE >= @@OPEN_DATE          
  AND END_DATE < DATEADD(D, 5, @FROM_DATE)          
...
DECLARE          
      @DAYS INT          
      SELECT @DAYS = DATEDIFF(DD,@FROM_DATE,@TO_DATE) +11          
...
FROM          
 #DPC_CHARGES D,          
 (SELECT          
           DPC_DATE = DATEADD(DD,NUMBER-1,DATEADD(D, -10, @FROM_DATE))          
      FROM          
            DBO.F_TABLE_NUMBER_RANGE( 1, @DAYS )          
  ) DT          
...
SELECT DISTINCT          
 ...
 AMOUNT_REV = 0,          
 DPC_PERCENTE          
FROM (SELECT DISTINCT CLTCODE, DPC_DATE,LONG_NAME,          
 BRANCH_CD,          
 SUB_BROKER,          
 TRADER,          
 REGION,          
 AREA,          
 CL_TYPE, DPC_PERCENTE FROM #ALL_DATES) A WHERE NOT EXISTS (SELECT DPC_DATE FROM #DPC_CHARGES TEMP WHERE A.CLTCODE = TEMP.CLTCODE AND A.DPC_DATE = TEMP.DPC_DATE)          
          
          
UPDATE D SET           
 DPC_BALANCE = (SELECT SUM(DPC_BALANCE) FROM #DPC_CHARGES D1 WHERE D.CLTCODE = D1.CLTCODE AND D1.DPC_DATE <= D.DPC_DATE)          
FROM #DPC_CHARGES D          
WHERE DPC_DATE >= @FROM_DATE          
...
WHERE D.CLTCODE = D1.CLTCODE AND T_DATE = DPC_DATE      
AND EXCHANGE = @EXCHANGE AND SEGMENT = @SEGMENT      
          
UPDATE D SET VDT_BALANCE = D1.VDT_BALANCE          
FROM DEBIT_DEPC_BILLS D, (SELECT CLTCODE, VDT_BALANCE FROM #DPC_CHARGES D1 WHERE DPC_DATE LIKE @FROM_DATE + '%') D1          
...
WHERE          
 ...
 AND DPC_DATE >= DATEADD(D, 5, @FROM_DATE)      
...
/*--INSERT INTO DPC_CHARGES (DPC_DATE, CLTCODE, LONG_NAME, BRANCH_CD, SUB_BROKER, TRADER, REGION, AREA, CL_TYPE, DPC_BALANCE, LEDGER_BALANCE, CASH_COLLATREAL, NONCASH_COLLATREAL, MARGIN_REQ, DPC_CHARGE, DPC_PERCENTE, AMOUNT_REV, EXCHANGE, SEGMENT, ENTITY,SEG_GROUP)          
SELECT          
