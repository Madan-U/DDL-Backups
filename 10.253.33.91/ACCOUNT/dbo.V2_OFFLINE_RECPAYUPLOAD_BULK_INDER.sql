-- Object: PROCEDURE dbo.V2_OFFLINE_RECPAYUPLOAD_BULK_INDER
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------



CREATE PROC [dbo].[V2_OFFLINE_RECPAYUPLOAD_BULK_INDER]                      
 @FNAME VARCHAR(100),                      
 @UNAME VARCHAR(25),                      
 @USERCAT SMALLINT,                  
 @EXCHANGE VARCHAR(3),                    
...
IF @@ERROR_COUNT > 0              
 ...
 END                 
   ...
   /*--------------------------VALIDATION FOR INACTIVE CLIENTS------------------------*/                      
 SELECT @@ERROR_COUNT = COUNT(1)                      
 FROM #RECPAY_TABLE RP , BSEDB_AB.DBO.CLIENT5 C5  WITH (NOLOCK)                 
 WHERE RP.CLIENT_CODE = C5.CL_CODE  
 AND C5.INACTIVEFROM <=GETDATE()      
              
            IF @@ERROR_COUNT > 0                      
             BEGIN                      
               SELECT 'FILE CANNOT BE UPLOADED AS THE FOLLOWING CLIENTS ARE INACTIVE IN MASTER', 'NA','NA'              
  UNION ALL              
  SELECT DISTINCT              
   RP.CLIENT_CODE, 'NA','NA'    
   FROM #RECPAY_TABLE RP , BSEDB_AB.DBO.CLIENT5 C5  WITH (NOLOCK)                 
 WHERE RP.CLIENT_CODE = C5.CL_CODE  
 AND C5.INACTIVEFROM <=GETDATE()   
