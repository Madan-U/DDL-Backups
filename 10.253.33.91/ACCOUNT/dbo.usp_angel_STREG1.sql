...
(select cltcode,vno,vamt,vtyp from ledger  where cltcode like '5%' and vno in (select vno from #t))b, #file1 a       where a.vno=b.vno  and a.vtyp=b.vtyp                               
----------------------------------------------------------------        
--select distinct x.vNo,x.cltcode,x.vamt,x.acname,x.vtyp into #temp2 from        
--(        
--select cltcode,Vno,vamt,acname,vtyp from ledger x (nolock) where cltcode like '5%' and exists        
...
(select cltcode,vno,vamt,acname,vtyp from ledger  where cltcode like '5%' and vno in ( select vno from #t))b,       
#file1 a            
 where a.vno=b.vno  and a.vtyp=b.vtyp                  
-------------------------------------        
--select distinct x.vNo,x.cltcode,x.vamt,x.acname into #temp1 from        
--(        
--select cltcode,Vno,vamt,acname from ledger x (nolock) where cltcode like '5%' and exists        
--(select * from #t y where x.vno = y.vno)        
--)x        
--inner join        
--(select * from #file1)y        
...
(        
select cltcode,Vno from ledger x (nolock) where exists        
(select * from #t y where x.vno = y.vno)        
)x        
inner join        
(select * from #file1)y        
...
(select cltcode,vno,vamt from ledger  where cltcode =@code and vno in ( select vno from #t))b,#file1 a             
where a.vno=b.vno                                                   
...
(select cltcode,vno,vamt from ledger  where cltcode =@code2 and vno in ( select vno from #t))b,#file1 a                  
where a.vno=b.vno                                    
