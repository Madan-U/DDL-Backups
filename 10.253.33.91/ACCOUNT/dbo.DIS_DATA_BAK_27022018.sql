-- Object: PROCEDURE dbo.DIS_DATA_BAK_27022018
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


--select * from [172.31.16.94].DMAT.CITRUS_USR.TBL_CLIENT_MASTER  
--DIS_DATA 'MAY 30 2016' , 'MAY 30 2016'          
          
CREATE PROC [dbo].[DIS_DATA_BAK_27022018]            
(            
  @FROMDATE varchar(11),            
  @TODATE varchar(11)            
 )                     
...
set @FROMDATE=convert(DATETIME,@FROMDATE,103)    
 ...
 [EMAIL_ADD]=A.EMAIL_ADD  
  into #TEMP2         
 FROM            
 [172.31.16.94].DMAT.CITRUS_USR.TBL_CLIENT_MASTER A         
  WHERE   A.ACTIVE_DATE >= @FROMDATE AND A.ACTIVE_DATE < =@TODATE + ' 23:59'                  
     
            
SELECT client_code,NISE_PARTY_CODE=isnull(NISE_PARTY_CODE,''),ACTIVE_DATE, SUB_TYPE,
 EMAIL_ADD,  
DIS_FLAG=isnull(dis_flag,''),APPLICATION_NO=isnull(APPLICATION_NO,'') FROM #TEMP2 T        
LEFT OUTER JOIN         
[172.31.16.57].CRMDB_A.DBO.API_KYC C        
