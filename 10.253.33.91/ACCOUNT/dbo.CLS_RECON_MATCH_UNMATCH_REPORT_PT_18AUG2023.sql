-- Object: PROCEDURE dbo.CLS_RECON_MATCH_UNMATCH_REPORT_PT_18AUG2023
...
CREATE  PROCEDURE [dbo].[CLS_RECON_MATCH_UNMATCH_REPORT_PT_18AUG2023]
		(
...
SET @NONCMS_MATCHBY='CHQNO + AMT'
			...
			--SELECT @FILTERDATE=CONVERT(VARCHAR(11), SDTCUR, 109) FROM PARAMETER WHERE   CONVERT(DATETIME,@FROM_DATE,103) BETWEEN SDTCUR		AND LDTCUR
			SET @FILTERDATE = DATEADD(DAY,-91,GETDATE())

			IF(@FILETYPE ='NON-CMS' OR @FILETYPE='TECHPROCESS' OR @FILETYPE='' OR @FILETYPE='BILLDESK' OR @FILETYPE='SBPAYOUT' OR @FILETYPE='AXISPAYOUT' OR  @FILETYPE='FEDERALPAYOUT')
			BEGIN			
			...
			END
				...
				CREATE NONCLUSTERED INDEX IXD_REPORTFLAG ON #TEMP_REPORT(REPORT_FLAG)
				CREATE NONCLUSTERED INDEX IXD_FILEUPLOADID ON #TEMP_REPORT(FILEUPLOADID)
...
BEGIN 
	...
	BEGIN TRAN
		...
		IF(@FILETYPE='NON-CMS' OR @FILETYPE='TECHPROCESS' OR  @FILETYPE='BILLDESK'  OR @FILETYPE='SBPAYOUT')
		BEGIN
		INSERT INTO RECON_BANK_DELETION_LOGS (UPLOADID, SEGMENT, BANKCODE, BANKNAME, FILETYPE, BANKACCOUNT, UPLOADFILENAME
		, MODIFIEDDATE, UPLOADSTATUS, UPLOADBY, FILEUPLOAD_SDATE, FILEUPLOAD_EDATE, VALUEDATE, ROWTYPE, DRCR, REFERENCENO
		, AMOUNT, DRAWER_NAME, CLTCODE, CLTACCNO, VNO, VTYP, BOOKTYPE, MATCHEDFLAG, UPLOADEDDATE, BOOKDATE, CROSS_NO
