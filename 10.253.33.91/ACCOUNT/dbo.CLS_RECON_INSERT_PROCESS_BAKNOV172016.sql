-- Object: PROCEDURE dbo.CLS_RECON_INSERT_PROCESS_BAKNOV172016
...
IF(@BANKNAME='YES BANK' OR @BANKNAME='STANDARD CHARTERED BANK')
	BEGIN
...
IF (@BANKNAME = 'ICICI BANK') PRINT 'EXEC CLS_RECON_CMS_ICICI_PROCESS @UPLOADID'

IF (@BANKNAME = 'KOTAK MAHINDRA BANK') PRINT 'EXEC CLS_RECON_CMS_KOTAK_PROCESS @UPLOADID'
...
IF (@BANKNAME <> 'YES BANK' OR @REPROCESS_FLAG = 'R') BEGIN
...
IF EXISTS(SELECT * FROM  #CMS_DATA)
	INSERT INTO EXISTS_RECORDS(RECORDSTATUS)VALUES(0)
	ELSE
	INSERT INTO EXISTS_RECORDS(RECORDSTATUS)VALUES(1)
...
IF (@BANKNAME = 'YES BANK') BEGIN
INSERT INTO @TEMPBANKACCOUNT
	SELECT DISTINCT
		ROWTYPE AS BANKACCOUNT
	FROM #CMS_DATA
...
SET @@SQL = ''
SET @@SQL = "INSERT INTO " +
+@@ACCOUNTSERVER + "." + @@ACCOUNTDB + ".[DBO].TEMP_CMS_DATA
										SELECT UPLOADID
										, "+@BANKCODE+"
										, ROWTYPE
										, VALUEDATE
...
END ELSE IF (@FILETYPE = 'NON-CMS') BEGIN
...
IF (@BANKNAME = 'ICICI BANK') EXEC CLS_RECON_NONCMS_ICICI_PROCESS @UPLOADID,@BANKCODE

IF (@BANKNAME = 'KOTAK MAHINDRA BANK') PRINT 'EXEC CLS_RECON_NONCMS_KOTAK_PROCESS @UPLOADID'
...
IF (@BANKNAME <> 'STANDARD CHARTERED BANK' OR @REPROCESS_FLAG = 'R') 
BEGIN
...
BEGIN
	...
	SELECT	@@CROSS_NO = ISNULL(MAX(CROSSREFERENCENO), 0)FROM BANKRECO(NOLOCK)
...
IF (@BANKNAME = 'STANDARD CHARTERED BANK' AND @REPROCESS_FLAG <> 'R') 
BEGIN
...
END ELSE PRINT 'PLEASE CHECK SUPPLY PARAMETERS'
