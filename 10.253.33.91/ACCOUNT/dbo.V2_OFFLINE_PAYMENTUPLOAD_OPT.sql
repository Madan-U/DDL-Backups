-- Object: PROCEDURE dbo.V2_OFFLINE_PAYMENTUPLOAD_OPT
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------



 --EXEC  V2_OFFLINE_PAYMENTUPLOAD_OPT 'ONLINE',61,'NSE', 'CAPITAL','JUN  3 2022', 'N'
    
      
CREATE PROC [dbo].[V2_OFFLINE_PAYMENTUPLOAD_OPT](                                
...
AS                                
 ...
 AND SEGMENT = @SEGMENT ORDER BY EXCHANGE                               
  ...
  SET @@SQLPARA = @@SQLPARA + "            ACC_NO,VTYP) "
			  
  IF @@ACCOUNTSVR <> 'AngelBSECM'                    
  BEGIN  
                 
  SET @@SQLPARA = @@SQLPARA + "  EXEC AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_FILL_REC_PAYMENT "+CONVERT(VARCHAR,@@VTYP)+",'"+@EXCHANGE+"', '"+@SEGMENT+"', '"+@IP_VDATE+"', '" + @UNAME + "'"                
  END                    
 ELSE                    
  BEGIN    
              
  SET @@SQLPARA = @@SQLPARA + " EXEC V2_OFFLINE_FILL_REC_PAYMENT "+CONVERT(VARCHAR,@@VTYP)+",'"+@EXCHANGE+"', '"+@SEGMENT+"', '"+@IP_VDATE+"',
  '"  + @UNAME + "'"           
  END            
    --PRINT @@SQLPARA           
 EXEC(@@SQLPARA)                                 
...
--return                        
  ...
  IF @@VNOFLAG <> 0                                
    BEGIN                                
     ...
     EXEC AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_UPLOAD_PUTLOG  @EXCHANGE, @SEGMENT, 3, @ERRORCODE, @MESSAGE, @UNAME                         
     RETURN                                
        END                                
    END                                
                 set @@BRANCHFLAG = 0               
...
IF @@ACCOUNTSVR <> 'AngelBSECM'          
 BEGIN                     
 SET @@SQLPARA = " EXEC AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_REV_UPDATE_PAYMENT '" + @@NEWVNO + "','" + @EXCHANGE + "','" + @SEGMENT + "' "            
 END                    
ELSE                    
 BEGIN                    
 SET @@SQLPARA = " EXEC V2_OFFLINE_REV_UPDATE_PAYMENT '" + @@NEWVNO + "','" + @EXCHANGE + "','" + @SEGMENT + "' "            
END               
--print @@SQLPARA                 
EXEC(@@SQLPARA)                                
