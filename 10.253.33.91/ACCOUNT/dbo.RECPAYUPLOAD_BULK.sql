-- Object: PROCEDURE dbo.RECPAYUPLOAD_BULK
...
CREATE TABLE [#RECPAY_TABLE]              
 ...
 CREATE NONCLUSTERED INDEX [RECPAY_TABLE_TMP] ON [dbo].[#RECPAY_TABLE_TMP]  (SERIAL_NO,CLIENT_CODE,REF_NO);

CREATE NONCLUSTERED INDEX [RECPAY_TABLE] ON [dbo].[#RECPAY_TABLE]  (SERIAL_NO,CLIENT_CODE,REF_NO);    
...
--select * from #RECPAY_TABLE    
--where voucher_Date > convert(varchar(11),getdate(),103)    
--return    
...
Set @DDATE = CONVERT(VARCHAR(11),GETDATE()-7,120)

SELECT * into #DDLEDGER FROM LEDGER L(NOLOCK) WHERE VDT >=@DDATE  AND L.VTYP IN (2,3)
AND EXiSTS (SELECT CLIENT_CODE FROM #RECPAY_TABLE WHERE CLIENT_CODE=CLTCODE)
...
FROM              
 (              
  ...
  FROM              
   #RECPAY_TABLE RP,              
   LEDGER1 L1 WITH (NOLOCK),#DDLEDGER L  WITH(NOLOCK)              
  WHERE  L.VDT>= CONVERT(VARCHAR(11),GETDATE()-7,120)  AND L1.VTYP IN (2,3)
    AND L.VNO=L1.VNO
   AND L.VTYP=L1.VTYP
   AND L.BOOKTYPE=L1.BOOKTYPE  
  AND  L1.DDNO = RP.REF_NO      AND  L.CLTCODE=RP.CLIENT_CODE     
