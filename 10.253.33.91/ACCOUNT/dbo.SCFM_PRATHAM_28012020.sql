-- Object: PROCEDURE dbo.SCFM_PRATHAM_28012020
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

--SP_HELPTEXT SCFM_PRATHAM_28012020
--SCFM_PRATHAM_bak


   ---sp_helptext SCFM_PRATHAM_28012020 'JAN  1 2019' 
 ---SCFM_PRATHAM 'JAN  1 2019'        
        
 CREATE  proc [dbo].[SCFM_PRATHAM_28012020]  (            
...
select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)                  
 ...
 select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)                  
 into #MTF                  
 from MTFTRADE.dbo.ledger where vdt >='2020-04-01 00:00:00'                 
 and edt <=@todate  + ' 23:59'        
 group by cltcode           
 ...
 select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)                  
 into #mcx        
 from ANGELCOMMODITY.accountMcdx.dbo.ledger where vdt >='2020-04-01 00:00:00'                  
 and edt <=@todate +' 23:59'        
 ...
 select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)                  
 into #ncx                  
 from ANGELCOMMODITY.accountncdx.dbo.ledger where vdt >='2020-04-01 00:00:00'                  
 and edt <=@todate +' 23:59'        
  and cltcode between 'a' and 'zzzz9999'                  
 group by cltcode          
  ...
  select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)                  
 into #BSEFO                  
 from ANGELCOMMODITY.accountBFO.dbo.ledger where vdt >='2020-04-01 00:00:00'                  
 and edt <=@todate +' 23:59'        
  and cltcode between 'a' and 'zzzz9999'              
 group by cltcode          
   ...
   select cltcode,NETAMT=sum(case when drcr='D' then -VAMT else VAMT end)                  
 into #BSX                 
 from ANGELCOMMODITY.accountCURBFO.dbo.ledger where vdt >='2020-04-01 00:00:00'                  
 and edt <=@todate +' 23:59'        
  and cltcode between 'a' and 'zzzz9999'                  
 group by cltcode              
...
--select convert(varchar,(sum(balance)/10000000)) +' - CR' AS Balance from #CC where balance >0            
-- union all          
--select convert(varchar,(sum(balance)/10000000)) +' - DR' AS Balance from #CC where balance <0            
