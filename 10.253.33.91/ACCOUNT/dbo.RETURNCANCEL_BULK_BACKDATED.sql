-- Object: PROCEDURE dbo.RETURNCANCEL_BULK_BACKDATED
...
--RENAME this SP under SRE-35257

---RETURNCANCEL_BULK_NEW
--RETURNCANCEL_BULK_BKP_20220907
CREATE PROC [dbo].[RETURNCANCEL_BULK_BACKDATED] 
(
  @VTYPE   SMALLINT, 
  @FNAME   VARCHAR(100), 
  @UNAME   VARCHAR(25), 
 ...
 IF OBJECT_ID('tempdb..#tmpInternalLdger_1') IS NOT NULL
          ...
          SELECT CLTCODE, 
                 ...
                 INTO #tmpInternalLdger_1
                                  FROM   LEDGER L WITH (NOLOCK)
                                         INNER JOIN  LEDGER1 L1  WITH(NOLOCK) ON L.VNO = L1.VNO AND L.VTYP = L1.VTYP AND L.BOOKTYPE = L1.BOOKTYPE AND L.LNO = L1.LNO  
                                  WHERE   L.VDT >DATEADD(DD,-100 ,GETDATE())
                                       --   AND dddt >DATEADD(DD,-10 ,GETDATE())
                                         AND L1.CLEAR_MODE <> 'C' 
                                         AND L.VTYP = 3 
                                         AND L1.RELDT = '' 
...
END 
    ...
    WHERE  L.VNO = R.VNO 
           ...
           AND VDT > DATEADD(DD, -100 ,GETDATE())
