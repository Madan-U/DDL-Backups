-- Object: PROCEDURE dbo.CLS_RECON_BANK_FILEUPLOAD_LOGS_ASONDEC06122016
...
CREATE PROCEDURE [dbo].[CLS_RECON_BANK_FILEUPLOAD_LOGS_ASONDEC06122016] (
...
BEGIN
		DECLARE @@SQL           VARCHAR(MAX),
				...
				@MASTERBANKCODE NVARCHAR(50)

SELECT @COMBINEFILE = FILEFORMATE,@MASTERBANKCODE = BANKCODE
...
IF ((@BANKNAME = 'HDFC BANK') OR (@BANKNAME = 'YES BANK' AND @FILETYPE = 'NON-CMS')) BEGIN

IF (CHARINDEX(@BANKACCOUNT, @UPLOADFILENAME) = 1 AND @BANKCODE=@MASTERBANKCODE)  
BEGIN
SET @PROCESS = 'Y'
...
IF @UPLOAD_FLAG = 'B' EXEC CLS_RECON_BULKUPLOD_FILES	@UPLOADID
														,@FILE_PATH
														,@UNAME
														,@SEGMENT
														,@COMBINEFILE
...
SELECT
	'YOU HAVE SELECTED WORNG SEGMENT FILE'
RETURN
END
END ELSE IF ((@BANKNAME = 'YES BANK' AND @FILETYPE = 'CMS') OR @BANKNAME = 'STANDARD CHARTERED BANK') BEGIN
...
END
IF (@PROCESS = 'Y' AND @REPROCESS_FLAG <> 'R') 
...
DECLARE @MBANKACCOUNT NVARCHAR(50)
	...
	BEGIN
		...
		SELECT @SEGMENT=SEGMENT,@BANKNAME=BANKNAME,@BANKCODE=BANKCODE,@MBANKACCOUNT=BANKACCOUNT,@FILETYPE=FILETYPE FROM @TEMPBANKMASTER
