-- Object: PROCEDURE dbo.PAYMENT_DATA
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

 --EXEC PAYMENT_DATA '18/03/2021','19/03/2021'  
                  
CREATE PROC [dbo].[PAYMENT_DATA] (  
  @FROMDATE VARCHAR(11),   
  @TODATE   VARCHAR(11)  
...
TRUNCATE TABLE PAYMENT_SSRS_DATA  
  BEGIN   
      SELECT *   
      INTO   #temp   
...
INSERT INTO PAYMENT_SSRS_DATA  
    ...
    DECLARE @FILE VARCHAR(MAX),@PATH VARCHAR(MAX) = 'J:\Backoffice\Automation\PAYMENT_DATA\'                       
SET @FILE = @PATH + 'PAYMENT_DATA' +'_'+ CONVERT(VARCHAR(11),GETDATE() , 112) + '.csv' --Folder Name     
DECLARE @S VARCHAR(MAX)                              
SET @S = 'EXEC MASTER.DBO.XP_CMDSHELL ''BCP "SELECT ''''CLTCODE'''',''''VDT'''',''''VDT_TIME'''',''''VNO'''',''''NARRATION'''',''''EXCHANGE'''',''''VAMT'''',''''DRCR'''',''''DDNO'''',''''VTYP'''',''''RELDT'''',''''Entry_DAte'''',''''ENTEREDBY'''',''''BOOK
TYPE'''',''''SHORT_NAME'''',''''REGION'''',''''BRANCH_CD'''',''''SUB_BROKER'''',''''BANK_NAME'''',''''AC_NUM'''''    --Column Name    
SET @S = @S + ' UNION ALL SELECT  cast([CLTCODE] as varchar),CONVERT (VARCHAR (11),VDT,109) as VDT,CONVERT (VARCHAR (11),VDT_TIME,109) as VDT_TIME, cast([VNO] as varchar), cast([NARRATION] as varchar), cast([EXCHANGE] as varchar), cast([VAMT] as varchar),
 cast([DRCR] as varchar),cast([DDNO] as varchar),cast([VTYP] as varchar),CONVERT (VARCHAR (11),RELDT,109) as RELDT,CONVERT (VARCHAR (11),Entry_DAte,109) as Entry_DAte ,cast([ENTEREDBY] as varchar),cast([BOOKTYPE] as varchar),cast([SHORT_NAME] as varchar),
cast([REGION] as varchar),cast([BRANCH_CD] as varchar) ,cast([SUB_BROKER] as varchar),cast([BANK_NAME] as varchar),cast([AC_NUM] as varchar) FROM [ACCOUNT].[dbo].[PAYMENT_SSRS_DATA]    " QUERYOUT ' --Convert data type if required    
