-- Object: PROCEDURE dbo.newasp_trialbalance_HIS
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

CREATE PROCEDURE  newasp_trialbalance_HIS
...
begin
   ...
   begin
      ...
      select @@fromtables = " from ledger_HIS l left outer join acmast a on l.cltcode=  a.cltcode " 
   ...
   begin
      if @sortbydate = 'vdt'
         ...
         begin
            ...
            select @@selectqury = @@selectqury + " from ledger_HIS l left outer join acmast a on l.cltcode=  a.cltcode " 
...
begin
   ...
   begin
      ...
      select @@fromtables = " from ledger_HIS l, ledger2_HIS l2 left outer join acmast a on l2.cltcode=  a.cltcode  " 
      select @@wherepart  = @@wherepart1 + @@addwhere
   end
   else if @openentryflag = 1  /* Opening entry ( vtyp = 18 ) found in Ledger for selected year */
   begin
   ...
   begin
      if @curryrstdate = @stdate 
         ...
         begin
           ...
           select @@fromtables = " from ledger2_HIS l2, ledger_HIS l left outer join acmast a on l.cltcode=  a.cltcode  " 
           select @@wherepart  = @@wherepart1 + @@addwhere
         end
   end
   else if @openentryflag = 1  /* Opening entry ( vtyp = 18 ) found in Ledger for selected year */
