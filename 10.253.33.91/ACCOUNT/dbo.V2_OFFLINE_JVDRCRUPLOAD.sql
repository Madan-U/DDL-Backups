-- Object: PROCEDURE dbo.V2_OFFLINE_JVDRCRUPLOAD
...
/*        
begin tran        
exec V2_OFFLINE_JVDRCRUPLOAD 'OFFLINE',1,'BSE','CAPITAL','sep  6 2010','N',8        
rollback        
*/         
...
IF ( (@@ACCOUNTSVR +( CASE WHEN REPLACE(@@SERVICENAME,'MSSQLSERVER', '') ='' THEN '' ELSE '\'+@@SERVICENAME  END ))<> @@SERVERNAME  )
 BEGIN             
SET @@SQLPARA = @@SQLPARA + "  FROM   " + @@ACCOUNTSVR + ".AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES O  "            
SET @@SQLPARA = @@SQLPARA + "   LEFT OUTER JOIN "+ @@ACCOUNTSVR +".AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_CC_ENTRIES CC "            
SET @@SQLPARA = @@SQLPARA + "   ON "            
SET @@SQLPARA = @@SQLPARA + "    O.FLDAUTO = CC.VOLE_REFNO "            
END            
ELSE            
 BEGIN            
SET @@SQLPARA = @@SQLPARA + " FROM   AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES O "            
SET @@SQLPARA = @@SQLPARA + "   LEFT OUTER JOIN AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_CC_ENTRIES CC "            
SET @@SQLPARA = @@SQLPARA + "   ON "            
SET @@SQLPARA = @@SQLPARA + "     O.FLDAUTO = CC.VOLE_REFNO "            
END            
...
IF ( (@@ACCOUNTSVR +( CASE WHEN REPLACE(@@SERVICENAME,'MSSQLSERVER', '') ='' THEN '' ELSE '\'+@@SERVICENAME  END ))<> @@SERVERNAME  )
 BEGIN            
 SET @@SQLPARA = "   UPDATE " + @@ACCOUNTSVR + ".AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES "            
 END            
ELSE            
 BEGIN            
 SET @@SQLPARA = "   UPDATE AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_LEDGER_ENTRIES  "            
END             
...
EXEC(@@SQLPARA)    
  ...
  IF @@VNOFLAG <> 0            
    BEGIN            
     ...
     EXEC AngelBSECM.ACCOUNT_AB.DBO.V2_OFFLINE_UPLOAD_PUTLOG  @EXCHANGE, @SEGMENT, 2, @ERRORCODE, @MESSAGE, @UNAME            
     RETURN            
        END            
    END            
...
UPDATE #RECPAY_TABLE SET LNO = LNO + (SELECT SUM(SNO) FROM #RECPAY_TABLE L1 WHERE #RECPAY_TABLE.VNO = L1.VNO AND #RECPAY_TABLE.SERIAL_NO <= L1.SERIAL_NO)    
