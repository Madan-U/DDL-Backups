-- Object: PROCEDURE dbo.acc_PrintReconcile_BKP_06APR2022
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------


    
CREATE PROCEDURE  [dbo].[acc_PrintReconcile_BKP_06APR2022]                      
@code varchar(10),                    
...
ON [PRIMARY]    
  ...
  AND EXISTS(SELECT VNO FROM #LEDGER_BANK LL WHERE L1.VNO = LL.VNO AND L1.VTYP = LL.VTYP AND L1.BOOKTYPE = LL.BOOKTYPE)
	...
	--INSERT INTO #LED1
		...
		AND L.CLTCODE=@code
		AND RELDT= CONVERT(DATETIME,CONVERT(VARCHAR(11),@tdate,109) ,103) 
		AND EDT>RELDT
		AND RELDT<>''
		ORDER BY     
