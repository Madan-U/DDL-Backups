-- Object: PROCEDURE dbo.VIRTUAL_MIS_LEDGER_CONVERTER_BKP_20230128
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

CREATE  PROCEDURE [dbo].[VIRTUAL_MIS_LEDGER_CONVERTER_BKP_20230128] 
(
...
set xact_abort on 
 ...
 return 0;   --- temporarary Commented by Ahsan Farooqui 
  ...
  WHERE  SRNO <= 2
    ...
    SELECT * INTO #VIRTUAL_MIS_LEDGER_tmp from VIRTUAL_MIS_LEDGER  
 ...
 INSERT INTO #VIRTUAL_MIS_LEDGER_tmp     
    ...
    VA_Number,Channel,Transaction_Amount,Transaction_Details1,Transaction_Details2,Transaction_Details3,Transaction_Details4,
    Transaction_Details5,Transaction_Details6,Transaction_Details7,Transaction_Details8,Transaction_Details9,Transaction_Details10,
...
--END AS Business_Date  INTO ##CLIENTS_KEYWORD from #VIRTUAL_MIS_LEDGER_tmp -- WHERE ISDATE(CAST(Business_Date AS DATE)) =1
  ...
  IF OBJECT_ID('tempdb..VIRTUAL_MIS_LEDGER_REPORT_11') IS NOT NULL
    DROP TABLE #VIRTUAL_MIS_LEDGER_REPORT_11
     Select * INTO #VIRTUAL_MIS_LEDGER_REPORT_11 from VIRTUAL_MIS_LEDGER_REPORT   WHERE TXN_DATE  = (SELECT MAX(CONVERT(datetime, BUSINESS_DATE)) FROm VIRTUAL_MIS_LEDGER WITH(NOLOCK))
 ...
 /*End 2022-07-23*/
  ...
  ---------------- END 
   ...
   --  APPROVALFLAG =1,  
    ...
    where 
    NOT EXISTS
    (SELECT CL_CODE FROM MSAJAG.DBO.CLIENT5 B WHERE  B.CL_CODE= A.CLTCODE)
