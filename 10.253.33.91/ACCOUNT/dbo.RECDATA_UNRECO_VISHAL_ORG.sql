-- Object: PROCEDURE dbo.RECDATA_UNRECO_VISHAL_ORG
-- Server: 10.253.33.91 | DB: ACCOUNT
--------------------------------------------------

      
      
--EXEC RECDATA_UNRECO_VISHAL_Test_03012021 'APR  1 2019', 'JUN 30 2019'        
        
        
        
                        
CREATE PROC [dbo].[RECDATA_UNRECO_VISHAL_ORG] (        
  @FROMDATE VARCHAR(11),         
  @TODATE   VARCHAR(11)        
...
[VAMT] =L.vamt,         
 ...
 L.drcr,         
   ...
   N1.[SACCOUNTNO],  -- = ACCOUNTNO,                                                                                
    ...
    FROM ANGELFO.BBO_FA.DBO.MFSS_LEDGER_BSE L (NOLOCK) , ANGELFO.BBO_FA.DBO.ACC_TBL4  T4 WITH(NOLOCK)  ,ANGELFO.BBO_FA.DBO.ACC_TBL1   T1   WITH(NOLOCK), INTRANET.MutualFund.dbo.vw_SIPPayinTransdetails N1 with (Nolock)                               
...
update f set ac_num = CltAccNo from #FINAL f (nolock), [AngelFO].accountcurfo.dbo.Recon_cms_data r  With(Nolock)      
 ...
 select * into #aa from  INTRANET.cms.dbo.ncms_segpayoutappamt with(nolock) where processdatetime>=@FROMDATE and processdatetime<= @TODATE + ' 23:59'  and SegApprovedAmt>0        
 ...
 select distinct porequestid,party_code,Req_datetime,PO_ApprovedAmt as amount,Req_PayBankId,Req_RefNo into #ncms        
  from  INTRANET.cms.dbo.ncms_po_request_hist with(nolock) where  po_statusid in (2,4)        
 ...
 a.req_refno=b.req_refno and a.requestid=b.porequestid        
  ...
  select CltCode,AccNO,AMT,dd_No=space(30), GeneratedDate=convert(varchar(11),GeneratedDate,120) from MIS.sccs.dbo.SCCS_ONFT_BankFile_hist with(nolock)  where Generateddate between @frmdate and @TODATE + ' 23:50:50'        
  union all        
  select Party_code,AccNum as AccNo,Amount,dd_No=space(30),GeneratedDate=convert(varchar(11),Updated_on,120) from MIS.sccs.dbo.sccs_INGV_BankFile_hist with(nolock)  where updated_on between @frmdate and @TODATE + ' 23:50:50'        
  union all        
  select CltCode,AccNO,AmT,dd_No=space(30), GeneratedDate=convert(varchar(11),GeneratedDate,120) from MIS.sccs.dbo.SCCS_neft_BankFile_hist with(nolock)  where Generateddate between @frmdate and @TODATE + ' 23:50:50'        
  Union all        
...
Create index #s on #tmp1(Cltcode,vtyp)        
  ...
  Select distinct exchange,Cltcode from #FINAL With(Nolock)  ) A      
...
sum(Case when Drcr='D' Then Vamt Else Vamt *-1 end)  Else 0 End) as Receipt,''         
 from ledger_all With(Nolock) where vdt =@fdate  and VTYP =18      
...
(Case When sum(Case when Drcr='D' Then Vamt Else Vamt *-1 end) >0 Then         
sum(Case when Drcr='D' Then Vamt Else Vamt *-1 end)  Else 0 End) as Receipt,''         
 from ledger_all With(Nolock) where vdt>=@fdate and vdt< @FROMDATE        
 Group by Exchange,Cltcode        
  END      
...
set @NSEQUERY = @NSEQUERY + ' from anand1.Account.dbo.Bfile  order by Sno " queryout j:\BankBook\BankBook_'+convert(varchar,@bank_code+'-'+@fexchange)+'.txt -c -t"," -Sanand1 -Uaolinhouse -Pe$$gnfDTVs2455GZAcc'        
