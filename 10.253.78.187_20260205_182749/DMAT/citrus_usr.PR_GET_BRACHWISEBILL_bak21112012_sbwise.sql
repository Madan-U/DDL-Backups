-- Object: PROCEDURE citrus_usr.PR_GET_BRACHWISEBILL_bak21112012_sbwise
...
CREATE PROC [citrus_usr].[PR_GET_BRACHWISEBILL_bak21112012_sbwise](@PA_FROM_DT DATETIME, @PA_TO_DT DATETIME)  
AS  
BEGIN  
...
exec PR_GET_BRACHWISEBILL_bak21112012_sbwise_brwisefilter  @pa_from_dt,@pa_to_dt
return
...
select br.entm_short_name brcdopn_corp,isnull(sb.entm_short_name,'') sbcdopn_corp ,count(distinct b.dpam_sba_no) noofacctopn_corp into #acctopn_corp from #tmp_dp_acct_mstr a ,#tempdata b  left outer join entity_relationship   
on entr_sba = b.dpam_sba_no   
left outer join entity_mstr br on  (entr_br = br.entm_id or entr_sb = br.entm_id )   
left outer join entity_mstr sb on  (entr_dummy1 = sb.entm_id or entr_dummy3 = sb.entm_id )   
where  accp_value between @PA_FROM_DT and @PA_TO_DT   
