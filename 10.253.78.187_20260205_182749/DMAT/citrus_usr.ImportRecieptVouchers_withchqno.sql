-- Object: PROCEDURE citrus_usr.ImportRecieptVouchers_withchqno
...
CREATE Proc [citrus_usr].[ImportRecieptVouchers_withchqno]
@pa_task_id bigint,
@pa_dpmid int,
@pa_finid int,
...
select top 1 @@filler = TMPREC_FILLER from TMP_RECIEPT_MSTR 

print @@voucher_dt 
print @pa_billing_from_dt

select @@narr = coalesce(@@narr + ',', '') + CONVERT(VARCHAR,TMPREC_SR_NO-1) from TMP_RECIEPT_MSTR  where isnull(TMPREC_NARRATION,'')=''
if isnull(@@narr,'') <> ''
begin 
--
UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : Narration is blank in line no ' + @@narr
...
select  isnull(TMPREC_FILLER,'') filler ,count(1) cnt into #TMP_RECIEPT_MSTR  from TMP_RECIEPT_MSTR WHERE isnull(TMPREC_FILLER,'')<>'' group by isnull(TMPREC_FILLER,'')
select @l_count = count(1) from #TMP_RECIEPT_MSTR

if @l_count>1
begin 
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : Single Bulk JV file should not be upload with multiple filler'
	WHERE  task_id = @pa_task_id
    drop table #TMP_RECIEPT_MSTR
	RETURN
end 


select  isnull(TMPREC_VTYPE,'') TMPREC_VTYPE ,count(1) cnt into #TMP_RECIEPT_MSTR1  from TMP_RECIEPT_MSTR WHERE isnull(TMPREC_FILLER,'')<>'' group by isnull(TMPREC_VTYPE,'')
select @l_count = count(1) from #TMP_RECIEPT_MSTR1
if @l_count>1
begin 
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : Single Bulk Payment file should not be upload with multiple voucher type'
	WHERE  task_id = @pa_task_id
    drop table #TMP_RECIEPT_MSTR1
	RETURN
end 

if exists (select TMPREC_SR_NO from tmp_reciept_mstr where TMPREC_CREDIT='')
begin 
--
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : In reciept upload credit column should not be blank, please verify & reimport'
	WHERE  task_id = @pa_task_id
...
DECLARE @RES VARCHAR(8000)
	...
	select @res = coalesce(@res + ',', '') +  CONVERT(VARCHAR,TMPREC_SR_NO-1) from TMP_RECIEPT_MSTR 
	  WHERE ISNULL(TMPREC_VTYPE,'')=''
 if @res<>''
begin
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : VOUCHER TYPE was found missing on Following  Line no  ' + @RES 
	WHERE  task_id = @pa_task_id
...
select @res = coalesce(@res + ',', '') +  CONVERT(VARCHAR,TMPREC_SR_NO-1) from TMP_RECIEPT_MSTR 
	  WHERE ISNULL(TMPREC_BANKGLCODE,'')=''

if @res<>''
begin
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : BANK CODE was found missing on Following  Line no  ' + @RES 
	WHERE  task_id = @pa_task_id
	RETURN
end 

select @res = coalesce(@res + ',', '') +  CONVERT(VARCHAR,TMPREC_SR_NO-1) from TMP_RECIEPT_MSTR 
	  WHERE ISNULL(TMPREC_BANKGLCODE,'') NOT IN  (SELECT ISNULL(FINA_ACC_CODE,'') FROM FIN_ACCOUNT_MSTR WHERE FINA_DELETED_IND=1)

if @res<>''
begin
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : BANK CODE was not found in ACCOUNT MASTER on Following  Line no  ' + @RES 
	WHERE  task_id = @pa_task_id
	RETURN
end 
      ...
      select @res = coalesce(@res + ',', '') +  CONVERT(VARCHAR,TMPREC_SR_NO-1) from TMP_RECIEPT_MSTR 
	  WHERE ISNULL(TMPREC_ACCT_NO,'') NOT IN  
(
SELECT ISNULL(FINA_ACC_CODE,'') FROM FIN_ACCOUNT_MSTR WHERE FINA_DELETED_IND=1
UNION SELECT DPAM_SBA_NO FROM DP_ACCT_MSTR WHERE DPAM_DELETED_IND=1
)

if @res<>''
begin
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : CLIENT CODE /GL CODE /BANK CODE was not found in  MASTER on Following  Line no  ' + @RES 
	WHERE  task_id = @pa_task_id
	RETURN
end 


select @res = coalesce(@res + ',', '') +  CONVERT(VARCHAR,TMPREC_SR_NO-1) from TMP_RECIEPT_MSTR WHERE 
 convert(varchar(11),TMPREC_TODATE,109)='Jan  1 1900'
if @res<>'' 
begin
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : Date was found missing on Following  Line no  ' + @RES 
	WHERE  task_id = @pa_task_id
	RETURN
end 

select @res = coalesce(@res + ',', '') +  CONVERT(VARCHAR,TMPREC_SR_NO-1) from TMP_RECIEPT_MSTR 	  where isnull(TMPREC_CHEQUE_NO,'')=''

if @res<>''
begin
	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : Cheque no. was found missing on Following  Line no  ' + @RES 
	WHERE  task_id = @pa_task_id
	RETURN
end 
...
SELECT @@ref_no = 'BR-' + replace(convert(varchar(11),@pa_billing_to_dt,103),'/','') + @@filler

PRINT @@ref_no
CREATE TABLE #TREMPDATA(FLG CHAR(1))

declare @fil varchar(5)
set  @@ssql  ='if exists (select top 1 * from ledger' +  convert(varchar,@pa_finid) + ' where ldg_ref_no= ''' +  convert(varchar,@@ref_no) + ''' + ''_'' + convert(varchar,ldg_voucher_no))'
set @@ssql = @@ssql + ' begin  '

set @@ssql = @@ssql +   ' INSERT INTO #TREMPDATA SELECT  ''y'''
...
SELECT * FROM #TREMPDATA


IF EXISTS (SELECT FLG FROM #TREMPDATA WHERE ISNULL(FLG,'') = 'Y')
begin

	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : SAME DATE FILLER IS FOUND , PLEASE VERIFY '
	WHERE  task_id = @pa_task_id
	RETURN
end

DROP TABLE #TREMPDATA
...
/* */
update TMP_RECIEPT_MSTR 
set    TMPREC_DPAM_ID = FINA_ACC_ID,
	   TMPREC_BANKGLTYPE = FINA_ACC_TYPE
from   fin_account_mstr 
where  TMPREC_ACCT_NO = FINA_ACC_CODE 
and    TMPREC_DPM_ID = FINA_DPM_ID 
and    FINA_ACC_TYPE in('G') 
and    FINA_DPM_ID = @pa_dpmid
and	   FINA_deleted_ind = 1 and isnull(TMPREC_DPAM_ID,'0')='0'
...
from   fin_account_mstr 
where  
--TMPREC_ACCT_NO = FINA_ACC_CODE 
TMPREC_BANKGLCODE = FINA_ACC_CODE 
and    TMPREC_DPM_ID = FINA_DPM_ID 
and    FINA_ACC_TYPE in('G') 
and    FINA_DPM_ID = @pa_dpmid
...
--	END
	...
	----
	--	UPDATE filetask
	--	SET    Status = 'FAILED',usermsg = 'ERROR : Following GL/Bank Not Mapped, Please map & reimport ' + CHAR(13) + citrus_usr.fn_merge_str('REC_ACPARTY_VOCUCHERS',0,'')  
	--	WHERE  task_id = @pa_task_id
	--	RETURN
	----
	--END
...
insert into TMP_RECIEPT_MSTR(TMPREC_SR_NO,TMPREC_DPAM_ID,TMPREC_DPM_ID,TMPREC_TODATE,TMPREC_ACCT_NO,TMPREC_CREDIT,TMPREC_BANKGLID,TMPREC_BANKGLTYPE,TMPREC_BANKGLCODE,TMPREC_CHEQUE_NO,TMPREC_NARRATION)
select TMPREC_SR_NO,TMPREC_BANKGLID,TMPREC_DPM_ID,TMPREC_TODATE,TMPREC_BANKGLCODE,TMPREC_CREDIT,0,'B',TMPREC_ACCT_NO,TMPREC_CHEQUE_NO,TMPREC_NARRATION
