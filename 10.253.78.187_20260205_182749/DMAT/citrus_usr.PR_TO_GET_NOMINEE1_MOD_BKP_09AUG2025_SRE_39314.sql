-- Object: PROCEDURE citrus_usr.PR_TO_GET_NOMINEE1_MOD_BKP_09AUG2025_SRE_39314
...
create  PROCEDURE [citrus_usr].[PR_TO_GET_NOMINEE1_MOD_BKP_09AUG2025_SRE_39314]
(
@PA_CRN_NO NUMERIC,
@PA_NOM_SRNO NUMERIC,
...
if @PA_SBA_NO <> ''
  begin
	SELECT @@PA_DPAM_ID = DPAM_ID  FROM DP_ACCT_MSTR_mak WHERE DPAM_SBA_NO = @PA_SBA_NO --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
  end
  else
  begin
	SELECT @@PA_DPAM_ID = DPAM_ID  FROM DP_ACCT_MSTR_mak WHERE DPAM_CRN_NO = @PA_CRN_NO --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
  ...
  --SELECT @@PA_DPAM_ID = DPAM_ID  FROM DP_ACCT_MSTR_mak WHERE DPAM_CRN_NO = @PA_Nom_dpam_id --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
	...
	begin 
		...
		begin
		print 'pp'
			SELECT @@PA_DPAM_ID = DPAM_ID FROM DP_ACCT_MSTR WHERE DPAM_SBA_NO = @PA_SBA_NO --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
		end
		else
		begin
			SELECT @@PA_DPAM_ID = DPAM_ID FROM DP_ACCT_MSTR WHERE DPAM_SBA_NO = @PA_CRN_NO --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
		end
	end
 -- SELECT @@PA_DPAM_ID = DPAM_ID  FROM DP_ACCT_MSTR_mak WHERE DPAM_CRN_NO = @PA_CRN_NO --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
	--if @@PA_DPAM_ID = 0
...
begin
	...
	begin
		...
		SELECT 
			...
			AND NOM_DPAM_ID NOT IN (SELECT NOM_DPAM_ID FROM Nominee_Multi_mak where Nom_DELETED_IND in (0,4,8) and M.nom_srno = nom_srno)
			--AND nom_srno = @PA_NOM_SRNO
			and Nom_dpam_id = @@PA_DPAM_ID
	...
	if @PA_ACTION = 'FIND_REASON_CD'
	begin
		SELECT DISTINCT nmcrcd_reason_cd FROM name_change_reason_cd,DP_ACCT_MSTR 
										WHERE nmcrcd_crn_no = DPAM_CRN_NO 
										AND DPAM_ID = @@PA_DPAM_ID 
										AND nmcrcd_deleted_ind IN  ('0','8' )
										and nmcrcd_holder_type = case	when @PA_NOM_SRNO = '1' then 'NOMINEE1'
																		when @PA_NOM_SRNO = '2' then 'NOMINEE2'
																		when @PA_NOM_SRNO = '3' then 'NOMINEE3'
																		when @PA_NOM_SRNO = '4' then 'NOMINEE1 GUARDIAN'
																		when @PA_NOM_SRNO = '5' then 'NOMINEE2 GUARDIAN'
																		when @PA_NOM_SRNO = '6' then 'NOMINEE3 GUARDIAN' END
	end
END
