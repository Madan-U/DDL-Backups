-- Object: PROCEDURE citrus_usr.pr_bulk_dpc9
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--select * from dp_acct_mstr where dpam_sba_no = '1234567890123455'
--select * from dp_daily_hldg_cdsl where DPHMCD_DPAM_ID = 64140 
--DELETE FROM dp_daily_hldg_cdsl where DPHMCD_DPAM_ID = 64140 
--select * from cdsl_holding_dtls where 
--begin tran
--F:\bulk dpc9\cLEARING_mEMBER_aCCOUNT\08DPC9U.300172
--exec PR_BULK_DPC9 'CDSL','HO','BULK','c:\BulkInsDbfolder\dpc9.8080.bak', '*|~*', '|*~|'  , ''
--select * from TMP_DPC9_CDSL_TRX_MSTR where TMPDPC9_BOID = 
--select * from TMP_DPC9_CDSL_TRX_HLDG
--rollback
--commit
--create table TMP_dpc9_SOURCE (details varchar(8000))
CREATE PROC [citrus_usr].[pr_bulk_dpc9]
(@PA_EXCH          VARCHAR(20)    
...
,TMPDPC9_CTR_SETTID)
select 1 TMPDPC9_ID, citrus_usr.fn_splitval_by(mod_details,11,'~') TMPDPC9_BOID, citrus_usr.fn_splitval_by(mod_details,9,'~') TMPDPC9_BONM, citrus_usr.fn_splitval_by(mod_details,9,'~') TMPDPC9_ISIN
...
when citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),1,'|') in ('OF-CR', 'OF-DR','ON-CR', 'ON-DR')  then case when left(citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),3,'|'),2) = 'TX' then citrus_usr.fn_splitval_by(citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),3,'|'),2,':') else citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),2,'|') end 
...
when citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),1,'|') in ('OF-CR', 'OF-DR','ON-CR', 'ON-DR')  then citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),1,'|')
...
when citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),1,'|') in ('OF-CR', 'OF-DR','ON-CR', 'ON-DR')  then citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),4,'|')
...
when citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),1,'|') in ('OF-CR', 'OF-DR','ON-CR', 'ON-DR')  then left(citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),4,'|'),8)
...
when citrus_usr.fn_splitval_by(replace(citrus_usr.fn_splitval_by(mod_details,3,'~'),' ','|'),1,'|') in ('OF-CR', 'OF-DR','ON-CR', 'ON-DR')  then case when isnumeric(citrus_usr.fn_splitval_by(citrus_usr.fn_splitval_by(mod_details,3,'~'),4,':'))= 1 then right(citrus_usr.fn_splitval_by(citrus_usr.fn_splitval_by(mod_details,3,'~'),4,':'),7) end 
...
and (isnumeric(citrus_usr.fn_splitval_by(mod_details,6,'~')) =1
or isnumeric(citrus_usr.fn_splitval_by(mod_details,5,'~')) = 1 )

INSERT INTO #TMP_DPC9_CDSL_TRX_MSTR (TMPDPC9_TRX_DT)
SELECT 'JUN 01 2010'
...
select citrus_usr.fn_splitval_by(mod_details,15,'~') , citrus_usr.fn_splitval_by(mod_details,15,'~') , citrus_usr.fn_splitval_by(mod_details,2,'~') , citrus_usr.fn_splitval_by(mod_details,2,'~') , ISNULL([TMPDPC9_TRX_DT],'JUN 01 2010') 
...
,''
,''
,'' from  #TMP_DPC9 LEFT OUTER JOIN  #TMP_DPC9_CDSL_TRX_MSTR 
					ON citrus_usr.fn_splitval_by(mod_details,15,'~') = TMPDPC9_BOID 
					AND citrus_usr.fn_splitval_by(mod_details,2,'~') = TMPDPC9_ISIN
where Left(#TMP_DPC9.MOD_DETAILS,2) = '07'
and (isnumeric(citrus_usr.fn_splitval_by(mod_details,6,'~')) =1
or isnumeric(citrus_usr.fn_splitval_by(mod_details,5,'~')) = 1 )
group by citrus_usr.fn_splitval_by(mod_details,15,'~') , citrus_usr.fn_splitval_by(mod_details,15,'~') , citrus_usr.fn_splitval_by(mod_details,2,'~') , citrus_usr.fn_splitval_by(mod_details,2,'~') , ISNULL([TMPDPC9_TRX_DT],'JUN 01 2010') , mod_details
...
select @l_dpmdpid= dpm_dpid from dp_mstr where dpm_excsm_id = default_dp and dpm_deleted_ind = 1 and left(dpm_dpid ,2) <> 'IN'
 ...
 select distinct TMPDPC9_HLDG_TRX_DT from #TMP_DPC9_CDSL_TRX_HLDG order by TMPDPC9_HLDG_TRX_DT
...
print @l_trx_dt
       ...
       exec [pr_ins_upd_dpc9] 'MIG',@l_dpmdpid,@l_counter,'N'
 FETCH NEXT FROM @c_access_cursor1 INTO @l_trx_dt 
