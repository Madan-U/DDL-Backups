-- Object: PROCEDURE citrus_usr.PR_SELECT_TRXGEN_CDSL_SP_XML_bAKJAN302020_BULK
...
CREATE PROCEDURE [citrus_usr].[PR_SELECT_TRXGEN_CDSL_SP_XML_bAKJAN302020_BULK]  
(  
...
select T.* into #TEMPDATANEW from #TEMPDATA T,dp_trx_dtls_cdsl M where T.dptdc_id=M.dptdc_id and M.DPTDC_SLIP_NO not like 'E%' -- changes on Jan 29 2020



--update #tempdata set Usn=@l_lid+id from #tempdata_Usn t ,#tempdata d where t.dptdc_id=d.dptdc_id and d.cd not in ('DMAT','DESTAT')
update #TEMPDATANEW set Usn=@l_lid+id from #tempdata_Usn t ,#TEMPDATANEW d where t.dptdc_id=d.dptdc_id and d.cd not in ('DMAT','DESTAT')

SELECT @L_CNT = COUNT(1) FROM #TEMPDATANEW
...
,GETDATE()
,1
FROM #TEMPDATANEW where cd not in ('DMAT','DESTAT')
...
from #TEMPDATANEW 
--where dptdc_created_by not in (select dpam_sba_no from DP_ACCT_MSTR where DPAM_DELETED_IND=1)
...
select  @L_TOT_REC = count(*) from #TEMPDATANEW --where dptdc_created_by not in (select dpam_sba_no from DP_ACCT_MSTR where DPAM_DELETED_IND=1)
