-- Object: PROCEDURE citrus_usr.pr_get_ob_fix_byisin_arch
...
--exec pr_get_ob_fix 3,'aug 02 2011','aug 02 2011','1201090000000101','1201090000000101',''
--exec pr_get_ob_fix_byisin 3,'aug 02 2011','aug 02 2011','1201090000000101','1201090000000101','INF732E01037',''
  
CREATE proc [citrus_usr].[pr_get_ob_fix_byisin_arch](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                    
...
, cdshm_dpam_id    
, cdshm_isin , convert(numeric(18,5),0) ob  into #tmp_cdsl_holding_dtls from dmat_archival.citrus_usr.cdsl_holding_dtls  
where  CDSHM_TRAS_DT >= @l_min_trx_dt  
and CDSHM_TRAS_DT between  @pa_from_dt   and @pa_to_dt
and cdshm_dpm_id = @pa_dpm_id  
and cdshm_tratm_cd in ('2246','2277','2201','3102','2270','2220','2205','2280')
...
--,sum(isnull(dphmcd_free_qty,0)) holdingbal   
,sum(isnull(dphmcd_free_qty,0))+sum(isnull(DPHMCD_PLEDGE_QTY,0))++sum(isnull(DPHMCD_LOCKIN_QTY,0))+sum(isnull(DPHMCD_SAFE_KEEPING_QTY,0)) holdingbal 
--we can use curr  qty also for above case lock in qty added on 08112012 by tushar as mosl reported issue     

from vw_holding_base_all   
