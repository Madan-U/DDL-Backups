-- Object: PROCEDURE citrus_usr.pr_app_dp_client_BKP_09AUG2025_SRE_39314
...
CREATE PROCEDURE [citrus_usr].[pr_app_dp_client_BKP_09AUG2025_SRE_39314](@pa_id                VARCHAR(8000)  
    ...
    DECLARE @c_clim                 CURSOR  
           ...
           ,@l_conc_acct_id        varchar(8000)
               ...
               declare @c_nom cursor,
						@c_nom_id numeric,
						@l_nom_id varchar(8000)
               set @l_nom_id = ''
               set @l_mod_crn=''

			    --add for ucc
				declare @c_ucc cursor,
				@c_ucc_id numeric,
				@l_ucc_id varchar(8000)
				set @l_ucc_id = ''
			   --add for ucc
    --      
      ...
      --      PRINT @l_error
         --   PRINT @pa_action
          ...
          IF EXISTS(SELECT nom_id  
                    ...
                    AND    Nom_dpam_id        = dpam.dpam_id   
                    AND    dpam_crn_no         = @l_crn_no --CONVERT(NUMERIC,@@currstring_id)  
                   )  
          BEGIN  
          --  
          
            SET    @c_nom               = CURSOR fast_forward FOR  
            SELECT nom_id  
            FROM   Nominee_Multi_mak    WITH(NOLOCK)   
                  ,dp_acct_mstr  dpam     WITH(NOLOCK)    
            WHERE  nom_deleted_ind   IN (0,4,8)   
            ...
            --  
            FETCH NEXT FROM @c_nom INTO @c_nom_id  
            --  
            
            WHILE @@fetch_status = 0  
            BEGIN  
            --  
              SET @l_nom_id = @l_nom_id+CONVERT(VARCHAR,@c_nom_id)+@rowdelimiter  
              --   
            ...
            print @l_nom_id
            print @l_crn_no
              exec pr_ins_upd_Nommulti @l_nom_id,@pa_action,@pa_login_name,@l_crn_no,'','','','','','','','','','','','','','','','','','','','','','','','','','','1','','','','',@rowdelimiter, @coldelimiter, @pa_errmsg = @l_error OUT
               
              insert into applog
              select @l_crn_no,'pr_ins_upd_Nommulti' ,GETDATE()
              
            IF left(@l_error ,1)= '0' or @l_error= ''  SET @l_error =  '0'  
            END  
            IF CONVERT(NUMERIC,@l_error) > 0  
            BEGIN  
            --  
            ...
            BEGIN  
            --  
              EXEC pr_ins_upd_list @l_crn_no, 'A','multi nom dtls', @pa_login_name,'*|~*','|*~|',''  
            --  
            END  
          --  
          END  
          ...
          IF EXISTS(SELECT CUDM_ID  
                    ...
                    AND    CUDM_DPAM_ID        = dpam.dpam_id   
                    AND    dpam_crn_no         = @l_crn_no --CONVERT(NUMERIC,@@currstring_id)  
                   )  
          BEGIN  
          --  
          
            SET    @c_ucc               = CURSOR fast_forward FOR  
            SELECT CUDM_ID  
            FROM   CDSL_UCC_DTLS_mak    WITH(NOLOCK)   
                  ,dp_acct_mstr  dpam     WITH(NOLOCK)    
            WHERE  CUDM_DELETED_IND   IN (0,4,8)   
            ...
            --  
            FETCH NEXT FROM @c_ucc INTO @c_ucc_id  
            --  
            
            WHILE @@fetch_status = 0  
            BEGIN  
            --  
              SET @l_ucc_id = @l_ucc_id+CONVERT(VARCHAR,@c_ucc_id)+@rowdelimiter  
              --   
            ...
            print @l_ucc_id
            print @l_crn_no
			declare @l_acct_no_for_ucc varchar(20), @l_dpam_id_for_ucc numeric
			select @l_acct_no_for_ucc = dpam_acct_no, @l_dpam_id_for_ucc = dpam_id from dp_acct_mstr where DPAM_DELETED_IND = 1 and DPAM_CRN_NO = @l_crn_no
			if @l_acct_no_for_ucc = ''
			begin
				select @l_acct_no_for_ucc = dpam_acct_no from dp_acct_mstr_mak where DPAM_DELETED_IND in (0,4,8) and DPAM_CRN_NO = @l_crn_no
			end
			print 'pankaj'
			print @l_acct_no_for_ucc
              exec PR_INS_UPD_CDSLUCCDTLS @l_ucc_id,@pa_action,@pa_login_name,@l_acct_no_for_ucc,'','','','','','','',@l_dpam_id_for_ucc,'1',@rowdelimiter, @coldelimiter, @pa_errmsg = @l_error OUT
              --insert into applog
              --select @l_crn_no,'pr_ins_upd_Nommulti' ,GETDATE()
			  print 'pankaj11'
            ...
            BEGIN  
            --  
              EXEC pr_ins_upd_list @l_crn_no, 'A','UCC Details', @pa_login_name,'*|~*','|*~|',''  
            --  
            END  
          --  
          END  
