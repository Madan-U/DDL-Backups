-- Object: PROCEDURE citrus_usr.pr_dp_select_mstrtemp
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE  [citrus_usr].[pr_dp_select_mstrtemp]( @pa_id  VARCHAR(800)                          
...
union all
   ...
   FROM   account_document_mstr                        
        ...
        , dp_acct_mstr                        
   WHERE  accdocm_doc_id   = accd_accdocm_doc_id                        
   AND    accdocm_cd    IN ('SIGN_POA')                        
   AND    dpam_id          = accd_clisba_id                        
   AND    dpam_sba_no     = isnull(citrus_usr.fn_splitval(@pa_id,1),'') 
   --and    ISNULL(CITRUS_USR.FN_UCC_ACCP(DPAM_id,'BBO_CODE',''),'') like  (citrus_usr.fn_splitval(@pa_id,2)) + '%'                  
   AND    accd_deleted_ind = 1                        
   AND    accdocm_deleted_ind = 1 and  exists
   (select 1 from dps8_pc5 where boid =isnull(citrus_usr.fn_splitval(@pa_id,1),'')  and TypeOfTrans in ('1','2','4',''))
   order by  accdocm_desc 
end
...
set @l_dpm_dpid='999'
    ...
    WHERE slibm_dpm_id = dpm_id                          
    and   slibm_tratm_id = trastm_id                          
    and   dpm_id    = case when @l_dpm_dpid                        ='3' then '999' else @l_dpm_dpid end
...
,case when AltPhInd = 'M' then isnull(AltPhNum,'') else '' end [mobile]    
