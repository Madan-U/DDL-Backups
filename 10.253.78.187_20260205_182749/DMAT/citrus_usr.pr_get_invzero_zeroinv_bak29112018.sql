-- Object: PROCEDURE citrus_usr.pr_get_invzero_zeroinv_bak29112018
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--exec pr_get_invzero_zeroinv 'INVTOZERO'
--exec pr_get_invzero_zeroinv 'ZEROTOINV'
create proc [citrus_usr].[pr_get_invzero_zeroinv_bak29112018](@pa_tab varchar(100),@pa_dt varchar(100))
as
begin 
...
-- above line is aded on 31082018 to avoid client which write off in for Zero scheme but not to convert normal clients- vishal ,jagan kadam - change by yogesh
				...
				select LDG_ACCOUNT_ID, SUM(ldg_amount) os  into #ledger from LEDGER5 where ldg_deleted_ind = 1 group by LDG_ACCOUNT_ID
