-- Object: PROCEDURE citrus_usr.pr_ins_upd_Nommulti_bkp_13AUG25_SRE_39423
...
CREATE  PROCEDURE [citrus_usr].[pr_ins_upd_Nommulti_bkp_13AUG25_SRE_39423]( @pa_id           VARCHAR(8000)  
						   ...
						   ---jul 22 2025
						   , @pa_NOM_TRXPRINTFLAG varchar(3)
						   , @pa_NOM_DRIVINGLIC varchar(100)
						   , @pa_NOM_PASSUIC varchar(100)
						   , @pa_NOM_ISDCODE varchar(100)
						   ---jul 22 2025
                           , @rowdelimiter    CHAR(4)       = '*|~*'  
                           , @coldelimiter    CHAR(4)       = '|*~|'  
     ...
     BEGIN  
       ...
       BEGIN  
         --  
		  ...
		  if exists (select 1 from Nominee_Multi where nom_dpam_id = @@PA_DPAM_ID and nom_srno = @pa_nom_srno  and Nom_DELETED_IND in (1)) 
		 begin 

		update Nominee_Multi  set Nom_DELETED_IND = 9 where  nom_dpam_id = @@PA_DPAM_ID and nom_srno = @pa_nom_srno  and Nom_DELETED_IND in (1)
       ...
       --  
		...
		update Nominee_Multi_mak  set Nom_DELETED_IND = 2 where nom_id = @pa_id 
		 ...
		 select 1 from Nominee_Multi_mak where nom_dpam_id = @@PA_DPAM_ID and nom_srno = @pa_nom_srno  and Nom_DELETED_IND in (0)

		   if exists (select 1 from Nominee_Multi_mak where nom_dpam_id = @@PA_DPAM_ID and nom_srno = @pa_nom_srno  and Nom_DELETED_IND in (0)) 
		 begin 

		update Nominee_Multi_mak  set Nom_DELETED_IND = 2 where  nom_dpam_id = @@PA_DPAM_ID and nom_srno = @pa_nom_srno  and Nom_DELETED_IND in (0)
    ...
    print '111'
     ...
     --  
       ...
       SET   nom.nom_fname= nomm.nom_fname
			...
			,nom.NOM_TRXPRINTFLAG = nomm.NOM_TRXPRINTFLAG
			,nom.NOM_DRIVINGLIC = nomm.NOM_DRIVINGLIC
			,nom.NOM_PASSUIC = nomm.NOM_PASSUIC
			,nom.NOM_ISDCODE = nomm.NOM_ISDCODE
...
,NOm_CREATED_DT
     ...
     BEGIN  
       ...
       SET   nom.nom_fname=@PA_nom_fname 
		...
		,nom.NOM_TRXPRINTFLAG = @pa_NOM_TRXPRINTFLAG
		,nom.NOM_DRIVINGLIC = @pa_NOM_DRIVINGLIC
		,nom.NOM_PASSUIC = @pa_NOM_PASSUIC
		,nom.NOM_ISDCODE = @pa_NOM_ISDCODE
       ...
       --  
		 ...
		 print 'edt'
		 select 1 from Nominee_Multi_mak where nom_id = @pa_id and Nom_DELETED_IND in (0,4,8)
		 print @pa_id
		  if exists (select 1 from Nominee_Multi_mak where nom_id = @pa_id and Nom_DELETED_IND in (0,4,8)) 
		 begin 
