-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_forselectonly_arch
...
CREATE proc [citrus_usr].[pr_get_holding_fix_forselectonly_arch](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
if exists(select name from sysobjects where name ='holdingall_test')  
DROP TABLE holdingall_test  
...
into #holdingdmtpenfing from  vw_holding_base_all, DP_aCCT_MSTR 
...
select cdshm_dpam_id , cdshm_isin , sum(cdshm_qty) qty ,cdshm_trans_no into #trasdmtpenfing  from dmat_archival.citrus_usr.cdsl_holding_dtls
...
select * 
into #tras 
from dmat_archival.citrus_usr.cdsl_holding_dtls
...
into #holdingdmtpenfing1 from  vw_holding_base_all, DP_aCCT_MSTR 
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt=@l_base_hldg_dt
and dpam_sba_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
and dphmcd_dpm_id = @PA_DPM_ID
...
select cdshm_dpam_id , cdshm_isin , sum(cdshm_qty  ) qty ,cdshm_trans_no into #trasdmtpenfing1  from dmat_archival.citrus_usr.cdsl_holding_dtls
...
INTO #TEMPDATA2 from   dmat_archival.citrus_usr.cdsl_holding_dtls                     
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_cr FROM dmat_archival.citrus_usr.cdsl_holding_dtls WHERE 
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_dr_remat FROM dmat_archival.citrus_usr.cdsl_holding_dtls 
WHERE CDSHM_TRATM_cD  = '2262' --=case when CDSHM_CDAS_TRAS_TYPE IN ('21','22','23') then '2262' else '2277' end 
and CDSHM_TRAS_DT between  @l_min_trx_dt and @pa_to_dt
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_dr_remat_cr FROM dmat_archival.citrus_usr.cdsl_holding_dtls 
WHERE CDSHM_TRATM_cD  = '2262' --=case when CDSHM_CDAS_TRAS_TYPE IN ('21','22','23') then '2262' else '2277' end 
and CDSHM_TRAS_DT between  @l_min_trx_dt and @pa_to_dt
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #availlenddata FROM dmat_archival.citrus_usr.cdsl_holding_dtls 
