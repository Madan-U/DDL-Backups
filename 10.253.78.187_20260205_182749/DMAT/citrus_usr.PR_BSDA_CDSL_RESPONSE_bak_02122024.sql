-- Object: PROCEDURE citrus_usr.PR_BSDA_CDSL_RESPONSE_bak_02122024
...
----		 select  *  from TMP_BSDA_CDSL_RESPONSE
------		 commit
		create   PROCEDURE [citrus_usr].[PR_BSDA_CDSL_RESPONSE_bak_02122024]
		(
		...
		SET @@SSQL ='BULK INSERT TMP_BSDA_CDSL_RESPONSE_VAL FROM ''' + @PA_FILLER1 + ''' WITH   
		(  
		FIELDTERMINATOR = ''~'',  
		ROWTERMINATOR = ''\n''  
...
--         --EXEC PR_INS_UPD_ACCP @L_CRN_NO ,'EDT','MIG',@L_DPAM_ID,@C_BEN_ACCT_NO1,'DP',@L_ACCP_VALUE,@L_ACCPD_VALUE ,0,'*|~*','|*~|','' 
 ...
 Update P set filler9=t.FILLER1 from TMP_BSDA_CDSL_RESPONSE t,dps8_pc1 p where t.boid=p.boid
		 and t.FILLER1 in ('Y','N','o')
