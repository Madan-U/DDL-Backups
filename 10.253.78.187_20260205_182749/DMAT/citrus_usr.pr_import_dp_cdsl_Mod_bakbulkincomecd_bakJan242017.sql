-- Object: PROCEDURE citrus_usr.pr_import_dp_cdsl_Mod_bakbulkincomecd_bakJan242017
...
--CDSL	01/05/2008	02/05/2008	N	45	3	HO	--bulk updated
CREATE PROCeDURE [citrus_usr].[pr_import_dp_cdsl_Mod_bakbulkincomecd_bakJan242017]( @pa_crn_no      varchar(8000)                 
                            , @pa_exch        VARCHAR(10)                  
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
								...
								--, MAX(case when clic_mod_action = 'SIGNATURE' then replace(ISNULL(citrus_usr.fn_ucc_doc_old(dpam_id,'SIGN_BO',''),'') ,'B'+dpam_sba_no,'') else '' end)
								 ,''--MAX(case when clic_mod_action = 'SIGNATURE' then '../CCRSDocuments/SIGN201502.BMP' END )
...
insert into @line2( ln_no                  
         ...
         --                         and    nmcrc.nmcrcd_sba_no = dpam.DPAM_SBA_NO
                                ...
                                , sub_ctgry_mstr            subcm                   
                                , exch_seg_mstr             excsm  
                                , client_list_modified  climmod ,INCOME13012017_BAK2 
...
and	isnull(climmod.clic_mod_batch_no,0) = 0            
                           ...
                           AND    isnull(cliba.cliba_deleted_ind,1) = 1                  
                           AND    isnull(banm.banm_deleted_ind,1)   = 1
                           and dpam.DPAM_SBA_NO=[client_code] and [client_code]=climmod.clic_mod_dpam_sba_no
         --                  and    nmcrc.nmcrcd_crn_no = dpam.dpam_crn_no
         --                  and    nmcrc.nmcrcd_sba_no = dpam.DPAM_SBA_NO
						   --and    nmcrc.nmcrcd_crn_no = clim.clim_crn_no
						   --and    nmcrc.nmcrcd_deleted_ind = 1 
...
----                         and    climmod.clic_mod_deleted_ind = 1
               ...
               insert into @line5( ln_no                   
                                ...
                                --changed by tushar on jan 09 2015
                                       ...
                                       ,''--d-max(case when clic_mod_action = 'OCCUPATION' then (CASE WHEN citrus_usr.fn_get_listing('occupation',isnull(citrus_usr.fn_ucc_entp(clim_crn_no,'occupation',''),'')) = '' THEN 'O' ELSE citrus_usr.fn_get_listing('occupation',isnull(citrus_usr.fn_ucc_entp(clim_crn_no,'occupation',''),'')) END)ELSE '' END) --occupation          
                                       , ''--LIFE STYLE  
									   ...
									   --changed by tushar on jan 09 2015
                                        ,''--d-max(case when clic_mod_action = 'EMAIL STMT FLAG' then (case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'Email_st_flag',''),'')  NOT IN ( 'NO','') then 'Y' else 'N' end) else '' end)  --EMAIL_STATEMENT_FLAG            
                                        ,''--d-max(case when clic_mod_action = 'CAS FLAG' then (case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'cas_flag',''),'')  =  'CAS NOT REQUIRED' OR  isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'Email_st_flag',''),'')='YES' then 'NO' else 'PH' end )  else '' end) --cas_flag
										--,case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'mental_flag',''),'')  NOT IN ( 'NO','') then 'Y' else 'N' end  --CAS_MODE   
										,''--d-max(case when clic_mod_action = 'mental_flag' then (case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'mental_flag',''),'')  NOT IN ( 'NO','') then 'Y' when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'mental_flag',''),'')  IN ( 'NO') then 'N' else '' end) else '' end ) --CAS_MODE  
										--,''            
									   ...
									   ,''--d--max(case when clic_mod_action = 'RGESS FLAG' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'RGESS_FLAG',''),'')='NO' THEN 'N' ELSE 'Y' END) else '' end )--RGESS FLAG)
									   --, '' --ANNUAL REPORT FLAG
									   --, '' --PLEDGE STANDING INSTRUCTION FLAG
									  ,'' --d--  , max(case when clic_mod_action = 'ANNUAL RPT FLG' then (case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'ANNUAL_REPORT',''),'00')  = 'ELECTRONIC'     then '2'
									   ...
									   ,'' --d-max(case when clic_mod_action = 'PLEDGE STANDING INSTRUCTION FLG' then (case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'PLEDGE_STANDING',''),'00')  = 'YES'     then 'Y' else 'N' end) else '' end )
									   ,'' --d- max(case when clic_mod_action = 'RTA Email Flg' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'RTA_EMAIL',''),'')='YES' THEN 'Y' ELSE 'N' END) else '' end )--'' --EMAIL RTA DOWNLOAD FLAG
									   --, CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BSDA',''),'') ='NO' THEN 'N' ELSE 'Y' END--BSDA FLAG
									   ,''--d- max(case when clic_mod_action = 'BSDA FLAG' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BSDA',''),'') ='NO' THEN 'N' ELSE 'Y' END) else '' end)--BSDA FLAG
...
--                                       , '000000'--citrus_usr.fn_get_listing('BANKCCY',isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BANKCCY',''),''))               
                                  ...
                                  FROM   dp_acct_mstr              dpam  with (nolock)                
                                       ...
                                       , client_ctgry_mstr         clicm    with (nolock)              
                                         ...
                                         on  subcm.subcm_clicm_id    = clicm.clicm_id and   subcm.subcm_deleted_ind = 1                  
                                         ,exch_seg_mstr  
                                              , client_list_modified  climmod  with (nolock)
                                              ,INCOME13012017_BAK2  with (nolock)
                                  WHERE  dpam.dpam_crn_no            = clim.clim_crn_no                   
                                  AND    dpam.dpam_enttm_cd          = enttm.enttm_cd                  
                                  AND    dpam.dpam_clicm_cd          = clicm.clicm_cd        
                                  AND    dpam.dpam_subcm_cd          = subcm.subcm_cd        
								  ...
								  AND   climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
								  and	isnull(climmod.clic_mod_batch_no,0) = 0
								  --d--AND	  climmod.clic_mod_action in ('Bank','GENERAL','INCOME DETAILS','SUB STATUS','BSDA FLAG','RGESS FLAG','FST HOLDER UID','EMAIL STMT FLAG','CAS FLAG','CONF WAIVED FLAG','FST HOLDER DOB','RTA Email Flg','FST HOLDER GENDER','OCCUPATION','RBI DETAILS','PLEDGE STANDING INSTRUCTION FLG','ANNUAL RPT FLG')
								  and    climmod.clic_mod_action = 'INCOME DETAILS' --@pa_mod_typ
								  and    climmod.clic_mod_action like case when upper(@pa_mod_typ) = 'ALL' then '%%' else @pa_mod_typ end
								  and [Client_code] =dpam.dpam_sba_no and [Client_code] =climmod.clic_mod_dpam_sba_no
...
--Added by pankaj for new column in client_list_modified for batch no column on 20/09/2013           
 ...
 UPDATE client_list_modified SET clic_mod_batch_no = @PA_BATCH_NO
	 FROM client_list_modified C1
	 ,INCOME13012017_BAK2
	 where C1.clic_mod_dpam_sba_no = [client_code]
	 --AND convert(varchar,c1.clic_mod_from_dt,103)  between @pa_from_dt and @pa_to_dt               
	 --AND convert(varchar,c1.clic_mod_to_dt,103)  between @pa_from_dt and @pa_to_dt
	 AND convert(datetime,convert(varchar(11),clic_mod_lst_upd_dt,109))  between convert(datetime,@pa_from_dt,103) and convert(datetime,@pa_to_dt,103)               
	 --AND convert(datetime,convert(varchar(11),clic_mod_to_dt,109))   between convert(datetime,@pa_from_dt,103) and convert(datetime,@pa_to_dt,103)
	 AND ISNULL(clic_mod_batch_no ,0) = 0
	 AND clic_mod_action ='INCOME DETAILS' --like case when upper(@pa_mod_typ )='ALL' then '%%' else @pa_mod_typ end
