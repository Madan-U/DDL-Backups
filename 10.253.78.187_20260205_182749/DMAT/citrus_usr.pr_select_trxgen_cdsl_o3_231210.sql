-- Object: PROCEDURE citrus_usr.pr_select_trxgen_cdsl_o3_231210
...
--rollback  
CREATE PROCEDURE [citrus_usr].[pr_select_trxgen_cdsl_o3_231210]    
(    
              ...
              IF @PA_BATCH_TYPE = 'BN'    
               ...
               BEGIN     
                Select convert(char(16),isnull(dpam_sba_no,''))    
                 ...
                 + convert(char(2),isnull(demrm_lockin_reason_cd,''))    
                 + convert(char(50),case when ltrim(rtrim(isnull(DEMRM_RMKS,'')))='' then space(50) else ltrim(rtrim(isnull(DEMRM_RMKS,''))) end )   
                 + case when convert(char(8),replace(convert(varchar(11),isnull(DEMRM_LOCKIN_RELEASE_DT,space(8)),103),'/','')) = '01011900'  or isnull(demrm_free_lockedin_yn,'N')='N' then  convert(char(8),'00000000') else convert(char(8),replace(convert(
varchar(11),isnull(DEMRM_LOCKIN_RELEASE_DT,space(8)),103),'/','')) end  DETAILS  
                                                                    ,abs(demrm_qty) qty  
                 FROM  demat_request_mstr    
                    left outer join dmat_dispatch on demrm_id=disp_demrm_id  
                 ,dp_acct_mstr    
