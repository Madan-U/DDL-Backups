...
, case when citrus_usr.ufn_countstring(CLIC_CHARGE_NAME,'_') >=1
 then replace (citrus_usr.fn_splitval_by(CLIC_CHARGE_NAME ,2 ,'_'),'ITRADE','ACMA') else CLIC_CHARGE_NAME end  charge_name 
...
group by dpam_sba_no,brom_desc,case when citrus_usr.ufn_countstring(CLIC_CHARGE_NAME,'_') >=1 then replace (citrus_usr.fn_splitval_by(CLIC_CHARGE_NAME ,2 ,'_'),'ITRADE','ACMA') else CLIC_CHARGE_NAME end 
union all 
select dpam_sba_no  , brom_desc 
