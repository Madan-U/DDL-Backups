-- Object: PROCEDURE citrus_usr.pr_bulk_dp57_new
...
CREATE proc [citrus_usr].[pr_bulk_dp57_new]
(  
...
case when citrus_usr.fn_splitval_by(value,2,'~') in ('2','3')  then 
											case when CITRUS_USR.fn_dp57_stuff('crdr',citrus_usr.fn_splitval_by(value,35,'~'),'','') = 'CR' and citrus_usr.fn_splitval_by(value,13,'~') <> ''  then 'ON-CR'  
							...
							when citrus_usr.fn_splitval_by(value,2,'~') in ('16') 
							  ...
							  when citrus_usr.fn_splitval_by(value,2,'~') in ('7') then 
											'DEMAT'  
											+ citrus_usr.fn_splitval_by(value,5,'~') +' '  
											+ case   
...
-- alter table CDSL_HOLDING_DTLS add cdshm_trans_cdas_code varchar(50)
		...
		,null
		,citrus_usr.fn_splitval_by(value,2,'~')+'|'+citrus_usr.fn_splitval_by(value,7,'~')
