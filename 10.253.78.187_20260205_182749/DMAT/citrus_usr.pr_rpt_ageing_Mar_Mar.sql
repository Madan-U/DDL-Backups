-- Object: PROCEDURE citrus_usr.pr_rpt_ageing_Mar_Mar
...
create proc [citrus_usr].[pr_rpt_ageing_Mar_Mar]  
(  
 @pa_fin_id numeric,  
    @pa_frmdt datetime,  
...
--select *, '' [more than 365 days] from ageing_08042015
--return
declare @l_sql varchar(8000),  
  @l_sql1 varchar(8000),  
        @l_enddt datetime  ,
@l_fromdt datetime
select @l_fromdt  = fin_start_dt , @l_enddt = FIN_END_DT from financial_yr_mstr where FIN_ID = @pa_fin_id  
...
set @l_sql1 = @l_sql1 + ' insert into #tushar1  
     ...
     , 0 balance   
        FROM ( select   
       ldg_account_id   
     ,CASE WHEN LDG_VOUCHER_NO = ''0'' THEN  ldg_voucher_dt   - 2 ELSE ldg_voucher_dt END  ldg_voucher_dt
     , sum(ldg_amount ) ldg_amount  
     , isnull((select sum(ldg_amount) from ledger' + convert(varchar(10),@pa_fin_id) +'  b   
         where b.ldg_account_id = a.ldg_account_id   
         and b.ldg_amount > 0  
    ...
    and ldg_account_type = ''P''  group by ldg_account_id,ldg_voucher_dt  , LDG_VOUCHER_NO
    ) A 
...
,diff365days  = case when datediffe between 181 and 363 then amount else 0 end   
,morethan365days  = case when datediffe >= 364 then amount else 0 end 
...
,sum(morethan365days) morethan365days  
,sum(diff8days)+sum(diff15days)+sum(diff30days)+sum(diff45days)+sum(diff90days)+sum(diff180days)+sum(diff365days)+sum(morethan365days) [total debit amount]  
...
abs(diff180days) diff180days,abs(diff365days) diff365days, abs(morethan365days) [more than 365 days],
abs([total debit amount])[total debit amount]  
