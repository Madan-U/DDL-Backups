-- Object: PROCEDURE citrus_usr.pr_import_isin_Converter
...
CREATE PROCEDURE [citrus_usr].[pr_import_isin_Converter](@PA_EXCH          VARCHAR(20)              
...
,isin_designation =CONTACT_PERSON_DESIGNATION
				...
				FROM    ISIN_MSTR M, TMPISIN_MSTR_ALLFIELD T            
				WHERE   M.ISIN_CD = T.ISIN_ALPHA_CODE            
				AND     M.ISIN_BIT  = 1     
				...
				update isin set  isin_adr1	= ADDR1
			,isin_adr2	= ADDR2 
			,isin_adr3	=ADDR3 
			,isin_adrcity	= CITY 
			,isin_adrstate= STATE 
			,isin_adrcountry  =CNTRY 	
			,isin_adrzip= PSTCD 
			,isin_email	= EMAIL 
			,isin_TELE	= isnull(OPRTNLPHNENB1,'') +'/'+isnull(OPRTNLPHNENB2,'') 
			,isin_FAX= OPRTNLFAXNB 
			 from ISIN_MSTR_CONVERTER , MBR_MSTR_Converter , isin_mstr  isin 
			where isin_cd = ISIN  
			and  DPTYP ='RT' and dpid  = REGAR--198385
