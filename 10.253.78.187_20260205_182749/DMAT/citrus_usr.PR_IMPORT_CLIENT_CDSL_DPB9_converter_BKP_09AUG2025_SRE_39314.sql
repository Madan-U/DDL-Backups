-- Object: PROCEDURE citrus_usr.PR_IMPORT_CLIENT_CDSL_DPB9_converter_BKP_09AUG2025_SRE_39314
...
CREATE PROCEDURE  [citrus_usr].[PR_IMPORT_CLIENT_CDSL_DPB9_converter_BKP_09AUG2025_SRE_39314]
			(
			 @pa_exch          VARCHAR(20)    
...
--end
  ...
  select @pa_task_id=task_id from filetask where status='running' and task_name like 'cln_mstr_%'




 	IF  @PA_MODE ='BULK'  and  citrus_usr.fn_splitval_by (@pa_db_source , citrus_usr.ufn_CountString(@pa_db_source,'\')+1 ,'\') 
	not like 'cln_mstr_%'
		BEGIN 
				UPDATE FILETASK
...
select GETDATE(),'dpb9','start'
  ...
  --  
       ...
       delete from [CLN_MSTR_CDSL]  
  
--       DECLARE @@ssql varchar(8000)  
--       SET @@ssql ='BULK INSERT [CLN_MSTR_CDSL] from ''' + @pa_db_source + ''' WITH   
--      ( 
--						FIELDQUOTE = ''"''
--						, FIELDTERMINATOR = '',''
--						, ROWTERMINATOR = ''\n''
...
exec (@@SSQL)
 ...
 set @l_counter = @l_counter + 1 
 	...
 	--IF (select count (distinct clntid) from CLN_MSTR_CDSL ) > 1 
	 
		--BEGIN 
		--		UPDATE FILETASK
		--		SET    USERMSG = 'ERROR : Client Master file contain more that one Client', STATUS='FAILED'
		--		, TASK_END_DATE = GETDATE()
		--		WHERE  TASK_ID = @PA_TASK_ID
...
EXEC pr_ins_upd_dps8_pc1
EXEC pr_ins_upd_dps8_pc2
EXEC pr_ins_upd_dps8_pc3
EXEC pr_ins_upd_dps8_pc5
EXEC pr_ins_upd_dps8_pc6
EXEC pr_ins_upd_dps8_pc7
EXEC pr_ins_upd_dps8_pc8
EXEC pr_ins_upd_dps8_pc12
EXEC pr_ins_upd_dps8_pc18
EXEC pr_ins_upd_dps8_pc19
...
when FreezeResCd ='27'THEN'ACCOUNT HOLDER RELATED KYC NON-COMPLIANT'
...
and DPAM_SBA_NO =@c_ben_acct_no1 and CLIBA_FLG='1'
                   ...
                   Update T set CLIBA_FLG='0' , cliba_lst_upd_dt = getdate() from client_bank_accts T ,dp_acct_mstr where dpam_id = cliba_clisba_id 
				   and DPAM_SBA_NO =@c_ben_acct_no1 
				   
				   --Update T set CLIBA_FLG='1' , cliba_lst_upd_dt = getdate()  from client_bank_accts T ,dp_acct_mstr where dpam_id = cliba_clisba_id 
...
WHERE  task_id = (select TOP 1 TASK_ID  from filetask where TASK_NAME LIKE 'cln_mstr_%' AND STATUS = 'RUNNING' )
