-- Object: PROCEDURE citrus_usr.pr_bulk_dp57_fromnewdp_bak_12032025
...
create proc [citrus_usr].[pr_bulk_dp57_fromnewdp_bak_12032025]
(  
...
select 'DPM DP57-CDSL FILE - CDSL-12033203 Auto Import From New DP',GETDATE(),'','RUNNING','AUTO',MAX(task_id)+1,'',null,null,null  
 from filetask 
 
 select top 1 @PA_TASK_ID = task_id from FileTask where task_name ='DPM DP57-CDSL FILE - CDSL-12033203 Auto Import From New DP'  and STATUS ='RUNNING' 
...
select GETDATE(),'insert tmp_dp57_o','end'
		...
		set @l_dpm_id = 0 
		select @l_dpm_id  = dpm_id  from dp_mstr where dpm_dpid like '%' + @l_dpm_dpid and  left(dpm_dpid,'2')<>'IN' -- and default_dp = dpm_excsm_id  
...
select GETDATE(),'Following Client Not Mapped','start'
		...
		IF EXISTS(SELECT distinct column_3 FROM #tmp_dp57_o 
				  WHERE column_1='D' 
				  AND column_2 not in ('8','11','10','4') --<> '8'
				  AND  column_3 not in (select dpam_sba_no from dp_acct_mstr where 
				  --dpam_dpm_id = @l_dpm_id and 
				  dpam_deleted_ind =1 
				  --and column_3 not in ('12033201')
				  ) ) 																					
		BEGIN
		--
		...
		FROM #tmp_dp57_o 
		WHERE column_3 
		not in (select dpam_sba_no from dp_acct_mstr where 
		--dpam_dpm_id = @l_dpm_id and 
		dpam_deleted_ind = 1 )
		AND column_1='D' and column_2 not in ('8','11','10','4') --<>'8' 
		--and column_3 not in ('12033201')
...
/*case added by latesh on may 07 2012 */
				...
				then 		  case when CITRUS_USR.fn_dp57_stuff('crdr',column_35,'','')  ='CR' 
						  ...
						  when column_2 = '4'  and  column_7 not in ('401','409','431','433') then -- 431 
							  case when CITRUS_USR.fn_dp57_stuff('crdr',column_35,'','')  ='CR' 
							  then convert(numeric(18,5),column_29) 
							  else convert(numeric(18,5),column_29)*-1 end  
						  else 
...
-----==============================================to check duplication with tushar sir only for 17012024=======================================

insert into tmp_dp57_o_bak_check (value
,qty
,closing_qty
,opening_qty
,cdasdesc
...
,column_34
,column_35
,column_37 , getdate () dt    from #tmp_dp57_o
...
Update FileTask set STATUS ='COMPLETED', TASK_END_DATE = GETDATE()
where task_name ='DPM DP57-CDSL FILE - CDSL-12033203 Auto Import From New DP'  and STATUS ='RUNNING' and task_id =@PA_TASK_ID  
