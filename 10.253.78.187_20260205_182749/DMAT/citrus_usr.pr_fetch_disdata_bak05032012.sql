-- Object: PROCEDURE citrus_usr.pr_fetch_disdata_bak05032012
...
--	           
create proc [citrus_usr].[pr_fetch_disdata_bak05032012](@pa_Action varchar(100)
...
print @l_count 
   ...
   begin   
		...
		insert into #temp1 select citrus_usr.fn_splitval(@pa_boid,@l_counter)  
	   ...
	   begin 	
				if @pa_exist = 'E'
					...
					select distinct dpm_dpid
                        ...
                        and sliim_dt between @pa_from_date and @pa_to_date
						and case when req_slip_no <> '' then  convert(numeric,req_slip_no)  else 1 end between case when req_slip_no <> '' then  convert(numeric,SLIIM_SLIP_NO_FR ) else 1 end and case when req_slip_no <> '' then  convert(numeric,SLIIM_SLIP_NO_To ) else 1 end 
										)	or 	req_slip_no = ''
							)
