-- Object: PROCEDURE citrus_usr.Pr_rpt_statement_for_test02052012
...
CREATE   Proc [citrus_usr].[Pr_rpt_statement_for_test02052012]                                
@pa_dptype varchar(4),                                        
@pa_excsmid int,                                        
...
select DPAM_BBO_CODE bbo , dpam_sba_no clientid     
into #bbocode from  dp_Acct_mstr  WITH (NOLOCK)    
where DPAM_BBO_CODE = @@bbocode  
...
update a set exedt = REPLACE(CONVERT(VARCHAR(10), convert(datetime,exedt) , 103), '/', '')+'000000'
from #tmp_cdsl_holding_dtls a 
where len(exedt)< 14 


update a set exetime = REPLACE(CONVERT(VARCHAR(10), convert(datetime,citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,9,'~')) , 103), '/', '')+'000000'
from #tmp_cdsl_holding_dtls a 
where len(exetime)< 14
...
,easiflg                       varchar(100)
  ...
  begin      
      ...
      DPAM_SBA_NAME,                            
      CDSHM_BEN_ACCT_NO,                                        
      CDSHM_TRATM_DESC=CASE WHEN CITRUS_USR.FN_SPLITVAL_BY(CDSHM_TRANS_CDAS_CODE,36,'~') ='68' THEN ' CA- TRF OF SHARES TO IEPF' +'-'+ ISNULL(CDSHM_INT_REF_NO,'')
...
WHEN CITRUS_USR.FN_SPLITVAL_BY(CDSHM_TRANS_CDAS_CODE,2,'~') ='7' AND CITRUS_USR.FN_SPLITVAL_BY(CDSHM_TRANS_CDAS_CODE,25,'~') ='R' THEN 'REMAT REJECT -' +  CITRUS_USR.FN_SPLITVAL_BY(CDSHM_TRANS_CDAS_CODE,38,'~')  
 ELSE REPLACE(CDSHM_TRATM_DESC,'        ','') END ,                                 
      CDSHM_TRANS_NO,                                        
      CDSHM_TRAS_DT,                                        
...
,citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,41,'~')    
   ...
   and CDSHM_TRAS_DT >=@pa_fromdate AND CDSHM_TRAS_DT <=@pa_todate                                        
      and cdshm_tratm_cd in('2246','2277','2201','3102','2270','2220','2262','2251','2202','2205','2280','2255','2225','2215') --'2252'  --,'4456'  '2280', -add 2205    
...
,citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,41,'~')                                   
  ...
  ,#ACLIST account                                  
      where   CDSHM_DPM_ID = @@dpmid      
   AND CDSHM_TRAS_DT >=@pa_fromdate AND CDSHM_TRAS_DT <=@pa_todate                                        
      and cdshm_tratm_cd in('2246','2277','2201','3102','2270','2220','2262','2251','2202','2205','2280','2255','2225','2215')       --,'2252','4456' '2280', -- 2205 added                                
      --and cdshm_tratm_cd in('2246','2277','2201','2205','2220','3102','2252','2270','2280','2262','2251','3202','2202','2212')         
      and convert(numeric,CDSHM_BEN_ACCT_NO) between convert(numeric,@pa_fromaccid) and convert(numeric,@pa_toaccid)                                                                 
   AND CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE ISNULL(CDSHM_SETT_TYPE,'') END = CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE @PA_SETTM_TYPE END    
      AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(CDSHM_SETT_NO,'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END  
...
where      
tratm_cd not in ('2246','2277','3102','2201','2270','2220','2262','2251','2202','2205','2280','2255','2225') --,'2252','4456' '2280',--added 2205    
