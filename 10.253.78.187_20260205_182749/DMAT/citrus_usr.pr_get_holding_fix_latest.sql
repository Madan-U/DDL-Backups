-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                          
...
ON #tmp_dp_daily_hldg_cdsl(dphmcd_dpm_id, dphmcd_dpam_id)
...
CREATE CLUSTERED INDEX IX_DPAM_ID
ON #FINALREMTHOLDI(DPHMCD_DPAM_ID,DPHMCD_ISIN)
...
--insert into #EARMARKQTY1
--SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY --into #EARMARKQTY1 
--from cdsl_holdING_DTLS a where  CDSHM_CDAS_TRAS_TYPE='4' and 	CDSHM_CDAS_SUB_TRAS_TYPE ='409' 
--and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to  
--and cdshm_tratm_cd in ('2265')
--and  exists 
--(select 1 from cdsl_holdING_DTLS b 
--where a.CDSHM_DPAM_ID = b.CDSHM_DPAM_ID 
--and a.CDSHM_ISIN = b.CDSHM_ISIN 
