-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_bak_newdev24012014
...
create PROC [citrus_usr].[Pr_Rpt_Holding_bak_newdev24012014]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
,case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end
  ...
  ,tradingid
		 ...
		 AND DPHMCD_ISIN LIKE @PA_ISINCD + '%'      
		-- AND ISNULL(DPHMCD_CURR_QTY,0) <> 0  
group BY DPAM_SBA_NO,DPAM_SBA_NAME  ,CLOPM_CDSL_RT,dpam_id,CLOPM_DT,ISIN_NAME,DPHMCD_ISIN 
,tradingid
...
,case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end
		 ...
		 ,CLOPM_DT ratedate
  ,tradingid
,isnull((CONVERT(NUMERIC(18,3),sum(DPHMCD_FREE_QTY))),0) FREE_QTY
