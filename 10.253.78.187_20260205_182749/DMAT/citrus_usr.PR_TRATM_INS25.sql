-- Object: PROCEDURE citrus_usr.PR_TRATM_INS25
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE  PROCEDURE [citrus_usr].[PR_TRATM_INS25]
                AS
                BEGIN
                  --
                    DECLARE
                       ...
                       ,@l_sub_cd    VARCHAR(50)
                        SET  @I =1
                        SET @l_name ='HO'
                        SET @l_code = 'RES_STAT_CD_CDSL' 
                        SET @l_desc ='RESPONSE FILE STATUS CDSL'

               

                     
                      SELECT @l_excm_id = excm_id FROM exchange_mstr where excm_cd='CDSL'
                     DELETE FROM TRANSACTION_SUB_TYPE_MSTR WHERE TRASTM_TRATM_ID=(SELECT trantm_id FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'RES_STAT_CD_CDSL')
		     DELETE FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'RES_STAT_CD_CDSL'
