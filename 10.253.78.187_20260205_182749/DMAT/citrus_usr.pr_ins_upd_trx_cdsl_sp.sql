-- Object: PROCEDURE citrus_usr.pr_ins_upd_trx_cdsl_sp
...
CREATE  PROCEDURE [citrus_usr].[pr_ins_upd_trx_cdsl_sp](@pa_id              VARCHAR(8000)      
              ,@pa_tab             VARCHAR(20)    
...
,DPTDC_CUSPA_CMBP_DPID
              ...
              ,@pa_slip_no    
              ,@l_isin    
              , case when @pa_tab in ('BOBO','ID') and  @pa_trdReasoncd in ('2','6','311','312','313','314','315','316','317','318','319','320','33') then @pa_cm_id  else convert(varchar,isnull(@line_no,'0')) end --@line_no    
              ,@l_qty    
              ,@pa_trans_no    
              ,@l_trastm_id    
              ,@pa_mkt_type    
              ...
              ,@pa_excm_id    
              ,@pa_cash_trf    
               ,case when @pa_tab in ('BOBO','ID') and  @pa_trdReasoncd in ('2','6','311','312','313','314','315','316','317','318','319','320','33')   then '' else @pa_cm_id     end --, ,@pa_cm_id    
              ,@pa_rmks    
              ,@pa_login_name    
              ,getdate()    
              ,@pa_login_name    
...
print 'jitesh1'  
      ...
      ,dptdc_reason_cd            = @pa_trdReasoncd    
       ,dptdc_line_no=case when @pa_tab in ('BOBO','ID') and @pa_trdReasoncd in ('2','3','311','312','313','314','315','316','317','318','319','320','33') then @pa_cm_id  else convert(varchar,isnull(@line_no,'0'))   end
...
,DPTDC_CUSPA_CMBP_DPID
                    ...
                    ,@pa_excm_id    
                    ,@pa_cash_trf    
                     ,case when @pa_tab in ('BOBO','ID') and  @pa_trdReasoncd in ('2','6','311','312','313','314','315','316','317','318','319','320','33')   then '' else @pa_cm_id     end --, ,@pa_cm_id,@pa_cm_id    
                    ,@pa_rmks    
                    ,@pa_login_name    
                    ,getdate()    
                    ,@pa_login_name    
...
select distinct DPHMCD_ISIN isin , rate rate into #latestrate from holdingallforview
create clustered index ix_1 on #latestrate (isin)


if exists (select 1  from dptdc_mak left outer join #latestrate  on   DPTDC_ISIN = isin
	,isin_mstr
...
into #temprate from closing_price_mstr_cdsl where CLOPM_ISIN_CD in
 (select dptdc_isin from dptdc_mak where dptdc_slip_no = @pa_slip_no)
...
create index ix_1 on #temprate_final (CLOPM_ISIN_CD)
	...
	from dptdc_mak left outer join #temprate_final on   DPTDC_ISIN = CLOPM_ISIN_CD
	,isin_mstr
	where dptdc_deleted_ind in (0,-1) 
	and DPTDC_ISIN = ISIN_CD 
	--and DPTDC_ISIN = CLOPM_ISIN_CD
...
insert into #tmpvaltbl1
	select distinct dptdc_slip_no , isnull(CONVERT(NUMERIC(18,3),(abs(dptdc_qty) * rate)),'0') Val
	 ,DPTDC_ISIN ,ABS(dptdc_qty) qty ,ISIN_NAME,dptdc_id
	from dptdc_mak left outer join #latestrate  on   DPTDC_ISIN = isin
	,isin_mstr
	where dptdc_deleted_ind in (0,-1) -- and rate is null
	and DPTDC_ISIN = ISIN_CD 
	--and DPTDC_ISIN = CLOPM_ISIN_CD
