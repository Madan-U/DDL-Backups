-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_Boisl
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--pr_rpt_holding 'NSDL',4,'Y','Nov  3 2010','','','','Y',1,'HO|*~|','','','',''				
...
CREATE PROC [citrus_usr].[Pr_Rpt_Holding_Boisl]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
			@PA_FROMACCID VARCHAR(50),                    
			@PA_TOACCID VARCHAR(50),                    
...
if @PA_FROMACCID <> '' 
set @PA_FROMACCID = right(@PA_FROMACCID,16) 
if @PA_TOACCID <> ''               
		set	@PA_TOACCID = right(@PA_TOACCID,16)      
...
CONVERT(NUMERIC(18,3),DPHMCD_BORROW_QTY),
     ...
     -------/*feb1010*/               , SETTM_DESC SETTM_DESC, right(dphmcd_cntr_settm_id,7)     setm_no                                                   
        ,isnull(dphmcd_cntr_settm_id,'') setm_no
FROM 
...
-------/*feb1010*/ AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(right(dphmc_cntr_settm_id,7),'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END                         
   ...
   INSERT INTO #ACLIST SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,EFF_FROM,EFF_TO FROM CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID)    
     ...
     SELECT distinct DPAM_SBA_NAME,DPAM_SBA_NO,DPHMCD_ISIN,ISIN_NAME,CONVERT(NUMERIC(18,3),DPHMCD_CURR_QTY),VALUATION=CONVERT(NUMERIC(18,2),DPHMCD_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0)),CONVERT(NUMERIC(18,3),DPHMCD_FREE_QTY),CONVERT(NUMERIC(18,3),DPHMCD_FREEZE_QTY),CONVERT(NUMERIC(18,3),DPHMCD_PLEDGE_QTY)      
