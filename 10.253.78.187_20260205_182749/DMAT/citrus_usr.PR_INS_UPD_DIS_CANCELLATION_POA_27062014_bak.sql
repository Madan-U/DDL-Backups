-- Object: PROCEDURE citrus_usr.PR_INS_UPD_DIS_CANCELLATION_POA_27062014_bak
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------


create PROCEDURE [citrus_usr].[PR_INS_UPD_DIS_CANCELLATION_POA_27062014_bak] 
(
	@PA_EXCSM_ID NUMERIC,
...
BEGIN
	...
	BEGIN
		...
		SELECT  USES_ID,
				...
				USES_DPAM_ACCT_NO
		FROM USED_SLIP_BLOCK, SLIP_ISSUE_MSTR_POA, 
	...
	BEGIN
		SELECT sliim_book_name , SLIIM_SERIES_TYPE
	...
	BEGIN
		...
		FROM SLIP_ISSUE_MSTR_POA 
		where SLIIM_BOOK_NAME=@PA_BOOK_NO 
...
--		WHERE USES_ID = @PA_USES_ID
	...
	BEGIN
		...
		END
		BEGIN TRANSACTION
		IF NOT EXISTS (SELECT 1 FROM USED_SLIP_BLOCK WHERE CONVERT(NUMERIC,@PA_FROM_SLIP) BETWEEN USES_SLIP_NO AND USES_SLIP_NO_to
					AND CONVERT(NUMERIC,@PA_TO_SLIP) BETWEEN USES_SLIP_NO AND USES_SLIP_NO_to AND USES_TRANTM_ID = @PA_TRXTYPE
		...
		INSERT INTO USED_SLIP_BLOCK(
			...
			BEGIN
				PRINT 'NOT INSERT'
				SET @T_ERRORSTR = 'SLIP HAD CANCELLED ALREADY!!!'
