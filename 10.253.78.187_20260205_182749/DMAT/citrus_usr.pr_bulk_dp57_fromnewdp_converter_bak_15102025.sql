-- Object: PROCEDURE citrus_usr.pr_bulk_dp57_fromnewdp_converter_bak_15102025
...
create proc [citrus_usr].[pr_bulk_dp57_fromnewdp_converter_bak_15102025]
(  
...
select 'COD- FILE - CDSL-12033204 Auto Import From New DP',GETDATE(),'','RUNNING','AUTO',MAX(task_id)+1,'',null,null,null  
 from filetask 
 
 select top 1 @PA_TASK_ID = task_id from FileTask where task_name ='COD- FILE - CDSL-12033204 Auto Import From New DP'  and STATUS ='RUNNING' 
...
--END 
		...
		drop table CITRUS_USR.tmp_dp57_o
...
truncate table CITRUS_USR.DP57bulkholding
...
,case when  isnull(TxnsType,'')  in   ('2','3') then isnull(CnsdrtnAmt,'0') else isnull(ConfRejQty,'0') end ,isnull(EarmrkdQty,''),isnull(EFTSCd.cdsl_old_value,'') EFTSCd
...
into #tmp_dp57_o from CITRUS_USR.DP57bulkholding
				

select *
into CITRUS_USR.tmp_dp57_o from #tmp_dp57_o


			create clustered index ix_1 on #tmp_dp57_o(id, value)
			create clustered index ix_1 on CITRUS_USR.tmp_dp57_o(id, value)
insert into Dp57log
select GETDATE(),'insert tmp_dp57_o','end'
		declare @l_dpm_dpid varchar(50)
		set @l_dpm_dpid  = ''
		select top  1 @l_dpm_dpid = LEFT (CLNTID,8 )--'120'+ CNTRLSCTIESDPSTRYPTCPT
		from CODD_CDSL  
...
,CDSHM_CDAS_TRAS_TYPE,CDSHM_CDAS_SUB_TRAS_TYPE
		...
		select DPAM_DPM_ID , column_3, dpam_id , column_35
		--,CITRUS_USR.fn_dp57_stuff('desc',column_2,'','') + '-'+ CITRUS_USR.fn_dp57_stuff('MAINDESC',column_7,'','')
        ,cdasdesc 
		,column_9
		,column_4 
...
where task_name ='COD- FILE - CDSL-12033204 Auto Import From New DP'  and STATUS ='RUNNING' and task_id =@PA_TASK_ID  
