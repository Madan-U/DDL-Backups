-- Object: PROCEDURE citrus_usr.ImportJVVouchers
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------


--begin tran
--[ImportJVVouchers_mosl] 557,	485038,	1,	'feb 07 2013',	'feb 07 2013',	'HO'
--rollback
CREATE Proc [citrus_usr].[ImportJVVouchers]
@pa_task_id bigint,
@pa_dpmid int,
@pa_finid int,
...
select top 1 @@filler = TMPBILL_FILLER from tmp_billimp_mstr 
...
SELECT @res = coalesce(@res + ',', '') + CASE WHEN SUM(TMPBILL_CREDIT-TMPBILL_DEBIT)=0 THEN 'Y' ELSE 'N' END  FROM  TMP_BILLIMP_MSTR	
print @res
if (@res='N')
begin

	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : Sum-Up check is not Zero in upload file , please check'
	WHERE  task_id = @pa_task_id
	RETURN
...
and    FINA_DPM_ID = @pa_dpmid -- case when @pa_dpmid= '3' then '999' else  @pa_dpmid end
and	   FINA_DELETED_IND = 1



IF EXISTS(SELECT tmpbill_dpam_id FROM tmp_billimp_mstr WHERE isnull(tmpbill_dpam_id,0) = 0) 																					
BEGIN
--
...
select LDG_ID=' + convert(varchar,isnull(@@lst_curr_id,0)) + ' + 1 ,LDG_DPM_ID=TMPBILL_DPM_ID,LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''01'',LDG_VOUCHER_NO=' + convert(varchar,@@Vno) + ',1,LDG_REF_NO=''' + @@ref_no + ''',''' + @@voucher_dt + ''',TMPBILL_DPAM_ID,LDG_ACCOUNT_TYPE=tmpbill_acct_type,CASE WHEN TMPBILL_CREDIT <> 0 THEN TMPBILL_CREDIT ELSE TMPBILL_DEBIT * -1 END,LDG_NARRATION=case when isnull(TMPBILL_NARR,'''')='''' then ''Recovered from trading A/C towards Curr O/S on ' + @@voucher_dt + ''' else TMPBILL_NARR end,LDG_BANK_ID='''',LDG_ACCOUNT_NO='''',LDG_INSTRUMENT_NO='''',LDG_BANK_CL_DATE='''',LDG_COST_CD_ID=0,LDG_BILL_BRKUP_ID=0,LDG_TRANS_TYPE=''AC'',LDG_STATUS=''P'',''' + @pa_loginname + ''',getdate(),''' + @pa_loginname + ''',getdate(),1,0 
...
Set @@ssql = 'insert into ledger' + convert(varchar,@pa_finid) + ' (LDG_ID,LDG_DPM_ID,LDG_VOUCHER_TYPE,LDG_BOOK_TYPE_CD,LDG_VOUCHER_NO,LDG_SR_NO,LDG_REF_NO,LDG_VOUCHER_DT,LDG_ACCOUNT_ID,LDG_ACCOUNT_TYPE,LDG_AMOUNT,LDG_NARRATION,LDG_BANK_ID,LDG_ACCOUNT_NO,LDG_INSTRUMENT_NO,LDG_BANK_CL_DATE,LDG_COST_CD_ID,LDG_BILL_BRKUP_ID,LDG_TRANS_TYPE,LDG_STATUS,LDG_CREATED_BY,LDG_CREATED_DT,LDG_LST_UPD_BY,LDG_LST_UPD_DT,LDG_DELETED_IND,LDG_BRANCH_ID)
select LDG_ID=' + convert(varchar,isnull(@@lst_curr_id,0)) + ' + TMPBILL_SR_NO,LDG_DPM_ID=TMPBILL_DPM_ID,LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''01'',LDG_VOUCHER_NO=' + convert(varchar,@@Vno) + ',TMPBILL_SR_NO,LDG_REF_NO=''' + @@ref_no + ''',''' + @@voucher_dt + ''',TMPBILL_DPAM_ID,LDG_ACCOUNT_TYPE=tmpbill_acct_type,CASE WHEN TMPBILL_CREDIT <> 0 THEN TMPBILL_CREDIT ELSE TMPBILL_DEBIT * -1 END  ,LDG_NARRATION=case when isnull(TMPBILL_NARR,'''')='''' then ''Recovered from trading A/C towards Curr O/S on ' + @@voucher_dt + ''' else TMPBILL_NARR end,LDG_BANK_ID='''',LDG_ACCOUNT_NO='''',LDG_INSTRUMENT_NO='''',LDG_BANK_CL_DATE='''',LDG_COST_CD_ID=0,LDG_BILL_BRKUP_ID=0,LDG_TRANS_TYPE=''AC'',LDG_STATUS=''P'',''' + @pa_loginname + ''',getdate(),''' + @pa_loginname + ''',getdate(),1,0 
