-- Object: PROCEDURE citrus_usr.pr_mak_roles_actions_BAK_11052015
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[pr_mak_roles_actions_BAK_11052015](@pa_id           VARCHAR(8000)  
...
delete from roles_actions  
                 ...
                 --  
                   ...
                   begin
                     ...
                     IF NOT EXISTS(SELECT rola_rol_id FROM roles_actions WHERE rola_act_id = (SELECT ISNULL(act_id,0)  
                                                                                              ...
                                                                                              AND  rola_rol_id =@l_rol_id and rola_deleted_ind = 1 )  
                     BEGIN  
                     --  
                       BEGIN TRANSACTION  
   ...
   if exists(select entro_rol_id from entity_roles , roles ,login_names 
			 where  entro_rol_id = rol_id and replace(rol_cd,'_'+logn_name,'') = @pa_rol_cd  
and logn_name = ENTRO_LOGN_NAME  and entro_deleted_ind = 1 and rol_deleted_ind = 1 )
