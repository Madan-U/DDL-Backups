-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_BAK06072012
...
CREATE PROC [citrus_usr].[Pr_Rpt_Holding_BAK06072012]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
set @l_bbo_code =''
set @l_bbo_code = citrus_usr.fn_splitval(@PA_WITHVALUE,2)
  ...
  IF @PA_ASONDATE = 'Y'                          
	  BEGIN  
			 SELECT 
						...
						ISIN_NAME,
						CONVERT(NUMERIC(18,3),sum(DPHMC_CURR_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_FREE_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_FREEZE_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_PLEDGE_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_DEMAT_PND_VER_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_REMAT_PND_CONF_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_DEMAT_PND_CONF_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_SAFE_KEEPING_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_LOCKIN_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_ELIMINATION_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_EARMARK_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_AVAIL_LEND_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_LEND_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMC_BORROW_QTY)),
						DPAM_ID, 
						HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109)                           
			FROM 
						dp_hldg_mstr_cdsl with (nolock) LEFT OUTER JOIN ISIN_MSTR with (nolock) ON DPHMC_ISIN = ISIN_CD,                    
			...
			group BY 
						DPAM_SBA_NO,ISIN_NAME,DPHMC_ISIN  ,DPAM_SBA_NAME ,DPAM_ID                       
			ORDER BY 
...
BEGIN   
		 ...
		 SELECT distinct DPAM_SBA_NAME,DPAM_SBA_NO,DPHMC_ISIN,ISIN_NAME,CONVERT(NUMERIC(18,3),sum(DPHMC_CURR_QTY)),VALUATION=CONVERT(NUMERIC(18,2),sum(DPHMC_CURR_QTY)*ISNULL(CLOPM_CDSL_RT,0)), CONVERT(NUMERIC(18,3),sum(DPHMC_FREE_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_FREEZE_QTY))      
		 ,CONVERT(NUMERIC(18,3),sum(DPHMC_PLEDGE_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_DEMAT_PND_VER_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_REMAT_PND_CONF_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_DEMAT_PND_CONF_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_SAFE_KEEPING_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_LOCKIN_QTY))      
		 ,CONVERT(NUMERIC(18,3),sum(DPHMC_ELIMINATION_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_EARMARK_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_AVAIL_LEND_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_LEND_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMC_BORROW_QTY)),DPAM_ID, HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109),CMP=ISNULL(convert(numeric(18,2),CLOPM_CDSL_RT),0.00)    
		 FROM dp_hldg_mstr_cdsl    with (nolock)           
		 LEFT OUTER JOIN ISIN_MSTR with (nolock) ON DPHMC_ISIN = ISIN_CD              
		 LEFT OUTER JOIN CLOSING_PRICE_MSTR_cDSL with (nolock) ON DPHMC_ISIN = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = DPHMC_ISIN and CLOPM_DT <= @PA_FORDATE and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc)   ,                    
		 #ACLIST ACCOUNT  with (nolock)                              
		 ...
		 AND DPHMC_ISIN LIKE @PA_ISINCD + '%'      
		-- AND ISNULL(DPHMCD_CURR_QTY,0) <> 0  
group BY DPAM_SBA_NO,ISIN_NAME,DPHMC_ISIN ,DPAM_SBA_NAME  ,CLOPM_CDSL_RT,dpam_id          
