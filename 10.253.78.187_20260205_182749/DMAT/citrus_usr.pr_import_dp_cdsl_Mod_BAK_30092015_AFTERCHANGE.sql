-- Object: PROCEDURE citrus_usr.pr_import_dp_cdsl_Mod_BAK_30092015_AFTERCHANGE
...
CREATE PROCeDURE [citrus_usr].[pr_import_dp_cdsl_Mod_BAK_30092015_AFTERCHANGE]( @pa_crn_no      varchar(8000)                 
                            , @pa_exch        VARCHAR(10)                  
 ...
 exec [citrus_usr].[pr_auto_nrn_poaid_mod] @pa_crn_no      
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
                                ...
                                , MAX(case when clic_mod_action = 'FST HOLDER NAME' then nmcrcd_reason_cd else '' end )
								, '' --IT CIRCLE
                           ...
                           AND    climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
						   AND	  climmod.clic_mod_action in ('C Address','Client Main Properties','General','Signature','Bank','PAN DETAILS','FIRST MOBILENO','First ResidenceNo','First OfficeNo','First EmailId','First EmailId Delete','First MobileNo Delete','First ResidenceNo Delete','First OfficeNo Delete','FST HOLDER NAME')
...
----                         and    climmod.clic_mod_deleted_ind = 1
               ...
               insert into @line5( ln_no                   
                                ...
                                --changed by tushar on jan 09 2015
									   ...
									   , max(case when clic_mod_action = 'RTA Email Flg' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'RTA_EMAIL',''),'')='NO' THEN 'N' ELSE 'Y' END) else '' end )--'' --EMAIL RTA DOWNLOAD FLAG
...
--                                       , '000000'--citrus_usr.fn_get_listing('BANKCCY',isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BANKCCY',''),''))               
								  ...
								  AND   climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
								  and	isnull(climmod.clic_mod_batch_no,0) = 0
								  AND	  climmod.clic_mod_action in ('Bank','GENERAL','INCOME DETAILS','SUB STATUS','BSDA FLAG','RGESS FLAG','FST HOLDER UID','EMAIL STMT FLAG','CAS FLAG','CONF WAIVED FLAG','FST HOLDER NAME','FST HOLDER DOB','RTA Email Flg')
...
--												,convert(char(11),dppd_setup)    
												,convert(varchar,case when clic_mod_action = 'POA ACTIVATION' then dppd_poa_id ELSE '0' END)--''	
...
--												,case when ISNULL(citrus_usr.fn_ucc_doc(dpam_id,'SIGN_POA',''),'') = '' then 'N' else 'Y'   end  --signature file flag           
                        ...
                        and    climmod.clic_mod_action like case when upper(@pa_mod_typ) = 'ALL' then '%%' else @pa_mod_typ end
						and	isnull(climmod.clic_mod_batch_no,0) = 0
						and not exists(select 1 from dps8_pc5 where MasterPOAId = dppd_master_id and boid = DPAM_SBA_NO and HolderNum = case when ltrim(rtrim(DPPD_HLD)) = '1ST HOLDER' then '1' when ltrim(rtrim(DPPD_HLD)) = '2ND HOLDER' then '2' else '3' end and TypeOfTrans in ('','1'))
...
--                                               ,convert(char(11),dppd_setup)   
											   ...
											   , convert(varchar,case when clic_mod_action = 'POA ACTIVATION' then dppd_poa_id ELSE '0' END)--''	convert(numeric,id)+convert(numeric,citrus_usr.FETCH_INCRIMENTAL_VALUE_OF_POAID())
											   , convert(char(11),getdate()) 
                                               ,case when dppd_poa_type = 'FULL' then 'Y' else 'N' END                  
                                               ,dppd_gpabpa_flg  
                                               ,dppd_eff_fr_dt--isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'',''),'')                    
