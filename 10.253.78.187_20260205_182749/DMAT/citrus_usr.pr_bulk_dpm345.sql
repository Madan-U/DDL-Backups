-- Object: PROCEDURE citrus_usr.pr_bulk_dpm345
...
CREATE proc [citrus_usr].[pr_bulk_dpm345]
(  
...
insert into dpm3log
select getdate (), 'No Data Found in File' , 'end'

insert into dpm3log
select getdate (), 'Holding Date is Not Available in the File' , 'START'

declare @l_holding_dt datetime 
declare @l_holding_dt_mak datetime 
select @l_holding_dt = convert(datetime,substring(@l_dt,1,2) + '/' 
+ substring(@l_dt,3,2) + '/' 
+ substring(@l_dt,5,4),103)
--from tmp_dpm5
...
insert into dpm3log
select getdate (), 'Holding Date is Not Available in the File' , 'END'

insert into dpm3log
select getdate (), 'DPID NOT MATCHED WITH IMPORTED FILE' , 'START'
...
END
insert into dpm3log
select getdate (), 'DPID NOT MATCHED WITH IMPORTED FILE' , 'END'

insert into dpm3log
select getdate (), ' DPID NOT MATCHED' , 'START'
...
SELECT @L_DPM_ID, DPAM_ID , column2,column11
,column3,0,column5,column12,column8
,column13,column7,column4,0
,column6,column9,column14,column15,@L_HOLDING_DT
,@pa_login_name,GETDATE(),@pa_login_name,GETDATE(),1
,column19
,column20
FROM bulkholding, dp_acct_mstr
where column1 = dpam_sba_no 
and dpam_deleted_ind = 1
