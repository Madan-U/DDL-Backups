-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_Bulk
...
CREATE PROC [citrus_usr].[Pr_Rpt_Holding_Bulk]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
--print @PA_TOACCID
 ...
 if @@L_CHILD_ENTM_ID <> 0  or  @PA_LOGIN_PR_ENTM_ID <> 1 
 begin 
			INSERT INTO #ACLIST 
			SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,EFF_FROM,isnull(EFF_TO,'dec 31 2900') 
			FROM CITRUS_USR.[fn_acct_list_bytushar](@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID,@PA_FROMACCID,@PA_TOACCID)    
			...
			INSERT INTO #ACLIST 
			SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,'Jan 01 1900',isnull('dec 31 2100','dec 31 2900') 
			FROM dp_acct_mstr where dpam_sba_no between  @PA_FROMACCID and @PA_TOACCID
...
SELECT TOP 1 @PA_FORDATE = convert(varchar(11),dphmcd_holding_dt,109) from  holdingallforview with (nolock)
			 ...
			 SELECT 
						...
						CONVERT(NUMERIC(18,3),sum(DPHMCD_FREE_QTY)) DPHMCD_FREE_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_FREEZE_QTY)) DPHMCD_FREEZE_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_PLEDGE_QTY)) DPHMCD_PLEDGE_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_DEMAT_PND_VER_QTY)) DPHMCD_DEMAT_PND_VER_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_REMAT_PND_CONF_QTY)) DPHMCD_REMAT_PND_CONF_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_DEMAT_PND_CONF_QTY)) DPHMCD_DEMAT_PND_CONF_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)),
						--CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)),
						case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end lock,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_ELIMINATION_QTY))DPHMCD_ELIMINATION_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY))DPHMCD_EARMARK_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_AVAIL_LEND_QTY))DPHMCD_AVAIL_LEND_QTY, 
						CONVERT(NUMERIC(18,3),sum(DPHMCD_LEND_QTY))DPHMCD_LEND_QTY,
						CONVERT(NUMERIC(18,3),sum(DPHMCD_BORROW_QTY))DPHMCD_BORROW_QTY,
						DPAM_ID, 
			...
			WHERE
						...
						AND DPHMCd_DPAM_ID = ACCOUNT.DPAM_ID    and isnumeric(dpam_sba_no) = '1'                    
...
begin
			 ...
			 SELECT 
						...
						DPAM_SBA_NO,DPHMCD_ISIN,
						ISIN_NAME,
						CONVERT(NUMERIC(18,3),DPHMCD_CURR_QTY)DPHMCD_CURR_QTY,
						CONVERT(NUMERIC(18,3),DPHMCD_FREE_QTY)DPHMCD_FREE_QTY,
						CONVERT(NUMERIC(18,3),DPHMCD_FREEZE_QTY)DPHMCD_FREEZE_QTY,
						CONVERT(NUMERIC(18,3),DPHMCD_PLEDGE_QTY)DPHMCD_PLEDGE_QTY,
						CONVERT(NUMERIC(18,3),DPHMCD_DEMAT_PND_VER_QTY)DPHMCD_DEMAT_PND_VER_QTY,
...
if @ISINGRPVAL <> 'Y'
begin
 SELECT distinct DPAM_SBA_NAME,DPAM_SBA_NO,DPHMCD_ISIN,replace(ISIN_NAME,'*','') ISIN_NAME,CONVERT(NUMERIC(18,3),sum(DPHMCD_CURR_QTY))DPHMCD_CURR_QTY,
...
,case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end DPHMCD_LOCKIN_QTY
		 ...
		 ,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_AVAIL_LEND_QTY)),0)
		 ...
		 ,DPAM_ID, HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109),CMP=ISNULL(convert(numeric(18,2),CLOPM_CDSL_RT),0.00)  
		 ,CLOPM_DT ratedate
  ,tradingid
		 ...
		 #ACLIST ACCOUNT  with (nolock)                              
		 WHERE ISNUMERIC (DPAM_SBA_NO) = 1  AND DPHMCD_HOLDING_DT = @PA_FORDATE AND DPHMCD_DPM_ID = @@DPMID                          
		 AND DPHMCD_DPAM_ID = ACCOUNT.DPAM_ID         and isnumeric(dpam_sba_no) = '1'               
		 AND (DPHMCD_HOLDING_DT BETWEEN EFF_FROM AND EFF_TO)                    
		 --AND (CONVERT(NUMERIC,DPAM_SBA_NO) BETWEEN CONVERT(NUMERIC,@PA_FROMACCID) AND  CONVERT(NUMERIC,@PA_TOACCID))                         
		 AND DPAM_SBA_NO BETWEEN @PA_FROMACCID AND @PA_TOACCID
...
,case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end
		 ...
		 WHERE  ISNUMERIC (DPAM_SBA_NO) = 1 and DPHMCD_HOLDING_DT = @PA_FORDATE AND DPHMCD_DPM_ID = @@DPMID  and dpamid = dphmcd_dpam_id 
		 AND DPHMCD_DPAM_ID = ACCOUNT.DPAM_ID                  and isnumeric(dpam_sba_no) = '1'      
		 AND (DPHMCD_HOLDING_DT BETWEEN EFF_FROM AND EFF_TO)                    
		-- AND (CONVERT(NUMERIC,DPAM_SBA_NO) BETWEEN CONVERT(NUMERIC,@PA_FROMACCID) AND  CONVERT(NUMERIC,@PA_TOACCID))                         
		AND DPAM_SBA_NO BETWEEN @PA_FROMACCID AND @PA_TOACCID
