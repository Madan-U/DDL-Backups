-- Object: FUNCTION citrus_usr.fn_get_high_val_new
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE function [citrus_usr].[fn_get_high_val_new](@pa_validation varchar(100),@pa_slip_no varchar(100))        
returns char(1)        
as        
begin        
--        
...
,@pa_acct_no varchar(100) 
,@PA_REQUEST_DATE datetime      
...
--  --      
   ...
   select @l_rate = sum(CLOPM_CDSL_RT*-1*dptdc_qty) from CLOSING_LAST_CDSL   , dptdc_mak 
   where CLOPM_ISIN_CD = dptdc_isin and dptdc_deleted_ind in (0,1) and dptdc_slip_no = @pa_slip_no 
   order by 1 desc          
               

   set @l_yn =  case when @l_rate >= @l_max_high_val then 'Y' ELSE 'N' END        
    --        
    end         
    IF @pa_validation = 'DORMANT'        
    begin        
    --       
--
select @pa_acct_no = dpam_sba_no , @PA_REQUEST_DATE = dptdc_request_dt 
from dp_acct_mstr,dptdc_mak  where dpam_id =dptdc_dpam_id 
and dptdc_deleted_ind in (1,0)
and dptdc_slip_no = @pa_slip_no

                 declare @l_act_dt datetime       
declare @l_act_dt_max datetime    
...
select top 1 @l_act_dt_max = max(cdshm_tras_dt) FROM cdsl_HOLDING_DTLS     with (nolock)  
WHERE   right(cdshm_ben_acct_no,8) = right(@pa_acct_no,8)   
--and cdshm_tras_dt < @pa_request_date
...
--    end      
  ...
  IF (@l_act_dt_max >= DATEADD(M,-6,@PA_REQUEST_DATE) AND @l_act_dt_max <= @PA_REQUEST_DATE)
