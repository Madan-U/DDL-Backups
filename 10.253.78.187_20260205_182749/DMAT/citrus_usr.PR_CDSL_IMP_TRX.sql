-- Object: PROCEDURE citrus_usr.PR_CDSL_IMP_TRX
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--begin tran
--PR_CDSL_IMP_TRX 'HO','50' ,3,'','','INTERDP','1',''
--select * from dptdc_mak where dptdc_dtls_id = 468
...
CREATE  PROCEDURE [citrus_usr].[PR_CDSL_IMP_TRX] (  
...
, @L_ERR_MISSING_ISIN_DOR VARCHAR(5000)
, @L_ERR_MISSING_ISIN_HV VARCHAR(5000)
...
create clustered index ix_1 on #tmp_dptdc (DPTDC_SLIP_NO)
create clustered index ix_1  on #tmp_dptdc_intdp (DPTDC_SLIP_NO)
create clustered index ix_1  on #tmp_dptdc_np (DPTDC_SLIP_NO)
create clustered index ix_1  on  #tmp_dptdc_ep (DPTDC_SLIP_NO)
...
--
		...
		--
               ...
               UPDATE TMP_DP_OFFM_DTLS_CDSL SET  TMPOFFM_SLIP_NO=ltrim(rtrim(convert(numeric,TMPOFFM_SLIP_NO)))
   ...
   -- SELECT @L_ERR_MISSING_ISIN = isnull(replace(citrus_usr.fn_merge_str('BBO_CODE_MISSING_OFFM',0,''),',',''),'')
   -- IF @L_ERR_MISSING_ISIN <> ''
   -- BEGIN
   --         UPDATE filetask
			--SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Client Does not have Broker BackOffice Code : ' + @L_ERR_MISSING_ISIN
			--,      status = 'FAILED'
			--WHERE  task_id = @pa_task_id
...
select TMPOFFM_SLIP_NO into #tempused_off from TMP_DP_OFFM_DTLS_CDSL, used_slip 
...
set @L_ERR_MISSING_ISIN = ''
    ...
    if not exists(select TMPOFFM_SLIP_NO,TMPOFFM_BO_ID 
                ...
                --  and SLIIM_ENTM_ID = dpam_id 
				  ...
				  select TMPOFFM_SLIP_NO,TMPOFFM_BO_ID 
                  from   TMP_DP_OFFM_DTLS_CDSL
                       , slip_issue_mstr_poa  
                       , dp_acct_mstr ,dp_poa_dtls
                  where  rtrim(ltrim(TMPOFFM_SLIP_NO)) like ltrim(rtrim(SLIIM_SERIES_TYPE)) + '%' 
                  and isnumeric(rtrim(ltrim(TMPOFFM_SLIP_NO))) = 1
                  and    convert(numeric,rtrim(ltrim(TMPOFFM_SLIP_NO))) between convert(numeric,SLIIM_SLIP_NO_FR) and convert(numeric,SLIIM_SLIP_NO_TO) 
                --  and SLIIM_ENTM_ID = dpam_crn_no 
				  and SLIIM_DPAM_ACCT_NO = dppd_master_id --dpam_sba_no 	
				  and DPAM_ID=DPPD_DPAM_ID and DPPD_DELETED_IND=1	
                  and dpam_sba_no = TMPOFFM_BO_ID
                  union 
                  select TMPOFFM_SLIP_NO
                        ,TMPOFFM_BO_ID 
...
SET @L_ERR_MISSING_ISIN_DOR=''
SET @L_ERR_MISSING_ISIN_HV=''
IF EXISTS (
SELECT TMPOFFM_SLIP_NO   FROM  TMP_DP_OFFM_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL('',0,'DORMANT',TMPOFFM_BO_ID,CONVERT(DATETIME,TMPOFFM_REQUEST_DT,103))='Y'
OR CITRUS_USR.FN_GET_HIGH_VAL(TMPOFFM_ISIN,TMPOFFM_QTY,'HIGH_VALUE',TMPOFFM_BO_ID,'') ='Y'
)
BEGIN 

SELECT DISTINCT @L_ERR_MISSING_ISIN_DOR =  'BUT FOLLOWING SLIPS ' + ISNULL(@L_ERR_MISSING_ISIN_DOR,'')  + TMPOFFM_SLIP_NO + ' ARE MARKED WITH DORMANT SO NEED TO DO DUAL CHECKER PROCESS : '   FROM  TMP_DP_OFFM_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL('',0,'DORMANT',TMPOFFM_BO_ID,CONVERT(DATETIME,TMPOFFM_REQUEST_DT,103))='Y'

SELECT DISTINCT @L_ERR_MISSING_ISIN_HV = 'BUT FOLLOWING SLIPS ' + ISNULL(@L_ERR_MISSING_ISIN_HV,'')  + TMPOFFM_SLIP_NO  + ' ARE MARKED WITH HIGH VALUE SO NEED TO DO DUAL CHECKER PROCESS : '  FROM  TMP_DP_OFFM_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL(TMPOFFM_ISIN,TMPOFFM_QTY,'HIGH_VALUE',TMPOFFM_BO_ID,'') ='Y'

UPDATE FILETASK
SET    USERMSG = 'FILE UPLOADED SUCCESSFULLY ' + @L_ERR_MISSING_ISIN_DOR + '  , ' + @L_ERR_MISSING_ISIN_HV
...
*/
    ...
    UPDATE  TMP
				SET     TMPOFFM_CMBP_YN =isnull(CASE WHEN ISNULL(accp_value,'')='' then 'N' else 'Y' end,'N') --CASE WHEN isnull(citrus_usr.fn_acct_entp(A.dpam_id,'CMBP_ID'),'') ='' THEN 'N' ELSE 'Y' END
				FROM    DP_ACCT_MSTR A left outer join ACCOUNT_PROPERTIES on  ACCP_CLISBA_ID=DPAM_ID AND ACCP_ACCPM_PROP_CD='CMBP_ID' AND ACCP_DELETED_IND=1
    							,TMP_DP_OFFM_DTLS_CDSL TMP
				WHERE   A.DPam_SBA_NO = convert(varchar,TMPOFFM_ACCT_ID)
			 AND     A.DPAM_DPM_ID  = @l_dpm_id
				AND     A.DPAM_DELETED_IND = 1 --and ISNULL(accp_value,'')=''
				


				UPDATE  TMP
				SET     TMPOFFM_COUNTER_CMBP_YN =isnull( CASE WHEN ISNULL(accp_value,'')='' then 'N' else 'Y' end,'N') --CASE WHEN isnull(citrus_usr.fn_acct_entp(B.dpam_id,'CMBP_ID'),'') = '' THEN 'N' ELSE 'Y' END
                FROM    DP_ACCT_MSTR B 
											,TMP_DP_OFFM_DTLS_CDSL TMP,ACCOUNT_PROPERTIES
				WHERE   B.DPam_SBA_NO = convert(varchar,TMPOFFM_COUNTER_DEMAT_ACCT_NO)
				AND     B.DPAM_DELETED_IND = 1 --and ISNULL(accp_value,'')=''
				AND ACCP_CLISBA_ID=DPAM_ID AND ACCP_ACCPM_PROP_CD='CMBP_ID' AND ACCP_DELETED_IND=1
   ...
   update TMP_DP_OFFM_DTLS_CDSL set TMPOFFM_REASON_CD ='0' where TMPOFFM_REASON_CD =''
				...
				TMPOFFM_ISIN,     
				TMPOFFM_BATCH_NO ,  
				case when TMPOFFM_REMARKS='' then 'RELEASE FOR CLIENT PAY OUT' else isnull(TMPOFFM_REMARKS,'RELEASE FOR CLIENT PAY OUT') end ,--TMPOFFM_LINE_NO,  
				TMPOFFM_QTY ,  
				TMPOFFM_INTERNAL_REF_NO,  
				TMPOFFM_TRANS_NO ,  
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPOFFM_BO_ID,convert(datetime,TMPoffm_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPoffm_ISIN,abs(TMPoffm_QTY),'HIGH_VALUE',TMPOFFM_BO_ID,'') end = 'Y' then -1 else 0 end,  --0,--case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPOFFM_ACCT_ID,convert(datetime,TMPOFFM_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPOFFM_ISIN,abs(TMPOFFM_QTY),'HIGH_VALUE','','') end = 'Y' then 0 else 0 end,  
				...
				NULL,
				NULL,
				isnull(TMPOFFM_REASON_CD,'6'),--NULL,
				'P' ,  
			CASE WHEN (TMPOFFM_CMBP_YN = 'N' AND isnull(TMPOFFM_MKT_TYPE,'') = '' ) THEN 'BOBO'
				     WHEN (TMPOFFM_CMBP_YN = 'Y' AND isnull(TMPOFFM_B_S_FLAG,'') IN ('S','B')) THEN 'CMBO'
				     WHEN (TMPOFFM_CMBP_YN = 'N' AND isnull(TMPOFFM_MKT_TYPE,'') <> '' ) THEN 'BOCM' 
			...
			--/*15062016*/
			if exists (select 1 from #tmp_dptdc where  [citrus_usr].[fn_get_high_val_byslip_broker](DPTDC_SLIP_NO)='Y')
			update dptdc set dptdc_deleted_ind = -1 from dptdc_mak dptdc , #tmp_dptdc tmp
			where dptdc.dptdc_slip_no= tmp.dptdc_slip_no
			--/*15062016*/
			/*
   ...
   */
			...
			SELECT TMPOFFM_INTERNAL_REF_NO INTERNAL_REF_NO , TMPOFFM_BROKER_INTERNAL_REF_NO BROKER_INTERNAL_REF_NO,
			 TMPOFFM_BROKERBATCH_NO BROKER_BATCH_NO,ERROR = ''
			FROM   TMP_DP_OFFM_DTLS_CDSL
...
--
		...
		BEGIN
		--
                UPDATE TMP_DP_INTDP_DTLS_CDSL SET  TMPINTDP_SLIP_NO=ltrim(rtrim(convert(numeric,TMPINTDP_SLIP_NO)))
				...
				IF EXISTS(SELECT TOP 1 ISNULL(DPTDC_ID,0) FROM DP_TRX_DTLS_CDSL , DP_ACCT_MSTR WHERE DPTDC_BROKERBATCH_NO IN (SELECT TMPINTDP_BROKERBATCH_NO FROM TMP_DP_INTDP_DTLS_CDSL) AND DPAM_ID = DPTDC_DPAM_ID AND DPAM_DPM_ID = @l_dpm_id AND DPAM_DELETED_IND = 1 
AND DPTDC_DELETED_IND = 1 and DPTDC_INTERNAL_TRASTM like 'ID') 
    ...
    END
 set @L_ERR_MISSING_ISIN = ''
    SELECT @L_ERR_MISSING_ISIN = isnull(replace(citrus_usr.fn_merge_str('BBO_CODE_MISSING_INTDP',0,''),',',''),'')
    IF @L_ERR_MISSING_ISIN <> ''
    BEGIN
            UPDATE filetask
			SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Client Does not have Broker BackOffice Code : ' + @L_ERR_MISSING_ISIN
    ...
    select TMPINTDP_SLIP_NO into #tempused_ind from TMP_DP_INTDP_DTLS_CDSL, used_slip 
   ...
   if exists(select 1 from #tempused_ind 
            )
    begin

     SELECT distinct @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  + TMPINTDP_SLIP_NO + ',' from #tempused_ind 
...
SET @L_ERR_MISSING_ISIN_HV=''
IF EXISTS (
SELECT TMPINTDP_SLIP_NO   FROM  TMP_DP_INTDP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL('',0,'DORMANT',TMPINTDP_BO_ID,CONVERT(DATETIME,TMPINTDP_REQUEST_DT,103))='Y'
OR CITRUS_USR.FN_GET_HIGH_VAL(TMPINTDP_ISIN,TMPINTDP_QTY,'HIGH_VALUE',TMPINTDP_BO_ID,'') ='Y'
)
BEGIN 

SELECT DISTINCT @L_ERR_MISSING_ISIN_DOR =  'BUT FOLLOWING SLIPS ' + ISNULL(@L_ERR_MISSING_ISIN_DOR,'')  + TMPINTDP_SLIP_NO + ' ARE MARKED WITH DORMANT SO NEED TO DO DUAL CHECKER PROCESS : '   FROM  TMP_DP_INTDP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL('',0,'DORMANT',TMPINTDP_BO_ID,CONVERT(DATETIME,TMPINTDP_REQUEST_DT,103))='Y'

SELECT DISTINCT @L_ERR_MISSING_ISIN_HV = 'BUT FOLLOWING SLIPS ' + ISNULL(@L_ERR_MISSING_ISIN_HV,'')  + TMPINTDP_SLIP_NO  + ' ARE MARKED WITH HIGH VALUE SO NEED TO DO DUAL CHECKER PROCESS : '  FROM  TMP_DP_INTDP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL(TMPINTDP_ISIN,TMPINTDP_QTY,'HIGH_VALUE',TMPINTDP_BO_ID,'') ='Y'
...
select @l_err_mstr = isnull(replace(citrus_usr.fn_merge_str('CDSL_IMP_INTDP',0,''),',',''),'')
    ...
    */
				...
				TMPINTDP_ISIN,     
				TMPINTDP_BATCH_NO ,  
				isnull(TMPINTDP_REMARKS,''), --TMPINTDP_LINE_NO,  
				TMPINTDP_QTY ,  
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPINTDP_BO_ID,convert(datetime,TMPintdp_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPintdp_ISIN,abs(TMPintdp_QTY),'HIGH_VALUE',TMPINTDP_BO_ID,'') end = 'Y' then -1 else 0 end, --0,--case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPINTDP_ACCT_ID,convert(datetime,TMPINTDP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPINTDP_ISIN,abs(TMPINTDP_QTY),'HIGH_VALUE','','') end = 'Y' then 0 else 0 end,  
				TMPINTDP_TRASTM_CD ,  
				@L_MAX_DPTDC_DTLS_ID1,  
				NULL, 
				NULL,
				NULL,
				isnull(TMPINTDP_REASON_CD,''),
				'P' ,  
				TMPINTDP_TRASTM_CD ,  
			...
			--/*15062016*/
			if exists (select 1 from #tmp_dptdc_intdp where  [citrus_usr].[fn_get_high_val_byslip_broker](DPTDC_SLIP_NO)='Y')
			update dptdc set dptdc_deleted_ind = -1 from dptdc_mak dptdc , #tmp_dptdc_intdp tmp
			where dptdc.dptdc_slip_no= tmp.dptdc_slip_no
...
--				END
						...
						IF EXISTS(SELECT TOP 1 ISNULL(DPTDC_ID,0) FROM DP_TRX_DTLS_CDSL , DP_ACCT_MSTR WHERE DPTDC_BROKERBATCH_NO IN (SELECT TMPNP_BROKERBATCH_NO FROM TMP_DP_NP_DTLS_CDSL) AND DPAM_ID = DPTDC_DPAM_ID AND DPAM_DPM_ID = @l_dpm_id AND DPAM_DELETED_IND = 1 AND 
DPTDC_DELETED_IND = 1 and DPTDC_INTERNAL_TRASTM like 'NP') 
...
SELECT @L_ERR_MISSING_ISIN  =  isnull(replace(citrus_usr.fn_merge_str('CDSL_ISIN_NP',0,''),',',''),'')    
 ...
 set @L_ERR_MISSING_ISIN = ''
    SELECT @L_ERR_MISSING_ISIN = isnull(replace(citrus_usr.fn_merge_str('BBO_CODE_MISSING_NP',0,''),',',''),'')
    IF @L_ERR_MISSING_ISIN <> ''
    BEGIN
            UPDATE filetask
			SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Client Does not have Broker BackOffice Code : ' + @L_ERR_MISSING_ISIN
...
--    return 
--    end 
select TMPNP_SLIP_NO into #tempused_np from TMP_DP_NP_DTLS_CDSL, used_slip 
  ...
  if exists(select 1 from #tempused_np )
    begin

     SELECT distinct @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  + TMPNP_SLIP_NO + ',' from #tempused_np 
...
SET @L_ERR_MISSING_ISIN_HV=''
IF EXISTS (
SELECT TMPNP_SLIP_NO   FROM  TMP_DP_NP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL('',0,'DORMANT',TMPnP_BO_ID,CONVERT(DATETIME,TMPNP_REQUEST_DT,103))='Y'
OR CITRUS_USR.FN_GET_HIGH_VAL(TMPnP_ISIN,TMPNP_QTY,'HIGH_VALUE',TMPNP_BO_ID,'') ='Y'
)
BEGIN 

SELECT DISTINCT @L_ERR_MISSING_ISIN_DOR =  'BUT FOLLOWING SLIPS ' + ISNULL(@L_ERR_MISSING_ISIN_DOR,'')  + TMPNP_SLIP_NO + ' ARE MARKED WITH DORMANT SO NEED TO DO DUAL CHECKER PROCESS : '   FROM  TMP_DP_NP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL('',0,'DORMANT',TMPNP_BO_ID,CONVERT(DATETIME,TMPNP_REQUEST_DT,103))='Y'

SELECT DISTINCT @L_ERR_MISSING_ISIN_HV = 'BUT FOLLOWING SLIPS ' + ISNULL(@L_ERR_MISSING_ISIN_HV,'')  + TMPNP_SLIP_NO  + ' ARE MARKED WITH HIGH VALUE SO NEED TO DO DUAL CHECKER PROCESS : '  FROM  TMP_DP_NP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL(TMPNP_ISIN,TMPNP_QTY,'HIGH_VALUE',TMPNP_BO_ID,'') ='Y'
...
END
        ...
        select @l_id2 = count (*) from TMP_DP_NP_DTLS_CDSL
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPNP_BO_ID,convert(datetime,TMPnP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPnP_ISIN,abs(TMPnP_QTY),'HIGH_VALUE',TMPNP_BO_ID,'') end = 'Y' then -1 else 0 end,  --0,
--case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPNP_ACCT_ID,convert(datetime,TMPNP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPNP_ISIN,abs(TMPNP_QTY),'HIGH_VALUE','','') end = 'Y' then 0 else 0 end,  
			...
			--/*15062016*/
			if exists (select 1 from #tmp_dptdc_np where  [citrus_usr].[fn_get_high_val_byslip_broker](DPTDC_SLIP_NO)='Y')
			update dptdc set dptdc_deleted_ind = -1 from dptdc_mak dptdc , #tmp_dptdc_np tmp
			where dptdc.dptdc_slip_no= tmp.dptdc_slip_no
...
--				END
					...
					IF EXISTS(SELECT TOP 1 ISNULL(DPTDC_ID,0) FROM DP_TRX_DTLS_CDSL , DP_ACCT_MSTR WHERE DPTDC_BROKERBATCH_NO IN (SELECT TMPEP_BROKERBATCH_NO FROM TMP_DP_EP_DTLS_CDSL) AND DPAM_ID = DPTDC_DPAM_ID AND DPAM_DPM_ID = @l_dpm_id AND DPAM_DELETED_IND = 1 AND DPTDC_DELETED_IND = 1 and DPTDC_INTERNAL_TRASTM like 'EP') 
														OR EXISTS(SELECT TOP 1 ISNULL(DPTDC_ID,0) FROM DPTDC_MAK, DP_ACCT_MSTR WHERE DPTDC_BROKERBATCH_NO IN (SELECT TMPEP_BROKERBATCH_NO FROM TMP_DP_EP_DTLS_CDSL) AND DPAM_ID = DPTDC_DPAM_ID AND DPAM_DPM_ID = @l_dpm_id AND DPAM_DELETED_IND  = 1 AND
 DPTDC_DELETED_IND = 0 and DPTDC_INTERNAL_TRASTM like 'EP')
								BEGIN
								--
										--SET @PA_ERRMSG = 'SELECTED BATCH NO ALREADY EXISTS.'
										--SELECT ERRMSG =  @PA_ERRMSG, ERROR ='B'
...
set @L_ERR_MISSING_ISIN = ''
    SELECT @L_ERR_MISSING_ISIN = isnull(replace(citrus_usr.fn_merge_str('BBO_CODE_MISSING_EP',0,''),',',''),'')
    IF @L_ERR_MISSING_ISIN <> ''
    BEGIN
            UPDATE filetask
			SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Client Does not have Broker BackOffice Code : ' + @L_ERR_MISSING_ISIN
...
--    return 
--    end 
select TMPEP_SLIP_NO into #tempused_ep from TMP_DP_EP_DTLS_CDSL, used_slip 
  ...
  if exists(select 1 from #tempused_ep 
            )
    begin

     SELECT distinct @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  + TMPEP_SLIP_NO + ',' from #tempused_ep 
...
SET @L_ERR_MISSING_ISIN_HV=''
IF EXISTS (
SELECT TMPEP_SLIP_NO   FROM  TMP_DP_EP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL('',0,'DORMANT',TMPEP_BO_ID,CONVERT(DATETIME,TMPEP_REQUEST_DT,103))='Y'
OR CITRUS_USR.FN_GET_HIGH_VAL(TMPEP_ISIN,TMPEP_QTY,'HIGH_VALUE',TMPEP_BO_ID,'') ='Y'
)
BEGIN 

SELECT DISTINCT @L_ERR_MISSING_ISIN_DOR =  'BUT FOLLOWING SLIPS ' + ISNULL(@L_ERR_MISSING_ISIN_DOR,'')  + TMPEP_SLIP_NO + ' ARE MARKED WITH DORMANT SO NEED TO DO DUAL CHECKER PROCESS : '   FROM  TMP_DP_EP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL('',0,'DORMANT',TMPEP_BO_ID,CONVERT(DATETIME,TMPEP_REQUEST_DT,103))='Y'

SELECT DISTINCT @L_ERR_MISSING_ISIN_HV = 'BUT FOLLOWING SLIPS ' + ISNULL(@L_ERR_MISSING_ISIN_HV,'')  + TMPEP_SLIP_NO  + ' ARE MARKED WITH HIGH VALUE SO NEED TO DO DUAL CHECKER PROCESS : '  FROM  TMP_DP_EP_DTLS_CDSL
WHERE CITRUS_USR.FN_GET_HIGH_VAL(TMPEP_ISIN,TMPEP_QTY,'HIGH_VALUE',TMPEP_BO_ID,'') ='Y'
...
END
 ...
 begin transaction
			 ...
			 ,dptdc_cm_id
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPEP_ACCT_ID,convert(datetime,TMPEP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPEP_ISIN,abs(TMPEP_QTY),'HIGH_VALUE','','') end = 'Y' then -1 else 0 end,  --0,--case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPEP_ACCT_ID,convert(datetime,TMPEP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPEP_ISIN,abs(TMPEP_QTY),'HIGH_VALUE','','') end = 'Y' then 0 else 0 end,  
				TMPEP_TRASTM_CD ,  
				@L_MAX_DPTDC_DTLS_ID1,  
				NULL, 
				NULL,
			...
			--/*15062016*/
			if exists (select 1 from #tmp_dptdc_ep where  [citrus_usr].[fn_get_high_val_byslip_broker](DPTDC_SLIP_NO)='Y')
			update dptdc set dptdc_deleted_ind = -1 from dptdc_mak dptdc , #tmp_dptdc_ep tmp
			where dptdc.dptdc_slip_no= tmp.dptdc_slip_no
...
INSERT INTO used_slip
--SELECT distinct @l_id_used_slip + ID1 , @l_dpm_id1 , TMPOFFM_BO_ID , 
--convert(numeric,replace(rtrim(ltrim(TMPOFFM_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),''))
...
--and isnumeric(rtrim(ltrim(TMPOFFM_SLIP_NO))) = 1
--and convert(numeric,rtrim(ltrim(TMPOFFM_SLIP_NO))) 
--between convert(numeric,SLIIM_SLIP_NO_FR) and convert(numeric,SLIIM_SLIP_NO_TO) 
--UNION ALL
SELECT distinct @l_id_used_slip + ID1 , @l_dpm_id1 , dppd_master_id , convert(numeric,replace(rtrim(ltrim(TMPOFFM_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),''))
,'1',SLIIM_SERIES_TYPE,'U','HO',getdate(),'HO',getdate(),1 ,''
FROM #tmp_dptdc,TMP_DP_OFFM_DTLS_CDSL with (nolock), slip_issue_mstr_POA
 with (nolock),DP_ACCT_MSTR with (nolock),DP_POA_DTLS 
with (nolock) WHERE TMPOFFM_SLIP_NO = DPTDC_SLIP_NO  
AND DPAM_SBA_NO=TMPOFFM_BO_ID AND DPAM_ID=DPPD_DPAM_ID AND DPPD_DELETED_IND=1 AND DPAM_DELETED_IND=1 
AND SLIIM_DPAM_ACCT_NO=dppd_master_id
and convert(numeric,rtrim(ltrim(TMPOFFM_SLIP_NO))) between convert(numeric,SLIIM_SLIP_NO_FR) 
and convert(numeric,SLIIM_SLIP_NO_TO) 
...
UNION ALL
SELECT distinct  @l_id_used_slip + ID1 , @l_dpm_id1 , dppd_master_id , convert(numeric,replace(rtrim(ltrim(TMPINTDP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'')),'1',SLIIM_SERIES_TYPE,'U','HO',getdate(),'HO',getdate(),1 ,''
FROM #tmp_dptdc_intdp,TMP_DP_INTDP_DTLS_CDSL , slip_issue_mstr_poa,DP_ACCT_MSTR,DP_POA_DTLS WHERE TMPINTDP_SLIP_NO = DPTDC_SLIP_NO 
--and rtrim(ltrim(TMPINTDP_SLIP_NO)) like ltrim(rtrim(SLIIM_SERIES_TYPE)) + '%' 
...
and isnumeric(rtrim(ltrim(TMPNP_SLIP_NO))) = 1
and convert(numeric,rtrim(ltrim(TMPNP_SLIP_NO))) between convert(numeric,SLIIM_SLIP_NO_FR) and convert(numeric,SLIIM_SLIP_NO_TO) 
UNION ALL
SELECT distinct @l_id_used_slip + ID1 , @l_dpm_id1 , dppd_master_id , convert(numeric,replace(rtrim(ltrim(TMPNP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'')),'1',SLIIM_SERIES_TYPE,'U','HO',getdate(),'HO',getdate(),1 ,''
FROM #tmp_dptdc_np,TMP_DP_NP_DTLS_CDSL, slip_issue_mstr_poa,DP_ACCT_MSTR,DP_POA_DTLS WHERE TMPNP_SLIP_NO = DPTDC_SLIP_NO 
--and rtrim(ltrim(TMPNP_SLIP_NO)) like ltrim(rtrim(SLIIM_SERIES_TYPE)) + '%' 
AND DPAM_SBA_NO=TMPNP_BO_ID AND DPAM_ID=DPPD_DPAM_ID AND DPPD_DELETED_IND=1 AND DPAM_DELETED_IND=1 AND SLIIM_DPAM_ACCT_NO=dppd_master_id
--and isnumeric(rtrim(ltrim(TMPNP_SLIP_NO))) = 1
and convert(numeric,rtrim(ltrim(TMPNP_SLIP_NO))) between convert(numeric,SLIIM_SLIP_NO_FR) and convert(numeric,SLIIM_SLIP_NO_TO) 
...
and isnumeric(rtrim(ltrim(TMPEP_SLIP_NO))) = 1
and convert(numeric,rtrim(ltrim(TMPEP_SLIP_NO))) between convert(numeric,SLIIM_SLIP_NO_FR) and convert(numeric,SLIIM_SLIP_NO_TO) 

UNION ALL
SELECT distinct @l_id_used_slip + ID1 , @l_dpm_id1 , dppd_master_id , convert(numeric,replace(rtrim(ltrim(TMPEP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'')),'1',SLIIM_SERIES_TYPE,'U','HO',getdate(),'HO',getdate(),1 ,''
FROM #tmp_dptdc_ep,TMP_DP_EP_DTLS_CDSL , slip_issue_mstr_poa,DP_ACCT_MSTR,DP_POA_DTLS WHERE TMPEP_SLIP_NO = DPTDC_SLIP_NO 
--and rtrim(ltrim(TMPEP_SLIP_NO)) like ltrim(rtrim(SLIIM_SERIES_TYPE)) + '%' 
AND DPAM_SBA_NO=TMPEP_BO_ID AND DPAM_ID=DPPD_DPAM_ID AND DPPD_DELETED_IND=1 AND DPAM_DELETED_IND=1 AND SLIIM_DPAM_ACCT_NO=dppd_master_id
--and isnumeric(rtrim(ltrim(TMPEP_SLIP_NO))) = 1
and convert(numeric,rtrim(ltrim(TMPEP_SLIP_NO))) between convert(numeric,SLIIM_SLIP_NO_FR) and convert(numeric,SLIIM_SLIP_NO_TO) 
...
drop table #tmp_dptdc_ep
drop table #tmp_dptdc_np
drop table #tmp_dptdc_intdp
drop table #tmp_dptdc
