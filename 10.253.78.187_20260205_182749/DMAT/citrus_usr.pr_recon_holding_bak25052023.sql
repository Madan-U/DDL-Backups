-- Object: PROCEDURE citrus_usr.pr_recon_holding_bak25052023
...
create proc [citrus_usr].[pr_recon_holding_bak25052023](@pa_excsm_id numeric
, @pa_date datetime
, @pa_flag char(15)
...
where convert(varchar(11),CDSHM_TRAS_DT,109) = convert(varchar(11),@pa_date,109)
and cdshm_dpm_id = @pa_dpm_id
and CDSHM_TRATM_CD in ('4456') and CDSHM_CDAS_SUB_TRAS_TYPE ='417' 
and CDSHM_BEN_ACCT_NO not in (select dpam_sba_no  from dp_Acct_mstr where dpam_subcm_cd in (select subcm_cd from SUB_CTGRY_MSTR where subcm_desc like '%cuspa%'))
