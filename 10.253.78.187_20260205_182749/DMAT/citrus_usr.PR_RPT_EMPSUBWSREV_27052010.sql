-- Object: PROCEDURE citrus_usr.PR_RPT_EMPSUBWSREV_27052010
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[PR_RPT_EMPSUBWSREV_27052010](@PA_EXCSM_ID NUMERIC ,@PA_ENTTM_CD VARCHAR(20),@PA_FROM_DT DATETIME, @PA_TO_DT DATETIME ,@PA_OUT VARCHAR(8000) OUT)
AS
BEGIN 
...
IF @PA_ENTTM_CD = 'EMP'
select COUNT(ENTR_SBA)*100 REV_VALUE, ENTM_SHORT_NAME ,entr_sba 
...
IF @PA_ENTTM_CD = 'SUB_BROK'
select COUNT(ENTR_SBA)*100 REV_VALUE, ENTM_SHORT_NAME  
from DP_ACCT_MSTR , entity_relationship , ENTITY_MSTR , #account_properties
WHERE DPAM_SBA_NO = ENTR_SBA AND ACCP_CLISBA_ID = DPAM_ID 
AND ENTR_DUMMY3 = ENTM_ID 
AND ENTM_ENTTM_CD = 'S_BRK'
AND ENTR_TO_DT >= 'JAN 01 2099'
AND DPAM_DPM_ID = @L_DPM_ID 
AND ACCP_VALUE BETWEEN @PA_FROM_DT AND @PA_TO_DT
AND ENTR_DELETED_IND = 1
