-- Object: PROCEDURE citrus_usr.PR_RPT_EOD_COLLECTION_SUMMARY_GRPBY
...
create PROCEDURE [citrus_usr].[PR_RPT_EOD_COLLECTION_SUMMARY_GRPBY](@PA_ID  NUMERIC  
...
IF @PA_BATCH_NO   = 'HO'
BEGIN 
exec [citrus_usr].[PR_RPT_EOD_COLLECTION_SUMMARY] @PA_ID   
                                   ,@PA_FROM_ACCT  
                                   ,@PA_TO_ACCT    
                                   ,@PA_FROM_DT   
                                  -- ,@PA_TO_DT     DATETIME  
...
select @@l_sql  = citrus_usr.fn_createheirarchy_update('#ACLIST',@PA_BATCH_NO)
print @@l_sql  
exec(@@l_sql)
...
--,EOD_GRP_CD VARCHAR(20)
,EOD_ENTM_NAME VARCHAR(100)  
)  
...
BEGIN  
 ...
 INSERT INTO #EOD_IND(EOD_DESC,EOD_PAY_MODE,EOD_DR_AMOUNT,EOD_CR_AMOUNT,EOD_USER_ID,EOD_ENTM_NAME)  
 SELECT '1',INWSR_PAY_MODE PAYMENT_MODE   
      , '0' DR_AMOUNT  
      , ISNULL(INWSR_UFCHARGE_COLLECTED,'0') CR_AMOUNT  
      , INWSR_CREATED_BY [ENTRY_ID],entm_name1 
 FROM   #LEDGER , INWARD_SLIP_REG    
    ,   DP_ACCT_MSTR DPAM  
    ,   DP_MSTR , #ACLIST ACCOUNT  , entity_mstr,login_names
 WHERE  LDG_CREATED_BY ='INWARDENTRY' and entm_short_name = LOGN_SHORT_NAME and logn_name =  INWSR_CREATED_BY   
 AND    ISNULL(INWSR_TRASTM_CD,'') + '_'+ ISNULL(convert(varchar,INWSR_ID),'') =  LDG_REF_NO  
 ...
 AND    convert(varchar(11),INWSR_RECD_DT,103) = convert(varchar(11),@PA_FROM_DT,103)  
 ...
 SELECT '1','AUTO CREDIT ' PAYMENT_MODE   
      ...
      , LDG_CREATED_BY [ENTRY_ID]   ,entm_name1 
 ...
 and    entm_short_name = LOGN_SHORT_NAME and logn_name =  LDG_CREATED_BY 
 AND    ACCOUNT.DPAM_ID = DPAM.DPAM_ID   
...
INSERT INTO #EOD_IND   
SELECT '2', INWCR_PAY_MODE ,'0' , inwcr_charge_collected , inwcr_created_by, entm_name1 FROM inw_client_reg, entity_mstr,login_names  
where convert(varchar(11),inwcr_created_dt,103) = convert(varchar(11),@PA_FROM_DT,103)  
and    entm_short_name = LOGN_SHORT_NAME and logn_name =  inwcr_CREATED_BY  
and inwcr_deleted_ind = 1   
  



SELECT EOD_DESC,EOD_PAY_MODE,SUM(EOD_DR_AMOUNT+EOD_CR_AMOUNT) AMOUNT,EOD_USER_ID,EOD_ENTM_NAME INTO #EOD_IND_SUMMARY FROM #EOD_IND  
GROUP BY EOD_DESC,EOD_PAY_MODE,EOD_USER_ID, EOD_ENTM_NAME  
...
--,EOD_GRP_BY    VARCHAR(20)
,EOD_ENTM_NAME1 VARCHAR(100)  
)  
  
INSERT INTO #EOD_COLLECTION_SUMMARY(EOD_DESC,EOD_USER_ID,EOD_ENTM_NAME1)  
SELECT DISTINCT EOD_DESC , EOD_USER_ID,EOD_ENTM_NAME   
FROM #EOD_IND  
...
SELECT distinct CASE WHEN EOD_DESC ='1' THEN 'Total Miscellaneous Credits:'
...
,EOD_AUTO_CR     
,EOD_AUTO_DR     
,EOD_WAIVER ,EOD_ENTM_NAME1    FROM #EOD_COLLECTION_SUMMARY  
