-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_080910
...
pr_dp_select_mstr	'0','CLIENT_INFO','HO','64219','','','','',0,'*|~*','|*~|',''	
 
*/

 --pr_dp_select_mstr '90123456','GETDORMANTCLTMSG','','0','','','01/09/2009','',0,'*|~*','|*~|','' 
 CREATE PROCEDURE  [citrus_usr].[pr_dp_select_mstr_080910]
	( 
	@pa_id  VARCHAR(20)                  
...
if  len(@pa_id) < 5            
  ...
  WHERE  excsm.excsm_compm_id         = compm.compm_id                  
       AND    excsm.excsm_id               = dpm.default_dp                
       AND    finy.fin_dpm_id              = dpm.dpm_id                
       --AND    convert(int,rola_access1) & power(1,bitrm_bit_location-1) = power(1,bitrm_bit_location-1)                 
...
,case WHEN abs(dptd_qty) * clopm_nsdl_rt > CONVERT(INT,@PA_VALUES) then 'Y' ELSE 'N' END [HIGH VALUE]                            
    ...
    IF @pa_cd = 'PLDG'                
      BEGIN                
      --                
		 Select  PLDT_ID [INSTRUCTION NO]
		,PLDT_ISIN           ISIN
		,abs(PLDT_QTY)       QTY
		,case when abs(PLDT_QTY)* clopm_nsdl_rt > CONVERT(INT,@PA_VALUES) then 'Y' ELSE 'N' END [HIGH VALUE]  
		from NpledgeD_mak
		left outer join                 
		closing_last_nsdl on PLDT_ISIN            = clopm_isin_cd 
		where  PLDT_DTLS_ID = @pa_id                
		and    PLDT_DELETED_IND in(0,-1)               
		order by PLDT_DTLS_ID  
...
print   '@pa_login_name ' +  @pa_login_name
print   '@pa_cd ' +  convert(varchar,@pa_cd)
print   '@l_dpm_dpid ' +  convert(varchar,@l_dpm_dpid)
print   '@l_stam_cd ' +  convert(varchar,@l_stam_cd)

       
        SELECT isnull(C.CLIM_NAME1,'')  + ' ' + isnull(C.CLIM_NAME2,'') + ' ' +isnull(C.CLIM_NAME3,'') CLIM_NAME1                
              ,isnull(D.DPHD_SH_FNAME,'') + ' ' + isnull(D.DPHD_SH_MNAME,'') + ' ' + isnull(D.DPHD_SH_LNAME,'')   DPHD_SH_FNAME             
              ,isnull(D.DPHD_TH_FNAME,'') + ' ' + isnull(D.DPHD_TH_MNAME,'') + ' ' + isnull(D.DPHD_TH_LNAME,'')   DPHD_TH_FNAME                
              ,A.DPAM_SBA_NO                  
       ...
       --                
          ...
          left outer join                   
          account_properties accp on dpam.DPAM_ID = accp.accp_clisba_id and accp.accp_accpm_prop_cd = 'CMBP_iD'   AND   isnull(accp.accp_value,'') = ''                          
       ...
       --                
          ...
          WHERE C.CLIM_CRN_NO = dpam.DPAM_CRN_NO                  
          AND   dpam.DPAM_SBA_NO =@pa_cd                
          and   dpam.dpam_stam_cd in (@l_stam_cd ,'03')               
...
AND    enttm_deleted_ind = 1                
 ...
 IF @pa_action = 'SETM_NO_CHK_DATEWISE_CSPL'
	begin
	select  top 1 SETM_NO from settlement_mstr
	where SETM_SETTM_ID = convert(numeric,@pa_cd) 
	and @pa_rmks < CONVERT(DATETIME,CONVERT(VARCHAR(11),SETM_PAYIN_DT,109) + ' ' +left(SETM_DEADLINE_TIME,2) +':' +right(SETM_DEADLINE_TIME,2) ,103)
	ORDER BY SETM_PAYIN_DT asc              
end                  
  ...
  --              
   ...
   and    trantm_deleted_ind       = 1            
   and    TRASTM_CD     in ('202','042','033','011','012','021','022','052','071','212','091','092','093','062','REPURCHASE')            
   order by trastm_cd            
    end            
  ...
  --                
   ...
   SELECT entm_name1                                                                    cm_nm  ,'' fh_name,'' sh_name,''th_name,'' name_of_cc               
   FROM   entity_mstr                 
  ...
  --            
    ...
    FROM   demat_request_mstr          demrm 
		   left outer join  demat_tran_tranm_dtls       demttd  on   demttd.demttd_demrm_id    = demrm.demrm_id         
    ...
    AND    convert(varchar,demrm.demrm_request_dt,103)      LIKE CASE WHEN LTRIM(RTRIM(@pa_cd)) = '' THEN convert(varchar,demrm.demrm_request_dt,103)  ELSE @pa_cd  END            
    ...
    and DEMRD_DISTINCTIVE_NO_FR = case when @RowDelimiter = 'M' then 'M' 
                                         WHEN DEMRD_DISTINCTIVE_NO_FR <> 'M' THEN  DEMRD_DISTINCTIVE_NO_FR  end                      
  ...
  --                
    ...
    --               
      SELECT distinct @l_dpm_dpid                  DPID                 
			...
			,REMRM_INTERNAL_REJ    
			,REMRM_COMPANY_OBJ                 
			,REMRM_CREDIT_RECD    , REMRM_REPURCHASE_FLG   
  ...
  --                
    ...
    END                
      ...
      SELECT compm_short_name +'-'+ excsm_exch_cd+'-'+excsm_seg_cd COMPANY                   
              ...
              ,dpm_name                 DPNAME                
              ,''                       ERRMSG 
              , isnull(convert(varchar(11),sliim_dt,103),'') sliim_dt                 
...
,citrus_usr.fn_acct_list(@l_dpm_dpid,@l_entm_id,0) dpam                 
  ...
  --                
    SELECT  dptd_counter_dp_id                            
     ...
     ,dptd_id   
      --,ISNULL(DPTD_TRANS_NO,'')  DPTD_TRANS_NO  
     ,CASE WHEN ISNULL(DPTD_TRANS_NO,'')='' THEN ISNULL(DPTD_ID,'') ELSE  ISNULL(DPTD_TRANS_NO,'') END DPTD_TRANS_NO      
,CASE WHEN DPTD_DELETED_IND IN (0,6,-1)  THEN 'MAKER ENTERED'
		WHEN ISNULL(DPTD_BATCH_NO,'') <> '' AND ISNULL(DPTD_TRANS_NO,'') <> ''  THEN 'BATCH UPLOADED' 
		WHEN ISNULL(DPTD_BATCH_NO,'') <> '' AND ISNULL(DPTD_TRANS_NO,'') =  ''  THEN 'BATCH GENERATED'
		WHEN DPTD_DELETED_IND = 1 AND ISNULL(DPTD_BATCH_NO,'') = '' AND ISNULL(DPTD_TRANS_NO,'') =  ''  THEN 'CHECKER APPROVED' END  STATUS
...
,CASE WHEN DPTD_DELETED_IND IN (0,6,-1)  THEN 'MAKER ENTERED'
 ...
 dptd.DPTD_EXECUTION_DT and (dptd_deleted_ind = -1 or isnull(ltrim(rtrim(dptd_mid_chk)),'') <> '' ) ),'NO')                
    ...
    and    isnull(dptd_brokerbatch_no,'') = '' 
    and    dptd_slip_no like case when @pa_rmks = '' then '%' else @pa_rmks end  
    ORDER BY DPTD_DTLS_ID               
  --                
  END                
    ...
    ,CASE WHEN ISNULL(DPTD_TRANS_NO,'')='' THEN ISNULL(DPTD_ID,'') ELSE  ISNULL(DPTD_TRANS_NO,'') END DPTD_TRANS_NO     
     ,CASE WHEN DPTD_DELETED_IND IN (0,6,-1)  THEN 'MAKER ENTERED'
		...
		WHEN DPTD_DELETED_IND = 1 AND ISNULL(DPTD_BATCH_NO,'') = '' AND ISNULL(DPTD_TRANS_NO,'') =  ''  THEN 'CHECKER APPROVED' END  STATUS
         ...
         ,settm_id                  TARGETSETLLEMENTTYPE                
            ,dptd_other_settlement_no    TRAGETSETTLEMENTNO                
            ,dptd_counter_cmbp_id        TARGETCMBPID               
    FROM   dp_trx_dtls                 dptd     ,settlement_type_mstr            settm                               
    WHERE  dptd_dtls_id              = @pa_id  
    AND    settm_id                  = dptd_other_settlement_type                             
    AND    dptd_deleted_ind          = 1                
    AND    dptd_trastm_cd            = '904'                
...
,CASE WHEN DPTD_DELETED_IND IN (0,6,-1)  THEN 'MAKER ENTERED'
		...
		WHEN DPTD_DELETED_IND = 1 AND ISNULL(DPTD_BATCH_NO,'') = '' AND ISNULL(DPTD_TRANS_NO,'') =  ''  THEN 'CHECKER APPROVED' END  STATUS
            ...
            ,dptd_other_settlement_no    TRAGETSETTLEMENTNO                
            ,dptd_counter_cmbp_id        TARGETCMBPID                 
    FROM   dptd_mak                    dptd          ,settlement_type_mstr            settm                
    WHERE  dptd_dtls_id              = @pa_id 
    AND    settm_id                  = dptd_other_settlement_type                               
    AND    dptd_deleted_ind          IN(0,6,-1)                
    AND    dptd_trastm_cd            = '904'                
...
,CASE WHEN DPTD_DELETED_IND IN (0,6,-1)  THEN 'MAKER ENTERED'
		...
		WHEN DPTD_DELETED_IND = 1 AND ISNULL(DPTD_BATCH_NO,'') = '' AND ISNULL(DPTD_TRANS_NO,'') =  ''  THEN 'CHECKER APPROVED' END  STATUS
                           --,settm_desc                  MARKETTYPE                
,dptd_mkt_type MARKETTYPE --settm_desc                  MARKETTYPE 
          ,dptd_settlement_no          SETTLEMENTNO            
...
,CASE WHEN DPTD_DELETED_IND IN (0,6,-1)  THEN 'MAKER ENTERED'
 ...
 dptd.DPTD_EXECUTION_DT and (dptd_deleted_ind = -1 or isnull(ltrim(rtrim(dptd_mid_chk)),'') <> '' ) ),'NO')                
    ...
    --AND    excsm_list.excsm_id      = excsm.excsm_id                
    --AND    dpam.dpam_dpm_id          = dpm.dpm_id                
    AND    convert(varchar,settm.settm_id)            = dptd.dptd_mkt_type                
    AND    dptd_deleted_ind         IN (0,4,6,-1)                
    AND    accp_clisba_id            = dpam.dpam_id                
    AND    accp.accp_accpm_prop_cd   = 'CMBP_ID'                
    --AND    dpm_excsm_id = convert(int,@pa_cd)                
...
,CASE WHEN DPTD_DELETED_IND IN (0,6,-1)  THEN 'MAKER ENTERED'
		...
		WHEN DPTD_DELETED_IND = 1 AND ISNULL(DPTD_BATCH_NO,'') = '' AND ISNULL(DPTD_TRANS_NO,'') =  ''  THEN 'CHECKER APPROVED' END  STATUS
                   ,settm1.settm_desc            TARGETSETLLEMENTTYPE                
         ,dptd_other_settlement_no    TRAGETSETTLEMENTNO             
            ,settm1.settm_id               TARGETSETLLEMENTTYPEID      
    FROM    dp_trx_dtls                 dptd       ,settlement_type_mstr        settm1                           
    WHERE   dptd_dtls_id              = @pa_id
    AND    convert(varchar,settm1.settm_id)           = dptd.dptd_other_settlement_type                                
    AND     dptd_deleted_ind          = 1                
    AND    dptd_trastm_cd           in ('907')                
...
,CASE WHEN DPTD_DELETED_IND IN (0,6,-1)  THEN 'MAKER ENTERED'
		...
		WHEN DPTD_DELETED_IND = 1 AND ISNULL(DPTD_BATCH_NO,'') = '' AND ISNULL(DPTD_TRANS_NO,'') =  ''  THEN 'CHECKER APPROVED' END  STATUS
             ...
             ,settm1.settm_id               TARGETSETLLEMENTTYPEID 
         ,dptd_other_settlement_no    TRAGETSETTLEMENTNO    
    FROM    dptd_mak                    dptd       ,settlement_type_mstr        settm1                        
    WHERE   dptd_dtls_id      = @pa_id    
AND    convert(varchar,settm1.settm_id)           = dptd.dptd_other_settlement_type                        
    AND     dptd_deleted_ind          IN (0,6,-1)                
    AND    dptd_trastm_cd           in ('907')                
...
dptd.DPTD_EXECUTION_DT and (dptd_deleted_ind = -1 or isnull(ltrim(rtrim(dptd_mid_chk)),'') <> '' ) ),'NO')                
    ...
    --AND    excsm_list.excsm_id      = excsm.excsm_id                
    AND    accp_clisba_id            = dpam.dpam_id                
    AND    convert(varchar,settm.settm_id)            = convert(varchar,dptd.dptd_mkt_type)                
    AND    convert(varchar,settm1.settm_id)          = dptd.dptd_other_settlement_type                
    AND    accp.accp_accpm_prop_cd   = 'CMBP_ID'                
    --AND    dpm_excsm_id              = CONVERT(INT,@pa_cd)                
    AND    dptd_deleted_ind         IN (0,4,6,-1)                
...
,DPTDC_MKT_TYPE 
 ...
 and    DPTDC_CREATED_BY like case when @pa_desc = '' then '%'  else @pa_desc end
   and   convert(varchar,dptdc_execution_dt,103)  = case when @pa_values = '' then convert(varchar,dptdc_execution_dt,103) else @pa_values end  
...
BEGIN        
  ...
  IF @pa_desc <> ''        
    ...
    --        
    SELECT distinct dpm.dpm_excsm_id      Depository        
    ,isnull(dpam.dpam_sba_no,'') AcctNo        ,fre_isin_cd     ISIN        
    ,fre_qty    Qty         
    ,CASE WHEN  fre_action = 'F' THEN 'FREEZE'  END        FrzAction        
    ,CASE WHEN  fre_type   = 'D' THEN 'Susp. for Debit' ELSE case when fre_type = 'A' THEN 'Susp. for All' else 'Account Closure' End END          FrzType                 
    ,case when  fre_req_int_by ='01' then 'Request by Investor' else case when fre_req_int_by ='02' THEN 'Other Reasons' else 'Request by Statutory Authority' end end REQ_INT_BY            
...
--                  
  ...
  --                
     SELECT distinct compm_short_name +'-'+ excsm_exch_cd+'-'+excsm_seg_cd  Depository                  
            ...
            ,case when @pa_desc = 'GENERAL' then 'GENERAL' else ttype.descp end           TransactionType                  
   ...
   LEFT OUTER JOIN FIN_ACCOUNT_MSTR ON    FINA_ACC_ID = INWSR_BANKID AND    FINA_DELETED_IND = 1                  
      ...
      AND     inwsr.inwsr_trastm_cd =  case when LTRIM(RTRIM(@pa_desc)) = 'GENERAL' then 'GENERAL' else ttype.cd   end 
   AND    dpm_excsm_id       = excsm.excsm_id                    
      AND     excsm_compm_id     = compm_id                   
      AND    excsm_list.excsm_id      = excsm.excsm_id                  
      AND    dpm.dpm_deleted_ind =1                        
      ...
      AND    inwsr.inwsr_trastm_cd = CASE WHEN LTRIM(RTRIM(@pa_desc)) = '' THEN ttype.cd when LTRIM(RTRIM(@pa_desc)) = 'GENERAL' then 'GENERAL' ELSE  @pa_desc END                                    
  ...
  --                
     SELECT compm_short_name +'-'+ excsm_exch_cd+'-'+excsm_seg_cd  Depository                  
                 ,dpam.dpam_sba_no    AccountNo                  
                 --,case when 'GENERAL' = 'GENERAL' then 'GENERAL' else ttype.descp end           TransactionType
				 ,case when LTRIM(RTRIM(@pa_desc)) = 'GENERAL' then 'GENERAL' else ttype.descp end           TransactionType                
                 ,INWSR_SLIP_NO        SlipNo                  
                 ,CONVERT(VARCHAR(11),INWSR_RECD_DT,103)   ReceivedDate                  
                 ,CONVERT(VARCHAR(11),INWSR_EXEC_DT,103)   executionDate                  
                 ,INWSR_NO_OF_TRANS                        TotalEntries                  
...
END                 
 ...
 --                
   ...
   FROM   fin_account_mstr left outer join ENTITY_MSTR on ISNULL(fina_branch_id,0) =entm_id                 
          AND fina_deleted_ind in (1) and entm_deleted_ind = 1                 
...
IF @pa_action ='VOUCHERDN_DTLS_SEL'                  
 ...
 IF @pa_action ='VOUCHERPAY_DTLS_SELM'                  
   BEGIN                  
   --                  
      set @l_sql ='        SELECT DISTINCT ldg_account_id = dpam_id                
     ,fina_acc_code = dpam_sba_no                
...
IF @pa_action ='VOUCHERCN_DTLS_SELM'                  
      BEGIN                  
      --                  
        set @l_sql ='        SELECT distinct ldg_account_id = dpam_id                
        ,fina_acc_code = dpam_sba_no                
     ...
     WHERE ldg_voucher_no   = ''' + @pa_cd               + '''             
      ...
      --                
  END         
 IF @pa_action ='VOUCHERDN_DTLS_SELM'                  
      BEGIN                  
      --                  
        set @l_sql ='        SELECT distinct ldg_account_id = dpam_id                
        ,fina_acc_code = dpam_sba_no                
...
*/
			...
			begin
				       ...
				       where accp_accpm_prop_cd = 'BILL_START_DT' and dpam_id = accp_clisba_id and dpam_sba_no =right(@pa_id,8)
						...
						and CDSHM_TRAS_DT >= DATEADD(m,-6,@pa_values) and CDSHM_TRAS_DT <= @pa_values)
                         or (abs(datediff(m,@l_act_dt,@pa_values))<=6)
						begin
							select @pa_id accno
						end
						else
 ...
 IF @PA_ACTION = 'CLT_LOGN_DP_FILL'                
  BEGIN                
  --                
   SELECT DISTINCT COMPM_SHORT_NAME + '-' + EXCSM_EXCH_CD + '-' + EXCSM_SUB_SEG_CD + '-' + DPM_DPID + '-' + DPAM_SBA_NO AS COMP , EXCSM_ID  ID                
...
,  CONVERT(VARCHAR,dpm_id) + '|*~|' + CONVERT(VARCHAR(11),fin_start_dt,109) + '|*~|' +  CONVERT(VARCHAR(11),fin_end_dt,109) + '|*~|' + CONVERT(VARCHAR,fin_id)  + '|*~|' + CONVERT(VARCHAR,EXCSM_ID)  + '|*~|'  id               
 ...
 --                
   ...
   select stam_desc [STATUS]                
        ...
        ,isnull([citrus_usr].[fn_conc_value](dpam_crn_no,'OFF_PH1'),'')  [off_ph]
         ...
         ,convert(char(8),ISNULL(citrus_usr.fn_ucc_accp(dpam_id,'CMBP_ID',''),''))  CMBPID
   from dp_acct_mstr                 
   left outer join                 
 ...
 --                
  ...
  select Excsm_id=default_dp,Acct_no=case  when  INWSR_TRASTM_CD not in ('904_P2C','907') then dpam_sba_no else citrus_usr.fn_acct_entp(dpam_id,'CMBP_ID') end,slip_no=INWSR_SLIP_NO,Request_dt=convert(varchar(11),INWSR_RECD_DT,103),Exec_dt=convert(varchar(11),INWSR_EXEC_DT,103),Trans_cnt=INWSR_NO_OF_TRANS,scr_cd = INWSR_TRASTM_CD,Scr_id= citrus_usr.Fn_GetSlipScreens(INWSR_TRASTM_CD,'2') ,UPPER(isnull(inwsr_transubtype_cd,'')) inwsr_transubtype_cd               
...
--                
  ...
  --                
   select ors_id                
       ...
       and  not exists (select slibm_id from slip_book_mstr where SLIBM_TRATM_ID =ors_tratm_id and SLIBM_SERIES_TYPE = ors_series_type and slibm_deleted_ind = 1 and ((SLIBM_FROM_NO between ors_from_slip and ors_to_slip) or (SLIBM_TO_NO between ors_from_slip and ors_to_slip)) )            
...
begin
 select * from financial_yr_mstr where fin_dpm_id=@l_dpm_dpid and fin_deleted_ind=1
 end

 if @pa_action = 'Outstanding_Amount'     
 begin
 ...
 (amount varchar(100)) 
  ...
  select @l_fin_id = fin_id from financial_yr_mstr where getdate() between fin_start_dt and FIN_END_DT and fin_deleted_ind =1 
 
  set @l_sql1 = 'insert into #temp select isnull(case when sum(ldg_amount)< 0 then convert(varchar,abs(sum(ldg_amount))) + '' '' + ''Dr'' else convert(varchar,abs(sum(ldg_amount))) + '' '' + ''Cr'' end  ,0) 
  from ledger'+convert(varchar,@l_fin_id ) +',dp_acct_mstr where ldg_account_id=dpam_id 
  and dpam_sba_no = ' + @pa_values 
  set @l_sql1 = @l_sql1  + ' and  not exists (select blocd_dpam_sba_no from blocked_client_dtls where dpam_sba_no		= blocd_dpam_sba_no and getdate() between blocd_blocked_from and isnull(blocd_blocked_to,''jan 01 2100'')) ' 
  set @l_sql1 = @l_sql1  + ' having sum(ldg_amount) <> ''0'' union select case when isnull(balance,''0'')+isnull(sum_of_charges,''0'') > 0 then convert(varchar,abs(isnull(balance,''0'')+isnull(sum_of_charges,''0''))) + '' Cr '' else convert(varchar,abs(isnull(balance,''0'')+isnull(sum_of_charges,''0''))) + '' Dr '' end  from blocked_client_dtls where blocd_dpam_sba_no = ' + @pa_values  + ' and getdate() between blocd_blocked_from and isnull(blocd_blocked_to,''jan 01 2100'') '  


print @l_sql1
exec(@l_sql1)

select @pa_ref_cur = amount from #temp
			...
			--	 		
			 ...
			 BEGIN  
			   --  
				  ...
				  FROM    BLK_CLIENT_EMAIL  
				   ,CLIENT_MSTR  
				  WHERE  blkce_entity_id = convert(varchar,CLIM_CRN_NO)  
				  AND CLIM_DELETED_IND=1  
				  AND  blkce_deleted_ind =1  
				  and  blkce_dpmdpid = @pa_rmks  
				  and     BLKCE_RPTNAME LIKE CASE WHEN LTRIM(RTRIM(@pa_cd)) = '' then '%' else @pa_cd end  
...
IF  @pa_action = 'CLIENTWITHEMAIL'     
  BEGIN
     SELECT DISTINCT ISNULL(CLIM_NAME1,'') + ' ' + ISNULL(CLIM_NAME2,'') + ' ' + ISNULL(CLIM_NAME3,'')  AS CLIENTMSTR ,CONC_VALUE ,DPAM_SBA_NO FROM CLIENT_MSTR,DP_ACCT_MSTR,ENTITY_ADR_CONC,CONTACT_CHANNELS WHERE ENTAC_CONCM_CD='EMAIL1' AND ENTAC_ADR_CONC_ID=CONC_ID
...
/*IF @pa_action = 'CLIENTWITHCONTACTDETAILS'             
	BEGIN
		SELECT DISTINCT ISNULL(CLIM_NAME1,'') + ' ' + ISNULL(CLIM_NAME2,'') + ' ' + ISNULL(CLIM_NAME3,'')  AS CLIENTMSTR ,CONC_VALUE ,DPAM_SBA_NO, ADR_1 + '' + ADR_2 ,entac_concm_cd
	    FROM CLIENT_MSTR,DP_ACCT_MSTR,ENTITY_ADR_CONC,CONTACT_CHANNELS,ADDRESSES 
		WHERE --ENTAC_CONCM_CD='EMAIL1' 
		 ENTAC_ADR_CONC_ID=CONC_ID
		AND CONC_VALUE<>'' AND CLIM_CRN_NO=ENTAC_ENT_ID AND CLIM_CRN_NO = DPAM_CRN_NO
		AND CLIM_CRN_NO = ADR_ID
		AND DPAM_STAM_CD='ACTIVE'
		ORDER BY  ISNULL(CLIM_NAME1,'') + ' ' + ISNULL(CLIM_NAME2,'') + ' ' + ISNULL(CLIM_NAME3,'') ASC
	END
--*/
