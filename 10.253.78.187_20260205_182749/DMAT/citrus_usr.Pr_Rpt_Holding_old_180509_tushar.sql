-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_old_180509_tushar
...
/* BEGIN TRANSACTION
 ...
 exec Pr_Rpt_Holding 'NSDL',4,'N','5/16/2009 12:00:00 AM','','','','N','1','HO|*~|'	,''

--Pr_Rpt_Holding 'nsdl',4,'Y','may 30 2008','','','','Y',1,'basis point|*~|',''  


*/
  
create Proc [citrus_usr].[Pr_Rpt_Holding_old_180509_tushar]  
@pa_dptype varchar(4),                    
@pa_excsmid int,                    
@pa_asondate char(1),                    
@pa_fordate datetime,                    
...
-------------------------------------------------------------------------------------------------
  ...
  ,DPDHM_ISIN ISINCD,isin_name ,convert(numeric(18,3),DPDHM_QTY)  Qty  
  ...
  select dpam_id=DPDHMD_DPAM_ID,DPAM_SBA_NAME AcctName,DPAM_sba_NO AcctNo,BEN_TYPE=BEN.DESCP + CASE WHEN LTRIM(RTRIM(DPDHMD_SETT_TYPE)) <> '00' AND ISNULL(LTRIM(RTRIM(DPDHMD_SETT_TYPE)),'') <> '' THEN  ' - ' + settm_desc +'/' + DPDHMD_SETT_NO  ELSE '' END
   
  ,DPDHMD_ISIN ISINCD,isin_name,convert(numeric(18,3),DPDHMD_QTY) Qty  
 ...
 INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,EFF_TO FROM citrus_usr.fn_acct_list(@@dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id)    
            print('c')          
 select DPAM_SBA_NAME,DPAM_sba_NO,DPHMC_ISIN,isin_name,convert(numeric(18,3),DPHMC_CURR_QTY),convert(numeric(18,3),DPHMC_FREE_QTY),convert(numeric(18,3),DPHMC_FREEZE_QTY),convert(numeric(18,3),DPHMC_PLEDGE_QTY),convert(numeric(18,3),DPHMC_DEMAT_PND_VER_QTY)      
 ...
 select DPAM_SBA_NAME,DPAM_sba_NO,DPHMCD_ISIN,isin_name,convert(numeric(18,3),DPHMCD_CURR_QTY),convert(numeric(18,3),DPHMCD_FREE_QTY),convert(numeric(18,3),DPHMCD_FREEZE_QTY)      
 ...
 BEGIN              
  ...
  ,BEN_TYPE=BEN.DESCP + CASE WHEN LTRIM(RTRIM(DPDHM_SETT_TYPE)) <> '00' AND ISNULL(LTRIM(RTRIM(DPDHM_SETT_TYPE)),'') <> '' THEN  ' - ' + settm_desc +'/' + DPDHM_SETT_NO  ELSE '' END   
  ,DPDHM_ISIN ISINCD,isin_name,convert(numeric(18,3),DPDHM_QTY)  Qty,Valuation= convert(numeric(18,3),DPDHM_QTY*isnull(clopm_nsdl_rt,0))     
  ,CASE WHEN DPDHM_BLK_LOCK_FLG = 'F' THEN 'FREE' WHEN  DPDHM_BLK_LOCK_FLG = 'L' THEN 'LOCK' WHEN DPDHM_BLK_LOCK_FLG = 'B' THEN 'BLOCK' ELSE '' END LockFlg  
  ,holding_dt=convert(varchar(11),@pa_fordate,109),cmp=isnull(clopm_nsdl_rt,0.00)   
  from DP_HLDG_MSTR_NSDL   
        left outer join  settlement_type_mstr  on DPDHM_SETT_TYPE  = SETTM_TYPE and settm_deleted_ind = 1                                        
  LEFT OUTER JOIN ISIN_MSTR ON DPDHM_ISIN = ISIN_CD              
  ...
  ,BEN_TYPE=BEN.DESCP + CASE WHEN LTRIM(RTRIM(DPDHMD_SETT_TYPE)) <> '00' AND ISNULL(LTRIM(RTRIM(DPDHMD_SETT_TYPE)),'') <> '' THEN  ' - ' + settm_desc +'/' + DPDHMD_SETT_NO  ELSE '' END   
  ,DPDHMD_ISIN ISINCD,isin_name,convert(numeric(18,3),DPDHMD_QTY) Qty,Valuation= convert(numeric(18,3),DPDHMD_QTY*isnull(clopm_nsdl_rt,0))                  
  ,CASE WHEN DPDHMD_BLK_LOCK_FLG = 'F' THEN 'FREE' WHEN  DPDHMD_BLK_LOCK_FLG = 'L' THEN 'LOCK' WHEN DPDHMD_BLK_LOCK_FLG = 'B' THEN 'BLOCK' ELSE '' END LockFlg  
  ,holding_dt=convert(varchar(11),@@tmpholding_dt,109),cmp=isnull(clopm_nsdl_rt,0.00)                           
  from fn_dailyholding(@@dpmid,@pa_fordate,@pa_fromaccid,@pa_toaccid,@pa_isincd,'',@pa_login_pr_entm_id,@@l_child_entm_id)   
        left outer join  settlement_type_mstr  on DPDHMD_SETT_TYPE  = SETTM_TYPE and settm_deleted_ind = 1                              
  LEFT OUTER JOIN ISIN_MSTR ON DPDHMD_ISIN = ISIN_CD              
 ...
 BEGIN                          
   ...
   BEGIN                      
     ...
     select DPAM_SBA_NAME,DPAM_sba_NO,DPHMC_ISIN,isin_name,convert(numeric(18,3),DPHMC_CURR_QTY),Valuation=convert(numeric(18,3),DPHMC_CURR_QTY*isnull(clopm_cdsl_rt,0)), convert(numeric(18,3),DPHMC_FREE_QTY),convert(numeric(18,3),DPHMC_FREEZE_QTY)      
     ,convert(numeric(18,3),DPHMC_PLEDGE_QTY),convert(numeric(18,3),DPHMC_DEMAT_PND_VER_QTY),convert(numeric(18,3),DPHMC_REMAT_PND_CONF_QTY),convert(numeric(18,3),DPHMC_DEMAT_PND_CONF_QTY),convert(numeric(18,3),DPHMC_SAFE_KEEPING_QTY),convert(numeric(18,3
),DPHMC_LOCKIN_QTY)      
   ...
   INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,EFF_TO FROM citrus_usr.fn_acct_list(@@dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id)    
     ...
     select DPAM_SBA_NAME,DPAM_sba_NO,DPHMCD_ISIN,isin_name,convert(numeric(18,3),DPHMCD_CURR_QTY),Valuation=convert(numeric(18,3),DPHMCD_CURR_QTY*isnull(clopm_cdsl_rt,0)),convert(numeric(18,3),DPHMCD_FREE_QTY),convert(numeric(18,3),DPHMCD_FREEZE_QTY),convert(numeric(18,3),DPHMCD_PLEDGE_QTY)      
