-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_fortest
...
CREATE PROCEDURE  [citrus_usr].[pr_dp_select_mstr_fortest]( @pa_id  VARCHAR(800)                          
...
,isnull(DPTDC_CUSPA_FLG,'') DPTDC_CUSPA_FLG
,isnull(DPTDC_CUSPA_CLTID,'') DPTDC_CUSPA_CLTID
,isnull(DPTDC_CUSPA_CMBP_DPID,'') DPTDC_CUSPA_CMBP_DPID
...
print 'second'         
      SELECT distinct  @l_dpm_dpid                                DPID                         
            ...
            ,convert(varchar,dptdc_execution_dt,103)           EXECUTIONDATE                        
            --,dptdc_slip_no                                     SLIPNO                         
            ,isnull((select top 1 DPTDC_SLIP_NO from edis_pre_DPTDC_MAK P where dptdc.DPTDC_SLIP_NO=p.DPTDC_TRANS_NO ),DPTDC_SLIP_NO) Slipno
...
left outer join maker_scancopy on slip_no = DPTDC_SLIP_NO  and deleted_ind =1
           ...
           on (convert(varchar,settm2.settm_id)            = dptdc.dptdc_other_settlement_type)                            
    WHERE  dpam.dpam_id                  = dptdc.dptdc_dpam_id     
    and dptdc.DPTDC_SLIP_NO not like 'e%'  
    
    and dptdc.dptdc_slip_no not in (Select cdsl_trxn_id from E_DIS_PROCESS_DATA with(nolock))           
    AND    dpm_excsm_id                  = excsm.excsm_id                     
AND    excsm_list.excsm_id      = excsm.excsm_id                        
    AND    dpam.dpam_dpm_id              = dpm.dpm_id                        
    AND    dptdc.dptdc_lst_upd_by        <> @pa_login_name    
...
and exists(select logn_name from login_names where logn_name = @pa_login_name and isnull(logn_level,'') = 2)    
then 'Y' else 'N' end ) ='Y'
--union all --- for new edis condition
-- SELECT distinct  dpm_dpid                                DPID                         
--          ,convert(varchar,dptdc_request_dt,103)             REQUESTDATE                         
--          ,convert(varchar,dptdc_execution_dt,103)           EXECUTIONDATE                        
--          ,dptdc_slip_no                                     SLIPNO                         
...
--    WHERE  dpam.dpam_id                  = dptdc.dptdc_dpam_id     
--    --and dptdc.DPTDC_SLIP_NO not like 'e%'  
--    and dptdc.dptdc_slip_no not in (Select slip_no from E_DIS_PROCESS_DATA with(nolock)) --- len(dptdc.DPTDC_SLIP_NO)<>'16'                
--    AND    dpm_excsm_id                  = excsm.excsm_id                     
--AND    excsm_list.excsm_id      = excsm.excsm_id                        
--    AND    dpam.dpam_dpm_id              = dpm.dpm_id                        
--    AND    dptdc.dptdc_lst_upd_by        <> @pa_login_name    
