-- Object: PROCEDURE citrus_usr.Pr_rpt_bill_profile_filter_Shilpa
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--PR_RPT_BILL_PROFILE_FILTER 	'CDSL',3,'JAN 26 2009','JUN 26 2009','N','N','1234567890123456','','','N','Y','',1,'HO|*~|',''	

CREATE PROC [citrus_usr].[Pr_rpt_bill_profile_filter_Shilpa] 
(
@PA_DPTYPE VARCHAR(4),                      
@PA_EXCSMID INT,                      
...
---return 0
---Chnages done under SRE-28696 date 10 AUG 2024
BEGIN 
...
--@PA_LOGIN_PR_ENTM_ID NUMERIC,                        
--@PA_LOGIN_ENTM_CD_CHAIN  VARCHAR(8000),                        
--@PA_OUTPUT VARCHAR(8000)
--SELECT 
--@PA_DPTYPE ='CDSL',
--@PA_EXCSMID =3,                            
--@PA_FROMDATE ='JAN 26 2020',                     
--@PA_TODATE ='JUN 26 2021',                      
--@PA_BULK_PRINTFLAG ='N',
--@PA_STOPBILLCLIENTS_FLAG ='N' ,
--@PA_FROMACCID ='1234567890123456',                     
--@PA_TOACCID  ='', 
--@PA_GROUP_CD ='',    
...
insert into bill_reprot_LOG
select GETDATE(), @PA_DPTYPE , @PA_EXCSMID ,@PA_FROMDATE ,@PA_TODATE ,@PA_BULK_PRINTFLAG , @PA_STOPBILLCLIENTS_FLAG , @PA_FROMACCID ,
@PA_TOACCID , 
@PA_GROUP_CD,
@PA_TRANSCLIENTSONLY ,@PA_HLDG_YN ,
@PA_PROFILE ,
@PA_LOGIN_PR_ENTM_ID ,
...
if @PA_TODATE <= 'MAR 31 2021 23:59:59'
begin 

	declare @p15 varchar(1)
	set @p15=NULL

	   exec [citrus_usr].[Pr_rpt_bill_profile_filter_Shilpa_arch] --Pr_rpt_bill_profile_filter_shilpa_arch 
			@pa_dptype,@pa_excsmid,@pa_fromdate,
...
SET @PA_GROUP_CD =  ISNULL(CITRUS_USR.FN_SPLITVAL(@PA_GROUP_CD,1),'')  
 ...
 IF OBJECT_ID('tempdb..#ACCOUNT_PROPERTIES_tmp') IS NOT NULL
    DROP TABLE #ACCOUNT_PROPERTIES_tmp
SELECT * INTO #ACCOUNT_PROPERTIES_tmp from ACCOUNT_PROPERTIES  WITH(NOLOCK)
WHERE ACCP_ACCPM_PROP_CD='BBO_CODE' AND ACCP_VALUE = @@BBOCODE

CREATE NONClustered INDEX IX_#ACCOUNT_PROPERTIES_tmp ON #ACCOUNT_PROPERTIES_tmp (ACCP_CLISBA_ID)

IF OBJECT_ID('tempdb..#BBOCODE') IS NOT NULL
DROP TABLE #BBOCODE
...
/*Add the Code Comments*/
	...
	--	  INTO #BBOCODE FROM ACCOUNT_PROPERTIES , 
	--	                      DP_ACCT_MSTR  
	--WHERE ACCP_ACCPM_PROP_CD='BBO_CODE'
	--AND ACCP_CLISBA_ID = DPAM_ID 
	--AND ACCP_VALUE = @@BBOCODE
	/*End Code Comments*/
/*New Code Added By Farooqui 16th Nov 2021*/
	SELECT ACCP_VALUE BBO , DPAM_SBA_NO CLIENTID 
	INTO #BBOCODE FROM #ACCOUNT_PROPERTIES_tmp A WITH(NOLOCK)  
	               INNER JOIN DP_ACCT_MSTR  B WITH(NOLOCK) ON A.ACCP_CLISBA_ID = B.DPAM_ID  
...
--SELECT * FROM #ACLIST WHERE DPAM_SBA_NO = '1201090000004621'
 ...
 BEGIN
		DELETE A FROM #ACLIST A,
		       BLK_CLIENT_PRINT_DTLS 
