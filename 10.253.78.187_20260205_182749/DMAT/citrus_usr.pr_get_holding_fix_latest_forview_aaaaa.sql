-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_forview_aaaaa
...
CREATE  proc [citrus_usr].[pr_get_holding_fix_latest_forview_aaaaa](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
select identity(numeric,1,1) dpam_id,cdshm_ben_acct_no dpam_sba_no  into #dp_acct_mstr from cdsl_holding_dtls		


update  cdsl_holding_dtls	 set CDSHM_DPAM_ID = dpam_id from 	cdsl_holding_dtls, #dp_acct_mstr where dpam_sba_no = cdshm_ben_acct_no 
and CDSHM_TRAS_DT = @pa_from_dt  


select 0                    DPHMCD_DPM_ID                    
,0  DPHMCD_DPAM_ID
,'' DPHMCD_ISIN
,0  DPHMCD_CURR_QTY
,0  DPHMCD_FREE_QTY
...
,'0'  DPHMCD_LST_UPD_BY
,'' DPHMCD_LST_UPD_DT
,'0' DPHMCD_DELETED_IND
,''  dphmcD_cntr_settm_id                         
 into #tmp_dp_daily_hldg_cdsl_prevdt 
-- from dp_hldg_mstr_cdsl with (nolock),#dp_Acct_mstr with (nolock)
