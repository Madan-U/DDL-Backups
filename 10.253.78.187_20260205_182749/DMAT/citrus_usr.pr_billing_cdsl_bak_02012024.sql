-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_bak_02012024
...
--ROLLBACK
create  procedure [citrus_usr].[pr_billing_cdsl_bak_02012024](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
...
delete a from qtr_park  a , #account_properties --, charge_mstr c , lateshscheme
where dpam_id = accp_clisba_id  --and a.charge_name = c.cham_slab_name 
  and accp_value between 'dec 01 2024' and 'Dec 31 2024'
  and charge_name in (
  select cham_Slab_name  from charge_mstr , profile_Charges , brokerage_mstr , lateshscheme
 where CHAM_CHARGE_TYPE = 'fq' and cham_slab_no = proc_slab_no 
 and PROC_PROFILE_ID = brom_id 
 and brom_id = id 
  and  filter_free is not null )
  and q_flg = 1 
...
insert into #temp_bill(trans_dt   
					...
					,charge_val
					,post_toacct)
select @pa_billing_from_dt , dpam_id
,charge_name
,charge_val
,post_toacct
from Qtr_Park a where DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0)  = Qtr 
...
, charge_mstr cham 
where not exists (select 1 from ytbill_after b where a.dpam_id = b.dpam_id ) 
...
from Qtr_Park main , ytbill a 
, charge_mstr cham 
where exists (select 1 from ytbill_after b where a.dpam_id = b.dpam_id ) 
and a.charge_name = cham.cham_slab_name 
and main.dpam_id = a.dpam_id 
and Qtr = DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0) 
and cham.cham_charge_type ='FQ'
