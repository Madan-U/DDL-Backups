-- Object: PROCEDURE citrus_usr.pr_import_missingdata
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE  proc [citrus_usr].[pr_import_missingdata]
as
begin
...
and exists(select 1 from   ABVSCITRUS.crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp  where dp_id = DPAM_sba_no)
...
select * into #CITRUS_DPACCACTIVATION from ABVSCITRUS.crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp  where dp_id in (select DPAM_SBA_NO from #data)
...
print @l_cur_boid
	...
	from #CITRUS_DPACCACTIVATION  , dp_Acct_mstr , brokerage_mstr where dp_id = @l_cur_boid
	...
	,entr_dummy4 = @l_sb
	--,entr_dummy5 = @l_bl
	from entity_relationship entr ,  #CITRUS_DPACCACTIVATION 
