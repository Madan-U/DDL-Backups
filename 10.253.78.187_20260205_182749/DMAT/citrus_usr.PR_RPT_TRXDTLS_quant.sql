-- Object: PROCEDURE citrus_usr.PR_RPT_TRXDTLS_quant
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE procedure  [citrus_usr].[PR_RPT_TRXDTLS_quant](@pa_exch varchar(100), @pa_batch_no varchar(50),@pa_date varchar(11), @pa_trans_type varchar(100))
AS
begin
	...
	begin
		...
		, convert(varchar,dptd_request_dt,103) request_dt,  convert(varchar,DPTD_EXECUTION_DT,103) execution_dt , '' series ,DPTD_SLIP_NO slip_no ,src_client.dpam_sba_no ACCOUNT_NO , src_client.dpam_sba_name CLIENT_NAME 
		...
		--Added on 23 aug, 2010 for pledge
    ...
    begin
     select distinct BATCHC_TRANS_TYPE
		, convert(varchar,dptdc_request_dt,103) request_dt ,  convert(varchar,DPTDc_EXECUTION_DT,103) execution_dt, '' series ,DPTDc_SLIP_NO slip_no ,src_client.dpam_sba_no ACCOUNT_NO , src_client.dpam_sba_name CLIENT_NAME 
        ...
        ,DPTDc_ISIN DPTD_ISIN, isin_name , abs(DPTDc_QTY) DPTD_QTY , isnull(DPTDc_TRANS_NO,'') TRANSACTION_NO , DPTDC_INTERNAL_REF_NO INTERNAL_REF_NO
		  from dp_trx_dtls_cdsl   left outer join dp_acct_mstr dst_client on dst_client.dpam_sba_no = DPTDc_COUNTER_DEMAT_ACCT_NO 
