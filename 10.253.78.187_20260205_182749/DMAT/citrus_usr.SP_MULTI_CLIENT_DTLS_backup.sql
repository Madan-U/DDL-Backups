-- Object: PROCEDURE citrus_usr.SP_MULTI_CLIENT_DTLS_backup
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--exec Sp_Multi_Client_Dtls @PA_FROM_ACCTNO='',@PA_TO_ACCTNO='',@PA_EXCSMID=3,@pa_login_pr_entm_id=1,@pa_login_entm_cd_chain='HO|*~|'
...
create  PROCEDURE [citrus_usr].[SP_MULTI_CLIENT_DTLS_backup]
( 
...
SELECT @@L_CHILD_ENTM_ID    =  CITRUS_USR.FN_GET_CHILD(@PA_LOGIN_PR_ENTM_ID , @PA_LOGIN_ENTM_CD_CHAIN)  
		...
		SELECT DISTINCT top 1 STAM_DESC [STATUS]
		,CLICM_DESC  [CATEGORY]
		...
		,case when CITRUS_USR.FN_SPLITVAL(CITRUS_USR.FN_ADDR_VALUE(BANM_ID,'COR_ADR1'),1) <> '' then  CITRUS_USR.FN_SPLITVAL(CITRUS_USR.FN_ADDR_VALUE(BANM_ID,'COR_ADR1'),1) else CITRUS_USR.FN_SPLITVAL(CITRUS_USR.FN_ADDR_VALUE(BANM_ID,'OFF_ADR1'),1) end [BADDRESS1]
		...
		, ISNULL(CITRUS_USR.FN_ACCT_ENTP(DPAM.DPAM_ID,'BILL_START_DT'),'') ACTDT
		, ISNULL(CITRUS_USR.FN_ACCT_ENTP(DPAM.DPAM_ID,'ACC_CLOSE_DT'),'') CLDT
		...
		,CASE WHEN ISNULL(CITRUS_USR.FN_ACCT_ENTP(DPAM.DPAM_ID,'SMS_FLAG'),'') = '1' THEN 'Y' ELSE 'N' END SMS  
        ,ISNULL(CITRUS_USR.FN_ACCT_ENTP(DPAM.DPAM_id,'SEBI_REG_NO'),'') SEBI_REG_NO
        ,ISNULL(CITRUS_USR.FN_ACCT_ENTPd(DPAM.DPAM_id,'RBI_REF_NO','RBI_APP_DT'),'') RBI_APP_DATE
		...
		,CASE WHEN ISNULL(CITRUS_USR.FN_ACCT_ENTP(dpam.DPAM_id,'CONFIRMATION'),'') = '1' THEN 'Y' ELSE 'N' END CONFIRMATION
		,CASE WHEN ISNULL(CITRUS_USR.FN_ACCT_ENTP(dpam.DPAM_id,'ECS_FLG'),'') = '1' THEN 'Y' ELSE 'N' END ECS
...
FROM DP_ACCT_MSTR DPAM
		...
		--,CLIENT_BANK_ACCTS
		--,BANK_MSTR  
		,[citrus_usr].[fn_acct_list_bytushar](@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID,@PA_FROM_ACCTNO,@PA_TO_ACCTNO) ACCOUNT         
	 
		WHERE isNumeric(dpam.DPAM_SBA_NO)=1 and dpam.DPAM_SBA_NO  BETWEEN @PA_FROM_ACCTNO AND @PA_TO_ACCTNO
