-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_BBOCODEt
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_BBOCODEt](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
,@pa_ben_acct_no_fr varchar(16),@pa_ben_acct_no_to varchar(16),@PA_BBO VARCHAR(100),@pa_out  varchar(8000) out)      
as      
begin       
  

SELECT DPAM_ID ID , DPAM_SBA_NO SBANO INTO #CLIENTLIST  FROM DP_ACCT_MSTR , ACCOUNT_PROPERTiES WHERE ACCP_CLISBA_ID = DPAM_ID AND 
...
,convert(numeric(18,5),0) DPHMCD_LEND_QTY    
,convert(numeric(18,5),0) DPHMCD_BORROW_QTY    
from dp_daily_hldg_cdsl a with (nolock) ,dp_acct_mstr  with (nolock), #CLIENTLIST where dphmcd_holding_dt ='jul 31 2011'    
...
--from citrus_usr.dp_daily_hldg_cdsl,dp_acct_mstr, #CLIENTLIST with(nolock)                      
...
into #holdingdmtpenfing from  dp_daily_hldg_cdsl with (nolock), DP_aCCT_MSTR  with (nolock), #CLIENTLIST
...
into #holdingdmtpenfing1 from  dp_daily_hldg_cdsl with (nolock) , DP_aCCT_MSTR with (nolock), #CLIENTLIST
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt='jul 31 2011'
--and dpam_sba_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
and dpam_sba_no = sbano
and dphmcd_dpm_id = @PA_DPM_ID
...
into #holdingdmtpenfing2 from  dp_daily_hldg_cdsl with (nolock), DP_aCCT_MSTR with (nolock), #CLIENTLIST
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt='jul 31 2011'
and dpam_sba_no = sbano
group by dphmcd_dpam_id,dphmcd_isin 
