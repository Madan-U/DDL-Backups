-- Object: VIEW citrus_usr.dp_client_master_ipo
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------



--select  * from [dp_client_master_1] where cm_cd  ='1201090001321086'  
--select * from dps8_pc1 where boid = '1201090001321086'
CREATE view [citrus_usr].[dp_client_master_ipo]  
as
select  dps8_pc1.name cm_firstname,  dps8_pc1.boid , case when ClosDt <> '' then right(ClosDt,4)+substring(ClosDt,3,2)+left(ClosDt,2) else '' end cm_acc_closuredate  
,'' cm_acctype  
, case when BOAcctStatus ='1' then 'ACTIVE' else '' end  cm_active  
...
, isnull( right(dps8_pc5.setupdate,4)+substring(dps8_pc5.setupdate,3,2)+left(dps8_pc5.setupdate,2),'')  cm_poaregdate  
, '' cm_productcd  
...
left outer join ( select distinct fre_Dpam_id , fre_Exec_date,fre_deleted_ind,fre_rmks,fre_status from freeze_unfreeze_dtls main with (nolock)   
)  main
on fre_Dpam_id = dpam.dpam_id and fre_deleted_ind = 1
and not exists (select 1 from freeze_unfreeze_dtls inn where inn.fre_Dpam_id=main.fre_Dpam_id and fre_action ='U' and fre_deleted_ind = 1  ) 
...
left outer join dps8_pc5 dps8_pc5  with (nolock)   on dps8_pc5.boid = dps8_pc1.boid and HolderNum ='1' and  dps8_pc5.TypeOfTrans in ('','1','2','4')
  left outer join (SELECT BOID,Name,MiddleName,SearchName ,FthName,Addr1,  
Addr2,Addr3,City,State,Country,PinCode,PANGIR,DateOfBirth,DateOfSetup D  
FROM dps8_pc6 with (nolock) ,dp_holder_dtls with(nolock), (select max(DateOfSetup) maxdt ,boid maxbo from dps8_pc6 group by boid ) maxnom
where maxbo = boid and DPHD_DPAM_SBA_NO = boid 
and replace(isnull(DPHD_NOM_FNAME,'') + ' ' + isnull(DPHD_NOM_MNAME,'')+' '+ isnull(DPHD_NOM_LNAME,' '), ' ','')
=replace(isnull(Name,'')+' '+ isnull(MiddleName,'')+' '+ isnull(SearchName,''),' ','')
and maxdt = DateOfSetup  )  dps8_pc6  on dps8_pc6.boid = dps8_pc1.boid   
  left outer join (SELECT BOID,Name,MiddleName,SearchName ,FthName,Addr1,  
Addr2,Addr3,City,State,Country,PinCode,PANGIR,DateOfBirth,DateOfSetup D  
FROM dps8_pc7 with (nolock) , (select max(DateOfSetup) maxdt ,boid maxbo from dps8_pc7 group by boid ) maxgau
where maxbo = boid 
and maxdt = DateOfSetup )  dps8_pc7  on dps8_pc7.boid = dps8_pc1.boid   
