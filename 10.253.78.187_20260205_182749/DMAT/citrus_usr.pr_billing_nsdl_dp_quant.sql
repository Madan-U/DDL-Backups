-- Object: PROCEDURE citrus_usr.pr_billing_nsdl_dp_quant
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE procedure [citrus_usr].[pr_billing_nsdl_dp_quant](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
  SET DATEFORMAT dmy
  ...
  --logic for charging for one time charges
	...
	and   dpam_id  not in(select cl_dpam_id from #account_properties_cl)
  ...
  --logic for account closure charge
	...
	and   CLSR_DPAM_ID not in(select cl_dpam_id from #account_properties_cl)
	AND   chacm_cham_id = CHAM_SLAB_NO
    AND   DPC_PROFILE_ID = proc_profile_id  
	AND   proc_slab_no  = cham_slab_no  
...
from nsdl_holding_dtls      
    ...
    and   nsdhm_dpam_id not in(select cl_dpam_id from #account_properties_cl)
	AND   isnull(cham_charge_graded,0) <> 1
