-- Object: PROCEDURE citrus_usr.pr_select_dp_chk_by_filter_22072015_bak
...
*/   
create PROCEDURE [citrus_usr].[pr_select_dp_chk_by_filter_22072015_bak]      
(      
  @pa_query_id   NUMERIC                  
  ,@pa_scr_id     NUMERIC                 
  ,@pa_tab        VARCHAR(20)                    
...
print 'sfsfs'
  ...
  and   clidb_deleted_ind in (0,4,8)  
   ...
   AND    clim.clim_crn_no      NOT IN (SELECT clim_crn_no                        
            ...
            and  not exists (select dpam_Crn_no from DP_ACCT_MSTR where DPAM_CRN_NO = clim.clim_crn_no and DPAM_STAM_CD ='active'           )
            and exists (select accd_clisba_id from ACCOUNT_DOCUMENTS,dp_acct_mstr where ACCD_DELETED_IND=1 and accd_clisba_id=DPAM_ID
            and DPAM_DELETED_IND=1 and ACCD_DELETED_IND=1 and clim.CLIM_CRN_NO=DPAM_CRN_NO and ACCD_ACCDOCM_DOC_ID='12')            
            --not exists removed bz client modification client not come on 06/07/2015
END 
