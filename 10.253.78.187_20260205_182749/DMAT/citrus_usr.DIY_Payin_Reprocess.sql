-- Object: PROCEDURE citrus_usr.DIY_Payin_Reprocess
...
---DIY_Payin_Reprocess '2020083','R'

CREATE proc [citrus_usr].[DIY_Payin_Reprocess] (@sett_no varchar(11) ,@flag varchar(1))
as 
...
If @flag ='R'
begin 
Exec NON_POA_trade_infor_PROCESS @sett_no,@flag
end
/*@flag
 ...
 Begin 
		...
		AND SETT_NO= @sett_no AND PROCESS_FLAG=0
		  ...
		  UPDATE  D SET POA_FLAG ='1'   FROM  citrus_usr.DIY_Reprocess_log D (nolock),#clt P  (nolock)
		   ...
		   SELECT @STDATE = MAX(START_DATE) FROM [ANGELDEMAT].MSAJAG.DBO.SETT_MST WITH(NOLOCK) WHERE SETT_NO =@sett_no  AND SETT_TYPE ='N'

		   SELECT @PREVDATE = MAX(START_DATE) FROM [ANGELDEMAT].MSAJAG.DBO.SETT_MST WITH(NOLOCK) WHERE START_DATE <@STDATE AND SETT_TYPE ='N'
	...
	create index #p on #PARTY (PARTYCODE,isin)
		   ...
		   SELECT DISTINCT PARTYCODE INTO #PARTY1 
			FROM  
			[172.31.16.75].Angel_WMS.DBO.po_acceptance_status a  
			WHERE date_time_stamp >= @PREVDATE +' 16:00:00.000' AND date_time_stamp<= @STDATE +' 23:59'
			--AND CreationDate <=CONVERT(VARCHAR(11),@date,120) +' 23:59' AND   
		   AND ACTION ='SELL'   --and  isnull(convert(float, replace(Quantity,',','')),0) <>0 
			...
			create index #p on #PARTY1 (PARTYCODE)

			UPDATE D SET DP_CONCERN ='A'  
		   FROM  citrus_usr.DIY_Reprocess_log  D,#PARTY1 P
		   WHERE PARTYCODE=PARTY_CODE AND SETT_NO= @sett_no AND PROCESS_FLAG=0 AND DP_CONCERN<>'Y'
       ...
       When SETT_TYPE='W' then 13 END ) AS pa_tr_setm_type,  
		...
		'1203320006951435' As pa_tr_acct_no  ,isin 
		  ...
		  from 
		   DIY_Reprocess_log  
		 where SETT_NO= @sett_no AND PROCESS_FLAG=0  AND  POA_FLAG  =''  AND  DP_CONCERN in ('Y','A') AND ADJ_QTY >0
		 --and ADJ_QTY<>475
		---- AND SETT_TYPE ='W'
