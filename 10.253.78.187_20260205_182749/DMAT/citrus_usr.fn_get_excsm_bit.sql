-- Object: FUNCTION citrus_usr.fn_get_excsm_bit
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE function [citrus_usr].[fn_get_excsm_bit](@pa_excsm_id   numeric
                                ,@pa_check      numeric
                                ,@pa_chk_yn    numeric
                                )
...
--
  ...
  BEGIN
  --
    SELECT @l_excsm_bit      = excsm_bit 
    FROM   exch_seg_mstr       WITH (NOLOCK)
    WHERE  excsm_id          = @pa_excsm_id
  ...
  --
    ...
    --
    IF @l_excsm_bit & power(2,@l_bitrm_bit_locaton-1) > 0 
    BEGIN
    --
