-- Object: VIEW citrus_usr.Synergy_Client_Master_fordump
...
--Altered this view under SRE-30719

CREATE  VIEW [citrus_usr].[Synergy_Client_Master_fordump]    
AS    
SELECT     
...
convert(varchar (50), Isnull(pri_email,EMailId)) CM_EMAIL,     
...
convert(numeric(5,2),CASE WHEN SecAccCd='' THEN '0' ELSE SecAccCd END ) CM_SECURITYCODE,  ----CHanges done under SRE-36711  
...
convert(numeric(5,2),(CASE WHEN StockExch='' THEN '0' ELSE StockExch END))CM_STOCKEXCHANGE,     ----CHANGE UNDER SRE-36711   
...
CM_CHGSSCHEME=LEFT((SELECT TOP 1 BROM_DESC FROM BROKERAGE_MSTR,DP_ACCT_MSTR,CLIENT_DP_BRKG WHERE DPAM_SBA_NO=C.BOId AND DPAM_ID=CLIDB_DPAM_ID),10),    
...
CM_POAREGDATE =(SELECT MAX( EffFormDate) FROM DPS8_PC5 WHERE BOID= C.BOID AND MasterPOAId='2203320000000014') ,    
