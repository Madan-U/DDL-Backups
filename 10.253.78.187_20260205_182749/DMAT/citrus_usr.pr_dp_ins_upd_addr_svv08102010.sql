-- Object: PROCEDURE citrus_usr.pr_dp_ins_upd_addr_svv08102010
...
create PROCEDURE [citrus_usr].[pr_dp_ins_upd_addr_svv08102010] (@pa_id           varchar(8000)  
...
--  
  ...
  --
    ...
    ,adr_2                varchar(200)
    ,adr_3                varchar(200)
    ,adr_city             varchar(200)
    ,adr_state            varchar(200)
    ,adr_country          varchar(200)
    ,adr_zip              varchar(20)
    )
    --
    SELECT @l_dpam_id       = dpam_id
    FROM   dp_acct_mstr       WITH (NOLOCK)  
    WHERE  dpam_sba_no      = convert(varchar, @pa_demat_no)
  ...
  --  
    ...
    --
        ...
        --  
          ...
          --
            ...
            --
              ...
              BEGIN--edt  
              --
                --Updation of old record at the top
                IF @pa_chk_yn = 1 or @pa_chk_yn = 2
                BEGIN--edt_1_2
                --
                  IF ISNULL(@pa_values,'') = ''
                  ...
                  --
                    ...
                    --
                     ...
                     SELECT @l_old_addr_value = adr_1+ISNULL(adr_2,'')+ISNULL(adr_3,'')+ISNULL(adr_city,'')+ISNULL(adr_state,'')+ISNULL(adr_country,'')+ISNULL(adr_zip,'')
                      ...
                      print @l_old_addr_value 
                      print @l_adr_1+ISNULL(@l_adr_2,'')+ISNULL(@l_adr_3,'')+ISNULL(@l_ADR_CITY,'')+ISNULL(@l_adr_state,'')+ISNULL(@l_adr_country,'')+ISNULL(@l_adr_zip,'')
                  ...
                  --
                    ...
                    --
                      ...
                      IF EXISTS(SELECT * FROM account_adr_conc WITH (NOLOCK)
                                WHERE    (accac_clisba_id      <> @l_dpam_id or (accac_clisba_id      = @l_dpam_id and ACCAC_CONCM_ID <> @l_concm_id))
      ...
      --
        ...
        --delete accac from account_adr_conc ACCAC, dp_Acct_mstr where dpam_id = accac_clisba_id and dpam_crn_no = @pa_clisba_id and  not exists(select adr_clisba_id , adr_concm_id from addr_acct_mak where adr_clisba_id = ACCAC_CLISBA_ID and  adr_concm_id = ACCAC_CONCM_ID  AND    adr_deleted_ind   IN (0,4,8))
        ...
        --
          IF EXISTS(SELECT * FROM account_adr_conc   WITH (NOLOCK)
                    WHERE    accac_adr_conc_id     = @l_accac_adr_conc_id
                    AND      (accac_clisba_id      <> @l_clisba_id or (accac_clisba_id      = @l_clisba_id and ACCAC_CONCM_ID <> @l_concm_id))
                    AND      accac_deleted_ind     = 1
                   )    
          BEGIN--ext_1
          --
  ...
  WHERE  accac_clisba_id    = @l_dpam_id 
  AND    accac_deleted_ind  = 1
