-- Object: PROCEDURE citrus_usr.pr_fetch_disdata
...
--begin tran
--pr_fetch_disdata	'FileGeneration','apr 17 2015','Apr 17 2015','','N'	
...
CREATE proc [citrus_usr].[pr_fetch_disdata](@pa_Action varchar(100)
...
truncate table tmp_classMF	
insert into tmp_classMF
Select distinct isnull(client_code,'')  from [dbo].[VW_CLIENT_DETAILS]  where templatecode='MF'	

--select distinct  isnull(client_code,'') into #tmp_classdata from [dbo].[VW_CLIENT_DETAILS]   where templatecode='MF'	

insert into FETCHDISDATA_MSTR
select 
BOID
,SLIP_NO
,SERIES_TYPE
...
fetchdisdata	
				...
				,@from varchar(20),@to varchar(20),@remarks varchar(1000),@dtval datetime, @l_sliim_id      NUMERIC
      , @l_sliimm_id     NUMERIC , @l_sliim_gen_id numeric,@l_dpm_id       NUMERIC,@dpam_id numeric
				...
				FOR
				SELECT  series_type,id,entm_id,rtrim(ltrim(boid)),rtrim(ltrim(series_from)),rtrim(ltrim(series_to)),rtrim(ltrim(remarks)),dpam_id
					...
					--exec pr_mak_sliim_new '0','INS','MIG',454,@series,@id,@entmid,@boid,@from,@to,'0',@dtval,'',@remarks,0,'','',''
				      ...
				      (					sliim_id
						...
						(			    @l_sliim_id   
										...
										,getdate()
										,1
										,convert(datetime,convert(varchar(11),getDate(),120))
										,'', @remarks  ,case when @series <> '0' then @series else convert(varchar,@l_sliim_gen_id) end ,getdate()
				      )
			...
			begin						
				select  DISTINCT 
				 dpam_sba_no 
				, dpam_sba_name
				, ltrim(rtrim(isnull(DPHD_SH_FNAME,''))) + ' ' +  ltrim(rtrim(isnull(DPHD_SH_MNAME,''))) +' ' +  ltrim(rtrim(isnull(DPHD_SH_LNAME,''))) sh_name
				,  ltrim(rtrim(isnull(DPHD_tH_FNAME,''))) + ' ' +  ltrim(rtrim(isnull(DPHD_tH_MNAME,''))) +' ' +  ltrim(rtrim(isnull(DPHD_tH_LNAME,''))) th_name
				
				, CONVERT(VARCHAR(11),citrus_usr.fn_ucc_accp(dpam_id,'Bill_Start_Dt',''),112) [CM_OPENDATE] --CONVERT(VARCHAR(11),sliim_dt,112) [CM_OPENDATE]
				--, sliim_rmks
				...
				,'' [cm_blsavingcd]
				, ''''+ISNULL(citrus_usr.FN_FORMATSTR(convert(varchar(12),SLIIM_SLIP_NO_FR),12,0,'L','0'),space(12)) SLIIM_SLIP_NO_FR,
				''''+ISNULL(citrus_usr.FN_FORMATSTR(convert(varchar(12),SLIIM_SLIP_NO_TO),12,0,'L','0'),space(12)) SLIIM_SLIP_NO_TO
				,case when isnull(citrus_usr.To_GetAngelMF(dpam_sba_no),'')='' then  isnull(replace(citrus_usr.fn_find_relations_Acctlvl(dpam_id,'BR'),'_BR',''),'')   + '-' + isnull(replace(citrus_usr.[fn_find_relations_ACCT_nm](dpam_sba_no,'BR'),'_BR',''),'') else 'MARGIN ANGEL FUNDING' end   [CM_BRANCH_NAME]
...
--and isnull(reco_yn,'') = ''  --commented on 04102013 as per MOSL request
				...
				--and not exists (select SLIIM_DPAM_ACCT_NO from bak_tmp_jan0516 t where t.SLIIM_DPAM_ACCT_NO=DPAM_SBA_NO  )--comment on 21/07/2016 mail from rakesh nakka
				--and ISNULL(SLIIM_SUCCESS_FLAG,'')=CASE WHEN sliim_dt>='2014-10-01 00:00:00.000' then 'S' else '' end 
                union
				select  DISTINCT 
				 dpam_sba_no 
				, dpam_sba_name
			, ltrim(rtrim(isnull(DPHD_SH_FNAME,''))) + ' ' +  ltrim(rtrim(isnull(DPHD_SH_MNAME,''))) +' ' +  ltrim(rtrim(isnull(DPHD_SH_LNAME,''))) sh_name
				...
				,'' [cm_blsavingcd]
				, ''''+ISNULL(citrus_usr.FN_FORMATSTR(convert(varchar(12),SLIIM_SLIP_NO_FR),12,0,'L','0'),space(12)) SLIIM_SLIP_NO_FR,
				''''+ISNULL(citrus_usr.FN_FORMATSTR(convert(varchar(12),SLIIM_SLIP_NO_TO),12,0,'L','0'),space(12)) SLIIM_SLIP_NO_TO--SLIIM_SLIP_NO_FR, SLIIM_SLIP_NO_TO
				,case when isnull(citrus_usr.To_GetAngelMF(dpam_sba_no),'')='' then  isnull(replace(citrus_usr.fn_find_relations_Acctlvl(dpam_id,'BR'),'_BR',''),'') + '-' + isnull(replace(citrus_usr.[fn_find_relations_ACCT_nm](dpam_sba_no,'BR'),'_BR',''),'') else 'MARGIN ANGEL FUNDING' end  [CM_BRANCH_NAME]
				,isnull([citrus_usr].[fn_conc_value](dpam_crn_no,'MOBILE1'),'')  [mobile]    
				from slip_issue_mstr main,dp_mstr , dp_acct_mstr left outer join dp_holder_dtls on dphd_dpam_id = dpam_id 
				,#account_properties , Dis_req_Dtls dis
...
--and isnull(reco_yn,'') = '' --commented on 04102013 as per MOSL request
				...
				update M Set m.flag='E'  from FETCHDISDATA_MSTR M, fetchdisdata	N where m.id=n.id	
    ...
    ----            ,case when left(citrus_usr.fn_splitval(citrus_usr.[fn_addr_value_vendorfilgn](DPAM_CRN_NO,'COR_ADR1'),3),1) in ('+','-') then isnull(substring(replace(citrus_usr.fn_splitval(citrus_usr.[fn_addr_value_vendorfilgn](DPAM_CRN_NO,'COR_ADR1'),3),',',''),2,len(replace(citrus_usr.fn_splitval(citrus_usr.[fn_addr_value_vendorfilgn](DPAM_CRN_NO,'COR_ADR1'),3),',',''))),'') else isnull(replace(citrus_usr.fn_splitval(citrus_usr.[fn_addr_value_vendorfilgn](DPAM_CRN_NO,'COR_ADR1'),3),',',''),'') end  [ADDRESS3]
				...
				and left(sliim_dpam_acct_no,2) <> '22'   --to eliminate poa 
				--and ISNULL(SLIIM_SUCCESS_FLAG,'')=CASE WHEN sliim_dt>='2014-10-01 00:00:00.000' then 'S' else '' end 
				and dpam_sba_no not in ('1203320010009684','1203320010020286','1203320010020307','1203320010020311')		
				
                update M Set m.flag='A'  from FETCHDISDATA_MSTR M, fetchdisdata	N where m.id=n.id	
                and m.boid not in ('1203320010009684','1203320010020286','1203320010020307','1203320010020311')							
			 end
	   ...
	   begin 	
				...
				if @pa_exist = 'E'
					...
					select  dpm_dpid
						...
						,#account_properties , Dis_req_Dtls d left outer join  fetchdisdata_mstr  M on   convert(varchar,gen_id)=series_type and d.boid = m.boid
						--,#temp1
						where dpam_deleted_ind =1 and accp_clisba_id = dpam_id 
						and dpam_dpm_id = dpm_id  and D.Boid = dpam_sba_no 
--and isnull(reco_yn,'') = ''  --commented on 04102013 as per MOSL request
						--and accp_value between @pa_from_date and @pa_to_date 
						and dpam_stam_cd in ('ACTIVE','04')
						and left(dpam_sba_no,2) <> '22'  --to eliminate poa 
						and slip_yn <> 'D'
						--and req_date between @pa_from_date and @pa_to_date + ' 23:59:59'
						and D.lst_upd_dt between @pa_from_date and @pa_to_date + ' 23:59:59'
      ...
      --                  and sliim_dt between @pa_from_date and @pa_to_date + ' 23:59:59'
						...
						--	)
						and not exists (
						select SLIIM_DPAM_ACCT_NO  from slip_issue_mstr  where SLIIM_DPAM_ACCT_NO =  dpam_sba_no and sliim_deleted_ind= 1
						and SERIES_FROM=SLIIM_SLIP_NO_FR and SERIES_TO=SLIIM_SLIP_NO_To
						)
						--and not exists (select SLIIM_DPAM_ACCT_NO from bak_tmp_jan0516 t where t.SLIIM_DPAM_ACCT_NO=DPAM_SBA_NO  )---comment on 21/07/2016 mail from rakesh nakka
