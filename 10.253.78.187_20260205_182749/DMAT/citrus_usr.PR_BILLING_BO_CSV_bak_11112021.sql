-- Object: PROCEDURE citrus_usr.PR_BILLING_BO_CSV_bak_11112021
...
CREATE  PROCEDURE [citrus_usr].[PR_BILLING_BO_CSV_bak_11112021]  
...
,'Demat account monthly maintenanace charges' + ' Dt : ' +  convert(varchar(11),MAX(CLIC_TRANS_DT ),109)   FINA_ACC_NAME
 ,'1'						
...
Into Last_Exported_Data FROM CLIENT_CHARGES_CDSL , FIN_ACCOUNT_MSTR WHERE   FINA_ACC_ID=CLIC_POST_TOACCT AND 
...
when charge_name like '%selling%' THEN 'NORMAL TRX'when charge_name like '%pledging%' then  'PLEDGE' else  charge_name end,convert (datetime, RIGHT(charge_name,11),103) charge_name_dt from #MAIN_BILL
where (CLTCODE not like 'ZR%' and CLTCODE not like '98%' )
