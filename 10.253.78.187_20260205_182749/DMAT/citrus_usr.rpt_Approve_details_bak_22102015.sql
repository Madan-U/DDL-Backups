-- Object: PROCEDURE citrus_usr.rpt_Approve_details_bak_22102015
...
create procedure [citrus_usr].[rpt_Approve_details_bak_22102015]
(@pa_id  varchar(10),
 @pa_type varchar(4),
 @pa_tab varchar(20),
 @pa_from_dt datetime,
...
when isnull(d1.demrm_res_cd_compobj,'') <> '' then convert(varchar(10),d1.demrm_lst_upd_dt,103) 
					...
					and d.DEMRM_DPAM_ID = d1.DEMRM_DPAM_ID left outer join dmat_dispatch with (nolock) on  d1.demrm_id = disp_demrm_id and  DISP_TO = 'C' --isnull(DISP_TYPE,'') = case when (isnull(d.demrm_company_obj,'')<>'' or isnull(d1.demrm_res_cd_compobj,'')<>'') then 'N' else 'IR' end --<> 'IR' --DISP_TO = 'C' and
					where 
...
when isnull(d1.demrm_res_cd_compobj,'') <> '' then convert(varchar(10),d1.demrm_lst_upd_dt,103) 
					...
					, case when (isnull(d1.demrm_internal_rej,'') <> '' or isnull(d.demrm_company_obj,'')<>'' or isnull(d1.demrm_res_cd_intobj,'')<>'' or isnull(d1.demrm_res_cd_compobj,'')<>'' ) then  isnull(DISP_DT,'') else isnull(DISP_DT,'') end   dispdate
					from  demrm_mak d1 with (nolock) left outer join demat_request_mstr d  with (nolock) on d.demrm_id = d1.demrm_id 	
and d.DEMRM_DPAM_ID = d1.DEMRM_DPAM_ID left outer join dmat_dispatch with (nolock) on  DISP_TO = 'C'--isnull(DISP_TYPE,'') = case when (isnull(d.demrm_company_obj,'')<>'' or isnull(d1.demrm_res_cd_compobj,'')<>'') then 'N' else 'IR' end --and DISP_TO = 'C' 
