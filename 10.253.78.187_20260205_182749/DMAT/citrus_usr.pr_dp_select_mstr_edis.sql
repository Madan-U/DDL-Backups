-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_edis
...
CREATE PROCEDURE  [citrus_usr].[pr_dp_select_mstr_edis]( @pa_id  VARCHAR(800)                          
...
distinct DPID                         
          ...
          ,OTHERSETTLEMENTTYPE         
          ,Makerid              
          ,'0' highval--sum(isnull(highval,0)) highval 
,slip_no,ClientOutstanding,ref_no
from

(
 SELECT distinct top 700  dpm_dpid                                DPID                         
          ,convert(varchar,dptdc_request_dt,103)             REQUESTDATE                         
          ,convert(varchar,dptdc_execution_dt,103)           EXECUTIONDATE                        
          ,dptdc_slip_no                                     SLIPNO                         
          ,isnull(settm1.settm_desc,settm2.settm_desc)                                 SETTLEMENT_TYPE                        
   ...
   --       LTRIM(RTRIM((SELECT TOP 1 REPLACE(ENTM_SHORT_NAME  ,'_' + ENTM_ENTTM_CD ,'') + '-' + ENTM_NAME1 FROM LOGIN_NAMES , ENTITY_MSTR 
			...
			--AND ENTM_DELETED_IND =1  AND LOGN_NAME = dptdc_created_by))) + ' )' as  Makerid
          ,highval = '0'
           --convert(numeric,((select top 1 CLOPM_CDSL_RT from closing_last_cdsl where 
           --CLOPM_ISIN_CD = dptdc_isin and DPTDC_SLIP_NO like case when @pa_rmks = '' then '%' else @pa_rmks end) * abs(dptdc_qty)))         
		,isnull(dptdc_slip_no,'')   slip_no
		,'0' ClientOutstanding--ClientOutstanding=citrus_usr.Toget_OutsBal(dpam_sba_no)
		,'0' ref_no --isnull(ref_no,'999999999') ref_no
    FROM  -- citrus_usr.fn_acct_list(@l_dpm_dpid,@l_entm_id,0) dpam                         
          dp_mstr                      dpm   with(nolock)                                           
          ,dp_acct_mstr                 dpam      with(nolock)                                         
          ,exch_seg_mstr                excsm     with(nolock)                                        
...
--left outer join maker_scancopy on slip_no = DPTDC_SLIP_NO  and deleted_ind =1
           ...
           settlement_type_mstr          settm2     with(nolock)                                          
           on (convert(varchar,settm2.settm_id)            = dptdc.dptdc_other_settlement_type)                            
    WHERE  dpam.dpam_id                  = dptdc.dptdc_dpam_id    AND DPAM.dpam_sba_no=dptdc.dptdc_created_by  
    --and dptdc.DPTDC_SLIP_NO like 'e%'                
    and dptdc.dptdc_slip_no in (
    Select cdsl_trxn_id from E_DIS_PROCESS_DATA with(nolock)
    union all
    select DPTDC_SLIP_NO from dptdc_mak I where i.dptdc_slip_no=dptdc.DPTDC_SLIP_NO and dptdc.DPTDC_SLIP_NO like 'e%'                
    )           
    AND    dpm_excsm_id                  = excsm.excsm_id                     
AND    excsm_list.excsm_id      = excsm.excsm_id                        
    AND    dpam.dpam_dpm_id              = dpm.dpm_id                        
