-- Object: PROCEDURE citrus_usr.E_DIS_Slip_gen_Process
...
CREATE proc [citrus_usr].[E_DIS_Slip_gen_Process] (@sett_no varchar(11) ,@flag varchar(1)='')
as 
	  ...
	  Declare @payin datetime,@payout varchar(8)

set @payin = (
select start_date from [ANGELDEMAT].msajag.dbo.sett_mst
...
where Sett_No=@sett_no  and sett_type='N' )
		...
		--INSERT  INTO #DIY_Reprocess_log
		----Exec [ANGELDEMAT].msajag.dbo.Rpt_ShortagePayin 'broker','broker',@sett_no,@sett_no,'W','%',-1
		--Exec [ANGELDEMAT].msajag.dbo.Rpt_DelPayinMatch 'broker', 'broker', @Fsett_no,'F','ALL','%',2
	...
	--	print @cnt
		...
		select @sdate=start_date from [ANGELDEMAT].msajag.dbo.sett_mst where sett_no=@sett_no and sett_type='N'
		 ...
		 where exists (select party_code from E_Dis_Trxn_Data t 
		 where party_code =t.Partycode and d.CERTNO=t.isin and dummy2=@payout
		 AND ISNULL(VALID,0)=0 and isnull(dummy3,'')=''
		 --and getdate() between Request_date
		 -- and Request_date+ NO_of_days + ' 23:59' 
		  --AND ISNULL(VALID,0)=0 and isnull(dummy3,'')=''
		  --and NO_of_days=5 and Request_date>='2021-02-19'
		  )
   ...
   --select * from DIY_Reprocess_log_e_dis
	     ...
	     where t.Party_code =n.Partycode and t.isin=n.isin  --and sett_no ='2021016'--and partycode='A133377'
       ...
       When SETT_TYPE='W' then 13 END ) AS pa_tr_setm_type,  
		...
		--'1203320006951435' As pa_tr_acct_no
		'1100001100017670' As pa_tr_acct_no ,isin ,ANGEL_TRXN_ID,	CDSL_TRXN_Id
		  Into #FinalTable   
		  from 
		   E_DIS_Process_Data  
		...
		declare @count int;  
		Declare @counter int=1,@totalcount int,@p26 varchar(100), @pa_login_name varchar(50),@pa_slip_no varchar(50),@pa_excm_id varchar(10),@p33 varchar(100), 
  ...
  /*
		...
		exec pr_ins_upd_trx_cdsl_sp_PreEDIS_AngelPre @pa_id='0',@pa_tab='BOCM',@pa_action='INS',@pa_login_name=@pa_login_name,  
		...
		exec pr_ins_upd_trx_cdsl_sp_PreEDIS_AngelPre @pa_id='0',@pa_tab='EP',@pa_action='INS',@pa_login_name=@pa_login_name,@pa_dpm_dpid='12033200',@pa_dpam_acct_no=@pa_login_name,
		@pa_slip_no=@pa_slip_no,@pa_mkt_type='',@pa_settlm_no=@pa_settlm_no,@pa_req_dt=@executionndate,@pa_cm_id='M50082',@pa_excm_id=@pa_excm_id,@pa_cash_trf='NA',@pa_exe_dt=@executionndate,
		@pa_tr_cmbpid='',@pa_tr_dp_id='',@pa_tr_setm_type=@pa_tr_setm_type,@pa_tr_setm_no='',@pa_tr_acct_no=@pa_tr_acct_no,@pa_values=@pa_values,@pa_rmks='',
		@pa_trdReasoncd='0',@pa_Paymode='',@pa_Bankacno='',@pa_Bankacname='',@pa_BankBrname='',@pa_TransfereeName='',@pa_DOI='',@pa_ChqRefno='',@pa_chk_yn=1,@RowDelimiter='*|~*',@ColDelimiter='|*~|',@PA_ERRMSG=@p33  output
