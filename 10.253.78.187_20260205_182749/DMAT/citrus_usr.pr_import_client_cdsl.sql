-- Object: PROCEDURE citrus_usr.pr_import_client_cdsl
...
CREATE PROCEDURE  [citrus_usr].[pr_import_client_cdsl]
			(
			 @pa_exch          VARCHAR(20)    
...
--                ,DPHD_NOM_lNAME  =  citrus_usr.fn_splitval_by(replace(isnull(ltrim(rtrim(TMPCLI_NOMI_NM)),''),' ','|'),3,'|')  
          ...
          ,    dp_acct_mstr   
          ,    TMP_CLIENT_DTLS_MSTR_CDSL  
		  left outer join     TMP_CDSL_AUTH_SIGNATORY on    BO_ID = TMPCLI_BOID 
        ...
        --  
          ...
          SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper(ISNULL(LTRIM(RTRIM(TMPCLI_ACCT_OPNG_DT)),'')) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'BILL_START_DT'  and TMPCLI_ACCT_OPNG_DT <> ''
          SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper(ISNULL(LTRIM(RTRIM(TMPCLI_CLO_APP_DT)),''))  + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'ACC_CLOSE_DT'   and TMPCLI_CLO_APP_DT <> ''
...
--          SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + ISNULL(LTRIM(RTRIM(ben_rbi_ref_no)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR WHERE Ben_Acct_No = @c_ben_acct_no AND ENTPM_CD = 'RBI_REF_NO'  
         ...
         , TMP_CLIENT_DTLS_MSTR_CDSL   
           ...
           AND DPAM_SBA_NO = TMPCLI_BOID   
           AND DPAM_SBA_NO =  @c_ben_acct_no1   
           AND accp_accpm_prop_cd = 'BILL_START_DT' AND accp_DELETED_IND = 1  and TMPCLI_ACCT_OPNG_DT <> ''
      ...
      --contact channels  
           ...
           SET @L_acc_conc = ''  
                 ...
                 SELECT @L_BANK_NAME = ltrim(rtrim(TMPCLI_BANK_NM))   
                      ...
                      ,@L_MICR_NO = TMPCLI_BANK_CD   ,@L_rtgs_cd=TMPCLI_DIVIDEND_BR_NO
                 FROM   TMP_CLIENT_DTLS_MSTR_CDSL 
				WHERE TMPCLI_BOID = @c_ben_acct_no1  
   
                             
                 IF EXISTS(SELECT BANM_ID FROM BANK_MSTR WHERE ((BANM_NAME = @L_BANK_NAME 
						  AND BANM_BRANCH = @L_BANM_BRANCH) or banm_micr = @L_MICR_NO ) and banm_rtgs_cd = @L_rtgs_cd)  
                  BEGIN  

				 

                     SELECT @L_BANM_ID = BANM_ID FROM BANK_MSTR WHERE ((BANM_NAME = @L_BANK_NAME AND BANM_BRANCH = @L_BANM_BRANCH) or banm_micr = @L_MICR_NO)  and banm_rtgs_cd = @L_rtgs_cd 
                  ...
                  BEGIN  
                    ...
                    EXEC pr_mak_banm  '0','INS','MIG',@L_BANK_NAME,@L_BANM_BRANCH,@L_MICR_NO,@L_rtgs_cd,'','',0,0,'',0,'','*|~*','|*~|',''  
                    SELECT @L_BANM_ID = BANM_ID FROM BANK_MSTR WHERE ((BANM_NAME = @L_BANK_NAME AND BANM_BRANCH = @L_BANM_BRANCH) or banm_micr = @L_MICR_NO  ) and banm_rtgs_cd=@L_rtgs_cd
...
begin  
 ...
 FROM TMP_CLIENT_DTLS_MSTR_CDSL , poa_mstr WHERE ltrim(rtrim(poam_name1)) = ltrim(rtrim(TMPCLI_POA_NM)) 
 and substring(poam_master_id,3,6)=substring(TMPCLI_BOID,3,6)
 and  tmpcli_boid = @c_ben_acct_no1 AND TMPCLI_POA_ID <>''   
  ...
  --brokerage
      ...
      --bank_mstr/addresses/conctact_channels  
            ...
            SELECT @L_ADDR_VALUE = 'COR_ADR1|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BANK_ADD_LN1)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BANK_ADD_LN2)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BANK_ADD_LN3)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BANK_ADD_CITY)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BANK_ADD_STATE)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BANK_ADD_CNTRY)),'')+'|*~|'+'*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL WHERE TMPCLI_BOID = @c_ben_acct_no  
             select @l_rtgs_cd = TMPCLI_DIVIDEND_BR_NO FROM   TMP_CLIENT_DTLS_MSTR_CDSL WHERE TMPCLI_BOID = @c_ben_acct_no  
             set @L_BANM_ID = 0  
             IF EXISTS(SELECT BANM_ID FROM BANK_MSTR WHERE (BANM_NAME = @L_BANK_NAME AND BANM_BRANCH = @L_BANM_BRANCH) or banm_micr = @L_MICR_NO ) -- and banm_rtgs_cd=@l_rtgs_cd)  
             BEGIN  
                SELECT @L_BANM_ID = BANM_ID FROM BANK_MSTR WHERE (BANM_NAME = @L_BANK_NAME AND BANM_BRANCH = @L_BANM_BRANCH) or banm_micr = @L_MICR_NO --and banm_rtgs_cd=@l_rtgs_cd 
             END  
             ELSE   
      ...
      --account_properties  
         ...
         SELECT DISTINCT @L_ACCP_VALUE = isnull(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','DIVIDEND_CURRENCY',upper(ISNULL(LTRIM(RTRIM(TMPCLI_DIVIDEND_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no AND ACCPM_PROP_CD   = 'DIVIDEND_CURRENCY'  
          ...
          SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper(ISNULL(LTRIM(RTRIM(TMPCLI_ACCT_OPNG_DT)),'')) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no AND ACCPM_PROP_CD   = 'BILL_START_DT'  and TMPCLI_ACCT_OPNG_DT <> ''
          SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper(ISNULL(LTRIM(RTRIM(TMPCLI_CLO_APP_DT)),''))  + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no AND ACCPM_PROP_CD   = 'ACC_CLOSE_DT'  and TMPCLI_CLO_APP_DT <> ''
...
--          SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + ISNULL(LTRIM(RTRIM(ben_rbi_ref_no)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR WHERE Ben_Acct_No = @c_ben_acct_no AND ENTPM_CD = 'RBI_REF_NO'  
        ...
        SET @L_ENTPD_VALUE = ''  
         ...
         FROM TMP_CLIENT_DTLS_MSTR_CDSL , poa_mstr WHERE  ltrim(rtrim(poam_name1)) = ltrim(rtrim(TMPCLI_POA_NM)) 
and substring(poam_master_id,3,6)=substring(TMPCLI_BOID,3,6)
and tmpcli_boid = @c_ben_acct_no  AND TMPCLI_POA_ID <>''   
