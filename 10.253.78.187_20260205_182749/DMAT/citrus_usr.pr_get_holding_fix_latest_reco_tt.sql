-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_reco_tt
...
create proc [citrus_usr].[pr_get_holding_fix_latest_reco_tt](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
having (isnull(sum(dphmcd_curr_qty)  ,0)                       
+ isnull(sum(DPHMCD_FREE_QTY)        ,0)                 
+ isnull(sum(DPHMCD_FREEZE_QTY)         ,0)                
+ isnull(sum(DPHMCD_PLEDGE_QTY )           ,0)              
+ isnull(sum(DPHMCD_DEMAT_PND_VER_QTY)        ,0)                  
+ isnull(sum(DPHMCD_REMAT_PND_CONF_QTY)          ,0)                
+ isnull(sum(DPHMCD_DEMAT_PND_CONF_QTY)             ,0)             
+ isnull(sum(DPHMCD_SAFE_KEEPING_QTY )                 ,0)        
+ isnull(sum(DPHMCD_LOCKIN_QTY )  ,0)                       
+ isnull(sum(DPHMCD_ELIMINATION_QTY),0)                       
+ isnull(sum(DPHMCD_EARMARK_QTY)       ,0)                   
+ isnull(sum(DPHMCD_AVAIL_LEND_QTY)       ,0)                   
+ isnull(sum(DPHMCD_LEND_QTY)                ,0)          
+ isnull(sum(DPHMCD_BORROW_QTY)     ,0) <> '0'    )
