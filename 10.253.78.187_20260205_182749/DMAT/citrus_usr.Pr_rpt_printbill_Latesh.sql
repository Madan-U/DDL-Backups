-- Object: PROCEDURE citrus_usr.Pr_rpt_printbill_Latesh
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--PR_RPT_PRINTBILL 'NSDL',4,'MAY 02 2008','AUG 31 2008 23:59:59','n','n','','','','N','y',1,20,1,'HO|*~|','u5bvdx45q5bavgrrz4m5uyyf','Y','Y','courier','HO',''	
--PR_RPT_PRINTBILL 'CDSL',3,'apr  1 2009','apr 30 2009 23:59:59','n','n','1205810000004801','','','N','y',1,20,1,'HO|*~|','u5bvdx45q5bavgrrz4m5uyyf','Y','Y','courier','HO',''	

create Proc [citrus_usr].[Pr_rpt_printbill_Latesh]            
@pa_dptype varchar(4),                    
@pa_excsmid int,                    
...
/* To print those client which are not block */


		--if @pa_bulk_printflag = 'N'
		--begin

		--		delete from #ACLIST_CDSL where  not exists (select blckpd_dpam_id from blk_client_print_dtls 
		--		where dpam_id = blckpd_dpam_id and blkcpd_dpmid = @@dpmid
		--		and blkcpd_rptname = 'BILL')
		-- end
			...
			family varchar(1000), valuation numeric(18,3)  
			)'            
			exec(@@ssql)
			...
			set @@ssql='insert into ' + @@temptable + '(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val,family,valuation)         
			...
			CDSHM_ID,                  
			cdshm_charge,
			ltrim(rtrim(entm_short_name)),0
			FROM CDSL_HOLDING_DTLS ,        
			#ACLIST_CDSL account  , entity_mstr , entity_relationship                        
			where   entr_sba = dpam_sba_no and entm_id = ENTR_DUMMY1   and  CDSHM_DPM_ID = ' +  convert(varchar,@@dpmid) + '
            ...
            set @@ssql = @@ssql + ' order by ltrim(rtrim(entm_short_name)),CDSHM_BEN_ACCT_NO,dpam_sba_name,CDSHM_ISIN,CDSHM_TRAS_DT'
...
print '101'
            ...
            begin
			 ...
			 begin        
				   ...
				   3,
				   0,      
				   0, entm_short_name , CONVERT(NUMERIC(18,2),DPHMCD_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0)) VALUATION                       
				   FROM DP_DAILY_HLDG_CDSL
                                   LEFT OUTER JOIN ISIN_MSTR ON DPHMCD_ISIN = ISIN_CD              
                                   LEFT OUTER JOIN CLOSING_PRICE_MSTR_CDSL ON DPHMCD_ISIN = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,''01/01/1900'') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = DPHMCD_ISIN and CLOPM_DT <= '''+convert(varchar(11),@@prevfromdate,109)+''' and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc) ,  				   
				   #ACLIST_CDSL account , entity_mstr , entity_relationship 
				   WHERE    entr_sba = dpam_sba_no and entm_id = ENTR_DUMMY1   and      
                   ...
                   set @@ssql = @@ssql + ' order by ltrim(rtrim(entm_short_name)),DPAM_SBA_NO,dpam_sba_name,DPHMCD_ISIN'
...
print @@ssql
		  ...
		  set @@ssql='insert into ' + @@temptable + '(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val,family,valuation)         
		  select clic_dpam_id,dpam_sba_name,dpam_sba_no,clic_charge_name,null,clic_trans_dt=''' +  convert(varchar,@pa_todate,109) + ''',null,null,null,2,null,sum(clic_charge_amt),entm_short_name ,0        
		  ...
		  set @@ssql = @@ssql + ' group by clic_dpam_id,dpam_sba_name,dpam_sba_no,clic_charge_name,entm_short_name'
	     ...
	     set @@ssql = 'insert into ' + @@temptable + '(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val,family,valuation) 
	     SELECT dpam_id , dpam_sba_name, dpam_sba_no , '''','''','''','''',0,0,4,0,0 ,entm_short_name ,0  FROM #ACLIST_CDSL a, entity_mstr , entity_relationship 
	     WHERE entr_sba = dpam_sba_no and entm_id = ENTR_DUMMY1   and     NOT EXISTS(SELECT dpam_id FROM ' + @@temptable + ' b WHERE a.dpam_id = b.dpam_id)
         and isnumeric(DPAM_SBA_NO) = 1    
         ...
         and convert(numeric,dpam_sba_no) between convert(numeric,''' + @pa_fromaccid + ''') and convert(numeric,''' + @pa_toaccid + ''')' 
         set @@ssql = @@ssql + ' order by ltrim(rtrim(entm_short_name)),dpam_sba_no,dpam_sba_name' 
         exec(@@ssql)
		...
		from (
		select distinct family,dpam_id,dpam_acctno from ' + @@temptable + ' 
		...
		group by family,dpam_id,dpam_acctno order by family,dpam_acctno'
		exec(@@ssql)
		...
		begin
          ...
          set @@l_sql='truncate table tmp_Billsummary_print'
          set @@l_sql= @@l_sql + ' insert into tmp_Billsummary_print'


          set @@l_sql= @@l_sql + ' select order_by,trx.dpam_id,dpam_sba_name,trx.dpam_acctno,                    
		  CDSHM_TRATM_DESC,            
		  dpm_trans_no,                    
		  trans_date,                    
		  trx.ISIN_CD,        
          ...
          set @@l_sql= @@l_sql + ' ( '
          set @@l_sql= @@l_sql + ' select order_by,t.dpam_id,dpam_sba_name,t.dpam_acctno,                    
		  CDSHM_TRATM_DESC=isnull(ltrim(rtrim(trans_desc)),''''),            
		  dpm_trans_no,                    
		  ...
		  page_cnt,
		  bill_due_dt = ''' + isnull(@@Billduedate,'') + ''',
		  line_no, isnull(t.family,'''') family
          ...
          and order_by in (1,2)'
		  --order by t.dpam_acctno,dpam_sba_name,order_by,Isin_name,trans_date,18,line_no'
          set @@l_sql= @@l_sql + ' union '

          set @@l_sql= @@l_sql + '  select  '''' ,t.dpam_id,dpam_sba_name,t.dpam_acctno,                    
		  '''',            
		  '''',                    
		  '''',                    
		  '''' ,        
		  ...
		  '''' ,
		  '''',
		  '''', isnull(t.family,'''') family

		  FROM ' + @@temptable + 'clients tc,' + @@temptable + ' t LEFT OUTER JOIN  isin_mstr i on t.isin_cd = i.isin_cd                   
		  where tc.dpam_id = t.dpam_id and tc.id > ' + convert(varchar,((@pa_currrecordctr-1) * @pa_reqdrecordcnt)) + ' and tc.id <=  ' + convert(varchar,(@pa_currrecordctr * @pa_reqdrecordcnt)) + '
          and order_by not in (1,2) group by t.dpam_id,dpam_sba_name,prev_bill_bal,t.dpam_acctno,isnull(t.family,'''')'
		  
          set @@l_sql= @@l_sql + ' ) trx '
          set @@l_sql= @@l_sql + '  order by trx.family,trx.dpam_acctno,dpam_sba_name,order_by,Isin_name,trans_date,18,line_no '	  
		  ...
		  page_cnt,
		  bill_due_dt = ''' + isnull(@@Billduedate,'') + ''',
		  trans_date, isnull(t.family,'''') family ,isnull(valuation,0) valuation 
		  FROM ' + @@temptable + 'clients tc,' + @@temptable + ' t LEFT OUTER JOIN  isin_mstr i on t.isin_cd = i.isin_cd                   
		  where tc.dpam_id = t.dpam_id and tc.id > ' + convert(varchar,((@pa_currrecordctr-1) * @pa_reqdrecordcnt)) + ' and tc.id <=  ' + convert(varchar,(@pa_currrecordctr * @pa_reqdrecordcnt)) + '
		  order by t.family,t.dpam_acctno,dpam_sba_name,order_by,Isin_name,trans_date,18,line_no'
