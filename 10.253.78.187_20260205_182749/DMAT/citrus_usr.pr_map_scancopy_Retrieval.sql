-- Object: PROCEDURE citrus_usr.pr_map_scancopy_Retrieval
...
CREATE  proc [citrus_usr].[pr_map_scancopy_Retrieval](
 @pa_id numeric
...
begin 
	...
	select @fromdt = convert(datetime,citrus_usr.FN_SPLITVAL_BY(@pa_entity_name,1,'|*~|'),103)
    select @todt = convert(datetime,citrus_usr.FN_SPLITVAL_BY(@pa_entity_name,2,'|*~|'),103)
    
    if @fromdt='Jan  1 1900 12:00AM'
    begin
	...
	and slip_no like  '%' +  @pa_slip_no  + '%'
	and deleted_ind = 1 		
	--and lst_upd_dt between @fromdt and @todt
    end
    else
    begin
	...
	and slip_no like  '%' +  @pa_slip_no  + '%'
	and deleted_ind = 1 		
	and lst_upd_dt between @fromdt and @todt    + '23:59:59'
    end
