-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_o3
...
create PROC [citrus_usr].[Pr_Rpt_Holding_o3]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
END                          
 ...
 SELECT 
			...
			DPAM_ID, 
			HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109)        
---/*feb1010*/, SETTM_DESC SETTM_DESC, right(dphmc_cntr_settm_id,7)     setm_no  
            , SETTM_DESC SETTM_DESC, right(dphmc_cntr_settm_id,7)     setm_no  
            --,isnull(dphmc_cntr_settm_id,'')  setm_no                
FROM 
			DP_HLDG_MSTR_CDSL LEFT OUTER JOIN ISIN_MSTR ON DPHMC_ISIN = ISIN_CD 
-------------/*feb1010*/ left outer join settlement_type_mstr on left(dphmc_cntr_settm_id,6)=SETTM_TYPE_CDSL and settm_deleted_ind=1,                
            left outer join settlement_type_mstr on left(dphmc_cntr_settm_id,6)=SETTM_TYPE_CDSL and settm_deleted_ind=1                
			,#ACLIST ACCOUNT                                
WHERE
...
-------------/*feb1010*/ AND CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE ISNULL(SETTM_TYPE,'') END = CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE @PA_SETTM_TYPE END
-------------/*feb1010*/ AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(right(dphmc_cntr_settm_id,7),'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END                         
			AND CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE ISNULL(SETTM_TYPE,'') END = CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE @PA_SETTM_TYPE END
			AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(right(dphmc_cntr_settm_id,7),'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END                         
...
CONVERT(NUMERIC(18,3),DPHMCD_BORROW_QTY),
		DPAM_ID,
		HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109) --CONVERT(VARCHAR(11),@@TMPHOLDING_DT,109)  
     , SETTM_DESC SETTM_DESC, right(dphmcd_cntr_settm_id,7)     setm_no                                                   
        -------/*feb1010*/               ,isnull(dphmcd_cntr_settm_id,'') setm_no
FROM 
		    DP_DAILY_HLDG_CDSL LEFT OUTER JOIN ISIN_MSTR ON DPHMCD_ISIN = ISIN_CD
--/*feb1010*/ left outer join settlement_type_mstr on left(dphmcd_cntr_settm_id,6)=SETTM_TYPE_CDSL and settm_deleted_ind=1,                                    
left outer join settlement_type_mstr on left(dphmcd_cntr_settm_id,6)=SETTM_TYPE_CDSL and settm_deleted_ind=1                                    
		    ,#ACLIST ACCOUNT                                      

WHERE 
		DPHMCD_HOLDING_DT = @@TMPHOLDING_DT AND DPHMCD_DPM_ID = @@DPMID                          
...
--/*feb1010*/ AND CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE ISNULL(SETTM_TYPE,'') END = CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE @PA_SETTM_TYPE END
--/*feb1010*/ AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(right(dphmcd_cntr_settm_id,7),'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END                         
		AND CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE ISNULL(SETTM_TYPE,'') END = CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE @PA_SETTM_TYPE END
		AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(right(dphmcd_cntr_settm_id,7),'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END                         
...
PRINT('F')
   ...
   BEGIN                      
     ...
     ,CONVERT(NUMERIC(18,3),DPHMC_ELIMINATION_QTY),CONVERT(NUMERIC(18,3),DPHMC_EARMARK_QTY),CONVERT(NUMERIC(18,3),DPHMC_AVAIL_LEND_QTY),CONVERT(NUMERIC(18,3),DPHMC_LEND_QTY),CONVERT(NUMERIC(18,3),DPHMC_BORROW_QTY),DPAM_ID, HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109),CMP=ISNULL(convert(numeric(18,2),CLOPM_CDSL_RT),0.00)    
      ,SETTM_DESC SETTM_DESC, right(dphmc_cntr_settm_id,7)     setm_no 
     ----/*feb1010*/,,isnull(dphmc_cntr_settm_id,'') setm_no
...
-------/*feb1010*/ AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(right(dphmc_cntr_settm_id,7),'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END                         
   ...
   INSERT INTO #ACLIST SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,EFF_FROM,EFF_TO FROM CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID)    
     ...
     SELECT distinct DPAM_SBA_NAME,DPAM_SBA_NO,DPHMCD_ISIN,ISIN_NAME,CONVERT(NUMERIC(18,0),DPHMCD_CURR_QTY),VALUATION=CONVERT(NUMERIC(18,2),DPHMCD_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0)),CONVERT(NUMERIC(18,0),DPHMCD_FREE_QTY),CONVERT(NUMERIC(18,0),DPHMCD_FREEZE_QTY),CONVERT(NUMERIC(18,0),DPHMCD_PLEDGE_QTY)      
...
,CONVERT(NUMERIC(18,3),DPHMCD_BORROW_QTY),DPAM_ID,HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109)--CONVERT(VARCHAR(11),@@TMPHOLDING_DT,109)
...
/*FEB1010*/LEFT OUTER JOIN SETTLEMENT_TYPE_MSTR ON LEFT(dphmcd_cntr_settm_id,6)=SETTM_TYPE_CDSL AND SETTM_DELETED_IND=1       
     ...
     #ACLIST ACCOUNT                                      
     WHERE DPHMCD_HOLDING_DT = @@TMPHOLDING_DT AND DPHMCD_DPM_ID = @@DPMID                          
     AND DPHMCD_DPAM_ID = ACCOUNT.DPAM_ID                      
     AND (DPHMCD_HOLDING_DT BETWEEN EFF_FROM AND EFF_TO)                    
   --  AND (CONVERT(NUMERIC,DPAM_SBA_NO) BETWEEN CONVERT(NUMERIC,@PA_FROMACCID) AND  CONVERT(NUMERIC,@PA_TOACCID))                          
	 AND DPAM_SBA_NO BETWEEN @PA_FROMACCID AND  @PA_TOACCID
