-- Object: PROCEDURE citrus_usr.PR_CDSL_IMP_TRX_airan_130510
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--begin tran
--PR_CDSL_IMP_TRX 'HO','50' ,3,'','','EP','1',''
--select * from dptdc_mak where dptdc_slip_no ='P2100'
--rollback
...
create  PROCEDURE [citrus_usr].[PR_CDSL_IMP_TRX_airan_130510] (  
...
set @L_ERR_MISSING_ISIN = ''
    ...
    begin
        ...
        if not exists(select TMPOFFM_SLIP_NO from TMP_DP_OFFM_DTLS_CDSL where left(TMPOFFM_SLIP_NO,1) = 'P' )
       begin 
 UPDATE filetask
        ...
        return 
                ...
                update bitmap_ref_mstr set bitrm_bit_location =  @L_MAX_DPTDC_DTLS_ID1+@l_id1  where bitrm_parent_cd = 'DPTDC_DTLS_ID' 
...
--
 ...
 if not exists(select TMPINTDP_SLIP_NO from TMP_DP_INTDP_DTLS_CDSL where left(TMPINTDP_SLIP_NO,1) = 'P' )
       begin 
 UPDATE filetask
		SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Slips are not Issued : ' + @L_ERR_MISSING_ISIN
		,      status  = 'FAILED'
...
if not exists(select TMPNP_SLIP_NO,TMPNP_BO_ID
    ...
    begin
     ...
     SELECT distinct  @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  +   TMPNP_SLIP_NO +' : ' + TMPNP_BO_ID +',' from TMP_DP_NP_DTLS_CDSL
        if not exists(select TMPNP_SLIP_NO from TMP_DP_NP_DTLS_CDSL where left(TMPNP_SLIP_NO,1) = 'P' )
       begin
 UPDATE filetask
		SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Slips are not Issued : ' + @L_ERR_MISSING_ISIN
		,      status = 'FAILED'
...
if not exists(select TMPEP_SLIP_NO,TMPEP_BO_ID 
    ...
    begin
        ...
        if not exists(select TMPEP_SLIP_NO from TMP_DP_EP_DTLS_CDSL where left(TMPEP_SLIP_NO,1) = 'P' )
        begin 
 UPDATE filetask
		SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Slips are not Issued : ' + @L_ERR_MISSING_ISIN
		,      status = 'FAILED'
...
if exists(select TMPOFFM_SLIP_NO from TMP_DP_OFFM_DTLS_CDSL where TMPOFFM_SLIP_NO not like 'p%')
	begin
	INSERT INTO used_slip
...
Else
	begin
	INSERT INTO used_slip
	SELECT top 1 @l_id_used_slip + ID1 , @l_dpm_id1 , TMPOFFM_BO_ID , DPTDC_SLIP_NO,'1','','U','HO',getdate(),'HO',getdate(),1 
...
IF @PA_TRX_TAB  = 'INTERDP'
begin
if exists(select TMPINTDP_SLIP_NO from TMP_DP_INTDP_DTLS_CDSL where TMPINTDP_SLIP_NO not like 'p%')
	BEGIN
	INSERT INTO used_slip
...
ELSE
	BEGIN
	INSERT INTO used_slip
	SELECT top 1 @l_id_used_slip + ID1 , @l_dpm_id1 , TMPINTDP_BO_ID , DPTDC_SLIP_NO,'1','','U','HO',getdate(),'HO',getdate(),1 
...
IF @PA_TRX_TAB  = 'NP'
BEGIN 
if exists (select TMPNP_SLIP_NO from TMP_DP_NP_DTLS_CDSL where TMPNP_SLIP_NO not like 'p%' )
	begin 
	INSERT INTO used_slip
	...
	INSERT INTO used_slip
	SELECT top 1  @l_id_used_slip + ID1 , @l_dpm_id1 , TMPNP_BO_ID , DPTDC_SLIP_NO,'1','','U','HO',getdate(),'HO',getdate(),1 
...
IF @PA_TRX_TAB  = 'EP'
BEGIN
if exists (select TMPEP_SLIP_NO from TMP_DP_EP_DTLS_CDSL where TMPEP_SLIP_NO not like 'p%')
	begin 
	INSERT INTO used_slip
	...
	begin 
	INSERT INTO used_slip
	SELECT top 1 @l_id_used_slip + ID1 , @l_dpm_id1 , TMPEP_BO_ID , DPTDC_SLIP_NO,'1','','U','HO',getdate(),'HO',getdate(),1 
