-- Object: PROCEDURE citrus_usr.pr_holding_process_dpc9_for_sp
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--alter table DP_DAILY_HLDG_CDSL add dphmcd_cntr_settm_id varchar(13)
--alter table DP_HLDG_MSTR_CDSL add dphmc_cntr_settm_id varchar(13)

CREATE procedure [citrus_usr].[pr_holding_process_dpc9_for_sp](@pa_holding_date datetime,@l_dpm_id numeric(10,0),@pa_login_name varchar(25),@pa_count int)
AS 
BEGIN
--
...
-- holding
 ...
 set  @l_max_holding_dt  = isnull(@l_max_holding_dt,'jan  1 1900')
            ...
            ,dphmcd_ctr_settm_id varchar(13)
			)
            ...
            ,dphmcd_ctr_settm_id
				...
				SELECT  dphmcd_dpam_id
					...
					and    not exists (select dphmcd_isin,dphmcd_dpam_id from #tmp_dp_daily_hldg_cdsl tmp 
                                       where tmp.dphmcd_isin = dpdhcd.dphmcd_isin and tmp.dphmcd_dpam_id = dpdhcd.dphmcd_dpam_id
                                       AND   tmp.dphmcd_ctr_settm_id = dpdhcd.dphmcd_cntr_settm_id)
  ...
  END
            ...
            AND    tmpcdslhld.TMPDPC9_HLDG_SETTID_04   = tmp.dphmcd_ctr_settm_id
   ...
   END
	...
	INSERT INTO #tmp_dp_daily_hldg_cdsl
		...
		and    tmpdpc9_hldg_trx_dt = @l_holding_dt
		AND    NOT EXISTS(select tmp.dphmcd_dpam_id from #tmp_dp_daily_hldg_cdsl tmp 
                          where tmp.dphmcd_dpam_id = tmpdpc9_hldg_dpam_id and tmp.dphmcd_isin= tmpdpc9_hldg_isin
                          AND    tmp.dphmcd_ctr_settm_id = TMPDPC9_HLDG_CTR_SETTID)
	   ...
	   --
		     ...
		     ,tmpdpc9_hldg_dpm_id 
						 ...
						 ,@l_next_holding_dt
							...
							,1
	       FROM   tmp_dpc9_cdsl_trx_hldg
	       WHERE  not exists (select dphmcd_isin,dphmcd_holding_dt,dphmcd_dpam_id from dp_daily_hldg_cdsl where dphmcd_holding_dt = @l_next_holding_dt and dphmcd_isin = tmpdpc9_hldg_isin and tmpdpc9_hldg_dpam_id = dphmcd_dpam_id)
