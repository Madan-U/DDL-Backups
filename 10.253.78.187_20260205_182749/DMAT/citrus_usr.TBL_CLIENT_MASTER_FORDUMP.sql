-- Object: VIEW citrus_usr.TBL_CLIENT_MASTER_FORDUMP
...
CREATE  view [citrus_usr].[TBL_CLIENT_MASTER_FORDUMP]  
as  
...
,convert(numeric,case when pc1.AnnIncomeCd  ='' then '0' else pc1.AnnIncomeCd   end ) INCOME_CODE  ---CHanges done under SRE-36711
...
,convert(numeric,CASE when pc1.DivAcctType='' THEN 10 ELSE pc1.DivAcctType END ) BANK_ACCOUNT_TYPE  
,convert(numeric,CASE when pc1.DivAcctType='' THEN 10 ELSE pc1.DivAcctType END  ) BANK_ACC_TYPE  
...
,convert(numeric,CASE when pc1.DivAcctType='' THEN 10 ELSE pc1.DivAcctType END) DIVIDEND_BANK_AC_TYPE  
...
,'' POA_ENABLED  
,'' POA_TYPE  
,convert(numeric,CASE when pc1.LangCd='' THEN 0 ELSE pc1.LangCd END  ) LANG_CODE  
...
,PC1.smart_flag  FIRST_SMS_FLAG-------CHANGE UNDER SRE-41447
,'' SMART_REMARKS  
...
,'' PSI_FLAG  
,'' RTA_FLAG  
,convert(varchar(1),pc1.Filler9  ) BSDA_FLAG ----CHANGE UNDER SRE-41447 Filler8 to Filler9
...
where dpam.DPAM_SBA_NO = pc1.BOId   and dpam.DPAM_SBA_NO <> dpam.DPAM_acct_NO
