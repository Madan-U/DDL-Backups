-- Object: PROCEDURE citrus_usr.pr_import_client_cdsl_JITESH
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--begin tran
--exec pr_import_client_cdsl 'CDSL','HO','normal','','*|~*','|*~|',''  
--exec pr_import_client_cdsl_JITESH 'CDSL','HO','normal','','*|~*','|*~|',''
...
CREATE PROCEDURE  [citrus_usr].[pr_import_client_cdsl_JITESH]
			(
			 @pa_exch          VARCHAR(20)    
...
--          and    clim_crn_no =  dpam_crn_no   
			...
			set    dpam_stam_cd = case when TMPCLI_ACCT_STAT = '01' or TMPCLI_ACCT_STAT = 'ACTIVE' then 'ACTIVE' 
                               ...
                               when TMPCLI_ACCT_STAT = 'To Be Closed' then '04' else TMPCLI_ACCT_STAT end
...
--                ,DPHD_NOM_lNAME  =  citrus_usr.fn_splitval_by(replace(isnull(ltrim(rtrim(TMPCLI_NOMI_NM)),''),' ','|'),3,'|')  
          ...
          declare @c_client_summary1  cursor  
				 ...
				 --where exists(select dpam_sba_no,dpam_acct_no from dp_Acct_mstr) and TMPCLI_ACCT_STAT <> '10' 
...
--      EXEC pr_reset_bill_printing @c_ben_acct_no1  
      ...
      --contact channels  
           ...
           SELECT @L_CONC = 'OFF_PH1|*~|'+ISNULL(ltrim(rtrim(TMPCLI_BO_TELPH_NO)),'')+'*|~*'+'MOBILE1|*~|'+ISNULL(ltrim(rtrim(TMPCLI_BO_TELPHP_NO)),'')+'*|~*'+'FAX1|*~|'+ISNULL(ltrim(rtrim(TMPCLI_BO_FAX_NO)),'')+'*|~*'+'FAX2|*~|'+ISNULL(ltrim(rtrim(TMPCLI_BO_FAXF_NO)),'')+'*|~*'  
...
--                            ) and banm_micr = TMPCLI_BANK_CD  
        ...
        select TMPCLI_BOID,TMPCLI_DP_INTREF_NO  from TMP_CLIENT_DTLS_MSTR_CDSL a where isnull(TMPCLI_FRST_HLDR_PAN_N0,'') <> ''  
        --and TMPCLI_BOID not in (select dpam_sba_no from dp_Acct_mstr) and TMPCLI_ACCT_STAT <> '10'  
        and not exists(select dpam_acct_no from dp_Acct_mstr b where b.dpam_sba_no = a.TMPCLI_BOID and b.dpam_acct_no = a.TMPCLI_DP_INTREF_NO) 
		and TMPCLI_ACCT_STAT <> '10'
        ...
        fetch next from @c_client_summary into @c_ben_acct_no,@c_acct_no   
      ...
      --dp_acct_mstr  
           ...
           select @l_dp_acct_values = isnull(convert(varchar,@l_compm_id),'')+'|*~|'+ isnull(convert(varchar,@l_excsm_id),'')+'|*~|'+isnull(convert(varchar,@l_dpm_dpid),'')+'|*~|'+isnull(@c_ben_acct_no,'')+'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_NM)),'')+'|*~|'+ ISNULL(ltrim(rtrim(TMPCLI_DP_INTREF_NO)),'') +'|*~|'+case when TMPCLI_ACCT_STAT = '01' or TMPCLI_ACCT_STAT = 'ACTIVE' then 'ACTIVE' 
							   when TMPCLI_ACCT_STAT = 'CLOSED' then '05' when TMPCLI_ACCT_STAT = 'To Be Closed' then '04'  else TMPCLI_ACCT_STAT end+'|*~|'+citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(TMPCLI_ACCT_CTRGY,''),'enttm') + '|*~|' + citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(TMPCLI_BO_STAT,''),'clicm')+'|*~|'+citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(ltrim(rtrim(TMPCLI_BO_SUB_STAT)),''),'subcm')+'|*~|0|*~|A|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL WHERE  tmpcli_boid = @c_ben_acct_no   
