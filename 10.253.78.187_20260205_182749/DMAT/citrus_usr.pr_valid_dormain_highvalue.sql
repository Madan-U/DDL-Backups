-- Object: PROCEDURE citrus_usr.pr_valid_dormain_highvalue
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

---pr_valid_dormain_highvalue '4','DORMENT',640,'Formno'    
CREATE procedure [citrus_usr].[pr_valid_dormain_highvalue](@pa_id numeric,@pa_action varchar(100), @pa_dtls_id varchar(100),@pa_output varchar(8000) output)    
as    
begin    
...
--,@pa_loginname =@l_login,@pa_req_dt ='',@l_out  = @pa_output out   
--set @pa_output = 'N'   
--  
--return   

exec [pr_chk_app_level_cdsl] @pa_id ='0',@pa_dtls_id =@l_dtls_id,@pa_loginname =@l_login,@pa_req_dt ='',  @pa_output=@l_out outPUT       
...
print 'ssss'  
 ...
 if exists(    
--select dptdc_dtls_id from dp_trx_dtls_cdsl where citrus_usr.fn_get_high_val(dptdc_isin,abs(dptdc_qty),'HIGH_VALUE','','') = 'Y' and dptdc_dtls_id = @pa_dtls_id and dptdc_deleted_ind = 1    
select dptdc_dtls_id from dptdc_mak where citrus_usr.fn_get_high_val(dptdc_isin,abs(dptdc_qty),'HIGH_VALUE',@l_dpam_acct_no,@l_req_dt) = 'Y' and dptdc_dtls_id = @pa_dtls_id and dptdc_deleted_ind = -1  
