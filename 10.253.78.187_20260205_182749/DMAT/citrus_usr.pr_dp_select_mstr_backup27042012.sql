-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_backup27042012
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

create  PROCEDURE  [citrus_usr].[pr_dp_select_mstr_backup27042012]( @pa_id  VARCHAR(800)                          
    ...
    --                        
      ...
      --                        
       ...
       SELECT settm.settm_id            id                        
        ...
        WHERE  dpam.dpam_id            = accp.accp_clisba_id                        
        AND    accp.accp_accpm_prop_cd = 'CMBPEXCH'                         
        AND    RIGHT(dpam.dpam_SBA_no,8)       = RIGHT(@pa_cd,8)                        
        AND    accp.accp_value         = excm.excm_cd                        
        AND    settm.settm_excm_id     = excm.excm_id                        
        AND    settm.settm_deleted_ind = 1                        
        AND    excm.excm_deleted_ind   = 1                        
...
,case when DEMRM_DELETED_IND = 0 and (isnull(demrm_res_cd_intobj,'') <> '' or isnull(demrm_res_cd_compobj,'') <> '') then 'REJECTED' end status1
 ...
 begin                 
    ...
    ,'' DRNNO                              
          ...
          ,isnull(demrm_lockin_reason_cd,'')      REASONCODE                     
         -- ,convert(varchar,demrm_lockin_release_dt,103)     RELEASEDATE                    
          ,CASE WHEN ISNULL(convert(varchar(11),demrm_lockin_release_dt,106),'') ='01 Jan 1900' THEN '' ELSE ISNULL(convert(varchar(11),demrm_lockin_release_dt,106),'') END RELEASEDATE
          ,@pa_id                    EXCHANGE                     
          ,demrm_id                    ID                     
          ,demttd_hld1                    
          ,demttd_hld2             
		  ...
		  ,DEMRM_CREATED_DT
 ,DEMRM_CREATED_BY ---added by mayur after error occured 24/04/2012 
    FROM   demrm_mak                   demrm                    
           left outer join                     
           demat_tran_tranm_dtls       demttd on demttd.demttd_demrm_id    = demrm.demrm_id                     
          LEFT OUTER JOIN  [FN_GETSUBTRANSDTLS]('DEMAT_REJ_CD_NSDL')   a    ON  a.cd                      = demrm.DEMRM_INTERNAL_REJ                
...
(
    SELECT distinct  @l_dpm_dpid                                DPID                         
          ...
          ,isnull(settm2.settm_desc,'')               OTHERSETTLEMENTTYPE         
          ,dptdc_created_by Makerid              
          ,highval = ((select top 1 CLOPM_CDSL_RT from closing_last_cdsl where  CLOPM_ISIN_CD = dptdc_isin and DPTDC_SLIP_NO like case when @pa_rmks = '' then '%' else @pa_rmks end) * abs(dptdc_qty))         
...
print @l_dpm_dpid      
  print @l_entm_id      
   ...
   and    uses_slip_no between convert(numeric,@pa_values) and convert(numeric,@pa_desc)                                   
...
--    order by SLIIM_ID desc  
		...
		select @L1 =  max(convert(numeric,sliim_slip_no_to))+ 1  from slip_issue_mstr where SLIIM_DELETED_IND = 1    
		select @L3 =  max(convert(numeric,sliim_book_name))+ 1  from slip_issue_mstr where SLIIM_DELETED_IND = 1    
		PRINT @L1
		--  
