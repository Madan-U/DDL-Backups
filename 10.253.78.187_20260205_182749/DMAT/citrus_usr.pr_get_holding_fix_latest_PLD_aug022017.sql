-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_PLD_aug022017
...
CREATE    proc [citrus_usr].[pr_get_holding_fix_latest_PLD_aug022017](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
select cdshm_dpam_id , cdshm_isin , sum(cdshm_qty  ) qty ,cdshm_trans_no into #trasdmtpenfing1  
from cdsl_holding_dtls with (nolock)
...
--select * from #FINALDEMATHOLDI where DPHMCD_DPAM_ID = 546295	and DPHMCD_isin ='INE058I01013'
...
--select * from #pCONVERQTY where DPHMCD_DPAM_ID = 546295	and DPHMCD_ISIN ='INE058I01013'
...
--SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY*-1) QTY into #availland_dr FROM CDSL_HOLDING_DTLS a  with (nolock)
--WHERE CDSHM_TRATM_cD  in ( '2220') --=case when CDSHM_CDAS_TRAS_TYPE IN ('21','22','23') then '2262' else '2277' end --2261 added by tushar on jun 03 2014
--and CDSHM_TRAS_DT between  @l_min_trx_dt and @pa_to_dt
--and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to 
...
----select * from #availland_dr where CDSHM_DPAM_ID ='635887' and cdshm_isin ='INE483A01010'

--update a set DPHMCD_AVAIL_LEND_QTY = DPHMCD_AVAIL_LEND_QTY + QTY 
--from #availland_dr  
...
----,a.DPHMCD_AVAIL_LEND_QTY =  case when  a.DPHMCD_AVAIL_LEND_QTY - case when QTY >= 0 then QTY       else 0 end  >=0 then  a.DPHMCD_AVAIL_LEND_QTY - case when QTY >= 0 then QTY       else 0 end
----when  a.DPHMCD_AVAIL_LEND_QTY - case when QTY >= 0 then QTY       else 0 end  <0 then  0 
----else a.DPHMCD_AVAIL_LEND_QTY  end 
--from #tmp_dp_daily_hldg_cdsl   a , #finaldmatpen2 f    
--where a.dphmcd_dpam_id= f.dphmcd_dpam_id     
--and a.dphmcd_isin = f.dphmcd_isin 

--update a      
--set a.DPHMCD_AVAIL_LEND_QTY =  a.DPHMCD_AVAIL_LEND_QTY - f.qty
--from #tmp_dp_daily_hldg_cdsl   a , #TEMPDATA2 f    
--where a.dphmcd_dpam_id= f.cdshm_dpam_id     
--and a.dphmcd_isin = f.cdshm_isin
--and a.DPHMCD_AVAIL_LEND_QTY<>0 
--and  a.DPHMCD_AVAIL_LEND_QTY - f.qty>=0
--and f.qty > 0  
