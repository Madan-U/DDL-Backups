-- Object: PROCEDURE citrus_usr.PR_IMPORT_CLIENT_CDSL_DPB9_bak_14122017
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------


create PROCEDURE  [citrus_usr].[PR_IMPORT_CLIENT_CDSL_DPB9_bak_14122017]
			(
			 @pa_exch          VARCHAR(20)    
...
/*changes done by tushar on may 22 2014 for nrn no update if nominee change*/
		 ...
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(ITCircle)),'')) + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARFLAG'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(ITCircle)),'')) + '|*~|*|~*'  FROM DPB9_PC2 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARSECHLDR'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(ITCircle)),'')) + '|*~|*|~*'  FROM DPB9_PC3 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARTHRDHLDR'  
...
SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(rel_WITH_BO)),'')) <> '' THEN citrus_usr.fn_get_cd_rel(UPPER(ISNULL(LTRIM(RTRIM(rel_WITH_BO)),''))) ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = '1ST_NRELATN_BO'
SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(perc_OF_SHARES)),''))<> '' THEN UPPER(ISNULL(LTRIM(RTRIM(perc_OF_SHARES)),'')) ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = '1ST_NPER_SHR'
SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(RES_SEC_FLg)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(RES_SEC_FLg)),''))='N' THEN 'NO' ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = '1ST_NRED_SEC_FLG'		
...
select @l_max= max(cliba_lst_upd_dt) from client_bank_accts ,dp_acct_mstr where dpam_id = cliba_clisba_id and DPAM_SBA_NO =@c_ben_acct_no1 and CLIBA_FLG='1'
...
SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(ECS)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'ECS_FLG'  
        ...
        SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + '1'  + '|*~|*|~*'  FROM DPB9_PC16 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'SMS_FLAG'
        
        
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(rel_WITH_BO)),'')) <> '' THEN citrus_usr.fn_get_cd_rel(UPPER(ISNULL(LTRIM(RTRIM(rel_WITH_BO)),''))) ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = '1ST_NRELATN_BO'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(perc_OF_SHARES)),''))<> '' THEN UPPER(ISNULL(LTRIM(RTRIM(perc_OF_SHARES)),'')) ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = '1ST_NPER_SHR'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(RES_SEC_FLg)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(RES_SEC_FLg)),''))='N' THEN 'NO' ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = '1ST_NRED_SEC_FLG'
