-- Object: FUNCTION citrus_usr.FN_CONSIDERATIONAMT_bak_30062021
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

--drop   FUNCTION [citrus_usr].[FN_CONSIDERATIONAMT]
create  FUNCTION [citrus_usr].[FN_CONSIDERATIONAMT_bak_30062021](@PA_ACTION VARCHAR(25),@PA_INTREF NUMERIC)
RETURNS VARCHAR(800)
AS
BEGIN
...
IF @PA_ACTION='CAMT'
BEGIN
SELECT  @L_TOTAL= case when dptdc_line_no='' then '0' else isnull(replace (replace(replace(replace(dptdc_line_no,',',''),'.',''),'/',''),'-',''),'0') end 
