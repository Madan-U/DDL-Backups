-- Object: PROCEDURE citrus_usr.pr_upd_nom_dpb9
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

CREATE proc [citrus_usr].[pr_upd_nom_dpb9]
as
begin
...
,NOM_TRXPRINTFLAG=filler_a
,NOM_DRIVINGLIC=driving_licence
,NOM_PASSUIC=passport_oci
,NOM_ISDCODE=filler_b
...
,case when  perc_OF_SHARES = '' then '0' else perc_OF_SHARES end perc_OF_SHARES
,filler_a
,driving_licence
,passport_oci
,filler_b
...
select @l_count = count(1) from #missingnom  
 ...
 SELECT @l_nom_id      = bitrm_bit_location  
     ...
     print  @l_nom_id    
		...
		case when DateOfBirth = '' then '1900-01-01 00:00:00.000' else  substring (DateOfBirth,5,4)+'-'+substring (DateOfBirth,3,2)+'-'+substring (DateOfBirth,1,2)+ ' ' +'00:00:00.000' end,Addr1,Addr2
,Addr3,City,State,Country,PinCode,PriPhInd,PriPhNum
,AltPhInd,AltPhNum,AddPhones,Fax,PANGIR,ItCircle
