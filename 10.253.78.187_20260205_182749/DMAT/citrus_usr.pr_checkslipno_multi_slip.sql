-- Object: PROCEDURE citrus_usr.pr_checkslipno_multi_slip
...
CREATE   procedure [citrus_usr].[pr_checkslipno_multi_slip]  
(  
...
SET @@series = ''  
 ...
 BEGIN  
  ...
  ELSE  
  BEGIN  
   IF EXISTS(SELECT uses_slip_no FROM used_slip WHERE  uses_dpam_acct_no = @accountno AND uses_trantm_id = @trans_code and abs(USES_SLIP_NO) = @@sr_no  and uses_dpm_id = @l_dpm_id and USES_SERIES_TYPE = @@series and USES_DELETED_IND = 1)  
   BEGIN  
    set @pa_errmsg =  'Slip no. has already been used' + '|*~|1'   
    set @pa_errmsg = ISNULL(@accountno,'') + '|*~|0'  
    RETURN
 ...
 BEGIN  
   ...
   BEGIN  
    ...
    IF EXISTS(SELECT uses_slip_no FROM used_slip WHERE   uses_trantm_id = @trans_code and abs(USES_SLIP_NO) = @@sr_no  and uses_dpm_id = @l_dpm_id and USES_SERIES_TYPE = @@series and USES_DELETED_IND = 1)  
