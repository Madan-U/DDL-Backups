-- Object: PROCEDURE citrus_usr.pr_ins_upd_onmC2P_trx_multi
...
--Delivery Instuction (For OnMarket deliveries) -- FROM CLIENT TO POOL
...
*/
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_onmC2P_trx_multi](@pa_id              VARCHAR(8000)  
...
--      , @l_tr_setm_no   VARCHAR(50)
      , @l_isin         VARCHAR(25) 
      , @l_qty          VARCHAR(25) 
		,@l_tr_cmbpid						 VARCHAR(20)
		,@l_tr_setm_type			 VARCHAR(20)
		,@l_tr_setm_no					 VARCHAR(20)
...
SET @l_trastm_id     = '904'
SET @l_trastm_desc   = 'C2P'

end
else
begin
 SET @l_trastm_id     = '905'
 SET @l_trastm_desc   = 'C2P_R'
end
...
--
		...
		,dptd_counter_cmbp_id varchar(20)
		,dptd_status char(1) 
...
SET @remainingstring = @pa_id  
								...
								--
								  ...
								  SET @l_action             = citrus_usr.fn_splitval(@currstring_value,1)      
										...
										--
												...
												set @l_tr_cmbpid		= citrus_usr.fn_splitval(@currstring_value,4) 
												set @l_tr_setm_type		= citrus_usr.fn_splitval(@currstring_value,5) 
												set @l_tr_setm_no		= citrus_usr.fn_splitval(@currstring_value,6) 
                                                set @l_res_cd		= citrus_usr.fn_splitval(@currstring_value,7) 
														...
														--
														  ...
														  UPDATE  dp_trx_dtls 
																SET     dptd_dpam_id              = @l_dpam_id
																							...
																							,dptd_isin                 = @l_isin
																							,dptd_qty                  = @l_qty
																							,dptd_other_settlement_type= @l_tr_setm_type
																							,dptd_other_settlement_no  = @l_tr_setm_no
																							,dptd_counter_cmbp_id      = @l_tr_cmbpid
																							,dptd_rmks																	= @pa_rmks
																							,dptd_lst_upd_dt           = getdate()
																							,dptd_lst_upd_by           = @pa_login_name
																WHERE   dptd_id                   = @l_id
														...
														--      
																	...
																	--
																			...
																			SET     dptd_dpam_id              = dptdm.dptd_dpam_id               
																										...
																										,dptd_counter_cmbp_id      = dptdm.dptd_counter_cmbp_id      
  ...
  IF left(ltrim(rtrim(@pa_errmsg)),5) <> 'ERROR'
		BEGIN
		--
				exec pr_checkslipno '','ONC2P_DP_TRX_SEL', @pa_dpm_dpid,@pa_dpam_acct_no,@pa_slip_no,@pa_login_name,''
		--
  END
--
END
