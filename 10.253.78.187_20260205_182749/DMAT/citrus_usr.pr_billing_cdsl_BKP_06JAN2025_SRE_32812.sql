-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_BKP_06JAN2025_SRE_32812
...
--ROLLBACK
CREATE procedure [citrus_usr].[pr_billing_cdsl_BKP_06JAN2025_SRE_32812](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
...
--insert into cdsl_holding_dtls_bak_dnd
--select *, 'back up before bill run' , getdate()  from cdsl_holding_dtls where cdshm_tras_dt = convert (varchar (11),getdate ()-1,109)
--insert into cdsl_holding_dtls_bak_dnd
--select *, 'back up before bill run' , getdate()  from cdsl_holding_dtls where cdshm_tras_dt = convert (varchar (11),getdate (),109)
...
insert into billlog
select 'DIS client list start', GETDATE () 
...
insert into billlog
select 'DIS client list end', GETDATE () 
...
insert into billlog
select 'DIS charge start', GETDATE () 
...
insert into billlog
select 'one time charge start', GETDATE () 
...
and not exists(select sba_no from #tempdataforwaive where sba_no = dpam_sba_no)
  ...
  insert into billlog
select 'one time charge end', GETDATE () 

 
 insert into billlog
select 'ledger data start', GETDATE () 
...
DROP TABLE #VOUCHERNO
 ...
 insert into billlog
select 'ledger data end', GETDATE () 
 ...
 insert into billlog
select 'Fixed data start', GETDATE () 
   ...
   ,charge_val
   ,post_toacct)
   select citrus_usr.getfixedchargedate_test(accp_value,@pa_billing_from_dt,@pa_billing_to_dt,cham_bill_period,cham_bill_interval,@pa_excmid)
			,dpam_id
			,cham_slab_name
			,sum(cham_charge_value )*-1 
			,cham_post_toacct
			...
			AND   citrus_usr.getfixedchargedate_test(accp_value,@pa_billing_from_dt,@pa_billing_to_dt,cham_bill_period,cham_bill_interval,@pa_excmid)  between clidb_eff_from_dt and clidb_eff_to_dt
			AND   clidb_brom_id = proc_profile_id
			AND   proc_slab_no  = cham_slab_no
			AND   cham_charge_type = 'F'
			AND   citrus_usr.getfixedchargedate_test(accp_value,@pa_billing_from_dt,@pa_billing_to_dt,cham_bill_period,cham_bill_interval,@pa_excmid) is not null
			and   cham_deleted_ind = 1
			and   clidb_deleted_ind = 1
			and   dpam_deleted_ind = 1
			and   proc_deleted_ind = 1
  ...
  /*Q Pro rata basis Fix Amc Oct 24 2024*/
     insert into #temp_bill
	(trans_dt 
	,dpam_id  
	,charge_name 
 ...
 select accp_value 
	,dpam_id  
	,cham_slab_name 
   ,sum(cham_charge_value) 
   ,cham_post_toacct from (
 select --'OLD A/c' [Account], accp_value , 
 DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_from_dt), MONTH(@pa_billing_from_dt ), DAY(@pa_billing_to_dt ))  )/3), 0)  accp_value
			,dpam_id  
			,cham_slab_name
			,cham_charge_value  *-1   cham_charge_value 
			,cham_post_toacct
			from dp_acct_mstr
				,client_dp_brkg
				,profile_charges
                ...
                ,charge_ctgry_mapping,#account_properties left outer join #account_properties_close on accp_clisba_id = accp_clisba_id_cl
			where dpam_dpm_id = @pa_dpm_id and accp_clisba_id = dpam_id  
			--and CHAM_CHARGE_TYPE ='f' and CHAM_BILL_PERIOD ='m' and cham_bill_interval='3' 
			--and   DPAM_STAM_CD not in('05','04','08','02_BILLSTOP','CI')
			and   ( DPAM_STAM_CD not in('05','04','08','02_BILLSTOP','CI') or @pa_billing_to_dt between  accp_value and isnull(accp_value_cl,'jan 31 2100'))
			and dpam_stam_cd not in ('CI','04','CLOS','05')
			AND   dpam_id     = clidb_dpam_id
            and   CHAM_SLAB_NO = chacm_cham_id
            and   case when chacm_subcm_cd = '0' then '0' else dpam_subcm_cd end = chacm_subcm_cd
			--and DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(GETDATE()), MONTH(accp_value), DAY(accp_value))  )/3), 0)  between @pa_billing_from_dt and @pa_billing_to_dt 
			AND   clidb_brom_id = proc_profile_id
			AND   proc_slab_no  = cham_slab_no
			AND   cham_charge_type = 'FQ'
			and   @pa_billing_to_dt between clidb_eff_from_dt and isnull(clidb_eff_to_dt,'Dec 31 2100')
			and   DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_from_dt), MONTH(@pa_billing_from_dt ), DAY(@pa_billing_to_dt ))  )/3), 0)  between @pa_billing_from_dt and @pa_billing_to_dt 
			AND   accp_value < DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_from_dt), MONTH(@pa_billing_from_dt ), DAY(@pa_billing_to_dt ))  )/3), 0)  
			and   cham_deleted_ind = 1
			and   clidb_deleted_ind = 1
			and   dpam_deleted_ind = 1
			and   proc_deleted_ind = 1
...
union all 
 ...
 select --'New A/c' [Account]  , accp_value , 
 accp_value Billing_Date
			,dpam_id  
			,cham_slab_name
			, round((cham_charge_value / 3.00 )* (3-case when accp_value >   DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_from_dt), MONTH(accp_value), DAY(accp_value))  )/3), 0) 
then datediff(mm,DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_from_dt), MONTH(accp_value), DAY(accp_value))  )/3), 0) ,accp_value )  
else 0 end ) ,0,0) *-1
			,cham_post_toacct
			from dp_acct_mstr
				,client_dp_brkg
...
AND   citrus_usr.getfixedchargedate_test(CONVERT(DATETIME,citrus_usr.fn_acct_entp(CLIDB_dpam_id,'BILL_START_DT'),103),@pa_billing_from_dt  ,@pa_billing_to_dt  ,cham_bill_period,cham_bill_interval,@pa_excmid)  between clidb_eff_from_dt and clidb_eff_to_dt   
...
/*MIN MAX CHARGES LOGIC*/
  ...
  ------
        ...
        set    cdshm_charge   =  
case when (cdshm_internal_trastm='INTDEP-DR' and CDSHM_INT_REF_NO  in ( 'Transmission','closure','T','TR','TRF') )  then 0 else
...
----							AND   cham_charge_type = cdshm_tratm_type_desc
----							--AND   (abs(cdshm_qty) >= 0)  and (abs(cdshm_qty) <= cham_to_factor)  
----							AND   cham_charge_type not in ( 'DEMAT') 
----							AND   cdshm_tratm_cd = '2277'
----							and   cham_deleted_ind = 1
----							and   clidb_deleted_ind = 1
----							and   cdshm_deleted_ind = 1
...
where CDSHM_ISIN =ISIN_CD AND ISIN_SEC_TYPE in ('20','3','37') AND ISIN_SECURITY_TYPE_DESCRIPTION in('Government Securities','Sovereign Gold Bonds') -- SGB added as per mail from vishal dated 07102024
and   cdshm_tras_dt between  @pa_billing_from_dt and @pa_billing_to_dt
and isnull(cdshm_charge,0) <> 0
-- ADDED FOR WAIVE OF ON GOV SEC BY LATESH P W ON AUG 4 16		
...
------new login nov 27 20
 ...
 insert into billlog
select 'Amc data delete  start', GETDATE () 

/*A Tushar */

--if exists (select 1 from  Qtr_Park 
--where DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0)   = DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(trans_dt), MONTH(trans_dt ), DAY(trans_dt ))  )/3), 0)   )
--and 1 = (select count(DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(trans_dt), MONTH(trans_dt ), DAY(trans_dt ))  )/3), 0)   ) from Qtr_Park )
--begin 

 insert into billlog
select 'insert into Qtr_Park  start', GETDATE () 

delete from Qtr_Park where trans_dt between @pa_billing_from_dt and @pa_billing_to_dt  and isnull(q_flg ,0) = 0 

--select * into tempdatA123456 from #temp_bill


CREATE INDEX IX_1 ON #temp_bill (charge_name)

CREATE INDEX IX_2 ON #temp_bill (dpam_id)


insert into Qtr_Park (trans_dt
,dpam_id
,charge_name
,charge_val
,post_toacct
...
,null FLG
,null Q_FLG
,DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0) Qtr 
from #temp_bill t, charge_mstr , #account_properties 
where not exists (select 1 from Qtr_Park main where main.dpam_id = t.dpam_id 
and Qtr = DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0) ) 
and cham_charge_type ='FQ'
and cham_slab_name = charge_name 
and accp_clisba_id = dpam_id 

 insert into billlog
select 'insert into Qtr_Park  End', GETDATE () 


insert into #temp_bill(trans_dt   
					,dpam_id    
					,charge_name   
					,charge_val
					,post_toacct)
select case when @pa_billing_from_dt <=  accp_value then accp_value else  @pa_billing_from_dt end  , dpam_id
,charge_name
,charge_val
,post_toacct
from Qtr_Park  a , #account_properties  where DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0)  = Qtr 
and not exists (select 1 from #temp_bill b , charge_mstr where CHAM_SLAB_NAME = b.charge_name 
and cham_charge_type ='FQ'
and a.dpam_id = b.dpam_id   
and a.trans_dt = b.trans_dt ) 
and a.dpam_id = accp_clisba_id 
and isnull(a.q_flg,0) = 0
--end 
--else 
--begin

--insert into Qtr_Park_log
--select *,getdate() from Qtr_Park

--delete from Qtr_Park 
...
/*A Tushar */ 
 ...
 insert into billlog
select 'delete from  #temp_bill  start', GETDATE () 
...
/*C only add 'FQ' with 'F' and 'O' Tushar */
--SELECT * FROM #temp_bill WHERE DPAM_ID = 21431854
delete a  from #temp_bill a where   exists (select   1  from AMCDec2019Oct2020 where bdpam_id = dpam_id and @pa_billing_to_dt < clic_nextamcdt ) 
--SELECT * FROM #temp_bill WHERE DPAM_ID = 21431854




select DPAM_ID INTO #TMPDPAM_ID   from client_trade_month,dp_acct_mstr  b where Client_COde = dpam_sba_no 
and isnull(last_trade ,'') between  @pa_billing_from_dt and @pa_billing_to_dt 

--INSERT INTO #TMPDPAM_ID 
--select DPHMCD_DPAM_ID  from holdingallforview with(nolock) group  by DPHMCD_DPAM_ID having SUM(ISNULL(rate,0)*dphmcd_curr_qty) >  5000 
...
and  cham_charge_type in ('F','o','FQ') and CHAM_BILL_INTERVAL in('1','3') AND BROM_ID NOT IN ('83') )
...
----------and not exists (select 1 from client_trade_month,dp_acct_mstr  b where Client_COde = dpam_sba_no and a.dpam_id = b.dpam_id and isnull(last_trade ,'') between  @pa_billing_from_dt and @pa_billing_to_dt 
----------UNION ALL 
----------select 1 from holdingallforview where DPHMCD_DPAM_ID = dpam_id group  by DPHMCD_DPAM_ID having SUM(ISNULL(rate,0)*dphmcd_curr_qty) >  5000 
----------)
and not exists (SELECT 1 FROM #TMPDPAM_ID B WHERE A.DPAM_ID = B.DPAM_ID ) 
...
--and (
 ...
 --or
 -- exists (select   1  from AMCDec2019Oct2020 where bdpam_id = dpam_id and @pa_billing_to_dt < clic_nextamcdt ) 
 -- or
 -- not exists (select   1  from AMCDec2019Oct2020 where bdpam_id = dpam_id   ) -- May 11 2021 for client cross 1 year for exceptional client
 --)
...
--not exists (select  1 from client_charges_cdsl where clic_dpam_id = dpam_id and CLIC_POST_TOACCT in(18,19,27) and CLIC_TRANS_DT between @pa_billing_from_dt and @pa_billing_to_dt ) 
...
delete a from #temp_bill a where 
 charge_name in (select CHAM_SLAB_NAME   from charge_mstr , brokerage_mstr , profile_charges  where brom_id = PROC_PROFILE_ID and PROC_SLAB_NO = CHAM_SLAB_NO 
and  cham_charge_type in ('F','o','FQ') and CHAM_BILL_INTERVAL  in('1','3')   AND BROM_ID  IN ('83') )
--and not exists (select 2 from cdsl_holding_dtls where cdshm_dpam_id = dpam_id and CDSHM_TRAS_DT between  @pa_billing_from_dt and @pa_billing_to_dt)
--and not exists (select 2 from [172.31.16.94].dmat.citrus_usr.client_trade_month,dp_acct_mstr  b where Client_COde = dpam_sba_no and a.dpam_id = b.dpam_id and isnull(last_trade ,'') between  @pa_billing_from_dt and @pa_billing_to_dt )
and not exists (SELECT 1 FROM #TMPDPAM_ID B WHERE A.DPAM_ID = B.DPAM_ID ) 
--any type of transaction ?
...
AND EXISTS (SELECT 1 FROM dp_acct_mstr b ,#account_properties ACCP 
WHERE A.DPAM_ID = B.DPAM_ID AND accp.ACCP_CLISBA_ID = b.DPAM_ID  AND DATEDIFF (DD,accp_value,@pa_billing_to_dt) > 1095)
--not exists (select  1 from client_charges_cdsl where clic_dpam_id = dpam_id and CLIC_POST_TOACCT in(18,19,27) and CLIC_TRANS_DT between @pa_billing_from_dt and @pa_billing_to_dt ) 
--and exists (select   1  from AMCDec2019Oct2020 where bdpam_id = dpam_id )-- exclude amc of bill Dec 19-oct20
--holding date ?


-- Ek ka teen scheme client delete not opened in billing month
delete a from #temp_bill a,#account_properties ACCP where accp.accp_clisba_id = a.dpam_id and 
CHARGE_NAME in (select CHAM_SLAB_NAME   from charge_mstr , brokerage_mstr , profile_charges  where brom_id = PROC_PROFILE_ID and PROC_SLAB_NO = CHAM_SLAB_NO 
and  cham_charge_type in ('F','FQ') and CHAM_BILL_INTERVAL  in('1','3')  AND BROM_ID  IN ('83') )
and TRANS_DT between  @pa_billing_from_dt and @pa_billing_to_dt 
and EXISTS (SELECT 1 FROM dp_acct_mstr b ,#account_properties ACCP 
WHERE A.DPAM_ID = B.DPAM_ID AND accp.ACCP_CLISBA_ID = a.DPAM_ID  AND DATEDIFF (DD,accp_value,@pa_billing_to_dt) <365 )
--EK KA TEEN_AMC--o
--EK KA TEEN_ACMAIN-F
-- Ek ka teen scheme client delete not opened in billing month
...
if @pa_billing_from_dt < 'dec 01 2021'
...
and cham_charge_type in ('F','o','FQ') and CHAM_BILL_INTERVAL  in('1','3') )
and exists (select   1  from AMCDec2019Oct2020 where bdpam_id = dpam_id and @pa_billing_to_dt < clic_nextamcdt)--change on may 20 2021 
--not exists (select  1 from client_charges_cdsl where clic_dpam_id = dpam_id and CLIC_POST_TOACCT in(18,19,27) and CLIC_TRANS_DT between @pa_billing_from_dt and @pa_billing_to_dt ) 
...
and TRANS_DT between @pa_billing_from_dt and @pa_billing_to_dt
and DATEDIFF (DD,accp_value,@pa_billing_to_dt) between case when Filter_Free in (1,2,3) then 0
when Filter_Free in (23) then 366  end and case when Filter_Free in (1) then 365
 when Filter_Free in (2) then 730
 when Filter_Free in (3) then 1095
...
/*C only add 'FQ' with 'F' and 'O' Tushar */ 
delete a 
from qtr_park a ,dp_acct_mstr b ,#account_properties  accp, client_dp_brkg 
, lateshscheme
where clidb_dpam_id = b.dpam_id and @pa_billing_to_dt  between clidb_eff_from_dt and ISNULL(clidb_eff_to_dt ,'dec 31 2100')
and CLIDB_BROM_ID = ID and  [Affect Yes / No] not in ('Y','N')
and b.DPAM_ID = a.DPAM_ID  
and accp.ACCP_CLISBA_ID = b.DPAM_ID 
and TRANS_DT between @pa_billing_from_dt and @pa_billing_to_dt
--and DATEDIFF (DD,accp_value,EOMONTH (DATEFROMPARTS(YEAR(GETDATE()),month(accp_value),case when PARSENAME(YEAR(GETDATE())/4.00,1) <> 0  and day(accp_value) =29 and month(accp_value) = 2 then '28' else day(accp_value) end))) between case when Filter_Free in (1,2,3) then 0
and DATEDIFF (DD,accp_value,@pa_billing_to_dt ) between case when Filter_Free in (1,2,3) then 0
when Filter_Free in (23) then 366  end and case when Filter_Free in (1) then 365
 when Filter_Free in (2) then 730
 when Filter_Free in (3) then 1095
...
UPDATE A SET charge_val = CASE WHEN  datediff (mm,trans_dt,
DATEADD(DAY ,1,EOMONTH( EOMONTH (DATEFROMPARTS(YEAR(GETDATE()),month(accp_value)
--,day(accp_value))),-1))) = 2 
,case when PARSENAME(YEAR(GETDATE())/4.00,1) <> 0  and day(accp_value) =29 and month(accp_value) = 2 then '28' else day(accp_value) end)),-1))) = 2 
and datediff(dd,accp_value,trans_dt) < Filter_Free * 365
THEN -20 
WHEN  datediff (mm,trans_dt,
DATEADD(DAY,1,EOMONTH( EOMONTH (DATEFROMPARTS(YEAR(GETDATE()),month(accp_value)
--,day(accp_value))),-1))) = 1 
,case when PARSENAME(YEAR(GETDATE())/4.00,1) <> 0  and day(accp_value) =29 and month(accp_value) = 2 then '28' else day(accp_value) end)),-1))) = 1 
and datediff(dd,accp_value,trans_dt) < Filter_Free * 365
THEN -40
WHEN  datediff (mm,trans_dt,
DATEADD(DAY,1,EOMONTH( EOMONTH (DATEFROMPARTS(YEAR(GETDATE()),month(accp_value)
--,day(accp_value))),-1))) = 0 
,case when PARSENAME(YEAR(GETDATE())/4.00,1) <> 0  and day(accp_value) =29 and month(accp_value) = 2 then '28' else day(accp_value) end)),-1))) = 0 
and datediff(dd,accp_value,trans_dt) < Filter_Free * 365
THEN -60
ELSE  -60 END from qtr_park  a ,dp_acct_mstr b ,#account_properties  accp, client_dp_brkg 
, lateshscheme
where clidb_dpam_id = b.dpam_id and @pa_billing_to_dt  between clidb_eff_from_dt and ISNULL(clidb_eff_to_dt ,'dec 31 2100')
and CLIDB_BROM_ID = ID and  [Affect Yes / No] not in ('Y','N')
and b.DPAM_ID = a.DPAM_ID  
...
/*B Tushar*/
 ...
 insert into billlog
select 'delete from  #temp_bill  end', GETDATE () 

update main set Q_FLG = 0 
from Qtr_Park main , ytbill a 
, charge_mstr cham 
where not exists (select 1 from ytbill_after b , charge_mstr  c where a.dpam_id = b.dpam_id and  b.charge_name = c.cham_slab_name  and c.cham_charge_type ='FQ'  ) 
and a.charge_name = cham.cham_slab_name 
and main.dpam_id = a.dpam_id 
and Qtr = DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0) 
and cham.cham_charge_type ='FQ'
 and isnull(Q_FLG ,0) not in (1) 

update main set Q_FLG = 1 , q_billed_dt = @pa_billing_from_dt  
from Qtr_Park main , ytbill a 
, charge_mstr cham 
where exists (select 1 from ytbill_after b , charge_mstr  c where a.dpam_id = b.dpam_id and  b.charge_name = c.cham_slab_name  and c.cham_charge_type ='FQ' ) 
and a.charge_name = cham.cham_slab_name 
and main.dpam_id = a.dpam_id 
and Qtr = DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0) 
and cham.cham_charge_type ='FQ'
and isnull(Q_FLG ,0)  in (0) 
...
,post_toacct
from Qtr_Park a , #account_properties where  DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0)  = Qtr 
and isnull(a.q_flg,0) =1
and not exists (select 1 from #temp_bill b , charge_mstr where CHAM_SLAB_NAME = b.charge_name 
and cham_charge_type ='FQ'
and a.dpam_id = b.dpam_id   
--and @pa_billing_from_dt = b.trans_dt
and case when @pa_billing_from_dt <=  accp_value then accp_value else  @pa_billing_from_dt end  = b.trans_dt 
) 
and q_billed_dt = @pa_billing_from_dt  
and a.dpam_id = accp_clisba_id 


/*B Tushar*/

insert into   qtr_park_bak_dnd_count
select q_billed_dt, Qtr,getdate() insert_dt,  count(1) cnt   from qtr_park group by  q_billed_dt, Qtr 

 
insert into qtr_park_bak_dnd
select *, getdate ()  insert_dt  from qtr_park where Q_FLG = 1

			insert into billlog
select 'Amc data delete  end', GETDATE () 
...
--						  logic for charge on amount like service tax
  ...
  -- to find out double pledge charges in jv - by yogesh -dt 14072021
--insert into   cdsl_holding_dtls_dcfo
--select *, GETDATE () dt    from cdsl_holding_dtls 
----where CDSHM_BEN_ACCT_NO = '1203320045822881'
--where  CDSHM_TRAS_DT between @pa_billing_from_dt and @pa_billing_to_dt
--and cdshm_charge <> 0 
...
insert into billlog
select 'Bill data delete start', GETDATE () 	 
...
insert into billlog
select 'Bill data delete end', GETDATE () 					 
					 
					 
insert into billlog
select 'Bill data insert start', GETDATE () 	
...
insert into billlog
select 'Bill data insert end', GETDATE () 					 
