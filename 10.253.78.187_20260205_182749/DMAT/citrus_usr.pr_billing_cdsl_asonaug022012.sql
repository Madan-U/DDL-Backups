-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_asonaug022012
...
*/
--[pr_billing_cdsl] 'jul 01 2012','jul 31 2012',3,'y'
create procedure [citrus_usr].[pr_billing_cdsl_asonaug022012](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
...
DROP TABLE #VOUCHERNO
 ...
 if (select month(fin_start_dt) from financial_yr_mstr where @pa_billing_from_dt between FIN_START_DT and FIN_END_DT and FIN_DPM_ID = @pa_dpm_id) = month(@pa_billing_from_dt)
begin 

declare @l_dt datetime 
select @l_dt = fin_start_dt from financial_yr_mstr where @pa_billing_from_dt between FIN_START_DT and FIN_END_DT and FIN_DPM_ID = @pa_dpm_id
  insert into #temp_bill
	(trans_dt 
	,dpam_id  
	,charge_name 
...
--            and   case when chacm_subcm_cd = '0' then '0' else dpam_subcm_cd end = chacm_subcm_cd
            ...
            and   CHAM_CHARGE_BASE = 'NORMAL'
			union all 
			select @pa_billing_to_dt,dpam_id,'CORPORATE AMC',500.00*-1,3
						from dp_acct_mstr
						,#account_properties left outer join #account_properties_close on accp_clisba_id = accp_clisba_id_cl
						where dpam_dpm_id = @pa_dpm_id and accp_clisba_id = dpam_id 
						--and   DPAM_STAM_CD not in('05','04','08','02_BILLSTOP','CI')
...
begin
		   ...
		   END
			   ...
			   ,post_toacct)
				...
				and   proc_deleted_ind = 1
				union all
				select @pa_billing_to_dt,dpam_id,'CORPORATE AMC',(500.00/(case when @@AMCPRORATA_INTERVAL = 'M' then 12 else 365 end))*isnull(@@months_to_charge,0)*-1,3
				from dp_acct_mstr
				,#account_properties  left outer join #account_properties_close on accp_clisba_id = accp_clisba_id_cl   
				where dpam_dpm_id = @pa_dpm_id and dpam_id = accp_clisba_id 
				--and   DPAM_STAM_CD not in('05','04','08','02_BILLSTOP','CI')
...
select @l_sert_posttoacct  = fina_acc_id from fin_account_mstr where fina_acc_code ='SERVICETAX'
					...
					,charge_val
					,post_toacct) 
					select @pa_billing_TO_dt, dpam_id , clic_charge_name 
					,Charge_amt*amount
					,posttoacct 
					from ( select dpam_id
