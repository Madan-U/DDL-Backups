-- Object: FUNCTION citrus_usr.FN_UCCMAP_DETAILS_Bak_14092022
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------


--arathi
CREATE  FUNCTION [citrus_usr].[FN_UCCMAP_DETAILS_Bak_14092022] (@PA_BOID VARCHAR(17),@PA_MSN VARCHAR(18))
RETURNS VARCHAR(800)
AS
BEGIN
...
--+ '<Rsn>' + convert(varchar,PLDTCM_REASON) + '</Rsn>'
 '<Rsn>' + isnull(case when convert(varchar(100),PLDTC_REASON_CODE)='Collateral -Debt issuance by Co./Grp Co.' then '1'
				 ...
				 when convert(varchar(100),PLDTC_REASON_CODE)='Personal use by promoters and PACs' then '5' else '' end,'')
+ '</Rsn>'
+ '<Poa>' + case when isnull(PLDTC_REASON_CODE,'')<>'' then isnull(convert(varchar(16),MasterPOAId),'') else ''  end + '</Poa>'

FROM cdsl_pledge_dtls ,dp_acct_mstr,dps8_pc5  WHERE pldtc_id=@PA_MSN
and PLDTC_DPAM_ID=dpam_id and DPAM_SBA_NO=boid and TypeOfTrans in ('','1')
END
ELSE
BEGIN
SELECT @L=
