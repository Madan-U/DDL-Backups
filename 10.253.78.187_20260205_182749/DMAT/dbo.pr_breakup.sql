-- Object: PROCEDURE dbo.pr_breakup
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------

create procedure [pr_breakup](@pa_from_dt datetime,@pa_to_dt datetime,@pa_login_name varchar(100))
as
begin 

TRUNCATE TABLE [citrus_usr].cdsl_holding_dtls_bak_forreve
insert into [citrus_usr].cdsl_holding_dtls_bak_forreve  
select * from [citrus_usr].CDSL_HOLDING_DTLS with(nolock) where CDSHM_TRAS_DT between @pa_from_dt and @pa_to_dt 
and (CDSHM_TRATM_CD ='2277'   or  CDSHM_CDAS_SUB_TRAS_TYPE in ('1002' ,'905','802'))---,'810'
--and CDSHM_BEN_ACCT_NO ='1203320000016304'

-- ADDED FOR WAIVE OF ON GOV SEC BY LATESH P W ON AUG 4 16
delete a FROM [citrus_usr].cdsl_holding_dtls_bak_forreve a , [citrus_usr].ISIN_MSTR
...
delete   FROM [citrus_usr].cdsl_holding_dtls_bak_forreve   
where  CDSHM_BEN_ACCT_NO in( '1203320019917044','1203320008188083')
and   cdshm_tras_dt between  @pa_from_dt and @pa_to_dt

delete   from [citrus_usr].cdsl_holding_dtls_bak_forreve  where CDSHM_TRAS_DT 
...
delete   from [citrus_usr].cdsl_holding_dtls_bak_forreve  where CDSHM_COUNTER_BOID = '1601480000243735'
...
from [citrus_usr].client_charges_cdsl with(nolock),[citrus_usr].dp_acct_mstr  with(nolock) ,[citrus_usr].client_dp_brkg  with(nolock) ,[citrus_usr].BROKERAGE_MSTR  with(nolock) 
...
from [citrus_usr].cdsl_holding_dtls  with(nolock) ,[citrus_usr].dp_acct_mstr  with(nolock) ,[citrus_usr].client_dp_brkg  with(nolock) ,[citrus_usr].BROKERAGE_MSTR   with(nolock) 
...
where exists(select 1 from [citrus_usr].client_charges_Cdsl , [citrus_usr].dp_Acct_mstr b where a.dpam_sba_no= b.dpam_sba_no 
...
FROM [citrus_usr].cdsl_holding_dtls_bak_forreve with (nolock)
where cdshm_tras_dt between @PA_from_DT and @PA_to_DT
...
--and (isdate(right(CDSHM_INT_REF_NO,4)+'-'+substring( CDSHM_INT_REF_NO,3,2) + '-' + left(CDSHM_INT_REF_NO,2) )=0 or CDSHM_INT_REF_NO like '%/%' ) 
----and not exists (select 1 from dps8_pc1 where boid = cdshm_ben_acct_no and replace(CONVERT(varchar(11),cdshm_tras_dt,103),'/','') = ClosAppDt)
and ((case when cdshm_ben_acct_no in (select dpam_sba_no from[citrus_usr].dp_acct_mstr where dpam_clicm_Cd ='26' and dpam_stam_cd ='active')
...
UNION ALL
select CDSHM_BEN_ACCT_NO,@l_amount  act_AMT,'Early Payin **' dsc from [citrus_usr].cdsl_holding_dtls_bak_forreve where cdshm_tratm_cd ='2277'
and cdshm_cdas_sub_tras_type ='409'
AND cdshm_tras_dt between @PA_from_DT and @PA_to_DT
and CITRUS_USR.fn_splitval_by(cdshm_trans_cdas_code,34,'~')='31'
and cdshm_isin not in ('INF732E01037' ,'IN0020120062') and CDSHM_BEN_ACCT_NO not in (SELECT dpam_sba_no FROM [citrus_usr].DP_ACCT_MSTR WHERE dpam_subcm_cd IN ( '102624','192624'))

UNION ALL 

select CDSHM_BEN_ACCT_NO  ,@l_amount  act_AMT ,'OnMkt Settlement **' dsc from [citrus_usr].cdsl_holding_dtls_bak_forreve
 where cdshm_tratm_cd ='2277'
and cdshm_cdas_sub_tras_type ='109'
AND CDSHM_BEN_ACCT_NO NOT IN (select dpam_sba_no from [citrus_usr].dp_acct_mstr where dpam_clicm_Cd ='26' and dpam_stam_cd ='active')
and cdshm_isin not in ('INF732E01037 ','IN0020120062')
UNION ALL 
SELECT CDSHM_BEN_ACCT_NO ,@l_amount  act_AMT ,'NSCCL **' dsc FROM [citrus_usr].cdsl_holding_dtls_bak_forreve 
WHERE CDSHM_TRATM_CD ='2277'
...
FROM [citrus_usr].cdsl_holding_dtls_bak_forreve WHERE CDSHM_TRATM_CD ='2277'
...
--and cdshm_isin not in ('INF732E01037','IN0020120062')
UNION ALL
SELECT CDSHM_BEN_ACCT_NO, '12' act_AMT ,'Pledge' dsc FROM [citrus_usr].cdsl_holding_dtls_bak_forreve WHERE CDSHM_CDAS_SUB_TRAS_TYPE in ('1002')
and cdshm_isin not in ('INF732E01037','IN0020120062')
UNION ALL
SELECT CDSHM_BEN_ACCT_NO, '12' act_AMT ,'Pledge' dsc FROM [citrus_usr].cdsl_holding_dtls_bak_forreve WHERE CDSHM_CDAS_SUB_TRAS_TYPE in ('905')
and cdshm_isin not in ('INF732E01037','IN0020120062')
UNION ALL
SELECT CDSHM_BEN_ACCT_NO, '12' act_AMT ,'Pledge' dsc FROM [citrus_usr].cdsl_holding_dtls_bak_forreve WHERE CDSHM_CDAS_SUB_TRAS_TYPE in ('802')
and cdshm_isin not in ('INF732E01037','IN0020120062')
) a 
...
from [citrus_usr].dp_acct_mstr pc1 with(nolock) left outer join #chargesummary summ on pc1.DPAM_SBA_NO = summ.dpam_sba_no 
,[citrus_usr].CLIENT_DP_BRKG with(nolock)
,[citrus_usr].BROKERAGE_MSTR brom with(nolock)
...
from #final f , [citrus_usr].dp_Acct_mstr dpam , [citrus_usr].dps8_pc1 pc1 where pc1.boid = dpam.dpam_sba_no 
...
from #final f , [citrus_usr].dp_Acct_mstr dpam , [citrus_usr].dps8_pc1 pc1 where pc1.boid = dpam.dpam_sba_no 
and dpam.dpam_sba_no = f.dpam_sba_no and amt + sgst + ugst + igst+cgst+ dp_charge <> 0
order by 1 
