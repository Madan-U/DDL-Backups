-- Object: PROCEDURE citrus_usr.pr_ucc_parameter_Insta
-- Server: 10.253.78.187 | DB: DMAT
--------------------------------------------------


CREATE  PROCEDURE [citrus_usr].[pr_ucc_parameter_Insta](@pa_from_dt       varchar(100)      
    ...
    --      
      ...
      FROM      
          (SELECT dpam.dpam_crn_no            dpam_crn_no      
           ...
           and    dpam.dpam_acct_no like 'W%'
           --and    citrus_usr.fn_ucc_entp(dpam_crn_no,'PAN_GIR_NO','CDSL') in (Select ISNULL(INSTA_PAN_GIR_NO,'') from INSTA_BOID_MSTR where INSTA_DELETED_IND=1)
           and ISNULL(citrus_usr.fn_ucc_accp(dpam_id,'INSTABOID','CDSL'),'')<>''
      ...
      FROM      
          (SELECT dpam.dpam_crn_no            dpam_crn_no      
           ...
           AND    dpam.dpam_excsm_id        = excsm.excsm_id  
           AND    dpam.dpam_deleted_ind     = 1    
           --and    dpam.dpam_acct_no not like 'W%' commneted for insta
           --and    citrus_usr.fn_ucc_entp(dpam_crn_no,'PAN_GIR_NO','CDSL') in (Select ISNULL(INSTA_PAN_GIR_NO,'') from INSTA_BOID_MSTR where INSTA_DELETED_IND=1)
           and ISNULL(citrus_usr.fn_ucc_accp(dpam_id,'INSTABOID','CDSL'),'')<>''
           ) x      
          , client_mstr                       clim      WITH (NOLOCK)    
