-- Object: PROCEDURE citrus_usr.pr_select_dp_paging
...
set @p9=NULL
exec pr_select_dp_paging @pa_startRowIndex=1,@pa_maximumRows=25,@pa_action='ENTITY',@pa_cd='BR',@pa_desc='',@pa_value='HO',@pa_code='HO',@pa_rmks='',@pa_ref_cur=@p9 output
select @p9
...
exec pr_select_dp_paging	1,25,'ENTITY','BR','','HO','HO','','dsentity','' 
exec pr_select_dp_paging	2,25,'ENTITY','BR','','HO','HO','','dsentity','' 
...
CREATE PROCEDURE [citrus_usr].[pr_select_dp_paging] (@pa_startRowIndex int,                                                      
                                    @pa_maximumRows int  ,                                                
...
,@l_count              totalrecord                        
   ...
   --                        
     ...
     AND isnull(banm_micr,'') like @pa_value +'%'  
     ...
     SELECT distinct banm.banm_name   banm_name                                                    
           ...
           , isnull(banm.banm_micr,'')                  banm_micr          
...
END  ---CLIENT SEARCH TAG ENDS HERE                            
 ...
 --                        
    ...
    --                        
      ...
      SET @@SSQL = @@SSQL + '  on acct_list.dpam_id = ACCBAL.ACCBAL_ACCT_ID and acct_list.acct_type = ACCBAL.ACCBAL_ACCT_TYPE 
      AND ACCBAL.ACCBAL_ACCT_TYPE IN (''B'',''C'')  '                        
      SET @@SSQL = @@SSQL + '  and ACCBAL.ACCBAL_DELETED_IND = 1 '                      
                
      SET @@SSQLFSTID= '  declare @first_id int,@second_id int,@total_rec bigint'                  
     SET @@SSQLFSTID= @@SSQLFSTID + '  DECLARE @tblbankacclist TABLE (TID int IDENTITY PRIMARY KEY,dpam_id numeric,dpam_crn_no numeric,dpam_sba_no varchar(100),dpam_sba_name varchar(100),acct_type varchar(5))'                  
      SET @@SSQLFSTID= @@SSQLFSTID + '  INSERT INTO @tblbankacclist 
       select dpam_id,dpam_crn_no,dpam_sba_no,dpam_sba_name,acct_type from  CITRUS_USR.[fn_gl_acct_list_bytushar_Gl_CL_Filter](' + CONVERT(VARCHAR,@l_dpmid ) + ',' + CONVERT(VARCHAR,@l_entityid) + ',0,''0'',''9999999999999999'',''F'')'             
    ...
    --                        
      ...
      DECLARE @TBLGLACCTLIST  TABLE (TID int identity primary key,dpam_id numeric,dpam_crn_no numeric,dpam_sba_no varchar(100),
      dpam_sba_name varchar(200),acct_type varchar(5))  
                        
                     declare @l_from numeric(38,0)
                     declare @l_to numeric(38,0)
                     select @l_from = case when @pa_code ='' or isnumeric(@pa_code ) = 0 then '0' else @pa_code end 
                     select @l_to  = case when @pa_code ='' or isnumeric(@pa_code ) = 0 then '9999999999999999' else @pa_code end 
              
              if  ISNUMERIC(@pa_code)=0    
      INSERT INTO @TBLGLACCTLIST                  
      SELECT dpam_id,dpam_crn_no,dpam_sba_no,dpam_sba_name,acct_type                   
      FROM  --CITRUS_USR.[fn_gl_acct_list_bytushar](@l_dpmid,@l_entityid,0,@l_from,@l_to)                  
      CITRUS_USR.[fn_gl_acct_list](@l_dpmid,@l_entityid,0)                  
