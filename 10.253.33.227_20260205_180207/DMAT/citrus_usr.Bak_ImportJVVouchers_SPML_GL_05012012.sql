-- Object: PROCEDURE citrus_usr.Bak_ImportJVVouchers_SPML_GL_05012012
...
--ImportJVVouchers_SPML_GL 1257,3,2,'may 01 2010','may 31 2010','HO'
--select* from ledger2 order by 1 desc
--ROLLBACK
--commit
create Proc [citrus_usr].[Bak_ImportJVVouchers_SPML_GL_05012012]
@pa_task_id bigint,
@pa_dpmid int,
@pa_finid int,
...
IF UPPER(@@voucher_dt) <> UPPER(convert(varchar(11),@pa_billing_to_dt,109))
BEGIN
--
...
Set @@ssql = 'insert into ledger' + convert(varchar,@pa_finid) + ' (LDG_ID,LDG_DPM_ID,LDG_VOUCHER_TYPE,LDG_BOOK_TYPE_CD,LDG_VOUCHER_NO,LDG_SR_NO,LDG_REF_NO,LDG_VOUCHER_DT,LDG_ACCOUNT_ID,LDG_ACCOUNT_TYPE,LDG_AMOUNT,LDG_NARRATION,LDG_BANK_ID,LDG_ACCOUNT_NO,LDG_INSTRUMENT_NO,LDG_BANK_CL_DATE,LDG_COST_CD_ID,LDG_BILL_BRKUP_ID,LDG_TRANS_TYPE,LDG_STATUS,LDG_CREATED_BY,LDG_CREATED_DT,LDG_LST_UPD_BY,LDG_LST_UPD_DT,LDG_DELETED_IND,LDG_BRANCH_ID)
select LDG_ID=' + convert(varchar,isnull(@@lst_curr_id,0)) + ' + TMPJV_SR_NO,LDG_DPM_ID=' + convert(varchar,@pa_dpmid) + ',LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''01'',LDG_VOUCHER_NO=' + convert(varchar,@@Vno) + ',TMPJV_SR_NO,LDG_REF_NO=''' + @@ref_no + ''',''' + @@voucher_dt + ''',TMPJV_DPAM_ID,LDG_ACCOUNT_TYPE=''P'',CASE WHEN TMPJV_CREDIT <> 0 THEN TMPJV_CREDIT * -1 ELSE TMPJV_DEBIT END  ,LDG_NARRATION=TMPJV_NARRATION,LDG_BANK_ID='''',LDG_ACCOUNT_NO='''',LDG_INSTRUMENT_NO=TMPJV_FILLER1,LDG_BANK_CL_DATE='''',LDG_COST_CD_ID=0,LDG_BILL_BRKUP_ID=0,LDG_TRANS_TYPE=''AC'',LDG_STATUS=''P'',''' + @pa_loginname + ''',getdate(),''' + @pa_loginname + ''',getdate(),1,0 
