-- Object: PROCEDURE citrus_usr.pr_import_client_cdsl_DDD
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--SELECT * FROM DP_ACCT_MSTR WHERE DPAM_SBA_NO = '1203270000125473'
--
--SELECT * FROM ACCOUNT_PROPERTIES WHERE ACCP_ACCPM_PROP_CD LIKE '%ACC_CLOSE_DT%' AND ACCP_ACCT_NO = '1203270000125473'



--SELECT CONVERT(VARCHAR(11),'04-OCT-2010',105)
...
CREATE PROCEDURE  [citrus_usr].[pr_import_client_cdsl_DDD]
			(
			 @pa_exch          VARCHAR(20)    
...
--			 
--             
--           UPDATE accp SET accp_VALUE = CONVERT(VARCHAR(11),CONVERT(DATETIME,TMPCLI_ACCT_CLO_SUSP_DT),103) FROM   
--           account_properties accp   
--         , DP_ACCT_MSTR   
--         , TMP_CLIENT_DTLS_MSTR_CDSL   
--           WHERE DPAM_id = accp_clisba_id  
...
--  
-- --+'|*~|'+ltrim(rtrim(poam_master_id))+'|*~|'+'0'+'|*~|'+'|*~|'+'A*|~*' 
...
--                          --select @l_compm_id = excsm_compm_id from exch_seg_mstr where excsm_id = @l_excsm_id   
--          select @l_dp_acct_values = isnull(convert(varchar,@l_compm_id),'')+'|*~|'+ isnull(convert(varchar,@l_excsm_id),'')+'|*~|'+isnull(convert(varchar,@l_dpm_dpid),'')+'|*~|'+isnull(@c_ben_acct_no,'')+'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_NM)),'')+'|*~|'+ ISNULL(ltrim(rtrim(TMPCLI_DP_INTREF_NO)),'') +'|*~|'+case when TMPCLI_ACCT_STAT = '01' or TMPCLI_ACCT_STAT = 'ACTIVE' then 'ACTIVE' when TMPCLI_ACCT_STAT = 'CLOSED' then '05' else TMPCLI_ACCT_STAT end+'|*~|'+citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(TMPCLI_ACCT_CTRGY,''),'enttm') + '|*~|' + citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(TMPCLI_BO_STAT,''),'clicm')+'|*~|'+citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(ltrim(rtrim(TMPCLI_BO_SUB_STAT)),''),'subcm')+'|*~|0|*~|A|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL WHERE  tmpcli_boid = @c_ben_acct_no   
--             
--             PRINT 'AKHIL AAA '+ @l_dp_acct_values
