-- Object: PROCEDURE citrus_usr.pr_rpt_trans_count_mod
...
CREATE procedure [citrus_usr].[pr_rpt_trans_count_mod](
 @pa_excsm_id int
,@pa_tab varchar(10)
...
--               and dpam_deleted_ind = 1 
			...
			select clt_cnt = count(clic_mod_dpam_sba_no)
			from client_list_modified
			where clic_mod_deleted_ind = 1
			and isnull(clic_mod_batch_no,'0') = 0
			and convert(varchar(11),clic_mod_from_dt) between @pa_request_dt and @pa_request_todt 
			and convert(varchar(11),clic_mod_to_dt) between @pa_request_dt and @pa_request_todt             
		  end 
	  else 
          begin  
				if @pa_tab = 'N'  
				...
				where dptdc_dpam_id = dpam_id and dpam_dpm_id = @dpm_id  
                ...
                rmt_cnt = 0 , pldt_cnt = 0 from demat_request_mstr 
				...
				OFM_cnt = 0,  				
				DMT_cnt = 0, 
                rmt_cnt = count(Remrm_id), pldt_cnt = 0 from remat_request_mstr 
      ...
      and   dptdc_request_dt BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_request_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_request_todt,103),106)+' 23:59:59'      
				...
				ID_cnt = 0,  
				OFM_cnt = 0,  				
				DMT_cnt = count(demrm_id), rmt_cnt = 0 , pldt_cnt = 0 from demat_request_mstr 
