-- Object: PROCEDURE citrus_usr.PR_SELECT_TRXGEN_CDSL_SP_XML_harmonization
...
CREATE  PROCEDURE [citrus_usr].[PR_SELECT_TRXGEN_CDSL_SP_XML_harmonization]  
(  
...
set @PA_TO_DT_Miss = CONVERT(varchar(11),convert(datetime,@PA_FROM_DT,103),109)
...
WHEN cd = 'REMAT' THEN  '<Tp>'+ case when ltrim(rtrim(substring(details , 20 , 12))) like 'INF%' then '28' else ltrim(rtrim(substring(details , 1 , 2))) end+'</Tp>'
+'<Flg>'+ case when ltrim(rtrim(substring(details , 20 , 12))) like 'INF%' then 'S' else ltrim(rtrim(substring(details , 3 , 1))) end +'</Flg>'
+'<Bnfcry>'+ ltrim(rtrim(substring(details , 4 , 16))) +'</Bnfcry>'
+'<ISIN>'+ ltrim(rtrim(substring(details , 20 , 12))) +'</ISIN>'
+'<Ref>'+ ltrim(rtrim(substring(details , 32 , 16))) +'</Ref>'
+ isnull(case when ltrim(rtrim(substring(details , 20 , 12))) like 'INF%' and ltrim(rtrim(convert(varchar,convert(numeric(18,3),qty)))) <> '0.000'  then '<QtyFlg>'+ 'P' +'</QtyFlg>'  when ltrim(rtrim(substring(details , 20 , 12))) like 'INF%' and ltrim(rtrim(convert(varchar,convert(numeric(18,3),qty)))) = '0.000'  then '<QtyFlg>'+ 'A' +'</QtyFlg>'  END ,'' )
+'<Qty>'+ case when ltrim(rtrim(substring(details , 20 , 12))) like 'INF%' then  ltrim(rtrim(convert(varchar,convert(numeric(18,3),qty)))) else  REPLACE(ltrim(rtrim(convert(varchar,convert(numeric,qty)))),'.000','')  end +'</Qty>'
--+ isnull(case when ltrim(rtrim(substring(details , 20 , 12))) like 'INE%' then '<Lot>'+  ltrim(rtrim(substring(details , 64 , 1)))  +'</Lot>'   end,'')
+ isnull(case when ltrim(rtrim(substring(details , 20 , 12))) like 'INE%' then '<Lot>'+  ltrim(rtrim(substring(details , 64 , 1)))  +'</Lot>' when  ltrim(rtrim(substring(details , 20 , 12))) like 'IN9%'  then  '<Lot>'+  ltrim(rtrim(substring(details , 64 , 1)))  +'</Lot>'   end,'')
+'<Lcksts>'+ ltrim(rtrim(substring(details , 65 , 1))) +'</Lcksts>'
+'<Lckid>'+ ltrim(rtrim(substring(details , 66 , 16))) +'</Lckid>'
+ isnull(case when ltrim(rtrim(substring(details , 20 , 12))) like 'INE%' then  '<Cert>'+  ltrim(rtrim(convert(varchar,convert(numeric(18,3),substring(details , 82 , 8)))))   +'</Cert>'    when ltrim(rtrim(substring(details , 20 , 12))) like 'IN9%' then  '<Cert>'+  ltrim(rtrim(convert(varchar,convert(numeric(18,3),substring(details , 82 , 8)))))   +'</Cert>' end,'')
+'<Remk>'+ ltrim(rtrim(substring(details , 93 , 40))) +'</Remk>'
+isnull(case when ltrim(rtrim(substring(details , 20 , 12))) like 'INF%' then '<Mftype>'+ case when (ltrim(rtrim(substring(details , 20 , 12))) like 'INF%' and isnull(TRXEFLG,'')='') then 'R' when (ltrim(rtrim(substring(details , 20 , 12))) like 'INF%' and isnull(TRXEFLG,'')='Y') then 'S' else 'S' end  +'</Mftype>'END,'' )
--+'<Amt>'+   +'</Mftype>' --in case of M in qtyflg
+'<Rcvdt>' + request_dt + '</Rcvdt>'
WHEN cd = 'TNT' THEN  + case when ltrim(rtrim(substring(details , 69 , 1)))='1' then '<Tp>'+ '30' +'</Tp>'   else '<Tp>'+ '31' +'</Tp>' end
+ case when ltrim(rtrim(substring(details , 69 , 1)))='1' then '<Idntfr>'+ 'O' +'</Idntfr>'   else '<Idntfr>'+ 'A' +'</Idntfr>' end
+'<Ctgry>'+ case when TRXEFLG='4' then 'D' else 'N' end  +'</Ctgry>'
+'<Bnfcry>'+ ltrim(rtrim(substring(details , 9 , 16))) +'</Bnfcry>'
+'<Ref>'+ ltrim(rtrim(substring(details , 71 , 16))) +'</Ref>'
+'<Rsn>'+ ltrim(rtrim(substring(details , 70 , 1))) +'</Rsn>'
+'<ISIN>'+ +'</ISIN>'
+'<Qty>'+  +'</Qty>'
+'<Remk>'+ ltrim(rtrim(substring(details , 68 , 1))) +'</Remk>'
+'<CntBo></CntBo>'
+'<Tran>'
+'<Clnt>'+ case when TRXEFLG='4' then '' else  ltrim(rtrim(right(ltrim(rtrim(substring(details , 25 , 16))),8))) end +'</Clnt>'
+'<Brkr>' + case when TRXEFLG='4' then left(ltrim(rtrim(substring(details , 25 , 16))),16) else  left(ltrim(rtrim(substring(details , 25 , 16))),8) end   + '</Brkr>'
+'<Prtqty></Prtqty>'
+'</Tran>'
+'<Rcvdt>' + ltrim(rtrim(substring(details , 1 , 8))) + '</Rcvdt>'


WHEN cd = 'FRZ' THEN case when qty<>0.0000 then '<Tp>'+ '12' +'</Tp>'
+'<Usn>'+ convert(varchar,usn) +'</Usn>'
+'<Frztp>'+ ltrim(rtrim(substring(details , 1 , 1))) +'</Frztp>'
+'<Lvl>'+ ltrim(rtrim(substring(details , 10 , 1))) +'</Lvl>'
+'<Intby>'+ ltrim(rtrim(substring(details , 11 , 1))) +'</Intby>'
+'<Subopt>'+ ltrim(rtrim(substring(details , 12 , 1))) +'</Subopt>'
+'<Bnfcry>'+ ltrim(rtrim(substring(details , 13 , 16))) +'</Bnfcry>'
+'<Qtytype>'+ ltrim(rtrim(substring(details , 42 , 1))) +'</Qtytype>'
+'<Qty>'+ ltrim(rtrim(convert(varchar,convert(numeric, qty)))) +'</Qty>'
+'<Frozfor>'+ ltrim(rtrim(substring(details , 58 , 1))) +'</Frozfor>'
+'<Actvtp>'+ ltrim(rtrim(substring(details , 59 , 1))) +'</Actvtp>'
+'<Actvdt>'+ CASE WHEN ltrim(rtrim(substring(details , 68 , 8)))in ('00000000','01011900') THEN '' ELSE ltrim(rtrim(substring(details , 60 , 8))) end +'</Actvdt>'
+'<Expdt>'+ CASE WHEN ltrim(rtrim(substring(details , 68 , 8))) in ('00000000','01011900') THEN '' ELSE ltrim(rtrim(substring(details , 68 , 8))) END +'</Expdt>'
+'<Rsn>'+ ltrim(rtrim(substring(details , 76 , 2))) +'</Rsn>'
+'<Ref>'+ ltrim(rtrim(substring(details , 78 , 16))) +'</Ref>'
+'<Remk>'+ ltrim(rtrim(substring(details , 94 , 100))) +'</Remk>'
+'<Rcvdt>' + request_dt + '</Rcvdt>'
else
 '<Tp>'+ '12' +'</Tp>'
 +'<Usn>'+ convert(varchar,usn) +'</Usn>'
...
WHEN cd = 'UNFRZ' THEN'<Tp>'+ '12' +'</Tp>'
+'<Usn>'+ convert(varchar,usn) +'</Usn>'
+'<Frztp>'+ ltrim(rtrim(substring(details , 1 , 1))) +'</Frztp>'
+'<Frzid>'+ ltrim(rtrim(convert(varchar,convert(numeric, qty)))) +'</Frzid>'
+'<Remk>'+ ltrim(rtrim(substring(details , 94 , 100))) +'</Remk>'
+'<Rcvdt>' + request_dt + '</Rcvdt>'

else details end as details , qty,dptdc_slip_no  slip_no , dptdc_id as Intdptdc_id
from #tempdata where cd not in ('DMAT')
--changes for NewDemat changes Feb 14 2020
union all
select  convert(varchar(8000),rngs)+convert(varchar,folio)+convert(varchar,certfrom)+convert(varchar,certto)+convert(varchar,DNfrm)+convert(varchar,dnto) ,'0',isnull(drfno,0) , 0 Intdptdc_id  from #Demattempdatarange -- isnull(convert(numeric,drfno),0)
--changes for NewDemat changes Feb 14 2020
)
T
--changes for NewDemat changes Feb 14 2020
...
'' + Case when len(@PA_BATCH_NO)=7 then '0'+@PA_BATCH_NO when len(@PA_BATCH_NO)=6 then '00'+@PA_BATCH_NO when len(@PA_BATCH_NO)=5 then '000'+@PA_BATCH_NO  when len(@PA_BATCH_NO)=4 then '0000'+@PA_BATCH_NO  when len(@PA_BATCH_NO)=3 then '00000'+@PA_BATCH_NO else @PA_BATCH_NO end 

+ ',' + @PA_LOGINNAME + ',' + '0' + ',' + ',' + 
case when isnull(REMRM_REPURCHASE_FLG,'') ='Y' and isnull(REMRM_ISIN,'') like  'INF%' then 'REP'  
when isnull(REMRM_REPURCHASE_FLG,'') =''  and isnull(REMRM_ISIN,'') like 'INF%' then 'RES' else 'RMT' end   
+ ',' + convert(varchar,ID) + ',' 
+ isnull(convert(varchar(100),ltrim(rtrim(xmlField.value('(Ref)[1]', 'nvarchar(max)')))),'') + ',' + '033200'  + ',' 
+ convert(varchar(100),xmlField.value('(Bnfcry)[1]', 'nvarchar(max)'))  + ',' 
+convert(varchar(100),xmlField.value('(ISIN)[1]', 'nvarchar(max)')) 
+','
+convert(varchar(100),xmlField.value('(Qty)[1]', 'nvarchar(max)'))-- + ',' 
+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','+','	
...
+','	+','	+','	+','	+','	+','	+','	+','	+','
--+isnull(convert(varchar(100),xmlField.value('(QtyFlg)[1]', 'nvarchar(max)')),'')	+','	
+
isnull((SELECT top 1 ltrim(rtrim(Standard_Value)) FROM standard_value_list WHERE iso_tags ='AllUnitsOrAmtInd' AND ISNULL(CDSL_Old_Values,'')<>'' 
and CDSL_Old_Values=isnull(convert(varchar(100),xmlField.value('(QtyFlg)[1]', 'nvarchar(max)')),'')),'')		+','	 
+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','
+convert(varchar(100),xmlField.value('(Ref)[1]', 'nvarchar(max)'))	+','	
+','	+','
--+convert(varchar(100),xmlField.value('(Lcksts)[1]', 'nvarchar(max)'))	+','	
+case when  isnull(ltrim(rtrim(convert(varchar(100),xmlField.value('(Lcksts)[1]', 'nvarchar(max)')))),'') = 'L' then 'LCK' else 'FRE' end +','	
+convert(varchar(100),xmlField.value('(Lckid)[1]', 'nvarchar(max)')) +','	
+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','
+CASE WHEN isnull(convert(varchar(100),xmlField.value('(Flg)[1]', 'nvarchar(max)')),'') = 'R' THEN 'REM' ELSE 'RES' END	+','
+isnull(convert(varchar(100),xmlField.value('(Ref)[1]', 'nvarchar(max)')),'')+','
+case when isnull(REMRM_REPURCHASE_FLG,'') ='Y'  then 'RED' else 'RES' end 	+','	
+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	+','	
--+CASE WHEN isnull(convert(varchar(100),xmlField.value('(Flg)[1]', 'nvarchar(max)')),'') = 'R' then '0,' else ',' end 	+
+replace(isnull(convert(varchar(100),xmlField.value('(Cert)[1]', 'nvarchar(max)')),''),'.000','')	+','+
','	+','	+','	+',' +','	+','
else '' end 
AS Detailshar 
,slip_no,ID
...
FROM tmp 
,Remrm_mak where 
REMRM_SLIP_SERIAL_NO=isnull(convert(varchar(100),ltrim(rtrim(xmlField.value('(Ref)[1]', 'nvarchar(max)')))),'')--citrus_usr.FN_SPLITVAL_BY(ltrim(rtrim(convert(varchar,xmlField.value('(Ref)[1]', 'nvarchar(max)')))),2,'/')
...
case when @PA_TRX_TAB='DESTAT*|~*' then (Select top 1 convert(varchar,count(1)) from demat_request_dtls where demrd_deleted_ind=1 and demrd_demrm_id=demrm_id)
when (select count (1)  from isin_mstr where isin_cfi_cd <> '0' and isin_cd = demrm_isin ) = 1 then CONVERT(VARCHAR,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0'))
else case when isnull(citrus_usr.FN_ACTINACTISIN(DEMRM_ISIN),'')='Y'
then case when isnull(ltrim(rtrim(convert(varchar,xmlField.value('(Ranges)[1]', 'nvarchar(max)')))),'') = '' then '0' 
else   CONVERT(VARCHAR,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0')) end  
else CONVERT(VARCHAR,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0')) end end
+','+ isnull(ltrim(rtrim(convert(varchar,xmlField.value('(Ranges)[1]', 'nvarchar(max)')))),'') + ','+','+
+','+ case when @PA_TRX_TAB='DESTAT*|~*' then (Select top 1 convert(varchar(100),DEMRD_FOLIO_NO) from demat_request_dtls where demrd_deleted_ind=1 and demrd_demrm_id=demrm_id) else '' end 
+ ','+','+','+','+  + ','+ case when @PA_TRX_TAB='DESTAT*|~*' then '' else  SUBSTRING(isnull(convert(varchar,xmlField.value('(Dspchdt)[1]', 'nvarchar(max)')),''), 5, 4) + '-' + SUBSTRING(isnull(convert(varchar,xmlField.value('(Dspchdt)[1]', 'nvarchar(max)')),''), 3, 2) + '-' + SUBSTRING(isnull(convert(varchar,xmlField.value('(Dspchdt)[1]', 'nvarchar(max)')),''), 1, 2) end + ','+',' +','+  ','+case when  @PA_TRX_TAB='DESTAT*|~*' then '' else 'AC' end +','+ +','+ ','+','
+isnull((SELECT top 1 ltrim(rtrim(Standard_Value)) FROM standard_value_list WHERE iso_tags ='LckInRsnCd' AND ISNULL(CDSL_Old_Values,'')<>'' and CDSL_Old_Values=isnull(convert(varchar,xmlField.value('(Lckcd)[1]', 'nvarchar(max)')),'')),'')	+',' + ',' + case when isnull(convert(varchar,xmlField.value('(Lckexpdt)[1]', 'nvarchar(max)')),'') <> '' then SUBSTRING(isnull(convert(varchar,xmlField.value('(Lckexpdt)[1]', 'nvarchar(max)')),''), 5, 4)  + '-' + SUBSTRING(isnull(convert(varchar,xmlField.value('(Lckexpdt)[1]', 'nvarchar(max)')),''), 3, 2) + '-' + SUBSTRING(isnull(convert(varchar,xmlField.value('(Lckexpdt)[1]', 'nvarchar(max)')),''), 1, 2) else '' end  + ','+convert(varchar(100),demrm_id)+','+ ','+ ','
+ case when  isnull(ltrim(rtrim(convert(varchar,xmlField.value('(Lcksts)[1]', 'nvarchar(max)')))),'') = 'L' then 'LCK' else 'FRE' end  +','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+ ',' +','+','+','++','+','+ ',' +'' + ','+','+','+','++','+','+','+','+','+ ','+','+','+','+','+','+','+','+','+','+','+',' +',' 
...
as Detailshar,0 id
union all
Select Detailshar,convert(numeric,id)  from #TmpRmat
 order by 2
end
else if @PA_TRX_TAB = 'FRZ*|~*' 
begin 


select convert(varchar,xmlField.value('(Tp)[1]', 'nvarchar(max)')) Tp ,
convert(varchar,xmlField.value('(Usn)[1]', 'nvarchar(max)')) Usn ,
convert(varchar,xmlField.value('(Frztp)[1]', 'nvarchar(max)')) Frztp ,
convert(varchar,xmlField.value('(Lvl)[1]', 'nvarchar(max)')) Lvl,
convert(varchar,xmlField.value('(Intby)[1]', 'nvarchar(max)')) Intby ,
convert(varchar,xmlField.value('(Subopt)[1]', 'nvarchar(max)')) Subopt ,
convert(varchar,xmlField.value('(Bnfcry)[1]', 'nvarchar(max)')) Bnfcry ,
convert(varchar,xmlField.value('(Frozfor)[1]', 'nvarchar(max)')) Frozfor ,
convert(varchar,xmlField.value('(Actvtp)[1]', 'nvarchar(max)')) Actvtp ,
convert(varchar,xmlField.value('(Actvdt)[1]', 'nvarchar(max)')) Actvdt ,
convert(varchar,xmlField.value('(Expdt)[1]', 'nvarchar(max)')) Expdt ,
convert(varchar,xmlField.value('(Rsn)[1]', 'nvarchar(max)')) Rsn ,
convert(varchar,xmlField.value('(Ref)[1]', 'nvarchar(max)')) Ref ,
convert(varchar,xmlField.value('(Remk)[1]', 'nvarchar(max)')) Remk ,
convert(varchar,xmlField.value('(Rcvdt)[1]', 'nvarchar(max)')) Rcvdt 
,ID	,qty,	slip_no,	Intdptdc_id
into  #tmp_frz from tmp
...
as Detailshar
union all 
select Case when len(@PA_BATCH_NO)=7 then '0'+@PA_BATCH_NO when len(@PA_BATCH_NO)=6 then '00'+@PA_BATCH_NO when len(@PA_BATCH_NO)=5 then '000'+@PA_BATCH_NO  when len(@PA_BATCH_NO)=4 then '0000'+@PA_BATCH_NO  when len(@PA_BATCH_NO)=3 then '00000'+@PA_BATCH_NO else @PA_BATCH_NO end + ',' + @PA_LOGINNAME + ',' + substring(Bnfcry,3,6) + ',' + ',' + isnull((SELECT ltrim(rtrim(Standard_Value)) FROM HARM_SOURCE_CDSL WHERE FIELD_DESCRIPTION='Transaction Type Upload' AND ISNULL(CDSL_OLD_VALUE,'')<>'' and CDSL_OLD_VALUE=tp),'')	+ + ',' + isnull(convert(varchar,ID),'') + ',' + convert(varchar(100),usn) +','+convert(varchar(100),substring(bnfcry,3,6)) +','+convert(varchar(100),bnfcry) 
+','+''+
','+''+
','+''+
','+''+
...
','+''+
','+''+
','+citrus_usr.fn_get_standard_value_harm ('FrzRsnCd',Rsn)+
','+''+
','+''+
','+''+
','+''+
...
','+''+
','+''+
','+CONVERT(VARCHAR(100),Intdptdc_id) +
','+''+
','+''+
','+''+
','+''+
...
','+''+
','+''+
','+citrus_usr.fn_get_standard_value_harm ('FrzTp',Frztp)+
','+ case when Lvl ='B' AND Frozfor ='1' THEN 'BFD'
 when Lvl ='I' AND Frozfor ='1' THEN 'BID'
 when Lvl ='B' AND Frozfor ='2' THEN 'BFC'
 when Lvl ='I' AND Frozfor ='2' THEN 'BIC'
 when Lvl ='B' AND Frozfor ='3' THEN 'BFDC'
 when Lvl ='I' AND Frozfor ='3' THEN 'BIDC' ELSE '' END +
','+ case when Intby ='3' THEN 'DP'
 when Intby ='1'  THEN 'BO'
 when Intby ='2'  THEN 'CD'
 ELSE '' END +
','+citrus_usr.fn_get_standard_value_harm ('FrzSubOptn',Subopt)+
','+ citrus_usr.fn_get_standard_value_harm ('FrznFor',Frozfor)+
','+citrus_usr.fn_get_standard_value_harm ('FrzActvtnTp',Actvtp)+
','+CASE WHEN Actvdt <> '' THEN RIGHT(Actvdt,4)+'-'+SUBSTRING(Actvdt,3,2)+'-'+LEFT(Actvdt,2) ELSE '' END +
','+''+--CASE WHEN EXPDT <> '' THEN RIGHT(EXPDT,4)+'-'+SUBSTRING(EXPDT,3,2)+'-'+LEFT(EXPDT,2) ELSE '' END
','+ ISNULL(REMK,'') +
','+''+
','+'0'+
','+''+
...
from #tmp_frz 
--union all
--Select Detailshar from #TmpDmat
end
else
begin
...
Select distinct
	...
	AND pldtc_trastm_cd IN ('CRTE') and @PA_TRX_TAB<>'DESTAT*|~*'
	union all  -- margin pledge
	select distinct '' 
	+ Case when len(@PA_BATCH_NO)=7 then '0'+@PA_BATCH_NO when len(@PA_BATCH_NO)=6 then '00'+@PA_BATCH_NO when len(@PA_BATCH_NO)=5 then '000'+@PA_BATCH_NO  when len(@PA_BATCH_NO)=4 then '0000'+@PA_BATCH_NO  when len(@PA_BATCH_NO)=3 then '00000'+@PA_BATCH_NO else @PA_BATCH_NO end + ',' + @PA_LOGINNAME + ',' + substring(DPM_DPID,3,5) + ',' + ',' + isnull((SELECT TOP 1 ltrim(rtrim(Standard_Value)) FROM HARM_SOURCE_CDSL WHERE FIELD_DESCRIPTION='Transaction Type Upload' AND ISNULL(CDSL_OLD_VALUE,'')<>'' and CDSL_OLD_VALUE=convert(varchar,xmlField.value('(Tp)[1]', 'nvarchar(max)'))),'')	  + ',' + isnull(convert(varchar,ID),'') + ',' + ltrim(rtrim(xmlField.value('(Usn)[1]', 'nvarchar(max)'))) + ',' + '033200' + ',' + isnull(convert(varchar,xmlField.value('(Bnfcry)[1]', 'nvarchar(max)')) + ',' + convert(varchar,xmlField.value('(Isin)[1]', 'nvarchar(max)')),'') + ',' + isnull(convert(varchar,QTY),'') + ' ,S,' + isnull(case when isnull(convert(varchar,xmlField.value('(Clnt)[1]', 'nvarchar(max)')),'')='' then ''  else case when len(convert(varchar,xmlField.value('(Clnt)[1]', 'nvarchar(max)')))=16 then left(convert(varchar,xmlField.value('(Clnt)[1]', 'nvarchar(max)')),8) else  convert(varchar,xmlField.value('(Clnt)[1]', 'nvarchar(max)')) end end ,'')+ ',' + isnull(case when isnull(convert(varchar,xmlField.value('(CtrPty)[1]', 'nvarchar(max)')),'')='' then '' else case when len(convert(varchar,xmlField.value('(CtrPty)[1]', 'nvarchar(max)')))=16 then  right(convert(varchar,xmlField.value('(CtrPty)[1]', 'nvarchar(max)')),16) else convert(varchar,xmlField.value('(CtrPty)[1]', 'nvarchar(max)')) end end,'')+ ',' + isnull(convert(varchar,xmlField.value('(Clnt)[1]', 'nvarchar(max)')),'') + ','+','+',' + isnull(left(convert(varchar,isnull(xmlField.value('(Sttlm)[1]', 'nvarchar(max)'),'')),6),'') + ',' + isnull(right(convert(varchar,isnull(xmlField.value('(Sttlm)[1]', 'nvarchar(max)'),'')),7),'') + ',' + isnull(left(convert(varchar,isnull(xmlField.value('(Sttlm)[1]', 'nvarchar(max)'),'')),6),'') + ',' + isnull(right(convert(varchar,isnull(xmlField.value('(Sttlm)[1]', 'nvarchar(max)'),'')),7),'') + ',' + substring(xmlField.value('(Rcvdt)[1]', 'nvarchar(max)'),5,4) + '-' +  substring(xmlField.value('(Rcvdt)[1]', 'nvarchar(max)'),3,2)+ '-' + substring(xmlField.value('(Rcvdt)[1]', 'nvarchar(max)'),1,2) + ',' + '' +  + ','+ ISNULL((SELECT TOP 1 Standard_Value FROM HARM_SOURCE_CDSL WHERE FIELD_DESCRIPTION like '%pLEDGE Reason Code%' and meaning=ISNULL(convert(varchar(1000),PLDTCM_REASON_CODE),'0')),'') + ',,' + case when convert(varchar,xmlField.value('(Txnelflg)[1]', 'nvarchar(max)'))='N' then 'PHY' else 'ESP' end + ',' + isnull(ltrim(rtrim(convert(varchar,xmlField.value('(Prf)[1]', 'nvarchar(max)')))),'')  + ',' + case when (SELECT TOP 1 Standard_Value FROM HARM_SOURCE_CDSL WHERE FIELD_DESCRIPTION='TRANSACTION TYPE upload' AND ISNULL(CDSL_OLD_VALUE,'')<>'' and CDSL_OLD_VALUE=convert(varchar,xmlField.value('(Tp)[1]', 'nvarchar(max)')))='4' then '' else convert(varchar,isnull(xmlField.value('(Poa)[1]', 'nvarchar(max)'),'')) end + ','+ +','+','+isnull(pldtcm_ucc,'') +','+ isnull(pldtcm_cmid,'')  +',' + isnull(pldtcm_tmcmid,'')+','+ ',' + 'CM' + ','+ 'NCL' + ','+ 'NSE' + ',' + PLDTCM_CREATED_BY  + ',' + PLDTCM_UPDATED_BY + ',' + '' + ','+ CONVERT(VARCHAR(23),convert(datetime,substring(xmlField.value('(Rcvdt)[1]', 'nvarchar(max)'),1,2) + '-' +  substring(xmlField.value('(Rcvdt)[1]', 'nvarchar(max)'),3,2)+ '-' + substring(xmlField.value('(Rcvdt)[1]', 'nvarchar(max)'),5,4),105),126) + ','+ ',' + +','+ +','+ +','+''+','+ +','+   +','+','+ isnull(PLDTCM_AGREEMENT_NO,'') + ','+','+','+','+  + ','+','+ isnull(convert(varchar(10),pldtcM_expiry_dt,120),'') + ','+','+ +','+','+','+','+','+','+','+ '' +','+','+','+','+','+'BO'+','+' ,'+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+' ,'+','+convert(varchar,ID)+','+','+','+',' + ',' + ','+ltrim(rtrim(convert(varchar,xmlField.value('(Ref)[1]', 'nvarchar(max)'))))+','+ +','+ 'STP' +','+ 'FRE' +','+ '' +','+ isnull(ltrim(rtrim(convert(varchar,xmlField.value('(Prf)[1]', 'nvarchar(max)')))),'') +  ',' + convert(varchar,isnull(convert(numeric(18,2),QTY*CLOPM_CDSL_RT),'0')) +','+ ltrim(rtrim(convert(varchar,xmlField.value('(Ref)[1]', 'nvarchar(max)')))) + ','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+','+ ',' +','+','+','+'HP'+','+','+ ',' +'' + ','+','+','+','+'DP'+','+','+','+','+','+convert(varchar,isnull(xmlField.value('(Clnt)[1]', 'nvarchar(max)'),''))+','+','+','+','+','+ '' +','+','+','+','+','+','+','+','
	as Detailshar ,ID

	FROM tmp,DP_ACCT_MSTR
	--left outer join DP_POA_DTLS on DPPD_DPAM_ID=dpam_id and DPPD_DELETED_IND=1 and getdate() between dppd_eff_fr_dt and isnull(dppd_eff_TO_dt,'2099-12-31 00:00:00.000')
	,DP_MSTR,cdsl_marginpledge_dtls 
			 LEFT OUTER JOIN ISIN_MSTR with (nolock) ON PLDTCM_ISIN = ISIN_CD              
			 LEFT OUTER JOIN CLOSING_PRICE_MSTR_cDSL with (nolock) ON PLDTCM_ISIN = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = PLDTCM_ISIN and CLOPM_DT <= PLDTCM_EXEC_DT and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc)                      

	where DPAM_DELETED_IND=1 
	and DPAM_SBA_NO=convert(varchar,xmlField.value('(Bnfcry)[1]', 'nvarchar(max)')) 
	AND DPAM_DPM_ID=DPM_ID AND default_dp=dpm_excsm_id
	--and PLDTC_SLIP_NO=convert(varchar,slip_no)--citrus_usr.FN_SPLITVAL_BY(ltrim(rtrim(convert(varchar,xmlField.value('(Ref)[1]', 'nvarchar(max)')))),2,'/')
	and Intdptdc_id=pldtcm_id
	and pldtcm_DELETED_IND=1
	AND pldtcm_trastm_cd IN ('CRTEMP') 
	)
	T
	order by T.ID
