-- Object: PROCEDURE citrus_usr.pr_get_ob_fix_newlogic
...
--exec [pr_get_ob_fix_newlogic] 3,'mar 10 2014','mar 10 2014','0','9999999999999999',''     
CREATE proc [citrus_usr].[pr_get_ob_fix_newlogic](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                          
...
if not exists (select top 1 dphmc_holding_dt from dp_hldg_mstr_cdsl where dphmc_dpm_id = @pa_dpm_id and dphmc_holding_dt < @pa_from_dt)
begin 
select @l_base_hldg_dt = max(dphmcd_holding_dt) from vw_holding_base_all
end 
else 
begin 
select @l_base_hldg_dt = max(dphmc_holding_dt) from dp_hldg_mstr_cdsl where dphmc_dpm_id = @pa_dpm_id and @pa_from_dt>dphmc_holding_dt
set @l_min_trx_dt = @l_base_hldg_dt+1
end 
...
create clustered index ix_1 on #tmp_cdsl_holding_dtls(cdshm_isin,cdshm_dpam_id,cdshm_dpm_id)                      
...
begin
	...
	,sum(isnull(dphmc_free_qty,0))+sum(isnull(DPHMC_PLEDGE_QTY,0))++sum(isnull(DPHMC_LOCKIN_QTY,0)) holdingbal     
	--we can use curr  qty also for above case lock in qty added on 08112012 by tushar as mosl reported issue         
	from dp_hldg_mstr_Cdsl   with(nolock)            
	where dphmc_holding_dt = @l_base_hldg_dt        
	group by  dphmc_dpm_id , dphmc_dpam_id , dphmc_isin) holding        
	where cdshm_dpm_id = dphmc_dpm_id         
	and dphmc_dpam_id = cdshm_dpam_id         
	and cdshm_isin = dphmc_isin    
...
and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to ) b          
group by  cdshm_dpm_id        
, cdshm_dpam_id          
, cdshm_isin        
) tranbal        
