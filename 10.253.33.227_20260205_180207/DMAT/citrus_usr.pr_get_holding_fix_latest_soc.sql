-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_soc
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_soc](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
AND EXISTS (SELECT TOC_BOID FROM TBL_OWN_ACCOUNT WHERE TOC_BOID=CDSHM_BEN_ACCT_NO)
...
AND EXISTS (SELECT TOC_BOID FROM TBL_OWN_ACCOUNT WHERE TOC_BOID=DPAM_SBA_NO)
