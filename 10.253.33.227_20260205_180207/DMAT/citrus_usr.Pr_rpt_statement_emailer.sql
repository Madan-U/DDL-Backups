-- Object: PROCEDURE citrus_usr.Pr_rpt_statement_emailer
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

/*  
exec Pr_rpt_statement 'cdsl',3,'Jun  1 2009','Jul  1 2009','N','N','','','','','N','y',1,'HO|*~|','','','',''  
*/   
	--[Pr_rpt_statement_emailer] 'CDSL',3,'Jun 30 2009','Jun 30 2010','N','N','','','','','N','Y',1,'0','','','',''				

  
CREATE  Proc [citrus_usr].[Pr_rpt_statement_emailer]                              
@pa_dptype varchar(4),                                      
@pa_excsmid int,                                      
...
-- for block email client start --  
  delete A 
  from #ACLIST A,blk_client_email_dtls   
  where dpam_id = blcked_dpam_id and blkced_dpmid = @@dpmid  
  and blkced_rptname = 'TRANSACTION'  

--[Pr_rpt_statement_emailer] 'nsdl',4,'Jun 30 2009','Jun 30 2010','N','N','','','','','N','Y',1,'0','','','',''				
-- for block email client end --  
 ...
 BEGIN                                
  ...
  select @pa_prevfromdate = max(dphmcd_holding_dt) from DP_DAILY_HLDG_CDSL with(nolock) where dphmcd_holding_dt <= convert(varchar(11),@pa_todate,109)  
    ...
    order_by int,          
    line_no bigint,  
    cdshm_trg_settm_no varchar(13)                          
    )                              
                           
                           
  if @pa_group_cd <> ''                
  begin                
      insert into #tempmaincdsl(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,cdshm_trg_settm_no)                           
  ...
  begin  
   ...
   if @pa_group_cd <> ''                
   begin                
    insert into #tempmaincdsl(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,dphmcd_cntr_settm_id)                           
   ...
   begin                
    insert into #tempmaincdsl(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,CDSHM_TRG_SETTM_NO)                           
    select DPHMCD_DPAM_ID,                            
    DPAM_SBA_NAME,                          
    DPAM_SBA_NO,                                      
    trans_desc=Convert(varchar,ISNULL(DPHMCD_CURR_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_FREE_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_LEND_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_LEND_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_FREEZE_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_LOCKIN_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_LOCKIN_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_AVAIL_LEND_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_AVAIL_LEND_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_PLEDGE_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_EARMARK_QTY,0.000)) + '|' + Convert(varchar,ISNULL(DPHMCD_BORROW_QTY,0.000)),          
...
isnull(dphmcd_cntr_settm_id,'')                                   
    ...
    and isnull(DPHMCD_CURR_QTY,0) <> 0  
     ...
     CDSHM_TRATM_DESC=lower(ltrim(rtrim(ISNULL(trans_desc,'')))),                              
     dpm_trans_no,                                      
  ...
  trans_date,cdshm_trg_settm_no                                                
     ...
     order by dpam_acctno,dpam_sba_name,order_by,Isin_name,14,line_no,cdshm_trg_settm_no                 
 ...
 select @pa_prevfromdate = max(dpdhmd_holding_dt) from DP_DAILY_HLDG_NSDL with(nolock) where dpdhmd_holding_dt < @pa_fromdate  
  ...
  begin       
    print '@PA_SETTM_TYPE '+@PA_SETTM_TYPE  
 ...
 update #tempmainnsdl_old set final_trans = 1,sett_type='',sett_no='' where Ben_acct_type in('14') and book_narr_cd='088'  
    ...
    --for non pool accounts                  
     ...
     -- for pool accounts      
      delete from #tempmainnsdl where Ben_acct_type in('20','30','40') and book_narr_cd in ('031','032','041','043','053','054','055','056','057','061','076','073','074','075','095','098','099','121','123','124','153','156','163','173','183','193','201','203','211','213')      
  ...
  begin                
   ...
   where exists(select dpam_id from #tempmainnsdl with(nolock) where dpam_id = dpdhmd_dpam_id)    
         ...
         AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(DPDHMD_SETT_NO,'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To 
END   
