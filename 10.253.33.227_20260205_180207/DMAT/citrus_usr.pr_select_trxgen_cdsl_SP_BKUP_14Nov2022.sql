-- Object: PROCEDURE citrus_usr.pr_select_trxgen_cdsl_SP_BKUP_14Nov2022
...
--rollback
CREATE  PROCEDURE [citrus_usr].[pr_select_trxgen_cdsl_SP_BKUP_14Nov2022]  
(  
						...
						--  
								...
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
										...
										,abs(demrm_qty) qty
										--,'','','','','','0'
										,convert(varchar ,@L_TOT_REC) Nofra
										,isnull(demrd_status,'')
										,isnull(DEMRD_FOLIO_NO,'')
										,isnull(DEMRD_CERT_NO,'')
										,isnull(DEMRD_DISTINCTIVE_NO_FR,'')
										,isnull(DEMRD_DISTINCTIVE_NO_TO,'')
										,replace(convert(varchar(11),isnull(DEMRM_REQUEST_DT,space(8)),103),'/','') DEMRM_REQUEST_DT
										...
										,dp_acct_mstr  , demat_request_dtls
											...
											SELECT @L_TOT_REC = COUNT(demrm_ID) FROM demat_request_mstr left outer join dmat_dispatch on demrm_id=disp_demrm_id and DISP_TO<> 'C',dp_acct_mstr
                                            --,demat_request_dtls
...
--										order by PLDTC_UPDATED_DATE										 
							...
							Select convert(char(1),case when pldtc_trastm_cd in('CRTE','CONF') then 'P'  when pldtc_trastm_cd in('CLOS') then 'U' when pldtc_trastm_cd in('UCLOS') then 'A' when pldtc_trastm_cd in('INVK') then 'C' else '' end )--'P','U','C','A'  
										 + convert(char(1),case when pldtc_trastm_cd = 'CRTE' and isnull(PLDTC_SUB_STATUS,'') = '' then 'S'
															  when pldtc_trastm_cd = 'CONF' and isnull(PLDTC_SUB_STATUS,'') = '' then 'A'
															  when pldtc_trastm_cd = 'CLOS' and isnull(PLDTC_SUB_STATUS,'') = '' and isnull(pldtc_closure_by,'')='PLEDGOR' then 'S'
															  when pldtc_trastm_cd = 'CLOS' and isnull(PLDTC_SUB_STATUS,'') = '' and isnull(pldtc_closure_by,'')='PLEDGEE' then 'A'
															  when pldtc_trastm_cd = 'UCLOS' and isnull(PLDTC_SUB_STATUS,'') = '' and isnull(pldtc_closure_by,'')='PLEDGOR' then 'S'
															   when pldtc_trastm_cd = 'UCLOS' and isnull(PLDTC_SUB_STATUS,'') = '' and isnull(pldtc_closure_by,'')='PLEDGEE' then 'S'
															    when pldtc_trastm_cd = 'INVK' and isnull(PLDTC_SUB_STATUS,'') = '' and isnull(pldtc_closure_by,'')='PLEDGEE' then 'S'
															  when isnull(PLDTC_SUB_STATUS,'') <> '' then isnull(PLDTC_SUB_STATUS,'') 
															  else '' end ) --'P' THEN 'S','A','R','C','E' --   'U' THEN 'S','A','R','C','E'--'A' THEN 'S','E'--'C' THEN 'S','E'
										 + convert(char(1),case when pldtc_trastm_cd = 'CRTE' and isnull(PLDTC_SUB_STATUS,'') = '' then isnull(PLDTC_SECURITTIES,'') else '' end)--'P''S' THEN 'F','L'   
										 + '0000000000000000'--convert(char(16),isnull(A.DPAM_SBA_NO,'')) --'P''S''L'   
--commented as want pledge slip no in new common format -- + convert(char(8),case when pldtc_trastm_cd = 'CRTE' and isnull(PLDTC_SUB_STATUS,'') = '' then '00000000' else isnull(PLDTC_PSN,'') end)  --'P''S' THEN '00000000' ELSE MEND PLDTC_PSN
										 ...
										 + convert(char(16),citrus_usr.FN_FORMATSTR(abs(PLDTC_QTY),13,3,'L','0'))--'P' OR ZEROS
										 --+ ltrim(rtrim(convert(char(16),abs(PLDTC_QTY))))--'P' OR ZEROS
									     + convert(char(15),isnull(pldtc_value,'0'))--'P' OR ZEROS--PLEDGE VALUE?????????
										 --+ convert(char(15),citrus_usr.FN_FORMATSTR(abs(isnull(CLOPM_CDSL_RT,0)*PLDTC_QTY),13,3,'L',' '))--'P' OR ZEROS--PLEDGE VALUE?????????
										 + convert(char(8),case when pldtc_trastm_cd = 'CRTE' and isnull(PLDTC_SUB_STATUS,'') = '' then replace(convert(varchar(11),PLDTC_EXPIRY_DT,103),'/','') else '00000000' end) --'P''S' ELSE ZEROS 
										 + convert(char(16),case when ((convert(char(1),case when pldtc_trastm_cd = 'CRTE' and isnull(PLDTC_SUB_STATUS,'') = '' then 'S'
															  when pldtc_trastm_cd = 'CONF' and isnull(PLDTC_SUB_STATUS,'') = '' then 'A'
															  when pldtc_trastm_cd = 'CLOS' and isnull(PLDTC_SUB_STATUS,'') = '' then 'S'
															  when pldtc_trastm_cd = 'UCLOS' and isnull(PLDTC_SUB_STATUS,'') = '' then 'S'
															  when isnull(PLDTC_SUB_STATUS,'') <> '' then isnull(PLDTC_SUB_STATUS,'') 
															  else '' end )) in ('A','R') and 
															  (case when pldtc_trastm_cd in('CRTE','CONF') then 'P'  when pldtc_trastm_cd in('CLOS','UCLOS') then 'U' else '' end) in ('P')) 
															  then convert(varchar(16),PLDTC_ID)
															  when ((convert(char(1),case when pldtc_trastm_cd = 'CRTE' and isnull(PLDTC_SUB_STATUS,'') = '' then 'S'
															  when pldtc_trastm_cd = 'CONF' and isnull(PLDTC_SUB_STATUS,'') = '' then 'A'
															  when pldtc_trastm_cd = 'CLOS' and isnull(PLDTC_SUB_STATUS,'') = '' then 'S'
															  ...
															  (case when pldtc_trastm_cd in('CRTE','CONF') then 'P'  when pldtc_trastm_cd in('CLOS','UCLOS') then 'U' else '' end) in ('U')) then convert(varchar(16),PLDTC_ID)
															  else '' end)  --convert(char(16),case when pldtc_trastm_cd = 'CONF'  then PLDTC_ID else '' end )  --'PLEDGEE DP INTERNAL REF NO'
									     + convert(char(16),case when pldtc_trastm_cd = 'CRTE'  then PLDTC_ID else '' end ) --'PLEDGEE DP INTERNAL REF NO'  
										 + convert(char(20),PLDTC_AGREEMENT_NO)  --'P''S'
										 + convert(char(4),'0000') 
										...
										--,citrus_usr.FN_FORMATSTR(convert(varchar(15),abs(PLDTC_QTY)),13,3,'L','0') QTY
										,ltrim(rtrim(convert(char(16),abs(PLDTC_QTY)))) QTY
,'',PLDTC_CLOSURE_BY,PLDTC_SLIP_NO,PLDTC_PSN,PLDTC_TRASTM_CD,PLDTC_ID,replace(convert(varchar(11),isnull(PLDTC_REQUEST_DT,space(8)),103),'/','') PLDTC_REQUEST_DT
										FROM  cdsl_pledge_dtls left outer join closing_price_mstr_cdsl on  CLOPM_ISIN_CD =   PLDTC_ISIN  
                                        and    ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = PLDTC_ISIN  and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc)
										,dp_acct_mstr  A
										WHERE PLDTC_DPAM_ID = A.dpam_id  
...
---MARGIN PLEDGE
						...
						IF @l_TRX_TAB= 'PLEDGEMP'  
						BEGIN  
						--  
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
										Select convert(char(1),case when pldtcm_trastm_cd in('CRTEMP','CONF','CREATEMR') then 'P'  when pldtcm_trastm_cd in('CLOS') then 'U' when pldtcm_trastm_cd in('UCLOS') then 'A' when pldtcm_trastm_cd in('INVK') then 'C' else '' end )--'P','U','C','A'  
										 + convert(char(1),case when pldtcm_trastm_cd in ('CRTEMP','CREATEMR') and isnull(PLDTCm_SUB_STATUS,'') = '' then 'S'
															  when pldtcm_trastm_cd = 'CONF' and isnull(PLDTCm_SUB_STATUS,'') = '' then 'A'
															  when pldtcm_trastm_cd = 'CLOS' and isnull(PLDTCm_SUB_STATUS,'') = '' and isnull(pldtcm_closure_by,'')='PLEDGOR' then 'S'
															  when pldtcm_trastm_cd = 'CLOS' and isnull(PLDTCm_SUB_STATUS,'') = '' and isnull(pldtcm_closure_by,'')='PLEDGEE' then 'A'
															  when pldtcm_trastm_cd = 'UCLOS' and isnull(PLDTCm_SUB_STATUS,'') = '' and isnull(pldtcm_closure_by,'')='PLEDGOR' then 'S'
															   when pldtcm_trastm_cd = 'UCLOS' and isnull(PLDTCm_SUB_STATUS,'') = '' and isnull(pldtcm_closure_by,'')='PLEDGEE' then 'S'
															    when pldtcm_trastm_cd = 'INVK' and isnull(PLDTCm_SUB_STATUS,'') = '' and isnull(pldtcm_closure_by,'')='PLEDGEE' then 'S'
															  when isnull(PLDTCm_SUB_STATUS,'') <> '' then isnull(PLDTCm_SUB_STATUS,'') 
															  else '' end ) --'P' THEN 'S','A','R','C','E' --   'U' THEN 'S','A','R','C','E'--'A' THEN 'S','E'--'C' THEN 'S','E'
										 + convert(char(1),case when pldtcm_trastm_cd in ('CRTEMP','CREATEMR') and isnull(PLDTCm_SUB_STATUS,'') = '' then isnull(PLDTCm_SECURITTIES,'') else '' end)--'P''S' THEN 'F','L'   
										 + '0000000000000000'--convert(char(16),isnull(A.DPAM_SBA_NO,'')) --'P''S''L'   
--commented as want pledge slip no in new common format -- + convert(char(8),case when pldtc_trastm_cd = 'CRTE' and isnull(PLDTC_SUB_STATUS,'') = '' then '00000000' else isnull(PLDTC_PSN,'') end)  --'P''S' THEN '00000000' ELSE MEND PLDTC_PSN
									     + convert(char(8),pldtcm_slip_no)
										 + convert(char(16),isnull(A.DPAM_SBA_NO,''))  
											+ case when len(PLDTCm_PLDG_CLIENTID)=8 then convert(char(16),isnull(pldtcm_pldg_dpid+PLDTCm_PLDG_CLIENTID,''))   else convert(char(16),isnull(PLDTCm_PLDG_CLIENTID,''))  end
										 + convert(char(12),isnull(PLDTCm_ISIN,''))  
										 + convert(char(16),citrus_usr.FN_FORMATSTR(abs(PLDTCm_QTY),13,3,'L','0'))--'P' OR ZEROS
										 --+ ltrim(rtrim(convert(char(16),abs(PLDTC_QTY))))--'P' OR ZEROS
									     --+ convert(char(15),citrus_usr.FN_FORMATSTR(abs(isnull(CLOPM_CDSL_RT,0)*PLDTC_QTY),13,3,'L','0'))--'P' OR ZEROS--PLEDGE VALUE?????????
										 + convert(char(15),citrus_usr.FN_FORMATSTR(abs(isnull(CLOPM_CDSL_RT,0)*PLDTCm_QTY),13,3,'L','0'))--'P' OR ZEROS--PLEDGE VALUE?????????
										 + convert(char(8),case when pldtcm_trastm_cd in ('CRTEMP','CREATEMR') and isnull(PLDTCm_SUB_STATUS,'') = '' then replace(convert(varchar(11),PLDTCm_EXPIRY_DT,103),'/','') else '00000000' end) --'P''S' ELSE ZEROS 
										 + convert(char(16),case when ((convert(char(1),case when pldtcm_trastm_cd = 'CRTE' and isnull(PLDTCm_SUB_STATUS,'') = '' then 'S'
															  when pldtcm_trastm_cd = 'CONF' and isnull(PLDTCm_SUB_STATUS,'') = '' then 'A'
															  when pldtcm_trastm_cd = 'CLOS' and isnull(PLDTCm_SUB_STATUS,'') = '' then 'S'
															  when pldtcm_trastm_cd = 'UCLOS' and isnull(PLDTCm_SUB_STATUS,'') = '' then 'S'
															  when isnull(PLDTCm_SUB_STATUS,'') <> '' then isnull(PLDTCm_SUB_STATUS,'') 
															  else '' end )) in ('A','R') and 
															  (case when pldtcm_trastm_cd in('CRTEMP','CONF','CREATEMR') then 'P'  when pldtcm_trastm_cd in('CLOS','UCLOS') then 'U' else '' end) in ('P')) 
															  then convert(varchar(16),PLDTCm_ID)
															  when ((convert(char(1),case when pldtcm_trastm_cd in ('CRTEMP','CREATEMR') and isnull(PLDTCm_SUB_STATUS,'') = '' then 'S'
															  when pldtcm_trastm_cd = 'CONF' and isnull(PLDTCm_SUB_STATUS,'') = '' then 'A'
															  when pldtcm_trastm_cd = 'CLOS' and isnull(PLDTCm_SUB_STATUS,'') = '' then 'S'
															  when pldtcm_trastm_cd = 'UCLOS' and isnull(PLDTCm_SUB_STATUS,'') = '' then 'S'
															  when isnull(PLDTCm_SUB_STATUS,'') <> '' then isnull(PLDTCm_SUB_STATUS,'') 
															  else '' end )) in ('S') and 
															  (case when pldtcm_trastm_cd in('CRTEMP','CONF','CREATEMR') then 'P'  when pldtcm_trastm_cd in('CLOS','UCLOS') then 'U' else '' end) in ('U')) then convert(varchar(16),PLDTCm_ID)
															  else '' end)  --convert(char(16),case when pldtc_trastm_cd = 'CONF'  then PLDTC_ID else '' end )  --'PLEDGEE DP INTERNAL REF NO'
									     + convert(char(16),case when pldtcm_trastm_cd in ('CRTEMP','CREATEMR')  then PLDTCm_ID else '' end ) --'PLEDGEE DP INTERNAL REF NO'  
										 + convert(char(20),PLDTCm_AGREEMENT_NO)  --'P''S'
										 + convert(char(4),'0000') 
										 + '0000000000000000'--convert(char(16),citrus_usr.FN_FORMATSTR(abs(PLDTC_QTY),13,3,'L','0'))--UNPLEDGE / AUTO UNPLEDGE / CONFISCATE PART QUANTITY
										 + convert(char(100),PLDTCm_RMKS) 
										 + CONVERT(char(16),isnull(PLDTCm_SLIP_NO,'')) DETAILS
										--,citrus_usr.FN_FORMATSTR(convert(varchar(15),abs(PLDTC_QTY)),13,3,'L','0') QTY
										,ltrim(rtrim(convert(char(16),abs(PLDTCm_QTY)))) QTY
,CASE WHEN PLDTCM_TRASTM_CD in('CRTEMP','CREATEMR') THEN '' ELSE  replace(convert(varchar(11),isnull(PLDTCm_EXEC_DT,space(8)),103),'/','')  END,PLDTCm_CLOSURE_BY,PLDTCm_SLIP_NO,PLDTCm_PSN,PLDTCm_TRASTM_CD,PLDTCm_ID
,replace(convert(varchar(11),isnull(PLDTCm_REQUEST_DT,space(8)),103),'/','') PLDTCm_REQUEST_DT
										FROM  cdsl_marginpledge_dtls left outer join closing_price_mstr_cdsl on  CLOPM_ISIN_CD =   PLDTCm_ISIN  
                                        and    ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = PLDTCm_ISIN  and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc)
										,dp_acct_mstr  A
										WHERE PLDTCm_DPAM_ID = A.dpam_id  
										--and   CLOPM_ISIN_CD =   PLDTC_ISIN
										--and   PLDTC_REQUEST_DT = CLOPM_DT
										AND   A.dpam_deleted_ind  = 1  
										AND   pldtcm_deleted_ind = 1  
										AND   isnull(pldtcm_status,'P')='P'  
										AND   ltrim(rtrim(isnull(pldtcm_batch_no,''))) = ''  
										AND   pldtcm_request_dt BETWEEN  CONVERT(VARCHAR,CONVERT(DATETIME, @PA_FROM_DT,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@PA_TO_DT,103),106)+' 23:59:59'       
										AND   dpam_sba_no LIKE CASE WHEN LTRIM(RTRIM(@PA_POOL_ACCTNO))     = '' THEN '%' ELSE @PA_POOL_ACCTNO  END
										--AND		dpam_dpm_id   = @L_DPM_ID
												AND		dpam_dpm_id   = case when @L_DPM_ID='3' then dpam_dpm_id else @L_DPM_ID end
										order by PLDTCm_UPDATED_DATE										 
						
										SELECT @L_TOT_REC = COUNT(pldtcm_id) FROM cdsl_marginpledge_dtls ,dp_acct_mstr  
										WHERE  isnull(pldtcm_status,'P')='P' and PLDTCm_DPAM_ID = dpam_id   
										--AND    dpam_dpm_id      = @l_dpm_id    
												AND		dpam_dpm_id   = case when @L_DPM_ID='3' then dpam_dpm_id else @L_DPM_ID end
										AND    ltrim(rtrim(isnull(pldtcm_batch_no,''))) = ''  
										AND    pldtcm_request_dt BETWEEN  CONVERT(VARCHAR,CONVERT(DATETIME, @PA_FROM_DT,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@PA_TO_DT,103),106)+' 23:59:59'  
										...
										BEGIN   
										/* Update in Demat_request_mstr*/   
										UPDATE D1 Set pldtcm_batch_no=@pa_batch_no FROM cdsl_marginpledge_dtls D1 ,dp_acct_mstr D2  
										WHERE  PLDTCm_DPAM_ID = dpam_id   
										--AND    dpam_dpm_id   = @l_dpm_id    
												AND		dpam_dpm_id   = case when @L_DPM_ID='3' then dpam_dpm_id else @L_DPM_ID end
										AND    isnull(pldtcm_status,'P')='P'  
										AND    ltrim(rtrim(isnull(pldtcm_batch_no,''))) = ''  
									...
									BEGIN   
										Select convert(char(1),case when pldtc_trastm_cd in('CRTE','CONF') then 'P'  when pldtc_trastm_cd in('CLOS') then 'U' else '' end )--'P','U','C','A'  
										 ...
										 + convert(char(4),'0000') --'UNPLEDGE / AUTO UNPLEDGE / CONFISCATE PART COUNTER' 
											+ convert(char(16),citrus_usr.FN_FORMATSTR(abs(PLDTC_QTY),13,3,'L','0'))--UNPLEDGE / AUTO UNPLEDGE / CONFISCATE PART QUANTITY
										 + convert(char(100),PLDTC_RMKS) 
										 + CONVERT(char(16),isnull(PLDTC_SLIP_NO,'')) 
						...
						--   
								...
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
										SELECT  replace(convert(varchar(11),dptdc_execution_dt,103),'/','')  
												+ isnull(dpam_sba_no,'')  
												+ isnull(case when (dpam_sba_no=dptdc_created_by and len(dptdc_counter_demat_acct_no)=16)  then right(isnull(dptdc_counter_demat_acct_no,''),16) when (dpam_sba_no=dptdc_created_by and len(dptdc_counter_demat_acct_no)=8) then  isnull(dptdc_counter_dp_id,'') + isnull(dptdc_counter_demat_acct_no,'') when (LEN(dptdc_counter_demat_acct_no)>8 and LEN(dptdc_counter_demat_acct_no)<>'16') then ISNULL(dptdc_counter_demat_acct_no,'') + SPACE(1) else isnull(dptdc_counter_dp_id,'') + isnull(dptdc_counter_demat_acct_no,'') end ,'')  
												+ isnull(convert(char(12),dptdc_isin),'')  
												+ convert(varchar(16),citrus_usr.FN_FORMATSTR(convert(varchar(15),abs(dptdc_qty)),12,3,'L','0'))    
												+ case  when dptdc_internal_trastm in('BOCM','CMCM') then 'S' when dptdc_internal_trastm = 'CMBO' then 'S' else 'S' end  
												+ isnull(dptdc_cash_trf,'')  
