-- Object: PROCEDURE citrus_usr.ImportJVVouchers_Quant
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--select * from tmp_jv_mstr
--select * from financial_yr_mstr
--BEGIN TRAN
--[ImportJVVouchers_Quant] 1257,2,4,'aug  9 2010','aug  9 2010',	'VISHAL'
--SELECT ldg_instrument_no,* FROM LEDGER4 where ldg_dpm_id=2 ORDER BY 2 DESC
--SELECT COUNT(*) FROM LEDGER4 -- SELECT 7882-8079
--ROLLBACK
CREATE Proc [citrus_usr].[ImportJVVouchers_Quant] -- require extra entry of service tax
@pa_task_id bigint,
@pa_dpmid int,
@pa_finid int,
...
select top 1 @@voucher_dt = Replace(TMPJV_FILLER1,'AMC','') FROM tmp_jv_mstr --convert(varchar(11),TMPJV_TODATE,109) from tmp_jv_mstr 
...
Select @@glaccountidSertax = ISNULL(BITRM_VALUES,0) FROM BITMAP_REF_MSTR WHERE BITRM_PARENT_CD='DP_GL_SERTAX_ACCNO'  AND BITRM_CHILD_CD = @pa_dpmid 

if isnull(@@glaccountidSertax,'0') = '0'
begin

	UPDATE filetask
	SET    Status = 'FAILED',usermsg = 'ERROR : Please contact your vendor for configuring GL Account(Service tax) for JV enties'
	WHERE  task_id = @pa_task_id
	RETURN
end
...
set @@ssql = @@ssql + ' select LDG_ID=' + convert(varchar,isnull(@@lst_curr_id,0)) + ' + 1 ,LDG_DPM_ID=' + convert(varchar,@pa_dpmid) + ',LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''01'',LDG_VOUCHER_NO=' + convert(varchar,@@Vno) + ',1,LDG_REF_NO=''' + @@ref_no + ''',''' + @@voucher_dt + ''',' + convert(varchar,@@glaccountidSertax) + ',LDG_ACCOUNT_TYPE=''G'',convert(numeric(10,2),sum(TMPJV_CREDIT-TMPJV_DEBIT)*.103)*-1,LDG_NARRATION= case when LEFT(TMPJV_FILLER1,3)=''AMC'' then ''BEING AMT DEBITED TOWARDS AMC CHARGES(SERVICE TAX)'' when LEFT(TMPJV_FILLER1,3)=''OTH'' then ''BEING AMT DEBITED TOWARDS TRANSACTION CHARGES'' else ''BEING AMT DEBITED TOWARDS POA CHARGES'' end ,LDG_BANK_ID='''',LDG_ACCOUNT_NO='''',LDG_INSTRUMENT_NO=TMPJV_FILLER1,LDG_BANK_CL_DATE='''',LDG_COST_CD_ID=0,LDG_BILL_BRKUP_ID=0,LDG_TRANS_TYPE=''AC'',LDG_STATUS=''P'',''' + @pa_loginname + ''',getdate(),''' + @pa_loginname + ''',getdate(),1,0 from tmp_jv_mstr' 
...
Set @@ssql = 'insert into ledger' + convert(varchar,@pa_finid) + ' (LDG_ID,LDG_DPM_ID,LDG_VOUCHER_TYPE,LDG_BOOK_TYPE_CD,LDG_VOUCHER_NO,LDG_SR_NO,LDG_REF_NO,LDG_VOUCHER_DT,LDG_ACCOUNT_ID,LDG_ACCOUNT_TYPE,LDG_AMOUNT,LDG_NARRATION,LDG_BANK_ID,LDG_ACCOUNT_NO,LDG_INSTRUMENT_NO,LDG_BANK_CL_DATE,LDG_COST_CD_ID,LDG_BILL_BRKUP_ID,LDG_TRANS_TYPE,LDG_STATUS,LDG_CREATED_BY,LDG_CREATED_DT,LDG_LST_UPD_BY,LDG_LST_UPD_DT,LDG_DELETED_IND,LDG_BRANCH_ID)
select LDG_ID=' + convert(varchar,isnull(@@lst_curr_id,0)) + ' + TMPJV_SR_NO,LDG_DPM_ID=' + convert(varchar,@pa_dpmid) + ',LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''01'',LDG_VOUCHER_NO=' + convert(varchar,@@Vno) + ',TMPJV_SR_NO,LDG_REF_NO=''' + @@ref_no + ''',''' + @@voucher_dt + ''',TMPJV_DPAM_ID,LDG_ACCOUNT_TYPE=''P'',CASE WHEN TMPJV_CREDIT <> 0 THEN convert(numeric(10,2),TMPJV_CREDIT+TMPJV_CREDIT*.103) ELSE convert(numeric(10,2),TMPJV_DEBIT+TMPJV_DEBIT*.103) * -1 END  ,LDG_NARRATION=TMPJV_NARRATION,LDG_BANK_ID='''',LDG_ACCOUNT_NO='''',LDG_INSTRUMENT_NO=TMPJV_FILLER1,LDG_BANK_CL_DATE='''',LDG_COST_CD_ID=0,LDG_BILL_BRKUP_ID=0,LDG_TRANS_TYPE=''AC'',LDG_STATUS=''P'',''' + @pa_loginname + ''',getdate(),''' + @pa_loginname + ''',getdate(),1,0 
