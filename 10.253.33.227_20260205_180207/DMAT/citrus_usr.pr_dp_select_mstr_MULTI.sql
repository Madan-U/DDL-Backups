-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_MULTI
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

/*

exec pr_dp_select_mstr @pa_id ='3',	@pa_action= 'OFFM_TRX_SELC_CDSL',@pa_login_name='HO',@pa_cd ='3',  @pa_desc='VISHAL',@pa_rmks='',@pa_values= '',@pa_roles='6*|~*',	@pa_scr_id='198',@RowDelimiter='*|~*',@ColDelimiter='|*~|',@pa_ref_cur=''	
exec pr_dp_select_mstr   @pa_id ='4',	@pa_action= 'INTERSETM_TRX_SELC', @pa_login_name='TUSHAR' ,@pa_cd ='', @pa_desc='',@pa_rmks='', @pa_values= '', @pa_roles='1*|~*8*|~*', @pa_scr_id='187',@RowDelimiter='*|~*',@ColDelimiter='|*~|',@pa_ref_cur=''	
exec pr_dp_select_mstr @pa_id='1|*~|3|*~|',@pa_action='VOUCHERREC_SEL',@pa_login_name='',@pa_cd='',@pa_desc='12/11/2008',@pa_rmks='203412',@pa_values='',@pa_roles='6*|~*',@pa_scr_id='217',@RowDelimiter='*|~*',@ColDelimiter='|*~|',@pa_ref_cur=''
exec pr_dp_select_mstr @pa_id='',@pa_action='CLSR_CHKR',@pa_login_name='BHAVIN',@pa_cd='',@pa_desc='',@pa_rmks='',@pa_values='',@pa_roles='1*|~*8*|~*',@pa_scr_id='224',@RowDelimiter='*|~*',@ColDelimiter='|*~|',@pa_ref_cur=''
exec pr_dp_select_mstr '','SETTM_DESC','','',4,'','','',0,'*|~*','|*~|',''	
exec pr_dp_select_mstr '','SETTM_DESC','','',3,'','','',0,'*|~*','|*~|',''	
...
*/
 ...
 CREATE PROCEDURE  [citrus_usr].[pr_dp_select_mstr_MULTI] 
	( 
	@pa_id  VARCHAR(20)                  
...
if @pa_action = 'SUBCM_NSDL'        
  ...
  --                
      ...
      --                
       ...
       --                
          select isnull(C.CLIM_NAME1,'')  + ' ' + isnull(C.CLIM_NAME2,'') + ' ' +isnull(C.CLIM_NAME3,'') CLIM_NAME1               
          ,isnull(DPHD_SH_FNAME,'') + ' ' + isnull(DPHD_SH_MNAME,'') + ' ' + isnull(DPHD_SH_LNAME,'')   DPHD_SH_FNAME             
              ,isnull(DPHD_TH_FNAME,'') + ' ' + isnull(DPHD_TH_MNAME,'') + ' ' + isnull(DPHD_TH_LNAME,'')   DPHD_TH_FNAME          
     ...
     BEGIN                
     --                
       SELECT isnull(details.CLIM_NAME1,'')  + ' ' + isnull(details.CLIM_NAME2,'') + ' ' +isnull(details.CLIM_NAME3,'') CLIM_NAME1                
             , details.DPHD_SH_FNAME + ' ' + details.DPHD_SH_MNAME + ' ' + details.DPHD_SH_LNAME DPHD_SH_FNAME                
             , details.DPHD_TH_FNAME + ' ' + details.DPHD_TH_MNAME + ' ' + details.DPHD_TH_LNAME DPHD_TH_FNAME                
              ,details.DPAM_SBA_NO + @coldelimiter + @pa_rmks DPAM_ACCT_NO                
                             
       FROM   (select hd.CLIM_CRN_NO                
                     ,ISNULL(hd.CLIM_NAME1,'')    CLIM_NAME1   ,ISNULL(hd.CLIM_NAME2,'')    CLIM_NAME2      ,ISNULL(hd.CLIM_NAME3,'')    CLIM_NAME3                             
                     ,ISNULL(hd.DPHD_SH_FNAME,'') DPHD_SH_FNAME                
                     ,ISNULL(hd.DPHD_TH_FNAME,'') DPHD_TH_FNAME  
  ,ISNULL(hd.DPHD_SH_MNAME,'') DPHD_SH_MNAME                
                     ,ISNULL(hd.DPHD_TH_MNAME,'') DPHD_TH_MNAME   
  ,ISNULL(hd.DPHD_SH_LNAME,'') DPHD_SH_LNAME                
                     ,ISNULL(hd.DPHD_TH_LNAME,'') DPHD_TH_LNAME               
                     ,ISNULL(hd.DPAM_SBA_NO,0)   DPAM_SBA_NO                 
                     ,isnull(hd.dpam_id ,0)       dpam_id                 
                     from                 
    (SELECT CLIM_CRN_NO                
                     ,ISNULL(CLIM_NAME1,'')    CLIM_NAME1       ,ISNULL(CLIM_NAME2,'')    CLIM_NAME2      ,ISNULL(CLIM_NAME3,'')    CLIM_NAME3               
                     ,ISNULL(DPHD_SH_FNAME,'') DPHD_SH_FNAME                
                     ,ISNULL(DPHD_TH_FNAME,'') DPHD_TH_FNAME   
...
WHERE setm.setm_deleted_ind   = 1                 
  ...
  --            
    ...
    AND    isnull(DEMRM_TRANSACTION_NO,'') LIKE CASE WHEN LTRIM(RTRIM(@pa_values)) = '' THEN '%' ELSE @pa_values END     
  ...
  --                
    ...
    --               
      ...
      AND    REMRM_TRANSACTION_NO  LIKE CASE WHEN LTRIM(RTRIM(@pa_values)) = '' THEN '%' ELSE @pa_values END              
      AND    dpam.dpam_sba_no          LIKE CASE WHEN LTRIM(RTRIM(@pa_desc)) = '' THEN '%' ELSE @pa_desc END                
  ...
  --                
    ...
    END                
      ...
      SELECT compm_short_name +'-'+ excsm_exch_cd+'-'+excsm_seg_cd COMPANY                   
              ...
              ,dpm_name                 DPNAME                
              ,''                       ERRMSG 
              , convert(varchar(11),sliim_dt,103) sliim_dt                 
...
--            ,dptd_settlement_no          SETTLEMENTNO                
  ...
  --                
  END                
  ELSE IF @pa_action = 'ONP2C_DP_TRX_SELM_DTLS'        
  BEGIN                
  --                
    SELECT  dptd_counter_demat_acct_no                
           ,dptd_counter_dp_id                            
...
END
 ...
 begin
  select @pa_ref_cur = case when sum(ldg_amount)< 0 then convert(varchar,abs(sum(ldg_amount))) + ' ' + 'Cr' else convert(varchar,abs(sum(ldg_amount))) + ' ' + 'Dr' end   
   from ledger1,dp_acct_mstr where ldg_account_id=dpam_id
