-- Object: PROCEDURE citrus_usr.pr_ins_upd_trx_cdsl_sp_bak_06072015
...
create PROCEDURE [citrus_usr].[pr_ins_upd_trx_cdsl_sp_bak_06072015](@pa_id              VARCHAR(8000)      
              ,@pa_tab             VARCHAR(20)    
...
create table #tmpvaltbl1(dptdc_slip_no varchar(20),Val numeric(18,3),isin varchar(15),qty numeric(18,3),isin_name varchar(200))

	
	insert into #tmpvaltbl1
	select distinct dptdc_slip_no , CONVERT(NUMERIC(18,3),(abs(dptdc_qty) * CLOPM_CDSL_RT)) Val ,DPTDC_ISIN ,ABS(dptdc_qty) qty ,ISIN_NAME
...
select @pa_errmsg = @pa_errmsg +
 dptdc_isin + '|*~|' + convert(varchar,ISIN_NAME) + '|*~|' + convert(varchar,dptdc_id) + '|*~|' + convert(varchar,abs(DPTDC_QTY)) + '|*~|' + isnull((select distinct convert(varchar,val)  from #tmpvaltbl1 where isin = DPTDC_ISIN and abs(qty) = abs(dptdc_qty)),'') + '*|~*'
