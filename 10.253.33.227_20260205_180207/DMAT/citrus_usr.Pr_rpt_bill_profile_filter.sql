-- Object: PROCEDURE citrus_usr.Pr_rpt_bill_profile_filter
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--Pr_rpt_bill_profile_filter 	'CDSL',3,'Jan 26 2009','Jun 26 2009','N','N','1234567890123456','','','N','Y','',1,'HO|*~|',''	

CREATE Proc [citrus_usr].[Pr_rpt_bill_profile_filter]              
@pa_dptype varchar(4),                      
@pa_excsmid int,                      
...
set @pa_group_cd =  isnull(citrus_usr.fn_splitval(@pa_group_cd,1),'')  
 ...
 select @@Ledgerid =fin_id from Financial_Yr_Mstr where   (@pa_fromdate between fin_start_dt and fin_end_dt) and fin_deleted_ind = 1
...
--select dpam_id,dpam_sba_no into #tmp_ecnflg from dp_acct_mstr where citrus_usr.fn_ucc_accp(dpam_id,'ECN_FLAG','')='NO'
...
-- for ecnflag
delete from #ACLIST where dpam_id in (select c.accp_clisba_id from #account_properties_ecn c where accp_value='NO')
-- for ecnflag
