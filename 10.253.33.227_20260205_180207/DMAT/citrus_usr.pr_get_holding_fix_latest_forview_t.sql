-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_forview_t
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_forview_t](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
where DPHMC_HOLDING_DT = (select max(DPHMC_HOLDING_DT) from dp_hldg_mstr_cdsl )-- dateadd(dd,-1,@pa_from_dt)
...
truncate table holdingallforview_t

insert into holdingallforview_t
select t.*,@pa_to_dt dphmcd_holding_dt ,isnull(CLOPM_CDSL_RT,0) CLOPM_CDSL_RT ,citrus_usr.fn_acct_entp(DPHMCD_DPAM_ID,'bbo_code') tradingid 
...
select * from holdingallforview_t 
