-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_byisin_allreco
...
--exec [pr_get_holding_fix_byisin_test_all] 'mar 31 2012','mar 31 2012','0','9999999999999999','',''        
--1201090000025122~INE083C01022        
        
        
        
CREATE proc [citrus_usr].[pr_get_holding_fix_byisin_allreco](@pa_from_dt datetime, @pa_to_dt datetime                            
,@pa_ben_acct_no_fr varchar(16),@pa_ben_acct_no_to varchar(16),@pa_isin varchar(20),@pa_out  varchar(8000) out)          
as          
begin    
...
if exists(select name from sysobjects where name ='holdingereco')      
DROP TABLE holdingereco     
...
into #tmp_vw_holding_base_all from cdsl_holding_dtls          
...
,a.DPHMCD_FREE_QTY= 0 --f.DPHMCD_FREE_QTY    --0--    
...
exec citrus_usr.pr_getrmtcnfpnd_all @pa_from_dt , @pa_to_dt,@pa_ben_acct_no_fr ,@pa_ben_acct_no_to ,@pa_isin ,''    
...
exec citrus_usr.pr_getdmacnfpnd_all @pa_from_dt , @pa_to_dt,@pa_ben_acct_no_fr ,@pa_ben_acct_no_to ,@pa_isin ,''    
    
    
SELECT DPHMCD_DPAM_ID , DPHMCD_ISIN , SUM(DMATPENDING) pVERQTY INTO  #pVERQTY    
FROM #FINALDEMATHOLDI     
WHERE NOT EXISTS(SELECT 1 FROM CDSL_HOLDING_DTLS WHERE CDSHM_DPAM_ID = DPHMCD_DPAM_ID AND DPHMCD_ISIN = CDSHM_ISIN     
...
FROM #FINALDEMATHOLDI     
WHERE EXISTS(SELECT 1 FROM CDSL_HOLDING_DTLS WHERE CDSHM_DPAM_ID = DPHMCD_DPAM_ID AND DPHMCD_ISIN = CDSHM_ISIN     
AND CDSHM_TRAS_DT < =@PA_TO_DT AND CDSHM_TRATM_cD = '2202' AND CDSHM_TRANS_NO = TRANS_NO)    
AND DMATPENDING <> 0    
GROUP BY  DPHMCD_DPAM_ID , DPHMCD_ISIN    
HAVING SUM(DMATPENDING) <> 0    
...
into #holdingdmtpenfing2 from  vw_holding_base_all, DP_aCCT_MSTR     
...
INTO #TEMPDATA2 from   cdsl_holding_dtls                         
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_cr FROM CDSL_HOLDING_DTLS WHERE 
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_dr_remat FROM CDSL_HOLDING_DTLS 
WHERE CDSHM_TRATM_cD  = '2262' --=case when CDSHM_CDAS_TRAS_TYPE IN ('21','22','23') then '2262' else '2277' end 
and CDSHM_TRAS_DT between  @l_min_trx_dt and @pa_to_dt
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_dr_remat_cr FROM CDSL_HOLDING_DTLS 
WHERE CDSHM_TRATM_cD  = '2262' --=case when CDSHM_CDAS_TRAS_TYPE IN ('21','22','23') then '2262' else '2277' end 
and CDSHM_TRAS_DT between  @l_min_trx_dt and @pa_to_dt
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #availlenddata FROM CDSL_HOLDING_DTLS 
...
select *,@pa_to_dt dphmcd_holding_dt INTO holdingereco from #tmp_vw_holding_base_all         
where         
(dphmcd_curr_qty <> '0'                            
