-- Object: PROCEDURE citrus_usr.pr_ins_upd_profile_new
...
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_profile_new](@pa_id              VARCHAR(8000)  
...
--
  DECLARE @t_errorstr      VARCHAR(8000)
      ...
      , @delimeter_value varchar(10)
		 			...
		 			, @l_slab_no  varchar(1000)
					, @l_slab_no1  varchar(1000)
		 			, @l_bromm_id numeric(10,0)
		 			, @l_excpm_id numeric
	SET @l_slab_no = ''	
	SET @l_slab_no1 = ''	 			
		 			SELECT @l_excpm_id = excpm_id FROM EXCSM_PROD_MSTR epm, EXCH_SEG_MSTR esm WHERE excsm_id = @pa_excsm_cd AND excpm_Excsm_id = excsm_id AND excpm_prom_id = 1
-----------------------------------------------------------------------------------------------------jitesh	 			
DECLARE	 @L_slab_name		VARCHAR(50)
		,@L_charge_type     VARCHAR(20)
		,@L_subcategory     VARCHAR(8000) 
		,@L_billing_prd		VARCHAR(20) 
		,@L_baseon 			VARCHAR(20)
		,@L_caltype			VARCHAR(20)
		,@L_post_toacc		NUMERIC
...
set @l_counter  = 1  
set @l_count1 = citrus_usr.ufn_countstring(@pa_slab_no ,'*|~*')  

while @l_count1 >= @l_counter  
begin   
		SET @L_STR = citrus_usr.FN_SPLITVAL_ROW(@pa_slab_no,@l_counter)
		print @L_STR
		SET @L_slab_name = @pa_profile_name + '_' + citrus_usr.fn_splitval(@L_STR,1)
		SET @L_charge_type = citrus_usr.fn_splitval(@L_STR,2)
		SET @L_subcategory = replace(citrus_usr.fn_splitval(@L_STR,3),',','|*~|') 
		SET @L_baseon = citrus_usr.fn_splitval(@L_STR,4) 
		SET @L_caltype = citrus_usr.fn_splitval(@L_STR,5) 
		SET @L_billing_prd = citrus_usr.fn_splitval(@L_STR,6) 


		SET @L_post_toacc = citrus_usr.fn_splitval(@L_STR,7) 
		SET @L_value = REPLACE(citrus_usr.fn_splitval(@L_STR,8),',','|*~|')  
		SET @L_rmks = citrus_usr.fn_splitval(@L_STR,9)  

		exec [citrus_usr].[pr_ins_upd_cham] @pa_id            
											,'INS'          
											,@pa_login_name      
											,@pa_excsm_cd      
											, @L_slab_name       
			...
			SELECT @l_slab_no = @l_slab_no + convert(varchar(50),CHAM_SLAB_NO) +'|*~|' FROM CHARGE_MSTR  
			where cham_slab_name = @L_slab_name and CHAM_CHARGE_TYPE = @L_charge_type and cham_deleted_ind = 1 
...
SET @remainingstring = @pa_id  
		...
		--
				...
				--
				  ...
				  --
				  END
						select @l_count = citrus_usr.ufn_countstring(@l_slab_no,'|*~|')
						set @l_id = 1

						WHILE  @l_count  >= @l_id
						BEGIN
						--
        set @l_slab_no1 = citrus_usr.fn_splitval(@l_slab_no,@l_id) 
        ...
        IF @pa_chk_yn = 1 
								...
								--
										...
										--
												...
												SELECT top 1 @l_excsm_id = excsm_id from exch_seg_mstr  where excsm_id = @pa_excsm_cd order by 1 
