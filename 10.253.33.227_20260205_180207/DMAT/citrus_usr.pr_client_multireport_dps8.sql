-- Object: PROCEDURE citrus_usr.pr_client_multireport_dps8
...
CREATE proc [citrus_usr].[pr_client_multireport_dps8](@pa_id numeric  
   ...
   begin   
     ...
     begin
			  			...
			  			set @l_filter_hiercy =  citrus_usr.fn_splitval_row(@pa_hiercy,@l_counter)  
							...
							from @l_hiercy  tmp , enttm_entr_mapping 
							, entity_mstr entm where ENTEM_ENTTM_CD = cd and entm_short_name = shortname  
		...
		--  if citrus_usr.fn_splitval(@pa_main_filters_values ,1) <> '' 
							...
							if citrus_usr.fn_splitval(@pa_main_filters_values ,9) <> '' 
							AND citrus_usr.fn_splitval(@pa_main_filters_values ,10) <> ''    
							begin
								if citrus_usr.fn_splitval(@pa_main_filters_values ,11) in( '05','04')
								begin
							...
							set @l_sql5 = @l_sql5 + ' order by ' + case when  @pa_grp_by ='BR'   then ' Groupby , ' 
else '' end + '[' + (select select_desc from tbl_select_criteria 
where id = citrus_usr.fn_splitval(@pa_main_filters_values ,12)) + '] '  
