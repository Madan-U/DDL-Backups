-- Object: PROCEDURE citrus_usr.PR_TRATM_INS15
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

CREATE    PROCEDURE [citrus_usr].[PR_TRATM_INS15]
                AS
                BEGIN
                  --
                    DECLARE
                       ...
                       ,@l_sub_cd    VARCHAR(50)
                        SET  @I =1
                        SET @l_name ='HO'
                        SET @l_code='CLOSURE_INI_BY_NSDL' 
                        SET @l_desc ='CLOSURE INITIALIZED BY'

               

                     
                      SELECT @l_excm_id = excm_id FROM exchange_mstr where excm_cd='NSDL'
                      DELETE FROM TRANSACTION_SUB_TYPE_MSTR WHERE TRASTM_TRATM_ID=(SELECT trantm_id FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'CLOSURE_INI_BY_NSDL')
		      DELETE FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'CLOSURE_INI_BY_NSDL'
		  ...
		  ,trantm_excm_id
		   ...
		   BEGIN
		    ...
		    BEGIN
		      --
                        SET @l_sub_cd   = '1'
                        SET @l_sub_desc = 'INITIALIZED BY BO'
		      --      
		    END 
		    ELSE IF @I = 2
		    BEGIN
		      --
			SET @l_sub_cd   = '2'
			SET @l_sub_desc = 'INITIALIZED BY DP'
		      --      
		    END 
