-- Object: PROCEDURE citrus_usr.PR_BILLING_BO_CSV
...
CREATE  PROCEDURE [citrus_usr].[PR_BILLING_BO_CSV]  
...
,case when FINA_ACC_NAME  = 'AMC CHARGES  LIFE SCHEME 1250' then 'Lifetime Account Maintenance Charges' 
--else 'Demat account monthly maintenance charges' + ' Dt : '    end +  convert(varchar(11),MAX(CLIC_TRANS_DT ),109) FINA_ACC_NAME--CASE DATEPART(QUARTER, MAX(CLIC_TRANS_DT ))WHEN 1 THEN'Q4'WHEN 2 THEN'Q1'WHEN 3 THEN'Q2'WHEN 4 THEN'Q3'END ,109)
else 'Demat account maintenance charges dated' + ' : '     end +  convert(varchar(11),MAX(CLIC_TRANS_DT ),109) FINA_ACC_NAME--CASE DATEPART(QUARTER, MAX(CLIC_TRANS_DT ))WHEN 1 THEN'Q4'WHEN 2 THEN'Q1'WHEN 3 THEN'Q2'WHEN 4 THEN'Q3'END ,109)
 ,'1'						
...
if @PA_LOGIN_NAME='EQCR'
						...
						--,'Demat account monthly maintenanace charges' + ' for BOID : ' +  DPAM_SBA_NO + ' Dt : ' +  convert(varchar(11),MAX(CLIC_TRANS_DT ),109)  FINA_ACC_NAME
						,case when FINA_ACC_NAME  = 'AMC CHARGES  LIFE SCHEME 1250' then 'Lifetime Account Maintenance Charges'
						--else 'Demat account monthly maintenance charges' + ' Dt : '  end  +  convert(varchar(11),MAX(CLIC_TRANS_DT ),109)   FINA_ACC_NAME
						else 'Demat account maintenance charges dated' + ' : '    end  +  convert(varchar(11),MAX(CLIC_TRANS_DT ),109)   FINA_ACC_NAME
						 ,'1'
						FROM   CLIENT_CHARGES_CDSL a WITH(NOLOCK)
						--LEFT OUTER JOIN ACCOUNT_PROPERTIES ON ACCP_CLISBA_ID = CLIC_DPAM_ID AND ACCP_ACCPM_PROP_CD = 'BBO_CODE' 
						--AND ACCP_DELETED_IND = 1 
