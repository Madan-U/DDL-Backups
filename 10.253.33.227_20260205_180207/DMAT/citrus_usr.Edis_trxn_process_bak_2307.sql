-- Object: PROCEDURE citrus_usr.Edis_trxn_process_bak_2307
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

Create Proc [citrus_usr].[Edis_trxn_process_bak_2307] (@date datetime)
as
...
WHILE (@ID3<=@CNT3)
	  ...
	  BEGIN   --select * from #pld1
			 ...
			 SELECT @ISIN3=ISIN,@QTY3=adj_qty ,@NEWID3 =dp_id ,@PARTY_CODE=party_code FROM #pld1  WHERE SR_NO =@ID3  
				...
				SELECT ROW_NUMBER() OVER (ORDER BY request_Date asc) SR_NO , * 
