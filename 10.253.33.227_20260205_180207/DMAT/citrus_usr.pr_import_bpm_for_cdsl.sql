-- Object: PROCEDURE citrus_usr.pr_import_bpm_for_cdsl
...
--[pr_import_bpm_for_cdsl] 'nsdl','MIG','BULK','C:\BULKINSDBFOLDER\BP MASTER\BPfull.txt','*|~*','|*~|',''  
--
--select * from dp_mstr order by 1 desc
CREATE procedure   [citrus_usr].[pr_import_bpm_for_cdsl]( @pa_exch          VARCHAR(20)    
...
--  
 ...
 /* UPDATE entity_mstr     
  ...
  SELECT @l_count = COUNT(*) from tmp_bp_mstr WHERE tmpbp_id NOT IN(SELECT entm_short_name FROM entity_mstr)  AND  tmpbp_role IN ('01')   
  ...
  INSERT INTO #temp_sh_name SELECT distinct tmpbp_id FROM tmp_bp_mstr WHERE  tmpbp_role IN ('01')   
  ...
  ,case when tmpbp_role = '01' then '03' else 'SR' end  
  ,'NRM'  
  ,null  
  ,''  
  ,@pa_login_name    
  ...
  SELECT @l_count = COUNT(*) from tmp_bp_mstr WHERE tmpbp_id NOT IN(SELECT entm_short_name FROM entity_mstr)  AND  tmpbp_role IN ('05')   
  ...
  INSERT INTO #temp_sh_name1 SELECT distinct tmpbp_id FROM tmp_bp_mstr WHERE  tmpbp_role IN ('05')   
  ...
  INSERT INTO #temp_sh_name SELECT tmpbp_bp_nm FROM tmp_bp_mstr WHERE  tmpbp_role IN ('01','05')   */
  -----dp_mstr   
  UPDATE dp_mstr  
  ...
  INSERT INTO #temp_dp_name SELECT distinct tmpbp_id FROM tmp_bp_mstr WHERE  tmpbp_role IN ('04','1','3')   
  ...
  AND   tmpbp_role IN ('04','4','1','3') group by tmpbp_id,id1  
  ...
  INSERT INTO #temp_dp_name SELECT tmpbp_id FROM tmp_bp_mstr WHERE  tmpbp_role IN ('04','3')   
  ...
  SELECT IDENTITY(INT, 1, 1) ID1, cc_cd INTO #tmp_identity_cc_name FROM #temp_cc_name  
