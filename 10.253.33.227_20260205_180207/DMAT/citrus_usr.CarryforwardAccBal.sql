-- Object: PROCEDURE citrus_usr.CarryforwardAccBal
...
--begin tran
--select * from dp_mstr 
--CarryforwardAccBal 12062100,'HO'  
--select sum(ldg_amount) from ledger3 -- 16255
--select * from ledger4 where ldg_account_id=20 and ldg_deleted_ind=1
--select * from ledger2 where ldg_account_id=16
...
--)
--commit -- rollback
CREATE Proc [citrus_usr].[CarryforwardAccBal]  
@pa_dpmid varchar(10),  
...
select @@profitlossacc = isnull(convert(varchar,fina_acc_id),'99999999999999999') from fin_account_mstr where fina_acc_name = 'P&L ACCOUNT' and fina_deleted_ind = 1
 
select @@dpm_id = '3'--dpm_id from dp_mstr where default_dp=dpm_excsm_id and dpm_deleted_ind=1 and dpm_dpid=@pa_dpmid
  

Declare fincursor cursor  for  
select prev_fin_id=Convert(varchar,fin_id),  
next_fin_id =isnull((SELECT top 1 Convert(varchar,fin_id) from Financial_Yr_Mstr f1 where f1.fin_deleted_ind = 1 and f1.fin_dpm_id = f.fin_dpm_id and f1.fin_start_dt > f.fin_start_dt order by f1.fin_start_dt),'COMPLETE')  
from Financial_Yr_Mstr f  
--where fin_dpm_id = @pa_dpmid   
where fin_dpm_id = @@dpm_id
and fin_start_dt >=   
isnull((select Min(fin_start_dt) from Financial_Yr_Mstr where fin_dpm_id = @@dpm_id  and isnull(fin_cf_balances,'N') = 'Y' and fin_deleted_ind = 1),'Dec 31 2100')   
...
begin  
 ...
 begin  
        ...
        select @@next_start_dt= convert(varchar(11),fin_start_dt,109),@@next_end_dt= convert(varchar(11),fin_end_dt,109) 
        from Financial_Yr_Mstr with(nolock) where fin_id = @@next_fin_id  and fin_deleted_ind = 1
    
  
  set @@ssql = 'update ledger' + @@next_fin_id + ' set LDG_DELETED_IND = 0 where LDG_VOUCHER_DT = '''
   + @@next_start_dt + ''' and LDG_VOUCHER_NO = 0 and ldg_trans_type = ''O'''
   print @@ssql 
   
  exec(@@ssql)  
  --  select ldg_trans_type,* from ledger4 where ldg_voucher_dt = 'Apr 19 2017'

  --
  SET @@SSQL = 'select IDENTITY(BIGINT,1,1) ID,LDG_DPM_ID,LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''0'',LDG_VOUCHER_NO=0,LDG_SR_NO=0,LDG_REF_NO='''',LDG_VOUCHER_DT=''' + @@next_start_dt + ''',LDG_ACCOUNT_ID,LDG_ACCOUNT_TYPE,sum(isnull(LDG_AMOUNT,0)) LDG_AMOUNT,LDG_NARRATION=''Opening Balance'',LDG_BANK_ID=null,LDG_ACCOUNT_NO=null,LDG_INSTRUMENT_NO=null,LDG_BANK_CL_DATE=null,LDG_COST_CD_ID=null,LDG_BILL_BRKUP_ID=null,LDG_TRANS_TYPE=''O'',LDG_STATUS=''A'',LDG_CREATED_BY=''' + @pa_loginname+ ''',LDG_CREATED_DT=getdate(),LDG_LST_UPD_BY=''' + @pa_loginname+ ''',LDG_LST_UPD_DT=getdate(),LDG_DELETED_IND=1,LDG_BRANCH_ID=null  
...
,LDG_BILL_BRKUP_ID,LDG_TRANS_TYPE,LDG_STATUS,LDG_CREATED_BY,LDG_CREATED_DT,LDG_LST_UPD_BY,LDG_LST_UPD_DT,LDG_DELETED_IND,LDG_BRANCH_ID
 from ledger' + @@prev_fin_id + ' where ldg_account_type =''P'' and ldg_deleted_ind = 1 ) a 
  group by LDG_ACCOUNT_ID  , LDG_ACCOUNT_TYPE,LDG_DPM_ID
  ...
  print @@ssql
  exec(@@ssql)  
  --select * from ledger4 where ldg_voucher_dt = 'Apr 19 2017'
...
--  having sum(isnull(LDG_AMOUNT,0)) <> 0'  
  ...
  set @@ssql = 'insert into ledger' + @@next_fin_id + '(LDG_ID,LDG_DPM_ID,LDG_VOUCHER_TYPE,LDG_BOOK_TYPE_CD,LDG_VOUCHER_NO,LDG_SR_NO,LDG_REF_NO,LDG_VOUCHER_DT,LDG_ACCOUNT_ID,LDG_ACCOUNT_TYPE,LDG_AMOUNT,LDG_NARRATION,LDG_BANK_ID,LDG_ACCOUNT_NO,LDG_INSTRUMENT_NO,LDG_BANK_CL_DATE,LDG_COST_CD_ID,LDG_BILL_BRKUP_ID,LDG_TRANS_TYPE,LDG_STATUS,LDG_CREATED_BY,LDG_CREATED_DT,LDG_LST_UPD_BY,LDG_LST_UPD_DT,LDG_DELETED_IND,LDG_BRANCH_ID)  
  select 0,LDG_DPM_ID=' + @@dpm_id + ',LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''0'',LDG_VOUCHER_NO=0,LDG_SR_NO=0,LDG_REF_NO='''',LDG_VOUCHER_DT=''' + @@next_start_dt + ''',LDG_ACCOUNT_ID=' + isnull(@@profitlossacc,'0') + ',''G'',sum(isnull(LDG_AMOUNT,0)),LDG_NARRATION=''Opening Balance'',LDG_BANK_ID=null,LDG_ACCOUNT_NO=null,LDG_INSTRUMENT_NO=null,LDG_BANK_CL_DATE=null,LDG_COST_CD_ID=null,LDG_BILL_BRKUP_ID=null,LDG_TRANS_TYPE=''O'',LDG_STATUS=''A'',LDG_CREATED_BY=''' + @pa_loginname+ ''',LDG_CREATED_DT=getdate(),LDG_LST_UPD_BY=''' + @pa_loginname+ ''',LDG_LST_UPD_DT=getdate(),LDG_DELETED_IND=1,LDG_BRANCH_ID=null  
  from ledger' + @@prev_fin_id + ' with(nolock) left outer join fin_account_mstr with(nolock) on ldg_account_id = fina_acc_id  
  where LDG_DELETED_IND = 1 and fina_deleted_ind = 1 and ((citrus_usr.Fn_ChildExists(' + @@incomegroup + ',fina_group_id) = 1 or citrus_usr.Fn_ChildExists(' + isnull(@@expensegroup,'0') + ',fina_group_id) = 1) --or LDG_ACCOUNT_ID = ' + isnull(@@profitlossacc,'0') + '
