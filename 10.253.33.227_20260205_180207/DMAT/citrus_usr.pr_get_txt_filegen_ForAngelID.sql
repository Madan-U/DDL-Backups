-- Object: PROCEDURE citrus_usr.pr_get_txt_filegen_ForAngelID
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------


--pr_get_txt_filegen_ForAngelID 'sUMMARY','12033200','1203320008431140','1203320008431140','jUL  1 2017','JUL 31 2017'
CREATE PROC [citrus_usr].[pr_get_txt_filegen_ForAngelID](@PA_ACTION VARCHAR(100),@PA_DPID VARCHAR(100)    
,@PA_BOID VARCHAR(100)    
,@PA_ISIN VARCHAR(100)    
...
CREATE NONCLUSTERED INDEX ABC ON #ACCOUNT_PROPERTIES_ECN (ACCP_CLISBA_ID)
...
CDSHM_TRAS_DT  BETWEEN CITRUS_USR.UFN_GETFIRSTDAYOFMONTH(@PA_FROM_DATE) AND @PA_TO_DATE
 ...
 AND CDSHM_TRATM_CD IN('2246','2277','2201','3102','2270','2220','2262','2251','2202','2205','2280','5101','2230') -- ,'5101','2230'
)  A 
...
--AND ACCP_ACCPM_PROP_CD ='ECN_FLAG')
  ...
  WHERE DPHMC_HOLDING_DT = @PA_TO_DATE
	...
	truncate table citrus_usr.FILEGEN_FORANGELID_holding_MIMANSA

     INSERT INTO citrus_usr.FILEGEN_FORANGELID_holding_MIMANSA
  ...
  exec [pr_get_ob_fix] 3,@pa_from_date,@pa_to_date,'0','9999999999999999',''
   ...
   and CDSHM_TRAS_DT between @pa_from_date and @pa_to_date   
    ...
    and CDSHM_TRATM_CD in  ('2246','2277','2201','3102','2270','2220','2205','5101','2280','2230')    --,'5101','2280','2230'
    ...
    from #tempfinal  order by id 
	 ...
	 truncate table citrus_usr.FILEGEN_FORANGELID_TRX_MIMANSA

     INSERT INTO citrus_usr.FILEGEN_FORANGELID_TRX_MIMANSA
  ...
  DROP TABLE  ##BILLED_RE
   ...
   truncate table citrus_usr.FILEGEN_FORANGELID_ledger_MIMANSA

   INSERT INTO citrus_usr.FILEGEN_FORANGELID_ledger_MIMANSA
  ...
  --PRINT @PA_ACTION     
   ...
   truncate table citrus_usr.FILEGEN_FORANGELID_SUMMARY_MIMANSA
 ...
 -- GROUP BY DPAM_SBA_NO
   ...
   INSERT INTO citrus_usr.FILEGEN_FORANGELID_SUMMARY_MIMANSA
     
   SELECT DISTINCT ISNULL(DPAM_SBA_NO,'') BOID ,   
...
WHEN  ISNULL(CLIC_CHARGE_NAME,'')='CGST @9%' THEN 'CGST'
WHEN  ISNULL(CLIC_CHARGE_NAME,'')='SGST @9%' THEN 'SGST'
WHEN  ISNULL(CLIC_CHARGE_NAME,'')='IGST @18%' THEN 'IGST'

 ELSE ISNULL(CLIC_CHARGE_NAME,'') END DESCRIPTION ,     
   +CONVERT(VARCHAR,ISNULL(CONVERT(NUMERIC(18,4),SUM(CLIC_CHARGE_AMT)),'0')*-1) AMOUNT ,
   --+CONVERT(VARCHAR,ISNULL(CONVERT(NUMERIC(18,4),SUM(AMT)),'0')*-1) 
   --,+CONVERT(VARCHAR,ISNULL(ABS(convert(numeric(18,4),SUM(CGSTAMT) )),0)*-1)
   --,+CONVERT(VARCHAR,ISNULL(ABS(convert(numeric(18,4),SUM(SGSTAMT))),0)*-1)
   --,+CONVERT(VARCHAR,ISNULL(ABS(convert(numeric(18,4),SUM(IGSTAMT))),0)*-1)
