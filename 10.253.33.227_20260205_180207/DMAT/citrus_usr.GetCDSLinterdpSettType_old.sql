-- Object: FUNCTION citrus_usr.GetCDSLinterdpSettType_old
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

CREATE function [citrus_usr].[GetCDSLinterdpSettType_old](@pa_mode char(1),@pa_settno varchar(10),@pa_setttype varchar(10)) returns varchar(3)  
as  
begin  
declare @@outsetttype varchar(3),  
@@exchange varchar(3)  
 select @@exchange = case when left(ltrim(rtrim(@pa_settno)),1) = '2' then 'NSE'   
      when left(ltrim(rtrim(@pa_settno)),1) = '2' then 'BSE'   
      else '' end  
 ...
 begin  
  select @@outsetttype = case when @@exchange = 'NSE' then   
         ...
         when @pa_setttype = 'NA' then 'A'  
         when @pa_setttype = 'TT' then 'W'  
        ...
        when @@exchange = 'BSE' then   
         ...
         when @pa_setttype = 'NA' then '111001'  --AR  
 ...
 begin  
  ...
  select @@outsetttype = case when @@exchange = 'NSE' then   
        ...
        when @@exchange = 'BSE' then   
         ...
         when @pa_setttype = '111001' then 'NA'  --AR  
