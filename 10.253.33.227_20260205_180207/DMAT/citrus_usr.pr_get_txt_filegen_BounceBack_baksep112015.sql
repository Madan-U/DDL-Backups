-- Object: PROCEDURE citrus_usr.pr_get_txt_filegen_BounceBack_baksep112015
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

create proc [citrus_usr].[pr_get_txt_filegen_BounceBack_baksep112015](@pa_action varchar(100),@pa_dpid varchar(100)    
,@pa_boid varchar(100)    
,@pa_isin varchar(100)    
...
INTO #ACCOUNT_PROPERTIES_ECN 
FROM dp_acct_mstr,TMP_BOUNCEBACK_MSTR
where DPAM_SBA_NO=[Client  id]
and from_dt=@pa_from_date and to_dt=@pa_to_date
...
--and ACCP_ACCPM_PROP_CD ='ECN_FLAG')
  ...
  where dphmc_holding_dt = @pa_to_date
     ...
     +'|'+convert(varchar,0)    
     +'|'+CONVERT(varchar,0)  
     +'|'+convert(varchar,convert(numeric(18,2),DPHMCD_free_QTY) )           
     +'|'+CONVERT(varchar,0)    
     +'|'+CONVERT(varchar,0)    
     +'|'+CONVERT(varchar,0)    
     +'|'+CONVERT(varchar,0)    
     ...
     and exists(select 1 from client_charges_cdsl where CLIC_DPAM_ID = DPHMCD_DPAM_ID 
  ...
  create table #ob(ob_cdshm_dpm_id numeric,ob_cdshm_dpam_id numeric,ob_cdshm_isin varchar(100),ob numeric(18,2))
  insert into #ob
  exec [pr_get_ob_fix_latest] 3,@pa_from_date,@pa_to_date,'0','9999999999999999',''
  
   
     
      
   select identity(numeric,1,1) id,cdshm_dpam_id ,cdshm_isin , convert(numeric(18,2),cdshm_qty) cdshm_qty, ISNULL(convert(numeric(18,2),ob),0) ob ,cdshm_tras_dt,  cdshm_ben_acct_no +'|'    
   ...
   +'|'+convert(varchar,abs(convert(numeric(18,2),cdshm_qty)))--1000    
   +'|'+convert(varchar,'$$$$')  
   ...
   and CDSHM_TRAS_DT between @pa_from_date and @pa_to_date   
    and exists(select 1 from client_charges_cdsl where CLIC_DPAM_ID = CDSHM_DPAM_ID  and CLIC_TRANS_DT between @pa_from_date and @pa_to_date )  
    ...
    and t2.id<=t1.id
    )  closingbalance   into #tempfinal from #tempdata t1 
    
    select --*,closingbalance-cdshm_qty openingbalance , 
    replace(replace(value,'$$$$',closingbalance),'####',closingbalance-cdshm_qty) as value
  ...
  select @sql  = @sql + ' and exists (select 1 from client_charges_cdsl where clic_dpam_id = dpam_id and CLIC_TRANS_DT between '''+ CONVERT(varchar(11),@pa_from_date,109) + ''' and  ''' +  CONVERT(varchar(11),@pa_to_date,109)
  ...
  select @sql22  = @sql22 + ' and exists (select 1 from client_charges_cdsl where clic_dpam_id = dpam_id and CLIC_TRANS_DT between '''+ CONVERT(varchar(11),@pa_from_date,109) + ''' and  ''' +  CONVERT(varchar(11),@pa_to_date,109)
  ...
  AND CLIC_CHARGE_NAME ='SERVICE TAX' GROUP BY DPAM_SBA_NO
    ...
    +'|'+EMailId      
    +'|'+isnull(citrus_usr.Fn_Ucc_Brom(dpam_id),'')--LIFE INVESTOR    
    +'|'+ CONVERT(VARCHAR,ISNULL(ABS(convert(numeric(18,2),AMT)),0))--curr bill amt (select top 1 convert(varchar,convert(numeric(18,2),SUM(clic_charge_amt)*-1)) from client_charges_cdsl where CLIC_DPAM_ID=dpam_id and CLIC_TRANS_DT between @pa_from_date and @pa_to_date and CLIC_CHARGE_NAME<>'SERVICE TAX')--11    
    +'|'+ CONVERT(VARCHAR,ISNULL(ABS(convert(numeric(18,2),STAMT)),0))--curr bill st (select top 1 convert(varchar,convert(numeric(18,2),SUM(clic_charge_amt)*-1)) from client_charges_cdsl where CLIC_DPAM_ID=dpam_id and CLIC_TRANS_DT between @pa_from_date and @pa_to_date and CLIC_CHARGE_NAME='SERVICE TAX')--11    --1.3596    
    ...
    +'|'+ CONVERT(VARCHAR,ISNULL(ABS(convert(numeric(18,2),OS22)),0)+ISNULL(ABS(convert(numeric(18,2),STAMT)),0)+ISNULL(ABS(convert(numeric(18,2),AMT)),0))  -- net amount convert(varchar,((select top 1 convert(numeric(18,2),SUM(clic_charge_amt)*-1) from client_charges_cdsl where CLIC_DPAM_ID=dpam_id and CLIC_TRANS_DT between @pa_from_date and @pa_to_date ) -     (select top  1 SUM(ldg_amount) from ledger2 where LDG_ACCOUNT_ID=dpam_id and LDG_DELETED_IND=1 and LDG_VOUCHER_DT<=@pa_to_date) ))--(select top 1 convert(numeric(18,2),SUM(clic_charge_amt))-convert(numeric(18,2),SUM(ldg_amount)) from client_charges_cdsl,ledger2 where CLIC_DPAM_ID=dpam_id and clic_dpam_id=ldg_account_id and ldg_deleted_ind=1 and ldg_voucher_dt<=@pa_to_date and CLIC_TRANS_DT between 'JUL  1 2015' and 'JUL 31 2015')--12.3596    
    +'|'+convert(varchar(8),convert(datetime,@pa_to_date,103),112)--20150228    
    +'|'+convert(varchar(8),convert(datetime,@pa_to_date,103),112)--20150228    
    +'|'+convert(varchar(8),convert(datetime,@pa_from_date,103),112) + '-' + convert(varchar(8),convert(datetime,@pa_to_date,103),112)--20150201-20150228    
    ...
    and exists(select 1 from client_charges_cdsl where CLIC_DPAM_ID = DPAM_ID  and CLIC_TRANS_DT between @pa_from_date and @pa_to_date )   
    and not exists (select client_code from angelownaccount where client_code=dpam_sba_no)
  ...
  select @sql1  = @sql1 + ' and exists (select 1 from client_charges_cdsl where clic_dpam_id = dpam_id and CLIC_TRANS_DT between '''+ CONVERT(varchar(11),@pa_from_date,109) + ''' and  ''' +  CONVERT(varchar(11),@pa_to_date,109)
  ...
  select @sql11  = @sql11 + ' and exists (select 1 from client_charges_cdsl where clic_dpam_id = dpam_id and CLIC_TRANS_DT between '''+ CONVERT(varchar(11),@pa_from_date,109) + ''' and  ''' +  CONVERT(varchar(11),@pa_to_date,109)
...
when  isnull(CLIC_CHARGE_NAME,'')='DEMAT REJECTION CHARGE' then 'DEMAT CHARGES'
