-- Object: PROCEDURE citrus_usr.pr_import_dp_cdsl_Mod_EmailbulkApr2115
...
--CDSL	01/05/2008	02/05/2008	N	45	3	HO	
create PROCeDURE [citrus_usr].[pr_import_dp_cdsl_Mod_EmailbulkApr2115]( @pa_crn_no      VARCHAR(8000)                  
                            , @pa_exch        VARCHAR(10)                  
    ...
    --                  
      ...
      --                    
        ...
        --                  
          ...
          --INSERT INTO @crn                   
          --SELECT clim_crn_no,dpam_sba_no, clim_stam_cd, clim_created_dt, clim_lst_upd_dt                  
          --FROM   client_mstr WITH (NOLOCK)  ,client_list_modified with (nolock)   , dp_acct_mstr with(nolock)             
          --WHERE   CONVERT(varchar(11),clic_mod_from_dt,109)='Apr 18 2015'
          --and clic_mod_dpam_sba_no=dpam_sba_no and clim_crn_no=DPAM_CRN_NO and DPAM_DELETED_IND=1
         --                  
        --                  
        END                  
      --                    
...
,send_ref_no1            
   ...
   --         WHERE  clim_crn_no = DPAM_CRN_NO and DPAM_SBA_NO=clic_mod_dpam_sba_no and CONVERT(varchar(11),clic_mod_from_dt,109)='Apr 18 2015'
      ...
      BEGIN                  
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
                                ...
                                , '' --bo middle name
                                , '' -- bo seach name
                                , MAX(case when clic_mod_action = 'GENERAL' then case when CLIM_GENDER in ('M','MALE') then 'MR'  when CLIM_GENDER in ('F','FEMALE') then 'MS' else 'M/S' end else '' end)  --bo title
                                , '' --bo suffix                 
                                , MAX(case when clic_mod_action = 'GENERAL' then ISNULL(dphd_fh_fthname,'') else '' end)  --father/husband name                  
                                ...
                                ,''--MAX(case when clic_mod_action = 'Contacts' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,1,'Y'),'') else '' end)  --CUST PH 1 INDC
                                ,''-- MAX(case when clic_mod_action = 'Contacts' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,1,'N'),'') else '' end) --convert(varchar(17),isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'RES_PH1'),''))--ph1                  
                                --, isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'Y'),'')  --'0' CUST PH 2 INDC                 
                                ,''-- MAX(case when clic_mod_action = 'Contacts' then case when isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'N'),'') = '' then '' else isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'Y'),'') end else '' end) --'0' CUST PH 2 INDC                 
                                ,''-- MAX(case when clic_mod_action = 'Contacts' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'N'),'') else '' end)  --isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'RES_PH2'),'')--ph2                  
                                ,''-- MAX(case when clic_mod_action = 'Contacts' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,3,'N'),'') else '' end)  --CUST ADDL PHONE             
								...
								, '' --IT CIRCLE
                                , MAX(case when clic_mod_action = 'Contacts' then CASE WHEN lower(isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'')) <> '' THEN lower(isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'')) ELSE lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'EMAIL1'),'')) END else '' end) --email                  
                                , '' --USER TEXT 1                 
                                , '' --USER TEXT 2                 
                                , '0000' --USER FIELD 3                  
                                , ''                  
                           ...
                           AND    climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
						   AND	  climmod.clic_mod_action in ('C Address','Client Main Properties','General','Signature','Contacts','Bank','PAN DETAILS')
...
----                         and    climmod.clic_mod_deleted_ind = 1
               ...
               insert into @line5( ln_no                   
                                       ...
                                       ,case when clic_mod_action = 'GENERAL' then (case when dpam_subcm_cd in ('2156','2155','2150','082104') then case when isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'INC_DOB',''),'') = '' then '00000000' else isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'INC_DOB',''),'00000000') end  else case when isnull(convert(varchar(11),clim.clim_dob,103),'00000000') = '01/01/1900' then '00000000' else   isnull(convert(varchar(11),clim.clim_dob,103),'00000000')  end   end) ELSE space(8) END
                                       --, isnull(clim.clim_gender,'')  --SEX CODE                
                                       ,case when clic_mod_action = 'GENERAL' then (case when citrus_usr.FN_FORMATSTR(convert(varchar(4),right(subcm_cd,2)),4,0,'L','0') = '0004' then SPACE(1) else isnull(clim.clim_gender,'') end) else '' end
									   ...
									   ,case when clic_mod_action = 'INCOME DETAILS' then citrus_usr.fn_get_listing('ANNUAL_INCOME',isnull(citrus_usr.fn_ucc_entp(clim_crn_no,'ANNUAL_INCOME',''),'')) else SPACE(4) end--isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,''),'')   --ann_income_cd                  
									   ...
									   ,case when clic_mod_action = 'Client Main Properties' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'RGESS_FLAG',''),'')='NO' THEN 'N' ELSE 'Y' END) else '' end --RGESS FLAG
									   , '' --ANNUAL REPORT FLAG
									   ...
									   , case when clic_mod_action = 'Client Main Properties' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BSDA',''),'') ='NO' THEN 'N' ELSE 'Y' END) else '' end--BSDA FLAG
...
--                                     , left(enttm_cd,2)  
                                       ...
                                       , case when clic_mod_action = 'BANK' then (case when banm_rtgs_cd in ('0','000000000')  then space(15) 
...
--                                       , isnull(citrus_usr.fn_ucc_accpd(dpam.dpam_id,'SMART_CARD_REQ','SMART_CARD_PIN',''),'0000000000')--isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'',''),'')  --smart_crd_pin                            
									   ...
									   , case when clic_mod_action = 'SUB STATUS' then citrus_usr.FN_FORMATSTR(convert(varchar(4),right(subcm_cd,2)),4,0,'L','0') else '' end
...
--                                       , '000000'--citrus_usr.fn_get_listing('BANKCCY',isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BANKCCY',''),''))               
								  ...
								  AND   climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
								  and	isnull(climmod.clic_mod_batch_no,0) = 0
								  AND	  climmod.clic_mod_action in ('Client Main Properties','Bank','GENERAL','INCOME DETAILS','SUB STATUS')
                                 ...
                                 insert into  @line7(ln_no                                    
                                  ...
                                  ORDER BY Purpose_code  DESC 
											...
											SELECT distinct '08'                  
														...
														,CASE WHEN CITRUS_USR.fn_acct_conc_value(DPAM_ID,'SMSMOB')<> '' THEN CITRUS_USR.fn_acct_conc_value(DPAM_ID,'SMSMOB')
																				ELSE CITRUS_USR.fn_conc_value(DPAM_CRN_NO,'SMSMOB') END
														
														--,CASE WHEN (select mobile_no from client_main_properties where dpam_sba_no = BOid) <> (SELECT CITRUS_USR.fn_conc_value(DPAM_CRN_NO,'SMSMOB'))
														--THEN (CASE WHEN CITRUS_USR.fn_acct_conc_value(DPAM_ID,'SMSMOB')<> '' THEN CITRUS_USR.fn_acct_conc_value(DPAM_ID,'SMSMOB')
														--						ELSE CITRUS_USR.fn_conc_value(DPAM_CRN_NO,'SMSMOB') END) ELSE '' END
														...
														FROM   dp_acct_mstr              dpam ,client_list_modified  climmod   ,account_properties accp,EXCH_SEG_MSTR esm      , @crn                       crn           
														 ...
														 AND   climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
                                                     and   (dpam_sba_no =  clic_mod_dpam_sba_no or case when clic_mod_pan_no <> '' then citrus_usr.fn_ucc_entp(dpam_crn_no,'pan_gir_no','') else '0' end = clic_mod_pan_no)              
                                                     AND	  climmod.clic_mod_action in ('Contacts')
                                                     and	isnull(climmod.clic_mod_batch_no,0) = 0
                                                    and    climmod.clic_mod_deleted_ind = 1
      ...
      print 'kkkkkk' 
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
                                ...
                                , ''--MAX(case when clic_mod_action = 'Contacts' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,1,'N'),'') else '' end)  --convert(varchar(17),isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'RES_PH1'),''))--ph1                  
                                --, isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'Y'),'')  --'0' CUST PH 2 INDC                 
                                , ''--MAX(case when clic_mod_action = 'Contacts' then case when isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'N'),'') = '' then '' else isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'Y'),'') end else '' end)
                                , ''--MAX(case when clic_mod_action = 'Contacts' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'N'),'') else '' end)  --isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'RES_PH2'),'')--ph2                  
                                , ''--MAX(case when clic_mod_action = 'Contacts' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,3,'N'),'') else '' end) --CUST ADDL PHONE             
                                , MAX(case when clic_mod_action = 'Contacts' then CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCFAX1'),'') <> '' THEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCFAX1'),'') ELSE isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'FAX1'),'') END else '' end)--fax                  
                                --, MAX(case when clic_mod_action in ('CLIENT MAIN PROPERTIES','GENERAL') then isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'PAN_GIR_NO',''),'') else '' end)  --pan card                  
...
and not exists (select 1 from @line2 o where o.acct_no = dpam_sba_no and o.crn_no  = clim_crn_no )
                       ...
                       insert into @line5( ln_no                   
                                       ...
                                       , ''--citrus_usr.fn_get_listing('EDUCATION',isnull(citrus_usr.fn_ucc_entp(clim_crn_no,'EDUCATION',''),''))    --edu                  
                                       --, SPACE(4) --'0000'--citrus_usr.fn_get_listing('ANNUAL_INCOME',isnull(citrus_usr.fn_ucc_entp(clim_crn_no,'ANNUAL_INCOME',''),''))--isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,''),'')   --ann_income_cd                  
									   ,case when clic_mod_action = 'INCOME DETAILS' then citrus_usr.fn_get_listing('ANNUAL_INCOME',isnull(citrus_usr.fn_ucc_entp(clim_crn_no,'ANNUAL_INCOME',''),'')) else SPACE(4) end
                                       , ''--citrus_usr.fn_get_listing('NATIONALITY',isnull(citrus_usr.fn_ucc_entp(clim_crn_no,'NATIONALITY',''),''))   --nat_cd                  
                                       --, CASE WHEN citrus_usr.fn_ucc_entp(clim.clim_crn_no,'legal_sta','')='' THEN '00' ELSE isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'legal_sta',''),'00') END  --legal_status_cd                  
                                       , SPACE(2)--'00' --LEGAL STATUS CODE
                                       --, CASE WHEN citrus_usr.fn_ucc_entp(clim.clim_crn_no,'bofeetype','')='' THEN '00' ELSE isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'bofeetype',''),'00') END    --bo_fee_type                  
									   ...
									   ,case when clic_mod_action = 'Client Main Properties' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'RGESS_FLAG',''),'')='NO' THEN '' ELSE 'Y' END) else '' end --RGESS FLAG
									   , '' --ANNUAL REPORT FLAG
									   , '' --PLEDGE STANDING INSTRUCTION FLAG
									   , '' --EMAIL RTA DOWNLOAD FLAG
									   --, CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BSDA',''),'') ='NO' THEN 'N' ELSE 'Y' END--BSDA FLAG
...
or dpam_sba_no in (select acct_no from @line8) 
      ...
      select distinct isnull(l2.sign_old_bo,'') sign_old_bo,isnull(l2.sign_bo,'') sign_bo  
			...
			+convert(char(15),l2.cust_uid) 		--Added by pankaj	                    
