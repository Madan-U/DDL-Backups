-- Object: PROCEDURE citrus_usr.pr_recon_holding_for_oneclient
...
create    proc [citrus_usr].[pr_recon_holding_for_oneclient](@pa_excsm_id numeric
, @pa_date datetime
, @pa_flag char(15)
,@pa_oneclient varchar (16)
...
exec [pr_get_holding_fix_latest_reco] @pa_dpm_id,@l_dt,@l_dt,@pa_oneclient,@pa_oneclient,''    
--
--
--if @pa_dpm_id <> 3 
--begin 
...
and CDSHM_BEN_ACCT_NO  =@pa_oneclient
...
select cdshm_ben_acct_no ac, cdshm_isin isin , sum(cdshm_qty  * -1 ) fill_qty  into #tempdata_forreco 
from cdsl_holding_dtls a where CDSHM_CDAS_SUB_TRAS_TYPE ='1103'  
and cdshm_tras_dt between getdate()-6 and getdate()
and exists (select 1 from cdsl_holding_dtls b where a.CDSHM_BEN_ACCT_NO = b.CDSHM_BEN_ACCT_NO 
and a.cdshm_isin = b.cdshm_isin and a.CDSHM_TRANS_NO =b.CDSHM_TRANS_NO 
and b.CDSHM_CDAS_SUB_TRAS_TYPE ='802'
and b.CDSHM_BEN_ACCT_NO = @pa_oneclient)
and CDSHM_BEN_ACCT_NO = @pa_oneclient
...
--		END
		...
		--IF NOT EXISTS (select top 1 a.* from #finalholding a left outer join #tempdata_forreco on ac = client_hldg  and isin = isin_hldg where edmat_hldg+case when edmat_hldg <> cdas_hldg then isnull(fill_qty,0)  else 0 end  <> cdas_hldg )
		--BEGIN
		--insert into cdsl_recon_log
		--select 'START' ,@pa_date,getdate(),'NO MISMATCH'
		--END 
...
--		select a.*, @pa_date  from #finalholding a left outer join #tempdata_forreco on ac = client_hldg  and isin = isin_hldg where edmat_hldg+case when edmat_hldg <> cdas_hldg then isnull(fill_qty,0)  else 0 end  <> cdas_hldg 


--insert into CDSL_recon_mismatch_data
--select *, @pa_date  from #finalholding where edmat_hldg <> cdas_hldg
