-- Object: PROCEDURE citrus_usr.pr_get_txt_filegen_BounceBack
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--PR_GET_TXT_FILEGEN_BOUNCEBACK 'summary','12033200','1203320008431140','1203320008431140','jUL  1 2017','JUL 31 2017'
CREATE proc [citrus_usr].[pr_get_txt_filegen_BounceBack](@pa_action varchar(100),@pa_dpid varchar(100)    
,@pa_boid varchar(100)    
,@pa_isin varchar(100)    
...
declare @pa_to_date_forholding   datetime
set @pa_to_date_forholding  = case when datename(dw ,@pa_to_date ) ='sunday' then @pa_to_date -1 else @pa_to_date end 
...
and CASE WHEN @pa_boid ='' THEN '1' ELSE DPAM_SBA_NO    END = CASE WHEN @pa_boid ='' THEN '1' ELSE @pa_boid END 
...
create clustered index ix_1 on #TMP_CLIENT_CHARGES_CDSL(clic_dpam_id,CLIC_TRANS_DT )
-- FOR ECNFLAG
...
--and ACCP_ACCPM_PROP_CD ='ECN_FLAG')
  ...
  if @pa_boid <> '' and not exists (select 1 from DP_HLDG_MSTR_CDSL_MONTHEND_HOLDING where dphmc_holding_dt=@pa_to_date_forholding)  
  ...
  if @pa_boid <> '' and  exists (select 1 from DP_HLDG_MSTR_CDSL_MONTHEND_HOLDING where dphmc_holding_dt=@pa_to_date_forholding)  
  insert into #holdingallforview(DPHMCD_DPM_ID   
  ,DPHMCD_DPAM_ID   
  ,DPHMCD_ISIN   
  ,DPHMCD_CURR_QTY   
  ...
  where dphmc_dpam_id = DPAM_ID and DPAM_SBA_NO = @pa_boid and  dphmc_holding_dt = @pa_to_date_forholding
  ...
  where dphmc_holding_dt = @pa_to_date_forholding
  ...
  AND CLIC_CHARGE_NAME NOT IN ('SERVICE TAX','CGST @9%','IGST @18%','SGST @9%') GROUP BY DPAM_SBA_NO
  ...
  AND CLIC_CHARGE_NAME in ('CGST @9%')
  ...
  AND CLIC_DELETED_IND = 1 
  --AND CLIC_CHARGE_NAME ='SERVICE TAX' 
  AND CLIC_CHARGE_NAME in ('SGST @9%')
  GROUP BY DPAM_SBA_NO
  ...
  AND CLIC_DELETED_IND = 1 
  --AND CLIC_CHARGE_NAME ='SERVICE TAX' 
  AND CLIC_CHARGE_NAME in ('IGST @18%')
  GROUP BY DPAM_SBA_NO
    ...
    +'|'+ ltrim(rtrim(isnull (entm_name1,'' ))) + '~' + replace (isnull (entm_short_name , ''),'_BR','') --PATNA BR-RIP LTD~PTN    
    +'|'+EMailId      
    ...
    from dp_acct_mstr with(nolock) left outer join entity_RElationship on dpam_sba_no = entr_sba and entr_from_dt between @pa_from_date and @pa_to_date and entr_deleted_ind = '1'  
    left outer join entity_mstr on entr_br = entm_id and  entm_deleted_ind = '1'
     LEFT OUTER JOIN  #billed_os ON OS_boid = DPAM_SBA_NO 
   ...
   -- LEFT OUTER JOIN  #STAMT ON CLIC_BO_ID_ST = DPAM_SBA_NO
    ...
    left outer join bo_bill_inv_mapping inv on inv.boid = dpam_sba_no
     and month(@pa_from_date) = billmonth and YEAR(@pa_from_date)= billyear
left outer join VW_BASELOCATION_MSTR v on   v.boid=dpam_sba_no
    , client_ctgry_mstr with(nolock), ENTITY_TYPE_MSTR with(nolock)
    , SUB_CTGRY_MSTR with(nolock), dps8_pc1 dps8_pc1  with(nolock) , STATUS_MSTR with(nolock)    
    ,#ACCOUNT_PROPERTIES_ECN
    
    where dps8_pc1.BOId = DPAM_SBA_NO and stam_cd = DPAM_STAM_CD    and ACCP_CLISBA_ID=dpam_id 
  ...
  create clustered index ix_1 on #billed_os_ledger(os_boid)  
 ...
 SELECT  DPAM_SBA_NO CLIC_BO_ID ,   SUM(CLIC_CHARGE_AMT) AMT INTO #BILLAMTSUMM FROM CLIENT_CHARGES_CDSL , DP_ACCT_MSTR 
  WHERE CLIC_TRANS_DT BETWEEN @pa_from_date and @pa_to_date 
  AND CLIC_DPAM_ID= DPAM_ID AND DPAM_DELETED_IND = 1
  AND CLIC_DELETED_IND = 1 
  AND CLIC_CHARGE_NAME NOT IN ('SERVICE TAX','CGST @9%','IGST @18%','SGST @9%') GROUP BY DPAM_SBA_NO
  ...
  -- SELECT DPAM_SBA_NO CLIC_BO_ID_CGST ,   SUM(CLIC_CHARGE_AMT) CGSTAMT INTO #CGSTAMTSUMM FROM CLIENT_CHARGES_CDSL , DP_ACCT_MSTR 
  --WHERE CLIC_TRANS_DT BETWEEN @pa_from_date and @pa_to_date 
  --AND CLIC_DPAM_ID= DPAM_ID AND DPAM_DELETED_IND = 1
  --AND CLIC_DELETED_IND = 1 
  ----AND CLIC_CHARGE_NAME ='SERVICE TAX' 
  ...
  --  SELECT DPAM_SBA_NO CLIC_BO_ID_SGST ,   SUM(CLIC_CHARGE_AMT) SGSTAMT INTO #SGSTAMTSUMM FROM CLIENT_CHARGES_CDSL , DP_ACCT_MSTR 
  --WHERE CLIC_TRANS_DT BETWEEN @pa_from_date and @pa_to_date 
  --AND CLIC_DPAM_ID= DPAM_ID AND DPAM_DELETED_IND = 1
  --AND CLIC_DELETED_IND = 1 
  ----AND CLIC_CHARGE_NAME ='SERVICE TAX' 
  ...
  --  SELECT DPAM_SBA_NO CLIC_BO_ID_IGST ,   SUM(CLIC_CHARGE_AMT) IGSTAMT INTO #IGSTAMTSUMM FROM CLIENT_CHARGES_CDSL , DP_ACCT_MSTR 
  --WHERE CLIC_TRANS_DT BETWEEN @pa_from_date and @pa_to_date 
  --AND CLIC_DPAM_ID= DPAM_ID AND DPAM_DELETED_IND = 1
  --AND CLIC_DELETED_IND = 1 
  ----AND CLIC_CHARGE_NAME ='SERVICE TAX' 
  ...
  --GROUP BY DPAM_SBA_NO
   ...
   select distinct isnull(dpam_sba_no,'')+'|'    
...
WHEN  ISNULL(CLIC_CHARGE_NAME,'')='CGST @9%' THEN 'CGST @9'
WHEN  ISNULL(CLIC_CHARGE_NAME,'')='SGST @9%' THEN 'SGST @9'
WHEN  ISNULL(CLIC_CHARGE_NAME,'')='IGST @18%' THEN 'IGST @18'
 else isnull(CLIC_CHARGE_NAME,'') end +'|'    
   +convert(varchar,isnull(convert(numeric(18,2),sum(clic_charge_amt)),'0')*-1)+'|'   
   --  +convert(varchar,isnull(convert(numeric(18,2),sum(AMT)),'0')*-1)+'|'   
   --+convert(varchar,isnull(convert(numeric(18,2),sum(CGSTAMT)),'0')*-1)+'|'
   --+convert(varchar,isnull(convert(numeric(18,2),sum(SGSTAMT)),'0')*-1)+'|'
   --+convert(varchar,isnull(convert(numeric(18,2),sum(IGSTAMT)),'0')*-1)+'|'
   --as value  
   from CLIENT_CHARGES_CDSL  with(nolock)    
     , #ACCOUNT_PROPERTIES_ECN  ,dp_acct_mstr with(nolock)  
   LEFT OUTER JOIN  #BILLAMTSUMM ON CLIC_BO_ID = DPAM_SBA_NO 
   
    --  LEFT OUTER JOIN  #CGSTAMTSUMM ON CLIC_BO_ID_CGST = DPAM_SBA_NO 
    --LEFT OUTER JOIN  #SGSTAMTSUMM ON CLIC_BO_ID_SGST = DPAM_SBA_NO 
    ----LEFT OUTER JOIN  #UGSTAMT ON CLIC_BO_ID_UGST = DPAM_SBA_NO 
    --LEFT OUTER JOIN  #IGSTAMTSUMM ON CLIC_BO_ID_IGST = DPAM_SBA_NO 
