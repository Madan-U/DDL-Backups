-- Object: PROCEDURE citrus_usr.PR_IMPORT_CLIENT_CDSL_CITRUS_bak_11012016
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

create PROCEDURE  [citrus_usr].[PR_IMPORT_CLIENT_CDSL_CITRUS_bak_11012016]
			(
			 @PA_EXCH          VARCHAR(20)    
...
--           BEGIN  
            ...
            EXEC PR_INS_UPD_CLIM  '0','INS','CITRUSA',@L_CLIENT_VALUES,0,'*|~*','|*~|','',''  
...
print @L_CRN_NO
print @L_ADR
           EXEC PR_INS_UPD_ADDR @L_CRN_NO,'INS','CITRUSA',@L_CRN_NO,'',@L_ADR,0,'*|~*','|*~|',''   
...
--           BEGIN  
             ...
             EXEC PR_INS_UPD_CONC @L_CRN_NO,'INS','CITRUSA',@L_CRN_NO,'',@L_CONC,0,'*|~*','|*~|',''  
...
--              END
      ...
      --DP_ACCT_MSTR  
           ...
           EXEC PR_INS_UPD_DPAM @L_CRN_NO,'INS','CITRUSA',@L_CRN_NO,@L_DP_ACCT_VALUES,0,'*|~*','|*~|',''
...
--              END
      ...
      --DP_ACCT_MSTR  
           SELECT @L_ADR = 'AC_PER_ADR1|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BO_ADDP_LIN1)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BO_ADDP_LIN2)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BO_ADDP_LIN3)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BO_ADDP_CITY)),'')+'|*~|'+ISNULL(TMPCLI_BO_ADDP_STATE,'')+'|*~|'+ISNULL(TMPCLI_BO_ADDP_CNTRY,'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BO_ADDP_CNTRY)),'')+'|*~|*|~*' FROM   TMP_CLIENT_DTLS_MSTR_CDSL_citrus WHERE TMPCLI_BOID = @C_BEN_ACCT_NO AND ISNULL(TMPCLI_BO_ADDP_LIN1,'') <> ''  
           ...
           SELECT @L_ADR = @L_ADR  + 'NOMINEE_ADR1|*~|' +ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_ADD_LN123)),'')+'|*~|'+ISNULL(LTRIM(RTRIM('')),'')+'|*~|'+ISNULL(LTRIM(RTRIM('')),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_CITY_STATE_CNTRY)),'')+'|*~|'+ISNULL('','')+'|*~|'+ISNULL('','')+'|*~|'+ISNULL(LTRIM(RTRIM('')),'')+'|*~|*|~*' FROM   TMP_CLIENT_DTLS_MSTR_CDSL_citrus WHERE TMPCLI_BOID = @C_BEN_ACCT_NO AND ISNULL(TMPCLI_NOMI_ADD_LN123,'') <> ''  
...
--              END
      ...
      --DP_HOLDER_DTLS/ADDRESSES/CONCTACT_CHANNELS  
           DECLARE @PA_FH_DTLS VARCHAR(8000)  
				  ...
				  ,@l_nrn_no numeric
--set @l_nrn_no = 0 
--select @l_nrn_no = bitrm_values 
--from bitmap_ref_mstr 
--where bitrm_parent_cd like '%NEW_NRN_NO_SYC_CITRUS%'
--
--update bitmap set bitrm_values = bitrm_values + 1 
...
--where bitrm_parent_cd like '%NEW_NRN_NO_SYC_CITRUS%'
       ...
       --    SELECT @PA_NOM_DTLS = CITRUS_USR.FN_SPLITVAL_BY(ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_NM)),''),1,'~')+'|*~|'+CITRUS_USR.FN_SPLITVAL_BY(ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_NM)),''),2,'~')+'|*~|'+CITRUS_USR.FN_SPLITVAL_BY(ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_NM)),''),3,'~')+'|*~|'+'|*~|'+ CONVERT(VARCHAR(11),CONVERT(DATETIME,TMPCLI_DOB_MINOR_DT,103),103)+'|*~|'+'|*~|'+convert(varchar,@l_nrn_no)+'|*~|'+'|*~|*|~*'  FROM   TMP_CLIENT_DTLS_MSTR_CDSL_citrus WHERE TMPCLI_BOID = @C_BEN_ACCT_NO   AND CITRUS_USR.FN_SPLITVAL_BY(ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_NM)),''),1,'~') <> ''
		   SELECT @PA_NOM_DTLS = CITRUS_USR.FN_SPLITVAL_BY(ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_NM)),''),1,'~')+'|*~|'+CITRUS_USR.FN_SPLITVAL_BY(ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_NM)),''),2,'~')+'|*~|'+CITRUS_USR.FN_SPLITVAL_BY(ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_NM)),''),3,'~')+'|*~|'+'|*~|'+ CONVERT(VARCHAR(11),CONVERT(DATETIME,TMPCLI_DOB_MINOR_DT,103),103)+'|*~|'+'|*~|'+'|*~|*|~*'  FROM   TMP_CLIENT_DTLS_MSTR_CDSL_citrus WHERE TMPCLI_BOID = @C_BEN_ACCT_NO   AND CITRUS_USR.FN_SPLITVAL_BY(ISNULL(LTRIM(RTRIM(TMPCLI_NOMI_NM)),''),1,'~') <> ''
...
--                           
							...
							IF @L_BROM_ID  <> ''
							EXEC PR_INS_UPD_CLIENT_BRKG @L_CRN_NO,'','CITRUSA',@L_CRN_NO,@L_BRKG_VAL,0,'*|~*','|*~|',''
...
--              END
      ...
      --BANK_MSTR/ADDRESSES/CONCTACT_CHANNELS  
		...
		SELECT @L_DPBA_VALUES = CONVERT(VARCHAR,@L_COMPM_ID)  +'|*~|'+CONVERT(VARCHAR,@L_EXCSM_ID) +'|*~|'+ ISNULL(@L_DPM_DPID,'') + '|*~|'+ LTRIM(RTRIM(@C_BEN_ACCT_NO)) +'|*~|' + LTRIM(RTRIM(REPLACE(TMPCLI_FRST_HLDR_NM,'~',' '))) + '|*~|'+ CONVERT(VARCHAR,@L_BANM_ID ) +'|*~|'+ TMPCLI_BENF_BANK_ACCT_TYP  +'|*~|' + CONVERT(VARCHAR,LTRIM(RTRIM(TMPCLI_BANK_ACCT_N0))) + '|*~|1|*~|0|*~|A*|~*'  
        FROM   TMP_CLIENT_DTLS_MSTR_CDSL_citrus WHERE TMPCLI_BOID = @C_BEN_ACCT_NO  

       EXEC PR_INS_UPD_DPBA '0','INS','CITRUSA',@L_CRN_NO,@L_DPBA_VALUES,0,'*|~*','|*~|',''  
...
--              END
      ...
      --ACCOUNT_PROPERTIES  
         ...
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','BANKCCY',UPPER(ISNULL(LTRIM(RTRIM(TMPCLI_BENF_ACCT_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL_citrus ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'BANKCCY'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','DIVBANKCCY',UPPER(ISNULL(LTRIM(RTRIM(TMPCLI_DIVIDEND_ACCT_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL_citrus ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'DIVBANKCCY'  
        ...
        EXEC PR_INS_UPD_ACCP @L_CRN_NO ,'EDT','CITRUSA',@L_DPAM_ID,@C_BEN_ACCT_NO,'DP',@L_ACCP_VALUE,@L_ACCPD_VALUE ,0,'*|~*','|*~|',''  
...
--          SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + ISNULL(LTRIM(RTRIM(BEN_RBI_REF_NO)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR WHERE BEN_ACCT_NO = @C_BEN_ACCT_NO AND ENTPM_CD = 'RBI_REF_NO'  
        ...
        --SELECT DISTINCT @L_ENTPD_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + CONVERT(VARCHAR,ENTDM_ID) + '|*~|'  + ISNULL(LTRIM(RTRIM(BEN_RBI_APP_DATE)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR , ENTPM_DTLS_MSTR  WHERE BEN_ACCT_NO = @C_BEN_ACCT_NO AND ENTPM_CD = 'RBI_REF_NO' AND ENTDM_CD = 'RBI_APP_DT' AND ENTDM_ENTPM_PROP_ID = ENTPM_PROP_ID  
         ...
         EXEC PR_INS_UPD_ENTP '1','EDT','CITRUSA',@L_CRN_NO,'',@L_ENTP_VALUE,@L_ENTPD_VALUE,0,'*|~*','|*~|',''    
...
--              END
         ...
         SET @L_DPPD_DETAILS = ''  
         SELECT @L_DPPD_DETAILS =   ISNULL(CONVERT(VARCHAR,@L_COMPM_ID),'')+'|*~|'+ ISNULL(CONVERT(VARCHAR,@L_EXCSM_ID),'')+'|*~|'+ISNULL(CONVERT(VARCHAR,@L_DPM_DPID),'')+'|*~|'+ISNULL(@C_BEN_ACCT_NO,'')+'|*~|'+ISNULL(LTRIM(RTRIM('')),'')+'|*~|'+ ISNULL(LTRIM(RTRIM('1ST HOLDER')),'') +'|*~|'+ISNULL(LTRIM(RTRIM(poam_name1)),'') +'|*~|'+ISNULL(LTRIM(RTRIM(poam_name2)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(poam_name3)),'')+'|*~|'+ISNULL(LTRIM(RTRIM('')),'')+'|*~|'+ISNULL(LTRIM(RTRIM('')),'')+'|*~|'+ISNULL(LTRIM(RTRIM('')),'')+'|*~|'+ISNULL(LTRIM(RTRIM('')),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_POA_ID)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_POA_TYPE)),'')  
         ...
         EXEC PR_INS_UPD_DPPD '1',@L_CRN_NO,'EDT','CITRUSA',@L_DPPD_DETAILS,0,'*|~*','|*~|',''
