-- Object: PROCEDURE citrus_usr.PR_IMPORT_CLIENT_CDSL_DPB9_open
...
CREATE   PROCEDURE  [citrus_usr].[PR_IMPORT_CLIENT_CDSL_DPB9_open]
			(
			 @pa_exch          VARCHAR(20)    
...
SELECT distinct boid column_3 into #temp  FROM dps8_pc1  
WHERE  boid  not in (select dpam_sba_no from dp_acct_mstr with (nolock) where dpam_dpm_id = 3 and dpam_deleted_ind =1 )  


delete  from dpb9_pc0 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc1 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc2 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc3 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc4 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc5 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc6 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc7 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc8 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc12 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc18 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc20 where boid not in (SELECT  column_3 from  #temp)
delete  from dpb9_pc22 where boid not in (SELECT  column_3 from  #temp)
...
--changes done for 408 ------
  ...
  --changes done for 408 ------
           ...
           FROM   DPB9_PC8 WHERE boid = @c_ben_acct_no and isnull(PriPhInd,'') <> ''  and NOM_Sr_No = 3                   
                ...
                SELECT @L_acc_conc = isnull(@L_acc_conc ,'') + 'GUAIC|*~|'+ISNULL(ltrim(rtrim(PriPhInd)),'')+'*|~*'
           +'GUARD_MOB|*~|'+ISNULL(ltrim(rtrim(PriPhNum)),'')+'*|~*'
           FROM   DPB9_PC7 WHERE boid = @c_ben_acct_no and isnull(PriPhInd,'') <> ''   
