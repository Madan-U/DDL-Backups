-- Object: PROCEDURE citrus_usr.Pr_rpt_printbill_emailer
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

CREATE procedure  [citrus_usr].[Pr_rpt_printbill_emailer]            
@pa_dptype varchar(4),                    
@pa_excsmid int,                    
 ...
 BEGIN              
	  ...
	  begin     
		 ...
		 begin
		  ...
		  set @@ssql='insert into ' + @@temptable + '(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val)         
		  select clic_dpam_id,dpam_sba_name,dpam_sba_no,clic_charge_name,null,clic_trans_dt=''' +  convert(varchar,@pa_todate,109) + ''',null,null,null,2,null,clic_charge_amt         
		  ...
		  if @pa_group_cd <> ''
		  begin
		  set @@ssql = ' and exists(select g.dpam_id from account_group_mapping g where g.dpam_id = clic_dpam_id and group_cd = ''' + @pa_group_cd + ''')'
		  end 
		  exec(@@ssql)     
		...
		begin
			...
			select @@header_lines =isnull(PAGE_HDR_MSG_SIZE,0)+ceiling(len(isnull(CITRUS_USR.[fn_splitstrin_byspace_for_print](page_footer_msg,75,'',1),''))/75.00)+ceiling(len(isnull(page_spl_msg,''))/75.00),@@footer_lines=isnull(PAGE_FTR_MSG_SIZE,0)
		  ...
		  opening_bal=isnull(opening_bal,0),          
		  DEBIT_QTY= CASE WHEN  QTY <= 0 THEN convert(varchar,ABS(QTY)) ELSE '''' END,                    
		  CREDIT_QTY= CASE WHEN QTY > 0 THEN convert(varchar,ABS(QTY)) ELSE '''' END,        
		  closing_bal = isnull(opening_bal,0) + isnull(QTY,0),        
		  ...
		  order by t.dpam_acctno,dpam_sba_name,order_by,Isin_name,18,line_no'
print @@ssql
...
-- for block email client end --
		...
		begin
			insert into #tmpnsdlbill        
			select clic_trans_dt=@pa_todate,clic_dpam_id,dpam_sba_no,dpam_sba_name,null,null,null,null,null,null,null,null,null,clic_charge_name,2,clic_charge_amt,null         
	...
	begin
		...
		select @@header_lines =isnull(PAGE_HDR_MSG_SIZE,0)+ceiling(len(isnull(CITRUS_USR.[fn_splitstrin_byspace_for_print](page_footer_msg,75,'',1),''))/75.00)+ceiling(len(isnull(page_spl_msg,''))/75.00)
...
*/
		...
		exec(@@ssql)	
		  ...
		  open_qty = CASE WHEN final_trans = 3 THEN closing_qty ELSE closing_qty - qty END,                  
		  DEBIT_QTY= CASE WHEN QTY <= 0 THEN convert(varchar,ABS(QTY)) ELSE '''' END,                    
		  CREDIT_QTY= CASE WHEN QTY > 0 THEN convert(varchar,ABS(QTY)) ELSE '''' END,              
		  closing_qty,        
		  charge_val= isnull(charge_val,0) * -1 ,
		  ...
		  trans_date        
		  from ' + @@temptable + 'clients tc,' + @@temptable + ' t left outer join isin_mstr i on t.isin_cd = i.isin_cd 
		  left outer join settlement_type_mstr s on t.sett_type = s.settm_type  and isnull(s.settm_type,'''') <> ''''        
