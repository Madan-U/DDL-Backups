-- Object: PROCEDURE citrus_usr.pr_ins_upd_dpba_new
...
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_dpba_new](@pa_id                varchar(8000)
                                 , @pa_action            char(3)
...
, @l_bank_name       varchar(100)
, @l_bank_branch     varchar(100)
, @l_micr            varchar(12)
, @l_adr1            varchar(50)
, @l_adr2            varchar(50)
, @l_adr3            varchar(50)
, @l_adrcity         varchar(25)
, @l_adrstate        varchar(25)
, @l_adrcountry      varchar(25)
, @l_adrzip          varchar(25)

, @L_ADDR_VALUE      varchar(1000)
      ...
      --
        ...
        --
          ...
          --
            ...
            /*New Changes */
            SET @l_bank_name       = citrus_usr.fn_splitval(@currstring_val, 11)
            SET @l_bank_branch     = citrus_usr.fn_splitval(@currstring_val, 12)
            SET @l_micr            = citrus_usr.fn_splitval(@currstring_val, 13)
            SET @l_adr1            = citrus_usr.fn_splitval(@currstring_val, 14)
			SET @l_adr2            = citrus_usr.fn_splitval(@currstring_val, 15)
			SET @l_adr3            = citrus_usr.fn_splitval(@currstring_val, 16)
			SET @l_adrcity         = citrus_usr.fn_splitval(@currstring_val, 17)
			SET @l_adrstate        = citrus_usr.fn_splitval(@currstring_val, 18)
			SET @l_adrcountry      = citrus_usr.fn_splitval(@currstring_val, 19)
			SET @l_adrzip          = citrus_usr.fn_splitval(@currstring_val, 20)
            /*New Changes */
            SET @l_action_type     = citrus_usr.fn_splitval(@currstring_val,21)
            --
            declare @l_old_addr_value varchar(8000)
            ,@l_exists_yn char(1)
            set @l_exists_yn = 'N'

            if exists(select banm_id from bank_mstr 
            where ltrim(rtrim(banm_name)) = ltrim(rtrim(@l_bank_name)) 
            and ltrim(rtrim(BANM_BRANCH)) = ltrim(rtrim(@l_bank_branch)) 
            and ltrim(rtrim(BANM_MICR)) = ltrim(rtrim(@l_micr))  
            and banm_deleted_ind = 1 
            )
            begin  
            
            set @l_exists_yn = 'Y' 
            select @l_old_addr_value  = 'COR_ADR1|*~|'+ISNULL(LTRIM(RTRIM(adr_1)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_2)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_3)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_city)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_state)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_country)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_zip)),'')+'*|~*'  
            from bank_mstr , addresses , entity_adr_conc 
            where entac_ent_id = banm_id and entac_concm_cd = 'COR_ADR1' and  ltrim(rtrim(banm_name)) = ltrim(rtrim(@l_bank_name)) 
            and ltrim(rtrim(BANM_BRANCH)) = ltrim(rtrim(@l_bank_branch)) 
            and ltrim(rtrim(BANM_MICR)) = ltrim(rtrim(@l_micr))  
            and banm_deleted_ind = 1  

            declare @l_old_banm_id numeric
            
            select @l_old_banm_id = banm_id from bank_mstr 
            where ltrim(rtrim(banm_name)) = ltrim(rtrim(@l_bank_name)) 
            and ltrim(rtrim(BANM_BRANCH)) = ltrim(rtrim(@l_bank_branch)) 
            and ltrim(rtrim(BANM_MICR)) = ltrim(rtrim(@l_micr))  
            and banm_deleted_ind = 1 
            ...
            set @L_ADDR_VALUE = 'COR_ADR1|*~|'+ISNULL(LTRIM(RTRIM(@l_adr1)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(@l_adr2)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(@l_adr3)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(@l_adrcity)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(@l_adrstate)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(@l_adrcountry)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(@l_adrzip)),'')+'*|~*' 
            if @l_action_type <> 'D'  and @l_exists_yn = 'N'  and isnull(@l_old_addr_value,'') <> @L_ADDR_VALUE
            begin
             print 'ff'           
             EXEC pr_mak_banm  '0','INS','MIG',@l_bank_name,@l_bank_branch,@l_micr,'','','',0,1,@L_ADDR_VALUE,0,'','*|~*','|*~|',''

            end 
            if @l_action_type <> 'D'  and @l_exists_yn = 'Y'  and isnull(@l_old_addr_value,'') <> @L_ADDR_VALUE
            begin
             print 'ff'           
             EXEC pr_mak_banm  @l_old_banm_id,'EDT','MIG',@l_bank_name,@l_bank_branch,@l_micr,'','','',0,1,@L_ADDR_VALUE,0,'','*|~*','|*~|',''

            end 


            select @l_dpba_banm_id = banm_id from bank_mstr , addresses , entity_adr_conc 
            where entac_ent_id = banm_id and entac_concm_cd = 'COR_ADR1' and ltrim(rtrim(banm_name)) = ltrim(rtrim(@l_bank_name)) 
            and ltrim(rtrim(BANM_BRANCH)) = ltrim(rtrim(@l_bank_branch)) 
            and ltrim(rtrim(BANM_MICR)) = ltrim(rtrim(@l_micr))  
            and 'COR_ADR1|*~|'+ISNULL(LTRIM(RTRIM(adr_1)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_2)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_3)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_city)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_state)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_country)),'')+'|*~|'+ISNULL(LTRIM(RTRIM(adr_zip)),'')+'*|~*'   = @L_ADDR_VALUE
            and banm_deleted_ind = 1 
            ...
            --
              ...
              --
                ...
                --
                 ...
                 END  --#exist
                 --
                 IF @l_action_type ='D' and not EXISTS(SELECT cliba_clisba_id FROM client_bank_accts WHERE cliba_clisba_id = @l_dpam_id AND cliba_banm_id = @l_dpba_banm_id and cliba_deleted_ind =1 )
                 BEGIN--***
                 --
                 ...
                 END--*** 
                 --
                 IF EXISTS(SELECT cliba_clisba_id FROM client_bank_accts WHERE cliba_clisba_id = @l_dpam_id )
                 BEGIN
                 --
                   SET @l_edt_del_id = 8
                 --
