-- Object: PROCEDURE citrus_usr.pr_vendor_res_report_test
...
create  procedure [citrus_usr].[pr_vendor_res_report_test]  (@pa_from_no varchar(50)
,@pa_to_no  varchar(50)
,@pa_from_dt datetime
,@pa_to_dt datetime
...
-- commented on 24092013		, case when convert(varchar(11),isnull(d.reco_datetime,''),103) = '01/01/1900' then '' else convert(varchar(19),d.reco_datetime,120) end AS "RECO DTTIME"

,'' AS "RECO DTTIME"

		--,convert(varchar(11),isnull(d.reco_datetime,''),103),isnull(reco_yn,'')
		from #ACLIST ACCOUNT,dis_req_dtls_MAK d1 left outer join vendor_response on boid = VR_boid 
left outer join dis_req_dtls d
		on d.req_slip_no = d1.req_slip_no
		and d.boid = d1.boid and d.req_date = d1.req_date
...
--		and d.boid = d1.boid
		...
		and d1.deleted_ind in (1)
		and d1.slip_yn in ('S','L')
		--and isnull(convert(varchar(19),d.reco_datetime,120),'') <> ''
		union  
		select  distinct --id as "VR ID" ,
		--VR_DPID as "DPID",
d1.created_dt,
        ...
        "STATUS" = case when d1.deleted_ind = 1 then 'SUCCESS'  
		            when d1.deleted_ind = 3 then 'REJECTED' end 
	-- commented on 24092013	, case when convert(varchar(11),isnull(d.reco_datetime,''),103) = '01/01/1900' then '' else convert(varchar(19),d.reco_datetime,120) end 
,'' AS "RECO DTTIME"
...
--		and d.boid = d1.boid
		and dpam_sba_no between  @pa_from_no and  @pa_to_no
		and d1.deleted_ind in (0,3)
		and not exists (select 1 from dis_req_dtls_MAK innerd where innerd.boid=d1.boid  and innerd.req_date= d1.req_date and innerd.req_slip_no=d1.req_slip_no and innerd.deleted_ind = 1 )
		and d1.slip_yn in ('S','L')
--and isnull(convert(varchar(19),d.reco_datetime,120),'') <> ''
