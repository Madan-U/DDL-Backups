-- Object: PROCEDURE citrus_usr.Pr_rpt_statement_bak30012012
...
create Proc [citrus_usr].[Pr_rpt_statement_bak30012012]                            
@pa_dptype varchar(4),                                    
@pa_excsmid int,                                    
...
--changed by tushar sep 18 2009    
  ...
  begin
     ...
     --         
		 ...
		 FROM DP_HLDG_MSTR_CDSL LEFT OUTER JOIN  isin_mstr i with(nolock) on DPHMC_ISIN = i.isin_cd 
		 LEFT OUTER JOIN CLOSING_PRICE_MSTR_CDSL ON DPHMC_ISIN = CLOPM_ISIN_CD 
		 AND ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = DPHMC_ISIN and CLOPM_DT <= @PA_FROMDATE and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc),
