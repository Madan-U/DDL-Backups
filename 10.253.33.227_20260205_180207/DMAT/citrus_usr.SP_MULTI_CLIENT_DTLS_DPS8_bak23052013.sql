-- Object: PROCEDURE citrus_usr.SP_MULTI_CLIENT_DTLS_DPS8_bak23052013
...
create PROCEDURE [citrus_usr].[SP_MULTI_CLIENT_DTLS_DPS8_bak23052013]  
(   
...
--				else STAM_DESC end end [STATUS]  
 SELECT distinct case when isnull(fre_action,'')= 'F' then 'FREEZED' else 
				  case when (@l_close_dt <> '' and dpam.dpam_stam_cd <> '04') then  STAM_DESC + ':' + convert(varchar(11),@l_close_dt,106) 
					   when (@l_close_dt = '' and dpam.dpam_stam_cd = '04') then STAM_DESC + ':' + REPLACE(CONVERT(VARCHAR(11),dpam.dpam_lst_upd_dt, 106), ' ', '-')
							else STAM_DESC end end [STATUS]  
--DISTINCT top 1 case when fre_action = 'F' then 'FREEZED' else STAM_DESC end [STATUS]  
  ,CLICM_DESC  [CATEGORY]  
  ,SUBCM_DESC  [SUBCATEGORY]  
  ,ENTTM_DESC  [TYPE]  
  ,convert(varchar(11),convert(datetime,case when isnull(dps8_pc1.DateOfBirth,'') <> '' then left(dps8_pc1.DateOfBirth,2)+'/'+substring(dps8_pc1.DateOfBirth,3,2)+'/'+right(dps8_pc1.DateOfBirth,4) else '' end ,103) ,103) clim_dob  
  ...
  ,citrus_usr.fn_splitval_by(isnull(BANM_NAME,''),'1','-') + ','+ isnull(TMPBA_DP_BANK_ADD1,'') [BADDRESS1]  
  ...
  ,citrus_usr.fn_splitval_by(isnull(BANM_NAME,''),'1','-') BANM_NAME  
...
,case when BenTaxDedStat  = '1'  then  'Exempt'                      
  ...
  ,case when isnull(EletConf,'') ='' then 'N' else  isnull(EletConf,'') end  CONFIRMATION  
