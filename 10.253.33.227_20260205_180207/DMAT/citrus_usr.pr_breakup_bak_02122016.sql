-- Object: PROCEDURE citrus_usr.pr_breakup_bak_02122016
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

create procedure [citrus_usr].[pr_breakup_bak_02122016](@pa_from_dt datetime,@pa_to_dt datetime,@pa_login_name varchar(100))
as
begin 
...
where CDSHM_ISIN =ISIN_CD AND ISIN_SEC_TYPE='20' AND ISIN_SECURITY_TYPE_DESCRIPTION='Government Securities'
...
,  isnull(amt,0) amt
,  isnull(amt*case when charge_name  in ('POAFIXED','POA','VERSION 2.5 LIFETIMEPOA') then 0 else 0.1400  end ,0.0000) service_tax_amt
,  isnull(amt*case when charge_name  in ('POAFIXED','POA','VERSION 2.5 LIFETIMEPOA') then 0 else 0.005  end ,0.000000) sbt_amt
,  isnull(amt*case when charge_name  in ('POAFIXED','POA','VERSION 2.5 LIFETIMEPOA') then 0 else 0.005  end ,0.000000) KKC_amt
