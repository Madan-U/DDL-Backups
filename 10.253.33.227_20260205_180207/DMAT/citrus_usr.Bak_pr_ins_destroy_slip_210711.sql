-- Object: PROCEDURE citrus_usr.Bak_pr_ins_destroy_slip_210711
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[Bak_pr_ins_destroy_slip_210711](@pa_id              VARCHAR(8000)      
...
--    
  DECLARE @t_errorstr      VARCHAR(8000)    
        ...
        , @l_slipno         NUMERIC    
          ...
          set @pa_dpam_acct_no = case when @pa_trantm_id = '10000' then '' else @pa_dpam_acct_no end   
  SELECT @l_dpm_id = dpm_id FROM dp_mstr WHERE dpm_dpid = @pa_dpm_dpid AND dpm_deleted_ind =1    
  SELECT @l_trastm_cd = CASE WHEN TRASTM_CD = '925' THEN '904_act' ELSE TRASTM_CD END  FROM TRANSACTION_SUB_TYPE_MSTR WHERE TRASTM_ID =CONVERT(BIGINT,@pa_trantm_id) AND TRASTM_DELETED_IND = 1    
