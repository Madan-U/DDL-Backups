-- Object: PROCEDURE citrus_usr.rpt_Approve_details_17072012
...
create procedure [citrus_usr].[rpt_Approve_details_17072012]
(@pa_id  varchar(10),
 @pa_type varchar(4),
 @pa_tab varchar(20),
 @pa_from_dt datetime,
...
set @@subtype = '' 
 ...
 SET @pa_trastm_cd=CASE WHEN @pa_trastm_cd = '904_ACT' THEN '904' ELSE @pa_trastm_cd END
	 ...
	 select @@l_child_entm_id    =  citrus_usr.fn_get_child(@pa_login_pr_entm_id , @pa_login_entm_cd_chain)                                      
		...
		INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,isnull(EFF_TO,'DEC 31 2100') FROM citrus_usr.fn_acct_list(@l_dpm_id ,@pa_login_pr_entm_id,@@l_child_entm_id)		
  ...
  --
			...
			begin
					...
					,ISNULL([checker1_dt],'') [checker1_dt]
					,ISNULL([checker2],'') [checker2]
					,ISNULL([checker2_dt],'') [checker2_dt]
					, slip_reco
					, image_scan
					, scan_dt
					, dptdc_rmks
...
--					  else 'D' end as [Status1]
					...
					, case when (dptdc_deleted_ind = 0 and isnull(dptdc_res_cd,'') = '' and isnull(dptdc_mid_chk,'')<>'') then CASE WHEN isnull(convert(varchar(11),DPTDC_LST_UPD_dt,106),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),DPTDC_LST_UPD_dt,109) END else '' end checker1_dt
--convert(varchar(11),DPTDC_LST_UPD_dt,103) else '' end as checker1_dt
					...
					, case when dptdc_deleted_ind = 1 then CASE WHEN isnull(convert(varchar(11),DPTDC_LST_UPD_dt,109),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),DPTDC_LST_UPD_dt,109) END else '' end checker2_dt
...
--					  else 'D' end as [Status1]
					...
					, case when (dptdc_deleted_ind = 0 and isnull(dptdc_res_cd,'') <> '')  then isnull(dptdc_mid_chk,'') else '' end as checker1
					--, case when dptdc_deleted_ind = 0 then CASE WHEN isnull(convert(varchar(11),DPTDC_LST_UPD_dt,109),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),DPTDC_LST_UPD_dt,109) END else '' end checker1_dt
					, case when (dptdc_deleted_ind = 0 and isnull(dptdc_res_cd,'') <> '') then CASE WHEN isnull(convert(varchar(11),DPTDC_LST_UPD_dt,106),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),DPTDC_LST_UPD_dt,109) END else '' end checker1_dt
--convert(varchar(11),DPTDC_LST_UPD_dt,103) else '' end as checker1_dt
					, case when dptdc_deleted_ind = 1 then isnull(DPTDC_LST_UPD_BY,'') end as checker2
					, case when dptdc_deleted_ind = 1 then CASE WHEN isnull(convert(varchar(11),DPTDC_LST_UPD_dt,109),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),DPTDC_LST_UPD_dt,109) END else '' end checker2_dt
--convert(varchar(11),DPTDC_LST_UPD_dt,103) else '' end as checker2_dt
...
--					,[Status1]
			select	inst_id 
					...
					, slip_reco
					, image_scan
					, case when scan_dt='1900-01-01 00:00:00.000' then '' when scan_dt='1/1/1900' then '' else scan_dt end scan_dt
					, dptdc_rmks
					, backoffice_code
					, reason
...
--						   When dptdc_deleted_ind = 1 then 'U'
--					  else 'D' end as [Status1]
					, case when ISNULL(dptd1.dptdc_trans_no,'') <> '' then 'RESPONSE FILE IMPORTED'  when ISNULL(dptd1.dptdc_batch_no,'') <> '' then 'BATCH GENERATED' when dptd1.dptdc_deleted_ind = 1 then 'CHECKER DONE' ELSE '' END AS [STATUS1]
					...
					, case when dptd.dptdc_deleted_ind in (0,1) and isnull(dptd.dptdc_mid_chk,'') <> '' then CASE WHEN isnull(convert(varchar(11),dptd1.DPTDC_LST_UPD_dt,109),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),dptd1.DPTDC_LST_UPD_dt,109) END 
						   when dptd.dptdc_deleted_ind in (0,1) and isnull(dptd.dptdc_mid_chk,'') = '' then CASE WHEN isnull(convert(varchar(11),dptd.DPTDC_LST_UPD_dt,109),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),dptd.DPTDC_LST_UPD_dt,109) END else '' end checker1_dt
--convert(varchar(11),dptd.DPTDC_LST_UPD_dt,103) else '' end as checker1_dt
					...
					, case when dptd.dptdc_deleted_ind = 1 and isnull(dptd.dptdc_mid_chk,'') <> '' then CASE WHEN isnull(convert(varchar(11),dptd.DPTDC_LST_UPD_dt,106),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),dptd.DPTDC_LST_UPD_dt,109) END else '' end checker2_dt										
					, case when recon_flag ='Y' then 'YES' else 'NO' end  slip_reco
					, case when isnull(client_id,'')='' then 'NO' else 'YES' end image_scan
					, case when isnull(convert(varchar(11),created_dt,106),'') = '01 Jan 1900' then '' else created_dt end  as scan_dt
					, isnull(dptd.dptdc_rmks,'') dptdc_rmks
					, ISNULL(CITRUS_USR.FN_UCC_ACCP(dptd.DPTDC_DPAM_ID,'BBO_CODE',''),'') backoffice_code
