-- Object: PROCEDURE citrus_usr.pr_rpt_nominee_poa
...
CREATE proc [citrus_usr].[pr_rpt_nominee_poa]
   (
    @pa_excsmid int,
...
into #tempdata_pc6  from dps8_pc6  pc6
	...
	--
		...
		If @pa_minor_flg='Y'
			...
			, ltrim(rtrim(isnull(tmpc6.Name,''))) + ' ' + ltrim(rtrim(isnull(tmpc6.MiddleName,''))) + ' ' + ltrim(rtrim(isnull(tmpc6.SearchName,''))) [NAME]
			,NAME_id = case when (isnull(NOM_NRN_NO,''))='' then 'N/A' else NOM_NRN_NO end 
			,left(tmpc6.dos ,2) + '/' + substring(tmpc6.dos ,3,2)+ '/' + right(tmpc6.dos ,4) SetupDate
			,Nom_Addr=isnull(tmpc6.Addr1 ,'') + '' +  isnull(tmpc6.Addr2,'')+ '' +isnull(tmpc6.Addr3,'')+ '' +isnull(tmpc6.City,'')+ '' +isnull(tmpc6.State,'')+ '' +isnull(tmpc6.Country,'')+ '' +isnull(tmpc6.PinCode,'')--replace(citrus_usr.fn_acct_addr_value(dpacct.dpam_id,'NOMINEE_ADR1'),'|*~|',' ')
			, ltrim(rtrim(isnull(pc8.Name,''))) + ' ' + ltrim(rtrim(isnull(pc8.MiddleName,''))) + ' ' + ltrim(rtrim(isnull(pc8.SearchName,''))) NOMGUARDNAME
			,Nom_Guard_Addr=isnull(pc8.Addr1 ,'') + '' +  isnull(pc8.Addr2,'')+ '' +isnull(pc8.Addr3,'')+ '' +isnull(pc8.City,'')+ '' +isnull(pc8.State,'')+ '' +isnull(pc8.Country,'')+ '' +isnull(pc8.PinCode,'')--replace(citrus_usr.fn_acct_addr_value(dpacct.dpam_id,'NOMINEE_ADR1'),'|*~|',' ')
			,isnull(DATEADD(DAY,1,(DATEADD(YEAR,18,convert(datetime,DPHD_NOM_DOB,103)))),'') major_dt
			--,tmpc6.RES_SEC_FLg [Sec_flag]
			--,tmpc6.NOM_Sr_No [Nom_Sr_no]
			--,case when tmpc6.rel_WITH_BO = '1' then  'SPOUSE'
			--	when tmpc6.rel_WITH_BO = '2' then 'SON'
			--	when tmpc6.rel_WITH_BO = '3' then 'DAUGHTER'
			--	when tmpc6.rel_WITH_BO = '4' then 'FATHER'
			--	when tmpc6.rel_WITH_BO = '5' then 'MOTHER'
			--	when tmpc6.rel_WITH_BO = '6' then 'BROTHER'
			--	when tmpc6.rel_WITH_BO = '7' then 'SISTER' 
			--	when tmpc6.rel_WITH_BO = '8' then  'GRAND-SON'
			--	when tmpc6.rel_WITH_BO = '9' then 'GRAND-DAUGHTER'

			--	when tmpc6.rel_WITH_BO = '10' then 'GRAND-FATHER'
			--	when tmpc6.rel_WITH_BO = '11' then 'GRAND-MOTHER'
			--	when tmpc6.rel_WITH_BO = '12' then 'NOT PROVIDED'
			--	when tmpc6.rel_WITH_BO = '13' then 'OTHERS'
			--	else tmpc6.rel_WITH_BO end [Nom_relation]
			--	,isnull((tmpc6.perc_OF_SHARES),'')  [Nom_Percentage]
			--,tmpc6.NOM_Sr_No
,'' Nom_Flag
...
left outer join dps8_pc8 pc8 on pc8.boid = dpam_sba_no and pc8.TypeOfTrans in ('1','')
			...
			and  convert(datetime,left(tmpc6.dos ,2) + '/' + substring(tmpc6.dos ,3,2)+ '/' + right(tmpc6.dos ,4),103) between convert(datetime,@pa_from_dt,103) and convert(datetime,@pa_to_dt,103)
			and   account.dpam_id = dpacct.dpam_id
			and   isnull(DPHD_NOM_FNAME,'')<>''
			and  dpacct.dpam_deleted_ind = 1
			and  DPHD_DELETED_IND = 1   and  tmpc6.dos <> ''
...
ltrim(rtrim(pc1.PinCode)) 
 as Client_Addr 
		...
		,isnull(p.dpam_sba_name,'') [POA NAME]
		--,isnull(dppd_master_id,'') NAME_id
		,isnull( pc5.MasterPOAId,'') NAME_id
...
--into POADataOct24Nov25
        from #ACLIST account--, dp_poa_dtls
		,dp_acct_mstr dpacct, account_properties ,poam p,dps8_pc5 pc5,dps8_pc1  pc1
       ...
       -- and  dppd_master_id = poam_master_id    
		 and  pc5.MasterPOAId = poam_master_id    
