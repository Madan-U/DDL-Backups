-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_bak_jul0712_newlogicasondt
...
COMMIT
EXEC PR_RPT_HOLDING 'CDSL',3,'Y','JUl 06 2012','1201090000000101','1201090000000101','','Y',1 ,'HO|*~|','','', '',''		
...
create PROC [citrus_usr].[Pr_Rpt_Holding_bak_jul0712_newlogicasondt]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
begin 
	...
	insert into  #tmphldg
	exec  [pr_get_holding_fix_latest_asondt]   @@DPMID,@PA_FORDATE,@PA_FORDATE,@PA_FROMACCID,@PA_TOACCID,''
  ...
  IF @PA_ASONDATE = 'Y'                          
	  BEGIN  
			 SELECT 
						...
						DPAM_ID, 
						HOLDING_DT=CONVERT(VARCHAR(11),dphmcd_holding_dt,109)                           
			FROM 
			...
			group BY 
						DPAM_SBA_NO,ISIN_NAME,DPHMCd_ISIN  ,DPAM_SBA_NAME ,DPAM_ID      ,dphmcd_holding_dt                 
			ORDER BY 
...
BEGIN   
		 ...
		 SELECT distinct DPAM_SBA_NAME,DPAM_SBA_NO,DPHMCD_ISIN,ISIN_NAME,CONVERT(NUMERIC(18,3),sum(DPHMCD_CURR_QTY)),VALUATION=CONVERT(NUMERIC(18,2),sum(DPHMCD_CURR_QTY)*ISNULL(CLOPM_CDSL_RT,0)), CONVERT(NUMERIC(18,3),sum(DPHMCd_FREE_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMCD_FREEZE_QTY))      
		 ...
		 ,CONVERT(NUMERIC(18,3),sum(DPHMCD_ELIMINATION_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY))
,CONVERT(NUMERIC(18,3),sum(DPHMCD_AVAIL_LEND_QTY)),CONVERT(NUMERIC(18,3),sum(DPHMCD_LEND_QTY))
,CONVERT(NUMERIC(18,3),sum(DPHMCD_BORROW_QTY)),DPAM_ID, HOLDING_DT=CONVERT(VARCHAR(11),dphmcd_holding_dt,109),CMP=ISNULL(convert(numeric(18,2),CLOPM_CDSL_RT),0.00)    
		 ...
		 #ACLIST ACCOUNT  with (nolock)                              
		 WHERE  DPHMCD_DPM_ID = @@DPMID                          
		 AND DPHMCD_DPAM_ID = ACCOUNT.DPAM_ID                      
		 AND (DPHMCD_HOLDING_DT BETWEEN EFF_FROM AND EFF_TO)                    
		 AND (CONVERT(NUMERIC,DPAM_SBA_NO) BETWEEN CONVERT(NUMERIC,@PA_FROMACCID) AND  CONVERT(NUMERIC,@PA_TOACCID))                         
		 AND DPHMCD_ISIN LIKE @PA_ISINCD + '%'      
		-- AND ISNULL(DPHMCD_CURR_QTY,0) <> 0  
group BY DPAM_SBA_NO,ISIN_NAME,DPHMCD_ISIN ,DPAM_SBA_NAME  ,CLOPM_CDSL_RT,dpam_id   ,dphmcd_holding_dt       
...
SELECT distinct DPAM_SBA_NAME,DPAM_SBA_NO,DPHMCD_ISIN,ISIN_NAME,CONVERT(NUMERIC(18,3),DPHMCD_CURR_QTY),VALUATION=CONVERT(NUMERIC(18,2),DPHMCD_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0)), CONVERT(NUMERIC(18,3),DPHMCD_FREE_QTY),CONVERT(NUMERIC(18,3),DPHMCD_FREEZE_QTY)      
		 ...
		 ,CONVERT(NUMERIC(18,3),DPHMCD_ELIMINATION_QTY),CONVERT(NUMERIC(18,3),DPHMCD_EARMARK_QTY)
,CONVERT(NUMERIC(18,3),DPHMCD_AVAIL_LEND_QTY),CONVERT(NUMERIC(18,3),DPHMCD_LEND_QTY)
,CONVERT(NUMERIC(18,3),DPHMCD_BORROW_QTY),DPAM_ID, HOLDING_DT=CONVERT(VARCHAR(11),dphmcd_holding_dt,109),CMP=ISNULL(convert(numeric(18,2),CLOPM_CDSL_RT),0.00)    
		 FROM #tmphldg with (nolock)               
		 LEFT OUTER JOIN ISIN_MSTR with (nolock) ON DPHMCD_ISIN = ISIN_CD              
		 LEFT OUTER JOIN CLOSING_PRICE_MSTR_cDSL with (nolock) ON DPHMCD_ISIN = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = DPHMCD_ISIN and CLOPM_DT <= @PA_FORDATE and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc)   ,                    
		 #ACLIST ACCOUNT with (nolock)                                
