-- Object: PROCEDURE citrus_usr.pr_client_multireport_KJMC211010
...
CREATE proc [citrus_usr].[pr_client_multireport_KJMC211010](@pa_id numeric  
  ...
  ,@l_filter_hiercy varchar(2000)  
   ...
   begin   
     ...
     begin
							...
							begin
								set @l_sql = ' select convert(datetime ,accp_value,103) as accp_value, ACCP_CLISBA_ID into #account_properties from account_properties 
											where accp_accpm_prop_cd like ''ACC_CLOSE_DT''
											and ltrim(rtrim(accp_value)) not in (''/ /'', ''/ /'','''',''//'') 
											and left(accp_value,2)<>''00''  select distinct '  
							
							set @l_sql = @l_sql + @l_sql1  
							if @pa_grp_by ='HO'  
							set @l_sql = @l_sql + ' , citrus_usr.[fn_find_relations_nm](DPAM_CRN_NO,''HO'') Groupby  '   
							if @pa_grp_by ='BR'  
							set @l_sql = @l_sql + ' , citrus_usr.[fn_find_relations_nm](DPAM_CRN_NO,''BR'')  Groupby  '   
							if @pa_grp_by ='CLIENT'  
							set @l_sql = @l_sql + ' , DPAM_SBA_NO Groupby  '   
					  
							set @l_sql2 = ' from client_mstr clim , dp_acct_mstr dpam left outer join client_bank_accts on cliba_clisba_id=dpam.dpam_id left outer join bank_mstr on cliba_banm_id=banm_id, dp_holder_dtls dphd , entity_relationship entr, client_dp_brkg clidb, brokerage_mstr brom , client_ctgry_mstr clicm ,sub_ctgry_mstr subcm , entity_type_mstr enttm , status_mstr stam , #account_properties accp'  
							set @l_sql2 = @l_sql2  + ' where clim.clim_crn_no = dpam.dpam_crn_no and dpam.dpam_sba_no = dphd.dphd_dpam_sba_no and entr.entr_sba = dpam.dpam_sba_no and dpam.dpam_enttm_cd = enttm.enttm_cd and dpam.dpam_clicm_cd = clicm.clicm_cd and dpam.dpam_stam_cd = stam.stam_cd and dpam.dpam_id = clidb.clidb_dpam_id and  dpam_subcm_cd = subcm.subcm_cd and clidb.clidb_brom_id = brom.brom_id and dpam.dpam_id = accp.ACCP_CLISBA_ID'  
							set @l_sql2 = @l_sql2  + ' and clim.clim_deleted_ind = 1 and dpam.dpam_deleted_ind = 1 and entr.entr_deleted_ind = 1 and clicm.clicm_deleted_ind = 1 and enttm.enttm_deleted_ind = 1 and dphd.dphd_deleted_ind =1 and stam.stam_deleted_ind = 1 and clidb_deleted_ind = 1 and clidb_eff_to_dt > getdate() and brom.brom_Deleted_ind = 1 '  
							...
							set @l_sql2 = ' from client_mstr clim , dp_acct_mstr dpam left outer join client_bank_accts on cliba_clisba_id=dpam.dpam_id left outer join bank_mstr on cliba_banm_id=banm_id, dp_holder_dtls dphd , entity_relationship entr, client_dp_brkg clidb, brokerage_mstr brom , client_ctgry_mstr clicm ,sub_ctgry_mstr subcm , entity_type_mstr enttm , status_mstr stam '  
							set @l_sql2 = @l_sql2  + ' where clim.clim_crn_no = dpam.dpam_crn_no and dpam.dpam_sba_no = dphd.dphd_dpam_sba_no and entr.entr_sba = dpam.dpam_sba_no and dpam.dpam_enttm_cd = enttm.enttm_cd and dpam.dpam_clicm_cd = clicm.clicm_cd and dpam.dpam_stam_cd = stam.stam_cd and dpam.dpam_id = clidb.clidb_dpam_id and  dpam_subcm_cd = subcm.subcm_cd and clidb.clidb_brom_id = brom.brom_id '  
							set @l_sql2 = @l_sql2  + ' and clim.clim_deleted_ind = 1 and dpam.dpam_deleted_ind = 1 and entr.entr_deleted_ind = 1 and clicm.clicm_deleted_ind = 1 and enttm.enttm_deleted_ind = 1 and dphd.dphd_deleted_ind =1 and stam.stam_deleted_ind = 1 and clidb_deleted_ind = 1 and clidb_eff_to_dt > getdate() and brom.brom_Deleted_ind = 1 '  
							set @l_sql = @l_sql + @l_sql2 + case when  @pa_filter_values <> '' then ' and ' else '' end   
							end
		...
		--  if citrus_usr.fn_splitval(@pa_main_filters_values ,1) <> '' 
							...
							begin 
								...
								begin  
									set @l_sql5 = @l_sql5 + ' and DPAM_LST_UPD_DT >= ''' +  convert(varchar(11),convert(datetime,citrus_usr.fn_splitval(@pa_main_filters_values ,9),103) ,109)  + ''' '  
								end
							end
