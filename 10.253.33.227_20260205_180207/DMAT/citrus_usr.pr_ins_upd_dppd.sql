-- Object: PROCEDURE citrus_usr.pr_ins_upd_dppd
...
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_dppd](@pa_id                varchar(8000)  
                                ,@pa_crn_no            numeric  
                                ,@pa_action            char(3)  
...
--  
  ...
  --  
   ...
   ,dppd_deleted_ind  
   ,dppd_master_id
    FROM   dp_poa_dtls  WITH (NOLOCK) , dp_acct_mstr  WITH (NOLOCK) 
    WHERE  dppd_deleted_ind = 1   
	and dppd_dpam_id = dpam_id and dpam_crn_no = @pa_crn_no
  --                            
  END--b  
  --  
...
--RETURN
            ...
            SET @l_action_type      =  case when citrus_usr.fn_splitval(@currstring,24)<>'' then citrus_usr.fn_splitval(@currstring,24) else citrus_usr.fn_splitval(@currstring,25) end       --citrus_usr.fn_splitval(@currstring,22)  
            SET @l_int_dppd_id      = case when @l_action_type = 'A' then citrus_usr.fn_splitval(@currstring,21) else citrus_usr.fn_splitval(@currstring,20) end --citrus_usr.fn_splitval(@currstring,20)  --citrus_usr.fn_splitval(@currstring,21)  
            SET @l_id               = CASE WHEN @l_action_type      ='D' THEN citrus_usr.fn_splitval(@currstring,20) ELSE @l_id END  
            print 'eerere111'
            print @l_action_type
              
              print citrus_usr.fn_splitval(@currstring,22) -- A
            --  
            --  
            IF @pa_chk_yn = 0  
            BEGIN  
            ...
            --  
              ...
              (
                ...
                AND    dpam_deleted_ind IN (0,8) 
				            AND    dpam_sba_no                  = @l_acct_no
				union
				SELECT  DPAM_ID---@l_dpam_id         =  dpam_id
                FROM   dp_acct_mstr        WITH (NOLOCK)  
                      ,dp_mstr             
                WHERE  dpam_crn_no        = @pa_crn_no  
                AND    dpam_dpm_id        = dpm_id  
   ...
   print @l_dpam_id
              ...
              print 'tudhar'
                ...
                --FROM  dp_poa_dtls WITH (NOLOCK)  
                  ...
                  SELECT @l_dppd_id  = ISNULL(MAX(dppd_id),0) +1 FROM  dp_poa_dtls_mak    
                    SELECT @l_dppd_id1  = ISNULL(MAX(dppd_id),0) +1 FROM   dp_poa_dtls    
                                                          
                    IF @l_dppd_id1 > @l_dppd_id    
                    BEGIN    
                    --    
                      SET @l_dppd_id = @l_dppd_id1    
                    --    
                    END  
