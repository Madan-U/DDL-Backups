-- Object: PROCEDURE citrus_usr.E_DIS_Slip_gen_Process_Bkp_20210302
...
Create proc [citrus_usr].[E_DIS_Slip_gen_Process_Bkp_20210302] (@sett_no varchar(11) ,@flag varchar(1)='')
as 
	  ...
	  --   Declare @fsett_no varchar(16)

		 --select @fsett_no=sett_No from [196.1.115.197].msajag.dbo.sett_mst where start_date in (
		 --select start_date from [196.1.115.197].MSAJAG.dbo.sett_mst where sett_No =@sett_No and sett_type ='N')
		 --and sett_type ='F'


		INSERT  INTO #DIY_Reprocess_log
		---Exec [196.1.115.197].msajag.dbo.Rpt_ShortagePayin 'broker','broker','2020018','2020018','N','%',-1
		Exec [196.1.115.197].msajag.dbo.Rpt_DelPayinMatch 'broker', 'broker', @sett_no,'N','ALL','%',2

		INSERT  INTO #DIY_Reprocess_log
		--Exec [196.1.115.197].msajag.dbo.Rpt_ShortagePayin 'broker','broker',@sett_no,@sett_no,'W','%',-1
		Exec [196.1.115.197].msajag.dbo.Rpt_DelPayinMatch 'broker', 'broker', @sett_no,'W','ALL','%',2

		--INSERT  INTO #DIY_Reprocess_log
		----Exec [196.1.115.197].msajag.dbo.Rpt_ShortagePayin 'broker','broker',@sett_no,@sett_no,'W','%',-1
		--Exec [196.1.115.197].msajag.dbo.Rpt_DelPayinMatch 'broker', 'broker', @Fsett_no,'F','ALL','%',2


		select party_code,certno,sum(qty) as qty into #hld from [196.1.115.197].msajag.dbo.deltrans 
	...
	--	print @cnt
		...
		select @sdate=start_date from [196.1.115.197].msajag.dbo.sett_mst where sett_no=@sett_no and sett_type='N'
		...
		UPDATE D SET DP_ID = Cltdpid FROM DIY_Reprocess_log_e_dis D,[196.1.115.197].MSAJAG.DBO.CLIENT4 C  
