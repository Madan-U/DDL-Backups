-- Object: PROCEDURE citrus_usr.PR_TBL_CLIENT_MASTER
...
create proc  [CITRUS_USR].[PR_TBL_CLIENT_MASTER](@pa_boid numeric) 
as  
...
,convert(varchar(16),'' ) POA_ID  ---convert(varchar(16),POARegNum ) POA_ID  
, '' POA_NAME  -- poam_name1  POA_NAME  
...
left outer join entity_relationship  with(nolock) on dpam_crn_no = entr_crn_no and 
...
from  bank_addresses_dtls with(nolock) group by TMPBA_DP_BR,TMPBA_DP_BANK) bankaddr  on pc1.DivBankCd = bankaddr.TMPBA_DP_BR  and pc1.DivIFScd = bankaddr.TMPBA_DP_BANK  
...
where dpam.DPAM_SBA_NO = pc1.BOId and pc1.BOId =@pa_boid   
and DPAM_CLICM_CD = clicm_cd  
and DPAM_STAM_CD = stam_Cd  
and DPAM_ENTTM_CD  = enttm_Cd  
and DPAM_subcm_CD  = SUBCM_CD  and isnumeric(dpam_sba_no)=1
