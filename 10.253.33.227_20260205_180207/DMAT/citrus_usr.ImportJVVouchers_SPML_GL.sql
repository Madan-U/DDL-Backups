-- Object: PROCEDURE citrus_usr.ImportJVVouchers_SPML_GL
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--BEGIN TRAN
--select * from ledger1 order by 1 desc --24
--select * from accountbal1 order by 1 desc --13
--ImportJVVouchers_SPML_GL 1257,3,1,'MAR 31 2010','MAR 31 2010','HO'
--SELECT ldg_instrument_no,* FROM LEDGER3 ORDER BY 2 DESC
--ROLLBACK
--commit
CREATE Proc [citrus_usr].[ImportJVVouchers_SPML_GL]
@pa_task_id bigint,
@pa_dpmid int,
@pa_finid int,
...
select top 1 @@voucher_dt = convert(varchar(11),TMPJV_TODATE,109) from tmp_cmjv_mstr 
...
select top 1 @@refadd =  isnull(TMPJV_filler1,'') from tmp_cmjv_mstr
select @@total_rec_cnt = isnull(count(TMPJV_SR_NO),0) from tmp_cmjv_mstr where (TMPJV_CREDIT <> 0 or TMPJV_DEBIT <> 0)
...
update tmp_cmjv_mstr 
set    TMPJV_dpam_id = FINA_ACC_ID 
from   FIN_ACCOUNT_MSTR 
where  TMPJV_acct_no = FINA_ACC_CODE 
and    TMPJV_dpm_id = FINA_DPM_ID 
and    FINA_DPM_ID = @pa_dpmid
and	   fina_deleted_ind = 1



	IF EXISTS(SELECT TMPJV_dpam_id FROM tmp_cmjv_mstr WHERE isnull(TMPJV_dpam_id,0) = 0) 																					
	BEGIN
	--
		UPDATE filetask
...
set @@ssql = 'insert into #tmpfiller(filler) select top 1 isnull(TMPJV_FILLER1,'''') from tmp_cmjv_mstr'
print @@ssql
...
select LDG_ID=' + convert(varchar,isnull(@@lst_curr_id,0)) + ' + 1 ,LDG_DPM_ID=' + convert(varchar,@pa_dpmid) + ',LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''01'',LDG_VOUCHER_NO=' + convert(varchar,@@Vno) + ',1,LDG_REF_NO=''' + @@ref_no + ''',''' + @@voucher_dt + ''',' + convert(varchar,@@glaccountid) + ',LDG_ACCOUNT_TYPE=''G'',sum(TMPJV_CREDIT-TMPJV_DEBIT)*-1,LDG_NARRATION=case when LEFT(TMPJV_FILLER1,3)=''AMC'' then ''BEING AMT DEBITED TOWARDS AMC CHARGES'' when LEFT(TMPJV_FILLER1,3)=''OTH'' then ''BEING AMT DEBITED TOWARDS TRANSACTION CHARGES'' else ''BEING AMOUNT OF DEMAT INCENTIVE  FOR THE M/O FEB & MAR - 2010'' end ,LDG_BANK_ID='''',LDG_ACCOUNT_NO='''',LDG_INSTRUMENT_NO=TMPJV_FILLER1,LDG_BANK_CL_DATE='''',LDG_COST_CD_ID=0,LDG_BILL_BRKUP_ID=0,LDG_TRANS_TYPE=''AC'',LDG_STATUS=''P'',''' + @pa_loginname + ''',getdate(),''' + @pa_loginname + ''',getdate(),1,0 
from tmp_cmjv_mstr 
WHERE (TMPJV_CREDIT <> 0 OR TMPJV_DEBIT <> 0) GROUP BY TMPJV_FILLER1 '

exec(@@ssql)

Set @@ssql = 'insert into ledger' + convert(varchar,@pa_finid) + ' (LDG_ID,LDG_DPM_ID,LDG_VOUCHER_TYPE,LDG_BOOK_TYPE_CD,LDG_VOUCHER_NO,LDG_SR_NO,LDG_REF_NO,LDG_VOUCHER_DT,LDG_ACCOUNT_ID,LDG_ACCOUNT_TYPE,LDG_AMOUNT,LDG_NARRATION,LDG_BANK_ID,LDG_ACCOUNT_NO,LDG_INSTRUMENT_NO,LDG_BANK_CL_DATE,LDG_COST_CD_ID,LDG_BILL_BRKUP_ID,LDG_TRANS_TYPE,LDG_STATUS,LDG_CREATED_BY,LDG_CREATED_DT,LDG_LST_UPD_BY,LDG_LST_UPD_DT,LDG_DELETED_IND,LDG_BRANCH_ID)
select LDG_ID=' + convert(varchar,isnull(@@lst_curr_id,0)) + ' + TMPJV_SR_NO,LDG_DPM_ID=' + convert(varchar,@pa_dpmid) + ',LDG_VOUCHER_TYPE=''3'',LDG_BOOK_TYPE_CD=''01'',LDG_VOUCHER_NO=' + convert(varchar,@@Vno) + ',TMPJV_SR_NO,LDG_REF_NO=''' + @@ref_no + ''',''' + @@voucher_dt + ''',TMPJV_DPAM_ID,LDG_ACCOUNT_TYPE=''G'',CASE WHEN TMPJV_CREDIT <> 0 THEN TMPJV_CREDIT  ELSE TMPJV_DEBIT *-1 END  ,LDG_NARRATION=TMPJV_NARRATION,LDG_BANK_ID='''',LDG_ACCOUNT_NO='''',LDG_INSTRUMENT_NO=TMPJV_FILLER1,LDG_BANK_CL_DATE='''',LDG_COST_CD_ID=0,LDG_BILL_BRKUP_ID=0,LDG_TRANS_TYPE=''AC'',LDG_STATUS=''P'',''' + @pa_loginname + ''',getdate(),''' + @pa_loginname + ''',getdate(),1,0 
from tmp_cmjv_mstr WHERE (TMPJV_CREDIT <> 0 OR TMPJV_DEBIT <> 0)'

exec(@@ssql)
-- CHANGE HERE FROM P TO G
...
set @@ssql          = @@ssql + ' from tmp_cmjv_mstr '
