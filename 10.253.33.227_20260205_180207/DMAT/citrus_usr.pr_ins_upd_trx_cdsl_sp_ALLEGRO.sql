-- Object: PROCEDURE citrus_usr.pr_ins_upd_trx_cdsl_sp_ALLEGRO
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--pr_ins_upd_trx_cdsl '0','EP','INS','HO','12345678','1234567890123456','9','11','2008068','20/04/2009','0M075765','3','N','20/04/2009','11001100','','11','','11001100','A|*~|INC000243997|*~|23|*~|*|~*','',1,1,'0','*|~*','|*~|',''   
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_trx_cdsl_sp_ALLEGRO](@pa_id              VARCHAR(8000)    
													 ,@pa_tab             VARCHAR(20)  
...
set @pa_cash_trf      = case when @pa_cash_trf = 'NA' then 'X' end       
IF @pa_tab = 'BOBO'  
BEGIN  
--  
...
--  set @pa_mkt_type      = @l_mkt_type_n  
  ...
  if @pa_tr_setm_no <> '' and  @pa_settlm_no  <> ''
  begin
    set @pa_tr_setm_no    = @l_setm_no_n  
	set @pa_settlm_no     = @pa_settlm_no   
  end
  else
  begin
    set @pa_tr_setm_no    = @pa_settlm_no  
   ...
   if @pa_tr_setm_type <> '' and  @pa_mkt_type  <> ''
  begin  
...
--  
  ...
  --  
    ...
    --  
      ...
      Select @l_max_high_val1 = bitrm_bit_location from bitmap_ref_mstr where bitrm_parent_cd = 'HIGH_VAL_CDSL'    and bitrm_deleted_ind = 1  
      ...
      set @l_high_val_yn        = case when @l_val_yn = 'Y' then 'Y' else case when [citrus_usr].[fn_get_DIS_high]('SAVE', '', @pa_values) > @l_max_high_val1 then 'Y' else 'N' end end 
    --  
    END  
    ELSE  
    BEGIN  
    ...
    END  
          ...
          --  
            ...
            --  
              ...
              BEGIN  
              --  
                delete used from dp_trx_dtls_cdsl,dp_acct_mstr,used_slip used where dpam_id = dptdc_dpam_id and uses_dpam_acct_no = dpam_sba_no and dptdc_dtls_id = convert(INT,@currstring)                    
       ...
       , @pa_trdReasoncd               
            ...
            --  
              ...
              IF exists(SELECT * FROM dptdc_mak WHERE dptdc_dtls_id = convert(numeric,@currstring) and dptdc_deleted_ind = 0)  
              BEGIN  
              --  
                if @l_action = 'D'   
                BEGIN  
            ...
            --  
              ...
              --        
                 ...
                 BEGIN  
                 --  
                   delete used from dp_trx_dtls_cdsl,dp_acct_mstr,used_slip used where dpam_id = dptdc_dpam_id and uses_dpam_acct_no = dpam_sba_no and dptdc_id          = convert(numeric,@c_dptd_id)   
            ...
            --  
              ...
              BEGIN TRANSACTION  
               ...
               AND    dptdc_deleted_ind in (0,4,6)  
