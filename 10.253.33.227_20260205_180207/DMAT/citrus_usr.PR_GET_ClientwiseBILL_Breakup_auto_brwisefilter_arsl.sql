-- Object: PROCEDURE citrus_usr.PR_GET_ClientwiseBILL_Breakup_auto_brwisefilter_arsl
...
CREATE  PROC [citrus_usr].[PR_GET_ClientwiseBILL_Breakup_auto_brwisefilter_arsl](@pa_from_dt DATETIME, @pa_to_dt DATETIME    ,@pa_login_name varchar(250)
...
declare @l_st numeric(18,4)
select @l_st  = amount from servicetax_mstr where @pa_from_dt between from_dt and to_dt and deleted_ind = 1 
...
AND CDSHM_CDAS_SUB_TRAS_TYPE ='521' and CDSHM_TRADE_NO = '' 
...
,case when clic_charge_name  like  '***%' then sum(charge_amt)  else 0.000 end   [Other charges]          
,case when (clic_charge_name = 'SERVICE TAX')  or (clic_charge_name ='***service tax')  then sum(charge_amt) else 0.000 end [ServiceTax]          
,case when (clic_charge_name like 'SWACHH BHARAT CESS%')    then sum(charge_amt) else 0.000 end [SWACHH BHARAT CESS @ 0.50%]          
--,'' [FR-Sharing]          
into #finaldata1 from #tempdata left outer join entity_relationship           
on entr_sba = dpam_sba_no           
and @pa_to_dt  between ENTR_FROM_DT and isnull(ENTR_TO_DT,'dec 31 2900')           
left outer join entity_mstr on  (entr_br = entm_id  )           
...
,abs(sum([Other charges]          ))[Other charges]          
,abs(sum([ServiceTax]  ) )[ServiceTax]          
,abs(sum([SWACHH BHARAT CESS @ 0.50%]    ) ) [SWACHH BHARAT CESS @ 0.50%]          
...
----,convert(numeric(30,2),abs(sum([SWACHH BHARAT CESS @ 0.50%]   ))) [SWACHH BHARAT CESS @ 0.50%]   
--,convert(numeric(30,2),abs(sum([TotalCost])+sum([TotalCost]*@l_st))) [Total CDSL TRX Charge with service tax]  -- into tempdataclient        
--,convert(numeric(30,2),abs(sum([SWACHH BHARAT CESS @ 0.50%]   ))) [SWACHH BHARAT CESS @ 0.50%]
...
select CDSHM_BEN_ACCT_NO ,cdshm_internal_trastm , SUM(cdshm_charge)  trxcharge into #temptrxdata 
from cdsl_holding_dtls_bak_forreve where cdshm_charge <> 0  
group by CDSHM_BEN_ACCT_NO , cdshm_internal_trastm 
order by 1,2 
...
,convert(numeric(30,2),abs(sum([AdministrationCharge+Cdslcorporateamccharges]))) [Administraton Charges]           
,convert(numeric(30,2),abs(sum( [AccountOpeingCharge+Document]))  )[DP DOCUMENT]        
,convert(numeric(30,2),abs(sum([AMCBilled]))+abs(sum([LIfetimeamc]))) [AMC charge]          
,abs(sum([CorporateAMC])) [corporate amc]
,convert(numeric(30,2),abs(sum([TransactionBilled]))) [TransactionBilled]          
,isnull(abs((select top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm = 'EP-DR' )),0) [EP-DR]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm ='BSEBOPAYIN-DR' )),0) [BSEBOPAYIN-DR]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm = 'NSCCL-DR' )),0) [NSCCL-DR]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm = 'INTDEP-DR' )),0) [INTDEP-DR]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm = 'BO' )),0) [BO]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm = 'OF-DR' )),0) [OF-DR]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm = 'REMAT' )),0) [REMAT]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm = 'ON-DR' )),0) [ON-DR]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm = 'DEMAT' )),0) [DEMAT]
,isnull(abs((select  top 1  trxcharge from #temptrxdata where cdshm_ben_acct_no = a.[dpam_sba_no] and cdshm_internal_trastm not in ('BO'
,'BSEBOPAYIN-DR'
,'DEMAT'
,'EP-DR'
,'INTDEP-DR'
...
,convert(numeric(30,2),abs(sum([DematcouirerChargeBilled]))  ) [DEMAT/REMAT (set up)]          
,convert(numeric(30,2),abs(sum([DematRejection])) ) [DEMAT/REMAT (rejection)]
,convert(numeric(30,2),abs(sum([Other charges]                    )) ) [Other charges]  
,convert(numeric(30,2),abs(sum([ServiceTax]))) [ServiceTax]     
,convert(numeric(30,2),abs(sum([SWACHH BHARAT CESS @ 0.50%]      ))) [SWACHH BHARAT CESS @ 0.50%]      
,convert(numeric(30,2),abs(sum([TotalCost])+sum([TotalCost]*@l_st))) [Total CDSL TRX Charge with service tax]  -- into tempdataclient        
 from #finaldata a, dp_acct_mstr b with (nolock)left outer join entity_relationship with (nolock)       
