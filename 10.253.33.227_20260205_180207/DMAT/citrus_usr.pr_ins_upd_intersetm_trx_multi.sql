-- Object: PROCEDURE citrus_usr.pr_ins_upd_intersetm_trx_multi
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[pr_ins_upd_intersetm_trx_multi](@pa_id              VARCHAR(8000)  
...
set @l_val_yn  = citrus_usr.fn_get_high_val('',0,'DORMANT',@pa_cmbp_id,convert(datetime,@pa_req_dt,103))           
          ...
          print @currstring_value								
								  SET @l_action             = citrus_usr.fn_splitval(@currstring_value,1)      
										...
										--
			                                    ...
			                                    SET @l_tr_setm_type		 = citrus_usr.fn_splitval(@currstring_value,4) 
												SET @l_tr_setm_no		 = citrus_usr.fn_splitval(@currstring_value,5) 
...
--																							,dptd_other_settlement_no  = @pa_tr_setm_no
  ...
  IF left(ltrim(rtrim(@pa_errmsg)),5) <> 'ERROR'
		BEGIN
  --
    DECLARE @PA_ACCT_NO  VARCHAR(20)
    SELECT @PA_ACCT_NO = dpam_sba_no FROM dp_acct_mstr ,ACCOUNT_PROPERTIES , ACCOUNT_PROPERTY_MSTR 
    WHERE accp_clisba_id = dpam_id  and ACCP_ACCPM_PROP_ID = ACCPM_PROP_ID AND ACCPM_PROP_CD = 'CMBP_ID' AND ACCP_VALUE = @PA_CMBP_ID
   
    exec pr_checkslipno '','INTERSETM_TRX_SEL', @pa_dpm_dpid,@PA_ACCT_NO ,@pa_slip_no,@pa_login_name,''
  --
  END
--
END
