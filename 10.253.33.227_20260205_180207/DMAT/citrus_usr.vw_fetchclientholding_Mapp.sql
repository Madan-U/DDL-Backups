-- Object: VIEW citrus_usr.vw_fetchclientholding_Mapp
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--select * from vw_fetchclientholding_Mapp where dphmcd_dpm_id = 3 and dphmcd_holding_dt = 'may 05 2012'
CREATE view [citrus_usr].[vw_fetchclientholding_Mapp]                       
as    
  
select dphmcd_dpm_id ,dpam_sba_no , dphmcd_dpam_id , dphmcd_isin                    
...
,DPHMCD_CURR_QTY  
 ...
 ,DPHMCD_FREEZE_QTY  
 ,DPHMCD_PLEDGE_QTY   
 ,case when not exists(select cdshm_dpam_id , cdshm_isin,CDSHM_TRAS_DT  from cdsl_holding_dtls  with(nolock) 
          WHERE CDSHM_TRAS_DT  >='aug 01 2011' and CDSHM_TRAS_DT   <= datevalu and cdshm_tratm_cd in ('2246','2277')    
...
where DPHMCd_HOLDING_DT = 'JUL 31 2011' --in (select isnull(min(CDSHM_TRAS_DT)-1,DPHMCd_HOLDING_DT) from citrus_usr.cdsl_holding_dtls )          
) a  , citrus_usr.l_datalist   ,dp_acct_mstr                   
where  cdshm_tras_dt <= datevalu     
and  dpam_id=a.DPHMCD_dpam_id   
                     
  
) b   
group by dphmcd_dpm_id,dpam_sba_no,dphmcd_holding_dt,dphmcd_dpam_id,dphmcd_isin   ,dphmcd_cntr_settm_id                    
