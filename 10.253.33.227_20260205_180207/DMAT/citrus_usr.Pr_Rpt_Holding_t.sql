-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_t
...
CREATE PROC [citrus_usr].[Pr_Rpt_Holding_t]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
alter table  #tmphldg add tradingid varchar(100)
	...
	insert into  #tmphldg
	select a.* from holdingallforview a, DP_aCCT_MSTR WHERE DPAM_ID = DPHMCD_DPAM_ID
	...
	insert into  #tmphldg
	select a.* from holdingallforview a, DP_aCCT_MSTR ,#bbocode WHERE DPAM_ID = DPHMCD_DPAM_ID and dpam_sba_no = clientid
	AND (CONVERT(NUMERIC,DPAM_SBA_NO) BETWEEN CONVERT(NUMERIC,@PA_FROMACCID) AND  CONVERT(NUMERIC,@PA_TOACCID)) 
...
--  ,tradingid
 SELECT distinct DPAM_SBA_NAME,DPAM_SBA_NO,DPHMCD_ISIN,replace(ISIN_NAME,'*','') ISIN_NAME,CONVERT(NUMERIC(18,3),sum(DPHMCD_CURR_QTY)),
  VALUATION=isnull((CONVERT(NUMERIC(18,3),sum(DPHMCD_FREE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_FREEZE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_PLEDGE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_DEMAT_PND_VER_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_REMAT_PND_CONF_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) )*ISNULL(CLOPM_CDSL_RT,0),0) ---CONVERT(NUMERIC(18,2),sum(DPHMCD_CURR_QTY)*ISNULL(CLOPM_CDSL_RT,0)),
