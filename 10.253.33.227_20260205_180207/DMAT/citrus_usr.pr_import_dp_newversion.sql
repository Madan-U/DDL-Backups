-- Object: PROCEDURE citrus_usr.pr_import_dp_newversion
...
CREATE PROCeDURE [citrus_usr].[pr_import_dp_newversion]( @pa_crn_no      VARCHAR(8000)                  
                            , @pa_exch        VARCHAR(10)                  
...
--  set @pa_to_dt   = @pa_to_dt + +' 23:59:59.999'
  ...
  --                  
    ...
    , ben_bank_IFSC varchar(11),noofpoa_fh VARCHAR(10),NOOFPOA_SH VARCHAR(10),NOOFPOA_TH VARCHAR(10)
    )                  
    --      
    ...
    --                  
      ...
      --      
        ...
        , dpam_sholder_sign                   
        , dpam_tholder_sign                   
        , ben_bank_IFSC , noofpoa_Fh , noofpoa_Sh , noofpoa_Th
        )     
        ...
        SELECT 0                  
            ...
            ,  CASE WHEN  convert(char(1),ISNULL(citrus_usr.fn_ucc_accp(dpam.dpam_id,'ADR_PREF_FLG',''),'')) <> '1' THEN 'Y'  
             ...
             , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'') <> '' then convert(char(13),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'')) ELSE convert(char(13),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'MOBILE1'),''))            end                                       --mobile no of first holder  --12                  
             ...
             , convert(CHAR(13),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'SH_MOBILE'),''))                                             --mobile no of second holder  --12                  
             ...
             ,  convert(CHAR(13),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'TH_MOBILE'),''))                                                --mobile no of third holder  --12                  
    ...
    , isnull(banm_rtgs_cd,''),(SELECT ISNULL(COUNT(*),0) FROM DP_POA_DTLS WHERE DPPD_DPAM_ID = DPAM_ID AND DPPD_POA_TYPE = 'FULL' AND DPPD_HLD = '1ST HOLDER' AND DPPD_DELETED_IND = 1)
		,(SELECT ISNULL(COUNT(*),0) FROM DP_POA_DTLS WHERE DPPD_DPAM_ID = DPAM_ID AND DPPD_POA_TYPE = 'FULL' AND DPPD_HLD = '2ND HOLDER' AND DPPD_DELETED_IND = 1)
		,(SELECT ISNULL(COUNT(*),0) FROM DP_POA_DTLS WHERE DPPD_DPAM_ID = DPAM_ID AND DPPD_POA_TYPE = 'FULL' AND DPPD_HLD = '3RD HOLDER' AND DPPD_DELETED_IND = 1)
       ...
       INSERT INTO @l_dp_pri_dtls(ln_number                  
        ...
        SELECT 0                  
           ...
           ,  CASE WHEN  convert(char(1),ISNULL(citrus_usr.fn_ucc_accp(dpam.dpam_id,'ADR_PREF_FLG',''),'')) <> '1' THEN 'Y'  
             ...
             ,isnull(banm_rtgs_cd,'')
,(SELECT ISNULL(COUNT(*),0) FROM DP_POA_DTLS WHERE DPPD_DPAM_ID = DPAM_ID AND DPPD_POA_TYPE = 'FULL' AND DPPD_HLD = '1ST HOLDER' AND DPPD_DELETED_IND = 1)
		,(SELECT ISNULL(COUNT(*),0) FROM DP_POA_DTLS WHERE DPPD_DPAM_ID = DPAM_ID AND DPPD_POA_TYPE = 'FULL' AND DPPD_HLD = '2ND HOLDER' AND DPPD_DELETED_IND = 1)
		,(SELECT ISNULL(COUNT(*),0) FROM DP_POA_DTLS WHERE DPPD_DPAM_ID = DPAM_ID AND DPPD_POA_TYPE = 'FULL' AND DPPD_HLD = '3RD HOLDER' AND DPPD_DELETED_IND = 1)
...
--			   + CONVERT(CHAR(45), ISNULL(min_nom_gur_name,''))
			   + '00000000'--+ CONVERT(CHAR(8), ISNULL(CONVERT(CHAR(8),dob_min,112),''))
			   + '00000000'--+ CONVERT(CHAR(8), ISNULL(CONVERT(CHAR(8),don_min_nom,112),''))
               + convert(char(1),stan_instr_ind)                
               + convert(char(30),ben_bank_acct_no)                  
               + convert(char(35),ben_bank_name)                     
               + convert(char(11),ben_bank_IFSC)     
               + SPACE(89)                                          
               ...
               + SPACE(15)                                         
			   + CONVERT(CHAR(5),noofpoa_Fh) 
               + convert(char(50),fh_email)                        
               + SPACE(8)     
               + convert(char(13),fh_mob_no)                       
...
--               + SPACE(5)      
               + SPACE(5)  
				+ CONVERT(CHAR(5),noofpoa_Sh)     
                                  + SPACE(4)  
               + convert(char(50),sh_email)                        
               --+ convert(char(9),sh_map_id)                        
               + SPACE(8)  
               + case when sh_mob_no = '' then convert(char(13),'0')  else   convert(char(13),sh_mob_no) end
               + convert(char(1),sh_sms_fac)                       
               + SPACE(1)                                          
               + convert(char(1),sh_pan_flg)                       
               + SPACE(5)
               + CONVERT(CHAR(5),noofpoa_Th)                                          
               + SPACE(4)
               + convert(char(50),th_email)              
--               + convert(char(9),th_map_id)   
			   + SPACE(8)                      
               + case when th_mob_no = '' then convert(char(13),'0')  else   convert(char(13),th_mob_no) end
               --+ convert(char(13),th_mob_no)                      
        ...
        --                  
          ...
          SELECT --'02'                  
              ...
              -- + 'A'                                                                            
               ...
               + SPACE(4)           
               + convert(char(50),sh_email)                        
                + SPACE(8)         --+ convert(char(9),sh_map_id)                        
               + case when sh_mob_no = '' then convert(char(13),'0')  else   convert(char(13),sh_mob_no) end
               + convert(char(1),sh_sms_fac)                       
               + SPACE(1)                                          
               + convert(char(1),sh_pan_flg)                       
               ...
               + CONVERT(CHAR(5),noofpoa_Th)                                        
               + convert(char(50),th_email)                        
                + SPACE(8)          --+ convert(char(9),th_map_id)                       
              -- + convert(char(13),th_mob_no)                      
               + case when th_mob_no = '' then convert(char(13),'0')  else   convert(char(13),th_mob_no) end
               + convert(char(1),th_sms_fac)                      
               + SPACE(1)                                         
...
--                                       , '000000'--citrus_usr.fn_get_listing('BANKCCY',isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BANKCCY',''),''))               
                                  ...
                                  FROM   dp_acct_mstr              dpam                  
                                         left outer join   
										   ...
										   left outer join           
                                           bank_mstr                 banm                 on  convert(int,cliba.cliba_banm_id)     = convert(int,banm.banm_id)   AND isnull(cliba_flg,'0') IN ('1','3')          AND    isnull(banm.banm_deleted_ind,1)       = 1         
                                       , client_mstr               clim                  
                                       , entity_type_mstr          enttm        
                                       , client_ctgry_mstr         clicm                  
                                         left outer join                  
