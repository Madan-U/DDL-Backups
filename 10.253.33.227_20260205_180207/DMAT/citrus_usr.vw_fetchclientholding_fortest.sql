-- Object: VIEW citrus_usr.vw_fetchclientholding_fortest
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

create  view [citrus_usr].[vw_fetchclientholding_fortest]                       
as    
...
,DPHMCD_CURR_QTY  
 ...
 ,DPHMCD_FREEZE_QTY  
 ,DPHMCD_PLEDGE_QTY   
 ,case when not exists(select cdshm_dpam_id , cdshm_isin,case when dphmcd_trans_no  = '' then '' else cdshm_trans_no end cdshm_trans_no from cdsl_holding_dtls     with(nolock)   
          WHERE CDSHM_TRAS_DT  >='aug 01 2011' and CDSHM_TRAS_DT   <= datevalu and (cdshm_tratm_cd in ('2246','2277','3102')    or (cdshm_tratm_cd in ('2251') and cdshm_tratm_desc like '%delete%') ) 
         -- AND ABS(CDSHM_QTY) = ABS(DPHMCD_DEMAT_PND_VER_QTY)    
          and cdshm_dpam_id = dphmcd_dpam_id   
       and  cdshm_isin  = dphmcd_isin   
and case when dphmcd_trans_no  = '' then '' else cdshm_trans_no end = dphmcd_trans_no   
       and isnull(DPHMCD_DEMAT_PND_VER_QTY ,0)<>0   
group by  cdshm_dpam_id , cdshm_isin,case when dphmcd_trans_no  = '' then '' else cdshm_trans_no end
having ABS(sum(case when cdshm_tratm_cd  ='2246' then CDSHM_QTY  when cdshm_tratm_cd  ='3102' then cdshm_qty * -1  when cdshm_tratm_cd in ('2251') and cdshm_tratm_desc like '%delete%' then cdshm_qty *-1 else 0 end  )) = ABS(DPHMCD_DEMAT_PND_VER_QTY)  
     --  and isnull(DPHMCD_FREE_QTY ,0)=0   
      -- and isnull(dphmcd_curr_qty,0) = 0 
)    then DPHMCD_DEMAT_PND_VER_QTY else 0 end  DPHMCD_DEMAT_PND_VER_QTY  
