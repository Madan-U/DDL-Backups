-- Object: PROCEDURE citrus_usr.pr_ins_upd_PledgeN_trx
...
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_PledgeN_trx](@pa_id              VARCHAR(8000)        
...
--      
  ...
  ,PLDT_INSTR_TYPE       
  ,PLDT_AGREEMENT_NO
  ,PLDT_SEQ_NO -- latesh on mar 08 2010       
  ,PLDT_PLEDGEE_DPID       
  ,PLDT_PLEDGEE_DEMAT_ACCT_NO       
  ,PLDT_REL_DT       
  ,PLDT_REL_RSN      
...
,pLDT_PLEDGEE_DEMAT_NAME    
            ...
            --      
              ...
              AND    PLDT_DELETED_IND in (0,4,6)      
				...
				DELETE USES 
                FROM USED_SLIP uses , NpledgeD_mak , DP_ACCT_MSTR 
                WHERE PLDT_DPAM_ID = DPAM_ID AND DPAM_SBA_NO =  USES_DPAM_ACCT_NO and isnumeric(REPLACE(PLDT_SLIP_NO,USES_SERIES_TYPE,'')) =1  
                AND USES_SERIES_TYPE + REPLACE(PLDT_SLIP_NO,USES_SERIES_TYPE,'')   = USES_SERIES_TYPE + USES_SLIP_NO
                AND PLDT_dtls_id     = convert(numeric,@currstring)
				and left(PLDT_trastm_Cd,3) = left(uses_trantm_id,3)      
