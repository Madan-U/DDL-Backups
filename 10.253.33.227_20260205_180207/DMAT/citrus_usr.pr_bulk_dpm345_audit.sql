-- Object: PROCEDURE citrus_usr.pr_bulk_dpm345_audit
...
CREATE proc [citrus_usr].[pr_bulk_dpm345_audit]
(  
...
delete from DP_HLDG_MSTR_CDSL_audit where DPHMC_DPM_ID = @l_dpm_id and dphmc_holding_dt = @l_holding_dt

if 3 = (select count(distinct dphmc_holding_dt ) from DP_HLDG_MSTR_CDSL_audit where DPHMC_DPM_ID = @l_dpm_id)
begin 

delete from DP_HLDG_MSTR_CDSL_audit
where DPHMC_DPM_ID = @l_dpm_id 
and dphmc_holding_dt = (select min(dphmc_holding_dt) from DP_HLDG_MSTR_CDSL_audit  where DPHMC_DPM_ID = @l_dpm_id )


end 

--insert remaing data 
insert into DP_HLDG_MSTR_CDSL_audit
select  * from dpm345

if citrus_usr.LastMonthDay(@L_HOLDING_DT)=@L_HOLDING_DT--dbo.ufn_LastBusinessDayOfMonth(@L_HOLDING_DT)=@L_HOLDING_DT --
begin

delete from DP_HLDG_MSTR_CDSL_audit_MONTHEND_HOLDING where DPHMC_DPM_ID = @l_dpm_id and dphmc_holding_dt = @l_holding_dt


 
insert into DP_HLDG_MSTR_CDSL_audit_MONTHEND_HOLDING
select  * from dpm345
