-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_BakAug022018
...
--ROLLBACK
Create procedure [citrus_usr].[pr_billing_cdsl_BakAug022018](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
  ...
  ,post_toacct numeric(10,0), FLG CHAR(1) NULL)
   ...
   create table #temp_bill_accural
  (trans_dt datetime
  ,dpam_id  numeric
  ,charge_name varchar(50)
  ,charge_val  numeric(18,5)
...
--            and   case when chacm_subcm_cd = '0' then '0' else dpam_subcm_cd end = chacm_subcm_cd
			...
			select @pa_billing_to_dt,dpam_id,'CORPORATE AMC',500.00*-1,18
				...
				,'042205','052309','052308','052334','062826'
...
update cdsl_holding_dtls
        set    cdshm_charge   =  
			    ...
			    ,cdshm_post_toacct = CHAM_POST_TOACCT
				  ...
				  and   cdshm_deleted_ind = 1
                  and   CHAM_CHARGE_BASE = 'NORMAL' 
			      and   proc_deleted_ind = 1 and CDSHM_CDAS_SUB_TRAS_TYPE not in ('813','804')
			      and   clopm_deleted_ind = 1
				--  AND   CDSHM_COUNTER_BOID NOT IN ('1206210000002228','1206210000002209')
...
delete from Temp_Bill where Actual_Trans_dt between @pa_billing_from_dt and @pa_billing_to_dt

insert into Temp_Bill(Actual_Trans_dt   
  ,dpam_id  
  ,dpam_sba_no 
  ,charge_name 
  ,charge_val  
 ...
 ,post_toacct
 ,0
 ,null,'MIG',getdate(),'MIG',getdate(),1
 from  #temp_bill_accural a, dp_acct_mstr b where  a.dpam_id = b.dpam_id 
 and  not exists (select 1 from  [196.1.115.239].INHOUSE.DBO.CLIENT_DETAILS where CL_CODE=dpam_sba_no and b2c = 'N')
  
  
  
  update a set Flg = 1 , Bill_charge_dt = @pa_billing_to_dt from Temp_Bill a  , dp_Acct_mstr  b  ,#account_properties
	where a.dpam_id = b.dpam_id and Actual_Trans_dt between  @pa_billing_from_dt and @pa_billing_to_dt and accp_clisba_id = a.dpam_id 
    and  exists (select 1 from  client_dp_brkg where clidb_dpam_id = a.dpam_id and @pa_billing_to_dt 
    between clidb_eff_from_dt and isnull(clidb_eff_to_dt ,'dec 31 2900') and clidb_brom_id in (28,29,30,31,32,33,34,73,74,21,22,3,4,45,6) )
    and Flg = 0 
    --and datediff(dd,accp_value,getdate())<=365
    
    
	update a set Flg = 1 , Bill_charge_dt = @pa_billing_to_dt from Temp_Bill a  , dp_Acct_mstr  b  ,#account_properties
	where a.dpam_id = b.dpam_id and Actual_Trans_dt between  @pa_billing_from_dt and @pa_billing_to_dt and accp_clisba_id = a.dpam_id 
    and  exists (select 1 from  [196.1.115.239].INHOUSE.DBO.CLIENT_DETAILS where CL_CODE=dpam_bbo_code and b2c = 'N' )
    and Flg = 0 
    and datediff(dd,accp_value,getdate())<=365
    
    update a set Flg = 1   , Bill_charge_dt = @pa_billing_to_dt 
    from Temp_Bill a, dp_Acct_mstr  b 
	where a.dpam_id = b.dpam_id --and trans_dt between  @pa_billing_from_dt and @pa_billing_to_dt
    and  exists (select 1 from   [196.1.115.182].general.dbo.Vw_LedgerBal  where client=dpam_bbo_code and net_available > 0 and abs(charge_val)*0.18 < net_available)
	and Flg = 0
	
	
CREATE TABLE #outstanding(ldg_account_id numeric, amount numeric(18,3))


declare @@SSQL1  varchar(5000)
set @@SSQL1 =''
SET @@SSQL1 ='TRUNCATE TABLE #outstanding'
SET @@SSQL1 ='insert into #outstanding select ldg_account_id , sum(ldg_amount) os from ledger' + convert(varchar,@@FINID) + ' where 
ldg_account_type =''P'' and ldg_deleted_ind = 1 group by ldg_account_id ' 
...
--select * from #outstanding
 ...
 update a set Flg = 1   , Bill_charge_dt = @pa_billing_to_dt 
    from Temp_Bill a, dp_Acct_mstr  b 
	where a.dpam_id = b.dpam_id --and trans_dt between  @pa_billing_from_dt and @pa_billing_to_dt
    and  exists (select 1 from   #outstanding  where ldg_account_id=a.dpam_id and amount > 0 and abs(charge_val)*0.18 < amount)
	and Flg = 0
	...
	insert into #temp_bill
						...
						,charge_val
						,post_toacct)
						select Bill_charge_dt 
						,dpam_id  
						,charge_name + ' ' +  case when Actual_Trans_dt  < @pa_billing_from_dt then +'(' +  convert(varchar(11),Actual_Trans_dt ,103) + ')' else '' end 
						,charge_val
						,post_toacct from Temp_Bill where Bill_charge_dt between  @pa_billing_from_dt and @pa_billing_to_dt
						and Flg = 1


	--update a set Flg = 1   , Bill_charge_dt = @pa_billing_from_dt 
 --   from Temp_Bill a, dp_Acct_mstr  b 
	--where a.dpam_id = b.dpam_id and trans_dt between  @pa_billing_from_dt and @pa_billing_to_dt
 --   and  exists (select 1 from   Vw_LedgerBal  
 --   where client=dpam_bbo_code and net_available > 0 
 --   and abs(charge_amt)*0.18 < net_available)
	--and Flg = 0
...
update cdsl_holding_dtls set cdshm_charge = 0 , cdshm_dp_charge = 0  FROM cdsl_holding_dtls , ISIN_MSTR
