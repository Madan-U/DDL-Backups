-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_bak17102008
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

create PROCEDURE  [citrus_usr].[pr_dp_select_mstr_bak17102008]( @pa_id             VARCHAR(20)                
...
WHERE setm.setm_deleted_ind   = 1               
  ...
  --            
   ...
   and    trantm_deleted_ind       = 1          
   and    TRASTM_CD     in ('202','042','033','011','012','021','022','052','071','212','091','092','093')          
   order by trastm_cd          
    end          
  ...
  --          
    SELECT DISTINCT @l_dpm_dpid                    DPID          
          ...
          ,demrm_qty                   QUANTITY          
          ,a.descp    DEMRM_INTERNAL_REJ  
    ,b.descp   DEMRM_COMPANY_OBJ   
  ...
  --              
    ...
    FROM demrd_mak              
    WHERE  demrd_demrm_id    =  @pa_id                
    AND    demrd_deleted_ind IN(0,4,6)              
  ...
  --              
    ...
    and   SLIBM_SERIES_TYPE       like case when @pa_desc = '' then '%' else @pa_desc  end                
    and   slibm_book_name >= @pa_cd and  slibm_book_name <= @pa_rmks               
    and slibm_deleted_ind = 1              
  ...
  --              
    ...
    END              
      ...
      SELECT compm_short_name +'-'+ excsm_exch_cd+'-'+excsm_seg_cd COMPANY                 
        ...
        AND    sliim_series_type         LIKE CASE WHEN LTRIM(RTRIM(@pa_cd)) = '' THEN '%' ELSE @pa_cd END                
...
BEGIN      
  ...
  BEGIN      
   ...
   AND    frzm.fre_deleted_ind in (0,4,6)      
    --       
  END      
  ELSE      
  BEGIN      
  ...
  --              
     SELECT compm_short_name +'-'+ excsm_exch_cd+'-'+excsm_seg_cd  Depository                
      ...
      FROM INWARD_SLIP_REG inwsr 
           ...
           ,  dp_acct_mstr     dpam                 
           ,  dp_mstr dpm                 
           , citrus_usr.fn_getsubtransdtls('slip_TYPE_NSDL') ttype               
           , company_mstr                       
           , exch_seg_mstr excsm                
           , citrus_usr.fn_exch_list(@pa_roles,@pa_scr_id) excsm_list                
  ...
  --              
     SELECT compm_short_name +'-'+ excsm_exch_cd+'-'+excsm_seg_cd  Depository                
           ...
           FROM INWARD_SLIP_REG inwsr 
                ...
                ,  dp_acct_mstr     dpam                 
                ,  dp_mstr dpm                 
                , citrus_usr.fn_getsubtransdtls('slip_TYPE_CDSL') ttype               
                , company_mstr                       
                , exch_seg_mstr excsm                
                , citrus_usr.fn_exch_list(@pa_roles,@pa_scr_id) excsm_list
           WHERE inwsr.inwsr_dpam_id = dpam.dpam_id                
...
FINA_DPM_ID              
 ...
 IF @pa_action ='VOUCHERCONTRA_SEL'                
  BEGIN                
   --                
    SELECT DISTINCT ldg_voucher_no   VOUCHERNO               
   ...
   BEGIN                
    ...
    IF @pa_values <> ''              
      ...
      SELECT DISTINCT ldg_voucher_no   VOUCHERNO               
       ...
       FROM    ledger1_mak              
        ,CITRUS_USR.FN_gl_ACCT_LIST(@pa_rmks,@pa_id,0)                
       WHERE ldg_deleted_ind IN (0,4,6)                 
       AND ldg_sr_no = 1              
       AND ldg_voucher_type = 4              
       AND dpam_id    = ldg_account_id              
       AND ldg_account_type = acct_type              
...
ORDER BY  ldg_id              
 ...
 IF @pa_action ='VOUCHERPAY_SELM'                
  ...
  IF @pa_action ='VOUCHERCONTRA_SELC'                
     ...
     ,ldg_book_type_cd              
              ...
              ,ldg_deleted_ind              
      FROM    ledger1_mak                        
        ,CITRUS_USR.FN_gl_ACCT_LIST(@pa_values,@pa_id,0)                
      WHERE ldg_sr_no = 1              
      AND dpam_id    = ldg_account_id              
      AND ldg_account_type = acct_type              
      AND ldg_voucher_type = 4              
      ...
      AND ldg_dpm_id = convert(int,@pa_values)              
      ORDER BY  ldg_id                 
     --                
 END               
 ...
 IF @pa_action ='VOUCHERCONTRA_DTLS_SEL'                
   BEGIN                
   --                
     SELECT distinct ldg_account_id = dpam_id              
 ...
 IF @pa_action ='VOUCHERCONTRA_DTLS_SELM'                
    BEGIN                
    --                
      SELECT distinct ldg_account_id              
      ,fina_acc_code = dpam_sba_no              
      ,fina_acc_name = dpam_sba_name              
    --,ldg_branch_id              
    --,entm_short_name              
...
,  CONVERT(VARCHAR,dpm_id) + '|*~|' + CONVERT(VARCHAR(11),fin_start_dt,109) + '|*~|' +  CONVERT(VARCHAR(11),fin_end_dt,109) + '|*~|' + CONVERT(VARCHAR,fin_id)  + '|*~|' + CONVERT(VARCHAR,EXCSM_ID)  + '|*~|'  id             
 ...
 --              
      ...
      begin              
  SELECT case when datediff(n,convert(varchar(11),setm.setm_deadline_dt,109) + ' ' + left(replACE(setm.setm_deadline_time,':',''),2) + ':' + right(replACE(setm.setm_deadline_time,':',''),2),getdate()) < 0 then 'N'               
        else case when datediff(n,convert(varchar(11),setm.setm_deadline_dt,109) + ' ' + left(replACE(setm.setm_deadline_time,':',''),2) + ':' + right(replACE(setm.setm_deadline_time,':',''),2),getdate()) > @pa_rmks then 'N'               
        else convert(varchar,datediff(n,convert(varchar(11),setm.setm_deadline_dt,109) + ' ' + left(replACE(setm.setm_deadline_time,':',''),2) + ':' + right(replACE(setm.setm_deadline_time,':',''),2),getdate())) end end       DeadLineDate               
  FROM  settlement_mstr     setm              
       ,settlement_type_mstr  settm              
...
11),INWSR_EXEC_DT,103),Trans_cnt=INWSR_NO_OF_TRANS,scr_cd = INWSR_TRASTM_CD,Scr_id= citrus_usr.Fn_GetSlipScreens(INWSR_TRASTM_CD,'2')              
  ...
  --              
    ...
    select @l_ss   = sliim_id               
        from slip_issue_mstr               
                 ...
                 and sliim_deleted_ind =1               
                 and ((convert(numeric,@pa_desc) between convert(numeric,sliim_slip_no_fr) and convert(numeric,sliim_slip_no_to))  or (convert(numeric,@pa_rmks) between convert(numeric,sliim_slip_no_fr) and convert(numeric,sliim_slip_no_to)))             
