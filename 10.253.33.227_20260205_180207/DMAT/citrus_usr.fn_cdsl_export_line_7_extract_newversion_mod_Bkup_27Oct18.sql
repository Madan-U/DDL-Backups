-- Object: FUNCTION citrus_usr.fn_cdsl_export_line_7_extract_newversion_mod_Bkup_27Oct18
...
create  function [citrus_usr].[fn_cdsl_export_line_7_extract_newversion_mod_Bkup_27Oct18](@pa_crn_no      VARCHAR(8000)    
...
--          ,case when isnull(clim_name3,'')='' then '.' else  isnull(clim_name3,'.') end     
    ...
    ,''                
          ,''--,case when CLIM_GENDER in ('M','MALE') then 'MR'  when CLIM_GENDER in ('F','FEMALE') then 'MS' else 'M/S' end                              
          ,''                
          ,case when climmod.clic_mod_action in ('P Address') then ISNULL(DPHD_FH_FTHNAME,'') else '' end  --father/husband name                    
           , case when climmod.clic_mod_action in ('P Address') then case when citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),1) <> ''   
               then citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),1)   
               else citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'PER_ADR1'),''),1) end else ''  end --adr1                    
        , case when climmod.clic_mod_action in ('P Address') then case when citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),1) <> ''  
               then citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),2)  
               else citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'PER_ADR1'),''),2) end else '' end --adr2                    
        , case when climmod.clic_mod_action in ('P Address') then case when citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),1) <> ''  
               then citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),3)  
               else  citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'PER_ADR1'),''),3) end else '' end--adr3                    
        , case when climmod.clic_mod_action in ('P Address') then case when citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),1) <> ''  
               then citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),4)  
               else citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'PER_ADR1'),''),4) end else '' end--adr_city                    
        , case when climmod.clic_mod_action in ('P Address') then case when citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),1) <> ''  
               then citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),5)  
               else citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'PER_ADR1'),''),5) end else '' end--adr_state                    
        , case when climmod.clic_mod_action in ('P Address') then case when citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),1) <> ''  
               then citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),6)  
               else citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'PER_ADR1'),''),6) end else '' end--adr_country                    
        , case when climmod.clic_mod_action in ('P Address') then case when citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),1) <> ''  
               then citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_PER_ADR1'),''),7)  
               else citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'PER_ADR1'),''),7) end else '' end --adr_zip                  
		  ...
		  ,case when climmod.clic_mod_action in ('P Address') then case when ISNULL(CONVERT(char(8),replace(convert(varchar,clim.clim_dob,103),'/','')),'') ='0101900' then '00000000' else ISNULL(CONVERT(char(8),replace(convert(varchar,clim.clim_dob,103),'/','')),'')  end  else '' end
          ,''--isnull(citrus_usr.fn_conc_value(clim.clim_crn_no,'FAX1'),'')--fax                    
          ,''--isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'PAN_GIR_NO',''),'')  --pan card                    
          ,''    
          ,case when climmod.clic_mod_action in ('First EmailId Delete') then replicate('@',50) else '' end--email                               
          ,''--usr_txt1                                   
          ,''--usr_txt2                                   
          ,0000--''--usr_fld3                                   
--          ,0000--''--usr_fld4    
   ...
   ,CASE WHEN exists (select 1 from dps8_pc12 where BOId = DPAM_SBA_NO and PurposeCode12 = '12' and TypeOfTrans = '3') THEN 'S' WHEN not exists (select 1 from dps8_pc12 where BOId = dpam_sba_no  and PurposeCode12 = '12') then 'S' WHEN exists (select 1 from dps8_pc12 where BOId = DPAM_SBA_NO and PurposeCode12 = '12' and (TypeOfTrans = '1' or TypeOfTrans ='2' or TypeOfTrans ='')) THEN 'M' ELSE 'D' END  
		  ...
		  ,CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'PRIIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'PRIIC'),'')) else '000000' END -- pri mob isd                   
 ...
 AND   climmod.clic_mod_action in ('P Address','First EmailId Delete')  
...
--          ,dpam_sba_no                    
--          ,'206' --''--Purpose_code                               
--          ,max(case when climmod.clic_mod_action = 'N NAMECHANGE' then isnull(DPHD_NOM_FNAME,'') else '' end)  
--          ,max(case when climmod.clic_mod_action = 'N NAMECHANGE' then isnull(DPHD_NOM_MNAME,'') else '' end)    
--          ,max(case when climmod.clic_mod_action = 'N NAMECHANGE' then (case when isnull(DPHD_NOM_LNAME,'')='' then '.' else isnull(DPHD_NOM_LNAME,'.') end) else '' end)  --isnull(DPHD_NOM_LNAME,'')       
--    --,''  
--    --,''  
--    --,''  
--          ,max(case when climmod.clic_mod_action = 'NOMINEE TITLE' then isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'NOM_TITLE',''),'') else '' end)--,''''--,case when DPHD_NOM_GENDER in ('M','MALE') then 'MR'  when DPHD_NOM_GENDER in ('F','FEMALE') then 'MS' else 'M/S' end                       
--          ,max(case when climmod.clic_mod_action = 'Nominee Suffix' then isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'NOM_SUFFIX',''),'') else '' end)--,''                
--          ,max(case when climmod.clic_mod_action = 'N FatherNameChange' then  isnull(DPHD_NOM_FTHNAME,'') else '' end)   
--          ,max(case when climmod.clic_mod_action = 'N Address' then  (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'NOMINEE_ADR1'),''),1)) else '' end)--adr1                    
--          ,max(case when climmod.clic_mod_action = 'N Address' then  (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'NOMINEE_ADR1'),''),2)) else '' end)--adr2                    
--          ,max(case when climmod.clic_mod_action = 'N Address' then  (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'NOMINEE_ADR1'),''),3)) else '' end)--adr3                    
--          ,max(case when climmod.clic_mod_action = 'N Address' then  (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'NOMINEE_ADR1'),''),4)) else '' end)--adr_city                    
--          ,max(case when climmod.clic_mod_action = 'N Address' then  (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'NOMINEE_ADR1'),''),5)) else '' end)--adr_state                    
--          ,max(case when climmod.clic_mod_action = 'N Address' then  (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'NOMINEE_ADR1'),''),6)) else '' end)--adr_country                    
--          ,max(case when climmod.clic_mod_action = 'N Address' then  (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'NOMINEE_ADR1'),''),7)) else '' end)--adr_zip                                 
--          ,max(case when climmod.clic_mod_action = 'N MobChange' then (isnull(citrus_usr.fn_fetch_ph('NOMINEE',dpam.dpam_id,1,'Y'),'')) else '' end)
--          ,max(case when climmod.clic_mod_action = 'N MobChange' then (isnull(citrus_usr.fn_fetch_ph('NOMINEE',dpam.dpam_id,1,'N'),'')) else '' end)
--          ,max(case when climmod.clic_mod_action = 'N ResPhChange' then (isnull(citrus_usr.fn_fetch_ph('NOMINEE',dpam.dpam_id,2,'Y'),'')) else '' end)
--          ,max(case when climmod.clic_mod_action = 'N ResPhChange' then (isnull(citrus_usr.fn_fetch_ph('NOMINEE',dpam.dpam_id,2,'N'),'')) else '' end)
--          ,max(case when climmod.clic_mod_action = 'N OffPhChange' then (isnull(citrus_usr.fn_fetch_ph('NOMINEE',dpam.dpam_id,3,'N'),'')) else '' end)
--		  ,max(case when climmod.clic_mod_action = 'N DOBChange' then (case when ISNULL(CONVERT(char(8),replace(convert(varchar,dphd_nom_dob,103),'/','')),'') ='0101900' then '00000000' else ISNULL(CONVERT(char(8),replace(convert(varchar,dphd_nom_dob,103),'/','')),'')  end) else '' end )
--          ,max(case when climmod.clic_mod_action = 'N FaxChange' then (isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'NOMINEE_FAX1'),'')) else '' end)--fax  
--          ,max(case when climmod.clic_mod_action = 'N PANChange' then isnull(DPHD_NOM_PAN_NO,'') else '' end      )
--          --,''
--          --,''
--          ,''    
--          ,''    
...
--		,max(citrus_usr.fn_get_modified_action_mod(DPAM_CRN_NO,climmod.clic_mod_action))
		
--        ,0000--''--usr_fld5  
--        		  ,'01'
--		  , max(citrus_usr.fn_get_cd_rel( isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRELATN_BO',''),'')))
--		  , max(citrus_usr.fn_get_perc(isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NPER_SHR',''),'')))
--		  ,max(case when  isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRED_SEC_FLG','') ,'')                   ='YES' then 'Y' else 'N' end)     
		  
--		  ,max(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('Country',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'NOMINEE_ADR1'),''),6 )),'')) -- cntry code
--,max(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('State',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'NOMINEE_ADR1'),''),5 )),''))  -- state code
--,max(isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'NOMINEE_ADR1'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'NOMINEE_ADR1'),''),4)),'' )) -- seqno
--,max(CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) else '000000' END) -- pri mob isd                   
...
--    and isnull(climmod.clic_mod_batch_no,0) = 0  
-- AND   climmod.clic_mod_action in ('N ADDRESS','NOMINEE DEL','N NAMENDTLS','N CONTACTS','N NAMECHANGE','Nominee Suffix','N FatherNameChange', 'N MobChange', 'N ResPhChange', 'N OffPhChange','N DOBChange', 'N FaxChange', 'N PANChange','NOMINEE TITLE')
-- --AND   climmod.clic_mod_action in ('N ADDRESS','NOMINEE DEL','N ResPhChange','N OffPhChange','N MobChange','N EmailChange','N NameChange','N FatherNameChange','N DOBChange','N PANChange','N GenderChange')    
--        group by clim_crn_no, dpam_sba_no
...
SELECT distinct '07'                  
		...
		,nom_Addphone
--,isnull(nom_dob,'')
,ISNULL(CONVERT(char(8),replace(convert(varchar,convert(datetime,nom_dob),103),'/','')),'')
         ,nom_fax--, isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'NOMINEE_FAX1'),'')--fax
		  ...
		  ,nom_res_sec_flag--case when  isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRED_SEC_FLG','') ,'')                   ='YES' then 'Y' else 'N' end 
		    ...
		    ,(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('Country',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE1_ADR1'),''),6 )),'')) -- cntry code
,(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('State',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE1_ADR1'),''),5 )),''))  -- state code
,(isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE1_ADR1'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE1_ADR1'),''),4)),'' )) -- seqno
,(CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) else space(6) END) -- pri mob isd                   
    ...
    AND    clim.clim_deleted_ind   = 1    
	and climmod.clic_mod_dpam_sba_no = dpam.DPAM_SBA_NO               
    --AND    clim_lst_upd_dt    BETWEEN CONVERT(DATETIME, @pa_from_dt + ' 00:00:00',103) AND CONVERT(DATETIME, @pa_to_dt+ ' 23:59:59',103)                          
    ...
    and		climmod.clic_mod_deleted_ind = 1              
    and		isnull(climmod.clic_mod_batch_no,0) = 0  
	AND		climmod.clic_mod_action in ('NOMINEE1')
...
,ISNULL(CONVERT(char(8),replace(convert(varchar,convert(datetime,nom_dob),103),'/','')),'')
         ,nom_fax--, isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'NOMINEE_FAX1'),'')--fax
		  ...
		  ,nom_res_sec_flag--case when  isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRED_SEC_FLG','') ,'')                   ='YES' then 'Y' else 'N' end 
		   ...
		   ,(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('Country',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE2_ADR1'),''),6 )),'')) -- cntry code
,(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('State',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE2_ADR1'),''),5 )),''))  -- state code
,(isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE2_ADR1'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE2_ADR1'),''),4)),'' )) -- seqno
,(CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) else space(6) END) -- pri mob isd                   
    ...
    and		climmod.clic_mod_deleted_ind = 1              
    and		isnull(climmod.clic_mod_batch_no,0) = 0  
	AND		climmod.clic_mod_action in ('NOMINEE2')
...
,ISNULL(CONVERT(char(8),replace(convert(varchar,convert(datetime,nom_dob),103),'/','')),'')
         ,nom_fax--, isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'NOMINEE_FAX1'),'')--fax
		  ...
		  , isnull(citrus_usr.fn_get_perc(nom_percentage),'')
		  ,nom_res_sec_flag--case when  isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRED_SEC_FLG','') ,'')                   ='YES' then 'Y' else 'N' end 
		   ,(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('Country',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE3_ADR1'),''),6 )),'')) -- cntry code
,(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('State',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE3_ADR1'),''),5 )),''))  -- state code
,(isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE3_ADR1'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE_ADR1'),''),4)),'' )) -- seqno
,(CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) else space(6) END) -- pri mob isd                   
    ...
    and		climmod.clic_mod_deleted_ind = 1              
    and		isnull(climmod.clic_mod_batch_no,0) = 0  
	AND		climmod.clic_mod_action in ('NOMINEE3')
    UNION    
    ...
    SELECT distinct '07'                    
          ...
          ,dpam_sba_no                    
          ,'107' --''--Purpose_code                               
          ,case when climmod.clic_mod_action = 'G NameChange' then isnull(DPHD_GAU_FNAME,'') else '' end   
          ,case when climmod.clic_mod_action = 'G NameChange' then isnull(DPHD_GAU_MNAME,'') else '' end
          ,case when climmod.clic_mod_action = 'G NameChange' then (case when isnull(DPHD_GAU_LNAME,'')='' then '.' else isnull(DPHD_GAU_LNAME,'.') end) else '' end --isnull(DPHD_GAU_LNAME,'')      
    --,''  
    --,''  
    --,''  
          ,case when climmod.clic_mod_action = 'GUARDIAN TITLE' then isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'GUA_TITLE',''),'') else '' end--''--,case when DPHD_GAU_gender in ('M','MALE') then 'MR'  when DPHD_GAU_gender in ('F','FEMALE') then 'MS' else 'M/S' end                       --,''--,case when DPHD_GAU_GENDER in ('M','MALE') then 'MR'  when DPHD_GAU_GENDER in ('F','FEMALE') then 'MS' else 'M/S' end                 
          ,case when climmod.clic_mod_action = 'Guardian Suffix' then isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'GUA_SUFFIX',''),'') else '' end--,''                
          ,case when climmod.clic_mod_action = 'G FatherNameChange' then  DPHD_GAU_FTHNAME else '' end
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),1)) else '' end--adr1
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),2)) else '' end--adr2                    
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),3)) else '' end--adr3                    
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),4)) else '' end--adr_city                    
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),5)) else '' end--adr_state                    
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),6)) else '' end--adr_country                    
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),7)) else '' end--adr_zip                                 
          ,case when climmod.clic_mod_action = 'G MobChange' then (isnull(citrus_usr.fn_fetch_ph('GUARD',dpam.dpam_id,1,'Y'),'')) else '' end
          ,case when climmod.clic_mod_action = 'G MobChange' then (isnull(citrus_usr.fn_fetch_ph('GUARD',dpam.dpam_id,1,'N'),'')) else '' end
          ,case when climmod.clic_mod_action = 'G ResPhChange' then (isnull(citrus_usr.fn_fetch_ph('GUARD',dpam.dpam_id,2,'Y'),'')) else '' end
          ,case when climmod.clic_mod_action = 'G ResPhChange' then (isnull(citrus_usr.fn_fetch_ph('GUARD',dpam.dpam_id,2,'N'),'')) else '' end
          ,case when climmod.clic_mod_action = 'G OffPhChange' then (isnull(citrus_usr.fn_fetch_ph('GUARD',dpam.dpam_id,3,'N'),'')) else '' end
		  ,case when climmod.clic_mod_action = 'G DOBChange' then (case when ISNULL(CONVERT(char(8),replace(convert(varchar,dphd_gau_dob,103),'/','')),'') ='0101900' then '00000000' else ISNULL(CONVERT(char(8),replace(convert(varchar,dphd_gau_dob,103),'/','')),'')  end ) else '' end
          ,''    
          ,case when climmod.clic_mod_action = 'G PANChange' then DPHD_GAU_PAN_NO else '' end 
          --,''  
          --,''
          ,''  
          ,''    
...
--          ,0000--''--usr_fld4     
    ...
    , CASE WHEN  exists (select 1 from dps8_pc7 where BOId = DPAM_SBA_NO and PurposeCode7 = '07' AND TypeOfTrans = '3' ) THEN 'S' 
    WHEN not exists (select 1 from dps8_pc7 where BOId = DPAM_SBA_NO and PurposeCode7 = '07' ) THEN 'S' 
     WHEN exists (select 1 from dp_holder_dtls where dphd_dpam_id = dpam_id and DPHD_GAU_FNAME = '') then 'D'
     when exists (select 1 from dps8_pc7 where BOId = DPAM_SBA_NO and PurposeCode7 = '07' 
     AND (TypeOfTrans = '1' or TypeOfTrans = '2' or TypeOfTrans = '') ) THEN 'M' else 'D' 
      END
          ,0000--''--usr_fld5   
          ,'','','00000',''     
		  
		   ,(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('Country',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'Guard_ADR1'),''),6 )),'')) -- cntry code
,(isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('State',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'Guard_ADR1'),''),5 )),''))  -- state code
,(isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'Guard_ADR1'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'Guard_ADR1'),''),4)),'' )) -- seqno
		  ,CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'GUAIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'GUAIC'),'')) else '000000' END -- pri mob isd                                    
 ...
 AND   climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'     
    ...
    and isnull(climmod.clic_mod_batch_no,0) = 0  
 AND   climmod.clic_mod_action in ('G Address','G NameNDtls','G Contacts','Guardian Suffix', 'G NameChange','G FatherNameChange' ,'G MobChange', 'G ResPhChange', 'G OffPhChange', 'G DOBChange', 'G PANChange','GUARDIAN TITLE')    
...
--		  ,CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMGIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMGIC'),'')) else '000000' END-- pri mob isd                   
...
,(isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'nominee_ADR1'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'Guard_ADR1'),''),4)),'' )) -- seqno
,(CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) else space(6) END) -- pri mob isd                   
    ...
    AND    dphd.nom_deleted_ind   = 1                   
    AND    clim.clim_deleted_ind   = 1     
	and dpam.dpam_sba_no = climmod.clic_mod_dpam_sba_no              
    --AND    clim_lst_upd_dt    BETWEEN CONVERT(DATETIME, @pa_from_dt + ' 00:00:00',103) AND CONVERT(DATETIME, @pa_to_dt+ ' 23:59:59',103)                          
    --AND    ISNULL(nom_fname,'') <> ''  
    and nom_srno in (4)
	 AND		climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'     
    and		climmod.clic_mod_deleted_ind = 1              
    and		isnull(climmod.clic_mod_batch_no,0) = 0 
    AND		climmod.clic_mod_action in ('NOMINEE1 GAURDIAN')
...
,(CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) else space(6) END) -- pri mob isd                   
    ...
    and		climmod.clic_mod_deleted_ind = 1              
    and		isnull(climmod.clic_mod_batch_no,0) = 0 
    AND		climmod.clic_mod_action in ('NOMINEE2 GAURDIAN')
...
,ISNULL(CONVERT(char(8),replace(convert(varchar,convert(datetime,nom_dob),103),'/','')),'')
    ...
    and		climmod.clic_mod_deleted_ind = 1              
    and		isnull(climmod.clic_mod_batch_no,0) = 0 
   AND		climmod.clic_mod_action in ('NOMINEE3 GAURDIAN')
...
----, max(citrus_usr.fn_get_perc(isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NPER_SHR',''),'')))
----, max(case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRED_SEC_FLG','') ,'')                     ='YES' then 'Y' else 'N' end )
--, max( case when climmod.clic_mod_action = 'NOMINEE DEL' then Space (2) else citrus_usr.fn_get_cd_rel( isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRELATN_BO',''),''))end)
--, max(case when climmod.clic_mod_action = 'NOMINEE DEL' then '00000' else citrus_usr.fn_get_perc(isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NPER_SHR',''),''))end)
--, max(case when climmod.clic_mod_action = 'NOMINEE DEL' then Space (1) else (case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRED_SEC_FLG','') ,'')                     ='YES' then 'Y' else 'N' end )end)
...
SELECT distinct '07'                  
		...
		,nom_Addphone
--,isnull(nom_dob,'')
--,ISNULL(CONVERT(char(8),replace(convert(varchar,nom_dob,103),'/','')),'')
,ISNULL(CONVERT(char(8),replace(convert(varchar, convert(datetime,nom_dob),103),'/','')),'')
         ,nom_fax--, isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'NOMINEE_FAX1'),'')--fax
		  ...
		  , nom_relation--isnull(citrus_usr.fn_get_cd_rel( isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRELATN_BO',''),'')),'')
		  , isnull(citrus_usr.fn_get_perc(nom_percentage),'')
		  ,nom_res_sec_flag--case when  isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRED_SEC_FLG','') ,'')      
...
,(CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) else space(6) END) -- pri mob isd                   
    ...
    --,''  
    --,''  
          ,case when climmod.clic_mod_action = 'GUARDIAN TITLE' then isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'GUA_TITLE',''),'') else '' end--''--,case when DPHD_GAU_gender in ('M','MALE') then 'MR'  when DPHD_GAU_gender in ('F','FEMALE') then 'MS' else 'M/S' end                       
          ,case when climmod.clic_mod_action = 'Guardian Suffix' then isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'GUA_SUFFIX',''),'') else '' end--,''                
          ,case when climmod.clic_mod_action = 'G FatherNameChange' then  DPHD_GAU_FTHNAME else '' end
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),1)) else '' end--adr1
          ,case when climmod.clic_mod_action = 'G Address' then (citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'GUARD_ADR'),''),2)) else '' end--adr2                    
...
,(CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> '' THEN  lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) else space(6) END) -- pri mob isd                   
    ...
    AND    clim.clim_deleted_ind   = 1   
	and dpam.dpam_sba_no = climmod.clic_mod_dpam_sba_no                
    --AND    climmod.clic_mod_lst_upd_dt    BETWEEN CONVERT(DATETIME, @pa_from_dt + ' 00:00:00',103) AND CONVERT(DATETIME, @pa_to_dt+ ' 23:59:59',103)                          
    --AND    ISNULL(nom_fname,'') <> ''  
    and nom_srno in (4)
	 AND		climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'     
    and		climmod.clic_mod_deleted_ind = 1              
