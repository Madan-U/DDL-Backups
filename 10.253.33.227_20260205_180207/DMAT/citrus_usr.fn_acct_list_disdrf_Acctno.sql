-- Object: FUNCTION citrus_usr.fn_acct_list_disdrf_Acctno
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

create    Function [citrus_usr].[fn_acct_list_disdrf_Acctno](@pa_dpm_id int ,@pa_ent_id int,@pa_child_id int, @pa_dpam_id numeric )   
returns @temp  TABLE (dpam_id INT,dpam_crn_no NUMERiC,dpam_sba_no varchar(16)  
     ,dpam_sba_name varchar(100),eff_from datetime,eff_to datetime,acct_type char(1)  
...
and DPAM_ID = case when @pa_dpam_id = 0 then DPAM_ID else @pa_dpam_id end 
and exists (select dpamid from  (select dptdc_dpam_id dpamid from dptdc_mak where dptdc_deleted_ind in (1,0,4,6,-1) and dptdc_dpam_id  =case when @pa_dpam_id = 0 then dptdc_dpam_id else @pa_dpam_id end 
union all 
select demrm_dpam_id dpamid  from demrm_mak where demrm_deleted_ind in (1,0,4,6,-1) and demrm_dpam_id   =case when @pa_dpam_id = 0 then demrm_dpam_id else @pa_dpam_id end ) a where a.dpamid = dpam_id  
)
...
,dpam_stam_cd      
  ...
  end   if @l_entem_col_name = 'ENTR_BR'    
  begin    
  insert into @temp(dpam_id,dpam_crn_no,dpam_sba_no,dpam_sba_name,eff_from,eff_to,acct_type ,child_id ,dpam_stam_cd)    
  SELECT dpam.dpam_id ,dpam_crn_no,dpam_sba_no,isnull(dpam_sba_name ,dpam_sba_no)   
     , entr.entr_from_dt     
