-- Object: PROCEDURE citrus_usr.pr_ins_cdsl_freebal
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--SELECT * FROM DP_MSTR WHERE DPM_EXCSM_ID=DEFAULT_DP
--begin tran
--Exec [pr_ins_cdsl_freebal] 'HO','12010907',1,'BULK','D:\BulkInsDbfolder\DPM DPC7(FREE BALANCE)-CDSL FILE\08DPC7U.149097','','',''
--select * from TMP_DPC7_CDSL_TRX_MSTR_MAIN
--rollback

CREATE procedure [citrus_usr].[pr_ins_cdsl_freebal](
...
WHERE  task_id = @pa_task_id
	...
	IF NOT EXISTS(SELECT TOP 1 * FROM TMP_DPC7_CDSL_TRX_MSTR_MAIN WHERE LEFT(VALUESDETAILS,2) IN ('01') 
				AND citrus_usr.fn_splitval_by (VALUESDETAILS,2,'~') = @pa_dpmdpid ) 																					
      BEGIN
      --
        UPDATE filetask
        SET    usermsg = 'ERROR : SELECTED DPID NOT MATCH WITH FILE'
        ,STATUS = 'FAILED'
...
select identity(bigint,1,1) id, * , CONVERT(VARCHAR(16),'') CLIENT_ID into #TMP_DPC7_CDSL_TRX_MSTR_MAIN from TMP_DPC7_CDSL_TRX_MSTR_MAIN 
SELECT @L_DPID = @pa_dpmdpid--DPM_DPID from dp_mstr ,exch_seg_mstr where dpm_excsm_id=excsm_id and excsm_exch_cd='CDSL' AND DPM_EXCSM_ID=DEFAULT_DP

CREATE NONCLUSTERED INDEX ix_VALUESDETAILS ON #TMP_DPC7_CDSL_TRX_MSTR_MAIN
(
    [ID] 
)INCLUDE ( [VALUESDETAILS])
...
UPDATE #TMP_DPC7_CDSL_TRX_MSTR_MAIN 
SET CLIENT_ID  = (CASE WHEN left(VALUESDETAILS,2) in ('01') THEN citrus_usr.fn_splitval_by(VALUESDETAILS,3,'~')  ELSE '' END )

UPDATE A
	SET    A.VALUESDETAILS = A.VALUESDETAILS + '~' + T.CLIENT_ID 
	FROM   (SELECT ID, 
				CLIENT_ID
				FROM   #TMP_DPC7_CDSL_TRX_MSTR_MAIN
				WHERE  CLIENT_ID <> '') T 
	,#TMP_DPC7_CDSL_TRX_MSTR_MAIN A
	WHERE  T.ID = (SELECT TOP 1 ID
					FROM   #TMP_DPC7_CDSL_TRX_MSTR_MAIN S
					WHERE  S.ID < A.ID
					AND S.CLIENT_ID <> ''
					ORDER BY 1 DESC)
	         AND (left(VALUESDETAILS,2) = '04')
...
--
--	UPDATE #TMP_DPC7_CDSL_TRX_MSTR_MAIN 
--	SET    #TMP_DPC7_CDSL_TRX_MSTR_MAIN.VALUESDETAILS = #TMP_DPC7_CDSL_TRX_MSTR_MAIN.VALUESDETAILS + '~' + T.CLIENT_ID 
--	FROM   (SELECT ID, 
--				citrus_usr.fn_splitval_by (VALUESDETAILS,3,'~') CLIENT_ID
--				FROM   #TMP_DPC7_CDSL_TRX_MSTR_MAIN
--				WHERE  citrus_usr.fn_splitval_by(VALUESDETAILS,3,'~') <> ''
--				AND    left(VALUESDETAILS,2) in ('01')
--				) T 
--	WHERE  T.ID = (SELECT TOP 1 ID
--					FROM   #TMP_DPC7_CDSL_TRX_MSTR_MAIN S
--					WHERE  S.ID < #TMP_DPC7_CDSL_TRX_MSTR_MAIN.ID
--					AND citrus_usr.fn_splitval_by(VALUESDETAILS,3,'~') <> ''
--					AND    left(VALUESDETAILS,2) in ('01')
--					ORDER BY 1 DESC)
...
,citrus_usr.fn_splitval_by(VALUESDETAILS,17,'~') from #TMP_DPC7_CDSL_TRX_MSTR_MAIN where left(VALUESDETAILS,2) = '04'
    	...
    	(DPHMC_DPM_ID
					...
					,DPHMC_LST_UPD_BY
					,DPHMC_LST_UPD_DT
					,DPHMC_DELETED_IND,DPHMC_counter_settmid)
					select @l_dpm_id 
					, TMPDPC7_ACCT_ID
					, TMPDPC7_ISIN
					, TMPDPC7_CURR_BAL
...
--      --
----      END				
--	  if not exists (select TOP 100 DPHMCD_DPM_ID,DPHMCD_DPAM_ID,DPHMCD_ISIN from DP_DAILY_HLDG_CDSL WHERE DPHMCD_DPM_ID = @l_dpm_id )
--	  begin 
--		BEGIN TRAN
--		
...
--	  SELECT DPHMC_DPM_ID,DPHMC_DPAM_ID,DPHMC_ISIN,DPHMC_CURR_QTY,DPHMC_FREE_QTY,DPHMC_FREEZE_QTY,DPHMC_PLEDGE_QTY,DPHMC_DEMAT_PND_VER_QTY,DPHMC_REMAT_PND_CONF_QTY,DPHMC_DEMAT_PND_CONF_QTY,DPHMC_SAFE_KEEPING_QTY,DPHMC_LOCKIN_QTY,DPHMC_ELIMINATION_QTY,DPHMC_EARMARK_QTY,DPHMC_AVAIL_LEND_QTY,DPHMC_LEND_QTY,DPHMC_BORROW_QTY,DPHMC_HOLDING_DT,DPHMC_CREATED_BY,DPHMC_CREATED_DT,DPHMC_LST_UPD_BY,DPHMC_LST_UPD_DT,DPHMC_DELETED_IND,DPHMC_counter_settmid
--	  FROM CDSl_FREE_BALANCE WHERE DPHMC_DPM_ID = @L_DPM_ID and  DPHMC_HOLDING_DT = @l_business_dt 
--
--if not exists(select top 1  * from DP_DAILY_HLDG_CDSL	WHERE DPHMCD_DPM_ID = @L_DPM_ID and DPHMCD_HOLDING_DT = @l_business_dt )
--begin 
--       DELETE FROM DP_DAILY_HLDG_CDSL	WHERE DPHMCD_DPM_ID = @L_DPM_ID and DPHMCD_HOLDING_DT = @l_business_dt 
--	 
--	   INSERT INTO DP_DAILY_HLDG_CDSL(DPHMCD_DPM_ID,DPHMCD_DPAM_ID,DPHMCD_ISIN,DPHMCD_CURR_QTY,DPHMCD_FREE_QTY
--,DPHMCD_FREEZE_QTY,DPHMCD_PLEDGE_QTY,DPHMCD_DEMAT_PND_VER_QTY,DPHMCD_REMAT_PND_CONF_QTY,DPHMCD_DEMAT_PND_CONF_QTY
--,DPHMCD_SAFE_KEEPING_QTY,DPHMCD_LOCKIN_QTY,DPHMCD_ELIMINATION_QTY,DPHMCD_EARMARK_QTY,DPHMCD_AVAIL_LEND_QTY
--,DPHMCD_LEND_QTY,DPHMCD_BORROW_QTY,DPHMCD_HOLDING_DT,DPHMCD_CREATED_BY,DPHMCD_CREATED_DT
--,DPHMCD_LST_UPD_BY,DPHMCD_LST_UPD_DT,DPHMCD_DELETED_IND,dphmcd_cntr_settm_id
--)
--	   SELECT DPHMC_DPM_ID,DPHMC_DPAM_ID,DPHMC_ISIN,DPHMC_CURR_QTY,DPHMC_FREE_QTY,DPHMC_FREEZE_QTY,DPHMC_PLEDGE_QTY,DPHMC_DEMAT_PND_VER_QTY,DPHMC_REMAT_PND_CONF_QTY,DPHMC_DEMAT_PND_CONF_QTY,DPHMC_SAFE_KEEPING_QTY,DPHMC_LOCKIN_QTY,DPHMC_ELIMINATION_QTY,DPHMC_EARMARK_QTY,DPHMC_AVAIL_LEND_QTY,DPHMC_LEND_QTY,DPHMC_BORROW_QTY,DPHMC_HOLDING_DT,DPHMC_CREATED_BY,DPHMC_CREATED_DT,DPHMC_LST_UPD_BY,DPHMC_LST_UPD_DT,DPHMC_DELETED_IND,DPHMC_counter_settmid
--	   FROM CDSl_FREE_BALANCE WHERE DPHMC_DPM_ID = @L_DPM_ID and  DPHMC_HOLDING_DT = @l_business_dt 
--end 
