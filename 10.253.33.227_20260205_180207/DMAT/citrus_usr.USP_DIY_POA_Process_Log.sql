-- Object: PROCEDURE citrus_usr.USP_DIY_POA_Process_Log
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------



CREATE Procedure [citrus_usr].[USP_DIY_POA_Process_Log]  
(  
@date datetime  
)  
As  
...
@pa_values varchar(50),@pa_settlm_no varchar(50),@executionndate datetime,@pa_tr_setm_type varchar(10),@pa_tr_acct_no varchar(20);  
   ...
   SELECT @STDATE = MAX(START_DATE) FROM [AngelNseCM].MSAJAG.DBO.SETT_MST WITH(NOLOCK) WHERE START_DATE  < CONVERT(VARCHAR(11),@date,120)
   AND SETT_TYPE ='N'
  ...
  PARTYCODE ClientID,DPID ,SYMBOL,ISIN, Exchange=(CASE WHEN EXCHANGE IN ('1','3') THEN 'NSE'   
                      ...
                      ELSE '' END ) , sum(convert(float, replace(left(Quantity,10),',',''))) Quantity,POAStatus AS ONL,  
  ...
  FROM  
   ...
   UPDATE D SET DP_ID = Cltdpid FROM DIY_POA_Process_Log D,ANGELDEMAT.MSAJAG.DBO.CLIENT4 C  
  ...
  UPDATE D SET DP_ID = Cltdpid FROM DIY_POA_Process_Log D,ANGELDEMAT.BSEDB.DBO.CLIENT4 C  
  WHERE D.Cl_code = C.CL_CODE AND  DEFDP=1 AND Depository ='CDSL'    
  ...
  UPDATE  D SET POA_STATUS_BO ='1'   FROM  DIY_POA_Process_Log D,TBL_CLIENT_POA P  
   WHERE DP_ID=CLIENT_CODE   
    ...
    SELECT  PARTY_CODE,D.SETT_NO,D.SETT_TYPE,I.ISIN ,QTY,'NSE' EXCHANGE INTO #BOSTOCKO  FROM  ANGELDEMAT.MSAJAG.DBO.DELIVERYCLT D ,    
    ANGELDEMAT.MSAJAG.DBO.SETT_MST M,ANGELDEMAT.MSAJAG.DBO.MULTIISIN I  
    ...
    AND VALID =1   
    UNION ALL  
    SELECT  PARTY_CODE,D.SETT_NO,D.SETT_TYPE,I.ISIN ,QTY ,'BSE' EXCHANGE  FROM  ANGELDEMAT.BSEDB.DBO.DELIVERYCLT D ,  ANGELDEMAT.BSEDB.DBO.SETT_MST M,ANGELDEMAT.BSEDB.DBO.MULTIISIN I  
    WHERE  CONVERT(VARCHAR(11),START_dATE,103) =CONVERT(VARCHAR(11),@date,103)  
   ...
   AND ISIN =HLD_ISIN_CODE   
    ...
    SELECT C.* INTO #POOL FROM ANGELDEMAT.MSAJAG.DBO.CLIENT_DEL_HOLD  C ,DIY_POA_Process_Log D
	WHERE C.PARTY_CODE=D.CL_CODE AND C.CERTNO=ISIN  AND  convert(varchar(11),CREATED_DATE,103) =CONVERT(VARCHAR(11),@date,103)

	UPDATE D SET POOL_HOLDING=P.QTY  FROM #POOL P, DIY_POA_Process_Log   D
	 where    POA_STATUS_BO =''  AND  BO_VALIDATION ='Y' AND DP_VALID <> ''  
	 AND P.PARTY_CODE=D.CL_CODE AND P.CERTNO=ISIN  AND  convert(varchar(11),CREATED_DATE,103) =CONVERT(VARCHAR(11),@date,103)
 ...
 'A|*~|'+ISIN+'|*~|'+convert(varchar(10),(case when CONVERT(float,DP_VALID)>=CONVERT(float,VALID_QTY-POOL_HOLDING) then VALID_QTY-POOL_HOLDING else CONVERT(float,DP_VALID) end))+'|*~|*|~*' pa_values,  
...
--update DIY_POA_Process_Log set DP_PROCESS_DT=@date
--from #FinalTable f where DIY_POA_Process_Log.SRNO=f.srno  and ID=@counter
