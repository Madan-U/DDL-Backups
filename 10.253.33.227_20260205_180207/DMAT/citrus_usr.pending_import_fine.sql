-- Object: PROCEDURE citrus_usr.pending_import_fine
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--pending_import_fine '31/05/2008'  
--select * from filetask where convert(varchar,task_start_dt,103) = convert(varchar,'MAy 30 2008',103)  
--select distinct convert(varchar,task_start_dt,103) from filetask  
  
CREATE procedure  [citrus_usr].[pending_import_fine](@pa_import_date varchar(25))  
as  
begin  
--  
  create table #l_temp_table (DPM_DPID varchar(500), SOT CHAR(1), COD char(1), autocorp char(1), CLOP char(1),soh char(1),  trans char(1), ISIN char(1), settlement char(1), client char(1), bp char(1))  
...
select distinct right(ltrim(rtrim(task_name)),8) from filetask where left(right(ltrim(rtrim(task_name)),8),2)  = 'IN'  
...
and    convert(varchar,task_start_dt,103) =  @pa_import_date  
