-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_bak30012012
...
create PROC [citrus_usr].[Pr_Rpt_Holding_bak30012012]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
-------/*feb1010*/ AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(right(dphmc_cntr_settm_id,7),'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END                         
   ...
   INSERT INTO #ACLIST SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,EFF_FROM,EFF_TO FROM CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID)    
     ...
     ,CONVERT(NUMERIC(18,0),DPHMCD_ELIMINATION_QTY),CONVERT(NUMERIC(18,0),DPHMCD_EARMARK_QTY)
,CONVERT(NUMERIC(18,0),DPHMCD_AVAIL_LEND_QTY),CONVERT(NUMERIC(18,0),DPHMCD_LEND_QTY)
,CONVERT(NUMERIC(18,0),DPHMCD_BORROW_QTY),DPAM_ID,HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109)--CONVERT(VARCHAR(11),@@TMPHOLDING_DT,109)
