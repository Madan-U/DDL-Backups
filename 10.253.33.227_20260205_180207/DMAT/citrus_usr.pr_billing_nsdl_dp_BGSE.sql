-- Object: PROCEDURE citrus_usr.pr_billing_nsdl_dp_BGSE
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

create procedure [citrus_usr].[pr_billing_nsdl_dp_BGSE](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
  SET DATEFORMAT dmy
...
--	and   ((NSDHM_BEN_ACCT_TYPE in ('10','11','30','20')) or (NSDHM_BEN_ACCT_TYPE = '14' and nsdhm_book_naar_cd = '093')) 
  ...
  ----transaction value wise charge\
	...
	case when NSDHM_COUNTER_DPM_ID = (select dpm_dpid from dp_mstr where default_dp= dpm_excsm_id and dpm_deleted_ind = 1) 
			   --and nsdhm_book_naar_cd in ('033','042') 
               and nsdhm_book_naar_cd in ('033','042','10001','10002','10004','10005','10006','10008')  
               and dpam_enttm_cd = '03' then 0 else 
               isnull(cham_charge_value,0) * -1 end 
     --isnull(cham_charge_value,0) *  -1
	from nsdl_holding_dtls ,dp_Acct_mstr      
	,dp_charges_mstr
	,profile_charges
	,charge_mstr 
	where nsdhm_dpm_id = @pa_dpm_id and dpam_sba_no = NSDHM_BEN_ACCT_NO
    AND   nsdhm_dpm_id  = dpc_dpm_id
	AND   dpc_profile_id = proc_profile_id
	AND   proc_slab_no  = cham_slab_no
	AND   nsdhm_transaction_dt between  @pa_billing_from_dt and @pa_billing_to_dt
  ...
  ----transaction instruction wise charge
    ...
    and   CHAM_CHARGE_BASE = 'NORMAL'
--	and   ((NSDHM_BEN_ACCT_TYPE in ('10','11','30')) or (NSDHM_BEN_ACCT_TYPE = '14' and nsdhm_book_naar_cd = '093')) 
    and   ((citrus_usr.fn_get_new_edpm_acct_type_for_bill('ACCTTYPE',NSDHM_BEN_ACCT_TYPE,NSDHM_TRANSACTION_DT)  in ('10','11','30')) or (citrus_usr.fn_get_new_edpm_acct_type_for_bill('ACCTTYPE',NSDHM_BEN_ACCT_TYPE,NSDHM_TRANSACTION_DT)  = '14' and nsdhm_book_naar_cd = '093')) 
...
--	AND   cham_charge_type = nsdhm_book_naar_cd
	...
	having (count(NSDHM_DM_TRANS_NO) >= cham_from_factor) and (count(NSDHM_DM_TRANS_NO) <= cham_to_factor) 
		...
		AND   nsdhm_transaction_dt between  @pa_billing_from_dt and @pa_billing_to_dt
		AND   nsdhm_transaction_dt >= dpc_eff_from_dt and  nsdhm_transaction_dt <= dpc_eff_to_dt
		AND   NSDHM_DPM_TRANS_NO = demrm_drf_no
		AND   isnull(cham_charge_graded,0) <> 1
		AND   cham_charge_baseon           = 'TRANSPERSLIP' 
	...
	--	AND   cham_charge_type = nsdhm_book_naar_cd
		...
		AND   nsdhm_transaction_dt between  @pa_billing_from_dt and @pa_billing_to_dt
		AND   nsdhm_transaction_dt >= dpc_eff_from_dt and  nsdhm_transaction_dt <= dpc_eff_to_dt
		AND   ISNULL(REMRM_REPURCHASE_FLG,'N') <>  'Y' 
...
update nsdl_holding_dtls 
		...
		else rEMRM_QTY *(CONVERT(NUMERIC(10,2),cham_charge_value)/CONVERT(NUMERIC(10,2),CHAM_CHARGE_MINVAL))* -1 end    ,0)  
		from nsdl_holding_dtls      
		,dp_charges_mstr
		,profile_charges
		,charge_mstr 
