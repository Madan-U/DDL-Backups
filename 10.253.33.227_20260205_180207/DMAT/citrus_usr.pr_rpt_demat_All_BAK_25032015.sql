-- Object: PROCEDURE citrus_usr.pr_rpt_demat_All_BAK_25032015
...
CREATE PROCEDURE [citrus_usr].[pr_rpt_demat_All_BAK_25032015]                                      
(
@PA_EXCSM_ID INT,
...
INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,isnull(EFF_TO,'2100-12-31 00:00:00.000'),NULL FROM citrus_usr.fn_acct_list(@@L_DPM_ID ,@PA_LOGIN_PR_ENTM_ID,@@l_child_entm_id)		
...
IF @PA_ACTION = 'DMT_REQ_VIOLATION_RPT'                                    
 ...
 --               
  ...
  SELECT DPAM_ID,DPAM_SBA_NO , DPAM_SBA_NAME , DEMRM_ISIN , DEMRM_TRANSACTION_NO DRN, DEMRM_SLIP_SERIAL_NO DRF, DEMRD_FOLIO_NO,DEMRD_DISTINCTIVE_NO_FR,DEMRD_DISTINCTIVE_NO_TO, DEMRD_CERT_NO,DEMRD_QTY,         
  CONVERT(VARCHAR,DEMRM_REQUEST_DT,103) DEMRM_REQUEST_DT, convert(numeric(18,3),ABS(DEMRd_QTY)) QTY,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
  ,ISNULL(DPHD_SH_FNAME,'') SECONDHOLDER,ISNULL(DPHD_TH_FNAME,'') THIRDHOLDER , DEMRM_FREE_LOCKEDIN_YN  LOCKIN, ENTM_NAME1 RTA_NAME         
  ,case when len(dpam_sba_no) = 16 then isnull(citrus_usr.fn_splitval(citrus_usr.fn_addr_value(entm_id,'OFF_ADR1'),1),'') else isnull(isin_adr1,'') end  adr1                          
...
, CONVERT(VARCHAR,DISP_DT,103) disp_dt , ISIN_NAME , ISNULL(DISP_DOC_ID,'0') disp_doc_id,disp_type =ISNULL(disp_type,'N')  
, isnull(isin_contactperson,'') contact_person
, datediff(dd,DISP_DT,getdate()) Duration
,DISP_REMINDER,DISP_DEMRM_ID,DISP_ID
	FROM DEMAT_REQUEST_MSTR , DMAT_DISPATCH,DEMAT_REQUEST_dtls, #ACLIST LEFT OUTER JOIN DP_HOLDER_DTLS DPHD   ON   DPHD_DPAM_ID = DPAM_ID      AND DPHD_DELETED_IND = 1 
...
print 'sss'
	SELECT distinct  '' brcd, '' BRDETAILS,DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,'' DRN
...
SELECT distinct '' brcd,'' BRDETAILS,DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,DEMRM_TRANSACTION_NO DRN
    ,DEMRM_SLIP_SERIAL_NO DRF,DEMRM_DPAM_ID                     
	,CONVERT(VARCHAR,DEMRM_REQUEST_DT,103) DEMRM_REQUEST_DT ,ISNULL(DEMRM_EXECUTION_DT,'') ,convert(numeric(18,3),DEMRM_QTY) QTY, ISNULL(DISP_TYPE,'')                      
	,CONVERT(VARCHAR,DISP_DT,103) ,ISNULL(DISP_DOC_ID,'0'),ISNULL(DISP_NAME,''),ISNULL(DISP_CONF_RECD,'') DISP_CONF_RECD                      
	,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
