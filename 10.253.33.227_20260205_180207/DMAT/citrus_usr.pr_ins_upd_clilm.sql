-- Object: PROCEDURE citrus_usr.pr_ins_upd_clilm
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------


CREATE PROC [citrus_usr].[pr_ins_upd_clilm](@PA_ACTION VARCHAR(100)  
...
declare @@dpmid int
		,@CLIM_RMKS_MAK varchar(255)
		,@CLIM_RMKS varchar(255)
		,@CLIM_RMKS_FINAL varchar(255)
IF @PA_ACTION ='INS'  
begin  
...
and dpam_crn_no=@PA_CLIC_MOD_DPAM_SBA_NO
	...
	IF EXISTS(SELECT 1 FROM name_change_reason_cd WHERE nmcrcd_crn_no = @PA_CLIC_MOD_DPAM_SBA_NO AND nmcrcd_deleted_ind = '0')
	BEGIN
--		set @CLIM_RMKS_FINAL = ''
--		set @CLIM_RMKS = ''
--		set @CLIM_RMKS_MAK = ''
...
--		set @CLIM_RMKS_FINAL = @CLIM_RMKS + '</BR>' + @CLIM_RMKS_MAK
		...
		where nmcrcd_crn_no = @PA_CLIC_MOD_DPAM_SBA_NO 
		AND nmcrcd_deleted_ind = '0'
		

		update client_mstr set CLIM_RMKS = @CLIM_RMKS_FINAL where clim_crn_no = @PA_CLIC_MOD_DPAM_SBA_NO
	END
end 
