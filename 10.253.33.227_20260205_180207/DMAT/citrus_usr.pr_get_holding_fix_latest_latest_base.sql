-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_latest_base
...
--select @p19      
--select * from dp_acct_mstr where dpam_sba_no ='1201090000004621'
--exec [pr_get_holding_fix_latest_latest_base] 3,'JAN 15 2014','JAN 15 2014','1201090000004621','1201090000004621',''    
--exec [pr_get_holding_fix_latest] 3,'JAN 15 2014','JAN 15 2014','1201090005272645','1201090005272645',''   
--select top 100 * from cdsl_holding_dtls where cdshm_dpm_id = 3 order by cdshm_ben_Acct_no desc
--1201090000025122~INE083C01022    
    
    
    
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_latest_base](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
set @minholdingdt  =''
select @minholdingdt   = min(dphmcd_holding_dt) from dp_daily_hldg_cdsl_last_base
print @mintrxdt  
print @minholdingdt   
...
,convert(numeric(18,5),0) DPHMCD_LEND_QTY    
,convert(numeric(18,5),0) DPHMCD_BORROW_QTY    
from dp_daily_hldg_cdsl_last_base a with (nolock) ,dp_acct_mstr  with (nolock) where dphmcd_holding_dt =@minholdingdt    
...
from citrus_usr.dp_daily_hldg_cdsl_last_base with(nolock) ,dp_acct_mstr with(nolock)                      
...
into #holdingdmtpenfing from  dp_daily_hldg_cdsl_last_base with (nolock), DP_aCCT_MSTR  with (nolock)
...
into #holdingdmtpenfing1 from  dp_daily_hldg_cdsl_last_base with (nolock) , DP_aCCT_MSTR with (nolock)
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt=@minholdingdt
and dpam_sba_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
and dphmcd_dpm_id = @PA_DPM_ID
...
into #holdingdmtpenfing2 from  dp_daily_hldg_cdsl_last_base with (nolock), DP_aCCT_MSTR with (nolock)
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt=@minholdingdt
group by dphmcd_dpam_id,dphmcd_isin 
