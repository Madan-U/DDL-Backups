-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_isinwise_latest_base
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_isinwise_latest_base](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
,convert(numeric(18,5),0) DPHMCD_LEND_QTY    
,convert(numeric(18,5),0) DPHMCD_BORROW_QTY    
from dp_daily_hldg_cdsl a with (nolock) ,dp_acct_mstr  with (nolock) where dphmcd_holding_dt =@minholdingdt    
...
from citrus_usr.dp_daily_hldg_cdsl with(nolock) ,dp_acct_mstr with(nolock)                      
