-- Object: PROCEDURE citrus_usr.pr_ins_upd_cmr_rpt
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

CREATE  PROCEDURE  [citrus_usr].[pr_ins_upd_cmr_rpt]  
                              (   
                                    @pa_id            VARCHAR(2000)  
...
begin 
	...
	select distinct @pa_out =  isnull(entr_sba,'')
	from entity_relationship,dp_acct_mstr
	where entr_sba like case when @pa_boid ='' then '%' else @pa_boid end 
	--and entr_sba=account.dpam_sba_no
	and (ENTR_HO =@pa_login_pr_entm_id or
	ENTR_RE=@pa_login_pr_entm_id or
	ENTR_AR=@pa_login_pr_entm_id or
	ENTR_BR=@pa_login_pr_entm_id or
	ENTR_SB=@pa_login_pr_entm_id or
	ENTR_DUMMY1=@pa_login_pr_entm_id or
	ENTR_DUMMY3=@pa_login_pr_entm_id or
	ENTR_DUMMY4=@pa_login_pr_entm_id or
	ENTR_DUMMY5=@pa_login_pr_entm_id or
	ENTR_DUMMY6=@pa_login_pr_entm_id or
	ENTR_DUMMY7=@pa_login_pr_entm_id or
	ENTR_DUMMY8=@pa_login_pr_entm_id or
	ENTR_DUMMY9=@pa_login_pr_entm_id or
	ENTR_DUMMY10=@pa_login_pr_entm_id)
...
begin  
  print 'mayur'
if not exists(select cmr_boid from CMR_dtls_mstr where cmr_deleted_ind=0 and cmr_boid=@pa_boid )
begin 
...
begin
	set @pa_out =''
	set @pa_out = 'ERROR: Request Already pending!'
end      
...
select distinct CMR_boid,CMR_reqdt 
from CMR_dtls_mstr ,entity_relationship--CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID) ACCOUNT 		         
where CMR_deleted_ind = 0   
--and CMR_boid = account.dpam_sba_no
and CMR_boid = entr_sba
