-- Object: PROCEDURE citrus_usr.pr_fetch_issued_slips_bak_05102015
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--exec pr_fetch_issued_slips '1201200000009023',''

create procedure [citrus_usr].[pr_fetch_issued_slips_bak_05102015]
...
union
select SLIIM_SLIP_NO_FR ,SLIIM_SLIP_NO_TO , SLIIM_SERIES_TYPE,TRASTM_CD
from slip_issue_mstr_poa ,transaction_sub_type_mstr,dp_acct_mstr,dp_poa_dtls
where dppd_master_id = @pa_acct_no and DPAM_ID=DPPD_DPAM_ID 
and DPAM_DELETED_IND=1 and DPPD_DELETED_IND=1  and sliim_dpam_acct_no=dppd_master_id
and SLIIM_TRATM_ID = TRASTM_ID
order by 1 
