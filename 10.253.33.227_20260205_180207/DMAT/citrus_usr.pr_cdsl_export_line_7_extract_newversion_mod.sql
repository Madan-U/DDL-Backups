-- Object: PROCEDURE citrus_usr.pr_cdsl_export_line_7_extract_newversion_mod
...
--drop function fn_cdsl_export_line_7_extract_newversion_mod
CREATE   proc  [citrus_usr].[pr_cdsl_export_line_7_extract_newversion_mod](@pa_crn_no      VARCHAR(8000)    
...
--  
 ...
 declare @l_table  TABLE (ln_no                   char(2)                    
                        ,crn_no                  numeric                    
                        ,acct_no                 varchar(20)                    
...
then '' else  nmcrcd_reason_cd end
          ...
          ,''--usr_txt2                                 
          ,0000--''--usr_fld3                                 
          ,case when clic_mod_action = 'NOMINEE DEL' then 'D' else 
...
then '' else  nmcrcd_reason_cd end
 ...
 or TypeOfTrans = '2' or TypeOfTrans = '')) THEN 'M' ELSE 'S' END) end
		  ...
		  , nom_relation--isnull(citrus_usr.fn_get_cd_rel( isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRELATN_BO',''),'')),'')
		 -- , isnull(citrus_usr.fn_get_perc(nom_percentage),'')
			,replace(format(nom_percentage ,'000.00'),'.','')
		  ,nom_res_sec_flag--case when  isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'1ST_NRED_SEC_FLG','') ,'')      
		  
		  	 -- ,isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('Country',nom_country ),'') -- cntry code
		  	,(Select top 1 ccm_cd from cdsl_country_mstr where ccm_country_name=nom_country) 
--,isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('State',nom_State),'')  -- state code
			,(Select top 1 csm_state_code_iso from cdsl_state_mstr where csm_state_name=nom_State)
--,isnull(citrus_usr.Fn_Toget_CitySeqno(nom_zip ,nom_city),'' ) -- seqno
...
,case when nom_phone1_ind <> '' THEN  convert(varchar(6),citrus_usr.FN_FORMATSTR(convert(varchar(6),lower(isnull(nom_phone1_ind,'')) ),6,0,'L','0'))  else '000000' END -- pri mob isd                   
 ...
 and    dpam.dpam_sba_no       = crn.acct_no                       
    ...
    and		climmod.clic_mod_deleted_ind = 1              
    and		isnull(climmod.clic_mod_batch_no,0) = 0  
	AND		climmod.clic_mod_action in ('NOMINEE1','NOMINEE DEL')
   and nmcrcd_created_dt BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
