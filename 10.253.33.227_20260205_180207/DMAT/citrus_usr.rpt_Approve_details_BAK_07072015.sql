-- Object: PROCEDURE citrus_usr.rpt_Approve_details_BAK_07072015
...
CREATE  procedure [citrus_usr].[rpt_Approve_details_BAK_07072015]
(@pa_id  varchar(10),
 @pa_type varchar(4),
 @pa_tab varchar(20),
 @pa_from_dt datetime,
...
--convert(varchar(11),DPTDC_LST_UPD_dt,103) else '' end as checker2_dt
					...
					,'' Rejectiondate,'' courier,'' podno,'' dispdate
...
print 'dddddddddddddddddddd1111111111111111111111'
			...
			select	distinct inst_id 
					...
					,QUANTITY
					,[DUAL CHECKER]
					,mkr + ' [ '  + ISNULL(ltrim(rtrim(branch_nm)),'') + ' ]' as mkr
					,convert(varchar(11),mkr_dt ,103)  + ' ' + convert(varchar(8),mkr_dt ,108)   mkr_dt 
					,ORDBY
					,ISIN_NAME
					,ISIN
					...
					--,  branch_nm = fn_find_relations_nm_on_dpamid(DPAM_ID,'BR')
					, branch_nm
					from
					(
...
--convert(varchar(11),dptd.DPTDC_LST_UPD_dt,103) else '' end as checker1_dt
					...
					, '' podno
					, '' dispdate
					,  branch_nm = isnull(citrus_usr.fn_find_relations_nm_on_dpamid(dptd.DPTDC_DPAM_ID,'BR'),'')
					FROM    dptdc_mak dptd  with (nolock) left outer join maker_scancopy  with (nolock) on DPTDC_SLIP_NO = slip_no and deleted_ind =1
...
when isnull(d1.demrm_res_cd_compobj,'') <> '' then convert(varchar(10),d1.demrm_lst_upd_dt,103) 
					...
					, case when (isnull(d.demrm_company_obj,'')<>'' or isnull(d1.demrm_res_cd_compobj,'')<>'') then  case when isnull(disp_cons_no,'') = '' then 'Original DRF yet to receive from RTA' else isnull(DISP_NAME,'') end else '' end courier
					...
					,  branch_nm = isnull(citrus_usr.fn_find_relations_nm_on_dpamid(d1.demrm_DPAM_ID,'BR'),'')
					from demrm_mak d1 with (nolock) left outer join demat_request_mstr d  with (nolock)  on d.demrm_id = d1.demrm_id 	
and d.DEMRM_DPAM_ID = d1.DEMRM_DPAM_ID left outer join dmat_dispatch with (nolock) on  d1.demrm_id = disp_demrm_id and DISP_TO = 'C' and isnull(DISP_TYPE,'') <> 'IR'
					where 
...
, '' RejectionDate
					...
					, '' podno
					, '' dispdate
					,  branch_nm = isnull(citrus_usr.fn_find_relations_nm_on_dpamid(REMRM_DPAM_ID,'BR'),'')
					from remat_request_mstr with (nolock)
					where REMRM_request_dt >= @pa_from_dt and REMRM_request_dt <= @pa_to_dt
					and  Remrm_deleted_ind =1
                    and @pa_trastm_cd like  case when isnull(@pa_trastm_cd,'')= '' then  '%' else 'RMAT' end
...
--						   When dptdc_deleted_ind = 1 then 'U'
--					  else 'D' end as [Status1]
					, case when isnull(dptd1.dptdc_errmsg,'') <> '' then dptd1.dptdc_errmsg else case when citrus_usr.gettranstype(ISNULL(dptd1.dptdc_trans_no,''),ISNULL(convert(varchar,dptd1.dptdc_dpam_id),'0'))<> '' then citrus_usr.gettranstype(ISNULL(dptd1.dptdc_trans_no,''),ISNULL(convert(varchar,dptd1.dptdc_dpam_id),'0'))
when ISNULL(dptd1.dptdc_trans_no,'') = '0' then 'REJECTED FROM CDSL' +'-'  + isnull(citrus_usr.fn_get_errordesc(dptd1.DPTDC_ERRMSG),'')
						when ISNULL(dptd1.dptdc_trans_no,'') <> '' then 'RESPONSE FILE IMPORTED'
...
when isnull(d1.demrm_res_cd_compobj,'') <> '' then convert(varchar(10),d1.demrm_lst_upd_dt,103) 
					...
					,  branch_nm = isnull(citrus_usr.fn_find_relations_nm_on_dpamid(d1.DEMRM_DPAM_ID,'BR'),'')
					from  demrm_mak d1 with (nolock) left outer join demat_request_mstr d  with (nolock) on d.demrm_id = d1.demrm_id 	
and d.DEMRM_DPAM_ID = d1.DEMRM_DPAM_ID left outer join dmat_dispatch with (nolock) on  DISP_TO = 'C' and isnull(DISP_TYPE,'') <> 'IR' 
