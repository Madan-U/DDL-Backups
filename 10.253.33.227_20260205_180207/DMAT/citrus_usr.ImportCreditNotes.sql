-- Object: PROCEDURE citrus_usr.ImportCreditNotes
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--begin tran
--[ImportCreditNotes] 557,	485038,	1,	'feb 07 2013',	'feb 07 2013',	'HO'
--sELECT * FROM TMP_crnt_MSTR
---delete from filetask where status='running'
--rollback
create Proc [citrus_usr].[ImportCreditNotes]
@pa_task_id bigint,
@pa_dpmid int,
@pa_finid int,
...
select top 1 @@voucher_dt = convert(varchar(11),TMPBILL_TODATE,109) from tmp_crnt_mstr 


select top 1 @@filler = TMPBILL_FILLER from TMP_CRNT_MSTR 
...
select @@total_rec_cnt = isnull(count(TMPBILL_SR_NO),0) from tmp_crnt_mstr where (TMPBILL_CREDIT <> 0 or TMPBILL_DEBIT <> 0)
...
IF EXISTS(SELECT tmpbill_dpam_id FROM tmp_crnt_mstr WHERE isnull(tmpbill_dpam_id,0) = 0) 																					
BEGIN
--
...
set @@ssql = 'insert into #tmpVoucher(Vno) select top 1 isnull(LDG_VOUCHER_NO,0) from ledger'+ convert(varchar,@pa_finid) + ' where ldg_dpm_id = ' + convert(varchar,@pa_dpmid) + ' and LDG_VOUCHER_TYPE =''6'' and ldg_account_no = ''' + convert(varchar,@@ref_no) +''' and ldg_deleted_ind = 1'
exec(@@ssql)
...
begin
	...
	set @@ssql          = @@ssql         + ' and    LDG_VOUCHER_TYPE = ''6'''       
	set @@ssql          = @@ssql         + ' and    LDG_VOUCHER_NO   = ' + convert(varchar,@@Vno)      
	exec(@@ssql)
	    

	set @@ssql = 'delete from ledger'+convert(varchar,@pa_finid) +' where  LDG_VOUCHER_TYPE = ''6'' and LDG_VOUCHER_NO   = ' + convert(varchar,@@Vno) + ' and ldg_account_no = ''' + convert(varchar,@@ref_no) + ''' and ldg_dpm_id = ' + convert(varchar,@pa_dpmid)   
	exec(@@ssql)
...
select LDG_ID=' + convert(varchar,isnull(@@lst_curr_id,0)) + ' + 1 ,LDG_DPM_ID=TMPBILL_DPM_ID,LDG_VOUCHER_TYPE=''6'',LDG_BOOK_TYPE_CD=''01'',LDG_VOUCHER_NO=' + convert(varchar,@@Vno) + ',1,LDG_REF_NO=TMPBILL_BILL_NO,''' + @@voucher_dt + ''',TMPBILL_DPAM_ID,LDG_ACCOUNT_TYPE=tmpbill_acct_type,CASE WHEN TMPBILL_CREDIT <> 0 THEN TMPBILL_CREDIT ELSE TMPBILL_DEBIT * -1 END,LDG_NARRATION=case when isnull(TMPBILL_NARR,'''')='''' then ''Recovered from trading A/C towards Curr O/S on ' + @@voucher_dt + ''' else TMPBILL_NARR end,LDG_BANK_ID='''',LDG_ACCOUNT_NO=''' + @@ref_no + ''',LDG_INSTRUMENT_NO='''',LDG_BANK_CL_DATE='''',LDG_COST_CD_ID=0,LDG_BILL_BRKUP_ID=0,LDG_TRANS_TYPE=''AC'',LDG_STATUS=''P'',''' + @pa_loginname + ''',getdate(),''' + @pa_loginname + ''',getdate(),1,0 
from tmp_crnt_mstr 
...
set @@ssql          = @@ssql + ' from tmp_crnt_mstr '
