-- Object: PROCEDURE citrus_usr.pr_rpt_trans_count
...
CREATE procedure [citrus_usr].[pr_rpt_trans_count](
 @pa_excsm_id int
,@pa_tab varchar(10)
  ...
  if @pa_exchange = 'CDSL' 
	  if @pa_type='C' 
          ...
          begin  
				...
				ID_cnt = SUM(ID_cnt),  
				OFM_cnt = SUM(OFM_cnt),  
				DMT_cnt = SUM(DMT_cnt) , DST_cnt = SUM(DST_cnt),  rmt_cnt = sum(rmt_cnt) 	, pldt_cnt = sum(pldt_cnt) , mr_pldt_cnt = sum(mr_pldt_cnt) 	
                FROM 
                ...
                DMT_cnt =0 ,
                DST_cnt =0 ,
                rmt_cnt = 0 , pldt_cnt = 0, mr_pldt_cnt = 0 
				...
				DST_cnt =0 ,
                rmt_cnt = 0 , pldt_cnt = 0 , mr_pldt_cnt = 0 from demat_request_mstr,dp_acct_mstr 
				...
				AND ISNULL(DEMRM_BATCH_NO,'') = '' 
				AND DEMRM_DELETED_IND = 1
				and demrm_typeofsec = 'MUTUAL FUND'
                UNION
				select EP_cnt = 0,  
				NP_cnt = 0,  
				ID_cnt = 0,  
				...
				DMT_cnt = 0, 
				DST_cnt = 0, 
                rmt_cnt = count(Remrm_id), pldt_cnt = 0, mr_pldt_cnt = 0 from remat_request_mstr ,dp_acct_mstr
				...
				mr_pldt_cnt = count(PLDTCM_ID)
				from CDSL_MARGINPLEDGE_DTLS, dp_acct_mstr
				where pldtcm_deleted_ind =1
				and PLDTCM_DPAM_ID = dpam_id
				and PLDTCM_REQUEST_DT between  @pa_request_dt and @pa_request_todt
				and isnull(PLDTCM_BATCH_NO,'') = ''
				...
				ID_cnt = isnull(sum(case when dptdc_internal_trastm = 'ID' then 1 else 0 end),0),  
				OFM_cnt = isnull(sum(case when dptdc_internal_trastm not in('EP','NP','ID') then 1 else 0 end),0),  
                DMT_cnt =0 , DST_cnt =0 , rmt_cnt = 0 , pldt_cnt = 0, mr_pldt_cnt = 0
				from dp_trx_dtls_cdsl with(nolock), dp_acct_mstr with(nolock)  
			...
			--	and  dptdc_request_dt between @pa_request_dt and @pa_request_todt
				...
				ID_cnt = 0,  
				OFM_cnt = 0,  				
				DMT_cnt = count(demrm_id), DST_cnt = 0, rmt_cnt = 0 , pldt_cnt = 0 , mr_pldt_cnt = 0 from demat_request_mstr,dp_acct_mstr 
				...
				ID_cnt = 0,  
				OFM_cnt = 0,  				
				DMT_cnt = 0, DST_cnt = count(demrm_id), rmt_cnt = 0 , pldt_cnt = 0 , mr_pldt_cnt = 0 from demat_request_mstr,dp_acct_mstr 
				WHERE --demrm_request_dt BETWEEN @pa_request_dt AND @pa_request_todt     
convert(varchar(11),demrm_request_dt,103) <=  @pa_request_dt
				and demrm_dpam_id = dpam_id 
				and dpam_dpm_id = case when @dpm_id  ='3' then dpam_dpm_id else @dpm_id  end--@dpm_id
				AND ISNULL(DEMRM_BATCH_NO,'') = '' 
				AND DEMRM_DELETED_IND = 1
				...
				DMT_cnt = 0, 
				DST_cnt = 0, 
                rmt_cnt = 0 , pldt_cnt = count(pldtC_id), mr_pldt_cnt = 0 --nsdl_pledge_dtls--PLDT_REQUEST_DT--PLDT_BATCH_NO
                from cdsl_pledge_dtls 
				WHERE PLDTC_REQUEST_DT between  @pa_request_dt and @pa_request_todt
				AND ISNULL(PLDTC_BATCH_NO,'') = '' 
				AND PLDTC_deleted_ind = 1
    ...
    -- for non broking transactions 
                ...
                do_cnt=sum( do_cnt) ,
                p2p_cnt = SUM(p2p_cnt),
				DMT_cnt = SUM(DMT_cnt) , DST_cnt = SUM(DST_cnt), rmt_cnt = sum(rmt_cnt) , pldt_cnt = sum(pldt_cnt)
				,mr_pldt_cnt = SUM(mr_pldt_cnt)
				FROM 
				(
				...
				DMT_cnt = 0, 
				DST_cnt = 0, 
                rmt_cnt = 0 , pldt_cnt = count(pldt_id),mr_pldt_cnt = 0 --nsdl_pledge_dtls--PLDT_REQUEST_DT--PLDT_BATCH_NO
				...
				WHERE PLDTM_REQUEST_DT between  @pa_request_dt and @pa_request_todt
				AND ISNULL(PLDTM_BATCH_NO,'') = '' 
