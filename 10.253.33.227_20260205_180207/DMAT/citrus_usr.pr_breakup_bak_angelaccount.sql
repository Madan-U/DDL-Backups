-- Object: PROCEDURE citrus_usr.pr_breakup_bak_angelaccount
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

CREATE  procedure [citrus_usr].[pr_breakup_bak_angelaccount](@pa_from_dt datetime,@pa_to_dt datetime,@pa_login_name varchar(100))
as
begin 

TRUNCATE TABLE cdsl_holding_dtls_bak_forreve_angelaccount
insert into cdsl_holding_dtls_bak_forreve_angelaccount  
...
and CDSHM_BEN_ACCT_NO in (select dpam_sba_no from angelaccount)
...
,'TRANSACTION CHARGES','SWACHH BHARAT CESS')
...
FROM cdsl_holding_dtls_bak_forreve_angelaccount with (nolock)
where cdshm_tras_dt between @PA_from_DT and @PA_to_DT
...
UNION ALL
select CDSHM_BEN_ACCT_NO,@l_amount  act_AMT,'Early Payin **' dsc from cdsl_holding_dtls_bak_forreve_angelaccount where cdshm_tratm_cd ='2277'
...
select CDSHM_BEN_ACCT_NO  ,@l_amount  act_AMT ,'OnMkt Settlement **' dsc from cdsl_holding_dtls_bak_forreve_angelaccount
...
and cdshm_isin not in ('INF732E01037')
UNION ALL 
SELECT CDSHM_BEN_ACCT_NO ,@l_amount  act_AMT ,'NSCCL **' dsc FROM cdsl_holding_dtls_bak_forreve_angelaccount 
WHERE CDSHM_TRATM_CD ='2277'
...
FROM cdsl_holding_dtls_bak_forreve_angelaccount WHERE CDSHM_TRATM_CD ='2277'
...
and cdshm_isin not like 'inF%'
UNION ALL 
SELECT CDSHM_BEN_ACCT_NO , '12' act_AMT,'Pledge' dsc FROM cdsl_holding_dtls_bak_forreve_angelaccount WHERE CDSHM_CDAS_SUB_TRAS_TYPE in ('810')
and cdshm_isin not in ('INF732E01037')
UNION ALL
SELECT CDSHM_BEN_ACCT_NO, '12' act_AMT ,'Pledge' dsc FROM cdsl_holding_dtls_bak_forreve_angelaccount WHERE CDSHM_CDAS_SUB_TRAS_TYPE in ('1002')
and cdshm_isin not in ('INF732E01037')
UNION ALL
SELECT CDSHM_BEN_ACCT_NO, '12' act_AMT ,'Pledge' dsc FROM cdsl_holding_dtls_bak_forreve_angelaccount WHERE CDSHM_CDAS_SUB_TRAS_TYPE in ('905')
and cdshm_isin not in ('INF732E01037')
) a 
...
,  isnull(amt,0) amt
,  isnull(amt*case when charge_name  in ('POAFIXED','POA') then 0 else 0.1400  end ,0.0000) service_tax_amt
,  isnull(amt*case when charge_name  in ('POAFIXED','POA') then 0 else 0.0050  end ,0.0000) SWACHH_BHARAT_TAX
...
insert into #final 
select boid , '',cdshm_internal_trastm , 0,0,0,bill_amt 
...
,	'Remat Charges'
,	'Settlement Charges'
) then 'CDSL CHARGE' else brom_desc end brom_desc ,charge_name,amt*-1 amt,service_tax_amt*-1 service_tax_amt, SWACHH_BHARAT_TAX*-1 SWACHH_BHARAT_TAX,dp_charge cdslcharge 
, LEFT(pc1.BOActDt ,2) + '/'+SUBSTRING(pc1.BOActDt ,3,2)+'/'+RIGHT(pc1.BOActDt ,4) activationdt
from #final f , dp_Acct_mstr dpam , dps8_pc1 pc1 where pc1.boid = dpam.dpam_sba_no and dpam.dpam_sba_no = f.dpam_sba_no and amt + service_tax_amt +SWACHH_BHARAT_TAX + dp_charge <> 0
and dpam.DPAM_SBA_NO in (select DPAM_SBA_NO from angelaccount)
