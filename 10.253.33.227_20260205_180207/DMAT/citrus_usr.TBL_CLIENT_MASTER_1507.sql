-- Object: VIEW citrus_usr.TBL_CLIENT_MASTER_1507
...
,convert(varchar(16),POARegNum ) POA_ID  
,poam_name1  POA_NAME  
...
,convert(varchar(50),pc1.EMailId) EMAIL_ADD  
...
,convert(varchar(90),pc6.Addr1  ) NOMI_GUARD_ADD1  
,convert(varchar(85),pc6.Addr2  ) NOMI_GUARD_ADD2  
...
,convert(varchar(10),brom_desc) TEMPLATE_CODE -- pending   
...
,'' CLIENT_CITY_CODE  
,convert(numeric,LEFT(dpam.dpam_sba_no ,2) ) BENEF_ACCNO  
...
,convert(numeric,pc1.AnnIncomeCd  ) INCOME_CODE  
...
,'' DIVIDEND_BRANCH_NO  
,convert(numeric,'0') DIVIDEND_ACC_CURRENCY  
,convert(numeric,'0') DIVIDEND_BANK_AC_TYPE  
,convert(numeric,'0') PURPOSE_ADDITIONAL_NAME  
,pc5.SetupDate  SETUP_DATE  
,pc5.EffFormDate  POA_START_DATE  
,pc5.EffToDate  POA_END_DATE  
,'' POA_ENABLED  
,'' POA_TYPE  
...
,'' FAX_INDEMNITY  
,'' EMAIL_FLAG  
,convert(varchar(12),pc1.PriPhNum  ) FIRST_HOLD_MOBILE  
,case when pc18.BOId  <> '' then 'Y' else 'N' end FIRST_SMS_FLAG  
,'' SMART_REMARKS  
,convert(varchar(25),pc1.PANGIR  ) FIRST_HOLD_PAN  
,POA_VER =(SELECT TOP 1 CASE WHEN DPPD_POA_TYPE='FULL' THEN '2' WHEN DPPD_POA_TYPE='PAYIN ONLY' THEN '1' ELSE '' END  FROM DP_POA_DTLS WHERE DPPD_DPAM_ID=DPAM_ID AND DPPD_DELETED_IND=1)--convert(numeric,'0') POA_VER  
,'' FIRST_HOLD_UID  
,'' SECOND_HOLD_UID  
,'' THIRD_HOLD_UID  
...
left outer join CLIENT_DP_BRKG   with(nolock) on DPAM_ID = clidb_dpam_id and GETDATE() between clidb_eff_from_dt  and  ISNULL(clidb_eff_to_dt ,'dec 31 2100')  
left outer join brokerage_mstr with(nolock) on brom_id = CLIDB_BROM_ID   
left outer join entity_relationship  with(nolock) on DPAM_SBA_NO = entr_sba and GETDATE() between ENTR_FROM_DT and  ISNULL(ENTR_TO_DT ,'dec 31 2100')  
...
left outer join bank_addresses_dtls bankaddr with(nolock) on pc1.DivBankCd = bankaddr.TMPBA_DP_BR  and pc1.DivIFScd = bankaddr.TMPBA_DP_BANK  
...
left outer join dps8_pc6 pc6 with(nolock) on pc1.BOId = pc6.BOId  and pc6.TypeOfTrans in ('','1','2')  
