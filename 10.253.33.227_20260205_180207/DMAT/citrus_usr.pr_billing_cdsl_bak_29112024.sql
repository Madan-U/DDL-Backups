-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_bak_29112024
...
--ROLLBACK
CREATE procedure [citrus_usr].[pr_billing_cdsl_bak_29112024](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
...
and  cham_charge_type in ('F','o') and CHAM_BILL_INTERVAL ='1' AND BROM_ID NOT IN ('83') )
...
delete a from #temp_bill a where 
 charge_name in (select CHAM_SLAB_NAME   from charge_mstr , brokerage_mstr , profile_charges  where brom_id = PROC_PROFILE_ID and PROC_SLAB_NO = CHAM_SLAB_NO 
and  cham_charge_type in ('F','o') and CHAM_BILL_INTERVAL ='1'   AND BROM_ID  IN ('83') )
--and not exists (select 2 from cdsl_holding_dtls where cdshm_dpam_id = dpam_id and CDSHM_TRAS_DT between  @pa_billing_from_dt and @pa_billing_to_dt)
--and not exists (select 2 from AGMUBODPL3.dmat.citrus_usr.client_trade_month,dp_acct_mstr  b where Client_COde = dpam_sba_no and a.dpam_id = b.dpam_id and isnull(last_trade ,'') between  @pa_billing_from_dt and @pa_billing_to_dt )
and not exists (select 2 from client_trade_month,dp_acct_mstr  b where Client_COde = dpam_sba_no and a.dpam_id = b.dpam_id and isnull(last_trade ,'') between  @pa_billing_from_dt and @pa_billing_to_dt )
--any type of transaction ?
...
CHARGE_NAME in (select CHAM_SLAB_NAME   from charge_mstr , brokerage_mstr , profile_charges  where brom_id = PROC_PROFILE_ID and PROC_SLAB_NO = CHAM_SLAB_NO 
and  cham_charge_type in ('F') and CHAM_BILL_INTERVAL ='1'   AND BROM_ID  IN ('83') )
