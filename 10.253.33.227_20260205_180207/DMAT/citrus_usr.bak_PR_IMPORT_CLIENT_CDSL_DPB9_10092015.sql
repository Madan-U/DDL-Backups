-- Object: PROCEDURE citrus_usr.bak_PR_IMPORT_CLIENT_CDSL_DPB9_10092015
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------


CREATE PROCEDURE  [citrus_usr].[bak_PR_IMPORT_CLIENT_CDSL_DPB9_10092015]
			(
			 @pa_exch          VARCHAR(20)    
...
UNION
SELECT DISTINCT ADDR1 , ADDR2 , ADDR3 ,CITY ,STATE , COUNTRY ,PINCODE ,'N' EXISTSFLAG
FROM DPB9_PC6 WHERE ISNULL(ADDR1,'') <>''
UNION
SELECT DISTINCT ADDR1 , ADDR2 , ADDR3 ,CITY ,STATE , COUNTRY ,PINCODE ,'N' EXISTSFLAG
...
--					   SELECT @PA_NOM_DTLS = ISNULL(NAME,'')+'|*~|'+ISNULL(MIDDLENAME,'')+'|*~|'+ISNULL(SEARCHNAME,'')+'|*~|'+ISNULL(LTRIM(RTRIM(FTHNAME)),'')+'|*~|'+CITRUS_USR.FNGETDATE(DATEOFBIRTH)+'|*~|'+ISNULL(LTRIM(RTRIM(PANGIR)),'')+'|*~|'+'|*~|*|~*'  FROM   DPB9_PC6 WHERE BOID = @C_BEN_ACCT_NO  
					   SELECT @PA_NOM_DTLS = ISNULL(NAME,'')+'|*~|'+ISNULL(MIDDLENAME,'')+'|*~|'+ISNULL(SEARCHNAME,'')+'|*~|'+ISNULL(LTRIM(RTRIM(FTHNAME)),'')+'|*~|'+CITRUS_USR.FNGETDATE(DATEOFBIRTH)+'|*~|'+ISNULL(LTRIM(RTRIM(PANGIR)),'')+'|*~|'+'|*~|'+convert(varchar,@l_nrn_no)+'|*~|*|~*'  FROM   DPB9_PC6 WHERE BOID = @C_BEN_ACCT_NO  
