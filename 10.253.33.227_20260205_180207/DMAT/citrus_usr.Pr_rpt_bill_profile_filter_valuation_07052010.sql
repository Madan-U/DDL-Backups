-- Object: PROCEDURE citrus_usr.Pr_rpt_bill_profile_filter_valuation_07052010
...
Pr_rpt_bill_profile_filter 	'CDSL',3,'jan 01 2010','jan 01 2010','N','N','','','','N','Y','',1,'HO|*~|',''	

Pr_rpt_bill_profile_filter 	'NSDL',4,'Sep 11 2008','Sep 11 2008 23:59:59','N','N','10000037','','','N','Y','',1,'HO|*~|',''

*/
create Proc [citrus_usr].[Pr_rpt_bill_profile_filter_valuation_07052010]              
@pa_dptype varchar(4),                      
@pa_excsmid int,                      
...
prev_bill_bal numeric(18,2)
   ...
   begin 
	   ...
	   begin     
			  ...
			  begin   
				 ...
				 ,account_group_mapping g with(nolock)              
				 ,#ACLIST account 
                 --LEFT OUTER JOIN CLOSING_PRICE_MSTR_NSDL ON DPDHM_ISIN = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_NSDL WHERE CLOPM_ISIN_CD = DPDHM_ISIN and CLOPM_DT <= @pa_prevfromdate and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc)                        
				 WHERE           
 ...
 begin  
  ...
  group by clic_dpam_id,dpam_sba_no,dpam_sba_name,clic_charge_name                     
   ...
   --, case when order_by = '3' then valuation else 0 end 
