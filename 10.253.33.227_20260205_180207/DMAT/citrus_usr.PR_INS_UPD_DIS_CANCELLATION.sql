-- Object: PROCEDURE citrus_usr.PR_INS_UPD_DIS_CANCELLATION
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--@PA_EXCSM_ID + '|' + 'INS'  +'|' +  @PA_TRXINIT_FLG + '|'+ @PA_CANCEL_DT + '|'+@PA_BOOK_NO+'|'+@PA_REMARKS+'|'+@PA_LOGIN_NAME+'|'+@PA_TRXTYPE+'|'+@PA_SERIES_TYPE+'|'+@PA_USES_ID


CREATE PROCEDURE [citrus_usr].[PR_INS_UPD_DIS_CANCELLATION] 
(
	@PA_EXCSM_ID NUMERIC,
...
print'1'
		SELECT  USES_ID,
				...
				USES_DPAM_ACCT_NO
		FROM USED_SLIP_BLOCK, SLIP_ISSUE_MSTR, SLIP_BOOK_MSTR ,
		...
		AND USES_TRANTM_ID = SLIBM_TRATM_ID
		AND slibm_book_name = CONVERT(NUMERIC,USED_BOOK_NAME)
		...
		AND USES_DPAM_ACCT_NO LIKE CASE WHEN ISNULL(@PA_ACCOUNT_ID,'') = '' THEN '%' ELSE @PA_ACCOUNT_ID END
...
print'2'
		...
		AND USES_TRANTM_ID =SLIBM_TRATM_ID
		AND USES_TRANTM_ID = SLIBM_TRATM_ID
		AND slibm_book_name = CONVERT(VARCHAR,USED_BOOK_NAME)
		AND USES_DPM_ID = SLIBM_DPM_ID
		AND USES_SERIES_TYPE = SLIBM_SERIES_TYPE
		AND SLIIM_SERIES_TYPE = USES_SERIES_TYPE
		AND TRASTM_ID = USES_TRANTM_ID
	...
	BEGIN
		SELECT isnull(slibm_book_name,'0') as sliim_book_name, SLIIM_SERIES_TYPE,SLIIM_DPAM_ACCT_NO
		FROM SLIP_ISSUE_MSTR left outer join  SLIP_BOOK_MSTR on  SLIIM_TRATM_ID=@PA_TRXTYPE 
		AND SLIIM_SLIP_NO_FR = SLIBM_FROM_NO
		AND SLIIM_SLIP_NO_TO = SLIBM_TO_NO
		AND SLIIM_SERIES_TYPE = SLIBM_SERIES_TYPE
		AND sliim_dpm_id = SLIBM_DPM_ID
		...
		and SLIIM_SERIES_TYPE LIKE CASE WHEN @PA_SERIES_TYPE = '' THEN '%' ELSE @PA_SERIES_TYPE END
		--AND SLIIM_DPAM_ACCT_NO = @PA_ACCOUNT_ID
		AND SLIIM_DPAM_ACCT_NO LIKE CASE WHEN @PA_ACCOUNT_ID = '' THEN '%' ELSE @PA_ACCOUNT_ID END
		AND SLIIM_DELETED_IND = 1
	END

	IF @PA_ACTION = 'SELECTSLIP'
	BEGIN
		SELECT SLIIM_SLIP_NO_FR,SLIIM_SLIP_NO_TO ,SLIIM_SERIES_TYPE,SLIIM_DPAM_ACCT_NO
		FROM SLIP_ISSUE_MSTR ,SLIP_BOOK_MSTR
		where slibm_book_name = CONVERT(NUMERIC,@PA_BOOK_NO)
		AND SLIIM_SLIP_NO_FR = SLIBM_FROM_NO
		AND SLIIM_SLIP_NO_TO = SLIBM_TO_NO
		AND SLIIM_SERIES_TYPE = SLIBM_SERIES_TYPE
		AND sliim_dpm_id = SLIBM_DPM_ID
	...
	BEGIN
		...
		exec pr_get_range_for_cancel @PA_SERIES_TYPE , @PA_FROM_SLIP,@PA_TO_SLIP,@PA_ACCOUNT_ID,'N' ,''
		
	END
	IF @PA_ACTION = 'SAVEAUTO'
	BEGIN
		--IF EXISTS (SELECT 1 FROM USED_SLIP WHERE USES_SLIP_NO>=CONVERT(NUMERIC,@PA_FROM_SLIP) AND USES_SLIP_NO<=CONVERT(NUMERIC,@PA_TO_SLIP)
		--AND USES_TRANTM_ID=@PA_TRXTYPE AND USES_SERIES_TYPE=@PA_SERIES_TYPE AND USES_DELETED_IND =1)
		--BEGIN
		...
		--SELECT 'Y' -- allowed
		--END
		declare @l_string varchar(max)
		set @l_string = ''
		set @l_string  = 		convert(varchar,@PA_EXCSM_ID ) + '|'  
		+ @PA_TRXINIT_FLG + '|'
		+ @PA_CANCEL_REASON + '|'
		+ convert(varchar(11),@PA_CANCEL_DT ,103)+ '|'
		+ @PA_BOOK_NO+'|'
		+ @PA_REMARKS+'|'
		+ @PA_LOGIN_NAME+'|'
		+ @PA_TRXTYPE+'|'
		+ @PA_SERIES_TYPE+'|'
		+ convert(varchar,@PA_USES_ID)+'|'
		+ @PA_ACCOUNT_ID+'|'

		exec pr_get_range_for_cancel @PA_SERIES_TYPE , @PA_FROM_SLIP,@PA_TO_SLIP,@PA_ACCOUNT_ID,'Y' ,@l_string
