-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_bak05062023
...
create PROC [citrus_usr].[Pr_Rpt_Holding_bak05062023]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
insert into Holding_report_log
select GETDATE(),@PA_DPTYPE , @PA_EXCSMID ,@PA_ASONDATE ,@PA_FORDATE ,@PA_FROMACCID ,@PA_TOACCID ,@PA_ISINCD ,@PA_WITHVALUE , --Y/N                    
@PA_LOGIN_PR_ENTM_ID,@PA_LOGIN_ENTM_CD_CHAIN  ,@PA_SETTM_TYPE ,@PA_SETTM_NO_FR ,@PA_SETTM_NO_TO ,@PA_OUTPUT
...
-- added relation ship join to get del 1 data like holding view on Aug 27 2015 by latesh p w as request by yogesh g
	if @l_bbo_code = '' 
	begin

	insert into  #tmphldg
	select distinct a.* from holdingallforview a, DP_aCCT_MSTR,entity_relationship WHERE DPAM_ID = DPHMCD_DPAM_ID
	AND DPAM_SBA_NO BETWEEN @PA_FROMACCID AND  @PA_TOACCID
	and ISNUMERIC(DPAM_SBA_NO )= 1 and ENTR_DELETED_IND=1 and getdate() between isnull(ENTR_FROM_DT,'jan 01 1900') and isnull(ENTR_TO_DT,'jan 01 2900') 
	and DPAM_SBA_NO=ENTR_SBA and ENTR_CRN_NO=DPAM_CRN_NO
	...
	insert into  #tmphldg
	select distinct a.* from holdingallforview a, DP_aCCT_MSTR ,#bbocode,entity_relationship WHERE DPAM_ID = DPHMCD_DPAM_ID and dpam_sba_no = clientid
	AND DPAM_SBA_NO BETWEEN  @PA_FROMACCID  AND @PA_TOACCID
and ISNUMERIC(DPAM_SBA_NO )= 1 and ENTR_DELETED_IND=1 and getdate() between isnull(ENTR_FROM_DT,'jan 01 1900') and isnull(ENTR_TO_DT,'jan 01 2900') 
and DPAM_SBA_NO=ENTR_SBA and ENTR_CRN_NO=DPAM_CRN_NO
-- added relation ship join to get del 1 data like holding view on Aug 27 2015 by latesh p w as request by yogesh g
...
SELECT TOP 1 @PA_FORDATE = convert(varchar(11),dphmcd_holding_dt,109) from  holdingallforview with (nolock)
			 ...
			 SELECT 
						...
						case when CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY))<>'0' then 
						case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY))+CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY))  
						else  CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY))+CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY))   end
						else case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' 
						then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) 
						else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end end,
						
						CONVERT(NUMERIC(18,3),sum(DPHMCD_ELIMINATION_QTY)),
						CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)) DPHMCD_EARMARK_QTY,
...
begin
			 ...
			 SELECT 
						...
						CONVERT(NUMERIC(18,3),DPHMCD_EARMARK_QTY) DPHMCD_EARMARK_QTY,
...
print 'qqqq'
 ...
 SELECT distinct DPAM_SBA_NAME,
 DPAM_SBA_NO + '  (' + tradingid + ')' DPAM_SBA_NO ,
 DPHMCD_ISIN,
 replace(ISIN_NAME,'*','') ISIN_NAME
 ,CONVERT(NUMERIC(18,3),sum(DPHMCD_CURR_QTY)) DPHMCD_CURR_QTY,
  VALUATION=isnull((CONVERT(NUMERIC(18,3),sum(DPHMCD_FREE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_FREEZE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_PLEDGE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_DEMAT_PND_VER_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_REMAT_PND_CONF_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_AVAIL_LEND_QTY)) )*ISNULL(convert(numeric(18,3),CLOPM_CDSL_RT),0),0) ---CONVERT(NUMERIC(18,2),sum(DPHMCD_CURR_QTY)*ISNULL(CLOPM_CDSL_RT,0)),
...
--,case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end DPHMCD_LOCKIN_QTY
...
,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)),0) DPHMCD_EARMARK_QTY --DPHMCD_EARMARK_QTY11
...
,DPAM_ID, HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109)
,CMP=ISNULL(convert(numeric(18,3),CLOPM_CDSL_RT),0.000)  
,CLOPM_DT ratedate
,tradingid
,'0' DPHMCD_MARGIN_PLEDGE
...
begin
SELECT distinct DPAM_SBA_NAME,DPAM_SBA_NO + '  (' + tradingid + ')' DPAM_SBA_NO,DPHMCD_ISIN,replace(ISIN_NAME,'*','') ISIN_NAME,CONVERT(NUMERIC(18,3),sum(DPHMCD_CURR_QTY)),
 ...
 VALUATION=isnull((CONVERT(NUMERIC(18,3),sum(DPHMCD_FREE_QTY)))*ISNULL(convert(numeric(18,3),CLOPM_CDSL_RT),0),0) ---CONVERT(NUMERIC(18,2),sum(DPHMCD_CURR_QTY)*ISNULL(CLOPM_CDSL_RT,0)),
...
--,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)),0)      
,case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end
		 ,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_ELIMINATION_QTY)),0)
		 ,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)),0) DPHMCD_EARMARK_QTY --DPHMCD_EARMARK_QTY22
		 ,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_AVAIL_LEND_QTY)),0),isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_LEND_QTY)),0),isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_BORROW_QTY)),0),DPAM_ID, HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109),CMP=ISNULL(convert(numeric(18,3),CLOPM_CDSL_RT),0.000)  
		 ,CLOPM_DT ratedate
  ,tradingid
...
--VALUATION=CONVERT(NUMERIC(18,2),DPHMCD_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0)), 
VALUATION=isnull((CONVERT(NUMERIC(18,3),sum(DPHMCD_FREE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_FREEZE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_PLEDGE_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_DEMAT_PND_VER_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_REMAT_PND_CONF_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) )*ISNULL(convert(numeric(18,3),CLOPM_CDSL_RT),0),0)
,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_FREE_QTY)),0),isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_FREEZE_QTY)),0)      
,

case when isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)),0)<>0 
then isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_PLEDGE_QTY)),0)-isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)),0) 
else isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_PLEDGE_QTY)),0) end
...
,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)),0)
--,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)),0)     
,case when (CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' and isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)),0)=0)  then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else
case when isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)),0)<>0 then isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)),0)+isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)),0) else  isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)),0) end    
end
lockinqty

,case when CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) <> '0' then CONVERT(NUMERIC(18,3),sum(DPHMCD_SAFE_KEEPING_QTY)) + CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) else CONVERT(NUMERIC(18,3),sum(DPHMCD_LOCKIN_QTY)) end
		 ,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_ELIMINATION_QTY)),0)
		 ,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_EARMARK_QTY)),0) DPHMCD_EARMARK_QTY --DPHMCD_EARMARK_QTY33
		 ,isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_AVAIL_LEND_QTY)),0),isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_LEND_QTY)),0),isnull(CONVERT(NUMERIC(18,3),sum(DPHMCD_BORROW_QTY)),0),DPAM_ID, HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109),
		 		 CMP=ISNULL(convert(numeric(18,3),CLOPM_CDSL_RT),0.000)    
		 ,CLOPM_DT ratedate
