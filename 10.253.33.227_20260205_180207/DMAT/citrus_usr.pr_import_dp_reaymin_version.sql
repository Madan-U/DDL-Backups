-- Object: PROCEDURE citrus_usr.pr_import_dp_reaymin_version
-- Server: 10.253.33.227 | DB: DMAT
--------------------------------------------------

--select * from dp_acct_mstr order by 1 desc
--select top 5 * from dp_Acct_mstr where dpam_batch_no = '112022'
--update dp_Acct_mstr set dpam_batch_no = null where dpam_batch_no = '112022'
--select * from batchno_nsdl_mstr where batchn_no = '112022'
--delete from batchno_nsdl_mstr where batchn_no = '112022'
...
--begin tran 
--rollback 
--UPDATE DP_ACCT_MSTR SET DPAM_BATCH_NO = NULL WHERE DPAM_ID = 120222
--[pr_import_dp_reaymin_version] '','CDSL','14/07/2008','04/09/2008','N','12121212',3,'HO',''   
--select * from dp_acct_mstr order by 1 desc  
--CDSL	01/05/2008	02/05/2008	N	45	3	HO	
CREATE PROCEDURE [citrus_usr].[pr_import_dp_reaymin_version]( @pa_crn_no      VARCHAR(8000)                  
                            , @pa_exch        VARCHAR(10)                  
    ...
    --                  
      ...
      --      
        ...
        SELECT 0                  
             ...
             , case when convert(char(1),ISNULL(citrus_usr.fn_ucc_accp(dpam.dpam_id,'ADD_PREF_FLG',''),'')) = '1' then 'Y' else 'N' end    --convert(char(1),ISNULL(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'',''),''))                                                  --address preference flag                   
             ...
             ,  CASE WHEN citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'FH_ADR1'),''),1) <> '' THEN 'Y'  
                     ELSE 'N' END  
             ...
             , case when isnull(convert(vARCHAR(30),BANM_MICR),'') = '0' then convert(char(9),'') else convert(char(9),isnull(convert(vARCHAR(30),BANM_MICR),'')) end
             ...
             , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'') <> '' THEN convert(char(50),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'')) ELSE convert(char(50),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'EMAIL1'),''))  end                                                --email id for first holder            
             , convert(char(1),ISNULL(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'MAPIN_ID',''),''))                                  --map in id of first holder   --9                  
             , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'') <> '' then convert(char(11),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'')) ELSE convert(char(11),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'MOBILE1'),''))            end                                       --mobile no of first holder  --12                  
             ...
             , convert(char(50),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'SH_EMAIL1'),''))                                              --email id for second holder                  
             ...
             , convert(char(50),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'TH_EMAIL1'),''))                                            --email id for third holder                  
			 ...
			 , CASE when dphd_gau_fname <> '' then   convert(datetime, clim_dob,103) END            
             , CASE WHEN (dphd_nom_fname <> '' AND DATEDIFF(YEAR, CONVERT(DATETIME,DPHD_NOM_DOB,103), CONVERT(DATETIME,GETDATE(),103)) < 18 ) THEN CONVERT(DATETIME,dphd_nom_dob) end  
...
--vishal  
       ...
       INSERT INTO @l_dp_pri_dtls(ln_number                  
        ...
        SELECT 0                  
             ...
             , case when isnull(convert(vARCHAR(30),BANM_MICR),'') = '0' then convert(char(9),'') else convert(char(9),isnull(convert(vARCHAR(30),BANM_MICR),'')) end
             , CASE WHEN convert(char(20),cliba_ac_type) IN( 'SAVINGS', 'SAVING')  THEN '10' WHEN convert(char(20),cliba_ac_type) IN('CURRENT') THEN  '11' WHEN convert(char(20),cliba_ac_type) IN( 'OTHERS') THEN  '13' END   --'11'                                                                                          --bank acct type                  
             , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'') <> '' THEN convert(char(50),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'')) ELSE convert(char(50),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'EMAIL1'),''))  end                                                --email id for first holder                                                           --email id for first holder            
             , convert(char(1),ISNULL(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'MAPIN_ID',''),''))                                  --map in id of first holder   --9                  
             , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'') <> '' then convert(char(11),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'')) ELSE convert(char(11),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'MOBILE1'),''))            end                                       --mobile no of first holder  --12                  
													             , CASE WHEN ((convert(char(11),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'MOBILE1'),'')) <> '' OR isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'') <> '' ) AND  convert(char(1),ISNULL(citrus_usr.fn_ACCT_entp(DPAM_ID,'SMS_FLAG'),''))= 1 )   THEN 'Y' 
               WHEN ((convert(char(11),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'MOBILE1'),'')) <> '' OR isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'') <> '' ) AND  convert(char(1),ISNULL(citrus_usr.fn_ACCT_entp(DPAM_ID,'SMS_FLAG'),''))= '')  THEN 'N' ELSE ' ' END 
...
,send_ref_no1            
 ...
 ,cust_fax                char(100)                   
      ...
      BEGIN                  
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
                                ...
                                , case when dpam_clicm_cd in ('21' , '24' , '27') then CITRUS_USR.inside_trim(DPAM_SBA_NAME,'1') 
                                  else dpam_sba_name  end
                                , case when dpam_clicm_cd in ('21' , '24' , '27') then CITRUS_USR.inside_trim(DPAM_SBA_NAME,'2')
                                  else '' end
                                , case when dpam_clicm_cd in ('21' , '24' , '27') then CITRUS_USR.inside_trim(DPAM_SBA_NAME,'3')
                                  else '.' end
                               ...
                               , case when citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(DPAM.DPAM_ID,'AC_COR_ADR1'),''),1) <> '' 
                                ...
                                , ''  
                                , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'') <> '' THEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'') ELSE isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'EMAIL1'),'') END --email                  
                                , ''                  
                                , ''                  
                                , '0000'                  
                                , ''                  
               ...
               insert into @line5( ln_no                   
                                  ...
                                  SELECT distinct '05'                  
                                      ...
                                      , case when dpam_clicm_cd in ('21' , '24' , '27') then  left(isnull(citrus_usr.fn_ucc_entp(clim_crn_no,'STAFF',''),''),1) else '' end              
                                       ...
                                       , case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'SMART_CARD_REQ',''),'') <> '' then 'Y' else 'N' end                           
                                       ...
                                       , '000000'--cc_id                  
                                       , isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'CMID_NO',''),'')                   
                                       , isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'STOCK_EXCH',''),'00')              
  ...
  AND    excsm_exch_cd           = @pa_exch  
               ...
               ,case when isnull(citrus_usr.fn_dp_import_conc(dpam.dpam_id,'POA_OFF_PH1'),'') <> '' then  'O' ELSE ''   
                     end                
               ,case when isnull(citrus_usr.fn_dp_import_conc(dpam.dpam_id,'POA_OFF_PH1'),'') <> '' then  isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'POA_OFF_PH1'),'')    
                     end                                   
               ,''--cust_ph2_ind                              
               ,''  
               ,''  
  ...
  ,Purpose_code                             
      ...
      from   @line2 l2  
             ...
             left outer join   
			 @line6 l6       on                  l5.acct_no = l6.acct_no 
			 left outer join   
			 @line7 l7       on                  l2.acct_no = l7.acct_no   
			 left outer join 
             @line3 l3       on                  l2.acct_no = l3.acct_no                           
             left outer join   
             @line4 l4       on                  l3.acct_no = l4.acct_no                           
      ...
      order by l2.acct_no ,l7.purpose_code   desc  
