-- Object: PROCEDURE dbo.IndexOptimize_Modified_Gather_Data
...
CREATE   PROC [dbo].[IndexOptimize_Modified_Gather_Data] @Databases nvarchar(max) = NULL, 
...
SELECT '@tbl_Databases__Hadr__' as RunningQuery, [@_isFreshStart] = @_isFreshStart, STUFF((SELECT ', ' + DBName as [text()] FROM @tbl_Databases FOR XML PATH('')),1,1,'') AS Dbs FROM @tbl_Databases WHERE HaDrEnabled = 1 

UNION ALL 

SELECT '@tbl_Databases__NonHadr__' as RunningQuery, [@_isFreshStart] = @_isFreshStart, STUFF((SELECT ', ' + DBName as [text()] FROM @tbl_Databases FOR XML PATH('')),1,1,'') AS Dbs FROM @tbl_Databases WHERE HaDrEnabled = 0; 
