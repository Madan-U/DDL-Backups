-- Object: PROCEDURE citrus_usr.FN_STRING_FOR_PRINT1231212121
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--exec [FN_STRING_FOR_PRINT1231212121] 'Any discrepancy in the statement should be brought to the notice of ''''BASIS POINT SECURITIES PRIVATE LIMITED'''' within one month from the date of the statement.<br>',158
...
--select citrus_usr.pr_pick_upto('Any discrepancy in the statement should be brough to the notice of ''BASIS POINT SECURITIES PRIVATE LIMITED'' within one month from the date<br>',158)
CREATE procedure [citrus_usr].[FN_STRING_FOR_PRINT1231212121](@PA_STRING VARCHAR(8000),@PA_LEN NUMERIC)
AS
BEGIN
...
if @PA_LEN = 158
BEGIN
PRINT citrus_usr.ufn_countstring(replace(left(@pa_string,@PA_LEN),' ','|'),'|')
PRINT citrus_usr.pr_pick_upto(left(@pa_string,158),2)
