-- Object: PROCEDURE citrus_usr.rpt_Approve_details
...
CREATE  procedure [citrus_usr].[rpt_Approve_details]
(@pa_id  varchar(10),
 @pa_type varchar(4),
 @pa_tab varchar(20),
 @pa_from_dt datetime,
...
-- otp status update
SELECT MAX(CDSHM_CDAS_SUB_TRAS_TYPE) SUBTYPECDAS,DPTDC_SLIP_NO , DPTDC_ISIN 
INTO #TMPOTP
 FROM DP_TRX_DTLS_CDSL with(nolock),CDSL_HOLDING_DTLS with(nolock) WHERE 
...
AND CDSHM_TRAS_DT>='Apr 01 2024'
AND DPTDC_INTERNAL_TRASTM IN ('BOBO','ID')
GROUP BY DPTDC_SLIP_NO, DPTDC_ISIN 
-- otp status update
...
when dptd.dptdc_reason_cd= '35'  and dptd.dptdc_Request_dt <= 'Dec 28 2025' then 'Transfer from Nominee/Surviving holder to the beneficiary' 
when dptd.dptdc_reason_cd= '35'  and dptd.dptdc_Request_dt > 'Dec 28 2025' then 'Transmission to Legal Heirs (TLH)' 
when dptd.dptdc_reason_cd= '36' then 'Transfer of securities from/to account of PMS provider to/from the account of its various clients'
when dptd.dptdc_reason_cd= '37' then 'Margin to Custodian' 
when dptd.dptdc_reason_cd= '38' then 'Margin returned by Custodian' 
when dptd.dptdc_reason_cd= '39' then 'Change of nominee shareholder appointed under section 187(1) of Companies Act, 2013'
when dptd.dptdc_reason_cd= '40' then 'Off-Market Swap' 
when dptd.dptdc_reason_cd= '41' then 'Delisting - Exit offer' 
when dptd.dptdc_reason_cd= '42' then 'Deposit of securities with Escrow Agent and its return'
when dptd.dptdc_reason_cd= '44' then 'Transfer From/To for Tri-party Repo collateral'
when dptd.dptdc_reason_cd= '43' then 'Transfer of constitute shares for creation of ETF units' 

when dptd.dptdC_trastm_cd = 'TNT' and  dptd.dptdc_reason_cd= '1' then 'Reverting to Maiden Name'
when dptd.dptdC_trastm_cd = 'TNT' and  dptd.dptdc_reason_cd= '2' then 'Change in BO name of Official Gadget'
when dptd.dptdC_trastm_cd = 'TNT' and  dptd.dptdc_reason_cd= '3' then 'Change in BO name upon marriage'
when dptd.dptdC_trastm_cd = 'TNT' and  dptd.dptdc_reason_cd= '4' then 'Shifting Account from one account to Other'
when dptd.dptdC_trastm_cd = 'TNT' and  dptd.dptdc_reason_cd= '5' then 'Death of Single Holder Account'
when dptd.dptdC_trastm_cd = 'TNT' and  dptd.dptdc_reason_cd= '6' then 'Death of One or More Holder in Joint account'
when dptd.dptdC_trastm_cd = 'TNT' and  dptd.dptdc_reason_cd= '7' then 'Shifting of Account within DP'
...
,case when dptd.dptdc_mid_chk <> '' then dptd.dptdc_mid_chk else isnull(dptd.DPTDC_LST_UPD_BY,'') end as checker1
					, case when dptd.dptdc_deleted_ind in (0,1) and isnull(dptd.dptdc_mid_chk,'') <> '' then CASE WHEN isnull(convert(varchar(11),dptd.DPTDC_LST_UPD_dt,109),'') ='jan 01 1900' THEN '' ELSE dptd.DPTDC_LST_UPD_dt END 
...
--convert(varchar(11),dptd.DPTDC_LST_UPD_dt,103) else '' end as checker1_dt
					...
					, case when dptd.dptdc_deleted_ind = 1 and isnull(dptd.dptdc_mid_chk,'') <> '' then CASE WHEN isnull(convert(varchar(11),dptd1.DPTDC_LST_UPD_dt,106),'') ='jan 01 1900' THEN '' ELSE dptd1.DPTDC_LST_UPD_dt END else '' end checker2_dt										
...
--					  else 'D' end as [Status1]
-- otp status update
,CASE WHEN EXISTS (SELECT 1 FROM #TMPOTP  OTP1 WHERE OTP1.DPTDC_SLIP_NO=dptd1.DPTDC_SLIP_NO aND OTP1.DPTDC_ISIN = dptd1.DPTDC_ISIN  AND SUBTYPECDAS IN ('543','330')) 
...
WHEN EXISTS (SELECT 1 FROM #TMPOTP  OTP2 WHERE OTP2.DPTDC_SLIP_NO=dptd1.DPTDC_SLIP_NO AND OTP2.DPTDC_ISIN = dptd1.DPTDC_ISIN  AND SUBTYPECDAS='332') THEN 'PENDING OTP AUTHENTICATION TRANSACTION CANCELLED IN EOD'
when exists (select 1 from #TMPOTP  OTP4 where otp4.DPTDC_SLIP_NO=dptd1.DPTDC_SLIP_NO aND OTP4.DPTDC_ISIN = dptd1.DPTDC_ISIN  and SUBTYPECDAS='545') then 'NON AUTHENTICATED OTP TRANSACTION CANCELLED'
--		
-- otp status update			, 
--case 
...
when ISNULL(dptd1.dptdc_trans_no,'') = '0' then 'REJECTED FROM CDSL' +'-'  + isnull(citrus_usr.fn_get_errordesc(dptd1.DPTDC_ERRMSG),'')
						when (ISNULL(DPTD1.DPTDC_TRANS_NO,'') <> '' 
						-- otp status update
						AND NOT EXISTS (SELECT 1 FROM #TMPOTP  OTP3 WHERE OTP3.DPTDC_SLIP_NO=dptd1.DPTDC_SLIP_NO AND SUBTYPECDAS IN ('543','330','332','545'))
						-- otp status update
) then 'RESPONSE FILE IMPORTED'
					...
					--, case when dptd.dptdc_deleted_ind = 0 then isnull(dptd.DPTDC_LST_UPD_BY,'') end as checker1
--					, case when dptd.dptdc_mid_chk <> '' then dptd.dptdc_mid_chk else isnull(dptd.DPTDC_LST_UPD_BY,'') end as checker1
--					, case when dptd.dptdc_deleted_ind in (0,1) and isnull(dptd.dptdc_mid_chk,'') <> '' then CASE WHEN isnull(convert(varchar(11),dptd.DPTDC_LST_UPD_dt,106),'') ='jan 01 1900' THEN '' ELSE dptd.DPTDC_LST_UPD_dt END 
--						   when dptd.dptdc_deleted_ind in (0,1) and isnull(dptd.dptdc_mid_chk,'') = '' then CASE WHEN isnull(convert(varchar(11),dptd.DPTDC_LST_UPD_dt,106),'') ='jan 01 1900' THEN '' ELSE dptd.DPTDC_LST_UPD_dt END else '' end checker1_dt
----convert(varchar(11),dptd.DPTDC_LST_UPD_dt,103) else '' end as checker1_dt
--					, case when dptd.dptdc_deleted_ind = 1 and isnull(dptd.dptdc_mid_chk,'') <> '' then isnull(dptd.DPTDC_LST_UPD_BY,'') else '' end as checker2
--					, case when dptd.dptdc_deleted_ind = 1 and isnull(dptd.dptdc_mid_chk,'') <> '' then CASE WHEN isnull(convert(varchar(11),dptd1.DPTDC_LST_UPD_dt,106),'') ='jan 01 1900' THEN '' ELSE dptd1.DPTDC_LST_UPD_dt END else '' end checker2_dt										
...
--(select 1 from #TMPOTP  OTP3 where otp3.DPTDC_SLIP_NO=dptd1.DPTDC_SLIP_NO and SUBTYPECDAS IN ('543','330','332')) ex,

-- otp status update
case 
when exists (select 1 from #TMPOTP  OTP1 where otp1.DPTDC_SLIP_NO=dptd1.DPTDC_SLIP_NO  AND otp1.DPTDC_ISIN =dptd1.DPTDC_ISIN  and SUBTYPECDAS IN ('543','330')) then 'REJECTED OTP AUTHENTICATED TRANSACTION'
 WHEN EXISTS (SELECT 1 FROM #TMPOTP  OTP4 WHERE OTP4.DPTDC_SLIP_NO=DPTD1.DPTDC_SLIP_NO  AND otp4.DPTDC_ISIN =dptd1.DPTDC_ISIN  AND SUBTYPECDAS='545') THEN 'NON AUTHENTICATED OTP TRANSACTION CANCELLED'
...
when ISNULL(dptd1.dptdc_trans_no,'') = '0' then 'REJECTED FROM CDSL' + ' - '  + upper(isnull(citrus_usr.fn_get_errordesc(dptd1.DPTDC_ERRMSG),''))
						...
						-- otp status update
						 and 
						not exists (select 1 from #TMPOTP  OTP3 where otp3.DPTDC_SLIP_NO=dptd1.DPTDC_SLIP_NO aND OTP3.DPTDC_ISIN = dptd1.DPTDC_ISIN  and SUBTYPECDAS IN ('543','330','332','545'))
						-- otp status update
						)
						then 'RESPONSE FILE IMPORTED'

						when ISNULL(dptd1.dptdc_batch_no,'') <> '' AND (ISNULL(dptd1.dptdc_trans_no,'') = '')then 'BATCH GENERATED' 
						--when dptd1.dptdc_deleted_ind = 1 then 'CHECKER DONE' 
						when dptd.dptdc_deleted_ind = 0 and isnull(dptdc_res_cd,'') <> '' then 'REJECTED'
						when dptd.dptdc_deleted_ind = -1 and isnull(dptdc_res_cd,'') = '' then 'MAKER ENTERED (INSTRUCTION WITH HIGH VALUE OR DORMANT)'
                        when dptd.dptdc_deleted_ind = 0 and isnull(dptdc_res_cd,'') = '' and isnull(dptdc_mid_chk,'')='' then 'MAKER ENTERED (INSTRUCTION WITHOUT HIGH VALUE OR DORMANT)' 
...
--					,''
                    ...
                    , isnull('' ,'') mkt_type , isnull('' ,'') other_mkt_type , isnull('','') [settlementno] , isnull('','') [othersettmno], isnull('','') [cmbp], isnull('','') counter_account ,'' counter_dpid
					--,''
					,case when citrus_usr.gettranstype(ISNULL(d.DEMRM_TRANSACTION_NO,''),ISNULL(convert(varchar,d.DEMRM_dpam_id),'0'))<> '' then citrus_usr.[gettranstype_DRF](ISNULL(d.DEMRM_TRANSACTION_NO,''),ISNULL(convert(varchar,d.DEMRM_dpam_id),'0'))

when ISNULL(d.DEMRM_TRANSACTION_NO,'') = '0' then 'REJECTED FROM CDSL' +'-'    + isnull(citrus_usr.fn_get_errordesc(d.DEMRM_ERRMSG),'')
when ISNULL(d.DEMRM_TRANSACTION_NO,'') <> '0' and isnull(d.DEMRM_COMPANY_OBJ ,'') <> '' then 'REJECTED FROM CDSL' +'-'    + isnull(citrus_usr.fn_get_errordesc(d.DEMRM_ERRMSG),'')
					when ISNULL(d.DEMRM_TRANSACTION_NO,'') <> '' then 'RESPONSE FILE IMPORTED'  
