-- Object: PROCEDURE citrus_usr.PR_SELECT_TRXGEN_CDSL_SP_XML_EDIS_bak_22072020
...
create  PROCEDURE [citrus_usr].[PR_SELECT_TRXGEN_CDSL_SP_XML_EDIS_bak_22072020]  
(  
...
select T.* into #TEMPDATANEW from #TEMPDATA T,dp_trx_dtls_cdsl M where T.dptdc_id=M.dptdc_id 
--and M.DPTDC_SLIP_NO like 'E%'
and M.dptdc_slip_no in (
    Select cdsl_trxn_id from E_DIS_PROCESS_DATA with(nolock)
    union all
    select DPTDC_SLIP_NO from dptdc_mak I where i.dptdc_slip_no=M.DPTDC_SLIP_NO and M.DPTDC_SLIP_NO like 'e%'                
    )           
