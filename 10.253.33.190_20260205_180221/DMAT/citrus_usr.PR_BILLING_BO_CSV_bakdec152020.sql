-- Object: PROCEDURE citrus_usr.PR_BILLING_BO_CSV_bakdec152020
...
Create  PROCEDURE [citrus_usr].[PR_BILLING_BO_CSV_bakdec152020]  
...
AMOUNT 
,DPAM_SBA_NO,'Demat Monthly Maintenance Charges' FINA_ACC_NAME ,'1'						
...
--and b.CLIC_TRANS_DT BETWEEN @PA_BILLING_FROM_DT AND @PA_BILLING_TO_DT and b.CLIC_CHARGE_NAME like '%gst%'),'0')) 
AMOUNT 
,DPAM_SBA_NO,'Charges for On Mkt / Off Mkt Transactions fOR boid ' + DPAM_SBA_NO FINA_ACC_NAME ,'2'						
						FROM   CLIENT_CHARGES_CDSL a 
						--LEFT OUTER JOIN ACCOUNT_PROPERTIES ON ACCP_CLISBA_ID = CLIC_DPAM_ID AND ACCP_ACCPM_PROP_CD = 'BBO_CODE' 
						--AND ACCP_DELETED_IND = 1 
						,DP_ACCT_MSTR, DP_MSTR , CLIENT_DP_BRKG , BROKERAGE_MSTR
...
--and b.CLIC_TRANS_DT BETWEEN @PA_BILLING_FROM_DT AND @PA_BILLING_TO_DT and b.CLIC_CHARGE_NAME like '%gst%'),'0')) 
AMOUNT 
,DPAM_SBA_NO,'Charges for Pledge/Unpledge of Securities' FINA_ACC_NAME	,'3'					
						FROM   CLIENT_CHARGES_CDSL a 
						--LEFT OUTER JOIN ACCOUNT_PROPERTIES ON ACCP_CLISBA_ID = CLIC_DPAM_ID AND ACCP_ACCPM_PROP_CD = 'BBO_CODE' 
						--AND ACCP_DELETED_IND = 1 
						,DP_ACCT_MSTR, DP_MSTR , CLIENT_DP_BRKG , BROKERAGE_MSTR
...
SELECT ''' +  CONVERT(VARCHAR(11),@PA_BILLING_TO_DT) + ''' , ''' +  CONVERT(VARCHAR(11),@PA_BILLING_TO_DT) + ''' ,  ISNULL(DPAM_BBO_CODE,'''') ,LDG_ACCOUNT_ID ,CASE WHEN SUM(LDG_AMOUNT)< 0 THEN ''D'' ELSE ''C'' END ,ABS(SUM(LDG_AMOUNT))  , dpam_sba_no,''Ledger Outstanding Amount'' FINA_ACC_NAME,''4''
FROM DP_ACCT_MSTR ,
...
--where drcr='C'
where (CLTCODE not like 'ZR%' or CLTCODE not like '98%' )
...
if @PA_LOGIN_NAME='EQCR'
						...
						--and b.CLIC_TRANS_DT BETWEEN @PA_BILLING_FROM_DT AND @PA_BILLING_TO_DT and b.CLIC_CHARGE_NAME like '%gst%'),'0')) 
						AMOUNT ,DPAM_SBA_NO
						,'Demat Monthly Maintenance Charges' FINA_ACC_NAME ,'1'
						FROM   CLIENT_CHARGES_CDSL a 
						--LEFT OUTER JOIN ACCOUNT_PROPERTIES ON ACCP_CLISBA_ID = CLIC_DPAM_ID AND ACCP_ACCPM_PROP_CD = 'BBO_CODE' 
						--AND ACCP_DELETED_IND = 1 
						,DP_ACCT_MSTR, DP_MSTR , CLIENT_DP_BRKG , BROKERAGE_MSTR
...
-----for transaction
						...
						,DPAM_SBA_NO
						,'Charges for On Mkt / Off Mkt Transactions FOR boid ' +DPAM_SBA_NO FINA_ACC_NAME,'2'
						FROM   CLIENT_CHARGES_CDSL a 
						--LEFT OUTER JOIN ACCOUNT_PROPERTIES ON ACCP_CLISBA_ID = CLIC_DPAM_ID AND ACCP_ACCPM_PROP_CD = 'BBO_CODE' 
						--AND ACCP_DELETED_IND = 1 
						,DP_ACCT_MSTR, DP_MSTR , CLIENT_DP_BRKG , BROKERAGE_MSTR
...
----for pledge
						...
						--and b.CLIC_TRANS_DT BETWEEN @PA_BILLING_FROM_DT AND @PA_BILLING_TO_DT and b.CLIC_CHARGE_NAME like '%gst%'),'0')) AMOUNT 
						,DPAM_SBA_NO
						,'Charges for Pledge/Unpledge of Securities' FINA_ACC_NAME,'3'
						FROM   CLIENT_CHARGES_CDSL a 
						--LEFT OUTER JOIN ACCOUNT_PROPERTIES ON ACCP_CLISBA_ID = CLIC_DPAM_ID AND ACCP_ACCPM_PROP_CD = 'BBO_CODE' 
						--AND ACCP_DELETED_IND = 1 
						,DP_ACCT_MSTR, DP_MSTR , CLIENT_DP_BRKG , BROKERAGE_MSTR
...
Set  @L_SQL = ' INSERT INTO  #MAIN_BILL
SELECT ''' +  CONVERT(VARCHAR(11),@PA_BILLING_TO_DT) + ''' , ''' +  CONVERT(VARCHAR(11),@PA_BILLING_TO_DT) + ''' ,  ISNULL(DPAM_BBO_CODE,'''') ,LDG_ACCOUNT_ID ,CASE WHEN SUM(LDG_AMOUNT)< 0 THEN ''C'' ELSE ''D'' END ,ABS(SUM(LDG_AMOUNT))  , dpam_sba_no,''Ledger Outstanding Amount'' FINA_ACC_NAME,''4''
FROM DP_ACCT_MSTR ,
LEDGER' + convert(varchar,@l_fin_id) + ' t
