-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------


CREATE PROCEDURE  [citrus_usr].[pr_dp_select_mstr]( @pa_id  VARCHAR(800)                          
...
and right(ONL_TRXFEREE_BOID,8) not  in ('00072218','20200624' )
UNION ALL
...
Union      
 ...
 Union 
 select 0,'FQ' , 'FIXED QUARTERLY CHARGE'
    order by 3         
    end                    
...
AND    trastm_desc          LIKE CASE WHEN LTRIM(RTRIM(@pa_desc)) = '' THEN '%' ELSE @pa_desc+'%'  END                        
  ...
  --                    
    SELECT DISTINCT @l_dpm_dpid                    DPID                    
          ...
          ,demrm_qty   QUANTITY                    
          ,isnull(a.cd,'')    DEMRM_INTERNAL_REJ            
		  ,isnull(b.cd ,'')  DEMRM_COMPANY_OBJ 
          ,DEMRM_CREDIT_RECD                  
...
,demrm_typeofsec
,isnull((Select distinct top 1 isnull(demrd_status,'') from demrd_mak where demrd_demrm_id=demrm.demrm_id and demrd_deleted_ind in (0,-1,2,4,6)) ,'')demrd_status
  ...
  --        
    ...
    BEGIN                    
    --   
    declare @l_demrm_dpam_id numeric 
    set @l_demrm_dpam_id = 0 
    
    select  @l_demrm_dpam_id = demrm_dpam_id  from demrm_mak  where demrm_id =  @pa_values
...
,case when DEMRM_DELETED_IND = 0 and (isnull(demrm_res_cd_intobj,'') <> '' or isnull(demrm_res_cd_compobj,'') <> '') then 'REJECTED' end status1
   ...
   ,demrm_typeofsec
   ,(Select distinct top 1 isnull(demrd_status,'') from demrd_mak where demrd_demrm_id=demrm.demrm_id and demrd_deleted_ind in (0,-1,2,4,6)) demrd_status
      FROM   demrm_mak                   demrm                    
          ...
          LEFT OUTER JOIN [FN_GETSUBTRANSDTLS]('DEMAT_REJ_CD_NSDL')   b    on    b.cd                      = demrm.DEMRM_COMPANY_OBJ                       
            ...
            , citrus_usr.fn_acct_list_disdrf_Acctno(@l_dpm_dpid,@l_entm_id,0,@l_demrm_dpam_id) dpam                    
  ...
  --                        
   ...
   ,REMRM_CREDIT_RECD            
      FROM   remrm_mak                   remrm                        
            ...
            --,exch_seg_mstr               excsm                        
            --,citrus_usr.fn_exch_list(@pa_roles,@pa_scr_id) excsm_list                        
            , citrus_usr.fn_acct_list_remat(@l_dpm_dpid,@l_entm_id,0) dpam                        
      WHERE  dpam.dpam_id              = remrm.remrm_dpam_id                        
      --AND    dpm_excsm_id              = excsm.excsm_id                        
      --AND    dpam.dpam_dpm_id          = dpm.dpm_id                        
      --AND    excsm_list.excsm_id      = excsm.excsm_id                        
