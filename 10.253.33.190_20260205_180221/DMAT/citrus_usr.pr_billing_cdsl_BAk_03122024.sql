-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_BAk_03122024
...
--ROLLBACK
CREATE procedure [citrus_usr].[pr_billing_cdsl_BAk_03122024](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
  ...
  update  qtr_park set    q_flg='0'   where month(trans_dt ) = 10 and  q_flg='1'  and month(@pa_billing_to_dt ) <> 12 
...
delete from Qtr_Park where trans_dt between @pa_billing_from_dt and @pa_billing_to_dt 
...
,charge_val
,post_toacct
from Qtr_Park a where Q_FLG = 0 
and DATEADD(month, 3*(DATEDIFF(month, 0,DATEFROMPARTS(YEAR(@pa_billing_to_dt), MONTH(@pa_billing_to_dt ), DAY(@pa_billing_to_dt ))  )/3), 0)  = Qtr 
...
--and not exists (select 2 from AGMUBODPL3.dmat.citrus_usr.client_trade_month,dp_acct_mstr  b where Client_COde = dpam_sba_no and a.dpam_id = b.dpam_id and isnull(last_trade ,'') between  @pa_billing_from_dt and @pa_billing_to_dt )
