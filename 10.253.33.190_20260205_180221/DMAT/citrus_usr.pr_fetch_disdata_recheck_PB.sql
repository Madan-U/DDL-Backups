-- Object: PROCEDURE citrus_usr.pr_fetch_disdata_recheck_PB
...
CREATE proc [citrus_usr].[pr_fetch_disdata_recheck_PB](@pa_Action varchar(100)
...
--'for poa
select  '1' + substring(left(dpam_sba_no,8),2,7) dpm_dpid
				, dpam_sba_no 
				, dpam_sba_name
...
--and isnull(reco_yn,'') = ''  --commented on 04102013 as per MOSL request
                ...
                and  SLIIM_SLIP_NO_FR>= 200000001 
				and  SLIIM_SLIP_NO_TO<= 300000000   
				and ISNULL(SLIIM_SUCCESS_FLAG,'')=CASE WHEN sliim_dt>='2014-09-27 00:00:00.000' then 'S' else '' end 
				and isnull(sliim_RECHK_FLAG,'')=''
                union
				select '1' + substring(left(dpam_sba_no,8),2,7) dpm_dpid
				, dpam_sba_no 
				, dpam_sba_name
				...
				update temp1 Set temp1.sliim_RECHK_FLAG='Y'  from slip_issue_mstr_poa temp1,  citrus_usr.Vw_POA_MSTR left outer join dp_holder_dtls on dphd_dpam_id = dpam_id 
				--,#account_properties --, Dis_req_Dtls dis
				--,#temp1 temp1 --,#temp2 temp2
				where  sliim_deleted_ind = 1 --and accp_clisba_id = dpam_id 
				--and dpam_dpm_id = dpm_id  --and dis.sliim_dpam_acct_no = dpam_sba_no 
