-- Object: PROCEDURE citrus_usr.pr_ins_upd_delout_trx
...
Trans_code = if reversible then 906 & if irrevesible then 912
...
*/
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_delout_trx](@pa_id              VARCHAR(8000)  
...
*********************************************************************************
 SYSTEM         : dp
 MODULE NAME    : pr_ins_upd_delout_trx
...
--
  ...
  BEGIN
  --
    SELECT @l_dtls_id = dptd_dtls_id FROM  dp_trx_dtls WHERE dptd_dtls_id = convert(numeric,@pa_id) AND dptd_trastm_cd in ('906','912')
  --
  END
  ELSE
  BEGIN
  --
    SELECT @l_dtls_id = dptd_dtls_id FROM  dptd_mak WHERE dptd_dtls_id = convert(numeric,@pa_id) AND dptd_trastm_cd in ('906','912')
  --
  END
-- 
END
...
SET @remainingstring = @pa_id  
		...
		--
								...
								--
								  ...
								  SET @l_action             = citrus_usr.fn_splitval(@currstring_value,1)      
										...
										--
										  ...
										  SET @l_qty               = CONVERT(VARCHAR(25),CONVERT(NUMERIC(18,3),-1*convert(numeric(18,3),@l_qty))) 
              ...
              IF EXISTS(select * from dp_trx_dtls where dptd_dtls_id = CONVERT(INT,@currstring) and dptd_deleted_ind = 1)
														...
														--
														    ...
														    ,dptd_rmks
														      ...
														      ,dptd_rmks
																				...
																				,@l_dpam_id
																				,convert(datetime,@pa_req_dt,103)
																				,convert(datetime,@pa_exe_dt,103),@pa_slip_no
																				,@l_isin
																				,@line_no
																				,@l_qty
																				,@l_trastm_id
														...
														--
																COMMIT TRANSACTION
																 ...
																 SET @t_errorstr = convert(varchar,CONVERT(INT,@currstring)) 
														--
														END
												--
												END
              ...
              IF EXISTS(SELECT * FROM dptd_mak where dptd_dtls_id = CONVERT(INT,@currstring) and dptd_deleted_ind IN (0,-1))
              BEGIN
              --
                 if @l_action = 'D' 
																	BEGIN
														...
														--     
																...
																--
																		...
																		SET     dptd_dpam_id              = dptdm.dptd_dpam_id              
																									...
																									,dptd_trastm_cd            = dptdm.dptd_trastm_cd            
  ...
  --
				...
				BEGIN
				--
						exec pr_checkslipno '','DELOUT_DP_TRX_SEL_R', @pa_dpm_dpid,@pa_dpam_acct_no,@pa_slip_no,@pa_login_name,''
				--
				END
				ELSE
				BEGIN
				--
						exec pr_checkslipno '','DELOUT_DP_TRX_SEL_I', @pa_dpm_dpid,@pa_dpam_acct_no,@pa_slip_no,@pa_login_name,''
				--
				END
		--
		END
