-- Object: PROCEDURE citrus_usr.Pr_rpt_bill_profile_filter_valuation_07052010_celestial
...
[Pr_rpt_bill_profile_filter_valuation_07052010] 'CDSL',3,'july 15 2010','july 15 2010','N','N','','','','N','Y','',1,'HO|*~|',''	

Pr_rpt_bill_profile_filter 	'NSDL',4,'Sep 11 2008','Sep 11 2008 23:59:59','N','N','10000037','','','N','Y','',1,'HO|*~|',''

*/
create Proc [citrus_usr].[Pr_rpt_bill_profile_filter_valuation_07052010_celestial]              
@pa_dptype varchar(4),                      
@pa_excsmid int,                      
...
, settm_id varchar(25),  tr_settm_id varchar(25), tratm_cd varchar(25) --changed by jitesh
    , valuation numeric(18,3) -- charged by tushar 
...
--   if @pa_group_cd <> ''      
--   begin            
--    insert into #tempmaincdsl(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val,settm_id,tr_settm_id,tratm_cd)           
...
-- end          
--      
----changed by jitesh july 13 2010
...
select CDSHM_DPAM_ID,                        
      ...
      CDSHM_QTY,                        
      1,                                  
      CDSHM_ID   ,0, CDSHM_SETT_NO ,  right(isnull(CDSHM_TRG_SETTM_NO,'') ,7),cdshm_tratm_cd                                 
      FROM CDSL_HOLDING_DTLS with(nolock),                        
      #ACLIST account  ,DMT_STATUS_MSTR                                      
      where   CDSHM_DPM_ID = @@dpmid                          
      AND CDSHM_TRAS_DT >=@pa_fromdate AND CDSHM_TRAS_DT <=@pa_todate                                    
...
--changed by tushar sep 18 2009   
   ...
   begin 
	   ...
	   begin     
			  ...
			  begin 
                 ...
                 ,ISNULL(convert(numeric(18,2),CLOPM_CDSL_RT),0.00)*DPHMCD_CURR_QTY                      
				 FROM DP_DAILY_HLDG_CDSL with(nolock)  
                  LEFT OUTER JOIN CLOSING_PRICE_MSTR_CDSL ON DPHMCD_ISIN = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_cdsl WHERE CLOPM_ISIN_CD = DPHMCD_ISIN and CLOPM_DT <= @pa_todate and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc)                        
				 ,account_group_mapping g with(nolock)              
				 ,#ACLIST account 
 ...
 begin  
  ...
  group by clic_dpam_id,dpam_sba_no,dpam_sba_name,clic_charge_name                     
   ...
   , case when order_by = '3' then valuation else 0 end as Valuation
   --,isnull([citrus_usr].[fn_acct_entp](t.dpam_id,'bbo_code'),'') as bbo_code
