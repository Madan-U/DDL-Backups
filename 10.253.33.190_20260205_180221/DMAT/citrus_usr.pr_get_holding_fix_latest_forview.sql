...
declare @l_rerun_insec numeric
set @l_rerun_insec = 0 
SELECT @l_rerun_insec = DATEDIFF(
           SECOND, (select max(dt) from holdingdumplog where status ='end'  ) ,
(select max(dt) from holdingdumplog where status ='start') )  
