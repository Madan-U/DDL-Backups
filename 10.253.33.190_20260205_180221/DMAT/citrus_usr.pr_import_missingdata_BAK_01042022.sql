-- Object: PROCEDURE citrus_usr.pr_import_missingdata_BAK_01042022
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

Create  proc [citrus_usr].[pr_import_missingdata_BAK_01042022]
as
begin



select IDENTITY(numeric,1,1) id , DPAM_SBA_NO into #data from dp_Acct_mstr where isnull(dpam_bbo_code ,'')=''
and exists(select 1 from  [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp where dp_id = DPAM_sba_no)
...
--select * from  [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATIOn where dp_id = '1203320009734135'
...
if exists (select 1 from dp_acct_mstr where DPAM_SBA_NO = @l_cur_boid and ISNULL(DPAM_BBO_CODE ,'')='') 
begin 
print @l_cur_boid
	/*brokerage*/
	update  [CLIENT_DP_BRKG] set clidb_deleted_ind = 0 
	where CLIDB_DPAM_ID in (select dpam_id from dp_acct_mstr where DPAM_SBA_NO = @l_cur_boid ) 
	...
	from [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp , dp_Acct_mstr , brokerage_mstr where dp_id = @l_cur_boid
	and DPAM_SBA_NO = dp_id and template_code = BROM_DESC 
    /*brokerage*/
    
	/*relationship*/
	select @l_br = ENTM_ID  from [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp, entity_mstr where ENTM_ENTTM_CD ='BR'and replace(entm_short_name,'_br','') = branch_cd and dp_id = @l_cur_boid
	select @l_sb = ENTM_ID  from [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp, entity_mstr where ENTM_ENTTM_CD ='sb'and replace(entm_short_name,'_sb','') = subbroker  and dp_id = @l_cur_boid
	--select @l_trd = ENTM_ID  from [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATIOn, entity_mstr where ENTM_ENTTM_CD ='trd'and replace(entm_short_name,'_trd','') = trader   and dp_id = @l_cur_boid
	select @l_re = ENTM_ID  from [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp, entity_mstr where ENTM_ENTTM_CD ='re'and replace(entm_short_name,'_re','') = region   and dp_id = @l_cur_boid
	select @l_ar  = ENTM_ID  from [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp, entity_mstr 
	where ENTM_ENTTM_CD ='ar'and replace(entm_short_name,'_ar','') = area  and dp_id = @l_cur_boid
	
	--select @l_bl  = ENTM_ID  from [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATIOn, entity_mstr ,
	...
	,entr_dummy4 = @l_sb
	--,entr_dummy5 = @l_bl
	from entity_relationship entr ,  [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp
    ...
    FROM [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp
   ...
   EXEC PR_INS_UPD_ENTP '1','EDT','MIG',@L_CRN_NO,'',@L_ENTP_VALUE,'',0,'*|~*','|*~|',''    
	...
	,ACCOUNT_PROPERTY_MSTR WHERE dp_id = @l_cur_boid AND ACCPM_PROP_CD = 'ECN_FLAG'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' +case when  UPPER(ISNULL(LTRIM(RTRIM(ECs_FLAG)),''))='yes' then '1' else '0' end  + '|*~|*|~*' 
          FROM [172.31.16.57].crmdb_a.dbo.VW_CITRUS_DPACCACTIVATION_For_new_dp ,ACCOUNT_PROPERTY_MSTR 
