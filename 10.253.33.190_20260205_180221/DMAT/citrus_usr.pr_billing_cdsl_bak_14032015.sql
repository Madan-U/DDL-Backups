-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_bak_14032015
...
--begin tran
--[pr_billing_cdsl] 'JAN 01 2015','JAN 31 2015',3,'y'
--ROLLBACK
create procedure [citrus_usr].[pr_billing_cdsl_bak_14032015](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
...
exec [citrus_usr].[pr_fetch_dis_clientlist] @pa_billing_from_dt,@pa_billing_TO_dt,@pa_dpm_id
	...
	and   proc_deleted_ind = 1
	and   cham_deleted_ind = 1
	and 1 < (select COUNT(1) from SLIP_ISSUE_MSTR sliim where DPAM_SBA_NO = sliim .SLIIM_DPAM_ACCT_NO)
  ...
  ------
        ...
        set    cdshm_charge   =  
			    ...
			    ,cdshm_post_toacct = CHAM_POST_TOACCT
			      ...
			      AND   cham_charge_baseon           = 'TV' 
			      --AND   cham_charge_type = cdshm_tratm_type_desc
				AND   cham_charge_type = case when cham_charge_type  = 'OF-DR(SAC)' AND  cdshm_tratm_type_desc   = 'OF-DR' and (CDSHM_COUNTER_BOID in (select account from SPECIFIED_ACCOUNT_LIST where cm_ben='B')
				...
				OR CDSHM_COUNTER_CMBPID in (select account from SPECIFIED_ACCOUNT_LIST where cm_ben='CMBP') )  then  'OF-DR(SAC)'
												...
												when cham_charge_type  = 'INTDEP-DR(SAC)' AND  cdshm_tratm_type_desc = 'INTDEP-DR' AND (CDSHM_COUNTER_BOID in (select account from SPECIFIED_ACCOUNT_LIST where cm_ben='B')
				OR CDSHM_COUNTER_DPID in (select account from SPECIFIED_ACCOUNT_LIST where cm_ben='CMBP')
...
and cdshm_charge  <> 0 
and cdshm_tratm_type_desc ='ON-DR'
