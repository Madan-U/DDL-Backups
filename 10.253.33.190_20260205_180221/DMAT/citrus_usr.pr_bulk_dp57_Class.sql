...
BEGIN 
		...
		set @l_dpm_id = 0 
		select @l_dpm_id  = '0' --dpm_id  from dp_mstr where dpm_dpid like '%' + @l_dpm_dpid -- and default_dp = dpm_excsm_id  
		...
		--not in (select dpam_sba_no from dp_acct_mstr where dpam_dpm_id = @l_dpm_id and dpam_deleted_ind = 1 ) and
		 citrus_usr.fn_splitval_by(value,1,'~')='D' and citrus_usr.fn_splitval_by(value,2,'~') not in ('8','11','10') --<>'8' 
...
create index ix_1 on #tempdatacdsl (cdshm_tras_dt 
,cdshm_ben_acct_no 
,cdshm_trans_no 
,cdshm_isin
,cdshm_qty 
