-- Object: PROCEDURE citrus_usr.pr_get_invzero_zeroinv_bak_30012018
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--exec pr_get_invzero_zeroinv 'INVTOZERO'
--exec pr_get_invzero_zeroinv 'ZEROTOINV'
create proc [citrus_usr].[pr_get_invzero_zeroinv_bak_30012018](@pa_tab varchar(100),@pa_dt varchar(100))
as
begin 
...
--select * from #final
				...
				where boid = DPAM_SBA_NO  and DPAM_ID = ACCP_CLISBA_ID  and CLIDB_DPAM_ID = DPAM_ID and CLIDB_BROM_ID = BROM_ID
		...
		print 'yogesh'
			select distinct convert(datetime,accp_value,103) accp_value, accp_clisba_id , accp_accpm_prop_cd 
				...
				select LDG_ACCOUNT_ID, SUM(ldg_amount) os  into #ledger from LEDGER4 where ldg_deleted_ind = 1 group by LDG_ACCOUNT_ID
