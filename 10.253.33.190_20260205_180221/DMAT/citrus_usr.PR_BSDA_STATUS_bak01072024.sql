-- Object: PROCEDURE citrus_usr.PR_BSDA_STATUS_bak01072024
...
create   procedure [citrus_usr].[PR_BSDA_STATUS_bak01072024]
(
@PA_DPM_ID INT
...
when sum((isnull(RATE,0)*isnull(DPHMCD_CURR_QTY-DPHMCD_DEMAT_PND_VER_QTY-DPHMCD_LOCKIN_QTY,0))) <='50000' then 'BSDA' 
when sum((isnull(RATE,0)*isnull(DPHMCD_CURR_QTY-DPHMCD_DEMAT_PND_VER_QTY-DPHMCD_LOCKIN_QTY,0))) BETWEEN '50001' AND '200000' then 'BSDA1' 
when sum((isnull(RATE,0)*isnull(DPHMCD_CURR_QTY-DPHMCD_DEMAT_PND_VER_QTY-DPHMCD_LOCKIN_QTY,0))) > '200001' then 'VERSION 2.5 NORMAL'
END,'') [Tariff_to_be_changed]
,isnull(case when  a.brom_desc LIKE '%LIF%' then 'LIFE' 
--when  a.brom_desc LIKE '%ZERO%' then 'LIFE'
when sum((isnull(RATE,0)*isnull(DPHMCD_CURR_QTY-DPHMCD_DEMAT_PND_VER_QTY-DPHMCD_LOCKIN_QTY,0))) <='50000' then 'NEED TO CHANGE BSDA' 
when sum((isnull(RATE,0)*isnull(DPHMCD_CURR_QTY-DPHMCD_DEMAT_PND_VER_QTY-DPHMCD_LOCKIN_QTY,0))) BETWEEN '50001' AND '200000' then 'NEED TO CHANGE BSDA1' 
when sum((isnull(RATE,0)*isnull(DPHMCD_CURR_QTY-DPHMCD_DEMAT_PND_VER_QTY-DPHMCD_LOCKIN_QTY,0))) > '200001' then 'NEED TO DEACTIVE BSDA'
...
AND BROM_DESC NOT IN ( 'FREE LT AMC')
