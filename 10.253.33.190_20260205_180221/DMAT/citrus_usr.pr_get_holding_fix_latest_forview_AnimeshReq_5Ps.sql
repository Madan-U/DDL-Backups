-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_forview_AnimeshReq_5Ps
...
create proc [citrus_usr].[pr_get_holding_fix_latest_forview_AnimeshReq_5Ps](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
,@pa_ben_acct_no_fr varchar(16),@pa_ben_acct_no_to varchar(16),@pa_out  varchar(8000) out)      
as      
begin       
  

insert into holdingdumplog_Mp 
...
,DPHMC_REPLEGE_BAL DPHMCd_REPLEGE_BAL                     
...
,convert(numeric(18,5),0) DPHMCd_REPLEGE_BAL  
...
,sum(DPHMCD_CURR_QTY) DPHMCD_CURR_QTY           
 ...
 ,sum(DPHMCd_REPLEGE_BAL) DPHMCd_REPLEGE_BAL                      
 ,dphmcd_holding_dt                      
...
,a.DPHMCd_REPLEGE_BAL= f.DPHMCd_REPLEGE_BAL 
...
CREATE TABLE #PLEDGEHOLDING(dphmcd_dpam_id	NUMERIC,dphmcd_isin	VARCHAR(20) ,qty	NUMERIC(18,3) , PLEDGETYPE VARCHAR(100))

INSERT INTO #PLEDGEHOLDING

EXEC PR_GET_PLEDGE_HOLDING 3,@pa_from_dt,@pa_from_dt,'0','9999999999999999',''



begin tran   
    
truncate table holdingallforview_5Ps_5Ps

insert into holdingallforview_5Ps_5Ps
...
,DPHMCD_FREE_QTY
,DPHMCD_FREEZE_QTY
,DPHMCD_PLEDGE_QTY-ISNULL((SELECT QTY FROM #PLEDGEHOLDING B WHERE A.DPHMCD_DPAM_ID=B.DPHMCD_DPAM_ID AND A.DPHMCD_ISIN= B.DPHMCD_ISIN AND B.PLEDGETYPE ='MARGIN') ,0 ) DPHMCD_PLEDGE_QTY
...
,DPHMCD_DELETED_IND
,dphmcD_cntr_settm_id  
,@pa_to_dt dphmcd_holding_dt ,isnull(CLOPM_CDSL_RT,0) CLOPM_CDSL_RT 
,isnull(DPAM_BBO_CODE,'')  tradingid 
,ISNULL((SELECT QTY FROM #PLEDGEHOLDING B WHERE A.DPHMCD_DPAM_ID=B.DPHMCD_DPAM_ID AND A.DPHMCD_ISIN= B.DPHMCD_ISIN AND B.PLEDGETYPE ='MARGIN'),0) DPHMCD_MARGINPLEDGE_QTY    
,DPHMCd_REPLEGE_BAL                 

from #tmp_dp_daily_hldg_cdsl t  
...
or DPHMCd_REPLEGE_BAL  <> '0') 

commit


select * from holdingallforview_5Ps 
...
--alter table holdingallforview_5Ps alter column  rate numeric(18,3)
