-- Object: PROCEDURE citrus_usr.Pr_Transactioncharges_Breakup_BKP_07NOV2024_SRE_31133
...
CREATE PROCEDURE [citrus_usr].[Pr_Transactioncharges_Breakup_BKP_01NOV2024_SRE_31133](
	@padpmid  VARCHAR(800),
	@pafromdate datetime,
	@patodate  datetime,
...
and CDSHM_CDAS_SUB_TRAS_TYPE in ('838')--1
 ...
 ,cdshm_counter_boid [Target Beneficiary]
 ,cdshm_internal_trastm   [TRXN Type]
 ,Meaning [TRXNTYPE]
 ,isnull(cdshm_charge ,0) * -1 [TRXN Charge]
 ,case when SexCd ='F'  and left(cdshm_isin, 3)  not in ('INF','IN0')   then cdsl_amt - case when trxtype in (2,3) then  0 else 0.25 end 
  ...
  else cdsl_amt  end  [CDSL Charge] 
 ,(isnull(cdshm_charge ,0) * -1  ) * 0.18 [GST]
 ,(isnull(cdshm_charge ,0) * -1  ) * 1.18  [Total Chrg]
 ,SexCd  [Gender]
 ,ISIN_SECURITY_TYPE_DESCRIPTION Isin_type  
