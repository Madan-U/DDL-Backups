-- Object: PROCEDURE citrus_usr.Pr_rpt_statement_for_test02052012_before29062012
...
CREATE Proc [citrus_usr].[Pr_rpt_statement_for_test02052012_before29062012]                            
@pa_dptype varchar(4),                                    
@pa_excsmid int,                                    
...
create clustered index ix_1 on #bbocode(clientid,bbo)  
  ...
  begin
	if @@bbocode <> ''
	INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,isnull(EFF_TO,'dec 31 2100') FROM citrus_usr.fn_acct_list(@@dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id) where dpam_stam_cd <> '02_BILLSTOP'	
  ...
  begin
	if @@bbocode <> ''
	INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,isnull(EFF_TO,'dec 31 2100') FROM citrus_usr.fn_acct_list(@@dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id)		
...
delete from #ACLIST where dpam_sba_no not in (select grp_client_code from group_mstr)
 ...
 BEGIN                              
  ...
  begin  
      ...
      insert into #tempmaincdsl(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd ,sett_type ,sett_no  ,opening_bal,qty,order_by,line_no,cdshm_trg_settm_no,tratm_cd,exedt,c_d_flag)                         
...
,convert(datetime, case when citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),16,'~') <> '' then substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),16,'~'),5,4)+'-'
+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),16,'~'),3,2)
+'-'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),16,'~'),1,2)
+ ' ' + 
replace(
substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),16,'~'),9,2)
+':'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),16,'~'),11,2)+':'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),13,2)
, '00:00:00', '23:59:59') else 
substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),5,4)+'-'
+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),3,2)
+'-'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),1,2)
+ ' ' + 
replace(
substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),9,2)
+':'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),11,2)+':'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),13,2)
, '00:00:00', '23:59:59') end )
    ,case when cdshm_qty > 0 then 'C' else 'D' end 
      ...
      and cdshm_tratm_cd in('2246','2277','2201','3102','2270','2220','2262','2251','2202','2205') --'2252'  --,'4456'  '2280', -add 2205
...
--      and cdshm_isin like '' + '%'        
  ...
  begin    
      ...
      ,isin_cd
      ,sett_type,sett_no
      ,opening_bal,qty,order_by,line_no,cdshm_trg_settm_no,tratm_cd,exedt,c_d_flag)                         
      select CDSHM_DPAM_ID,                        
      DPAM_SBA_NAME,                        
      CDSHM_BEN_ACCT_NO,                                    
      CDSHM_TRATM_DESC=replace(CDSHM_TRATM_DESC,'        ',''),                            
...
--+ ' ' + 
--replace(
--substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),16,'~'),9,2)+':'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),16,'~'),11,2)+':'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),13,2), '00:00:00', '23:59:59') else 
--substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),5,4)+'-'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),3,2)+'-'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),1,2)+ ' ' + 
--replace(
--substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),9,2)+':'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),11,2)+':'+substring(citrus_usr.fn_splitval_by(substring(cdshm_trans_cdas_code,CHARINDEX('D', cdshm_trans_cdas_code),len(cdshm_trans_cdas_code)),9,'~'),13,2), '00:00:00', '23:59:59') end )
...
where  
tratm_cd not in ('2246','2277','3102','2201','2270','2220','2262','2251','2202','2205') --,'2252','4456' '2280',--added 2205
...
--changed by tushar sep 18 2009    
  ...
  begin
     ...
     --        
		 ...
		 FROM #tmphldg LEFT OUTER JOIN  isin_mstr i with(nolock) on DPHMCd_ISIN = i.isin_cd 
...
order by dpam_acctno,dpam_sba_name,order_by,Isin_name,convert(datetime,trans_date),exedt,dpm_trans_no,c_d_flag,cdshm_trg_settm_no    		 
		 end 
