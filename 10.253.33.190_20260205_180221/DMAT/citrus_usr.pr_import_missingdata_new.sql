-- Object: PROCEDURE citrus_usr.pr_import_missingdata_new
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE proc [citrus_usr].[pr_import_missingdata_new]
as
begin
...
and exists(select 1 from VW_CITRUS_DPACCACTIVATIOn_new  where dp_id = DPAM_sba_no) and  DPAM_SBA_NO <>'1203320023623131'
...
--select * from  VW_CITRUS_DPACCACTIVATIOn_new where dp_id = '1203320009734135'

Create index #d on #data (id)
...
begin 
 --print @l_cur_boid
	...
	from VW_CITRUS_DPACCACTIVATIOn_new, dp_Acct_mstr , brokerage_mstr where dp_id = @l_cur_boid
	and DPAM_SBA_NO = dp_id and template_code = BROM_DESC 
    /*brokerage*/
    
	/*relationship*/
	select @l_br = ENTM_ID  from VW_CITRUS_DPACCACTIVATIOn_new, entity_mstr where ENTM_ENTTM_CD ='BR'and replace(entm_short_name,'_br','') = branch_cd and dp_id = @l_cur_boid
	select @l_sb = ENTM_ID  from VW_CITRUS_DPACCACTIVATIOn_new, entity_mstr where ENTM_ENTTM_CD ='sb'and replace(entm_short_name,'_sb','') = subbroker  and dp_id = @l_cur_boid
	--select @l_trd = ENTM_ID  from VW_CITRUS_DPACCACTIVATIOn_new, entity_mstr where ENTM_ENTTM_CD ='trd'and replace(entm_short_name,'_trd','') = trader   and dp_id = @l_cur_boid
	select @l_re = ENTM_ID  from VW_CITRUS_DPACCACTIVATIOn_new, entity_mstr where ENTM_ENTTM_CD ='re'and replace(entm_short_name,'_re','') = region   and dp_id = @l_cur_boid
	select @l_ar  = ENTM_ID  from VW_CITRUS_DPACCACTIVATIOn_new, entity_mstr 
	where ENTM_ENTTM_CD ='ar'and replace(entm_short_name,'_ar','') = area  and dp_id = @l_cur_boid
	
	--select @l_bl  = ENTM_ID  from VW_CITRUS_DPACCACTIVATIOn_new, entity_mstr ,
	...
	,entr_dummy4 = @l_sb
	--,entr_dummy5 = @l_bl
	from entity_relationship entr ,  VW_CITRUS_DPACCACTIVATIOn_new
    ...
    FROM VW_CITRUS_DPACCACTIVATIOn_new
   ...
   EXEC PR_INS_UPD_ENTP '1','EDT','MIG',@L_CRN_NO,'',@L_ENTP_VALUE,'',0,'*|~*','|*~|',''    
	...
	,ACCOUNT_PROPERTY_MSTR WHERE dp_id = @l_cur_boid AND ACCPM_PROP_CD = 'ECN_FLAG'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' +case when  UPPER(ISNULL(LTRIM(RTRIM(ECs_FLAG)),''))='yes' then '1' else '0' end  + '|*~|*|~*' 
          FROM VW_CITRUS_DPACCACTIVATIOn_new ,ACCOUNT_PROPERTY_MSTR 
