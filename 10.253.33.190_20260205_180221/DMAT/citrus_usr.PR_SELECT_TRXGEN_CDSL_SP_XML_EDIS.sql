-- Object: PROCEDURE citrus_usr.PR_SELECT_TRXGEN_CDSL_SP_XML_EDIS
...
CREATE PROCEDURE [citrus_usr].[PR_SELECT_TRXGEN_CDSL_SP_XML_EDIS]  
(  
...
exec PR_SELECT_TRXGEN_CDSL_SP_EDIS --_MOSL 
@l_TRX_TAB  ,  
@PA_FROM_DT ,  
@PA_TO_DT ,  
@PA_BATCH_NO ,  
...
select TOP 9990 T.* into #TEMPDATANEW from #TEMPDATA T,dp_trx_dtls_cdsl M where T.dptdc_id=M.dptdc_id 
--and M.DPTDC_SLIP_NO like 'E%'
and M.dptdc_slip_no in (
    Select cdsl_trxn_id from E_DIS_PROCESS_DATA with(nolock)
    union all
    select DPTDC_SLIP_NO from dptdc_mak I where i.dptdc_slip_no=M.DPTDC_SLIP_NO 
    and M.DPTDC_SLIP_NO like 'e%'    ---- need to uncomment and comment in case of 17 digit  for evening batch rohan nagda 01122020              
    )           
...
+'<Dis>'+ dptdc_slip_no +'</Dis>'
+'<Mkropid>'+ ltrim(rtrim(left(dptdc_created_by,12))) +'</Mkropid>'
+'<Ckropid>'+ ltrim(rtrim(left('E19709',12))) +'</Ckropid>'
+'<Vfropid>'+ verifier +'</Vfropid>'
...
+'<Conamt>'+  '0'  +'</Conamt>' --isnull(citrus_usr.FN_CONSIDERATIONAMT('CAMT',citrus_usr.FN_SPLITVAL_BY(ltrim(rtrim(substring(details , 71, 16))),2,'/')),'') 
...
+'<Mkropid>'+ ltrim(rtrim(left(dptdc_created_by,12))) +'</Mkropid>'
+'<Ckropid>'+ ltrim(rtrim(left('E19709',12))) +'</Ckropid>'
+'<Vfropid>'+ ltrim(rtrim(left(verifier,12))) +'</Vfropid>'
...
from #TEMPDATANEW where dptdc_created_by in (select dpam_sba_no from DP_ACCT_MSTR where DPAM_DELETED_IND=1)
...
select  @L_TOT_REC = count(*) from #TEMPDATANEW where dptdc_created_by in (select dpam_sba_no from DP_ACCT_MSTR where DPAM_DELETED_IND=1) 
--and dptdc_lst_upd_by like 'e%'
...
--	end
												...
												AND		dpam_dpm_id   = case when @L_DPM_ID='3' then dpam_dpm_id else @L_DPM_ID end
															...
															AND D1.DPTDC_SLIP_NO IN (SELECT DPTDC_SLIP_NO FROM #TEMPDATANEW)       

--------------------------- for normal payin on jan 8 2021
															UPDATE D1 Set dptdc_batch_no=@pa_batch_no FROM dp_trx_dtls_cdsl D1 ,dp_acct_mstr D2  
															WHERE  dptdc_dpam_id = dpam_id   
															--AND    dpam_dpm_id   = @l_dpm_id    
												AND		dpam_dpm_id   = case when @L_DPM_ID='3' then dpam_dpm_id else @L_DPM_ID end
															...
															AND    ltrim(rtrim(isnull(dptdc_batch_no,''))) = ''  
															AND    dptdc_request_dt BETWEEN  CONVERT(VARCHAR,CONVERT(DATETIME, @PA_FROM_DT,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@PA_TO_DT,103),106)+' 23:59:59'  
															AND    dptdc_internal_trastm in ('NP','BOCM')  
															AND    d2.dpam_sba_no LIKE CASE WHEN LTRIM(RTRIM(@PA_POOL_ACCTNO))     = '' THEN '%' ELSE @PA_POOL_ACCTNO  END
															--and    D1.DPTDC_SLIP_NO like 'E%'
																														and D1.dptdc_slip_no in (
															Select cdsl_trxn_id from E_DIS_PROCESS_DATA with(nolock)
															...
															AND D1.DPTDC_id IN (SELECT DPTDC_id FROM #TEMPDATANEW)       
--------------------------- for normal payin on jan 8 2021
