-- Object: PROCEDURE citrus_usr.PR_BULK_DP57_Converter
...
CREATE   proc [citrus_usr].[PR_BULK_DP57_Converter]
(  
...
insert into CODD_CDSL_log15092025
...
UPDATE FILETASK
SET    USERMSG = @l_miss_trxtype--, STATUS='FAILED'
--, TASK_END_DATE = GETDATE()
WHERE  TASK_ID = @PA_TASK_ID
...
,case when TxnTyp ='DMT' then isnull(DmatRejCd.CDSL_Old_Values,'')  when TxnTyp ='ATC' then isnull(XferTypFlg.CDSL_Old_Values,'')  when TxnTyp ='DES' then isnull(DstatRejCd.CDSL_Old_Values,'')  else isnull(TxnRsnCd.cdsl_old_value,'') end TxnRsnCd,isnull(AccptdQty,''),isnull(ActlQty,'')
...
,isnull(EPTXNIDCUSPA,'')
 ...
 left outer join STANDARD_VALUE_LIST  catyp  on catyp.ISO_Tags ='catyp'  and catyp.CDSL_Old_Values <> '' and catyp = catyp.Standard_Value
 ...
 left outer join STANDARD_VALUE_LIST  DstatRejCd  on DstatRejCd.ISO_Tags ='DstatRejCd'  and DstatRejCd.CDSL_Old_Values <> '' and DstatRejCd = DstatRejCd.Standard_Value
   left outer join STANDARD_VALUE_LIST  XferTypFlg  on XferTypFlg.ISO_Tags ='XferTypFlg'  and XferTypFlg.CDSL_Old_Values <> '' and XferTypFlg = XferTypFlg.Standard_Value
...
into tmp_dp57_o from #tmp_dp57_o
		...
		set @l_dpm_dpid  = ''
		select top  1 @l_dpm_dpid = '120'+ CNTRLSCTIESDPSTRYPTCPT
		from CODD_CDSL  
...
if exists(select 1 from CODD_CDSL  where left(clntid ,8)='12033200' )
begin 

declare @l_old_dp_cnt numeric
select @l_old_dp_cnt  = count(1) from CODD_CDSL  where  left(clntid ,8)='12033200'

select * into #tempdata_toolddp from (
select * from CODD_CDSL  where left(clntid ,8)='12033200'
) a


delete from AGMUBODPL3.dmat.citrus_usr.CODD_CDSL



insert into AGMUBODPL3.dmat.citrus_usr.CODD_CDSL 
select * from #tempdata_toolddp 

exec [AGMUBODPL3].dmat.citrus_usr.[pr_bulk_dp57_fromnewdp_converter] 'MIG','12033200',0,'','','*|~*', '|*~|','0',''
...
if exists(select 1 from CODD_CDSL where left(clntid ,8)='12033202' )
begin 

declare @l_old_dp_cnt_202 numeric
select @l_old_dp_cnt_202  = count(1) from CODD_CDSL  where left(clntid ,8)='12033202'

select * into #tempdata_toolddp_202 from (
select * from CODD_CDSL  where  left(clntid ,8)='12033202'
) a
 

delete from [ATMUMBODPL03].dmat.citrus_usr.CODD_CDSL  

insert into [ATMUMBODPL03].dmat.citrus_usr.CODD_CDSL 
select * from #tempdata_toolddp_202 

exec [ATMUMBODPL03].dmat.citrus_usr.[pr_bulk_dp57_fromnewdp_converter] 'MIG','12033202',0,'','','*|~*', '|*~|','0',''
...
if exists(select 1 from CODD_CDSL where left(clntid ,8)='12033203' )
begin 

declare @l_old_dp_cnt_203 numeric
select @l_old_dp_cnt_203  = count(1) from CODD_CDSL  where left(clntid ,8)='12033203'

select * into #tempdata_toolddp_203 from (
select * from CODD_CDSL  where left(clntid ,8)='12033203'

) a
 

delete from [ABVSDP203].dmat.citrus_usr.CODD_CDSL  

insert into [ABVSDP203].dmat.citrus_usr.CODD_CDSL 
select * from #tempdata_toolddp_203 

exec [ABVSDP203].dmat.citrus_usr.[pr_bulk_dp57_fromnewdp_converter] 'MIG','12033203',0,'','','*|~*', '|*~|','0',''
...
/*	cdsl issue cuspa account 07062024	for 203 dp	*/
if exists(select 1 from CODD_CDSL where left(clntid ,8)='12033204' )
begin 

declare @l_old_dp_cnt_204 numeric
select @l_old_dp_cnt_204  =  count(1) from CODD_CDSL  where left(clntid ,8)='12033204'


select * into #tempdata_toolddp_204 from (
select * from CODD_CDSL  where left(clntid ,8)='12033204'
) a
 

delete from [ABVSDP204].dmat.citrus_usr.CODD_CDSL  

insert into [ABVSDP204].dmat.citrus_usr.CODD_CDSL 
select * from #tempdata_toolddp_204 

exec [ABVSDP204].dmat.citrus_usr.[pr_bulk_dp57_fromnewdp_converter] 'MIG','12033204',0,'','','*|~*', '|*~|','0',''
...
--added above line as per mail from vishal dated 25012023
		...
		and column_3 =dpam_sba_no) and LEFT(column_3,8) not in ('12033200','16010100','12033202','12033203','12033204','12011300') ) 	
		...
		and LEFT(column_3,8) not in ('12033200','16010100','12033202','12033203','12011300') --- added apr 04 2023
...
-------------------------- added on 21/10/2022--For early payin reversal dr -------------------------
						  ...
						  when column_2 = '4'  and  column_7 not in ('401','409','430','431','433','417') then -- 431 
							  case when CITRUS_USR.fn_dp57_stuff('crdr',column_35,'','')  ='CR' 
							  then convert(numeric(18,5),column_29) 
							  else convert(numeric(18,5),column_29)*-1 end  
						  else 
...
,CDSHM_CDAS_TRAS_TYPE,CDSHM_CDAS_SUB_TRAS_TYPE
		...
		and not exists(select cdshm_tras_dt
						...
						and cdshm_trans_no = column_5 
						and isnull(cdshm_isin,'') = isnull(column_4 ,'')
						and cdshm_qty = CASE 
WHEN column_7 ='829' THEN  ABS(qty)
WHEN column_7 ='831' THEN ABS(qty)
WHEN column_7 ='832' THEN ABS(qty)*-1
WHEN column_7 ='833' THEN ABS(qty)
WHEN column_7 ='837' THEN ABS(qty)
WHEN column_7 ='838' THEN ABS(qty)
ELSE qty END    
 
						and cdshm_tratm_cd = column_35
