-- Object: PROCEDURE citrus_usr.summaryholding_valuation
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--Select Top 10 * From MOSLEDP..tblDPValuation
--Select Top 10 * From MOSLEDP..tblSummaryHolding_Valuation

-- drop proc summaryholding_valuation    
--exec summaryholding_valuation '0','mar 01 2013','mar 01 2013','0','9999999999999999',''    
--exec summaryholding_valuation '0','mar 31 2013','mar 31 2013','0','9999999999999999',''    
--exec summaryholding_valuation '0','mar 01 2013','mar 01 2013','1201090001321086','1201090001321086',''    
--exec summaryholding_valuation '0','mapr 30 2013','mar 31 2013','1201090001321086','1201090001321086',''    
CREATE proc [citrus_usr].[summaryholding_valuation](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                              
...
,convert(numeric(18,5),0) DPHMCD_LEND_QTY          
,convert(numeric(18,5),0) DPHMCD_BORROW_QTY          
from dp_daily_hldg_cdsl a with (nolock) ,dp_acct_mstr  with (nolock) where dphmcd_holding_dt ='jul 31 2011'          
...
--WHERE CDSHM_TRAS_DT  >='aug 01 2011' and CDSHM_TRAS_DT   <= @pa_to_dt          
...
from citrus_usr.dp_daily_hldg_cdsl,dp_acct_mstr with(nolock)                            
...
into #holdingdmtpenfing from  dp_daily_hldg_cdsl with (nolock), DP_aCCT_MSTR  with (nolock)      
...
select cdshm_dpam_id , cdshm_isin , sum(cdshm_qty) qty ,cdshm_trans_no into #trasdmtpenfing  from cdsl_holding_dtls a with (nolock)      
where CDSHM_TRATM_CD='2205'      
and CDSHM_BEN_ACCT_NO between @pa_ben_acct_no_fr and @pa_ben_acct_no_to          
--and CDSHM_DPM_ID = @PA_DPM_ID      
And cdshm_tras_dt between 'aug 01 2011' and @PA_TO_DT    
and exists (select  cdshm_dpam_id , cdshm_isin , cdshm_trans_no from   
cdsl_holding_dtls b where a.cdshm_dpam_id = b.cdshm_dpam_id   
and a.cdshm_isin = b.cdshm_isin   
and a.cdshm_trans_no = b.cdshm_trans_no   
and b.CDSHM_TRATM_CD='2277'   
and b.cdshm_tras_dt between 'aug 01 2011' and @PA_TO_DT    
)    
...
into #holdingdmtpenfing1 from  dp_daily_hldg_cdsl with (nolock) , DP_aCCT_MSTR with (nolock)      
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt='jul 31 2011'      
and dpam_sba_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to          
--and dphmcd_dpm_id = @PA_DPM_ID      
...
into #holdingdmtpenfing2 from  dp_daily_hldg_cdsl with (nolock), DP_aCCT_MSTR with (nolock)      
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt='jul 31 2011'      
group by dphmcd_dpam_id,dphmcd_isin       
...
when cdshm_tratm_cd in ('2280') and cdshm_tratm_type_desc in ('UNPLEDGE','CONFISCATE','AUTO PLEDGE','PLEDGE')   
...
and CDSHM_TRAS_DT between  'aug 01 2011' and @pa_to_dt      
AND CDSHM_CDAS_TRAS_TYPE IN ('20','21','22','23') --'12' -- 20 added by tushar on jul 16 2012      
GROUP BY CDSHM_DPAM_ID ,  CDSHM_ISIN       
--  
--SELECT * FROM  #tmp_dp_daily_hldg_cdsl WHERE  dphmcd_dpam_id  = 865192 and dphmcd_isin ='INE002A01018'  
--RETURN   
...
update a set DPHMCD_LOCKIN_QTY = DPHMCD_LOCKIN_QTY - DPHMCD_PLEDGE_QTY from #locindata_dr , #tmp_dp_daily_hldg_cdsl  a      
where dphmcd_dpam_id = cdshm_dpam_id       
and dphmcd_isin = cdshm_isin       
and  DPHMCD_LOCKIN_QTY <> 0   
and DPHMCD_LOCKIN_QTY  = DPHMCD_CURR_QTY  
...
-- - ISNULL(DPHMCD_avail_lend_qty,'0')   
-ISNULL(DPHMCD_SAFE_KEEPING_QTY,'0') -ISNULL(DPHMCD_LEND_QTY,'0') -       
...
select a.*,@pa_to_dt dphmcd_holding_dt  ,isnull(CLOPM_CDSL_RT,0) CLOPM_CDSL_RT       
into #holdingall       
from #tmp_dp_daily_hldg_cdsl   a      
left outer join (select max(CLOPM_DT) lastdt , clopm_isin_cd isincd from closing_price_mstr_cdsl       
     where CLOPM_DT <= @pa_from_dt      
...
select dphmcd_holding_dt,dpam_sba_no,replace(replace(entm_short_name,'_BA',''),'_BR','') entm_short_name  
,dphmcd_isin isin
,sum(DPHMCD_FREE_QTY*CLOPM_CDSL_RT) valuation       
,sum(DPHMCD_PLEDGE_QTY*CLOPM_CDSL_RT) pledge_valuation       
,sum(DPHMCD_LOCKIN_QTY*CLOPM_CDSL_RT) lockin_valuation       
,sum(DPHMCD_DEMAT_PND_VER_QTY*CLOPM_CDSL_RT) demat_valuation       
-- into tempdatatocheck   
from #holdingall,dp_acct_mstr left outer join entity_relationship on entr_sba = dpam_sba_no       
left outer join entity_mstr on (entm_id = entr_sb or entm_id = entr_br)        
where dphmcd_dpam_id = dpam_id       
AND GETDATE() BETWEEN ISNULL(ENTR_FROM_DT ,'JAN 01 1900')  AND ISNULL(ENTR_TO_DT ,'DEC 31 2100')  
group by dphmcd_holding_dt,dpam_sba_no,entm_short_name  ,dphmcd_isin   
