-- Object: FUNCTION citrus_usr.fn_get_clicm_bit
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE function [citrus_usr].[fn_get_clicm_bit](@pa_clicm_id   numeric
                                ,@pa_check      numeric
                                ,@pa_chk_yn     numeric
                                )
...
--
  ...
  BEGIN
  --
    SELECT @l_clicm_bit      = clicm_bit
    FROM   client_ctgry_mstr    WITH (NOLOCK)
    WHERE  clicm_id          = @pa_clicm_id
  ...
  --
    ...
    --
    IF @l_clicm_bit & power(2,@l_bitrm_bit_locaton-1) > 0
    BEGIN
    --
