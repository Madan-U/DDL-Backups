-- Object: PROCEDURE citrus_usr.pr_search_clidp_bak16072012
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--exec [pr_search_clidp] 'a','','','','','','','','','','client_dp_new','HO',2,'*|~*','|*~|',''
create  PROCEDURE [citrus_usr].[pr_search_clidp_bak16072012](@pa_crn_no        varchar(1000)        
 ...
 DECLARE @l_ent_id           NUMERIC        
  ...
  --    
   ...
   --    
     ...
     IF isnull(@l_ent_id,'0') <> '0'
     SET @l_string = @l_string + 'and entr.ENTR_CRN_NO = cm.clim_crn_no and entr.entr_deleted_ind = 1 and (entr_ho =' + convert(varchar,@l_ent_id )  + ' or entr_ar = ' + convert(varchar,@l_ent_id )  +  ' or entr_br = ' + convert(varchar,@l_ent_id )  + ' or entr_re = ' + convert(varchar,@l_ent_id )  + ' or entr_sb = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy1 = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy2 = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy3 = ' + convert(varchar,@l_ent_id )  + ')'     
     -- 
  ...
  --    
    ...
    --    
      ...
      IF isnull(@l_ent_id,'0') <> '0'
       SET @l_string = @l_string + 'and entr.ENTR_CRN_NO = clim.clim_crn_no and entr.entr_deleted_ind = 1 and (entr_ho =' + convert(varchar,@l_ent_id )  + ' or entr_ar = ' + convert(varchar,@l_ent_id )  +  ' or entr_br = ' + convert(varchar,@l_ent_id )  + ' or entr_re = ' + convert(varchar,@l_ent_id )  + ' or entr_sb = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy1 = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy2 = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy3 = ' + convert(varchar,@l_ent_id )  + ')'     
       if isnull(@pa_crn_no,'') <> ''
	 ...
	 SET @l_string = @l_string   + ' AND   isnull(entp.entp_value,''%'')            LIKE CASE WHEN LTRIM(RTRIM('''+@pa_entp_value+''')) = '''' THEN ''%''     ELSE ''' +@pa_entp_value + + '%'' END    '
      ...
      SET @l_string = @l_string   + ' AND    enttm.enttm_cd                         = ISNULL(clim.clim_enttm_cd,''CLI'')  '   
      IF isnull(@l_ent_id,'0') <> '0'
      SET @l_string = @l_string + 'and entr.ENTR_CRN_NO = clim.clim_crn_no  and (entr_ho =' + convert(varchar,@l_ent_id )  + ' or entr_ar = ' + convert(varchar,@l_ent_id )  +  ' or entr_br = ' + convert(varchar,@l_ent_id )  + ' or entr_re = ' + convert(varchar,@l_ent_id )  + ' or entr_sb = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy1 = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy2 = ' + convert(varchar,@l_ent_id )  + ' or entr_dummy3 = ' + convert(varchar,@l_ent_id )  + ')'     
       if isnull(@pa_crn_no,'') <> ''
	  SET @l_string = @l_string   + ' AND   clim.clim_name1 + '' '' + isnull(clim.clim_name2,'''')+ '' '' + isnull(clim.clim_name3,'''')                        LIKE CASE WHEN LTRIM(RTRIM('''+@pa_crn_no+'''))     = '''' THEN ''%''      ELSE '''+ @pa_crn_no        +'%''  END          '
      if isnull(@pa_short_name,'') <> ''
						SET @l_string = @l_string   + ' AND   clim.clim_short_name                   LIKE CASE WHEN LTRIM(RTRIM('''+@pa_short_name+''')) = '''' THEN ''%''      ELSE '''+ @pa_short_name  +'%'' END         '
