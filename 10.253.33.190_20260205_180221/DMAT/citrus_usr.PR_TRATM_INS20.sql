-- Object: PROCEDURE citrus_usr.PR_TRATM_INS20
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

/*  EXECUTE PR_TRATM_INS20
     ...
     select * from transaction_sub_type_mstr
      ...
      sp_help transaction_type_mstr8 */
                   ...
                   CREATE  PROCEDURE [citrus_usr].[PR_TRATM_INS20]
                   AS
                   BEGIN
                     --
                       DECLARE
                       ...
                       ,@l_sub_cd  VARCHAR(50) 
                        SET  @I =1
                        SET @l_name ='HO'
                        SET @l_code='INT_TRANS_TYPE_NSDL'
                        SET  @l_desc ='INTERNAL TRANS TYPES'
                        SELECT @l_excm_id =excm_id FROM exchange_mstr where excm_cd='NSDL'
                        DELETE FROM TRANSACTION_SUB_TYPE_MSTR WHERE TRASTM_TRATM_ID=(SELECT TRANTM_ID FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE ='INT_TRANS_TYPE_NSDL')
		        DELETE FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'INT_TRANS_TYPE_NSDL'
		    ...
		    BEGIN
		      --
			SET @l_sub_cd    = '033'
			SET @l_sub_desc  = 'INTER DELIVERY FOR OFF MARKET /MARKET TRADE'
		      --
		    END
                    ELSE IF  @I = 10
		    BEGIN
