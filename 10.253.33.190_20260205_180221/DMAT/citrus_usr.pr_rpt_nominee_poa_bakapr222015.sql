-- Object: PROCEDURE citrus_usr.pr_rpt_nominee_poa_bakapr222015
...
create proc [citrus_usr].[pr_rpt_nominee_poa_bakapr222015]
   (
    @pa_excsmid int,
...
print @@dtoption
	...
	--
		...
		If @pa_minor_flg='Y'
			...
			,isnull(dphd_nomgau_fname,'') + ' ' + isnull(dphd_nomgau_mname,'') + ' ' + isnull(dphd_nomgau_lname,'') NOMGUARDNAME
			,Nom_Guard_Addr=replace(citrus_usr.fn_acct_addr_value(dpacct.dpam_id,'NOM_GUARDIAN_ADDR'),'|*~|',' ')
			,isnull(DATEADD(DAY,1,(DATEADD(YEAR,18,convert(datetime,DPHD_NOM_DOB,103)))),'') major_dt
,'' Nom_Flag
		from #ACLIST account, dp_holder_dtls,dp_acct_mstr dpacct  left outer join dps8_pc6 on boid = dpam_sba_no and TypeOfTrans ='1'
            ,account_properties 
