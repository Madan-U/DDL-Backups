-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_before29062012
...
create proc [citrus_usr].[pr_get_holding_fix_latest_before29062012](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
,@pa_ben_acct_no_fr varchar(16),@pa_ben_acct_no_to varchar(16),@pa_out  varchar(8000) out)      
as      
begin       
  
if exists(select name from sysobjects where name ='holdingall31mar2012')  
DROP TABLE holdingall31mar2012  
...
into #holdingdmtpenfing1 from  dp_daily_hldg_cdsl, DP_aCCT_MSTR 
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt='jul 31 2011'
and dpam_sba_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
--and dphmcd_dpm_id = @PA_DPM_ID
...
select cdshm_dpam_id , cdshm_isin , sum(case when cdshm_tratm_cd = '3108' then cdshm_qty* - 1 else cdshm_qty end ) qty ,cdshm_trans_no into #trasdmtpenfing1  from cdsl_holding_dtls
where CDSHM_TRATM_CD in ('2201','3108')
and CDSHM_BEN_ACCT_NO between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
--and CDSHM_DPM_ID = @PA_DPM_ID
...
select a.*,A.QTY-isnull(b.qty,0) DMATPENDING 
...
select *,@pa_to_dt dphmcd_holding_dt into holdingall31mar2012 from #tmp_dp_daily_hldg_cdsl     
where     
(dphmcd_curr_qty <> '0'                        
