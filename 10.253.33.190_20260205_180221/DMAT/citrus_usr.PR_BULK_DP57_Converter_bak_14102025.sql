-- Object: PROCEDURE citrus_usr.PR_BULK_DP57_Converter_bak_14102025
...
create   proc [citrus_usr].[PR_BULK_DP57_Converter_bak_14102025]
(  
...
,case when TxnTyp ='DMT' then isnull(DmatRejCd.CDSL_Old_Values,'') when TxnTyp ='DES' then isnull(DstatRejCd.CDSL_Old_Values,'')  else isnull(TxnRsnCd.cdsl_old_value,'') end TxnRsnCd,isnull(AccptdQty,''),isnull(ActlQty,'')
...
union all
select replace(value,'~'+citrus_usr.fn_splitval_by(value ,2,'~') +'~','~'+CONVERT(varchar,@l_old_dp_cnt_204)+'~') from tmp_dp57_o  
where left(value ,1)='T'
) a
