-- Object: PROCEDURE citrus_usr.Pr_rpt_bill_profile_filter_qaunt
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

/*
Pr_rpt_bill_profile_filter 'CDSL',3,'Oct 26 2008','Oct 26 2008 23:59:59','N','N','','','','N','Y','',1,'HO|*~|',''

Pr_rpt_bill_profile_filter 	'CDSL',3,'Jan 26 2009','Jun 26 2009','N','N','1234567890123456','','','N','Y','',1,'HO|*~|',''	

Pr_rpt_bill_profile_filter 	'NSDL',4,'Sep 11 2008','Sep 11 2008 23:59:59','N','N','10000037','','','N','Y','',1,'HO|*~|',''	

*/
CREATE Proc [citrus_usr].[Pr_rpt_bill_profile_filter_qaunt]              
@pa_dptype varchar(4),                      
@pa_excsmid int,                      
...
prev_bill_bal numeric(18,2)
 ...
 if @pa_bulk_printflag = 'N'
 begin
		delete A from #ACLIST A,blk_client_email_dtls 
		where dpam_id = blcked_dpam_id and blkced_dpmid = @@dpmid
		and blkced_rptname = 'BILL'
 end
 ...
 BEGIN                
   ...
   begin            
    ...
    1,  
    CDSHM_ID,                    
    cdshm_charge   ,   CDSHM_SETT_NO ,  right(isnull(CDSHM_TRG_SETTM_NO,'') ,7)               
 ...
 BEGIN                
   ...
   line_no bigint  , c_d_flg char(1)            
   )              
                  
      /*logic for getting as on date balance qty on dec 03 2010*/            
	 create table #tempmainnsdl_old                            
	 (                            
	 trans_date datetime,                            
	 dpam_id bigint,                            
	 dpam_acctno varchar(16),                          
 ...
 begin                 
   ...
   ,0          
   ,nsdhm_charge,NSDHM_LN_NO , case when NSDHM_QTY >=  0 then 'C' else 'D' end 
   ...
   --and ltrim(rtrim(NSDHM_TRASTM_CD)) <> ''        
    ...
    NSDHM_BOOK_NAAR_CD,NSDHM_BOOK_TYPE,NSDHM_BEN_CTGRY,NSDHM_BEN_ACCT_TYPE,                        
    TRANS_DESC=isnull(NSDHM_TRN_DESCP,'')                    
    ,0,NSDHM_LN_NO  ,  NSDHM_COUNTER_CMBP_ID,NSDHM_CC_ID  , NSDHM_TRANSACTION_DT                        
    from NSDL_HOLDING_DTLS with(nolock),              
    account_group_mapping g with(nolock),                           
    #ACLIST account                                  
    where                         
    nsdhm_dpm_id = @@dpmid                                    
    and NSDHM_TRANSACTION_DT < @pa_fromdate 
    and isnumeric(NSDHM_BEN_ACCT_NO) = 1                                      
    and convert(numeric,NSDHM_BEN_ACCT_NO) between convert(numeric,@pa_fromaccid) and convert(numeric,@pa_toaccid)                                     
    and g.dpam_id = NSDHM_dpam_id           
    and group_cd =  @pa_group_cd                
 ...
 begin  
  ...
  --for non pool accounts 
		  ...
		  delete from #tempmainnsdl where Ben_acct_type not in('20','30','40') and book_narr_cd in ('031','032','041','043','053','054','055','056','057','061','076','073','074','075','095','098','099','121','123','124','153','156','163','173','183','193','201','203','213','101')               
     	  delete from #tempmainnsdl_old where Ben_acct_type not in('20','30','40') and book_narr_cd in ('031','032','041','043','053','054','055','056','057','061','076','073','074','075','095','098','099','121','123','124','153','156','163','173','183','193','201','203','213','101')                   
		  ...
		  update m set final_trans = 1,sett_type='',sett_no=''                  
		  from #tempmainnsdl_old m                  
		  where Ben_acct_type not in('20','30','40') and convert(int,m.book_narr_cd) = isnull((select max(convert(int,book_narr_cd)) from #tempmainnsdl_old m1 where m.dpm_trans_no = m1.dpm_trans_no and m.isin_cd = m1.isin_cd and m.dpam_id = m1.dpam_id and m.Ben_acct_type = m1.Ben_acct_type),0)               
			...
			update #tempmainnsdl set final_trans = 1,sett_type='',sett_no='' where Ben_acct_type in('12','13') 
				update #tempmainnsdl_old set final_trans = 1,sett_type='',sett_no='' where Ben_acct_type in('12','13') 
			--added for demat & remat pending transactions
	    ...
	    update #tempmainnsdl_old set final_trans = 1,sett_type='',sett_no='' where Ben_acct_type in('14') and book_narr_cd='088'
	    --For bonus account
        ...
        update #tempmainnsdl_old set final_trans = 1,sett_type='',sett_no='' where Ben_acct_type in('14','11') and book_narr_cd='091'
        --For pledge credit account

	     -- for pool accounts
		     delete from #tempmainnsdl where Ben_acct_type in('20','30','40') and book_narr_cd in ('031','032','041','043','053','054','055','056','057','061','076','073','074','075','095','098','099','121','123','124','153','156','163','173','183','193','201','203','213','211')
		
		     update #tempmainnsdl set final_trans = 1 where  Ben_acct_type in('20','30','40') and  sett_type <> '00'
		     
		     delete from #tempmainnsdl_old where Ben_acct_type in('20','30','40') and book_narr_cd in ('031','032','041','043','053','054','055','056','057','061','076','073','074','075','095','098','099','121','123','124','153','156','163','173','183','193','201','203','211','213')    
     
             update #tempmainnsdl_old set final_trans = 1 where  Ben_acct_type in('20','30','40') and  sett_type <> '00'    
   ...
   begin              
	   ...
	   begin                 
		 insert into #tempmainnsdl              
		 select @pa_prevfromdate,dpdhmd_dpam_id,dpam_sba_no,dpam_sba_name,dpdhmd_isin,dpdhmd_sett_type,dpdhmd_sett_no,null,sum(dpdhmd_qty),null,null,dpdhmd_benf_cat,dpdhmd_benf_acct_typ,'*',3,0,null,null              
	   ...
	   begin   
   		...
   		insert into #tempmainnsdl              
		 select @pa_prevfromdate,dpdhmd_dpam_id,h.dpam_sba_no,h.dpam_sba_name,dpdhmd_isin,dpdhmd_sett_type,dpdhmd_sett_no,null,sum(dpdhmd_qty),null,null,dpdhmd_benf_cat,dpdhmd_benf_acct_typ,'*',3,0,null,null
  ...
  begin   
    insert into #tempmainnsdl          
    select clic_trans_dt=@pa_todate,clic_dpam_id,dpam_sba_no,dpam_sba_name,null,null,null,null,null,null,null,null,null,clic_charge_name,2,Sum(clic_charge_amt),null ,null          
	...
	group by clic_dpam_id,dpam_sba_no,dpam_sba_name,clic_charge_name   
     ...
     -- for pool accounts
	     delete from #tempmainnsdl where Ben_acct_type in('20','30','40') and isnull(sett_type,'') = '00'
	      delete from #tempmainnsdl_old where Ben_acct_type in('20','30','40') and sett_type = '00'   
     -- for pool accounts
     --for non pool accounts 
       update #tempmainnsdl set sett_type='',sett_no='' where final_trans = 3 and Ben_acct_type not in('20','30','40')
       update #tempmainnsdl_old set sett_type='',sett_no='' where final_trans = 2 and Ben_acct_type not in('20','30','40')
...
m.line_no ,closing_qty=0.000   
   ...
   order by dpam_id,isin_cd,Ben_ctgry,Ben_acct_type,sett_type,sett_no,trans_date,c_d_flg,line_no,dpm_trans_no,Book_Narr_cd
...
/*new condition*/            
 update t                             
  set closing_qty = closing_qty + isnull((select sum(isnull(QTY,0)) 
  from #tempmainnsdl_old t1 with(nolock) where t1.DPAM_ID =t.dpam_id and t1.Ben_ctgry = t.Ben_ctgry
  and t1.Ben_acct_type  = t.Ben_acct_type and t1.isin_cd = t.isin_cd  
  --and t1.NSDHM_SETT_TYPE=t.sett_type 
  --and t1.NSDHM_SETT_NO = t.sett_no
  ),0)
  from #temprunning t with(nolock)             
/*new condition*/
   ...
   TRANS_DESC=CASE WHEN ISNULL(CITRUS_USR.TOGETACADESC(dpm_trans_no,t.isin_cd),'')<>'' THEN ISNULL(CITRUS_USR.TOGETACADESC(dpm_trans_no,t.isin_cd),'') ELSE UPPER(ltrim(rtrim(isnull(TRANS_DESC,'')))) END   ,              
