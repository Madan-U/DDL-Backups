-- Object: PROCEDURE citrus_usr.pr_getrmtcnfpnd_All
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--exec citrus_usr.pr_getrmtcnfpnd 3,'mar 31 2012','mar 31 2012','0','9999999999999999','',''    
create proc [citrus_usr].[pr_getrmtcnfpnd_All](@pa_from_dt datetime, @pa_to_dt datetime                        
...
--and CDSHM_DPM_ID = @PA_DPM_ID
And cdshm_tras_dt between 'aug 01 2011' and @PA_TO_DT
and (cdshm_trans_cdas_code like '%D~7~%' or cdshm_trans_cdas_code like '%D~33~%')




select CDSHM_TRATM_CD,cdshm_dpam_id , cdshm_isin , cdshm_trans_no,sum(case when  CITRUS_USR.FN_SPLITVAL_BY(SUBSTRING(cdshm_trans_cdas_code,CHARINDEX('D~',cdshm_trans_cdas_code),LEN(cdshm_trans_cdas_code)),7,'~') in( '709','703') then cdshm_qty*-1
															else cdshm_qty end ) cdshm_qty
into #trasdmtcnfrej from #tras A 
where cdshm_tratm_cd in ('2255','2277') and CITRUS_USR.FN_SPLITVAL_BY(SUBSTRING(cdshm_trans_cdas_code,CHARINDEX('D~',cdshm_trans_cdas_code),LEN(cdshm_trans_cdas_code)),7,'~') IN ('707','709','703','3304','705')
and CDSHM_BEN_ACCT_NO between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
--and CDSHM_DPM_ID = @PA_DPM_ID
and (CITRUS_USR.FN_SPLITVAL_BY(SUBSTRING(cdshm_trans_cdas_code,CHARINDEX('D~',cdshm_trans_cdas_code),LEN(cdshm_trans_cdas_code)),2,'~') IN ('33','7'))
and cdshm_tras_dt between 'aug 01 2011' and @PA_TO_DT
