-- Object: PROCEDURE citrus_usr.pr_select_dp_paging_bak_24jun2010
...
create PROCEDURE [citrus_usr].[pr_select_dp_paging_bak_24jun2010] (@pa_startRowIndex int,                                                      
                                    @pa_maximumRows int  ,                                                
...
union
  select 0,0,'NSE POA','NSE POA',''  where @pa_desc like 'NSE'     
 union
  select 0,0,'BSE POA','BSE POA',''  where @pa_desc like 'BSE'          
 union
  select 0,0,'BOTH','BOTH',' 'where @pa_desc like 'BOTH'             
...
IF @pa_action ='ACCOUNT_BGSE'                        
 ...
 --                        
    ...
    set @l_brcd = citrus_usr.fn_splitval(@l_inputstring,5)                         
    SET @l_tblacctbal_id = citrus_usr.fn_splitval(@l_inputstring,6)                       
    SET @l_entityid = '1'--citrus_usr.fn_splitval(@l_inputstring,7)                       
...
PRINT @@SSQLREC                      
    ...
    --                        
      ...
      DECLARE @TBLGLACCTLIST1  TABLE (TID int identity primary key,dpam_id numeric,dpam_crn_no numeric,dpam_sba_no varchar(16),dpam_sba_name varchar(100),acct_type varchar(5))                    
                     
      INSERT INTO @TBLGLACCTLIST1                  
      SELECT dpam_id,dpam_crn_no,dpam_sba_no,dpam_sba_name,acct_type                   
      FROM  CITRUS_USR.FN_gl_ACCT_LIST(@l_dpmid,@l_entityid,0)                  
      WHERE ACCT_TYPE IN (@@l_acct_chk,@@l_gl_chk)                  
      AND   dpam_sba_name LIKE @pa_cd +'%'                  
     ...
     BEGIN                
     --                
       SELECT @l_count = COUNT(TID) FROM @TBLGLACCTLIST1                
     --      
     END                
