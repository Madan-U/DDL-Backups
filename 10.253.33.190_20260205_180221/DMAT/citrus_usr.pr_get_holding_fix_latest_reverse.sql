-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_reverse
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_reverse](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                          
...
begin   
select @l_min_trx_dt = @pa_from_dt --min(curr_fr_dt) from archival_details   
select @l_base_hldg_dt = max(DPHMC_HOLDING_DT) from dp_hldg_mstr_cdsl --vw_holding_base_all  
end   
else   
begin   
...
select distinct DPHMC_DPM_ID dphmcd_dpm_id , dphmc_dpm_id dphmcd_dpm_id , dphmc_isin dphmcd_isin       
,convert(numeric(18,5),0) DPHMCD_CURR_QTY      
,convert(numeric(18,5),0) DPHMCD_FREE_QTY      
,convert(numeric(18,5),0) DPHMCD_FREEZE_QTY      
,convert(numeric(18,5),0) DPHMCD_PLEDGE_QTY      
...
,convert(numeric(18,5),0) DPHMCD_LEND_QTY      
,convert(numeric(18,5),0) DPHMCD_BORROW_QTY      
from dp_hldg_mstr_Cdsl  a with (nolock) ,dp_acct_mstr  with (nolock) where dphmc_holding_dt =@l_base_hldg_dt      
...
and not exists(select b.dphmcd_dpam_id,b.dphmcd_isin,b.dphmcd_dpm_id from #tmp_dp_daily_hldg_cdsl b with (nolock) where a.dphmc_dpm_id = b.dphmcd_dpm_id      
and a.dphmc_dpam_id = b.dphmcd_dpam_id and a.dphmc_isin = b.dphmcd_isin)      
