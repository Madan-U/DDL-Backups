-- Object: PROCEDURE citrus_usr.pr_import_setm_17052010
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--BEGIN transaction
--pr_import_setm 'NSDL','HO','BULK','C:\BulkInsDbfolder\SETTLEMENT MASTER\cc','*|~*','|*~|',''
--SELECT *  FROM t123456789 where tmpsetm_nsdl_cc_setm_num in(2010301,1011999)
--select * from tmpsetm_mstr
--select count(*) from settlement_mstr
--ROLLBACK transaction

CREATE PROCEDURE  [citrus_usr].[pr_import_setm_17052010]( @pa_exch          VARCHAR(20)  
...
--
  ...
  --
				...
				--
						...
						SET @@ssql ='BULK INSERT CITRUS_USR.tmpsetm_mstr	 from ''' + @pa_db_source + ''' WITH 
						(
									FIELDTERMINATOR = ''~'',
									ROWTERMINATOR = ''\n''
						)'
				...
				and case when len(tmpsetm_setm_id) > 7 then ltrim(rtrim(substring(tmpsetm_setm_id,1,len(tmpsetm_setm_id)-7))) else convert(varchar,tmpsetm_setm_type) end = ltrim(rtrim(settm_type_cdsl)) 
				...
				SELECT IDENTITY(INT, 1, 1) ID1, tmpsetm_setm_type setm_type , RIGHT(tmpsetm_setm_id,7) setm_no, citrus_usr.fn_get_mapping('excm_id',@pa_exch,tmpsetm_exch_id) exch_id INTO #tmp_identity_setm FROM tmpsetm_mstr 
				...
				AND   case when len(tmpsetm_setm_id) > 7 then ltrim(rtrim(substring(tmpsetm_setm_id,1,len(tmpsetm_setm_id)-7))) else convert(varchar,tmpsetm.tmpsetm_setm_type) end = ltrim(rtrim(settm_type_cdsl)) 
				AND   tmpsetmid.setm_no     = right(tmpsetm_setm_id,7)  
                AND    convert(datetime,tmpsetm_setm_dt) >= 'APR  1 2008'
                and   isnull(citrus_usr.fn_get_mapping('CC_id','cdsl',tmpsetm_cc_id),0)  in (select ccm_id from cc_mstr)
                and   isnull(citrus_usr.fn_get_mapping('CC_id','cdsl',tmpsetm_cc_id),0) <> 0 
                AND   ISNULL(citrus_usr.fn_get_mapping('excm_id',@pa_exch,tmpsetm_exch_id),0) <> 0                
                and  not exists(select setm_id from settlement_mstr where setm_excm_id = citrus_usr.fn_get_mapping('excm_id',@pa_exch,tmpsetm_exch_id)  
                and setm_no = right(TMPSETM_SETM_ID,7) and setm_settm_id = settm.settm_id)
		...
		--
		  IF @pa_mode = 'BULK'
				...
				SET   --setm_ccm_id                   =  isnull(citrus_usr.fn_get_mapping('CC_id','cdsl',tmpsetm_nsdl_cc_id),0),
					...
					, setm_lst_upd_dt                = getdate()
					, setm_lst_upd_by                = @pa_login_name
                    , setm_deadline_dt               = tmpsetm_nsdl_cc_nsdl_deadline_dt
				    , setm_deadline_time             = tmpsetm_nsdl_cc_nsdl_deadline_time
				...
				SELECT distinct 
                 ...
                 , settm_id																																																																	
				, case when left(tmpsetm_nsdl_cc_setm_num,1) in ('0','1') then 4  when left(tmpsetm_nsdl_cc_setm_num,1) = '2' then 3 else 0 end  
				...
				and   settm.settm_excm_id = case when left(tmpsetm_nsdl_cc_setm_num,1) in  ('0','1') then 4  when left(tmpsetm_nsdl_cc_setm_num,1) = '2' then 3 else 0 end  
                ...
                and   tmpsetm_nsdl_cc_id in (select ccm_cd from cc_mstr)
                and  isnull(citrus_usr.fn_get_mapping('CC_id','cdsl',tmpsetm_nsdl_cc_id),0) <> 0
                and  not exists(select setm_id from settlement_mstr where settm_excm_id = case when left(tmpsetm_nsdl_cc_setm_num,1) in ('0','1') then 4  when left(tmpsetm_nsdl_cc_setm_num,1) = '2' then 3 else 0 end  
                and setm_no = tmpsetm_nsdl_cc_setm_num and setm_settm_id  = settm_id) 
