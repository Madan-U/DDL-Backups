-- Object: PROCEDURE citrus_usr.pr_ins_upd_Nommulti_mod
...
CREATE  PROCEDURE [citrus_usr].[pr_ins_upd_Nommulti_mod]( @pa_id           VARCHAR(8000)  
...
--  
  ...
  DECLARE @@t_errorstr       VARCHAR(8000)  
		...
		declare @PA_REASON_CD char(2),@PA_REASON_DESC varchar(200), @PA_NOMINEE_TYPE varchar(50), @PA_CRN_NO NUMERIC(10)
  --  
  set @@PA_DPAM_ID = 0
  if @PA_Nom_DPAM_SBA_NO <> ''
  begin
	SELECT @@PA_DPAM_ID = DPAM_ID  FROM DP_ACCT_MSTR_mak WHERE DPAM_SBA_NO = @PA_Nom_DPAM_SBA_NO --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
  end
  else
  begin
  ...
  --SELECT @@PA_DPAM_ID = DPAM_ID  FROM DP_ACCT_MSTR_mak WHERE DPAM_CRN_NO = @PA_Nom_dpam_id --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
	...
	begin 
		...
		begin
		print 'pp'
			SELECT @@PA_DPAM_ID = DPAM_ID FROM DP_ACCT_MSTR WHERE DPAM_SBA_NO = @PA_Nom_DPAM_SBA_NO --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
		end
		else
		begin
			SELECT @@PA_DPAM_ID = DPAM_ID FROM DP_ACCT_MSTR WHERE DPAM_SBA_NO = @PA_Nom_dpam_id --AND DPAM_DELETED_IND = '1' --AND DPAM_STAM_CD = 'ACTIVE'
		end
	end
  SET @@l_error         = 0  
  SET @@t_errorstr      = ''  
     ...
     BEGIN  
       ...
       BEGIN  
		...
		,@pa_nom_res_sec_flag
		 ...
		 if exists(select 1 from Nominee_Multi where nom_srno = @PA_nom_srno and  Nom_dpam_id = @@PA_DPAM_ID and Nom_DELETED_IND = 1)
		 begin

		 print 'PANKAJ111'
			----select * from Nominee_Multi mst, Nominee_Multi_mak mak where 
			----mst.Nom_dpam_id = mst.Nom_dpam_id and mst.nom_srno = @PA_nom_srno and 
			----mst.Nom_dpam_id = @@PA_DPAM_ID and mst.Nom_DELETED_IND = 1 and mst.nom_fname <> mak.nom_fname
			--declare @PA_REASON_CD char(2),@PA_REASON_DESC varchar(200), @PA_NOMINEE_TYPE varchar(50), @PA_CRN_NO NUMERIC(10)

			set @PA_REASON_CD = citrus_usr.FN_SPLITVAL_BY(@pa_conc_values,1,'~')
			set @PA_REASON_DESC = citrus_usr.FN_SPLITVAL_BY(@pa_conc_values,2,'~')
			IF @PA_REASON_CD = '--SELECT--'
			BEGIN 
				SET @PA_REASON_CD = ''
			END
			...
			SET @PA_NOMINEE_TYPE = CASE WHEN @PA_nom_srno = '1' THEN 'NOMINEE1'
										WHEN @PA_nom_srno = '2' THEN 'NOMINEE2'
										WHEN @PA_nom_srno = '3' THEN 'NOMINEE3'
										WHEN @PA_nom_srno = '4' THEN 'NOMINEE1 GAURDIAN'
										WHEN @PA_nom_srno = '5' THEN 'NOMINEE2 GAURDIAN'
										WHEN @PA_nom_srno = '6' THEN 'NOMINEE3 GAURDIAN' 
										
										WHEN @PA_nom_srno = '01' THEN 'NOMINEE1'
										WHEN @PA_nom_srno = '02' THEN 'NOMINEE2'
										WHEN @PA_nom_srno = '03' THEN 'NOMINEE3'
										WHEN @PA_nom_srno = '04' THEN 'NOMINEE1 GAURDIAN'
										WHEN @PA_nom_srno = '05' THEN 'NOMINEE2 GAURDIAN'
										WHEN @PA_nom_srno = '06' THEN 'NOMINEE3 GAURDIAN' 
										END
		SELECT @PA_Nom_DPAM_SBA_NO = DPAM_SBA_NO,@PA_CRN_NO = DPAM_CRN_NO  FROM DP_ACCT_MSTR WHERE DPAM_ID = @@PA_DPAM_ID
		
		
		
			exec PR_INS_UPD_MULTI_NOMINEE_MOD_BULKMSTR 0,@@PA_DPAM_ID,@PA_CRN_NO,0,@PA_REASON_CD,@PA_REASON_DESC,'INS',@PA_NOMINEE_TYPE ,@pa_login_name,@PA_Nom_DPAM_SBA_NO,''
       ...
       --  
		 ...
		 if exists (select 1 from Nominee_Multi_mak where Nom_DPAM_SBA_NO = @PA_Nom_DPAM_SBA_NO and Nom_DELETED_IND in (0,4,8) AND nom_srno = @PA_nom_srno) 
		 begin 
			update Nominee_Multi_mak  set Nom_DELETED_IND = 2 where  Nom_DPAM_SBA_NO = @PA_Nom_DPAM_SBA_NO and Nom_DELETED_IND in (0,4,8) AND nom_srno = @PA_nom_srno
		 end 
		...
		,@pa_nom_res_sec_flag
		 ...
		 if NOT exists(select 1 from Nominee_Multi where nom_srno = @PA_nom_srno and  Nom_dpam_id = @@PA_DPAM_ID and Nom_DELETED_IND = 1)
		 begin

		 print 'PANKAJ111'
			select * from Nominee_Multi mst, Nominee_Multi_mak mak where mst.Nom_dpam_id = mst.Nom_dpam_id and mst.nom_srno = @PA_nom_srno and mst.Nom_dpam_id = @@PA_DPAM_ID and mst.Nom_DELETED_IND = 1 and mst.nom_fname <> mak.nom_fname
			--declare @PA_REASON_CD char(2),@PA_REASON_DESC varchar(200), @PA_NOMINEE_TYPE varchar(50), @PA_CRN_NO NUMERIC(10)
			...
			SET @PA_NOMINEE_TYPE = CASE WHEN @PA_nom_srno = '1' THEN 'NOMINEE1'
										...
										WHEN @PA_nom_srno = '4' THEN 'NOMINEE1 GAURDIAN'
										WHEN @PA_nom_srno = '5' THEN 'NOMINEE2 GAURDIAN'
										WHEN @PA_nom_srno = '6' THEN 'NOMINEE3 GAURDIAN' END
		SELECT @PA_Nom_DPAM_SBA_NO = DPAM_SBA_NO,@PA_CRN_NO = DPAM_CRN_NO  FROM DP_ACCT_MSTR WHERE DPAM_ID = @@PA_DPAM_ID
			exec PR_INS_UPD_MULTI_NOMINEE_MOD 0,@@PA_DPAM_ID,@PA_CRN_NO,0,@PA_REASON_CD,@PA_REASON_DESC,'INS',@PA_NOMINEE_TYPE ,@pa_login_name,@PA_Nom_DPAM_SBA_NO,''
