-- Object: PROCEDURE citrus_usr.pr_import_dp_cdsl_Mod_bak30082023
...
--drop procedure pr_import_dp_cdsl_Mod
create  PROCeDURE [citrus_usr].[pr_import_dp_cdsl_Mod_bak30082023]( @pa_crn_no      varchar(8000)                 
                            , @pa_exch        VARCHAR(10)                  
...
,max(case when clic_mod_action ='C ADDRESS' then isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'cor_ADR1'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_addr_value(clim.clim_crn_no,'cor_ADR1'),''),4)),'' )else '00' end)-- city seq no
...
--                         and    climmod.clic_mod_deleted_ind = 1
               ...
               insert into @line5( ln_no                   
                                ...
                                --changed by tushar on jan 09 2015
									   ...
									   , max(case when clic_mod_action = 'BSDA FLAG' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BSDA',''),'') ='0' THEN 'N' ELSE 'Y' END) else '' end)--BSDA FLAG
...
--                                              ,case when ISNULL(citrus_usr.fn_ucc_doc(dpam_id,'SIGN_POA',''),'') = '' then 'N' else 'Y'   end  --signature file flag           
                                        ...
                                        FROM   dp_acct_mstr               dpam          with (nolock)
										 ...
										 and    climmod.clic_mod_action like case when upper(@pa_mod_typ) = 'ALL' then '%%' else @pa_mod_typ end
										 and	isnull(climmod.clic_mod_batch_no,0) = 0
										  and    dppd_gpabpa_flg<>'A'
										 and not exists(select 1 from dps8_pc5 where MasterPOAId = dppd_master_id and boid = DPAM_SBA_NO and HolderNum = case when ltrim(rtrim(DPPD_HLD)) = '1ST HOLDER' then '1'
										 when ltrim(rtrim(DPPD_HLD)) = '2ND HOLDER' then '2' else '3' end and TypeOfTrans in ('','1') AND	  climmod.clic_mod_action in ('POA ACTIVATION')
										 and GPABPAFlg=dppd_gpabpa_flg
										 )
                                         --order by  dppd_eff_fr_dt   
...
--update bitmap_ref_mstr set bitrm_values=bitrm_values + convert(varchar,@l_06cnt ) where BITRM_PARENT_CD = 'POAID_AUTO'  
                                 ...
                                 insert into  @line7(ln_no                                    
									...
									,ln7_Pri_mob_ISD                          
											...
											,isnull(dob,'')                               
                                            ,isnull(cust_fax ,'')                                
                                            ,isnull(hldr_in_tax_pan ,'')
											,isnull(uid,'')
											,isnull(name_ch_reason_cd  ,'')                        
                                            ,isnull(it_crl  ,'')                                 
                                            ,isnull(cust_email ,'')                              
                                            ,isnull(usr_txt1  ,'')                               
                                            ,isnull(usr_txt2 ,'')                                
									...
									,res_sec_flag   ,ln7_cust_adr_cntry_cd,ln7_cust_adr_state_cd,ln7_city_seq_no,ln7_Pri_mob_ISD      
                                            ...
                                            AND   excsm_exch_cd = @pa_exch
                                            and   (dpam_sba_no =  clic_mod_dpam_sba_no)
                                             --or case when clic_mod_pan_no <> '' then citrus_usr.fn_ucc_entp(dpam_crn_no,'pan_gir_no','') else '' end = clic_mod_pan_no)              
                                            AND   climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
                                            and    climmod.clic_mod_deleted_ind = 1
                                            and	isnull(climmod.clic_mod_batch_no,0) = 0
                                            --and    climmod.clic_mod_action = @pa_mod_typ
                                            and    climmod.clic_mod_action like case when upper(@pa_mod_typ) = 'ALL' then '%%' else @pa_mod_typ end
                                            AND   climmod.clic_mod_action in ('G Address','G NameNDtls','G Contacts','Guardian Suffix', 'G NameChange','G FatherNameChange' ,'G MobChange', 'G ResPhChange', 'G OffPhChange', 'G DOBChange', 'G PANChange','GUARDIAN TITLE','NG Address','NG NameNDtls','NG Contacts','P ADDRESS','NOMINEE3 GAURDIAN','NOMINEE3','NOMINEE2 GAURDIAN','NOMINEE2','NOMINEE1 GAURDIAN','NOMINEE1','NOMINEE DEL') 
                                            ORDER BY Purpose_code  DESC
...
--            +CONVERT(char(1),l5.trading_id        )    
     ...
     --      +case when CONVERT(char(3),right(ltrim(rtrim(l7.Purpose_code)),3)) <>'212' then CONVERT(char(10),l7.hldr_in_tax_pan ) ELSE '' END 
		   ...
		   +case when CONVERT(char(3),right(ltrim(rtrim(l7.Purpose_code)),3)) <>'212' then CONVERT(char(10),l7.hldr_in_tax_pan ) when CONVERT(char(3),right(ltrim(rtrim(l7.Purpose_code)),3)) ='212' then CONVERT(char(10),l7.hldr_in_tax_pan ) ELSE case when CONVERT(char(3),right(ltrim(rtrim(l7.Purpose_code)),3)) <>'212' then  SPACE(10) else char(10) end END 
