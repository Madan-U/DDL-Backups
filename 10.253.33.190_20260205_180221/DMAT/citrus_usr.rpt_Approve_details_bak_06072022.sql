-- Object: PROCEDURE citrus_usr.rpt_Approve_details_bak_06072022
...
CREATE  procedure [citrus_usr].[rpt_Approve_details_bak_06072022]
(@pa_id  varchar(10),
 @pa_type varchar(4),
 @pa_tab varchar(20),
 @pa_from_dt datetime,
...
-- otp status update
case 
when exists (select 1 from #TMPOTP  OTP1 where otp1.DPTDC_SLIP_NO=dptd1.DPTDC_SLIP_NO and SUBTYPECDAS IN ('543','330')) then 'REJECTED OTP AUTHENTICATED TRANSACTION'
 WHEN EXISTS (SELECT 1 FROM #TMPOTP  OTP4 WHERE OTP4.DPTDC_SLIP_NO=DPTD1.DPTDC_SLIP_NO AND SUBTYPECDAS='545') THEN 'NON AUTHENTICATED OTP TRANSACTION CANCELLED'
