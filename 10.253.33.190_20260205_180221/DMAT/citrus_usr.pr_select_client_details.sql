-- Object: PROCEDURE citrus_usr.pr_select_client_details
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[pr_select_client_details] (@pa_id          NUMERIC  = 0                      
 ...
 ELSE CASE WHEN R=3 THEN entr.entr_ar                      
    ...
    --                      
      SELECT TOP 1 @I =  bitrm_bit_location FROM bitmap_ref_mstr WHERE bitrm_parent_cd = 'BUS_' + @pa_value  AND bitrm_deleted_ind = 1                    
			...
			begin 
				  SELECT  concm.concm_cd                   concm_cd                      
						, concm.concm_desc                 concm_desc                      
						,'' adr_1--case when concm.concm_cd ='OFF_ADR1' then isnull(citrus_usr.fn_get_br_add_fromview(@pa_crn_no ,'1','adr'),'') else ISNULL(a.adr_1,'') end                adr_1                          
						,'' value--case when concm.concm_cd ='OFF_ADR1' then isnull(citrus_usr.fn_get_br_add_fromview(@pa_crn_no ,'2','adr'),'') else isnull(a.value,'')  end             value                      
				   FROM  (SELECT entac.entac_concm_id      concm_id                      
							   , adr.adr_1                 adr_1                      
							   , adr.adr_city              adr_city                      
							   , adr.adr_state             adr_state                      
    ...
    --                      
      ...
      SELECT distinct concm.concm_cd                   concm_cd                      
       ...
       FROM  (SELECT entac.entac_concm_id      concm_id                      
                   ...
                   , adr.adr_country   adr_country                      
                   , adr.adr_zip               adr_zip                      
                   , entac.entac_concm_cd+'|*~|'+ltrim(rtrim(adr.adr_1))+'|*~|'+ltrim(rtrim(isnull(adr.adr_2,'')))+'|*~|'+ltrim(rtrim(isnull(adr.adr_3,'')))+'|*~|'+ltrim(rtrim(isnull(adr.adr_city,'')))+'|*~|'+ltrim(rtrim(isnull(adr.adr_state,'')))+'|*~|'+ltrim(rtrim(isnull(adr.adr_country,'')))+'|*~|'+ltrim(rtrim(isnull(adr.adr_zip,''))) value       
       ...
       AND    1 & concm.concm_cli_yn         = 1                      
       AND    2 & concm.concm_cli_yn         = 0     
       AND    concm.concm_cd IN ('COR_ADR1','PER_ADR1')                 
       --AND    power(2,@I-1) & concm.concm_cli_yn  > 0   
       --ORDER  BY concm.concm_desc       
       ...
       --SELECT ltrim(rtrim(concm.concm_cd))+'|*~|'+isnull(ltrim(rtrim(a.adr_1)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_2)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_3)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_city)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_state)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_country)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_zip)),'')+'|*~|'+ltrim(rtrim(concm.concm_desc))+'|*~|'+'*|~*' value
        SELECT distinct ltrim(rtrim(concm.concm_cd)) concm_cd 
			   ,ltrim(rtrim(concm.concm_desc)) concm_desc
			   ,isnull(ltrim(rtrim(a.adr_1)),'') adr_1
			--,ltrim(rtrim(concm.concm_cd))+'|*~|'+isnull(ltrim(rtrim(a.adr_1)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_2)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_3)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_city)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_state)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_country)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_zip)),'')+'|*~|'+ltrim(rtrim(concm.concm_desc))+'|*~|'+'*|~*' value
              ,ltrim(rtrim(concm.concm_cd))+'|*~|'+isnull(ltrim(rtrim(a.adr_1)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_2)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_3)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_city)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_state)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_country)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_zip)),'') value
			   --,ISNULL(a.value, ' ')             value	
               --+'|*~|'+isnull(ltrim(rtrim(a.adr_1)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_2)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_3)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_city)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_state)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_country)),'')+'|*~|'+ISNULL(ltrim(rtrim(a.adr_zip)),'')+'|*~|'+ltrim(rtrim(concm.concm_desc))+'|*~|'+'*|~*' value
		...
		AND    1 & concm.concm_cli_yn         = 1  
        AND    2 & concm.concm_cli_yn         = 0 
        --AND    concm.concm_cd IN ('NOMINEE_ADR1','NOM_GUARDIAN_ADDR','GUARD_ADR','AC_COR_ADR1') 
		AND    concm.concm_cd IN ('NOMINEE_ADR1','NOM_GUARDIAN_ADDR','GUARD_ADR','SH_ADR1','TH_ADR1') 
        --and    isnull(a.adr_1,'') <> ''
       ORDER  BY concm.concm_desc               
    --                      
    END--ADDR                     
  ...
  SELECT TOP 1 @I = bitrm_bit_location FROM bitmap_ref_mstr WHERE bitrm_parent_cd = 'BUS_' + @pa_value  AND bitrm_deleted_ind = 1                    
			...
			begin 
				SELECT concm.concm_cd                   concm_cd                      
					   , concm.concm_desc                 concm_desc                      
					   , '' value--case when concm.concm_cd ='email2' then citrus_usr.fn_get_br_add_fromview(@pa_crn_no,'1','conc') 
...
, concm.concm_desc                concm_desc                      
      ...
      FROM   contact_channels_mak            concmak                      
             RIGHT OUTER JOIN                      
             conc_code_mstr                   concm  WITH (NOLOCK)                      
             ON concm.concm_id=CONCMAK.CONC_concm_id AND concmak.conc_ent_id = @pa_crn_no    AND    concmak.conc_deleted_ind  IN (0, 8)                 
      WHERE  concm.concm_deleted_ind      = 1                      
      AND    1 & concm.concm_cli_yn         = 1                      
      AND    2 & concm.concm_cli_yn         = 2                      
      AND    power(2,@I-1) & concm.concm_cli_yn  > 0               
