-- Object: PROCEDURE citrus_usr.pr_import_dp_mod
...
--[pr_import_dp] '','CDSL','02/03/2009','02/03/2009','N','76',3,'HO',''   
--select * from dp_acct_mstr order by 1 desc  
--CDSL	01/05/2008	02/05/2008	N	45	3	HO	
CREATE PROCeDURE [citrus_usr].[pr_import_dp_mod]( @pa_crn_no      VARCHAR(8000)                  
                            , @pa_exch        VARCHAR(10)                  
    ...
    --                  
      ...
      --      
        ...
        SELECT 0                  
             ...
             , isnull(convert(char(45),clim.clim_name1+' ' + ISNULL(clim.clim_name2,'') + ' ' +  ISNULL(clim.clim_name3,'')),'')--convert(char(45),LTRIM(RTRIM(dpam_sba_name))) --isnull(convert(char(45),clim.clim_name1+' ' + ISNULL(clim.clim_name2,'') + ' ' +  ISNULL(clim.clim_name3,'')),'')           --beneficiary first holder name                  
            ...
            ,  CASE WHEN  convert(char(1),ISNULL(citrus_usr.fn_ucc_accp(dpam.dpam_id,'ADR_PREF_FLG',''),'')) <> '1' THEN 'Y'  
             ...
             , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'') <> '' THEN lower(convert(char(50),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),''))) ELSE lower(convert(char(50),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'EMAIL1'),'')))  end                                                --email id for first holder            
       ...
       INSERT INTO @l_dp_pri_dtls(ln_number                  
        ...
        SELECT 0                  
           ...
           ,  CASE WHEN  convert(char(1),ISNULL(citrus_usr.fn_ucc_accp(dpam.dpam_id,'ADR_PREF_FLG',''),'')) <> '1' THEN 'Y'  
             ...
             , case when isnull(convert(vARCHAR(30),BANM_MICR),'') = '0' then case when ISNULL(citrus_usr.fn_ACCT_entp(dpam_id,'BENMICR'),'') <> '' then convert(char(9),ISNULL(citrus_usr.fn_ACCT_entp(dpam_id,'BENMICR'),'')) else convert(char(9),'') end  else convert(char(9),isnull(convert(vARCHAR(30),BANM_MICR),'')) end
             , CASE WHEN convert(char(20),cliba_ac_type) IN( 'SAVINGS', 'SAVING')  THEN '10' WHEN convert(char(20),cliba_ac_type) IN('CURRENT') THEN  '11' WHEN convert(char(20),cliba_ac_type) IN( 'OTHERS') THEN  '13' END   --'11'                                                                                          --bank acct type                  
             , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'') <> '' THEN lower(convert(char(50),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),''))) ELSE lower(convert(char(50),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'EMAIL1'),'')))  end                                                --email id for first holder                                                           --email id for first holder            
             , convert(char(1),ISNULL(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'MAPIN_ID',''),''))                                  --map in id of first holder   --9                  
             , CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'') <> '' then convert(char(12),isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCMOBILE1'),'')) ELSE convert(char(12),ISNULL(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'MOBILE1'),''))            end                                       --mobile no of first holder  --12                  
        ...
        --                  
          ...
          SELECT --'02'                  
              ...
              -- + 'A'                                                                            
               ...
               + case when convert(char(2),ben_type) + convert(char(2),ben_sub_type)  not in ('0402','0401','0101','0703','0702','0701','0700','0405','0404','0105') then ' ' else case when nom_gur_ind = 'N'  then 'N' else 'Y' end end  
               + SPACE(13)                                         
      ...
      BEGIN                  
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
                                ...
                                , case when dpam_clicm_cd in ('21' , '24' , '27') then clim.clim_name1
                                  else clim.clim_name1  end
                                , case when dpam_clicm_cd in ('21' , '24' , '27') then ISNULL(clim.clim_name2,'')
                                  else '' end
                                , case when dpam_clicm_cd in ('21' , '24' , '27') then  ISNULL(clim.clim_name3,'')
                                  else '.' end
