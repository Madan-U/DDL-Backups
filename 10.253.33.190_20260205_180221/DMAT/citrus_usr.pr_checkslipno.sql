-- Object: PROCEDURE citrus_usr.pr_checkslipno
...
CREATE  procedure [citrus_usr].[pr_checkslipno]
(          
  ...
  print @pa_errmsg  
    ...
    ELSE          
     BEGIN        
      ...
      BEGIN        
      --     
         select fre.* into #freeze2 from freeze_unfreeze_dtls fre , dp_acct_mstr where DPAM_ID = fre_Dpam_id 
      and dpam_sba_no = case when len(@dpid)=16 then @dpid else  @dpid+@accountno  end 
  ...
  RETURN        
    END     
      ...
      --    
         ...
         select fre.* into #freeze1 from freeze_unfreeze_dtls fre , dp_acct_mstr where DPAM_ID = fre_Dpam_id 
      and dpam_sba_no = case when len(@dpid)=16 then @dpid else  @dpid+@accountno  end 
...
print @pa_errmsg  
 ...
 --     
      ...
      --create clustered  index ix_11 on  freeze_unfreeze_dtls(fre_action,fre_Isin_code,fre_Dpam_id,fre_deleted_ind)  
       select fre.* into #freeze from freeze_unfreeze_dtls fre , dp_acct_mstr where DPAM_ID = fre_Dpam_id 
     ...
     and dpam_deleted_ind = 1
     and fre_deleted_ind = 1 and fre_action<>'U' and fre_Isin_code=''
      and not exists (select fre_Dpam_id from #freeze O where i.fre_Dpam_id=o.fre_Dpam_id
     and i.fre_Exec_date<o.fre_Exec_date and o.fre_action='U')
     )  
   BEGIN          
  ...
  if exists(
  select 1 from ABVSCITRUS.Crmdb_A.dbo.MOBILE_EMAIL_MODI,dp_acct_mstr with(nolock) where DPAM_BBO_CODE=BO_PARTYCODE 
