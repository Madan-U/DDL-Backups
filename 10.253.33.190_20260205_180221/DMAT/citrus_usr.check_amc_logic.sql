...
select *   from [196.1.115.182].general.dbo.Vw_LedgerBal where client=@l_bbocode

select  cl_code,case  when b2c = 'Y' then 'N' else 'Y' end  B2B   from  [MIDDLEWARE].INHOUSE.DBO.CLIENT_DETAILS where cl_code = @l_bbocode
...
select *   from [196.1.115.182].general.dbo.Vw_LedgerBal where client=@bbo_code

select  cl_code,case  when b2c = 'Y' then 'N' else 'Y' end  B2B   from  [MIDDLEWARE].INHOUSE.DBO.CLIENT_DETAILS 
