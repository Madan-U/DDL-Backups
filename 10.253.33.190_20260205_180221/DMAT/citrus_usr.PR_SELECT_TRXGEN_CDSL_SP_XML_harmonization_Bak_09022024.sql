-- Object: PROCEDURE citrus_usr.PR_SELECT_TRXGEN_CDSL_SP_XML_harmonization_Bak_09022024
...
create  PROCEDURE [citrus_usr].[PR_SELECT_TRXGEN_CDSL_SP_XML_harmonization_Bak_09022024]  
(  
...
else details end as details , qty ,dptdc_slip_no  slip_no --, len(replace(details ,' ','|')) v
...
--changes for NewDemat changes Feb 14 2020
union all
select  convert(varchar(8000),rngs)+convert(varchar,folio)+convert(varchar,certfrom)+convert(varchar,certto)+convert(varchar,DNfrm)+convert(varchar,dnto) ,'0',isnull(drfno,0)  from #Demattempdatarange
)
T
--changes for NewDemat changes Feb 14 2020
