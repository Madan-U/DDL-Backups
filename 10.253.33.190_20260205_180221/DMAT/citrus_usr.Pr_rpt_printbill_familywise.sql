-- Object: PROCEDURE citrus_usr.Pr_rpt_printbill_familywise
...
--[Pr_rpt_printbill_familywise] 'CDSL',3,'JAN 01 2009','JUN 26 2009 23:59:59','N','N','1234567890123456','1234567890123456','','N','Y',1,20,1,'HO|*~|','jqq5fw554brejy551z44pm45','N','N','','HO',''	

CREATE Proc [citrus_usr].[Pr_rpt_printbill_familywise]            
@pa_dptype varchar(4),                    
@pa_excsmid int,                    
 ...
 BEGIN              
	  ...
	  begin     
		 ...
		 begin
			...
			set @@ssql='insert into ' + @@temptable + '(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val,settm_id, tr_settm_id,family,valuation)         
			...
			1,
			CDSHM_ID,                  
			cdshm_charge , isnull(CDSHM_SETT_NO,''''), isnull(CDSHM_TRG_SETTM_NO ,'''') , entm_name1  ,0               
            ...
            begin
			 ...
			 begin        
				   ...
				   3,
				   0,      
				   0   , '''', right(isnull(dphmcd_cntr_settm_id ,''''),7)  , entm_name1  , CONVERT(NUMERIC(18,2),DPHMCD_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0)) VALUATION                  
...
print @@ssql
		  ...
		  select clic_dpam_id,dpam_sba_name,dpam_sba_no,clic_charge_name,null,clic_trans_dt=''' +  convert(varchar,@pa_todate,109) + ''',null,null,null,2,null,sum(clic_charge_amt)      , entm_name1,0
		  ...
		  set @@ssql = @@ssql + ' group by clic_dpam_id,dpam_sba_name,dpam_sba_no,clic_charge_name,entm_name1'
	     ...
	     set @@ssql = 'insert into ' + @@temptable + '(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val,family,valuation) 
	     SELECT dpam_id , dpam_sba_name, dpam_sba_no , '''','''','''','''',0,0,4,0,0,entm_name1 ,0 FROM #ACLIST_CDSL a, entity_mstr , entity_relationship 
		...
		begin
		  ...
		  trans_date, isnull(settm_id ,'''') settm_id , isnull(tr_settm_id,'''')  tr_settm_id, isnull(t.family,'') family,
          isnull(valuation,0) valuation 
		  ...
		  order by t.family ,t.dpam_acctno,dpam_sba_name,order_by,trans_date,Isin_name,18,line_no'
	...
	begin		         
		  ...
		  page_cnt,
		  bill_due_dt = ''' + @@Billduedate + ''',
		  trans_date, isnull(t.family,'') family,
		  isnull(valuation,0) valuation 
		  FROM ' + @@temptable + 'clients tc,' + @@temptable + ' t LEFT OUTER JOIN  isin_mstr i on t.isin_cd = i.isin_cd 
		  where tc.dpam_id = t.dpam_id and tc.id > ' + convert(varchar,((@pa_currrecordctr-1) * @pa_reqdrecordcnt)) + ' and tc.id <=  ' + convert(varchar,(@pa_currrecordctr * @pa_reqdrecordcnt)) + ' 
		  order by t.family ,t.dpam_acctno,dpam_sba_name,order_by,trans_date,Isin_name,18,line_no'
