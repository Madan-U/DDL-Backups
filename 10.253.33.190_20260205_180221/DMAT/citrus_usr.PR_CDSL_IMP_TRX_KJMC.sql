-- Object: PROCEDURE citrus_usr.PR_CDSL_IMP_TRX_KJMC
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--PR_CDSL_IMP_TRX 'HO','50' ,3,'BULK','c:\BulkInsDbfolder\BrokerFile\CDSL\off_mkt.txt','OFFM','',0
--update tmp_dp_trx_dtls set tmpdptd_acct_id ='12345678' , TMPDPTD_INTERNAL_REF_NO = ''
--DELETE  FROM DPTD_MAK

CREATE  PROCEDURE [citrus_usr].[PR_CDSL_IMP_TRX_KJMC] (  
...
create table #tmp_dptdc (ID1 int IDENTITY( 1, 1),DPTDC_SLIP_NO varchar(20))	
create table #tmp_dptdc_intdp (ID1 int IDENTITY(1, 1),DPTDC_SLIP_NO varchar(20))	
create table #tmp_dptdc_np (ID1 int IDENTITY(1, 1),DPTDC_SLIP_NO varchar(20))	
create table #tmp_dptdc_ep (ID1 int IDENTITY(1, 1),DPTDC_SLIP_NO varchar(20))	
...
--
		...
		--
               ...
               UPDATE TMP_DP_OFFM_DTLS_CDSL SET  TMPOFFM_SLIP_NO=ltrim(rtrim(TMPOFFM_SLIP_NO))
                ...
                update bitmap_ref_mstr set bitrm_bit_location =  @L_MAX_DPTDC_ID1+@l_id2+1  where bitrm_parent_cd = 'DPTDC_ID' 
    ...
    if exists(select TMPOFFM_SLIP_NO from TMP_DP_OFFM_DTLS_CDSL, used_slip where rtrim(ltrim(TMPOFFM_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO)))
    begin

     SELECT @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  +  TMPOFFM_SLIP_NO  + ',' from TMP_DP_OFFM_DTLS_CDSL, used_slip where rtrim(ltrim(TMPOFFM_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO)) 
...
--
--
--    if not exists(select TMPOFFM_SLIP_NO,TMPOFFM_BO_ID from TMP_DP_OFFM_DTLS_CDSL, slip_issue_mstr  , dp_acct_mstr where rtrim(ltrim(TMPOFFM_SLIP_NO)) like SLIIM_SERIES_TYPE + '%' and replace(rtrim(ltrim(TMPOFFM_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'') between SLIIM_SLIP_NO_FR and SLIIM_SLIP_NO_TO and SLIIM_ENTM_ID = dpam_id and dpam_sba_no = TMPOFFM_BO_ID)
--    begin
--
--     SELECT @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  +   TMPOFFM_SLIP_NO + ',' from TMP_DP_OFFM_DTLS_CDSL, slip_issue_mstr  , dp_acct_mstr where rtrim(ltrim(TMPOFFM_SLIP_NO)) like SLIIM_SERIES_TYPE + '%' and replace(rtrim(ltrim(TMPOFFM_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'') between SLIIM_SLIP_NO_FR and SLIIM_SLIP_NO_TO and SLIIM_ENTM_ID = dpam_id and dpam_sba_no = TMPOFFM_BO_ID
--
--        UPDATE filetask
...
*/
    ...
    UPDATE  TMP
			 ...
			 AND     A.DPAM_DPM_ID  = @l_dpm_id
				...
				,TMPOFFM_INTERNAL_REF_NO =  convert(varchar,(@L_MAX_DPTDC_ID1  + (TMPOFFM_ID-1)))
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPOFFM_ACCT_ID,convert(datetime,TMPOFFM_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPOFFM_ISIN,abs(TMPOFFM_QTY),'HIGH_VALUE','','') end = 'Y' then -1 else 0 end,  
...
--
  ...
  if exists(select TMPINTDP_SLIP_NO from TMP_DP_INTDP_DTLS_CDSL, used_slip where rtrim(ltrim(TMPINTDP_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO)))
    begin
     ...
     SELECT @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  + TMPINTDP_SLIP_NO + ',' from TMP_DP_INTDP_DTLS_CDSL, used_slip where rtrim(ltrim(TMPINTDP_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO)) 
...
--
--
--    if not exists(select TMPINTDP_SLIP_NO,TMPINTDP_BO_ID from TMP_DP_INTDP_DTLS_CDSL, slip_issue_mstr  , dp_acct_mstr where rtrim(ltrim(TMPINTDP_SLIP_NO)) like SLIIM_SERIES_TYPE + '%' and replace(rtrim(ltrim(TMPINTDP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'') between SLIIM_SLIP_NO_FR and SLIIM_SLIP_NO_TO and SLIIM_ENTM_ID = dpam_id and dpam_sba_no = TMPINTDP_BO_ID)
--    begin
--
--     SELECT @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  +   TMPINTDP_SLIP_NO + ',' from TMP_DP_INTDP_DTLS_CDSL, slip_issue_mstr  , dp_acct_mstr where rtrim(ltrim(TMPINTDP_SLIP_NO)) like SLIIM_SERIES_TYPE + '%' and replace(rtrim(ltrim(TMPINTDP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'') between SLIIM_SLIP_NO_FR and SLIIM_SLIP_NO_TO and SLIIM_ENTM_ID = dpam_id and dpam_sba_no = TMPINTDP_BO_ID
--
--        UPDATE filetask
--		SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Slips are not Issued : ' + @L_ERR_MISSING_ISIN
--		,      status = 'FAILED'
...
select @l_err_mstr = isnull(replace(citrus_usr.fn_merge_str('CDSL_IMP_INTDP',0,''),',',''),'')
    ...
    */
				...
				SET     TMPINTDP_ACCT_ID = DPAM_ID
											,TMPINTDP_INTERNAL_REF_NO =  convert(varchar,(@L_MAX_DPTDC_ID1  + (TMPINTDP_ID-1)))
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPINTDP_ACCT_ID,convert(datetime,TMPINTDP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPINTDP_ISIN,abs(TMPINTDP_QTY),'HIGH_VALUE','','') end = 'Y' then -1 else 0 end,  
				TMPINTDP_TRASTM_CD ,  
				id1+@L_MAX_DPTDC_DTLS_ID1,  
				NULL, 
				NULL,
...
--    end 
  ...
  if exists(select TMPNP_SLIP_NO from TMP_DP_NP_DTLS_CDSL, used_slip where rtrim(ltrim(TMPNP_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO)))
    begin
     ...
     SELECT @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  + TMPNP_SLIP_NO + ',' from TMP_DP_NP_DTLS_CDSL, used_slip where rtrim(ltrim(TMPNP_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO))
...
--if not exists(select TMPNP_SLIP_NO,TMPNP_BO_ID from TMP_DP_NP_DTLS_CDSL, slip_issue_mstr  , dp_acct_mstr where rtrim(ltrim(TMPNP_SLIP_NO)) like SLIIM_SERIES_TYPE + '%' and replace(rtrim(ltrim(TMPNP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'') between SLIIM_SLIP_NO_FR and SLIIM_SLIP_NO_TO and SLIIM_ENTM_ID = dpam_id and dpam_sba_no = TMPNP_BO_ID)
--    begin
--
--     SELECT @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  +   TMPNP_SLIP_NO + ',' from TMP_DP_NP_DTLS_CDSL, slip_issue_mstr  , dp_acct_mstr where rtrim(ltrim(TMPNP_SLIP_NO)) like SLIIM_SERIES_TYPE + '%' and replace(rtrim(ltrim(TMPNP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'') between SLIIM_SLIP_NO_FR and SLIIM_SLIP_NO_TO and SLIIM_ENTM_ID = dpam_id and dpam_sba_no = TMPNP_BO_ID
--
--        UPDATE filetask
--		SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Slips are not Issued : ' + @L_ERR_MISSING_ISIN
--		,      status = 'FAILED'
...
--    end 
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPNP_ACCT_ID,convert(datetime,TMPNP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPNP_ISIN,abs(TMPNP_QTY),'HIGH_VALUE','','') end = 'Y' then -1 else 0 end,  
				TMPNP_TRASTM_CD ,  
				id1+@L_MAX_DPTDC_DTLS_ID1,  
				NULL, 
				NULL,
...
--    end 
  ...
  if exists(select TMPEP_SLIP_NO from TMP_DP_EP_DTLS_CDSL, used_slip where rtrim(ltrim(TMPEP_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO)))
    begin
     ...
     SELECT @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  + TMPEP_SLIP_NO + ',' from TMP_DP_EP_DTLS_CDSL, used_slip where rtrim(ltrim(TMPEP_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO))
...
--if not exists(select TMPEP_SLIP_NO,TMPEP_BO_ID from TMP_DP_EP_DTLS_CDSL, slip_issue_mstr  , dp_acct_mstr where rtrim(ltrim(TMPEP_SLIP_NO)) like SLIIM_SERIES_TYPE + '%' and replace(rtrim(ltrim(TMPEP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'') between SLIIM_SLIP_NO_FR and SLIIM_SLIP_NO_TO and SLIIM_ENTM_ID = dpam_id and dpam_sba_no = TMPEP_BO_ID)
--    begin
--
--     SELECT @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  +   TMPEP_SLIP_NO + ',' from TMP_DP_EP_DTLS_CDSL, slip_issue_mstr  , dp_acct_mstr where rtrim(ltrim(TMPEP_SLIP_NO)) like SLIIM_SERIES_TYPE + '%' and replace(rtrim(ltrim(TMPEP_SLIP_NO)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'') between SLIIM_SLIP_NO_FR and SLIIM_SLIP_NO_TO and SLIIM_ENTM_ID = dpam_id and dpam_sba_no = TMPEP_BO_ID
--
--        UPDATE filetask
--		SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Following Slips are not Issued : ' + @L_ERR_MISSING_ISIN
--		,      status = 'FAILED'
...
--    end 
			 ...
			 ,dptdc_cm_id
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPEP_ACCT_ID,convert(datetime,TMPEP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPEP_ISIN,abs(TMPEP_QTY),'HIGH_VALUE','','') end = 'Y' then -1 else 0 end,  
				TMPEP_TRASTM_CD ,  
				id1+@L_MAX_DPTDC_DTLS_ID1,  
				NULL, 
				NULL,
...
BEGIN 
INSERT INTO used_slip
SELECT @l_id_used_slip + ID1 , @l_dpm_id1 , TMPOFFM_BO_ID , DPTDC_SLIP_NO,'1','','U','HO',getdate(),'HO',getdate(),1 
FROM #tmp_dptdc,TMP_DP_OFFM_DTLS_CDSL WHERE TMPOFFM_SLIP_NO = DPTDC_SLIP_NO
END
IF @PA_TRX_TAB  = 'INTERDP'
BEGIN
INSERT INTO used_slip
SELECT @l_id_used_slip + ID1 , @l_dpm_id1 , TMPINTDP_BO_ID , DPTDC_SLIP_NO,'1','','U','HO',getdate(),'HO',getdate(),1 
FROM #tmp_dptdc_intdp,TMP_DP_INTDP_DTLS_CDSL WHERE TMPINTDP_SLIP_NO = DPTDC_SLIP_NO
END
IF @PA_TRX_TAB  = 'NP'
BEGIN 
INSERT INTO used_slip
SELECT @l_id_used_slip + ID1 , @l_dpm_id1 , TMPNP_BO_ID , DPTDC_SLIP_NO,'1','','U','HO',getdate(),'HO',getdate(),1 
FROM #tmp_dptdc_np,TMP_DP_NP_DTLS_CDSL WHERE TMPNP_SLIP_NO = DPTDC_SLIP_NO
END
IF @PA_TRX_TAB  = 'EP'
BEGIN
INSERT INTO used_slip
SELECT @l_id_used_slip + ID1 , @l_dpm_id1 , TMPEP_BO_ID , DPTDC_SLIP_NO,'1','','U','HO',getdate(),'HO',getdate(),1 
FROM #tmp_dptdc_ep,TMP_DP_EP_DTLS_CDSL WHERE TMPEP_SLIP_NO = DPTDC_SLIP_NO
END 
