-- Object: PROCEDURE citrus_usr.pr_rpt_used_unused
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--[pr_rpt_used_unused] 'UNUSED_SLIP','904_ACT','10000037',4,1,'ho','',''
CREATE PROC [citrus_usr].[pr_rpt_used_unused]
(@pa_type varchar(50)
...
ELSE IF @pa_type = 'UNUSED_SLIP'
  ...
  --
    ...
    declare  @temp_dp table(dpm_dpid varchar(20),sliim_dpam_acct_no varchar(16),slip_no varchar(20),trastm_desc varchar(100),sliim_series_type varchar(20)) 
 ...
 --   ,@l_sliim_series_type varchar(20)
    ...
    LowRange BIGINT,
    HighRange BIGINT,
    uses_series_type vaRCHAR(1000)
);

INSERT INTO #ranges
SELECT uses_dpam_acct_no,'1',uses_slip_no,USES_SLIP_NO_to,uses_series_type FROM  used_slip_block WHERE uses_dpam_acct_no = @pa_client_id

;
WITH each AS
(
    SELECT ColA, ColB, LowRange AS n, HighRange,uses_series_type
      FROM #ranges
    UNION ALL
    SELECT ColA, ColB, n + 1, HighRange,uses_series_type
      FROM each
     WHERE n + 1 <= HighRange
)
SELECT ColA, ColB, n,uses_series_type into #temp 
FROM each
