-- Object: PROCEDURE citrus_usr.pr_client_multireport_bak28122011
...
create proc [citrus_usr].[pr_client_multireport_bak28122011](@pa_id numeric  
	...
	print @l_sql1				  
							...
							begin
								set @l_sql = ' select convert(datetime ,accp_value,103) as accp_value, ACCP_CLISBA_ID into #account_properties from account_properties 
											where accp_accpm_prop_cd like ''BILL_START_DT''
								...
								set @l_sql2 = ' from client_mstr clim , dp_acct_mstr dpam left outer join client_bank_accts on cliba_clisba_id=dpam.dpam_id left outer join bank_mstr on cliba_banm_id=banm_id LEFT OUTER JOIN dp_holder_dtls dphd  ON dpam.dpam_sba_no = dphd.dphd_dpam_sba_no and dphd.dphd_deleted_ind =1, entity_relationship entr, client_ctgry_mstr clicm ,sub_ctgry_mstr subcm , entity_type_mstr enttm , status_mstr stam , #account_properties accp'  
								set @l_sql2 = @l_sql2  + ' where clim.clim_crn_no = dpam.dpam_crn_no and  entr.entr_sba = dpam.dpam_sba_no and dpam.dpam_enttm_cd = enttm.enttm_cd and dpam.dpam_clicm_cd = clicm.clicm_cd and dpam.dpam_stam_cd = stam.stam_cd and  dpam_subcm_cd = subcm.subcm_cd and dpam.dpam_id = accp.ACCP_CLISBA_ID'  
							...
							begin
								...
								set @l_sql2 = ' from #account_properties,client_mstr clim, dp_acct_mstr dpam left outer join client_bank_accts on cliba_clisba_id=dpam.dpam_id left outer join bank_mstr on cliba_banm_id=banm_id LEFT OUTER JOIN dp_holder_dtls dphd ON dpam.dpam_sba_no = dphd.dphd_dpam_sba_no and dphd.dphd_deleted_ind =1, entity_relationship entr, client_ctgry_mstr clicm ,sub_ctgry_mstr subcm , entity_type_mstr enttm , status_mstr stam '  
								set @l_sql2 = @l_sql2  + ' where dpam_id= accp_clisba_id and  clim.clim_crn_no = dpam.dpam_crn_no  and entr.entr_sba = dpam.dpam_sba_no and dpam.dpam_enttm_cd = enttm.enttm_cd and dpam.dpam_clicm_cd = clicm.clicm_cd and dpam.dpam_stam_cd = stam.stam_cd and dpam_subcm_cd = subcm.subcm_cd '  
								set @l_sql2 = @l_sql2  + ' and clim.clim_deleted_ind = 1 and dpam.dpam_deleted_ind = 1 and entr.entr_deleted_ind = 1 and clicm.clicm_deleted_ind = 1 and enttm.enttm_deleted_ind = 1 and stam.stam_deleted_ind = 1 '  
								set @l_sql = @l_sql + @l_sql2 + case when  @pa_filter_values <> '' then ' and ' else '' end   
							end
