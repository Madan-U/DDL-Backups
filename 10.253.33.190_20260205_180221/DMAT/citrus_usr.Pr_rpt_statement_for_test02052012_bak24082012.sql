-- Object: PROCEDURE citrus_usr.Pr_rpt_statement_for_test02052012_bak24082012
...
create Proc [citrus_usr].[Pr_rpt_statement_for_test02052012_bak24082012]                            
@pa_dptype varchar(4),                                    
@pa_excsmid int,                                    
...
select * ,case when citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,16,'~') =  '' then REPLACE(CONVERT(VARCHAR(10), cdshm_tras_dt, 103), '/', '')+'000000' else citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,16,'~') end exedt
,citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,9,'~') exetime into #tmp_cdsl_holding_dtls from cdsl_holding_dtls WITH (NOLOCK)
...
--changed by tushar sep 18 2009    
  ...
  begin
     ...
     --        
		 ...
		 --trans_date,
		 cdshm_trg_settm_no,
		 VALUATION=CONVERT(NUMERIC(18,2),DPHMCd_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0))  ,tratm_cd                                            
		 ...
		 WHERE DPHMCD_HOLDING_DT = @pa_todate --and t.isin_cd = DPHMCd_ISIN
		 ...
		 order by dpam_acctno,dpam_sba_name,order_by,Isin_name,convert(datetime,trans_date),exedt,case when isnumeric(dpm_trans_no) = 1 then convert(numeric,dpm_trans_no) else 1 end  ,c_d_flag,cdshm_trg_settm_no    		 
		 end 
		 ...
		 --trans_date,
		 cdshm_trg_settm_no,
		 VALUATION=CONVERT(NUMERIC(18,2),DPHMC_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0))   , tratm_cd                                           
		 FROM #tmphldg LEFT OUTER JOIN  isin_mstr i with(nolock) on DPHMCd_ISIN = i.isin_cd 
		 LEFT OUTER JOIN CLOSING_PRICE_MSTR_CDSL ON DPHMCd_ISIN = CLOPM_ISIN_CD 
		 AND ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = DPHMCd_ISIN and CLOPM_DT <= @PA_FROMDATE and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc),
		 #tempmaincdsl t with(nolock),group_mstr 
		 where  dpam_acctno    =grp_client_code    
		 AND DPHMCD_HOLDING_DT = @pa_todate   --and t.isin_cd = DPHMCd_ISIN 
		 --order by dpam_acctno,dpam_sba_name,order_by,Isin_name,convert(datetime,trans_date),t.sett_no,dpm_trans_no,line_no,cdshm_trg_settm_no
		 order by dpam_acctno,dpam_sba_name,order_by,Isin_name,convert(datetime,trans_date),exedt,case when isnumeric(dpm_trans_no) = 1 then convert(numeric,dpm_trans_no) else 1 end,c_d_flag,cdshm_trg_settm_no    		 
		 end
