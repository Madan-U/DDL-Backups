-- Object: PROCEDURE citrus_usr.pr_ins_upd_ledger_contra_VISHAL
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[pr_ins_upd_ledger_contra_VISHAL](@pa_id              VARCHAR(8000)      
...
DECLARE @@l_acct_type    CHAR(1)    
	...
	BEGIN
		  ...
		  BEGIN TRANSACTION
		  UPDATE FINANCIAL_YR_MSTR SET Ledger_currid = isnull(Ledger_currid,0) + 2,VchNo_Contra=ISNULL(VchNo_Contra,0) + 1 WHERE fin_id = @@fin_id and fin_dpm_id = @pa_dpm_id and fin_deleted_ind =1
          SELECT @@l_led_id    = ISNULL(Ledger_currid,0) -1,@pa_voucher_no=VchNo_Contra FROM FINANCIAL_YR_MSTR WHERE fin_id = @@fin_id and fin_dpm_id = @pa_dpm_id and fin_deleted_ind =1
		  COMMIT TRANSACTION
