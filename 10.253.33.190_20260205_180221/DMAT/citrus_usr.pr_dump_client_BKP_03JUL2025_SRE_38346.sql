   ...
   Begin 
		...
		IF (SELECT COUNT(*) FROM TBL_CLIENT_MASTER_TMP )>1
			...
			COMMIT	
				...
				FROM TBL_CLIENT_POA WITH(NOLOCK) WHERE MASTER_POA ='2203320100000010' 
				GROUP  BY CLIENT_CODE
               
			   SELECT * INTO #POA  FROM (
               SELECT T.CLIENT_CODE,MAX(T.POA_DATE_FROM) AS POA_DATE_FROM,POA_STATUS 
			   FROM TBL_CLIENT_POA T WITH(NOLOCK) ,#POA_DATE P WITH(NOLOCK) WHERE MASTER_POA ='2203320100000010'
			   AND T.CLIENT_CODE =P.CLIENT_CODE AND T.POA_DATE_FROM =P.POA_DATE_FROM  
				...
				WHERE T.CLIENT_CODE=C.CLIENT_CODE AND MASTER_POA='2203320100000010')
				AND STATUS ='ACTIVE'
