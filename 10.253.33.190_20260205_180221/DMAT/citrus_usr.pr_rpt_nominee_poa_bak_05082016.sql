-- Object: PROCEDURE citrus_usr.pr_rpt_nominee_poa_bak_05082016
...
create proc [citrus_usr].[pr_rpt_nominee_poa_bak_05082016]
   (
    @pa_excsmid int,
...
--
	...
	WHERE  isnumeric(DPAM_sba_NO)=1 and (convert(numeric,DPAM_sba_NO) between convert(numeric,@pa_fromaccid) 
	AND  convert(numeric,@pa_toaccid))
--
...
print @@dtoption
	...
	--
		...
		If @pa_minor_flg='Y'
			...
			,left(pc6.DateOfSetup ,2) + '/' + substring(pc6.DateOfSetup ,3,2)+ '/' + right(pc6.DateOfSetup ,4) SetupDate
...
,'' Nom_Flag
	...
	--
		...
		,account.dpam_sba_no client_id 
		, account.dpam_sba_name client_name 
		, ltrim(rtrim(isnull(Name,''))) + ' ' + ltrim(rtrim(isnull(MiddleName,''))) + ' ' + ltrim(rtrim(isnull(SearchName,''))) NAME
		--, isnull(NOM_NRN_NO,'') NAME_id
		,NAME_id=case when  isnull(NOM_NRN_NO,'')='' then 'N/A' else NOM_NRN_NO end 
      ,left(DateOfSetup ,2) + '/' + substring(DateOfSetup ,3,2)+ '/' + right(DateOfSetup ,4) SetupDate
	...
	,isnull(DATEADD(DAY,1,(DATEADD(YEAR,18,convert(datetime,DPHD_NOM_DOB,103)))),'') major_dt
		...
		from #ACLIST account, dp_holder_dtls,dp_acct_mstr dpacct  left outer join dps8_pc6 on boid = dpam_sba_no and TypeOfTrans in ('1','')
		, account_properties
		where account.dpam_id = dphd_dpam_id
        and   accp_clisba_id = dpacct.dpam_id
        and   ACCP_ACCPM_PROP_CD = 'bill_start_dt' 
...
,ltrim(rtrim(substring(setupdate,1,2) + '/' + substring(setupdate,3,2) + '/' + substring(setupdate,5,4))) SetupDate
        ...
        ,poa_addr = IsNull(ADDR1,'') +''+ IsNull(ADDR2,'') + ' '+ IsNull(Addr3,'')+ ' ' + IsNull(city,'') + ''+ IsNull(state,'') + ' '+ IsNull(country,'') + ' ' + IsNull(pincode,'') 
