-- Object: PROCEDURE citrus_usr.Pr_ValidateAccount
...
CREATE Proc [citrus_usr].[Pr_ValidateAccount]            
@pa_excsmid int,            
...
when len(@pa_acct_no) = 5 then right(dpam_sba_no,5)end = @pa_acct_no   
  ...
  print @@sbano  
		select isnull(@count + '|' + right(@@sbano,8),'') Msg		
	end
ELSE if exists(select poam_master_id from POAM
   	where right(poam_master_id,3) = @pa_acct_no 
	)
	begin
		select @@sbano = poam_master_id from POAM
   	where right(poam_master_id,3) = @pa_acct_no 

        select @count = count(poam_master_id) from POAM
   	where right(poam_master_id,3) = @pa_acct_no 
...
--     set @pa_acct_no = @pa_dpm_dpid + @pa_acct_no                        
  ...
  begin            
	...
	END       
ELSE if exists(select poam_master_id from POAM
   	where poam_master_id = @pa_acct_no 
	)
	begin
		SELECT ''	MSG
	end
