-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_archtest
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_archtest](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
begin 
select @l_min_trx_dt ='Apr 01 2017'-- min(curr_fr_dt) from archival_details 
select @l_base_hldg_dt ='Mar 31 2017'-- max(dphmcd_holding_dt) from vw_holding_base_all
end 
else 
begin 
...
,convert(numeric(18,5),0) DPHMCD_LEND_QTY    
,convert(numeric(18,5),0) DPHMCD_BORROW_QTY    
from [vw_holding_base_all_ARCHTEST] a with (nolock) ,dp_acct_mstr  with (nolock) where dphmcd_holding_dt =@l_base_hldg_dt    
...
from citrus_usr.[vw_holding_base_all_ARCHTEST] with(nolock) ,dp_acct_mstr with(nolock)                      
...
into #holdingdmtpenfing from  [vw_holding_base_all_ARCHTEST] with (nolock), DP_aCCT_MSTR  with (nolock)
...
into #holdingdmtpenfing1 from  [vw_holding_base_all_ARCHTEST] with (nolock) , DP_aCCT_MSTR with (nolock)
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt=@l_base_hldg_dt
and dpam_sba_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
and dphmcd_dpm_id = @PA_DPM_ID
...
into #holdingdmtpenfing2 from  [vw_holding_base_all_ARCHTEST] with (nolock), DP_aCCT_MSTR with (nolock)
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt=@l_base_hldg_dt
group by dphmcd_dpam_id,dphmcd_isin 
