-- Object: PROCEDURE citrus_usr.pr_ins_upd_ledgerR_VISHAL
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[pr_ins_upd_ledgerR_VISHAL](@pa_id              VARCHAR(8000)      
...
DECLARE @@l_acct_type    CHAR(1)    
	...
	BEGIN
	 Create table #tmpvch(Acct_id bigint,dpm_id int,Vch_dt datetime,Vch_no bigint,Amt money,clrg_dt datetime)
	END

	IF @PA_ACTION = 'APP'  
	BEGIN
			  SET @@TOTALRECORDS = citrus_usr.ufn_countstring(@pa_values,@rowdelimiter)
			  ...
			  BEGIN
					  ...
					  IF @@CTR = 1
					  BEGIN
							SELECT @@fin_id = FIN_ID FROM FINANCIAL_YR_MSTR WHERE FIN_DPM_ID = @pa_dpm_id AND (CONVERT(DATETIME,@pa_voucher_dt) BETWEEN FIN_START_DT AND FIN_END_DT)AND FIN_DELETED_IND =1  
					  END
					  ...
					  BEGIN    
							...
							SET @@SSQL = 'Update acbal 
										  ...
										  FROM #tmpvch,' + @@Accbaltable + ' acbal
										  WHERE Acct_id = ldg_account_id AND dpm_id = accbal_dpm_id'
							EXEC(@@SSQL)
					  ...
					  END
						...
						SET @@SSQL= 'Insert into ledger' + @@fin_id + ' (LDG_ID,LDG_DPM_ID,LDG_VOUCHER_TYPE,LDG_BOOK_TYPE_CD,LDG_VOUCHER_NO,LDG_SR_NO,LDG_REF_NO,LDG_VOUCHER_DT,LDG_ACCOUNT_ID,LDG_ACCOUNT_TYPE,LDG_AMOUNT,LDG_NARRATION,LDG_BANK_ID,LDG_ACCOUNT_NO,LDG_INSTRUMENT_NO,LDG_BANK_CL_DATE,LDG_COST_CD_ID,LDG_BILL_BRKUP_ID,LDG_TRANS_TYPE,LDG_STATUS,LDG_CREATED_BY,LDG_CREATED_DT,LDG_LST_UPD_BY,LDG_LST_UPD_DT,LDG_DELETED_IND,LDG_BRANCH_ID)
									 ...
									 and ldg_voucher_type = ' + @pa_voucher_type + ' and ldg_voucher_no   = ' + @pa_voucher_no + ' and ldg_dpm_id = ' + @pa_dpm_id + ' and ldg_deleted_ind in (0,4,6)'
						EXEC(@@SSQL)

						SET @@SSQL=   'Update ledger' + @@fin_id + '_mak  
									  Set ldg_delted_ind = LDG_DELETED_IND+1  
									  WHERE ldg_voucher_no   = ' + @pa_voucher_no + '
									  AND   ldg_voucher_dt   = convert(datetime,''' + @pa_voucher_dt + ''',103)  
									  AND   ldg_dpm_id       = ' + @pa_dpm_id + '
									  AND   ldg_voucher_type = ' + @pa_voucher_type + '
						...
						SET @@SSQL =  'Update accbal
									   ...
									   FROM ACCOUNTBAL' + @@fin_id + ',ledger' + @@fin_id + '
									   accbal_acct_id = ldg_account_id and accbal_dpm_id = LDG_DPM_ID
									   and ldg_voucher_no   = ' + @pa_voucher_no + '
									   and ldg_voucher_type = ' + @pa_voucher_type + ' 
									   and ldg_book_type_cd = ' + @pa_book_type_cd + '
									   and ldg_deleted_ind=1
									   and exsits(select accbal_acct_id from ACCOUNTBAL' + @@fin_id + ' where accbal_acct_id = LDG_ACCOUNT_ID and accbal_dpm_id = LDG_DPM_ID and accbal_deleted_ind = 1)'
						EXEC(@@SSQL)
						...
						SET @@SSQL =  'INSERT INTO ACCOUNTBAL' + @@fin_id + '(accbal_dpm_id,accbal_acct_id,accbal_acct_type,accbal_amount,accbal_created_by,accbal_created_dt,accbal_lst_upd__by,accbal_lst_upd__dt,accbal_deleted_ind)
									   ...
									   and not exsits(select accbal_acct_id from ACCOUNTBAL' + @@fin_id + ' where accbal_acct_id = LDG_ACCOUNT_ID and accbal_dpm_id = LDG_DPM_ID and accbal_deleted_ind = 1)'
						EXEC(@@SSQL)
	...
	BEGIN
			  ...
			  BEGIN
					  ...
					  WHERE  ldg_voucher_no   = ' + @pa_voucher_no + '    
					  AND    ldg_voucher_dt   = ' + @pa_voucher_dt + '   
					  AND    ldg_dpm_id       = ' + @pa_dpm_id + '   
					  AND    ldg_voucher_type = ' + @pa_voucher_type + '   
					  AND    ldg_book_type_cd = ' + @pa_book_type_cd + '
					  AND    ldg_deleted_ind in (0,4,6)'
	...
	BEGIN
		  SELECT @@fin_id = FIN_ID FROM FINANCIAL_YR_MSTR WHERE FIN_DPM_ID = @pa_dpm_id AND (CONVERT(DATETIME,@pa_voucher_dt) BETWEEN FIN_START_DT AND FIN_END_DT)AND FIN_DELETED_IND =1  
		  BEGIN TRANSACTION
		  UPDATE FINANCIAL_YR_MSTR SET Ledger_currid = Ledger_currid + 2,VchNo_Reciept=ISNULL(VchNo_Reciept,0) + 1 WHERE fin_id = @@fin_id and fin_dpm_id = @pa_dpm_id and fin_deleted_ind =1
          SELECT @@l_led_id    = ISNULL(Ledger_currid,0) -1,@pa_voucher_no=VchNo_Reciept FROM FINANCIAL_YR_MSTR WHERE fin_id = @@fin_id and fin_dpm_id = @pa_dpm_id and fin_deleted_ind =1
		  COMMIT TRANSACTION
				...
				SET @@l_amount    = convert(money,citrus_usr.fn_splitval(@@currstring_value,3)) * -1
				...
				VALUES(' + @@l_led_id + ',' + @pa_dpm_id + ',''' + @pa_voucher_type +''',''' + @pa_book_type_cd + ''',''' + @pa_voucher_no +''',1 ,''' + @pa_ref_no + ''',convert(datetime,''' + @pa_voucher_dt+ ''',103) ,' + @@l_account_id  + ',''' + @@l_acct_type + ''',' + @@l_amount + ',''' + @@l_narration + ''',''' + @@l_acct_no + ''','''','''',0,0 ,''N'',''P'' ,''' + @pa_login_name + ''',getdate(),''' + @pa_login_name+ ''',getdate() ,1 )'     
				EXEC(@@SSQL)
				...
				BEGIN    
						SET @@SSQL =   'IF EXISTS(SELECT accbal_acct_id FROM ACCOUNTBAL' + @@fin_id + ' WHERE accbal_dpm_id = ' + @pa_dpm_id + ' and accbal_acct_id = ' + @@l_account_id + ' and accbal_deleted_ind = 1)    
										BEGIN    
												...
												SET    accbal_amount = accbal_amount + ' + @@l_amount + '    
										...
										BEGIN    
												...
												VALUES (' + @pa_dpm_id + ',' + @@l_account_id + ',''' + @@l_acct_type + ''',' + @@l_amount + ',''' + @pa_login_name + ''',getdate(),''' + @pa_login_name + ''',getdate(),1)    
										END'    
						EXEC(@@SSQL)
				END
				...
				SET @@l_amount    = convert(money,citrus_usr.fn_splitval(@@currstring_value2,5))    
				...
				SET @@SSQL = 'INSERT INTO ' + @@ledgertable + '(ldg_id,ldg_dpm_id,ldg_voucher_type,ldg_book_type_cd,ldg_voucher_no,ldg_sr_no,ldg_ref_no,ldg_voucher_dt,ldg_account_id,ldg_account_type,ldg_amount,ldg_narration,ldg_account_no,ldg_instrument_no,ldg_branch_id,ldg_cost_cd_id,ldg_bill_brkup_id,ldg_trans_type,ldg_status,ldg_created_by,ldg_created_dt,ldg_lst_upd_by,ldg_lst_upd_dt,ldg_deleted_ind)
				VALUES(' + @@l_led_id  + ',' + @pa_dpm_id + ',''' + @pa_voucher_type +''',''' + @pa_book_type_cd + ''',''' + @pa_voucher_no +''',2,''' + @pa_ref_no + ''',convert(datetime,''' + @pa_voucher_dt+ ''',103) ,' + @@l_account_id  + ',''' + @@l_acct_type + ''',' + @@l_amount + ',''' + @@l_narration + ''',''' + @@l_acct_no + ''',''' + @@l_cheque + ''',''' + @@l_branch_id + ''',0,0 ,''N'',''P'' ,''' + @pa_login_name + ''',getdate(),''' + @pa_login_name+ ''',getdate() ,1 )'     
				EXEC(@@SSQL)
	...
	BEGIN
			  ...
			  BEGIN TRANSACTION
			  UPDATE FINANCIAL_YR_MSTR SET Ledger_currid = Ledger_currid + 2 WHERE fin_id = @@fin_id and fin_dpm_id = @pa_dpm_id and fin_deleted_ind =1
			  SELECT @@l_led_id    = ISNULL(Ledger_currid,0) -1 FROM FINANCIAL_YR_MSTR WHERE fin_id = @@fin_id and fin_dpm_id = @pa_dpm_id and fin_deleted_ind =1
			  COMMIT TRANSACTION
	...
	BEGIN
				...
				BEGIN    
					  ...
					  SET    accbal_amount  = accbal_amount - ldg_amount 
					  FROM   ' + @@Accbaltable + ' accb, ' + @@ledgertable + '
