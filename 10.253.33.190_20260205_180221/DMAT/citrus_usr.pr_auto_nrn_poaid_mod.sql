-- Object: PROCEDURE citrus_usr.pr_auto_nrn_poaid_mod
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

 --exec [pr_auto_nrn_poaid] ''175994|*~|448510|*~|*|~*176000|*~| 449293|*~|*|~*176004|*~| 449929|*~|*|~*176007|*~| 450216|*~|*|~*' ,'CDSL','jan 01 1900','jan 31 2100','','',0,'',''    
 CREATE PROCeDURE [citrus_usr].[pr_auto_nrn_poaid_mod]( @pa_crn_no      VARCHAR(8000)                      
                            , @pa_exch        VARCHAR(10)                      
...
DECLARE @crn TABLE (crn          numeric                      
 ...
 begin    
  ...
  begin     
  PRINT 'YYY'    
  PRINT CONVERT(DATETIME,@pa_from_dt ,103) 
  PRINT CONVERT(DATETIME,@pa_to_dt,103)                    
    ...
    INSERT INTO @crn                       
          ...
          and clic_mod_lst_upd_dt between CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'                             
           ...
           and clic_mod_action in ('N NAMECHANGE')    and clic_mod_dpam_sba_no = DPAM_SBA_NO and isnull(clic_mod_batch_no  ,'0')='0'   
         --                      
 ...
 begin    
   ...
   INSERT INTO @crn                       
          ...
          and DPAM_CRN_NO = CLIM_CRN_NO 
         and  clic_mod_dpam_sba_no = DPAM_SBA_NO and isnull(clic_mod_batch_no  ,'0')='0' and clic_mod_action in ('N NAMECHANGE')
