-- Object: PROCEDURE citrus_usr.pr_select_trxgen_cdsl_SP_bak_10092018
...
--rollback
create  PROCEDURE [citrus_usr].[pr_select_trxgen_cdsl_SP_bak_10092018]  
(  
								...
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
												Select convert(char(8),replace(convert(varchar(11),dptdc_execution_dt,103),'/',''))  
														...
														+ case when isnull(convert(varchar,dptdc_reason_cd),'0')='0' then space(1) else  isnull(convert(varchar,dptdc_reason_cd),'') end 
														DETAILS
														, abs(dptdc_qty) qty , 
...
--UNFRZ
						...
						BEGIN
										...
										BEGIN  
											...
											DETAILS
											,abs(remrm_qty) qty
											,'','','','','','0',replace(convert(varchar(11),isnull(rEMRM_REQUEST_DT,space(8)),103),'/','') rEMRM_REQUEST_DT
