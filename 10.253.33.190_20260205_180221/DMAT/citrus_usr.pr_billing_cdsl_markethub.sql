-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_markethub
...
*/
--[pr_billing_cdsl] 'feb 01 2008','june 01 2008',2,'y'
CREATE procedure [citrus_usr].[pr_billing_cdsl_markethub](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
...
select distinct convert(datetime,accp_value,103) accp_value_cl, accp_clisba_id accp_clisba_id_cl , accp_accpm_prop_cd accp_accpm_prop_cd_cl into #account_properties_close from account_properties where accp_accpm_prop_cd = 'ACC_CLOSE_DT' 
  ...
  ,dpam_id
  ,cham_slab_name
  ,case when cham_slab_name ='DOCUMENT CHARGES' 
             and accp_value <= 'dec 31 2010' then 0 
        else 
		sum(cham_charge_value )* -1 end 
  ,cham_post_toacct
  from dp_acct_mstr
      ,client_dp_brkg
      ,profile_charges
...
begin
		   ...
		   ,charge_val
		   ,post_toacct)
			select convert(datetime,citrus_usr.fn_acct_entp(dpam_id,'BILL_START_DT'),103),dpam_id,cham_slab_name,cham_charge_value*-1,cham_post_toacct
			from dp_acct_mstr
			,client_dp_brkg
			,profile_charges
			,charge_mstr
...
/*MIN MAX CHARGES LOGIC*/
  ...
  ----transaction qty wise charge
			...
			begin
				...
				update cdsl_holding_dtls
				set    cdshm_charge   =  isnull(cdshm_charge,0)  + 
			     case when (CDSHM_COUNTER_BOID not in (select trg_boid from Client_Cm_details where trg_boid <>'') and 
									 (CDSHM_COUNTER_CMBPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'') and 
									  CDSHM_COUNTER_DPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'' ))) or (proc_profile_id in (11,12,13))  then isnull(case when cham_val_pers  = 'V' then cham_charge_value * -1 
																																											 else case when (cham_charge_value/100)*(abs(cdshm_qty)*clopm_cdsl_rt) <= cham_charge_minval then cham_charge_minval* -1
																																				                             Else (cham_charge_value/100)*(abs(cdshm_qty)*clopm_cdsl_rt)* -1 end end  ,0)
                      ELSE
                        case when PROC_PROFILE_ID in (2)  then '-25'
								when PROC_PROFILE_ID in (3,9,10)  then '-20'
								when PROC_PROFILE_ID in (4,5,6)  then '-15'
								when PROC_PROFILE_ID in (7)  then '-10'
								when PROC_PROFILE_ID in (8)  then '-6'
								else '0' end 
                      end 
				...
				set    cdshm_charge   =  isnull(cdshm_charge,0)  + 
			     case when (CDSHM_COUNTER_BOID not in (select trg_boid from Client_Cm_details where trg_boid <>'') and 
			      ...
			      and   proc_deleted_ind = 1
			      and   clopm_deleted_ind = 1
	      		  and (not exists(select dppd_master_id from dp_poa_dtls 
				  where dppd_dpam_id = cdshm_dpam_id and dppd_deleted_ind = 1
				  and   dppd_setup <= cdshm_tras_dt ) and cham_charge_type <> 'of-dr')
			     ...
			     case when (CDSHM_COUNTER_BOID not in (select trg_boid from Client_Cm_details where trg_boid <>'') and 
									 (CDSHM_COUNTER_CMBPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'') and 
									  CDSHM_COUNTER_DPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'' ))) or (proc_profile_id in (11,12,13))  then isnull(case when cham_val_pers  = 'V' then cham_charge_value * -1 
																																				                             ...
																																				                             Else (cham_charge_value/100)*(abs(cdshm_qty)*clopm_cdsl_rt)* -1 end end  ,0)
                      ELSE
                         case when PROC_PROFILE_ID in (2)  then '-20'
								when PROC_PROFILE_ID in (3,9,10)  then '-15'
								when PROC_PROFILE_ID in (4)  then '-10'
								when PROC_PROFILE_ID in (5,6,7,8)  then '0'
								else '0' end 
                       end 
			      ...
			      and   proc_deleted_ind = 1
			      and   clopm_deleted_ind = 1
	      		  and ( exists(select dppd_master_id from dp_poa_dtls 
				  where dppd_dpam_id = cdshm_dpam_id and dppd_deleted_ind = 1
				  and   dppd_setup <= cdshm_tras_dt ) and cham_charge_type <> 'of-dr')
 ...
 update cdsl_holding_dtls
						...
						+  case when (CDSHM_COUNTER_BOID not in (select trg_boid from Client_Cm_details where trg_boid <>'') and 
									 (CDSHM_COUNTER_CMBPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'') and 
									  CDSHM_COUNTER_DPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'' ))) or (proc_profile_id in (11,12,13))  then (isnull(cham_charge_value,0) * -1 ) else
                         case when PROC_PROFILE_ID in (2)  then '-25'
								when PROC_PROFILE_ID in (3,9,10)  then '-20'
								when PROC_PROFILE_ID in (4,5,6)  then '-15'
								when PROC_PROFILE_ID in (7)  then '-10'
                           ...
                           from cdsl_holding_dtls      
							...
							and   CHAM_CHARGE_BASE = 'NORMAL'
							and cham_charge_type = 'of-dr'
			                AND CDSHM_TRATM_DESC not like '%EX 11%' and CDSHM_TRATM_DESC not like '%SETTLEMENT-DR%'
...
--              CDSHM_COUNTER_DPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'' )))
              AND CDSHM_TRATM_DESC not like '%EX 11%' and CDSHM_TRATM_DESC not like '%SETTLEMENT-DR%'
               /*feb 04 10 to waive those trx which hav ex 11*/ 
               and (exists(select dppd_master_id from dp_poa_dtls 
				where dppd_dpam_id = cdshm_dpam_id and dppd_deleted_ind = 1
				and   dppd_setup <= cdshm_tras_dt ) and cham_charge_type <> 'of-dr')
			...
			begin
				...
				AND   cham_charge_type = 'DEMAT'
				--AND   cdshm_tratm_cd = '2246'
                AND   cdshm_tratm_cd = '2201' -- on vishesh request we are charging demat charge from credit stage to request stage
				and   cham_deleted_ind = 1
				and   clidb_deleted_ind = 1
				and   cdshm_deleted_ind = 1
				and   proc_deleted_ind = 1
...
update cdsl_holding_dtls
			set    cdshm_charge   =  isnull(cdshm_charge,0) +  case when (CDSHM_COUNTER_BOID not in (select trg_boid from Client_Cm_details where trg_boid <>'') and 
              (CDSHM_COUNTER_CMBPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'') and 
              CDSHM_COUNTER_DPID not in (select trg_cmbpid from Client_Cm_details where trg_cmbpid <>'' ))) or (proc_profile_id in (11,12,13))  then (isnull(cham_charge_value,0) * -1 ) else 
                case when PROC_PROFILE_ID in (2)  then '-25'
				when PROC_PROFILE_ID in (3,9,10)  then '-20'
