-- Object: FUNCTION citrus_usr.fn_cdsl_export_line_7_extract_newversion_mod_bak_11082015
...
create function [citrus_usr].[fn_cdsl_export_line_7_extract_newversion_mod_bak_11082015](@pa_crn_no      VARCHAR(8000)    
...
--          ,0000--''--usr_fld4   
    ...
    --,CASE WHEN not exists (select 1 from dpb9_mod where BOId = DPAM_SBA_NO and PurposeCode12 = '6') THEN 'S' ELSE 'M' END  
		...
		,Case when climmod.clic_mod_action = 'NOMINEE DEL' then 'D' else (CASE WHEN exists (select 1 from dps8_pc6 where BOId = DPAM_SBA_NO and PurposeCode6 = '06' and TypeOfTrans = '3') THEN 'S' when not exists (select 1 from dps8_pc6 where BOId = DPAM_SBA_NO and PurposeCode6 = '06') then 'S' else '' END) end
