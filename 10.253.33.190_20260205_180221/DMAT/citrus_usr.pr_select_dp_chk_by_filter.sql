-- Object: PROCEDURE citrus_usr.pr_select_dp_chk_by_filter
...
*/   
CREATE PROCEDURE [citrus_usr].[pr_select_dp_chk_by_filter]      
(      
  @pa_query_id   NUMERIC                  
  ,@pa_scr_id     NUMERIC                 
  ,@pa_tab        VARCHAR(20)                    
...
--                
  ...
  print '11'
    ...
    AND    climm.clim_lst_upd_by      IN (SELECT distinct  LOGN_NAME FROM LOGIN_NAMES,ENTITY_TYPE_MSTR WHERE ENTTM_ID = LOGN_ENTTM_ID          
       ...
       and not exists (select dpam_Crn_no from DP_ACCT_MSTR where DPAM_CRN_NO = climm.clim_crn_no and DPAM_STAM_CD in ('active' , '04','05'  )           )
    UNION                
  ...
  and   clidb_deleted_ind in (0,4,8) 
    ...
    and not exists (select dpam_Crn_no from DP_ACCT_MSTR where DPAM_CRN_NO = clim.clim_crn_no and DPAM_STAM_CD  in ( 'active','04','05')           )                
  ...
  and   clidb_deleted_ind in (0,4,8) 
   ...
   AND    @pa_login_name    not in (SELECT clim_lst_upd_by        
    ...
    AND    climm.clim_crn_no        IN (SELECT clim_crn_no                
                                        ...
                                        (not exists (select dpam_Crn_no from DP_ACCT_MSTR where DPAM_CRN_NO = climm.clim_crn_no and DPAM_STAM_CD in ('active' ,'04','05')          )        
...
.logn_name              
  ...
  and   clidb_deleted_ind in (0,4,8)
    ...
    AND    clim.clim_crn_no      NOT IN (SELECT clim_crn_no                        
                                        ...
                                        and DPAM_STAM_CD in ('active' , '04','05'  )         )                                                     
                                        --or exists (select	 1 
...
print 'sfsfs'
  ...
  and    exists(select 1 clidb_dpam_id   where @pa_filter IN('0','1','2')  
   ...
   AND    climm.clim_crn_no        IN (SELECT clim_crn_no                
            FROM   client_list WITH (NOLOCK)                
            WHERE  clim_deleted_ind = 1                
            AND    clim_status      in (10,3)  and CLIM_TAB='dp poa dtls'           
            )   and climm.clim_lst_upd_by  like  ltrim(rtrim(@rowdelimiter1 )) +'%'
            and not exists (select dpam_Crn_no from DP_ACCT_MSTR where DPAM_CRN_NO = climm.clim_crn_no and DPAM_STAM_CD in ( '04','05'  )           )
            and exists (select accd_clisba_id from accd_mak,dp_acct_mstr_mak where  accd_clisba_id=DPAM_ID
            and DPAM_DELETED_IND in(0,1) and ACCD_DELETED_IND in (0,1) and climm.CLIM_CRN_NO=DPAM_CRN_NO and ACCD_ACCDOCM_DOC_ID='12')
            --not exists removed bz client modification client not come on 06/07/2015
