-- Object: PROCEDURE citrus_usr.Pr_rpt_PreBilling
...
CREATE proc [citrus_usr].[Pr_rpt_PreBilling]  
(@PA_EXCSMID   INT  
,@PA_TAB varchar(25)        
...
IF   @PA_TAB = 'TRXWISE'     
   ...
   -- condition for pledge account type = 14 as the records also come in individual account type  
     ...
     (    
     select NSDHM_BOOK_NAAR_CD,nsdhm_charge,nsdhm_dpam_id  , nsdhm_transaction_dt
     ...
     and   cli_list.dpam_id  = tmp.nsdhm_dpam_id  and nsdhm_transaction_dt between EFF_FROM AND EFF_TO
...
*/  
  ...
  begin  
   ...
   BEGIN    
   --    
    select CONVERT(VARCHAR(11),INWCR_RECVD_DT,109) INWCR_RECVD_DT,inwcr_frmno,INWCR_NAME,inwcr_rmks,FINA_ACC_NAME,INWCR_PAY_MODE,INWCR_cheque_no,inwcr_charge_collected   from inw_client_reg , FIN_ACCOUNT_MSTR  
...
convert(varchar(11),FIN_END_DT,109) ) AND FIN_DELETED_IND = 1   
  ...
  BEGIN  
    ...
    SeLECT DISTINCT citrus_usr.fn_ucc_accp(dpam_id ,'ACC_CLOSE_DT','') dt ,DPAM_SBA_NAME,DPAM_SBA_NO   
    FROM citrus_usr.DP_ACCT_MSTR, citrus_usr.EXCH_SEG_MSTR  
    WHERE DPAM_EXCSM_ID = EXCSM_ID --AND  DPAM_ACTION ='E'  
    AND   dpam_stam_cd ='05'  
    AND   EXCSM_EXCH_CD =@PA_DP_TYPE  
