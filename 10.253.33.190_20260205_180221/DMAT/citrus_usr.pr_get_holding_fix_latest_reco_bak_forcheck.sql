-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_reco_bak_forcheck
...
CREATE  proc [citrus_usr].[pr_get_holding_fix_latest_reco_bak_forcheck](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
select cdshm_dpam_id , cdshm_ben_acct_no , cdshm_isin , sum(cdshm_qty*-1) ep_rej into #tmp_ep_rej 
...
PRINT CONVERT(VARCHAR(26), GETDATE(), 109)  
  ...
  from #tmp_ep_rej epr , #tmp_dp_daily_hldg_cdsl a 
 where a.dphmcd_dpam_id= epr.cdshm_dpam_id       
and a.dphmcd_isin = epr.cdshm_isin  

  update a  set DPHMCD_CURR_QTY = DPHMCD_CURR_QTY + ep_rej 
  from #tmp_ep_rej_full epr , #tmp_dp_daily_hldg_cdsl a 
 where a.dphmcd_dpam_id= epr.cdshm_dpam_id       
and a.dphmcd_isin = epr.cdshm_isin  
