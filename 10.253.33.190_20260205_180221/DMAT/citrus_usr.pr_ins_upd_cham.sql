-- Object: PROCEDURE citrus_usr.pr_ins_upd_cham
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--0	INS	HO	CDSL	r	F	IN0019780025|*~|IN0019780074|*~|	M					A|*~||*~||*~|4|*~|*|~*		0	*|~*	|*~|	
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_cham](@pa_id              VARCHAR(8000)  
...
--
END
IF @pa_chk_yn = 0 AND  @PA_ACTION = 'EDT'
BEGIN
--
  SELECT @l_dtls_id   = CHAM_SLAB_NO FROM charge_mstr where cham_slab_no = @pa_id 
--
END
...
SET @remainingstring = @pa_id  
		...
		--
			...
			PRINT @l_action															  
								  ...
								  BEGIN
								  --
								    IF @PA_ACTION = 'INS' and  @l_action = 'A'
												BEGIN
												--
												  BEGIN TRANSACTION
											...
											--
											  BEGIN TRANSACTION
												...
												update charge_mstr
												set cham_slab_name                      = @pa_slab_name
												,cham_charge_type						= @pa_charge_type
												,cham_charge_base						= @pa_charge_base
												,cham_bill_period						= case when @pa_billing_period	 ='' then @pa_months ELSE @pa_billing_period end
												,cham_bill_interval						= case when @pa_months <> '' then @pa_days when @pa_billing_period	 = 'Y' then '12'when @pa_billing_period	 = 'Q' then '3'when @pa_billing_period	 = 'H'	then '6' when @pa_billing_period	 = 'M' then '1'			end 					                            
												,cham_charge_baseon						= @pa_based_on
												,cham_from_factor						= CONVERT(NUMERIC(20,2),@l_from_no)
												,cham_to_factor							= CONVERT(NUMERIC(20,2),@l_to_no)					
												,cham_val_pers							= @l_val_per
												,cham_charge_value						= @l_charge_val
												,cham_charge_minval						=@l_mim_charge_val
												,cham_charge_graded						=@pa_calculationtype
												,cham_chargebitfor						=case when @pa_excsm_cd <> 'ALL' then @l_excsm_id else '0' end 
												,cham_remarks							=@pa_rmks
												,cham_post_toacct						=@pa_cham_post_toacct
												,cham_lst_upd_by						=@pa_login_name
												,cham_lst_upd_dt						=getdate()
												,cham_deleted_ind						=1
												,CHAM_PER_MIN							=@l_min
												,CHAM_PER_MAX							=@l_max                                                                                 
                                                where cham_slab_no = @pa_id
													...
													--
														...
														delete chacm 
														from charge_ctgry_mapping chacm
														where chacm_cham_id = @pa_id 
