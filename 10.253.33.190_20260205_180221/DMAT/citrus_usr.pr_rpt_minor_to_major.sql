-- Object: PROCEDURE citrus_usr.pr_rpt_minor_to_major
...
*/
CREATE PROC [citrus_usr].[pr_rpt_minor_to_major]
(@pa_type varchar(25),
@pa_excsmid int,
...
--
  ...
  BEGIN
  --
		 select dpam_sba_name clim_name1 
				,clim_short_name 
				,dpam_sba_no
				,convert(varchar,clim_dob,103) [dob]
				--,convert(varchar(10),DATEADD(DAY,1,(DATEADD(YEAR,18,convert(datetime,clim_dob,103)))),103) MAJOR_DATE	
				,convert(varchar(10),(DATEADD(YEAR,18,convert(datetime,clim_dob,103))),103) MAJOR_DATE	
				...
				and    (datediff(day,convert(datetime,clim_dob,103),convert(datetime,@pa_fromdate,103)) < 6570
				or    datediff(day,convert(datetime,clim_dob,103),convert(datetime,@pa_todate,103)) > 6570)
				and DPAM_CLICM_CD in ('01','21')
				AND dpam_subcm_cd NOT IN  ( '082104') AND DPAM_STAM_CD  = 'ACTIVE'
  --
  END
  ...
  --
    select dpam_sba_name clim_name1
					  ...
					  ,convert(varchar(10),DATEADD(DAY,1,(DATEADD(YEAR,18,convert(datetime,dphd_nom_dob,103)))),103) MAJOR_DATE	
