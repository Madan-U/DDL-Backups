-- Object: PROCEDURE citrus_usr.Edis_trxn_process_Bkp_20210302
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

Create Proc [citrus_usr].[Edis_trxn_process_Bkp_20210302] (@date datetime)
as
...
TRUNCATE TABLE HOLD_REP_N
	 ...
	 select party_code,isin,adj_qty ,dp_id,ROW_NUMBER()over(order by party_code,isin desc) SR_NO into #pld1 from DIY_Reprocess_log_e_dis with(nolock)
		 ...
		 where exists (select party_code from DIY_Reprocess_log_e_dis d where d.party_code =t.Partycode and d.isin=t.isin and getdate() between Request_date
		  and Request_date+NO_of_days+ ' 23:59') and Request_date >='2021-02-20' and NO_of_days=5 --AND ISNULL(VALID,0)=0 and isnull(dummy3,'')=''
