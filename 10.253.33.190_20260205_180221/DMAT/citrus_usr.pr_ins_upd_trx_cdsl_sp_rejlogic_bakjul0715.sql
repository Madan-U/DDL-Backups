-- Object: PROCEDURE citrus_usr.pr_ins_upd_trx_cdsl_sp_rejlogic_bakjul0715
...
create PROCEDURE [citrus_usr].[pr_ins_upd_trx_cdsl_sp_rejlogic_bakjul0715](@pa_id              VARCHAR(8000)    
													 ,@pa_tab             VARCHAR(20)  
...
if @pa_action ='PREAPP'
begin
	update maker_scancopy set preappflg='Y' where slip_no = @pa_slip_no and deleted_ind =1 and isnull(recon_flag,'')=''
end
...
PRINT @l_qty
            ...
            --  
              ...
              --        
                 ...
                 --  
					...
					if exists(select * from dptdc_mak where dptdc_id = convert(numeric,@c_dptd_id) and dptdc_deleted_ind in (0,-1) and isnull(DPTDC_EXECUTION_DT,'') ='')
					begin
						print 'innnn1'
						update  dptdc_mak set dptdc_execution_dt = getdate() 
						where dptdc_id = convert(numeric,@c_dptd_id) and dptdc_deleted_ind in (0,-1) and isnull(DPTDC_EXECUTION_DT,'') =''
					end
  ...
  PRINT 'SHILPA'
			   ...
			   if exists (select slip_no from maker_scancopy where ref_no = convert(numeric,@currstring)
						  and recon_flag = 'Y')
				begin
