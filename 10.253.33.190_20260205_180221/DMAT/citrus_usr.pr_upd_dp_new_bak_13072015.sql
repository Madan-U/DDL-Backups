-- Object: PROCEDURE citrus_usr.pr_upd_dp_new_bak_13072015
...
create PROCEDURE [citrus_usr].[pr_upd_dp_new_bak_13072015](@pa_id           varchar(8000)  
    ...
    --  
     -- BEGIN TRANSACTION  
      --      
				...
				BEGIN
							...
							FROM                 CLIENT_MSTR               clim
												,dp_acct_mstr              dpam     
												,tmp_client_res
							WHERE DPAM_CRN_NO=CLIM_CRN_NO AND DPAM_ACCT_NO=tmp_formno
							...
							and ISNULL(DPAM_BBO_CODE,'')=''		and DPAM_SBA_NO  like '120332%' and tmp_formno=DPAM_ACCT_NO
      ...
      FROM   dp_acct_mstr              dpam      
          ,tmp_client_res
           , dp_holder_dtls            dphd  
           where DPAM_ID=DPHD_DPAM_ID and DPAM_ACCT_NO=tmp_formno 
      ...
      UPDATE dpentr     WITH (ROWLOCK)  
						...
						WHERE DPAM_ACCT_NO=ENTR_SBA and DPAM_ACCT_NO=tmp_formno 
      ...
      FROM   dp_acct_mstr              dpam     
           ...
           WHERE ACCAC_CLISBA_ID=DPAM_ID and DPAM_ACCT_NO=tmp_formno 
      --  
      --IF @l_records > 1  
      --BEGIN   
      ----  
...
, tmp_client_res
where  DPAM_ACCT_NO=tmp_formno							
and status ='A'  and DPAM_BBO_CODE=KIT_NO
