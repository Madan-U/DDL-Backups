-- Object: PROCEDURE citrus_usr.pr_ins_upd_ledgerR_CHQRETURN
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--[pr_ins_upd_ledgerR_CHQRETURN] '0','INS','HO','IN300175','904_ACT','042','10000004','418338','11/02/2009','2','6','B|*~|1|*~|100|*~|*|~*P|*~|8180|*~|0|*~||*~|100|*~||*~|ss|*~|01190029547         |*~|*|~*',1,'*|~*','|*~|',''	
--[pr_ins_upd_ledgerR_CHQRETURN]	'0','EDT','HO','2','2','01','1','1','09/07/2008','|*~|1|*~|1020.0000|*~|*|~*P|*~|43356|*~|0|*~||*~|1020.0000|*~||*~|G edited|*~|01190015512         |*~|*|~*',1,'*|~*','|*~|',''	
--[pr_ins_upd_ledgerR_CHQRETURN]	'13|*~|10/07/2008|*~|2|*~|2|*~|01|*~|*|~*12|*~|10/07/2008|*~|2|*~|2|*~|01|*~|*|~*','REJ','VISHAL','0','0','0','','','','0',1,'*|~*','|*~|',''	
--[pr_ins_upd_ledgerR_CHQRETURN]  '16|*~|10/07/2008|*~|2|*~|2|*~|01|*~|*|~*','REJ','VISHAL','0','0','0','','','','0',1,'*|~*','|*~|',''
--	0	INS	HO	IN300175	904_ACT	042	10000004	418338	11/02/2009	2	6	11/02/2009	HAND DELIVERY	4	CASH			0	ONC2P_DP_TRX_SEL		*|~*	|*~|	

CREATE PROCEDURE [citrus_usr].[pr_ins_upd_ledgerR_CHQRETURN](@pa_id  VARCHAR(8000)      
...
DECLARE @@l_acct_type    CHAR(1)    
	...
	BEGIN
		  ...
		  UPDATE FINANCIAL_YR_MSTR SET Ledger_currid = isnull(Ledger_currid,0) + 2,VchNo_Reciept=ISNULL(VchNo_Reciept,0) + 1 WHERE fin_id = @@fin_id and fin_dpm_id = @pa_dpm_id and fin_deleted_ind =1
          SELECT @@l_led_id    = ISNULL(Ledger_currid,0) -2,@pa_voucher_no=VchNo_Reciept FROM FINANCIAL_YR_MSTR WHERE fin_id = @@fin_id and fin_dpm_id = @pa_dpm_id and fin_deleted_ind =1
print @@fin_id
print @pa_dpm_id
		  ...
		  COMMIT TRANSACTION
				...
				SET @@l_amount    = convert(numeric(18,2),citrus_usr.fn_splitval(@@currstring_value,3)) * -1
				SET @@l_flag = citrus_usr.fn_splitval(@@currstring_value,4)  
				...
				ELSE
				BEGIN
					SET @@SSQL = 'INSERT INTO ' + @@ledgertable + '(ldg_id,ldg_dpm_id,ldg_voucher_type,ldg_book_type_cd,ldg_voucher_no,ldg_sr_no,ldg_ref_no,ldg_voucher_dt,ldg_account_id,ldg_account_type,ldg_amount,ldg_narration,ldg_account_no,ldg_instrument_no,ldg_branch_id,ldg_cost_cd_id,ldg_bill_brkup_id,ldg_trans_type,ldg_status,ldg_created_by,ldg_created_dt,ldg_lst_upd_by,ldg_lst_upd_dt,ldg_deleted_ind,ldg_bank_cl_date)
					VALUES(' + convert(varchar,@@l_led_id) + ',' + @pa_dpm_id + ',' + @pa_voucher_type +',''' + @pa_book_type_cd + ''',' + @pa_voucher_no + ',1,''' + @pa_ref_no + ''',convert(datetime,''' + @pa_voucher_dt+ ''',103) ,' + @@l_account_id  + ',''' + @@l_acct_type + ''',' + convert(varchar,@@l_amount) + ',''' + @@l_narration + ''',''' + isnull(@@l_acct_no,'') + ''','''',0,0,0 ,''N'',''P'' ,''' + @pa_login_name + ''',getdate(),''' + @pa_login_name+ ''',getdate() ,' + @@delind + ',convert(datetime,''' + @pa_voucher_dt+ ''',103)' + ')'    
				END
print @@SSQL
				EXEC(@@SSQL)
				...
				SET @@l_amount    = convert(NUMERIC(18,2),citrus_usr.fn_splitval(@@currstring_value2,5))    
...
begin 
SET @@SSQL = 'INSERT INTO ' + @@ledgertable + '(ldg_id,ldg_dpm_id,ldg_voucher_type,ldg_book_type_cd,ldg_voucher_no,ldg_sr_no,ldg_ref_no,ldg_voucher_dt,ldg_account_id,ldg_account_type,ldg_amount,ldg_narration,ldg_account_no,ldg_instrument_no,ldg_branch_id,ldg_cost_cd_id,ldg_bill_brkup_id,ldg_trans_type,ldg_status,ldg_created_by,ldg_created_dt,ldg_lst_upd_by,ldg_lst_upd_dt,ldg_deleted_ind,ldg_bank_cl_date)
				VALUES(' + convert(varchar,@@l_led_id)  + ',' + @pa_dpm_id + ',' + @pa_voucher_type +',''' + @pa_book_type_cd + ''',''' + @pa_voucher_no +''',2,''' + @pa_ref_no + ''',convert(datetime,''' + @pa_voucher_dt+ ''',103) ,' + @@l_account_id  + ',''' + @@l_acct_type + ''',' + convert(varchar,@@l_amount) + ',''' + @@l_narration + ''',''' + isnull(@@l_acct_no,'') + ''',''' + @@l_cheque + ''',0,0,0 ,''N'',''P'' ,''' + @pa_login_name + ''',getdate(),''' + @pa_login_name+ ''',getdate() ,' + @@delind + ',convert(datetime,''' + @pa_voucher_dt+ ''',103)' + ')'     
end 
print @@SSQL
				EXEC(@@SSQL)
