-- Object: PROCEDURE citrus_usr.pr_select_trxgen_cdsl_SP_EDIS
...
--rollback
CREATE PROCEDURE [citrus_usr].[pr_select_trxgen_cdsl_SP_EDIS]  
(  
								...
								BEGIN  
										Select convert(char(2),isnull(excm_short_name,0))  
												...
												order by dptdc_lst_upd_dt 
															...
															--AND    isnull(DPTDC_BROKERBATCH_NO,'') <> ''
															----and    D1.DPTDC_SLIP_NO like 'E%'
															--and D1.dptdc_slip_no in (
															--Select cdsl_trxn_id from E_DIS_PROCESS_DATA with(nolock)
															--union all
															--select DPTDC_SLIP_NO from dptdc_mak I where i.dptdc_slip_no=D1.DPTDC_SLIP_NO and D1.DPTDC_SLIP_NO like 'e%'                
															--)           
								...
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
										...
										SELECT  replace(convert(varchar(11),dptdc_execution_dt,103),'/','')  
												+ isnull(dpam_sba_no,'')  
												+ isnull(dptdc_counter_dp_id,'') + case when dpam_sba_no=dptdc_created_by then case when len(dptdc_counter_demat_acct_no)=8 then  right(isnull(dptdc_counter_demat_acct_no,''),8) else dptdc_counter_demat_acct_no end else isnull(dptdc_counter_demat_acct_no,'') end  
														...
														IF @PA_BATCH_TYPE = 'BN'  
															BEGIN  
																	...
																	+ case when dptdc_internal_trastm in ('CMCM','BOCM') 
																	   ...
																	   DETAILS--space(13) DETAILS  
																	, abs(dptdc_qty) qty
																	, case  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='E' then 'E' when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='F' then 'F' when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='D' then 'D'  else  'Y' end  TRXEFLG --case when dpam_sba_no=dptdc_created_by then 'Y' else  'N' end TRXEFLG --'N' TRXEFLG 
																	,case  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'') in ('D','E','F') then dptdc_slip_no else citrus_usr.fn_get_slip_no_export(dpam_sba_no,dptdc_slip_no) end dptdc_slip_no ---citrus_usr.fn_get_slip_no_export(dpam_sba_no,dptdc_slip_no) dptdc_slip_no 
																	, dptdc_created_by , case when isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'')='' then dptdc_lst_upd_by else isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'') end dptdc_lst_upd_by , case when isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'')<>'' then dptdc_lst_upd_by else '' end verifier
																	,dptdc_id    as USN,''
																	FROM   dp_trx_dtls_cdsl   
																		   left outer join  settlement_type_mstr  A on convert(varchar,A.settm_id) = dptdc_mkt_type and dptdc_mkt_type <> ''
																		   left outer join  settlement_type_mstr  B on convert(varchar,B.settm_id) = DPTDC_OTHER_SETTLEMENT_TYPE and DPTDC_OTHER_SETTLEMENT_TYPE <> ''
...
--																									END  
	...
	+ convert(char(22),isnull(DPTDC_CHQ_REFNO,''))
																			DETAILS
																			, abs(dptdc_qty) qty, 
																			case  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='E' then 'E' when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='F' then 'F'  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='D' then 'D'  else  'N' end  TRXEFLG, --case when dpam_sba_no=dptdc_created_by then 'Y' else 'N' end  TRXEFLG ,
																			case  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'') in ('D','E','F') then dptdc_slip_no else citrus_usr.fn_get_slip_no_export(dpam_sba_no,dptdc_slip_no) end dptdc_slip_no --citrus_usr.fn_get_slip_no_export(dpam_sba_no,dptdc_slip_no) dptdc_slip_no
																			 , dptdc_created_by ,
...
--																																																											END  
	...
	+ convert(char(22),isnull(DPTDC_CHQ_REFNO,''))
                                                   ...
                                                   DETAILS--space(13) DETAILS  
												, abs(dptdc_qty) qty
												,case  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='E' then 'E' when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='F' then 'F' when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='F' then 'F' else  'Y' end  TRXEFLG -- case when dpam_sba_no=dptdc_created_by then 'Y' else  'N' end TRXEFLG  -- 'N' TRXEFLG 
												,case  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'') in ('D','E','F') then dptdc_slip_no else citrus_usr.fn_get_slip_no_export(dpam_sba_no,dptdc_slip_no) end dptdc_slip_no --citrus_usr.fn_get_slip_no_export(dpam_sba_no,dptdc_slip_no) dptdc_slip_no 
												, dptdc_created_by , case when isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'')='' then dptdc_lst_upd_by else isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'') end dptdc_lst_upd_by , case when isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'')<>'' then dptdc_lst_upd_by else '' end verifier
												,dptdc_id   as USN,''
											FROm   dp_trx_dtls_cdsl   
											...
											IF @L_TOT_REC > 0   
												...
												AND		dpam_dpm_id   = case when @L_DPM_ID='3' then dpam_dpm_id else @L_DPM_ID end
															...
															AND    dptdc_request_dt BETWEEN  CONVERT(VARCHAR,CONVERT(DATETIME, @PA_FROM_DT,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@PA_TO_DT,103),106)+' 23:59:59'  
															AND    dptdc_internal_trastm in ('CMBO','CMCM','BOBO','BOCM')  
															AND    dpam_sba_no LIKE CASE WHEN LTRIM(RTRIM(@PA_POOL_ACCTNO))     = '' THEN '%' ELSE 
															@PA_POOL_ACCTNO  END  
												...
												--			Select cdsl_trxn_id from E_DIS_PROCESS_DATA with(nolock)
												--			union all
												--			select DPTDC_SLIP_NO from dptdc_mak I where i.dptdc_slip_no=D1.DPTDC_SLIP_NO 
												--			and D1.DPTDC_SLIP_NO like 'e%'                
												--			)           
...
--																			END  
	...
	+ convert(char(22),isnull(DPTDC_CHQ_REFNO,''))
														...
														, abs(dptdc_qty) qty
														, case  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='E' then 'E' when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='F' then 'F'  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'')='D' then 'D'  else  'N' end  TRXEFLG --case when dpam_sba_no=dptdc_created_by then 'Y' else 'N' end TRXEFLG 
														,case  when isnull(citrus_usr.FN_PREPOSTOFF(dptdc_slip_no),'') in ('D','E','F') then dptdc_slip_no else citrus_usr.fn_get_slip_no_export(dpam_sba_no,dptdc_slip_no) end dptdc_slip_no 
														, dptdc_created_by , case when isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'')='' then dptdc_lst_upd_by else isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'') end dptdc_lst_upd_by , case when isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'')<>'' then dptdc_lst_upd_by else '' end verifier
														,dptdc_id    as USN,''
											FROM   dp_trx_dtls_cdsl   
												   left outer join settlement_type_mstr A on convert(varchar,A.settm_id) = dptdc_other_settlement_type and dptdc_other_settlement_type <> ''
