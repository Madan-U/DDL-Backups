-- Object: PROCEDURE citrus_usr.PR_CHK_APP_IMPORT_SPEEDE
...
CREATE PROCEDURE [citrus_usr].[PR_CHK_APP_IMPORT_SPEEDE] 
...
--  
  ...
  BEGIN TRANSACTION
    ...
    --
      ...
      begin 
         UPDATE filetask
						SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : '
						,      STATUS = 'FAILED' 
						WHERE  task_id = @pa_task_id
         return 
      end 
 ...
 if exists(select dptdc_id from dptdc_mak, TMP_NSDL_BRK_UPD_SPEEDE where dptdc_id = TMPBRK_INT_REF_NO and dptdc_created_by = @pa_login_name)
      begin 
         UPDATE filetask
						SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : '
						,      STATUS = 'FAILED' 
    ...
    select DPTD_ID
			...
			,DPTD_BROKER_INTERNAL_REF_NO
			from dptd_mak 
			where dptd_id in (select TMPBRK_INT_REF_NO  from TMP_NSDL_BRK_UPD_SPEEDE)
			and   case when @pa_auto_yn = 'N' then dptd_deleted_ind else 0 end = 0
			 ...
			 IF @pa_auto_yn = 'Y'
				...
				--
						...
						and   dptd_id in (select TMPBRK_INT_REF_NO  from TMP_NSDL_BRK_UPD_SPEEDE)
