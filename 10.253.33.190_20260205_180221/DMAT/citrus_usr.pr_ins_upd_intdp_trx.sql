-- Object: PROCEDURE citrus_usr.pr_ins_upd_intdp_trx
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[pr_ins_upd_intdp_trx](@pa_id              VARCHAR(8000)  
...
*********************************************************************************
 SYSTEM         : dp
 MODULE NAME    : pr_ins_upd_intdp_trx
...
--
		...
		-- 
	  	...
	  	INSERT INTO #temp_id VALUES(case when citrus_usr.FN_SPLITVAL(@l_str2,1) = 'D' then   citrus_usr.FN_SPLITVAL(@l_str2,2) else citrus_usr.FN_SPLITVAL(@l_str2,7) end)
...
--
		...
		-- 
				SELECT @l_str2 = citrus_usr.FN_SPLITVAL_ROW(@l_str1,@l_counter)
				INSERT INTO #temp_id VALUES(citrus_usr.FN_SPLITVAL(@l_str2,7))
...
declare @l_max_high_val1 numeric
Select @l_max_high_val1 = bitrm_bit_location from bitmap_ref_mstr where bitrm_parent_cd = 'HIGH_VAL_NSDL'    and bitrm_deleted_ind = 1  
...
SET @remainingstring = @pa_id  
						...
						--
								...
								--
								  ...
								  SET @l_action             = citrus_usr.fn_splitval(@currstring_value,1)      
										...
										BEGIN
										--
												SET @l_tr_dpid            = left(citrus_usr.fn_splitval(@currstring_value,2),8)      
												SET @l_tr_account_no      = citrus_usr.fn_splitval(@currstring_value,2)
												SET @l_tr_sett_type       = citrus_usr.fn_splitval(@currstring_value,3)      
												SET @l_tr_setm_no         = citrus_usr.fn_splitval(@currstring_value,4)   
												SET @l_isin               = citrus_usr.fn_splitval(@currstring_value,5)      				     							  
  										SET @l_qty                = citrus_usr.fn_splitval(@currstring_value,6) 
												SET @l_id                 = citrus_usr.fn_splitval(@currstring_value,7) 
												SET @l_qty               = case when @pa_type = 'D' then CONVERT(VARCHAR(25),CONVERT(NUMERIC(18,5),-1*CONVERT(NUMERIC(18,5),citrus_usr.fn_splitval(@currstring_value,6))) ) else CONVERT(NUMERIC(18,5),@l_qty) end
						      set @l_high_val_yn        = case when @l_val_yn = 'Y' then 'Y' else citrus_usr.fn_get_high_val(@l_isin,abs(@l_qty),'HIGH_VALUE','','') end
            --set @l_high_val_yn        = case when @l_val_yn = 'Y' then 'Y' else case when [citrus_usr].[fn_get_DIS_high_nsdl]('SAVE', '', @pa_values) > @l_max_high_val1 then 'Y' else 'N' end end 
              ...
              IF @pa_action = 'EDT' AND @pa_values  = '0'
              BEGIN
              --
																UPDATE  dp_trx_dtls 
														...
														--
														  UPDATE  dp_trx_dtls 
																SET     dptd_dpam_id              = @l_dpam_id
																							...
																							,dptd_mkt_type             = @pa_mkt_type
																							,dptd_settlement_no        = @pa_settlm_no
																							,dptd_counter_demat_acct_no= @l_tr_account_no
																							,dptd_other_settlement_type= @l_tr_sett_type
																							,dptd_other_settlement_no  = @l_tr_setm_no
																							,dptd_counter_dp_id        = @l_tr_dpid
																							,dptd_rmks																	= @pa_rmks
																							,dptd_lst_upd_dt           = getdate()
																							,dptd_lst_upd_by           = @pa_login_name
              ...
              IF EXISTS(select * from dp_trx_dtls where dptd_dtls_id = CONVERT(INT,@currstring) and dptd_deleted_ind = 1) and @l_action in ('A','E')
														BEGIN
														--
																SET @l_deleted_ind = 6
              ...
              SET @@c_access_cursor =  CURSOR fast_forward FOR      
														...
														--    
																...
																--
																END
																ELSE IF exists(select * from dptd_mak where dptd_dtls_id = convert(numeric,@currstring) and dptd_deleted_ind = 6)
																BEGIN
																--
																		UPDATE  dptd 
  ...
  BEGIN
  --
    exec pr_checkslipno '','INT_DP_TRX_SEL', @pa_dpm_dpid,@pa_dpam_acct_no,@pa_slip_no,@pa_login_name,''
  --
  END
--
END
