-- Object: PROCEDURE citrus_usr.Pr_rpt_bill_vishesh
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--[Pr_rpt_bill] 'CDSL',3,'JUN  1 2009','JUN 30 2009 23:59:59','n','N','','','','n','Y',1,'ho|*~|',''	

create Proc [citrus_usr].[Pr_rpt_bill_vishesh]              
@pa_dptype varchar(4),                      
@pa_excsmid int,                      
...
jv_amt numeric(18,2)
 ...
 BEGIN                
	...
	order_by integer,  
	line_no bigint,          
	charge_val numeric(19,2) , tratm_cd varchar(25)         
	)      
   ...
   if @pa_group_cd <> ''      
   begin            
    insert into #tempmaincdsl(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val,tratm_cd)           
    ...
    and (cdshm_tratm_cd in('2246','2277','2280','2201') or isnull(cdshm_charge,0) <> 0)          
...
select CDSHM_DPAM_ID,                        
      DPAM_SBA_NAME,                        
      CDSHM_BEN_ACCT_NO,                                    
      CDSHM_TRATM_DESC=replace(CDSHM_TRATM_DESC,'        ','') + '-' + '(Demat Rejected)' + '-' + DMTS_REJ_REASON ,                            
      CDSHM_TRANS_NO,                                    
      CDSHM_TRAS_DT,                                    
      CDSHM_ISIN,                                    
      isnull(cdshm_opn_bal,0),                          
      CDSHM_QTY,                        
      1,                                  
      CDSHM_ID   ,0,cdshm_tratm_cd                                 
      FROM CDSL_HOLDING_DTLS with(nolock),                        
      #ACLIST account  ,DMT_STATUS_MSTR                                      
      where   CDSHM_DPM_ID = @@dpmid                          
      AND CDSHM_TRAS_DT >=@pa_fromdate AND CDSHM_TRAS_DT <=@pa_todate                                    
      and convert(numeric,CDSHM_BEN_ACCT_NO) between convert(numeric,@pa_fromaccid) and convert(numeric,@pa_toaccid)                                                             
      and cdshm_tratm_cd in('2251') 
      --and right(isnull(CDSHM_INT_REF_NO,''),1)='R'      --// condition for rejected demat records                             
      AND CDSHM_DPAM_ID = account.dpam_id                            
      and (CDSHM_TRAS_DT between eff_from and eff_to)   
      AND  ABS(CDSHM_TRANS_NO)=ABS(DMTS_DMT_REQ_NO)                    
      
      and (isnull(DMTS_REJ_CD,'')<>'' or isnull(DMTS_REJ_REASON,'')<>'')
 end  
 else  
 begin  
PRINT '111111'
...
where  tratm_cd not in ('2246','2277','2280','2201') 
...
IF @pa_transclientsonly <> 'Y'    
   ...
   AND  ISNUMERIC(DPAM_SBA_NO)=1 AND @pa_fromdate  BETWEEN  eff_from AND eff_to 
   and convert(numeric,DPAM_SBA_NO) between convert(numeric,@pa_fromaccid) and convert(numeric,@pa_toaccid)   

   END 	


set @@ssql ='insert into #ledgerjv
	select dpam_id,sum(isnull(ldg_amount,0) )
	from (
	select distinct dpam_id from #tempmaincdsl
	) tmp left outer join Ledger' + convert(varchar,@@Ledgerid) + ' on dpam_id = ldg_account_id 
    and ldg_voucher_dt between ''' + convert(varchar(11),@pa_fromdate,109) + ''' and ''' + convert(varchar(11),@pa_todate,109) 
    + ''' and ldg_deleted_ind = 1 and ldg_voucher_no <> ''0'' and ldg_voucher_type = ''3''
	group by dpam_id'
...
--added by tushar -- adjustment --19032010
insert into #tempmaincdsl(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,charge_val)
select a.dpam_id,dpam_sba_name,dpam_sba_no,'adjustment',null,clic_trans_dt=@pa_todate,null,null,null,2,0,sum(jv_amt)           
from #ledgerjv a,#ACLIST b
where a.dpam_id = b.dpam_id 
group by a.dpam_id,dpam_sba_no,dpam_sba_name
...
--	from (
--	select distinct dpam_id from #tempmaincdsl
--	) tmp left outer join Ledger' + convert(varchar,@@Ledgerid) + ' on dpam_id = ldg_account_id and ldg_ref_no not like ''ac-%'' and ldg_account_type = ''P'' and ldg_voucher_dt <= ''' + convert(varchar(11),@pa_todate,109) + ''' and ldg_deleted_ind = 1
--	group by dpam_id'
--	exec(@@ssql)

--select * into temp_12345  from #tempmaincdsl 
...
set @@ssql ='insert into #ledgerBal
    ...
    and case when month(ldg_voucher_dt) = ' + convert(varchar,month(@pa_fromdate)) + ' and year(ldg_voucher_dt) = ' + convert(varchar,year(@pa_fromdate)) + ' then ldg_ref_no else ''1'' end  not like case when month(ldg_voucher_dt) = ' + convert(varchar,month(@pa_fromdate)) + ' and year(ldg_voucher_dt) = ' + convert(varchar,year(@pa_fromdate)) + ' then ''ac-%'' else ''0'' end 
    and ldg_account_type = ''P'' and ldg_voucher_dt <= ''' + convert(varchar(11),@pa_todate,109)
	...
	CDSHM_TRATM_DESC= ISNULL(replace(lower(ltrim(rtrim(trans_desc))),isnull(dpm_trans_no,''),''),''),              
	dpm_trans_no,                      
