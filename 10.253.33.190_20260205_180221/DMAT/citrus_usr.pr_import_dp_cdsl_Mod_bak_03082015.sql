-- Object: PROCEDURE citrus_usr.pr_import_dp_cdsl_Mod_bak_03082015
...
create PROCeDURE [citrus_usr].[pr_import_dp_cdsl_Mod_bak_03082015]( @pa_crn_no      varchar(8000)                 
                            , @pa_exch        VARCHAR(10)                  
      ...
      BEGIN                  
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
                                ...
                                , MAX(case when clic_mod_action = 'First OfficeNo' then case when isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'N'),'') = '' then '' else isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'Y'),'') end else '' end) --'0' CUST PH 2 INDC                 
                                , MAX(case when clic_mod_action = 'First OfficeNo' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'N'),'') else '' end)  --isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'RES_PH2'),'')--ph2                  
                                , MAX(case when clic_mod_action = 'First ResidenceNo' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,3,'N'),'') else '' end)  --CUST ADDL PHONE             
								...
								, '' --IT CIRCLE
                                , MAX(case when clic_mod_action = 'First EmailId' then CASE WHEN lower(isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'')) <> '' THEN lower(isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCEMAIL1'),'')) ELSE lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'EMAIL1'),'')) END else '' end) --email                  
                                , '' --USER TEXT 1                 
                                , '' --USER TEXT 2                 
                                , '0000' --USER FIELD 3                  
                                , ''                  
                           ...
                           AND    climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
						   AND	  climmod.clic_mod_action in ('C Address','Client Main Properties','General','Signature','Bank','PAN DETAILS','FIRST MOBILENO','First ResidenceNo','First OfficeNo','First EmailId')
                                  ...
                                  ORDER BY Purpose_code  DESC 
											...
											SELECT distinct '08'                  
														...
														FROM   dp_acct_mstr              dpam ,client_list_modified  climmod   ,account_properties accp,EXCH_SEG_MSTR esm      , @crn                       crn           
														 ...
														 AND   climmod.clic_mod_lst_upd_dt              BETWEEN CONVERT(VARCHAR,CONVERT(DATETIME, @pa_from_dt,103),106)+' 00:00:00' AND CONVERT(VARCHAR,CONVERT(DATETIME,@pa_to_dt,103),106)+' 23:59:59'   
                                                     and   (dpam_sba_no =  clic_mod_dpam_sba_no or case when clic_mod_pan_no <> '' then citrus_usr.fn_ucc_entp(dpam_crn_no,'pan_gir_no','') else '0' end = clic_mod_pan_no)              
                                                     AND	  climmod.clic_mod_action in ('MOBILE SMS')
                                                     --and    climmod.clic_mod_action = @pa_mod_typ
                                                     and    climmod.clic_mod_action like case when upper(@pa_mod_typ) = 'ALL' then '%%' else @pa_mod_typ end
                                                     and	isnull(climmod.clic_mod_batch_no,0) = 0
                                                    and    climmod.clic_mod_deleted_ind = 1
      ...
      print 'kkkkkk' 
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
                                ...
                                , MAX(case when clic_mod_action = 'First MobileNo' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,1,'N'),'') else '' end)  --convert(varchar(17),isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'RES_PH1'),''))--ph1                  
                                --, isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'Y'),'')  --'0' CUST PH 2 INDC                 
                                , MAX(case when clic_mod_action = 'First OfficeNo' then case when isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'N'),'') = '' then '' else isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'Y'),'') end else '' end)
                                , MAX(case when clic_mod_action = 'First OfficeNo' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,2,'N'),'') else '' end)  --isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'RES_PH2'),'')--ph2                  
                                , MAX(case when clic_mod_action = 'First ResidenceNo' then isnull(citrus_usr.fn_fetch_ph('PRI',dpam.dpam_id,3,'N'),'') else '' end) --CUST ADDL PHONE             
                                , MAX(case when clic_mod_action = 'contacts' then CASE WHEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCFAX1'),'') <> '' THEN isnull(citrus_usr.fn_acct_conc_value(dpam.dpam_id,'ACCFAX1'),'') ELSE isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'FAX1'),'') END else '' end)--fax                  
                                --, MAX(case when clic_mod_action in ('CLIENT MAIN PROPERTIES','GENERAL') then isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'PAN_GIR_NO',''),'') else '' end)  --pan card                  
