-- Object: VIEW citrus_usr.VW_CLIENT_DP_DATA_VIEW
...
CREATE VIEW  [citrus_usr].[VW_CLIENT_DP_DATA_VIEW]    
AS    
...
,prop.OCCUPATION CM_OCCUPATION--OCCUPATION_CODE       
,TAX_DEDUCTION CM_TAXDEDUCTION       
,PAN_GIR_NO CB_PANNO       
,'' KYC_FLAG       
,C_ADR1 CM_ADD1       
,C_ADR2  CM_ADD2       
,left(C_ADR3,50) CM_ADD3       
,C_ADR_CITY      CM_CITY       
...
,O_PH1 CM_TELE1       
,'' CM_TELE2       
,case when PriPhInd ='M' then PriPhNum when AltPhInd ='M' then AltPhNum  else '' end CM_MOBILE       
,pc1.EMailId CM_EMAIL       
,case when isnull(BANM_NAME,'')='' then (select top 1 i.banm_name from bank_mstr I where i.banm_rtgs_cd=DivIFScd ) else BANM_NAME end  BK_NAME       
,case when ISNULL(banm_branch,'')='' then (select top 1 i.BANM_BRANCH from bank_mstr I where i.banm_rtgs_cd=DivIFScd ) else banm_branch end BANK_BRANCH       
,CASE WHEN ISNULL(TMPBA_DP_BANK_CITY,'') <> '' THEN ISNULL(TMPBA_DP_BANK_CITY,'')  ELSE ISNULL(TMPBA_DP_BANK_CITY,'') END BK_CITY       
,case when isnull(CLIBA_AC_NO,'')='' then DivBankAcctNo else CLIBA_AC_NO end  CM_DIVBANKACNO       
,'' PAYMODE       
...
,'' NOMINEE_RELATION     
,'' CM_CD      
, CASE WHEN ISNULL (CLIBA_AC_TYPE,'') = '' THEN  CASE WHEN  DivAcctType  = '10' THEN 'SAVINGS' WHEN DivAcctType  = '11' THEN 'CURRENT' ELSE 'OTHERS' END  ELSE  CLIBA_AC_TYPE end BANK_AC_TYPE       
,'' STAT_COMM_MODE       
,'CDSL' DP_TYPE       
...
,'' ADDEDBY       
,'' ADDEDON       
,replace(BILL_START_DT,'//','') ACTIVE_FROM       
,'' INACTIVE_FROM       
...
FROM  dps8_pc1 pc1 WITH(NOLOCK)    ,  DP_ACCT_MSTR DPAM WITH(NOLOCK)   
  --LEFT OUTER JOIN    
  LEFT OUTER JOIN     
...
--       BROKERAGE_MSTR  WITH(NOLOCK) ON BROM_ID = CLIDB_BROM_ID      
  ...
  LEFT OUTER JOIN    
  CLIENT_BANK_ACCTS WITH(NOLOCK) ON DPAM_ID = CLIBA_CLISBA_ID AND CLIBA_DELETED_IND = 1  and CLIBA_FLG=1  
  LEFT OUTER JOIN    
  BANK_MSTR    WITH(NOLOCK) ON   CLIBA_BANM_ID      = BANM_ID AND  BANM_DELETED_IND = 1    
LEFT OUTER JOIN     
bank_addresses_dtls WITH(NOLOCK)    on banm_micr = TMPBA_DP_BANK and banm_rtgs_cd = TMPBA_DP_BR    
--        LEFT OUTER JOIN    
...
--  AND   DPAM.DPAM_ENTTM_CD = ENTTM_CD    
  ...
  AND   CLIM_CRN_NO        = DPAM.DPAM_CRN_NO  and pc1.boid = dpam.dpam_sba_no   
 and DPAM.dpam_subcm_cd not in ('392144','022545','413046','392179','392180')    
--- and dpam_bbo_code='S309021'  
