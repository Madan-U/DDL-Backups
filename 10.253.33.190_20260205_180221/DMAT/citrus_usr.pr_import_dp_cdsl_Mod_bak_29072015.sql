-- Object: PROCEDURE citrus_usr.pr_import_dp_cdsl_Mod_bak_29072015
...
create PROCeDURE [citrus_usr].[pr_import_dp_cdsl_Mod_bak_29072015]( @pa_crn_no      varchar(8000)                 
                            , @pa_exch        VARCHAR(10)                  
...
----                         and    climmod.clic_mod_deleted_ind = 1
               ...
               insert into @line5( ln_no                   
									   ...
									   ,max(case when clic_mod_action = 'Client Main Properties' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'RGESS_FLAG',''),'')='NO' THEN 'N' ELSE 'Y' END) else '' end )--RGESS FLAG)
									   , '' --ANNUAL REPORT FLAG
									   ...
									   , max(case when clic_mod_action = 'Client Main Properties' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BSDA',''),'') ='NO' THEN 'N' ELSE 'Y' END) else '' end)--BSDA FLAG
...
and not exists (select 1 from @line2 o where o.acct_no = dpam_sba_no and o.crn_no  = clim_crn_no )
                       ...
                       insert into @line5( ln_no                   
									   ...
									   ,max(case when clic_mod_action = 'Client Main Properties' then (CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'RGESS_FLAG',''),'')='NO' THEN '' ELSE 'Y' END) else '' end )--RGESS FLAG
									   , '' --ANNUAL REPORT FLAG
									   , '' --PLEDGE STANDING INSTRUCTION FLAG
									   , '' --EMAIL RTA DOWNLOAD FLAG
									   --, CASE WHEN isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'BSDA',''),'') ='NO' THEN 'N' ELSE 'Y' END--BSDA FLAG
