-- Object: PROCEDURE citrus_usr.PR_RPT_REVENUESHAREGEN_29122010
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[PR_RPT_REVENUESHAREGEN_29122010]
(
...
--BEGIN TRAN
delete from commition_branchwise_dtls where from_dt>=@PA_BILL_STDT and to_dt<='MAY 31 2010'
...
--BEGIN TRAN
UPDATE  commition_branchwise_dtls SET AMOUNT = AMOUNT - AMOUNT*0.103 WHERE from_dt= @PA_BILL_STDT AND to_dt= @PA_BILL_ENDDT 
--COMMIT

INSERT INTO commition_branchwise_dtls
SELECT * FROM #tdsDATA 

SELECT CONVERT(VARCHAR(11),from_dt,109) from_dt,
CONVERT(VARCHAR(11),to_dt,109) to_dt,
amount,
account_COde,
account_id,'Revenue Sharing' Narration FROM commition_branchwise_dtls 
union all
SELECT CONVERT(VARCHAR(11),getdate(),109) FROM_DT,CONVERT(VARCHAR(11),getdate(),109) TO_DT,'0.00',0,0,'Revenue Sharing' Narration 
union all
SELECT CONVERT(VARCHAR(11),getdate(),109) FROM_DT,CONVERT(VARCHAR(11),getdate(),109) TO_DT,'0.00',0,0,'Revenue Sharing' Narration 
--
ORDER BY ACCOUNT_ID
