-- Object: PROCEDURE citrus_usr.PR_IMPORT_CLIENT_CDSL_DPB9_angel
...
CREATE PROCEDURE  [citrus_usr].[PR_IMPORT_CLIENT_CDSL_DPB9_angel]
			(
			 @pa_exch          VARCHAR(20)    
...
/*changes done by tushar on may 22 2014 for nrn no update if nominee change*/
			...
			--							  when AltPhInd = 'R'  then 'RES_PH2|*~|' else 'RES_PH2|*~|' end +ISNULL(ltrim(rtrim(AltPhNum)),'')+'*|~*'
				...
				SELECT @L_CONC = @L_CONC  +'MOBSMS|*~|'+ISNULL(ltrim(rtrim(PriPhNum)),'')+'*|~*' 
...
--changes done for 408 ------   
		 ...
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'94') + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UnqIdenNum)),'')) + '|*~|*|~*'  FROM DPB9_PC1  WHERE BOID = @C_BEN_ACCT_NO1-- AND ACCPM_PROP_CD  = 'ADHAARFLAG'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'95') + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UniqueId)),'')) + '|*~|*|~*'  FROM DPB9_PC2  WHERE BOID = @C_BEN_ACCT_NO1 --AND ACCPM_PROP_CD  = 'ADHAARSECHLDR'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'96') + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UniqueId)),'')) + '|*~|*|~*'  FROM DPB9_PC3  WHERE BOID = @C_BEN_ACCT_NO1-- AND ACCPM_PROP_CD  = 'ADHAARTHRDHLDR'  
...
--          SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + ISNULL(LTRIM(RTRIM(BEN_RBI_REF_NO)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR WHERE BEN_ACCT_NO = @C_BEN_ACCT_NO AND ENTPM_CD = 'RBI_REF_NO'  
		...
		/*new kyc aug 2021*/
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,'15') + '|*~|' + case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567','0225105','0225104') then '' else  UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_income',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) end    + '|*~|*|~*' FROM DPB9_PC1 , dp_Acct_mstr    WHERE BOID = @C_BEN_ACCT_NO1 and boid = dpam_sba_no  and UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_income',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),''))))  <> '' 
			
		SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,'85') + '|*~|' + case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567','0225105','0225104') then  UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_incomeNI',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) else '' end  + '|*~|*|~*' FROM DPB9_PC1 , dp_Acct_mstr    WHERE BOID = @C_BEN_ACCT_NO1 and boid = dpam_sba_no
		/*new kyc aug 2021*/
...
if exists (select 1 from client_bank_accts ,dp_acct_mstr, dpb9_pc1  where dpam_id = cliba_clisba_id and DPAM_SBA_NO =@c_ben_acct_no1 
	and boid = dpam_sba_no and CLIBA_AC_NO   = DIVBANKACCTNO  and 	 cliba_banm_id = @L_BANM_ID )
begin 
...
and DPAM_SBA_NO =@c_ben_acct_no1 and CLIBA_FLG='1'
				   ...
				   and DPAM_SBA_NO =@c_ben_acct_no1 and  exists (select 1 from DPB9_PC1 I where boid=dpam_sba_no
				   and	CLIBA_AC_NO   = DIVBANKACCTNO )
...
SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(Filler3)),''))='PH' THEN 'PHYSICAL CAS REQUIRED' when  UPPER(ISNULL(LTRIM(RTRIM(Filler3)),''))='NO' then 'CAS NOT REQUIRED' else ''END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'cas_flag' --and Filler3 <> ''
		...
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'77') + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(rel_WITH_BO)),'')) <> '' THEN citrus_usr.fn_get_cd_rel(UPPER(ISNULL(LTRIM(RTRIM(rel_WITH_BO)),''))) ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6  WHERE BOID = @C_BEN_ACCT_NO AND NOM_Sr_No = 1 --AND ACCPM_PROP_CD  = '1ST_NRELATN_BO'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'78') + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(perc_OF_SHARES)),''))<> '' THEN UPPER(ISNULL(LTRIM(RTRIM(perc_OF_SHARES)),'')) ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6  WHERE BOID = @C_BEN_ACCT_NO  AND NOM_Sr_No = 1--AND ACCPM_PROP_CD  = '1ST_NPER_SHR'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'79') + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(RES_SEC_FLg)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(RES_SEC_FLg)),''))='N' THEN 'NO' ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC6  WHERE BOID = @C_BEN_ACCT_NO  AND NOM_Sr_No = 1--AND ACCPM_PROP_CD  = '1ST_NRED_SEC_FLG'

				 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'94') + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UnqIdenNum)),'')) + '|*~|*|~*'  FROM DPB9_PC1  WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD  = 'ADHAARFLAG'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'95') + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UniqueId)),'')) + '|*~|*|~*'  FROM DPB9_PC2  WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD  = 'ADHAARSECHLDR'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'96') + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UniqueId)),'')) + '|*~|*|~*'  FROM DPB9_PC3  WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD  = 'ADHAARTHRDHLDR'  		

	SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'58') + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(filler2)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(filler2)),''))='N' THEN 'NO' ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC1  WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD  = 'Email_st_flag'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'66') + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(filler8)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(filler8)),''))='N' THEN 'NO' ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC1  WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD  = 'RTA_EMAIL'
		

		/*new kyc aug 2021*/
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'102') + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('BOOS',UPPER(ISNULL(LTRIM(RTRIM(AFILLER4)),'')))) + '|*~|*|~*' FROM DPB9_PC1   WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD   = 'BOSTMNTCYCLE'  
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'99') + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('BONAFIDE',UPPER(ISNULL(LTRIM(RTRIM(AFILLER5)),'')))) + '|*~|*|~*' FROM DPB9_PC1   WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD   = 'BOSTMNTCYCLE'  
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'103') + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC',UPPER(ISNULL(LTRIM(RTRIM(PANVerCode)),'')))) + '|*~|*|~*' FROM DPB9_PC1   WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD   = 'BOSTMNTCYCLE'  
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'104') + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC2',UPPER(ISNULL(LTRIM(RTRIM(PANVerCd)),'')))) + '|*~|*|~*' FROM DPB9_PC2   WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD   = 'BOSTMNTCYCLE'  
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,'105') + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC3',UPPER(ISNULL(LTRIM(RTRIM(PANVerCd)),'')))) + '|*~|*|~*' FROM DPB9_PC3   WHERE BOID = @C_BEN_ACCT_NO --AND ACCPM_PROP_CD   = 'BOSTMNTCYCLE'  
...
--          SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + ISNULL(LTRIM(RTRIM(BEN_RBI_REF_NO)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR WHERE BEN_ACCT_NO = @C_BEN_ACCT_NO AND ENTPM_CD = 'RBI_REF_NO'  
      ...
      /*new kyc aug 2021*/
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,'15') + '|*~|' + case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567','0225105','0225104') then '' else  UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_income',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) end    + '|*~|*|~*' FROM DPB9_PC1 , dp_Acct_mstr    WHERE BOID = @C_BEN_ACCT_NO and boid = dpam_sba_no  and UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_income',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),''))))  <> '' 
			
		SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,'85') + '|*~|' + case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567','0225105','0225104') then  UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_incomeNI',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) else '' end  + '|*~|*|~*' FROM DPB9_PC1 , dp_Acct_mstr    WHERE BOID = @C_BEN_ACCT_NO and boid = dpam_sba_no
		/*new kyc aug 2021*/
