-- Object: PROCEDURE citrus_usr.pr_holding_process_cdsl_for_sp
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE procedure [citrus_usr].[pr_holding_process_cdsl_for_sp](@pa_holding_date datetime,@l_dpm_id numeric(10,0),@pa_login_name varchar(25),@pa_count int)
AS 
BEGIN
--
...
-- holding
	...
	select top 1 @l_prev_holding_dt = dphmcd_holding_dt from DP_DAILY_HLDG_CDSL 
    WHERE dphmcd_holding_dt < @l_holding_dt order by dphmcd_holding_dt desc
 ...
 --
		...
		CREATE TABLE #tmp_dp_hldg_cdsl
			...
			GROUP BY tmphldg_dpam_id,tmphldg_dpm_id,tmphldg_boid,tmphldg_isin,tmphldg_holding_dt,TMPHLDG_SETTID_04
 ...
 (dphmcd_dpam_id
  ...
  END
            ...
            AND    tmpcdslhld.TMPHLDG_SETTID_04  = tmp.dphmcd_ctr_settm_id
   ...
   END
	...
	INSERT INTO #tmp_dp_daily_hldg_cdsl
		...
		AND    NOT EXISTS(select tmp.dphmcd_dpam_id from #tmp_dp_daily_hldg_cdsl tmp
                          where tmp.dphmcd_dpam_id = tmphldg_dpam_id and tmp.dphmcd_isin= tmphldg_isin
                          AND   tmp.dphmcd_ctr_settm_id = TMPHLDG_CTR_SETTID)
