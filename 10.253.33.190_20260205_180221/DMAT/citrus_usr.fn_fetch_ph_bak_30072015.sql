-- Object: FUNCTION citrus_usr.fn_fetch_ph_bak_30072015
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE function [citrus_usr].[fn_fetch_ph_bak_30072015](@PA_TAB VARCHAR(25),@pa_id numeric,@pa_ph_count int,@pa_ind_yn char(1))        
...
--        
  ...
  begin      
   ...
   begin        
  if @pa_ph_count = 1        
  set @l_value  =  case when ( isnull(citrus_usr.fn_acct_conc_value(@pa_id,'ACCRES_PH1'),'') <> '' OR isnull(citrus_usr.fn_dp_import_conc(@l_crn_no,'RES_PH1'),'') <> '') then 'R'         
       else case when (isnull(citrus_usr.fn_acct_conc_value(@pa_id,'ACCMOBILE1'),'') <> '' OR isnull(citrus_usr.fn_dp_import_conc(@l_crn_no,'MOBILE1'),'')<>'') then 'M' else '' end end         
  ...
  set @l_value  =  case when  isnull(citrus_usr.fn_acct_conc_value(@pa_id,'ACCOFF_PH1'),'') <> '' OR  isnull(citrus_usr.fn_dp_import_conc(@l_crn_no,'OFF_PH1'),'')<> '' then 'O'  else '' end         
   ...
   begin        
    if @pa_ph_count = 1        
    set @l_value  =  CASE WHEN isnull(citrus_usr.fn_acct_conc_value(@pa_id,'ACCRES_PH1'),'') <> ''  or isnull(citrus_usr.fn_dp_import_conc(@l_crn_no,'RES_PH1'),'') <> ''  then  
                               case when  isnull(citrus_usr.fn_acct_conc_value(@pa_id,'ACCRES_PH1'),'') <> ''    
         ...
         then isnull(citrus_usr.fn_dp_import_conc(@l_crn_no,'RES_PH1'),'')    
                              ...
                              case when isnull(citrus_usr.fn_acct_conc_value(@pa_id,'ACCMOBILE1'),'') <> ''   
