-- Object: PROCEDURE citrus_usr.PR_SELECT_TRXGEN_CDSL_SP_COMMON
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--begin tran
--select * from dp_mstr where default_dp=dpm_excsm_id
--[PR_SELECT_TRXGEN_CDSL_SP_COMMON] 'DESTAT|*~|*|~*','04/01/2012','04/01/2012','1','BN',3,'HO','','N','*|~*','|*~|',''
--rollback
CREATE PROCEDURE [citrus_usr].[PR_SELECT_TRXGEN_CDSL_SP_COMMON]  
(  
								...
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
										SELECT  replace(convert(varchar(11),dptdc_execution_dt,103),'/','')  
												+ isnull(dpam_sba_no,'')  
												+  CONVERT(CHAR(16),isnull(dptdc_counter_dp_id,'') + isnull(dptdc_counter_demat_acct_no,'')  )
								...
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
												Select convert(char(8),replace(convert(varchar(11),dptdc_execution_dt,103),'/',''))  
                                                        ...
                                                        --+ case when isnull(dptdc_other_settlement_no,'') <> '' then convert(char(13),isnull(citrus_usr.[GetCDSLinterdpSettType_SP]('',dptdc_other_settlement_no,A.settm_type_cdsl),'')   +isnull(dptdc_other_settlement_no,''))  else convert(char(13),'') END DETAILS
														, abs(dptdc_qty) qty
											FROM   dp_trx_dtls_cdsl   
...
--UNFRZ
						...
						--  
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
												...
												and d1.fre_deleted_ind = 1
												and d3.dpam_deleted_ind = 1  order by d1.fre_lst_upd_dt 
					...
					END  
						...
						--  
								...
								+'<Qty>' + case when demrm_qty <>0 then convert(varchar(16),convert(numeric(18,3),demrm_qty)) else '0' end  + '</Qty>'
