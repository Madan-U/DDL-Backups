-- Object: PROCEDURE citrus_usr.Pr_Rpt_Summary
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--Pr_Rpt_Holding 'nsdl',4,'Y','','10000004','10000054','','Y',1,'HO|*~|',''
--select * from dp_mstr
create Proc [citrus_usr].[Pr_Rpt_Summary]
@pa_dptype varchar(4),                  
@pa_excsmid int,                  
@pa_asondate char(1),                  
@pa_fordate datetime,                  
 ...
 BEGIN            
  ...
  BEGIN            
      ...
      BEGIN                        
		...
		select dpam_id,DPAM_SBA_NAME AcctName,DPAM_sba_NO AcctNo,BEN_TYPE=BEN.DESCP + CASE WHEN LTRIM(RTRIM(DPDHM_SETT_TYPE)) <> '00' AND ISNULL(LTRIM(RTRIM(DPDHM_SETT_TYPE)),'') <> '' THEN ' (' + DPDHM_SETT_TYPE +'/' + DPDHM_SETT_NO + ')' ELSE '' END 
