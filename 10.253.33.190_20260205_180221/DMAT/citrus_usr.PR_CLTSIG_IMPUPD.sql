-- Object: PROCEDURE citrus_usr.PR_CLTSIG_IMPUPD
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[PR_CLTSIG_IMPUPD]
(
...
DP_ACCT_MSTR D1,TMP_CLTSIGN_NSDL D2,ACCOUNT_DOCUMENTS D3,ACCOUNT_DOCUMENT_MSTR D4,CLIENT_CTGRY_MSTR

WHERE --ACCDOCM_CD ='SIGN_BO'
ACCDOCM_DOC_ID=ACCD_ACCDOCM_DOC_ID AND ACCD_CLISBA_ID=DPAM_ID
 and CLICM_cD =DPAM_CLICM_CD  and ACCDOCM_CLICM_ID=clicm_id
--AND TMPSIGN_AUTH_NM=DPAM_SBA_NO 
and LTRIM(RTRIM(SUBSTRING(TMPSIGN_sba_no,2,16))) = dpam_sba_no
and   exists(select ACCD_ACCDOCM_DOC_ID from account_documents where accd_clisba_id = dpam_id and ACCD_ACCDOCM_DOC_ID = ACCDOCM_DOC_ID )
AND DPAM_DELETED_IND=1 AND DPAM_STAM_CD='ACTIVE'
END
if exists (select * from tmp_cltsign_nsdl where TMPSIGN_AUTH_NM=TMPSIGN_SBA_NO)
begin
UPDATE D3 SET D3.ACCD_BINARY_IMAGE=d2.CLI_IMAGE , D3.ACCD_DOC_PATH='../CCRSDOCUMENTS/' + TMPSIGN_SBA_NO + tmpsign_ref_no  FROM 
DP_ACCT_MSTR_mak D1,TMP_CLTSIGN_NSDL D2,ACCD_MAK D3,ACCOUNT_DOCUMENT_MSTR D4,API_CLIENT_MASTER_SYNERGY_dp,CLIENT_CTGRY_MSTR
...
if exists (select * from tmp_cltsign_nsdl where cli_image is not null)
	...
	,1
    ,cli_image
	from dp_acct_mstr , TMP_CLTSIGN_NSDL, ACCOUNT_DOCUMENT_MSTR,CLIENT_CTGRY_MSTR
