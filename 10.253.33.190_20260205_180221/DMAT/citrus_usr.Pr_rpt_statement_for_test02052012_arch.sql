-- Object: PROCEDURE citrus_usr.Pr_rpt_statement_for_test02052012_arch
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------


CREATE Proc [citrus_usr].[Pr_rpt_statement_for_test02052012_arch]                                
@pa_dptype varchar(4),                                        
@pa_excsmid int,                                        
...
begin  
		...
		insert into #tmphldg    
		 exec  pr_get_holding_fix_latest_BBOCODE_arch   @@DPMID,@pa_fromdate,@pa_todate,@PA_FROMACCID,@PA_TOACCID,@@bbocode,''  
		...
		begin 
		 ...
		 insert into #tmphldg    
		 exec  [pr_get_holding_fix_latest_acrh]   @@DPMID,@pa_fromdate,@pa_todate,@PA_FROMACCID,@PA_TOACCID,''    
		end 
...
begin 
  ...
  begin     
	...
	insert into #vw_fethob    
	exec pr_get_ob_fix_latest_arch @@dpmid,@pa_fromdate,@pa_todate,@pa_fromaccid,@pa_toaccid,''    
  end 
  else 
  begin 
...
print @pa_isincd
	...
	insert into #vw_fethob    
	exec pr_get_ob_fix_byisin_arch @@dpmid,@pa_fromdate,@pa_todate,@pa_fromaccid,@pa_toaccid,@pa_isincd,''    
  end 	
end     
else     
begin  
...
begin     
  ...
  begin 
	...
	begin 
	insert into #vw_fethob    
	exec pr_get_ob_fix_latest_arch @@dpmid,@pa_fromdate,@pa_todate,@l_acct_no,@l_acct_no,''    
	end 
	else 
	begin 
	insert into #vw_fethob    
	exec pr_get_ob_fix_byisin_arch @@dpmid,@pa_fromdate,@pa_todate,@l_acct_no,@l_acct_no,@pa_isincd,''    
	end 
  end 	
...
CREATE TABLE #forstat_cdsl_holding_dtls(  
 ...
 [cdshm_charge] [numeric](18, 5) NULL,  
 [CDSHM_DP_CHARGE] [numeric](18, 5) NULL,  
...
select * from dmat_archival.citrus_usr.cdsl_holding_dtls WITH (NOLOCK)    
...
--  select @pa_prevfromdate = max(dphmcd_holding_dt) from vw_holding_base_all with(nolock) where dphmcd_holding_dt <= convert(varchar(11),@pa_todate,109)    
...
,citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,41,'~')    
   ...
   and CDSHM_TRAS_DT >=@pa_fromdate AND CDSHM_TRAS_DT <=@pa_todate                                        
      and cdshm_tratm_cd in('2246','2277','2201','3102','2270','2220','2251','2202','2205','2280') --'2252'  --,'4456'  '2280', -add 2205    --remove on jan 12 2017,'2262'
...
,citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,41,'~')                                   
  ...
  ,#ACLIST account                                  
      where   CDSHM_DPM_ID = @@dpmid      
   AND CDSHM_TRAS_DT >=@pa_fromdate AND CDSHM_TRAS_DT <=@pa_todate                                        
      and cdshm_tratm_cd in('2246','2277','2201','3102','2270','2220','2251','2202','2205','2280')       --,'2252','4456' '2280', -- 2205 added                                 --remove on jan 12 2017,'2262'
      --and cdshm_tratm_cd in('2246','2277','2201','2205','2220','3102','2252','2270','2280','2262','2251','3202','2202','2212')         
      and convert(numeric,CDSHM_BEN_ACCT_NO) between convert(numeric,@pa_fromaccid) and convert(numeric,@pa_toaccid)                                                                 
   AND CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE ISNULL(CDSHM_SETT_TYPE,'') END = CASE WHEN ISNULL(@PA_SETTM_TYPE,'')='' THEN '0' ELSE @PA_SETTM_TYPE END    
      AND CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE ISNULL(CDSHM_SETT_NO,'') END BETWEEN CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_FR END and CASE WHEN ISNULL(@PA_SETTM_NO_FR,'')='' THEN '0' ELSE @PA_SETTM_NO_To END  
...
where      
tratm_cd not in ('2246','2277','3102','2201','2270','2220','2251','2202','2205','2280') --,'2252','4456' '2280',--added 2205     --remove on jan 12 2017,'2262'
...
--         +convert(numeric(18,3),citrus_usr.fn_splitval_by(trans_desc+'|',14,'|'))    
       else '0' end totalhldg,citrus_usr.fn_ucc_accp(dpam_id,'bbo_code','') bbo_code                                                
 ...
 CLOPM_DT desc)    
   ...
   order by dpam_acctno,dpam_sba_name,order_by,isin_name,isin_cd,convert(datetime,trans_date),exedt,case when isnull(easiflg,'0')='4' then '0' else '1' end,c_d_flag,case when isnumeric(dpm_trans_no) = 1 then convert(numeric,dpm_trans_no) else 1 end  ,cdshm_trg_settm_no           
   end     
...
,  
   ...
   --trans_date,    
   cdshm_trg_settm_no,    
   VALUATION=CONVERT(NUMERIC(18,2),convert(numeric(18,3),case when order_by = '1' then citrus_usr.fn_splitval_by(trans_desc,1,'|') 
else '0' end)*ISNULL(CLOPM_CDSL_RT,0)) ,rate = ISNULL(CLOPM_CDSL_RT,0)   , isnull(tratm_cd ,'')    tratm_cd                                     
