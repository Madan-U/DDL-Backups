-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_Class_tocheck_arch
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_Class_tocheck_arch](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
if @pa_ben_acct_no_fr <> @pa_ben_acct_no_to
begin 
set @pa_dpm_id = 3 
end 
else if @pa_ben_acct_no_fr = @pa_ben_acct_no_to
begin 
select @pa_dpm_id =  dpm_id from dp_mstr where dpm_dpid = left(@pa_ben_acct_no_to,8) and dpm_deleted_ind = 1 
end 

TRUNCATE TABLE tbl_hld_class
...
into #tmp_dp_daily_hldg_cdsl from dmat_archival.citrus_usr.cdsl_holding_dtls      with (nolock)
where  CDSHM_TRAS_DT >= @l_min_trx_dt      
and CDSHM_TRAS_DT <= @pa_to_dt     
and case when @pa_dpm_id ='0' then '0' else       cdshm_dpm_id end = @pa_dpm_id      
...
and case when @pa_dpm_id ='0' then '0' else       dphmcd_dpm_id     end = @pa_dpm_id      
...
INSERT into tbl_hld_class
select dpam_sba_no ,h.* 
from #holdingall h ,dp_acct_mstr 
