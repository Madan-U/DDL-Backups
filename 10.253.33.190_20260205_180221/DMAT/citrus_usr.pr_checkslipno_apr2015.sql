-- Object: PROCEDURE citrus_usr.pr_checkslipno_apr2015
...
create   procedure [citrus_usr].[pr_checkslipno_apr2015]        
(        
...
if not exists(select * from slip_issue_mstr where convert(numeric,@slipno) between convert(numeric,sliim_slip_no_fr) and convert(numeric,sliim_slip_no_to) and SLIIM_SERIES_TYPE = isnull(@@series,''))
	begin
		set @pa_errmsg = 'Slip not issued to client' + '|*~|1' 
		return
	end
...
print @pa_errmsg
    ...
    IF EXISTS (
    SELECT * FROM SLIP_ISSUE_MSTR WHERE   @@sr_no BETWEEN   SLIIM_SLIP_NO_FR and     SLIIM_SLIP_NO_TO 
...
print @l_dpm_id 
    ...
    IF EXISTS(SELECT uses_slip_no FROM used_slip WHERE   (left(uses_trantm_id,3) = left(@trans_code,3) or USES_DPAM_ACCT_NO = 'BOTH')     
    and isnumeric(USES_SLIP_NO )  = 1  
              --and convert(numeric,USES_SLIP_NO ) = convert(numeric,@@sr_no)   
				and USES_SLIP_NO  = @@sr_no
             -- and uses_dpm_id = @l_dpm_id       
...
print @pa_errmsg
    ...
    and SLIIM_SERIES_TYPE=@@series and isnull(SLIIM_EXPIRY_DATE,'')<>''
 --and  convert(varchar(11),getdate(),109) >=convert(varchar(11),SLIIM_EXPIRY_DATE,109)
and getdate()>=convert(varchar(11),SLIIM_EXPIRY_DATE,109)
             )        
    BEGIN       
     set @pa_errmsg =  'This is Old Slip and it is Blocked Now!' + '|*~|1'        
	 print @pa_errmsg
...
--	  if exists(select Panno from View_DMAT_ClientMaster_ClientType where clienttype in ('REM','IFA','BAO','BAF') and Panno=@panno) --[10.228.50.6/dev].msajag.dbo.
--	  begin
