-- Object: PROCEDURE citrus_usr.pr_ins_upd_onmP2C_trx_multi
...
sett_no
 ...
 Target_DP_id  + coldel + Target_Account_no  + coldel + Target_client_name  + coldel + Isin + coldel + Qty + coldel + rowdel A|*~|69874564|*~||*~|236541|*~|IN0019780074|*~|6|*~|*|~*

 
*/
--pr_ins_upd_onmP2C_trx '584','EDT','VISHAL','IN300175','IN504316','1828611','11','2009085','21/05/2009','21/05/2009','E|*~|IN300175|*~|10000037|*~|COIMBATORE CAP (P) LTD|*~|INC000012087|*~|23.000|*~|1040|*~|*|~*','','1','*|~*','|*~|',''	
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_onmP2C_trx_multi](@pa_id              VARCHAR(8000)  
...
*********************************************************************************
 SYSTEM         : dp
 MODULE NAME    : pr_ins_upd_onmP2C_trx
...
DECLARE @t_errorstr      VARCHAR(8000)
      ...
      , @l_tr_cl_name    varchar(100)
      , @l_id         varchar(20)
      , @@c_access_cursor cursor
      , @c_deleted_ind    varchar(20)
      , @c_dptd_id        varchar(20)
      , @l_dpam_acct_no   varchar(16)
      , @l_tr_acct_no     varchar(20)
      	,@l_mkt_type        VARCHAR(20) 
       ,@l_settlm_no		 VARCHAR(20) 
...
--
		...
		,dptd_others_cl_name varchar(100)
		,dptd_status CHAR(1) 
...
--
		...
		-- 
	  	...
	  	INSERT INTO #temp_id VALUES(case when citrus_usr.FN_SPLITVAL(@l_str2,1) = 'D' then   citrus_usr.FN_SPLITVAL(@l_str2,2) else citrus_usr.FN_SPLITVAL(@l_str2,9) end)
...
--
 SELECT * FROM #dptd_mak
		...
		-- 
				...
				INSERT INTO #temp_id VALUES(citrus_usr.FN_SPLITVAL(@l_str2,9))
...
SET @remainingstring = @pa_id  
								...
								--
								  ...
								  SET @l_action             = citrus_usr.fn_splitval(@currstring_value,1)     
								   ...
								   IF @l_action = 'A' OR @l_action ='E'
											BEGIN
											--
													SET @l_tr_dpid            = citrus_usr.fn_splitval(@currstring_value,2) 
													SET @l_tr_acct_no         = citrus_usr.fn_splitval(@currstring_value,3)  
													SET @l_tr_cl_name         = citrus_usr.fn_splitval(@currstring_value,4)   
													SET @l_isin               = citrus_usr.fn_splitval(@currstring_value,5)      				     							  
													SET @l_qty                = citrus_usr.fn_splitval(@currstring_value,6)
                                                    set @l_mkt_type        = citrus_usr.fn_splitval(@currstring_value,7)
                                                    set @l_settlm_no		 = citrus_usr.fn_splitval(@currstring_value,8)
									 ...
									 IF @pa_chk_yn = 0
										...
										--
										  ...
										  AND    accp_value         = @pa_dpam_acct_no
														...
														--
														  UPDATE  dp_trx_dtls 
																SET     dptd_dpam_id              = @l_dpam_id
																							...
																							,dptd_isin                 = @l_isin
																							,dptd_qty                  = @l_qty
																							,dptd_mkt_type             = @l_mkt_type
																							,dptd_settlement_no        = @l_settlm_no
																							,dptd_counter_demat_acct_no  = @l_tr_acct_no
																							,dptd_counter_dp_id        = @l_tr_dpid
																							,dptd_rmks                 = @pa_rmks
																							,dptd_others_cl_name       = @l_tr_cl_name
																							,dptd_lst_upd_dt           = getdate()
																							,dptd_lst_upd_by           = @pa_login_name
																	WHERE  dptd_id                   = @l_id
															 	AND    dptd_deleted_ind          = 1
 ...
 DELETE USES 
              ...
              SET @@c_access_cursor =  CURSOR fast_forward FOR      
														...
														--      
																 BEGIN TRANSACTION
																	...
																	IF EXISTS(select * from dptd_mak where dptd_id = @c_dptd_id and dptd_deleted_ind = 6)
																	BEGIN
																	--
																			UPDATE  dptd 
																			SET     dptd_dpam_id              = dptdm.dptd_dpam_id               
																										...
																										,dptd_others_cl_name       = dptdm.dptd_others_cl_name      
																	...
																	--
																	END
																	ELSE IF exists(select * from dptd_mak where dptd_id = convert(numeric,@c_dptd_id) and dptd_deleted_ind in (0,-1))
																	BEGIN
																	--
  ...
  IF left(ltrim(rtrim(@pa_errmsg)),5) <> 'ERROR'
		BEGIN
		--
				exec pr_checkslipno '','ONP2C_DP_TRX_SEL', @pa_dpm_dpid,@l_dpam_acct_no,@pa_slip_no,@pa_login_name,''
		--
  END
--
END
