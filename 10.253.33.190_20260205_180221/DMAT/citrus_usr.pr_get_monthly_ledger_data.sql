-- Object: PROCEDURE citrus_usr.pr_get_monthly_ledger_data
-- Server: 10.253.33.190 | DB: DMAT
--------------------------------------------------

--exec [pr_get_monthly_ledger_data] 'MAy 01 2013','MAy 31 2013'
CREATE proc [citrus_usr].[pr_get_monthly_ledger_data](@pa_from_dt datetime,@pa_to_dt datetime)
as
begin 
...
truncate  table monthlyledgerdata
...
delete  from monthlyledgerdata where dpcode in (select distinct dpcode from monthlyledgerdata group by dpcode 
having count(1)=1--418595
) and debit = 0 and credit = 0 

delete a from monthlyledgerdata a
