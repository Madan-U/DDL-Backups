-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_Csv_thirdprty
...
EXEC [Pr_Rpt_Holding_Csv]	'CDSL','3','Y','','','','','N',1,'HO|*~|','','','',''	
*/
 
create PROC [citrus_usr].[Pr_Rpt_Holding_Csv_thirdprty]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
DPDHM_ISIN ISINCD,
ISIN_NAME,
CONVERT(NUMERIC(18,3),DPDHM_QTY)  QTY,
...
SELECT 
			...
			DPDHMD_ISIN ISINCD,
			ISIN_NAME,
			CONVERT(NUMERIC(18,3),DPDHMD_QTY) QTY,
...
BEGIN     
			...
			SELECT DISTINCT
			account.DPAM_SBA_NAME,
			account.DPAM_SBA_NO,isnull(DPHMC_ISIN,'') DPHMC_ISIN,
			isnull(ISIN_NAME,'') ISIN_NAME,
			CONVERT(NUMERIC(18,3),DPHMC_CURR_QTY) DPHMC_CURR_QTY,
			CONVERT(NUMERIC(18,3),DPHMC_FREE_QTY) DPHMC_FREE_QTY,
			CONVERT(NUMERIC(18,3),DPHMC_FREEZE_QTY),
			CONVERT(NUMERIC(18,3),DPHMC_PLEDGE_QTY) DPHMC_PLEDGE_QTY,
			...
			,case when ltrim(rtrim(SPARK_PARTY_CODE)) = '0' then '' else ltrim(rtrim(SPARK_PARTY_CODE)) end SPARK_PARTY_CODE  
			--,ISNULL(amount,'0') AMOUNT               
            ,CONVERT(NUMERIC(18,3),DPHMC_CURR_QTY)-CONVERT(NUMERIC(18,0),DPHMC_PLEDGE_QTY) AMOUNT
             ,CMP=(CONVERT(NUMERIC(18,3),DPHMC_FREE_QTY)*ISNULL(convert(numeric(18,2),CLOPM_CDSL_RT),0.00))
             ,address = CITRUS_USR.FN_SPLITVAL(CITRUS_USR.FN_ADDR_VALUE(DPAM_CRN_NO,'COR_ADR1'),1)   + ' '+
	CITRUS_USR.FN_SPLITVAL(CITRUS_USR.FN_ADDR_VALUE(DPAM_CRN_NO,'COR_ADR1'),2)+ ' ' +
	ISNULL(CITRUS_USR.FN_SPLITVAL(CITRUS_USR.FN_ADDR_VALUE(DPAM_CRN_NO,'COR_ADR1'),3),'')  + ' ' +
	...
	CITRUS_USR.FN_SPLITVAL(CITRUS_USR.FN_ADDR_VALUE(DPAM_CRN_NO,'COR_ADR1'),7) 
			FROM 
			dp_acct_mstr c , DP_HLDG_MSTR_CDSL 
			...
			#ACLIST ACCOUNT   LEFT OUTER JOIN  TMP_GET_SPARKPARTYCODE ON account.DPAM_SBA_NO =  ltrim(rtrim(demat_id))                        
            LEFT OUTER JOIN #temptable ON ACCOUNT_ID = account.DPAM_ID
            ...
            AND ISNUMERIC(account.DPAM_SBA_NO)=1                   
			...
			and c.dpam_id=DPHMC_DPAM_ID and dpam_deleted_ind=1                     
			ORDER BY 
			account.DPAM_SBA_NO,ISIN_NAME,DPHMC_ISIN   
     
         END -- @PA_SETTM_TYPE    
     ELSE
     BEGIN  -- if @PA_SETTM_TYPE=1 then we are shwing holding with total valuation
			SELECT DISTINCT
			
			DPAM_SBA_NO
			,case when ltrim(rtrim(SPARK_PARTY_CODE)) = '0' then '' else ltrim(rtrim(SPARK_PARTY_CODE)) end SPARK_PARTY_CODE  
             ,CMP=SUM((CONVERT(NUMERIC(18,3),DPHMC_FREE_QTY)*ISNULL(convert(numeric(18,2),CLOPM_CDSL_RT),0.00)))
			FROM 
			DP_HLDG_MSTR_CDSL 
			LEFT OUTER JOIN  CLOSING_PRICE_MSTR_CDSL ON DPHMC_ISIN = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( select top 1 CLOPM_DT from CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = DPHMC_ISIN and CLOPM_DT <= @PA_FORDATE and CLOPM_DELETED_IND = 1  order by CLOPM_DT desc)
			LEFT OUTER JOIN ISIN_MSTR ON DPHMC_ISIN = ISIN_CD,       
			...
			AND ISNULL(DPHMC_CURR_QTY,0) <> 0                           
			AND DPHMC_ISIN LIKE @PA_ISINCD + '%' 
            GROUP BY  DPAM_SBA_NO,SPARK_PARTY_CODE                        
			ORDER BY 
			DPAM_SBA_NO,SPARK_PARTY_CODE  
...
CONVERT(NUMERIC(18,3),DPHMCD_BORROW_QTY),
		DPAM_ID,
		HOLDING_DT=CONVERT(VARCHAR(11),@@TMPHOLDING_DT,109),case when ltrim(rtrim(SPARK_PARTY_CODE)) = '0' then '' else ltrim(rtrim(SPARK_PARTY_CODE)) end SPARK_PARTY_CODE                                      
FROM 
...
PRINT('D')
  ...
  INSERT INTO #ACLIST SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,EFF_FROM,EFF_TO FROM CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID)    
	...
	SELECT 
				...
				ISIN_NAME,
				CONVERT(NUMERIC(18,3),DPDHM_QTY)  QTY,
				VALUATION= CONVERT(NUMERIC(18,3),DPDHM_QTY*ISNULL(CLOPM_NSDL_RT,0)),CASE WHEN DPDHM_BLK_LOCK_FLG = 'F' THEN 'FREE' WHEN  DPDHM_BLK_LOCK_FLG = 'L' THEN 'LOCK' WHEN DPDHM_BLK_LOCK_FLG = 'B' THEN 'BLOCK' ELSE '' END LOCKFLG,
				HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109),
				CMP=ISNULL(convert(numeric(18,3),CLOPM_NSDL_RT),0.00)   ,
...
PRINT('H')       
 ...
 SELECT TOP 1 @@TMPHOLDING_DT = CASE WHEN DPDHM_HOLDING_DT > @PA_FORDATE THEN @PA_FORDATE ELSE DPDHM_HOLDING_DT END FROM DP_HLDG_MSTR_NSDL WHERE DPDHM_DELETED_IND =1   
  ...
  SELECT 
			...
			CMP=ISNULL(convert(numeric(18,3),CLOPM_NSDL_RT),0.00)    , SETTM_DESC SETTM_DESC, DPDHMD_SETT_NO     setm_no                    
  FROM 
			FN_DAILYHOLDING
(
			@@DPMID,
...
PRINT('F')
   ...
   BEGIN                      
     ...
     SELECT DISTINCT DPAM_SBA_NAME,DPAM_SBA_NO,DPHMC_ISIN,ISIN_NAME,CONVERT(NUMERIC(18,3),DPHMC_CURR_QTY),VALUATION=CONVERT(NUMERIC(18,3),DPHMC_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0)), CONVERT(NUMERIC(18,3),DPHMC_FREE_QTY),CONVERT(NUMERIC(18,3),DPHMC_FREEZE_QTY)      
   ...
   INSERT INTO #ACLIST SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,EFF_FROM,EFF_TO FROM CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID)    
     ...
     ,CONVERT(NUMERIC(18,3),DPHMCD_ELIMINATION_QTY),CONVERT(NUMERIC(18,3),DPHMCD_EARMARK_QTY),CONVERT(NUMERIC(18,3),DPHMCD_AVAIL_LEND_QTY),CONVERT(NUMERIC(18,3),DPHMCD_LEND_QTY),CONVERT(NUMERIC(18,3),DPHMCD_BORROW_QTY),DPAM_ID,HOLDING_DT=CONVERT(VARCHAR(11),@@TMPHOLDING_DT,109),CMP=ISNULL(convert(numeric(18,3),CLOPM_CDSL_RT),0.00)                                     
...
END              
	--EXEC [citrus_usr].[pr_import_property_acct_NEW] 'BBO_CODE' --added on 11/07/2011 for bbo code updation 
