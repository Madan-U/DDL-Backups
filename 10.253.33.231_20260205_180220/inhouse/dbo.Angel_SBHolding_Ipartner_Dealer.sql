-- Object: PROCEDURE dbo.Angel_SBHolding_Ipartner_Dealer
-- Server: 10.253.33.231 | DB: inhouse
--------------------------------------------------

  
/*            
EXEC Angel_SBHolding_Ipartner_Dealer 'CBHO001', 'A100706', 'SB'        
*/            
CREATE PROC [dbo].[Angel_SBHolding_Ipartner_Dealer]            
(            
...
EXEC Angel_SBHolding_Ipartner_Dealer_NEW @CODE,@PARTY,@ACCESSTO
...
IF LTRIM(RTRIM(@ACCESSTO)) = 'Dealer'            
BEGIN            
 INSERT INTO #Y            
 SELECT DISTINCT X.PARTY_CODE,LONG_NAME,BRANCH_CD, EMP_NO SUB_BROKER            
 FROM [196.1.115.132].RISK.DBO.CLIENT_DETAILS X WITH (NOLOCK)             
 inner join [196.1.115.132].[Mimansa].[dbo].[Vw_B2B_CurrentClientDealerMapping] DM  
 ON DM.party_code=X.PARTY_CODE  
 WHERE EMP_NO LIKE @CODE AND X.PARTY_CODE LIKE @PARTY            
END            
...
(SELECT DISTINCT SCRIPNAME,ISIN FROM [196.1.115.197].MSAJAG.DBO.V_ISIN WITH (NOLOCK))S    
 ...
 FROM HOLDING WITH (NOLOCK)  ,  #party Y
