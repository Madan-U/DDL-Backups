-- Object: PROCEDURE citrus_usr.Usp_buyback_dp_holding_eliQty
-- Server: 10.253.33.231 | DB: DMAT
--------------------------------------------------

    
--exec Usp_buy_back_dp_holding 'INE285J01028'    
CREATE proc [citrus_usr].[Usp_buyback_dp_holding_eliQty]     
(    
@ISIN VARCHAR(20) = ''   
)    
AS    
BEGIN    
 ...
 from KYC1DB.[AngelBrokingWebDB].dbo.tbl_buyback_scripmaster  with (nolock) where convert(date,getdate()) between convert(date,fromdate) and convert(date,Todate)
...
CONVERT(DECIMAL(18,2),Valuation) as Valuation,HOLDINGDT into   #buy_back_ISIN from citrus_usr.[HOLDING] A with (nolock)  
