-- Object: PROCEDURE citrus_usr.PR_NSDL_IMP_TRX_SPEEDe
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--ROLLBACK TRAN
--PR_NSDL_IMP_TRX 'HO','2000640' ,'4','','','',16,''
--update TMP_DP_TRX_DTLS_SPEEDe set tmpdptd_acct_id ='12345678' , TMPDPTD_INTERNAL_REF_NO = ''
...
CREATE  PROCEDURE [citrus_usr].[PR_NSDL_IMP_TRX_SPEEDe] (  
...
IF EXISTS(SELECT TOP 1 ISNULL(DPTD_ID,0) FROM DP_TRX_DTLS , DP_ACCT_MSTR WHERE DPTD_BROKERBATCH_NO IN (SELECT TMPDPTD_BROKERBATCH_NO FROM TMP_DP_TRX_DTLS_SPEEDe) AND DPAM_ID = DPTD_DPAM_ID AND DPAM_DPM_ID = @l_dpm_id AND DPAM_DELETED_IND = 1 AND DPTD_DELETED_IND = 1 ) 
  OR EXISTS(SELECT TOP 1 ISNULL(DPTD_ID,0) FROM DPTD_MAK, DP_ACCT_MSTR WHERE DPTD_BROKERBATCH_NO IN (SELECT TMPDPTD_BROKERBATCH_NO FROM TMP_DP_TRX_DTLS_SPEEDe) AND DPAM_ID = DPTD_DPAM_ID AND DPAM_DPM_ID = @l_dpm_id AND DPAM_DELETED_IND  = 1 AND DPTD_DELETED_IND = 0 )
BEGIN
--
...
--
END
/*ELSE IF NOT EXISTS(SELECT TOP 1 ISNULL(DPAM_ID,0) FROM DP_ACCT_MSTR WHERE DPAM_SBA_NO IN (SELECT TMPDPTD_ACCT_ID FROM TMP_DP_TRX_DTLS_SPEEDe) AND DPAM_DPM_ID = @L_DPM_ID AND DPAM_DELETED_IND =1)
BEGIN
--  
  SELECT DISTINCT TMPDPTD_ACCT_ID , ERROR = 'C' FROM TMP_DP_TRX_DTLS_SPEEDe
  WHERE TMPDPTD_ACCT_ID NOT IN (SELECT DPAM_sba_NO FROM DP_ACCT_MSTR WHERE DPAM_DELETED_IND = 1 )
--
END
ELSE*/

    select @l_err_mstr = isnull(replace(citrus_usr.fn_merge_str_Speede('NSDL_IMP',0,''),',',''),'')
...
--    select @l_err_mstr = isnull(replace(citrus_usr.fn_merge_str_Speede('NSDL_CMBP',0,''),',',''),'')
--
--				IF @l_err_mstr  <> ''
--				BEGIN
--				--
...
--    END
    ...
    select @l_err_mstr = isnull(replace(citrus_usr.fn_merge_str_Speede('NSDL_ACCT',0,''),',',''),'')
    ...
    select @l_err_MISSING_ISIN = isnull(replace(citrus_usr.fn_merge_str_Speede('NSDL_ISIN',0,''),',',''),'')
...
BEGIN
--
  SELECT distinct IDENTITY(INT, 1, 1) ID1,  TMPDPTD_SLIP_NO DPTD_SLIP_NO INTO #tmp_dptd FROM TMP_DP_TRX_DTLS_SPEEDe
...
select @l_id1 = count (distinct TMPDPTD_SLIP_NO) from TMP_DP_TRX_DTLS_SPEEDe
select @l_id2 = count (*) from TMP_DP_TRX_DTLS_SPEEDe
...
select ACCP_CLISBA_ID, accp_value from account_properties where ACCP_ACCPM_PROP_CD = 'CMBP_ID' and isnull(accp_value ,'') <> ''
  ...
  FROM    TMP_DP_TRX_DTLS_SPEEDe TMP
...
set @L_ERR_MISSING_ISIN = ''
    if exists(select TMPDPTD_SLIP_NO from TMP_DP_TRX_DTLS_SPEEDe, used_slip where USES_TRANTM_ID = TMPDPTD_TRASTM_CD  and rtrim(ltrim(TMPDPTD_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO)))
    begin

     SELECT distinct @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  + TMPDPTD_SLIP_NO + ',' from TMP_DP_TRX_DTLS_SPEEDe, used_slip where USES_TRANTM_ID = TMPDPTD_TRASTM_CD  and rtrim(ltrim(TMPDPTD_SLIP_NO)) = ltrim(rtrim(USES_SLIP_NO))
...
--	begin
--	  if exists(select TMPDPTD_INTERNAL_TRASTM from TMP_DP_TRX_DTLS_SPEEDe where TMPDPTD_INTERNAL_TRASTM <> 'C2P')
--	  begin
--	   if exists(select TMPDPTD_SLIP_NO from TMP_DP_TRX_DTLS_SPEEDe where not exists(select sliim_id from slip_issue_mstr , transaction_sub_type_mstr 
...
--	   begin
--
--        SELECT distinct @L_ERR_MISSING_ISIN = ISNULL(@L_ERR_MISSING_ISIN,'')  +  TMPDPTD_SLIP_NO + ':' + dpam_sba_no  + ',' from TMP_DP_TRX_DTLS_SPEEDe , dp_Acct_mstr 
...
--	end  
  ...
  INSERT INTO DPTD_MAK
	 ...
	 ,DPTD_BATCH_NO
		...
		TMPDPTD_TRANS_NO ,  
		CASE WHEN TMPDPTD_TRASTM_CD  = '904'  THEN cASE WHEN TMPDPTD_INTERNAL_TRASTM = 'P2C' THEN  citrus_usr.fn_merge_str_Speede( 'settm_type_NSDL',0,TMPDPTD_MKT_TYPE)  ELSE '' END ELSE  citrus_usr.fn_merge_str_Speede( 'settm_type_NSDL',0,TMPDPTD_MKT_TYPE) END,  
        ...
        CASE WHEN TMPDPTD_TRASTM_CD  = '904'  THEN CASE WHEN TMPDPTD_INTERNAL_TRASTM = 'C2P' THEN   citrus_usr.fn_merge_str_Speede( 'settm_type_NSDL',0,TMPDPTD_MKT_TYPE) ELSE '' END ELSE  citrus_usr.fn_merge_str_Speede( 'settm_type_NSDL',0,TMPDPTD_OTHER_SETTLEMENT_TYPE) END,  
		...
		@PA_LOGIN_NAME,  
		GETDATE() ,  
		case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPDPTD_ACCT_ID,convert(datetime,TMPDPTD_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPDPTD_ISIN,abs(TMPDPTD_QTY),'HIGH_VALUE','','') end = 'Y' then -1 else 0 end,
		TMPDPTD_TRASTM_CD ,  
