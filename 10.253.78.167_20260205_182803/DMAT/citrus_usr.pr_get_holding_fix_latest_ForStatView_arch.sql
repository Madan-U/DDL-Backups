-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_ForStatView_arch
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_ForStatView_arch](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
into #tmp_vw_holding_base_all from dmat_archival.citrus_usr.cdsl_holding_dtls      with (nolock)
...
select cdshm_dpam_id , cdshm_isin , sum(cdshm_qty) qty ,cdshm_trans_no into #trasdmtpenfing  from dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock)
...
select cdshm_dpam_id , cdshm_isin , sum(cdshm_qty  ) qty ,cdshm_trans_no into #trasdmtpenfing1  from dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock)
...
WHERE NOT EXISTS(SELECT 1 FROM dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock) WHERE CDSHM_DPAM_ID = DPHMCD_DPAM_ID AND DPHMCD_ISIN = CDSHM_ISIN 
...
FROM #FINALDEMATHOLDI 
WHERE EXISTS(SELECT 1 FROM dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock) WHERE CDSHM_DPAM_ID = DPHMCD_DPAM_ID AND DPHMCD_ISIN = CDSHM_ISIN 
AND CDSHM_TRAS_DT < =@PA_TO_DT AND CDSHM_TRATM_cD = '2202' AND CDSHM_TRANS_NO = TRANS_NO)
AND DMATPENDING <> 0
GROUP BY  DPHMCD_DPAM_ID , DPHMCD_ISIN
HAVING SUM(DMATPENDING) <> 0
...
INTO #TEMPDATA2 from   dmat_archival.citrus_usr.cdsl_holding_dtls                     with (nolock)
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_cr FROM dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock) WHERE 
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_dr FROM dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock)
WHERE CDSHM_TRATM_cD  in ('2261', '2262') --=case when CDSHM_CDAS_TRAS_TYPE IN ('21','22','23') then '2262' else '2277' end 
and CDSHM_TRAS_DT between  @l_min_trx_dt and @pa_to_dt
AND CDSHM_CDAS_TRAS_TYPE IN ('12','21','22','23')
GROUP BY CDSHM_DPAM_ID ,  CDSHM_ISIN 
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_dr_remat FROM dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock)
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #locindata_dr_remat_cr FROM dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock)
WHERE CDSHM_TRATM_cD  = '2262' --=case when CDSHM_CDAS_TRAS_TYPE IN ('21','22','23') then '2262' else '2277' end 
and CDSHM_TRAS_DT between  @l_min_trx_dt and @pa_to_dt
...
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #availlenddata FROM dmat_archival.citrus_usr.cdsl_holding_dtls with (nolock)
