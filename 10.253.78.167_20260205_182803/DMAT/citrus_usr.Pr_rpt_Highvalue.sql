-- Object: PROCEDURE citrus_usr.Pr_rpt_Highvalue
...
CREATE Proc [citrus_usr].[Pr_rpt_Highvalue]              
@pa_dptype varchar(4),              
@pa_excsmid int,              
...
BEGIN              
 ...
 declare @@l_child_entm_id      numeric  , @pa_dorfromdate datetime             
 ...
 END              
  ...
  INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,EFF_TO FROM citrus_usr.fn_acct_list_disdrf (@@dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id)		
...
CREATE TABLE  #TEMPLASTTRX
(SBANO VARCHAR(16)
,SBANAME VARCHAR(1000)
,LASTTRXDT VARCHAR(100)--,ACCP_VALUE VARCHAR(1000)
)

set @pa_dorfromdate=dateadd(dd,-1,@pa_fromdate)
INSERT INTO #TEMPLASTTRX
EXEC  PR_RPT_DORMANT_CLIENTS 'CDSL',3,@pa_dorfromdate
,6,'1'
,'HO|*~|',''

DELETE FROM #TEMPLASTTRX WHERE LASTTRXDT ='NA'

---select * from #TEMPLASTTRX where sbano = '1203320000122471'
		 ...
		 and bitrm_deleted_ind = 1        
		   ...
		   dpam_sba_name,dpam_sba_no
+ '<font color="red"><b>' + CASE WHEN abs(dptdc_qty)*isnull(clopm_cdsl_rt,0) >= @@highvalue         THEN 
			': HIGH VALUE'
WHEN ABS(DATEDIFF (MM, @pa_fromdate , CONVERT(DATETIME,LASTTRXDT,106))) > = '6'   THEN ': CLIENT IS DORMANT (LAST TRANS DT : ' + LASTTRXDT + ')'

WHEN ISNULL(DPTDC_BROKERBATCH_NO ,'') <> ''  THEN ': BROKER FILE'
ELSE '' END  + '</b></font>' dpam_sba_no
,        
		   ...
		   value = 
			CONVERT(VARCHAR,convert(numeric(18,3),abs(dptdc_qty*isnull(clopm_cdsl_rt,0)))        ) 
		   FROM          
		   CLOSING_LAST_CDSL right outer join dp_trx_dtls_cdsl MSTR  on  clopm_isin_cd = dptdc_isin        
		   left outer join isin_mstr on dptdc_isin = isin_cd,              
		   #ACLIST account   
LEFT OUTER JOIN #TEMPLASTTRX ON SBANO = DPAM_SBA_NO 
, (SELECT DPTDC_DPAM_ID MAKDPAM_ID , DPTDC_SLIP_NO MAKSLIPNO , DPTDC_ISIN MAKISIN FROM DPTDC_MAK MAK  
							    WHERE ISNULL(DPTDC_MID_CHK ,'') <> '' )     MAKDTLS

		   where  DPTDC_SLIP_NO =   MAKSLIPNO  AND   MAKDPAM_ID = DPTDC_DPAM_ID AND DPTDC_ISIN= MAKISIN       
		   AND  DPTDC_REQUEST_DT >=@pa_fromdate AND DPTDC_REQUEST_DT <=@pa_todate              
		   --and abs(dptdc_qty)*isnull(clopm_cdsl_rt,0) >= @@highvalue        
		   and convert(numeric,dpam_sba_no) between convert(numeric,@pa_fromaccid) and convert(numeric,@pa_toaccid)               
		   and dptdc_isin like @pa_isincd + '%' 
AND (abs(dptdc_qty)*isnull(clopm_cdsl_rt,0) >= @@highvalue               OR  ABS(DATEDIFF (MM, @pa_fromdate , CONVERT(DATETIME,LASTTRXDT,106))) > = '6' ) 
