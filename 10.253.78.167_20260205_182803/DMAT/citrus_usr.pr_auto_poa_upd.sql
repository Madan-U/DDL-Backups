-- Object: PROCEDURE citrus_usr.pr_auto_poa_upd
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

CREATE proc [citrus_usr].[pr_auto_poa_upd]
as
begin 
...
print @l_lst_mig
        ...
        --  
			...
			+'|*~|'+ltrim(rtrim(dpam_sba_no))+'|*~|'+'0'+'|*~|'+'0|*~|0|*~|0|*~|'+'A*|~*'    
			 ...
			 and not exists (select 1 from dps8_pc5  main where main.boid = mig.boid 
...
insert into client_list_modified	
select @c_ben_acct_no1,'','POA ACTIVATION',getdate(),getdate(),'MIG',getdate(),'MIG',getdate(),1,0
where not exists (select 1 from client_list_modified where  clic_mod_dpam_sba_no = @c_ben_acct_no1 and clic_mod_action = 'POA ACTIVATION'
--and clic_mod_batch_no = '0' and  clic_mod_from_dt > = 'Jan 21 2023'
)
