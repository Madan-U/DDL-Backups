-- Object: PROCEDURE citrus_usr.pr_get_invzero_zeroinv
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--exec pr_get_invzero_zeroinv 'INVTOZERO'
--exec pr_get_invzero_zeroinv 'ZEROTOINV'
CREATE proc [citrus_usr].[pr_get_invzero_zeroinv](@pa_tab varchar(100),@pa_dt varchar(100))
as
begin 
...
create table #final (boid varchar(16))
insert into #final
exec pr_get_client_list @pa_dt-- 'feb 29 2016'
...
--select * from #final
				...
				, CONVERT (NUMERIC (18,2),ISNULL(rate,0))* ISNULL(DPHMCD_FREE_QTY,0) [Holding Value]
				...
				,BROKERAGE_MSTR , #final
				where boid = DPAM_SBA_NO  and DPAM_ID = ACCP_CLISBA_ID  and CLIDB_DPAM_ID = DPAM_ID 
				...
				when [brom_desc] ='TRADERS' then 'ZEROTRD' 
				when [brom_desc] ='VERSION 2.5 NORMAL' then 'ZEROINV' 
				--when [brom_desc] ='ANGEL ELITE' then 'ZEROINV' 
				--when [brom_desc] ='ANGEL CLASSIC' then 'ZEROINV' 
				when [brom_desc] ='Itrade Prime VBB' then 'ZERO ITRADE PRIME VBB'
				when [brom_desc] ='ANGEL PREMIER VBB' then 'ZERO ANGEL PREMIER VBB'
				when [brom_desc] ='ANGEL PREFERRED VBB' then 'ZERO ANGEL PREFFERRED VBB'
				when [brom_desc] ='ANGEL CLASSIC 699 VBB' then 'ZERO ANGEL CLASSIC 699 VBB'
				when [brom_desc] ='ANGEL CLASSIC 10000 VBB' then 'ZERO ANGEL CLASSIC 10000 VBB'
				when [brom_desc] ='DIY 699' then 'ZERO DIY699'
				when [brom_desc] ='ANGEL ELITE' then 'ZERO ANGEL ELITE'
				when [brom_desc] ='ANGEL PREFERRED' then 'ZERO ANGEL PREFFERRED'
				when [brom_desc] ='ANGEL CLASSIC' then 'ZERO ANGEL CLASSIC'
				when [brom_desc] ='ANGEL PREMIER' then 'ZERO ANGEL PREMIER'

								
				else [brom_desc] end  [TO BE CHANGED]
...
-- below line is aded on 31082018 to avoid client which write off in for Zero scheme but not to convert normal clients - vishal ,jagan kadam - change by yogesh
			delete from #account_properties1 where accp_Clisba_id in (select dpam_id from ytemp31082018_toberemovedfromzerotoinv )
-- above line is aded on 31082018 to avoid client which write off in for Zero scheme but not to convert normal clients- vishal ,jagan kadam - change by yogesh


				select DPHMCD_DPAM_ID, sum(rate*DPHMCD_FREE_QTY ) val into #hldg from  holdingallforview  
				group by dphmcd_dpam_id  having sum(rate*DPHMCD_FREE_QTY)>750



 
				select LDG_ACCOUNT_ID, SUM(ldg_amount) os  into #ledger from ledger10 where ldg_deleted_ind = 1 group by LDG_ACCOUNT_ID
				 
				 
				

				select  DPHMCD_DPAM_ID,val,os into #maindpam from #hldg, #ledger where LDG_ACCOUNT_ID =DPHMCD_DPAM_ID and val > abs(os) and os < 0 
				
			
				
				insert into #maindpam
				select LDG_ACCOUNT_ID,0,os  from #ledger where   os >= 0
				and not exists (select 1 from #maindpam a where a.DPHMCD_DPAM_ID=LDG_ACCOUNT_ID)
				...
				, CONVERT(NUMERIC (18,2),isnull(citrus_usr.fn_get_ob(DPAM_ID ),0)) [Outstanding Amount]
				,brom_desc
				 into #tempdatafinal1 from dp_acct_mstr  with(nolock), #maindpam
				 --, (select DPHMCD_DPAM_ID, sum(rate*DPHMCD_FREE_QTY ) val from  holdingallforview  
				 --group by dphmcd_dpam_id  having sum(rate*DPHMCD_FREE_QTY )>750 ) holding 
				...
				and brom_desc like '%zero%' and CLIDB_BROM_ID not in (61,62) and DPHMCD_DPAM_ID = dpam_id
				and clidb_deleted_ind = '1' --and entr_deleted_ind = '1' and dpam_deleted_ind = '1'
...
--				when [brom_desc] = 'ZEROTRD'  then 'TRADERS'  else [brom_desc] end from #tempdatafinal1
			...
			,[Class transaction status]
				...
				when [brom_desc] = 'ZEROCOMI' then  'COMMINV' 
				when [brom_desc] = 'ZEROCORP' then  'INV CORP'
				when [brom_desc] = 'ZEROINV' then 'VERSION 2.5 NORMAL'
				when [brom_desc] = 'ZEROJK' then   'VERSION 2.5 NORMAL' 
				when [brom_desc] = 'ZERONBCC' then 'NBC CORP' 
				when [brom_desc] = 'ZERONBCI' then 'NBC' 
				when [brom_desc] = 'ZERONBFCIN'   then 'NBFC INV'
				when [brom_desc] = 'ZEROTRD'  then 'TRADERS'  
				when [brom_desc] ='ZERO ITRADE PRIME VBB' then 'Itrade Prime VBB'
				when [brom_desc] ='ZERO ANGEL PREMIER VBB' then 'ANGEL PREMIER VBB'
				when [brom_desc] ='ZERO ANGEL PREFFERRED VBB' then 'ANGEL PREFERRED VBB'
				when [brom_desc] ='ZERO ANGEL CLASSIC 699 VBB' then 'ANGEL CLASSIC 699 VBB'
				when [brom_desc] ='ZERO ANGEL CLASSIC 10000 VBB' then 'ANGEL CLASSIC 10000 VBB'
				when [brom_desc] ='ZERO DIY699' then 'DIY 699'
				when [brom_desc] ='ZERO ANGEL ELITE' then 'ANGEL ELITE'
				when [brom_desc] ='ZERO ANGEL PREFFERRED' then 'ANGEL PREFERRED'
				when [brom_desc] ='ZERO ANGEL CLASSIC' then 'ANGEL CLASSIC'
				when [brom_desc] ='ZERO ANGEL PREMIER' then 'ANGEL PREMIER'
				...
				when [brom_desc] ='ZERO ANGEL CLASSIC' then 'ANGEL CLASSIC'
				when [brom_desc] ='ZERO ANGEL PREMIER' then 'ANGEL PREMIER'
				 else [brom_desc] end <> [brom_desc]  group by  
				[Demat account no]
				,[BBO code]
				,[Account activation date / AMC date]
				,[Process run date]   
