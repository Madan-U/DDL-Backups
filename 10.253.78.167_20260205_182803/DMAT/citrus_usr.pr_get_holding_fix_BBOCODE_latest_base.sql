-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_BBOCODE_latest_base
...
CREATE proc [citrus_usr].[pr_get_holding_fix_BBOCODE_latest_base](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
,@pa_ben_acct_no_fr varchar(16),@pa_ben_acct_no_to varchar(16),@PA_BBO VARCHAR(100),@pa_out  varchar(8000) out)      
as      
begin       
  

declare @mintrxdt datetime 
set @mintrxdt  =''
select @mintrxdt  = min(cdshm_tras_dt) from cdsl_holding_dtls_sushant
declare @minholdingdt datetime 
set @minholdingdt  =''
select @minholdingdt   = min(dphmcd_holding_dt) from dp_daily_hldg_cdsl_last_base_last_base
print @mintrxdt  
print @minholdingdt   
...
into #tmp_dp_daily_hldg_cdsl_last_base from cdsl_holding_dtls      with (nolock), #CLIENTLIST
where  CDSHM_TRAS_DT >= @mintrxdt      
...
insert into #tmp_dp_daily_hldg_cdsl_last_base     
...
,convert(numeric(18,5),0) DPHMCD_LEND_QTY    
,convert(numeric(18,5),0) DPHMCD_BORROW_QTY    
from dp_daily_hldg_cdsl_last_base a with (nolock) ,dp_acct_mstr  with (nolock), #CLIENTLIST where dphmcd_holding_dt =@minholdingdt    
...
and not exists(select b.dphmcd_dpam_id,b.dphmcd_isin,b.dphmcd_dpm_id from #tmp_dp_daily_hldg_cdsl_last_base b with (nolock) where a.dphmcd_dpm_id = b.dphmcd_dpm_id    
and a.dphmcd_dpam_id = b.dphmcd_dpam_id and a.dphmcd_isin = b.dphmcd_isin)    
                      
     
    
    
--create index ix_1 on #tmp_dp_daily_hldg_cdsl_last_base(DPHMCD_DPM_ID,DPHMCD_DPAM_ID,dphmcd_isin)                    
--select * into #dematpendin from cdsl_holding_dtls     with(nolock)     
--WHERE CDSHM_TRAS_DT  >=@mintrxdt and CDSHM_TRAS_DT   <= @pa_to_dt    
...
from citrus_usr.dp_daily_hldg_cdsl_last_base,dp_acct_mstr, #CLIENTLIST with(nolock)                      
where DPHMCd_HOLDING_DT =@minholdingdt    
...
from #tmp_dp_daily_hldg_cdsl_last_base   a with (nolock), #finalholding f    with (nolock)
...
into #holdingdmtpenfing from  dp_daily_hldg_cdsl_last_base with (nolock), DP_aCCT_MSTR  with (nolock), #CLIENTLIST
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt=@minholdingdt
--and dpam_sba_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
and dphmcd_dpm_id = @PA_DPM_ID
and sbano = dpam_sba_no
...
And cdshm_tras_dt between @mintrxdt and @PA_TO_DT
...
from #tmp_dp_daily_hldg_cdsl_last_base   a , #FINALREMTHOLDI f    
where a.dphmcd_dpam_id= f.dphmcd_dpam_id     
and a.dphmcd_isin = f.dphmcd_isin 
and RMATPENDING <> 0 
...
into #holdingdmtpenfing1 from  dp_daily_hldg_cdsl_last_base with (nolock) , DP_aCCT_MSTR with (nolock), #CLIENTLIST
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt=@minholdingdt
--and dpam_sba_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
and dpam_sba_no = sbano
and dphmcd_dpm_id = @PA_DPM_ID
...
from #tmp_dp_daily_hldg_cdsl_last_base   a , #pVERQTY f    
where a.dphmcd_dpam_id= f.dphmcd_dpam_id     
and a.dphmcd_isin = f.dphmcd_isin 
...
from #tmp_dp_daily_hldg_cdsl_last_base   a , #pCONVERQTY f    
where a.dphmcd_dpam_id= f.dphmcd_dpam_id     
and a.dphmcd_isin = f.dphmcd_isin 
...
into #holdingdmtpenfing2 from  dp_daily_hldg_cdsl_last_base with (nolock), DP_aCCT_MSTR with (nolock), #CLIENTLIST
where DPHMCD_DPAM_ID = DPAM_ID AND dphmcd_holding_dt=@minholdingdt
and dpam_sba_no = sbano
group by dphmcd_dpam_id,dphmcd_isin 
...
from #tmp_dp_daily_hldg_cdsl_last_base   a , #finaldmatpen2 f    
where a.dphmcd_dpam_id= f.dphmcd_dpam_id     
and a.dphmcd_isin = f.dphmcd_isin 
...
--from   #tmp_dp_daily_hldg_cdsl_last_base   a
...
update a set DPHMCD_LOCKIN_QTY = qty from #locindata_cr , #tmp_dp_daily_hldg_cdsl_last_base  a
...
update a set DPHMCD_LOCKIN_QTY = DPHMCD_LOCKIN_QTY + qty from #locindata_dr , #tmp_dp_daily_hldg_cdsl_last_base  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
...
update a set DPHMCD_LOCKIN_QTY = DPHMCD_LOCKIN_QTY + qty from #locindata_dr_remat , #tmp_dp_daily_hldg_cdsl_last_base  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
and  DPHMCD_LOCKIN_QTY <> 0 
...
update a set DPHMCD_LOCKIN_QTY = DPHMCD_LOCKIN_QTY + qty from #locindata_dr_remat_cr , #tmp_dp_daily_hldg_cdsl_last_base  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
...
update a set DPHMCD_AVAIL_LEND_QTY = DPHMCD_AVAIL_LEND_QTY - qty from #availlenddata , #tmp_dp_daily_hldg_cdsl_last_base  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
...
into #holdingall 
from #tmp_dp_daily_hldg_cdsl_last_base     
where     
(dphmcd_curr_qty <> '0'                        
