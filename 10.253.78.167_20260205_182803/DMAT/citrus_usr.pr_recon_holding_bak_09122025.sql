-- Object: PROCEDURE citrus_usr.pr_recon_holding_bak_09122025
...
CREATE proc [citrus_usr].[pr_recon_holding_bak_09122025](@pa_excsm_id numeric
, @pa_date datetime
, @pa_flag char(15)
...
------EXEC pr_get_holding_fix_latest_reco_tUSHAR_wORKING  @pa_dpm_id,@l_dt,@l_dt,'0','9999999999999999',''     
--
--
--if @pa_dpm_id <> 3 
--begin 
...
select a.* from #finalholding a 
left outer join #tempdata_forreco on ac = client_hldg  and isin = isin_hldg 
where edmat_hldg+case when edmat_hldg <> cdas_hldg then isnull(fill_qty,0)  else 0 end  <> cdas_hldg 
