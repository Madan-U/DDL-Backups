-- Object: PROCEDURE citrus_usr.pr_ins_upd_trx_cdsl_sp_PreEDIS_AngelPre_baknov142022
...
create  PROCEDURE [citrus_usr].[pr_ins_upd_trx_cdsl_sp_PreEDIS_AngelPre_baknov142022](@pa_id              VARCHAR(8000)      
              ,@pa_tab             VARCHAR(20)    
...
select @pa_errmsg = @pa_errmsg +
 isnull(dptdc_isin,'') + '|*~|' + convert(varchar,isnull(ISIN_NAME,'')) + '|*~|' + convert(varchar,dptdc_id) + '|*~|'
  + convert(varchar,abs(DPTDC_QTY)) + '|*~|' + isnull((select distinct convert(varchar,val)  from #tmpvaltbl1 
  where isin = DPTDC_ISIN and abs(qty) = abs(dptdc_qty) and dptdc_id=tmpdptdc_id),'') + '*|~*'
