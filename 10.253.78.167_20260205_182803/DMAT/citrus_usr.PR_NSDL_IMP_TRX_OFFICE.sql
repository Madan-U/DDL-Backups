-- Object: PROCEDURE citrus_usr.PR_NSDL_IMP_TRX_OFFICE
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--begin tran
--ROLLBACK TRAN
--PR_NSDL_IMP_TRX 'HO','0000450' ,'4','','','',11366,''
--select * from dptd_mak where dptd_brokerbatch_no='0000450'
--select * from used_slip where uses_slip_no='000000066'
...
CREATE  PROCEDURE [citrus_usr].[PR_NSDL_IMP_TRX_OFFICE] (  
...
from (
select distinct  @l_dpm_id dpm_id 
,dpam_sba_no
...
and CASE WHEN dptd_trastm_cd = '912' THEN '906' ELSE dptd_trastm_cd END = left(TRASTM_CD,3) 
and   DPTD_BROKERBATCH_NO = @PA_BATCH_NO
and ltrim(rtrim(SLIIM_SERIES_TYPE)) = left(dptd_slip_no,5) 
--and isnumeric(replace(rtrim(ltrim(dptd_slip_no)),ltrim(rtrim(SLIIM_SERIES_TYPE)),'')) = 1
and abs(convert(numeric,replace(dptd_slip_no, ltrim(rtrim(SLIIM_SERIES_TYPE)),''))) between convert(numeric,SLIIM_SLIP_NO_FR) and convert(numeric,SLIIM_SLIP_NO_TO)
)  a 
print '22'
