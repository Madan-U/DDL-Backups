-- Object: PROCEDURE citrus_usr.PR_CHK_APP_IMPORT_bak_15062016
...
CREATE PROCEDURE [citrus_usr].[PR_CHK_APP_IMPORT_bak_15062016]   
...
if exists(select dptdc_id from dptdc_mak, TMP_INTDP_BRK_UPD where dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and   
isnull(dptdc_mid_chk,'')<>'')  
begin  
  
update t set  dptdc_deleted_ind=0 , dptdc_lst_upd_by=@PA_LOGIN_NAME,dptdc_mid_chk=@PA_LOGIN_NAME  from dptdc_mak t , TMP_INTDP_BRK_UPD where   
dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and isnull(dptdc_mid_chk,'')<>''   
