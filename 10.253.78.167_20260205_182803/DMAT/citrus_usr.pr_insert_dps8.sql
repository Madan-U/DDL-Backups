-- Object: PROCEDURE citrus_usr.pr_insert_dps8
...
CREATE  proc [citrus_usr].[pr_insert_dps8](@pa_level numeric)
as
begin
...
,NOMINATION_opt_dt
,COMM_PREFER,BOId,TransSystemDate)
...
--		print @l_str
	...
	begin 
		...
		and not exists(SELECT BOID FROM DPS8_PC4 b WHERE b.BOID = a.boid and b.FreezeId	=	citrus_usr.fn_splitval_by(a.value,	3	,'~'))   
...
select boid  cltid,citrus_usr.fn_splitval_by(value,	3	,'~') mastpoaid  
,max(lastmoddate) maxmoddt 
,citrus_usr.fn_splitval_by(value,	10	,'~') HOLDER into #maxpoachange
from dps8_source111 where  left(value ,2) = '05'  
group by boid 
...
select identity(numeric,1,1) id ,
		'insert into dps8_pc5 values('''+ replace(replace(a.value+'~'+A.BOID+'~'+A.lastmoddate ,'''',''''''),'~',''',''') +''')' insertstr 
		into #tmpdps8_pc5 
		from dps8_source111 a  , #maxpoachange b 
		where left(a.value ,2) = '05' 
		and not exists(SELECT BOID FROM DPS8_PC5 b WHERE b.BOID = A.BOID 
		and MasterPOAId = citrus_usr.fn_splitval_by(value,	3	,'~')
		and SetupDate	=	citrus_usr.fn_splitval_by(value,	5	,'~')
...
,TransSystemDate
		...
		and not exists(SELECT BOID FROM DPS8_PC6 b WHERE b.BOID = a.boid 
		and citrus_usr.fn_splitval_by(a.value,	49	,'~')=b.nom_sr_no
		and  citrus_usr.fn_splitval_by(a.value,	2	,'~')=b.TypeOfTrans) 
