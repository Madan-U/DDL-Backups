-- Object: PROCEDURE citrus_usr.pr_billing_cdsl_bak_01082017
...
--ROLLBACK
create  procedure [citrus_usr].[pr_billing_cdsl_bak_01082017](@pa_billing_from_dt datetime,@pa_billing_to_dt datetime,@pa_dpm_id numeric,@pa_billing_status   char(1))
as
begin
--
...
and clidb_eff_to_dt >getdate()
  ...
  and   proc_deleted_ind = 1
  and   CHAM_CHARGE_BASE = 'NORMAL'
and proc_profile_id  in (select brom_id from brokerage_mstr where brom_desc  in ('VERSION 2.5 LIFETIME'))
  --and not exists(select sba_no from #tempdataforwaive where sba_no = dpam_sba_no)
