-- Object: PROCEDURE citrus_usr.pr_import_setm
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--BEGIN transaction
--pr_import_setm 'NSDL','HO','BULK','c:\BulkInsDbfolder\SETTLEMENT MASTER\cc.flt','*|~*','|*~|',''
--SELECT *  FROM tmpsetm_nsdl_mstr
--select count(*) from settlement_mstr
--ROLLBACK transaction

CREATE PROCEDURE  [citrus_usr].[pr_import_setm]( @pa_exch          VARCHAR(20)  
...
--
  ...
  --
				...
				and case when len(tmpsetm_setm_id) > 7 then ltrim(rtrim(substring(tmpsetm_setm_id,1,len(tmpsetm_setm_id)-7))) else convert(varchar,tmpsetm_setm_type) end = CASE WHEN SETTM_EXCM_ID = 3 and settm_type_cdsl = 'NR' THEN 'N' ELSE ltrim(rtrim(settm_type_cdsl)) END 
				...
				SELECT IDENTITY(INT, 1, 1) ID1, tmpsetm_setm_type setm_type , RIGHT(tmpsetm_setm_id,7) setm_no
...
print @pa_exch
				...
				, getdate()
				, right(CONVERT(varchar,TMPSETM_EARMARK_DT,108),5) --CONVERT(varchar,getdate(),108)
				, tmpsetm_earmark_dt
				, tmpsetm_setm_dt
				, tmpsetm_setm_dt
				, @pa_login_name
