...
Select * Into #DP_MSTR from CITRUS_USR.DP_ACCT_MSTR Where DPAM_SBA_NO =@DP_ID
...
IF (@LTYPE ='L' ) 
		BEGIN 
			...
			SET @STRSQL = ' SELECT LD_CLIENTCD,LD_DT,LD_DT1,LD_AMOUNT,LD_AMOUNT_D,LD_AMOUNT_C,LD_PARTICULAR,LD_CHEQUENO,LD_DEBITFLAG,LD_DOCUMENTTYPE,LD_DOCUMENTNO,LD_ENTRYNO,LD_COSTCENTER,MKRID,MKRDT,LD_ACCYEAR,LD_DPID,LD_COMMONDT,LD_COMMON FROM ( '
			...
			SET @STRSQL = @STRSQL + ' ,convert(varchar(12),LDG_VOUCHER_DT,105)  LD_DT ,LDG_VOUCHER_DT LD_DT1,CONVERT(NUMERIC(18,4),LDG_AMOUNT  ) LD_AMOUNT,CASE WHEN LDG_AMOUNT < 0 THEN LDG_AMOUNT END LD_AMOUNT_D,0 LD_AMOUNT_C '   
			...
			SET @STRSQL = @STRSQL + ' LEFT OUTER JOIN  #DP_MSTR  WITH(NOLOCK)ON  LDG_ACCOUNT_ID  = DPAM_ID '     
			...
			SET @STRSQL = @STRSQL + ' UNION   SELECT LD_CLIENTCD,LD_DT,LD_DT1,LD_AMOUNT,LD_AMOUNT_D,LD_AMOUNT_C,LD_PARTICULAR,LD_CHEQUENO,LD_DEBITFLAG,LD_DOCUMENTTYPE,LD_DOCUMENTNO,LD_ENTRYNO,LD_COSTCENTER,MKRID,MKRDT,LD_ACCYEAR,LD_DPID,LD_COMMONDT,LD_COMMON FROM (  SELECT  CASE WHEN LDG_ACCOUNT_TYPE =''P'' THEN DPAM_SBA_NO  ' 
			SET @STRSQL = @STRSQL + ' ELSE '''' END  '
			SET @STRSQL = @STRSQL + ' LD_CLIENTCD     '
			SET @STRSQL = @STRSQL + ' ,convert(varchar(12),LDG_VOUCHER_DT,105)  LD_DT ,LDG_VOUCHER_DT LD_DT1,CONVERT(NUMERIC(18,4),LDG_AMOUNT  ) LD_AMOUNT ,0 LD_AMOUNT_D,CASE WHEN LDG_AMOUNT > 0 THEN LDG_AMOUNT END LD_AMOUNT_C '   
			...
			--SET @STRSQL = @STRSQL + ' LEFT OUTER JOIN  CITRUS_USR.FIN_ACCOUNT_MSTR   WITH(NOLOCK)ON  LDG_ACCOUNT_ID  =   FINA_ACC_ID '    
			SET @STRSQL = @STRSQL + ' WHERE LDG_DELETED_IND=1  )A '
			SET @STRSQL = @STRSQL + ' WHERE  LD_DEBITFLAG=''C'' and LD_CLIENTCD = ''' + @DP_ID   + ''' ORDER BY LD_DT1   '
