-- Object: PROCEDURE citrus_usr.pr_rpt_trans_count_bak_14082012_new
...
CREATE procedure [citrus_usr].[pr_rpt_trans_count_bak_14082012_new](
 @pa_excsm_id int
,@pa_tab varchar(10)
  ...
  if @pa_exchange = 'CDSL' 
	  if @pa_type='C' 
          ...
          begin  
				...
				ID_cnt = isnull(sum(case when dptdc_internal_trastm = 'ID' then 1 else 0 end),0),  
				OFM_cnt = isnull(sum(case when dptdc_internal_trastm not in('EP','NP','ID') then 1 else 0 end),0),  
                DMT_cnt =0 , DST_cnt =0 , rmt_cnt = 0 , pldt_cnt = 0
				from dp_trx_dtls_cdsl with(nolock), dp_acct_mstr with(nolock)  
			...
			--	and  dptdc_request_dt between @pa_request_dt and @pa_request_todt
				...
				ID_cnt = 0,  
				OFM_cnt = 0,  				
				DMT_cnt = count(demrm_id), DST_cnt = 0, rmt_cnt = 0 , pldt_cnt = 0 from demat_request_mstr,dp_acct_mstr 
				...
				ID_cnt = 0,  
				OFM_cnt = 0,  				
				DMT_cnt = 0, DST_cnt = count(demrm_id), rmt_cnt = 0 , pldt_cnt = 0 from demat_request_mstr,dp_acct_mstr 
				WHERE --demrm_request_dt BETWEEN @pa_request_dt AND @pa_request_todt     
