-- Object: PROCEDURE citrus_usr.pr_get_ob_fix_arch
...
CREATE proc [citrus_usr].[pr_get_ob_fix_arch](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                          
...
, cdshm_dpam_id          
, cdshm_isin , convert(numeric(18,5),0) ob  into #tmp_cdsl_holding_dtls from dmat_archival.citrus_usr.cdsl_holding_dtls    with(nolock)     
...
--from  dmat_archival.citrus_usr.cdsl_holding_dtls   outr   with(nolock)                         
--where CDSHM_TRAS_DT < @pa_from_dt        
--and cdshm_tras_dt >=@l_min_trx_dt        
--and cdshm_tratm_cd in ('2201')    
--and not exists (select  cdshm_dpm_id,cdshm_dpam_id ,cdshm_isin ,cdshm_qty    
--from  dmat_archival.citrus_usr.cdsl_holding_dtls   inn  with(nolock)                             
