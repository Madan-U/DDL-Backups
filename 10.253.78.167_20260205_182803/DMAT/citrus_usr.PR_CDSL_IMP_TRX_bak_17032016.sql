-- Object: PROCEDURE citrus_usr.PR_CDSL_IMP_TRX_bak_17032016
...
create  PROCEDURE [citrus_usr].[PR_CDSL_IMP_TRX_bak_17032016] (  
...
select @l_err_mstr = isnull(replace(citrus_usr.fn_merge_str('CDSL_IMP_INTDP',0,''),',',''),'')
    ...
    */
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				0,--case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPINTDP_ACCT_ID,convert(datetime,TMPINTDP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPINTDP_ISIN,abs(TMPINTDP_QTY),'HIGH_VALUE','','') end = 'Y' then 0 else 0 end,  
				TMPINTDP_TRASTM_CD ,  
				@L_MAX_DPTDC_DTLS_ID1,  
				NULL, 
				NULL,
...
if not exists(select TMPNP_SLIP_NO,TMPNP_BO_ID
    ...
    return 
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				0,--case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPNP_ACCT_ID,convert(datetime,TMPNP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPNP_ISIN,abs(TMPNP_QTY),'HIGH_VALUE','','') end = 'Y' then 0 else 0 end,  
				TMPNP_TRASTM_CD ,  
				@L_MAX_DPTDC_DTLS_ID1,  
				NULL, 
				NULL,
...
--    end 
 ...
 begin transaction
			 ...
			 ,dptdc_cm_id
				...
				@PA_LOGIN_NAME,  
				GETDATE() ,  
				0,--case when case when citrus_usr.fn_get_high_val('',0,'DORMANT',TMPEP_ACCT_ID,convert(datetime,TMPEP_REQUEST_DT,103)) = 'Y' then 'Y' else citrus_usr.fn_get_high_val(TMPEP_ISIN,abs(TMPEP_QTY),'HIGH_VALUE','','') end = 'Y' then 0 else 0 end,  
				TMPEP_TRASTM_CD ,  
				@L_MAX_DPTDC_DTLS_ID1,  
				NULL, 
				NULL,
