-- Object: PROCEDURE citrus_usr.pr_get_txt_filegen_ForAngelID_m
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--PR_GET_TXT_FILEGEN 'CLIENT','12033200','1203320000714231','1203320000714231','AUG 01 2015','AUG 31 2015'
CREATE PROC [citrus_usr].[pr_get_txt_filegen_ForAngelID_m](@PA_ACTION VARCHAR(100),@PA_DPID VARCHAR(100)    
,@PA_BOID VARCHAR(100)    
,@PA_ISIN VARCHAR(100)    
...
--AND ACCP_ACCPM_PROP_CD ='ECN_FLAG')
  ...
  WHERE DPHMC_HOLDING_DT = @PA_TO_DATE
	 ...
	 insert into  citrus_usr.FILEGEN_FORANGELID_ledger_holding
  ...
  exec [pr_get_ob_fix] 3,@pa_from_date,@pa_to_date,'0','9999999999999999',''
   ...
   and CDSHM_TRAS_DT between @pa_from_date and @pa_to_date   
    ...
    replace(replace(value,'$$$$',closingbalance),'####',closingbalance-case when CDSHM_TRATM_CD ='2201' then 0 else cdshm_qty end) as value into #dtmp
    from #tempfinal  order by id 
     
    Select distinct citrus_usr.FN_SPLITVAL_BY(value,1,'|') CLIENT_CODE,
...
WHEN  ISNULL(CLIC_CHARGE_NAME,'')='1_ACMAIN' THEN 'ANNUAL MAINTAINENCE CHARGES'
 ELSE ISNULL(CLIC_CHARGE_NAME,'') END DESCRIPTION ,     
   +CONVERT(VARCHAR,ISNULL(CONVERT(NUMERIC(18,2),SUM(CLIC_CHARGE_AMT)),'0')*-1) AMOUNT  
