-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_2_mpmu_BKUP_31Aug2022
...
CREATE procedure [citrus_usr].[pr_dp_select_mstr_2_mpmu_BKUP_31Aug2022]                                                
(@pa_id             VARCHAR(20)                                                  
...
--                                    
  SELECT DISTINCT DPM_DPID    DPID                                                
        , DPAM_SBA_NO [CLIENT ID]                                                
        , PLDTCM_SLIP_NO SLIPNO                               
        , CONVERT(VARCHAR,PLDTCM_REQUEST_DT,103) REQUESTDATE                                                
        , PLDTCM_AGREEMENT_NO AGREMENTNO                                                
        , CONVERT(VARCHAR,PLDTCM_SETUP_DT,103) SETUPDATE                                  
        , CONVERT(VARCHAR,PLDTCM_EXPIRY_DT,103) EXPIRYDATE                                  
        , PLDTCM_RMKS REMARKS                                                       
  , CONVERT(VARCHAR,PLDTCM_EXEC_DT,103)    EXECDATE                            
        , @PA_SEARCH_C1 [EXCHANGE]                                              
        , PLDTCM_DTLS_ID [DTLS ID]                                                
        , PLDTCM_PLDG_DPID [PLDGEEPLDGOR DPID]                                    
        , PLDTCM_PLDG_CLIENTID [CLIENTID]                                  
        , PLDTCM_PLDG_CLIENTNAME  [CLIENTNAME]                                             
        , PLDTCM_TRASTM_CD  [DESC]                                   
        ,PLDTCM_CLOSURE_BY                                   
        ,PLDTCM_SECURITTIES                                    
        ,PLDTCM_SUB_STATUS ,PLDTCM_id   
		,isnull(PLDTCM_VALUE,'0') PLDTCM_VALUE
		,PLDTCM_UCC
		,PLDTCM_EXID
		,PLDTCM_SEGID
		,PLDTCM_CMID
  ...
  ,    DP_MSTR                                                
  ,    DP_ACCT_MSTR                                                 
  WHERE DPAM_ID  = PLDTCM_DPAM_ID -- PLDTCM_DTLS_ID                                                
  AND   DPM_ID   =  PLDTCM_DPM_ID                                                
        AND   DEFAULT_DP = @PA_SEARCH_C1                                                
        AND   DPAM_DELETED_IND = 1                                                
  AND   DPM_DELETED_IND = 1                                 
  AND   PLDTCM_DELETED_IND = 1                                                
  AND   CONVERT(VARCHAR,PLDTCM_REQUEST_DT,103) = @PA_SEARCH_C2                                                  
  AND   DPAM_SBA_NO LIKE   CASE WHEN @PA_SEARCH_C3 = '' THEN '%' ELSE @PA_SEARCH_C3 END                                       
  AND   PLDTCM_SLIP_NO LIKE   CASE WHEN @PA_SEARCH_C4 = '' THEN '%' ELSE @PA_SEARCH_C4 END                                                 
  AND   PLDTCM_TRASTM_CD LIKE   CASE WHEN @PA_SEARCH_C5 = '' THEN '%' ELSE @PA_SEARCH_C5 END                        
  AND  ISNULL(PLDTCM_BROKER_BATCH_NO,'') = ''   
order by PLDTCM_id                                         
...
--                                                
  ...
  SELECT PLDTCM_ISIN                                                
        ,ABS(PLDTCM_QTY)  PLDTCM_QTY                        
        ,ISNULL(PLDTCM_PSN,'')  PLDTCM_PSN                                                
        ,ISNULL(PLDTCM_REASON,'') REASON 
		,PLDTCM_VALUE                                             
        ,PLDTCM_ID        
		,CASE WHEN isnull(PLDTCM_BATCH_NO,'') = '' THEN PLDTCM_ID ELSE ISNULL(PLDTCM_TRANS_NO,'') END INTERNALNO                                    
  FROM  CDSL_MARGINPLEDGE_DTLS                                                
  WHERE PLDTCM_DTLS_ID = @PA_SEARCH_C1                                                
  AND   PLDTCM_DELETED_IND = 1                                              
--                                                
END                                  
...
--                                                
  ...
  FROM  CMPLEDGED_MAK                                                
  WHERE PLDTCM_DTLS_ID = @PA_SEARCH_C1                                             
  AND   PLDTCM_DELETED_IND  IN (0,4,6,-1)                                              
--                                                
END                                    
ELSE IF @PA_ACTION = 'PLEDGEE_SELM_CDSL'                                                
BEGIN                                                
--    
select  @PA_SEARCH_C1 = dpm_excsm_id from dp_mstr where dpm_id like (select distinct PLDTCM_DPM_ID from CMPLEDGED_MAK where PLDTCM_DTLS_ID = @PA_SEARCH_C6)                                            
  IF ISNULL(@PA_SEARCH_C6,'') <> ''                       
  BEGIN                                             
    SELECT DISTINCT DPM_DPID    DPID                                                
        , DPAM_SBA_NO [CLIENT ID]                                                
  ...
  AND   DPM_DELETED_IND = 1                                                
  AND   PLDTCM_DELETED_IND IN (0,4,6,-1)                                          
  AND   CONVERT(VARCHAR,PLDTCM_REQUEST_DT,103) LIKE CASE WHEN @PA_SEARCH_C2 = '' THEN '%' ELSE @PA_SEARCH_C2 END                                                
...
--        AND   CONVERT(VARCHAR,PLDT_REQUEST_DT,103) = @PA_SEARCH_C2                                                  
  ...
  --AND   PLDTCM_TRASTM_CD  LIKE   CASE WHEN @PA_SEARCH_C5 = '' THEN '%' ELSE @PA_SEARCH_C5 END                                                   
   AND  ISNULL(PLDTCM_BROKER_BATCH_NO,'') = ''                                            
   AND  PLDTCM_DTLS_ID = @PA_SEARCH_C6     
   order by    PLDTCM_ID                    
...
--                                                
  ...
  , CONVERT(VARCHAR,PLDTCM_EXEC_DT,103)    EXECDATE                                               
		...
		,PLDTCM_EID
		,PLDTCM_TMCMID    
		,PLDTCM_TRASTM_CD = case when PLDTCM_TRASTM_CD = 'crtemp' then 'Margin Pledge' when PLDTCM_TRASTM_CD = 'createmr' then 'RePledge' else '' end                            
  FROM CMPLEDGED_MAK                                                
  ,    DP_MSTR                                                
  ,    DP_ACCT_MSTR                               
  WHERE DPAM_ID  =  PLDTCM_DPAM_ID
  AND   DPM_ID   =  PLDTCM_DPM_ID                                                       
  AND   DPAM_DELETED_IND = 1                                                
  AND   DPM_DELETED_IND = 1                                                
  AND   PLDTCM_DELETED_IND IN (0,4,6,-1)                                                 
  AND   PLDTCM_TRASTM_CD LIKE   CASE WHEN @PA_SEARCH_C5 = '' THEN '%' ELSE @PA_SEARCH_C5 END                                                   
  AND   ISNULL(PLDTCM_BROKER_BATCH_NO,'') = ''    
  AND   PLDTCM_UPDATED_BY <> @pa_login_name                                  
--                                                
END                 
