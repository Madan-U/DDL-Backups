-- Object: PROCEDURE citrus_usr.pr_fetch_disdata_recheck
...
CREATE  proc [citrus_usr].[pr_fetch_disdata_recheck](@pa_Action varchar(100)
...
--and isnull(reco_yn,'') = ''  --commented on 04102013 as per MOSL request
				...
				and dis.lst_upd_dt between @pa_from_date and @pa_to_date + ' 23:59:59'
				...
				and isnull(flag,'')='E' and isnull(RECHK_FLAG,'')=''
                union
				select dpm_dpid
				, dpam_sba_no 
				, dpam_sba_name
...
--and isnull(reco_yn,'') = '' --commented on 04102013 as per MOSL request
				...
				update temp1 Set temp1.RECHK_FLAG='Y'   from slip_issue_mstr main,dp_mstr , dp_acct_mstr left outer join dp_holder_dtls on dphd_dpam_id = dpam_id 
				,#account_properties , Dis_req_Dtls dis				
				,fetchdisdata_mstr temp1
				--,#temp2 temp2
				where dpam_deleted_ind =1 and sliim_deleted_ind = 1 and accp_clisba_id = dpam_id 
				...
				--/*30 days expire logic'
				
				update U Set SLIIM_EXPIRY_DATE=case when REQ_BDINIT_FLAG='B' then getdate() else dateadd(m,1,sliim_dt) end  
				from slip_issue_mstr U,Dis_req_Dtls where  not exists (
				select main.sliim_id  from slip_issue_mstr main,dp_mstr , dp_acct_mstr left outer join dp_holder_dtls on dphd_dpam_id = dpam_id 
				,#account_properties , Dis_req_Dtls dis				
				,fetchdisdata_mstr temp1
				--,#temp2 temp2
				where dpam_deleted_ind =1 and sliim_deleted_ind = 1 and accp_clisba_id = dpam_id 
				...
				and left(sliim_dpam_acct_no,2) <> '22'   --to eliminate poa 
				and ISNULL(SLIIM_SUCCESS_FLAG,'')=CASE WHEN sliim_dt>='2014-10-01 00:00:00.000' then 'S' else '' end 
				and isnull(flag,'')='E'	and isnull(RECHK_FLAG,'')='' and u.sliim_id=main.sliim_id and u.sliim_dpam_acct_no=main.sliim_dpam_acct_no)
				and slip_yn <> 'D'
				and lst_upd_dt between @pa_from_date and @pa_to_date + ' 23:59:59'
				and Boid = sliim_dpam_acct_no   and REQ_BDINIT_FLAG in ('B','D')	
			 ...
			 begin			
				...
				and left(sliim_dpam_acct_no,2) <> '22'   --to eliminate poa 
				and ISNULL(SLIIM_SUCCESS_FLAG,'')=CASE WHEN sliim_dt>='2014-10-01 00:00:00.000' then 'S' else '' end 
				and isnull(flag,'')='A' and isnull(RECHK_FLAG,'')=''
