-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_isinwise
...
CREATE  proc [citrus_usr].[pr_get_holding_fix_latest_isinwise](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
into #tmp_vw_holding_base_all from cdsl_holding_dtls      with (nolock)
...
and not exists(select b.dphmcd_dpam_id,b.dphmcd_isin,b.dphmcd_dpm_id from #tmp_vw_holding_base_all b with (nolock) where a.dphmcd_dpm_id = b.dphmcd_dpm_id    
...
from #tmp_vw_holding_base_all   a with (nolock), #finalholding f    with (nolock)
...
/* Margin Obligation with column 52 ep trx no link and ep reject for confiscat trx  date nov 6 2025 */



select cdshm_ben_acct_no , cdshm_isin, cdshm_trans_no , cdshm_qty ,  
cdshm_tras_dt,cdshm_trans_cdas_code
,  citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,52,'~') ep_trx_no 
into #Confiscat_trx_with_eptrx_rej 
from CDSL_HOLDING_DTLS with(nolock) where 
 cdshm_tras_dt between @l_min_trx_dt and @PA_TO_DT  
and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to   
and cdshm_isin = @pa_isin
and citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,52,'~') <> '' 
and CDSHM_CDAS_TRAS_TYPE	='11'
and CDSHM_CDAS_SUB_TRAS_TYPE='1103'


select cdshm_dpam_id , cdshm_ben_acct_no , cdshm_isin , sum(cdshm_qty*-1) ep_rej into #tmp_ep_rej from CDSL_HOLDING_DTLS eprej with(nolock) 
where 
 cdshm_tras_dt between @l_min_trx_dt and @PA_TO_DT  
and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to   
and cdshm_isin = @pa_isin
and CDSHM_TRATM_CD ='4466'
and CDSHM_CDAS_TRAS_TYPE='4'  
and CDSHM_CDAS_SUB_TRAS_TYPE ='426'
and exists (select 1 from #Confiscat_trx_with_eptrx_rej  conf where ep_trx_no = eprej.cdshm_trans_no and 
conf.CDSHM_BEN_ACCT_NO = eprej.CDSHM_BEN_ACCT_NO
and conf.cdshm_isin = eprej.cdshm_isin 
)
and abs(cdshm_qty ) <> citrus_usr.FN_SPLITVAL_BY (cdshm_trans_cdas_code,6,'~')
 group by  cdshm_dpam_id , cdshm_ben_acct_no , cdshm_isin 

 

select cdshm_dpam_id , cdshm_ben_acct_no , cdshm_isin , sum(cdshm_qty*-1) ep_rej into #tmp_ep_rej_full 
from CDSL_HOLDING_DTLS eprej with(nolock) 
where 
 cdshm_tras_dt between @l_min_trx_dt and @PA_TO_DT  
and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to   
and cdshm_isin = @pa_isin
...
union ALL
select cdshm_dpam_id , cdshm_isin,qty from #trasdmtpenfing2
union all 
select cdshm_dpam_id , cdshm_isin,ep_rej  from #tmp_ep_rej
union all 
select cdshm_dpam_id , cdshm_isin,ep_rej  from #tmp_ep_rej_full
) a 
group by dphmcd_dpam_id,dphmcd_isin
PRINT CONVERT(VARCHAR(26), GETDATE(), 109)

 update a  set DPHMCD_CURR_QTY = DPHMCD_CURR_QTY + ep_rej 
  from #tmp_ep_rej epr , #tmp_vw_holding_base_all a 
 where a.dphmcd_dpam_id= epr.cdshm_dpam_id       
and a.dphmcd_isin = epr.cdshm_isin  

  update a  set DPHMCD_CURR_QTY = DPHMCD_CURR_QTY + ep_rej 
  from #tmp_ep_rej_full epr , #tmp_vw_holding_base_all a 
 where a.dphmcd_dpam_id= epr.cdshm_dpam_id       
and a.dphmcd_isin = epr.cdshm_isin   
...
from #tmp_vw_holding_base_all   a , #finaldmatpen2 f    
where a.dphmcd_dpam_id= f.dphmcd_dpam_id     
and a.dphmcd_isin = f.dphmcd_isin 
...
--from   #tmp_vw_holding_base_all   a
...
update a set DPHMCD_LOCKIN_QTY = DPHMCD_LOCKIN_QTY + qty from #locindata_cr , #tmp_vw_holding_base_all  a
...
update a set DPHMCD_LOCKIN_QTY = DPHMCD_LOCKIN_QTY + qty from #locindata_dr , #tmp_vw_holding_base_all  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
...
update a set DPHMCD_LOCKIN_QTY = DPHMCD_LOCKIN_QTY + qty from #locindata_dr_remat , #tmp_vw_holding_base_all  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
and  DPHMCD_LOCKIN_QTY <> 0 
...
update a set DPHMCD_LOCKIN_QTY = DPHMCD_LOCKIN_QTY + qty from #locindata_dr_remat_cr , #tmp_vw_holding_base_all  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
...
update a set DPHMCD_AVAIL_LEND_QTY = DPHMCD_AVAIL_LEND_QTY - qty from #availlenddata , #tmp_vw_holding_base_all  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
...
--from cdsl_holdING_DTLS a With(Nolock) where  CDSHM_CDAS_TRAS_TYPE='4' and 	CDSHM_CDAS_SUB_TRAS_TYPE ='409'  and cdshm_tratm_cd ='2215'
--and cdshm_isin = @pa_isin
--and not exists 
--(select 1 from cdsl_holdING_DTLS b With(Nolock)
--where a.CDSHM_DPAM_ID = b.CDSHM_DPAM_ID 
--and a.CDSHM_ISIN = b.CDSHM_ISIN 
--and a.CDSHM_TRANS_NO = b.cdshm_trans_no and b.cdshm_isin = @pa_isin
...
--update a set DPHMCD_EARMARK_QTY = DPHMCD_EARMARK_QTY + qty from #EARMARKQTY1  , #tmp_vw_holding_base_all  a
--where dphmcd_dpam_id = cdshm_dpam_id 
--and dphmcd_isin = cdshm_isin 
...
update a set DPHMCD_EARMARK_QTY = DPHMCD_EARMARK_QTY + qty from #EARMARKQTY  , #tmp_vw_holding_base_all  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
...
/*Early Payin May 15 2021*/
SELECT CDSHM_DPAM_ID , CDSHM_ISIN , SUM(CDSHM_QTY) QTY into #EARMARKQTY1 
from cdsl_holdING_DTLS a where  CDSHM_CDAS_TRAS_TYPE='4' and 	CDSHM_CDAS_SUB_TRAS_TYPE in ('409','431','528','433')  and cdshm_tratm_cd ='2215'
--and not exists 
--(select 1 from cdsl_holdING_DTLS b 
--where a.CDSHM_DPAM_ID = b.CDSHM_DPAM_ID 
--and a.CDSHM_ISIN = b.CDSHM_ISIN 
...
from cdsl_holdING_DTLS a where  CDSHM_CDAS_TRAS_TYPE='4'   and cdshm_tratm_cd in ('2277','4466')
and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to 
and cdshm_isin = @pa_isin 
and exists 
(select 1 from cdsl_holdING_DTLS b 
...
update a set DPHMCD_EARMARK_QTY = DPHMCD_EARMARK_QTY + qty from #EARMARKQTY2  , #tmp_vw_holding_base_all  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
and DPHMCD_EARMARK_QTY + qty  >= 0
...
drop table #holdingall 
drop table #tmp_vw_holding_base_all
