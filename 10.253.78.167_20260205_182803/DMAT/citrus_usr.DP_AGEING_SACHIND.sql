-- Object: PROCEDURE citrus_usr.DP_AGEING_SACHIND
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--drop table #temp

CREATE procedure [citrus_usr].[DP_AGEING_SACHIND]
...
select LD_CLIENTCD,BALAMT=SUM(LD_AMOUNT*-1),
		...
		FROM (SELECT LD_CLIENTCD,round(LD_AMOUNT,2)LD_AMOUNT,LD_DT   FROM SYNERGY_LEDGER  WHERE LD_DT <='2018-09-30 23:59'
AND LD_CLIENTCD LIKE '120332%'
UNION ALL
SELECT CLIENT_CODE,round(cast(amount as float ),2)*-1 LD_AMOUNT,'2018-09-30 00:00:00'AS LD_DT FROM DP_DE ) A
...
SUM(CR360Total)CR360Total,SUM(DR360Total) DR360Total ,SUM(CR361Total)CR361Total,SUM(DR361Total) DR361Total INTO #TOTAL
 ...
 SELECT * INTO #HOLD FROM SYNERGY_HOLDING WHERE HLD_HOLD_DATE ='2018-09-29'

  SELECT V.* INTO #CLOSIN FROM  VW_ISIN_RATE_MASTER V,
  (SELECT ISIN,MAX(RATE_DATE) AS RD FROM  VW_ISIN_RATE_MASTER  WHERE  RATE_DATE <='2018-09-29' GROUP BY ISIN ) B
  WHERE  RATE_DATE <='2018-09-29' AND V.ISIN=B.ISIN AND RD=RATE_DATE

  SELECT HLD_AC_CODE,SUM(HLD_AC_POS*CLOSE_PRICE)VALUE INTO #HOLD_MAR FROM #HOLD H, #CLOSIN WHERE ISIN=HLD_ISIN_CODE  GROUP BY HLD_AC_CODE

  	ALTER TABLE  #CREDITORS
	ADD VALUATION MONEY
		ALTER TABLE  #DEBTORS
	ADD VALUATION MONEY
	    UPDATE #DEBTORS SET VALUATION= VALUE   FROM #HOLD_MAR WHERE   CLTCODE=HLD_AC_CODE
		...
		SELECT *,DEF=(CASE WHEN ISNULL(VALUATION,0)>NET_LD_DOCUMENTTYPE THEN 'Secured' else 'RISK' END) FROM #DEBTORS
				...
				--select Sum(round(cast(amount as float ),2)) LD_AMOUNT from DP_DE

				--select top 10 * from DP_DE
