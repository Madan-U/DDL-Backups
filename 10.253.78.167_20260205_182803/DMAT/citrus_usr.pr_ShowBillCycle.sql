-- Object: PROCEDURE citrus_usr.pr_ShowBillCycle
...
--Exec pr_ShowBillCycle '12345678','2010'
CREATE proc [citrus_usr].[pr_ShowBillCycle]    
(@dpmid varchar(16)    
,@foryear char(40)    
)    
as    
begin    
 ...
 @l_due_date datetime,
 @l_post_date datetime,@p_fromdt datetime ,@p_todt  datetime
 SET @lastbilldate = ''    
 ...
 begin    
 set @p_fromdt =citrus_usr.fn_splitval_by(@foryear,'1','-')
 set @p_todt =citrus_usr.fn_splitval_by(@foryear,'2','-')

if len(@foryear)='4'
begin
  select billc_Dpm_id,convert(varchar(11),billc_from_dt,103) billc_from_dt  ,convert(varchar(11),billc_to_dt,103) billc_to_dt,billc_flg,billc_posted_flg,convert(varchar(11),billc_from_dt,103) +'-'+ convert(varchar(11),billc_to_dt,103) billperoid,ord_dt=billc_to_dt ,isnull(billc_lockunlock_flg,'')  billc_lockunlock_flg
  from Bill_cycle where 
  convert(varchar,year(billc_to_dt)) = @foryear  and 
  billc_dpm_id = @l_dpm_id
  --and billc_from_dt >= convert(datetime,@p_fromdt)
  --and billc_to_dt <= convert(datetime,@p_todt)
  order by 7 desc 
end
else
begin
