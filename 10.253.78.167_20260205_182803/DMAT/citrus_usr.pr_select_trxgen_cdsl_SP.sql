-- Object: PROCEDURE citrus_usr.pr_select_trxgen_cdsl_SP
...
--rollback
CREATE  PROCEDURE [citrus_usr].[pr_select_trxgen_cdsl_SP]  
(  
...
--																																																											END  
	...
	+ convert(char(22),isnull(DPTDC_CHQ_REFNO,''))
												...
												,CASE WHEN ISNULL(CITRUS_USR.FN_GET_DDPI_FLAG(DPAM_SBA_NO,''),'') IN ('S','A') THEN ISNULL(CITRUS_USR.FN_GET_DDPI_FLAG(DPAM_SBA_NO,''),'') ELSE   CASE WHEN DPAM_SBA_NO=DPTDC_CREATED_BY THEN 'Y' ELSE 'N' END END TRXEFLG 
												,citrus_usr.fn_get_slip_no_export(dpam_sba_no,dptdc_slip_no) dptdc_slip_no , dptdc_created_by , case when isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'')='' then dptdc_lst_upd_by else isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'') end dptdc_lst_upd_by , case when isnull(citrus_usr.Fn_Toget_Midchk(dptdc_slip_no,dptdc_id),'')<>'' then dptdc_lst_upd_by else '' end verifier
												,dptdc_id   as USN,''
						...
						--  
								...
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
										...
										+ case when convert(char(8),replace(convert(varchar(11),isnull(Disp_dt,space(8)),103),'/','')) = '01011900' then  convert(char(8),replace(convert(varchar(11),isnull(getdate(),space(8)),103),'/','')) else convert(char(8),replace(convert(varchar(11),isnull(Disp_dt,space(8)),103),'/','')) end 
...
,convert(numeric(18,3),demrm_qty) qty,'',DEMRM_SLIP_SERIAL_NO,'','','','0',replace(convert(varchar(11),isnull(DEMRM_REQUEST_DT,space(8)),103),'/','') DEMRM_REQUEST_DT
...
--UNFRZ
						...
						BEGIN
										...
										BEGIN  
											...
											+ case when convert(char(1),isnull(remrm_free_lockedin_yn,''))in('Y','T') then 'L' else 'F' end  
