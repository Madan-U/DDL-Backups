-- Object: PROCEDURE citrus_usr.rpt_Approve_details_bak_04052015
...
create  procedure [citrus_usr].[rpt_Approve_details_bak_04052015]
(@pa_id  varchar(10),
 @pa_type varchar(4),
 @pa_tab varchar(20),
 @pa_from_dt datetime,
 ...
 if @pa_id = '--ALL--'
  ...
  begin
	 ...
	 select @@l_child_entm_id    =  citrus_usr.fn_get_child(@pa_login_pr_entm_id , @pa_login_entm_cd_chain)                                      
		...
		if @l_dpm_id <> '999'
		begin
			INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,isnull(EFF_TO,'DEC 31 2100') FROM citrus_usr.fn_acct_list(@l_dpm_id ,@pa_login_pr_entm_id,@@l_child_entm_id) where dpam_sba_no like case when isnull(@pa_clientid,'') = '' then '%' else '' + isnull(@pa_clientid,'%') +'' end
		end
else
begin
INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,'',isnull('','DEC 31 2100') FROM dp_acct_mstr where dpam_sba_no like case when isnull(@pa_clientid,'') = '' then '%' else '' + isnull(@pa_clientid,'%') +'' end
	end
...
--convert(varchar(11),DPTDC_LST_UPD_dt,103) else '' end as checker2_dt
					...
					, '' batchno, isnull(recon_datetime,'') recon_datetime
   					 FROM    dptdc_mak                    dptd 
					 left outer join maker_scancopy  on DPTDC_SLIP_NO = slip_no 
                     left outer join settlement_type_mstr settm1 on convert(varchar,settm1.settm_id) = dptd.DPTDc_MKT_TYPE and isnull(settm1.settm_deleted_ind,1) = 1 
                     left outer join settlement_type_mstr settm2 on convert(varchar,settm2.settm_id) = dptd.DPTDc_OTHER_SETTLEMENT_TYPE and isnull(settm2.settm_deleted_ind,1) = 1 
...
--					  else 'D' end as [Status1]
					SELECT convert(varchar(50),dptd1.DPTDC_ID) inst_id 
					,convert(varchar(11),dptd1.dptdc_request_dt,103)             REQUESTDATE 
					,convert(varchar(11),dptd1.dptdc_execution_dt,103)           EXECUTIONDATE
					,REPLACE(Replace(Replace(Replace(Replace(Replace(Replace(dptd1.DPTDC_INTERNAL_TRASTM,'EP','EARLY PAYIN'),'NP',' NORMAL PAYIN'),'ID','INTER DEPOSITORY'),'DMAT','DEMAT'),'BOBO','OFF MARKET - BO TO BO'),'BOCM','OFF MARKET - BO TO CM'),'CMBO','OFF MARKET - CM TO BO') trans_descp     
					,dptd1.dptdc_slip_no                SLIPNO 
...
--					  else 'D' end as [Status1]
					...
					, case when dptd.dptdc_deleted_ind in (0,1) and isnull(dptd.dptdc_mid_chk,'') <> '' then CASE WHEN isnull(convert(varchar(11),dptd1.DPTDC_LST_UPD_dt,109),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),dptd1.DPTDC_LST_UPD_dt,109) END 
...
--convert(varchar(11),dptd.DPTDC_LST_UPD_dt,103) else '' end as checker1_dt
					...
					, case  when dptd.dptdc_reason_cd= '1' then 'Gift' 
							...
							when dptd.dptdc_reason_cd= '7' then 'Trnsfr between family members' else '' end reason
					,recon_datetime as recon_datetime
					,dptd1.dptdc_batch_no
					FROM    dptdc_mak dptd  left outer join maker_scancopy  on DPTDC_SLIP_NO = slip_no and deleted_ind =1
...
--					,''
					select CONVERT(VARCHAR(50),d.demrm_id) + '/' + CONVERT(VARCHAR(50),ISNULL(demrm_transaction_no,''))  inst_id
...
--						then 'CHECKER DONE' ELSE '' END AS [STATUS1]
					...
					, case when d1.DEMRM_DELETED_IND = 1 then CASE WHEN isnull(convert(varchar(11),d1.DEMRM_LST_UPD_DT,109),'') ='jan 01 1900' THEN '' ELSE convert(varchar(24),d1.DEMRM_LST_UPD_DT,109) END else '' end checker1_dt
					...
					, '' recon_datetime
					, d.DEMRM_BATCH_NO
					from demat_request_mstr d,demrm_mak d1
...
--					,''
					select convert(varchar(50),Remrm_id) inst_id
					,convert(varchar(11),Remrm_request_dt,103)
					,convert(varchar(11),Remrm_request_dt,103)
					,'REMAT REQUEST'
					,REMRM_SLIP_SERIAL_NO
...
--                    ,[cmbp],counter_account
--					,[Status1]
print 'dddddddddddddddddddd1111111111111111111111'
...
--					  else 'D' end as [Status1]
					, case when citrus_usr.gettranstype(ISNULL(dptd1.dptdc_trans_no,''),ISNULL(convert(varchar,dptd1.dptdc_dpam_id),'0'))<> '' then citrus_usr.gettranstype(ISNULL(dptd1.dptdc_trans_no,''),ISNULL(convert(varchar,dptd1.dptdc_dpam_id),'0'))
					    when dptd.dptdc_deleted_ind = 0 and isnull(dptdc_res_cd,'') = '' and isnull(dptdc_mid_chk,'')='' and isnull(dptd.dptdc_execution_dt,'')='' and isnull(preappflg,'')='Y' then 'PREAPPROVED'
...
--convert(varchar(11),dptd.DPTDC_LST_UPD_dt,103) else '' end as checker1_dt
					...
					, '' dispdate
					FROM    dptdc_mak dptd  left outer join maker_scancopy  on DPTDC_SLIP_NO = slip_no and deleted_ind =1
                    left outer join DP_TRX_DTLS_CDSL dptd1 on dptd.dptdc_id = dptd1.dptdc_id 
                    left outer join settlement_type_mstr settm1 on convert(varchar,settm1.settm_id) = dptd1.DPTDC_MKT_TYPE and isnull(settm1.settm_deleted_ind,1) = 1 
                    left outer join settlement_type_mstr settm2 on convert(varchar,settm2.settm_id) = dptd1.DPTDC_OTHER_SETTLEMENT_TYPE and isnull(settm2.settm_deleted_ind,1) = 1 
					--left outer join cdsl_holding_dtls on cdshm_dpam_id = dptdc_dpam_id and cdshm_trans_no = dptd1.dptdc_trans_no
					WHERE   --dptd.dptdc_id = dptd1.dptdc_id 
...
--						then 'CHECKER DONE' ELSE '' END AS [STATUS1]
					...
					, '' recon_datetime
					, d.DEMRM_BATCH_NO
					--, isnull(d.DEMRM_RESPONSE_DT,'') RejectionDate
...
when isnull(d1.demrm_res_cd_compobj,'') <> '' then convert(varchar(10),d1.demrm_lst_upd_dt,103) 
					...
					, case when (isnull(d1.demrm_internal_rej,'') <> '' or isnull(d.demrm_company_obj,'')<>'' or isnull(d1.demrm_res_cd_intobj,'')<>'' or isnull(d1.demrm_res_cd_compobj,'')<>'' ) then  isnull(DISP_NAME,'') else '' end courier
					, case when (isnull(d1.demrm_internal_rej,'') <> '' or isnull(d.demrm_company_obj,'')<>'' or isnull(d1.demrm_res_cd_intobj,'')<>'' or isnull(d1.demrm_res_cd_compobj,'')<>'' ) then  isnull(disp_cons_no,'') else '' end   podno
					, case when (isnull(d1.demrm_internal_rej,'') <> '' or isnull(d.demrm_company_obj,'')<>'' or isnull(d1.demrm_res_cd_intobj,'')<>'' or isnull(d1.demrm_res_cd_compobj,'')<>'' ) then  isnull(DISP_DT,'') else '' end   dispdate
					from demrm_mak d1 left outer join demat_request_mstr d   on d.demrm_id = d1.demrm_id 	
and d.DEMRM_DPAM_ID = d1.DEMRM_DPAM_ID , dmat_dispatch
					where 
...
and d1.demrm_id = disp_demrm_id
					...
					--AND d.DEMRM_SLIP_SERIAL_NO like case when @pa_slip_no = '' then '%' else @pa_slip_no end
					AND d1.DEMRM_SLIP_SERIAL_NO like case when @pa_slip_no = '' then '%' else @pa_slip_no end	
					and d.DEMRM_TRANSACTION_NO like case when @pa_drnno = '' then '%' else @pa_drnno end		                  					
...
--						   When dptdc_deleted_ind = 1 then 'U'
--					  else 'D' end as [Status1]
					, case when isnull(dptd1.DPTDC_ERRMSG,'') <> '' then [citrus_usr].[fn_get_errorcodes](dptd1.DPTDC_ERRMSG) else case when citrus_usr.gettranstype(ISNULL(dptd1.dptdc_trans_no,''),ISNULL(convert(varchar,dptd1.dptdc_dpam_id),'0'))<> '' then citrus_usr.gettranstype(ISNULL(dptd1.dptdc_trans_no,''),ISNULL(convert(varchar,dptd1.dptdc_dpam_id),'0'))
						when ISNULL(dptd1.dptdc_trans_no,'') <> '' then 'RESPONSE FILE IMPORTED'
						when ISNULL(dptd1.dptdc_batch_no,'') <> '' then 'BATCH GENERATED' 
						--when dptd1.dptdc_deleted_ind = 1 then 'CHECKER DONE' 
						when dptd.dptdc_deleted_ind = 0 and isnull(dptdc_res_cd,'') <> '' then 'REJECTED'
...
--,case when isnull(d1.demrm_internal_rej,'') <> '' then d1.demrm_lst_upd_dt 
--	 when isnull(d.demrm_company_obj,'') <> '' then d.demrm_lst_upd_dt 
--else d1.demrm_lst_upd_dt  end RejectionDate
,case when citrus_usr.gettranstype(ISNULL(d.demrm_transaction_no,''),ISNULL(convert(varchar,d.demrm_dpam_id),'0')) <> '' then
citrus_usr.gettranstype_date(ISNULL(d.demrm_transaction_no,''),ISNULL(convert(varchar,d.demrm_dpam_id),'0')) 
when isnull(d1.demrm_internal_rej,'') <> '' then convert(varchar(10),d1.demrm_lst_upd_dt ,103)
	 when isnull(d.demrm_company_obj,'') <> '' then convert(varchar(10),d.demrm_lst_upd_dt ,103)
...
when isnull(d1.demrm_res_cd_compobj,'') <> '' then convert(varchar(10),d1.demrm_lst_upd_dt,103) 
					...
					, isnull(disp_cons_no,'') podno
					, isnull(DISP_DT,'') dispdate
					from  demrm_mak d1 left outer join demat_request_mstr d  on d.demrm_id = d1.demrm_id 	
and d.DEMRM_DPAM_ID = d1.DEMRM_DPAM_ID left outer join dmat_dispatch on  d1.demrm_id = disp_demrm_id and disp_to = 'C'
					where 
					--d1.DEMRM_request_dt >= @pa_from_dt and d1.DEMRM_request_dt <= @pa_to_dt
					--and  d.demrm_deleted_ind = 1
