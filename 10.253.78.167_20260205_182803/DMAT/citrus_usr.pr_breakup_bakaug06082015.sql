-- Object: PROCEDURE citrus_usr.pr_breakup_bakaug06082015
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

create procedure [citrus_usr].[pr_breakup_bakaug06082015](@pa_from_dt datetime,@pa_to_dt datetime,@pa_login_name varchar(100))
as
begin 
...
AND CDSHM_CDAS_SUB_TRAS_TYPE IN ('707','3304') AND citrus_usr.fn_splitval_by(cdshm_trans_cdas_code,26,'~') = 'J'
UNION ALL 
SELECT CDSHM_BEN_ACCT_NO , '12' AMT , cdshm_internal_trastm FROM cdsl_holding_dtls_bak_forreve WHERE CDSHM_CDAS_SUB_TRAS_TYPE in ('810')
UNION ALL
...
select '''' +  dpam_sba_no dpam_sba_no , brom_desc ,charge_name,amt,service_tax_amt ,dp_charge  from #final where amt + service_tax_amt + dp_charge <> 0
order by 1 
