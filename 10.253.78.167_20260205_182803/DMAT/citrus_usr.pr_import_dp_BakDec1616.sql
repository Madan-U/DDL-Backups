-- Object: PROCEDURE citrus_usr.pr_import_dp_BakDec1616
...
CREATE PROCeDURE [citrus_usr].[pr_import_dp_BakDec1616]( @pa_crn_no      VARCHAR(8000)                  
                            , @pa_exch        VARCHAR(10)                  
...
,res_sec_flag char(1)        
				  ...
				  ,Filler1            
						 ...
						 ,BSDA_FLAG                                                      
                                  ...
                                  SELECT distinct '05'                  
                                       ...
                                       , case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567') then case when isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'INC_DOB',''),'') = '' then '00000000' else isnull(citrus_usr.fn_ucc_entp(clim.clim_crn_no,'INC_DOB',''),'00000000') end  else case when isnull(convert(varchar(11),clim.clim_dob,103),'00000000') = '01/01/1900' then '00000000' else   isnull(convert(varchar(11),clim.clim_dob,103),'00000000')  end                end
 ...
 , case when isnull(citrus_usr.fn_ucc_accp(dpam_id,'RGESS_FLAG',''),'00')  = 'YES'     then 'Y' else 'N' end
  ...
  ,Purpose_code                             
								  ...
								  ,sharepercent
								  ,res_sec_flag        
                                  FROM fn_cdsl_export_line_7_extract_Bakdec1616(@pa_crn_no, @pa_from_dt, @pa_to_dt), dp_acct_mstr , EXCH_SEG_MSTR esm  
...
--changed by tushar on jan 09 2015      
           ...
           +CASE WHEN CONVERT(char(4),l7.usr_fld5) =0 THEN '0000' ELSE CONVERT(char(4),l7.usr_fld5) END,'') 
		   	...
		   	+CASE WHEN CONVERT(char(2),right(ltrim(rtrim(l7.Purpose_code)),2)) ='12' then '' else CONVERT(char(2),l7.nom_serial_no) end
			+CASE WHEN CONVERT(char(2),right(ltrim(rtrim(l7.Purpose_code)),2)) ='12' then '' else CONVERT(char(2),l7.rel_withbo) end
			+CASE WHEN CONVERT(char(2),right(ltrim(rtrim(l7.Purpose_code)),2)) ='12' then '' else CONVERT(char(5),l7.sharepercent) end
			+CASE WHEN CONVERT(char(2),right(ltrim(rtrim(l7.Purpose_code)),2)) ='12' then '' else CONVERT(char(1),l7.res_sec_flag) end
