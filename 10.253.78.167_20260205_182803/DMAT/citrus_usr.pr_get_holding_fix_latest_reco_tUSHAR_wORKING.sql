-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_reco_tUSHAR_wORKING
...
CREATE proc [citrus_usr].[pr_get_holding_fix_latest_reco_tUSHAR_wORKING](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
ON #tmp_dp_daily_hldg_cdsl(dphmcd_dpm_id, dphmcd_dpam_id,DPHMCD_ISIN)
...
SELECT * INTO #TEMPDATA_P FROM (select cdshm_dpam_id  dphmcd_dpam_id,cdshm_isin dphmcd_isin 
...
union  all                 
select cdshm_dpam_id  dphmcd_dpam_id,cdshm_isin dphmcd_isin 
,sum(case when cdshm_tratm_cd in ('4456') then cdshm_qty else 0 end)  DPHMCD_CURR_QTY    
,sum(case when cdshm_tratm_cd in ('2246','2277') then cdshm_qty else 0 end)  DPHMCD_FREE_QTY                        
,0 DPHMCD_FREEZE_QTY                        
,0 DPHMCD_PLEDGE_QTY                        
...
,cdshm_dpm_id dphmcd_dpm_id , cdshm_trans_no dphmcd_trans_no                     
from   citrus_usr.cdsl_holding_dtls        with(nolock)                     
where cdshm_tratm_cd in ('4456') and CDSHM_CDAS_SUB_TRAS_TYPE ='417'    and CDSHM_TRAS_DT<'may 01 2023' 
--and (CITRUS_USR.FN_SPLITVAL_BY(SUBSTRING(cdshm_trans_cdas_code,CHARINDEX('D~',cdshm_trans_cdas_code),LEN(cdshm_trans_cdas_code)),2,'~') NOT IN ('33','7')) 
and   cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to    
--and   cdshm_isin = @pa_isin              
and   CDSHM_TRAS_DT  >=@l_min_trx_dt      
...
,sum(DPHMCD_CURR_QTY) DPHMCD_CURR_QTY           
 ...
 ,sum(DPHMCD_FREEZE_QTY)   DPHMCD_FREEZE_QTY                        
 ,sum(DPHMCD_PLEDGE_QTY )   DPHMCD_PLEDGE_QTY                        
 ,sum(0)    DPHMCD_DEMAT_PND_VER_QTY                        
 ,sum(DPHMCD_REMAT_PND_CONF_QTY)    DPHMCD_REMAT_PND_CONF_QTY                        
 ,sum(DPHMCD_DEMAT_PND_CONF_QTY)    DPHMCD_DEMAT_PND_CONF_QTY                        
 ,sum(DPHMCD_SAFE_KEEPING_QTY )   DPHMCD_SAFE_KEEPING_QTY                        
 ,sum(DPHMCD_LOCKIN_QTY )   DPHMCD_LOCKIN_QTY                        
...
group by dphmcd_dpm_id,dphmcd_dpam_id,dphmcd_isin                         
having (sum(dphmcd_curr_qty) <> '0'                        
or sum(DPHMCD_FREE_QTY) <> '0'                        
or sum(DPHMCD_FREEZE_QTY) <> '0'                        
or sum(DPHMCD_PLEDGE_QTY ) <> '0'                        
...
-- commented as on 16032020 - rmat pending qty issue - changes done by tushar p
--update a      
...
-- added as on 16032020 - rmat pending qty issue - changes done by tushar p
...
SELECT * INTO #TMP_CDSL_UNPLEDGE_CONF FROM    cdsl_holding_dtls          a            with (nolock)
...
SELECT  * ,  CASE WHEN CDSHM_TRAS_DT > 'OCT 11 2025' THEN citrus_usr.fn_splitval_by (a.cdshm_trans_cdas_code,52,'~') ELSE '' END   epTXNNO 
INTO #TMP_CDSL_UNPLEDGE_CONF_EPTXN   FROM #TMP_CDSL_UNPLEDGE_CONF  A 

select * INTO #TMP_EP_REJ  from cdsl_holding_dtls  b with(nolock) where b.CDSHM_TRATM_CD ='4466'  
and cdshm_tras_dt between @l_min_trx_dt and @PA_TO_DT
and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to 

CREATE NONCLUSTERED  INDEX IX_1 ON #TMP_CDSL_UNPLEDGE_CONF_EPTXN (epTXNNO,cdshm_tratm_cd,cdshm_tras_dt,cdshm_ben_acct_no,cdshm_tratm_type_desc)
CREATE NONCLUSTERED INDEX IX_1 ON #TMP_EP_REJ (cdshm_trans_no,cdshm_tratm_cd,cdshm_tras_dt,cdshm_ben_acct_no,cdshm_isin)



select cdshm_dpam_id  ,cdshm_isin 
,case when cdshm_tratm_cd in ('2230','2246') and cdshm_tratm_type_desc = 'PLEDGE' then cdshm_qty 
when cdshm_tratm_cd in ('2280') and cdshm_tratm_type_desc in ('UNPLEDGE','CONFISCATE','AUTO PLEDGE','PLEDGE') and 
not exists (select 1 from #TMP_EP_REJ  b with(nolock) where b.cdshm_trans_no = epTXNNO
                and b.CDSHM_BEN_ACCT_NO = a.CDSHM_BEN_ACCT_NO
                and a.cdshm_isin = b.cdshm_isin
                and b.CDSHM_TRATM_CD ='4466' 
                ) 
then cdshm_qty else 0 end QTY                    
                  ,case when cdshm_tratm_cd in ('2280') and cdshm_tratm_type_desc in ('UNPLEDGE','CONFISCATE','AUTO PLEDGE','PLEDGE') and 
 exists (select 1 from #TMP_EP_REJ  b with(nolock) where b.cdshm_trans_no = epTXNNO
                and b.CDSHM_BEN_ACCT_NO = a.CDSHM_BEN_ACCT_NO
                and a.cdshm_isin = b.cdshm_isin
                and b.CDSHM_TRATM_CD ='4466' 
                ) 
then cdshm_qty else 0 end  rej_qty 
INTO #TEMPDATA2 from   #TMP_CDSL_UNPLEDGE_CONF_EPTXN          a            with (nolock)
where cdshm_tratm_cd in ('2246','2277','2201','2230','5101','2280')   
and cdshm_tras_dt between @l_min_trx_dt and @PA_TO_DT
and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to 
and cdshm_tratm_type_desc in ('UNPLEDGE','CONFISCATE','AUTO PLEDGE','PLEDGE')

select * from  #TEMPDATA2  where cdshm_isin ='INE155A01022'

select cdshm_dpam_id , cdshm_isin , sum(QTY) qty , sum(rej_qty) rej_qty  into #trasdmtpenfing2  from #TEMPDATA2
group by cdshm_dpam_id,cdshm_isin

PRINT CONVERT(VARCHAR(26), GETDATE(), 109)


select dphmcd_dpam_id,dphmcd_isin,sum(qty) qty , sum(rej_qty) rej_qty  into #finaldmatpen2 from (
select dphmcd_dpam_id,dphmcd_isin,qty , 0 rej_qty from #holdingdmtpenfing2
union ALL
select cdshm_dpam_id , cdshm_isin,qty , rej_qty from #trasdmtpenfing2) a 
group by dphmcd_dpam_id,dphmcd_isin
PRINT CONVERT(VARCHAR(26), GETDATE(), 109)

--select * from #finaldmatpen2  where dphmcd_isin ='INE155A01022'



update a      
set a.DPHMCD_PLEDGE_QTY  = case when QTY >= 0 then QTY       else 0 end  
--, a.DPHMCD_CURR_QTY = a.DPHMCD_CURR_QTY + (rej_qty *-1)
...
update a set DPHMCD_EARMARK_QTY = DPHMCD_EARMARK_QTY + qty
 from #EARMARKQTY2  , #tmp_dp_daily_hldg_cdsl  a
where dphmcd_dpam_id = cdshm_dpam_id 
and dphmcd_isin = cdshm_isin 
