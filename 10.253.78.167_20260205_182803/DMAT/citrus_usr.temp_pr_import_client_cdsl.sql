-- Object: PROCEDURE citrus_usr.temp_pr_import_client_cdsl
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--exec temp_pr_import_client_cdsl 'CDSL','HO','normal','','*|~*','|*~|',''  
	...
	CREATE PROCEDURE  [citrus_usr].[temp_pr_import_client_cdsl]
				(
				@pa_exch          VARCHAR(20)    
...
print('a')
	...
	--          and    clim_crn_no =  dpam_crn_no   
			  ...
			  when TMPCLI_ACCT_STAT = 'CLOSED' then '05' else TMPCLI_ACCT_STAT end  
	...
	SELECT * from client_bank_accts cliba  
					   ,    dp_acct_mstr   
					   ,     TMP_CLIENT_DTLS_MSTR_CDSL   
	...
	if not exists(select dppd_id from dp_poa_dtls , TMP_CLIENT_DTLS_MSTR_CDSL , dp_acct_mstr where dpam_id = dppd_dpam_id and TMPCLI_BOID = dpam_sba_no and dpam_sba_no = @c_ben_acct_no1)  
    ...
    SET @l_dppd_details = ''  
    SELECT @l_dppd_details = isnull(convert(varchar,@l_compm_id),'')+'|*~|'+ isnull(convert(varchar,@l_excsm_id),'')+'|*~|'+isnull(convert(varchar,@l_dpm_dpid),'')+'|*~|'+isnull(@c_ben_acct_no1,'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ ISNULL(ltrim(rtrim('1ST HOLDER')),'') +'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_POA_NM)),'') +'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_POA_ID)),'')+'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_POA_TYPE)),'')+'|*~|'+CASE WHEN ISDATE(CONVERT(datetime,TMPCLI_SETUP_DT,103))= 1 THEN convert(varchar,CONVERT(datetime,TMPCLI_SETUP_DT,103),103) ELSE '' END   
    +'|*~|'+CASE WHEN ISDATE(CONVERT(datetime,TMPCLI_POA_START_DT,103))= 1 THEN convert(varchar,CONVERT(datetime,TMPCLI_POA_START_DT,103),103) ELSE '' END +'|*~|'+CASE WHEN ISDATE(CONVERT(datetime,TMPCLI_POA_END_DT,103))= 1 THEN convert(varchar,CONVERT(datetime,TMPCLI_POA_END_DT,103),103) ELSE '' END   
    +'|*~|'+ltrim(rtrim(poam_master_id))+'|*~|'+'0'+'|*~|'+'|*~|'+'A*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL , poa_mstr WHERE ltrim(rtrim(poam_name1)) = ltrim(rtrim(TMPCLI_POA_NM)) and  tmpcli_boid = @c_ben_acct_no1 AND TMPCLI_POA_ID <>''   
...
fetch next from @c_client_summary1 into @c_ben_acct_no1   
	...
	--                            ) and banm_micr = TMPCLI_BANK_CD  
	     ...
	     --SELECT  @L_CLIENT_VALUES = ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_NM)),'') + '|*~|' + '|*~||*~|'+ ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_NM)),'') + '|*~|'+ISNULL(TMPCLI_BO_SEX,'')+'|*~|'+ISNULL(TMPCLI_BO_DOB,'')+'|*~||*~|ACTIVE|*~||*~|1|*~|' + ISNU	LL(ltrim(rtrim(TMPCLI_FRST_HLDR_PAN_N0)),'') +'|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL WHERE  TMPCLI_BOID = @c_ben_acct_no  
	     SELECT  @L_CLIENT_VALUES = ISNULL(ltrim(rtrim(case when @@BOCTGRY in ('21' , '24' , '27') then CITRUS_USR.inside_trim(LTRIM(RTRIM(TMPCLI_FRST_HLDR_NM)),'1') else LTRIM(RTRIM(TMPCLI_FRST_HLDR_NM)) end)),'') + '|*~|' + ISNULL(case when @@BOCTGRY	in ('21' , '24' , '27') then CITRUS_USR.inside_trim(LTRIM(RTRIM(TMPCLI_FRST_HLDR_NM)),'2') else '' end,'') + '|*~|' + ISNULL(case when @@BOCTGRY in ('21' , '24' , '27') then CITRUS_USR.inside_trim(LTRIM(RTRIM(TMPCLI_FRST_HLDR_NM)),'3') else '.' end,'') + 	'|*~|'+ LEFT(LTRIM(RTRIM(ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_PAN_N0)),'') + ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_NM)),''))),50) + '|*~|'+ISNULL(TMPCLI_BO_SEX,'')+'|*~|'+CASE WHEN isdate(CONVERT(datetime,TMPCLI_BO_DOB,103)) = 1 THEN  ISNULL(TMPCLI_BO_DOB,''	) ELSE '' END +'|*~||*~|ACTIVE|*~||*~|1|*~|' + ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_PAN_N0)),'') +'|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL WHERE  TMPCLI_BOID = @c_ben_acct_no           
...
select @pa_nom_dtls = CITRUS_USR.inside_trim(isnull(ltrim(rtrim(TMPCLI_NOMI_NM)),''),1)+'|*~|'+CITRUS_USR.inside_trim(isnull(ltrim(rtrim(TMPCLI_NOMI_NM)),''),2)+'|*~|'+CITRUS_USR.inside_trim(isnull(ltrim(rtrim(TMPCLI_NOMI_NM)),''),3)+'|*~|'+'|*~|'+'|*~|'+'|*~|'+'|*~|*|~*'  FROM   TMP_CLIENT_DTLS_MSTR_CDSL WHERE TMPCLI_BOID = @c_ben_acct_no   
...
select @l_crn_no = dpam_crn_no from dp_acct_mstr,  TMP_CLIENT_DTLS_MSTR_CDSL  where dpam_sba_no  =  tmpcli_boid and tmpcli_boid = @c_ben_acct_no  
...
--account_properties  
SET @L_ACCP_VALUE = ''  
SELECT DISTINCT  @L_ACCP_VALUE = CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper(ISNULL(LTRIM(RTRIM(TMPCLI_SEBI_REGI_NO)),'')) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR WHERE TMPCLI_BOID = @c_ben_acct_no AND ACCPM_PROP_CD
= 'SEBI_REG_NO'  
...
SET @l_dppd_details = ''  
SELECT @l_dppd_details =   isnull(convert(varchar,@l_compm_id),'')+'|*~|'+ isnull(convert(varchar,@l_excsm_id),'')+'|*~|'+isnull(convert(varchar,@l_dpm_dpid),'')+'|*~|'+isnull(@c_ben_acct_no,'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ ISNULL(ltrim(rtrim('1ST HOLDER')),'') +'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_POA_NM)),'') +'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_POA_ID)),'')+'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_POA_TYPE)),'')+'|*~|'+CASE WHEN ISDATE(CONVERT(datetime,TMPCLI_SETUP_DT,103))= 1 THEN convert(varchar,CONVERT(datetime,TMPCLI_SETUP_DT,103),103) ELSE '' END +'|*~|'+CASE WHEN ISDATE(CONVERT(datetime,TMPCLI_POA_START_DT,103))= 1 THEN convert(varchar,CONVERT(datetime,TMPCLI_POA_START_DT,103),103) ELSE '' END +'|*~|'+CASE WHEN ISDATE(CONVERT(datetime,TMPCLI_POA_END_DT,103))= 1 THEN convert(varchar,CONVERT(datetime,TMPCLI_POA_END_DT,103),103) ELSE '' END +'|*~|'+ltrim(rtrim(poam_master_id))+'|*~|'+'0'+'|*~|'+'|*~|'+'A*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL , poa_mstr WHERE  ltrim(rtrim(poam_name1)) = ltrim(rtrim(TMPCLI_POA_NM)) and tmpcli_boid = @c_ben_acct_no  AND TMPCLI_POA_ID <>''          
print @l_dppd_details  
