-- Object: PROCEDURE citrus_usr.pr_ins_upd_trx_cdsl_sp_rejlogic
...
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_trx_cdsl_sp_rejlogic](@pa_id              VARCHAR(8000)    
													 ,@pa_tab             VARCHAR(20)  
													 ...
													 ,@pa_res_cd varchar(25)
													 ,@pa_res_desc varchar(250)   
													 ,@pa_rmks            VARCHAR(250)    
													...
													,@PA_UCC VARCHAR(20)
													 ...
													 ,@PA_CUSPA_FLG CHAR(1)
,@PA_CUSPA_CLTID VARCHAR(8)
,@PA_CUSPA_CMBP_DPID VARCHAR(8)
...
--  
  ...
  ,DPTDC_DELETED_IND                     smallint   
				...
				,DPTDC_CUSPA_FLG CHAR(1)
,DPTDC_CUSPA_CLTID VARCHAR(8)
,DPTDC_CUSPA_CMBP_DPID VARCHAR(8)
  )  
...
set @pa_cash_trf      = case when @pa_cash_trf = 'NA' then 'X'  else @pa_cash_trf end 
...
--  
  ...
  --  
    ...
    --  
      ...
      SET @l_qty                = convert(numeric,citrus_usr.fn_splitval(@currstring_value,3))   
      SET @l_id                = citrus_usr.fn_splitval(@currstring_value,4)   
      SET @l_qty               = CONVERT(VARCHAR(25),CONVERT(NUMERIC(18,3),-1*@l_qty                ) )  
      set @l_high_val_yn        = case when @l_val_yn = 'Y' then 'Y' else citrus_usr.fn_get_high_val(@l_isin,abs(@l_qty),'HIGH_VALUE',@pa_dpam_acct_no,'') end
PRINT @l_qty
    --  
    END  
    ELSE  
...
,DPTDC_TMCMID=@PA_TMCMID
	...
	,DPTDC_CUSPA_FLG=@PA_CUSPA_FLG
,DPTDC_CUSPA_CLTID=@PA_CUSPA_CLTID
,DPTDC_CUSPA_CMBP_DPID=@PA_CUSPA_CMBP_DPID
...
,DPTDC_CUSPA_CMBP_DPID
                    ...
                    ,@pa_login_name  
                    ,getdate()  
                    ,case when @l_high_val_yn = 'Y' then -1 else 0 end  --@l_deleted_ind  
       , @pa_trdReasoncd       
	   ,@pa_Paymode 
			,@pa_Bankacno 
			,@pa_Bankacname 
...
,DPTDC_CUSPA_CMBP_DPID
            ...
            --  
              ...
              SET @@c_access_cursor =  CURSOR  forward_only static FOR        
              SELECT dptdc_id, dptdc_deleted_ind  FROM dptdc_mak 
              where dptdc_dtls_id = convert(numeric,@currstring) and dptdc_deleted_ind in (0,4,6,-1)  
              ...
              --        
                 ...
                 --  
                   ...
                   SET     dptdc_dpam_id               = dptdcm.dptdc_dpam_id  
                          ...
                          ,dptdc_rmks                  = case when isnull(@pa_rmks,'')='' then @pa_res_desc else isnull(@pa_rmks,'') end --dptdcm.dptdc_rmks  
					...
					,DPTDC_TMCMID=dptdcm.DPTDC_TMCMID     
											...
											,DPTDC_CUSPA_FLG=dptdcm.DPTDC_CUSPA_FLG
,DPTDC_CUSPA_CLTID=dptdcm.DPTDC_CUSPA_CLTID
,DPTDC_CUSPA_CMBP_DPID=dptdcm.DPTDC_CUSPA_CMBP_DPID
...
,DPTDC_CUSPA_CMBP_DPID
                  ...
                  ,dptdc_cash_trf  
                  ,dptdc_cm_id  
                  ,case when isnull(@pa_rmks,'')='' then @pa_res_desc else isnull(@pa_rmks,'') end --isnull(@pa_rmks,'')--dptdc_rmks  
                  ,dptdc_created_by  
                  ,dptdc_created_dt  
                  ,@pa_login_name--dptdc_lst_upd_by  
                  ,getdate()--dptdc_lst_upd_dt  
                  ,1  
                     ,dptdc_reason_cd  
...
,DPTDC_CUSPA_CMBP_DPID
                   ...
                   --  
                    ...
                    if exists(select dptdc_id from dptdc_mak where dptdc_id = convert(numeric,@c_dptd_id) AND dptdc_deleted_ind = 0 )
                     ...
                     SET    dptdc_deleted_ind = 1  
                          --, dptdc_lst_upd_dt  = getdate()  
                          , dptdc_lst_upd_dt  = case when isnull(dptdc_mid_chk,'') <> '' then dptdc_lst_upd_dt else getdate()   end 
                          , dptdc_lst_upd_by  = @pa_login_name  
                     WHERE  dptdc_id          = convert(numeric,@c_dptd_id)  
                     ...
                     SET    dptdc_deleted_ind = 0  
						  ...
						  , dptdc_rmks = case when isnull(@pa_rmks,'')='' then @pa_res_desc else isnull(@pa_rmks,'') end  --isnull(@pa_rmks,'')
                     WHERE  dptdc_id          = convert(numeric,@c_dptd_id)  
                     AND    dptdc_deleted_ind = -1
                     end   
                 ...
                 BEGIN  
                 --  
                   delete used from dp_trx_dtls_cdsl,dp_acct_mstr,used_slip used where dpam_id = dptdc_dpam_id 
                   and uses_dpam_acct_no = dpam_sba_no and dptdc_id          = convert(numeric,@c_dptd_id)   
                   and ltrim(rtrim(uses_slip_no)) = ltrim(rtrim(dptdc_slip_no))
  ...
  PRINT 'SHILPA'
               ...
               UPDATE dptdc_mak   
               SET    dptdc_res_cd = @pa_res_cd 
					, dptdc_res_desc = @pa_res_desc 
                    , dptdc_deleted_ind = 0
                    , dptdc_lst_upd_dt  = getdate()  
                    , dptdc_lst_upd_by  = @pa_login_name  
  		...
  		UPDATE maker_scancopy   
                 SET  llst_upd_by=@pa_login_name  
  				,lst_upd_dt =getdate()
                      
                 WHERE  ref_no     = convert(numeric,@currstring)  
               AND  deleted_ind=1
