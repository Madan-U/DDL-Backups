-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_2_21042012
...
--pr_dp_select_mstr_2 '','PLEDGEE_SELM','HO','4','','','','','7','','','','','1*|~*',307,'*|~*','|*~|',''         
...
CREATE procedure [citrus_usr].[pr_dp_select_mstr_2_21042012]                                                
(@pa_id             VARCHAR(20)                                                  
...
BEGIN                
		...
		SELECT DISTINCT Convert(varchar(11),CDSHM_TRAS_DT,103) DATE,CDSHM_ISIN ISIN,ISIN_NAME=ISNULL(ISIN_NAME,''),                 
		Last_Closing_Rate=                
		isnull((                
		SELECT top 1 clopm_cdsl_rt  FROM CLOSING_PRICE_MSTR_CDSL WHERE                 
		CLOPM_DT < @pa_search_c3 and CDSHM_ISIN = CLOPM_ISIN_CD order by CLOPM_DT desc                
		),0)                 
		FROM CDSL_HOLDING_DTLS LEFT OUTER JOIN ISIN_MSTR ON CDSHM_ISIN = ISIN_CD
		WHERE  NOT EXISTS (SELECT CLOPM_ISIN_CD FROM CLOSING_PRICE_MSTR_CDSL WHERE                 
    ...
    ,disp_cons_no CONS_NO           
     ...
     --           
		...
		,disp_cons_no CONS_NO                    
		 ...
		 AND CASE WHEN @PA_SEARCH_C3 <> '' THEN @PA_SEARCH_C3 ELSE '' END = CASE WHEN @PA_SEARCH_C3 <> '' THEN disp_cons_no ELSE '' END                
		 AND  ISNULL(DEMRM_TRANSACTION_NO,'')<>''  
         and DEMRM_SLIP_SERIAL_NO = case when @PA_SEARCH_C4 <> '' then @PA_SEARCH_C4 else '' end                    
		 AND DPAM_DELETED_IND = 1
		 AND ENTM_DELETED_IND = 1
		 AND DEMRM_DELETED_IND = 1
     --                
