...
select DPAM_BBO_CODE AS PARTY_CODE, DPAM_SBA_NO CLIENT_CODE, sum(clic_charge_amt)*-1 BROK_BAL, '0' CURR_BAL  from             
 ...
 dp_acct_mstr , client_charges_cdsl where clic_dpam_id = dpam_id         
  ...
  and not exists (select ldg_account_id from ledger12 where LDG_ACCOUNT_ID=dpam_id and LDG_DELETED_IND=1 and LDG_VOUCHER_TYPE='5'        
  and LDG_VOUCHER_DT=dbo.ufn_GetLastDayOfMonth(DATEADD(month,-1,GETDATE()))        
  union all        
  select ldg_account_id from ledger12 where LDG_ACCOUNT_ID=dpam_id and LDG_DELETED_IND=1 and LDG_VOUCHER_TYPE='5'        
  and LDG_VOUCHER_DT=dbo.ufn_GetLastDayOfMonth(DATEADD(month,-1,GETDATE())))        
  ...
  from LEDGER12, DP_ACCT_MSTR where LDG_ACCOUNT_ID = DPAM_ID and LDG_ACCOUNT_TYPE ='P' and LDG_DELETED_IND = 1 and DPAM_DELETED_IND = 1         
...
-----select * from dbo.ufn_GetLastDayOfMonth(DATEADD(month,-1,GETDATE())))       
   ...
   ---CHANGES DOE UNDER SRE SRE_35452-ledger12 to LEDGER12
