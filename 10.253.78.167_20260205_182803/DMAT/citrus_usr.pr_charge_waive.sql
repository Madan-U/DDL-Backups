-- Object: PROCEDURE citrus_usr.pr_charge_waive
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--pr_charge_waive '3','chargetype','','0','0','','','','',''
--pr_charge_waive '3','TRANSACTION','EP-DR','1205810000000615','1205810000000615','apr 01 2009','apr 30 2009','INE772A01016','',''
--pr_charge_waive '3','WaiveFlagupdate','','0','0','apr 01 2009','apr 30 2009','','1,2,3,4,10',''
--select * from waivedata
CREATE proc [citrus_usr].[pr_charge_waive]
(@pa_id numeric
,@pa_action varchar(100)
,@pa_cd varchar(100)
,@pa_from_acct numeric(18,0)
,@pa_to_acct numeric(18,0)
,@pa_from_dt datetime
,@pa_to_dt datetime
,@pa_isin  varchar(50)
,@pa_filter varchar(8000)
,@pa_out varchar(8000) out)
as
begin 

declare @l_dpm_id numeric
select @l_dpm_id =  dpm_id from dp_mstr where default_dp = dpm_excsm_id and dpm_excsm_id = @pa_id  and dpm_deleted_ind = 1   

if @pa_from_acct = 0 
begin
set  @pa_from_acct = 0 
if @pa_to_acct = 0
  set  @pa_to_acct = '999999999999999999' 
end 

if @pa_action = 'chargetype'
begin
select distinct trastm_id                   
      ,trastm_cd  code                
      ,trastm_desc  description                
   from   transaction_sub_type_mstr                 
     , transaction_type_mstr, charge_mstr                  
   where  trastm_tratm_id          = trantm_id                
   and    trastm_cd  = CHAM_CHARGE_TYPE
   and    trantm_code              = 'INT_TRANS_TYPE_CDSL'             
   and    trantm_deleted_ind       = 1            
   order by trastm_cd            
end 
----pass code 
----drop table waivedata 
--create table waivedata(id numeric identity(1,1)
-- , transaction_dt datetime
-- , tran_no varchar(50)
-- , tratm_Cd varchar(50)
-- , client_id varchar(25)
-- , isin varchar(50)
-- , qty numeric(18,0)
-- , waive_yn char(1))

if @pa_action = 'TRANSACTION'
begin
    --delete  from waivedata where transaction_dt between @pa_from_dt and @pa_to_dt
    --alter table CDSL_HOLDING_DTLS ADD WAIVE_FLAG CHAR(1)
	--insert into waivedata
	select distinct convert(varchar(11),cdshm_tras_dt,109) cdshm_tras_dt,CDSHM_TRANS_NO,cdshm_tratm_cd,CDSHM_BEN_ACCT_NO,CDSHM_ISIN,CDSHM_QTY, cdshm_tratm_type_desc,case when WAIVE_FLAG='Y' then 'YES' else 'NO' end  flag from cdsl_holding_dtls      
	,client_dp_brkg
	,profile_charges
	,charge_mstr 
	where cdshm_dpm_id = @l_dpm_id 
	--AND   month(cdshm_tras_dt) = 4 and year(cdshm_tras_dt) = 2009 
	AND   cdshm_tras_dt between  @pa_from_dt and @pa_to_dt
    and   CDSHM_BEN_ACCT_NO between @pa_from_acct and @pa_to_acct
    and   CDSHM_ISIN like case when @pa_isin = '' then '%' else @pa_isin+'%' end
	and   cdshm_tras_dt >= clidb_eff_from_dt and  cdshm_tras_dt <= clidb_eff_to_dt
	AND   cdshm_dpam_id = clidb_dpam_id 
	AND   clidb_brom_id = proc_profile_id
	AND   proc_slab_no  = cham_slab_no
	AND cdshm_tratm_type_desc = @pa_cd
	    --AND   (ISNULL(cdshm_charge,0) <> 0 OR  ISNULL(CDSHM_DP_CHARGE,0) <> 0)
	AND   isnull(cham_charge_graded,0) <> 1
	AND   cham_charge_type = case when cdshm_tratm_type_desc in ('NSCCL-DR','CSECH-DR','BSECH-DR','ASECH-DR','OTCEI-DR','DSECH-DR') then 'EXCHPAYIN'
								  when cdshm_tratm_type_desc in ('NSCCL-CR','CSECH-CR','BSECH-CR','ASECH-CR','OTCEI-CR','DSECH-CR') then 'EXCHPAYOUT'
								  else cdshm_tratm_type_desc end 
	AND   cdshm_tratm_cd in ('2277','2246')
	and   cham_deleted_ind = 1
	and   clidb_deleted_ind = 1
	and   cdshm_deleted_ind = 1
	and   CHAM_CHARGE_BASE = 'NORMAL' 
	and   proc_deleted_ind = 1
    



	
end 
--return with id with comma seperator
--1,2,3,4
if @pa_action = 'WaiveFlagupdate'
begin

--declare @l_sql varchar(1000)
--set @l_sql  = 'update waivedata set waive_yn = ''Y'' where id in ( ' + @pa_filter + ' )'
--print @l_sql  
--exec(@l_sql)
--select * from waivedata where transaction_dt between @pa_from_dt and @pa_to_dt

UPDATE A SET WAIVE_FLAG = TMP_STATUS , cdshm_charge = 0 , CDSHM_DP_CHARGE = 0
FROM cdsl_holding_dtls A
, TMP_TRXREC_WAIVEDOFF_CHRG B
WHERE A.cdshm_tras_dt = B.TMP_TRXDATE 
AND A.CDSHM_TRANS_NO = B.TMP_TRXNO
AND A.cdshm_tratm_cd = B.TMP_TRATM_CD
AND A.CDSHM_BEN_ACCT_NO = B.TMP_BEN_ACCT_NO
AND A.CDSHM_ISIN = B.TMP_ISIN_CD
AND A.CDSHM_QTY = B.TMP_QTY
AND A.cdshm_tratm_type_desc = ltrim(rtrim(B.TMP_TRANS_DESC))

end 
--

--whenever billing completed then join with above table and waive charge which have flag 'Y'


end

GO
