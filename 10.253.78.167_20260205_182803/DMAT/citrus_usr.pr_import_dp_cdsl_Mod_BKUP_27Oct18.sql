-- Object: PROCEDURE citrus_usr.pr_import_dp_cdsl_Mod_BKUP_27Oct18
...
create PROCeDURE [citrus_usr].[pr_import_dp_cdsl_Mod_BKUP_27Oct18]( @pa_crn_no      varchar(8000)                 
                            , @pa_exch        VARCHAR(10)                  
 ...
 exec [citrus_usr].[pr_auto_nrn_poaid_mod] @pa_crn_no      
        ...
        insert into @line2( ln_no                  
                           ...
                           SELECT distinct '02'                  
								...
								,max(case when isnull(citrus_usr.fn_ucc_accp(dpam.dpam_id,'SMART_REG',''),'')  ='YES' then 'Y' else ' ' end )
...
--update bitmap_ref_mstr set bitrm_values=bitrm_values + convert(varchar,@l_06cnt ) where BITRM_PARENT_CD = 'POAID_AUTO'  
                                 ...
                                 insert into  @line7(ln_no                                    
									...
									,res_sec_flag   ,ln7_cust_adr_cntry_cd,ln7_cust_adr_state_cd,ln7_city_seq_no,ln7_Pri_mob_ISD      
                                            ...
                                            --and    climmod.clic_mod_action = @pa_mod_typ
                                            and    climmod.clic_mod_action like case when upper(@pa_mod_typ) = 'ALL' then '%%' else @pa_mod_typ end
                                            AND   climmod.clic_mod_action in ('G Address','G NameNDtls','G Contacts','Guardian Suffix', 'G NameChange','G FatherNameChange' ,'G MobChange', 'G ResPhChange', 'G OffPhChange', 'G DOBChange', 'G PANChange','GUARDIAN TITLE','NG Address','NG NameNDtls','NG Contacts','P ADDRESS') 
                                            ORDER BY Purpose_code  DESC
...
--            +CONVERT(char(1),l5.trading_id        )    
           ...
           +case when CONVERT(char(3),right(ltrim(rtrim(l7.Purpose_code)),3)) <>'212' then CONVERT(char(10),'L'+l7.hldr_in_tax_pan ) ELSE '' END 
