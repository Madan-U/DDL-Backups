-- Object: PROCEDURE citrus_usr.PR_TRATM_INS18
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

CREATE   PROCEDURE [citrus_usr].[PR_TRATM_INS18]
                AS
                BEGIN
                  --
                    DECLARE
                       ...
                       ,@l_sub_cd    VARCHAR(50)
                        SET  @I =1
                        SET @l_name ='HO'
                        SET @l_code='REMA_BAL_NSDL' 
                        SET @l_desc ='REMAINING BALANCE'
                      ...
                      DELETE FROM TRANSACTION_SUB_TYPE_MSTR WHERE TRASTM_TRATM_ID=(SELECT trantm_id FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'REMA_BAL_NSDL')
		      DELETE FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'REMA_BAL_NSDL'
		  ...
		  ,trantm_excm_id
		   ...
		   BEGIN
		    ...
		    BEGIN
		      --
			   SET @l_sub_cd   = 'B'
			   SET @l_sub_desc = 'BOTH REMATRIALIZE AND TRANSFERED'
		      --      
		    END 
		    ELSE IF @I = 2
		    BEGIN
