-- Object: PROCEDURE dbo.Pr_rpt_bill_profile_filter_NXT
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--PR_RPT_BILL_PROFILE_FILTER 	'CDSL',3,'JAN 26 2009','JUN 26 2009','N','N','1234567890123456','','','N','Y','',1,'HO|*~|',''	

create PROC [Pr_rpt_bill_profile_filter_NXT]              
@PA_DPTYPE VARCHAR(4),                      
@PA_EXCSMID INT,                      
...
set @p15=NULL
exec citrus_usr.Pr_rpt_bill_profile_filter_shilpa_arch @pa_dptype,@pa_excsmid,@pa_fromdate,
...
SET @PA_GROUP_CD =  ISNULL(CITRUS_USR.FN_SPLITVAL(@PA_GROUP_CD,1),'')  
 ...
 SELECT @@DPMID = DPM_ID FROM citrus_usr.DP_MSTR WITH(NOLOCK) WHERE DEFAULT_DP = @PA_EXCSMID AND DPM_DELETED_IND =1                      
 SELECT @@L_CHILD_ENTM_ID    =  CITRUS_USR.FN_GET_CHILD(@PA_LOGIN_PR_ENTM_ID , @PA_LOGIN_ENTM_CD_CHAIN)
 SELECT @@LEDGERID =FIN_ID FROM citrus_usr.FINANCIAL_YR_MSTR WHERE FIN_DPM_ID = @@DPMID AND  (@PA_FROMDATE BETWEEN FIN_START_DT AND FIN_END_DT) AND FIN_DELETED_IND = 1
...
INTO #BBOCODE FROM citrus_usr.ACCOUNT_PROPERTIES , citrus_usr.DP_ACCT_MSTR  
...
PREV_BILL_BAL NUMERIC(18,2)
  ...
  BEGIN
    ...
    BEGIN 
		...
		FROM CITRUS_USR.fn_acct_list_bytushar (@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID,@PA_FROMACCID,@PA_TOACCID) , citrus_usr.CLIENT_DP_BRKG 
...
--SELECT * FROM #ACLIST WHERE DPAM_SBA_NO = '1201090000004621'
 ...
 BEGIN
		DELETE A FROM #ACLIST A,citrus_usr.BLK_CLIENT_PRINT_DTLS 
...
,CITRUS_USR.FN_SPLITVAL_BY(CDSHM_TRANS_CDAS_CODE,9,'~') EXETIME INTO #TMP_CDSL_HOLDING_DTLS FROM citrus_usr.CDSL_HOLDING_DTLS
...
AND CDSHM_BEN_ACCT_NO BETWEEN @PA_FROMACCID AND @PA_TOACCID
 ...
 BEGIN                
	...
	SELECT @PA_PREVFROMDATE = MAX(DPHMCD_HOLDING_DT) FROM citrus_usr.DP_DAILY_HLDG_CDSL WITH(NOLOCK) WHERE DPHMCD_HOLDING_DT <= CONVERT(VARCHAR(11),@PA_TODATE,109)
...
, '00:00:00', '00:00:00') END )
 ...
 LEFT OUTER JOIN  #VW_FETHOB   ON V_CDSHM_DPAM_ID = CDSHM_DPAM_ID 
    ...
    ,citrus_usr.ACCOUNT_GROUP_MAPPING G WITH(NOLOCK)               
    ,#ACLIST ACCOUNT                            
    WHERE   CDSHM_DPM_ID = @@DPMID            
    AND CDSHM_TRAS_DT >=@PA_FROMDATE AND CDSHM_TRAS_DT <=@PA_TODATE                      
    AND (CDSHM_TRATM_CD IN('2246','2277','2201','3102','2270','2220','2262','2251','2202','2205','2280','2255') OR ISNULL(CDSHM_CHARGE,0) <> 0)          
...
PRINT 'SS'
 ...
 BEGIN           
  ...
  FROM citrus_usr.CLIENT_CHARGES_CDSL WITH(NOLOCK)  
  ,citrus_usr.ACCOUNT_GROUP_MAPPING G WITH(NOLOCK)              
  , #ACLIST ACCOUNT             
  WHERE           
  CLIC_DPM_ID = @@DPMID                      
 ...
 END  
   ...
   END 	
	...
	) TMP LEFT OUTER JOIN citrus_usr.LEDGER' + CONVERT(VARCHAR,@@LEDGERID) + ' ON DPAM_ID = LDG_ACCOUNT_ID AND LDG_ACCOUNT_TYPE = ''P'' AND LDG_VOUCHER_DT <= ''' + CONVERT(VARCHAR(11),@PA_TODATE,109) + ''' AND LDG_DELETED_IND = 1
	GROUP BY DPAM_ID'
	EXEC(@@SSQL)
...
when  isnull(TRANS_DESC,'')='1_ACMAIN' then 'ANNUAL MAINTAINENCE CHARGES'
	...
	CDSHM_TRATM_DESC= UPPER(LTRIM(RTRIM(TRANS_DESC))),              
...
,case when  isnull(cl_gst_no,'')  <> '' then 'Y' else 'N' end GST_no_YN 
	...
	left outer join citrus_usr.bo_bill_inv_mapping on boid = dpam_acctno and month(trans_date) = billmonth and YEAR(trans_date)= billyear
...
, CITRUS_USR.FN_UCC_accp(dpam_id ,'CLIENT GST','')  cl_gst_no
 FROM Ecitrus_usr.NTITY_RELATIONSHIP E,citrus_usr.ENTITY_MSTR,citrus_usr.TIN_MSTR , citrus_usr.dp_Acct_mstr WHERE GETDATE() BETWEEN ENTR_FROM_DT AND ISNULL(ENTR_TO_DT,'DEC 31 2100')
AND ENTR_DUMMY5=ENTM_ID and DPAM_SBA_NO = ENTR_SBA 
AND REPLACE(ENTM_SHORT_NAME,'_BL','')=STATE_NAME) v on   v.boid=dpam_acctno
	LEFT OUTER JOIN  citrus_usr.ISIN_MSTR I WITH(NOLOCK) ON T.ISIN_CD = I.ISIN_CD
 LEFT OUTER JOIN citrus_usr.CLOSING_PRICE_MSTR_CDSL ON T.ISIN_CD = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( SELECT TOP 1 CLOPM_DT FROM CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = T.ISIN_CD AND CLOPM_DT <= @PA_TODATE AND CLOPM_DELETED_IND = 1  ORDER BY CLOPM_DT DESC)
...
, CASE WHEN ISNULL(CITRUS_USR.FN_UCC_ENTP(ENTR_BR,'BL',''),'')<>'' THEN CITRUS_USR.FN_UCC_ENTP(ENTR_BR,'BL','') + '/' + STATE_CODE ELSE '' END  MULTLOC,STATE_CODE
, CITRUS_USR.FN_UCC_accp(dpam_id ,'CLIENT GST','')  cl_gst_no
 FROM citrus_usr.ENTITY_RELATIONSHIP E,citrus_usr.ENTITY_MSTR,citrus_usr.TIN_MSTR , citrus_usr.dp_Acct_mstr WHERE GETDATE() BETWEEN ENTR_FROM_DT AND ISNULL(ENTR_TO_DT,'DEC 31 2100')
AND ENTR_DUMMY5=ENTM_ID and DPAM_SBA_NO = ENTR_SBA 
AND REPLACE(ENTM_SHORT_NAME,'_BL','')=STATE_NAME) v on   v.boid=dpam_acctno
	LEFT OUTER JOIN  citrus_usr.ISIN_MSTR I WITH(NOLOCK) ON T.ISIN_CD = I.ISIN_CD
LEFT OUTER JOIN citrus_usr.CLOSING_PRICE_MSTR_CDSL ON T.ISIN_CD = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( SELECT TOP 1 CLOPM_DT FROM CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = T.ISIN_CD AND CLOPM_DT <= @PA_TODATE AND CLOPM_DELETED_IND = 1  ORDER BY CLOPM_DT DESC)
 ...
 BEGIN                
	...
	SELECT @PA_PREVFROMDATE = MAX(DPDHMD_HOLDING_DT) FROM citrus_usr.DP_DAILY_HLDG_NSDL WITH(NOLOCK) WHERE DPDHMD_HOLDING_DT < @PA_FROMDATE
 ...
 BEGIN                 
  ...
  ,NSDHM_ISIN,NSDHM_SETT_TYPE,NSDHM_SETT_NO,CONVERT(NUMERIC,NSDHM_DPM_TRANS_NO),NSDHM_QTY,              
   ...
   FROM citrus_usr.NSDL_HOLDING_DTLS WITH(NOLOCK)  
    ,citrus_usr.ACCOUNT_GROUP_MAPPING G WITH(NOLOCK)                       
   ,#ACLIST ACCOUNT                      
   WHERE           
   NSDHM_DPM_ID = @@DPMID                      
 ...
 END         
  ...
  --FOR NON POOL ACCOUNTS 
   ...
   BEGIN              
	   ...
	   BEGIN                 
		 ...
		 FROM citrus_usr.FN_DAILYHOLDING(@@DPMID,@PA_PREVFROMDATE,@PA_FROMACCID,@PA_TOACCID,'',@PA_GROUP_CD,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID)            
	   ...
	   BEGIN   
		 ...
		 FROM citrus_usr.FN_DAILYHOLDING(@@DPMID,@PA_PREVFROMDATE,@PA_FROMACCID,@PA_TOACCID,'',@PA_GROUP_CD,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID) H,#ACLIST A
  ...
  BEGIN   
    ...
    FROM citrus_usr.CLIENT_CHARGES_NSDL WITH(NOLOCK)  
    ,citrus_usr.ACCOUNT_GROUP_MAPPING G WITH(NOLOCK)          
    , #ACLIST ACCOUNT             
    WHERE           
    CLIC_DPM_ID = @@DPMID                      
 ...
 END            
   ...
   BILL_DUE_DT =@@BILLDUEDATE,
   TRANS_DATE           
   FROM #TEMPRUNNING T WITH(NOLOCK) LEFT OUTER JOIN citrus_usr.ISIN_MSTR I WITH(NOLOCK) ON T.ISIN_CD = I.ISIN_CD 
   LEFT OUTER JOIN citrus_usr.SETTLEMENT_TYPE_MSTR S WITH(NOLOCK) ON T.SETT_TYPE = S.SETTM_TYPE AND ISNULL(S.SETTM_TYPE,'') <> ''  AND S.SETTM_DELETED_IND = 1          
