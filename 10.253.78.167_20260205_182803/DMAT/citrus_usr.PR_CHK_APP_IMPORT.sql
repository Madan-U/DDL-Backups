-- Object: PROCEDURE citrus_usr.PR_CHK_APP_IMPORT
...
CREATE PROCEDURE [citrus_usr].[PR_CHK_APP_IMPORT]   
...
--    
  ...
  --  
    ...
    BEGIN  
    --  
if exists(select dptdc_id from dptdc_mak, TMP_OFFM_BRK_UPD where dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and   
isnull(dptdc_mid_chk,'')='')  
begin  
  
update t set  dptdc_deleted_ind=0 , dptdc_lst_upd_by=@PA_LOGIN_NAME,dptdc_mid_chk=@PA_LOGIN_NAME  from dptdc_mak t , TMP_OFFM_BRK_UPD where   
dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and isnull(dptdc_mid_chk,'')=''   
  
UPDATE filetask  
SET    usermsg = 'THIS IS HIGH VALUE/DORMANT TRANSACTION ,NEED SECOND VERIFICATION FILE UPLOAD WITH SAME OUT FILE BUT WITH DIFFERENT LOGIN'  
,      STATUS = 'COMPLETED'   
      ...
      INSERT INTO DP_TRX_DTLS_CDSL  
         ...
         --  
           ...
           set    dptdc_deleted_ind = case when dptdc_deleted_ind = -1 then 0 when dptdc_deleted_ind = 0 then 1 end   
                 ,dptdc_mid_chk = case when dptdc_deleted_ind = -1 then @pa_login_name  when (dptdc_deleted_ind = 0 and isnull(dptdc_mid_chk,'')='') then null else dptdc_mid_chk end   
         ...
         --  
           ...
           ,dptdc_mid_chk     = case when dptdc_deleted_ind = -1 then @pa_login_name when (dptdc_deleted_ind = 0 and isnull(dptdc_mid_chk,'')='') then null else dptdc_mid_chk  end --'HO'  
...
if exists(select dptdc_id from dptdc_mak, TMP_INTDP_BRK_UPD where dptdc_id = TMPBRK_INT_REF_NO 
and dptdc_deleted_ind=-1 and   
isnull(dptdc_mid_chk,'')='')  
begin  
  print 'yeh'
update t set  dptdc_deleted_ind=0 , dptdc_lst_upd_by=@PA_LOGIN_NAME,dptdc_mid_chk=@PA_LOGIN_NAME 
 from dptdc_mak t , TMP_INTDP_BRK_UPD where   
dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and isnull(dptdc_mid_chk,'')=''   
...
return  
      ...
      if exists(select dptdc_id from dptdc_mak, TMP_INTDP_BRK_UPD where 
      dptdc_id = TMPBRK_INT_REF_NO and dptdc_created_by = @pa_login_name)  
      begin   
         UPDATE filetask  
      SET    usermsg = 'File Could Not Be Uploaded Please Verify. ERROR : Checker Login and Maker Login must be different.'  
      ,      STATUS = 'FAILED'   
...
if exists(select dptdc_id from dptdc_mak, TMP_EP_BRK_UPD where dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and   
isnull(dptdc_mid_chk,'')<>'')  
begin  
  
update t set  dptdc_deleted_ind=0 , dptdc_lst_upd_by=@PA_LOGIN_NAME,dptdc_mid_chk=@PA_LOGIN_NAME  from dptdc_mak t , TMP_EP_BRK_UPD where   
dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and isnull(dptdc_mid_chk,'')<>''   
...
if exists(select dptdc_id from dptdc_mak, TMP_NP_BRK_UPD where dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and   
isnull(dptdc_mid_chk,'')<>'')  
begin  
  
update t set  dptdc_deleted_ind=0 , dptdc_lst_upd_by=@PA_LOGIN_NAME,dptdc_mid_chk=@PA_LOGIN_NAME  from dptdc_mak t , TMP_NP_BRK_UPD where   
dptdc_id = TMPBRK_INT_REF_NO and dptdc_deleted_ind=-1 and isnull(dptdc_mid_chk,'')<>''   
