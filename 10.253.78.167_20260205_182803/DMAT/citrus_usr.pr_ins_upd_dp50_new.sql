-- Object: PROCEDURE citrus_usr.pr_ins_upd_dp50_new
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--[pr_ins_upd_dp50] 'ho','12054100',21
--SELECT * FROM DP_MSTR
Create procedure [citrus_usr].[pr_ins_upd_dp50_new](@pa_login_name varchar(25)
...
--
    ...
    select top 1 @l_dpmdpid = LEFT(tmpeod_boid,8) , @l_dpm_id  = dpm_id from tmp_cdsl_trx_mstr , dp_mstr where dpm_dpid = LEFT(tmpeod_boid,8) and dpm_deleted_ind = 1 and dpm_dpid = @pa_dpmdpid
    ...
    --
      ...
      update  tmp_cdsl_trx_mstr  
						...
						where   tmpeod_trx_no         = dptdc_trans_no
						and     dptdc_dpam_id           = tmpeod_dpam_id
						and     dptdc_isin              = tmpeod_isin
						and     tmpeod_busn_dt          = dptdc_execution_dt
						AND     dptdC_trastm_cd       = tmpeod_trx_cd
      ...
      IF EXISTS(SELECT tmpeod_boid FROM tmp_cdsl_trx_mstr WHERE tmpeod_dpam_id = 0 ) 																					
      BEGIN
      --
        UPDATE filetask
      ...
      update demat_request_mstr
					 ...
					 exec pr_holding_process_cdsl @l_holding_dt,@l_dpm_id,@pa_login_name,1
					COMMIT TRANSACTION
