-- Object: FUNCTION citrus_usr.fn_cdsl_export_line_7_extract
...
CREATE function [citrus_usr].[fn_cdsl_export_line_7_extract](@pa_crn_no      VARCHAR(8000)  
...
,''
 ...
 union
    ...
    SELECT distinct '07'                  
          ...
          ,case when  isnull(CUDM_EXID,'')='01' then '12' when  isnull(CUDM_EXID,'')='02' then '11'  when  isnull(CUDM_EXID,'')='12' then '12'  when  isnull(CUDM_EXID,'')='11' then '11' else '29' end  --isnull(DPPD_MNAME,'')   
...
--,isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('State',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE_ADR1'),''),5 )),'')  -- state code
--,isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE_ADR1'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOMINEE_ADR1'),''),4)),'' ) -- seqno
--,CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) <> ''
-- THEN convert(varchar(6),citrus_usr.FN_FORMATSTR(convert(varchar(6),lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMIC'),'')) ),6,0,'L','0'))   else '000000' END -- pri mob isd                   
...
--		  ,isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('Country',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOM_GUARDIAN_ADDR'),''),6 )),'') -- cntry code
--		  ,isnull(citrus_usr.FN_TOGETCDSLCOUNTRYSTATECODE('State',citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOM_GUARDIAN_ADDR'),''),5 )),'')  -- state code
--		  ,isnull(citrus_usr.Fn_Toget_CitySeqno(citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOM_GUARDIAN_ADDR'),''),7) ,citrus_usr.fn_splitval(ISNULL(citrus_usr.fn_acct_addr_value(dpam_id,'NOM_GUARDIAN_ADDR'),''),4)),'' ) -- seqno
--		  ,CASE WHEN lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMGIC'),'')) <> '' THEN 
--		  convert(varchar(6),citrus_usr.FN_FORMATSTR(convert(varchar(6),lower(isnull(citrus_usr.fn_dp_import_conc(clim.clim_crn_no,'NOMGIC'),'')) ),6,0,'L','0'))   else '000000' END-- pri mob isd                   
