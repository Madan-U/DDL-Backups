-- Object: PROCEDURE citrus_usr.pr_bulk_ins_dpb9_BKP_09AUG2025_SRE_39314
...
CREATE proc [citrus_usr].[pr_bulk_ins_dpb9_BKP_09AUG2025_SRE_39314]    
 (    
    @pa_exch          VARCHAR(20)        
...
insert into dpb9log
select GETDATE(),'dpb9optimise','start'

exec [pr_DPb9_optimise]   @pa_db_source


insert into dpb9log
select GETDATE(),'dpb9optimise','end'
...
truncate table DPB9_PC22 -- added line   23/06/2020
...
where left(#dpb9_source.value ,2) = '00'    
    ...
    create index ix_1 on #dpb9_pc0 (id)
...
where left(#dpb9_source.value ,2) = '01'    
 ...
 create index ix_1 on #dpb9_pc1 (id)
...
SET @@SSQL1 = ''    
SELECT @@SSQL1 = 'INSERT INTO DPB9_PC1(PurposeCode1,TypeOfTrans,Title,Name,MiddleName,SearchName,Suffix,FthName,Addr1,Addr2,Addr3,City,statecode,State
...
,AFILLER1,AFILLER6,AFILLER7,AFILLER8,AFILLER9,AFILLER10,AFILLER2,AFILLER3,AFILLER4,AFILLER5,NOMINATION_OPT_OUT
,NOMINATION_opt_dt
,COMM_PREFER,BOId,TransSystemDate) VALUES(''' +replace( REPLACE(value,'''',''''''),'~',''',''') + ''') '     
from #dpb9_pc1     
where ID = @L_COUNTER    
...
where left(#dpb9_source.value ,2) = '02'    
     ...
     create index ix_1 on #dpb9_pc2 (id)
...
SET @@SSQL1 = ''    
SELECT @@SSQL1 = 'INSERT INTO DPB9_PC2(PurposeCode2,TypeOfTrans,Title,Name,MiddleName,SearchName,Suffix,FthName,PANExCd
...
from #dpb9_pc2     
where ID = @L_COUNTER    AND len(value) - len(replace(value,'~','')) > 40 
    if @@SSQL1 = ''
SELECT @@SSQL1 = 'INSERT INTO DPB9_PC2(PurposeCode2,TypeOfTrans,Title,Name,MiddleName,SearchName,Suffix,FthName,PANExCd
...
where left(#dpb9_source.value ,2) = '03'    
     ...
     create index ix_1 on #dpb9_pc3 (id)
...
SET @@SSQL1 = ''    
SELECT @@SSQL1 = 'INSERT INTO DPB9_PC3(PurposeCode3,TypeOfTrans,Title,Name,MiddleName,SearchName,Suffix,FthName,PANExCd
,PANGIR,PANVerCd,ITCircle,Addr1,Addr2,Addr3,City,statecode,State,countrycode,Country,PinCode,DateofSetup,DateofBirth,Email
,UniqueId,Filler1,pri_isd,pri_ph_no,Filler2,Filler11,Filler12,Filler13,Filler14,Filler15,Filler3,Filler4,Filler5,Filler6,Filler7,Filler8,Filler9,BOId,TransSystemDate
) VALUES(''' +replace( REPLACE(value,'''',''''''),'~',''',''') + ''') '     
from #dpb9_pc3     
...
where left(#dpb9_source.value ,2) = '04'    
     ...
     create index ix_1 on #dpb9_pc4 (id)
...
where left(#dpb9_source.value ,2) = '05'    
     ...
     create index ix_1 on #dpb9_pc5 (id)
...
select citrus_usr.fn_splitval_by(value,	45	,'~') nomsr, boid , max(lastmoddate) lastmod  into #tmp_pc6_last  
from dps8_source111  a  where left(a.value ,2) = '06'     
group by citrus_usr.fn_splitval_by(value,	45	,'~'), boid 




SELECT IDENTITY (NUMERIC, 1,1) ID , value + '~' + a.boid+ '~' + a.lastmoddate  value INTO #dpb9_pc6 from dps8_source111 a , #tmp_pc6_last b --#dpb9_source     
where left(a.value ,2) = '06'    
and a.boid = b.boid 
and b.nomsr  =  citrus_usr.fn_splitval_by(a.value,	45	,'~')  
and lastmod = lastmoddate 


    
     create index ix_1 on #dpb9_pc6 (id)
...
where left(#dpb9_source.value ,2) = '07'    
     ...
     create index ix_1 on #dpb9_pc7 (id)
...
where left(#dpb9_source.value ,2) = '08'    
     ...
     create index ix_1 on #dpb9_pc8 (id)
...
where left(#dpb9_source.value ,2) = '12'    
     ...
     create index ix_1 on #dpb9_pc12 (id)
...
SET @@SSQL1 = ''    
SELECT @@SSQL1 = 'INSERT INTO DPB9_PC12(PurposeCode12
,TypeOfTrans
,Addr1
,Addr2
,Addr3
...
where left(#dpb9_source.value ,2) = '16'    
     ...
     create index ix_1 on #dpb9_pc16 (id)
...
where left(#dpb9_source.value ,2) = '17'    
     ...
     create index ix_1 on #dpb9_pc17 (id)  
...
where left(#dpb9_source.value ,2) = '18'    
     ...
     create index ix_1 on #dpb9_pc18 (id) 
...
where left(#dpb9_source.value ,2) = '19'    
     ...
     create index ix_1 on #dpb9_pc19 (id)
...
END  
 ...
 /*copy this part added line   23/06/2020*/   
...
SELECT IDENTITY (NUMERIC, 1,1) ID , value value INTO #dpb9_pc22 from #dpb9_source     
where left(#dpb9_source.value ,2) = '22'    
    
   
     create index ix_1 on #dpb9_pc22 (id) 
    
SET @L_COUNTER =  1    
SELECT @L_COUNT = MAX(ID)  FROM #dpb9_pc22   
...
SET @@SSQL1 = ''    
SELECT @@SSQL1 = 'INSERT INTO DPB9_PC22 VALUES(''' +replace( REPLACE(value,'''',''''''),'~',''',''') + ''') '     
from #dpb9_pc22  
where ID = @L_COUNTER    
...
select boid , max(TransSystemDate) TransSystemDate into  #DPB9_PC22_ORG from DPB9_PC22 group by boid  
  
delete a from DPB9_PC22 a  
where not exists(select * from #DPB9_PC22_ORG b where a.boid = b.boid and a.TransSystemDate = b.TransSystemDate  )
