-- Object: PROCEDURE citrus_usr.pr_get_ob_uuu
...
create proc [citrus_usr].[pr_get_ob_uuu](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                  
,@pa_ben_acct_no_fr varchar(16),@pa_ben_acct_no_to varchar(16),@pa_out  varchar(8000) out)                  
as                  
begin                  
              --cdsl_holding_dtls
              --(select * from cdsl_holding_dtls union all select * from dmatarc.citrus_usr.cdsl_holding_dtls) a
select distinct cdshm_id,cdshm_tras_dt,cdshm_isin,cdshm_dpam_id , cdshm_trans_no  ,cdshm_trans_cdas_code        
into #tempdata_cdsl             
from (select * from cdsl_holding_dtls union all select * from dmatarc.citrus_usr.cdsl_holding_dtls) a where cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to                
...
from (select * from cdsl_holding_dtls union all select * from dmatarc.citrus_usr.cdsl_holding_dtls) a left outer join dp_daily_hldg_cdsl                   
