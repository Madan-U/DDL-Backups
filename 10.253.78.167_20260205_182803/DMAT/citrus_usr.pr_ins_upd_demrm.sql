-- Object: PROCEDURE citrus_usr.pr_ins_upd_demrm
...
CREATE PROCEDURE [citrus_usr].[pr_ins_upd_demrm](@pa_id              VARCHAR(8000)  
...
DECLARE @t_errorstr      VARCHAR(8000)
	  ...
	  ,@l_usedselect varchar(20)

	  ---change on Feb 3 2020 for Demat changes 
,@ldoctype varchar(5)
Set @ldoctype = citrus_usr.FN_SPLITVAL_BY(@pa_typeofsec,2,'|')
Set @pa_typeofsec = citrus_usr.FN_SPLITVAL_BY(@pa_typeofsec,1,'|')
---change on Feb 3 2020 for Demat changes 
...
set @pa_rmks=replace(ltrim(rtrim(@pa_rmks)),'  ','')
...
print 'ffffff'
print @currstring
						select @l_usedselect = DEMRM_SLIP_SERIAL_NO from demrm_mak WHERE demrm_id = CONVERT(INT,@currstring)
print 'shilpa'
print @l_usedselect

				        DELETE FROM demrm_mak WHERE demrm_id = CONVERT(INT,@currstring)
						delete from used_slip where uses_slip_no = '' + @l_usedselect + '' and USES_TRANTM_ID = '901'
...
--
												...
												SET @l_status          = @ldoctype ---''     ---change on Feb 3 2020 for Demat changes 
												SET @l_rej_cd          = ''
												SET @l_rmks            = ''
										--
										END
...
print '23232323223'
  ...
  --  exec pr_checkslipno_demat 'N','DEMRM_SEL', @pa_dpm_dpid,@pa_dpam_acct_no,@pa_slp_sr_no,@pa_login_name,''
  ----
  --END

  if (exists (select demrm_slip_serial_no from demrm_mak where demrm_slip_serial_no=@pa_slp_sr_no and demrm_deleted_ind = 0)-- and isnull(demrm_res_cd_intobj,'') = '') and 
 and  (not exists (select uses_slip_no from used_slip where uses_slip_no = @pa_slp_sr_no and uses_deleted_ind =1)))
  -- this is done to add slip in used slip table ater insert in demrm_mak tble-- for duplicate slips issue
  begin
	 exec pr_checkslipno_demat 'N','DEMRM_SEL', @pa_dpm_dpid,@pa_dpam_acct_no,@pa_slp_sr_no,@pa_login_name,''
  end 
