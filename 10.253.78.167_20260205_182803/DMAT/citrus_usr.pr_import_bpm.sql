-- Object: PROCEDURE citrus_usr.pr_import_bpm
...
CREATE PROCEDURE  [citrus_usr].[pr_import_bpm]( @pa_exch          VARCHAR(20)    
...
--  
  ...
  declare @pa_task_id numeric,@l_cnt varchar(8000)
  
  select @pa_task_id=task_id from filetask where status='running' and task_name like '%BP MSTR IMPORT - NSDL%'
  ...
  AND   tmpbp_role IN ('03')  --'01','05',
  AND   bitrm_parent_cd = 'ACCESS1'
  AND   bitrm_child_cd = ISNULL(EXCSM_DESC,'')--excsm_exch_cd + '_' + excsm_seg_cd + '_' + excsm_seg_cd + '_' + convert(varchar,excsm_compm_id)
  AND   power(2,bitrm_bit_location-1) = ccm_excsm_bit
