-- Object: PROCEDURE citrus_usr.pr_client_multireport_bak_22052012
...
create proc [citrus_usr].[pr_client_multireport_bak_22052012](@pa_id numeric  
 ...
 where accp_accpm_prop_cd = 'BILL_START_DT' 
   ...
   begin   
     ...
     begin
							...
							if @pa_grp_by ='BR'  
							set @l_sql = @l_sql + ' , citrus_usr.[fn_find_relaions_nm](DPAM_CRN_NO,''BR'')  Groupby  '   
							if @pa_grp_by ='BA'  
							set @l_sql = @l_sql + ' , citrus_usr.[fn_find_relations_nm](DPAM_CRN_NO,''BA'') Groupby  '   
							if @pa_grp_by ='AR'  
							set @l_sql = @l_sql + ' , citrus_usr.[fn_find_relations_nm](DPAM_CRN_NO,''AR'') Groupby  '   
							if @pa_grp_by ='ONW'  
							set @l_sql = @l_sql + ' , citrus_usr.[fn_find_relations_nm](DPAM_CRN_NO,''ONW'') Groupby  '   
							if @pa_grp_by ='RE'  
							set @l_sql = @l_sql + ' , citrus_usr.[fn_find_relations_nm](DPAM_CRN_NO,''RE'') Groupby  '   
							if @pa_grp_by ='REM'  
							set @l_sql = @l_sql + ' , citrus_usr.[fn_find_relations_nm](DPAM_CRN_NO,''REM'') Groupby  '   
							if @pa_grp_by ='CLIENT'  
