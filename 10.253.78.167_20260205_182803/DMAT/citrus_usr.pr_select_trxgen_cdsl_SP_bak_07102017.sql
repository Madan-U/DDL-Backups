-- Object: PROCEDURE citrus_usr.pr_select_trxgen_cdsl_SP_bak_07102017
...
--rollback
create  PROCEDURE [citrus_usr].[pr_select_trxgen_cdsl_SP_bak_07102017]  
(  
								...
								IF @PA_BATCH_TYPE = 'BN'  
									BEGIN  
										SELECT  replace(convert(varchar(11),dptdc_execution_dt,103),'/','')  
												+ isnull(dpam_sba_no,'')  
												+ isnull(dptdc_counter_dp_id,'') + case when dpam_sba_no=dptdc_created_by then right(isnull(dptdc_counter_demat_acct_no,''),8) else isnull(dptdc_counter_demat_acct_no,'') end  
