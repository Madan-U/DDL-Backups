-- Object: PROCEDURE citrus_usr.SP_MULTI_CLIENT_DTLS_DPS8_Cmr_tst
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------


CREATE PROCEDURE [citrus_usr].[SP_MULTI_CLIENT_DTLS_DPS8_Cmr_tst]  
(   
...
--DISTINCT top 1 case when fre_action = 'F' then 'FREEZED' else STAM_DESC end [STATUS]  
  ...
  ,ENTTM_DESC  [TYPE]  
  ,case when isnumeric(right(dps8_pc1.DateOfBirth,4))=1 and right(dps8_pc1.DateOfBirth,4) > 1900 then convert(varchar(11),convert(datetime,case when isnull(dps8_pc1.DateOfBirth,'') <> '' then left(dps8_pc1.DateOfBirth,2)+'/'+substring(dps8_pc1.DateOfBirth,3,2)+'/'+right(dps8_pc1.DateOfBirth,4) else '' end ,103) ,103) else dps8_pc1.DateOfBirth end + space(8) + '(' + isnull(dps8_pc1.usrtxt1,'') + ')' clim_dob  
...
,  dps8_pc12.EMailId [p_EMAIL] --email  
 ...
 ,  dps8_pc12.Fax [p_FAX1]--fax1  
  ...
  ,DPAM.dpam_sba_name  CLIM_NAME_fULL  
...
,case when isnull(DPS8_PC8.DateOfBirth,'') <> '' then left(DPS8_PC8.DateOfBirth,2) +'/'+ substring(DPS8_PC8.DateOfBirth,3,2)+'/'+right(DPS8_PC8.DateOfBirth,4) else '' end DPHD_nomgau_DOB  
  ...
  ,case when Occupation  = 'B'then 'Business'   
       ...
       when Occupation  = 'O' then 'Others'                                                                     
         ...
         when Occupation  = 'PV' then 'PRIVATE SECTOR'  
      when Occupation  = 'PS' then 'PUBLIC SECTOR'  
      when Occupation  = 'GS' then 'GOVERNMENT SERVICES' else '' end   OCCUPATION2   
    ,dpam_acct_no formno  
...
left outer join freeze_unfreeze_dtls on fre_Dpam_id = dpam.dpam_id and fre_deleted_ind = 1   
  left outer join dps8_pc1  dps8_pc1  on dps8_pc1.boid = dpam_sba_no   left outer join dps8_pc4 dps8_pc4 on dps8_pc4.boid = dpam_sba_no and dps8_pc1.boid = dps8_pc4.boid
left outer join  
