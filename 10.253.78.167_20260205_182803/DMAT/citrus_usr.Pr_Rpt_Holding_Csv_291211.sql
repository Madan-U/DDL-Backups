-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_Csv_291211
...
EXEC [Pr_Rpt_Holding_Csv]	'CDSL','3','Y','','','','','N',1,'HO|*~|','','NOW - NONPOA','',''	
*/
 
CREATE PROC [citrus_usr].[Pr_Rpt_Holding_Csv_291211]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
IF @PA_SETTM_NO_TO <> ''
BEGIN  
	INSERT INTO #ACLIST_1 SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,EFF_FROM,EFF_TO FROM CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID)    
...
FROM 
            ...
            (
				...
				FROM [10.11.12.6\dev].MSAJAG.DBO.MULTIISIN WHERE VALID = 1 				
...
CONVERT(NUMERIC(18,0),DPHMCD_BORROW_QTY),
		DPAM_ID,
		HOLDING_DT=CONVERT(VARCHAR(11),@@TMPHOLDING_DT,109),ltrim(rtrim(SPARK_PARTY_CODE)) SPARK_PARTY_CODE  
FROM 
