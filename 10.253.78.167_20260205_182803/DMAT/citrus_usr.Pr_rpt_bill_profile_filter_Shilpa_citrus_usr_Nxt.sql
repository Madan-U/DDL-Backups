-- Object: PROCEDURE citrus_usr.Pr_rpt_bill_profile_filter_Shilpa_citrus_usr_Nxt
...
Create  PROC [citrus_usr].[Pr_rpt_bill_profile_filter_Shilpa_citrus_usr_Nxt]              
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED   
 ...
 SET @@BBOCODE = ''
  ...
  SELECT DPAM_ID,DPAM_SBA_NO,DPAM_BBO_CODE INTO #DPMSTR FROM DP_ACCT_MSTR WHere DPAM_SBA_NO =@PA_FROMACCID
...
SET @PA_GROUP_CD =  ISNULL(CITRUS_USR.FN_SPLITVAL(@PA_GROUP_CD,1),'')  
 ...
 SELECT @@BILLDUEDATE = CONVERT(VARCHAR(11),BILLC_DUE_DATE,109) FROM CITRUS_USR.BILL_CYCLE WHERE BILLC_DPM_ID = @@DPMID AND BILLC_FROM_DT = CONVERT(VARCHAR(11),@PA_FROMDATE,109) AND BILLC_TO_DT = CONVERT(VARCHAR(11),@PA_TODATE,109)  
...
PRINT 'SS'
     ...
     PRINT @PA_PREVFROMDATE
	  ...
	  Create index #c on #DPMSTR(DPAM_ID)

  SELECT C.* INTO #CDSL_TRANS FROM CITRUS_USR.CLIENT_CHARGES_CDSL C (NOLOCK) ,#DPMSTR D
  WHERE CLIC_DPAM_ID =DPAM_ID AND CLIC_TRANS_DT >=@PA_FROMDATE AND CLIC_TRANS_DT <=@PA_TODATE    

    Create index #c on #CDSL_TRANS(CLIC_DPM_ID)
...
, CITRUS_USR.FN_UCC_accp(dpam_id ,'CLIENT GST','')  cl_gst_no
 ...
 FROM CITRUS_USR.ENTITY_RELATIONSHIP E,CITRUS_USR.ENTITY_MSTR,TIN_MSTR , CITRUS_USR.dp_Acct_mstr WHERE GETDATE() BETWEEN ENTR_FROM_DT AND ISNULL(ENTR_TO_DT,'DEC 31 2100')
...
AND REPLACE(ENTM_SHORT_NAME,'_BL','')=STATE_NAME) v on   v.boid=dpam_acctno
 ...
 LEFT OUTER JOIN CITRUS_USR.CLOSING_PRICE_MSTR_CDSL ON T.ISIN_CD = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( SELECT TOP 1 CLOPM_DT FROM CITRUS_USR.CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = T.ISIN_CD AND CLOPM_DT <= @PA_TODATE AND CLOPM_DELETED_IND = 1  ORDER BY CLOPM_DT DESC)
...
LEFT OUTER JOIN CITRUS_USR.CLOSING_PRICE_MSTR_CDSL ON T.ISIN_CD = CLOPM_ISIN_CD AND ISNULL(CLOPM_DT,'01/01/1900') = ( SELECT TOP 1 CLOPM_DT FROM CITRUS_USR.CLOSING_PRICE_MSTR_CDSL WHERE CLOPM_ISIN_CD = T.ISIN_CD AND CLOPM_DT <= @PA_TODATE AND CLOPM_DELETED_IND = 1  ORDER BY CLOPM_DT DESC)
	...
	LEFT OUTER JOIN #LEDGERBAL L ON T.DPAM_ID = L.DPAM_ID , CITRUS_USR.GROUP_MSTR
