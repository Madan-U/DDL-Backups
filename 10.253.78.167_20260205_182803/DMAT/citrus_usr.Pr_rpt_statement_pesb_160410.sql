-- Object: PROCEDURE citrus_usr.Pr_rpt_statement_pesb_160410
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--Pr_rpt_statement 'CDSL',3,'MAY 13 2008','MAY 14 2008','','','','','Y',1,'BASIS POINT|*~|',''                           
create Proc [citrus_usr].[Pr_rpt_statement_pesb_160410]                            
@pa_dptype varchar(4),                                    
@pa_excsmid int,                                    
...
BEGIN                                    
 ...
 BEGIN                              
    ...
    qty numeric(19,3),                        
    order_by int,        
    line_no bigint    ,tratm_cd varchar(25)                        
    )                            
                         
                         
  if @pa_group_cd <> ''              
  begin              
      insert into #tempmaincdsl(dpam_id,dpam_sba_name,dpam_acctno,trans_desc,dpm_trans_no,trans_date,isin_cd,opening_bal,qty,order_by,line_no,tratm_cd)                         
      ...
      and cdshm_tratm_cd in('2246','2277','2280','2201','2251','2252','2230')     
...
where  tratm_cd not in ('2246','2277','2280') 
...
--changed by tushar sep 18 2009                         
  ...
  begin
     ...
     CDSHM_TRATM_DESC=replace(ltrim(rtrim(ISNULL(trans_desc,''))),dpm_trans_no,''),                            
     dpm_trans_no,                                    
 ...
 CREATE INDEX IX_1 on #temprunning(dpam_id,Ben_ctgry,Ben_acct_type,isin_cd)                         
  ...
  open_qty = CASE WHEN FINAL_TRANS = 2 THEN closing_qty ELSE closing_qty - qty END,                                  
  DEBIT_QTY= CASE WHEN QTY <= 0 THEN convert(varchar,ABS(QTY)) ELSE '' END,                                    
  CREDIT_QTY= CASE WHEN QTY > 0 THEN convert(varchar,ABS(QTY)) ELSE '' END,                              
  closing_qty,
  trans_date                          
