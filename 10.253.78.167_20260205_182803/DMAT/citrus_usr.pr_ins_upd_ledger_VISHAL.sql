-- Object: PROCEDURE citrus_usr.pr_ins_upd_ledger_VISHAL
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

CREATE PROCEDURE [citrus_usr].[pr_ins_upd_ledger_VISHAL](@pa_id              VARCHAR(8000)      
...
DECLARE @@l_acct_type    CHAR(1)    
	...
	BEGIN
		  ...
		  COMMIT TRANSACTION
				...
				VALUES(' + convert(varchar,@@l_led_id) + ',' + @pa_dpm_id + ',' + @pa_voucher_type +',''' + @pa_book_type_cd + ''',' + @pa_voucher_no +',1 ,''' + @pa_ref_no + ''',convert(datetime,''' + @pa_voucher_dt+ ''',103) ,' + @@l_account_id  + ',''' + @@l_acct_type + ''',' + convert(varchar,@@l_amount) + ',''' + @@l_narration + ''',''' + isnull(@@l_acct_no,0) + ''','''',0,0,0 ,''N'',''P'' ,''' + @pa_login_name + ''',getdate(),''' + @pa_login_name+ ''',getdate(),' + @@delind + ')'     
				EXEC(@@SSQL)
