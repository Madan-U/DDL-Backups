-- Object: PROCEDURE citrus_usr.PR_GET_INFO_FOR_CLIENTLOGIN
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--- EXEC[PR_GET_INFO_FOR_CLIENTLOGIN]  3,'VIEW','APR 29 2009','APR 29 2009',	1234567800000089, '','',''			
--	3	SAVE	01/04/2009	02/04/2009	
...
CREATE  PROCEDURE [citrus_usr].[PR_GET_INFO_FOR_CLIENTLOGIN]
(
@PA_ID NUMERIC,
...
BEGIN  
	...
	SELECT DPAM_SBA_NAME     
	 ...
	 AND   RIGHT(DPAM_SBA_NO,8) NOT IN (SELECT LOGN_NAME FROM LOGIN_NAMES)  
...
BEGIN    
 ...
 BEGIN
    ...
    SELECT RIGHT(DPAM_SBA_NO,8)  DPAM_SBA_NO   
	 , PAN_AS_PSW PAN_NO     
	 , ENTTM_ID     
	 , DPAM_CRN_NO    
	 , CLIM_SHORT_NAME     
	 ...
	 , 'DEC 31 2100'     TO_DT 
	 --, 1 , 'MIG',GETDATE(),'MIG',GETDATE(), 1, 0, 0, 0, 'A' ,''   
	 ,(Select top 1 isnull(ltrim(rtrim(pri_email)),'') from dps8_pc1 where boid=dpam_sba_no  ) EMAIL --ISNULL( CITRUS_USR.FN_CONC_VALUE(DPAM_CRN_NO , 'EMAIL1'),'') EMAIL 
	 ,'V'     PREF
	 INTO #TEMPCLIENTDATA 
	 FROM CLIENT_MSTR , DP_ACCT_MSTR , ENTITY_TYPE_MSTR , TEMP_CLIENT_DATA_FOR_LOGIN    
	 WHERE CLIM_CRN_NO = DPAM_CRN_NO     
   ...
   SELECT DPAM_SBA_NO     
    ...
    SELECT RIGHT(DPAM_SBA_NO,8) , FROM_DT, @L_ROL_ID, 'MIG',GETDATE(),'MIG',GETDATE(), 1 FROM #TEMPCLIENTDATA WHERE @L_ROL_ID <> 0
 END 
 ELSE IF @PA_VALUES <> ''
 BEGIN
    ...
    SELECT RIGHT(DPAM_SBA_NO,8)      DPAM_SBA_NO
	 ...
	 , 'DEC 31 2100'     TO_DT 
	 --, 1 , 'MIG',GETDATE(),'MIG',GETDATE(), 1, 0, 0, 0, 'A' ,''   
	 , (Select top 1 isnull(ltrim(rtrim(pri_email)),'') from dps8_pc1 where boid=dpam_sba_no  ) EMAIL ,'V'     PREF
   ...
   SELECT DPAM_SBA_NO     
    ...
    INSERT INTO ENTITY_ROLES 
    SELECT RIGHT(DPAM_SBA_NO,8) , FROM_DT, @L_ROL_ID, 'MIG',GETDATE(),'MIG',GETDATE(), 1 FROM #TEMPCLIENTDATA1 WHERE @L_ROL_ID <> 0
 END 
