-- Object: PROCEDURE citrus_usr.PR_BATCH_RESET_ALLTRAN
...
--rollback
CREATE PROCEDURE [citrus_usr].[PR_BATCH_RESET_ALLTRAN]  
(  
@PA_ID INTEGER,  
...
--		END 
					...
					IF NOT EXISTS(SELECT DEMRM_ID FROM DEMAT_REQUEST_MSTR WHERE DEMRM_batch_no = @PA_BATCHNO  AND  isnull(DEMRM_status, 'P') <> 'P' AND DEMRM_deleted_ind = 1 AND DEMRM_REQUEST_DT >= 'JAN 01 2013')  
					BEGIN--6 CDSL
...
PRINT 'DMAT'
        			...
        			IF NOT EXISTS(SELECT REMRM_ID FROM REMAT_REQUEST_MSTR WHERE REMRM_batch_no = @PA_BATCHNO  AND  isnull(REMRM_status, 'P') <> 'P' AND REMRM_deleted_ind = 1 AND REMRM_REQUEST_DT >= 'JAN 01 2013')  
					BEGIN--6 CDSL
