-- Object: PROCEDURE citrus_usr.Pr_Rpt_Ledger_bak07042016
...
create Proc [citrus_usr].[Pr_Rpt_Ledger_bak07042016]        
@pa_dpmid int,        
@pa_finyearid int,        
...
CREATE TABLE #ACLIST(dpam_id BIGINT,dpam_sba_no VARCHAR(16),dpam_sba_name VARCHAR(150),eff_from DATETIME,eff_to DATETIME,acct_type varchar(2))
 ...
 begin 
	...
	INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,EFF_TO,acct_type 
	FROM citrus_usr.fn_gl_acct_list(@pa_dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id)		
	
	if @pa_fromaccid<> '' and @pa_toaccid<> '' and @pa_ledgertype ='P'
	INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,EFF_TO,acct_type 
	FROM citrus_usr.fn_gl_acct_list_bytushar(@pa_dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id,@pa_fromaccid,@pa_toaccid)
    ...
    WHERE isnull(fina_dpm_id,@pa_dpmid) = @pa_dpmid    
    AND   (fina_branch_id = @pa_login_pr_entm_id or fina_branch_id =0)  
 and fina_deleted_ind = 1   
	...
	--FROM citrus_usr.fn_gl_acct_list_bytushar(@pa_dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id,@pa_fromaccid,@pa_toaccid)			
	--union 
	--SELECT FINA_ACC_ID,FINA_ACC_CODE,FINA_ACC_NAME,'JAN 01 1900','JAN 01 2100',FINA_ACC_TYPE 
	--FROM fin_account_mstr 
	...
	if @pa_dpmid ='0'
   	INSERT INTO #ACLIST 
	SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,'jan 01 1900',isnull('jan 01 2100' ,'jan 01 2100'),'P' 
 ...
 begin
	...
	begin 
	if @pa_fromaccid= '' and @pa_toaccid =''
	INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,EFF_TO,'P' 
	FROM citrus_usr.fn_acct_list(@pa_dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id)		
	
	if @pa_fromaccid<> '' and @pa_toaccid <>''
	INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,EFF_FROM,EFF_TO,'P' 
	FROM citrus_usr.fn_acct_list_bytushar(@pa_dpmid ,@pa_login_pr_entm_id,@@l_child_entm_id,@pa_fromaccid,@pa_toaccid)		
	
	
	end 

	if @pa_dpmid ='0'
   	INSERT INTO #ACLIST SELECT DPAM_ID,dpam_sba_no,dpam_sba_name,'jan 01 1900',isnull('jan 01 2100' ,'jan 01 2100'),'P' 
	FROM dp_acct_mstr --, entity_relationship where entr_sba = dpam_sba_no 
	--and  getdate() between ENTR_FROM_DT and isnull(ENTR_TO_DT ,'jan 01 2100')
	--
...
(        
 ...
 set @@ssql = @@ssql + ' where ldg_dpm_id = ' + case when convert(varchar,@pa_dpmid)        ='0' then ' ldg_dpm_id ' else convert(varchar,@pa_dpmid) end  + '  and ldg_voucher_dt >= ''' + convert(varchar(11),@pa_fromdate,109) + ''' and ldg_voucher_dt <= ''' + convert(varchar(11),@pa_todate,109) + ' 23:59:59'''        
...
Exec(@@ssql)    
 ...
 -- voucher_type = 0 -- opening        
  set @@ssql = 'insert into #temptable '        
  set @@ssql = @@ssql + ' select trans_type='''',voucher_type=0,Voucher_no=0,refno=''''
,ldg_account_id,dpam_sba_no,dpam_sba_name,ldg_account_type
,voucher_date= Dateadd(day,-1,cast(''' + convert(varchar(11),@pa_fromdate,109) + ''' as datetime)) 

,bank_cl_date= '''','        --/*Dateadd(day,-1,cast(''' + convert(varchar(11),@pa_fromdate,109) + ''' as datetime))*//*convert(varchar(11),ldg_voucher_dt,109) */
  ...
  set @@ssql = @@ssql + ' where  ldg_voucher_no = ''0'' and  ldg_dpm_id = ' + case when convert(varchar,@pa_dpmid)        ='0' then ' ldg_dpm_id ' else convert(varchar,@pa_dpmid) end + '  and ldg_voucher_dt <=''' + convert(varchar(11),@pa_fromdate,109) + ''' '
  else 
  set @@ssql = @@ssql + ' where ldg_dpm_id = ' + case when convert(varchar,@pa_dpmid)        ='0' then ' ldg_dpm_id ' else convert(varchar,@pa_dpmid) end  + '  and ldg_voucher_dt <''' + convert(varchar(11),@pa_fromdate,109) + ''' ' 
  set @@ssql = @@ssql + ' and ldg_account_id = account.dpam_id '            
  set @@ssql = @@ssql + ' and ldg_account_type = account.acct_type '            
...
'        
 ...
 ---print @@ssql        CLOSED ON JUL 04 2015
 ---Exec(@@ssql)        CLOSED ON JUL 04 2015
 --FOR OPENING RECORDS     
...
print(@@ssql)
 ...
 begin        
  ...
  where  amount <> 0       
  order by account_cd,account_id,Voucher_date,voucher_type,Voucher_no        
 ...
 begin        
  select voucher_type= citrus_usr.GetVoucherDescp(voucher_type)
	,Voucher_no=isnull(Voucher_no,0),account_id=account_id
	,account_cd,account_name,voucher_date= Convert(varchar(11)
   ,voucher_date,103),bank_cl_date= isnull(Convert(varchar(11),bank_cl_date,103),''),        
  debit = case when amount <=0 then Abs(amount) else 0 end,credit= case when amount >0 then Abs(amount) else 0 end ,        
  ...
  from #temptable t    
  where  amount <> 0          
  order by account_cd,account_name,account_type,12,voucher_type,Voucher_no        
