-- Object: PROCEDURE citrus_usr.pr_dp_select_mstr_quant
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--3	DEMRM_SELM	HO				16	1*|~*8*|~*	181	*|~*	|*~|	


--[pr_dp_select_mstr] '4','OFFM_DP_TRX_SELM','VISHAL','','','','504','6*|~*',191,'*|~*','|*~|',''   
...
--exec pr_dp_select_mstr @pa_id='1|*~|3|*~|',@pa_action='VOUCHERREC_SEL',@pa_login_name='',@pa_cd='',@pa_desc='12/11/2008',@pa_rmks='203412',@pa_values='',@pa_roles='6*|~*',@pa_scr_id='217',@RowDelimiter='*|~*',@ColDelimiter='|*~|',@pa_ref_cur=''
 ...
 CREATE PROCEDURE  [citrus_usr].[pr_dp_select_mstr_quant]( @pa_id  VARCHAR(20)                  
...
--                
  ...
  --select top 1 @l_max_date = clopm_dt , @l_rate = clopm_nsdl_rt from closing_price_mstr_nsdl order by 1 desc                
    ...
    BEGIN                
    --                
      select distinct dptd_internal_ref_no [INSTRUCTION NO]                
            ,dptd_isin            ISIN                
            ,abs(dptd_qty)             QTY                
  ...
  --                
    ...
    FROM   citrus_usr.fn_acct_list(@l_dpm_dpid,@l_entm_id,0) dpam                 
          ...
          ,dp_trx_dtls_cdsl             dptdc                 
           left outer join                  
           settlement_type_mstr          settm1                  
           on (settm1.settm_id            = dptdc.dptdc_mkt_type     )            
           left outer join                  
           settlement_type_mstr          settm2                  
           on (settm2.settm_id            = dptdc.dptdc_other_settlement_type)                
