-- Object: PROCEDURE citrus_usr.pr_import_client_cdsl_bak
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--SELECT * FROM CLIENT_MSTR ORDER BY 1 DESC  
--select * from dp_acct_mstr order by 1 desc
--select * from client_bank_accts order by cli
--pr_delete_client 146380
--begin transaction
----[pr_import_client_cdsl] 'CDSL','HO','','','*|~*','|*~|',''
...
create PROCEDURE  [citrus_usr].[pr_import_client_cdsl_bak](@pa_exch          VARCHAR(20)  
  ...
  IF @pa_mode = 'BULK'
	...
	--
     ...
     and    clim_crn_no =  dpam_crn_no 
				 ...
				 Update dpam 
				 set    dpam_stam_cd = case when TMPCLI_ACCT_STAT = '01' then 'ACTIVE' when TMPCLI_ACCT_STAT = '02_BILLSTOP' then '02_BILLSTOP' else TMPCLI_ACCT_STAT end
                ...
                update dphd
                set   DPHD_FH_FTHNAME  = isnull(ltrim(rtrim(TMPCLI_FRST_HLDR_FATH_HUSB_NM)),'') , DPHD_SH_FNAME   = isnull(ltrim(rtrim(TMPCLI_SND_HLDR_NM)),'')
                      ,DPHD_SH_PAN_NO  = isnull(ltrim(rtrim(TMPCLI_SND_HLDR_PAN_N0)),'')
                      ,DPHD_TH_FNAME   = isnull(ltrim(rtrim(TMPCLI_THRD_HLDR_NM)),'')
                ...
                declare @c_client_summary1  cursor
	             ...
	             --
						...
						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper(ISNULL(LTRIM(RTRIM(TMPCLI_CM_ID)),'')) + '|*~|*|~*' 	FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD  = 'CMBP_ID'
						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper(ISNULL(LTRIM(RTRIM(TMPCLI_RBI_REGI_NO)),'')) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'RBI_REF_NO'
						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','GROUP_CD',upper(ISNULL(LTRIM(RTRIM(TMPCLI_GROUP_CD)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'group_cd'

						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + case when upper(ISNULL(LTRIM(RTRIM(TMPCLI_ELECTRONOC_DIV)),''))='Y' then '1' else '0' end  + '|*~|*|~*' 	FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD  = 'ECS_FLG'
						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','BANKCCY',upper(ISNULL(LTRIM(RTRIM(TMPCLI_BENF_ACCT_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'BANKCCY'
						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','DIVBANKCCY',upper(ISNULL(LTRIM(RTRIM(TMPCLI_DIVIDEND_ACCT_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'DIVBANKCCY'
						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','DIVIDEND_CURRENCY',upper(ISNULL(LTRIM(RTRIM(TMPCLI_DIVIDEND_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'DIVIDEND_CURRENCY'
						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + case when upper(ISNULL(LTRIM(RTRIM(TMPCLI_PURC_WAIVER)),''))='Y' then '1' else '0' end  + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'CONFIRMATION'

						SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','BOSTMNTCYCLE',upper(ISNULL(LTRIM(RTRIM(TMPCLI_BO_STMT_CYL_CD)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'BOSTMNTCYCLE'
						SET @L_accpd_VALUE = ''
...
--						    SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + ISNULL(LTRIM(RTRIM(ben_rbi_ref_no)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR WHERE Ben_Acct_No = @c_ben_acct_no AND ENTPM_CD = 'RBI_REF_NO'
                           ...
                           SELECT DISTINCT @L_ENTP_VALUE = @L_ENTP_VALUE+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','OCCUPATION',upper(ISNULL(LTRIM(RTRIM(TMPCLI_PROFESSION_CD)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ENTITY_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND entpm_CD   = 'OCCUPATION'
                           SELECT DISTINCT @L_ENTP_VALUE = @L_ENTP_VALUE+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','GEOGRAPHICAL',upper(ISNULL(LTRIM(RTRIM(TMPCLI_GEO_AREA_CD)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ENTITY_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND entpm_CD   = 'GEOGRAPHICAL'
                           SELECT DISTINCT @L_ENTP_VALUE = @L_ENTP_VALUE+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','EDUCATION',upper(ISNULL(LTRIM(RTRIM(TMPCLI_EDU_CD)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ENTITY_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND entpm_CD   = 'EDUCATION'
                           SELECT DISTINCT @L_ENTP_VALUE = @L_ENTP_VALUE+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + upper(ISNULL(LTRIM(RTRIM(TMPCLI_TAX_DEDUC_STAT)),'')) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ENTITY_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND entpm_CD   = 'TAX_DEDUCTION'
                           SELECT DISTINCT @L_ENTP_VALUE = @L_ENTP_VALUE+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','LANGUAGE',upper(ISNULL(LTRIM(RTRIM(TMPCLI_LANG_CD)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ENTITY_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND entpm_CD   = 'LANGUAGE'
                           SELECT DISTINCT @L_ENTP_VALUE = @L_ENTP_VALUE+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + CASE WHEN upper(ISNULL(LTRIM(RTRIM(TMPCLI_STAFF_RELATIVE_DP)),'')) = 'N' THEN 'NONE' 
                           ...
                           SELECT DISTINCT @L_ENTP_VALUE = @L_ENTP_VALUE+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','ANNUAL_INCOME',upper(ISNULL(LTRIM(RTRIM(TMPCLI_INCOME_CD)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ENTITY_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND entpm_CD   = 'ANNUAL_INCOME'
...
--						EXEC pr_reset_bill_printing @c_ben_acct_no1
						...
						--contact channels
						 ...
						 SELECT @L_CONC = 'OFF_PH1|*~|'+ISNULL(ltrim(rtrim(TMPCLI_BO_TELPH_NO)),'')+'*|~*'+'OFF_PH2|*~|'+ISNULL(ltrim(rtrim(TMPCLI_BO_TELPHP_NO)),'')+'*|~*'+'FAX1|*~|'+ISNULL(ltrim(rtrim(TMPCLI_BO_FAX_NO)),'')+'*|~*'+'FAX2|*~|'+ISNULL(ltrim(rtrim(TMPCLI_BO_FAXF_NO)),'')+'*|~*'
                         ...
                         SELECT @L_ADR = @L_ADR  + 'NOMINEE_ADR1|*~|' +ISNULL(ltrim(rtrim(TMPCLI_NOMI_ADD_LN123)),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_NOMI_CITY_STATE_CNTRY)),'')+'|*~|'+ISNULL('','')+'|*~|'+ISNULL('','')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|*|~*' FROM   TMP_CLIENT_DTLS_MSTR_CDSL WHERE TMPCLI_BOID = @c_ben_acct_no1 and isnull(TMPCLI_NOMI_ADD_LN123,'') <> ''
						...
						set @L_ADR   = ''
                          ...
                          IF EXISTS(SELECT BANM_ID FROM BANK_MSTR WHERE BANM_NAME = @L_BANK_NAME AND BANM_BRANCH = @L_BANM_BRANCH and banm_micr = @L_MICR_NO)
                           BEGIN
                              SELECT @L_BANM_ID = BANM_ID FROM BANK_MSTR WHERE (BANM_NAME = @L_BANK_NAME AND BANM_BRANCH = @L_BANM_BRANCH ) and  banm_micr = @L_MICR_NO
...
--                            ) and banm_micr = TMPCLI_BANK_CD
			     ...
			     select TMPCLI_BOID  from TMP_CLIENT_DTLS_MSTR_CDSL where isnull(TMPCLI_FRST_HLDR_PAN_N0,'') <> ''
                 and TMPCLI_BOID not in (select dpam_sba_no from dp_Acct_mstr) and TMPCLI_ACCT_STAT <> '10'
                 
                 select @l_dpm_dpid = dpm_dpid , @l_excsm_id = default_dp from dp_mstr , exch_seg_mstr where default_dp = excsm_id and  excsm_exch_cd= 'CDSL'
	             ...
	             --
						  ...
						  SELECT  @L_CLIENT_VALUES = ISNULL(ltrim(rtrim(case when @@BOCTGRY in ('21' , '24' , '27') then CITRUS_USR.inside_trim(LTRIM(RTRIM(TMPCLI_FRST_HLDR_NM)),'1') else LTRIM(RTRIM(TMPCLI_FRST_HLDR_NM)) end)),'') + '|*~|' + ISNULL(case when @@BOCTGRY in ('21' , '24' , '27') then CITRUS_USR.inside_trim(LTRIM(RTRIM(TMPCLI_FRST_HLDR_NM)),'2') else '' end,'') + '|*~|' + ISNULL(case when @@BOCTGRY in ('21' , '24' , '27') then CITRUS_USR.inside_trim(LTRIM(RTRIM(TMPCLI_FRST_HLDR_NM)),'3') else '.' end,'') + '|*~|'+ LEFT(LTRIM(RTRIM(ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_PAN_N0)),'') + ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_NM)),''))),50) + '|*~|'+ISNULL(TMPCLI_BO_SEX,'')+'|*~|'+ISNULL(TMPCLI_BO_DOB,'')+'|*~||*~|ACTIVE|*~||*~|1|*~|' + ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_PAN_N0)),'') +'|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL WHERE  TMPCLI_BOID = @c_ben_acct_no
						  ...
						  begin
							  ...
							  select distinct @l_crn_no = clim_crn_no from client_mstr, TMP_CLIENT_DTLS_MSTR_CDSL WHERE  TMPCLI_BOID = @c_ben_acct_no and clim_short_name =  LEFT(LTRIM(RTRIM(ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_PAN_N0)),'') + ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_NM)),''))),50)                 
						  end
						  else
						  begin
							  select distinct @l_crn_no = clim_crn_no from client_mstr,entity_properties , TMP_CLIENT_DTLS_MSTR_CDSL  WHERE  TMPCLI_BOID = @c_ben_acct_no AND clim_crn_no =entp_ent_id AND entp_entpm_cd = 'pan_gir_no' AND entp_value = TMPCLI_FRST_HLDR_PAN_N0 AND entp_deleted_ind = 1                    
						  end
						...
						--contact channels
						 ...
						 SELECT @L_CONC = 'OFF_PH1|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BO_TELPH_NO)),'')+'*|~*'+'OFF_PH1|*~|'+ISNULL(LTRIM(RTRIM(TMPCLI_BO_TELPHP_NO)),'')+'*|~*'
						...
						--dp_acct_mstr
                          ...
                          --select @l_compm_id = excsm_compm_id from exch_seg_mstr where excsm_id = @l_excsm_id 
						  select @l_dp_acct_values = isnull(convert(varchar,@l_compm_id),'')+'|*~|'+ isnull(convert(varchar,@l_excsm_id),'')+'|*~|'+isnull(convert(varchar,@l_dpm_dpid),'')+'|*~|'+isnull(@c_ben_acct_no,'')+'|*~|'+ISNULL(ltrim(rtrim(TMPCLI_FRST_HLDR_NM)),'')+'|*~|'+ ISNULL(ltrim(rtrim(TMPCLI_DP_INTREF_NO)),'') +'|*~|ACTIVE|*~|'+citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(TMPCLI_ACCT_CTRGY,''),'enttm') + '|*~|' + citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(TMPCLI_BO_STAT,''),'clicm')+'|*~|'+citrus_usr.cdsl_ctgry_enttm_subcm_mapping (isnull(ltrim(rtrim(TMPCLI_BO_SUB_STAT)),''),'subcm')+'|*~|0|*~|A|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL WHERE  tmpcli_boid = @c_ben_acct_no 
						...
						declare @pa_fh_dtls varchar(8000)
						  ...
						  select @pa_sh_dtls = isnull(ltrim(rtrim(TMPCLI_SND_HLDR_NM)),'')+'|*~|'+isnull('','')+'|*~|'+isnull('','')+'|*~|'+isnull(ltrim(rtrim('')),'')+'|*~|'++'|*~|'+isnull(ltrim(rtrim(TMPCLI_SND_HLDR_PAN_N0)),'')+'|*~|'++'|*~|*|~*'  FROM   TMP_CLIENT_DTLS_MSTR_CDSL WHERE TMPCLI_BOID = @c_ben_acct_no
						  select @pa_th_dtls = isnull(ltrim(rtrim(TMPCLI_THRD_HLDR_NM)),'')+'|*~|'+isnull('','')+'|*~|'+isnull('','')+'|*~|'+isnull(ltrim(rtrim('')),'')+'|*~|'++'|*~|'+isnull(ltrim(rtrim(TMPCLI_THRD_HLDR_PAN_N0)),'')+'|*~|'++'|*~|*|~*' FROM  TMP_CLIENT_DTLS_MSTR_CDSL WHERE TMPCLI_BOID = @c_ben_acct_no
						  select @pa_nomgau_dtls = '|*~|'+'|*~|'+'|*~|'+'|*~|'+'|*~|'+'|*~|'+'|*~|*|~*'  FROM   TMP_CLIENT_DTLS_MSTR_CDSL WHERE TMPCLI_BOID = @c_ben_acct_no 
						  select @pa_nom_dtls = isnull(ltrim(rtrim(TMPCLI_NOMI_NM)),'')+'|*~|'+'|*~|'+'|*~|'+'|*~|'+'|*~|'+'|*~|'+'|*~|*|~*'  FROM   TMP_CLIENT_DTLS_MSTR_CDSL WHERE TMPCLI_BOID = @c_ben_acct_no 
         ...
         select @l_crn_no = clim_crn_no from client_mstr,  TMP_CLIENT_DTLS_MSTR_CDSL  where clim_short_name =  isnull(TMPCLI_FRST_HLDR_NM,'') and tmpcli_boid = @c_ben_acct_no
						...
						--account_properties
						  ...
						  SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + case when upper(ISNULL(LTRIM(RTRIM(TMPCLI_ELECTRONOC_CONFIR)),''))='Y' then '1' else '0' end  + '|*~|*|~*' 	FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD  = 'ELEC_CONF'
						  SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','BANKCCY',upper(ISNULL(LTRIM(RTRIM(TMPCLI_BENF_ACCT_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'BANKCCY'
						  SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','DIVBANKCCY',upper(ISNULL(LTRIM(RTRIM(TMPCLI_DIVIDEND_ACCT_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'DIVBANKCCY'
						  SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + upper([citrus_usr].[fn_reverse_mapping]('CDSL','DIVIDEND_CURRENCY',upper(ISNULL(LTRIM(RTRIM(TMPCLI_DIVIDEND_CURR)),'')))) + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'DIVIDEND_CURRENCY'
                          SELECT DISTINCT @L_ACCP_VALUE = @L_ACCP_VALUE+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + case when upper(ISNULL(LTRIM(RTRIM(TMPCLI_PURC_WAIVER)),''))='Y' then '1' else '0' end  + '|*~|*|~*' FROM TMP_CLIENT_DTLS_MSTR_CDSL ,ACCOUNT_PROPERTY_MSTR  WHERE TMPCLI_BOID = @c_ben_acct_no1 AND ACCPM_PROP_CD   = 'CONFIRMATION'
...
SELECT IDENTITY(int,1,1) id1 , * INTO #tmp FROM TMP_POA_DTLS_MSTR_CDSL tpdmc 
	  ...
	  AND   poam_id NOT IN (SELECT isnull(poaam_poam_id,'0') FROM poa_auth_mstr)
