-- Object: PROCEDURE citrus_usr.pr_get_holding_fix_latest_BBOCODE
...
CREATE PROC [citrus_usr].[pr_get_holding_fix_latest_BBOCODE](@PA_DPM_ID NUMERIC,@PA_FROM_DT DATETIME, @PA_TO_DT DATETIME                        
...
from CDSL_HOLDING_DTLS with(nolock) , #CLIENTLIST where 
...
select cdshm_dpam_id , cdshm_ben_acct_no , cdshm_isin , sum(cdshm_qty*-1) ep_rej into #tmp_ep_rej from CDSL_HOLDING_DTLS eprej with(nolock)  , #CLIENTLIST 
where 
 cdshm_tras_dt between @l_min_trx_dt and @PA_TO_DT  
 AND CDSHM_BEN_ACCT_NO = SBANO
...
from CDSL_HOLDING_DTLS eprej with(nolock)   , #CLIENTLIST  
where 
 cdshm_tras_dt between @l_min_trx_dt and @PA_TO_DT  
  AND CDSHM_BEN_ACCT_NO = SBANO
--and cdshm_ben_acct_no between @pa_ben_acct_no_fr and @pa_ben_acct_no_to   
