-- Object: PROCEDURE citrus_usr.pr_ins_upd_demrm_bak21122011
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

create PROCEDURE [citrus_usr].[pr_ins_upd_demrm_bak21122011](@pa_id              VARCHAR(8000)  
		      ...
		      and    dpm_deleted_ind   = 1
				    ...
				    --
				      ...
				      (demrm_id
						...
						,demrm_lst_upd_by
						,demrm_lst_upd_dt
						,demrm_deleted_ind ,DEMRM_TOTAL_CERTIFICATES
				      )VALUES
				      (@l_demrm_id   
				      ,@pa_isin
				      ,@l_dpam_id    
...
--										SET @L_COUNT_CERTI = 0
 ...
 delete from uses from used_slip uses, demrm_mak , dp_acct_mstr 
  ...
  IF left(ltrim(rtrim(@pa_errmsg)),5) <> 'ERROR'
		BEGIN
  --
    exec pr_checkslipno '','DEMRM_SEL', @pa_dpm_dpid,@pa_dpam_acct_no,@pa_slp_sr_no,@pa_login_name,''
  --
  END
