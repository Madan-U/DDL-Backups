-- Object: PROCEDURE citrus_usr.PR_IMPORT_CLIENT_CDSL_DPB9_dhani
...
CREATE     PROCEDURE  [citrus_usr].[PR_IMPORT_CLIENT_CDSL_DPB9_dhani]
			(
			 @pa_exch          VARCHAR(20)    
...
set dateformat dmy
 truncate table lastbsdastatus
 insert into lastbsdastatus
 select boid, filler9 , getdate() lstupddate from dps8_pc1 
...
/*Changed on 26/06/12 by Tushar */ 
     ...
     BEGIN
          ...
          and    clim_crn_no =  dpam_crn_no   
			...
			, DPB9_PC1
			where  dpam_sba_no =  BOID and  dpam_stam_cd <> '02_BILLSTOP' and dpam_stam_cd <> '05' 
...
update  f set fre_action ='U', fre_rmks = case when FreezeResCd ='1' then 'By BO'
when FreezeResCd ='2' then ' ITO Attachement'
when FreezeResCd ='3' then ' SEBI Directive'
when FreezeResCd ='4' then ' Disinvestment / Pvt. Deal'
when FreezeResCd ='5' then ' Court Order'
...
UPDATE BITMAP_REF_MSTR SET BITRM_BIT_LOCATION = @L_BITLOCATION + (SELECT COUNT(*) FROM #TMP_ADDRESSES1) + 10
...
/*changes done by tushar on may 22 2014 for nrn no update if nominee change*/
			...
			SELECT @L_CONC = case when PriPhInd = 'O'  then 'OFF_PH1|*~|'
			when PriPhInd = 'M'  then 'MOBILE1|*~|'
								...
								------+'RES_PH1|*~|'+ISNULL(ltrim(rtrim(PRIPHNUM)),'')+'*|~*'
										...
										+ case when AltPhInd = 'O'  then 'OFF_PH1|*~|'
										  when AltPhInd = 'R'  then 'RES_PH1|*~|' else 'RES_PH1|*~|' end +ISNULL(ltrim(rtrim(AltPhNum)),'')+'*|~*'
										+'OFF_PH1|*~|'+ISNULL(ltrim(rtrim(AddPh)),'')+'*|~*'
				...
				SELECT @L_CONC = @L_CONC  +'MOBSMS|*~|'+ISNULL(ltrim(rtrim(MobileNum)),'')+'*|~*' 
					--+ 'EMAIL1|*~|'+ISNULL(LTRIM(RTRIM(EmailId)),'')+'*|~*' 						    
					   FROM   DPB9_PC16 WHERE BOID = @C_BEN_ACCT_NO1  
			and MobileNum <> ''
...
--changes done for 408 ------   
          select @l_crn_no = clim_crn_no from client_mstr,  DP_ACCT_MSTR where DPAM_CRN_NO = CLIM_CRN_NO AND DPAM_SBA_NO =  @c_ben_acct_no1  
             ...
             SET @L_ACCP_VALUE = ''  
         SELECT DISTINCT  @L_ACCP_VALUE = CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(SEBIREGNUM)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD = 'SEBI_REG_NO'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(CLMEMID)),'')) + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'CMBP_ID'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UnqIdenNum)),'')) + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARFLAG'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UniqueId)),'')) + '|*~|*|~*'  FROM DPB9_PC2 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARSECHLDR'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UniqueId)),'')) + '|*~|*|~*'  FROM DPB9_PC3 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARTHRDHLDR'  
         
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(RBIREFNUM)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'RBI_REF_NO'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','GROUP_CD',UPPER(ISNULL(LTRIM(RTRIM(GROUPCD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'GROUP_CD'  
         --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(ELETCONF)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ELEC_CONF'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','BANKCCY',UPPER(ISNULL(LTRIM(RTRIM(DIVBANKCURR)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'BANKCCY'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','DIVBANKCCY',UPPER(ISNULL(LTRIM(RTRIM(DIVBANKCURR)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'DIVBANKCCY'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','DIVIDEND_CURRENCY',UPPER(ISNULL(LTRIM(RTRIM(DIVBANKCURR)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'DIVIDEND_CURRENCY'  
		 --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(CLOSDT)),''))  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'ACC_CLOSE_DT'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + + LEFT(ISNULL(LTRIM(RTRIM(CLOSDT)),''),2) + '/'+ SUBSTRING(ISNULL(LTRIM(RTRIM(CLOSDT)),''),3,2) + '/' + RIGHT(ISNULL(LTRIM(RTRIM(CLOSDT)),''),4)  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'ACC_CLOSE_DT'  and  CLOSDT <> ''
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(BENTAXDEDSTAT)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'TAX_DEDUCTION'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(CONFWAIVED)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'CONFIRMATION'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(ECS)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ECS_FLG'  

         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','BOSTMNTCYCLE',UPPER(ISNULL(LTRIM(RTRIM(BOSTATCYCLECD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'BOSTMNTCYCLE'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + + LEFT(ISNULL(LTRIM(RTRIM(BOActDt)),''),2) + '/'+ SUBSTRING(ISNULL(LTRIM(RTRIM(BOActDt)),''),3,2) + '/' + RIGHT(ISNULL(LTRIM(RTRIM(BOActDt)),''),4)  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'BILL_START_DT'  and BOActDt <> ''
		 --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(TRADINGID)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'BBO_CODE'  
		 --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(STOCKEXCH)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'CMBPEXCH'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(filler9)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'bsda'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(AnnlRep)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'RGESS_FLAG'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + '1'  + '|*~|*|~*'  FROM DPB9_PC16 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'SMS_FLAG'
...
/**/
		...
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(filler2)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(filler2)),''))='N' THEN 'NO' ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'Email_st_flag'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(filler8)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(filler8)),''))='N' THEN 'NO' ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'RTA_EMAIL'
...
--changes done for 408 ------		
		...
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + 'YES'  + '|*~|*|~*'  FROM DPB9_PC6 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'MULTI_NOM_FLG'
        
--changes done for 408 ------

		--SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(filler3)),''))='PH' THEN 'PHYSICAL CAS REQUIRED' WHEN UPPER(ISNULL(LTRIM(RTRIM(filler3)),''))='NO' THEN 'CAS NOT REQUIRED' else '' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'cas_flag'
		--SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(filler4)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(filler4)),''))='N' THEN 'NO' ELSE '' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'mental_flag'
		
		/*new kyc aug 2021*/
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('BOOS',UPPER(ISNULL(LTRIM(RTRIM(AFILLER4)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'BOOS'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('BONAFIDE',UPPER(ISNULL(LTRIM(RTRIM(AFILLER5)),'')))) + '|*~|*|~*' FROM DPB9_PC1,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'BONAFIDE'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC',UPPER(ISNULL(LTRIM(RTRIM(PANVerCode)),'')))) + '|*~|*|~*'FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'PANVC' 
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC2',UPPER(ISNULL(LTRIM(RTRIM(PANVerCd)),'')))) + '|*~|*|~*' FROM DPB9_PC2 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'PANVC2'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC3',UPPER(ISNULL(LTRIM(RTRIM(PANVerCd)),'')))) + '|*~|*|~*' FROM DPB9_PC3 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'PANVC3'
		...
		/**/
         SET @L_ACCPD_VALUE = ''  
         SELECT DISTINCT @L_ACCPD_VALUE = CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CONVERT(VARCHAR,ACCDM_ID) + '|*~|'  + LEFT(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),2) + '/'+ SUBSTRING(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),3,2) + '/' + RIGHT(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),4)  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR , ACCPM_DTLS_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD = 'RBI_REF_NO' AND ACCDM_CD = 'RBI_APP_DT' AND ACCDM_ACCPM_PROP_ID = ACCPM_PROP_ID  
...
--          SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + ISNULL(LTRIM(RTRIM(BEN_RBI_REF_NO)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR WHERE BEN_ACCT_NO = @C_BEN_ACCT_NO AND ENTPM_CD = 'RBI_REF_NO'  
        ...
        SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','NATIONALITY',UPPER(ISNULL(LTRIM(RTRIM(NATCD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'NATIONALITY'  
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','OCCUPATION',UPPER(ISNULL(LTRIM(RTRIM(OCCUPATION)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'OCCUPATION'  
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','GEOGRAPHICAL',UPPER(ISNULL(LTRIM(RTRIM(GEOGCD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'GEOGRAPHICAL'  
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','EDUCATION',UPPER(ISNULL(LTRIM(RTRIM(EDU)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'EDUCATION'  
          
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','LANGUAGE',UPPER(ISNULL(LTRIM(RTRIM(LANGCD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'LANGUAGE'  
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(STAFF)),'')) = 'N' THEN 'NONE'   
                                                                                                      WHEN UPPER(ISNULL(LTRIM(RTRIM(STAFF)),'')) = 'R' THEN 'RELATIVE'  
                                                                                                      WHEN UPPER(ISNULL(LTRIM(RTRIM(STAFF)),'')) = 'S' THEN 'STAFF' ELSE '' END   + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'STAFF'  
        --SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','ANNUAL_INCOME',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'ANNUAL_INCOME'  
	/*new kyc aug 2021*/
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+  CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567','0225105','0225104') then '' else  UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_income',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) end    + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR, dp_acct_mstr   WHERE BOID = DPAM_SBA_NO  and BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'ANNUAL_INCOME'    and UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_income',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),''))))  <> '' 
			
		SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+  CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567','0225105','0225104') then  UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_incomeNI',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) else '' end  + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR, dp_acct_mstr   WHERE BOID = DPAM_SBA_NO  and BOID = @C_BEN_ACCT_NO1 AND ENTPM_CD   = 'annual_incomeNI'    
		/*new kyc aug 2021*/                    
...
if exists (select 1 from client_bank_accts ,dp_acct_mstr where dpam_id = cliba_clisba_id and DPAM_SBA_NO =@c_ben_acct_no1 )
begin 
...
--								WHERE ISNULL(DPAM_BATCH_NO ,'0') <>'0' AND DPAM_ACCT_NO = DPAM_SBA_NO ) 
		...
		and right(dpm_dpid ,5) = (select top 1  citrus_usr.FN_SPLITVAL_BY (value ,2,'~' ) from dpb9_source where citrus_usr.FN_SPLITVAL_BY (value , 1,'~' )='H')
...
SELECT @L_CONC = @L_CONC  +'MOBSMS|*~|'+ISNULL(ltrim(rtrim(MobileNum)),'')+'*|~*' 
		--+ 'EMAIL1|*~|'+ISNULL(LTRIM(RTRIM(EmailId)),'')+'*|~*' 						    
           FROM   DPB9_PC16 WHERE BOID = @C_BEN_ACCT_NO  
...
--					   SELECT @PA_NOM_DTLS = ISNULL(NAME,'')+'|*~|'+ISNULL(MIDDLENAME,'')+'|*~|'+ISNULL(SEARCHNAME,'')+'|*~|'+ISNULL(LTRIM(RTRIM(FTHNAME)),'')+'|*~|'+CITRUS_USR.FNGETDATE(DATEOFBIRTH)+'|*~|'+ISNULL(LTRIM(RTRIM(PANGIR)),'')+'|*~|'+'|*~|*|~*'  FROM   DPB9_PC6 WHERE BOID = @C_BEN_ACCT_NO  
  ...
  --brokerage
      ...
      --account_properties  
           SET @L_ACCP_VALUE = ''  
         SELECT DISTINCT  @L_ACCP_VALUE = CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(SEBIREGNUM)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD = 'SEBI_REG_NO'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UnqIdenNum)),'')) + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARFLAG'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UniqueId)),'')) + '|*~|*|~*'  FROM DPB9_PC2 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARSECHLDR'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(UniqueId)),'')) + '|*~|*|~*'  FROM DPB9_PC3 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD  = 'ADHAARTHRDHLDR'  
         
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(CLMEMID)),'')) + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'CMBP_ID'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(RBIREFNUM)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'RBI_REF_NO'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','GROUP_CD',UPPER(ISNULL(LTRIM(RTRIM(GROUPCD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'GROUP_CD'  
         --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(ELETCONF)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'ELEC_CONF'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','BANKCCY',UPPER(ISNULL(LTRIM(RTRIM(DIVBANKCURR)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'BANKCCY'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','DIVBANKCCY',UPPER(ISNULL(LTRIM(RTRIM(DIVBANKCURR)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'DIVBANKCCY'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','DIVIDEND_CURRENCY',UPPER(ISNULL(LTRIM(RTRIM(DIVBANKCURR)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'DIVIDEND_CURRENCY'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + + + LEFT(ISNULL(LTRIM(RTRIM(CLOSDT)),''),2) + '/'+ SUBSTRING(ISNULL(LTRIM(RTRIM(CLOSDT)),''),3,2) + '/' + RIGHT(ISNULL(LTRIM(RTRIM(CLOSDT)),''),4)  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'ACC_CLOSE_DT'    and CLOSDT <> ''
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(BENTAXDEDSTAT)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'TAX_DEDUCTION'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(CONFWAIVED)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'CONFIRMATION'  
SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(ECS)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'ECS_FLG'  
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','BOSTMNTCYCLE',UPPER(ISNULL(LTRIM(RTRIM(BOSTATCYCLECD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'BOSTMNTCYCLE'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + + LEFT(ISNULL(LTRIM(RTRIM(BOActDt)),''),2) + '/'+ SUBSTRING(ISNULL(LTRIM(RTRIM(BOActDt)),''),3,2) + '/' + RIGHT(ISNULL(LTRIM(RTRIM(BOActDt)),''),4)  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'BILL_START_DT'   and BOActDt <> ''
		 --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(TRADINGID)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'BBO_CODE'  
		 --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(STOCKEXCH)),'')) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'CMBPEXCH'  
		 /*Change on Aug 25 2015 */
		 --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + case when UPPER(ISNULL(LTRIM(RTRIM(Filler2)),'')) ='Y' then 'YES' else 'NO' end + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'Email_st_flag'  
		 --SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + case when UPPER(ISNULL(LTRIM(RTRIM(Filler3)),'')) ='NO' then 'CAS NOT REQUIRED' when UPPER(ISNULL(LTRIM(RTRIM(Filler3)),'')) ='PH' then 'PHYSICAL CAS REQUIRED'  else '' end + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO1 AND ACCPM_PROP_CD   = 'cas_flag'  
		 
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + case when UPPER(ISNULL(LTRIM(RTRIM(Filler4)),'')) ='C' then 'CDSL' when UPPER(ISNULL(LTRIM(RTRIM(Filler4)),'')) ='N' then 'NSDL'  else '' end + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD   = 'PDF'  
	     /*Change on Aug 25 2015 */
	     
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(filler9)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'bsda'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(AnnlRep)),''))='Y' THEN '1' ELSE '0' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'RGESS_FLAG'
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(Filler2 )),''))='Y' THEN 'YES' ELSE 'NO' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'Email_st_flag'  and Filler2 <> ''
         ...
         SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(Filler4)),''))='Y' THEN 'YES' ELSE 'NO' END  + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'mental_flag' and Filler4 <> ''
		...
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(Filler7)),''))='Y' THEN 'YES' WHEN UPPER(ISNULL(LTRIM(RTRIM(Filler7)),''))='N' THEN 'NO'  ELSE '0' END  + '|*~|*|~*'   FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'PLEDGE_STANDING'
		 ...
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(ITCircle)),'')) + '|*~|*|~*'  FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'ADHAARFLAG'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(ITCircle)),'')) + '|*~|*|~*'  FROM DPB9_PC2 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'ADHAARSECHLDR'  
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER(ISNULL(LTRIM(RTRIM(ITCircle)),'')) + '|*~|*|~*'  FROM DPB9_PC3 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'ADHAARTHRDHLDR'  
         
		 SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + 'YES'  + '|*~|*|~*'  FROM DPB9_PC6 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'MULTI_NOM_FLG'
...
--changes done for 408 ------	
/*new kyc aug 2021*/
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('BOOS',UPPER(ISNULL(LTRIM(RTRIM(AFILLER4)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'BOOS'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('BONAFIDE',UPPER(ISNULL(LTRIM(RTRIM(AFILLER5)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'BONAFIDE'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC',UPPER(ISNULL(LTRIM(RTRIM(PANVerCode)),'')))) + '|*~|*|~*'FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'PANVC' 
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC2',UPPER(ISNULL(LTRIM(RTRIM(PANVerCd)),'')))) + '|*~|*|~*' FROM DPB9_PC2 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'PANVC2'
		SELECT DISTINCT @L_ACCP_VALUE = ISNULL(@L_ACCP_VALUE,'')+ CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('PANVC3',UPPER(ISNULL(LTRIM(RTRIM(PANVerCd)),'')))) + '|*~|*|~*' FROM DPB9_PC3 ,ACCOUNT_PROPERTY_MSTR WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD  = 'PANVC3'

		/*new kyc aug 2021*/


         SET @L_ACCPD_VALUE = ''  
         SELECT DISTINCT @L_ACCPD_VALUE = CONVERT(VARCHAR,ACCPM_PROP_ID) + '|*~|' + CONVERT(VARCHAR,ACCDM_ID) + '|*~|'  + LEFT(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),2) + '/'+ SUBSTRING(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),3,2) + '/' + RIGHT(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),4)  + '|*~|*|~*' FROM DPB9_PC1 ,ACCOUNT_PROPERTY_MSTR , ACCPM_DTLS_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ACCPM_PROP_CD = 'RBI_REF_NO' AND ACCDM_CD = 'RBI_APP_DT' AND ACCDM_ACCPM_PROP_ID = ACCPM_PROP_ID  
         AND ISDATE(RIGHT(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),2) + '/'+ SUBSTRING(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),5,2) + '/' + LEFT(ISNULL(LTRIM(RTRIM(RBIAPPDT)),''),4)) = 1          
...
--          SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + ISNULL(LTRIM(RTRIM(BEN_RBI_REF_NO)),'') + '|*~|*|~*' FROM DP_CLIENT_SUMMARY ,ENTITY_PROPERTY_MSTR WHERE BEN_ACCT_NO = @C_BEN_ACCT_NO AND ENTPM_CD = 'RBI_REF_NO'  
        ...
        SELECT DISTINCT @L_ENTP_VALUE = CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','NATIONALITY',UPPER(ISNULL(LTRIM(RTRIM(NATCD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'NATIONALITY'  
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','OCCUPATION',UPPER(ISNULL(LTRIM(RTRIM(OCCUPATION)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'OCCUPATION'  
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','GEOGRAPHICAL',UPPER(ISNULL(LTRIM(RTRIM(GEOGCD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'GEOGRAPHICAL'  
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','EDUCATION',UPPER(ISNULL(LTRIM(RTRIM(EDU)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'EDUCATION'  
          
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','LANGUAGE',UPPER(ISNULL(LTRIM(RTRIM(LANGCD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'LANGUAGE'  
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + CASE WHEN UPPER(ISNULL(LTRIM(RTRIM(STAFF)),'')) = 'N' THEN 'NONE'   
                                                                                                      WHEN UPPER(ISNULL(LTRIM(RTRIM(STAFF)),'')) = 'R' THEN 'RELATIVE'  
                                                                                                      WHEN UPPER(ISNULL(LTRIM(RTRIM(STAFF)),'')) = 'S' THEN 'STAFF' ELSE '' END   + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'STAFF'  
        --SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+ CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + UPPER([CITRUS_USR].[FN_REVERSE_MAPPING]('CDSL','ANNUAL_INCOME',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR  WHERE BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'ANNUAL_INCOME'  
    /*new kyc aug 2021*/
        SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+  CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567','0225105','0225104') then '' else  UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_income',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) end    + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR, dp_acct_mstr   WHERE BOID = DPAM_SBA_NO  and BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'ANNUAL_INCOME'    and UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_income',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),''))))  <> '' 
			
		SELECT DISTINCT @L_ENTP_VALUE = ISNULL(@L_ENTP_VALUE,'')+  CONVERT(VARCHAR,ENTPM_PROP_ID) + '|*~|' + case when dpam_subcm_cd in ('2156','2155','2150','082104','022512','022552','102624','122624','192624','022545','512624','022567','0225105','0225104') then  UPPER([CITRUS_USR].fn_get_PanBOBONA_Val_reverse('annual_incomeNI',UPPER(ISNULL(LTRIM(RTRIM(ANNINCOMECD)),'')))) else '' end  + '|*~|*|~*' FROM DPB9_PC1 ,ENTITY_PROPERTY_MSTR, dp_acct_mstr   WHERE BOID = DPAM_SBA_NO  and BOID = @C_BEN_ACCT_NO AND ENTPM_CD   = 'annual_incomeNI'    
		/*new kyc aug 2021*/                    
...
SELECT @l_dppd_details = @l_dppd_details + isnull(convert(varchar,@l_compm_id),'')+'|*~|'+ isnull(convert(varchar,@l_excsm_id),'')+'|*~|'+isnull(convert(varchar,@l_dpm_dpid),'')+'|*~|'+isnull(@c_ben_acct_no,'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ case when HolderNum ='1' then ISNULL(ltrim(rtrim('1ST HOLDER')),'') when HolderNum ='2' then ISNULL(ltrim(rtrim('2ND HOLDER')),'')  else ISNULL(ltrim(rtrim('1ST HOLDER')),'')  end  +'|*~|'+ISNULL(ltrim(rtrim(poam_name1)),'') +'|*~|'+ISNULL(ltrim(rtrim(poam_name2)),'')+'|*~|'+ISNULL(ltrim(rtrim(poam_name3)),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim('')),'')+'|*~|'+ISNULL(ltrim(rtrim(POARegNum)),'')+'|*~|'+ISNULL(ltrim(rtrim(GPABPAFlg)),'')  
 ...
 FROM DPB9_PC5 , citrus_usr.poam WHERE MasterPOAId = poam_master_id and  boid = @c_ben_acct_no AND POARegNum <>''   
   ...
   update entity_relationship set entr_dummy5 = 15765299 where  entr_dummy5 = 0   
  
--exec 		pr_upd_nom_dpb9
--exec 		pr_upd_nom_g_dpb9
  
     exec citrus_usr.pr_get_refund
