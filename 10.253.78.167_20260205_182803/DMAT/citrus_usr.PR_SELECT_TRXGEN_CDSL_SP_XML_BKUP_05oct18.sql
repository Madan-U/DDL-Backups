-- Object: PROCEDURE citrus_usr.PR_SELECT_TRXGEN_CDSL_SP_XML_BKUP_05oct18
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------



create PROCEDURE [citrus_usr].[PR_SELECT_TRXGEN_CDSL_SP_XML_BKUP_05oct18]  
(  
...
+
case when len(replace(details ,' ','|')) ='112' then '<Rsn>'+ ltrim(rtrim(substring(details , 112 , 1))) +'</Rsn>'
+'<Txnelflg>'+ TRXEFLG +'</Txnelflg>'
...
else '<Rsn>'+ ltrim(rtrim(substring(details , 112 , 2))) +'</Rsn>'
+'<Txnelflg>'+ TRXEFLG +'</Txnelflg>'
+ case when citrus_usr.fn_get_poa_id(dptdc_slip_no) = '' then '' else +'<Poa>'+ citrus_usr.fn_get_poa_id(dptdc_slip_no)  +'</Poa>' end  
...
+case when len(replace(details ,' ','|')) ='112' then '<Rsn>'+ ltrim(rtrim(substring(details , 70 , 1))) +'</Rsn>'
...
else '<Rsn>'+ ltrim(rtrim(substring(details , 70 , 2))) +'</Rsn>'
