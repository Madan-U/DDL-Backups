-- Object: PROCEDURE citrus_usr.pr_rpt_demat_All
...
CREATE  PROCEDURE [citrus_usr].[pr_rpt_demat_All]                                      
(
@PA_EXCSM_ID INT,
...
@l_drfno varchar(25),
@l_drnno varchar(25),@l_rejtyp varchar(20)


if (@PA_ACTION='OUT_CLT_DISP_RPT|*~||*~|1' or @PA_ACTION='OUT_CLT_DISP_RPT|*~||*~|0')
begin 
 
	if [citrus_usr].[FN_SPLITVAL_BY](@PA_ACTION,3,'|*~|')='0'
	begin
	set @l_rejtyp='COMP'
	set @l_drnno=''
	set @PA_LOGIN_ENTM_CD_CHAIN=REPLACE(@PA_LOGIN_ENTM_CD_CHAIN,'0|*~|','')
	end 
	if [citrus_usr].[FN_SPLITVAL_BY](@PA_ACTION,3,'|*~|')='1'
	begin
	set @l_rejtyp='INTERNAL'
	set @l_drnno=''
	set @PA_LOGIN_ENTM_CD_CHAIN=REPLACE(@PA_LOGIN_ENTM_CD_CHAIN,'1|*~|','')
	end 
end
 


select @l_drfno = [citrus_usr].[FN_SPLITVAL_BY](@PA_ACTION,2,'|*~|')
 
select @l_drnno = [citrus_usr].[FN_SPLITVAL_BY](@PA_ACTION,3,'|*~|')
select @PA_ACTION = [citrus_usr].[FN_SPLITVAL_BY](@PA_ACTION,1,'|*~|')
...
IF @PA_ACTION not in ( 'OUT_RTA_DISP_RPT','OUT_CLT_DISP_RPT')   
begin 
INSERT INTO #ACLIST SELECT b.DPAM_ID,b.dpam_sba_no,b.dpam_sba_name,EFF_FROM,isnull(EFF_TO,'2100-12-31 00:00:00.000')
,b.dpam_clicm_Cd  FROM citrus_usr.fn_acct_list(@@L_DPM_ID ,@PA_LOGIN_PR_ENTM_ID,@@l_child_entm_id)	a , dp_acct_mstr b   where 	a.dpam_id = b.dpam_id 
end 

create index ix_1 on #ACLIST(DPAM_ID  )
...
,DISP_REMINDER,DISP_DEMRM_ID,DISP_ID,ENTM_NAME1, isnull(isin_adr1,'') replacestr
	...
	AND DEMRM_REQUEST_DT BETWEEN EFF_FROM AND EFF_TO
	--AND ISNULL(DEMRM_INTERNAL_REJ,'')  = ''       
	AND (ISNULL(DEMRM_INTERNAL_REJ,'')  = ''      or isnull (DEMRM_STATUS,'') =  'S'	    )
	AND ISNULL(DEMRM_COMPANY_OBJ,'') = ''      
	AND ISNULL(DEMRM_CREDIT_RECD,'') <> 'Y'         
...
-- where cdshm_dpam_id=demrm_dpam_id and demrm_isin=cdshm_isin  and cdshm_tratm_cd  in ('2246'))
 ...
 declare @l_dpam_id table (id numeric)

 if @l_drfno <> '' 
 insert into @l_dpam_id
 select DEMRM_DPAM_ID from demrm_mak with(nolock) where DEMRM_SLIP_SERIAL_NO =  @l_drfno 

 
 if @l_drnno <> '' 
 insert into @l_dpam_id
 select DEMRM_DPAM_ID from demat_request_mstr  with(nolock) where DEMRM_TRANSACTION_NO =  @l_drnno 


 if exists (select 1 from @l_dpam_id)
 begin 

  INSERT INTO #ACLIST SELECT b.DPAM_ID,b.dpam_sba_no,b.dpam_sba_name,EFF_FROM,isnull(EFF_TO,'2100-12-31 00:00:00.000')
,b.dpam_clicm_Cd  FROM citrus_usr.fn_acct_list(@@L_DPM_ID ,@PA_LOGIN_PR_ENTM_ID,@@l_child_entm_id)	a , dp_acct_mstr b , @l_dpam_id  where 	a.dpam_id = id and a.dpam_id = b.dpam_id 
...
--select * into temp_ACLIST_21122015 from #ACLIST --where dpam_sba_no in ('1201090004837631')

   --      print @l_drfno
   --      print @l_drnno
		 --return
		 
	if @l_drfno <> '' and @l_drnno = ''
	begin
 print '1'
			SELECT distinct   isnull (citrus_usr.fn_find_relations_nm_by_dpamno(dpam_sba_no,'BR'),'') brcd  , '' BRDETAILS,DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,'' DRN
			...
			,CONVERT(VARCHAR,DISP_DT,103) DISP_DT,ISNULL(DISP_DOC_ID,'0'),ISNULL(DISP_NAME,''),ISNULL(DISP_CONF_RECD,'') DISP_CONF_RECD                      
			,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
			,ISNULL(DPHD_SH_FNAME,'') SECONDHOLDER,ISNULL(DPHD_TH_FNAME,'') THIRDHOLDER , DEMRM_FREE_LOCKEDIN_YN  LOCKIN 
			, case when isnull(demrm_res_desc_intobj,'') <> '' then isnull(demrm_res_desc_intobj,'') else isnull(demrm_res_desc_compobj,'') end
			+ case when isnull(demrm_RMKS,'') = '' then  '' else '-' end  + isnull(demrm_RMKS,'')  REJ --DESCP REJ 
			, CASE WHEN ISNULL(DEMRM_COMPANY_OBJ,'') <> '' THEN 'COMP' ELSE 'INTERNAL' END OBJ_TYPE      
			, ISNULL(DISP_DOC_ID,'0') disp_doc_id  
			,demrm_RMKS  REMARKS
			,CONVERT(VARCHAR,DEMRM_LST_UPD_DT,103) DEMRM_LST_UPD_DT -- CHNAGING AS PER MOSL REQUEST ON 15032013,CONVERT(VARCHAR,DEMRM_LST_UPD_DT,103) DEMRM_LST_UPD_DT
			...
			AND DPAM_SBA_NO >= @PA_FROM_ACCT AND DPAM_SBA_NO  <= @PA_TO_ACCT
			AND DEMRM_REQUEST_DT BETWEEN EFF_FROM AND EFF_TO
			and demrm_slip_serial_no = case when @l_drfno = '' then '%' else @l_drfno end	
			AND A_CLICM_CD=CLICM_CD          
			AND CLICM_DELETED_IND=1 
		  and demrm_deleted_ind = 0
		union  all

		SELECT distinct isnull (citrus_usr.fn_find_relations_nm_by_dpamno(dpam_sba_no,'BR'),'') brcd,'' BRDETAILS,DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,DEMRM_TRANSACTION_NO DRN
			,DEMRM_SLIP_SERIAL_NO DRF,DEMRM_DPAM_ID                     
			,CONVERT(VARCHAR,DEMRM_REQUEST_DT,103) DEMRM_REQUEST_DT ,ISNULL(DEMRM_EXECUTION_DT,'') ,convert(numeric(18,3),DEMRM_QTY) QTY, ISNULL(DISP_TYPE,'')                      
			,CONVERT(VARCHAR,DISP_DT,103) DISP_DT,ISNULL(DISP_DOC_ID,'0'),ISNULL(DISP_NAME,''),ISNULL(DISP_CONF_RECD,'') DISP_CONF_RECD                      
			,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
			,ISNULL(DPHD_SH_FNAME,'') SECONDHOLDER,ISNULL(DPHD_TH_FNAME,'') THIRDHOLDER , DEMRM_FREE_LOCKEDIN_YN  LOCKIN 
			, case when isnull('','') <> '' then isnull('','') else isnull(DESCP,'') end 
			 REJ --DESCP REJ 
			, CASE WHEN ISNULL(DEMRM_COMPANY_OBJ,'') <> '' THEN 'COMP' ELSE 'INTERNAL' END OBJ_TYPE      
			, ISNULL(DISP_DOC_ID,'0') disp_doc_id  
			,demrm_RMKS  REMARKS
		  ,(select top 1 CONVERT(VARCHAR,cdshm_tras_dt,103) from cdsl_holding_dtls where cdshm_trans_no=demrm_transaction_no and cdshm_dpam_id=dpam_id
		and cdshm_isin=demrm_isin and CDSHM_TRATM_CD='3102' and CDSHM_CDAS_SUB_TRAS_TYPE='608') DEMRM_LST_UPD_DT --,CONVERT(VARCHAR,DEMRM_LST_UPD_DT,103) DEMRM_LST_UPD_DT
		...
		,#ACLIST  LEFT OUTER JOIN DP_HOLDER_DTLS DPHD ON DPHD_DPAM_ID = DPAM_ID    
		  ...
		  and demrm_deleted_ind = 1
			...
			union  all --- for new demat int rej on feb 18 2020

		SELECT distinct isnull (citrus_usr.fn_find_relations_nm_by_dpamno(dpam_sba_no,'BR'),'') brcd,'' BRDETAILS,DPAM_SBA_NO
		,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,DEMRM_TRANSACTION_NO DRN
			,DEMRM_SLIP_SERIAL_NO DRF,DEMRM_DPAM_ID                     
			,CONVERT(VARCHAR,DEMRM_REQUEST_DT,103) DEMRM_REQUEST_DT ,ISNULL(DEMRM_EXECUTION_DT,'') ,convert(numeric(18,3),DEMRM_QTY) QTY, ISNULL(DISP_TYPE,'')                      
			,CONVERT(VARCHAR,DISP_DT,103) DISP_DT,ISNULL(DISP_DOC_ID,'0'),ISNULL(DISP_NAME,''),ISNULL(DISP_CONF_RECD,'') DISP_CONF_RECD                      
			,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
			,ISNULL(DPHD_SH_FNAME,'') SECONDHOLDER,ISNULL(DPHD_TH_FNAME,'') THIRDHOLDER , DEMRM_FREE_LOCKEDIN_YN  LOCKIN 
			, ISNULL(DEMRM_INTERNAL_REJ,'') REJ  --case when isnull('','') <> '' then isnull('','') else isnull(DESCP,'') end REJ --DESCP REJ 
			, CASE WHEN ISNULL(DEMRM_COMPANY_OBJ,'') <> '' THEN 'COMP' ELSE 'INTERNAL' END OBJ_TYPE      
			, ISNULL(DISP_DOC_ID,'0') disp_doc_id  
			,demrm_RMKS  REMARKS
		  ,(select top 1 CONVERT(VARCHAR,cdshm_tras_dt,103) from cdsl_holding_dtls where cdshm_trans_no=demrm_transaction_no and cdshm_dpam_id=dpam_id
		...
		,#ACLIST  LEFT OUTER JOIN DP_HOLDER_DTLS DPHD ON DPHD_DPAM_ID = DPAM_ID    
			...
			--AND (ISNULL(DEMRM_INTERNAL_REJ,'')  <> '' OR ISNULL(DEMRM_COMPANY_OBJ,'') <> '')      
			--AND (ISNULL(DEMRM_COMPANY_OBJ,'') = CD)    
			AND ISNULL(DEMRM_INTERNAL_REJ,'') <> '' and ISNULL(DEMRM_ERRMSG,'')<>''      
			AND DEMRM_DPAM_ID = DPAM_ID       
			AND DPAM_SBA_NO >= @PA_FROM_ACCT AND DPAM_SBA_NO  <= @PA_TO_ACCT
			AND DEMRM_REQUEST_DT BETWEEN EFF_FROM AND EFF_TO
			and demrm_slip_serial_no = case when @l_drfno = '' then '%' else @l_drfno end	
		  ...
		  and demrm_deleted_ind = 1
			...
			,DEMRM_SLIP_SERIAL_NO ,DEMRM_DPAM_ID   --- for new demat int rej on feb 18 2020
			
		end
else if @l_drnno <> '' and @l_drfno =''
		begin	
 print '2'
			SELECT distinct isnull (citrus_usr.fn_find_relations_nm_by_dpamno(dpam_sba_no,'BR'),'') brcd,'' BRDETAILS,DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,DEMRM_TRANSACTION_NO DRN
			,DEMRM_SLIP_SERIAL_NO DRF,DEMRM_DPAM_ID                     
			...
			--,CLICM.CLICM_DESC CATEGORY
			, '' CATEGORY
			,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
			,ISNULL(DPHD_SH_FNAME,'') SECONDHOLDER,ISNULL(DPHD_TH_FNAME,'') THIRDHOLDER , DEMRM_FREE_LOCKEDIN_YN  LOCKIN 
			, case when isnull('','') <> '' then isnull('','') else isnull(DESCP,'') end REJ --DESCP REJ 
			, CASE WHEN ISNULL(DEMRM_COMPANY_OBJ,'') <> '' THEN 'COMP' ELSE 'INTERNAL' END OBJ_TYPE      
			, ISNULL(DISP_DOC_ID,'0') disp_doc_id  
			,demrm_RMKS  REMARKS
			...
			AND DEMRM_REQUEST_DT BETWEEN EFF_FROM AND EFF_TO
		--	and demrm_slip_serial_no = case when @l_drfno = '' then '%' else @l_drfno end
			and DEMRM_TRANSACTION_NO = case when @l_drnno = '' then '%' else @l_drnno end		
		 --   AND A_CLICM_CD=CLICM_CD          
			--AND CLICM_DELETED_IND=1 
			and demrm_deleted_ind = 1
			ORDER BY DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID 
		...
		begin
			...
			if @l_rejtyp='COMP'
				...
				--AND A_CLICM_CD=CLICM_CD          
				--AND CLICM_DELETED_IND=1 
			 -- and demrm_deleted_ind = 0 --and ISNULL(DEMRM_COMPANY_OBJ,'') <> ''
				--union  all

				SELECT distinct isnull(citrus_usr.fn_find_relations_nm_by_dpamno(dpam_sba_no,'BR'),'') brcd
				,'' BRDETAILS,DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,isnull(DEMRM_TRANSACTION_NO,'') DRN
					,DEMRM_SLIP_SERIAL_NO DRF,DEMRM_DPAM_ID                     
					,CONVERT(VARCHAR,DEMRM_REQUEST_DT,103) DEMRM_REQUEST_DT ,ISNULL(DEMRM_EXECUTION_DT,'') ,convert(numeric(18,3),DEMRM_QTY) QTY, ISNULL(DISP_TYPE,'')                      
					,CONVERT(VARCHAR,DISP_DT,103) ,ISNULL(DISP_DOC_ID,'0'),ISNULL(DISP_NAME,''),ISNULL(DISP_CONF_RECD,'') DISP_CONF_RECD                      
					,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
				...
				,#ACLIST  LEFT OUTER JOIN DP_HOLDER_DTLS DPHD ON DPHD_DPAM_ID = DPAM_ID    
					...
					AND A_CLICM_CD=CLICM_CD          
					AND CLICM_DELETED_IND=1 
					and demrm_deleted_ind = 1 and ISNULL(DEMRM_COMPANY_OBJ,'') <> ''
					ORDER BY DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID 
					,DEMRM_SLIP_SERIAL_NO ,DEMRM_DPAM_ID 
					end --- rejtype
		...
		if @l_rejtyp='INTERNAL'
				...
				,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
				,ISNULL(DPHD_SH_FNAME,'') SECONDHOLDER,ISNULL(DPHD_TH_FNAME,'') THIRDHOLDER , DEMRM_FREE_LOCKEDIN_YN  LOCKIN 
				, case when isnull(demrm_res_desc_intobj,'') <> '' then isnull(demrm_res_desc_intobj,'') else isnull(demrm_res_desc_compobj,'') end + case when isnull(demrm_RMKS,'') = '' then  '' else '-' end  + isnull(demrm_RMKS,'')REJ --DESCP REJ 
				, CASE WHEN ISNULL(DEMRM_COMPANY_OBJ,'') <> '' THEN 'COMP' ELSE 'INTERNAL' END OBJ_TYPE      
				, ISNULL(DISP_DOC_ID,'0') disp_doc_id  
				,demrm_RMKS  REMARKS
				,CONVERT(VARCHAR,GETDATE(),103) DEMRM_LST_UPD_DT -- CHNAGING AS PER MOSL REQUEST ON 15032013,CONVERT(VARCHAR,DEMRM_LST_UPD_DT,103) DEMRM_LST_UPD_DT
			...
			SELECT distinct isnull(citrus_usr.fn_find_relations_nm_by_dpamno(dpam_sba_no,'BR'),'') brcd,'' BRDETAILS,DPAM_SBA_NO,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,isnull(DEMRM_TRANSACTION_NO,'') DRN
				,DEMRM_SLIP_SERIAL_NO DRF,DEMRM_DPAM_ID                     
				,CONVERT(VARCHAR,DEMRM_REQUEST_DT,103) DEMRM_REQUEST_DT ,ISNULL(DEMRM_EXECUTION_DT,'') ,convert(numeric(18,3),DEMRM_QTY) QTY, ISNULL(DISP_TYPE,'')                      
				,CONVERT(VARCHAR,DISP_DT,103) ,ISNULL(DISP_DOC_ID,'0'),ISNULL(DISP_NAME,''),ISNULL(DISP_CONF_RECD,'') DISP_CONF_RECD                      
				,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
   ...
   ELSE if isnull(@l_rejtyp,'')=''
				...
				,CLICM.CLICM_DESC CATEGORY,ISNULL(DEMRM_TOTAL_CERTIFICATES,'0') NO_OF_CERTIFICATE          
				,ISNULL(DPHD_SH_FNAME,'') SECONDHOLDER,ISNULL(DPHD_TH_FNAME,'') THIRDHOLDER , DEMRM_FREE_LOCKEDIN_YN  LOCKIN 
				, case when isnull(demrm_res_desc_intobj,'') <> '' then isnull(demrm_res_desc_intobj,'') else isnull(demrm_res_desc_compobj,'') end + '-' + isnull(demrm_RMKS,'') REJ --DESCP REJ 
				, CASE WHEN ISNULL(DEMRM_COMPANY_OBJ,'') <> '' THEN 'COMP' ELSE 'INTERNAL' END OBJ_TYPE      
				, ISNULL(DISP_DOC_ID,'0') disp_doc_id  
				,demrm_RMKS  REMARKS
				,CONVERT(VARCHAR,GETDATE(),103) DEMRM_LST_UPD_DT -- CHNAGING AS PER MOSL REQUEST ON 15032013,CONVERT(VARCHAR,DEMRM_LST_UPD_DT,103) DEMRM_LST_UPD_DT
		...
		,DPAM_SBA_NAME,DEMRM_ID,DEMRM_ISIN+'/'+ISIN_NAME DEMRM_ISIN,DEMRM_TRANSACTION_NO DRN
			...
			, ISNULL(DISP_DOC_ID,'0') disp_doc_id  
			,demrm_RMKS  REMARKS
		  ,isnull((select top 1 CONVERT(VARCHAR,cdshm_tras_dt,103) from cdsl_holding_dtls where cdshm_trans_no=demrm_transaction_no and cdshm_dpam_id=dpam_id
		and cdshm_isin=demrm_isin and CDSHM_TRATM_CD='3102' and CDSHM_CDAS_SUB_TRAS_TYPE='608'),'') DEMRM_LST_UPD_DT --,CONVERT(VARCHAR,DEMRM_LST_UPD_DT,103) DEMRM_LST_UPD_DT
			FROM DMAT_DISPATCH , demat_request_mstr left outer join ISIN_MSTR on  DEMRM_ISIN = ISIN_CD
		,#ACLIST  LEFT OUTER JOIN DP_HOLDER_DTLS DPHD ON DPHD_DPAM_ID = DPAM_ID    
			AND DPHD_DELETED_IND=1    
