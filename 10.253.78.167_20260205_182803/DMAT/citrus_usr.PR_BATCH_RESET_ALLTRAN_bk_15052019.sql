-- Object: PROCEDURE citrus_usr.PR_BATCH_RESET_ALLTRAN_bk_15052019
...
--rollback
create PROCEDURE [citrus_usr].[PR_BATCH_RESET_ALLTRAN_bk_15052019]  
(  
@PA_ID INTEGER,  
...
--		END 
					...
					IF NOT EXISTS(SELECT DEMRM_ID FROM DEMAT_REQUEST_MSTR WHERE DEMRM_batch_no = @PA_BATCHNO  AND  isnull(DEMRM_status, '') <> 'P' AND DEMRM_deleted_ind = 1 AND DEMRM_REQUEST_DT >= 'JAN 01 2013')  
					BEGIN--6 CDSL

					  UPDATE T1 SET T1.DEMRM_BATCH_NO=NULL,T1.DEMRM_STATUS = 'P' FROM DEMAT_REQUEST_MSTR T1,DP_ACCT_MSTR T2 WHERE T1.DEMRM_BATCH_NO = @PA_BATCHNO AND  T1.DEMRM_DPAM_ID=T2.DPAM_ID AND DPAM_DPM_ID=@L_DPM_ID AND T1.DEMRM_DELETED_IND=1 AND DPAM_DELETED_IND=1 AND DEMRM_REQUEST_DT >= 'JAN 01 2013'
					  --DELETE B1 FROM BATCHNO_CDSL_MSTR B1  WHERE BATCHC_NO = @PA_BATCHNO  AND BATCHC_DPM_ID = @L_DPM_ID AND BATCHC_TYPE='T' AND BATCHC_TRANS_TYPE = @PA_TRXTYPE 
					  --UPDATE BATCHNO_CDSL_MSTR SET BATCHC_DELETED_IND=9,BATCHC_STATUS='C' WHERE BATCHC_NO = @PA_BATCHNO  AND BATCHC_DPM_ID = @L_DPM_ID AND BATCHC_TYPE='T' AND BATCHC_TRANS_TYPE = @PA_TRXTYPECDSL  AND BATCHC_DELETED_IND=1
					END 
					ELSE
...
PRINT 'DMAT'
        			...
        			IF NOT EXISTS(SELECT REMRM_ID FROM REMAT_REQUEST_MSTR WHERE REMRM_batch_no = @PA_BATCHNO  AND  isnull(REMRM_status, '') <> 'P' AND REMRM_deleted_ind = 1 AND REMRM_REQUEST_DT >= 'JAN 01 2013')  
					BEGIN--6 CDSL
					  UPDATE T1 SET T1.REMRM_BATCH_NO=NULL, T1.REMRM_STATUS = 'P' FROM REMAT_REQUEST_MSTR T1,DP_ACCT_MSTR T2 WHERE T1.REMRM_BATCH_NO = @PA_BATCHNO AND  T1.REMRM_DPAM_ID=T2.DPAM_ID AND DPAM_DPM_ID=@L_DPM_ID AND T1.REMRM_DELETED_IND=1 AND DPAM_DELETED_IND=1 AND REMRM_REQUEST_DT >= 'JAN 01 2013'   
					  --DELETE B1 FROM BATCHNO_CDSL_MSTR B1  WHERE BATCHC_NO = @PA_BATCHNO  AND BATCHC_DPM_ID = @L_DPM_ID AND BATCHC_TYPE='T' AND BATCHC_TRANS_TYPE = @PA_TRXTYPE 
					  --UPDATE BATCHNO_CDSL_MSTR SET BATCHC_DELETED_IND=9,BATCHC_STATUS='C' WHERE BATCHC_NO = @PA_BATCHNO  AND BATCHC_DPM_ID = @L_DPM_ID AND BATCHC_TYPE='T' AND BATCHC_TRANS_TYPE = @PA_TRXTYPECDSL  AND BATCHC_DELETED_IND=1
					END 
					ELSE
...
PRINT 'REMAT'
					...
					IF NOT EXISTS(SELECT PLDTC_ID FROM cdsl_pledge_dtls WHERE PLDTC_batch_no = @PA_BATCHNO  AND  isnull(PLDTC_status, '') <> 'P' AND PLDTC_deleted_ind = 1 AND PLDTC_REQUEST_DT >='JAN 01 2013')  
						BEGIN
							UPDATE T1 SET T1.PLDTC_BATCH_NO=NULL,T1.PLDTC_STATUS = 'P' FROM cdsl_pledge_dtls T1,DP_ACCT_MSTR T2 WHERE T1.PLDTC_BATCH_NO = @PA_BATCHNO AND  T1.PLDTC_DPAM_ID=T2.DPAM_ID AND DPAM_DPM_ID=@L_DPM_ID AND T1.PLDTC_DELETED_IND=1 AND DPAM_DELETED_IND=1 AND PLDTC_REQUEST_DT >='JAN 01 2013'
							--UPDATE BATCHNO_CDSL_MSTR SET BATCHC_DELETED_IND=9,BATCHC_STATUS='C' WHERE BATCHC_NO = @PA_BATCHNO  AND BATCHC_DPM_ID = @L_DPM_ID AND BATCHC_TYPE='T' AND BATCHC_TRANS_TYPE = @PA_TRXTYPECDSL  AND BATCHC_DELETED_IND=1
						END 
					ELSE
						BEGIN
PRINT 'PL'
					...
					IF NOT EXISTS(SELECT dptdc_id FROM dp_trx_dtls_cdsl WHERE dptdc_batch_no = @PA_BATCHNO  AND isnull(dptdc_status, '') <> 'P' AND dptdc_deleted_ind = 1 AND DPTDC_REQUEST_DT >= 'JAN 01 2013' )  
					BEGIN--6 CDSL  
PRINT 'PANKAJ'
					  UPDATE T1 SET T1.DPTDC_BATCH_NO=NULL,T1.DPTDC_STATUS = 'P' FROM DP_TRX_DTLS_CDSL T1,DP_ACCT_MSTR T2 WHERE DPTDC_BATCH_NO = @PA_BATCHNO AND  T1.DPTDC_DPAM_ID=T2.DPAM_ID AND DPAM_DPM_ID=@L_DPM_ID AND DPTDC_DELETED_IND=1 AND DPAM_DELETED_IND=1 AND DPTDC_REQUEST_DT >= 'JAN 01 2013' 
					  --DELETE B1 FROM BATCHNO_CDSL_MSTR B1  WHERE BATCHC_NO = @PA_BATCHNO  AND BATCHC_DPM_ID = @L_DPM_ID AND BATCHC_TYPE='T' AND BATCHC_TRANS_TYPE = @PA_TRXTYPE 
...
PRINT 'TRAN'
					...
					END
--print 'before batch no'
--print @PA_BATCHNO
--print @L_DPM_ID
--print @PA_TRXTYPECDSL

					UPDATE BATCHNO_CDSL_MSTR SET BATCHC_DELETED_IND=9,BATCHC_STATUS='C'
					WHERE BATCHC_NO = @PA_BATCHNO  AND BATCHC_DPM_ID = @L_DPM_ID AND BATCHC_TYPE='T'
					AND BATCHC_TRANS_TYPE = 'ALL'  AND BATCHC_DELETED_IND=1
--print 'after batch no'
