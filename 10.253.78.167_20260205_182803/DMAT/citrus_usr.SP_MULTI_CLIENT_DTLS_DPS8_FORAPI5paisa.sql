-- Object: PROCEDURE citrus_usr.SP_MULTI_CLIENT_DTLS_DPS8_FORAPI5paisa
...
CREATE PROCEDURE [citrus_usr].[SP_MULTI_CLIENT_DTLS_DPS8_FORAPI5paisa]  
(   
...
SET @PA_TO_ACCTNO = citrus_usr.fn_splitval(@PA_TO_ACCTNO,1)    
 ...
 END              
  ...
  set @l_close_dt = ''
  select distinct  accp_clisba_id,accp_value into #account_properties from account_properties where accp_acct_no = @PA_FROM_ACCTNO  and
  accp_accpm_prop_cd ='Bill_start_dt'
...
--                         else STAM_DESC end end [STATUS]  
 SELECT distinct case when isnull(citrus_usr.to_get_freezestat(dpam.dpam_sba_no),'')<>'' then isnull(citrus_usr.to_get_freezestat(dpam.dpam_sba_no),'')  else case when isnull(fre_action,'')= 'F' and  dpam.dpam_stam_cd  <> '05' then 'FREEZED' else 
...
--isnull(REPLACE(CONVERT(VARCHAR(11),dps8_pc1.ClosDt, 106), ' ', '-'),'')
                                                else STAM_DESC end end  +  '       -' + isnull(fre_rmks,'') end
                                                + case when  citrus_usr.fn_get_high_val('',0,'DORMANT',dpam.dpam_sba_no,convert(datetime,convert(varchar(11),getdate(),103),103))   ='Y' then  '- DORMANT '   else '' end  [STATUS]  
...
,case when isnull(dps8_pc1.AnnIncomeCd,'') = '1' then 'Upto 1 Lakhs'                     
                        when isnull(dps8_pc1.AnnIncomeCd,'') =  '2'  then '1 Lakh To 2 Lakhs'                    
                        when isnull(dps8_pc1.AnnIncomeCd,'') = '3'  then     '2 Lakh To 5 Lakhs'                
                        when isnull(dps8_pc1.AnnIncomeCd,'') = '4'  then '5 Lakhs & above'                                                                 
                        when isnull(dps8_pc1.AnnIncomeCd,'') =  '5' then 'Not Available'      
when isnull(dps8_pc1.AnnIncomeCd,'') = '6' then '1 LAC - 5 LACS'
when isnull (dps8_pc1.AnnIncomeCd,'') = '7' then '5 LACS - 10 LACS'
when isnull(dps8_pc1.AnnIncomeCd,'') = '8' then '10 LACS - 25 LACS'
when isnull(dps8_pc1.AnnIncomeCd,'') = '9' then '> 25 LACS'
when isnull(dps8_pc1.AnnIncomeCd,'') = '10' then '25 LACS - 1 CRORE'
when isnull (dps8_pc1.AnnIncomeCd,'') = '11' then '> 1 CRORE'               
                        else '' end     Income_dtls
                                         ,dps8_pc1.UnqIdenNum  Aadhar_no
                                         
,ltrim(rtrim(isnull(DPS8_PC6_2.Name,''))) +' '+ ltrim(rtrim(isnull(DPS8_PC6_2.MiddleName,'')))+' '+ltrim(rtrim(isnull(DPS8_PC6_2.SearchName ,'')))   DPHD_NOM2_FNAME 
,ltrim(rtrim(isnull(DPS8_PC6_3.Name,''))) +' '+ ltrim(rtrim(isnull(DPS8_PC6_3.MiddleName,'')))+' '+ltrim(rtrim(isnull(DPS8_PC6_3.SearchName ,'')))   DPHD_NOM3_FNAME 
,dps8_pc6.rel_with_bo BO_RELATION_NOM1
,dps8_pc6_2.rel_with_bo BO_RELATION_NOM2
,DPS8_PC6_3.rel_with_bo BO_RELATION_NOM3
,dps8_pc6.perc_of_shares PERC_SHARES_NOM1 
,dps8_pc6_2.perc_of_shares PERC_SHARES_NOM2
,DPS8_PC6_3.perc_of_shares PERC_SHARES_NOM3   
...
Addr2,Addr3,City,State,Country,PinCode,PANGIR,DateOfBirth,TypeOfTrans,MAX(DateOfSetup)  D  ,RES_SEC_FLg
,NOM_Sr_No
,case when  rel_with_bo ='1' then 'Spouse'
...
when  rel_with_bo ='12' then 'Not Provided (if the relationship is not provided on nomination form)'
when  rel_with_bo ='13' then 'Others' else '' end  rel_with_bo

,perc_OF_SHARES 

FROM dps8_pc6 where TypeOfTrans <> '3' and isnull(NOM_Sr_No,'0') in ('0','1')  GROUP BY BOID,Name,MiddleName,SearchName ,FthName,Addr1,Addr2,  
Addr3,City,State,Country,PinCode,PANGIR,DateOfBirth,TypeOfTrans 
,RES_SEC_FLg
,NOM_Sr_No
,case when  rel_with_bo ='1' then 'Spouse'
when  rel_with_bo ='2' then 'Son'
...
when  rel_with_bo ='11' then 'Grand-Mother'
when  rel_with_bo ='12' then 'Not Provided (if the relationship is not provided on nomination form)'
when  rel_with_bo ='13' then 'Others' else '' end 

,perc_OF_SHARES
)  dps8_pc6  on dps8_pc6.boid = dps8_pc1.boid  
left outer join (SELECT BOID,Name,MiddleName,SearchName ,FthName,Addr1,  
Addr2,Addr3,City,State,Country,PinCode,PANGIR,DateOfBirth,MAX(DateOfSetup) D ,RES_SEC_FLg
,NOM_Sr_No
,case when  rel_with_bo ='1' then 'Spouse'
when  rel_with_bo ='2' then 'Son'
when  rel_with_bo ='3' then 'Daughter'
...
,perc_OF_SHARES 
FROM dps8_pc6 where TypeOfTrans <> '3' and isnull(NOM_Sr_No,'0') in ('2')GROUP BY BOID,Name,MiddleName,SearchName ,FthName,Addr1,Addr2,  
...
,perc_OF_SHARES
)  dps8_pc6_2  on dps8_pc6_2.boid = dps8_pc1.boid  and  dps8_pc6.TypeOfTrans  not in ('4','3')
left outer join (SELECT BOID,Name,MiddleName,SearchName ,FthName,Addr1,  
Addr2,Addr3,City,State,Country,PinCode,PANGIR,DateOfBirth,MAX(DateOfSetup) D ,RES_SEC_FLg
,NOM_Sr_No
,case when  rel_with_bo ='1' then 'Spouse'
...
,perc_OF_SHARES 
FROM dps8_pc6 where TypeOfTrans <> '3' and isnull(NOM_Sr_No,'0') in ('3') GROUP BY BOID,Name,MiddleName,SearchName ,FthName,Addr1,Addr2,  
Addr3,City,State,Country,PinCode,PANGIR,DateOfBirth
,RES_SEC_FLg
,NOM_Sr_No
,case when  rel_with_bo ='1' then 'Spouse'
when  rel_with_bo ='2' then 'Son'
...
,perc_OF_SHARES
)  dps8_pc6_3  on dps8_pc6_3.boid = dps8_pc1.boid   
  left outer join (SELECT BOID,Name,MiddleName,SearchName ,FthName,Addr1,  
...
--  DP_HOLDER_DTLS ON  DPAM_ID = DPHD_DPAM_ID AND DPHD_DELETED_IND =1  
  ...
  ,STATUS_MSTR  
  ,SUB_CTGRY_MSTR   
  ,CLIENT_MSTR  ,#account_properties
  --,CLIENT_BANK_ACCTS  
  --,BANK_MSTR    
  --,CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID) ACCOUNT            
    
    
     WHERE isNumeric(dpam.DPAM_SBA_NO)=1   and accp_clisba_id=dpam_id
...
and dps8_pc1.boid  BETWEEN @PA_FROM_ACCTNO AND @PA_TO_ACCTNO  
  ...
  AND   CLIM_CRN_NO        = DPAM.DPAM_CRN_NO   and isnull(BOActDt,'')<>''
  --and   convert(datetime,left(BOActDt,2) + '/' + substring(BOActDt,3,2) + '/' + right(BOActDt,4)) 
  and accp_value between @pa_fromdt and       @pa_todt      
