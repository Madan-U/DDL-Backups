-- Object: PROCEDURE citrus_usr.PR_TRATM_INS12
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

CREATE  PROCEDURE [citrus_usr].[PR_TRATM_INS12]
                AS
                BEGIN
                  --
                    DECLARE
                       ...
                       ,@l_sub_cd    VARCHAR(50)
                        SET  @I =1
                        SET @l_name ='HO'
                        SET @l_code='CLOSURE_RSN_CD_NSDL' 
                       ...
                       SELECT @l_excm_id = excm_id FROM exchange_mstr where excm_cd='NSDL'
                        DELETE FROM TRANSACTION_SUB_TYPE_MSTR WHERE TRASTM_TRATM_ID=(SELECT trantm_id FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'CLOSURE_RSN_CD_NSDL')
                        DELETE FROM TRANSACTION_TYPE_MSTR WHERE TRANTM_CODE = 'CLOSURE_RSN_CD_NSDL'
