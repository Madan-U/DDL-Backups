-- Object: PROCEDURE citrus_usr.pr_get_ob_fix_byisin
...
CREATE proc [citrus_usr].[pr_get_ob_fix_byisin](@pa_dpm_id numeric,@pa_from_dt datetime, @pa_to_dt datetime                        
...
,sum(isnull(dphmcd_free_qty,0))+sum(isnull(DPHMCD_PLEDGE_QTY,0))++sum(isnull(DPHMCD_LOCKIN_QTY,0))+sum(isnull(DPHMCD_SAFE_KEEPING_QTY,0)) +sum(isnull(DPHMCD_EARMARK_QTY,0)) holdingbal     
...
and cdshm_tratm_cd in ('2246','2277','2270','2220')  
and CDSHM_CDAS_SUB_TRAS_TYPE <> '816'    ---,'2205' removed for restat / remat set up dtd 28042020
