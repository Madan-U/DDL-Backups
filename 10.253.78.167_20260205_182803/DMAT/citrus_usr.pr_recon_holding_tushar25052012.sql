-- Object: PROCEDURE citrus_usr.pr_recon_holding_tushar25052012
-- Server: 10.253.78.167 | DB: DMAT
--------------------------------------------------

--exec pr_recon_holding 3,'may 24 2012','ALL',''
--exec pr_recon_holding 3,'may 24 2012','MATCH',''
--exec pr_recon_holding 485031,'may 24 2012','MISMATCH',''
create proc [citrus_usr].[pr_recon_holding_tushar25052012](@pa_excsm_id numeric
, @pa_date datetime
, @pa_flag char(15)
...
select dpam_sba_no client_hldg,DPHMc_ISIN isin_hldg,DPHMC_CURR_QTY qty_hldg 
into #priviousholding from dp_hldg_mstr_cdsl
,dp_acct_mstr 
where dpam_id = DPHMC_DPAM_ID 
and DPAM_DPM_ID = @l_dpm_id
and DPHMC_HOLDING_DT = (select top 1 DPHMC_HOLDING_DT from dp_hldg_mstr_cdsl where DPHMC_DPM_ID = @l_dpm_id and DPHMC_HOLDING_DT <  @pa_date order by 1 desc)
...
select dpam_sba_no client_hldg_ason,DPHMC_ISIN isin_hldg_ason,DPHMC_CURR_QTY qty_hldg_ason 
