-- Object: PROCEDURE citrus_usr.SP_MULTI_CLIENT_DTLS_DPS8
...
CREATE PROCEDURE [citrus_usr].[SP_MULTI_CLIENT_DTLS_DPS8]  
(   
...
--				else STAM_DESC end end [STATUS]  
 SELECT distinct case when isnull(fre_action,'')= 'F' and  dpam.dpam_stam_cd  <> '05' then 'FREEZED'
 when 	dps8_pc4.TypeOfTrans in ('','1','2') and  dpam.dpam_stam_cd  <> '05'then 'Freeze'	
			when 	dps8_pc4.TypeOfTrans in ('3')		then 'UnFreeze'
  else 
...
--isnull(REPLACE(CONVERT(VARCHAR(11),dps8_pc1.ClosDt, 106), ' ', '-'),'')
							...
							+ 
 case when 	dps8_pc4.TypeOfTrans in ('','1','2')	 and  dpam.dpam_stam_cd  <> '05'	then space(8)+ '-' + Freezermks	
			when 	dps8_pc4.TypeOfTrans in ('3')	 and  dpam.dpam_stam_cd  <> '05'	then space(8)+ '-' + Freezermks
			else space(8)+ '-' + isnull(fre_rmks,'')  end 
...
,  convert(varchar(11),case when isdate(case when isnull(dps8_pc1.DateOfBirth,'') <> '' then right(dps8_pc1.DateOfBirth,4)  +'-'+substring(dps8_pc1.DateOfBirth,3,2)+'-'+left(dps8_pc1.DateOfBirth,2) else '1900-01-01' end  )=1 then convert(datetime,case when isnull(dps8_pc1.DateOfBirth,'') <> '' then left(dps8_pc1.DateOfBirth,2)+'/'+substring(dps8_pc1.DateOfBirth,3,2)+'/'+right(dps8_pc1.DateOfBirth,4) else '' end ,103) else '' end  ,103) + space(8)  clim_dob  
 ...
 -- ,CASE WHEN  dps8_pc16.MobileNum<>'' THEN 'Y' ELSE 'N' END  SMS   
  ,isnull( dps8_pc1.smart_flag,'')  SMS
...
and not exists (select 1  from freeze_unfreeze_dtls   b where a.fre_Dpam_id = b.fre_Dpam_id and a.fre_lst_upd_dt < b.fre_lst_upd_dt and b.fre_action ='U')
  ...
  left outer join dps8_pc2  dps8_pc2  on dps8_pc2.boid = dps8_pc1.boid   and  isnull(dps8_pc2.TypeOfTrans,'') in ('1','2','')
left outer join dps8_pc3  dps8_pc3  on dps8_pc3.boid = dps8_pc1.boid   and  isnull(dps8_pc3.TypeOfTrans,'') in ('1','2','')
  left outer join dps8_pc4  dps8_pc4  on dps8_pc4.boid = dps8_pc1.boid   and isnull(dps8_pc4.TypeOfTrans,'') in ('1','2','')
