-- Object: PROCEDURE citrus_usr.Pr_Rpt_Holding_celestial
...
create PROC [citrus_usr].[Pr_Rpt_Holding_celestial]  
			@PA_DPTYPE VARCHAR(4),                    
			@PA_EXCSMID INT,                    
			@PA_ASONDATE CHAR(1),                    
			@PA_FORDATE DATETIME,                    
...
END                          
 ...
 SELECT 
			...
			DPAM_ID, 
			HOLDING_DT=CONVERT(VARCHAR(11),@PA_FORDATE,109)--,
			--isnull([citrus_usr].[fn_acct_entp](account.dpam_id,'bbo_code'),'') as bbo_code                           
FROM 
...
PRINT('F')
   ...
   INSERT INTO #ACLIST SELECT DPAM_ID,DPAM_SBA_NO,DPAM_SBA_NAME,EFF_FROM,EFF_TO FROM CITRUS_USR.FN_ACCT_LIST(@@DPMID ,@PA_LOGIN_PR_ENTM_ID,@@L_CHILD_ENTM_ID)    
     ...
     SELECT DPAM_SBA_NAME,DPAM_SBA_NO,DPHMCD_ISIN,ISIN_NAME,CONVERT(NUMERIC(18,3),DPHMCD_CURR_QTY),VALUATION=CONVERT(NUMERIC(18,2),DPHMCD_CURR_QTY*ISNULL(CLOPM_CDSL_RT,0)),CONVERT(NUMERIC(18,3),DPHMCD_FREE_QTY),CONVERT(NUMERIC(18,3),DPHMCD_FREEZE_QTY),CONVERT(NUMERIC(18,3),DPHMCD_PLEDGE_QTY)      
